...
console.log("Starting local browser check...");
...
try {
	...
	console.log("Local console loaded.");
	const welcome = page.getByRole("button", { name: /^(Commencer|Get started)$/i });
	const heading = page.getByRole("heading", { name: "Communication", exact: true });
	await expect(welcome.or(heading).first()).toBeVisible({ timeout: 30000 });
	if (await welcome.isVisible()) await welcome.click();
	await expect(heading).toBeVisible({ timeout: 30000 });
	console.log("Plugin controls ready.");
	...
	for (const enabled of [!initial, initial]) {
		...
		console.log(`Plugin operation returned ${response.status()}.`);
