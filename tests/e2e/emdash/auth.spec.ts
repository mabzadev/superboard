...
test.describe("Signup Page", () => {
	test("displays signup page", async ({ admin }) => {
		...
		await admin.waitForHydration();
		await expect(
			admin.page.getByRole("heading", { level: 1, name: "Create an account", exact: true }),
		).toBeVisible();
		await expect(
			admin.page.getByRole("textbox", { name: "Email address", exact: true }),
		).toBeVisible();
		await expect(admin.page.getByRole("button", { name: "Continue", exact: true })).toBeVisible();
	});
});
