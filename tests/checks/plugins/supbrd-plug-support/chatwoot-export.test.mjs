...
import {
	buildChatwootCutover,
	renderArtifacts,
} from "../../../../packages/plugins/supbrd-plug-support/scripts/chatwoot-cutover.mjs";
import { loadRenderedChatwoot } from "../../../../packages/plugins/supbrd-plug-support/scripts/chatwoot-cutover/apply.mjs";
import { loadChatwootBundle } from "../../../../packages/plugins/supbrd-plug-support/scripts/chatwoot-cutover/core.mjs";
import {
	assertProtectedDirectory,
	chatwootOriginFromTarget,
	exportChatwoot,
	paged,
} from "../../../../packages/plugins/supbrd-plug-support/scripts/chatwoot-export.mjs";
...
test("Chatwoot export paginates API data, downloads allowlisted files and emits checksums", async () => {
	...
	try {
		const fetchImpl = async (input, init = {}) => {
			...
			if (url.pathname.endsWith("/contacts")) {
				return Response.json({
					payload: url.searchParams.get("page") === "1" ? [{ id: 10, identifier: "user-1" }] : [],
				});
			}
										...
										attachments: [
											{
												id: 40,
												file_name: "proof.txt",
												data_url: "https://files.example.test/proof.txt",
											},
										],
									},
								]
		...
		assert.equal(
			result.manifest.artifacts.every((artifact) => /^[a-f0-9]{64}$/u.test(artifact.sha256)),
			true,
		);
		const message = JSON.parse((await readFile(join(output, "messages.ndjson"), "utf8")).trim());
		assert.equal(message.message.attachments[0]._superboard_export.bytes, 18);
		assert.equal(
			requests.find((request) => request.url.startsWith("https://files.example.test"))?.headers
				.api_access_token,
			undefined,
		);
		...
		assert.match(
			await readFile(join(rendered.destination, "support-import.sql"), "utf8"),
			/support_message_attachments/u,
		);
		const verified = await loadRenderedChatwoot(rendered.destination);
		assert.equal(
			verified.uploads.objects[0].source_path,
			join(output, "attachments/20/30/0-proof.txt"),
		);
	} finally {
...
test("Chatwoot export origin is owned by the application target", async () => {
	const { target } = await loadTarget("reference-production");
	assert.equal(chatwootOriginFromTarget(target), "https://chat.reference.example");
	assert.throws(
