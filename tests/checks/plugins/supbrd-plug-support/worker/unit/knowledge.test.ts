...
} from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/knowledge";
...
describe("Support semantic knowledge", () => {
	it("queries only the project namespace and revalidates vector metadata in D1", async () => {
		...
		await expect(semanticArticleMatches(env, 12, "billing question", 5)).resolves.toEqual([
			{
			...
			embedSupportText(
				{
					...
					SUPPORT_EMBEDDING_MODEL: "embedding-model",
				} as unknown as Parameters<typeof embedSupportText>[0],
				"query",
			),
