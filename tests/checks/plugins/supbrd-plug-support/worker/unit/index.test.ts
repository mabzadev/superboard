...
import worker, {
	mapConversationItem,
} from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/index";
import {
	decryptSecret,
	encryptSecret,
...
describe("support worker", () => {
	...
	it("fails closed when the public client selects another project", async () => {
		const { requireProject } =
			await import("../../../../../../packages/plugins/supbrd-plug-support/worker/src/auth");
	...
	it("encrypts webhook secrets with authenticated encryption", async () => {
		const encrypted = await encryptSecret("support-encryption-key", "support-private-secret");
		expect(encrypted).not.toContain("support-private-secret");
		await expect(decryptSecret("support-encryption-key", encrypted)).resolves.toBe(
			"support-private-secret",
		);
		await expect(decryptSecret("different-key", encrypted)).rejects.toMatchObject({
