...
import { verifyWidgetIdentity } from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/widget-auth";
...
describe("Support widget authentication", () => {
	...
	it("binds the signature to body, method, public path, project, visitor, and idempotency key", async () => {
		...
		for (const request of vectors) {
			...
			await expect(verifyWidgetIdentity(env, request, nowSeconds)).rejects.toMatchObject({
				code: "widget_identity_invalid",
				status: 401,
			});
		}
	});
	...
	it("rejects malformed signatures and stale or future timestamps", async () => {
		const env = await environment();
		await expect(
			verifyWidgetIdentity(env, await signedRequest({ signature: "0".repeat(64) }), nowSeconds),
