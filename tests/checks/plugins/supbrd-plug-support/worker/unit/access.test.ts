...
} from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/access";
...
describe("Support role resolution", () => {
	it("keeps Identity owner/admin as project-wide overrides", async () => {
		await expect(
			resolveSupportPrincipal(database(null), {
				projectId: 12,
				actorId: 9,
				role: "owner",
			}),
	...
	it("resolves members only through an active Support membership", async () => {
		await expect(
			resolveSupportPrincipal(database({ id: "membership-1", role: "supervisor" }), {
				projectId: 12,
				actorId: 9,
				role: "member",
			}),
