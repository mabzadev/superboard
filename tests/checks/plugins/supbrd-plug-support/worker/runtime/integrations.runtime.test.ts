...
} from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/integration-oauth";
import {
	decryptCredentialPayload,
	encryptCredentialPayload,
} from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/secrets";
import type { SupportQueueJob } from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/types";
...
describe("native Support workflow integrations", () => {
	...
	it("consumes a signed authorization state once and stores only encrypted tokens", async () => {
      ...
      WHERE integration.id = ? AND integration.project_id = 12`)
		...
		await expect(
			decryptCredentialPayload([encryptionKey], stored!.encrypted_payload),
		...
		await expect(completeIntegrationOAuth(env, callback, "linear")).rejects.toMatchObject({
			code: "support_oauth_state_invalid",
			status: 422,
		});
	});
	...
	it("uses only bounded official adapters and suppresses replayed side effects", async () => {
      ...
      VALUES (?, 12, ?, ?, 'Payment investigation', 812, 'urgent', 'Customer needs an update')`)
			.bind(conversationId, `integration-user-${suffix}`, `integration-client-${suffix}`)
			.run();
		...
		for (const integration of integrations) {
			...
			await env.DB.batch([
				env.DB.prepare(`INSERT INTO support_integrations
          (id, project_id, provider, display_name, status, settings_json)
          VALUES (?, 12, ?, ?, 'configured', ?)`).bind(
					integration.id,
					integration.provider,
					`Runtime ${integration.provider} ${suffix}`,
		...
		vi.stubGlobal(
			...
			vi.fn(async (input: RequestInfo | URL) => {
				...
				if (url.hostname === "api.linear.app") {
					return Response.json({
						data: { issueCreate: { success: true, issue: { id: "linear-issue" } } },
					});
				}
		...
		expect((await getQueueResult(replay, createExecutionContext())).explicitAcks).toEqual([
			"integration-runtime-replay",
		]);
      ...
      ORDER BY action_type`)
		...
		expect(
			rows.results.every(
				(row) =>
					row.status === "completed" && row.attempt_count === 1 && row.last_error_code == null,
			),
		).toBe(true);
		const states = await env.DB.prepare(`SELECT status FROM support_integrations
      WHERE project_id = 12 AND id IN (?, ?, ?, ?)`)
			.bind(...integrations.map((integration) => integration.id))
			.all<{ status: string }>();
	...
	it("classifies transient failures for Queue retry without exposing credentials", async () => {
        ...
        VALUES (?, 12, ?, ?, 'Retry integration')`).bind(
			...
			env.DB.prepare(`INSERT INTO support_integrations
        (id, project_id, provider, display_name, status, settings_json)
        VALUES (?, 12, 'slack', ?, 'configured', ?)`).bind(
				integrationId,
				`Retry ${suffix}`,
				JSON.stringify({
					workflow_action: "post_message",
		...
		await expect(
			env.DB.prepare(`SELECT delivery.status, delivery.last_error_code,
      ...
      WHERE delivery.integration_id = ? AND delivery.idempotency_key = ?`)
				.bind(integrationId, body.resourceId)
				.first(),
		).resolves.toMatchObject({
			status: "failed",
	...
	it("refreshes an expired authorization atomically before executing", async () => {
		...
		await env.DB.batch([
			env.DB.prepare(`INSERT INTO conversations
        (id, project_id, external_user_id, client_conversation_id, subject)
        VALUES (?, 12, ?, ?, 'Refresh integration')`).bind(
				conversationId,
				`refresh-user-${suffix}`,
				`refresh-client-${suffix}`,
			),
			env.DB.prepare(`INSERT INTO support_integrations
        (id, project_id, provider, display_name, status, settings_json)
        VALUES (?, 12, 'linear', ?, 'configured', ?)`).bind(
				integrationId,
				`Refresh ${suffix}`,
				JSON.stringify({
					workflow_action: "create_issue",
      ...
      FROM support_integration_credentials WHERE integration_id = ? AND project_id = 12`)
			.bind(integrationId)
			.first<{ encrypted_payload: string; credential_version: number }>();
