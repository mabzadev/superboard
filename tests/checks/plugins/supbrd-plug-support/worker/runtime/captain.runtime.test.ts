...
import type {
	Env,
	SupportQueueJob,
} from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/types";
...
describe("automatic Captain runtime", () => {
	it("queues one task per explicit link and delivers only automatic results through the room", async () => {
        ...
        VALUES (?, 12, ?, ?, 'published', 'runtime')`).bind(
				portalId,
				`Captain portal ${suffix}`,
				`captain-${suffix}`,
			),
		]);
		await env.DB.batch([
        ...
        VALUES (?, 12, ?, 'Returns policy', ?, 'Returns are accepted for thirty days.',
          'published', 'runtime', strftime('%Y-%m-%dT%H:%M:%fZ', 'now'))`).bind(
				articleId,
				portalId,
				`returns-${suffix}`,
			),
        ...
        VALUES (?, 12, 'article', ?, 'Returns policy', ?, 'project:12', 'indexed', 1, 'runtime')`).bind(
				documentId,
				articleId,
				`hash-${suffix}`,
			),
      ...
      ORDER BY assistant_id`)
		...
		await expect(
			env.DB.prepare(`SELECT COUNT(*) count FROM support_assistant_tasks
      WHERE project_id = 13 AND conversation_id = ?`)
				.bind(conversationId)
				.first(),
		).resolves.toMatchObject({ count: 0 });
		await expect(
			env.DB.prepare(`SELECT COUNT(*) count FROM support_scheduled_jobs
      WHERE project_id = 12 AND job_type = 'captain.task'
        AND resource_id IN (?, ?)`)
				.bind(tasks.results[0].id, tasks.results[1].id)
				.first(),
		...
		const suggestionTask = tasks.results.find(
			(task) => task.assistant_id === suggestionAssistantId,
		)!;
      ...
      WHERE project_id = 12 AND job_type = 'captain.task'
        AND resource_id IN (?, ?) AND status = 'completed'`)
				.bind(suggestionTask.id, automaticTask.id)
				.first(),
		).resolves.toMatchObject({ count: 2 });
      ...
      FROM messages WHERE conversation_id = ? ORDER BY sequence`)
		...
		await expect(
			env.DB.prepare(`SELECT COUNT(*) count FROM support_provider_deliveries
      WHERE project_id = 12 AND conversation_id = ? AND message_id = ?`)
				.bind(conversationId, messages.results[1].id)
				.first(),
		).resolves.toMatchObject({ count: 1 });
		await expect(
			env.DB.prepare(`SELECT COUNT(*) count FROM support_assistant_tasks
      WHERE project_id = 12 AND conversation_id = ?`)
				.bind(conversationId)
				.first(),
		).resolves.toMatchObject({ count: 2 });
      ...
      FROM support_assistant_tasks WHERE id = ? AND project_id = 12`)
		...
		await expect(
			env.DB.prepare(`SELECT COUNT(*) count FROM messages WHERE conversation_id = ?`)
				.bind(conversationId)
				.first(),
		).resolves.toMatchObject({ count: 2 });
	...
	it("returns a transient generation failure to Queue retry and resumes the same task", async () => {
        ...
        VALUES (?, 12, ?, ?, 'api')`).bind(
				inboxId,
				`Captain retry ${suffix}`,
				`captain-retry-${suffix}`,
			),
			env.DB.prepare(`INSERT INTO conversations
        (id, project_id, external_user_id, client_conversation_id, subject, inbox_id)
        VALUES (?, 12, ?, ?, 'Retry assistance', ?)`).bind(
				conversationId,
				`captain-retry-user-${suffix}`,
				`captain-retry-client-${suffix}`,
				inboxId,
			),
      ...
      VALUES (12, ?, ?, 1, 'runtime')`)
			.bind(assistantId, inboxId)
			.run();
		expect(
			(
				await postRoomMessage(conversationId, "user", `captain-retry-user-${suffix}`, {
      ...
      WHERE project_id = 12 AND conversation_id = ?`)
		...
		await expect(
			env.DB.prepare(`SELECT status, last_error FROM support_assistant_tasks WHERE id = ?`)
				.bind(task!.id)
				.first(),
		...
		await expect(
			processCaptainTask(captainEnv(successfulGeneration, []), task!.id, 2),
		).resolves.toBe("acked");
		await expect(
			env.DB.prepare(`SELECT status, last_error FROM support_assistant_tasks WHERE id = ?`)
				.bind(task!.id)
				.first(),
		).resolves.toMatchObject({ status: "completed", last_error: null });
		await expect(
			env.DB.prepare(`SELECT COUNT(*) count FROM messages
	...
	it("fails a disabled human handoff closed and blocks later automation after an enabled handoff", async () => {
        ...
        VALUES (?, 12, ?, ?, 'api')`).bind(
				inboxId,
				`Captain handoff ${suffix}`,
				`captain-handoff-${suffix}`,
			),
			env.DB.prepare(`INSERT INTO conversations
        (id, project_id, external_user_id, client_conversation_id, subject, inbox_id, assigned_user_id)
        VALUES (?, 12, ?, ?, 'Handoff assistance', ?, 'human-agent')`).bind(
				conversationId,
				`captain-handoff-user-${suffix}`,
				`captain-handoff-client-${suffix}`,
				inboxId,
			),
      ...
      VALUES (12, ?, ?, 1, 'runtime')`)
		...
		expect((await getQueueResult(disabledBatch, createExecutionContext())).explicitAcks).toEqual([
			disabledBatch.messages[0].id,
		]);
		await expect(
			env.DB.prepare(`SELECT status, last_error FROM support_assistant_tasks WHERE id = ?`)
				.bind(disabledTaskId)
		...
		await expect(
			env.DB.prepare(`SELECT assigned_user_id FROM conversations WHERE id = ?`)
				.bind(conversationId)
				.first(),
		).resolves.toMatchObject({ assigned_user_id: "human-agent" });

		await env.DB.prepare(
			`UPDATE support_assistants SET handoff_enabled = 1 WHERE id = ? AND project_id = 12`,
		)
			.bind(assistantId)
			.run();
		const enabledTaskId = `captain-handoff-enabled-${suffix}`;
		await handoffTask(enabledTaskId, assistantId, conversationId);
		await expect(processCaptainTask(captainEnv(vi.fn(), []), enabledTaskId, 1)).resolves.toBe(
			"acked",
		);
		await expect(
			env.DB.prepare(`SELECT status FROM support_assistant_tasks WHERE id = ?`)
				.bind(enabledTaskId)
				.first(),
		).resolves.toMatchObject({ status: "completed" });
		await expect(
			env.DB.prepare(`SELECT assigned_user_id, status FROM conversations WHERE id = ?`)
				.bind(conversationId)
				.first(),
		).resolves.toMatchObject({ assigned_user_id: null, status: "open" });

		expect(
			(
				await postRoomMessage(conversationId, "user", `captain-handoff-user-${suffix}`, {
...
function assistantStatement(
    ...
    VALUES (?, ?, ?, 'Answer only from supplied Support sources.', ?, ?, 'runtime')`).bind(
		id,
		projectId,
		`${responseMode}-${id}-${suffix}`,
		responseMode,
		handoffEnabled,
	);
}
...
function assistantInboxStatement(
    ...
    (project_id, assistant_id, inbox_id, automatic_enabled, configured_by)
    VALUES (?, ?, ?, ?, 'runtime')`).bind(projectId, assistantId, inboxId, automatic ? 1 : 0);
}
...
async function handoffTask(taskId: string, assistantId: string, conversationId: string) {
    ...
    VALUES (?, 12, ?, ?, 'handoff', '{}', 'runtime')`)
		.bind(taskId, assistantId, conversationId)
		.run();
}
