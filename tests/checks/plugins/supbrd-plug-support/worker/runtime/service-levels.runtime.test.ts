...
import { claimScheduledSupportJobs } from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/index";
import {
	...
	recordSlaMessage,
	recordSlaResolution,
} from "../../../../../../packages/plugins/supbrd-plug-support/worker/src/service-levels";
...
async function insertInbox(projectId: number, inboxId: string) {
	await env.DB.prepare(
     ...
     VALUES (?, ?, ?, ?, 'api', 'active', 1)`,
		...
		.bind(inboxId, projectId, `Inbox ${inboxId}`, `identifier-${inboxId}`)
		.run();
}
...
async function signedCreationRequest(projectId: number, body: Record<string, unknown>) {
	...
	return new Request(`https://support.internal${pathname}`, {
		...
		headers: {
			...
			"x-context-version": "1",
			"x-context-signature": await signProjectContext(context, "support-runtime-secret"),
		},
...
describe("native Support assignment engine", () => {
	it("balances by utilization while enforcing inbox, team, presence, leave, and capacity", async () => {
       ...
       VALUES ('balanced-policy', ?, 'Balanced policy', 'balanced', 5, ?, ?, 1)`,
			...
			.bind(projectId, JSON.stringify([inboxId]), JSON.stringify([teamId]))
			.run();
		...
		const result = await assignConversation(env.DB, projectId, conversationId, now);
	...
	it("applies assignment and SLA policies through the native creation route", async () => {
		...
		await env.DB.batch([
         ...
         VALUES (?, ?, ?, 30, 20, 120, 0, ?, 1)`,
				...
				projectId,
				`Route SLA ${suffix}`,
				JSON.stringify([{ field: "inbox_id", operator: "equals", value: inboxId }]),
			),
		]);

		const response = await SELF.fetch(
			await signedCreationRequest(projectId, {
       ...
       WHERE project_id = ? AND conversation_id = ?`,
				...
				.bind(projectId, payload.data.id)
				.first(),
		).resolves.toMatchObject({
	...
	it("round-robins by the oldest assignment and journals each selection", async () => {
       ...
       VALUES ('round-robin-policy', ?, 'Round robin', 'round_robin', ?, '[]', 1)`,
			...
			.bind(projectId, JSON.stringify([inboxId]))
			.run();
       ...
       VALUES ('sla-hours', ?, ?, 'Europe/Zurich', ?, '["2026-10-26"]', 1)`,
		...
		await env.DB.batch([
         ...
         VALUES ('sla-unmatched', ?, 'Unmatched', 10, 10, 20, 0, ?, 1,
           '2026-01-01T00:00:00.000Z')`,
			).bind(projectId, JSON.stringify([{ field: "priority", operator: "equals", value: "low" }])),
			env.DB.prepare(
				`INSERT INTO support_sla_policies
          (id, project_id, name, first_response_minutes, next_response_minutes,
           resolution_minutes, business_hours_only, conditions_json, active,
