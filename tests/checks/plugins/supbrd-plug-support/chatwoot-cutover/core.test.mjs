...
} from "../../../../../packages/plugins/supbrd-plug-support/scripts/chatwoot-cutover/core.mjs";
			...
			webhooks: [
				{
					...
					nested: {
						credentials: { private_key: "nested-secret-must-not-migrate" },
						label: "preserved",
					},
				},
			],
...
test("Chatwoot conversion blocks unlinked identities and incomplete attachment exports", () => {
	...
	delete input.messages[0].message.attachments[0]._superboard_export;
	const result = transformChatwootBundle(input, { projectId: 12 });
	assert.equal(result.ready, false);
	assert.deepEqual(
		new Set(result.blockers.map((item) => item.code)),
		new Set(["conversation_identity_missing", "attachment_export_missing"]),
	);
