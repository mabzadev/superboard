...
} from "../../../../../../../packages/plugins/supbrd-plug-support/src/front/api/messaging/settingsService.js";
...
describe("Support configuration dashboard service contracts", () => {
	...
	it("loads and updates project settings on the canonical route", async () => {
		...
		expect(api.GET).toHaveBeenCalledWith("/api/v1/support/projects/10-test/settings");
		expect(api.PATCH).toHaveBeenCalledWith("/api/v1/support/projects/10-test/settings", settings);
	});
	...
	it("rotates and revokes encrypted webhook secrets", async () => {
		await rotateSupportWebhookSecret("10-test", "entity/1", "a-long-shared-secret");
