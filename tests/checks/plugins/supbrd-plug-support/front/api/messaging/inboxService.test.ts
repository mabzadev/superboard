...
vi.mock(
	"../../../../../../../packages/plugins/supbrd-plug-support/src/front/transport.js",
	() => api,
);
...
import { inboxDeepLink } from "../../../../../../../packages/plugins/supbrd-plug-support/src/front/api/messaging/inboxDeepLink.js";
import {
...
} from "../../../../../../../packages/plugins/supbrd-plug-support/src/front/api/messaging/inboxService.js";
...
describe("Inbox realtime", () => {
	it("uses the explicitly configured target API origin instead of process-global config", () => {
		expect(
			inboxRealtimeUrl(
				"project-2",
				"conversation-2",
		...
		).toBe("wss://api.other.example/api/v1/support/realtime/ticket%2Fencoded");
	});
	it("reports a missing public API origin without inventing a WebSocket target", () => {
		expect(() => inboxRealtimeUrl("project-2", "conversation-2", "ticket", "")).toThrow(
			"Public Support API endpoint is not configured",
		);
	});

	it("builds a secure WebSocket URL on the native Support route", () => {
		expect(inboxRealtimeUrl("project-1", "conversation 1", "ticket-value")).toBe(
			"wss://api.example.test/api/v1/support/realtime/ticket-value",
		);
	});
	...
	it("rejects malformed, unknown, and oversized events", () => {
		...
		expect(parseInboxRealtimeEvent(JSON.stringify({ type: "unexpected" }))).toBeNull();
...
describe("Support inbox dashboard service contracts", () => {
	...
	it("connects private notes, assignment updates, and realtime tickets", async () => {
		...
		await expect(createInboxRealtimeTicket("10-test", "conversation/1")).resolves.toEqual({
			ticket: "signed",
			expires_at: "later",
		});
	...
	it("uploads and downloads attachments through canonical Support routes", async () => {
		...
		await expect(downloadInboxAttachment("10-test", "conversation/1", "message/1")).resolves.toBe(
			blob,
		);
