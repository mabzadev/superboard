...
} from "../../../../../../../packages/plugins/supbrd-plug-support/src/front/api/messaging/operationsService.js";
...
describe("Support contacts dashboard service contracts", () => {
	...
	it("loads contacts, contact history, notes, and companies from canonical Support routes", async () => {
		...
		expect(api.GET).toHaveBeenNthCalledWith(4, "/api/v1/support/projects/10-test/companies");
	});
	...
	it("creates and updates real Support resources with retry disabled", async () => {
		...
		await createMessagingContactNote("10-test", "contact/1", "Priority customer");
	...
	it("loads quality, audit, CSAT, notifications, and conversation drafts", async () => {
		...
		expect(api.GET).toHaveBeenNthCalledWith(1, "/api/v1/support/projects/10-test/quality");
		expect(api.GET).toHaveBeenNthCalledWith(2, "/api/v1/support/projects/10-test/audit");
		expect(api.GET).toHaveBeenNthCalledWith(3, "/api/v1/support/projects/10-test/csat");
		expect(api.GET).toHaveBeenNthCalledWith(
			4,
	...
	it("connects draft autosave, macro execution, and notification acknowledgement", async () => {
		...
		await saveMessagingDraft("10-test", "conversation/1", "Draft reply", [{ key: "file-1" }]);
