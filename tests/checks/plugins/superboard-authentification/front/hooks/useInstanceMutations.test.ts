...
} from "../../../../../../packages/plugins/superboard-core/src/front/settings/hooks/mutations/useInstanceMutations.js";
...
describe("useInstanceMutations", () => {
	...
	describe("useAddMemberMutation", () => {
		it("calls add member API and invalidates members", async () => {
			...
			expect(mockedAddMember).toHaveBeenCalledWith("inst-1", "new@test.com", "member");
			expect(invalidateSpy).toHaveBeenCalledWith({
	...
	describe("useCompleteSetupStepMutation", () => {
		it("calls complete step API and invalidates setup progress", async () => {
			...
			const { result } = renderHook(() => useCompleteSetupStepMutation("inst-1"), {
				wrapper: createWrapper(queryClient),
			});
			...
			expect(mockedCompleteStep).toHaveBeenCalledWith("inst-1", "ios", "bundle_id");
			expect(invalidateSpy).toHaveBeenCalledWith({
