...
import { expect, describe, it, vi, beforeEach, beforeAll, Mock } from "vitest";

import ChangeEmail, {
	ChangeEmailProps,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/blocks/ChangeEmail";
import { changeEmail } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/locale";
...
describe("ChangeEmail Component", () => {
	...
	const setup = (props = defaultProps) => {
		...
		render(<ChangeEmail {...props} />, container);
	...
	it("renders success view correctly when success prop is true", () => {
		const props = {
			...defaultProps,
			success: true,
		};
		...
		const successElement = getByText(container, successMessage);
	...
	it("renders form view correctly when success prop is false", () => {
		...
		const emailLabel = getByText(container, changeEmail.email.en);
		...
		const submitButton = getByText(container, primaryButtonTitle);
	...
	it("calls handleSubmit when form is submitted", () => {
		const container = setup();
		const formElement = container.querySelector("form");
		const changeEmailButton = getByText(container, changeEmail.sendCode.en);
	...
	it("renders MFA related fields when mfaCode is not null", () => {
		const mfaCode = [""];
		const props = {
			...defaultProps,
			values: {
		...
		const resendButton = getByText(container, secondaryButtonText);
		expect(resendButton).toBeDefined();

		// CodeInput label should be rendered
		const codeLabel = getByText(container, changeEmail.code.en);
		expect(codeLabel).toBeDefined();

		// PrimaryButton should display confirm text when mfaCode is provided
		const confirmButton = getByText(container, changeEmail.confirm.en);
		expect(confirmButton).toBeDefined();
	});

	it("calls handleResend when SecondaryButton is clicked", () => {
		const mfaCode = [""];
		const props = {
			...defaultProps,
			values: {
		...
		const resendButton = getByText(container, changeEmail.resend.en);
		expect(resendButton).toBeDefined();
	...
	it("displays resent text when resent prop is true", () => {
		const mfaCode = [""];
		const props = {
			...defaultProps,
			values: {
		...
		const resentButton = getByText(container, changeEmail.resent.en);
		expect(resentButton).toBeDefined();
		expect((resentButton as HTMLButtonElement).disabled).toBe(true);
	});

	it("displays submit error message when submitError is provided", () => {
	...
	it("calls handleChange when code input changes", () => {
		const props = {
			...defaultProps,
			values: {
				...defaultProps.values,
	...
	it("handles null mfaCode by using empty array in CodeInput", () => {
		const props = {
			...defaultProps,
			values: {
				...defaultProps.values,
		...
		const newCodeInputs = container.querySelectorAll(
	...
	it("do not display spinner in PrimaryButton when isSubmitting is false", () => {
		const props = {
			...defaultProps,
			isSubmitting: false,
		};
	...
	it("displays spinner in PrimaryButton when isSubmitting is true", () => {
		const props = {
			...defaultProps,
