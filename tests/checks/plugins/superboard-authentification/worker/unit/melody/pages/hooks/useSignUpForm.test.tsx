...
import { routeConfig } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/configs";
import { View } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useCurrentView";
import { InitialProps } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useInitialProps";
import useSignUpForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useSignUpForm";
import { validateError } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/locale";
import { AuthorizeParams } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/param";
import * as requestModule from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/request";
...
test("allows submission without names when namesIsRequired is false", async () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
...
test("excludes names from submission when enableNames is false", async () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
	...
	const handleAuthorizeSpy = vi
		.spyOn(requestModule, "handleAuthorizeStep")
		.mockImplementation((response, locale, onSwitchViewFn) => {
	...
	expect(requestBody).toHaveProperty("password", "Password1!");
...
describe("custom user attributes", () => {
	...
	test("handleChange updates custom attribute values correctly", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();

		const mockUserAttributes = [
			createMockUserAttribute(1, "department", true, "en", "Department"),
			createMockUserAttribute(2, "phone", false, "en", "Phone Number"),
		];
		...
		act(() => {
			result.current.handleChange(1, "Engineering");
			result.current.handleChange(2, "123-456-7890");
		});
	...
	test("validates required custom attributes", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();

		const mockUserAttributes = [createMockUserAttribute(3, "company", true, "en", "Company")];
	...
	test("submits custom attributes in form payload", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();

		const mockUserAttributes = [
			createMockUserAttribute(4, "department", true, "en", "Department"),
			createMockUserAttribute(5, "phone", false, "en", "Phone Number"),
		];
		...
		act(() => {
			...
			result.current.handleChange("firstName", "John");
			result.current.handleChange("lastName", "Doe");
			result.current.handleChange(4, "Engineering");
			result.current.handleChange(5, "123-456-7890");
		});
		...
		expect(requestBody).toHaveProperty("email", "test@example.com");
		expect(requestBody).toHaveProperty("firstName", "John");
		expect(requestBody).toHaveProperty("lastName", "Doe");
	...
	test("does not include attributes in submission when no custom attributes exist", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
	...
	test("getSignUpInfo fetches and sets user attributes", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();

		const mockUserAttributes = [
			createMockUserAttribute(6, "position", true, "en", "Position"),
			createMockUserAttribute(7, "manager", false, "en", "Manager"),
		];
	...
	test("getSignUpInfo calls onSubmitError on fetch failure", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const fakeError = new Error("Network error");
		const fetchSpy = vi.spyOn(global, "fetch").mockRejectedValue(fakeError);
	...
	test("getSignUpInfo does not fetch when enableUserAttribute is false", () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const fetchSpy = vi.spyOn(global, "fetch");
