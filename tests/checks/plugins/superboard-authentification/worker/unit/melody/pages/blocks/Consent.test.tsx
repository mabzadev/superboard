...
import { GetAppConsentRes } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/handlers/identity/main";
import Consent, {
	ConsentProps,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/blocks/Consent";
import { consent } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/locale";
...
describe("Consent Component", () => {
	...
	const setup = (props: ConsentProps = defaultProps) => {
		...
		render(<Consent {...props} />, container);
	...
	it("renders the title and consent info when provided", () => {
		const container = setup();
		const titleElement = getByText(container, consent.title.en);
		expect(titleElement).toBeDefined();

		const consentText = `${defaultConsentInfo.appName} ${consent.requestAccess.en}`;
		const consentInfoElement = getByText(container, consentText);
		expect(consentInfoElement).toBeDefined();
	});

	it("renders only the title when consentInfo is null", () => {
	...
	it("renders the scopes list filtering out OpenId and OfflineAccess", () => {
		const container = setup();
		const profileElement = getByText(container, "Profile");
		expect(profileElement).toBeDefined();
		const emailElement = getByText(container, "Email");
	...
	it("renders submit error if provided", () => {
	...
	it("calls handleDecline when the decline button is clicked", () => {
		const container = setup();
		const declineButton = getByText(container, consent.decline.en);
	...
	it("calls handleAccept when the accept button is clicked", () => {
		const container = setup();
		const acceptButton = getByText(container, consent.accept.en);
		expect(acceptButton).toBeDefined();
		fireEvent.click(acceptButton);
		expect(defaultProps.onAccept).toHaveBeenCalledTimes(1);
	});

	it("renders scope name when no translation is available for current locale", () => {
		...
		const scopeElement = getByText(container, "profile");
