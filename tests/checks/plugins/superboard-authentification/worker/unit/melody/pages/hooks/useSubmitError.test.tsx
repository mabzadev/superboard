...
import useSubmitError from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useSubmitError";
import {
	requestError,
	validateError,
...
describe("useSubmitError", () => {
	...
	test("initializes with null submitError", () => {
	...
	test("clears error when null is passed", () => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale,
	...
	])("sets correct error message for %s error", (errorInput, expectedMessage) => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale,
	...
	test("handles WrongAuthCode error by redirecting and switching view", () => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale,
	...
	test("handles WrongAuthCode error when no redirect_uri is present", () => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale,
	...
	test("sets default error message for unknown errors", () => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale,
	...
	test("handles duplicate attribute value error with correct parsing", () => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale,
		...
		expect(result.current.submitError).toBe(
			'Duplicate value "EMP001" for attribute "employee_id".',
		);
	});

	test("handles duplicate attribute value error with different values", () => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale,
		...
		expect(result.current.submitError).toBe(
			'Duplicate value "user@example.com" for attribute "email".',
		);
	});

	test("handles malformed duplicate attribute error without proper format", () => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale,
	...
	test("handles validation failed error", () => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale,
		...
		act(() => {
			result.current.handleSubmitError(
				'Value for attribute "employee_id" does not match the validation rule',
			);
		});
	...
	test("handles validation failed error with french locale", () => {
		const { result } = renderHook(() =>
			useSubmitError({
				onSwitchView,
				locale: "fr",
		...
		expect(result.current.submitError).toBe(
			'La valeur pour l\'attribut "employee_id" ne correspond pas à la règle de validation.',
		);
	});
});
