...
import SignUp from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/blocks/SignUp";
import {
	InitialProps,
	View,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks";
import { signUp } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/locale";
...
describe("SignUp Component", () => {
	...
	it("renders the title", () => {
		...
		const title = getByText(container, signUp.title.en);
	...
	it("renders firstName and lastName fields when enableNames is true", () => {
		const container = setup();
	...
	it("does not render firstName and lastName fields when enableNames is false", () => {
		const props = {
			...defaultProps,
			initialProps: {
	...
	it("calls handleChange when input fields change", () => {
		...
		expect(defaultProps.onChange).toHaveBeenCalledWith("password", "123456");

		const confirmPasswordField = container.querySelector(
			'input[name="confirmPassword"]',
		) as HTMLInputElement;
		fireEvent.input(confirmPasswordField, { target: { value: "123456" } });
		expect(defaultProps.onChange).toHaveBeenCalledWith("confirmPassword", "123456");
		...
		if (defaultProps.initialProps.enableNames) {
			...
			expect(defaultProps.onChange).toHaveBeenCalledWith("lastName", "Doe");
		}
	});

	it("calls handleSubmit when PrimaryButton is clicked", () => {
		const container = setup();
		const signUpButton = getByText(container, signUp.signUp.en);
	...
	it("calls onSwitchView with SignIn when SecondaryButton is clicked", () => {
		const container = setup();
		const signInButton = getByText(container, signUp.signIn.en);
		fireEvent.click(signInButton);
		expect(defaultProps.onSwitchView).toHaveBeenCalledWith(View.SignIn);
	});

	it("renders terms and privacy policy links when provided", () => {
		const container = setup();
	...
	it("does not render terms and privacy section when both links are missing", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("renders only terms link when privacy policy link is missing", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("renders only privacy policy link when terms link is missing", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("displays email error message when errors.email is provided", () => {
	...
	it("toggles password visibility when eye icon is clicked", async () => {
		...
		const toggleButton = passwordRow.querySelector(
		...
		const toggleButtonAfter = passwordRow.querySelector(
	...
	describe("userAttributes", () => {
		it("renders userAttribute fields when userAttributes are provided", () => {
		...
		it("does not render userAttribute fields when userAttributes array is empty", () => {
			const props = {
				...defaultProps,
				userAttributes: [],
			};
		...
		it("uses fallback name when locale is not found in userAttribute locales", () => {
			const mockUserAttributes = [
				{
					id: 3,
					name: "customField",
		...
		it("calls onChange with correct attribute id when userAttribute field changes", () => {
			const mockUserAttributes = [
				{
					id: 4,
					name: "company",
			...
			fireEvent.input(companyField, { target: { value: "Acme Corp" } });

			expect(defaultProps.onChange).toHaveBeenCalledWith(4, "Acme Corp");
		});

		it("displays userAttribute error message when error is provided", () => {
		...
		it("renders userAttribute fields with correct values", () => {
			const mockUserAttributes = [
				{
					id: 6,
					name: "position",
		...
		it("renders validationLocales as note when provided", () => {
			const mockUserAttributes = [
				{
					id: 7,
					name: "phone",
