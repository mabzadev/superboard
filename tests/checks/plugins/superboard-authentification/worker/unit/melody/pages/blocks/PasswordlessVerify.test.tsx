...
import PasswordlessVerify, {
	PasswordlessVerifyProps,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/blocks/PasswordlessVerify";
import { passwordlessCode } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/locale";
...
describe("PasswordlessVerify Component", () => {
	...
	const setup = (props: PasswordlessVerifyProps = defaultProps) => {
		...
		render(<PasswordlessVerify {...props} />, container);
	...
	it("renders view title correctly", () => {
		...
		const titleElement = getByText(container, passwordlessCode.title.en);
		expect(titleElement).toBeDefined();
	});

	it("calls handleSubmit on form submission", () => {
		const container = setup();
		const button = getByText(container, passwordlessCode.verify.en);
		expect(button).toBeDefined();
		fireEvent.click(button);
		expect(defaultProps.onSubmit).toHaveBeenCalledTimes(1);
	});

	it("renders secondary button with resend text when resent is false and triggers sendPasswordlessCode on click", () => {
		const props = {
			...defaultProps,
			resent: false,
		};
		const container = setup(props);
		const secondaryButton = getByText(container, passwordlessCode.resend.en);
	...
	it("renders secondary button with resent text when resent is true and is disabled", () => {
		const props = {
			...defaultProps,
			resent: true,
		};
		const container = setup(props);
		const secondaryButton = getByText(container, passwordlessCode.resent.en);
		expect(secondaryButton).toBeDefined();
	...
	it("renders primary button with verify text", () => {
		const container = setup();
		const primaryButton = getByText(container, passwordlessCode.verify.en);
		expect(primaryButton).toBeDefined();
	});

	it("handles undefined or null mfaCode by using empty array", () => {
		...
		const codeInputsUndefined = containerUndefined.querySelectorAll(
