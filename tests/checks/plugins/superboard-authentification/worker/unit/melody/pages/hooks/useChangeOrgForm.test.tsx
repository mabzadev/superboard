...
import useChangeOrgForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useChangeOrgForm";
...
test("getUserOrgsInfo successfully fetches and updates orgs and activeOrgSlug", async () => {
	...
	expect(fetchSpy).toHaveBeenCalledWith(`${routeConfig.IdentityRoute.ChangeOrg}${qs}`, {
		method: "GET",
		headers: {
...
test("handleSwitchOrg submits org switch and sets success to true", async () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
	const fakeResponse = {
		ok: true,
	...
	expect(fetchSpy).toHaveBeenCalledWith(routeConfig.IdentityRoute.ChangeOrg, {
		method: "POST",
		headers: {
			Accept: "application/json",
			"Content-Type": "application/json",
...
test("resetSuccess sets success to false", () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
	const { result } = renderHook(() =>
		useChangeOrgForm({
...
test("resetSuccess can be called multiple times", () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
	const { result } = renderHook(() =>
		useChangeOrgForm({
...
test("redirectUri is correctly returned from followUpParams", () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
	const { result } = renderHook(() =>
		useChangeOrgForm({
