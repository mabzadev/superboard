...
import useOtpMfaForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useOtpMfaForm";
...
test("handleChange updates rememberDevice and resets onSubmitError", () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
	const { result } = renderHook(() =>
		useOtpMfaForm({
...
test("getOtpSetupInfo fetches OTP setup info and updates otpUri", async () => {
	...
	expect(fetchSpy).toHaveBeenCalledWith(`${routeConfig.IdentityRoute.OtpMfaSetup}${qs}`, {
		method: "GET",
		headers: {
...
test("getOtpMfaInfo fetches OTP MFA info and updates allowFallbackToEmailMfa", async () => {
	...
	expect(fetchSpy).toHaveBeenCalledWith(`${routeConfig.IdentityRoute.ProcessOtpMfa}${qs}`, {
		method: "GET",
		headers: {
			Accept: "application/json",
			"Content-Type": "application/json",
...
test("handleMfa submits data successfully when valid", async () => {
	...
	expect(fetchSpy).toHaveBeenCalledWith(routeConfig.IdentityRoute.ProcessOtpMfa, {
		method: "POST",
		headers: {
			Accept: "application/json",
			"Content-Type": "application/json",
