...
import useRecoveryCodeEnrollForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useRecoveryCodeEnrollForm";
...
test("returns initial state with null recoveryCodeEnrollInfo", () => {
...
test("getRecoveryCodeEnrollInfo successfully fetches and updates recoveryCodeEnrollInfo", async () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
...
test("getRecoveryCodeEnrollInfo calls onSubmitError on fetch failure", async () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
...
test("handleContinue submits successfully and calls handleAuthorizeStep", async () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
	...
	expect(fetchSpy).toHaveBeenCalledWith(routeConfig.IdentityRoute.ProcessRecoveryCodeEnroll, {
		method: "POST",
		headers: {
			Accept: "application/json",
			"Content-Type": "application/json",
...
test("handleContinue calls onSubmitError on fetch failure", async () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
...
test("handleContinue sends correct request body with follow-up parameters", async () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
