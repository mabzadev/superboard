...
import usePasswordlessVerifyForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/usePasswordlessVerifyForm";
...
test("returns initial state with empty mfaCode and no errors", () => {
	...
	expect(result.current.values).toEqual({ mfaCode: Array.from({ length: 6 }, () => "") });
...
test("sendPasswordlessCode sends request and sets resent when isResend true", async () => {
	const onSubmitError = vi.fn();
	const onSwitchView = vi.fn();
	const { result } = renderHook(() =>
		usePasswordlessVerifyForm({
