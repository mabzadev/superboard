...
import Layout from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/blocks/Layout";
import { layout } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/locale";
...
describe("Layout Component", () => {
	...
	const setup = (props: Partial<Parameters<typeof Layout>[0]> = {}) => {
		...
		render(<Layout {...defaultProps} />, container);
	...
	it("renders children content", () => {
		...
		expect(getByText(container, "Child Content")).toBeDefined();
	});
	...
	it("renders LocaleSelector when multiple locales are provided", () => {
		const container = setup();
		const localeSelectorDiv = container.querySelector("div.absolute.right-0");
		expect(localeSelectorDiv).toBeDefined();
	});

	it("does not render LocaleSelector when only one locale is provided", () => {
		const container = setup({ locales: singleLocale });
		const localeSelectorDiv = container.querySelector("div.absolute.right-0");
		expect(localeSelectorDiv).toBeNull();
	});

	it("renders the powered-by link with correct href and text", () => {
		const container = setup();
