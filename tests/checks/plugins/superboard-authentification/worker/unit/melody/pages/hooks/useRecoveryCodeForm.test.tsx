...
import useRecoveryCodeForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useRecoveryCodeForm";
import * as formModule from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/form";
...
describe("useRecoveryCodeForm", () => {
	...
	test("handleChange updates email and resets onSubmitError", () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const { result } = renderHook(() =>
			useRecoveryCodeForm({
	...
	test("handleChange updates recoveryCode and resets onSubmitError", () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const { result } = renderHook(() =>
			useRecoveryCodeForm({
		...
		act(() => {
			result.current.handleChange("recoveryCode", "123456");
		});
	...
	test("handleSubmit submits successfully and calls handleAuthorizeStep", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		...
		handleAuthorizeSpy.mockImplementation((response, locale, onSwitchViewFn) => {
		...
		expect(fakeEvent.preventDefault).toHaveBeenCalled();
		expect(fetchSpy).toHaveBeenCalledWith(routeConfig.IdentityRoute.AuthorizeRecoveryCode, {
			method: "POST",
			headers: {
		...
		expect(handleAuthorizeSpy).toHaveBeenCalledWith(fakeResponseData, "en", onSwitchView);
	...
	test("handleSubmit surfaces a rotated recovery code and defers the redirect until continue", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
	...
	test("manages isSubmitting state correctly during submission", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
	...
	test("shows validation errors only when fields are touched", () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
