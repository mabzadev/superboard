...
import ManagePasskey, {
	ManagePasskeyProps,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/blocks/ManagePasskey";
...
describe("ManagePasskey Component", () => {
	...
	const setup = (props: ManagePasskeyProps = defaultProps) => {
		...
		render(<ManagePasskey {...props} />, container);
	...
	it("renders success message when provided", () => {
		...
		const successMessageElement = getByText(container, "Operation Successful");
		expect(successMessageElement).toBeDefined();
	});

	it("renders passkey details and remove button when passkey is provided", () => {
		...
		const removeButton = getByText(container, managePasskey.remove.en);
	...
	it("renders enroll view when passkey is not provided", () => {
		const props = {
			...defaultProps,
			passkey: null,
		};
		...
		const noPasskeyElement = getByText(container, managePasskey.noPasskey.en);
		expect(noPasskeyElement).toBeDefined();

		// Check enroll button is rendered
		const enrollButton = getByText(container, managePasskey.enroll.en);
