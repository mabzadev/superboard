...
import usePasskeyEnrollForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/usePasskeyEnrollForm";
...
describe("usePasskeyEnrollForm", () => {
	const locale = "en";

	test("initial state returns null enrollOptions and rememberSkip false", () => {
	...
	test("handleRememberSkip updates rememberSkip state", () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const { result } = renderHook(() =>
			usePasskeyEnrollForm({
	...
	test("getEnrollOptions updates enrollOptions on successful fetch", async () => {
	...
	test("getEnrollOptions calls onSubmitError on fetch failure", async () => {
	...
	test("handleEnroll does nothing if enrollOptions is null", async () => {
		const enrollModule = await import("@simplewebauthn/browser");
		const enrollMock = vi.spyOn(enrollModule, "startRegistration");
	...
	test("handleEnroll calls enroll and submitEnroll on successful enrollment", async () => {
		...
		const enrollModule = await import("@simplewebauthn/browser");
		const enrollMock = vi
			.spyOn(enrollModule, "startRegistration")
	...
	test("handleEnroll does not continue if enroll returns undefined", async () => {
		// Set enrollOptions.
		const fakeEnrollOptions = { option: "value" };
		const fakeGetEnrollResponse = {
			ok: true,
	...
	test("handleDecline sends POST request and calls onSwitchView on success", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const { result } = renderHook(() =>
			usePasskeyEnrollForm({
	...
	test("handleDecline calls onSubmitError on fetch failure", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const { result } = renderHook(() =>
			usePasskeyEnrollForm({
	...
	test("handleEnroll calls onSubmitError when submitEnroll fails", async () => {
		...
		const fetchSpyPost = vi.spyOn(global, "fetch").mockRejectedValue(error);
