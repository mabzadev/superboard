...
import useAcceptInvitationForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useAcceptInvitationForm";
import { getInvitationParams } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/param";
...
test("sets isTokenValid to false when invitationToken is empty", async () => {
...
test("sets isTokenValid to true when token check succeeds", async () => {
	(getInvitationParams as unknown as Mock).mockReturnValue({
		locale: "en",
...
test("sets isTokenValid to false when token check fails", async () => {
	(getInvitationParams as unknown as Mock).mockReturnValue({
		locale: "en",
	...
	const fetchSpy = vi.spyOn(global, "fetch").mockRejectedValue(new Error("Token invalid"));
...
test("handleSubmit posts and sets success on valid submission", async () => {
	(getInvitationParams as unknown as Mock).mockReturnValue({
		locale: "en",
		invitationToken: "valid-token",
		signinUrl: "",
...
test("handleSubmit sets signinUrl when provided in invitation params", async () => {
	(getInvitationParams as unknown as Mock).mockReturnValue({
		locale: "en",
		invitationToken: "valid-token",
