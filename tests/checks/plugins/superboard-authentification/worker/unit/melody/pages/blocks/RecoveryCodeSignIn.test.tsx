...
import RecoveryCodeSignIn, {
	RecoveryCodeSignInProps,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/blocks/RecoveryCodeSignIn";
import { View } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks";
import {
	recoveryCodeSignIn,
	recoveryCodeEnroll,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/locale";
...
describe("RecoveryCodeSignIn Component", () => {
	...
	const setup = (props: RecoveryCodeSignInProps = defaultProps) => {
		...
		render(<RecoveryCodeSignIn {...props} />, container);
	...
	it("renders view title correctly", () => {
		...
		const titleElement = getByText(container, recoveryCodeSignIn.title.en);
		expect(titleElement).toBeDefined();
	});

	it("renders email field with correct properties", () => {
		const container = setup();
	...
	it("renders recovery code field with correct properties", () => {
		const container = setup();
		const recoveryCodeField = container.querySelector(
	...
	it("calls onChange when email field value changes", () => {
		const container = setup();
		const emailField = container.querySelector('input[name="email"]') as HTMLInputElement;
	...
	it("calls onChange when recovery code field value changes", () => {
		const container = setup();
		const recoveryCodeField = container.querySelector(
			'input[name="recoveryCode"]',
		) as HTMLInputElement;
		fireEvent.input(recoveryCodeField, { target: { value: "ABC123DEF456" } });
		expect(defaultProps.onChange).toHaveBeenCalledWith("recoveryCode", "ABC123DEF456");
	});
	...
	it("calls onSwitchView with View.SignIn when secondary button is clicked", () => {
		const container = setup();
		const signInButton = getByText(container, recoveryCodeSignIn.signIn.en);
	...
	it("renders email field error message when errors.email is provided", () => {
	...
	it("renders recovery code field error message when errors.recoveryCode is provided", () => {
	...
	it("renders both field error messages when both errors are provided", () => {
	...
	it("shows loading state on primary button when isSubmitting is true", () => {
		const props = {
			...defaultProps,
			isSubmitting: true,
		};
	...
	it("does not show loading state on primary button when isSubmitting is false", () => {
		const props = {
			...defaultProps,
			isSubmitting: false,
		};
	...
	it("renders form with correct autoComplete attribute", () => {
		const container = setup();
	...
	it("shows the new recovery code and hides the form when newRecoveryCode is provided", () => {
		const props = {
			...defaultProps,
	...
	it("calls handleContinue when the continue button is clicked on the new code view", () => {
		const props = {
			...defaultProps,
			newRecoveryCode: "NEW-RECOVERY-CODE-123",
		};
		const container = setup(props);
		const continueButton = getByText(container, recoveryCodeEnroll.continue.en);
