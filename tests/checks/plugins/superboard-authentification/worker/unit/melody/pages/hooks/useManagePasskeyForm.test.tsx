...
import useManagePasskeyForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useManagePasskeyForm";
import { managePasskey } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/locale";
...
describe("useManagePasskeyForm hook", () => {
	const locale = "en";
	...
	test("getManagePasskeyInfo updates passkey and enrollOptions on successful fetch", async () => {
	...
	test("handleRemove updates successMessage and resets passkey on successful DELETE", async () => {
		const fakeResponse = {
			ok: true,
			json: async () => ({}),
		};
		...
		expect(fetchSpy).toHaveBeenCalledWith("/identity/v1/manage-passkey", {
			method: "DELETE",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json",
	...
	test("handleRemove calls onSubmitError on DELETE failure", async () => {
	...
	test("handleEnroll does nothing if enrollOptions is null", () => {
		const { result } = renderHook(() =>
			useManagePasskeyForm({
				locale,
				onSubmitError,
	...
	test("handleEnroll calls enroll and submitEnroll on successful enrollment", async () => {
		...
		const fetchSpyGet = vi.spyOn(global, "fetch").mockResolvedValueOnce({
			ok: true,
		...
		expect(startRegistration).toHaveBeenCalledWith({
			optionsJSON: fakeManageResponse.enrollOptions,
		});
