...
import useManageRecoveryCodeForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useManageRecoveryCodeForm";
...
describe("useManageRecoveryCodeForm hook", () => {
	const locale = "en";
	...
	test("handleRegenerate updates recoveryCode and successMessage on successful POST", async () => {
		...
		expect(fetchSpy).toHaveBeenCalledWith("/identity/v1/manage-recovery-code", {
			method: "POST",
			headers: {
	...
	test("handleRegenerate calls onSubmitError on fetch failure", async () => {
	...
	test("handleRegenerate sets isGenerating to false even on error", async () => {
	...
	test("handleRegenerate with different locale uses correct locale value", async () => {
		const testLocale = "fr";
