...
import { Policy } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/dtos/oauth";
import SignIn from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/blocks/SignIn";
import {
	InitialProps,
	View,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks";
import * as hooks from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks";
import { signIn } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/tools/locale";
...
describe("SignIn Component", () => {
	...
	it("shows magic link sent message and hides form when magicLinkSent is true", () => {
		const props = {
			...defaultProps,
			magicLinkSent: true,
		};
	...
	it("renders the title", () => {
		...
		const title = getByText(container, signIn.title.en);
		expect(title).toBeDefined();
	});

	it("renders the email input field and calls handleChange on change", () => {
		const container = setup();
		...
		expect(defaultProps.onChange).toHaveBeenCalledWith("email", "user@example.com");
	});

	it("renders the PasswordField when enablePasswordSignIn is true and shouldLoadPasskeyInfo is false and calls handleChange on change", () => {
		const container = setup();
		...
		expect(defaultProps.onChange).toHaveBeenCalledWith("password", "password123");
	});

	it("calls handleSubmit when form is submitted via the submit button click", () => {
		const container = setup();
		const submitButton = getByText(container, signIn.submit.en);
		expect(submitButton).toBeDefined();
		fireEvent.click(submitButton);
		expect(defaultProps.onSubmit).toHaveBeenCalledTimes(1);
	});

	it("renders the social sign in section when client IDs are provided", () => {
		const container = setup();
	...
	it("does not render social sign in section when no client IDs are provided", () => {
		const props = {
			...defaultProps,
			initialProps: {
	...
	it("renders only google sign in button", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("renders only facebook sign in button", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("renders only github sign in button", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("renders only discord sign in button", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("renders only apple sign in button", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("calls onSwitchView with SignUp when the SignUp SecondaryButton is clicked", () => {
		const container = setup();
		const signUpButton = getByText(container, signIn.signUp.en);
		fireEvent.click(signUpButton);
		expect(defaultProps.onSwitchView).toHaveBeenCalledWith(View.SignUp);
	});

	it("calls onSwitchView with ResetPassword when the PasswordReset SecondaryButton is clicked", () => {
		const container = setup();
		const passwordResetButton = getByText(container, signIn.passwordReset.en);
		fireEvent.click(passwordResetButton);
		expect(defaultProps.onSwitchView).toHaveBeenCalledWith(View.ResetPassword);
	});

	it("renders correctly for passwordless sign in without password field", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
		...
		const continueButton = getByText(container, signIn.continue.en);
	...
	it("handles email input in passwordless mode", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
		...
		expect(defaultProps.onChange).toHaveBeenCalledWith("email", "test@example.com");
	});

	it("does not render SignUp and PasswordReset buttons when both are disabled", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("renders only PasswordReset button when SignUp is disabled", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
		...
		const resetPasswordButton = getByText(container, signIn.passwordReset.en);
		expect(resetPasswordButton).toBeDefined();
	});

	it("renders only SignUp button when PasswordReset is disabled", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("renders OIDC sign in buttons when oidcProviders are provided", () => {
		...
		vi.spyOn(hooks, "useSocialSignIn").mockImplementation(() => ({
		...
		expect(oidcProvider1?.getAttribute("href")).toContain(
			"redirect_uri=http://localhost:3000/identity/v1/authorize-oidc/provider1",
		);
		expect(oidcProvider1?.getAttribute("href")).toContain('"codeVerifier":"test-code-verifier"');
		expect(oidcProvider2).not.toBeNull();
		expect(oidcProvider2?.getAttribute("href")).toContain(
			"redirect_uri=http://localhost:3000/identity/v1/authorize-oidc/provider2",
		);
	...
	it("renders recovery code button when allowRecoveryCode is true", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
		...
		const recoveryCodeButton = getByText(container, signIn.recoveryCode.en);
		expect(recoveryCodeButton).toBeDefined();
	});

	it("does not render recovery code button when allowRecoveryCode is false", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("calls onSwitchView with RecoveryCodeSignIn when recovery code button is clicked", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("does not render bottom section when all options are disabled", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
		...
		const resetPasswordButton = container.querySelector(
			`button[title="${signIn.passwordReset.en}"]`,
		);
		const recoveryCodeButton = container.querySelector(`button[title="${signIn.recoveryCode.en}"]`);
	...
	it("renders passkey button when allowPasskey is true", () => {
		...
		const passkeyButton = getByText(container, signIn.withPasskey.en);
	...
	it("calls onVerifyPasskey when passkey button is clicked", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("renders continue button in passwordless mode when allowPasskey is false", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	it("does not render continue button in passwordless mode when allowPasskey is true", () => {
		const props = {
			...defaultProps,
			initialProps: {
				...defaultProps.initialProps,
	...
	describe("Banner rendering", () => {
		it("renders no banners when appBanners array is empty", () => {
			const props = {
				...defaultProps,
				appBanners: [],
			};
		...
		it("renders single banner with correct content", () => {
			const mockBanner = {
				id: 1,
				type: "info",
				...
				locales: [
					...
					{
						locale: "fr",
						value: "Bienvenue dans notre service!",
					},
				],
		...
		it("renders multiple banners in correct order", () => {
			const mockBanners = [
				{
					id: 1,
					type: "info",
		...
		it("renders banner with localized content for current locale", () => {
			const mockBanner = {
				id: 1,
				type: "success",
				text: "Default text",
				locales: [
					{
						locale: "en",
						value: "English success message",
					},
					{
						locale: "fr",
						value: "Message de succès en français",
					},
					{
						locale: "es",
						value: "Mensaje de éxito en español",
					},
				],
				createdAt: "2023-01-01T00:00:00Z",
				updatedAt: "2023-01-01T00:00:00Z",
		...
		it("falls back to default text when locale not found", () => {
			const mockBanner = {
				id: 1,
				type: "info",
		...
		it("renders banner with empty locales array using default text", () => {
			const mockBanner = {
				id: 1,
				type: "warning",
		...
		it("renders banners with different types showing correct styling", () => {
			const mockBanners = [
				{
					id: 1,
					type: "info",
		...
		it("renders banner with unknown type without specific styling", () => {
			const mockBanner = {
				id: 1,
				type: "custom-type",
