...
import useSignInForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useSignInForm";
...
describe("useSignInForm", () => {
	...
	test("handlePasswordlessSignIn submits email successfully and calls handleAuthorizeStep", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const params = { extra: "data" } as any;
		const { result } = renderHook(() =>
	...
	test("handlePasswordlessSignIn calls onSubmitError when fetch fails", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const { result } = renderHook(() =>
			useSignInForm({
	...
	test("handlePasswordlessSignIn sends magic link and sets magicLinkSent when usePasswordlessAsMagicLink is true", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const { result } = renderHook(() =>
			useSignInForm({
	...
	test("handlePasswordlessSignIn calls handleAuthorizeStep when usePasswordlessAsMagicLink is false", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const { result } = renderHook(() =>
			useSignInForm({
	...
	test("handlePasswordlessSignIn stops execution when email validation error exists", async () => {
		const onSubmitError = vi.fn();
		const onSwitchView = vi.fn();
		const { result } = renderHook(() =>
			useSignInForm({
