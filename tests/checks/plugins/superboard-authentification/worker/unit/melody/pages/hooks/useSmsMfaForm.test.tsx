...
import useSmsMfaForm from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/pages/hooks/useSmsMfaForm";
...
describe("useSmsMfaForm hook", () => {
	...
	test("handleChange updates phoneNumber and mfaCode and resets error", () => {
		const { result } = renderHook(() =>
			useSmsMfaForm({
				locale: dummyLocale,
				onSubmitError,
	...
	test("handleChange updates rememberDevice and resets error", () => {
		const { result } = renderHook(() =>
			useSmsMfaForm({
				locale: dummyLocale,
				onSubmitError,
		...
		act(() => {
			result.current.handleChange("rememberDevice", true);
		});
		...
		act(() => {
			result.current.handleChange("rememberDevice", false);
		});
	...
	test("getSmsMfaInfo updates state on successful fetch", async () => {
		...
		expect(result.current.values.mfaCode).toEqual(Array.from({ length: 6 }, () => ""));
	...
	test("handleResend calls ResendSmsMfa when currentNumber exists", async () => {
	...
	test("handleResend calls requestSetupMfa when currentNumber is not set", async () => {
		...
		expect(setupFetchSpy).toHaveBeenCalledWith(routeConfig.IdentityRoute.SetupSmsMfa, {
			method: "POST",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json",
	...
	test("handleSubmit calls requestSetupMfa when mfaCode is null and no validation errors", async () => {
		const { result } = renderHook(() =>
			useSmsMfaForm({
				locale: dummyLocale,
				onSubmitError,
	...
	test("handleSubmit processes mfaCode when provided", async () => {
		const { result } = renderHook(() =>
			useSmsMfaForm({
				locale: dummyLocale,
				onSubmitError,
		...
		const handleAuthorizeSpy = vi
			...
			.mockImplementation((response, locale, onSwitch) => {
	...
	test("handleSubmit calls onSubmitError on fetch failure when mfaCode is provided", async () => {
		const { result } = renderHook(() =>
			useSmsMfaForm({
				locale: dummyLocale,
				onSubmitError,
	...
	test("requestSetupMfa calls onSubmitError on fetch failure", async () => {
		const { result } = renderHook(() =>
			useSmsMfaForm({
				locale: dummyLocale,
				onSubmitError,
		...
		const setupFetchSpy = vi.spyOn(global, "fetch").mockRejectedValue(error);
	...
	test("handleResend calls onSubmitError when ResendSmsMfa fails", async () => {
		const { result } = renderHook(() =>
			useSmsMfaForm({
				locale: dummyLocale,
				onSubmitError,
		...
		const resendFetchSpy = vi.spyOn(global, "fetch").mockRejectedValue(error);
	...
	test("handleSubmit stops execution when there are validation errors", async () => {
		const { result } = renderHook(() =>
			useSmsMfaForm({
				locale: dummyLocale,
				onSubmitError,
	...
	test("handleResend stops execution when there is a phoneNumber validation error", async () => {
		const { result } = renderHook(() =>
			useSmsMfaForm({
				locale: dummyLocale,
				onSubmitError,
