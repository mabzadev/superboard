...
describe("get /app-consent", () => {
	test("should get app consent", async () => {
		...
		const consentRes = await app.request(
			routeConfig.EmbeddedRoute.AppConsent.replace(":sessionId", sessionId),
			{ method: "GET" },
			mock(db),
		);
	...
	test("should throw error when app consent is not enabled", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;

		const { sessionId } = await sendSignUpRequest(db, {
			email: "test1@email.com",
			password: "Password1!",
		});
	...
	test("should throw error when sessionId is not found", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;

		const consentRes = await app.request(
			routeConfig.EmbeddedRoute.AppConsent.replace(":sessionId", "abc"),
			{ method: "GET" },
			mock(db),
		);
...
describe("post /app-consent", () => {
	test("should post app consent", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
