...
describe("get all", () => {
	...
	test("should return no group group for a different org", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
...
describe("create", () => {
	test("should create org group", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should create org group with write_org scope", async () => {
		process.env.ENABLE_ORG = true as unknown as string;

		db.prepare("INSERT INTO org (name) VALUES (?)").run("test org");
		await attachIndividualScopes(db);
		const { res } = await createNewOrgGroup(await getS2sToken(db, Scope.WriteOrg));
		const json = await res.json();
	...
	test("should return 401 without proper scope", async () => {
		...
		db.prepare("INSERT INTO org (name) VALUES (?)").run("test org");
		const { res } = await createNewOrgGroup(await getS2sToken(db, Scope.ReadOrg));
	...
	test("should return 400 if feature not enabled", async () => {
		db.prepare("INSERT INTO org (name) VALUES (?)").run("test org");
		const { res } = await createNewOrgGroup();
		expect(res.status).toBe(400);
		expect(await res.text()).toBe(messageConfig.ConfigError.OrgNotEnabled);
...
describe("update", () => {
	test("should update org group", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
...
describe("delete", () => {
	test("should delete org group", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
...
describe("get users by org group id", () => {
	test("should return all users by org group id", async () => {
		process.env.ENABLE_ORG = true as unknown as string;

		db.prepare("INSERT INTO org (name, slug) VALUES (?, ?)").run("test org", "test-org");
		await createNewOrgGroup();
	...
	test("should return with read user and read org scope", async () => {
		...
		const res = await app.request(
			...
			{
				headers: {
					Authorization: `Bearer ${await getS2sToken(db, "read_user read_org")}`,
				},
			},
			mock(db),
		);
