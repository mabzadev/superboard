...
import { migrate, mockedKV, getSmsResponseMock, mock, fetchMock } from "tests/mock";
...
describe("auth-code token exchange", () => {
	test("should override system mfa enrollment config", async () => {
		db.prepare('update app set "useSystemMfaConfig" = ? where id = ?').run(0, 1);
		await insertUsers(db);
		const tokenRes = await exchangeWithAuthToken(db);
		expect(tokenRes.status).toBe(200);
	});

	test("should override system email mfa enrollment config", async () => {
	...
	test("should override system otp mfa enrollment config", async () => {
	...
	test("should override system sms mfa enrollment config", async () => {
	...
	test("could enable app level email mfa", async () => {
	...
	test("could enable app level otp mfa", async () => {
	...
	test("could enable app level sms mfa", async () => {
	...
	test("could disable fall back to email mfa at app level", async () => {
		process.env.TWILIO_ACCOUNT_ID = "123";
		process.env.TWILIO_AUTH_TOKEN = "abc";
		process.env.TWILIO_SENDER_NUMBER = "+1231231234";

		db.prepare(
			'update app set "useSystemMfaConfig" = ?, "allowEmailMfaAsBackup" = ?, "requireSmsMfa" = ? where id = ?',
		).run(0, 0, 1, 1);
	...
	test("could enable fall back to email mfa at app level", async () => {
