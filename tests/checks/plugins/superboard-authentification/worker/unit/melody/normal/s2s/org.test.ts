...
import { attachIndividualScopes, dbTime, getS2sToken } from "tests/util";
...
import { insertUsers, user1, user2 } from "./user.test";
...
describe("get all", () => {
	test("should return all orgs", async () => {
	...
	test("should return all org with read_org scope", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
		...
		const res = await app.request(
			...
			{
				headers: {
					Authorization: `Bearer ${await getS2sToken(db, Scope.ReadOrg)}`,
				},
			},
			mock(db),
		);
	...
	test("should return 401 if org not enabled in config", async () => {
		await attachIndividualScopes(db);
		const res = await app.request(
			BaseRoute,
			{
	...
	test("should return 401 without proper scope", async () => {
		...
		const res = await app.request(
			...
			{
				headers: {
					Authorization: `Bearer ${await getS2sToken(db, Scope.WriteOrg)}`,
				},
			},
			mock(db),
		);
...
describe("get by id", () => {
	test("should return org by id", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should return 404 when can not find org by id", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
...
describe("create", () => {
	...
	test("should create org with allowPublicRegistration to false and onlyUseForBrandingOverride to true", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should trigger unique constraint on name", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;

		await createNewOrg();
		const res1 = await createNewOrg(undefined, { slug: "another slug" });
		expect(res1.status).toBe(500);

		global.process.env.ENABLE_ORG = false as unknown as string;
	});

	test("should trigger unique constraint on slug", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;

		await createNewOrg();
		const res1 = await createNewOrg(undefined, { name: "another name" });
		expect(res1.status).toBe(500);

		global.process.env.ENABLE_ORG = false as unknown as string;
	});

	test("should create org with write_org scope", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;

		await attachIndividualScopes(db);
		const res = await createNewOrg(await getS2sToken(db, Scope.WriteOrg));
		const json = await res.json();
	...
	test("should return 401 without proper scope", async () => {
		...
		const res = await createNewOrg(await getS2sToken(db, Scope.ReadOrg));
		expect(res.status).toBe(401);
...
describe("update", () => {
	...
	test("could update org with allowPublicRegistration and onlyUseForBrandingOverride", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should update org with write_org scope", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should update user orgSlug when update org slug", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should throw error if org not found", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
...
describe("delete", () => {
	...
	test("should delete org with write_org scope", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should throw error if org has users", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
...
describe("get all active users", () => {
	...
	test("should only get user from queried org", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should return all users with read_user and read org scope", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		...
		const res = await app.request(
			...
			{
				headers: {
					Authorization: `Bearer ${await getS2sToken(db, `${Scope.ReadUser} ${Scope.ReadOrg}`)}`,
				},
			},
			mock(db),
		);
	...
	test("should return 401 without proper scope", async () => {
		...
		const res2 = await app.request(BaseRoute, {}, mock(db));
...
describe("get all users", () => {
	test("should return all users in org", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should only get users from queried org", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should return all users with read_user and read_org scope", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
