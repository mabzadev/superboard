...
import { dbTime, getS2sToken } from "tests/util";
...
describe("create user org group", () => {
	test("should create user org group", async () => {
		...
		const userOrgGroup = await db
			.prepare('SELECT * FROM "user_org_group" WHERE "userId" = 1 AND "orgGroupId" = 1')
			.get();
	...
	test("should throw error if user does not belong to org", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should throw error if org group not found", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should throw error if feature flag is disabled", async () => {
		await insertUsers(db);
		await insertOrgGroups(db);
...
describe("delete user org group", () => {
	test("should delete user org group", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
