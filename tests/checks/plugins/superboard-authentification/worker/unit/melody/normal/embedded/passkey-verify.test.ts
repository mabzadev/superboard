...
import { migrate, mock, mockedKV, passkeyVerifyMock } from "tests/mock";
...
describe("get /authorize-passkey-verify", () => {
	test("could get passkey verify options by email", async () => {
		...
		const res = await app.request(
			`${routeConfig.EmbeddedRoute.PasskeyVerify.replace(":sessionId", sessionId)}`,
			{},
			mock(db),
		);
...
describe("post /passkey-verify", () => {
	...
	test("should throw error if wrong challenge provided", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
