...
describe("get /authorize-apple", () => {
	const mockAppleFetch = vi.fn(async (url, params) => {
		if (url === "https://appleid.apple.com/auth/token" && params.body.get("code") === "aaa") {
			...
			const idToken = await jwtService.signWithKid(c, {
	...
	test("should redirect to consent with a new Apple account", async () => {
		...
		expect(res.headers.get("Location")).toContain(
			`${routeConfig.IdentityRoute.ProcessView}?state=123&code=`,
		);
		expect(res.headers.get("Location")).toContain("&step=consent");
		expect(res.headers.get("Location")).toContain(
			"&locale=en&redirect_uri=http://localhost:3000/en/dashboard",
		);
	...
	test("should throw error if no state provided", async () => {
		global.process.env.APPLE_AUTH_CLIENT_ID = "123";
		global.process.env.APPLE_AUTH_CLIENT_SECRET = "abc";

		const params = new URLSearchParams();
		params.append("code", "aaa");
	...
	test("should throw error if state is malformed json", async () => {
		global.process.env.APPLE_AUTH_CLIENT_ID = "123";
		global.process.env.APPLE_AUTH_CLIENT_SECRET = "abc";

		const params = new URLSearchParams();
		params.append("code", "aaa");
		params.append("state", "not-json");
	...
	test("should redirect back to app with a new Apple account when consent not need", async () => {
		global.process.env.APPLE_AUTH_CLIENT_ID = "123";
		global.process.env.APPLE_AUTH_CLIENT_SECRET = "abc";
		global.process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		const res = await getAppleRequest();
		expect(res.headers.get("Location")).toContain(
			"http://localhost:3000/en/dashboard?state=123&code=",
		);
	...
	test("should be blocked if not enable in config", async () => {
		const res = await prepareRequest();
		expect(res.status).toBe(400);
		expect(await res.text()).toBe(messageConfig.ConfigError.AppleSignInNotEnabled);
	});

	test("should be blocked if no secret provided in config", async () => {
		global.process.env.APPLE_AUTH_CLIENT_ID = "123";
		const res = await prepareRequest();
		expect(res.status).toBe(400);
		expect(await res.text()).toBe(messageConfig.ConfigError.AppleSignInNotEnabled);
	...
	test("should be blocked if no id provided in config", async () => {
		global.process.env.APPLE_AUTH_CLIENT_SECRET = "abc";
		const res = await prepareRequest();
		expect(res.status).toBe(400);
		expect(await res.text()).toBe(messageConfig.ConfigError.AppleSignInNotEnabled);
	...
	test("could throw error if wrong credential provided", async () => {
		global.process.env.APPLE_AUTH_CLIENT_ID = "123";
		global.process.env.APPLE_AUTH_CLIENT_SECRET = "abc";
	...
	test("should sign in with an existing apple account", async () => {
		global.process.env.APPLE_AUTH_CLIENT_ID = "123";
		global.process.env.APPLE_AUTH_CLIENT_SECRET = "abc";
		const res = await getAppleRequest();
		expect(res.headers.get("Location")).toContain(
	...
	test("could store org slug after sign up", async () => {
	...
	test("should not store org slug after sign up if allowPublicRegistration is false", async () => {
		...
		db.exec(
			'insert into "org" (name, slug, "companyEmailLogoUrl", "allowPublicRegistration", "onlyUseForBrandingOverride") values (\'test\', \'default\', \'https://test_logo.com\', 0, 0)',
		);
