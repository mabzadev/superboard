...
export const sendInitiateRequest = async (
	...
	const body = await postInitiateBody(appRecord, {
		orgSlug,
		redirectUri,
	});
...
describe("/initiate", () => {
	test("should return session id", async () => {
		...
		const res = await sendInitiateRequest(db, appRecord);
	...
	test("could generate session with org id", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;

		const appRecord = await getApp(db);

		const res = await sendInitiateRequest(db, appRecord, { orgSlug: "default" });
	...
	test("should reject plain code challenge method by default", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
	...
	test("should throw error if wrong redirect uri used", async () => {
		...
		const res = await sendInitiateRequest(db, appRecord, { redirectUri: "http://localhost:3001" });
		expect(res.status).toBe(401);
