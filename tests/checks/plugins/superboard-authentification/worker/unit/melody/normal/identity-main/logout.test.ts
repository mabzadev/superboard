...
describe("post /logout", () => {
	...
	test("should throw error if can not parse refresh token body", async () => {
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		const tokenJson = await prepareLogout();
	...
	test("could logout without post logout redirect uri", async () => {
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		const appRecord = await getApp(db);
		const tokenJson = await prepareLogout();
	...
	test("should pass through even if token has wrong client", async () => {
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		const appRecord = await getApp(db);
		await insertUsers(db);
		const res = await postSignInRequest(db, appRecord);
		...
		await mockedKV.put(
			tokenKey,
			JSON.stringify({
				...JSON.parse((await mockedKV.get(tokenKey)) ?? ""),
				authId: "123",
			}),
		);
		const logoutRes = await app.request(
