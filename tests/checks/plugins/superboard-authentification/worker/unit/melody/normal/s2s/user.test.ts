...
import { emailLogRecord, emailResponseMock, fetchMock, migrate, mock, mockedKV } from "tests/mock";
import {
...
import {
	...
	messageConfig,
	routeConfig,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/configs";
import { oauthDto } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/dtos";
import app from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/index";
import {
	userAppConsentModel,
	userModel,
	userPasskeyModel,
	orgModel,
} from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/models";
...
describe("get all", () => {
	...
	test("could get users by pagination", async () => {
		await insertUsers(db);
		await db.exec(`
          INSERT INTO "user"
          ("authId", locale, email, "socialAccountId", "socialAccountType", password, "firstName", "lastName")
	...
	test("should return all users with read_user scope", async () => {
		await insertUsers(db);
		...
		const res = await app.request(
			...
			{
				headers: {
					Authorization: `Bearer ${await getS2sToken(db, Scope.ReadUser)}`,
				},
			},
			mock(db),
		);
	...
	test("should return 401 without proper scope", async () => {
		await attachIndividualScopes(db);
		const res = await app.request(
			BaseRoute,
			{
				headers: {
					Authorization: `Bearer ${await getS2sToken(db, Scope.WriteUser)}`,
				},
			},
			mock(db),
		);
		...
		const res1 = await app.request(BaseRoute, {}, mock(db));
		expect(res1.status).toBe(401);
	});
});

describe("get by id", () => {
	test("should return user by authId 1-1-1-1", async () => {
		await insertUsers(db);
	...
	test("should return user with attributes", async () => {
	...
	test("should not return user with attributes if feature flag is disabled", async () => {
		await insertUsers(db);
	...
	test("should return user with org info", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;

		await db.exec(
			"insert into \"org\" (name, slug, \"termsLink\", \"privacyPolicyLink\") values ('test', 'default', 'https://google1.com', 'https://microsoft1.com')",
		);
	...
	test("should return user with org info and org groups", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should return user by authId 1-1-1-2", async () => {
		await insertUsers(db);
	...
	test("should return invitation expiry as a utc iso string", async () => {
		await insertUsers(db);
		await db
			.prepare(`
          update "user"
          ...
          where "authId" = ?
        `)
			.run(0, "pending-invite-token", "2099-01-01 00:00:00", "1-1-1-2");
	...
	test("should return 404 when can not find user by id", async () => {
		const res = await app.request(
			`${BaseRoute}/1-1-1-1`,
			{ headers: { Authorization: `Bearer ${await getS2sToken(db)}` } },
			mock(db),
...
describe("get locked ips", () => {
	test("should return all locked ips", async () => {
		await insertUsers(db);
		await mockedKV.put(
	...
	test("placeholder test for user has no email", async () => {
		await insertUsers(db);
...
describe("delete locked ips", () => {
	test("should delete all locked ips", async () => {
		await insertUsers(db);
...
describe("update", () => {
	...
	test("should not activate user with pending invitation", async () => {
		await insertUsers(db);
		await db
			.prepare(`
          update "user"
	...
	test("should update user attributes", async () => {
		...
		const attributeValues = await db
			.prepare('select * from "user_attribute_value" order by id asc')
			.all();
	...
	test("should not update if feature flag is disabled", async () => {
		await insertUsers(db);
		await insertUserAttributes(db);
		...
		await db
			.prepare(
				'insert into user_attribute_value ("userId", "userAttributeId", "value") values (?, ?, ?)',
			)
			.run(1, 3, "test value 3");
	...
	test("should fail to update user with duplicate unique attribute value", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should successfully update user with same unique attribute value (no change)", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should successfully update user with new unique attribute value", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should successfully create new unique attribute value for user", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should fail to create new unique attribute value with duplicate", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should allow non-unique attributes to have duplicate values", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should fail to update user with attribute value not matching regex pattern", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should successfully update user with attribute value matching regex pattern", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should allow any value when attribute has no validationRegex", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should fail to update existing attribute value with invalid regex pattern", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should successfully update existing attribute value with valid regex pattern", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should handle both unique and regex validation on same attribute", async () => {
		global.process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
		...
		expect(await res1.text()).toBe(
			'Value for attribute "employee_code" does not match the validation rule',
		);
	...
	test("should update user and get org info", async () => {
		process.env.ENABLE_ORG = true as unknown as string;

		await insertUsers(db);
		await db.prepare("insert into role (name) values (?)").run("test");

		await db.prepare('insert into "org" (name, slug) values (?, ?)').run("test", "test");

		await db.prepare('insert into "org" (name, slug) values (?, ?)').run("test1", "test1");

		await db.prepare('insert into "user_org" ("userId", "orgId") values (?, ?)').run(2, 1);

		await db.prepare('insert into "user_org" ("userId", "orgId") values (?, ?)').run(2, 2);
	...
	test("throw error if switch to an org that the user is not in", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("return null if can not find user org by slug", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		await insertUsers(db);
		await db.prepare("insert into role (name) values (?)").run("test");
	...
	test("return when nothing updated", async () => {
		await insertUsers(db);
		await db.prepare("insert into role (name) values (?)").run("test");
	...
	test("should update user with write_user scope", async () => {
		await insertUsers(db);
		await attachIndividualScopes(db);
...
describe("post invitation", () => {
	...
	test("should invite a user with roles", async () => {
		await db.prepare("insert into role (name) values (?)").run("test");
	...
	test("should invite a user with names, locale and signinUrl", async () => {
		emailResponseMock.mockClear();
		const mockFetch = emailResponseMock;
		global.fetch = mockFetch as Mock;
	...
	test("should invite a user with org slug", async () => {
		await db.exec(
			"insert into \"org\" (name, slug, \"termsLink\", \"privacyPolicyLink\", \"onlyUseForBrandingOverride\") values ('org1', 'org1', 'https://google1.com', 'https://microsoft1.com', 0)",
		);
	...
	test("should throw error if org does not allow invite", async () => {
		await db.exec(
			"insert into \"org\" (name, slug, \"termsLink\", \"privacyPolicyLink\", \"onlyUseForBrandingOverride\") values ('org2', 'org2', 'https://google2.com', 'https://microsoft2.com', 1)",
		);
...
describe("post resend invitation", () => {
	...
	test("should resend invitation", async () => {
		...
		const user = (await db
			.prepare('select * from "user" where "authId" = ?')
			.get("1-1-1-2")) as any;
	...
	test("should resend invitation with locale and signinUrl", async () => {
		await insertInvitedUser();
	...
	test("should throw error if user already active", async () => {
		await insertUsers(db);
...
describe("delete invitation", () => {
	...
	test("should throw error if user already active", async () => {
		...
		await db
          ...
          where "authId" = ?
        `)
			.run("pending-invite-token", "2099-01-01 00:00:00", "1-1-1-2");
...
describe("send verification email", () => {
	...
	test("could log email when failed", async () => {
		global.process.env.ENABLE_EMAIL_LOG = true as unknown as string;
		await insertUsers(db);
	...
	test("pass through if failed to send email", async () => {
		await insertUsers(db);
	...
	test("throw error if no email sender set", async () => {
	...
	test("should throw error if user has no email", async () => {
		await insertUsers(db);
		await db.prepare('update "user" set "email" = ?').run(null);
...
describe("delete", () => {
	...
	test("should throw error if can not find user", async () => {
		await insertUsers(db);
...
describe("get user app consents", () => {
	test("should return user app consents", async () => {
		await insertUsers(db);
...
describe("delete app consent", () => {
	test("should delete app consent", async () => {
		await insertUsers(db);
		...
		const checkJson = (await checkRes.json()) as {
			consentedApps: userAppConsentModel.ConsentedApp[];
		};
		expect(checkJson.consentedApps).toStrictEqual([]);
	});
});

describe("enroll email mfa", () => {
	...
	test("should enroll email mfa", async () => {
	...
	test("if email is enforced by config", async () => {
	...
	test("if user already enrolled with email", async () => {
		await insertUsers(db);
	...
	test("should throw error for wrong id", async () => {
		await insertUsers(db);
...
describe("Unenroll email mfa", () => {
	...
	test("should unenroll email mfa", async () => {
		await insertUsers(db);
	...
	test("if user is not enrolled", async () => {
		insertUsers(db);
		await handleUnenrollRequest();
	});
...
describe("enroll otp mfa", () => {
	...
	test("should enroll otp mfa", async () => {
		const userRes = await enrollAndCheckUser();
	...
	test("if otp is enforced by config", async () => {
...
describe("Unenroll otp mfa", () => {
	...
	test("should unenroll otp mfa", async () => {
		await insertUsers(db);
...
describe("enroll sms mfa", () => {
	...
	test("should enroll sms mfa", async () => {
		const userRes = await enrollAndCheckUser();
	...
	test("if sms is enforced by config", async () => {
...
describe("Unenroll sms mfa", () => {
	...
	test("should unenroll sms mfa", async () => {
		await insertUsers(db);
...
describe("link account", () => {
	test("should linking account", async () => {
		await insertUsers(db);
	...
	test("should throw error if account already linked", async () => {
		await insertUsers(db);
		await db.exec(`
          INSERT INTO "user"
          ("authId", locale, email, "socialAccountId", "socialAccountType", password, "firstName", "lastName")
...
describe("unlink account", () => {
	test("should unlinking account", async () => {
		await insertUsers(db);
...
describe("delete passkey", () => {
	test("should delete user passkey", async () => {
		await insertUsers(db);
...
describe("impersonate user", () => {
	...
	test("should throw error if no impersonatorToken provided", async () => {
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
	...
	test("should throw error if impersonator is not super admin", async () => {
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
	...
	test("should throw error if impersonate non-SPA app", async () => {
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
	...
	test("should throw error if consent is required", async () => {
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
	...
	test("should throw error if not using root scope", async () => {
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
...
describe("get user orgs", () => {
	test("should return user orgs", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		...
		await db.exec(
			"insert into \"org\" (name, slug, \"termsLink\", \"privacyPolicyLink\", \"onlyUseForBrandingOverride\") values ('org2', 'org2', 'https://google2.com', 'https://microsoft2.com', 0)",
		);
		await db.exec(
			"insert into \"org\" (name, slug, \"termsLink\", \"privacyPolicyLink\", \"onlyUseForBrandingOverride\") values ('org3', 'org3', 'https://google3.com', 'https://microsoft3.com', 0)",
		);
	...
	test("should return empty array if user has no orgs", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should filter out branding-only orgs", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should return 401 without proper scope", async () => {
		...
		const res1 = await app.request(`${BaseRoute}/1-1-1-1/orgs`, {}, mock(db));
		expect(res1.status).toBe(401);

		process.env.ENABLE_ORG = false as unknown as string;
	});

	test("should return 404 when org feature is not enabled", async () => {
		process.env.ENABLE_ORG = false as unknown as string;
...
describe("post user orgs", () => {
	test("should update user orgs", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should add new orgs while keeping existing ones", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
	...
	test("should remove all orgs if empty array provided", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
...
describe("get /users/:authId/active-sessions", () => {
	...
	test("should return multiple active sessions after multiple sign-ins", async () => {
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		await insertUsers(db);
	...
	test("should return empty array for user with no active sessions", async () => {
		await insertUsers(db);
	...
	test("should work with read_user scope", async () => {
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		await insertUsers(db);
		await attachIndividualScopes(db);
	...
	test("should return 401 without authorization", async () => {
		...
		const res = await app.request(`${BaseRoute}/1-1-1-1/active-sessions`, {}, mock(db));
		expect(res.status).toBe(401);
	});

	test("should return 401 with wrong scope", async () => {
		await insertUsers(db);
		await attachIndividualScopes(db);
...
describe("delete /users/:authId/active-sessions/:sessionId", () => {
	const signInAndGetTokens = async () => {
		const tokenRes = await exchangeWithAuthToken(db);
		return tokenRes.json() as Promise<{ refresh_token: string; access_token: string }>;
	};

	test("should delete an active session for a user", async () => {
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		await insertUsers(db);
		await signInAndGetTokens();
		...
		const { activeSessions: sessions } = (await listRes.json()) as {
			activeSessions: { sessionId: string }[];
		};
	...
	test("should delete only the specified session when multiple sessions exist", async () => {
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		await insertUsers(db);
		await signInAndGetTokens();
		await signInAndGetTokens();
	...
	test("should return 404 when session does not belong to user", async () => {
		await insertUsers(db);
...
describe("privileged role assignment guard", () => {
	const updateRolesReq = async (authId: string, roles: string[], token: string) =>
		await app.request(
			`${BaseRoute}/${authId}`,
			{
				method: "PUT",
	...
	const inviteWithRolesReq = async (roles: string[], token: string) =>
		await app.request(
			`${BaseRoute}/invitations`,
			{
				method: "POST",
	...
	test("should reject updating a user with a privileged role from a write_user caller", async () => {
		await insertUsers(db);
		await attachIndividualScopes(db);
		const token = await getS2sToken(db, Scope.WriteUser);

		const res = await updateRolesReq("1-1-1-1", ["super_admin"], token);
	...
	test("should allow updating a user with a privileged role from a root caller", async () => {
		await insertUsers(db);

		const res = await updateRolesReq("1-1-1-1", ["super_admin"], await getS2sToken(db));
		const json = (await res.json()) as { user: userModel.ApiRecordFull };
	...
	test("should allow updating a user with a non-privileged role from a write_user caller", async () => {
		await insertUsers(db);
		await db.prepare("insert into role (name) values (?)").run("test");
		await attachIndividualScopes(db);
		const token = await getS2sToken(db, Scope.WriteUser);

		const res = await updateRolesReq("1-1-1-1", ["test"], token);
		const json = (await res.json()) as { user: userModel.ApiRecordFull };
	...
	test("should reject inviting a user with a privileged role from a write_user caller", async () => {
		await attachIndividualScopes(db);
		const token = await getS2sToken(db, Scope.WriteUser);
		...
		const res = await inviteWithRolesReq(["super_admin"], token);
	...
	test("should allow inviting a user with a privileged role from a root caller", async () => {
		const mockFetch = emailResponseMock;
		global.fetch = mockFetch as Mock;

		const res = await inviteWithRolesReq(["super_admin"], await getS2sToken(db));
		const json = (await res.json()) as { user: userModel.ApiRecordFull };
	...
	test("should allow inviting a user with a non-privileged role from a write_user caller", async () => {
		await db.prepare("insert into role (name) values (?)").run("test");
		await attachIndividualScopes(db);
		const token = await getS2sToken(db, Scope.WriteUser);
		...
		const res = await inviteWithRolesReq(["test"], token);
		const json = (await res.json()) as { user: userModel.ApiRecordFull };
