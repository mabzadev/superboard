...
describe("post /authorize-facebook", () => {
	const mockFbFetch = vi.fn(async (url) => {
		if (
			url ===
			"https://graph.facebook.com/oauth/access_token?client_id=123&client_secret=abc&grant_type=client_credentials"
		) {
			...
			url === "https://graph.facebook.com/debug_token?input_token=aaa&access_token=token123"
		) {
			return Promise.resolve({
				ok: true,
				json: () => ({
	...
	test("should sign in with a new facebook account", async () => {
	...
	test("should reject a token issued to another Facebook app", async () => {
		global.process.env.FACEBOOK_AUTH_CLIENT_ID = "123";
		global.process.env.FACEBOOK_AUTH_CLIENT_SECRET = "abc";
	...
	test("should sign in with an existing facebook account", async () => {
		global.process.env.FACEBOOK_AUTH_CLIENT_ID = "123";
		global.process.env.FACEBOOK_AUTH_CLIENT_SECRET = "abc";
		await postFacebookRequest();
		await postFacebookRequest();
