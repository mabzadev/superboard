...
describe("get /app-banners", () => {
	test("should get app banners", async () => {
		...
		const res = await app.request(
			routeConfig.EmbeddedRoute.AppBanners.replace(":sessionId", sessionId),
			{ method: "GET" },
			mock(db),
		);
	...
	test("should throw error when session id is invalid", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_APP_BANNER = true as unknown as string;
		...
		const res = await app.request(
			routeConfig.EmbeddedRoute.AppBanners.replace(":sessionId", "invalid-session-id"),
			{ method: "GET" },
			mock(db),
		);
	...
	test("should return empty banners when no banners exist", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_APP_BANNER = true as unknown as string;
	...
	test("should throw error when embedded auth origin is invalid", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = [] as unknown as string;
		process.env.ENABLE_APP_BANNER = true as unknown as string;
		...
		const res = await app.request(
			routeConfig.EmbeddedRoute.AppBanners.replace(":sessionId", "123"),
			{ method: "GET" },
			mock(db),
		);
