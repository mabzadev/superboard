...
import { getApp, insertUsers, markEmbeddedSessionAsSecured } from "tests/identity";
import { migrate, mock, mockedKV, passkeyEnrollMock } from "tests/mock";
...
export const sendVerifiedSignInRequest = async (
	...
	await insertUsers(db, withConsent);
...
export const sendCorrectEnrollPasskeyReq = async ({
	...
	const res = await app.request(
		routeConfig.EmbeddedRoute.PasskeyEnroll.replace(":sessionId", sessionId),
		{
			method: "POST",
...
describe("get /passkey-enroll", () => {
	test("should get passkey enroll", async () => {
	...
	test("should throw error if feature not allowed", async () => {
		...
		expect(await passkeyEnrollRes.text()).toBe(
	...
	test("should throw error if use wrong code", async () => {
		...
		const passkeyEnrollRes = await app.request(
			routeConfig.EmbeddedRoute.PasskeyEnroll.replace(":sessionId", "abc"),
			{ method: "GET" },
			mock(db),
		);
...
describe("post /passkey-enroll", () => {
	test("should enroll passkey", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		...
		const passkey = db
			.prepare('SELECT * from user_passkey WHERE "userId" = 1 AND "credentialId" = ?')
			.get(passkeyEnrollMock.rawId);
	...
	test("should throw error if can not find challenge", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
	...
	test("should throw error if consent is not completed", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = true as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;

		const { sessionId } = await sendVerifiedSignInRequest(db, { withConsent: false });
	...
	test("should throw error if mfa is not completed", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
	...
	test("should throw error if return wrong challenge", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
	...
	test("should throw error if use wrong session id", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
	...
	test("should throw error if can not verify passkey", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
...
describe("post /passkey-enroll-decline", () => {
	test("should skip passkey enroll", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		...
		const res = await app.request(
			routeConfig.EmbeddedRoute.PasskeyEnrollDecline.replace(":sessionId", sessionId),
			{
				method: "POST",
	...
	test("should skip passkey enroll and remember", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
	...
	test("should throw error if use session id", async () => {
		process.env.ALLOW_PASSKEY_ENROLLMENT = true as unknown as string;
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;

		const res = await app.request(
			routeConfig.EmbeddedRoute.PasskeyEnrollDecline.replace(":sessionId", "abc"),
			{
				method: "POST",
				body: JSON.stringify({ remember: false }),
			},
