...
import { prepareFollowUpBody, insertUsers, markAuthCodeAsSecured } from "tests/identity";
...
const sendCorrectGetChangeOrgRequest = async ({
	...
	await db.prepare('UPDATE "user" SET "orgSlug" = ? WHERE id = ?').run("default-org", 1);
...
describe("get /change-org", () => {
	...
	test("should return empty orgs when user has no org memberships", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		global.process.env.BLOCKED_POLICIES = [] as unknown as string;
	...
	test("should throw error if org feature is not enabled", async () => {
		process.env.ENABLE_ORG = false as unknown as string;
		global.process.env.BLOCKED_POLICIES = [] as unknown as string;
	...
	test("should throw error if change org policy is blocked", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		global.process.env.BLOCKED_POLICIES = [Policy.ChangeOrg] as unknown as string;
...
describe("post /change-org", () => {
	test("should change user org", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		global.process.env.BLOCKED_POLICIES = [] as unknown as string;
	...
	test("should return success when changing to same org", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		global.process.env.BLOCKED_POLICIES = [] as unknown as string;
	...
	test("should throw error when changing to non-existent org", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		global.process.env.BLOCKED_POLICIES = [] as unknown as string;
	...
	test("should throw error when changing to org user does not belong to", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		global.process.env.BLOCKED_POLICIES = [] as unknown as string;
