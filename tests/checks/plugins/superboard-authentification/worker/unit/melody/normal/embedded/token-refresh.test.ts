...
const sendTokenRefreshRequest = async (
	...
	await app.request(
		routeConfig.EmbeddedRoute.SignIn.replace(":sessionId", correctSessionId),
		{
			method: "POST",
			body: JSON.stringify({
...
describe("/token-refresh", () => {
	test("should refresh access token", async () => {
		...
		const { res } = await sendTokenRefreshRequest(db, {});
	...
	test("should throw error if no session found", async () => {
		...
		const { res } = await sendTokenRefreshRequest(db, { refreshToken: "abc" });
