...
const sendCorrectChangeEmailCodeReq = async ({
	...
	const body = await prepareFollowUpBody(db, policy);
...
const sendCorrectChangeEmailReq = async ({
	...
	const correctVerificationCode = await mockedKV.get(
		`${adapterConfig.BaseKVKey.ChangeEmailCode}-1-test_new@email.com`,
	);
...
describe("post /change-email-code", () => {
	test("could send code", async () => {
	...
	test("pass through if failed send email", async () => {
		const mockFetch = vi.fn(async () => {
	...
	test("should stop after reach threshold", async () => {
		global.process.env.CHANGE_EMAIL_EMAIL_THRESHOLD = 2 as unknown as string;

		const { res, correctBody } = await sendCorrectChangeEmailCodeReq();
		const json = await res.json();
		expect(json).toStrictEqual({ success: true });
		expect(
			await mockedKV.get(`${adapterConfig.BaseKVKey.ChangeEmailAttempts}-test@email.com`),
		).toBe("1");
	...
	test("should throw error if wrong auth code used", async () => {
		await insertUsers(db, false);
		const body = await prepareFollowUpBody(db, Policy.ChangeEmail);
		const res = await app.request(
			routeConfig.IdentityRoute.ChangeEmailCode,
			{
				method: "POST",
	...
	test("should throw error if auth code has the wrong policy", async () => {
	...
	test("should throw error if policy is blocked", async () => {
	...
	test("should throw error for social account", async () => {
		...
		const credential = await jwtService.signWithKid(c, {
...
describe("post /change-email", () => {
	test("could change email", async () => {
		...
		expect(
			await mockedKV.get(`${adapterConfig.BaseKVKey.ChangeEmailCode}-1-test_new@email.com`),
		).toBeFalsy();

		const appRecord = await getApp(db);
		const reLoginRes = await postSignInRequest(db, appRecord, { email: "test_new@email.com" });
	...
	test("throw error if change to same email", async () => {
		await insertUsers(db, false);
		const body = await prepareFollowUpBody(db, Policy.ChangeEmail);
		await markAuthCodeAsSecured(body.code);
		const correctBody = {
		...
		const correctVerificationCode = await mockedKV.get(
			`${adapterConfig.BaseKVKey.ChangeEmailCode}-1-test@email.com`,
		);
	...
	test("should throw 400 if use wrong auth code", async () => {
	...
	test("should throw 400 if auth code has the wrong policy", async () => {
		await insertUsers(db, false);
		const body = await prepareFollowUpBody(db, Policy.SignInOrSignUp);
		await markAuthCodeAsSecured(body.code);
	...
	test("should throw 400 if use wrong verification code", async () => {
		const { correctBody } = await sendCorrectChangeEmailCodeReq();
		const { res } = await sendCorrectChangeEmailReq({
			code: correctBody.code,
			verificationCode: "123456",
	...
	test("should lock change email code verification after failed attempts", async () => {
		...
		expect(await mockedKV.get(`${adapterConfig.BaseKVKey.FailedChangeEmailCodeAttempts}-1`)).toBe(
			"2",
		);
	...
	test("should not touch failed code attempts when change email code threshold is disabled", async () => {
