...
describe("get /authorize-discord", () => {
	const mockDiscordFetch = vi.fn(async (url, params) => {
	...
	test("should redirect to consent with a new Discord account", async () => {
	...
	test("could load user email if exists", async () => {
		global.process.env.DISCORD_AUTH_CLIENT_ID = "123";
		global.process.env.DISCORD_AUTH_CLIENT_SECRET = "abc";

		const mockDiscordFetchWithEmail = vi.fn(async (url, params) => {
			if (url === "https://discord.com/api/v10/oauth2/token" && params.body.get("code") === "aaa") {
				return Promise.resolve({
					ok: true,
					json: () => ({ access_token: "token123" }),
	...
	test("should redirect back to app with a new Discord account when consent not need", async () => {
		global.process.env.DISCORD_AUTH_CLIENT_ID = "123";
		global.process.env.DISCORD_AUTH_CLIENT_SECRET = "abc";
	...
	test("should sign in with an existing discord account", async () => {
		global.process.env.DISCORD_AUTH_CLIENT_ID = "123";
		global.process.env.DISCORD_AUTH_CLIENT_SECRET = "abc";
		const res = await getDiscordRequest();
		expect(res.headers.get("Location")).toContain(
