...
describe("get /otp-mfa-setup", () => {
	test("should get recovery code enroll", async () => {
		...
		const recoveryCodeEnrollRes = await app.request(
			routeConfig.EmbeddedRoute.RecoveryCodeEnroll.replace(":sessionId", sessionId),
			{ method: "GET" },
			mock(db),
		);
	...
	test("should throw error if recovery code is already set", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_RECOVERY_CODE = true as unknown as string;
		...
		expect(await recoveryCodeEnrollRes1.text()).toBe(
	...
	test("should throw error if session id is invalid", async () => {
		...
		const recoveryCodeEnrollRes = await app.request(
			routeConfig.EmbeddedRoute.RecoveryCodeEnroll.replace(":sessionId", "123456"),
			{ method: "GET" },
			mock(db),
		);
