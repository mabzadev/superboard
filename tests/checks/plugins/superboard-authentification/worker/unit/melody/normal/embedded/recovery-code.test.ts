...
const sendSignInRequest = async (
	...
	const res = await app.request(
		routeConfig.EmbeddedRoute.RecoveryCode.replace(":sessionId", sessionId),
		{
			method: "POST",
			body: JSON.stringify({
...
describe("/recovery-code", () => {
	test("should sign in with recovery code", async () => {
		...
		const { res, sessionId } = await sendSignInRequest(db, {});
	...
	test("should throw error if no user found", async () => {
		...
		const { res } = await sendSignInRequest(db, { recoveryCode: "1234567890" });
