...
describe("post /reset-password-code", () => {
	test("should send reset code", async () => {
	...
	test("should return true if user is inactive", async () => {
		await insertUsers(db);
		disableUser(db);
		const { res } = await sendCorrectResetPasswordCodeReq();
		const json = await res.json();
...
describe("post /authorize-reset", () => {
	test("should reset password and unlock account", async () => {
		global.process.env.ACCOUNT_LOCKOUT_THRESHOLD = 1 as unknown as string;
		const appRecord = await getApp(db);
		await insertUsers(db);

		await postSignInRequest(db, appRecord, { password: "Password2!" });
		expect(
		...
		const signInRes = await postSignInRequest(db, appRecord, { password: "Password2!" });
		expect(await signInRes.json()).toBeTruthy();
		global.process.env.ACCOUNT_LOCKOUT_THRESHOLD = 5 as unknown as string;
	});

	test("should throw error with wrong code", async () => {
		await insertUsers(db);
	...
	test("should lock password reset code verification after failed attempts", async () => {
		...
		expect(await mockedKV.get(`${adapterConfig.BaseKVKey.FailedPasswordResetCodeAttempts}-1`)).toBe(
			"2",
		);
	...
	test("should not touch failed code attempts when password reset code threshold is disabled", async () => {
	...
	test("should throw error when reset with same password", async () => {
		await insertUsers(db);
	...
	test("should not reset if user is inactive", async () => {
		await insertUsers(db);
	...
	test("should not reset if use a wrong email", async () => {
		await insertUsers(db);
	...
	test("could disable account unlock by reset password", async () => {
		global.process.env.ACCOUNT_LOCKOUT_THRESHOLD = 1 as unknown as string;
	...
	test("should not clear failed login attempts when account lockout threshold is disabled", async () => {
		...
		const clearFailedLoginAttemptsByIPSpy = vi.spyOn(kvService, "clearFailedLoginAttemptsByIP");
		const { res } = await sendCorrectResetPasswordReq();
