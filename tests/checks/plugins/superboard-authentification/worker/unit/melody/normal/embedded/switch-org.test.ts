...
const insertUserOrgs = (db: Database, userId: number, orgIds: number[]) => {
...
const sendVerifiedSignInRequest = async (
	...
	const initiateRes = await sendInitiateRequest(db, appRecord, { orgSlug });
	...
	await db.prepare('UPDATE "user" SET "orgSlug" = ? WHERE id = ?').run(orgSlug ?? "default-org", 1);
...
describe("get /user-orgs", () => {
	test("should get user orgs", async () => {
		...
		await insertOrgs(db);
		const { sessionId } = await sendVerifiedSignInRequest(db, { orgSlug: "default-org" });

		// Insert user_org relationships
		await insertUserOrgs(db, 1, [1, 2, 3]);

		const userOrgsRes = await app.request(
			routeConfig.EmbeddedRoute.UserOrgs.replace(":sessionId", sessionId),
			{ method: "GET" },
			mock(db),
		);
	...
	test("should throw error when sessionId is invalid", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_ORG = true as unknown as string;
		process.env.ALLOW_USER_SWITCH_ORG_ON_SIGN_IN = true as unknown as string;
		...
		const userOrgsRes = await app.request(
			routeConfig.EmbeddedRoute.UserOrgs.replace(":sessionId", "invalid-session-id"),
			{ method: "GET" },
			mock(db),
		);
	...
	test("should throw error when org feature is disabled", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_ORG = false as unknown as string;
		process.env.ALLOW_USER_SWITCH_ORG_ON_SIGN_IN = true as unknown as string;
	...
	test("should throw error when user switch org feature is disabled", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_ORG = true as unknown as string;
		process.env.ALLOW_USER_SWITCH_ORG_ON_SIGN_IN = false as unknown as string;
...
describe("post /user-orgs", () => {
	test("should switch user org", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_ORG = true as unknown as string;
		process.env.ALLOW_USER_SWITCH_ORG_ON_SIGN_IN = true as unknown as string;
	...
	test("should throw error when switching to non-existent org", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_ORG = true as unknown as string;
		process.env.ALLOW_USER_SWITCH_ORG_ON_SIGN_IN = true as unknown as string;
	...
	test("should throw error when switching to org user does not belong to", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_ORG = true as unknown as string;
		process.env.ALLOW_USER_SWITCH_ORG_ON_SIGN_IN = true as unknown as string;
	...
	test("should throw error when org parameter is missing", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_ORG = true as unknown as string;
		process.env.ALLOW_USER_SWITCH_ORG_ON_SIGN_IN = true as unknown as string;
