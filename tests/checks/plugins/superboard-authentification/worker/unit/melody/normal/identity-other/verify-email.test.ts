...
const sendCorrectVerifyEmailReq = async ({
	code,
}: {
	code?: string;
} = {}) => {
...
describe("post /authorize-account", () => {
	test("should send verification email after sign up", async () => {
...
describe("get /verify-email-view", () => {
	test("should show verify email page", async () => {
		await prepareUserAccount(db);
		const currentUser = (await db
			.prepare('select * from "user" where id = 1')
			.get()) as userModel.Raw;
		...
		expect(
			((await mockedKV.get(`${adapterConfig.BaseKVKey.EmailVerificationCode}-1`)) ?? "").length,
		).toBe(6);
		...
		expect(html).toContain(
			`locales: "${(process.env.SUPPORTED_LOCALES as unknown as string[]).join(",")}"`,
		);
		...
		expect(html).toContain(
			`<link rel="icon" type="image/x-icon" href="${process.env.COMPANY_LOGO_URL}"/>`,
		);
		expect(html).toContain(
			`<link href="${variableConfig.DefaultBranding.FontUrl?.replace(
				"&",
				"&amp;",
			)}" rel="stylesheet"/>`,
		);
		...
		expect(html).toContain(
			`--primary-button-color:${variableConfig.DefaultBranding.PrimaryButtonColor}`,
		);
		expect(html).toContain(
			`--primary-button-label-color:${variableConfig.DefaultBranding.PrimaryButtonLabelColor}`,
		);
		expect(html).toContain(
			`--primary-button-border-color:${variableConfig.DefaultBranding.PrimaryButtonBorderColor}`,
		);
		expect(html).toContain(
			`--secondary-button-color:${variableConfig.DefaultBranding.SecondaryButtonColor}`,
		);
		expect(html).toContain(
			`--secondary-button-label-color:${variableConfig.DefaultBranding.SecondaryButtonLabelColor}`,
		);
		expect(html).toContain(
			`--secondary-button-border-color:${variableConfig.DefaultBranding.SecondaryButtonBorderColor}`,
		);
		expect(html).toContain(
			`--critical-indicator-color:${variableConfig.DefaultBranding.CriticalIndicatorColor}`,
		);
	});
...
describe("post /verify-email", () => {
	...
	test("should not verify if wrong code provided", async () => {
		await prepareUserAccount(db);
	...
	test("should not verify if user already verified", async () => {
		await prepareUserAccount(db);

		const { res, correctCode } = await sendCorrectVerifyEmailReq();
		expect(res.status).toBe(200);
	...
	test("should not verify if user is disabled", async () => {
		await prepareUserAccount(db);
	...
	test("should lock after EMAIL_VERIFICATION_CODE_THRESHOLD failed attempts", async () => {
	...
	test("should not track failed attempts when EMAIL_VERIFICATION_CODE_THRESHOLD is 0", async () => {
		process.env.EMAIL_VERIFICATION_CODE_THRESHOLD = 0 as unknown as string;

		await prepareUserAccount(db);

		const getSpy = vi.spyOn(kvService, "getFailedEmailVerificationCodeAttemptsByIP");
		const setSpy = vi.spyOn(kvService, "setFailedEmailVerificationCodeAttempts");
