...
describe("get /app-banners", () => {
	...
	test("should throw error when use wrong clientId", async () => {
		process.env.ENABLE_APP_BANNER = true as unknown as string;
	...
	test("should throw error when clientId is not provided", async () => {
		process.env.ENABLE_APP_BANNER = true as unknown as string;

		await insertAppBanner(db);

		const res = await app.request(`${routeConfig.IdentityRoute.AppBanners}`, {}, mock(db));
	...
	test("should throw error when when app is not SPA", async () => {
		process.env.ENABLE_APP_BANNER = true as unknown as string;
