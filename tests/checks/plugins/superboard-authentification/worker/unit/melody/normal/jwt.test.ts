...
const signToken = async (payload: object) =>
	jwtService.signWithKid({ env: mock(db) } as never, payload);
...
const requestImpersonation = async (impersonatorToken: string) =>
	app.request(
		impersonationRoute,
		{
			method: "POST",
...
describe("getAccessTokenBody issuer validation", () => {
	test("should accept a well-formed access token issued by this server", async () => {
	...
	test("should reject an access token whose issuer does not match the server", async () => {
		await insertTestUser();

		const accessToken = await signToken(
			validAccessTokenPayload({ iss: "http://attacker.example.com" }),
		);
...
describe("getAccessTokenBody token type validation", () => {
	...
	test("should reject an id token presented as the impersonator token", async () => {
		await insertTestUser();
	...
	test("should reject an impersonator token without a scope claim", async () => {
		const currentTimestamp = timeUtil.getCurrentTimestamp();
	...
	test("should reject an impersonator token whose issuer does not match the server", async () => {
