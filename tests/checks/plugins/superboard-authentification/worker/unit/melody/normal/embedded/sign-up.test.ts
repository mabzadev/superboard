...
import { getApp, insertUsers } from "tests/identity";
...
const sendSignUpRequest = async (
	...
	const initiateRes = await sendInitiateRequest(db, appRecord);
	...
	const res = await app.request(
		routeConfig.EmbeddedRoute.SignUp.replace(":sessionId", sessionId),
		{
			method: "POST",
			body: JSON.stringify({
				email,
...
describe("get sign up info", () => {
	test("should return sign up info", async () => {
    ...
    `);
		...
		const res = await app.request(routeConfig.EmbeddedRoute.SignUp, { method: "GET" }, mock(db));
	...
	test("should return sign up info with regex validation", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should return empty sign up info if user attribute is disabled", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
...
describe("/signup", () => {
	test("should return next step", async () => {
		...
		const { res, sessionId } = await sendSignUpRequest(db, {
		...
		const user = (await db
			...
			.get("test1@email.com")) as userModel.Raw;
	...
	test("should sign up with no names", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		const { res, sessionId } = await sendSignUpRequest(db, {
	...
	test("should sign up with attributes", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
		...
		const attributeValues = (await db
			.prepare('select * from "user_attribute_value" where "userId" = 2')
	...
	test("should throw error if required attributes are not provided", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
		...
		const { res } = await sendSignUpRequest(db, {
			email: "test1@email.com",
			password: "Password1!",
			attributes: {
				1: "value1",
		...
		expect(await res.text()).toStrictEqual(
			`${messageConfig.RequestError.AttributeIsRequired}: test3`,
		);
	...
	test("should sign up without attributes if feature flag is disabled", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
	...
	test("should throw error if sessionId is invalid", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		...
		const res = await app.request(
			routeConfig.EmbeddedRoute.SignUp.replace(":sessionId", "123"),
			{
				method: "POST",
				body: JSON.stringify({
					email: "test1@email.com",
	...
	test("should throw error if names are required", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.NAMES_IS_REQUIRED = true as unknown as string;
	...
	test("should throw error if sign up is disabled", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
	...
	test("should indicate consent step if required", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
	...
	test("should fail to create user with duplicate unique attribute value", async () => {
		...
		const res1 = await sendSignUpRequestWithoutInsertUsers(db, sessionId, {
			email: "test1@email.com",
			password: "Password1!",
			attributes: { 1: "EMP001" },
		});
		...
		const res2 = await sendSignUpRequestWithoutInsertUsers(db, sessionId, {
	...
	test("should handle mixed unique and non-unique attributes correctly", async () => {
		...
		const res3 = await sendSignUpRequestWithoutInsertUsers(db, sessionId, {
	...
	test("should allow same user to keep their unique attribute value", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should handle both unique and regex validation on same attribute", async () => {
		...
		const res4 = await sendSignUpRequestWithoutInsertUsers(db, sessionId, {
