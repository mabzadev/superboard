...
import { prepareFollowUpBody, insertUsers, getApp, postSignInRequest } from "tests/identity";
import { migrate, mock, mockedKV } from "tests/mock";
...
import { userModel } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/models";
...
const generateOtpAtTime = (secret: string, timestamp: number) => {
...
const prepareFollowUpBodyByEmail = async (email: string) => {
	...
	const res = await postSignInRequest(db, appRecord, { email });
...
const postOtpMfa = (body: { code: string; locale: string }, mfaCode: string) =>
	app.request(
...
describe("get /otp-mfa-setup", () => {
	test("should return otp mfa setup", async () => {
	...
	test("should throw error if otp already verified", async () => {
		await insertUsers(db, false);
		await enrollOtpMfa(db);
	...
	test("should throw error if wrong auth code used", async () => {
		await insertUsers(db, false);
		await enrollOtpMfa(db);
...
describe("get /process-otp-mfa", () => {
	test("should return not allow fallback to email mfa", async () => {
		await insertUsers(db, false);
	...
	test("should return allow fallback to email mfa", async () => {
		await insertUsers(db, false);
		await enrollOtpMfa(db);
		const { res } = await sendCorrectGetProcessOtpMfaReq();
		expect(res.status).toBe(200);
...
describe("post /process-otp-mfa", () => {
	test("should pass otp mfa", async () => {
	...
	test("should accept current time-step otp mfa code", async () => {
		await insertUsers(db);
		await enrollOtpMfa(db);
		await sendCorrectGetOtpMfaSetupReq();
		const body = await prepareFollowUpBody(db);
		...
		const token = generateOtpAtTime(currentUser.otpSecret, fixedOtpTime.getTime());

		const res = await postOtpMfa(body, token);
		expect(res.status).toBe(200);
		const json = (await res.json()) as { code: string };
		expect(await mockedKV.get(`${adapterConfig.BaseKVKey.OtpMfaCode}-${json.code}`)).toBe("1");
	});

	test("should accept previous and next time-step otp mfa codes", async () => {
		await insertUsers(db);
		await enrollOtpMfa(db);
		await sendCorrectGetOtpMfaSetupReq();
		...
		const previousStepRes = await postOtpMfa(previousStepBody, previousStepToken);
		expect(previousStepRes.status).toBe(200);
		const previousStepJson = (await previousStepRes.json()) as { code: string };
		expect(
			await mockedKV.get(`${adapterConfig.BaseKVKey.OtpMfaCode}-${previousStepJson.code}`),
		).toBe("1");

		const nextStepRes = await postOtpMfa(nextStepBody, nextStepToken);
		expect(nextStepRes.status).toBe(200);
		const nextStepJson = (await nextStepRes.json()) as { code: string };
		expect(await mockedKV.get(`${adapterConfig.BaseKVKey.OtpMfaCode}-${nextStepJson.code}`)).toBe(
			"1",
		);
	});

	test("should reject otp mfa codes outside one time-step skew", async () => {
		await insertUsers(db, false);
		await enrollOtpMfa(db);
		await sendCorrectGetOtpMfaSetupReq();
		...
		const pastStepRes = await postOtpMfa(pastStepBody, pastStepToken);
		expect(pastStepRes.status).toBe(401);
		expect(await pastStepRes.text()).toBe(messageConfig.RequestError.WrongMfaCode);
		expect(
			await mockedKV.get(`${adapterConfig.BaseKVKey.OtpMfaCode}-${pastStepBody.code}`),
		).toBeNull();

		const futureStepRes = await postOtpMfa(futureStepBody, futureStepToken);
		expect(futureStepRes.status).toBe(401);
		expect(await futureStepRes.text()).toBe(messageConfig.RequestError.WrongMfaCode);
		expect(
			await mockedKV.get(`${adapterConfig.BaseKVKey.OtpMfaCode}-${futureStepBody.code}`),
		).toBeNull();
	});

	test("should reject replay of the same user otp time step", async () => {
		await insertUsers(db);
		await enrollOtpMfa(db);
		await sendCorrectGetOtpMfaSetupReq();
		...
		const firstRes = await postOtpMfa(firstBody, token);
		expect(firstRes.status).toBe(200);

		const secondRes = await postOtpMfa(secondBody, token);
		expect(secondRes.status).toBe(401);
		expect(await secondRes.text()).toBe(messageConfig.RequestError.WrongMfaCode);
		expect(
			await mockedKV.get(`${adapterConfig.BaseKVKey.OtpMfaCode}-${secondBody.code}`),
		).toBeNull();
	});

	test("should reject replay of the same otp code after the current step advances within the skew window", async () => {
		await insertUsers(db);
		await enrollOtpMfa(db);
		await sendCorrectGetOtpMfaSetupReq();
		const firstBody = await prepareFollowUpBody(db);
	...
	test("should allow the same user to verify on the next time step after succeeding on a previous one", async () => {
		await insertUsers(db);
		await enrollOtpMfa(db);
		await sendCorrectGetOtpMfaSetupReq();
		const firstBody = await prepareFollowUpBody(db);
		...
		vi.useFakeTimers({ toFake: ["Date"] });
		vi.setSystemTime(fixedOtpTime);
		const firstToken = generateOtpAtTime(currentUser.otpSecret, fixedOtpTime.getTime());
		const nextStepMs = fixedOtpTime.getTime() + totpPeriodMs;
		const secondToken = generateOtpAtTime(currentUser.otpSecret, nextStepMs);

		const firstRes = await postOtpMfa(firstBody, firstToken);
		expect(firstRes.status).toBe(200);

		vi.setSystemTime(new Date(nextStepMs));

		const secondRes = await postOtpMfa(secondBody, secondToken);
		expect(secondRes.status).toBe(200);
		const secondJson = (await secondRes.json()) as { code: string };
		expect(await mockedKV.get(`${adapterConfig.BaseKVKey.OtpMfaCode}-${secondJson.code}`)).toBe(
			"1",
		);
	});

	test("should allow same time-step otp code for different users", async () => {
		await insertUsers(db, true);
		await enrollOtpMfa(db);
		await sendCorrectGetOtpMfaSetupReq();
        ...
        `)
			.run("$2a$10$3HtEAf8YcN94V4GOR6ZBNu9tmoIflmEOqb9hUf0iqS4OjYVKe.9/C", firstUser.otpSecret);
		await db
			.prepare(
				'INSERT INTO user_app_consent ("userId", "appId", "scopes") values (2, 1, \'["profile","openid","offline_access"]\')',
			)
			.run();
		...
		vi.useFakeTimers({ toFake: ["Date"] });
		vi.setSystemTime(fixedOtpTime);
		const token = generateOtpAtTime(firstUser.otpSecret, fixedOtpTime.getTime());
	...
	test("should pass mfa if requied by config", async () => {
	...
	test("should throw error if otp secret not exists", async () => {
		await mockedKV.put(
	...
	test("should be blocked after 5 attempts", async () => {
	...
	test("could fallback to email mfa", async () => {
		await insertUsers(db);
		await enrollOtpMfa(db);
	...
	test("should set remember device cookie when rememberDevice is true", async () => {
	...
	test("should not set remember device cookie when rememberDevice is false", async () => {
		process.env.ENABLE_MFA_REMEMBER_DEVICE = true as unknown as string;
	...
	test("should not set remember device cookie when ENABLE_MFA_REMEMBER_DEVICE is false", async () => {
		process.env.ENABLE_MFA_REMEMBER_DEVICE = false as unknown as string;
	...
	test("should bypass otp mfa on subsequent login when device is remembered", async () => {
		process.env.ENABLE_MFA_REMEMBER_DEVICE = true as unknown as string;
	...
	test("should not bypass otp mfa when invalid remember device cookie is provided", async () => {
		process.env.ENABLE_MFA_REMEMBER_DEVICE = true as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
