...
export const sendTokenExchangeRequest = async (
	...
	await app.request(
		routeConfig.EmbeddedRoute.SignIn.replace(":sessionId", sessionId ?? correctSessionId),
		{
			method: "POST",
			body: JSON.stringify({
...
describe("/token-exchange", () => {
	test("should exchange auth code for access token, refresh token, id token", async () => {
		...
		const { res, sessionId } = await sendTokenExchangeRequest(db, {});
		...
		const sessionStore = await mockedKV.get(
	...
	test("should not allow the same session to be exchanged twice", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
	...
	test("should throw error if no session found", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;

		const { res } = await sendTokenExchangeRequest(db, { sessionId: "abc" });
