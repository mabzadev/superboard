...
describe("post /email-mfa-code", () => {
	test("should post email mfa code", async () => {
		...
		const consentRes = await app.request(
			routeConfig.EmbeddedRoute.EmailMfaCode.replace(":sessionId", sessionId),
			{ method: "POST" },
			mock(db),
		);
	...
	test("should throw error with invalid session id", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.EMAIL_MFA_IS_REQUIRED = true as unknown as string;
		...
		const consentRes = await app.request(
			routeConfig.EmbeddedRoute.EmailMfaCode.replace(":sessionId", "abc"),
			{ method: "POST" },
			mock(db),
		);
...
describe("post /process-email-mfa", () => {
	test("should process email mfa", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.EMAIL_MFA_IS_REQUIRED = true as unknown as string;
		...
		const processRes = await app.request(
			routeConfig.EmbeddedRoute.EmailMfa.replace(":sessionId", sessionId),
			{
				method: "POST",
	...
	test("should throw error with invalid session id", async () => {
		...
		const processRes = await app.request(
			routeConfig.EmbeddedRoute.EmailMfa.replace(":sessionId", "abc"),
			{
				method: "POST",
				body: JSON.stringify({ mfaCode }),
			},
	...
	test("should throw error if wrong mfa code is provided", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.EMAIL_MFA_IS_REQUIRED = true as unknown as string;
