...
import { insertUsers, getApp, postAuthorizeBody } from "tests/identity";
import { migrate, mockedKV, mock, fetchMock } from "tests/mock";
...
describe("post /authorize-passwordless", () => {
	test("should redirect to passwordless verify page", async () => {
	...
	test("should create a new user if user does not exist", async () => {
		...
		const user = (await db
			...
			.get("test@email.com")) as userModel.Raw;
	...
	test("should create a new user for an org", async () => {
		process.env.ENABLE_PASSWORDLESS_SIGN_IN = true as unknown as string;
		...
		await db
			.prepare(
				'INSERT INTO "org" (name, slug, "allowPublicRegistration", "onlyUseForBrandingOverride") VALUES (?, ?, ?, ?)',
			)
			.run("test", "test", 1, 0);
	...
	test("should throw error if scope is not provided", async () => {
		process.env.ENABLE_PASSWORDLESS_SIGN_IN = true as unknown as string;
		...
		expect(json[0].constraints).toStrictEqual({
			arrayMinSize: "scopes must contain at least 1 elements",
		});

		process.env.ENABLE_PASSWORDLESS_SIGN_IN = false as unknown as string;
	});

	test("should throw error if user is disabled", async () => {
		process.env.ENABLE_PASSWORDLESS_SIGN_IN = true as unknown as string;
	...
	test("should throw error if feature is disabled", async () => {
		const appRecord = await getApp(db);
		await insertUsers(db);
...
describe("post /send-passwordless-code", () => {
	test("should send passwordless code", async () => {
		process.env.ENABLE_PASSWORDLESS_SIGN_IN = true as unknown as string;
		...
		expect(
			((await mockedKV.get(`${adapterConfig.BaseKVKey.PasswordlessCode}-${body.code}`)) ?? "")
				.length,
		).toBe(6);
	...
	test("should send magic link email when USE_PASSWORDLESS_AS_MAGIC_LINK is true", async () => {
		process.env.ENABLE_PASSWORDLESS_SIGN_IN = true as unknown as string;
	...
	test("should throw error if email mfa is locked", async () => {
		process.env.ENABLE_PASSWORDLESS_SIGN_IN = true as unknown as string;
		...
		await mockedKV.put(`${adapterConfig.BaseKVKey.EmailMfaEmailAttempts}-1`, "1");
...
describe("post /process-passwordless-code", () => {
	...
	test("could process magic link code", async () => {
		process.env.ENABLE_PASSWORDLESS_SIGN_IN = true as unknown as string;
		process.env.USE_PASSWORDLESS_AS_MAGIC_LINK = true as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
	...
	test("should throw error if mfa code is wrong", async () => {
		process.env.ENABLE_PASSWORDLESS_SIGN_IN = true as unknown as string;
