...
import { fetchMock, migrate, mock, mockedKV } from "tests/mock";
...
import { AuthCodeBody } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/configs/type";
...
describe("get /authorize-account", () => {
	test("should get empty user attributes", async () => {
	...
	test("should get user attributes", async () => {
		...
		await db.exec(
			'insert into "user_attribute" (name, "includeInSignUpForm", "requiredInSignUpForm", "includeInIdTokenBody", "includeInUserInfo", "unique") values (\'test\', 1, 1, 1, 1, 1)',
		);
		await db.exec(
			'insert into "user_attribute" (name, "includeInSignUpForm", "requiredInSignUpForm", "includeInIdTokenBody", "includeInUserInfo", "unique") values (\'test2\', 0, 0, 0, 0, 0)',
		);
		await db.exec(
			'insert into "user_attribute" (name, locales, "includeInSignUpForm", "requiredInSignUpForm", "includeInIdTokenBody", "includeInUserInfo", "unique") values (\'test3\', \'{"en": "test3 en", "fr": "test3 fr"}\', 1, 0, 0, 0, 0)',
		);
	...
	test("should get user attributes with regex validation", async () => {
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should return empty if feature is disabled", async () => {
		await db.exec(
			'insert into "user_attribute" (name, "includeInSignUpForm", "requiredInSignUpForm", "includeInIdTokenBody", "includeInUserInfo") values (\'test\', 1, 1, 1, 1)',
		);
...
describe("post /authorize-account", () => {
	test("should get auth code after sign up", async () => {
		...
		const codeStore = JSON.parse(
			(await mockedKV.get(`${adapterConfig.BaseKVKey.AuthCode}-${code}`)) ?? "",
		) as AuthCodeBody;
		...
		const verificationCode =
			(await mockedKV.get(`${adapterConfig.BaseKVKey.EmailVerificationCode}-1`)) ?? "";
		...
		expect(body).toContain(
			`${routeConfig.IdentityRoute.VerifyEmailView}?id=${codeStore.user.authId}&amp;locale=en`,
		);
	...
	test("filters scopes that are not assigned to the app", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		...
		const codeStore = JSON.parse(
			(await mockedKV.get(`${adapterConfig.BaseKVKey.AuthCode}-${json.code}`)) ?? "",
		) as AuthCodeBody;
	...
	test("could override verification email with branding config", async () => {
	...
	test("could override verification email with welcome email", async () => {
	...
	test("should store org slug after sign up", async () => {
		...
		db.exec(
			'insert into "org" (name, slug, "companyEmailLogoUrl", "allowPublicRegistration", "onlyUseForBrandingOverride") values (\'test\', \'default\', \'https://test_logo.com\', 1, 0)',
		);
		...
		const currentUserOrg = (await db
			.prepare('select * from "user_org" where "userId" = 1')
			.get()) as userOrgModel.Record;
		expect(currentUserOrg.orgId).toBe(1);

		process.env.ENABLE_ORG = false as unknown as string;
	});

	test("should not store org slug after sign up if allowPublicRegistration is false", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		...
		db.exec(
			"insert into \"org\" (name, slug, \"companyEmailLogoUrl\", \"allowPublicRegistration\") values ('test', 'default', 'https://test_logo.com', 0)",
		);
	...
	test("should not store org slug after sign up if allowPublicRegistration is true and onlyUseForBrandingOverride is true", async () => {
		process.env.ENABLE_ORG = true as unknown as string;
		...
		db.exec(
			'insert into "org" (name, slug, "companyEmailLogoUrl", "allowPublicRegistration", "onlyUseForBrandingOverride") values (\'test\', \'default\', \'https://test_logo.com\', 1, 1)',
		);
	...
	test("could disable email verification", async () => {
	...
	test("could force otp mfa", async () => {
	...
	test("could force otp mfa use app level config", async () => {
		global.process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;

		db.prepare(
			'update app set "useSystemMfaConfig" = 0, "requireEmailMfa" = 0, "requireOtpMfa" = 1, "requireSmsMfa" = 0, "allowEmailMfaAsBackup" = 0',
		).run();
	...
	test("could disable otp mfa use app level config", async () => {
		global.process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		global.process.env.OTP_MFA_IS_REQUIRED = true as unknown as string;

		db.prepare(
			'update app set "useSystemMfaConfig" = 0, "requireEmailMfa" = 0, "requireOtpMfa" = 0, "requireSmsMfa" = 0, "allowEmailMfaAsBackup" = 0',
		).run();
	...
	test("could force email mfa", async () => {
	...
	test("could force email mfa use app level config", async () => {
		global.process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;

		db.prepare(
			'update app set "useSystemMfaConfig" = 0, "requireEmailMfa" = 1, "requireOtpMfa" = 0, "requireSmsMfa" = 0, "allowEmailMfaAsBackup" = 0',
		).run();
	...
	test("could disable email mfa use app level config", async () => {
		global.process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		global.process.env.EMAIL_MFA_IS_REQUIRED = true as unknown as string;
	...
	test("could force sms mfa", async () => {
	...
	test("could force sms mfa use app level config", async () => {
		global.process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		global.process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;

		db.prepare(
			'update app set "useSystemMfaConfig" = 0, "requireEmailMfa" = 0, "requireOtpMfa" = 0, "requireSmsMfa" = 1, "allowEmailMfaAsBackup" = 0',
		).run();
	...
	test("could disable sms mfa use app level config", async () => {
		global.process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		global.process.env.SMS_MFA_IS_REQUIRED = true as unknown as string;
	...
	test("could set names as required", async () => {
	...
	test("should success if name provided when required", async () => {
		global.process.env.NAMES_IS_REQUIRED = true as unknown as string;
	...
	test("could sign up with names using non binary strings", async () => {
		global.process.env.NAMES_IS_REQUIRED = true as unknown as string;
	...
	test("should throw error if not enable sign up", async () => {
	...
	test("should throw error if passwordless sign in is enabled", async () => {
...
describe("post /authorize-account with user attribute", () => {
	test("should store attribute values after sign up", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;

		db.exec(
			'insert into "user_attribute" (name, "includeInSignUpForm", "requiredInSignUpForm") values (\'test1\', 1, 1)',
		);
		db.exec(
			'insert into "user_attribute" (name, "includeInSignUpForm", "requiredInSignUpForm") values (\'test2\', 1, 0)',
		);
		...
		const attributeValues = (await db
			.prepare('select * from "user_attribute_value" where "userId" = 1')
			.all()) as userAttributeValueModel.Record[];
	...
	test("could skip unrequired attribute values", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("could throw error if missing required attribute values", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should skip if feature not enabled", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
	...
	test("should successfully create user with unique attribute value", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should fail to create user with duplicate unique attribute value", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
		...
		const attributeValues = (await db
			.prepare('select * from "user_attribute_value"')
			.all()) as userAttributeValueModel.Record[];
		expect(attributeValues.length).toBe(1);
	...
	test("should allow multiple users with same non-unique attribute value", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should handle mixed unique and non-unique attributes correctly", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should successfully create user with attribute value matching regex pattern", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should fail to create user with attribute value not matching regex pattern", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
		...
		expect(await res.text()).toBe(
			'Value for attribute "secondary_email" does not match the validation rule',
		);
	...
	test("should handle mixed regex and non-regex attributes correctly", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
	...
	test("should fail when regex attribute fails but non-regex attribute passes", async () => {
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.ENABLE_USER_ATTRIBUTE = true as unknown as string;
