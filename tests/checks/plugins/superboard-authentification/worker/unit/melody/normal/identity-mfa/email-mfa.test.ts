...
import { prepareFollowUpBody, insertUsers, getApp } from "tests/identity";
import { fetchMock, migrate, mock, mockedKV } from "tests/mock";
import { enrollEmailMfa, enrollOtpMfa, enrollSmsMfa } from "tests/util";
...
describe("post /send-email-mfa", () => {
	test("should send email mfa code", async () => {
	...
	test("could protect against threshold", async () => {
		...
		const getEmailMfaEmailAttemptsByIPSpy = vi.spyOn(kvService, "getEmailMfaEmailAttemptsByIP");
		const setEmailMfaEmailAttemptsSpy = vi.spyOn(kvService, "setEmailMfaEmailAttempts");
...
describe("post /process-email-mfa", () => {
	test("could use original code", async () => {
		const mockFetch = vi.fn(async () => {
			return Promise.resolve({ ok: true });
		});
		global.fetch = mockFetch as Mock;
		...
		const mfaCode = await mockedKV.get(
			`${adapterConfig.BaseKVKey.EmailMfaCode}-${requestBody.code}`,
		);
		expect(mfaCode?.length).toBe(6);
		expect(mockFetch).toBeCalledTimes(1);
	...
	test("should not track failed attempts when MFA_CODE_VERIFY_THRESHOLD is 0", async () => {
		...
		const getSpy = vi.spyOn(kvService, "getFailedMfaCodeAttemptsByIP");
		const setSpy = vi.spyOn(kvService, "setFailedMfaCodeAttempts");
	...
	test("should throw error if not enrolled with email mfa and fallback is not allowed", async () => {
	...
	test("could use as otp mfa fallback", async () => {
		const mockFetch = vi.fn(async () => {
			return Promise.resolve({ ok: true });
		});
		global.fetch = mockFetch as Mock;
	...
	test("could use as sms mfa fallback", async () => {
		const mockFetch = vi.fn(async () => {
			return Promise.resolve({ ok: true });
		});
		global.fetch = mockFetch as Mock;
	...
	test("could use as fallback for both sms and otp at same time", async () => {
		const mockFetch = vi.fn(async () => {
			return Promise.resolve({ ok: true });
		});
		global.fetch = mockFetch as Mock;
	...
	test("could use as fallback for both sms and otp setup at same time", async () => {
		const mockFetch = vi.fn(async () => {
			return Promise.resolve({ ok: true });
		});
		global.fetch = mockFetch as Mock;
	...
	test("should bypass email mfa on subsequent login when device is remembered", async () => {
		process.env.ENABLE_MFA_REMEMBER_DEVICE = true as unknown as string;
	...
	test("should not bypass email mfa when invalid remember device cookie is provided", async () => {
		process.env.ENABLE_MFA_REMEMBER_DEVICE = true as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
