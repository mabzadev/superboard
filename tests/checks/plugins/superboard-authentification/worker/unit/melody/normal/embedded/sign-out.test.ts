...
describe("/token-exchange", () => {
	test("should exchange auth code for access token, refresh token, id token", async () => {
		...
		const { res } = await sendTokenExchangeRequest(db, {});
	...
	test("should throw error if wrong client id provided", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
	...
	test("should throw error if wrong refresh token provided", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
