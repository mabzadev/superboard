...
import { samlIdpModel } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/models";
...
describe("get all", () => {
	test("should return all idps", async () => {
	...
	test("should return 400 if saml not enabled in config", async () => {
		const res = await app.request(
			BaseRoute,
			{ headers: { Authorization: `Bearer ${await getS2sToken(db)}` } },
			mock(db),
...
describe("get by id", () => {
	test("should return idp by id", async () => {
		global.process.env.ENABLE_SAML_SSO_AS_SP = true as unknown as string;
	...
	test("should return 404 when can not find idp by id", async () => {
		global.process.env.ENABLE_SAML_SSO_AS_SP = true as unknown as string;
...
describe("create", () => {
	...
	test("should create idp with nullable values", async () => {
		global.process.env.ENABLE_SAML_SSO_AS_SP = true as unknown as string;
	...
	test("should trigger unique constraint on name", async () => {
		...
		await createNewIdp();
		const res1 = await createNewIdp(undefined, { name: "test name" });
	...
	test("should return 401 without proper scope", async () => {
		...
		await attachIndividualScopes(db);
		const res = await createNewIdp(await getS2sToken(db, Scope.ReadUser));
		expect(res.status).toBe(401);
...
describe("update", () => {
	...
	test("could update idp with nullable values", async () => {
		global.process.env.ENABLE_SAML_SSO_AS_SP = true as unknown as string;
	...
	test("should throw error if idp not found", async () => {
		global.process.env.ENABLE_SAML_SSO_AS_SP = true as unknown as string;
