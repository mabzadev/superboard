...
import { cryptoUtil } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/utils";
...
const sendVerifiedSignInRequest = async (
	...
	await db.prepare('UPDATE "user" SET "otpSecret" = ? WHERE id = ?').run(otpSecret, 1);
	...
	const res = await app.request(
		routeConfig.EmbeddedRoute.SignIn.replace(":sessionId", sessionId),
		{
			method: "POST",
			body: JSON.stringify({
...
describe("get /otp-mfa-setup", () => {
	test("should get otp mfa setup", async () => {
		...
		const { sessionId, otpSecret } = await sendVerifiedSignInRequest(db, {});

		const otpRes = await app.request(
			routeConfig.EmbeddedRoute.OtpMfaSetup.replace(":sessionId", sessionId),
			{ method: "GET" },
			mock(db),
		);
	...
	test("should throw error if otp mfa is already set", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.OTP_MFA_IS_REQUIRED = true as unknown as string;

		const { sessionId } = await sendVerifiedSignInRequest(db, { markAsVerified: true });
	...
	test("should throw error if session id is invalid", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.OTP_MFA_IS_REQUIRED = true as unknown as string;
		...
		const otpRes = await app.request(
			routeConfig.EmbeddedRoute.OtpMfaSetup.replace(":sessionId", "123456"),
			{ method: "GET" },
			mock(db),
		);
	...
	test("could generate secret if not set", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.OTP_MFA_IS_REQUIRED = true as unknown as string;

		const { sessionId } = await sendVerifiedSignInRequest(db, { genSecret: false });
		...
		const { otpUri, otpSecret } = (await otpRes.json()) as { otpUri: string; otpSecret: string };
		expect(otpSecret.length).toBe(32);
		expect(otpUri).toBe(
			`otpauth://totp/Admin Panel (SPA):test@email.com?secret=${otpSecret}&issuer=melody-auth&algorithm=SHA1&digits=6&period=30`,
		);

		const user = (await db.prepare('select * from "user" where id = ?').get(1)) as userModel.Raw;
		expect(user.otpSecret).toBe(otpSecret);

		const sessionBody = await mockedKV.get(
			`${adapterConfig.BaseKVKey.EmbeddedSession}-${sessionId}`,
		);
		expect((JSON.parse(sessionBody!) as typeConfig.EmbeddedSessionBody).user?.otpSecret).toBe(
			otpSecret,
		);
...
describe("get /otp-mfa", () => {
	test("should get otp mfa config", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.OTP_MFA_IS_REQUIRED = true as unknown as string;

		const { sessionId } = await sendVerifiedSignInRequest(db, {});

		const otpRes = await app.request(
			routeConfig.EmbeddedRoute.OtpMfa.replace(":sessionId", sessionId),
			{ method: "GET" },
			mock(db),
		);
		expect(otpRes.status).toBe(200);
	...
	test("should get false if otp mfa is not enrolled", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
	...
	test("should get false if user enrolled email mfa", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
	...
	test("should get false if fallback is not allowed", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.OTP_MFA_IS_REQUIRED = true as unknown as string;
...
describe("post /otp-mfa", () => {
	test("should verify otp mfa", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.OTP_MFA_IS_REQUIRED = true as unknown as string;
	...
	test("should throw error if mfa code is invalid", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.OTP_MFA_IS_REQUIRED = true as unknown as string;
	...
	test("should throw error if user has no otp secret", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.OTP_MFA_IS_REQUIRED = true as unknown as string;
	...
	test("should bypass otp mfa on subsequent login when device is remembered", async () => {
		...
		const secondSignInRes = await app.request(
			routeConfig.EmbeddedRoute.SignIn.replace(":sessionId", newSessionId),
			{
				method: "POST",
				body: JSON.stringify({
