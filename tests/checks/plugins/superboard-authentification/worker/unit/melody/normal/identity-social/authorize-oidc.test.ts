...
import { GetAuthorizeOidcConfigsRes } from "../../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/handlers/identity/social";
...
describe("get /authorize-oidc-configs", () => {
	test("could get provider configs", async () => {
		...
		expect(
			await mockedKV.get(`${adapterConfig.BaseKVKey.OidcCodeVerifier}-${json.codeVerifier}`),
		).toBe("1");
		process.env.OIDC_AUTH_PROVIDERS = undefined as unknown as string;
	});

	test("should throw error if no provider configs", async () => {
...
describe("get /authorize-oidc", () => {
	const prepareRequest = async ({
		...
		const mockOidcFetch = vi.fn(async (url, params) => {
			if (
				url === variableConfig.OIDCProviderConfigs.Auth0.tokenEndpoint &&
				params.body.includes("code=aaa")
			) {
		...
		if (!clearCodeVerifier) {
			await mockedKV.put(`${adapterConfig.BaseKVKey.OidcCodeVerifier}-abcd`, "1");
		}
	...
	test("should redirect to consent with a new OIDC account", async () => {
	...
	test("should redirect back to app with a new OIDC account when consent not need", async () => {
		global.process.env.OIDC_AUTH_PROVIDERS = ["Auth0"] as unknown as string;
	...
	test("could throw error if issuer does not match provider config", async () => {
		global.process.env.OIDC_AUTH_PROVIDERS = ["Auth0"] as unknown as string;
	...
	test("could throw error if no code_verifier exists in kv", async () => {
		global.process.env.OIDC_AUTH_PROVIDERS = ["Auth0"] as unknown as string;
	...
	test("could throw error if no code_verifier provided", async () => {
		global.process.env.OIDC_AUTH_PROVIDERS = ["Auth0"] as unknown as string;
	...
	test("should sign in with an existing oidc account", async () => {
		global.process.env.OIDC_AUTH_PROVIDERS = ["Auth0"] as unknown as string;
		const res = await getOidcRequest("Auth0");
		expect(res.headers.get("Location")).toContain(
			`${routeConfig.IdentityRoute.ProcessView}?state=123&code=`,
