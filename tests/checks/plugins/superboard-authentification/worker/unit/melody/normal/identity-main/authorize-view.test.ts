...
import { migrate, mockedKV } from "tests/mock";
...
describe("get /authorize-view", () => {
	test("should show authorize view page", async () => {
		const appRecord = await getApp(db);
		const res = await getSignInRequest(db, routeConfig.IdentityRoute.AuthorizeView, appRecord);
		...
		expect(html).toContain(
			`usePasswordlessAsMagicLink: ${process.env.USE_PASSWORDLESS_AS_MAGIC_LINK ?? false}`,
		);
	...
	test("should load different source under dev environment", async () => {
	...
	test("should override variables using config", async () => {
		...
		global.process.env.OIDC_AUTH_PROVIDERS = [
			"oidc-provider-1",
			"oidc-provider-2",
	...
	test("social client id should be empty string if not set", async () => {
	...
	test("set passwordless sign in to true should override other features", async () => {
	...
	test("should include usePasswordlessAsMagicLink when set to true", async () => {
		process.env.ENABLE_PASSWORDLESS_SIGN_IN = true as unknown as string;
	...
	test("set passwordless sign in to true should override password signin for policies", async () => {
	...
	test("should not override variables using org config if allowPublicRegistration is false", async () => {
		global.process.env.ENABLE_ORG = true as unknown as string;
