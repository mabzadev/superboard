...
describe("post /sms-mfa-setup", () => {
	test("should post sms mfa setup", async () => {
		...
		const { sessionId } = await sendVerifiedSignInRequest(db, { insertPhoneNumber: false });

		const otpSetupRes = await app.request(
			routeConfig.EmbeddedRoute.SmsMfaSetup.replace(":sessionId", sessionId),
			{
				method: "POST",
				body: JSON.stringify({ phoneNumber: "+16471231234" }),
			},
			mock(db),
		);
		...
		const otpRes = await app.request(
			routeConfig.EmbeddedRoute.SmsMfa.replace(":sessionId", sessionId),
			{
				method: "POST",
	...
	test("should throw error if phone number is already enrolled", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.SMS_MFA_IS_REQUIRED = true as unknown as string;
	...
	test("should throw error if session id is invalid", async () => {
		...
		const otpSetupRes = await app.request(
			routeConfig.EmbeddedRoute.SmsMfaSetup.replace(":sessionId", "invalid-session-id"),
			{
				method: "POST",
				body: JSON.stringify({ phoneNumber: "+16471231234" }),
			},
...
describe("get /sms-mfa", () => {
	test("should get sms mfa config", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.SMS_MFA_IS_REQUIRED = true as unknown as string;
	...
	test("should throw error if session id is invalid", async () => {
		...
		const otpRes = await app.request(
			routeConfig.EmbeddedRoute.SmsMfa.replace(":sessionId", "invalid-session-id"),
			{ method: "GET" },
			mock(db),
		);
	...
	test("should return fallback as false if not enabled", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.SMS_MFA_IS_REQUIRED = true as unknown as string;
	...
	test("should return fallback as false if email mfa is required", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.SMS_MFA_IS_REQUIRED = true as unknown as string;
...
describe("post /sms-mfa", () => {
	test("should post sms mfa config", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.SMS_MFA_IS_REQUIRED = true as unknown as string;
	...
	test("should set remember device cookie when rememberDevice is true", async () => {
		...
		const mfaCode =
			(await mockedKV.get(`${adapterConfig.BaseKVKey.SmsMfaCode}-${sessionId}`)) ?? "";
		expect(mfaCode.length).toBe(6);
...
describe("post /sms-mfa-code", () => {
	test("should post sms mfa code", async () => {
		process.env.EMBEDDED_AUTH_ORIGINS = ["http://localhost:3000"] as unknown as string;
		process.env.ENFORCE_ONE_MFA_ENROLLMENT = [] as unknown as string;
		process.env.ENABLE_USER_APP_CONSENT = false as unknown as string;
		process.env.SMS_MFA_IS_REQUIRED = true as unknown as string;
		...
		const otpRes = await app.request(
			routeConfig.EmbeddedRoute.SmsMfaCode.replace(":sessionId", sessionId),
			{ method: "POST" },
			mock(db),
		);
	...
	test("should throw error if session id is invalid", async () => {
		...
		const otpRes = await app.request(
			routeConfig.EmbeddedRoute.SmsMfaCode.replace(":sessionId", "invalid-session-id"),
			{ method: "POST" },
			mock(db),
		);
