...
import { attachIndividualScopes, getS2sToken } from "tests/util";
...
describe("get system info", () => {
	test("should require an s2s access token", async () => {
		const res = await app.request(`${BaseRoute}/info`, {}, mock(db));

		expect(res.status).toBe(401);
	});

	test("should return system info regardless of which s2s scope the token carries", async () => {
		...
		for (const scope of [Scope.ReadUser, Scope.WriteUser]) {
			const res = await app.request(
				...
				{
					headers: {
						Authorization: `Bearer ${await getS2sToken(db, scope)}`,
					},
				},
				mock(db),
			);
...
describe("get openid config", () => {
	test("should return openid config", async () => {
		const res = await app.request(`${BaseRoute}/.well-known/openid-configuration`, {}, mock(db));
		const json = (await res.json()) as { configs: object };
...
describe("get jwks", () => {
	test("should return jwks", async () => {
		const res = await app.request(`${BaseRoute}/.well-known/jwks.json`, {}, mock(db));
	...
	test("should throw error if no jwt public secret", async () => {
...
describe("get swagger", () => {
	test("should return swagger", async () => {
		const res = await app.request(`${BaseRoute}/api/v1/swagger`, {}, mock(db));
