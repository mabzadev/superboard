...
describe("post /change-password", () => {
	test("should change password", async () => {
		...
		const reLoginRes = await postSignInRequest(db, appRecord, { password: "Password2!" });
	...
	test("should throw error if use same password", async () => {
		await insertUsers(db, false);

		const body = await prepareFollowUpBody(db, Policy.ChangePassword);
		await markAuthCodeAsSecured(body.code);
		const res = await app.request(
			routeConfig.IdentityRoute.ChangePassword,
			{
	...
	test("should throw error if user does not have password", async () => {
		await insertUsers(db, false);
