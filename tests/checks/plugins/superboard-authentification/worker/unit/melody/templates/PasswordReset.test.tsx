...
import PasswordReset from "../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/templates/PasswordReset";
...
describe("PasswordReset", () => {
	...
	it("renders description and reset code", () => {
		...
		expect(paragraph?.textContent).toContain(
			localeConfig.passwordResetEmail.desc[mockProps.locale].replace(
				"{{expiresIn}}",
	...
	it("renders correctly with a different locale", () => {
		...
		const paragraph = document.querySelector("p");
		expect(paragraph?.textContent).toContain(
			localeConfig.passwordResetEmail.desc[propsFr.locale].replace(
				"{{expiresIn}}",
				String(variableConfig.systemConfig.passwordResetCodeExpiresIn / 3600),
			),
		);
