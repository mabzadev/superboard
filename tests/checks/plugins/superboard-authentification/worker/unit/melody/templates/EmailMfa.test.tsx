...
import EmailMfa from "../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/templates/EmailMfa";
...
describe("EmailMfa", () => {
	...
	it("renders description and MFA code", () => {
		...
		expect(paragraph?.textContent).toContain(
			localeConfig.emailMfaEmail.desc[mockProps.locale].replace(
				"{{expiresIn}}",
	...
	it("renders correctly with a different locale", () => {
		...
		expect(paragraph).toBeTruthy();
		expect(paragraph?.textContent).toContain(
			localeConfig.emailMfaEmail.desc[propsFr.locale].replace(
				"{{expiresIn}}",
				String(variableConfig.systemConfig.emailMfaCodeExpiresIn / 60),
			),
		);
