...
import EmailVerification from "../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/templates/EmailVerification";
import { requestUtil } from "../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/utils";
...
describe("EmailVerification", () => {
	...
	it("renders description and verification code", () => {
		...
		expect(paragraph?.textContent).toContain(
			localeConfig.emailVerificationEmail.desc[mockProps.locale],
		);
	...
	it("renders verification link correctly", () => {
		...
		expect(link?.textContent?.trim()).toBe(
			localeConfig.emailVerificationEmail.verify[mockProps.locale],
		);
	});
	...
	it("renders expiry text", () => {
		...
		const expiryParagraph = Array.from(document.querySelectorAll("p")).find((p) => {
			return (
				p.textContent?.trim() ===
				localeConfig.emailVerificationEmail.expiry[mockProps.locale].replace(
					"{{expiresIn}}",
	...
	it("renders correctly with a different locale", () => {
		...
		expect(paragraph).toBeTruthy();
		expect(paragraph?.textContent).toContain(
			localeConfig.emailVerificationEmail.desc[propsFr.locale],
		);
		const span = paragraph?.querySelector("span");
		...
		expect(actualHref).toBe(expectedHref);
		expect(link?.textContent?.trim()).toBe(
			localeConfig.emailVerificationEmail.verify[propsFr.locale],
		);
		...
		const expiryParagraph = Array.from(document.querySelectorAll("p")).find((p) => {
			return (
				p.textContent?.trim() ===
				localeConfig.emailVerificationEmail.expiry[propsFr.locale].replace(
					"{{expiresIn}}",
					String(variableConfig.systemConfig.emailVerificationCodeExpiresIn / 3600),
				)
			);
