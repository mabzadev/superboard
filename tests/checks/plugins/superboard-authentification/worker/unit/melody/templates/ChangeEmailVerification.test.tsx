...
import ChangeEmailVerification from "../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/templates/ChangeEmailVerification";
...
describe("ChangeEmailVerification", () => {
	it("renders the component with correct title", () => {
		...
		expect(header?.textContent).toBe(
			localeConfig.changeEmailVerificationEmail.title[mockProps.locale],
		);
	});
	...
	it("renders description and verification code", () => {
		...
		expect(paragraph?.textContent).toContain(
			localeConfig.changeEmailVerificationEmail.desc[mockProps.locale].replace(
				"{{expiresIn}}",
	...
	it("renders correctly with a different locale", () => {
		...
		const header = document.querySelector("h1");
		expect(header?.textContent).toBe(
			localeConfig.changeEmailVerificationEmail.title[propsFr.locale],
		);
		...
		expect(paragraph).toBeTruthy();
		expect(paragraph?.textContent).toContain(
			localeConfig.changeEmailVerificationEmail.desc[propsFr.locale].replace(
				"{{expiresIn}}",
				String(variableConfig.systemConfig.changeEmailVerificationCodeExpiresIn / 3600),
			),
		);
