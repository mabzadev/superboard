...
} from "../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/templates";
import {
	bcryptCompare,
	bcryptText,
	redactMessageBody,
} from "../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/utils/crypto";
...
describe("bcrypt helpers", () => {
	test("should generate password hashes with cost 12", () => {
		const hash = bcryptText("Password1!");

		expect(hash.split("$")[2]).toBe("12");
		expect(bcryptCompare("Password1!", hash)).toBe(true);
	});

	test("should compare legacy cost 10 password hashes", () => {
		const legacyCostTenHash = "$2a$10$/KBURsxnGm5OClLQFPNwZ.HcaM27Gogp/mA1xRxfEbY3bs0cuu8Fm";

		expect(bcryptCompare("Password1!", legacyCostTenHash)).toBe(true);
	});
});

describe("redactMessageBody", () => {
	test("should redact standalone six digit codes", () => {
		const result = redactMessageBody("<span>123456</span>");

		expect(result).toBe("<span>[REDACTED_CODE]</span>");
	});

	test("should redact six digit magic link otp params", () => {
		const body = [
