...
import { dbTime, getS2sToken, superAdminRole } from "tests/util";
...
import {
	cryptoUtil,
	timeUtil,
} from "../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/utils";
...
describe("get jwks", () => {
	test("should return jwks", async () => {
		const res = await app.request("/.well-known/jwks.json", {}, mock(db));
...
const createNewRole = async (token?: string) =>
	await app.request(
...
describe("get all", () => {
	...
	test("should use deprecated key", async () => {
		await createNewRole();
