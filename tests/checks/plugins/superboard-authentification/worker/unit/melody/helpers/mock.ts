...
import { adapterConfig } from "../../../../../../../../packages/plugins/superboard-authentification/worker/src/melody/configs";
import {
	userModel,
	appModel,
...
const convertQuery = (query: string, params: string[]) => {
	let prepareQuery = query;
	for (let i = 0; i < params.length; i++) {
		prepareQuery = prepareQuery.replace(`$${i + 1}`, "?");
	}
	return prepareQuery;
};
	...
	get: async (key: string) => {
		switch (key) {
			case adapterConfig.BaseKVKey.JwtPublicSecret:
				return fs.readFileSync(path.resolve(adapterConfig.FileLocation.NodePublicKey), "utf8");
			case adapterConfig.BaseKVKey.JwtPrivateSecret:
				return fs.readFileSync(path.resolve(adapterConfig.FileLocation.NodePrivateKey), "utf8");
			...
			case adapterConfig.BaseKVKey.SamlSpCert:
				return fs.readFileSync(path.resolve(adapterConfig.FileLocation.NodeSamlSpCert), "utf8");
			case adapterConfig.BaseKVKey.SamlSpKey:
				return fs.readFileSync(path.resolve(adapterConfig.FileLocation.NodeSamlSpKey), "utf8");
	...
	({
		prepare: (query: string) => {
			return {
				bind: (...params: string[]) => ({
					all: async () => {
						const prepareQuery = convertQuery(query, params);
...
export const migrate = async () => {
	if (isTestingNode) {
		...
		const getRows = (result: any, query: string) => {
		...
		const getRow = (record: any, query: string) => {
			let row = record;
			if (query.includes(' "user" ')) {
		...
		dbAdapter
			.prepare(
				'update app set "useSystemMfaConfig" = 1, "requireEmailMfa" = 0, "requireOtpMfa" = 0, "requireSmsMfa" = 0, "allowEmailMfaAsBackup" = 0',
			)
			.run();
...
export const getSmsResponseMock = () =>
	vi.fn(async () => {
		return Promise.resolve({
			ok: true,
			text: () => "test response",
...
export const fetchMock = vi.fn(async (url) => {
	if (url === "https://www.googleapis.com/oauth2/v3/certs") {
		const key = fs.readFileSync(path.resolve(adapterConfig.FileLocation.NodePublicKey), "utf8");
...
export const passkeyEnrollMock = {
	...
	response: {
		attestationObject:
			"o2NmbXRkbm9uZWdhdHRTdG10oGhhdXRoRGF0YVikSZYN5YgOjGh0NBcPZHZgW4_krrmihjLHmVzzuoMdl2NFAAAAAAAAAAAAAAAAAAAAAAAAAAAAIIflIUGwkvu6K7Ip6vEXRm7fFM0vQfy_1CKSg1sqAeUVpQECAyYgASFYII5eFDvbkOZhbFAkOoii666ptszT4ranzvul1NIifUjuIlggsUcAFk2c7Y6yEhcnqKdsCsMLz2cLwZgZeWjI0uFgdts",
		authenticatorData:
			"SZYN5YgOjGh0NBcPZHZgW4_krrmihjLHmVzzuoMdl2NFAAAAAAAAAAAAAAAAAAAAAAAAAAAAIIflIUGwkvu6K7Ip6vEXRm7fFM0vQfy_1CKSg1sqAeUVpQECAyYgASFYII5eFDvbkOZhbFAkOoii666ptszT4ranzvul1NIifUjuIlggsUcAFk2c7Y6yEhcnqKdsCsMLz2cLwZgZeWjI0uFgdts",
		clientDataJSON:
			"eyJ0eXBlIjoid2ViYXV0aG4uY3JlYXRlIiwiY2hhbGxlbmdlIjoiR3UwOUhueFRzYzAxc213YUN0QzZ5SEUwTUVnX2QtcUtVU3BLaTVCYkxnVSIsIm9yaWdpbiI6Imh0dHA6Ly9sb2NhbGhvc3Q6ODc4NyIsImNyb3NzT3JpZ2luIjpmYWxzZX0",
		publicKey:
			"MFkwEwYHKoZIzj0CAQYIKoZIzj0DAQcDQgAEjl4UO9uQ5mFsUCQ6iKLrrqm2zNPitqfO-6XU0iJ9SO6xRwAWTZztjrISFyeop2wKwwvPZwvBmBl5aMjS4WB22w",
...
export const passkeyVerifyMock = {
	...
	rawId: "h-UhQbCS-7orsinq8RdGbt8UzS9B_L_UIpKDWyoB5RU",
	response: {
		authenticatorData: "SZYN5YgOjGh0NBcPZHZgW4_krrmihjLHmVzzuoMdl2MFAAAAAQ", // gitleaks:allow -- public WebAuthn test vector
		clientDataJSON:
			"eyJ0eXBlIjoid2ViYXV0aG4uZ2V0IiwiY2hhbGxlbmdlIjoiaEo5NUo1VGM1MmhrSmxXYVdkQlhxUFVobkxHa0dSM05xa24yVndQakFYYyIsIm9yaWdpbiI6Imh0dHA6Ly9sb2NhbGhvc3Q6ODc4NyIsImNyb3NzT3JpZ2luIjpmYWxzZX0",
		signature:
			"MEQCIGoIJgDiEA5W3umY_csiGJbFsPnPh7orU0OZER_8xCanAiA4VXNHLjggdaxm4J4bRsMHqcTWI3L2GXxKLRAqM74trw",
		userHandle: "MTE",
	},
	type: "public-key",
};
