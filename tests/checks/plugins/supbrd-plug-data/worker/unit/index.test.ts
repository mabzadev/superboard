...
import app, {
	...
	type FilesEnv,
} from "../../../../../../packages/plugins/supbrd-plug-data/worker/src/index";
...
describe("SuperBoard Files Worker", () => {
	...
	it("issues owner-scoped short-lived download tickets for internal processors", async () => {
		...
		expect(new Date(body.download.expires_at).valueOf()).toBeGreaterThan(Date.now());
	});
	...
	it("signs tickets with the active key and verifies an overlap key without exposing owners", async () => {
		...
		await expect(
			verifyDownloadTicket(ticket, ["active-signing-key", "previous-signing-key"], 600, now),
	...
	it("enforces the application-specific exact and wildcard MIME policy", () => {
		...
		expect(allowedContentTypes(env)).toEqual(["application/pdf", "audio/*", "image/png"]);
		expect(() => assertAllowedContentType(env, "audio/wav")).not.toThrow();
		expect(() => assertAllowedContentType(env, "application/pdf")).not.toThrow();
		expect(() => assertAllowedContentType(env, "video/mp4")).toThrow(/not allowed/u);
	});
...
function environment(overrides: Partial<FilesEnv> = {}): FilesEnv {
	return {
		...
		MAX_FILE_BYTES: "1024",
		ALLOWED_FILE_CONTENT_TYPES_JSON: '["application/pdf","audio/*","image/png"]',
