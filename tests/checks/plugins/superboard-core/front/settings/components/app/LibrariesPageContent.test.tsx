...
import { render, screen, within } from "../../../../../../packages/supbrd-front-ui/render.js";
...
vi.mock(
	"../../../../../../../../packages/plugins/superboard-core/src/front/settings/api/platform/platformService.js",
	() => ({
...
import LibrariesPageContent from "../../../../../../../../packages/plugins/superboard-core/src/front/settings/components/app/LibrariesPageContent.js";
...
describe("LibrariesPageContent", () => {
	beforeEach(() => {
		...
		mocks.getPlatformLibraries.mockResolvedValue({
			...
			libraries: [
				{
					...
					install: "superboard_flutterflow: immutable",
				...
				},
				{
					id: "javascript",
					lifecycle: "archived",
					displayName: "SuperBoard JavaScript",
					ecosystem: "npm",
					packageName: "@mbzadev/superboard-js-sdk",
					sourcePath: "sdks/web",
					license: "MIT",
					licensePath: "sdks/web/LICENSE",
					versionSource: "sdks/web/package.json",
					sourceVersion: "1.0.2",
					latestReleaseVersion: "1.0.2",
					releaseRef: "sdk-js-v1.0.2",
					releaseStatus: "released",
					releaseSha: "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb",
					install: "npm install @mbzadev/superboard-js-sdk@1.0.2",
					distribution: {
						...
						authentication: {
							required: true,
							tokenEnvironmentVariable: "SUPERBOARD_GITHUB_PACKAGES_TOKEN",
						},
					},
				},
				{
					...
					license: "MIT",
					licensePath: "sdks/web/flows/upstream/packages/js/LICENSE",
					versionSource: "sdks/web/flows/upstream/packages/js/package.json",
					sourceVersion: "1.23.3",
					releaseStatus: "unreleased",
					publicationTarget: "public-npm",
					notes: "Initial reviewed source import. No npm publication exists yet.",
				},
			],
			customCode: {
				schemaVersion: 1,
			...
			flutterFlowLibrary: {
				...
				source: {
					path: "scripts/clients/flutterflow-library/dsl/edit.dart",
					testPath: "tests/checks/clients/flutterflow-library/app_test.dart",
				...
				actions: { identity: ["superboardApplicationInitialize"] },
				forbiddenAppState: ["superboardApplicationAccessToken"],
			},
		});
	});
	...
	it("shows whether the Git-owned FlutterFlow project can be updated", async () => {
		...
		expect(await screen.findByText("FlutterFlow library project · SuperBoard")).toBeInTheDocument();
		expect(screen.getByText("1 immutable tag pending")).toBeInTheDocument();
		expect(screen.getByRole("link", { name: "Open Git-owned DSL" })).toHaveAttribute(
			"href",
			"https://github.com/mabzadev/superboard/blob/dev/scripts/clients/flutterflow-library/dsl/edit.dart",
		);
		expect(screen.getByRole("link", { name: "Open controlled update workflow" })).toHaveAttribute(
			"href",
	...
	it("does not present public package metadata as anonymous installation", async () => {
		...
		expect(await screen.findByText("SuperBoard JavaScript")).toBeInTheDocument();
		expect(screen.getByText("Archived")).toBeInTheDocument();
		expect(screen.getByText("Public metadata")).toBeInTheDocument();
		expect(screen.getByText("Authentication required")).toBeInTheDocument();
		expect(screen.getByText("Anonymous install unavailable")).toBeInTheDocument();
		expect(screen.getByText("SUPERBOARD_GITHUB_PACKAGES_TOKEN")).toBeInTheDocument();
		expect(screen.getByText("https://npm.pkg.github.com")).toBeInTheDocument();
		expect(screen.getByText("Historical dependency")).toBeInTheDocument();
	});

	it("shows the staged SuperBoard coordinate without replacing the release baseline", async () => {
		render(<LibrariesPageContent />);

		const library = within(await screen.findByTestId("library-flutterflow"));
		expect(library.getByText("superboard_flutterflow")).toBeInTheDocument();
		expect(library.getByText("Migration candidate")).toBeInTheDocument();
		expect(library.getByText("Release pending")).toBeInTheDocument();
		expect(library.getAllByText("sdk-flutterflow-v2.2.5")).not.toHaveLength(0);
	});
