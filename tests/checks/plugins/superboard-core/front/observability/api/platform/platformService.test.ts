...
vi.mock(
	"../../../../../../../../packages/plugins/superboard-core/src/front/observability/transport.js",
...
} from "../../../../../../../../packages/plugins/superboard-core/src/front/observability/api/platform/platformService.js";
...
describe("platform library catalogue service", () => {
	...
	it("loads redacted account-erasure operations from the control plane", async () => {
		...
		expect(requests.get).toHaveBeenCalledWith("/api/v1/platform/account-erasures?limit=50", {
			timeout: 10_000,
			maxRetries: 1,
		});
	});
	...
	it("loads email operations and sends explicit DLQ decisions to the control plane", async () => {
		...
		expect(requests.get).toHaveBeenCalledWith("/api/v1/platform/email/operations?limit=50", {
			timeout: 10_000,
			maxRetries: 1,
		});
		...
		await expect(replayPlatformEmailDeadLetter("dead-letter-1")).resolves.toMatchObject({
			status: "replayed",
			messageId: "mail-1",
		});
		await expect(discardPlatformEmailDeadLetter("dead-letter-2")).resolves.toMatchObject({
			status: "discarded",
		});
