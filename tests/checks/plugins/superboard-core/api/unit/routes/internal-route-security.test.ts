...
import automation from "../../../../../../../packages/plugins/superboard-core/api/src/routes/automation";
import diagnostics from "../../../../../../../packages/plugins/superboard-core/api/src/routes/diagnostics";
...
describe("internal route credential transport", () => {
	...
	it("never accepts the diagnostics key from the query string", async () => {
		const response = await diagnostics.request("/test_logs?api_key=diagnostics-secret", undefined, {
			DIAGNOSTICS_API_KEY: "diagnostics-secret",
		} as never);
			...
			automation.request(
				"/run_maintenance",
				{
					...
					headers: {
						...
						"x-maintenance-key": "maintenance-secreu",
					},
					body: "{}",
				},
				{ MAINTENANCE_PROCESS_KEY: "maintenance-secret" } as never,
