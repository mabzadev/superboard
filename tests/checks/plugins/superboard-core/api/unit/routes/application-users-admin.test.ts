...
vi.mock(
	"../../../../../../../packages/plugins/superboard-core/api/src/lib/domain-modules",
	async (load) => ({
		...(await load<
			typeof import("../../../../../../../packages/plugins/superboard-core/api/src/lib/domain-modules")
		>()),
		resolveAuthorizedProjectContext: mocks.resolveAuthorizedProjectContext,
	}),
);

import applicationUsersAdminRoutes from "../../../../../../../packages/plugins/superboard-core/api/src/routes/application-users-admin";
...
describe("application user administration gateway", () => {
	...
	it("forwards only sanitized list parameters through the target Identity binding", async () => {
		const identityFetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
			...
			await expect(
				verifyInternalProjectContextRequest(request, "module-token", "identity"),
	...
	it("encodes the application user id and forwards no dashboard credential", async () => {
		const identityFetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
			const request = new Request(input, init);
			expect(request.url).toBe("https://identity.internal/internal/v1/admin/users/user%3A123");
			expect(request.headers.get("authorization")).toBeNull();
	...
	it("requires a valid dashboard session and an owner or administrator role", async () => {
		...
		const unauthenticated = await app().request("/10-prod/users", {}, fixture(vi.fn()));
	...
	it.each([
		["GET", "security?cursor=session-1"],
		["POST", "resume"],
		["POST", "sessions/session-1/revoke"],
	])("protects and signs %s account security requests to %s", async (method, suffix) => {
		mocks.getRequestAuthContext.mockResolvedValue({
			userId: 0,
			siteOperator: { operator_id: "operator-1" },
		});
		const transport = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
			...
			expect(verified).toMatchObject({
				ok: true,
				context: { operatorId: "operator-1", projectId: 11, method },
			});
			expect(new URL(forwarded.url).pathname).toBe(
				`/internal/v1/admin/profiles/user-1/${suffix.split("?")[0]}`,
			);
			expect(forwarded.headers.get("Idempotency-Key")).toBe("security-operation-1");
			const payload = method === "POST" ? await forwarded.json() : null;
			expect(payload).toEqual(method === "POST" ? { user_id: "user-1" } : null);
			return Response.json({ data: { status: "ok" } });
		});
		const options = {
			method,
			headers: { "Content-Type": "application/json", "Idempotency-Key": "security-operation-1" },
			...(method === "POST" ? { body: JSON.stringify({ user_id: "user-1" }) } : {}),
		};
		expect(
			(await app().request(`/10-test/profiles/user-1/${suffix}`, options, fixture(transport)))
				.status,
		).toBe(200);
		mocks.getRequestAuthContext.mockResolvedValue(null);
		expect(
			(await app().request(`/10-test/profiles/user-1/${suffix}`, options, fixture(transport)))
				.status,
		).toBe(401);
