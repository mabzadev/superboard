...
describe("MCP back-office contract", () => {
					...
					projects: [
						{
							id: "10-prod",
							name: "Sample",
							identifier: "sample_prod_10",
							environment: "production",
						},
						{
							id: "10-test",
							name: "Sample Test",
							identifier: "sample_test_10",
							environment: "test",
						},
					],
				},
	...
	it("reports activity without quota or SuperBoard subscription fields", async () => {
		const db = mcpDatabase(({ op, sql }) => {
			...
			if (op === "first" && sql.includes("COUNT(DISTINCT e.device_id) AS total"))
				return { total: 123 };
			return undefined;
		});

		const response = await mcp.request(
			"/usage?instance_id=10",
			{
				headers: { Authorization: "Bearer mcp_token" },
			},
			env(db),
