...
vi.mock(
	"../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-service",
	async (importOriginal) => {
		const original =
			await importOriginal<
				typeof import("../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-service")
			>();
...
vi.mock(
	"../../../../../../../packages/plugins/superboard-core/api/src/lib/device-certification",
	async (importOriginal) => {
		const original =
			await importOriginal<
				typeof import("../../../../../../../packages/plugins/superboard-core/api/src/lib/device-certification")
			>();
		return { ...original, recordDeviceCertificationResult: mocks.recordResult };
	},
);

import purchasesV2Sdk from "../../../../../../../packages/plugins/superboard-core/api/src/routes/purchases-v2-sdk";
...
describe("Purchases SDK device certification route", () => {
	...
	it("binds the result to the verified customer and SDK application context", async () => {
		...
		await expect(response.json()).resolves.toMatchObject({
			data: { id: "device-result-104", duplicate: false },
		});
...
function deviceRequest() {
	return new Request("https://sdk.example.com/certification/device-results", {
		...
		headers: {
			...
			"X-SuperBoard-App-Version": "1.4.0",
			"X-SuperBoard-Build-Number": "104",
			"X-SuperBoard-SDK-Version": "2.1.3",
		},
		body: JSON.stringify({
			id: "device-result-104",
			run_id: "run-device-1",
...
function env(): Env {
	const db = createFakeD1((call) => {
		if (call.op === "first" && call.sql.includes("FROM projects p"))
			return {
				id: 11,
				is_test: 1,
				purchases_enabled: 1,
