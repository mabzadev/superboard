...
vi.mock(
	"../../../../../../../packages/plugins/superboard-core/api/src/routes/platform-status",
	() => ({
		buildPlatformStatus,
	}),
);

import mcp from "../../../../../../../packages/plugins/superboard-core/api/src/routes/mcp";
...
function database(role: "admin" | "member") {
	return createFakeD1((call) => {
		if (call.op === "first" && call.sql.includes("SELECT user_id FROM mcp_tokens")) {
			return { user_id: 7 };
		}
		if (call.op === "run" && call.sql.includes("UPDATE mcp_tokens SET last_used_at")) {
			return true;
		}
		if (call.op === "first" && call.sql.includes("role IN ('owner', 'admin')")) {
