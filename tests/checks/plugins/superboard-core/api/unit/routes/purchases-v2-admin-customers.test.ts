...
vi.mock(
	"../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-identity",
	async (load) => {
		const actual =
			await load<
				typeof import("../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-identity")
			>();
		return {
			...actual,
...
describe("billing customer administration", () => {
	it("returns project/customer-scoped paywall activity with customer detail", async () => {
		const db = customerDb((call) => {
			if (call.op === "first" && call.sql.includes("FROM billing_customers WHERE id = ?")) {
				return {
					id: "customer-1",
					project_id: "20",
