...
import { configuredMcpConsentUrl } from "../../../../../../../packages/plugins/superboard-core/api/src/routes/mcp-oauth";

describe("MCP OAuth consent configuration", () => {
	it("derives the consent page only from the target-owned Dashboard origin", () => {
		expect(configuredMcpConsentUrl({ APP_URL: "https://dashboard.example.test/" })).toBe(
			"https://dashboard.example.test/mcp/authorize",
		);
		expect(
			configuredMcpConsentUrl({
	...
	it("fails closed without configuration or with an insecure remote origin", () => {
		...
		expect(() => configuredMcpConsentUrl({ APP_URL: "http://dashboard.example.test" })).toThrow(
			"absolute HTTPS URL",
		);
	});
});
