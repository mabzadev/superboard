...
import customSdk from "../../../../../../../packages/plugins/superboard-core/api/src/routes/custom-sdk";
import type {
	AppVariables,
	Env,
...
describe("custom SDK jobs", () => {
	...
	it("derives project, subject and scoped idempotency while stripping client identity fields", async () => {
		...
		const response = await probe().request(
			"/jobs",
			{
				...
				body: JSON.stringify({
					capability: "reference-production.media.convert",
		...
		const [url, init] = fixture.custom.fetch.mock.calls[0] as unknown as [string, RequestInit];
	...
	it("requires a verified application identity and never reaches the custom Worker on failure", async () => {
		...
		const response = await probe().request(
			"/jobs",
			{
				...
				body: JSON.stringify({
					capability: "reference-production.voice.clone",
					payload: {},
				}),
			},
			fixture.env,
	...
	it("scopes reads to the verified subject and only forwards documented filters", async () => {
		...
		const [url, init] = fixture.custom.fetch.mock.calls[0] as unknown as [string, RequestInit];
		expect(url).toBe("https://custom.internal/internal/v1/jobs?limit=10&status=failed");
		expect(new Headers(init.headers).get("x-custom-worker-subject")).toBe("user-43");
		expect(new Headers(init.headers).get("x-custom-worker-project")).toBe("10-prod");
	});
	...
	it("scopes job detail to the verified subject and validates opaque identifiers", async () => {
		...
		expect(new Headers(init.headers).get("x-custom-worker-subject")).toBe("user-44");
		expect(new Headers(init.headers).get("x-custom-worker-project")).toBe("10-prod");
