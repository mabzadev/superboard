...
} from "../../../../../../../packages/plugins/superboard-core/api/src/routes/messaging-admin";
import messaging from "../../../../../../../packages/plugins/superboard-core/api/src/routes/messaging-admin";
...
describe("Messaging realtime tickets", () => {
	it("stores only a SHA-256 hash and expires the ticket after 60 seconds", async () => {
		...
		const issued = await issueMessagingRealtimeTicket(
			issuingDatabase(captured),
			{
				projectId: "42",
	...
	it("atomically consumes an unexpired ticket within its project and conversation scope", async () => {
		...
		await expect(consumeMessagingRealtimeTicket(db, params, now)).resolves.toMatchObject({
			user_id: "operator-1",
		});
	...
	it("forwards an authorized upgrade without browser credentials or the ticket", async () => {
		...
		expect((forwarded as Request).headers.get("X-SuperBoard-Internal-Token")).toBe(
			"internal-capability",
		);
...
describe("Messaging administration permissions", () => {
	it("allows members to operate conversations and read configuration", () => {
		expect(() =>
			requireMessagingAccess("POST", "/conversations/conversation-1/messages", "member"),
		).not.toThrow();
		expect(() => requireMessagingAccess("GET", "/settings", "member")).not.toThrow();
	});

	it("requires an owner or admin for configuration mutations", () => {
		expect(() => requireMessagingAccess("PATCH", "/settings", "member")).toThrow(
			"Owner or admin access",
		);
		expect(() => requireMessagingAccess("POST", "/settings/entities", "member")).toThrow(
			"Owner or admin access",
		);
		expect(() =>
			requireMessagingAccess("DELETE", "/settings/entities/entity-1", "admin"),
		).not.toThrow();
	});
});
