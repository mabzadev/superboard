...
import {
	mapConversationItem,
	mapRefundItem,
} from "../../../../../../../packages/plugins/superboard-core/api/src/routes/inbox-admin";
...
describe("unified Inbox projection", () => {
	it("keeps source identifiers and actions as references", () => {
		...
		expect(item).toMatchObject({
			...
			destination: "/inbox?type=conversation&id=conversation-1",
			priority: "high",
		});
	...
	it("prioritizes imminent refund deadlines without mutating the case", () => {
		...
		const source = {
			...
			deadline_at: "2026-08-04T08:00:00.000Z",
			updated_at: "2026-08-03T10:00:00.000Z",
