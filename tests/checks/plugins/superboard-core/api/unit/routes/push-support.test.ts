...
import { internalPush } from "../../../../../../../packages/plugins/superboard-core/api/src/routes/push";
...
describe("Support notification internal ingress", () => {
	...
	it("materializes browser only, never signals push, and replays idempotently", async () => {
		...
		expect(
			db.calls.some(
				(call) => call.op === "run" && call.sql.startsWith("INSERT INTO rpush_notifications"),
			),
		).toBe(false);
		...
		expect(
			db.calls.filter(
				(call) => call.op === "run" && call.sql.startsWith("INSERT INTO notifications"),
			),
		).toHaveLength(1);
	});
	...
	it("targets mobile only and signals the existing push processor", async () => {
		...
		expect(send).toHaveBeenCalledWith({ type: "push.process", limit: 25 }, { contentType: "json" });
		const audienceInsert = db.calls.find(
			(call) => call.op === "run" && call.sql.startsWith("INSERT INTO notification_messages"),
		);
