...
vi.mock(
	"../../../../../../../packages/plugins/superboard-core/api/src/lib/account-erasure",
	async (load) => ({
		...(await load<
			typeof import("../../../../../../../packages/plugins/superboard-core/api/src/lib/account-erasure")
		>()),
		eraseApplicationAccount: mocks.eraseApplicationAccount,
	}),
);

import accountSdk from "../../../../../../../packages/plugins/superboard-core/api/src/routes/account-sdk";
...
describe("application account SDK", () => {
	beforeEach(() => {
		...
		mocks.eraseApplicationAccount.mockResolvedValue({
			...
			status: "completed",
			completedSteps: ["app", "marketing", "support", "custom", "billing", "identity"],
		});
	});
	...
	it("derives the erasure subject from Identity and never trusts request data", async () => {
		...
		await expect(response.json()).resolves.toEqual({
			data: {
				...
				completed_steps: ["app", "marketing", "support", "custom", "billing", "identity"],
			},
		});
		...
		IDENTITY_SERVICE: {
			fetch: vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
				...
				expect(request.headers.get("authorization")).toBe(
					identityStatus === 200 ? "Bearer verified-identity-token" : "Bearer rejected-token",
				);
