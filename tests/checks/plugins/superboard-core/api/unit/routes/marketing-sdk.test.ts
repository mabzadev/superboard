...
import marketingSdk from "../../../../../../../packages/plugins/superboard-core/api/src/routes/marketing-sdk";
import type {
	AppVariables,
	Env,
...
describe("application Marketing preferences", () => {
	it("forwards a verified Identity profile through a signed project context", async () => {
		...
		expect(forwarded.headers.get("x-superboard-application-user-id")).toBe("application-user-1");
		expect(forwarded.headers.get("x-superboard-application-email")).toBe("user@example.test");
		expect(forwarded.headers.get("x-superboard-application-user-id")).toBe("application-user-1");
		expect(forwarded.headers.get("x-superboard-application-email")).toBe("user@example.test");
		...
		const response = await app().request(
			"/preferences",
			{
				...
				headers: {
					...
					"x-superboard-application-email": "attacker@example.test",
				},
				body: JSON.stringify({
					consented: true,
