...
import identityAdminRoutes from "../../../../../../../packages/plugins/superboard-core/api/src/routes/identity-admin";
...
describe("Identity administration gateway", () => {
	...
	it("exposes configuration info at the project root", async () => {
		const identityFetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
			const request = new Request(input, init);
			expect(request.url).toBe("https://identity.internal/internal/v1/melody-admin/info");
	...
	it("requires a dashboard session and an owner or administrator role", async () => {
		...
		const unauthenticated = await testApp().request("/10-prod/api/v1/apps", {}, fixture(vi.fn()));
