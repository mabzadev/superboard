...
import redirectRoutes from "../../../../../../../packages/plugins/superboard-core/api/src/routes/redirect";
...
function redirectFixture(overrides: Record<string, unknown> = {}) {
	const state = {
		...
		notifications: [
			{
				id: "message-1",
				html: '<h1 onclick="alert(1)">Hello</h1><script>alert(1)</script><a href="javascript:bad()">bad</a>',
			},
		],
		link: {
			id: 77,
			...
			image_url: null,
			data: JSON.stringify({ deeplink: "superboard://promo" }),
			default_fallback: "https://fallback.test/promo",
			project_id: 101,
			instance_id: 10,
			project_name: "Production",
			uri_scheme: "superboard",
...
function redirectD1Handler(call: FakeD1Call, state: ReturnType<typeof redirectFixture>["state"]) {
	...
	if (op === "first" && sql.includes("SELECT id FROM devices WHERE ip = ? AND user_agent = ?")) {
		return (
			state.devices.find((device) => device.ip === args[0] && device.user_agent === args[1]) || null
		);
	}
	...
	if (op === "first" && sql.includes("FROM redirects")) {
		...
		if (platform === "ios")
			return { fallback_url: "https://apps.apple.test/superboard", appstore: 0 };
		if (platform === "android")
			return { fallback_url: "https://play.test/superboard", appstore: 0 };
...
describe("public redirects", () => {
	it("redirects the short-link root and unknown links to the dashboard", async () => {
		...
		const unknown = await redirectRoutes.request("/unknown", { redirect: "manual" }, env);
	...
	it("creates an action but suppresses view events for bots", async () => {
		...
		const response = await redirectRoutes.request(
			"/promo",
			{
	...
	it("renders iOS app-link handling HTML with deeplink and fallback", async () => {
		...
		expect(response.status).toBe(200);
		const html = await response.text();
		expect(html).toContain("superboard://promo");
		expect(html).toContain("https://apps.apple.test/superboard");
	});
	...
	it("renders Android app-link handling HTML with deeplink and fallback", async () => {
		...
		const html = await response.text();
		expect(html).toContain("superboard://promo");
		expect(html).toContain("https://play.test/superboard");
	});
