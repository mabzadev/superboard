...
import {
	...
	signToken,
	verifyPassword,
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/crypto";
import { tokenDigest } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/token-storage";
import authRoutes from "../../../../../../../packages/plugins/superboard-core/api/src/routes/auth";
import instancesRoutes from "../../../../../../../packages/plugins/superboard-core/api/src/routes/instances";
import oauthRoutes from "../../../../../../../packages/plugins/superboard-core/api/src/routes/oauth";
import usersRoutes from "../../../../../../../packages/plugins/superboard-core/api/src/routes/users";
import { Env } from "../../../../../../../packages/plugins/superboard-core/api/src/types";
...
async function authFixture() {
	...
	const env = {
		...
		MAIL_FROM: "SuperBoard <noreply@example.test>",
	} satisfies Env;
...
function authD1Handler(call: FakeD1Call, state: Awaited<ReturnType<typeof authFixture>>["state"]) {
	...
	const userByEmail = (email: unknown) =>
		state.users.find((user) => user.email === String(email).toLowerCase());
	...
	if (op === "first" && sql.includes("WHERE uid = 'superboard-dashboard-reference-production'")) {
	...
	if (op === "first" && sql.includes("FROM users WHERE invitation_token")) {
		return (
			state.users.find(
				(user) => args.includes(user.invitation_token || "") && !user.invitation_accepted_at,
			) || null
		);
	}
	...
	if (op === "first" && sql.includes("FROM users WHERE reset_password_token")) {
		const user = state.users.find(
			(candidate) =>
				args.includes(candidate.reset_password_token || "") && candidate.reset_password_sent_at,
		);
	...
	if (op === "first" && sql.includes("FROM instance_roles WHERE user_id = ? AND instance_id = ?")) {
		return (
			state.roles.find(
				(role) => role.user_id === Number(args[0]) && role.instance_id === Number(args[1]),
			) || null
		);
	}
	...
	if (
		op === "first" &&
		sql.includes("FROM instance_roles WHERE user_id = ? ORDER BY id ASC LIMIT 1")
	) {
		return state.roles.find((role) => role.user_id === Number(args[0])) || null;
	}
	...
	if (op === "first" && sql.includes("FROM instances i JOIN instance_roles r")) {
		...
		return role
			? state.instances.find((instance) => instance.id === role.instance_id) || null
			: null;
	}
	...
	if (
		...
		sql.includes("refresh_token = ?")
	) {
		return (
			state.tokens.find(
				(candidate) => args.includes(candidate.refresh_token) && !candidate.revoked_at,
			) || null
		);
	}
	...
	if (
		op === "first" &&
		sql.includes("FROM oauth_access_tokens") &&
		(sql.includes("token = ?") || sql.includes("token IN (?, ?)"))
	) {
	...
	if (op === "run" && sql.startsWith("UPDATE oauth_access_tokens SET revoked_at = datetime")) {
		...
		for (const token of state.tokens) {
			if (args.includes(token.token) || args.includes(token.refresh_token))
	...
	if (
		op === "run" &&
		sql.includes("SET password_hash = ?") &&
		sql.includes("reset_password_token = NULL")
	) {
...
describe("OAuth/dashboard auth routes", () => {
	...
	it("rejects non-allowlisted registration before creating any records", async () => {
		...
		const env = {
			...
			REGISTRATION_REALM: "reference-production:production",
		} satisfies Env;
	...
	it("rotates refresh tokens and revokes the previous access/refresh pair", async () => {
		...
		const refreshResponse = await oauthRoutes.request(
			"/token",
			json({
	...
	it("accepts 2FA password grant login with a valid OTP code", async () => {
		...
		const response = await passwordGrant(env, {
			otp_code: await currentTotp(state.users[0].otp_secret),
		});
		expect(response.status).toBe(200);
	...
	it("runs password reset request and token-based password change", async () => {
		...
		const resetResponse = await usersRoutes.request(
			"/reset_password",
		...
		const changeResponse = await usersRoutes.request(
			"/change_password",
			json({
	...
	it("forbids members and allows admins on admin-only instance member routes", async () => {
		...
		const adminResponse = await instancesRoutes.request(
			"/10/members",
			{
