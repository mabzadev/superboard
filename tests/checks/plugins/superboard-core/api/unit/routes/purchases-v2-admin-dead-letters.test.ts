...
import adminRoutes from "../../../../../../../packages/plugins/superboard-core/api/src/routes/purchases-v2-admin";
...
describe("billing dead-letter administration", () => {
	it("lists quarantine metadata without exposing job payloads", async () => {
		const db = deadLetterDb((call) => {
			if (call.op === "all" && call.sql.includes("FROM billing_dead_letters")) {
				return [
					{
						id: "dead-1",
						queue_name: "superboard-billing-dlq",
		...
		expect(
			db.calls.find((call) => call.sql.includes("FROM billing_dead_letters"))?.sql,
	...
	it("verifies integrity, atomically claims the job, queues it once, and audits the replay", async () => {
		...
		const db = deadLetterDb((call) => {
			if (call.op === "first" && call.sql.includes("FROM billing_dead_letters")) {
				return {
					id: "dead-1",
					status: "quarantined",
					job_valid: 1,
		...
		const claim = db.calls.find(
			(call) => call.op === "run" && call.sql.includes("SET status = 'replay_queued'"),
		);
		expect(claim?.sql).toContain("status = 'quarantined'");
		const audit = db.calls.find(
			(call) => call.op === "run" && call.sql.includes("billing_admin_audit_logs"),
		);
	...
	it("returns the claim to quarantine when queue delivery fails", async () => {
		...
		const restore = db.calls.find(
			(call) => call.op === "run" && call.sql.includes("SET status = 'quarantined'"),
		);
		expect(restore).toBeDefined();
		expect(db.calls.some((call) => call.sql.includes("billing_admin_audit_logs"))).toBe(false);
	});
	...
	it("allows an administrator to discard a quarantined job with an audit trail", async () => {
		...
		const discard = db.calls.find(
			(call) => call.op === "run" && call.sql.includes("SET status = 'discarded'"),
		);
...
function deadLetterDb(handler: (call: FakeD1Call) => unknown) {
	return createFakeD1((call) => {
		...
		if (
			call.op === "first" &&
			call.sql.includes("SELECT id, name, identifier, instance_id, is_test FROM projects")
		) {
			return {
				id: 20,
				name: "Production",
...
function actorHeaders() {
	return { "X-SuperBoard-Internal-Actor": "7" };
}

function baseEnv(db: D1Database, send = vi.fn().mockResolvedValue(undefined)): Env {
	return {
		DB: db,
		KV: {} as KVNamespace,
