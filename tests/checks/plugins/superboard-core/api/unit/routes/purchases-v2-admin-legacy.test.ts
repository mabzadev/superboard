...
describe("legacy inventory administration", () => {
	it("cancels only an active project-scoped inventory and records an audit event", async () => {
		...
		const response = await adminRoutes.request(
			"/10-prod/legacy/revenuecat/inventory-runs/inventory-1/cancel",
			{
				method: "POST",
		...
		const cancellation = db.calls.find(
			(call) => call.op === "first" && call.sql.includes("UPDATE billing_legacy_inventory_runs"),
		)!;
