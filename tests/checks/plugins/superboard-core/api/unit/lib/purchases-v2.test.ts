...
import {
	...
	targetingMatches,
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/purchases-v2";
	...
	it("requires every condition and supports missing values", () => {
		expect(
			targetingMatches(
				...
				{ country: "CH", attributes: {} },
			),
		).toBe(false);
		expect(
			targetingMatches([{ field: "attribute.vip", operator: "not_exists" }], { attributes: {} }),
		).toBe(true);
	});
});
