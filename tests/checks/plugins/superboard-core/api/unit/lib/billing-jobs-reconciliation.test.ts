...
import { reconcileStoreSubscription } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-jobs";
...
describe("Provider subscription reconciliation state", () => {
	it("persists a stable failure when provider verification data is missing", async () => {
							...
							async first() {
								if (sql.includes("FROM billing_subscriptions s")) {
									return {
										...
										original_transaction_id: "order-1",
										store_product_id: "yearly-plan",
										purchase_token: null,
									};
								}
								return null;
		...
		const failure = updates.find((update) =>
			update.sql.includes("provider_verification_status = 'failed'"),
		);
