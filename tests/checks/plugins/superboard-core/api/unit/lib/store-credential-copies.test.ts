...
import { encryptStoreCredentialCopies } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/store-credential-copies";
...
describe("Store credential copies", () => {
	it("creates two independently encrypted copies inside the Billing domain", async () => {
		...
		await expect(decryptCredential(env, copies.sourceCiphertext)).resolves.toBe(
			'{"secret":"value"}',
		);
		await expect(decryptCredential(env, copies.billingCiphertext)).resolves.toBe(
			'{"secret":"value"}',
		);
	});
	...
	it("requests the Billing copy through the private binding from the API domain", async () => {
		const fetch = vi
			.fn()
			.mockResolvedValue(new Response(JSON.stringify({ data: { ciphertext: "v1.billing-copy" } })));
		const env = {
	...
	it("fails closed when Billing does not return its credential copy", async () => {
		...
		await expect(encryptStoreCredentialCopies(env, '{"secret":"value"}')).rejects.toMatchObject({
