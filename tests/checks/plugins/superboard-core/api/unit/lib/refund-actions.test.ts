...
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/refund-actions";
...
describe("refund provider actions", () => {
	it("treats provider-terminal refund cases as immutable", async () => {
		...
		await expect(executeRefundProviderAction({ DB: db } as BillingEnv, "action-1")).rejects.toThrow(
			/terminal refund case/i,
		);
	...
	it("requires explicit Apple consent and validates delivery consistency", () => {
		expect(() =>
			validateRefundActionPayload("apple", "submit_consumption_info", {
	...
	it("restricts every provider to an explicit action and payload allowlist", () => {
		expect(supportedRefundActions("google")).toEqual([
			"review_google_refund",
			"refund_google_order",
			"revoke_google_subscription",
		]);
		expect(refundActionDefinitions("apple")).toEqual([
			expect.objectContaining({
				action_type: "submit_consumption_info",
				recommended_evidence_type: "apple_consumption_consent",
			}),
		]);
		expect(() => validateRefundActionPayload("google", "create_external_refund", {})).toThrow(
			/not supported/i,
		);
		expect(supportedRefundActions("external")).toEqual([]);
		expect(
			validateRefundActionPayload("google", "revoke_google_subscription", {
				refund_type: "item",
				product_id: "addon",
			}),
		).toEqual({ refund_type: "item", product_id: "addon" });
		expect(
			validateRefundActionPayload("google", "review_google_refund", {
	...
	it("does not execute a provider request when another worker holds the action lease", async () => {
		...
		await expect(
			executeRefundProviderAction({ DB: db } as BillingEnv, "action-1"),
...
function actionRow(overrides: Partial<Record<string, unknown>> = {}) {
	return {
		...
		order_id: "GPA.test",
		purchase_token: null,
		store_product_id: "reference-production_weekly_999",
