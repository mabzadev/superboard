...
describe("native Support gateway", () => {
	it("maps the client root once, normalizes the project header, and preserves the Worker error envelope", async () => {
		...
		expect(forwarded?.url).toBe("https://support.internal/v1/conversations?limit=25");
	...
	it("forwards only provider signature headers and derives the canonical Twilio URL", async () => {
		...
		expect(forwarded?.headers.get("x-twilio-signature")).toBe("twilio-signature");
		expect(forwarded?.headers.get("content-type")).toContain("application/x-www-form-urlencoded");
		...
		expect(forwarded?.headers.get("x-superboard-inbound-normalized")).toBeNull();
		expect(forwarded?.headers.get("x-superboard-email-provider-verified")).toBeNull();
		await expect(forwarded?.text()).resolves.toBe("MessageSid=SM123&Body=Hello");
	});
	...
	it("normalizes inbound email through Email before delivering the canonical envelope to Support", async () => {
		...
		const emailFetch = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
		...
		expect(response.status).toBe(202);
		expect(new URL(emailRequest!.url).pathname).toBe(EMAIL_SERVICE_INBOUND_NORMALIZE_PATH);
		expect(emailRequest?.headers.get("x-internal-token")).toBe("email-internal-secret");
		expect(emailRequest?.headers.get("x-superboard-email-provider")).toBe("google");
		expect(emailRequest?.headers.get("x-superboard-email-provider-verified")).toBeNull();
		...
		expect(supportRequest?.headers.get("x-superboard-inbound-normalized")).toBe("email-v1");
	...
	it("forwards an integration callback without browser credentials or internal headers", async () => {
		...
		expect(forwarded?.headers.get("x-request-id")).toBe("integration-oauth-callback-1");
	});
	...
	it("forwards only the public widget authentication contract", async () => {
		...
		expect(forwarded?.headers.get("authorization")).toBe("Bearer supported-identity-alternative");
		expect(forwarded?.headers.get("idempotency-key")).toBe("widget-create-1");
		expect(forwarded?.headers.get("origin")).toBe("https://support.example.test");
		expect(forwarded?.headers.get("x-superboard-project-id")).toBe("project-public-reference");
		expect(forwarded?.headers.get("x-superboard-widget-key")).toBe("widget-public-key");
		expect(forwarded?.headers.get("x-superboard-widget-visitor")).toBe("opaque-client-visitor");
		expect(forwarded?.headers.get("x-superboard-widget-signature")).toBe("a".repeat(64));
		expect(forwarded?.headers.get("x-superboard-widget-timestamp")).toBe("1800000000");
