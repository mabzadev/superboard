...
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/legacy-subscription-inventory";
import { encryptSecret } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/secrets";
...
describe("Legacy subscription inventory", () => {
			...
			normalizeLegacySubscription({
				...
				entitlements: {
					items: [
						{
							products: {
								items: [{ id: "provider-product-1", store_identifier: "weekly_product" }],
							},
						},
					],
				},
	...
	it("never inventories a subscription that does not currently give access", () => {
		expect(
			normalizeLegacySubscription({ id: "expired", gives_access: false, store: "play_store" }),
		).toBeNull();
	});
	...
	it("keeps unsupported legacy stores unresolved", () => {
		expect(
			...
			environment: "sandbox",
			storeProductId: "legacy-product",
		});
	});

	it("accepts only pagination cursors on the expected official API path", () => {
		const expectedPath = "/v2/projects/project-1/customers";
		expect(nextPageCursor(`${expectedPath}?starting_after=customer-10`, expectedPath)).toBe(
			"customer-10",
		);
		expect(() =>
			nextPageCursor("https://example.com/collect?starting_after=secret", expectedPath),
		).toThrow(/invalid pagination URL/);
		expect(() =>
			nextPageCursor("/v2/projects/other/customers?starting_after=customer-10", expectedPath),
		).toThrow(/invalid pagination URL/);
	});
	...
	it("uses only the Billing credential copy inside the isolated Billing domain", async () => {
		...
		const source = {
			...
			configuration_encrypted: await encryptSecret(
				JSON.stringify({ api_key: "api-domain-key" }),
				encryptionKey,
			),
		...
		const fetchImpl = async (input: RequestInfo | URL, init?: RequestInit) => {
			expect(String(input)).toBe(
				"https://api.revenuecat.com/v2/projects/provider-project-1/customers?limit=1",
			);
			...
			return new Response(
				JSON.stringify({
					object: "list",
					items: [],
					url: "/v2/projects/provider-project-1/customers",
				}),
				{
					status: 200,
	...
	it("does not contact the provider when another queue consumer owns the page lease", async () => {
		...
		const db = createFakeD1((call) => {
			if (call.op === "first" && call.sql.startsWith("UPDATE billing_legacy_inventory_runs"))
				return null;
			return undefined;
		});
		await expect(
