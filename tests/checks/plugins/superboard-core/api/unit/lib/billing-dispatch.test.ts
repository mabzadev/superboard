...
import { isBillingQueueJob } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-dispatch";
...
describe("billing queue routing", () => {
	it("recognizes financial jobs and excludes unrelated queues", () => {
		...
		expect(isBillingQueueJob({ type: "billing.legacy.inventory.page", runId: "" })).toBe(false);
