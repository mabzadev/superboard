...
vi.mock(
	"../../../../../../../packages/plugins/superboard-core/api/src/lib/refunds",
	async (importOriginal) => ({
		...(await importOriginal<
			typeof import("../../../../../../../packages/plugins/superboard-core/api/src/lib/refunds")
		>()),
		recordGooglePendingRefundReview: refundMocks.record,
	}),
);

import { processGooglePendingRefundReview } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-jobs";
...
describe("Google pending refund review processing", () => {
	it("loads the durable provider payload, records the case, and completes the event", async () => {
		...
		await expect(
			processGooglePendingRefundReview({ DB: db } as BillingEnv, {
				eventId: "event-1",
				projectId: "11",
		...
		expect(
			writes.some((sql) => sql.includes("event_type = 'PENDING_REFUND_REVIEW_CHARGEBACK'")),
		).toBe(true);
	});
});
