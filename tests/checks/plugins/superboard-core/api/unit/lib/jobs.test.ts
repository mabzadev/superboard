...
import { dispatchQueueJob } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/jobs";
...
function jobsDb() {
	return createFakeD1((call: FakeD1Call) => {
		...
		if (op === "run" && sql.includes("DELETE FROM mcp_authorization_codes"))
...
describe("queue job dispatch", () => {
	it("rejects unsupported jobs so Cloudflare Queues can retry/dead-letter them", async () => {
		await expect(dispatchQueueJob(envWithDb(jobsDb()), { type: "unknown.job" })).rejects.toThrow(
			"Unsupported queue job",
		);
	});

	it("dispatches push processing jobs", async () => {
		await expect(
			dispatchQueueJob(envWithDb(jobsDb()), { type: "push.process", limit: 10 }),
		).resolves.toEqual({ processed: 0, results: [] });
	});

	it("dispatches MCP cleanup jobs and returns deleted/revoked counts", async () => {
		await expect(dispatchQueueJob(envWithDb(jobsDb()), { type: "mcp.cleanup" })).resolves.toEqual({
	...
	it("dispatches orphan action maintenance jobs", async () => {
		await expect(
			dispatchQueueJob(envWithDb(jobsDb()), { type: "actions.cleanup" }),
