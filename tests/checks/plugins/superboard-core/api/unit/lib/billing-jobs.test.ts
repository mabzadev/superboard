...
import { reconcileRefundDeadlines } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-jobs";
...
describe("billing refund deadline reconciliation", () => {
	it("marks overdue non-terminal deadlines once and appends immutable audit events", async () => {
		...
		expect(db.calls.find((call) => call.sql.includes("'deadline.missed', 'system'"))?.args[0]).toBe(
			"refund-deadline:deadline-1:missed",
		);
	});
