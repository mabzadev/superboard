...
describe("target-owned CORS policy", () => {
	...
	it("allows only an exact configured HTTPS origin", () => {
		expect(allowedCorsOrigin("https://grow.example.test", configured)).toBe(
			"https://grow.example.test",
		);
		expect(allowedCorsOrigin("https://reference.example.test", configured)).toBe(
			"https://reference.example.test",
		);
	...
	it("accepts browser widget origins but rejects non-origin or credentialed values", () => {
		...
		expect(allowedEmbeddableSdkOrigin("https://support.example.test/widget")).toBeUndefined();
