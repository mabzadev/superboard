...
import {
	...
	verifiedAppUserId,
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-identity";
	...
	const env = {
		DB: {
			prepare() {
				return {
					bind() {
						return { first: async () => ({ issuer, audience, jwks_uri: jwksUri }) };
					},
				};
			},
		...
		PURCHASES_SIGNING_KEYSET: JSON.stringify({
			active_kid: "purchases-key-1",
			keys: [purchasesPrivateJwk],
		}),
	} as Env;
	const token = async (
		overrides: { audience?: string; expiration?: string | number; subject?: string } = {},
	) =>
		new SignJWT({})
...
describe("billing OIDC identity", () => {
	...
	it("accepts a correctly signed subject with strict issuer and audience", async () => {
		...
		await expect(
			verifiedAppUserId(fixture.env, 1, `Bearer ${await fixture.token()}`),
		).resolves.toBe("opaque-user-42");
	});
	...
	it("rejects a wrong audience and an expired JWT", async () => {
		...
		await expect(
			verifiedAppUserId(
				fixture.env,
				1,
				`Bearer ${await fixture.token({ audience: "another-api" })}`,
			),
		).rejects.toThrow();
		await expect(
			verifiedAppUserId(
				fixture.env,
				1,
				`Bearer ${await fixture.token({ expiration: Math.floor(Date.now() / 1000) - 60 })}`,
			),
		).rejects.toThrow();
	});
	...
	it("refreshes JWKS once when a provider rotates its signing key", async () => {
		...
		fixture.cache.set(
			`billing:jwks:${await crypto.subtle
				.digest("SHA-256", new TextEncoder().encode(fixture.jwksUri))
				.then((buffer) =>
		...
		vi.stubGlobal(
			"fetch",
			vi.fn(async () => new Response(JSON.stringify({ keys: [rotatedJwk] }), { status: 200 })),
		);
		const token = await new SignJWT({})
			.setProtectedHeader({ alg: "RS256", kid: "key-2" })
			.setIssuer("https://identity.example.test/tenant")
			.setAudience("superboard")
			.setSubject("rotated-user")
			.setIssuedAt()
			.setExpirationTime("5m")
			.sign(rotated.privateKey);
		await expect(verifiedAppUserId(fixture.env, 1, `Bearer ${token}`)).resolves.toBe(
			"rotated-user",
		);
	});
	...
	it("signs CustomerInfo with ES256 and publishes only public verification keys", async () => {
		...
		const signature = await signCustomerInfoPayload(fixture.env, 42, "opaque-user-42", info);
