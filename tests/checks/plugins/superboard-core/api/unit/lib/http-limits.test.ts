...
import {
	...
	readTextLimited,
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/http-limits";

describe("bounded HTTP body readers", () => {
	it("reads a bounded JSON object", async () => {
		await expect(readJsonObjectLimited(Response.json({ status: "ok" }), 128)).resolves.toEqual({
			status: "ok",
		});
	});
	...
	it("does not treat arrays or primitives as provider response objects", async () => {
		await expect(readJsonObjectLimited(new Response("[1,2,3]"), 128)).resolves.toEqual({});
		await expect(readJsonObjectLimited(new Response("null"), 128)).resolves.toEqual({});
	});
});
