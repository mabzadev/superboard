...
import {
	scopedStoreCredential,
	timingSafeEqual,
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/secrets";
...
describe("scoped Store credentials", () => {
	...
	it("selects only the API copy in the public API execution domain", () => {
		expect(scopedStoreCredential(connection, { CREDENTIAL_KEY_SCOPE: "api" } as any)).toBe(
			"api-v1.ciphertext",
		);
	...
	it("fails closed when the Billing copy is absent", () => {
		expect(scopedStoreCredential(connection, { CREDENTIAL_KEY_SCOPE: "billing" } as any)).toBe(
			"billing-v1.ciphertext",
		);
		expect(
			scopedStoreCredential({ configuration_encrypted: "api-only" }, {
