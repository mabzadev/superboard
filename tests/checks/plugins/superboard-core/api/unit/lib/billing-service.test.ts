...
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-service";
...
describe("Billing service binding", () => {
	it("allows credential preparation through the private binding before traffic cutover", async () => {
		const fetch = vi.fn().mockResolvedValue(
			new Response(JSON.stringify({ data: { ciphertext: "billing-v1.value" } }), {
				status: 200,
		...
		await expect(
			callBillingServiceBinding(env, "/internal/v1/credentials/encrypt", { credential: "secret" }),
	...
	it("does not route financial traffic through Billing before service mode is enabled", async () => {
		...
		await expect(callBillingService(env, "/internal/v1/receipts/verify", {})).rejects.toMatchObject(
			{ code: "billing_service_unavailable", retryable: true },
		);
	...
	it("uses Billing as the sole CustomerInfo signing authority in service mode", async () => {
		...
		await expect(customerInfoFromBillingAuthority(env, "11", "customer-1")).resolves.toEqual(
			customerInfo,
		);
		expect(fetch).toHaveBeenCalledOnce();
	...
	it("fails closed when Billing returns unsigned customer data or private key material", async () => {
		...
		await expect(customerInfoFromBillingAuthority(env, "11", "customer-1")).rejects.toMatchObject({
			code: "billing_authority_invalid_response",
			retryable: true,
		});
		await expect(purchasesJwksFromBillingAuthority(env)).rejects.toMatchObject({
			code: "billing_authority_invalid_response",
			retryable: true,
		});
	});
	...
	it("resolves and identifies SDK customers only through Billing in service mode", async () => {
		...
		const fetch = vi
			...
			.mockResolvedValueOnce(
				Response.json({
					data: {
						customer: { id: "customer-1", project_id: "11" },
						appUserId: "user-1",
						identified: true,
					},
				}),
		...
		await expect(
			resolveCustomerFromBillingAuthority(env, {
				projectId: "11",
				authorization: "Bearer identity-token",
				anonymousId: "$superboard_anon_device",
			}),
		...
		await expect(
			identifyCustomerFromBillingAuthority(env, {
				projectId: "11",
				authorization: "Bearer identity-token",
				currentAppUserId: "$superboard_anon_device",
			}),
		).resolves.toEqual(customerInfo);

		expect(fetch).toHaveBeenCalledTimes(2);
		expect((fetch.mock.calls[0][0] as Request).url.endsWith("/internal/v1/customers/resolve")).toBe(
			true,
		);
		await expect((fetch.mock.calls[0][0] as Request).json()).resolves.toMatchObject({
			project_id: "11",
			authorization: "Bearer identity-token",
			anonymous_id: "$superboard_anon_device",
		});
		expect(
			(fetch.mock.calls[1][0] as Request).url.endsWith("/internal/v1/customers/identify"),
		).toBe(true);
	});
	...
	it("delegates provider event persistence and enqueue to Billing in service mode", async () => {
		...
		await expect(
			ingestProviderEventWithBillingAuthority(env, {
				projectId: "11",
				store: "google",
				environment: "sandbox",
