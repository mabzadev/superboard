...
import { deliverBillingWebhook } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/billing-jobs";
...
function deliveryEnv(
	...
	const db = createFakeD1((call) => {
		if (
			call.op === "run" &&
			call.sql.startsWith("UPDATE billing_webhook_deliveries SET claim_token")
		) {
		...
		if (call.op === "first" && call.sql.includes("JOIN billing_webhook_endpoints")) {
			return {
				...
				attempts: 0,
				url: "https://api.example.com/webhooks/superboard/entitlements",
				signing_secret_encrypted: "env:SUPERBOARD_ENTITLEMENT_WEBHOOK_SECRET",
	...
	return {
		DB: db,
		SUPERBOARD_ENTITLEMENT_WEBHOOK_SECRET: "projection-secret",
	} as unknown as Env;
}
...
describe("billing webhook delivery", () => {
	it("claims, signs, delivers, and records one successful attempt", async () => {
		...
		const fetcher = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
			...
			expect(String(new Headers(init?.headers).get("X-SuperBoard-Signature"))).toMatch(/^v1=/);
			expect(String(new Headers(init?.headers).get("X-SuperBoard-Signature"))).toMatch(/^v1=/);
	...
	it("does not redeliver when another consumer owns or completed the delivery", async () => {
		...
		await expect(
			deliverBillingWebhook(deliveryEnv({ claimChanges: 0, status: "delivered" }), "delivery-1"),
	...
	it("honors an application retryable response and persists the failure", async () => {
		...
		vi.stubGlobal(
			...
			vi.fn(
				async () =>
					new Response('{"code":"user_not_found","retryable":true}', {
						status: 404,
	...
	it("marks an inactive endpoint skipped without making a network request", async () => {
		...
		await expect(deliverBillingWebhook(deliveryEnv({ active: 0 }), "delivery-1")).resolves.toEqual({
			skipped: true,
			status: "inactive",
		});
		expect(fetcher).not.toHaveBeenCalled();
