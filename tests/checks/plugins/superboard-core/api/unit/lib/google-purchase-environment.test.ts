...
import { googlePurchaseEnvironment } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/store-verification";
...
describe("Google Play purchase environment", () => {
	...
	it("recognizes one-time license tests from current and legacy provider markers", () => {
		expect(
			googlePurchaseEnvironment({ testPurchaseContext: { fopType: "TEST" } }, "non_consumable"),
		).toBe("sandbox");
