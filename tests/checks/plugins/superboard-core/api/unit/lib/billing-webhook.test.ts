...
import { queueCustomerEntitlementChanged } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/billing";
...
describe("customer entitlement projection webhook", () => {
	it("queues a normalized entitlement mirror event only for subscribed endpoints", async () => {
		...
		const db = createFakeD1((call) => {
			if (call.sql.startsWith("SELECT * FROM billing_customers")) {
				return { id: "customer-1", primary_app_user_id: "reference-production-user-1" };
			}
		...
		expect(payload).toMatchObject({
			...
			app_user_id: "reference-production-user-1",
			transaction_id: "transaction-1",
			reason: "DID_RENEW",
			data: {
				premium: true,
				subscription: true,
				active_subscriptions: ["reference-production_weekly_999"],
			},
		});
	...
	it("keeps every durable delivery pending when initial queue dispatch fails", async () => {
		...
		await expect(
			queueCustomerEntitlementChanged(env, {
				projectId: "project-1",
				customerId: "customer-1",
				environment: "production",
