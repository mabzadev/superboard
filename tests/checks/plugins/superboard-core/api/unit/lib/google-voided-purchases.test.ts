...
import {
	googleVoidedPurchasesUrl,
	reconcileGoogleVoidedPurchases,
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/google-voided-purchases";
import type { BillingEnv } from "../../../../../../../packages/plugins/superboard-core/api/src/types";
...
vi.mock(
	"../../../../../../../packages/plugins/superboard-core/api/src/lib/store-verification",
	() => ({
		googlePlayAccess: providerMocks.access,
	}),
);
vi.mock(
	"../../../../../../../packages/plugins/superboard-core/api/src/lib/billing",
	async (loadOriginal) => ({
		...(await loadOriginal<
			typeof import("../../../../../../../packages/plugins/superboard-core/api/src/lib/billing")
		>()),
...
describe("Google Voided Purchases reconciliation", () => {
	it("builds a bounded request for subscriptions, partial refunds, and token pagination", () => {
		const first = new URL(
			googleVoidedPurchasesUrl("com.example.app", { startTime: 1000, endTime: 2000 }),
		);
		...
		const next = new URL(
			googleVoidedPurchasesUrl("com.example.app", { startTime: 1000, endTime: 2000 }, "page-2"),
		);
	...
	it("paginates verified provider results and applies matched voids through the canonical purchase pipeline", async () => {
		providerMocks.access.mockResolvedValue({
			token: "google-token",
			packageName: "com.example.app",
		});
		providerMocks.apply.mockResolvedValue({
			transactionId: "refund-transaction",
			duplicate: false,
		});
		...
		vi.stubGlobal(
			...
			vi.fn(async (input: string | URL | Request) => {
				...
				return Response.json(
					url.searchParams.get("pageSelection.token")
		...
		expect(providerMocks.apply).toHaveBeenCalledWith(
			expect.anything(),
	...
	it("never changes an entitlement when the verified provider event cannot match a local transaction", async () => {
		...
		const result = await reconcileGoogleVoidedPurchases(
			{ DB: db } as BillingEnv,
			"11",
			Date.now() - 1_000,
		);
...
function reconciliationDatabase(
	...
	return createFakeD1((call) => {
		if (call.op === "first" && call.sql.includes("SELECT watermark_ms"))
			return { watermark_ms: null };
		if (call.op === "first" && call.sql.includes("FROM billing_transactions t"))
			return transaction();
		if (call.op === "all" && call.sql.includes("event_type = 'voided_purchase_detected'"))
			return [];
		if (call.op === "run") {
