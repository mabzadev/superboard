...
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/device-certification";
...
describe("authenticated device certification evidence", () => {
	...
	it("stores a verified identity result with structured assertions and a digest", async () => {
		...
		const db = createFakeD1((call) => {
			if (call.op === "first" && call.sql.includes("FROM billing_certification_device_results"))
				return null;
			if (call.op === "first" && call.sql.includes("FROM billing_certification_runs"))
				return {
					id: "run-device-1",
					...
					challenge_expires_at: "2026-08-04T16:00:00.000Z",
		...
		const result = await recordDeviceCertificationResult(
			db,
			{
		...
		const insert = db.calls.find(
			(call) =>
				call.op === "run" && call.sql.includes("INSERT INTO billing_certification_device_results"),
		)!;
		...
		await expect(
			recordDeviceCertificationResult(
				db,
				{
					...
					runId: "run-device-1",
					challenge: "challenge",
					checkKey: "cross_platform.signed_customer_info",
					outcome: "passed",
					customerId: "customer-1",
					projectId: "11",
					sourcePlatform: "ios",
			...
			code: "device_certification_assertions_incomplete",
		});
	...
	it("rejects evidence from the wrong provider platform", async () => {
		...
		await expect(
			recordDeviceCertificationResult(
				db,
				{
					id: "device-result-105",
					runId: "run-device-2",
					challenge,
					checkKey: "apple.restore",
					outcome: "failed",
					customerId: "customer-1",
					projectId: "11",
					...
					applicationIdentifier: "com.example.app",
					buildNumber: "104",
					assertions: { restore_completed: false },
				},
				now,
			),
		).rejects.toMatchObject({ code: "device_certification_platform_mismatch" });
	});

	it("defines an assertion schema for every check that accepts SDK evidence", () => {
		expect(requiredDeviceCertificationAssertions("apple.network_loss")).toContain(
			"outbox_retained",
		);
		expect(requiredDeviceCertificationAssertions("apple.weekly_purchase")).toEqual([
			"purchase_completed",
			"provider_transaction_verified",
			"premium_entitlement_active",
			"transaction_finalized_once",
		]);
		expect(requiredDeviceCertificationAssertions("google.refund")).toEqual([
			"refund_observed",
			"entitlement_revoked",
	...
	it("accepts authenticated SDK evidence for provider-backed native checks", async () => {
		const challenge = "challenge-native-provider-result";
		const challengeHash = await sha256(challenge);
		const db = createFakeD1((call) => {
			if (call.op === "first" && call.sql.includes("FROM billing_certification_device_results"))
				return null;
		...
		await expect(
			recordDeviceCertificationResult(
				db,
				{
					id: "device-result-native-104",
					runId: "run-device-native",
					challenge,
					checkKey: "apple.weekly_purchase",
					outcome: "passed",
					customerId: "customer-1",
					projectId: "11",
					sourcePlatform: "ios",
					...
					assertions: {
						...
						provider_transaction_verified: true,
						premium_entitlement_active: true,
						transaction_finalized_once: true,
					},
				},
				now,
			),
