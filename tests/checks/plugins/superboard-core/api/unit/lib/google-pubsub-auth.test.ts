...
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/google-pubsub-auth";
...
describe("Google Pub/Sub authenticated push", () => {
	it("verifies the Google signature, issuer, audience, email, and email verification claim", async () => {
		...
		await expect(
			authenticateGooglePubSub(
				env,
				new Request(`${audience}?token=legacy`, {
					method: "POST",
	...
	it("rejects an otherwise valid token with the wrong audience", async () => {
		...
		await expect(
			authenticateGooglePubSub(
				environment(new Map()),
				new Request("https://api.example.test/api/v1/iap/google/10", {
					method: "POST",
					headers: { Authorization: `Bearer ${token}` },
				}),
				{
	...
	it("keeps the legacy token header-only during migration", async () => {
		...
		await expect(
			authenticateGooglePubSub(
				...
				new Request("https://api.example.test/api/v1/iap/google/10", {
					method: "POST",
					headers: { "X-Goog-Channel-Token": "migration-token" },
				}),
				{
					instanceId: 10,
					expectedEmail: null,
	...
	it("rejects a legacy verification token placed in the URL", async () => {
		...
		await expect(
			authenticateGooglePubSub(
				env,
				new Request("https://api.example.test/api/v1/iap/google/10?token=migration-token", {
					method: "POST",
				}),
				{
					instanceId: 10,
	...
	it("records release readiness only after OIDC authentication", async () => {
		...
		await persistGooglePubSubAuthentication(db, "101", {
			mode: "legacy_token",
			email: null,
			audience: "https://api.example.test/api/v1/iap/google/10",
			subject: null,
		});
