...
import { app } from "../../../../../../../packages/plugins/superboard-core/api/src/index";
import { signToken } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/crypto";
import {
	extractDomainRoute,
	resolveAuthorizedProjectContext,
	resolveSdkProjectContext,
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/domain-modules";
import type { Env } from "../../../../../../../packages/plugins/superboard-core/api/src/types";
import { createFakeD1, type FakeD1Call } from "../../helpers/fake-d1";
...
describe("domain module route contract", () => {
	...
	])("maps %s to the exact internal route", (url, projectHeader, projectRef, internalPath) => {
		const headers = projectHeader ? { "X-Project-Id": projectHeader } : undefined;
		expect(extractDomainRoute(new Request(url, { headers }), "marketing")).toEqual({
			ok: true,
			projectRef,
			internalPath,
		});
	...
	it("rejects ambiguous project context", () => {
		const result = extractDomainRoute(
			new Request("https://api.test/api/v1/marketing/projects/10-prod/statistics", {
	...
	it("resolves the numeric D1 project id while authorizing the instance", async () => {
		...
		await expect(resolveAuthorizedProjectContext(db, 7, "10-prod")).resolves.toEqual({
			ok: true,
			context: {
				projectId: 11,
				projectRef: "10-prod",
...
describe("gateway to domain service binding", () => {
	it("rejects the retired email bridge on module routes", async () => {
	...
	it("forwards a signed, canonical context and fixes the root route", async () => {
		...
		expect(internalRequest?.url).toBe("https://onboardings.internal/internal/v1");
	...
	it("keeps legacy dashboard traffic on the old Worker until this project is verified and enabled", async () => {
		...
		expect(internalRequest?.url).toBe("https://paywalls.internal/internal/v1/statistics");
	});
	...
	it("preserves the canonical project-ref segment for native Flows admin routes", async () => {
		...
		await expect(
			verifyInternalProjectContextRequest(internalRequest!, FLOWS_INTERNAL_SECRET, "flows"),
		).resolves.toMatchObject({
			ok: true,
			context: { projectRef: "10-prod", module: "flows" },
		});
		await expect(
			verifyInternalProjectContextRequest(internalRequest!, INTERNAL_SECRET, "flows"),
		).resolves.toMatchObject({
			ok: false,
...
describe("Access Key SDK module gateway", () => {
	it.each([
		...
		async ({ moduleName, bindingName, publicPath, internalPath }) => {
			...
			expect(response.status).toBe(200);
			expect(internalRequest?.url).toBe(`https://${moduleName}.internal${internalPath}`);
	...
	it.each([
		["ios", "com.superboard.ios"],
		["web", "app.superboard.test"],
	])("validates the configured %s application identity", async (platform, identifier) => {
		const testEnv = sdkGatewayEnv({});
		...
		await expect(resolveSdkProjectContext(testEnv.DB, request)).resolves.toMatchObject({
			ok: true,
	...
	it.each([
		...
		["paywalls", "PAYWALLS_MODULE", "/api/v1/paywalls/events", "/internal/v1/events"],
		["analytics", "ANALYTICS_MODULE", "/api/v1/analytics/events", "/internal/v1/events"],
		["onboardings", "ONBOARDINGS_MODULE", "/api/v1/onboardings/events", "/internal/v1/events"],
	] as const)(
		...
		async (moduleName, bindingName, publicPath, internalPath) => {
			...
			expect(forwarded.url).toBe(`https://${moduleName}.internal${internalPath}`);
...
describe("public Flows SDK gateway", () => {
	it.each([
		...
		async (method, publicPath, internalPath) => {
			...
			expect(forwarded?.headers.get("x-flows-version")).toBe("@superboard/flows-js@1.0.0");
			expect(forwarded?.headers.get("x-superboard-flows-sdk-key")).toBe("environment-secret");
			expect(forwarded?.headers.get("upgrade")).toBe(method === "GET" ? "websocket" : null);
		},
	);
...
describe("public Marketing gateway", () => {
	it.each([
		...
		["GET", "/api/v1/marketing/opt-in/confirmation-token", "/public/v1/opt-in/confirmation-token"],
		["POST", "/api/v1/marketing/opt-in/confirmation-token", "/public/v1/opt-in/confirmation-token"],
		[
			"POST",
			"/api/v1/marketing/provider-webhooks/provider-1",
			"/public/v1/provider-webhooks/provider-1",
		],
	])("forwards %s %s without a Dashboard session", async (method, publicPath, internalPath) => {
		let forwarded: Request | undefined;
		const environment = gatewayEnv({
			MARKETING_MODULE: {
				fetch: vi.fn(async (request: Request) => {
		...
		expect(new URL(forwarded!.url).pathname + new URL(forwarded!.url).search).toBe(internalPath);
		expect(forwarded?.headers.get("x-webhook-secret")).toBe("provider-secret");
		expect(forwarded?.headers.get("authorization")).toBeNull();
	});
});
...
describe("public Support realtime gateway", () => {
	it("forwards a one-use realtime ticket without a Dashboard bearer token", async () => {
		...
		expect(response.status).toBe(204);
		expect(new URL(forwarded!.url).pathname).toBe("/public/v1/realtime/signed-ticket");
	...
	it("forwards the authenticated application Support API through api.<app>", async () => {
		...
		expect(response.status).toBe(200);
		const target = new URL(forwarded!.url);
		expect(`${target.pathname}${target.search}`).toBe("/v1/conversations?limit=25");
		expect(forwarded?.headers.get("authorization")).toBe("Bearer application-identity");
...
async function accessToken(environment: Env): Promise<string> {
	return signToken({ sub: 7, instanceId: 10, type: "access" }, environment, "2h");
}
...
function sdkGatewayEnv(
	...
	const db = createFakeD1((call: FakeD1Call) => {
		if (call.sql.includes("FROM instances i") && call.sql.includes("INNER JOIN projects p")) {
		...
		if (call.sql.includes("FROM flows_legacy_cutover_state")) {
			return (options.flowsCutover ?? Boolean(overrides.FLOWS_MODULE)) ? { enabled: 1 } : null;
		}
...
function sdkHeaders(overrides: Record<string, string> = {}): Record<string, string> {
	return {
		Host: "sdk.test",
		"PROJECT-KEY": "access-key",
		PLATFORM: "android",
		IDENTIFIER: "com.superboard.app",
		"Content-Type": "application/json",
		...overrides,
	};
}
