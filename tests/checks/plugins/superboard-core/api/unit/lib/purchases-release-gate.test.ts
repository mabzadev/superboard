...
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/purchases-release-gate";
...
describe("Purchases release gate", () => {
	...
	it("rejects a native provider observation without authenticated device evidence", () => {
		...
		const check = buildReleaseGate(passedChecks(), [], observations).checks.find(
	...
	it("accepts a provider-backed native observation with a frozen authenticated device result", () => {
		const check = buildReleaseGate(passedChecks(), [], passedObservations()).checks.find(
			(item) => item.key === "apple.weekly_purchase",
		);
	...
	it("restricts certification runs to the provider platform and test environment", () => {
		...
		expect(certificationRunCompatibility(inventory, "cross_platform", "production").valid).toBe(
			true,
		);
	});
	...
	it("requires structured device evidence for native and FlutterFlow checks", () => {
		...
		expect(validateReleaseGateEvidence(native, { reference: "test-run-1" })).toEqual({
			valid: false,
			missing: ["build", "device"],
		});
	});

	it("does not accept paywall telemetry as native purchase certification", () => {
		expect(
			RELEASE_GATE_CHECKS.find((check) => check.key === "apple.pending")?.reference_types,
		).toEqual(["test_run"]);
		expect(
			RELEASE_GATE_CHECKS.find((check) => check.key === "google.restore")?.reference_types,
		).toEqual(["test_run"]);
	});
