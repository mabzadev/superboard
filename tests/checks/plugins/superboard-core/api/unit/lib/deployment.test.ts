...
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/deployment";
...
describe("deployment access policy", () => {
	...
	it("allows an active email case-insensitively in its exact realm", async () => {
		const env = environment({
			realm: "reference-production:production",
			rows: [{ realm: "reference-production:production", email: "owner@example.com", active: 1 }],
		});
	...
	it("denies absent, revoked, and cross-realm emails", async () => {
		const env = environment({
			...
			rows: [
				{ realm: "reference-production:production", email: "owner@example.com", active: 1 },
	...
	it("keeps public-mode compatibility without reading D1", async () => {
		await expect(
			isRegistrationAllowed(environment({ mode: "public" }), "any@example.com"),
		).resolves.toBe(true);
	});

	it("records successful allowlisted registrations in the normalized realm entry", async () => {
		const env = environment({ realm: "reference-production:production" });
		await recordSuccessfulRegistration(env, " Owner@Example.com ");
		const update = env.DB.calls.find((call) => call.op === "run");
		expect(update?.args).toEqual(["reference-production:production", "owner@example.com"]);
	});
