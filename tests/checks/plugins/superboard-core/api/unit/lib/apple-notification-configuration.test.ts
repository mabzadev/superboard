...
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/apple-notification-configuration";
...
describe("Apple notification configuration", () => {
	it("builds project-scoped HTTPS ingress URLs for both environments", () => {
		expect(requiredAppleNotificationUrls("api.example.com", "20", "21")).toEqual({
			production_url:
				"https://api.example.com/api/v2/purchases/providers/webhooks/apple/production/20",
			sandbox_url: "https://api.example.com/api/v2/purchases/providers/webhooks/apple/sandbox/21",
			production_version: "V2",
			sandbox_version: "V2",
		});
		expect(() => requiredAppleNotificationUrls("http://api.example.com", "20", "21")).toThrow(
			"public API domain must be an HTTPS origin",
		);
	});
	...
	it("patches both V2 URLs in one explicit provider update and verifies the result", async () => {
					...
					data: {
						...
						attributes: {
							...
							subscriptionStatusUrlForSandbox:
								"https://api.example.com/api/v2/purchases/providers/webhooks/apple/sandbox/21",
							subscriptionStatusUrlVersion: "V2",
		...
		await expect(
			configureAppleNotificationConfiguration(baseEnv(), "20", "21"),
	...
	it("refreshes configured projects once per six-hour period", async () => {
		...
		await expect(refreshAppleNotificationConfigurationsIfDue(env, now)).resolves.toMatchObject({
			skipped: true,
		});
		expect(fetch).toHaveBeenCalledOnce();
	});
