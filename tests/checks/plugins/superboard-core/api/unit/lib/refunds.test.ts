...
import type { VerifiedPurchase } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/billing";
import {
	recordRefundCaseForPurchase,
	refundProjectionVersion,
} from "../../../../../../../packages/plugins/superboard-core/api/src/lib/refunds";
...
function purchase(overrides: Partial<VerifiedPurchase> = {}): VerifiedPurchase {
	return {
		...
		storeProductId: "reference-production_weekly_999",
...
describe("Refund Center projection", () => {
	...
	it("creates a lost refund case and immutable provider audit event", async () => {
		...
		expect(
			writes.some((write) =>
				write.sql.startsWith("INSERT OR IGNORE INTO billing_refund_audit_events"),
			),
		).toBe(true);
	});
	...
	it("prepares Apple consumption information for human approval", async () => {
		...
		const deadline = writes.find(({ sql }) => sql.startsWith("INSERT INTO billing_refund_cases"))
			?.args[12];
	...
	it("does not extend an Apple consumption deadline when the provider event is replayed later", async () => {
		...
		try {
			...
			const db = createFakeD1((call) => {
				...
				if (
					call.sql.startsWith("SELECT id, status, projection_version FROM billing_refund_cases")
				) {
			...
			expect(deadlines).toEqual(["2026-08-04T20:00:00.000Z", "2026-08-04T20:00:00.000Z"]);
		} finally {
			vi.useRealTimers();
		}
	});
	...
	it("opens a Google chargeback review with a 24-hour deadline and a human-approved draft", async () => {
		...
		const { recordGooglePendingRefundReview } =
			await import("../../../../../../../packages/plugins/superboard-core/api/src/lib/refunds");
		const result = await recordGooglePendingRefundReview({ DB: db } as unknown as Env, {
			projectId: "project-1",
			environment: "production",
			eventOccurredAt,
			payload: {
		...
		const caseInsert = writes.find(({ sql }) => sql.startsWith("INSERT INTO billing_refund_cases"));
	...
	it("does not reopen a terminal case from an older provider event", async () => {
		...
		expect(writes.some(({ sql }) => sql.startsWith("INSERT INTO billing_refund_deadlines"))).toBe(
			false,
		);
		expect(
			writes.some(({ sql }) =>
		...
		expect(
			writes.some(({ sql }) => sql.startsWith("INSERT OR IGNORE INTO billing_refund_audit_events")),
		).toBe(true);
	});
	...
	it("closes open deadlines and unsent actions after a terminal provider outcome", async () => {
		...
		expect(writes.some((sql) => sql.startsWith("UPDATE billing_refund_provider_actions"))).toBe(
			true,
		);
	});
});
