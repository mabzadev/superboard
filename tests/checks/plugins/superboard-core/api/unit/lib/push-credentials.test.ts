...
import { migrateLegacyPushCredentials } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/push-credentials";
import { decryptCredential } from "../../../../../../../packages/plugins/superboard-core/api/src/lib/secrets";
...
function envWith(rows: Record<string, Array<Record<string, unknown>>>) {
	...
	const db = createFakeD1((call) => {
		if (call.op === "all" && call.sql.includes("FROM ios_push_configurations"))
			return rows.ios || [];
		if (call.op === "all" && call.sql.includes("FROM android_push_configurations"))
			return rows.android || [];
		...
		if (call.op === "all" && call.sql.includes("encrypted_legacy_credentials"))
			return rows.legacy || [];
...
describe("legacy push credential migration", () => {
	it("encrypts all supported legacy credential locations and clears plaintext atomically", async () => {
		...
		const cleartexts = await Promise.all(
			updates.map((update) => decryptCredential(env, String(update.args[0]))),
		);
