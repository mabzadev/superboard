...
import worker, {
	allowedEmbeddableSdkOrigin,
} from "../../../../../../packages/plugins/superboard-core/api/src/index";
...
function env(overrides: Partial<Env> = {}): Env {
	const db = createFakeD1((call: FakeD1Call) => {
		...
		if (call.op === "all" && call.sql.includes("FROM application_account_erasures")) return [];
		if (call.op === "all" && call.sql.includes("FROM analytics_fact_outbox")) return [];
		if (call.op === "all" && call.sql.includes("FROM projects production")) return [];
		if (call.op === "all" && call.sql.includes("FROM rpush_notifications rn JOIN rpush_apps ra"))
			return [];
		if (call.op === "all" && call.sql.includes("encrypted_")) return [];
		if (call.op === "first" && call.sql.includes("FROM instances WHERE api_key = ?"))
			return { id: 10 };
		if (
			call.op === "first" &&
...
describe("Flows SDK CORS", () => {
	it("allows exact HTTP(S) embedding origins and rejects malformed origins", () => {
		expect(allowedEmbeddableSdkOrigin("https://customer.example")).toBe("https://customer.example");
		expect(allowedEmbeddableSdkOrigin("http://localhost:3000")).toBe("http://localhost:3000");
		expect(allowedEmbeddableSdkOrigin("https://customer.example/")).toBe(
			"https://customer.example",
		);
		expect(allowedEmbeddableSdkOrigin("https://customer.example/path")).toBeUndefined();
...
describe("Worker scheduled and queue handlers", () => {
	it("reports real API dependency readiness and fails closed when D1 is unavailable", async () => {
		const readyDb = createFakeD1((call) => {
			if (call.op === "first" && call.sql === "SELECT 1 AS ready") return { ready: 1 };
	...
	it("routes the files custom domain only to the authenticated Files surface", async () => {
		...
		expect(forwarded!.headers.get("authorization")).toBe("Bearer application-token");
	});
	...
	it("serves the Melody RS256 JWKS on the dedicated auth domain", async () => {
		...
		expect(forwarded!.url).toBe("https://identity.internal/.well-known/jwks.json");
		expect(forwarded!.headers.get("x-superboard-auth-gateway")).toBe("1");
	});
	...
	it("acks queue messages after successful job dispatch", async () => {
		...
		await worker.queue?.(
			{ queue: "superboard-push", messages: [message] } as any,
			env(),
			{} as any,
		);
	...
	it("retries queue messages after failed job dispatch", async () => {
		...
		await worker.queue?.(
			{ queue: "superboard-maintenance", messages: [message] } as any,
			env(),
			{} as any,
		);
	...
	it("persists and redacts a DLQ message before acknowledging it", async () => {
		...
		expect(inserted?.args[4]).toBe('{"type":"maintenance.run","secret":"[REDACTED]"}');
