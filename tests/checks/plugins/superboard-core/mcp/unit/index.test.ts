...
import worker, { type Env } from "../../../../../../packages/plugins/superboard-core/mcp/src/index";

function env(validateStatus = 200): Env {
	return {
		ENVIRONMENT: "development",
		SUPERBOARD_TARGET: "reference",
		SUPERBOARD_INSTANCE_ID: "reference",
		...
		API_SERVICE: {
			fetch: vi.fn(async () =>
				Response.json(validateStatus === 200 ? { valid: true } : { error: "invalid_token" }, {
...
describe("SuperBoard MCP Worker", () => {
	...
	it("publishes OAuth protected-resource discovery for the API authorization server", async () => {
		const response = await worker.fetch(mcpRequest("/.well-known/oauth-protected-resource"), env());
	...
	it("challenges unauthenticated MCP requests with this resource's metadata", async () => {
		const response = await worker.fetch(mcpRequest("/mcp", { method: "POST" }), env());
