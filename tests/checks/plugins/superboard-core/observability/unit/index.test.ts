...
import worker from "../../../../../../packages/plugins/superboard-core/observability/src/index";
...
describe("SuperBoard observability Worker", () => {
	...
	it("queries a bounded, whitelisted Analytics Engine window", async () => {
		const analyticsFetch = vi.fn(async (_input: RequestInfo | URL, init?: RequestInit) => {
			expect(String(init?.body)).toContain("INTERVAL '15' MINUTE");
			expect(String(init?.body)).toContain("FROM superboard_mbza_development");
		...
		expect(await response?.json()).toMatchObject({
			status: "ok",
			windowMinutes: 15,
			rows: [{ service: "superboard-api-dev", invocations: 4, averageCpuMs: 2.5 }],
		});
	});
});

function environment(points: AnalyticsEngineDataPoint[], overrides: Record<string, unknown> = {}) {
	return {
		ANALYTICS: {
		...
		ANALYTICS_DATASET: "superboard_mbza_development",
