...
const gate = resolve(root, "packages/plugins/superboard-support/scripts/parity-check.mjs");
const publicationGate = resolve(
	root,
	"packages/plugins/superboard-support/scripts/public-leak-check.mjs",
);
...
test("the internal Support reference has exactly 758 mapped route records", () => {
	const manifest = JSON.parse(
		readFileSync(
			resolve(
				root,
				"packages/plugins/superboard-support/scripts/support-audit/reference-routes.json",
			),
			"utf8",
		),
	);
...
test("the Support parity and publication gates pass", () => {
	for (const script of [
		"packages/plugins/superboard-support/scripts/parity-check.mjs",
		"packages/plugins/superboard-support/scripts/public-leak-check.mjs",
	]) {
...
test("the publication gate rejects an internal audit fingerprint in Site artifacts", (context) => {
	...
	const auditDirectory = join(
		fixtureRoot,
		"packages/plugins/superboard-support/scripts/support-audit",
	);
...
test("the publication gate rejects migration vocabulary from a published Support surface", (context) => {
	...
	const supportDirectory = join(
		fixtureRoot,
		"packages/plugins/superboard-support/src/front/components/modules",
	);
	mkdirSync(auditDirectory, { recursive: true });
...
function parityFixture({ source, tokens }) {
	...
	const sourceDirectory = join(fixtureRoot, "packages/plugins/superboard-support/worker/src");
	const testDirectory = join(fixtureRoot, "tests/checks/plugins/superboard-support/worker/unit");
	mkdirSync(auditDirectory, { recursive: true });
	...
	const proof = {
		file: "tests/checks/plugins/superboard-support/worker/unit/domain.test.ts",
				...
				capabilities: [
					{
						...
						authority: "Fixture",
						dimensions: {
							behavior: ["packages/plugins/superboard-support/worker/src/domain.ts"],
							persistence: ["packages/plugins/superboard-support/worker/src/domain.ts"],
							asynchronous: ["packages/plugins/superboard-support/worker/src/domain.ts"],
							rbac: ["packages/plugins/superboard-support/worker/src/domain.ts"],
							interface: ["packages/plugins/superboard-support/worker/src/domain.ts"],
							tests: [proof],
						},
					},
				],
