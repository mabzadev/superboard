...
import { render, screen, waitFor, within } from "../../../../../packages/supbrd-front-ui/render.js";
...
vi.mock(
	"../../../../../../../packages/plugins/superboard-support/src/front/api/support/integrationsService.js",
	() => ({
...
import SupportIntegrationsPage from "../../../../../../../packages/plugins/superboard-support/src/front/components/modules/SupportIntegrationsPage.js";
...
describe("Support native integrations", () => {
	...
	it("keeps integration settings in their own page without an extra Settings link", () => {
	...
	it("creates then configures an HTTPS webhook and clears its secret", async () => {
		...
		await user.type(screen.getByLabelText("Endpoint URL"), "https://hooks.example.com/incidents");
		...
		expect(services.saveCredentials).toHaveBeenCalledWith("project-1", "webhook-new", {
		...
		expect(notifications.success).toHaveBeenCalledWith("Support integration configured");
	});
	...
	it("rejects unsafe webhook endpoints before creating an integration", async () => {
		...
		await user.type(screen.getByLabelText("Endpoint URL"), "https://127.0.0.1/webhook");
		await user.type(screen.getByLabelText("Signing secret"), "x".repeat(32));

		expect(screen.getByText(/Use a public HTTPS URL without credentials/i)).toBeInTheDocument();
		expect(screen.getByRole("button", { name: "Add and configure" })).toBeDisabled();
	...
	it("keeps a partially created webhook in configuration_required without retaining the failed secret", async () => {
		const user = userEvent.setup();
		services.saveCredentials.mockRejectedValueOnce(new Error("Credential vault unavailable"));
		...
		expect(
			await screen.findByText(/The integration was created, but its credentials were not saved/i),
		).toBeInTheDocument();
	...
	it("masks configured credentials and replaces them without reading a secret", async () => {
		...
		expect(screen.getByLabelText("Stored credentials are masked")).toHaveTextContent("••••••••");
		...
		await user.type(screen.getByLabelText("Endpoint URL"), "https://hooks.example.com/replacement");
		await user.type(replacementSecret, "r".repeat(32));
		await user.click(screen.getByRole("button", { name: "Save securely" }));

		await waitFor(() =>
			expect(services.updateIntegration).toHaveBeenCalledWith("project-1", "webhook-1", {
				settings: {
		...
		expect(services.saveCredentials).toHaveBeenCalledWith("project-1", "webhook-1", {
	...
	it("enters a busy state while a webhook is being configured", async () => {
		...
		await screen.findByText("Escalations");
		await user.click(screen.getByRole("button", { name: "Replace credentials" }));
		await user.type(screen.getByLabelText("New signing secret"), "z".repeat(32));
		await user.click(screen.getByRole("button", { name: "Save securely" }));
	...
	it("does not expose secret values in the rendered configured card", async () => {
		...
		expect(within(card.closest("div") || container).queryByText(/secret/i)).toBeNull();
	...
	it("persists the native Slack action settings before starting authorization", async () => {
		...
		await waitFor(() =>
			expect(services.createIntegration).toHaveBeenCalledWith(
				"project-1",
		...
		expect(services.saveCredentials).toHaveBeenCalledWith("project-1", "slack-new", {
			client_id: "slack-client",
			client_secret: "slack-secret",
			signing_secret: "signing-secret",
		});
		await waitFor(() =>
			expect(services.startOAuth).toHaveBeenCalledWith("project-1", "slack-new", {
				callback_uri: "https://api.example.test/api/v1/support/providers/slack/oauth/callback",
				return_uri: expect.stringMatching(
					/^http:\/\/localhost:\d+\/support\/integrations\?integration=slack-new$/u,
				),
			}),
		);
