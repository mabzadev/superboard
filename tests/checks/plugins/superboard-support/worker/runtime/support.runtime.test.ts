...
import { encryptCredentialPayload } from "../../../../../../packages/plugins/superboard-support/worker/src/secrets";
import { handleSupportQueue } from "../../../../../../packages/plugins/superboard-support/worker/src/webhooks";
...
describe("Support in the Workers runtime", () => {
	it("promotes a configured provider only after a signed inbound event", async () => {
		...
		const credentials = await encryptCredentialPayload(env.SUPPORT_CREDENTIAL_ENCRYPTION_KEY, {
			app_secret: providerSecret,
			verify_token: "runtime-verify-token",
		});
		await env.DB.batch([
			env.DB.prepare(`INSERT INTO support_provider_endpoints
        (id, project_id, provider, display_name, status, settings_json)
        VALUES (?, 12, 'whatsapp_cloud', ?, 'configured', '{}')`).bind(
				endpointId,
				`Runtime signed inbound ${endpointId}`,
			),
			env.DB.prepare(`INSERT INTO support_provider_credentials
        (endpoint_id, project_id, encrypted_payload)
        VALUES (?, 12, ?)`).bind(endpointId, credentials),
		]);
		...
		await expect(
			env.DB.prepare(`SELECT status FROM support_provider_endpoints WHERE id = ?`)
				.bind(endpointId)
				.first(),
		...
		await expect(
			env.DB.prepare(`SELECT status, last_validated_at, last_event_at, last_error_code
      FROM support_provider_endpoints WHERE id = ?`)
				.bind(endpointId)
				.first(),
    ...
    `)
		...
		const unread = await SELF.fetch(
			await signedRequest("/internal/v1/projects/12/notifications/unread-count", 12),
		);
		await expect(unread.json()).resolves.toMatchObject({ data: { unread_count: 1 } });

		const snoozed = await SELF.fetch(
			await signedRequest(
		...
		const hidden = await SELF.fetch(
			await signedRequest("/internal/v1/projects/12/notifications?unread=true", 12),
		);
		await expect(hidden.json()).resolves.toMatchObject({ data: [] });

		const unsnoozed = await SELF.fetch(
			await signedRequest(
				`/internal/v1/projects/12/notifications/${createdBody.data.id}/snooze`,
				12,
				"PATCH",
		...
		const readAll = await SELF.fetch(
			await signedRequest(
		...
		const visible = await SELF.fetch(
			await signedRequest("/internal/v1/projects/12/notifications", 12),
		);
		await expect(visible.json()).resolves.toMatchObject({ data: [] });
	});

	it("reports non-sensitive operational database metrics", async () => {
		const response = await SELF.fetch("https://support.internal/internal/v1/health");
    ...
    `,
		...
		const listed = await SELF.fetch(
			await signedRequest("/internal/v1/projects/12/settings/operations/dead-letters", 12),
		);
		...
		await expect(
			env.DB.prepare(`SELECT status FROM support_dead_letters WHERE id = ?`)
				.bind(deadLetter?.id)
				.first(),
		).resolves.toMatchObject({ status: "discarded" });
		await expect(
			env.DB.prepare(`SELECT action FROM support_operations_audit_events
      WHERE project_id = 12 AND resource_type = 'dead_letter' AND resource_id = ?`)
				.bind(deadLetter?.id)
				.first(),
		).resolves.toMatchObject({ action: "replayed" });
	});
	...
	it("dispatches proactive Support idempotently and applies provider acknowledgements", async () => {
        ...
        VALUES (?, 12, ?, 'whatsapp_cloud', ?, 'validated', '{}')`).bind(
				endpointId,
				inboxId,
				`WhatsApp ${suffix}`,
			),
        ...
        VALUES (?, 12, ?, ?, ?, 1)`).bind(crypto.randomUUID(), contactId, inboxId, externalUserId),
			env.DB.prepare(`INSERT INTO support_provider_credentials
        (endpoint_id, project_id, encrypted_payload)
        VALUES (?, 12, ?)`).bind(endpointId, credentials),
        ...
        (id, project_id, inbox_id, name, campaign_type, message, audience_json,
         status, created_by)
        VALUES (?, 12, ?, ?, 'one_off', 'A proactive Support update', ?, 'running', '2')`).bind(
				campaignId,
				inboxId,
		...
		const dispatch = () =>
			createMessageBatch("support-test-bulk", [
				{
		...
		expect((await getQueueResult(firstBatch, createExecutionContext())).explicitAcks).toEqual([
			firstBatch.messages[0].id,
		]);
		const replayBatch = dispatch();
		await handleSupportQueue(replayBatch, env);
		expect((await getQueueResult(replayBatch, createExecutionContext())).explicitAcks).toEqual([
			replayBatch.messages[0].id,
		]);
    ...
    `)
			.bind(inboxId, inboxId, campaignId, endpointId)
			.first<{
    ...
    `)
			.bind(endpointId)
			.first<{ id: string; conversation_id: string; message_id: string }>();
		expect(providerDelivery).toBeTruthy();
		await expect(
			env.DB.prepare(`SELECT delivery_status FROM messages WHERE id = ?`)
				.bind(providerDelivery?.message_id)
				.first(),
		).resolves.toMatchObject({ delivery_status: "pending" });
      ...
      VALUES (?, 12, ?, 'whatsapp_cloud', ?, 'delivery.updated', ?, ?)`)
			.bind(
				...
				JSON.stringify({
					entry: [
						{
							changes: [
								{ value: { statuses: [{ id: "wamid.runtime.outbound", status: "delivered" }] } },
							],
						},
					],
				}),
		...
		expect((await getQueueResult(receiptBatch, createExecutionContext())).explicitAcks).toEqual([
			receiptBatch.messages[0].id,
		]);
		await expect(
			env.DB.prepare(`SELECT status FROM support_provider_deliveries WHERE id = ?`)
				.bind(providerDelivery?.id)
				.first(),
		...
		await expect(
			env.DB.prepare(`SELECT status FROM support_campaigns WHERE id = ?`).bind(campaignId).first(),
		...
		await env.DB.batch([
			...
			env.DB.prepare("DELETE FROM conversations WHERE project_id = 12 AND inbox_id = ?").bind(
				inboxId,
			),
	...
	it("routes real provider messages through automations and persisted realtime", async () => {
		...
		await env.DB.batch([
			env.DB.prepare(`INSERT INTO support_inboxes
        (id, project_id, name, identifier, channel_type)
        VALUES (?, 12, 'Inbound runtime', ?, 'whatsapp_cloud')`).bind(inboxId, `inbound-${suffix}`),
			env.DB.prepare(`INSERT INTO support_provider_endpoints
        (id, project_id, inbox_id, provider, display_name, status, settings_json)
        VALUES (?, 12, ?, 'whatsapp_cloud', ?, 'validated', '{}')`).bind(
				endpointId,
				inboxId,
				`Inbound WhatsApp ${suffix}`,
			),
			env.DB.prepare(`INSERT INTO support_automation_rules
        (id, project_id, name, event_name, conditions_json, actions_json, created_by)
        VALUES (?, 12, ?, 'message_created', ?, ?, '2')`).bind(
				crypto.randomUUID(),
      ...
      VALUES (?, 12, ?, 'whatsapp_cloud', ?, 'message.created', ?, ?)`)
							...
							changes: [
								{
									value: {
										...
										messages: [
											{ id: `wamid.${suffix}`, from: sender, text: { body: "Please help" } },
										],
									},
								},
							],
		...
		await handleSupportQueue(batch, env);
		expect((await getQueueResult(batch, createExecutionContext())).explicitAcks).toEqual([
			batch.messages[0].id,
		]);
      ...
      WHERE project_id = 12 AND inbox_id = ? AND external_user_id = ?`)
			.bind(inboxId, sender)
			.first<{ id: string; priority: string }>();
		expect(conversation).toMatchObject({ priority: "urgent" });
		await expect(
			env.DB.prepare(`SELECT status FROM support_provider_events WHERE id = ?`)
				.bind(eventId)
				.first(),
		).resolves.toMatchObject({ status: "processed" });
		await expect(
			env.DB.prepare(`SELECT sender_kind, body FROM messages WHERE conversation_id = ?`)
		...
		await env.DB.batch([
			env.DB.prepare(
				"DELETE FROM support_automation_rules WHERE project_id = 12 AND name = ?",
			...
			env.DB.prepare(
				"DELETE FROM support_contacts WHERE project_id = 12 AND external_user_id = ?",
			).bind(sender),
			env.DB.prepare("DELETE FROM support_inboxes WHERE id = ?").bind(inboxId),
		]);
	});
	...
	it("persists isolated conversations and replays idempotent mutations", async () => {
		...
		const list = await SELF.fetch(await signedRequest("/internal/v1/conversations", 12));
		expect(list.status).toBe(200);
		await expect(list.json()).resolves.toMatchObject({
			data: [{ id: created.data.id, project_id: 12 }],
		});

		const otherProject = await SELF.fetch(await signedRequest("/internal/v1/conversations", 11));
    ...
    `,
		...
		const history = await SELF.fetch(
			await signedRequest(`/internal/v1/conversations/${conversationId}/messages`, 12),
		);
		const historyPayload = await history.json<{
			data: Array<{ id: string; attachments_json: string }>;
		}>();
		const migratedShape = historyPayload.data.find((item) => item.id === attachmentPayload.data.id);
		...
		await runInDurableObject(room, async (_instance, state) => {
			...
			const events = state.storage.sql
				...
				.toArray();
			expect(events.map((event) => event.event_type)).toContain("message.created");
			expect(events.map((event) => event.event_type)).toContain("presence.updated");
			for (const event of events) {
				expect(() => parseSupportRealtimeEvent(JSON.parse(event.payload_json))).not.toThrow();
	...
	it("encrypts and rotates webhook secrets without exposing them", async () => {
		...
		await expect(configured.text()).resolves.not.toContain("support-webhook-private-value");
		...
		expect(stored?.encrypted_secret).not.toContain("support-webhook-private-value");
		...
		await expect(
			env.DB.prepare(`SELECT COUNT(*) total FROM support_secret_audit_events WHERE webhook_id = ?`)
				.bind(entity.data.id)
				.first(),
	...
	it("manages contacts and companies with bounded cursor pagination and project isolation", async () => {
		...
		const firstPage = await SELF.fetch(
			await signedRequest("/internal/v1/projects/12/contacts?q=Cursor&limit=2", 12),
		);
		...
		const companySearch = await SELF.fetch(
			await signedRequest("/internal/v1/projects/12/companies?q=northwind&limit=1", 12),
		);
		...
		const detail = await SELF.fetch(
			await signedRequest(`/internal/v1/projects/12/companies/${company.data.id}`, 12),
		);
	...
	it("merges contacts idempotently while preserving labels, inbox identities and audit history", async () => {
        ...
        (id, project_id, name, color) VALUES (?, 12, ?, '#2563eb')`).bind(
				labelId,
				`Priority ${labelId}`,
			),
		]);
        ...
        VALUES (?, 12, ?, 'Keep this note', '2')`).bind(crypto.randomUUID(), source.data.id),
		...
		expect(replay.headers.get("idempotency-replayed")).toBe("true");
		const [sourceCount, targetRelations, conversationCount, audit] = await env.DB.batch([
          ...
          (SELECT COUNT(*) FROM support_contact_notes WHERE project_id = 12 AND contact_id = ?) notes`).bind(
				target.data.id,
				target.data.id,
				target.data.id,
			),
          ...
          WHERE project_id = 12 AND resource_type = 'contact' AND resource_id = ? AND action = 'merged'`).bind(
				target.data.id,
			),
		]);
	...
	it("binds providers to an active same-project inbox and validates runtime delivery fields", async () => {
      ...
      VALUES (?, 12, ?, ?, 'twilio_voice', 'active')`)
			.bind(inboxId, `Voice ${suffix}`, `voice-${suffix}`)
			.run();

		const missingInbox = await SELF.fetch(
			await signedRequest(
		...
		const mismatchedInbox = await SELF.fetch(
			await signedRequest(
				"/internal/v1/projects/12/providers",
				12,
				"POST",
	...
	it("routes integration authorization through the native project and public callback surfaces", async () => {
		...
		const encrypted = await encryptCredentialPayload("support-runtime-credential-encryption-key", {
			client_id: "route-client",
			client_secret: "route-secret",
		});
		await env.DB.batch([
			env.DB.prepare(`INSERT INTO support_integrations
        (id, project_id, provider, display_name, status, settings_json)
        VALUES (?, 12, 'linear', ?, 'configuration_required', ?)`).bind(
				integrationId,
				`OAuth route ${suffix}`,
				JSON.stringify({
        ...
        (integration_id, project_id, encrypted_payload) VALUES (?, 12, ?)`).bind(
				integrationId,
				encrypted,
			),
		]);
		const startedResponse = await SELF.fetch(
			await signedRequest(
		...
		await expect(
			env.DB.prepare(`SELECT status FROM support_integrations
      WHERE id = ? AND project_id = 12`)
				.bind(integrationId)
				.first(),
	...
	it("persists contact import and export jobs before enqueueing them on the bulk queue", async () => {
		...
		await env.ATTACHMENTS.put(
			exportStorageKey,
			JSON.stringify({ data: [{ id: "contact-exported" }] }),
			{
				httpMetadata: { contentType: "application/json" },
			},
		);
		await env.DB.prepare(`UPDATE support_export_jobs SET status = 'completed', storage_key = ?
      WHERE project_id = 12 AND id = ?`)
			.bind(exportStorageKey, exported.data.id)
			.run();
		const exportStatus = await SELF.fetch(
			await signedRequest(`/internal/v1/projects/12/contacts/exports/${exported.data.id}`, 12),
		);
		...
		expect(download.headers.get("content-disposition")).toContain(
			`support-contacts-${exported.data.id}.json`,
		);
	...
	it("supports native conversation mute, transfer, bulk actions, merge and secret-free transcripts", async () => {
        ...
        (id, project_id, name) VALUES (?, 12, ?)`).bind(teamId, `Operations ${teamId}`),
			env.DB.prepare(`INSERT INTO support_labels
        (id, project_id, name, color) VALUES (?, 12, ?, '#0f766e')`).bind(
				labelId,
				`Escalated ${labelId}`,
			),
		]);
		...
		const inboxPage = await SELF.fetch(
			await signedRequest("/internal/v1/projects/12/conversations?q=transcript&limit=1", 12),
		);
		...
		const invalidInboxLimit = await SELF.fetch(
			await signedRequest("/internal/v1/projects/12/conversations?limit=101", 12),
		);
		...
		const [attachment, source, audit] = await env.DB.batch([
			env.DB.prepare(`SELECT conversation_id FROM support_message_attachments WHERE id = ?`).bind(
				attachmentId,
			),
        ...
        FROM conversations WHERE project_id = 12 AND id = ?`).bind(sourceId),
			env.DB.prepare(`SELECT COUNT(*) total FROM support_operations_audit_events
        WHERE project_id = 12 AND resource_type = 'conversation' AND resource_id = ? AND action = 'merged'`).bind(
				targetId,
			),
		]);
	...
	it("authenticates the public widget, isolates its endpoint project, and issues a user realtime ticket", async () => {
		...
		const encryptedPayload = await encryptCredentialPayload(env.SUPPORT_CREDENTIAL_ENCRYPTION_KEY, {
    ...
    `,
		...
		const items = await SELF.fetch(
			await signedRequest("/internal/v1/items?type=conversation&status=open", 12),
		);
		...
		expect(projected.data.find((item) => item.source_id === conversationId)).toMatchObject({
...
it("report exports honor date bounds and project isolation in the actual queue consumer", async () => {
	...
	await env.DB.batch(
		fixtures.map(([marker, projectId, occurredAt]) =>
			env.DB.prepare(
	...
	const requested = await SELF.fetch(
		await signedRequest(
			"/internal/v1/projects/12/reports/exports",
			12,
			"POST",
			{ filters: { from: "2035-04-06", to: "2035-04-06" } },
			crypto.randomUUID(),
		),
	);
	expect(requested.status).toBe(202);
	const result = await requested.json<{ data: { id: string } }>();
	const batch = createMessageBatch("support-test-bulk", [
		{
			id: crypto.randomUUID(),
			timestamp: new Date(),
			attempts: 1,
			body: { type: "support.export.requested.v1", projectId: 12, exportId: result.data.id },
		},
	]);
	await handleSupportQueue(batch, env);
	expect((await getQueueResult(batch, createExecutionContext())).explicitAcks).toHaveLength(1);
	const download = await SELF.fetch(
		await signedRequest(`/internal/v1/projects/12/reports/exports/${result.data.id}/download`, 12),
	);
	expect(download.status).toBe(200);
	const exported = await download.json<{ data: Array<{ dimensions_json: string }> }>();
	expect(exported.data).toHaveLength(3);
	expect(exported.data.map((row) => JSON.parse(row.dimensions_json).marker)).toEqual(
		expect.arrayContaining(["late", "offset", "start"]),
	);
});

it("isolates Inbox reports and Help Center lists before pagination", async () => {
	const create = async (resource: string, body: unknown) => {
		const response = await SELF.fetch(
			await signedRequest(
				`/internal/v1/projects/12/${resource}`,
				12,
				"POST",
				body,
				crypto.randomUUID(),
	...
	const firstInbox = await create("workforce/inboxes", {
		name: "First",
		identifier: crypto.randomUUID(),
		channel_type: "api",
		status: "active",
	});
	const secondInbox = await create("workforce/inboxes", {
		name: "Second",
		identifier: crypto.randomUUID(),
		channel_type: "api",
		status: "active",
	});
	for (const inboxId of [firstInbox, secondInbox]) {
		await env.DB.prepare(
			"INSERT INTO conversations(id,project_id,external_user_id,client_conversation_id,subject,inbox_id) VALUES(?,?,?,?,?,?)",
		)
			.bind(crypto.randomUUID(), 12, "scope-user", crypto.randomUUID(), "Scoped question", inboxId)
			.run();
	}
	const report = await SELF.fetch(
		await signedRequest(`/internal/v1/projects/12/reports?inbox_id=${firstInbox}`, 12),
	);
	expect(report.status).toBe(200);
	expect(
		(await report.json<{ data: { totals: { conversations: number } } }>()).data.totals
			.conversations,
	).toBe(1);

	const exportedResponse = await SELF.fetch(
		await signedRequest(
			"/internal/v1/projects/12/reports/exports",
			12,
			"POST",
			{ filters: { inbox_id: firstInbox } },
			crypto.randomUUID(),
		),
	);
	expect(exportedResponse.status).toBe(202);
	const exportId = (await exportedResponse.json<{ data: { id: string } }>()).data.id;
	const exportBatch = createMessageBatch("support-test-bulk", [
		{
			id: crypto.randomUUID(),
			timestamp: new Date(),
			attempts: 1,
			body: { type: "support.export.requested.v1", projectId: 12, exportId },
		},
	]);
	await handleSupportQueue(exportBatch, env);
	const download = await SELF.fetch(
		await signedRequest(`/internal/v1/projects/12/reports/exports/${exportId}/download`, 12),
	);
	expect(download.status).toBe(200);
	expect((await download.json<{ data: unknown[] }>()).data).toHaveLength(1);
	const excludedRule = await create("automations", {
		name: "Not this Inbox",
		event_name: "conversation.created",
		condition_mode: "all",
		conditions: [{ field: "inbox_id", operator: "not_equals", value: firstInbox }],
		actions: [{ type: "set_priority", value: "high" }],
		position: 0,
		active: true,
	});
	const rules = await SELF.fetch(
		await signedRequest(`/internal/v1/projects/12/automations?inbox_id=${firstInbox}`, 12),
	);
	expect((await rules.json<{ data: { id: string }[] }>()).data.map((row) => row.id)).not.toContain(
		excludedRule,
	);

	const scopedRule = await create("automations", {
		name: "Only first Inbox",
		event_name: "conversation.created",
		condition_mode: "all",
		conditions: [{ field: "inbox_id", operator: "equals", value: firstInbox }],
		actions: [{ type: "set_priority", value: "high" }],
		position: 0,
		active: true,
	});
	const removed = await SELF.fetch(
		await signedRequest(
			`/internal/v1/projects/12/automations/${scopedRule}`,
			12,
			"DELETE",
			undefined,
			crypto.randomUUID(),
	...
	const auditResponse = await SELF.fetch(
		await signedRequest(`/internal/v1/projects/12/audit?inbox_id=${firstInbox}`, 12),
	);
	expect(auditResponse.status).toBe(200);
	const auditEntries = (
		await auditResponse.json<{ data: { source: string; action: string; payload_json: string }[] }>()
	).data;
	expect(
		auditEntries.some(
			(entry) =>
				entry.source === "automations" &&
				entry.action === "deleted" &&
				JSON.parse(entry.payload_json).inbox_ids.includes(firstInbox),
		),
	).toBe(true);
	const portals = await Promise.all(
		["First", "Second"].map((name) =>
			create("help-center/portals", {
				name,
	...
	const firstArticle = await create("help-center/articles", {
		portal_id: portals[0],
		title: "First article",
		slug: crypto.randomUUID(),
		content: "First",
	...
	await create("help-center/articles", {
		portal_id: portals[1],
		title: "Other portal article",
		slug: crypto.randomUUID(),
		content: "Second",
		status: "draft",
	});
	const articles = await SELF.fetch(
		await signedRequest(
			`/internal/v1/projects/12/help-center/articles?portal_id=${portals[0]}&limit=1`,
			12,
		),
	);
	...
	expect(result.data.map((row) => row.id)).toEqual([firstArticle]);
