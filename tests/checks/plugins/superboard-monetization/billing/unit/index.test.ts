	...
	it("encrypts a Store credential with the Billing-only keyring", async () => {
		const env = {
			...
			STORE_CREDENTIALS_ENCRYPTION_KEYS: JSON.stringify({
				"billing-v1": "billing-test-key-material",
			}),
		} as any;
		const response = await worker.fetch(
			new Request("https://billing.internal/internal/v1/credentials/encrypt", {
				method: "POST",
		...
		await expect(decryptCredential(env, payload.data.ciphertext)).resolves.toBe(
			JSON.stringify({ secret_key: "sk_test_example" }),
		);
	});
});
