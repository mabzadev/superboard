...
import { applyVerifiedPurchase } from "../../../../../../packages/plugins/superboard-core/api/src/lib/billing";
import worker from "../../../../../../packages/plugins/superboard-monetization/billing/src/index";
import type { BillingRuntimeEnv } from "./apply-migrations";

const projectId = "11";
const anonymousId = "$superboard_anon_runtime";
...
describe("Billing authority in the Workers runtime", () => {
	...
	it("reports isolated readiness without exposing secret material", async () => {
		const response = await SELF.fetch("https://billing.internal/internal/v1/health");
	...
	it("publishes only the public ES256 verification key", async () => {
		const response = await SELF.fetch("https://billing.internal/internal/v1/jwks");
		expect(response.status).toBe(200);
	...
	it("resolves one durable anonymous customer and signs its CustomerInfo", async () => {
		...
		const jwksResponse = await SELF.fetch("https://billing.internal/internal/v1/jwks");
		const keySet = await jwksResponse.json<JSONWebKeySet>();
		const verified = await jwtVerify(infoPayload.data.signature, createLocalJWKSet(keySet), {
			audience: "superboard-sdk",
			issuer: "superboard-purchases",
			subject: anonymousId,
		});
	...
	it("persists a provider event once before queue delivery", async () => {
		...
		const firstResponse = await jsonRequest("/internal/v1/provider-events/ingest", request);
		const secondResponse = await jsonRequest("/internal/v1/provider-events/ingest", request);
	...
	it("retries unsupported queue messages instead of losing them", async () => {
		...
		expect(result.retryMessages).toEqual([{ msgId: "unsupported-runtime-job" }]);
	});
	...
	it("claims and closes an inactive entitlement delivery without retrying it", async () => {
        ...
        INSERT INTO billing_webhook_endpoints (
          ...
          'https://api.example.com/webhooks/superboard/entitlements',
          'env:SUPERBOARD_ENTITLEMENT_WEBHOOK_SECRET', '["sandbox"]',
	...
	it("returns stable public errors for malformed requests", async () => {
		const response = await SELF.fetch("https://billing.internal/internal/v1/customers/resolve", {
			method: "POST",
			headers: {
	...
	it("pseudonymizes billing identity while retaining financial truth", async () => {
      ...
      `,
		...
		const unauthorized = await SELF.fetch("https://billing.internal/internal/v1/customers/erase", {
			method: "POST",
		...
		const [customer, aliases, transaction, event, paywall] = await testEnv.DB.batch([
			testEnv.DB.prepare(`
