...
} from "../../../../packages/plugins/superboard-monetization/scripts/billing-preflight.mjs";
...
test("blocks cutover when the API still owns Billing or the DLQ is unconsumed", () => {
	const report = evaluateBillingPreflight({
		...readyInput(),
		mainConsumer: "superboard-api",
		dlqConsumer: null,
	});
	assert.equal(report.ready, false);
	assert.deepEqual(
		report.blockers.map((item) => item.key),
		["main_queue_consumer", "dead_letter_queue_consumer"],
	);
});
...
test("parses applied migration names from Wrangler D1 output", () => {
	assert.deepEqual(
		parseD1MigrationNames('[{"results":[{"name":"0044.sql"},{"name":"0045.sql"}],"success":true}]'),
		["0044.sql", "0045.sql"],
	);
});
...
test("parses named operational counters from multiple D1 result batches", () => {
	assert.deepEqual(
		parseD1Metrics(
			'[{"results":[{"metric":"failed","value":0}]},{"results":[{"metric":"stale","value":2}]}]',
		),
		{
			failed: 0,
			stale: 2,
...
function readyInput() {
	return {
		...
		expectedMainConsumer: "superboard-billing",
		expectedDlqConsumer: "superboard-billing",
		mainConsumer: "superboard-billing",
		dlqConsumer: "superboard-billing",
