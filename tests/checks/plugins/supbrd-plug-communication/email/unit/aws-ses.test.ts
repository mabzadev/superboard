...
import {
	...
	type AwsSnsEnvelope,
} from "../../../../../../packages/plugins/supbrd-plug-communication/email/src/aws-ses";
...
describe("AWS SNS and SES verification", () => {
	it("verifies a version 2 SNS signature with the certificate public key", async () => {
		...
		await expect(verifyAwsSnsEnvelope(envelope, TOPIC, fetchCertificate)).resolves.toBeUndefined();
	...
	it("rejects another topic and certificate hosts before network access", async () => {
		...
		const envelope = parseAwsSnsEnvelope({
			...
			SigningCertURL: "https://attacker.example/SimpleNotificationService-1234567890abcdef.pem",
		});
		await expect(verifyAwsSnsEnvelope(envelope, TOPIC, fetchCertificate)).rejects.toThrow(
	...
	it("allows only the exact signed SNS subscription confirmation URL", () => {
		...
		expect(trustedAwsSnsConfirmationUrl(envelope).hostname).toBe("sns.eu-central-1.amazonaws.com");
		expect(() =>
			trustedAwsSnsConfirmationUrl({
				...envelope,
...
function derElement(tag: number, content: Uint8Array): Uint8Array {
	return concatenate([new Uint8Array([tag]), derLength(content.length), content]);
}
