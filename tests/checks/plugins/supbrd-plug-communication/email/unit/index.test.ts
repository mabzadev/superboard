...
import worker from "../../../../../../packages/plugins/supbrd-plug-communication/email/src/index";
...
describe("Email Worker health", () => {
	...
	it("deduplicates identical business events and rejects key reuse with another payload", async () => {
			...
			prepare(sql: string) {
				return {
					bind(...args: unknown[]) {
						return {
							first: async () => {
								if (sql.includes("WHERE environment = ? AND idempotency_key = ?")) {
									return byKey.get(`${String(args[0])}:${String(args[1])}`) || null;
								}
								return null;
							},
		...
		const first = await worker.fetch(request("Verify email"), env, {} as ExecutionContext);
		const replay = await worker.fetch(request("Verify email"), env, {} as ExecutionContext);
		const conflict = await worker.fetch(request("Different message"), env, {} as ExecutionContext);
	...
	it.each(["smtp", "aws-ses"] as const)(
		...
		async (provider) => {
			...
			expect(delegatedMessage.messageId).toMatch(/^<superboard-[a-f0-9]{48}@example\.test>$/);
	...
	it("never re-sends a key when D1 fails after provider acceptance", async () => {
		...
		const first = await worker.fetch(delegatedSmtpRequest(), env, {} as ExecutionContext);
		const reuse = await worker.fetch(delegatedSmtpRequest(), env, {} as ExecutionContext);
	...
	it("returns a body-free operational projection through internal authentication", async () => {
		...
		const response = await worker.fetch(
			new Request("https://email.internal/internal/v1/operations?status=failed", {
	...
	it("returns a failed replay to quarantine when Queue dispatch rejects it", async () => {
		...
		expect(log).toHaveBeenCalledWith(expect.stringContaining("email_dead_letter_replay_failed"));
		log.mockRestore();
	});
});
