...
function request(body: string, headers: Record<string, string> = {}): Request {
	return new Request(`https://email.internal${EMAIL_SERVICE_INBOUND_NORMALIZE_PATH}`, {
		method: "POST",
		headers: {
...
describe("inbound email normalization endpoint", () => {
						...
						attachments: [
							{
								...
								size: 128,
								downloadUrl: "https://mail.example.test/attachments/attachment-1",
							},
						],
	...
	it("requires internal authentication", async () => {
		...
		const response = await worker.fetch(unauthorized, environment(), {} as ExecutionContext);
