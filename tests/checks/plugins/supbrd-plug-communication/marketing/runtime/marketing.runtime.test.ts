...
import {
	resolveStoredEmail,
	claimEmailFrequency,
} from "../../../../../../packages/plugins/supbrd-plug-communication/marketing/src/email-studio.js";
import {
	dispatchCampaign,
	handleMarketingQueue,
} from "../../../../../../packages/plugins/supbrd-plug-communication/marketing/src/queue";
import { decryptJson } from "../../../../../../packages/plugins/supbrd-plug-communication/marketing/src/secrets";
import { trackingToken } from "../../../../../../packages/plugins/supbrd-plug-communication/marketing/src/tracking";
...
async function signedRequest(
	...
	if (role === "application") {
		headers.set("x-superboard-application-user-id", "application-user-runtime");
		headers.set("x-superboard-application-email", "preference-runtime@example.com");
		headers.set("x-superboard-application-name", "Preference Runtime");
	}
...
describe("Marketing in the Workers runtime", () => {
	it("keeps the published email available while its next revision is a draft", async () => {
		...
		const created = await SELF.fetch(
			await signedRequest(
				...
				97,
				"POST",
				{ name: "Stable live email", studio_document: document, expected_revision: 0 },
				crypto.randomUUID(),
			),
		);
		...
		const publish = await SELF.fetch(
			await signedRequest(
				"/internal/v1/studio/templates/" + template.id + "/publish",
				97,
				"POST",
				{ expected_revision: 1 },
				crypto.randomUUID(),
		...
		await SELF.fetch(
			await signedRequest(
				"/internal/v1/templates/" + template.id,
				97,
				"PATCH",
				{ studio_document: document, expected_revision: 1 },
				crypto.randomUUID(),
			),
		);
		expect(
			await resolveStoredEmail(
				env.DB,
				97,
				template.id,
				crypto.randomUUID(),
		...
		).toMatchObject({ status: "ready", subject: "Ready" });
	});
	it("admits only one concurrent marketing message while allowing access emails and retries", async () => {
		...
		const create = async (name: string) => {
			const response = await SELF.fetch(
				await signedRequest(
					...
					94,
					"POST",
					{ name, template_type: "campaign", studio_document: design, expected_revision: 0 },
					crypto.randomUUID(),
				),
			);
			...
			await SELF.fetch(
				await signedRequest(
					"/internal/v1/studio/templates/" + id + "/publish",
					94,
					"POST",
					{ expected_revision: 1 },
					crypto.randomUUID(),
		...
		for (const id of [first, second])
			await resolveStoredEmail(env.DB, 94, template, id, { profile: { locale: "en" } }, "template");
		const claims = await Promise.all([
			claimEmailFrequency(env.DB, 94, first, "same@example.test"),
			claimEmailFrequency(env.DB, 94, second, "same@example.test"),
		]);
		expect(claims.filter(Boolean)).toHaveLength(1);
		expect(
			await claimEmailFrequency(env.DB, 94, claims[0] ? first : second, "same@example.test"),
		).toBe(true);
	...
	it("resolves and freezes a transactional translation before a template is edited", async () => {
		...
		design.locales.fr = {
			...
			subject: "Votre reçu",
			blocks: [{ id: "receipt", type: "text", text: "Reçu pour {{name}}" }],
		};
		const response = await SELF.fetch(
			await signedRequest(
				"/internal/v1/templates",
		...
		await SELF.fetch(
			await signedRequest(
				"/internal/v1/studio/templates/" + template.data.id + "/publish",
				93,
				"POST",
				{ expected_revision: 1 },
				crypto.randomUUID(),
		...
		const sent = await SELF.fetch(
			await signedRequest(
				"/internal/v1/transactional",
				93,
				"POST",
				{
					template_id: template.data.id,
					recipient: "receipt@example.test",
					name: "Alex",
					attributes: { locale: "en", billing_locale: "fr-CH" },
				},
				crypto.randomUUID(),
			),
		);
		...
		const contact = (
			await contacts.json<{ data: Array<{ email: string; consent_status: string }> }>()
		).data.find((item) => item.email === "receipt@example.test");
		expect(contact?.consent_status).toBe("pending");
		const rendered = await resolveStoredEmail(
			env.DB,
			93,
			delivery.data.campaign_id,
			delivery.data.delivery_id,
			{ profile: { name: "Alex", locale: "en", billing_locale: "fr-CH" } },
		);
		expect(rendered).toMatchObject({ status: "ready", locale: "fr", subject: "Votre reçu" });
		design.locales.fr.subject = "Modifié ensuite";
		await SELF.fetch(
			await signedRequest(
				"/internal/v1/templates/" + template.data.id,
				93,
				"PATCH",
				{ studio_document: design, expected_revision: 1 },
				crypto.randomUUID(),
			),
		);
		expect(
			await resolveStoredEmail(env.DB, 93, delivery.data.campaign_id, delivery.data.delivery_id, {
				profile: { name: "Other", locale: "en" },
			}),
		).toMatchObject({ status: "ready", locale: "fr", subject: "Votre reçu" });
	});
	it("stores multilingual template revisions, rejects stale saves and isolates project history", async () => {
		...
		const first = await created.json<{
			data: { id: string; studio_revision: number; studio_document: unknown };
		}>();
		...
		const updated = await SELF.fetch(
			await signedRequest(
				`/internal/v1/templates/${first.data.id}`,
				91,
				"PATCH",
				{ studio_document: document, expected_revision: 1 },
				crypto.randomUUID(),
		...
		const history = await SELF.fetch(
			await signedRequest(`/internal/v1/studio/templates/${first.data.id}/history`, 91),
		);
		expect((await history.json<{ data: unknown[] }>()).data).toHaveLength(2);
		const other = await SELF.fetch(
			await signedRequest(`/internal/v1/studio/templates/${first.data.id}/history`, 92),
		);
	...
	it("persists authenticated application consent without exposing private lists or weakening suppressions", async () => {
		...
		expect(initialBody.data.lists).not.toEqual(
			expect.arrayContaining([expect.objectContaining({ id: privateList.data.id })]),
		);
		...
		await expect(optedIn.json()).resolves.toMatchObject({
			data: {
				...
				status: "enabled",
				attributes: { locale: "fr-CH", plan: "reference" },
				lists: [expect.objectContaining({ id: publicList.data.id, selected: true })],
			},
		});
    ...
    `,
		...
		expect(String(redacted?.email)).toMatch(/^erased\+[a-f0-9]{32}@invalid\.superboard$/u);
		await expect(
			env.DB.prepare(
				`
	...
	it("persists subscribers, consent, lists, segments and campaigns with project isolation", async () => {
		...
		const projectRows = await SELF.fetch(await signedRequest("/internal/v1/email/subscribers", 12));
		await expect(projectRows.json()).resolves.toMatchObject({
			data: [{ email: "customer@example.com" }],
		});
		const foreignRows = await SELF.fetch(await signedRequest("/internal/v1/email/subscribers", 11));
	...
	it("encrypts stored SMTP credentials in D1", async () => {
		...
		await expect(response.text()).resolves.not.toContain("smtp-private-password");
	});
    ...
    `,
		...
		await expect(
			env.DB.prepare("SELECT status FROM campaigns WHERE id = ?").bind(campaignId).first(),
		).resolves.toMatchObject({ status: "running" });
		await expect(
			env.DB.prepare("SELECT status FROM email_deliveries WHERE campaign_id = ?")
				.bind(campaignId)
				.first(),
    ...
    `,
		...
		const listed = await SELF.fetch(await signedRequest("/internal/v1/settings/dead-letters", 12));
	...
	it("enforces owner/admin writes while retaining member reads", async () => {
		...
		const read = await SELF.fetch(
			await signedRequest("/internal/v1/lists", 12, "GET", undefined, undefined, "member"),
		);
    ...
    `,
		...
		const subscriberResponses = await Promise.all([
			...
			SELF.fetch(
				await signedRequest(`/internal/v1/email/subscribers/${createdSubscriber.data.id}`, 12),
			),
			SELF.fetch(
				await signedRequest(
		...
		await expect(
			env.DB.prepare("SELECT consent_status, optin_token_hash FROM subscribers WHERE id = ?")
		...
		expect(
			(
				await SELF.fetch(`https://marketing.internal/public/v1/opt-in/${payload.token}`, {
					method: "POST",
				})
			).status,
		).toBe(422);
	...
	it("deduplicates tracking and processes signed unsubscribe requests", async () => {
		...
		expect(
			(await SELF.fetch(`https://marketing.internal/public/v1/tracking/open/${open}`)).status,
		).toBe(200);
		expect(
			(await SELF.fetch(`https://marketing.internal/public/v1/tracking/open/${open}`)).status,
		).toBe(200);
		...
		await expect(
			env.DB.prepare("SELECT status FROM subscribers WHERE id = ?").bind(subscriberId).first(),
	...
	it("authenticates provider webhooks and suppresses complaints", async () => {
		...
		await expect(
			env.DB.prepare("SELECT reason FROM suppressions WHERE project_id = 12 AND email = ?")
	...
	it("deduplicates Analytics signals and advances a versioned event journey exactly once", async () => {
			...
			await signedRequest(
				...
				"POST",
				{
					...
					definition: {
						...
						edges: [{ from: "promote", to: "complete", outcome: "default" }],
					},
				},
       ...
       WHERE project_id = ? AND journey_id = ?`,
		...
		for (const [messageId, attempts] of [
			...
			const execution = createExecutionContext();
			await handleMarketingQueue(batch, env);
			expect((await getQueueResult(batch, execution)).explicitAcks).toEqual([messageId]);
		}
		await expect(
			env.DB.prepare(`SELECT status FROM marketing_journey_enrollments WHERE id = ?`)
				.bind(enrollment!.id)
				.first(),
		).resolves.toMatchObject({ status: "completed" });
		const updated = await env.DB.prepare(`SELECT attributes_json FROM subscribers WHERE id = ?`)
