...
import {
	isEmailTransportInProgress,
	sendSmtpMessage,
} from "../../../../../../packages/plugins/supbrd-plug-communication/marketing/src/email-service";
import type { Env } from "../../../../../../packages/plugins/supbrd-plug-communication/marketing/src/types";
...
describe("Marketing Email service binding", () => {
	...
	it("preserves the transport in-progress signal for Queue retry safety", async () => {
		const fetch = vi
			.fn()
			.mockResolvedValue(Response.json({ error: "email_transport_in_progress" }, { status: 409 }));
		await expect(
			sendSmtpMessage(
				{
					EMAIL_SERVICE: { fetch },
	...
	it("preserves an unconfirmed SMTP acceptance for same-profile retry", async () => {
		const fetch = vi
			.fn()
			.mockResolvedValue(Response.json({ error: "smtp_outcome_unknown" }, { status: 503 }));
		...
		await delivery.catch((error) => expect(isEmailTransportInProgress(error)).toBe(true));
	});
