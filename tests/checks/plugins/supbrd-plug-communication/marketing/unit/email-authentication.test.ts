...
} from "../../../../../../packages/plugins/supbrd-plug-communication/marketing/src/email-authentication";

describe("sender DNS authentication", () => {
	it("requires SPF, DKIM and DMARC before a sender becomes production-ready", async () => {
		const fetcher = vi
			.fn()
			.mockResolvedValueOnce(dnsTxtResponse("example.com", "v=spf1 include:mail.example.net -all"))
			.mockResolvedValueOnce(dnsTxtResponse("_dmarc.example.com", "v=DMARC1; p=reject"))
			.mockResolvedValueOnce(
				dnsTxtResponse("mail._domainkey.example.com", "v=DKIM1; k=rsa; p=publickey"),
			);
	...
	it("does not confuse a successful SMTP connection with authenticated sending", async () => {
		const fetcher = vi
			.fn()
			.mockResolvedValueOnce(dnsTxtResponse("example.com", "google-site-verification=123"))
	...
	it("rejects malformed DNS wire responses", () => {
		expect(() => parseDnsMessage(Uint8Array.from([0, 1, 2]))).toThrow(/truncated/);
	});
	...
	it("rejects an oversized DNS response before buffering it", async () => {
		...
		await expect(
			verifyEmailAuthentication("news@example.com", null, fetcher as typeof fetch),
...
function dnsResponse(question: Uint8Array, answers: Uint8Array[]): Response {
	const bytes = new Uint8Array(
		12 + question.length + answers.reduce((total, answer) => total + answer.length, 0),
	);
...
function dnsQuestion(name: string): Uint8Array {
	...
	const bytes = new Uint8Array(labels.reduce((total, label) => total + label.length + 1, 0) + 5);
	let offset = 0;
