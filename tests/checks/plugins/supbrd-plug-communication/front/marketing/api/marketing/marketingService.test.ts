...
vi.mock(
	"../../../../../../../../packages/plugins/supbrd-plug-communication/src/front/marketing/transport.js",
	() => api,
);
...
} from "../../../../../../../../packages/plugins/supbrd-plug-communication/src/front/marketing/api/marketing/marketingService.js";
...
describe("Marketing dashboard service contracts", () => {
	...
	it("unwraps subscriber list, detail, and privacy export responses", async () => {
		...
		await expect(getEmailSubscriber("10-test", "subscriber-1")).resolves.toMatchObject({
			id: "subscriber-1",
		});
		await expect(getEmailSubscriberExport("10-test", "subscriber-1")).resolves.toMatchObject({
			subscriber: { id: "subscriber-1" },
		});
	...
	it("maps template, campaign, and media operations without legacy rewrites", async () => {
		...
		await expect(downloadMarketingMedia("10-test", "media-1")).resolves.toBe(blob);
		...
		expect(api.GET).toHaveBeenCalledWith("/api/v1/marketing/projects/10-test/media/media-1", {
			responseType: "blob",
		});
	});
	...
	it("maps versioned journeys and omnichannel connectors", async () => {
		...
		expect(api.GET).toHaveBeenCalledWith("/api/v1/marketing/projects/10-test/journeys");
