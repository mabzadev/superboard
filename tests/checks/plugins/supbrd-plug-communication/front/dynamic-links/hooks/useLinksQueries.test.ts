...
import {
	...
	useRandomPathQuery,
} from "../../../../../../../packages/plugins/supbrd-plug-analytics/src/front/hooks/queries/useLinksQueries.js";
import type { GetLinksParams } from "../../../../../../../packages/supbrd-front-ui/src/shared/types/index.js";
import { renderHook, waitFor } from "../../../../../packages/supbrd-front-ui/render.js";
import { createTestQueryClient } from "./query-test-utils.js";

vi.mock(
	"../../../../../../../packages/plugins/supbrd-plug-analytics/src/front/api/links/linksService.js",
	() => ({
...
} from "../../../../../../../packages/plugins/supbrd-plug-analytics/src/front/api/links/linksService.js";
...
describe("useLinksQueries", () => {
	...
	describe("useLinksByIdsQuery", () => {
		it("is disabled when projectId is undefined", () => {
			const { result } = renderHook(() => useLinksByIdsQuery(undefined, { ids: ["1"] }), {
				wrapper: createWrapper(queryClient),
			});
			expect(result.current.fetchStatus).toBe("idle");
		});
		...
		it("fetches links by ids", async () => {
			...
			const { result } = renderHook(() => useLinksByIdsQuery("proj-1", { ids: ["l1", "l2"] }), {
				wrapper: createWrapper(queryClient),
			});
	...
	describe("usePathAvailableQuery", () => {
		...
		it("is disabled when projectId is undefined", () => {
			const { result } = renderHook(() => usePathAvailableQuery(undefined, "my-path"), {
				wrapper: createWrapper(queryClient),
			});
			expect(result.current.fetchStatus).toBe("idle");
		});
		...
		it("fetches path availability", async () => {
			...
			const { result } = renderHook(() => usePathAvailableQuery("proj-1", "my-path"), {
				wrapper: createWrapper(queryClient),
			});
