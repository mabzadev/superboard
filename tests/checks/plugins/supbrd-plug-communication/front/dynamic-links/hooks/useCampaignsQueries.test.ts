...
} from "../../../../../../../packages/plugins/supbrd-plug-communication/src/front/dynamic-links/hooks/queries/useCampaignsQueries.js";
import type {
	GetCampaignsParams,
	DateRangeQuery,
...
describe("useCampaignsQueries", () => {
	...
	describe("useCampaignsListQuery", () => {
		...
		it("is disabled when params is null", () => {
			const { result } = renderHook(() => useCampaignsListQuery("proj-1", null), {
				wrapper: createWrapper(queryClient),
			});
			expect(result.current.fetchStatus).toBe("idle");
		});
