...
it("saves pending content before allowing navigation away from the editor", async () => {
	const initial = createEmailDesign("en");
	...
	render(
		<EmailEditor
			...
			onSave={async (name, document, revision) => {
				storedSubject = document.locales.en?.subject ?? "";
				return {
					id: "pending",
					name,
					template_type: "campaign",
	...
	fireEvent.change(screen.getByLabelText("Subject"), { target: { value: "Latest subject" } });
	let allowed = false;
	await act(async () => {
		if (leave) allowed = await leave();
	});
	expect(storedSubject).toBe("Latest subject");
