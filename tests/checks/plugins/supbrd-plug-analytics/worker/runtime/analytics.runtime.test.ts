...
import { evaluateAnalyticsAlerts } from "../../../../../../packages/plugins/supbrd-plug-analytics/worker/src/automations";
import { handleAnalyticsQueue } from "../../../../../../packages/plugins/supbrd-plug-analytics/worker/src/ingestion";
...
async function projectEvent(projectId: number, eventId: string, messageId: string, bindings = env) {
	...
	await handleAnalyticsQueue(batch, bindings);
	return getQueueResult(batch, execution);
}

describe("Analytics in the Workers runtime", () => {
	it("projects events and retains integration delivery when Marketing is absent", async () => {
		const event = {
			schema_version: 1,
			event_id: "runtime-independent-install",
			event_name: "superboard.analytics.installation.created.v1",
			occurred_at: new Date().toISOString(),
			source: "system",
			application_id: "independent-app",
			app_instance_id: "independent-installation",
			session_id: "independent-session",
			properties: { attribution_id: "independent-attribution" },
		};
		const accepted = await SELF.fetch(
			await signedRequest("/internal/v1/events", 49, "POST", event, undefined, "system"),
		);
		expect(accepted.status).toBe(202);
		const independent = { ...env };
		Reflect.deleteProperty(independent, "MARKETING_MODULE");
		Reflect.deleteProperty(independent, "EMAIL_SERVICE");
		const result = await projectEvent(49, event.event_id, "independent-projection", independent);
		expect(result.explicitAcks).toEqual(["independent-projection"]);
		await expect(
			env.DB.prepare(
				"SELECT status FROM analytics_event_receipts WHERE project_id = '49' AND event_id = ?",
			)
				.bind(event.event_id)
				.first(),
		...
		await expect(
			env.DB.prepare(
				"SELECT status, attempt_count FROM analytics_marketing_signal_outbox WHERE project_id = '49' AND event_id = ?",
			)
				.bind(event.event_id)
				.first(),
		).resolves.toMatchObject({ status: "pending", attempt_count: 0 });
		await expect(
			env.DB.prepare(
				"SELECT COUNT(*) AS total FROM analytics_installations WHERE project_id = '49'",
			).first(),
		).resolves.toMatchObject({ total: 1 });
	});

	it("records triggered alerts when the Email integration is absent", async () => {
		const created = await SELF.fetch(
			await signedRequest(
				...
				"POST",
				{
					name: "No incoming data without Email",
					alert_type: "no_data",
					definition: { window_minutes: 60 },
					channels: [{ type: "email", to: "analytics@example.test" }],
					cooldown_minutes: 60,
				},
				"runtime-independent-alert",
			),
		);
		expect(created.status).toBe(201);
		const independent = { ...env };
		Reflect.deleteProperty(independent, "EMAIL_SERVICE");
		await evaluateAnalyticsAlerts(independent);
		const incident = await env.DB.prepare(
			"SELECT notification_status, notifications_json FROM analytics_alert_incidents WHERE project_id = '50'",
		).first<{ notification_status: string; notifications_json: string }>();
		expect(incident?.notification_status).toBe("failed");
		expect(JSON.parse(incident?.notifications_json ?? "[]")).toEqual([
			{ type: "email", status: "failed", error: "Email service is unavailable" },
		]);
		await evaluateAnalyticsAlerts(independent);
		await expect(
			env.DB.prepare(
				"SELECT COUNT(*) AS total FROM analytics_alert_incidents WHERE project_id = '50'",
			).first(),
		).resolves.toMatchObject({ total: 1 });
	});

	it("reports schema and non-sensitive pipeline health", async () => {
		const response = await SELF.fetch("https://analytics.internal/internal/v1/health");
	...
	it("counts an installation and session once across ingestion and queue replays", async () => {
		...
		const accepted = await SELF.fetch(
			await signedRequest("/internal/v1/events", 12, "POST", event, undefined, "system"),
		);
		expect(accepted.status).toBe(202);
		...
		const firstProjection = await projectEvent(12, event.event_id, "projection-1");
		expect(firstProjection.explicitAcks).toEqual(["projection-1"]);
		const replayProjection = await projectEvent(12, event.event_id, "projection-2");
	...
	it("accepts only verified canonical purchase facts once", async () => {
		...
		expect(
			(
				await SELF.fetch(
					await signedRequest("/internal/v1/events", 12, "POST", refund, undefined, "system"),
				)
			).status,
		).toBe(202);
		await projectEvent(12, refund.event_id, "purchase-projection-refund-1");

		const overview = await SELF.fetch(await signedRequest("/internal/v1/overview", 12));
	...
	it("projects Countly-style views, crashes and ratings exactly once", async () => {
		...
		const views = await SELF.fetch(await signedRequest("/internal/v1/views", 22));
	...
	it("applies collection and hot-retention settings without dropping canonical facts", async () => {
		...
		const accepted = await SELF.fetch(
			await signedRequest("/internal/v1/events", 55, "POST", canonical, undefined, "system"),
		);
		expect(accepted.status).toBe(202);
	...
	it("stops SDK collection for a disabled application", async () => {
		...
		expect(
			(await SELF.fetch(await signedRequest("/internal/v1/events", 56, "POST", initialEvent)))
				.status,
		).toBe(202);
	...
	it("keeps an erasure tombstone so an event replay cannot restore subject data", async () => {
		...
		expect(
			(await SELF.fetch(await signedRequest("/internal/v1/events", 12, "POST", event))).status,
		).toBe(202);
		await projectEvent(12, event.event_id, "erasure-projection-1");

		const erased = await SELF.fetch(
			await signedRequest("/internal/v1/application/users/private-user-to-erase", 12, "DELETE"),
		);
		...
		const replay = await SELF.fetch(await signedRequest("/internal/v1/events", 12, "POST", event));
	...
	it("isolates projects and replays saved-report mutations", async () => {
		...
		const createRequest = () =>
			signedRequest("/internal/v1/reports", 12, "POST", payload, "runtime-report-create");
		...
		const own = await SELF.fetch(await signedRequest("/internal/v1/reports", 12));
		await expect(own.json()).resolves.toMatchObject({
			data: { items: [expect.objectContaining({ id: createdBody.data.id })] },
		});
		const other = await SELF.fetch(await signedRequest("/internal/v1/reports", 13));
