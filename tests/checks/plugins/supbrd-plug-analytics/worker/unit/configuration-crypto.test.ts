...
} from "../../../../../../packages/plugins/supbrd-plug-analytics/worker/src/configuration-crypto";
...
describe("analytics configuration cryptography", () => {
	it("encrypts hook secrets with authenticated encryption", async () => {
		...
		await expect(
			decryptAnalyticsConfiguration<{ secret: string }>("active-key", encrypted),
		).resolves.toEqual({ secret: "webhook-secret" });
		await expect(decryptAnalyticsConfiguration("wrong-key", encrypted)).rejects.toThrow();
	});
