...
import app from "../../../../../../packages/plugins/superboard-acquisition/paywalls/src/index";

describe("paywalls worker health", () => {
	it("reports a connected D1 binding", async () => {
		const response = await app.request("/internal/v1/health", {}, environment(true));
	...
	it("fails closed when the D1 query fails", async () => {
		const response = await app.request("/internal/v1/health", {}, environment(false));
		...
		DB: {
			prepare: (query: string) => {
				const first = async () =>
					!healthy
						? null
						: query.includes("superboard_health_check")
