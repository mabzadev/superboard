...
import {
	...
	parseTargeting,
	parseVersion,
} from "../../../../../../packages/plugins/superboard-acquisition/paywalls/src/contracts";
...
describe("paywalls contracts v1", () => {
	it("validates versioned visual definitions", () => {
		const parsed = parseVersion({
			definition: {
				schema_version: 1,
				theme: { color: "#fff" },
				components: [{ id: "title", type: "heading", props: { text: "Upgrade" } }],
			},
		});
	...
	it("accepts targeting and a resolve context", () => {
		expect(parseTargeting({ platforms: ["ios"], countries: ["ch"] }).countries).toEqual(["CH"]);
		expect(
			parsePlacement({
				placement: "main",
				platform: "ios",
	...
	it("validates bounded statistics ranges and IANA timezones", () => {
		...
		expect(() => parseStatistics({ timezone: "Mars/Olympus" })).toThrow("timezone");
	});
});
