...
import { consumeFlowEvents } from "../../../../../../packages/plugins/superboard-acquisition/flows/src/queue/consumer";
	...
	it("projects the same client event id independently for two projects", async () => {
		...
		await vi.waitFor(async () => {
         ...
         WHERE event_id = ? ORDER BY project_id`,
				...
				.bind("shared-client-event")
				.all<{ project_id: number; event_id: string }>();
	...
	it("never lets a late old delivery overwrite a newer workflow state", async () => {
       ...
       VALUES ('workflow-ordering', ?, 'workflow-ordering', 'Ordering',
         'once', 'active', 'flows', 1, '2', ?, ?)`,
			...
			.bind(firstProject.projectId, "2026-08-13T11:00:00.000Z", "2026-08-13T11:00:00.000Z")
			.run();
       ...
       WHERE project_id = ? AND environment_id = ?
         AND user_id_hash = ? AND workflow_id = ?`,
			...
			.bind(recent.projectId, recent.environmentId, recent.userIdHash, recent.workflowId)
	...
	it("uses generation and revision when transitions share the same timestamp", async () => {
       ...
       WHERE project_id = ? AND environment_id = ? AND user_id_hash = ?
         AND workflow_id = ?`,
				...
				.bind(firstProject.projectId, environment!.id, newer.userIdHash, workflowId)
				.first(),
		).resolves.toEqual({ state: "completed", generation: 3, revision: 2 });
	});
});
...
async function provision(project: Project): Promise<void> {
	await responseData(
		await projectRequest(project, "GET", `/internal/v1/projects/${project.projectRef}/project`),
	);
}
