...
import type { FlowUserRuntime } from "../../../../../../packages/plugins/superboard-acquisition/flows/src/index";
import type { FlowRuntimeCommand } from "../../../../../../packages/plugins/superboard-acquisition/flows/src/types";
...
describe("FlowUserRuntime durability", () => {
	it("retries a persisted Delay start when Workflow creation first fails", async () => {
		...
		await runInDurableObject(stub, async (instance, state) => {
			const runtimeEnv = (
				instance as FlowUserRuntime as unknown as {
					env: Cloudflare.Env;
				}
			).env;
	...
	it("starts a fresh Workflow attempt after a terminal Delay execution lease expires", async () => {
		...
		await runInDurableObject(stub, async (instance, state) => {
           ...
           WHERE id = 'terminal-delay-event:delay:delay'`,
				...
				expect(
					ids.every((id) => id.length <= 100 && /^[a-zA-Z0-9_][a-zA-Z0-9-_]*$/u.test(id)),
				).toBe(true);
			} finally {
				runtimeEnv.FLOW_DELAY_EXECUTION = originalWorkflow;
			}
		...
		await runInDurableObject(stub, async (instance, state) => {
         ...
         VALUES ('old-resumed', 'workflow', 'version', 'delay', 'target',
           'resumed', ?, ?)`,
			...
			(instance as unknown as { compactRuntimeStorage(now: number): void }).compactRuntimeStorage(
				Date.now(),
			);

			expect(
				state.storage.sql
					.exec<{ count: number }>("SELECT COUNT(*) AS count FROM runtime_idempotency")
					.one().count,
			).toBe(0);
			expect(
				state.storage.sql
					.exec<{ event_id: string }>("SELECT event_id FROM runtime_outbox")
					.toArray(),
			).toEqual([{ event_id: "old-pending" }]);
			expect(
				state.storage.sql
					.exec<{ count: number }>("SELECT COUNT(*) AS count FROM runtime_delays")
					.one().count,
			).toBe(0);
		});
	});
	...
	it("rejects stale or forged block state events without creating an outbox event", async () => {
		...
		const initial = await runInDurableObject(stub, (instance) =>
			instance.execute({
				event: {
		...
		const before = await runInDurableObject(
			stub,
			async (_instance, state) => outboxCount(state, "delivered") + outboxCount(state, "pending"),
		);
		await expect(
			runInDurableObject(stub, (instance) =>
				instance.execute({
	...
	it("persists and starts a chained Delay and projects the resumed state", async () => {
		...
		await runInDurableObject(stub, async (instance, state) => {
			...
			try {
				...
				const firstPayload = started[0]!
					.params as import("../../../../../../packages/plugins/superboard-acquisition/flows/src/types").FlowDelayPayload;
				...
				expect(
					state.storage.sql
						.exec<{ status: string }>("SELECT status FROM runtime_delays ORDER BY created_at, id")
						.toArray(),
				).toEqual([{ status: "resumed" }, { status: "scheduled" }]);

				const resumeEvent = state.storage.sql
	...
	it("continues Workflow Triggers in both directions after Delay wake-ups without HTTP polling", async () => {
		...
		await runInDurableObject(stub, async (instance) => {
			...
			try {
				...
				expect((await instance.getSnapshot(parentWorkflowId))?.activeBlockIds).toEqual([
					"parent-trigger",
				]);
				expect((await instance.getSnapshot(targetWorkflowId))?.activeBlockIds).toEqual([
					"target-delay",
				]);
				expect(started).toHaveLength(2);
	...
	it("restarts an already completed every-time trigger target after an asynchronous Delay", async () => {
		...
		const targetGraph = delayedManualTargetGraph();
		await seedRuntimeRelease(projectId, environmentId, targetWorkflowId, targetGraph, "every-time");
	...
	it("starts the next Launchpad workflow immediately when a transition frees capacity", async () => {
		...
		expect(
			await runInDurableObject(stub, (instance) => instance.getSnapshot(nextWorkflowId)),
		).toBeNull();
		...
		expect(
			(await runInDurableObject(stub, (instance) => instance.getSnapshot(firstWorkflowId)))?.state,
		).toBe("completed");
		expect(
			(await runInDurableObject(stub, (instance) => instance.getSnapshot(nextWorkflowId)))?.state,
		).toBe("in-progress");
		expect(
         ...
         WHERE project_id = ? AND environment_id = ? AND user_id_hash = ?
           AND workflow_id = ?`,
				...
				.bind(projectId, environmentId, userHash, nextWorkflowId)
				.first(),
		...
		await runInDurableObject(stub, async (instance, state) => {
			...
			try {
				...
				for (const execution of started.slice()) {
					await instance.resumeDelay(
						execution.params as import("../../../../../../packages/plugins/superboard-acquisition/flows/src/types").FlowDelayPayload,
					);
				}
				const resumeEvents = state.storage.sql
...
function queueEvent(eventId: string, environmentId: string, workflowId?: string): FlowQueueEvent {
	return {
		schemaVersion: 1,
		eventId,
		projectId: 11,
...
function delayedManualTargetGraph(): FlowGraph {
	return {
		...
		paths: [flowPath("target-manual", "target-delay"), flowPath("target-delay", "target-end")],
	};
}
...
function editorBlock(
	...
	return {
		...
		data,
		propertyMeta: [],
		exitNodes: type === "workflow-trigger" ? ["workflow_completed"] : ["default"],
		position: { x: 0, y: 0 },
	};
}
