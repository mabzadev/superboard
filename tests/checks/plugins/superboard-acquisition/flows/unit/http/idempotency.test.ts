...
import type { FlowApp } from "../../../../../../../packages/plugins/superboard-acquisition/flows/src/http/auth";
import { errorResponse } from "../../../../../../../packages/plugins/superboard-acquisition/flows/src/http/errors";
import { replayProjectMutation } from "../../../../../../../packages/plugins/superboard-acquisition/flows/src/http/idempotency";
import type { Env } from "../../../../../../../packages/plugins/superboard-acquisition/flows/src/types";
	...
	private run(sql: string, bindings: unknown[]): { meta: { changes: number } } {
		...
		if (sql.includes("SET response_json = ?, created_at = ?")) {
			...
			const [
				...
				expectedResponse,
				expectedCreatedAt,
			] = bindings as [string, string, number, string, string, string, number, string, string?];
			const row = this.rows.get(this.storageKey(projectId, scope, key));
			if (
				!row ||
				row.requestHash !== requestHash ||
				row.responseStatus !== responseStatus ||
				row.responseJson !== expectedResponse ||
		...
		if (sql.includes("SET response_status = ?, response_json = ?")) {
			const [
				...
				expectedStatus,
				expectedResponse,
			] = bindings as [number, string, number, string, string, string, number, string];
			const row = this.rows.get(this.storageKey(projectId, scope, key));
			if (
				!row ||
				row.requestHash !== requestHash ||
				row.responseStatus !== expectedStatus ||
				row.responseJson !== expectedResponse
			)
				return { meta: { changes: 0 } };
			row.responseStatus = responseStatus;
		...
		if (sql.includes("DELETE FROM flow_idempotency_keys")) {
			const [projectId, scope, key, requestHash, responseStatus, responseJson] = bindings as [
				number,
				string,
				string,
				string,
...
describe("Flows project mutation idempotency", () => {
	it("atomically lets one concurrent request mutate and replays its exact response", async () => {
		...
		expect(
			[first, second].filter((response) => response.headers.get("idempotency-replayed") === "true"),
		).toHaveLength(1);
	});
...
function testApp(database: FakeD1, mutation: () => Response | Promise<Response>): Hono<FlowApp> {
