...
import { selectLaunchpadReleases } from "../../../../../../../packages/plugins/superboard-acquisition/flows/src/sdk/service";
...
describe("selectLaunchpadReleases", () => {
	...
	it("does not reserve capacity for a higher-priority workflow that is not targeted", () => {
		...
		expect(selectLaunchpadReleases([notTargeted, targeted], { plan: "free" })).toEqual([
			expect.objectContaining({ workflowId: targeted.workflowId }),
		]);
	});
