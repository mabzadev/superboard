...
import { sha256 } from "../../../../../../../packages/plugins/superboard-acquisition/flows/src/d1/helpers";
import {
	FLOW_SDK_KEY_HEADER,
	readFlowSdkCredential,
	verifyFlowSdkKey,
} from "../../../../../../../packages/plugins/superboard-acquisition/flows/src/sdk/auth";
...
describe("Flows SDK credentials", () => {
	...
	it("rejects missing or conflicting credentials", () => {
		expect(() =>
			readFlowSdkCredential(new Request("https://flows.test/v2/sdk/blocks"), "42-test"),
	...
	it("compares the derived hash without comparing plaintext credentials", async () => {
		...
		await expect(verifyFlowSdkKey("environment-secret", stored)).resolves.toBe(true);
		await expect(verifyFlowSdkKey("wrong-secret", stored)).resolves.toBe(false);
		await expect(verifyFlowSdkKey("environment-secret", null)).resolves.toBe(false);
	});
});
