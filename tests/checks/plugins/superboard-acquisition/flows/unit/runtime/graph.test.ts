...
import { executeGraph } from "../../../../../../../packages/plugins/superboard-acquisition/flows/src/runtime/graph";
...
describe("executeGraph", () => {
	...
	it("only starts a matching targeted Manual Start for workflow-start", () => {
		...
		const execute = (blockKey: string, plan: string) =>
			executeGraph({
				graph,
				userProperties: { plan },
				activeBlockIds: [],
	...
	it("triggers another branch without exiting a block-trigger component", () => {
		const graph = simpleGraph();
		graph.blocks[1]!.propertyMeta = [{ key: "show-hint", type: "block-trigger" }];
		graph.blocks.push({
			id: "hint",
			key: "hint",
			type: "component",
	...
	it("schedules delay blocks and never renders them", () => {
		...
		expect(result.delays).toEqual([{ blockId: "delay", targetBlockId: "card", delayMs: 300_000 }]);
