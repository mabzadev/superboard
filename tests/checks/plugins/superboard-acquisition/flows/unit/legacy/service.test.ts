...
import {
	...
	legacyVariantBucket,
	stableBucket,
} from "../../../../../../../packages/plugins/superboard-acquisition/flows/src/legacy/service";
...
describe("legacy experience assignment parity", () => {
	it("matches the removed Paywalls SHA-256 project-scoped seeds", async () => {
		...
		expect(await legacyTrafficEligible("paywalls", projectId, placementId, subject, 3_700)).toBe(
			(await legacyPaywallsBucket(`${projectId}:${placementId}:${subject}`, 100)) < 37,
		);
		expect(await legacyVariantBucket("paywalls", projectId, experienceId, subject, 97)).toBe(
	...
	it("matches the removed Onboardings single FNV subject bucket", async () => {
		...
		expect(
			await legacyTrafficEligible("onboardings", 11, "ignored-placement", subject, 5_000),
		).toBe(bucket % 10_000 < 5_000);
		expect(
			await legacyVariantBucket("onboardings", 11, "ignored-experience", subject, 10_000),
		).toBe(bucket % 10_000);
...
async function legacyPaywallsBucket(value: string, modulo: number) {
	...
	return (
		(((digest[0]! << 24) | (digest[1]! << 16) | (digest[2]! << 8) | digest[3]!) >>> 0) % modulo
	);
}
