...
import {
	parseEvents,
	validateDefinition,
} from "../../../../../../packages/plugins/superboard-acquisition/onboardings/src/contracts";
...
describe("onboardings contracts v1", () => {
	...
	it("bounds event batches", () =>
		expect(() => parseEvents({ project_id: "p1", events: [] })).toThrow("1 to 100"));
		...
		expect(() =>
			validateDefinition({
				screens: [
					{
						id: "question",
						blocks: [{ id: "goal", type: "question", props: { attribute: "goal", options } }],
					},
				],
			}),
		).toThrow();
