...
vi.mock(
	"../../../../../../../../packages/plugins/superboard-acquisition/src/front/flows/transport.js",
	() => ({
		GET: requests.get,
		POST: requests.post,
...
import {
	flowsApi,
	type FlowGraph,
} from "../../../../../../../../packages/plugins/superboard-acquisition/src/front/flows/api/flows/flowsService.js";
...
describe("Flows dashboard API", () => {
	...
	it("uses project-scoped routes exclusively", async () => {
		...
		expect(requests.get).toHaveBeenNthCalledWith(1, "/api/v1/flows/projects/project%2Fdev/project");
		expect(requests.get).toHaveBeenNthCalledWith(
			2,
			"/api/v1/flows/projects/project%2Fdev/workflows?origin=paywalls&status=active",
		);
		expect(requests.get.mock.calls.flat().join(" ")).not.toMatch(/\/organizations?\//i);
	});
	...
	it("saves graph drafts with optimistic revision control", async () => {
		...
		await expect(flowsApi.saveDraft("project-1", "flow-1", graph, 7)).resolves.toMatchObject({
			revision: 8,
		});
	...
	it("does not expose removed tenant or commercial administration methods", () => {
		expect(Object.keys(flowsApi).join(" ")).not.toMatch(/organization|member|invitation|billing/i);
	});
});
