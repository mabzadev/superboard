...
import { createBlock } from "../../../../../../../../../packages/plugins/superboard-acquisition/src/front/flows/features/flows/editor/graph.js";
import { InspectorPanel } from "../../../../../../../../../packages/plugins/superboard-acquisition/src/front/flows/features/flows/editor/InspectorPanel.js";
...
describe("Flows inspector", () => {
	...
	it("persists translated content on blur through the workflow contract", async () => {
		...
		expect(onTranslate).toHaveBeenCalledWith(block.key, "title", "fr", "Bienvenue");
		expect(await screen.findAllByText(/^(translated|traduite)$/i)).not.toHaveLength(0);
	});
	...
	it("switches a block to an exact versioned Basics V2 definition", async () => {
		...
		await user.click(screen.getByRole("combobox", { name: /^Component definition/ }));
