...
import { BlockCatalog } from "../../../../../../../../../packages/plugins/superboard-acquisition/src/front/flows/features/flows/editor/BlockCatalog.js";
import { BLOCK_CATALOG } from "../../../../../../../../../packages/plugins/superboard-acquisition/src/front/flows/features/flows/editor/graph.js";
...
describe("Flows block catalog", () => {
	it("renders every canonical block category and filters by behavior", async () => {
		...
		for (const definition of BLOCK_CATALOG) {
			expect(
				screen.getByRole("button", {
					name: (name) => name === `${definition.label} ${definition.description}`,
				}),
			).toBeInTheDocument();
		}
		...
		expect(screen.queryByRole("button", { name: /^Manual start/ })).not.toBeInTheDocument();
	...
	it("publishes the stable drag payload consumed by the graph canvas", () => {
		...
		expect(setData).toHaveBeenCalledWith("application/superboard-flow-block", "delay");
	});
	...
	it("uses the API component definitions instead of generic SDK placeholders", async () => {
		...
		render(<BlockCatalog components={[tooltip]} onAdd={onAdd} onAddComponent={onAddComponent} />);

		expect(screen.getByText("Components")).toBeInTheDocument();
		expect(screen.queryByRole("button", { name: /^ComponentRender/ })).not.toBeInTheDocument();
