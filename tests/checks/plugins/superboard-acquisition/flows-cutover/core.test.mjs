...
import {
	FixtureFlowsCutoverAdapter,
	RemoteFlowsCutoverAdapter,
} from "../../../../../packages/plugins/superboard-acquisition/scripts/flows-cutover/adapters.mjs";
import {
	canonicalJson,
	sha256,
} from "../../../../../packages/plugins/superboard-acquisition/scripts/flows-cutover/core-primitives.mjs";
import {
...
} from "../../../../../packages/plugins/superboard-acquisition/scripts/flows-cutover/core.mjs";
import {
	FLOW_CUTOVER_ENTITIES,
	FLOW_CUTOVER_ENTITY_BY_ID,
	FLOW_SOURCE_QUERIES,
} from "../../../../../packages/plugins/superboard-acquisition/scripts/flows-cutover/registry.mjs";
...
test("cutover registry is strictly project/environment scoped and excludes SaaS billing tables", () => {
	for (const entity of FLOW_CUTOVER_ENTITIES) {
		...
		assert.doesNotMatch(
			entity.table,
			/organization|member|invitation|billing|mtu|usage/u,
			entity.id,
		);
	}
	assert.deepEqual(FLOW_CUTOVER_ENTITY_BY_ID.get("paywalls.claims").keys, [
		"project_id",
		"source_module",
		"event_id",
	]);
	assert.deepEqual(FLOW_CUTOVER_ENTITY_BY_ID.get("paywalls.analytics").keys, [
		"project_id",
		"event_id",
	]);
});
...
test("backup evidence covers API, both legacy sources and Flows before any schema or routing write", () => {
	...
	assert.equal(
		backupPlan.databases.find((database) => database.logical_name === "api").database_name,
		"superboard-dev-db",
	);
	assert.ok(
		backupPlan.databases.every(
			(database) =>
...
test("backup receipt validation fails closed when API D1 evidence is absent", () => {
	...
	assert.throws(() => validateBackupReceiptForBackupPlan(receipt, backupPlan), /API|api/u);
});
...
test("remote user hash resolution is ordered, batched and never receives the HMAC secret", async () => {
	...
	const resolved = await adapter.resolveUserHashes({ project_ref: "42-test" }, rawUserIds);
	...
	assert.equal(
		resolved.get("user-0"),
		createHmac("sha256", "server-only-test-key").update("42-test\nuser-0").digest("hex"),
	);
});
...
test("routing verification evidence is exact-plan bound and rejects incomplete reports", async () => {
	...
	failed.ready = false;
	assert.throws(() => routingVerificationEvidence(plan, failed), /successful verification report/u);
	const foreign = structuredClone(verification);
	foreign.project.project_id = 999;
	assert.throws(() => routingVerificationEvidence(plan, foreign), /exact protected plan\/project/u);
});
...
test("the API promotion routing gate requires both fixed MBZA project scopes", async () => {
	...
	assert.deepEqual(
		report.scopes.map((scope) => scope.matches),
		[true, true],
	);
...
test("remote routing writes are direct, project-scoped, idempotent and evidence-bound", async () => {
	...
	assert.match(
		executed[0].sql,
		/project_id,enabled,window_id,plan_id,verification_checksum_sha256/u,
	);
...
test("direct maintenance writes and verifies the API D1 state without an operator token", async () => {
	...
	assert.doesNotMatch(executed[0].sql, /SUPERBOARD_CUTOVER_TOKEN/u);
});
...
test("a routing receipt is replayed only when its payload and active state still match", async () => {
	...
	await assert.rejects(
		adapter.setFlowsRouting("42-test", state, `flows-routing-enable-${"c".repeat(64)}`),
...
test("protected local hash resolution matches the Worker without requiring the new API route", async () => {
	...
	const resolved = await adapter.resolveUserHashes({ project_ref: "42-test" }, [
		"customer-1",
		"customer-1",
		"customer-2",
	]);
...
test("plan conversion and canonical checksums are deterministic", async () => {
	...
	assert.deepEqual(
		first.entities.map((item) => item.expected),
		second.entities.map((item) => item.expected),
	);
	...
	const analytics = first.entities.flatMap((item) =>
		item.id.endsWith(".analytics") ? item.rows : [],
	);
	assert.equal(analytics.length, 2);
	assert.deepEqual(analytics.map((row) => row.event_id).sort(), [
		"legacy:onboardings:paywall-event-1",
		"legacy:paywalls:paywall-event-1",
	]);
	assert.deepEqual(analytics.map((row) => row.source_event_id).sort(), [
		"paywall-event-1",
		"paywall-event-1",
	]);
	assert.equal(
		analytics.find((row) => row.source_module === "paywalls").user_id_hash,
		createHmac("sha256", "fixture-flow-user-hash-key").update("42-test\ncustomer-1").digest("hex"),
	);
	...
	assert.deepEqual(
		claims.map((row) => row.event_id).sort(),
		analytics.map((row) => row.source_event_id).sort(),
	);
	const onboardingExperiment = first.entities.find((item) => item.id === "onboardings.experiments")
		.rows[0];
	...
	assert.ok(
		analytics.every((row) => !new Set(["purchase", "installation", "install"]).has(row.event_name)),
	);
	assert.ok(
		analytics.every((row) => {
			const properties = JSON.parse(row.properties_json);
			return (
				properties.analytics_namespace === "flows.legacy" &&
				properties.verified_fact_eligible === false &&
				properties.legacy.imported_history === true &&
				new Set(["paywalls", "onboardings"]).has(properties.legacy.source) &&
				properties.excluded_metrics.includes("purchase") &&
				properties.excluded_metrics.includes("installation")
			);
		}),
	);
	assert.equal(
		JSON.parse(analytics.find((row) => row.source_module === "paywalls").properties_json).legacy
			.authoritative_purchase,
		true,
	);
	assert.equal(
		JSON.parse(analytics.find((row) => row.source_module === "onboardings").properties_json).legacy
			.authoritative_purchase,
		false,
	);
	...
	assert.ok(
		!first.entities.some((item) =>
			/purchase|installation|billing|mtu|usage/u.test(item.target_table),
		),
	);
	...
	assert.deepEqual(
		datasetEvidence(rows, workflows),
		datasetEvidence([...rows].reverse(), workflows),
	);
...
test("apply is idempotent and a verified checkpoint avoids duplicate writes", async () => {
	...
	let checkpoint;
	const first = await applyFlowsCutoverPlan({
		plan,
		adapter,
		safety,
		onCheckpoint: (value) => {
	...
	const second = await applyFlowsCutoverPlan({
		plan,
		adapter,
		safety,
		checkpoint,
...
test("an interrupted apply resumes only after the last verified entity", async () => {
	...
	assert.equal(
		adapter.upsertCalls.length,
		successfulBeforeResume +
			plan.entities.filter((item, index) => index >= 2 && item.expected.count > 0).length,
	);
});
...
test("runtime legacy events may append during freeze without invalidating imported checksums", async () => {
	...
	const currentRowsByEntity = Object.fromEntries(
		await Promise.all(
			plan.entities.map(async (item) => [
				item.id,
				await adapter.readTarget(FLOW_CUTOVER_ENTITY_BY_ID.get(item.id)),
			]),
		),
	);
...
test("tampered rows fail closed and identical raw ids are safely namespaced across modules", async () => {
	...
	assert.throws(
		() => assertPlanIntegrity(organizationScoped),
		/scoped only by project and environment/u,
	);
	const preCompositeEventKeyPlan = structuredClone(plan);
	preCompositeEventKeyPlan.tool_version = "2.1.0";
	assert.throws(
		() => assertPlanIntegrity(preCompositeEventKeyPlan),
		/Unsupported Flows cutover plan/u,
	);
	...
	const namespaced = await buildFlowsCutoverPlan({
		snapshot: collision,
		adapter: new FixtureFlowsCutoverAdapter(),
		environmentId: "env-1",
	});
...
test("a canonical Paywall release preserves simultaneous placements on different versions", async () => {
	...
	assert.deepEqual([...new Set(commerce.map((block) => block.slotId))].sort(), [
		"settings",
		"upgrade",
	]);
	assert.ok(
		commerce.some(
			(block) =>
				block.slotId === "settings" &&
				block.data.original_definition.components[0].text === "Premium",
		),
	);
	assert.ok(
		commerce.some(
			(block) =>
				block.slotId === "upgrade" &&
				block.data.original_definition.components[0].text === "Business",
		),
	);
	const split = graph.blocks.find(
		(block) =>
			block.id ===
			`${release.workflow_id}:placement:paywall-placement-1:experiment:paywall-experience-1:split`,
	);
	...
	const holdoutPath = graph.paths.find(
		(candidate) => candidate.sourceBlockId === split.id && candidate.sourceExitNode === "holdout",
	);
	...
	assert.ok(
		graph.blocks
			.filter((block) => ["start", "component"].includes(block.type))
			.every((block) => Number.isSafeInteger(block.data.legacy_priority)),
	);
});
...
test("a Paywall release preserves simultaneous placements sharing one version", async () => {
	...
	assert.ok(
		graph.blocks.some(
			(block) => block.id === `${release.workflow_id}:placement:paywall-placement-2:start`,
		),
	);
});
...
test("a canonical Onboarding release preserves each placement version, rules and experiment", async () => {
	...
	assert.deepEqual([...new Set(tours.map((block) => block.slotId))].sort(), [
		"first-run",
		"profile-setup",
	]);
	assert.ok(
		tours.some(
			(block) =>
				block.slotId === "first-run" && block.data.original_definition.screens[0].id === "intro",
		),
	);
	assert.ok(
		tours.some(
			(block) =>
				block.slotId === "profile-setup" &&
				block.data.original_definition.screens[0].id === "profile",
		),
	);
	const firstSplit = graph.blocks.find(
		(block) =>
			block.id ===
			`${release.workflow_id}:placement:onboarding-placement-1:experiment:onboarding-experience-1:split`,
	);
	const secondSplit = graph.blocks.find(
		(block) =>
			block.id ===
			`${release.workflow_id}:placement:onboarding-placement-2:experiment:onboarding-experience-2:split`,
	);
	...
	assert.ok(
		graph.blocks.some(
			(block) =>
				block.id ===
				`${release.workflow_id}:placement:onboarding-placement-1:rule:target-rule-1:start`,
		),
	);
	assert.ok(
		graph.blocks.some(
			(block) =>
				block.id ===
				`${release.workflow_id}:placement:onboarding-placement-2:rule:target-rule-2:start`,
		),
	);
});
...
test("legacy traffic holdout preserves exact zero and full basis-point boundaries", async () => {
	...
	const splitBySource = Object.fromEntries(
		["paywalls", "onboardings"].map((source) => {
			const release = plan.entities.find((item) => item.id === `${source}.releases`).rows[0];
			const version = plan.entities
				.find((item) => item.id === `${source}.versions`)
				.rows.find((row) => row.id === release.workflow_version_id);
			const split = JSON.parse(version.graph_json).blocks.find(
				(block) => block.type === "traffic-split",
			);
			return [source, split];
		}),
	);
...
test("an Onboarding release preserves simultaneous placements sharing one version", async () => {
	...
	assert.ok(
		graph.blocks.some(
			(block) => block.id === `${release.workflow_id}:placement:onboarding-placement-2:start`,
		),
	);
});
...
test("the same raw legacy event id in both modules stays isolated and reversible", async () => {
	...
	const eventIds = plan.entities.flatMap((item) =>
		item.id.endsWith(".analytics") ? item.rows.map((row) => row.event_id) : [],
	);
...
test("apply authority is bound to MBZA, the exact D1 identities, window, freeze and confirmation", async () => {
	...
	assert.throws(
		() => assertMbzaDevelopment("reference-production", "production"),
		/production cutover is intentionally disabled/u,
	);
	...
	const backupPlan = createBackupPlan({
		...
		resources: fakeResources(),
		workers: fakeWorkers(),
		outputDirectory: "/protected/apply",
		generatedAt: CAPTURED_AT,
	});
	const backupReceipt = {
		schema_version: FLOWS_CUTOVER_SCHEMA_VERSION,
	...
	const freezeReceipt = {
		...
		plan_id: "pre-freeze-plan-id",
		window_id: window.window_id,
		enabled: true,
		confirmed_at: CAPTURED_AT,
	};
	const confirm = confirmationValue(
		"CUTOVER",
	...
	assert.throws(
		() =>
			validateApplySafety({
				...
				window,
				backupPlan,
				backupReceipt: forged,
				freezeReceipt,
				confirm,
				now: new Date(CAPTURED_AT),
			}),
		/canonical backup plan|protected import plan/u,
	);
});
...
test("reverse delta and rollback simulation preserve evidence and never delete, restore, or thaw", async () => {
	...
	const onboardingExperience = delta.destinations.find(
		(item) => item.source_module === "onboardings" && item.table === "experiences",
	);
	...
	const paywallWorkflow = delta.destinations.find(
		(item) => item.source_module === "paywalls" && item.table === "paywalls",
	);
	assert.equal(paywallWorkflow.rows[0].id, "paywall-1");
	const paywallVersion = delta.destinations.find(
		(item) => item.source_module === "paywalls" && item.table === "paywall_versions",
	);
	assert.equal(paywallVersion.rows[0].id, "paywall-version-1");
	assert.equal(paywallVersion.rows[0].paywall_id, "paywall-1");
	const paywallPlacement = delta.destinations.find(
		(item) => item.source_module === "paywalls" && item.table === "placements",
	);
	...
	const paywallVariant = delta.destinations.find(
		(item) => item.source_module === "paywalls" && item.table === "variants",
	);
	...
	const onboardingRule = delta.destinations.find(
		(item) => item.source_module === "onboardings" && item.table === "targeting_rules",
	);
	...
	assert.ok(
		rollback.manual_emergency_only.every((item) => item.warning.includes("never executed")),
	);
	...
	const anotherBackupPlan = createBackupPlan({
		...
		resources: fakeResources(),
		workers: fakeWorkers(),
		outputDirectory: "/protected/other",
		generatedAt: CAPTURED_AT,
	});
	assert.throws(
		() =>
	...
	assert.ok(
		blocked.blockers.some((item) => item.includes("deletions_require_manual_reconciliation")),
	);
});
...
test("the complete generated import executes twice with identical raw ids isolated across projects", async () => {
	...
	try {
		const migrationDirectory = resolve("packages/plugins/superboard-acquisition/flows/migrations");
    ...
    `;
		const otherProjectSql = otherPlan.entities
			.map((item) => upsertSql(FLOW_CUTOVER_ENTITY_BY_ID.get(item.id), item.rows))
			.join("\n");
		const importSql = plan.entities
			.map((item) => upsertSql(FLOW_CUTOVER_ENTITY_BY_ID.get(item.id), item.rows))
			.join("\n");
		const execution = spawnSync("sqlite3", [database], {
			input: [...migrations, seeds, otherProjectSql, importSql].join("\n"),
			encoding: "utf8",
		...
		for (const item of plan.entities) {
			...
			assert.equal(
				queryResult.status,
				0,
				`${entity.id}: ${queryResult.stderr || queryResult.stdout}`,
			);
		}
		const query = spawnSync(
			...
			[
				database,
				"SELECT (SELECT count(*) FROM flow_workflows),(SELECT count(*) FROM flow_analytics_events),(SELECT count(*) FROM flow_legacy_event_claims);",
			],
			{ encoding: "utf8" },
		);
		assert.equal(query.status, 0, query.stderr || query.stdout);
		assert.equal(query.stdout.trim(), "4|4|4");
		const repeatedEvents = spawnSync(
			"sqlite3",
			[
				database,
				"SELECT count(*) FROM (SELECT event_id FROM flow_analytics_events GROUP BY event_id HAVING count(*) = 2);",
			],
			{ encoding: "utf8" },
		);
		...
		const isolatedGlobalIds = spawnSync(
			"sqlite3",
			[
				database,
				`
    ...
    `,
		...
		const forbiddenTables = spawnSync(
			"sqlite3",
			[
				database,
				"SELECT count(*) FROM sqlite_master WHERE type='table' AND name IN ('flow_organizations','flow_members','flow_invitations','flow_billing_cycles','flow_mtu_users','flow_usage_alerts');",
			],
			{ encoding: "utf8" },
		);
...
test("every source extraction query executes against the real legacy D1 schemas", async () => {
	...
	try {
		for (const source of ["paywalls", "onboardings"]) {
			...
			const migrationDirectory = resolve(`${servicePackagePath(source)}/migrations`);
			const migrationNames = (await readdir(migrationDirectory))
				.filter((name) => name.endsWith(".sql"))
				.sort();
			const migrations = await Promise.all(
			...
			const inserts = Object.entries(snapshot.sources[source].tables).flatMap(([table, rows]) =>
				rows.map((row) => {
			...
			for (const descriptor of FLOW_SOURCE_QUERIES[source]) {
				...
				assert.equal(
					result.status,
					0,
					`${source}.${descriptor.table}: ${result.stderr || result.stdout}`,
				);
			}
		}
	} finally {
		...
		sources: {
			paywalls: {
				...
				tables: {
					...
					paywall_versions: [
						{
							...
							definition_json: JSON.stringify({
								schema_version: 1,
								components: [{ type: "title", text: "Premium" }],
								theme: { accent: "#f00" },
					...
					placements: [
						{
							id: "paywall-placement-1",
							project_id: 7,
							key: "settings",
							paywall_id: "paywall-1",
							active_version_id: "paywall-version-1",
							experience_id: "paywall-experience-1",
							targeting_json: JSON.stringify({ platforms: ["ios"] }),
							priority: 100,
							active: 1,
							created_at: "2026-08-01T10:00:00.000Z",
							updated_at: CAPTURED_AT,
					...
					variants: [
						{
							id: "paywall-variant-1",
							project_id: 7,
							experience_id: "paywall-experience-1",
							version_id: "paywall-version-1",
							key: "control",
							weight: 100,
							active: 1,
							created_at: "2026-08-01T10:00:00.000Z",
					...
					events: [
						{
							...
							payload_json: JSON.stringify({ product_id: "premium.monthly" }),
							paywall_id: "paywall-1",
							version_id: "paywall-version-1",
							experience_id: "paywall-experience-1",
							variant_id: "paywall-variant-1",
							platform: "ios",
			...
			onboardings: {
				...
				tables: {
								...
								screens: [
									{
										id: "intro",
										blocks: [
											{ type: "marketing_consent", props: { default: true, required: true } },
										],
									},
								],
								theme: {},
					...
					targeting_rules: [
						{
							id: "target-rule-1",
							project_id: 7,
							placement_id: "onboarding-placement-1",
							name: "French",
							priority: 100,
							conditions_json: JSON.stringify([{ key: "locale", operator: "equals", value: "fr" }]),
							active: 1,
							created_at: "2026-08-01T11:00:00.000Z",
							updated_at: CAPTURED_AT,
						},
					...
					experience_variants: [
						{
							id: "onboarding-variant-1",
							project_id: 7,
							experience_id: "onboarding-experience-1",
							name: "control",
							weight: 10000,
							version_id: "onboarding-version-1",
							created_at: "2026-08-01T11:00:00.000Z",
						},
					],
					events: [
						{
							...
							payload_json: JSON.stringify({ completed: true }),
							platform: "web",
							onboarding_id: "onboarding-1",
							version_id: "onboarding-version-1",
							experience_id: "onboarding-experience-1",
							variant_id: "onboarding-variant-1",
							step_id: "intro",
...
function fakeWorkers() {
	return Object.fromEntries(
		["paywalls", "onboardings", "api", "dashboard", "flows"].map((service) => [
			service,
