...
test("generates independent RS256 JWT and SAML material", async () => {
	const secrets = await generateMelodyAuthSecrets(new Date("2026-08-13T00:00:00.000Z"));
	...
	assert.deepEqual(Object.keys(JSON.parse(serialized)).sort(), ["jp", "ju", "sc", "sp", "ss", "v"]);
});
