...
describe("Identity Worker with D1", () => {
	it("reports the applied D1 schema revision", async () => {
		...
		await expect(response.json()).resolves.toMatchObject({
			...
			schema: {
				status: "current",
				expectedMigration: "0150_application_user_administration.sql",
				latestMigration: "0150_application_user_administration.sql",
				appliedMigrationCount: 52,
			},
			project_scope: { ready: true, unscoped_rows: 0 },
		});
	});

	it("rotates anonymous sessions and issues a distinct SuperBoard identity token", async () => {
		...
		const missingLinkAuth = await SELF.fetch("https://identity.test/auth/link/google", {
			method: "POST",
			headers: { "content-type": "application/json" },
			body: JSON.stringify({ token: "not-accepted-without-authentication" }),
		});
		expect(missingLinkAuth.status).toBe(401);
		const unsupportedLink = await SELF.fetch("https://identity.test/auth/link/microsoft", {
			method: "POST",
			headers: {
		...
		const exchanged = await json<{ access_token: string; expires_in: number }>(
			await SELF.fetch("https://identity.test/auth/superboard-token", {
				method: "POST",
		...
		const afterLogout = await SELF.fetch("https://identity.test/auth/superboard-token", {
			method: "POST",
			headers: { authorization: `Bearer ${refreshed.access_token}` },
		});
	...
	it("does not reveal whether a password-reset email exists", async () => {
		const response = await SELF.fetch("https://identity.test/auth/request-password-reset", {
			method: "POST",
	...
	it("erases identity only through the authenticated internal contract", async () => {
		...
		const incompletePublicDeletion = await SELF.fetch("https://identity.test/auth/me", {
			method: "DELETE",
			headers: { authorization: `Bearer ${created.access_token}` },
		});
		...
		const eraseRequest = async () =>
			SELF.fetch(`https://identity.test/internal/v1/users/${created.user.id}`, {
				method: "DELETE",
				headers: await projectHeaders("DELETE", `/internal/v1/users/${created.user.id}`),
			});
		...
		const afterErasure = await SELF.fetch("https://identity.test/auth/superboard-token", {
			method: "POST",
			headers: { authorization: `Bearer ${created.access_token}` },
		});
	...
	it("exposes sanitized authentication state only through the internal admin contract", async () => {
		...
		const unauthorized = await SELF.fetch("https://identity.test/internal/v1/admin/users");
		expect(unauthorized.status).toBe(401);
		...
		const detailResponse = await SELF.fetch(
			...
			{
				headers: await projectHeaders("GET", `/internal/v1/admin/users/${created.user.id}`),
			},
		);
		...
		expect(detailText).not.toContain("provider-subject-hash-must-never-leave-identity");
	...
	it("isolates users, providers, sessions and details between projects", async () => {
		const projectOne = await anonymous("shared-installation-identity-isolation", "192.0.2.31");
		...
		expect(projectOneList.data.map(({ id }) => id)).toContain(projectOne.user.id);
		expect(projectOneList.data.map(({ id }) => id)).not.toContain(projectTwo.user.id);

		const crossProjectDetail = await SELF.fetch(
			`https://identity.test/internal/v1/admin/users/${projectTwo.user.id}`,
			{
				headers: await projectHeaders("GET", `/internal/v1/admin/users/${projectTwo.user.id}`),
			},
		);
	...
	it("serves the native Melody OIDC engine only through the auth gateway", async () => {
		...
		const discovery = await json<Record<string, unknown>>(
			await SELF.fetch("https://identity.test/.well-known/openid-configuration", {
				headers: gatewayHeaders,
			}),
		);
		expect(discovery).toMatchObject({
			issuer: "https://auth.example.test",
			authorization_endpoint: "https://auth.example.test/oauth2/v1/authorize",
	...
	it("renders Melody's hosted authorization UI and bundled assets", async () => {
		const database = env as unknown as { DB: D1Database };
		const client = await database.DB.prepare("SELECT clientId FROM app WHERE id=1").first<{
			clientId: string;
		}>();
		...
		expect(response.headers.get("location")).toContain("/identity/v1/view/authorize?");
	...
	it("runs the native Cloudflare SAML SP flow with signed assertions and replay protection", async () => {
		...
		const certificate = secrets.samlCertificatePem.replace(
       ...
       VALUES (?,?,?,?,?,?,1)`,
			...
			.bind("runtime", "uid", "email", "first_name", "last_name", metadata)
			.run();

		const spMetadata = await SELF.fetch("https://identity.test/saml/sp/v1/metadata", {
			headers: { "x-superboard-auth-gateway": "1" },
		});
		...
		expect(await spMetadata.text()).toContain("https://auth.example.test/saml/sp/v1/acs");

		const client = await runtime.DB.prepare("SELECT clientId FROM app WHERE id=1").first<{
			clientId: string;
		}>();
		const authorize = new URL("https://identity.test/oauth2/v1/authorize");
		authorize.searchParams.set("client_id", client?.clientId ?? "");
		...
		const login = await SELF.fetch(new URL(begin.headers.get("location") ?? "", authorize), {
			redirect: "manual",
			headers: { "x-superboard-auth-gateway": "1" },
		});
		...
		const postAssertion = () =>
			SELF.fetch("https://identity.test/saml/sp/v1/acs", {
				method: "POST",
				redirect: "manual",
				headers: {
	...
	it("isolates Melody applications behind the signed SuperBoard project realm", async () => {
		...
		expect(projectOneList.apps.map(({ id }) => id)).toContain(projectOneApp.app.id);
		expect(projectOneList.apps.map(({ id }) => id)).not.toContain(projectTwoApp.app.id);

		const crossProject = await melodyAdmin("GET", `/api/v1/apps/${projectTwoApp.app.id}`);
	...
	it("isolates roles and other Melody administration resources between projects", async () => {
		...
		const crossProject = await melodyAdmin("GET", `/api/v1/roles/${projectTwoRole.role.id}`);
		expect(crossProject.status).toBe(404);
	...
	it("isolates Melody audit logs and scoped deletion between projects", async () => {
		...
		await database.DB.batch([
         ...
         VALUES (1,?,?,?,?)`,
			).bind("project-one@example.test", "runtime-project-one-email", "project one email", 101),
			database.DB.prepare(
				`INSERT INTO email_log
           (success,receiver,response,content,"projectId")
         VALUES (1,?,?,?,?)`,
			).bind("project-two@example.test", "runtime-project-two-email", "project two email", 202),
			database.DB.prepare(
				`INSERT INTO sms_log
           (success,receiver,response,content,"projectId")
         VALUES (1,?,?,?,?)`,
			).bind("+1555010101", "runtime-project-one-sms", "project one sms", 101),
			database.DB.prepare(
				`INSERT INTO sms_log
           (success,receiver,response,content,"projectId")
         VALUES (1,?,?,?,?)`,
			).bind("+1555020202", "runtime-project-two-sms", "project two sms", 202),
			database.DB.prepare(
				`INSERT INTO sign_in_log
           ("userId",ip,detail,"projectId") VALUES (0,?,?,?)`,
			).bind("192.0.2.101", "runtime-project-one-sign-in", 101),
			database.DB.prepare(
				`INSERT INTO sign_in_log
           ("userId",ip,detail,"projectId") VALUES (0,?,?,?)`,
			).bind("192.0.2.202", "runtime-project-two-sign-in", 202),
		]);
		...
		for (const resource of resources) {
			...
			const projectTwo = await melodyAdminJson<{
				logs: Array<{ id: number }>;
			}>("GET", `/api/v1/logs/${resource.route}`, undefined, secondProject);
			...
			expect(
				(await melodyAdmin("GET", `/api/v1/logs/${resource.route}/${resource.ids.two}`)).status,
			).toBe(404);
			expect(
				(
					await melodyAdmin(
			...
			expect(
				(await melodyAdmin("GET", `/api/v1/logs/${resource.route}/${resource.ids.one}`)).status,
			).toBe(404);
			expect(
				(
					await melodyAdmin(
	...
	it("reuses the canonical application user id instead of duplicating an invited user", async () => {
       ...
       VALUES (?,?,?, ?,0,CURRENT_TIMESTAMP)`,
			...
			.bind(canonicalId, 101, "canonical@example.test", "Existing SuperBoard user")
			.run();
	...
	it("fails closed while a migrated legacy identity remains unscoped", async () => {
		const database = env as unknown as { DB: D1Database };
		await database.DB.prepare("DROP TRIGGER application_users_project_required_insert").run();
		await database.DB.prepare("INSERT INTO application_users (id,is_anonymous) VALUES (?,1)")
			.bind("legacy-unscoped-runtime-user")
			.run();
		try {
			const admin = await SELF.fetch("https://identity.test/internal/v1/admin/users", {
				headers: await projectHeaders("GET", "/internal/v1/admin/users"),
			});
			...
			await database.DB.prepare("DELETE FROM application_users WHERE id=? AND project_id IS NULL")
				.bind("legacy-unscoped-runtime-user")
				.run();
			await database.DB.prepare(
...
async function melodyAdmin(
	...
	const target = new URL(`/internal/v1/melody-admin${suffix}`, "https://identity.test");
...
async function melodyServiceToken(app: { clientId: string; secret: string }): Promise<string> {
