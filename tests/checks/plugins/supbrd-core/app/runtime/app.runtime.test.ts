...
describe("App Worker with D1", () => {
	it("aggregates acquisition activity and returns a stable customer upsert id", async () => {
		...
		const events: Array<Record<string, unknown>> = eventTypes.map((type, index) => ({
			id: `event-a-${index}`,
		...
		expect(
			(await mutate("POST", "/internal/v1/customer-events", { events }, "events-a")).status,
		).toBe(200);

		const customers = await json<{
			data: Array<Record<string, unknown>>;
			meta: { total: number };
		}>(await request("GET", "/internal/v1/customers?from=2026-08-07&to=2026-08-07&platform=ios"));
		expect(customers.data.find((row) => row.external_id === "invited-a")).toMatchObject({
			total_views: 1,
			total_opens: 1,
			total_installs: 1,
			total_reinstalls: 1,
		...
		const referrals = await data<Array<Record<string, unknown>>>(
			await request("GET", "/internal/v1/referrals?from=2026-08-07&to=2026-08-07&platform=ios"),
		);
		...
		const access = await mutate("POST", "/internal/v1/access-key/rotate", {}, "access-a");
			...
			(
				await mutate(
					...
					"/internal/v1/setup/ios",
					{
						...
						store_url: "https://apps.example.test/reference-production",
					},
					"setup-a",
				)
			).status,
		...
		await expect(
			(await mutate("POST", "/internal/v1/setup/ios/test", {}, "setup-test-a")).json(),
	...
	it("isolates metrics for multiple referrers and invited customers", async () => {
		...
		const referrals = await data<Array<Record<string, unknown>>>(
			await request("GET", "/internal/v1/referrals?from=2026-08-07&to=2026-08-07"),
		);
		expect(referrals.find((row) => row.code === "REFERRER-A")).toMatchObject({
			views: 1,
			total_revenue: 1299,
	...
	it("applies IANA timezone day boundaries and rejects invalid dates", async () => {
		const customer = await createCustomer("timezone-customer", "timezone-customer");
		const events = [
		...
		expect(
			(await mutate("POST", "/internal/v1/customer-events", { events }, "timezone-events")).status,
		).toBe(200);
	...
	it("enforces composite project foreign keys", async () => {
		...
		await expect(
			env.DB.prepare(
				...
				.bind("direct-cross-project-event", "12", owned.id, "view", "2026-08-07T11:00:00Z")
				.run(),
		).rejects.toThrow();
		await expect(
			env.DB.prepare("INSERT INTO referrals (id,project_id,customer_id,code) VALUES (?,?,?,?)")
	...
	it("cascades customer deletion through referrals and customer events", async () => {
		...
		expect(
			(await mutate("DELETE", `/internal/v1/customers/${invited.id}`, undefined, "delete-customer"))
				.status,
		).toBe(200);
		expect(
			await env.DB.prepare("SELECT id FROM referrals WHERE id=?").bind(referral.id).first(),
		).toBeNull();
		expect(
