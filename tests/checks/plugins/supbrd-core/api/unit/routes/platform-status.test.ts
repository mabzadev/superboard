...
describe("platform status", () => {
	...
	it("aggregates common Worker health and database counters for admins", async () => {
		const db = database();
		const healthy = {
			fetch: vi.fn().mockImplementation(async () => Response.json({ status: "ok" })),
		};
		const email = {
		...
		const observability = {
			fetch: vi.fn().mockResolvedValue(
				Response.json({
					...
					dataset: "superboard_mbza_development",
					windowMinutes: 60,
		...
		const response = await platform.request(
			...
			},
			{
				...
				SUPERBOARD_RELEASE: "abcdef123456",
		...
		expect(body.services).toHaveLength(JSON.parse(workerCatalog()).workers.length);
		expect(body.services.find((service: any) => service.id === "api")).toMatchObject({
		...
		expect(body.services.find((service: any) => service.id === "email")).toMatchObject({
			workerName: "superboard-email-dev",
			jobs: { messagesQueued: 2, deliveriesFailed: 2 },
		});
		expect(body.dataStores.find((store: any) => store.id === "central")).toMatchObject({
			kind: "D1",
			owner: "api",
			status: "ok",
			schema: currentSchema("0057_application_account_erasure.sql", 57),
		});
		expect(body.dataStores.find((store: any) => store.id === "email")).toMatchObject({
			owner: "email",
			status: "ok",
		});
		expect(body.dataStores.find((store: any) => store.id === "support")).toMatchObject({
			status: "misconfigured",
		});
		expect(body.services.find((service: any) => service.id === "billing").status).toBe("ok");
		expect(body.services.find((service: any) => service.id === "custom").status).toBe(
			"misconfigured",
		);
		expect(body.services.find((service: any) => service.id === "observability").status).toBe("ok");
		expect(body.runtime.rows[0]).toMatchObject({
			service: "superboard-api-dev",
			invocations: 4,
		});
		expect(body.api.capabilities.map((capability: any) => capability.id)).toContain("files");
		expect(body.api.capabilities.map((capability: any) => capability.id)).toContain("support");
		expect(body.api.capabilities.map((capability: any) => capability.id)).toContain("custom-jobs");
		expect(
			body.api.capabilities.find((capability: any) => capability.id === "marketing-consent"),
		).toMatchObject({
			access: "Verified application identity; private signed gateway to Marketing",
			entrypoints: ["/api/v1/sdk/marketing/v1/preferences"],
		});
		expect(
			body.api.capabilities.find((capability: any) => capability.id === "email-operations"),
		).toMatchObject({
			access: "Authenticated Dashboard owner or administrator",
			entrypoints: ["/api/v1/platform/email/*"],
		});
		expect(
			body.api.capabilities.find((capability: any) => capability.id === "files"),
		).toMatchObject({
	...
	it("distinguishes target-disabled Workers from missing required bindings", async () => {
		...
		const body = (await response.json()) as any;
		expect(body.services.find((service: any) => service.id === "messaging")).toMatchObject({
			enabled: false,
			status: "disabled",
			workerName: null,
		});
		expect(body.services.find((service: any) => service.id === "billing")).toMatchObject({
			enabled: true,
			status: "misconfigured",
			workerName: "superboard-billing-dev",
	...
	it("includes target-managed Workers and checks their service-binding reachability", async () => {
		...
		const managedBinding = {
			fetch: vi
				.fn()
				.mockResolvedValue(Response.json({ error: "Method not allowed" }, { status: 405 })),
		};
		const response = await platform.request(
			"/status",
			{ headers: { authorization: "Bearer dashboard-token" } },
		...
		expect(
			body.services.find((service: any) => service.id === "managed-vocals-orchestrator"),
		).toMatchObject({
			kind: "managed",
	...
	it("treats an unknown service health contract as degraded", async () => {
		...
		const body = (await response.json()) as any;
		expect(body.status).toBe("degraded");
		expect(body.services.find((service: any) => service.id === "app")).toMatchObject({
			status: "degraded",
		});
	});
	...
	it("fails closed when a D1 service reports healthy without a current schema", async () => {
		...
		expect(body.dataStores.find((store: any) => store.id === "app")).toMatchObject({
			status: "degraded",
			schema: null,
		});
	});
	...
	it("serves the Git-owned SDK and FlutterFlow catalog only to administrators", async () => {
		...
		expect(body.data.libraries.find((library: any) => library.id === "flutterflow")).toMatchObject({
			lifecycle: "active",
			license: "MIT",
		...
		expect(body.data.customCode.actions.purchases).toContain("superboardGetEntitlements");
		expect(body.data.customCode.streams.support).toContain("superboardSupportEventJsonStream");
		...
		expect(body.data.flutterFlowLibrary.actions.support).toContain("superboardSupportDispose");
	...
	it("propagates a service-reported degraded state", async () => {
		...
		const body = (await response.json()) as any;
		expect(body.status).toBe("degraded");
		expect(body.services.find((service: any) => service.id === "support")).toMatchObject({
			status: "degraded",
	...
	it("rejects oversized or malformed observability summaries without buffering them", async () => {
		...
		expect(
			oversizedBody.services.find((service: any) => service.id === "observability"),
		).toMatchObject({
			status: "unavailable",
		...
		expect(
			malformedBody.services.find((service: any) => service.id === "observability"),
		).toMatchObject({
			status: "unavailable",
		...
		const custom = {
			fetch: vi.fn(async (request: Request | string) => {
				...
				if (url.pathname.endsWith("/retry")) {
					return Response.json({ id: "job-1", status: "dispatched" }, { status: 202 });
				}
				return Response.json({
						...
						results: [
							{
								id: "erase-1",
								project_id: 20,
								project_ref: "reference-production",
...
function workerCatalog(managedWorkers: unknown[] = []) {
	...
	return JSON.stringify({
		...
		workers: [
			...ids.map((id) => ({
				id,
				workerName: id === "messaging" ? null : `superboard-${id}-dev`,
				enabled: id !== "messaging",
				publicSurfaceIds:
					id === "api" ? ["api", "sdk", "shortlinks"] : id === "site" || id === "mcp" ? [id] : [],
			})),
			...managedWorkers,
		],
		customDependencies: [],
