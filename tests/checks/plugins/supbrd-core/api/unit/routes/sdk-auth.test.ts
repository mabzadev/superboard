...
import { sdkMiddleware } from "../../../../../../../packages/plugins/supbrd-core/api/src/middleware/auth";
import sdkRoutes from "../../../../../../../packages/plugins/supbrd-core/api/src/routes/sdk";
import { AppVariables, Env } from "../../../../../../../packages/plugins/supbrd-core/api/src/types";
...
function sdkFixture() {
	const state = {
		...
		applications: [
			{ id: 1, instance_id: 10, platform: "ios", identifier: "com.superboard.ios" },
			{ id: 2, instance_id: 10, platform: "android", identifier: "com.superboard.android" },
			{ id: 3, instance_id: 10, platform: "web", identifier: "app.superboard.test" },
			{ id: 4, instance_id: 10, platform: "desktop", identifier: "io.superboard.desktop" },
		],
		visitors: [
			{
				id: "visitor-101",
				...
				platform: "ios",
				push_token: null,
				sdk_identifier: "user-101",
				sdk_attributes: JSON.stringify({ tier: "gold" }),
			},
			{
				id: "visitor-201",
				uuid: "uuid-201",
		...
		notificationMessages: [
			{
				...
				subtitle: "Hello",
				auto_display: 1,
				notification_updated_at: "2026-01-01T00:00:00.000Z",
			},
			{
				message_id: 902,
				notification_id: 802,
...
function sdkD1Handler(call: FakeD1Call, state: ReturnType<typeof sdkFixture>["state"]) {
	...
	if (op === "first" && sql.includes("FROM projects WHERE instance_id = ? AND is_test = ?")) {
		return (
			state.projects.find(
				(project) => project.instance_id === Number(args[0]) && project.is_test === Number(args[1]),
			) || null
		);
	}
	...
	if (op === "first" && sql.includes("JOIN ios_configurations")) {
		...
		return (
			state.applications.find(
				(app) =>
					app.instance_id === Number(instanceId) &&
					app.platform === "ios" &&
	...
	if (op === "first" && sql.includes("FROM projects WHERE id = ? AND instance_id = ?")) {
		return (
			state.projects.find(
				(project) => project.id === Number(args[0]) && project.instance_id === Number(args[1]),
			) || null
		);
	}
	...
	if (op === "first" && sql.includes("FROM visitors v JOIN devices d ON d.id = v.device_id")) {
		...
		return (
			state.visitors.find(
				(visitor) =>
					(visitor.id === linksquared || visitor.uuid === linksquared) &&
					visitor.project_id === projectId,
			) || null
		);
	}
	...
	if (op === "first" && sql.includes("FROM links l LEFT JOIN domains d ON d.id = l.domain_id")) {
		...
		return (
			state.links.find((link) => {
				...
				const config = state.redirectConfigs.find(
					(candidate) => candidate.id === Number(link.redirect_config_id),
				);
				return (
					link.path === path &&
					(domain?.project_id === projectId || config?.project_id === projectId)
				);
			}) || null
		);
	}
	...
	if (
		op === "first" &&
		sql.startsWith("SELECT id FROM visitors WHERE project_id = ? AND device_id = ?")
	) {
		const visitor = state.visitors.find(
			(candidate) =>
				candidate.project_id === Number(args[0]) && candidate.device_id === Number(args[1]),
		);
	...
	if (op === "all" && sql.includes("FROM notification_messages nm JOIN notifications n")) {
		...
		return state.notificationMessages.filter((message) => {
			...
			if (sql.includes("COALESCE(n.auto_display, 0) = 1") && message.auto_display !== 1)
				return false;
			return true;
		});
	}
	...
	if (op === "first" && sql.includes("SELECT COUNT(*) AS total FROM notification_messages")) {
		return {
			total: state.notificationMessages.filter(
				(message) => message.visitor_id === args[0] && message.read === 0,
			).length,
		};
	}

	if (op === "first" && sql.startsWith("UPDATE notification_messages")) {
		const message = state.notificationMessages.find(
			(candidate) => candidate.message_id === Number(args[0]) && candidate.visitor_id === args[1],
		);
	...
	if (op === "run" && sql.startsWith("INSERT OR IGNORE INTO analytics_fact_outbox")) {
		if (
			!state.analyticsFacts.some(
				(row) => row.project_id === String(args[1]) && row.fact_key === String(args[2]),
			)
		) {
	...
	if (op === "first" && sql.includes("FROM analytics_fact_outbox")) {
		return (
			state.analyticsFacts.find(
				(row) =>
					row.project_id === String(args[0]) &&
					row.fact_key === String(args[1]) &&
	...
	if (
		op === "first" &&
		sql.includes("SELECT id, instance_id, is_test FROM projects WHERE id = ?")
	) {
...
describe("SDK auth contract", () => {
	it("accepts the dashboard access key for the production mobile project", async () => {
		...
		const response = await probeApp().request(
			"/authenticate",
			{
				method: "POST",
				headers: {
					...
					PLATFORM: "android",
					IDENTIFIER: "com.superboard.android",
				},
			},
			env,
		);
	...
	it("rejects API credentials passed in the URL query string", async () => {
		const { env } = sdkFixture();
		const response = await probeApp().request(
			"/authenticate?api_key=server-api-key",
			{
				method: "POST",
			},
			env,
	...
	it("rejects LINKSQUARED visitors from another project", async () => {
		...
		const response = await sdkRoutes.request(
			"/event",
			{
				method: "POST",
				headers: {
	...
	it("ingests SDK events for the current project visitor", async () => {
		...
		expect(state.events).toMatchObject([
			{ device_id: 500, project_id: 101, event: "purchase", platform: "ios", engagement_time: 42 },
		]);
	});

	it("creates SDK links and reads them back scoped to the server SDK project", async () => {
		const { env, state } = sdkFixture();
		const create = await sdkRoutes.request(
			"/generate_link",
			{
				method: "POST",
				headers: {
		...
		const read = await sdkRoutes.request(
			"/link/server-link",
			{
				headers: {
					"PROJECT-KEY": "server-api-key",
		...
		await expect(read.json()).resolves.toMatchObject({
			link: { path: "server-link", title: "Server Link", data: { campaign: "spring" } },
		});
	});
	...
	it("reads and updates visitor attributes including push token", async () => {
		...
		await expect(current.json()).resolves.toMatchObject({
			visitor: { id: "visitor-101", sdk_identifier: "user-101", sdk_attributes: { tier: "gold" } },
		});

		const update = await sdkRoutes.request(
			"/visitor_attributes",
			{
				method: "POST",
				headers,
				body: JSON.stringify({
					sdk_identifier: "user-updated",
					sdk_attributes: { tier: "platinum" },
	...
	it("lists unread notifications and marks them as read", async () => {
		...
		const unread = await sdkRoutes.request(
			"/notifications?device_id=500&project_id=101",
			{ headers },
			env,
		);
		expect(unread.status).toBe(200);
		await expect(unread.json()).resolves.toMatchObject({
			notifications: [{ id: "901", read: false, title: "Welcome" }],
		});

		const count = await sdkRoutes.request(
			"/number_of_unread_notifications?device_id=500&project_id=101",
			{ headers },
			env,
		);
		...
		const mark = await sdkRoutes.request(
		...
		const after = await sdkRoutes.request(
			"/number_of_unread_notifications?device_id=500&project_id=101",
			{ headers },
			env,
		);
