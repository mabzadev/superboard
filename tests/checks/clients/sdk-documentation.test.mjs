...
} from "../../../scripts/clients/sdk-documentation.mjs";
...
test("a catalogue promotion regenerates documentation without frozen state", async () => {
	...
	assert.equal(result.ok, true);
	assert.deepEqual(result.errors, []);
	assert.ok(promotedDocuments.get("sdks/flutterflow/README.md").includes(promotedRef));
});
...
test("all package coordinates and versions are derived state-independently", async () => {
	...
	assert.match(sections.get("javascript"), /@example\/superboard-browser@7\.8\.9/u);
	assert.match(sections.get("react-native"), /@example\/superboard-native@4\.5\.6/u);
	assert.match(sections.get("react-native"), /dev\.example:superboard-android:3\.2\.1/u);
...
test("registry documentation separates public metadata from authenticated installation", async () => {
	...
	for (const source of [javascript, reactNative, android]) {
		...
		assert.match(source, /does not make the[\s\S]*registry anonymously installable/u);
		assert.match(source, /401 Unauthorized/u);
		assert.match(source, /read:packages/u);
		assert.match(source, /SUPERBOARD_GITHUB_PACKAGES_TOKEN/u);
	...
	for (const source of [javascript, reactNative]) {
		...
		assert.match(
			source,
			/\/\/npm\.pkg\.github\.com\/:_authToken=\$\{SUPERBOARD_GITHUB_PACKAGES_TOKEN\}/u,
		);
		assert.ok(source.indexOf("SUPERBOARD_GITHUB_PACKAGES_TOKEN") < source.indexOf("npm install"));
	}

	for (const source of [android, reactNative]) {
		assert.match(source, /https:\/\/maven\.pkg\.github\.com\/mabzadev\/superboard-platform/u);
		assert.match(source, /SUPERBOARD_GITHUB_PACKAGES_USER/u);
		assert.match(source, /SUPERBOARD_GITHUB_PACKAGES_TOKEN/u);
		assert.ok(
			source.indexOf("SUPERBOARD_GITHUB_PACKAGES_USER") <
				source.indexOf("io.superboard:superboard-android-sdk:1.0.3"),
		);
	}

	assert.match(javascript, /npm install @mbzadev\/superboard-js-sdk@1\.0\.2/u);
	assert.match(reactNative, /npm install @mbzadev\/superboard-react-native-sdk@1\.0\.2/u);
...
test("registry shell preflights stop npm and Gradle when credentials are absent", async () => {
	...
	const javascript = sections.get("javascript");
	const android = sections.get("android");
	const javascriptPreflight = [...javascript.matchAll(/```bash\n([\s\S]*?)\n```/gu)]
		.map(([, source]) => source)
	...
	const directory = mkdtempSync(resolve(tmpdir(), "superboard-sdk-preflight-"));
	const marker = resolve(directory, "invoked");
	const fakeNpm = resolve(directory, "npm");
	const fakeGradle = resolve(directory, "gradlew");
	writeFileSync(fakeNpm, '#!/bin/sh\ntouch "$SUPERBOARD_PREFLIGHT_MARKER"\n');
	writeFileSync(fakeGradle, '#!/bin/sh\ntouch "$SUPERBOARD_PREFLIGHT_MARKER"\n');
	chmodSync(fakeNpm, 0o755);
	chmodSync(fakeGradle, 0o755);
	const cleanEnvironment = { ...process.env };
	delete cleanEnvironment.SUPERBOARD_GITHUB_PACKAGES_USER;
	delete cleanEnvironment.SUPERBOARD_GITHUB_PACKAGES_TOKEN;
	cleanEnvironment.SUPERBOARD_PREFLIGHT_MARKER = marker;
	...
	try {
		...
		const userOnlyResult = spawnSync("bash", ["-c", androidPreflight], {
			...
			env: {
				...cleanEnvironment,
				SUPERBOARD_GITHUB_PACKAGES_USER: "developer",
			},
		});
		...
		const authenticatedResult = spawnSync("bash", ["-c", androidPreflight], {
			...
			env: {
				...cleanEnvironment,
				SUPERBOARD_GITHUB_PACKAGES_USER: "developer",
				SUPERBOARD_GITHUB_PACKAGES_TOKEN: "test-token",
			},
		});
...
test("iOS documentation rejects an unsupported CocoaPods Trunk promise", async () => {
	...
	assert.doesNotMatch(ios, /cocoapods\.org\/pods\/SuperBoard/u);
	assert.doesNotMatch(ios, /^pod ['"]SuperBoard['"]$/mu);
...
test("missing, duplicate and reversed documentation markers fail closed", async () => {
	...
	documents.set(
		flutterPath,
		documents.get(flutterPath).replace("<!-- superboard-sdk-documentation:flutter:end -->", ""),
	);
