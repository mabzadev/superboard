...
import {
	...
	releaseTagFor,
	validateSdkCatalog,
} from "../../../scripts/clients/sdk-catalog.mjs";
...
function pendingRename(catalog, id) {
	...
	Object.assign(library, {
		...
		install: installation(`previous_${id}`, published.version),
...
test("schema v5 records published baselines and honest initial Flows packages", async () => {
	...
	assert.deepEqual(immutableCoordinates, {
		"flutterflow-support": {
			packageName: "superboard_flutterflow_messaging",
		...
		"react-native": {
			packageName: "@mbzadev/superboard-react-native-sdk",
			latestReleaseVersion: "1.0.2",
	...
	assert.deepEqual(
		Object.fromEntries(
			flows.map(({ id, packageName, sourceVersion, releaseStatus, publicationTarget }) => [
				id,
	...
	for (const library of flows) {
		for (const field of ["latestReleaseVersion", "releaseRef", "releaseSha", "install"]) {
			assert.equal(
...
test("lifecycle rules freeze non-active packages and preserve release SHAs", async () => {
	...
	assert.ok(result.errors.includes("libraries.javascript.releaseStatus must be released"));
	assert.ok(
		result.errors.includes(
...
test("minimal brand guard protects active names and candidate installs", async () => {
	...
	wrongRepository.repository = "https://github.com/mabzadev/superboard-platform";
	result = await validateSdkCatalog(wrongRepository);
	assert.ok(result.errors.includes("repository must be https://github.com/mabzadev/superboard"));
});
...
test("registry history stays honest, authenticated and secret-free", async () => {
	...
	for (const id of ["android", "javascript", "react-native"]) {
		...
		assert.equal(
			...
			"SUPERBOARD_GITHUB_PACKAGES_TOKEN",
		);
	}
	...
	assert.ok(
		result.errors.some(
			(error) =>
				error.includes("/distribution/authentication") && error.includes("additional properties"),
		),
	);
...
test("FlutterFlow catalogue validates the unified canonical surface", async () => {
	...
	const manifest = JSON.parse(
		await readFile(
			new URL("../../../scripts/config/flutterflow-custom-code.json", import.meta.url),
			"utf8",
		),
	);
	...
	assert.ok(
		result.errors.some((error) =>
			error.includes("FlutterFlow public symbol superboardGetEntitlements is not declared"),
		),
	);
});
test("SDK catalogue rejects a missing or non-MIT package licence", async () => {
	...
	assert.ok(result.errors.includes("libraries.flutterflow.license must be MIT"));
	assert.ok(result.errors.includes("libraries.flutterflow.licensePath does not exist"));
});
...
test("catalogue candidate validation blocks a burned immutable version", async () => {
	...
	Object.assign(ios, {
		...
		releaseRef: "1.0.0",
		releaseStatus: "pending-release",
		install: '.package(url: "https://github.com/mabzadev/superboard-platform.git", exact: "1.0.0")',
	});
	...
	assert.ok(
		result.errors.some((error) => error.includes("bump the SDK version before publishing")),
	);
});
