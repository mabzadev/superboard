...
test("the Flutter podspec consumes only the embedded monorepo links", async () => {
	...
	assert.match(podspec, /'EmbeddedSuperBoard\/\*\*\/\*\.swift'/u);
	assert.match(podspec, /'EmbeddedSuperBoard\/\*\*\/\*\.\{xib\}'/u);
	assert.doesNotMatch(podspec, /\.\.\/\.\.\/ios\/Sources\/SuperBoard/u);
	assert.doesNotMatch(podspec, /s\.dependency\s+['"]SuperBoard/u);
});
