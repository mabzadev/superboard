...
} from "../../../scripts/clients/react-native-npm-audit.mjs";
...
function runImageSizeFixture(source, modulePath) {
	return spawnSync(process.execPath, ["-e", source, modulePath], {
		cwd: reactNativeRoot,
		encoding: "utf8",
		timeout: 2_000,
	});
...
function assertFixtureFinishes(source) {
	const sdkRequire = createRequire(resolve(reactNativeRoot, "package.json"));
	const modules = new Set();
	for (const chain of [
		["react-native", "@react-native/community-cli-plugin", "metro"],
		["react-native-builder-bob", "metro-config", "metro"],
	]) {
		let consumerRequire = sdkRequire;
		for (const name of chain)
			consumerRequire = createRequire(consumerRequire.resolve(`${name}/package.json`));
		assert.equal(
			consumerRequire("image-size/package.json").version,
			imageSizePatchContract.sourceVersion,
		);
		modules.add(consumerRequire.resolve("image-size"));
	}
	for (const modulePath of modules) {
		const child = runImageSizeFixture(source, modulePath);
...
test("image-size lock proof rejects a comment-only marker and extra locator", async () => {
	...
	const commentOnlyLock = inputs.lockSource
		.replace(`"${imageSizePatchDescriptor}":`, `# "${imageSizePatchDescriptor}":`)
		.replace(
...
function parseNpmPackOutput(output) {
	...
	const payload = jsonStart === -1 ? normalized : normalized.slice(jsonStart + 1);
