...
test("immutable SDK failures are strictly recorded with their burned refs", async () => {
	...
	assert.deepEqual(
		history.immutableFailures.map(
			({ libraryId, version, releaseTag, packageRefs, releaseSha, workflowRunId }) => ({
				libraryId,
				version,
				releaseTag,
				packageRefs,
...
test("release history rejects schema drift and invalid immutable metadata", async () => {
	...
	assert.equal((await validateSdkReleaseHistory(invalidSha, catalogue)).ok, false);

	const invalidTag = structuredClone(source);
	invalidTag.immutableFailures[0].releaseTag = "sdk-ios-v9.9.9";
	assert.ok(
		(await validateSdkReleaseHistory(invalidTag, catalogue)).errors.some((error) =>
			error.includes("releaseTag must be sdk-ios-v1.0.1"),
		),
	);

	const duplicateRef = structuredClone(source);
	duplicateRef.immutableFailures[1].releaseTag = "sdk-ios-v1.0.1";
	const duplicateResult = await validateSdkReleaseHistory(duplicateRef, catalogue);
	assert.ok(duplicateResult.errors.some((error) => error.includes("is duplicated")));
});
...
test("release history cannot classify the current successful release as failed", async () => {
	...
	assert.ok(
		result.errors.some((error) => error.includes("conflicts with the current successful release")),
	);
});
...
test("a burned immutable version requires a version bump", async () => {
	...
	assert.doesNotThrow(() => assertSdkReleaseCandidateNotFailed(history, "ios", "1.0.2"));
});
