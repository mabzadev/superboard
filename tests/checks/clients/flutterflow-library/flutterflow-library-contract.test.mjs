...
test("the FlutterFlow catalog publishes only the 18 canonical and 12 extended Support actions", async () => {
	const [library, surface, integrationTest] = await Promise.all([
		readFile(join(root, "scripts/config/flutterflow-library.json"), "utf8").then(JSON.parse),
		readFile(join(root, "scripts/config/flutterflow-custom-code.json"), "utf8").then(JSON.parse),
		readFile(join(root, "tests/checks/sdks/flutterflow/support/actions_test.dart"), "utf8"),
	]);
	...
	assert.deepEqual([...surface.actions.support].sort(), [...actions].sort());
	assert.ok(surface.actions.compatibilityAliases.every((name) => !actions.includes(name)));
...
test("mutable or SSH dependencies are rejected", async () => {
	const source = await readFile(
		join(root, "scripts/clients/flutterflow-library/dsl/edit.dart"),
		"utf8",
	);
	const result = await validateFlutterFlowLibraryContract({
