...
} from "../../../scripts/clients/flutterflow-client-release.mjs";
import {
...
} from "../../../scripts/clients/flutterflow-source-verify.mjs";
...
test("the application source resolves from one portable environment contract", (t) => {
	...
	assert.throws(
		...
		/--source or SUPERBOARD_CLIENT_SOURCE_TEST is required \(SUPERBOARD_CLIENT_SOURCE_TEST remains a migration alias\)/u,
	);
});

test("a generated source change invalidates the reviewed snapshot", (t) => {
	const fixture = createFixture(t);
	writeFileSync(join(fixture.sourcePath, "lib/flutterflow_project/schemas.dart"), "tampered");
...
test("a reviewed snapshot can remain authentic while client convergence is blocked", (t) => {
	...
	write(fixture.sourcePath, "generated_code/lib/main.dart", "void canonicalBootstrap() {}\n");
...
test("an absent scan path fails closed instead of proving a literal absent", (t) => {
	...
	assert.deepEqual(result.convergence.checks[0].missingPaths, ["generated_code/lib"]);
});
...
test("bounded FlutterFlow analyzer defects do not hide new diagnostics", (t) => {
	...
	const sourceRunPath = join(fixture.sourcePath, ".flutterflow/last_run.json");
...
test("the snapshot schema rejects undeclared fields and escaping convergence paths", (t) => {
	...
	assert.throws(() => verifyFlutterFlowSource(fixture), /Invalid FlutterFlow snapshot manifest/u);
});
...
test("an accepted client receipt binds the reviewed snapshot, policy and source tree", (t) => {
	...
	const committedReceipt = new URL(
		"../../../scripts/config/flutterflow-releases/example.json",
		import.meta.url,
	);
	const emittedSchema = JSON.parse(
		readFileSync(new URL(receipt.$schema, committedReceipt), "utf8"),
	);
	assert.equal(new Ajv2020({ strict: false }).compile(emittedSchema)(receipt), true);
...
`,
	...
	const files = {
		...
		"page:first": descriptor(sourcePath, "lib/flutterflow_project/pages/first.dart"),
		"page:second": descriptor(sourcePath, "lib/flutterflow_project/pages/second.dart"),
		"component:first": descriptor(sourcePath, "lib/flutterflow_project/components/first.dart"),
	};
	const lastRun = {
	...
	write(sourcePath, ".flutterflow/generated_code_state.json", JSON.stringify(generatedState));
	write(sourcePath, ".flutterflow/project_sdk_meta.json", JSON.stringify(sdkMeta));
	...
	const manifest = {
		...
		fingerprints: Object.fromEntries(
			...
			].map((relativePath) => [relativePath, hash(join(sourcePath, relativePath))]),
		),
		inventory,
		diagnostics: {
			total: 2,
