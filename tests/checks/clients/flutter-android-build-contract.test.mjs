...
const flutterPubspecUrl = new URL("../../../sdks/flutter/pubspec.yaml", import.meta.url);
const flutterExampleLockUrl = new URL(
	"../../../sdks/flutter/example/pubspec.lock",
	import.meta.url,
);
const flutterFlowLockUrl = new URL("../../../sdks/flutterflow/pubspec.lock", import.meta.url);
...
const semanticVersionPattern = /^[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9A-Za-z.-]+)?(?:\+[0-9A-Za-z.-]+)?$/;
...
function lockedPubVersion(lock, dependency) {
	const block = lock.match(new RegExp(`^  ${dependency}:\\n(?:(?!^  [a-z]).*\\n)*`, "m"))?.[0];
	...
	const visit = async (directory) => {
		for (const entry of await readdir(directory, { withFileTypes: true })) {
			...
			if (entry.isDirectory()) {
				...
				new Set([".dart", ".gradle", ".kt", ".kts", ".java", ".swift"]).has(extname(entry.name))
			) {
				files.push(path);
			}
		}
...
test("Flutter 4 registers one canonical SuperBoard plugin", async () => {
	...
	assert.match(pubspec, /^version: 4\.0\.0$/m);
...
test("Android SDK sources do not generate or consume the obsolete test server field", async () => {
	...
	const files = (
		await Promise.all(sourceRoots.map((sourceRoot) => sourceFilesBelow(sourceRoot)))
	).flat();
	const offenders = [];
