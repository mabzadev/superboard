...
} from "../../../scripts/clients/react-native-native-contract.mjs";
...
test("React Native native contract is derived from published SDK baselines", async () => {
	...
	const repository = ios.install.match(/\.package\(url: "([^"]+)", exact:/u)?.[1];
	assert.ok(repository, "the iOS catalogue exposes its immutable repository");
	const repositoryPath = new URL(repository).pathname.replace(/^\//, "").replace(/\.git$/, "");
...
test("React Native consumers contain no retired Android coordinate", async () => {
	const sources = await Promise.all(
		[
			...
			"../../../sdks/react-native/plugin/withSuperBoardAndroid.js",
			"../../../sdks/react-native/README.md",
		].map((path) => readFile(new URL(path, import.meta.url), "utf8")),
	);
	const source = sources.join("\n");
	assert.doesNotMatch(source, /io\.superboard:SuperBoard:/);
	assert.doesNotMatch(source, /io\.superboard:superboard-android:/);
});
...
test("React Native example substitutes the catalog coordinate with local Android source", async () => {
	const [settings, fixtureSettings, fixtureBuild] = await Promise.all(
		[
			...
			"../../fixtures/clients/react-native-android-contract/settings.gradle",
			"../../fixtures/clients/react-native-android-contract/build.gradle",
		].map((path) => readFile(new URL(path, import.meta.url), "utf8")),
	);
	for (const source of [settings, fixtureSettings]) {
		...
		assert.match(
			source,
			/substitute module\([^)]*\.android\.packageName\) using project\(':SuperBoard'\)/,
		);
		assert.match(source, /includeBuild\(/);
	}
	assert.match(settings, /includeBuild\(superBoardAndroidSource\)/);
	assert.match(fixtureBuild, /dependency\.selected\.id instanceof ProjectComponentIdentifier/);
	assert.match(fixtureBuild, /resolved from a registry instead of sdks\/android/);
});
...
test("React Native iOS consumers use the immutable catalog podspec", async () => {
	...
	assert.doesNotMatch(podspec, /s\.dependency\s+["']SuperBoard["']\s*,\s*["']~>/);
	assert.match(podfile, /native_contract\["ios"\]\["podspecUrl"\]/);
	assert.match(readme, new RegExp(contract.ios.releaseRef.replaceAll(".", "\\.")));
...
test("React Native example locks a supported Ruby and patched build gems", async () => {
	...
	for (const [gem, minimum] of [
		...
		assert.match(
			gemfile,
			new RegExp(`^gem ['"]${gem}['"], ['"]${minimum.replaceAll(".", "\\.")}['"]$`, "m"),
