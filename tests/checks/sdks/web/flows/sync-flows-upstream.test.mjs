...
import {
	checkFlowsUpstream,
	PINNED_SOURCES,
} from "../../../../../sdks/web/flows/scripts/sync-flows-upstream.mjs";

const root = resolve(dirname(fileURLToPath(import.meta.url)), "../../../../..");
const upstream = resolve(root, "sdks/web/flows/upstream");
...
function listFiles(directory) {
	...
	const visit = (current) => {
		for (const name of readdirSync(current)) {
			if (["node_modules", "dist", "coverage", ".cache"].includes(name)) continue;
...
test("imported lint configurations resolve their inheritance chains", () => {
	const manifest = JSON.parse(read("manifest.json"));
	const visit = (path, ancestors = new Set()) => {
		assert.equal(ancestors.has(path), false, `Circular lint configuration: ${path}`);
		const config = JSON.parse(readFileSync(path, "utf8"));
		for (const parent of config.extends ?? []) {
			visit(resolve(dirname(path), parent), new Set([...ancestors, path]));
		}
	};
	for (const { output_path: path } of manifest.files) {
		if (path.endsWith(".oxlintrc.json")) visit(resolve(upstream, path));
	}
});

test("E2E imports declare their workspace dependencies without relying on hoisting", () => {
	const manifest = JSON.parse(read("manifest.json"));
	const pkg = JSON.parse(read("tests/e2e/package.json"));
	const dependencies = { ...pkg.dependencies, ...pkg.devDependencies };
	const missing = new Set();
	for (const { output_path: path } of manifest.files) {
		if (!path.startsWith("tests/e2e/") || !/\.[jt]sx?$/u.test(path)) continue;
		for (const match of read(path).matchAll(/from ["'](@superboard\/[^/"']+)/gu)) {
			if (!dependencies[match[1]]) missing.add(match[1]);
		}
	}
	assert.deepEqual([...missing], []);
});

test("every executable SDK package is installed by the root pnpm workspace", () => {
	const manifest = JSON.parse(read("manifest.json"));
	const workspaces = new Set(
		JSON.parse(readFileSync(resolve(root, "scripts/config/emdash-root.overlay.json"), "utf8"))
			.package.workspaces,
	);
	const missing = manifest.files
		.map(({ output_path: path }) => path)
		.filter((path) => /^(?:packages\/[^/]+|tests\/e2e)\/package.json$/u.test(path))
		.map((path) => `sdks/web/flows/upstream/${dirname(path)}`)
		.filter((path) => !workspaces.has(path));
	assert.deepEqual(missing, []);
});
...
test("runtime defaults to the same-origin SuperBoard API and preserves custom apiUrl support", () => {
	...
	assert.match(
		...
		/blockStateId: block\.blockStateId/u,
	);
	...
	assert.match(read("packages/js/src/lib/blocks.ts"), /firstWebSocketOpen/u);
	assert.match(
	...
	assert.match(read("tests/e2e/tests/init.spec.ts"), /@superboard\\\/flows-/u);
...
test("imported runtime has no upstream service URL or injected upstream branding", () => {
	...
	for (const runtimeRoot of runtimeRoots) {
		for (const path of listFiles(runtimeRoot)) {
			...
			assert.doesNotMatch(readFileSync(path, "utf8"), removedTenantAndBillingModel, path);
		}
	}
	assert.match(read("packages/react-components/src/internal-components/branding.tsx"), /=> null/u);
	assert.match(read("packages/js-components/src/internal-components/branding.ts"), /html`\s*`/u);
});
...
test("product references cannot reintroduce a Flows organization or MTU billing domain", () => {
	...
	for (const referenceRoot of referenceRoots) {
		for (const path of listFiles(referenceRoot)) {
			const extension = extname(path).toLowerCase();
			if (
				![".css", ".html", ".js", ".json", ".md", ".mdx", ".mjs", ".svg", ".ts", ".tsx"].includes(
					extension,
				)
			)
				continue;
