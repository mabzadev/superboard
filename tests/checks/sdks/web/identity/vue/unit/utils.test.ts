import { isValidTokens, handleError, ErrorType } from "@melody-auth/shared";
...
import {
	...
	acquireToken,
} from "../../../../../../../sdks/web/identity/vue/src/utils";
...
vi.mock("@melody-auth/shared", () => {
	return {
		handleError: vi.fn().mockImplementation((e, errorType) => `handled ${errorType}`),
		isValidTokens: vi.fn(),
		ErrorType: {
...
describe("handleTokenExchangeByAuthCode", () => {
	let state: AuthState;
	beforeEach(() => {
		state = {
	...
	it("should update state on successful exchange with accessTokenStorage and refreshTokenStorage", async () => {
		const mockResponse = {
		...
		handleTokenExchangeByAuthCode("code123", "reqState", state, "en");
	...
	it("should set isAuthenticating to false if response has no accessTokenStorage", async () => {
		const mockResponse = {
		...
		handleTokenExchangeByAuthCode("code123", "reqState", state);
	...
	it("should handle error properly if exchangeTokenByAuthCode rejects", async () => {
		...
		expect(handleErrorSpy).toHaveBeenCalledWith(error, ErrorType.ObtainAccessToken);
...
describe("acquireToken", () => {
	...
	it("should return access token if access token is valid", async () => {
	...
	it("should exchange token when refresh token is valid", async () => {
	...
	it("should set error for invalid tokens", async () => {
		const mockedIsValidTokens = isValidTokens as unknown as vi.Mock;
		mockedIsValidTokens.mockReturnValue({
			hasValidAccessToken: false,
			hasValidRefreshToken: false,
	...
	it("should handle error when exchangeTokenByRefreshToken rejects", async () => {
		...
		expect(token).toBeUndefined();
		expect(handleError).toHaveBeenCalledWith(error, ErrorType.ExchangeAccessToken);
