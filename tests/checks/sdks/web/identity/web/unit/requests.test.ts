...
} from "../../../../../../../sdks/web/identity/web/src/requests";
...
describe("requests", () => {
	...
	describe("getAuthorize", () => {
		...
		it("should redirect to authorization URL with required parameters", async () => {
			await getAuthorize(mockProviderConfig, {
				state: "test-state",
		...
		it('should open popup and handle message event when authorizeMethod is "popup"', async () => {
			...
			if (messageHandler) {
				messageHandler(new MessageEvent("message", { data: eventData }));
			}
	...
	describe("API requests", () => {
		...
		describe("getUserInfo", () => {
			it("should fetch user info with access token", async () => {
				...
				const result = await getUserInfo(mockProviderConfig, { accessToken: "test-token" });

				expect(fetch).toHaveBeenCalledWith("https://test.server/oauth2/v1/userinfo", {
					method: "GET",
			...
			it("should throw error on failed request", async () => {
				...
				await expect(
					getUserInfo(mockProviderConfig, { accessToken: "test-token" }),
				).rejects.toThrow("Error message");
			});
		});

		describe("postLogout", () => {
			it("should post logout request with tokens", async () => {
				...
				const result = await postLogout(mockProviderConfig, {
					accessToken: "test-access-token",
					refreshToken: "test-refresh-token",
					postLogoutRedirectUri: "https://post.logout",
				});

				expect(fetch).toHaveBeenCalledWith("https://test.server/identity/v1/logout", {
					method: "POST",
					headers: {
			...
			it("should return post logout redirect URI if no redirect in response", async () => {
				vi.mocked(fetch).mockResolvedValueOnce({
					ok: true,
		...
		describe("postTokenByAuthCode", () => {
			it("should exchange auth code for tokens", async () => {
				...
				const result = await postTokenByAuthCode(mockProviderConfig, {
					code: "test-code",
					codeVerifier: "test-verifier",
				});

				expect(fetch).toHaveBeenCalledWith("https://test.server/oauth2/v1/token", {
					method: "POST",
			...
			it("should throw error on failed request", async () => {
				...
				await expect(
					postTokenByAuthCode(mockProviderConfig, {
						code: "test-code",
						codeVerifier: "test-verifier",
					}),
		...
		describe("postTokenByRefreshToken", () => {
			it("should refresh tokens", async () => {
				...
				const result = await postTokenByRefreshToken(mockProviderConfig, {
			...
			it("should throw error on failed request", async () => {
				...
				await expect(
					postTokenByRefreshToken(mockProviderConfig, { refreshToken: "old-refresh-token" }),
