...
import { logout } from "../../../../../../../../sdks/web/identity/web/src/flows/logout";
import { postLogout } from "../../../../../../../../sdks/web/identity/web/src/requests";
...
describe("logout", () => {
	...
	it("should perform remote logout when tokens are provided", async () => {
		...
		expect(postLogout).toHaveBeenCalledWith(mockProviderConfig, {
	...
	it("should skip remote logout when localOnly is true", async () => {
		await logout(
			mockProviderConfig,
			"test-access-token",
			"test-refresh-token",
	...
	it("should handle missing refresh token", async () => {
		await logout(mockProviderConfig, "test-access-token", null, "https://default.redirect", false);
	...
	it("should handle remote logout failure", async () => {
		...
		expect(mockConsoleError).toHaveBeenCalledWith(
			"Failed to logout remotely: Error: Logout failed",
		);
	...
	it("clears credentials when a storage adapter has no removeItem method", async () => {
		const values = new Map<string, string>([
			[StorageKey.RefreshToken, "refresh"],
			[StorageKey.IdToken, "identity"],
		]);
		...
		try {
			await logout(mockProviderConfig, "access", "refresh", "https://default.redirect", true);
			expect(values.get(StorageKey.RefreshToken)).toBe("");
			expect(values.get(StorageKey.IdToken)).toBe("");
			expect(mockWindow.location.href).toBe("https://default.redirect");
		} finally {
			vi.unstubAllGlobals();
		}
