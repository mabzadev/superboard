...
import { triggerLogin } from "../../../../../../../../sdks/web/identity/web/src/flows/triggerLogin";
import {
	genCodeVerifierAndChallenge,
	genAuthorizeState,
} from "../../../../../../../../sdks/web/identity/web/src/generators";
import { getAuthorize } from "../../../../../../../../sdks/web/identity/web/src/requests";
...
vi.mock("../../../../../../../../sdks/web/identity/web/src/generators", () => ({
...
describe("triggerLogin", () => {
	...
	it("should handle undefined additionalProps", async () => {
		await triggerLogin("redirect", mockProviderConfig);

		// Verify authorize call
		expect(getAuthorize).toHaveBeenCalledWith(mockProviderConfig, {
			state: "test-state",
			codeChallenge: "test-challenge",
			locale: undefined,
			policy: undefined,
	...
	it("should initiate login redirect with generated values", async () => {
		await triggerLogin("redirect", mockProviderConfig, {});
	...
	it("should use provided state instead of generating one", async () => {
		const customState = "custom-state";

		await triggerLogin("redirect", mockProviderConfig, { state: customState });
	...
	it("should pass through additional properties", async () => {
		...
		await triggerLogin("redirect", mockProviderConfig, additionalProps);
	...
	it("should throw error when authorize fails", async () => {
		...
		await expect(triggerLogin("redirect", mockProviderConfig, {})).rejects.toThrow(
			"Failed to initial authorize flow: Error: Authorize failed",
		);
	});
});
