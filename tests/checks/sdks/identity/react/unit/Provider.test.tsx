...
import { AuthProvider, reducer } from "../../../../../../sdks/identity/react/src/Provider";

// --- Mock the Setup component so that it doesn't interfere ---
vi.mock("../../../../../../sdks/identity/react/Setup", () => ({
...
const Consumer: React.FC = () => {
	...
	return (
		<div>
			<div data-testid="checkedStorage">{state.checkedStorage ? "true" : "false"}</div>
			<div data-testid="refreshTokenStorage">
				{state.refreshTokenStorage ? JSON.stringify(state.refreshTokenStorage) : "null"}
			</div>
			<div data-testid="account">{state.account ? JSON.stringify(state.account) : "null"}</div>
			<div data-testid="idToken">{state.idToken ? state.idToken : "null"}</div>
		</div>
	);
};
...
describe("AuthProvider", () => {
	...
	it("dispatches setAuth when a valid refresh token is found in localStorage", async () => {
		...
		window.localStorage.setItem(StorageKey.IdToken, JSON.stringify(idTokenBody));
		...
		await waitFor(() => {
			...
			expect(
				JSON.parse(refreshTokenStorages[refreshTokenStorages.length - 1].textContent ?? "{}"),
			).toStrictEqual(validToken);
		});
	});
	...
	it("dispatch when there is only refreshToken", async () => {
		const now = Math.floor(Date.now() / 1000);
		const validToken = {
			refreshToken: "valid-token",
			expiresOn: now + 60,
	...
	it("dispatch when there is only idToken", async () => {
		const now = Math.floor(Date.now() / 1000);
	...
	it("do not dispatch idToken if it is expired", async () => {
		const now = Math.floor(Date.now() / 1000);
		const validToken = {
			refreshToken: "valid-token",
			expiresOn: now + 60,
	...
	it("dispatches setAuth with null idTokenBody when no stored account is found", async () => {
		const now = Math.floor(Date.now() / 1000);
		const validToken = {
			refreshToken: "valid-token",
			expiresOn: now + 60,
	...
	it("dispatches setAuth when a valid refresh token is provided via URL parameters", async () => {
