...
import { melodyAuthInjectionKey } from "../../../../../../sdks/identity/vue/src/context";
import { useAuth } from "../../../../../../sdks/identity/vue/src/useAuth";
import * as utils from "../../../../../../sdks/identity/vue/src/utils";
...
vi.mock("../../../../../../sdks/identity/vue/src/utils", () => ({
...
describe("useAuth", () => {
	...
	it("should throw error if melody-auth plugin is not installed", () => {
	...
	it("should provide auth properties and methods", async () => {
		...
		wrapper = mount(TestHarness, { slots: { default: AuthConsumer } });
	...
	it("should call triggerLogin with redirect method", async () => {
	...
	it("should call handleTokenExchangeByAuthCode when authorizePopupHandler is called", async () => {
		const TestHarness = createTestHarness({
			...mockState,
			isAuthenticated: false,
		});
	...
	it("should handle login errors and set loginError", async () => {
		...
		expect(shared.handleError).toHaveBeenCalledWith(testError, shared.ErrorType.LoginFailed);
		expect(stateWithoutAuth.loginError).toBe("Error message from handleError");
	});

	it("should call triggerLogin with popup method", async () => {
		const TestHarness = createTestHarness({
			...mockState,
			isAuthenticated: false,
		});
	...
	it("should throw error when trying to login while already authenticated", async () => {
		const TestHarness = createTestHarness(mockState);
		...
		await expect(auth.loginRedirect()).rejects.toThrow(
			"Already authenticated, please logout first",
		);
	});

	it("should throw error when trying to login while authenticating", async () => {
		const TestHarness = createTestHarness({
			...mockState,
	...
	it("should call acquireToken", async () => {
	...
	it("should acquire user info", async () => {
		utils.acquireToken.mockResolvedValue("new-access-token");
		...
		expect(webModule.getUserInfo).toHaveBeenCalledWith(stateWithoutUserInfo.config, {
	...
	it("should handle errors when acquiring user info", async () => {
		...
		expect(shared.handleError).toHaveBeenCalledWith(testError, shared.ErrorType.FetchUserInfo);
	...
	it("should handle errors during logout", async () => {
		...
		expect(shared.handleError).toHaveBeenCalledWith(testError, shared.ErrorType.LogoutFailed);
		expect(stateWithoutLogoutError.logoutError).toBe("Error during logout");
	});

	it("should handle local-only logout", async () => {
