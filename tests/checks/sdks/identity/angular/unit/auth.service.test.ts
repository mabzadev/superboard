...
import { triggerLogin, getUserInfo, logout } from "@melody-auth/web";
import { describe, it, expect, beforeEach, vi } from "vitest";

import { AuthService } from "../../../../../../sdks/identity/angular/src/auth.service";
import { acquireToken } from "../../../../../../sdks/identity/angular/src/utils";
...
vi.mock("@melody-auth/shared", async () => {
...
vi.mock("../../../../../../sdks/identity/angular/src/utils", () => ({
	acquireToken: vi.fn(),
...
describe("AuthService", () => {
	...
	describe("getters", () => {
		it("should return accessToken from state", () => {
		...
		it("should return isAuthenticated from state", () => {
			fakeState.set({ isAuthenticated: true });
			expect(authService.isAuthenticated).toBe(true);
		});

		it("should return isAuthenticating from state", () => {
		...
		it("should return null for accessToken when no accessTokenStorage is present", () => {
			fakeState.set({ accessTokenStorage: undefined });
			expect(authService.accessToken).toBeNull();
		});

		it("should return null for refreshToken when no refreshTokenStorage is present", () => {
			fakeState.set({ refreshTokenStorage: undefined });
			expect(authService.refreshToken).toBeNull();
		});

		it("should return null for accessToken when accessToken is missing in accessTokenStorage", () => {
			fakeState.set({ accessTokenStorage: {} });
			expect(authService.accessToken).toBeNull();
		});

		it("should return null for refreshToken when refreshToken is missing in refreshTokenStorage", () => {
	...
	describe("login method", () => {
		it("should return early if authenticationError exists", async () => {
			fakeState.set({ authenticationError: "Some error" });
			await authService.login("popup");
			expect(triggerLogin).not.toHaveBeenCalled();
		});

		it("should throw error if isAuthenticating is true", async () => {
			fakeState.set({ isAuthenticating: true });
			await expect(authService.login("popup")).rejects.toThrow(
				"Please wait until isAuthenticating=false",
			);
		});

		it('should throw error if already authenticated with no policy or with "sign_in_or_sign_up" policy', async () => {
			fakeState.set({ isAuthenticated: true });
			await expect(authService.login("popup")).rejects.toThrow(
				"Already authenticated, please logout first",
			);
		});

		it("should throw error when already authenticated and policy is sign_in_or_sign_up", async () => {
			fakeState.set({ isAuthenticated: true });
			await expect(authService.login("popup", { policy: "sign_in_or_sign_up" })).rejects.toThrow(
				"Already authenticated, please logout first",
			);
		});

		it("should call triggerLogin with correct parameters on successful login", async () => {
			...
			await authService.login("popup", props);
		...
		it("should handle errors thrown by triggerLogin and update state.loginError", async () => {
		...
		it("should invoke authorizePopupHandler and call handleTokenExchangeByAuthCode with correct parameters", async () => {
			...
			const utils = await import("../../../../../../sdks/identity/angular/src/utils");
			const handleTokenExchangeSpy = vi
				.spyOn(utils, "handleTokenExchangeByAuthCode")
	...
	describe("loginRedirect and loginPopup", () => {
		it('loginRedirect should call login with method "redirect"', async () => {
		...
		it('loginPopup should call login with method "popup"', async () => {
			fakeState.isAuthenticated = false;
			fakeState.isAuthenticating = false;
			fakeState.authenticationError = "";
	...
	describe("acquireToken", () => {
		it("should call acquireToken and return its value", async () => {
	...
	describe("acquireUserInfo", () => {
		it("should return existing userInfo if available", async () => {
		...
		it("should fetch userInfo if not available and update state accordingly", async () => {
			(acquireToken as any).mockResolvedValue("acquired-token");
		...
		it("should handle errors when fetching userInfo and update state", async () => {
	...
	describe("logoutRedirect", () => {
		it("should call logout with appropriate parameters when tokens are available", async () => {
			(acquireToken as any).mockResolvedValue("acquired-token");
			fakeState.set({ refreshTokenStorage: { refreshToken: "fake-refresh-token" } });
			await authService.logoutRedirect({
				postLogoutRedirectUri: "http://redirect.com",
				localOnly: false,
			});
			expect(acquireToken).toHaveBeenCalledTimes(1);
		...
		it("should call logout with isLocalOnly true when tokens are missing", async () => {
		...
		it("should call logout with isLocalOnly true even if tokens exist", async () => {
		...
		it("should handle errors during logout and update state.logoutError", async () => {
			const error = new Error("");
			(acquireToken as any).mockResolvedValue("acquired-token");
			(logout as any).mockRejectedValue(error);
			const handleErrorSpy = vi
				.mocked(handleError)
				.mockReturnValue("Unable to initial logout flow");
			await authService.logoutRedirect({
				postLogoutRedirectUri: "",
				localOnly: false,
			});
