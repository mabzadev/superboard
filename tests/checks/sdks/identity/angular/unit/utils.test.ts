import { ErrorType, isValidTokens } from "@melody-auth/shared";
...
import {
	...
	acquireToken,
} from "../../../../../../sdks/identity/angular/src/utils";
...
describe("handleTokenExchangeByAuthCode", () => {
	...
	it("should update state correctly on a successful token exchange", async () => {
		...
		handleTokenExchangeByAuthCode("sample-code", "sample-state", state, "en");
	...
	it("should update state with error on token exchange failure", async () => {
		const initialState = {
		...
		handleTokenExchangeByAuthCode("code", "reqState", state);
...
describe("acquireToken", () => {
	...
	it("should immediately return the accessToken if valid", async () => {
		const initialState = {
			config: { testConfig: true },
	...
	it("should exchange refresh token if access token is invalid and return new token", async () => {
		const initialState = {
			config: { testConfig: true },
			accessTokenStorage: null,
			refreshTokenStorage: { refreshToken: "dummy-refresh" },
	...
	it("should update state with error if refresh token exchange fails", async () => {
		const initialState = {
			config: { testConfig: true },
			accessTokenStorage: null,
			refreshTokenStorage: { refreshToken: "dummy-refresh" },
	...
	it("should update state with error if no valid tokens are available", async () => {
		const initialState = {
			config: { testConfig: true },
			accessTokenStorage: null,
			refreshTokenStorage: null,
...
describe("handleTokenExchangeByAuthCode", () => {
	...
	it("should set isAuthenticating to false if no accessTokenStorage is returned", async () => {
		const onLoginSuccess = vi.fn();
		const initialState = {
		...
		// Call the function under test.
		handleTokenExchangeByAuthCode("dummy-code", "dummy-state", state);
