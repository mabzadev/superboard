...
class LinkGenerationE2ETest {
    ...
    fun `Generate link with all parameters returns valid link`() {
        ...
        E2ETestUtils.enqueueGenerateLinkResponse(mockWebServer, link = "https://test.superboard.io/full-params")
        ...
        assertEquals("Should return mocked link URL", "https://test.superboard.io/full-params", result)
    ...
    fun `Generate link with minimal parameters returns valid link`() {
        ...
        E2ETestUtils.enqueueAuthenticationResponse(mockWebServer)
        E2ETestUtils.enqueueDeviceResponse(mockWebServer)
        E2ETestUtils.enqueueGenerateLinkResponse(mockWebServer, link = "https://test.superboard.io/minimal")
        ...
        assertEquals("Should return mocked link URL", "https://test.superboard.io/minimal", result)
    }
    ...
    fun `Generate link before authentication throws exception`() {
        runBlocking {
            ...
            // Act & Assert
            try {
                SuperBoard.generateLink(title = "Test")
    ...
    fun `Generate link with custom data payload returns valid link`() {
        ...
        E2ETestUtils.enqueueAuthenticationResponse(mockWebServer)
        E2ETestUtils.enqueueDeviceResponse(mockWebServer)
        E2ETestUtils.enqueueGenerateLinkResponse(mockWebServer, link = "https://test.superboard.io/with-data")
        ...
        assertTrue("Result should contain link URL", result?.contains("superboard.io") == true)
    ...
    fun `Generate link handles network timeout gracefully`() {
        ...
        val result = E2ETestUtils.runWithLooperPumping(5_000) {
            SuperBoard.generateLink(title = "Timeout Test")
        }
