...
void main() {
  ...
  test(
    ...
    () async {
      ...
      expect(certificationRequest?.headers['x-superboard-build-number'], '104');
      expect(
        certificationRequest?.headers['x-superboard-app-version'],
        '1.4.0',
      );
      final body =
...
class MemoryPurchaseStorage implements SuperBoardPurchaseStorage {
  ...
  Future<void> write({required String key, required String? value}) async {
    ...
    events.add(
      key == 'superboard.purchases.outbox.v1'
          ? 'outbox.persisted'
  ...
  List<dynamic> get outboxEntries =>
      jsonDecode(values['superboard.purchases.outbox.v1'] ?? '[]')
          as List<dynamic>;
  ...
  void makeOutboxRetriesDue() {
    ...
    values['superboard.purchases.outbox.v1'] = jsonEncode(entries);
  }
}
