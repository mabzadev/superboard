...
} from "../../../../sdks/javascript/src/support/index.js";
...
test("token refresh and transient retries reuse one mutation idempotency key", async () => {
	...
	assert.deepEqual(
		...
		["Bearer old-token", "Bearer fresh-token", "Bearer fresh-token"],
	);
	assert.deepEqual(
	...
	const client = new SuperBoardSupportClient({
		...
		fetch: async (url) => {
			...
			if (
				...
				path.includes("/proactive-support") ||
...
test("realtime obtains a new one-use ticket when reconnecting", async () => {
	...
	assert.equal(
		events.some((event) => event.type === "error" && event.error.retryable),
		true,
	);
...
test("the isolated widget fails safely outside a browser document", () => {
	...
	try {
		...
		assert.throws(
			() => widget.mount({}),
			(error) =>
				error instanceof SuperBoardSupportException && error.code === "widget_dom_unavailable",
		);
	} finally {
