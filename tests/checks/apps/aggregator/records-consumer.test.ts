...
} from "../../../../apps/aggregator/src/pds-verify.js";
import {
	applyDelete,
	type ConsumerDeps,
	IngestError,
...
describe("drainDeadLetterBatch", () => {
	it("acks each message and writes a forensics row", async () => {
		const { drainDeadLetterBatch: drain } =
			await import("../../../../apps/aggregator/src/records-consumer.js");
...
describe("processBatch isolates per-message failures", () => {
	it("retries the failing message and continues processing the rest of the batch", async () => {
		const { processBatch } = await import("../../../../apps/aggregator/src/records-consumer.js");
