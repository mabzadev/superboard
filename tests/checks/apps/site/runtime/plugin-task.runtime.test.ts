...
test.each([503, 200])(
	"a non-JSON authority response (%s) fails closed with a usable HTTP error",
	async (status) => {
		let executed = false;
		const worker = withPluginTaskLifecycle(
			pluginId,
		...
		const response = await worker.fetch!(
			new Request("https://worker.example/operation"),
			{
				...moduleEnvironment(),
				API_SERVICE: {
					fetch: async () => new Response("Service temporarily unavailable", { status }),
				},
			},
			createExecutionContext(),
		);
		expect(response.status).toBe(status === 200 ? 502 : 503);
		expect(await response.json()).toMatchObject({
			error: { code: "PLUGIN_TASK_AUTHORITY_UNAVAILABLE" },
		});
		expect(executed).toBe(false);
	},
);

test("a disconnected task authority returns a controlled availability error", async () => {
	await expect(
		sendPluginTaskCommand(
			{
				...moduleEnvironment(),
	...
	).rejects.toMatchObject({ code: "PLUGIN_TASK_AUTHORITY_UNAVAILABLE", status: 503 });
});

test("an empty successful authority response cannot authorize plugin work", async () => {
	let executed = false;
	const worker = withPluginTaskLifecycle(
		pluginId,
		{
