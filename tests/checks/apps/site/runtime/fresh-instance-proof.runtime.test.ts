...
import { mountNativeFrontRenderer } from "../../../../../apps/site/src/lib/native-front-plugins.js";
...
test("the isolated runtime receives both compiled target artifacts", () => {
	...
	expect(
		plan.adapters.every(({ artifact_checksum }) => CHECKSUM_PATTERN.test(artifact_checksum)),
	).toBe(true);
...
test.each(compiledPlan.adapters)(
	...
	async (targetArtifact) => {
		...
		const activeRouteEvidence = renderReleaseRoutes(release, active!, instanceId, frontOperator);
