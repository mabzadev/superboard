...
import { dispatchPluginApiAdapter } from "../../../../../apps/site/src/lib/plugin-api-adapter.js";
import {
...
test("Email alone owns SMTP settings and captures an idempotent transactional message", async () => {
	...
	const entries = (
		await log.json<{ data: { items: Array<{ id: string; subject: string; status: string }> } }>()
	).data.items;
	expect(entries).toContainEqual(
		expect.objectContaining({ subject: "Standalone Email", status: "captured" }),
	);
	const detail = await emailRequest("GET", "/internal/v1/admin/messages/" + entries[0]!.id);
	expect(await detail.json()).toMatchObject({
		data: { text_body: "Email works without Marketing." },
	});
	expect(
		(await emailRequest("GET", "/internal/v1/admin/messages/" + entries[0]!.id, undefined, 82))
			.status,
	).toBe(404);
...
test("migrates legacy SMTP profiles through real Workers and keeps later Email edits authoritative", async () => {
	...
	const newSender = await emailRequest(
		"PUT",
		"/internal/v1/admin/settings/smtp",
		{
			...profile,
			id: crypto.randomUUID(),
			name: "New Email sender",
			from_email: "new@example.test",
			password: "new-sender-secret",
		},
		81,
		"new-email-sender",
	);
	...
	try {
		...
		expect(candidates[0]).toMatchObject({
			id: newSenderBody.data.id,
			managed: true,
			encrypted_config: "",
		});
		expect(JSON.stringify(candidates)).not.toContain("new-sender-secret");
		expect(JSON.parse(candidates[0]!.public_config_json)).toMatchObject({
			from_email: "new@example.test",
		});
	} finally {
		await emailRequest(
			"DELETE",
			"/internal/v1/admin/settings/smtp/" + newSenderBody.data.id,
			undefined,
			81,
			"remove-new-test-sender",
		);
	}
});
