...
} from "../../../../apps/reference/scripts/reference-sdk-lock.mjs";
...
test("the application lockfile resolves only the coordinated native candidates", () => {
	...
	assert.equal(paths.get("superboard_flutter")?.version, "4.0.0");
	assert.equal(paths.get("superboard_flutterflow")?.version, "4.0.0");
...
test("candidate lock validation rejects path and version drift", () => {
	...
	assert.throws(() => candidateSdkLocks(project, movedPathLock), /lock path must be/u);
	const movedVersionLock = lockSource.replace(
		/(^  superboard_flutterflow:[\s\S]*?^    version: )"4\.0\.0"/mu,
		'$1"4.0.1"',
	);
	assert.throws(
		() => candidateSdkLocks(project, movedVersionLock),
		/lock version must be 4\.0\.0/u,
	);
});
...
test("remote tag resolution supports annotated and lightweight tags", () => {
	...
	assert.equal(resolveRemoteTag(`${"c".repeat(40)}\trefs/tags/${tag}\n`, tag), "c".repeat(40));
});
...
test("remote verification fails closed when an immutable tag moves", async () => {
	...
	await assert.rejects(
		verifyRemoteImmutableSdkTags({
			...
			listRemote: async (_repository, releaseRef) => `${"f".repeat(40)}\trefs/tags/${releaseRef}\n`,
		}),
