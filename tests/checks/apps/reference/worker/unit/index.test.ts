...
} from "../../../../../../apps/reference/worker/src/index";
...
describe("custom Worker protocol", () => {
	...
	it("rejects unknown project reference formats", () => {
		expect(() =>
			parseJob({
				idempotencyKey: "job:123",
				projectRef: "reference-production-production",
				capability: "reference.echo",
				payload: {},
	...
	it("validates the target-defined job retention window", () => {
		...
		expect(() => referenceRetentionDays("0")).toThrow(/reference_retention_invalid/);
		expect(() => referenceRetentionDays("3651")).toThrow(/reference_retention_invalid/);
		expect(() => referenceRetentionDays("thirty")).toThrow(/reference_retention_invalid/);
	});
});
