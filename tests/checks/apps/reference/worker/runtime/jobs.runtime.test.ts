...
import { runReferenceMaintenance } from "../../../../../../apps/reference/worker/src/index";

describe("reference custom Worker with D1", () => {
	beforeEach(async () => {
		await runtimeEnv.REFERENCE_DB.prepare("DELETE FROM reference_custom_jobs").run();
	});
	...
	it("persists a revision-bound acceptance receipt and rejects incomplete evidence", async () => {
		...
		const invalidExemption = acceptanceJob("reference:acceptance:invalid-exemption");
		...
		const secretEvidence = acceptanceJob("reference:acceptance:secret-evidence");
		secretEvidence.payload.journeys[0].evidence = "Bearer header-content-must-not-be-persisted";
	...
	it("scopes detail and list reads to the authenticated project and subject", async () => {
		...
		const hiddenCancellation = await authorized(`/internal/v1/jobs/${receipt.id}/cancel`, {
			method: "POST",
			headers: { "x-custom-worker-subject": "user-2" },
		});
		expect(hiddenCancellation.status).toBe(404);
		const terminalCancellation = await authorized(`/internal/v1/jobs/${receipt.id}/cancel`, {
			method: "POST",
			headers: { "x-custom-worker-subject": "user-1" },
		});
	...
	it("rejects incomplete or mismatched public SDK scope", async () => {
		const incomplete = await SELF.fetch("https://custom.test/internal/v1/jobs", {
			headers: {
	...
	it("requires the private token and checks the D1 binding in health", async () => {
		expect((await SELF.fetch("https://custom.test/internal/v1/jobs")).status).toBe(401);
...
function authorized(path: string, init: RequestInit = {}) {
	...
	if (headers.has("x-custom-worker-subject") && !headers.has("x-custom-worker-project")) {
