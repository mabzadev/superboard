...
import { selectOfficialPlatformRef } from "../../../../apps/reference/scripts/reference-platform-ref.mjs";
...
test("repository dispatch requires and preserves an exact Platform SHA", () => {
	assert.deepEqual(select({ eventName: "repository_dispatch", dispatchPlatformSha }), {
		ref: dispatchPlatformSha,
		source: "dispatch-exact-sha",
	});
	assert.throws(() => select({ eventName: "repository_dispatch" }), /40-character commit SHA/u);
});
...
test("forged bridge states and unsafe refs fail closed", () => {
	for (const historyBridgeValidated of ["TRUE", " true", "1", "yes"]) {
		assert.throws(() => select({ historyBridgeValidated }), /must be (?:exactly|an exact string)/u);
	}
	for (const baseRef of ["", "../main", " main", "main.lock"]) {
		assert.throws(() => select({ baseRef }), /safe Git branch name|exact, non-empty/u);
	}
});

test("unknown events fail closed", () => {
	for (const eventName of ["", "schedule", "pull_request_target"]) {
		assert.throws(() => select({ eventName }), /exact string|Unsupported GitHub event/u);
	}
});

test("production and development branch metadata must be distinct and safe", () => {
	assert.throws(() => select({ productionBranch: deploymentBranch }), /must differ/u);
	assert.throws(() => select({ productionBranch: "../main" }), /safe Git branch name/u);
});
