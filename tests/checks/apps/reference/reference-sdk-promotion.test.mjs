...
} from "../../../../apps/reference/scripts/reference-sdk-promotion.mjs";
	...
	return {
		...
		libraries: coverageManifest.libraries.map((library) => {
			...
			return {
				...
				latestReleaseVersion: active ? library.sourceVersion : library.baselineVersion,
...
test("v4 promotion is atomic and migrates only the two active packages", () => {
	...
	assert.deepEqual(result.promotions, [
		{ library: "flutter", version: "4.0.0", releaseRef: "sdk-flutter-v4.0.0" },
		{
			library: "flutterflow",
			version: "4.0.0",
			releaseRef: "sdk-flutterflow-v4.0.0",
		},
	]);
	assert.match(
		result.pubspec,
		/dependencies:[\s\S]*superboard_flutterflow:[\s\S]*ref: sdk-flutterflow-v4\.0\.0/u,
	);
	assert.match(
		result.pubspec,
		/dependency_overrides:[\s\S]*superboard_flutter:[\s\S]*ref: sdk-flutter-v4\.0\.0/u,
	);
	assert.doesNotMatch(result.pubspec, /superboard_flutterflow_messaging:/u);
	assert.doesNotMatch(result.dependencySnippet, /superboard_flutterflow_messaging:/u);
	assert.equal(result.project.libraries.superboard_flutterflow_messaging, undefined);
	assert.equal(result.project.libraries.superboard_flutterflow.releaseVersion, "4.0.0");
	assert.deepEqual(
		result.coverageManifest.libraries.map(({ id, lifecycle }) => [id, lifecycle]),
		coverageManifest.libraries.map(({ id, lifecycle }) => [id, lifecycle]),
	);
	assert.equal(
		result.coverageManifest.libraries.find(({ id }) => id === "flutterflow-support").coverageMode,
		"historical-release",
	);
});
...
test("promotion refuses a pending, partial, missing or drifted lifecycle set", () => {
	...
	Object.assign(flutter, {
		packageName: "superboard_flutter",
...
test("dependency migration is exact, supports renames and removes archived support", () => {
	const source =
		"dependencies:\n  sample:\n    git:\n      ref: dev\n\n  archived:\n    git:\n      ref: old\n";
	assert.equal(
		replaceGitDependency(source, "sample", "superboard_sample", "sdk-sample-v4.0.0"),
		"dependencies:\n  superboard_sample:\n    git:\n      ref: sdk-sample-v4.0.0\n\n  archived:\n    git:\n      ref: old\n",
	);
	assert.equal(
