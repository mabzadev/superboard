...
} from "../../../../apps/reference/scripts/reference-history-bridge.mjs";
...
test("bridge and audit names are derived only from reviewed remote evidence", () => {
	...
	assert.equal(auditBranch, `audit/pre-superboard-main-${baseSha.slice(0, 12)}`);
});
...
test("non-PR and cross-repository bridge attempts fail closed", () => {
	...
	assert.throws(
		() =>
			validateHistoryBridgeEvidence({
				...validEvidence,
				headRepository: "attacker/superboard-reference",
			}),
