...
} from "../../../../apps/reference/scripts/reference-sdk-coverage.mjs";

const root = new URL("../../../../apps/reference/", import.meta.url);
const manifest = JSON.parse(await readFile(new URL("config/sdk-coverage.json", root), "utf8"));
...
function legacyCatalogueFixture() {
	return {
		schemaVersion: 3,
		repository: `https://github.com/mabzadev/${"superboard"}-platform`,
		libraries: manifest.libraries.map((library) => ({
			id: library.id,
			packageName: library.packageName,
			sourcePath: library.sourcePath,
...
test("coverage v2 models the seven reference SDKs from catalogue v5", () => {
	...
	assert.deepEqual(
		...
		[
			{ id: "flutter", baselineVersion: "2.1.4", candidateVersion: "4.0.0" },
			{ id: "flutterflow", baselineVersion: "2.2.5", candidateVersion: "4.0.0" },
		],
	);
});
...
test("the executable consumes the coordinated native Dart candidates", () => {
	...
	assert.match(
		lockSource,
		/superboard_flutter:[\s\S]*path: "\.\.\/\.\.\/sdks\/flutter"[\s\S]*source: path[\s\S]*version: "4\.0\.0"/u,
	);
	assert.doesNotMatch(lockSource, /superboard_flutterflow_messaging:/u);
});
...
test("the pre-migration Platform v3 catalogue is accepted only as an exact immutable baseline", () => {
	...
	stale.libraries.find(({ id }) => id === "flutter").sourceVersion = "4.0.0";
	assert.throws(
		() => verifyCatalogueCoverage(manifest, stale),
...
test("secretless remote gates verify immutable baselines, including both iOS tags", async () => {
	...
	assert.deepEqual(
		releaseCalls,
		manifest.libraries.map(({ baselineTag }) => baselineTag),
	);
...
test("coverage CI uses public reads, catalogue v5 and canonical variables", () => {
	...
	assert.doesNotMatch(workflow, /SUPERBOARD_REFERENCE_DISPATCH_TOKEN/u);
