...
} from "../../../scripts/github/reconcile.mjs";
	...
	return {
		...
		repositories: {
			platform: {
				...
				description: "SuperBoard platform",
				settings: {
					issues: true,
					projects: false,
					wiki: false,
...
test("reconciliation plans structure but leaves every secret value manual", () => {
			...
			environments: [
				{
					name: "development",
					exists: true,
					variables: [{ name: "SUPERBOARD_TARGET", exists: true, configured: false }],
	...
	assert.deepEqual(
		...
		["put-branch-protection", "set-default-branch", "update-environment-variable"],
	);
	assert.deepEqual(
			...
			environments: [
				{
					name: "development",
					exists: true,
					variables: [{ name: "SUPERBOARD_TARGET", exists: true, configured: true }],
...
test("reconciliation enables least-privilege workflow PR creation explicitly", () => {
	...
	assert.deepEqual(plan.repositories[0].operations, [{ type: "set-workflow-permissions" }]);
	const request = mutationRequest(configuration.repositories.platform, {
		type: "set-workflow-permissions",
	});
	assert.equal(
		request.args.includes("repos/mabzadev/superboard-platform/actions/permissions/workflow"),
		true,
	);
	assert.deepEqual(request.body, {
...
test("reconciliation plans security and future immutable-release hardening without moving tags", () => {
	...
	const state = {
		...
		releaseProtection: {
			...
			releaseInventoryReadable: true,
			mutablePublishedReleases: [{ id: 42, tagName: "sdk-flutter-v2.1.4", assetCount: 0 }],
		},
		branches: [
			{ name: "dev", exists: true, protectionMatches: true },
			{ name: "main", exists: true, protectionMatches: true },
	...
	assert.equal(rulesetRequest.args.includes("repos/mabzadev/superboard-platform/rulesets"), true);
	...
	const plan = buildGitHubReconcilePlan(configuration, [
		{
			...
			releaseProtection: {
				...
				releaseInventoryReadable: true,
				mutablePublishedReleases: [{ id: 42, tagName: "sdk-flutter-v2.1.4", assetCount: 1 }],
			},
			branches: [
				{ name: "dev", exists: true, protectionMatches: true },
				{ name: "main", exists: true, protectionMatches: true },
...
test("confirmed reconciliation uses JSON stdin and never mutates secrets", () => {
	...
	assert.equal(calls[2].body.required_pull_request_reviews.require_code_owner_reviews, true);
});
...
test("GitHub mutation requests use the current versioned REST contract", () => {
	...
	assert.equal(request.args.includes("repos/mabzadev/superboard-platform"), true);
});
...
test("pending external Environment hardening is reported but never planned as a mutation", () => {
	...
	assert.deepEqual(plan.repositories[0].operations, []);
	assert.deepEqual(
		plan.repositories[0].manual.filter(({ type }) => type === "activate-environment-protection"),
		[
			{
...
test("enforced Environment hardening plans reviewers, timer and missing branch-tag policies without deleting drift", () => {
			...
			environments: [
				{
					...
					protection: {
						...
						remote: {
							deploymentPolicies: [{ id: 99, type: "branch", pattern: "legacy" }],
						},
					},
				},
			],
	...
	assert.equal(
		plan.repositories[0].manual.some(
			({ type, pattern }) =>
				type === "review-unexpected-environment-deployment-policy" && pattern === "legacy",
		),
		true,
	);
	assert.equal(
...
test("enforcement is structurally blocked when declared reviewers lack verified read access", () => {
	...
	assert.equal(
		plan.repositories[0].blockers.some(({ type }) => type === "environment-reviewer-access"),
		true,
	);
	assert.equal(
		plan.repositories[0].operations.some(({ type }) => type === "put-environment-protection"),
		false,
	);
});
