...
import {
	...
	inspectGitHistory,
	remoteMatches,
} from "../../../scripts/github/history-plan.mjs";
...
test("history inspection detects unrelated remote main and its audit ref", () => {
	...
	const results = new Map([
		...
		["remote get-url origin", ok("https://github.com/mabzadev/superboard-platform.git")],
		["rev-parse --verify refs/remotes/origin/main", ok(remoteMain)],
		["rev-parse --verify refs/remotes/origin/dev", fail()],
		[
			`rev-parse --verify refs/remotes/origin/audit/pre-superboard-main-${remoteMain.slice(0, 12)}`,
			fail(),
		],
	...
	const plan = buildGitHubHistoryPlan({ repositories: { platform } }, [
		{
			...
			clean: true,
			changedPaths: 0,
			archiveBranch: `audit/pre-superboard-main-${remoteMain.slice(0, 12)}`,
			remoteArchive: null,
...
test("published remote main and dev without a merge base block readiness and emit a safe bridge procedure", () => {
	const head = "a".repeat(40);
	const remoteMain = "b".repeat(40);
	const archiveBranch = `audit/pre-superboard-main-${remoteMain.slice(0, 12)}`;
	const plan = buildGitHubHistoryPlan({ repositories: { platform } }, [
		{
			nameWithOwner: platform.nameWithOwner,
			branch: "dev",
	...
	assert.deepEqual(bridge.requiredVerification.exactParentsInOrder, [head, remoteMain]);
	...
	assert.equal(
		bridge.protectedMainProcedure.some((step) => /squash or rebase/u.test(step)),
		true,
	);
	...
	const plan = buildGitHubHistoryPlan({ repositories: { platform } }, [
		{
			...
			clean: false,
			changedPaths: 32,
			archiveBranch: "audit/pre-superboard-main-bbbbbbbbbbbb",
			remoteArchive: null,
			remoteMainPreserved: false,
		},
	]);
...
test("origin matching accepts canonical HTTPS and SSH forms only", () => {
	assert.equal(
		remoteMatches("https://github.com/mabzadev/superboard-platform.git", platform.nameWithOwner),
		true,
	);
	assert.equal(
		remoteMatches("git@github.com:mabzadev/superboard-platform.git", platform.nameWithOwner),
		true,
	);
	assert.equal(
		remoteMatches("https://github.com/another/superboard-platform.git", platform.nameWithOwner),
		false,
	);
});
...
test("an occupied audit ref or divergent remote dev blocks publication", () => {
	...
	assert.deepEqual(
		plan.repositories[0].blockers.map(({ type }) => type),
		["audit-branch-conflict", "remote-dev-diverged", "remote-main-dev-unrelated"],
	);
	assert.deepEqual(plan.repositories[0].operations, []);
});
