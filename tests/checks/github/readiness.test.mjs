...
import {
	...
	tagRulesetMatches,
} from "../../../scripts/github/readiness.mjs";
...
function repositoryPayload(name, overrides = {}) {
	return {
		full_name: name,
		description: "SuperBoard test repository",
		visibility: "public",
		default_branch: "dev",
		has_issues: true,
		has_projects: false,
...
test("GitHub control-plane manifest is strict and contains names, never secret values", async () => {
	...
	assert.equal(manifest.repositories.platform.nameWithOwner, "mabzadev/superboard");
	...
	assert.deepEqual(manifest.repositories.platform.workflowPermissions, workflowPermissions());
	assert.deepEqual(manifest.repositories.platform.security, securityPolicy());
	assert.equal(manifest.repositories.platform.releaseProtection.immutableReleases, true);
	assert.deepEqual(manifest.repositories.platform.releaseProtection.tagRuleset.rules, [
		"update",
		"deletion",
	]);
	assert.equal(manifest.repositories.platform.visibility, "public");
	assert.deepEqual(Object.keys(manifest.repositories), ["platform"]);
	assert.equal(manifest.repositories.platform.branches.dev.requiredCheck, "CI gate");
	assert.deepEqual(
		manifest.repositories.platform.environments.development.protection.deploymentPolicies,
		[{ type: "branch", pattern: "dev" }],
	);
	assert.equal(
		manifest.repositories.platform.environments.production.protection.enforcement,
		"pending-external",
	);
	assert.equal(
		manifest.repositories.platform.environments.production.protection.minimumEligibleReviewers,
		2,
	);
	assert.equal(
		manifest.repositories.platform.environments.production.protection.reviewers.length,
		1,
	);
	assert.equal(
		manifest.repositories.platform.environments["sdk-release"].protection.deploymentPolicies.filter(
			({ type }) => type === "tag",
		).length,
		11,
	);
	assert.doesNotMatch(serialized, /ghp_|github_pat_|api[_-]?token\s*[:=]\s*[A-Za-z0-9]/i);
});
...
test("offline GitHub readiness plan exposes no secret values and includes protection intent", async () => {
	...
	assert.equal(plan.repositories.platform.releaseProtection.immutableReleases, true);
	...
	assert.deepEqual(sdkRelease.protection.reviewers, [{ type: "User", name: "mabzadev" }]);
...
test("remote GitHub inspection requires branches, protection, variables and secret names", () => {
	...
	const payloads = new Map([
		[`${repositoryApiPrefix} repos/mabzadev/ready`, repositoryPayload("mabzadev/ready")],
		[
		...
		["api repos/mabzadev/ready/automated-security-fixes", { enabled: true, paused: false }],
		["api repos/mabzadev/ready/immutable-releases", { enabled: true, enforced_by_owner: false }],
		["api repos/mabzadev/ready/releases?per_page=100", []],
		["api repos/mabzadev/ready/branches/dev", { name: "dev", commit: { sha: "d".repeat(40) } }],
		["api repos/mabzadev/ready/branches/main", { name: "main", commit: { sha: "a".repeat(40) } }],
		["api repos/mabzadev/ready/compare/main...dev", { merge_base_commit: { sha: "c".repeat(40) } }],
		[
		...
		["api repos/mabzadev/ready/environments", { environments: [{ name: "development" }] }],
		[
		...
		["api repos/mabzadev/ready/actions/secrets", { secrets: [{ name: "READ_TOKEN" }] }],
	]);
	const run = (args) => {
		const payload = payloads.get(args.join(" "));
		return payload ? { ok: true, stdout: JSON.stringify(payload) } : { ok: false, stdout: "" };
	};
...
test("remote GitHub inspection compares Environment reviewers, timers, bypass and branch-tag policies", () => {
	...
	const payloads = new Map([
		[`${repositoryApiPrefix} repos/mabzadev/protected`, repositoryPayload("mabzadev/protected")],
		[
		...
		["api repos/mabzadev/protected/automated-security-fixes", { enabled: true, paused: false }],
		["api repos/mabzadev/protected/immutable-releases", { enabled: true }],
		["api repos/mabzadev/protected/releases?per_page=100", []],
		["api repos/mabzadev/protected/environments", { environments: [{ name: "production" }] }],
		["api repos/mabzadev/protected/environments/production/variables", { variables: [] }],
		["api repos/mabzadev/protected/environments/production/secrets", { secrets: [] }],
		[
	...
	assert.deepEqual(
		state.environments[0].protection.remote.deploymentPolicies.map(({ type, pattern }) => ({
			type,
			pattern,
		})),
		[
	...
	payloads.set("api repos/mabzadev/protected/collaborators/release-two/permission", {
		permission: "maintain",
		user: { id: 23, login: "renamed-account" },
	});
	...
	assert.equal(identityDrift.environments[0].protection.eligibleReviewerCount, 1);
	assert.equal(identityDrift.environments[0].protection.eligibleReviewersReady, false);
});
...
test("pending external Environment protection is explicit and cannot report ready", () => {
	...
	const payloads = new Map([
		[`${repositoryApiPrefix} repos/mabzadev/pending`, repositoryPayload("mabzadev/pending")],
		[
		...
		["api repos/mabzadev/pending/automated-security-fixes", { enabled: true, paused: false }],
		["api repos/mabzadev/pending/immutable-releases", { enabled: true }],
		["api repos/mabzadev/pending/releases?per_page=100", []],
		["api repos/mabzadev/pending/environments", { environments: [{ name: "production" }] }],
		["api repos/mabzadev/pending/environments/production/variables", { variables: [] }],
		["api repos/mabzadev/pending/environments/production/secrets", { secrets: [] }],
		[
	...
	assert.equal(state.environments[0].protection.enforcement, "pending-external");
...
test("remote GitHub inspection rejects a superficially matching but weak branch rule", () => {
	...
	const payloads = new Map([
		[`${repositoryApiPrefix} repos/mabzadev/weak`, repositoryPayload("mabzadev/weak")],
		[
		...
		["api repos/mabzadev/weak/automated-security-fixes", { enabled: true, paused: false }],
...
test("remote GitHub inspection rejects repository settings drift", () => {
	...
	const payloads = new Map([
		...
		["api repos/mabzadev/drifted/automated-security-fixes", { enabled: true, paused: false }],
