...
} from "../../../scripts/database/d1-backup-crypto.mjs";
import {
	buildMigrationBatchReceipt,
	writeMigrationBatchReceipt,
} from "../../../scripts/database/migration-batch.mjs";

test("production backup artifacts are authenticated, encrypted and restorable", async () => {
	const directory = await mkdtemp(join(tmpdir(), "superboard-encrypted-backup-"));
	...
	try {
		...
		assert.doesNotMatch((await readFile(encrypted)).toString("utf8"), /protected_customer_data/u);
		...
		await assert.rejects(
			decryptBackupFile(encrypted, join(directory, "restored", "wrong.sql"), Buffer.alloc(32, 8)),
...
test("backup encryption rejects malformed keys", () => {
	assert.throws(() => backupEncryptionKey("not-a-key"), /base64-encoded 32-byte/u);
});

test("backup encryption refuses an empty successful artifact set", async () => {
	const directory = await mkdtemp(join(tmpdir(), "superboard-empty-backup-"));
	const key = backupEncryptionKey(Buffer.alloc(32, 7).toString("base64"));
	try {
		await assert.rejects(
...
test("directory encryption keeps plaintext and emits no index after an encryption failure", async () => {
	const directory = await mkdtemp(join(tmpdir(), "superboard-atomic-backup-"));
	const key = backupEncryptionKey(Buffer.alloc(32, 7).toString("base64"));
...
test("successful production retention verifies the complete migration batch", async () => {
	const directory = await mkdtemp(join(tmpdir(), "superboard-complete-backup-"));
	const key = backupEncryptionKey(Buffer.alloc(32, 7).toString("base64"));
	...
	try {
		...
		assert.equal(index.artifacts.filter(({ path }) => path.endsWith(".enc")).length, 5);
	} finally {
		await rm(directory, { recursive: true, force: true });
	}
});
