...
import { FixtureAdapter } from "../../../scripts/database/module-cutover-adapters.mjs";
import {
	applyPlan,
	buildPlan,
	buildReverseDelta,
...
import { MODULE_CUTOVER_REGISTRY } from "../../../scripts/database/module-cutover-registry.mjs";

const root = resolve(dirname(fileURLToPath(import.meta.url)), "../../..");

export async function buildMigrationRehearsalProof({ pluginIds } = {}) {
	const topology = JSON.parse(
		readFileSync(resolve(root, "scripts/config/emdash-plugin-topology.json"), "utf8"),
	);
	...
	for (const { manifest } of topology.plugins) {
		...
		for (const store of manifest.stores) {
			...
			const schemaProof = "tests/checks/database/module-cutover-registry-schema.test.mjs";
			const runtimeProof = "tests/checks/apps/site/runtime/plugin-store-authority.runtime.test.ts";
			const migrationKind = migratableEntities.length > 0 ? "source_to_store" : "new_empty_store";
