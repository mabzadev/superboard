...
const targetsDirectory = fileURLToPath(new URL("../../../infra/targets/", import.meta.url));

test("every enabled target D1 owner builds a healthy fresh schema", async (t) => {
	const temporary = mkdtempSync(join(tmpdir(), "superboard-target-schemas-"));
	...
	for (const targetName of targetNames) {
		...
		for (const environment of Object.keys(target.environments)) {
			descriptors.push(...targetD1Descriptors(target, targetName, environment));
		}
	}
	...
	for (const descriptor of uniqueOwners.values()) {
		await t.test(`${descriptor.service}: ${descriptor.migrationsDirectory}`, async () => {
