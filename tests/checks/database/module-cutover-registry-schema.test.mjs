...
import { normalizeRows, upsertSql } from "../../../scripts/database/module-cutover-core.mjs";
import {
...
const temporary = mkdtempSync(join(tmpdir(), "superboard-cutover-schema-"));
const databases = {};
...
before(() => {
  ...
  `,
	...
	for (const module of [
		...
		databases[module] = createDatabase(module, `${servicePackagePath(module)}/migrations`);
	}
});
...
test("every registry extraction and destination query matches the checked-in D1 schemas", () => {
	for (const entity of MODULE_CUTOVER_REGISTRY) {
		assert.doesNotThrow(
			() => query(databases[entity.source.database], render(entity.source.query)),
			`${entity.id} source query`,
		);
		const targetDatabase = entity.repositoryOnly
			? databases[entity.source.database]
			: databases[entity.module];
		assert.doesNotThrow(
			() => query(targetDatabase, render(entity.target.query)),
			`${entity.id} target query`,
		);
	}
	for (const guard of MODULE_CUTOVER_GUARDS) {
		assert.doesNotThrow(
			() => query(databases[guard.source.database], render(guard.source.query)),
			`${guard.id} guard query`,
		);
	}
});
...
test("every registry upsert conflict target matches a real destination constraint", () => {
	for (const entity of MODULE_CUTOVER_REGISTRY) {
		...
		assert.match(
			sql,
			new RegExp(`ON CONFLICT \\(${conflict.replace(/[.*+?^${}()|[\\]\\]/g, "\\\\$&")}\\)`),
			entity.id,
		);
	}
	for (const id of [
		...
		assert.deepEqual(MODULE_CUTOVER_REGISTRY.find((entity) => entity.id === id).keys, [
			"project_id",
			"id",
		]);
	}
...
test("projection queries collapse cross-store products and event history without duplicate destination keys", () => {
  ...
  `,
	...
	assert.deepEqual(
		analytics.map((row) => row.event_type),
		["view", "time_spent"],
	);
	...
	assert.deepEqual(
		conversions.map((row) => row.id),
		["legacy-purchase-purchase-14", "legacy-purchase-reversal-17"],
	);
	assert.deepEqual(
		conversions.map((row) => row.revenue_cents),
		[2800, 700],
	);
	assert.equal(conversions[0].campaign_id, "8");
	const customerEvents = sourceRows("app.customer_events");
	assert.deepEqual(
		customerEvents.map((row) => row.event_type),
		["view", "time_spent"],
	);
	...
	assert.deepEqual(
		customerPurchases.map((row) => row.id),
		["legacy-purchase-purchase-14", "legacy-purchase-refund-15", "legacy-purchase-reversal-17"],
	);
	assert.deepEqual(
		customerPurchases.map((row) => row.event_type),
		["purchase", "refund", "purchase"],
	);
	assert.deepEqual(
		customerPurchases.map((row) => row.revenue_cents),
		[2800, 400, 700],
	);
	...
	const socialPreviewEntity = MODULE_CUTOVER_REGISTRY.find(
		(candidate) => candidate.id === "dynamic-links.social_preview",
	);
	...
	const packageGuard = MODULE_CUTOVER_GUARDS.find(
		(guard) => guard.id === "products.package_multi_product_requires_schema",
	);
	assert.deepEqual(
		JSON.parse(query(databases.billing, render(packageGuard.source.query)) || "[]"),
		[],
	);
});
...
function createDatabase(name, migrationsDirectory) {
	...
	for (const filename of readdirSync(directory)
		...
		execFileSync("sqlite3", [database], {
			input: readFileSync(join(directory, filename)),
...
function query(database, sql) {
	return execFileSync("sqlite3", ["-json", database, sql], {
		encoding: "utf8",
