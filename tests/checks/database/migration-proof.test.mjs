...
test("a target-scoped rehearsal converges only Stores owned by its installed plugins", async () => {
	...
	assert.deepEqual(
		[...new Set(proof.stores.map(({ plugin_id: pluginId }) => pluginId))].toSorted(),
		pluginIds,
	);
	assert.ok(proof.stores.length > 0);
	assert.ok(
		proof.stores.every(({ schema_proof: schemaProof }) =>
			CHECKSUM_PATTERN.test(schemaProof.checksum),
		),
	);
});
