...
const cli = resolve(repositoryRoot, "scripts/database/module-cutover.mjs");
const targetArgs = ["--target", "reference-production", "--environment", "production"];

test("backup-receipt hashes every required local export and attaches evidence to the window", () => {
	const directory = mkdtempSync(join(tmpdir(), "superboard-cutover-cli-"));
	try {
		const names = [
			"legacy-api",
			"legacy-messaging",
		...
		const databaseExports = names.map((name) => {
			...
			return {
				name,
				command: ["npx", "wrangler", "d1", "export", name, "--remote", "--output", output],
			};
		});
		...
		writeFileSync(
			backupPlanPath,
			JSON.stringify({ project_ref: "10-prod", database_exports: databaseExports }),
		);
		writeFileSync(
			windowPath,
			JSON.stringify({ schema_version: 1, project_ref: "10-prod", window_id: "window-1234" }),
		);
		execFileSync(
			process.execPath,
			[
		...
		assert.equal(
			receipt.artifacts.every(
				(artifact) => artifact.bytes > 0 && /^[a-f0-9]{64}$/u.test(artifact.sha256),
			),
			true,
		);
...
test("backup plans omit decommissioned legacy Messaging resources", async () => {
	const { createBackupPlan } = await import("../../../scripts/database/module-cutover-core.mjs");
	...
	assert.deepEqual(
		report.database_exports.map(({ name }) => name),
		["legacy-api", "module-support"],
	);
	assert.deepEqual(
		report.worker_versions.map(({ service }) => service),
		["api", "support"],
	);
});
...
test("default plan remains offline unless --remote-read is supplied", () => {
	const output = execFileSync(
		process.execPath,
		[cli, "--project-ref", "10-test", "--modules", "app", ...targetArgs],
		{
			cwd: repositoryRoot,
			encoding: "utf8",
			stdio: ["ignore", "pipe", "pipe"],
...
test("static plans can be restricted to explicit analytics entities", () => {
	const output = execFileSync(
		...
		[
			...
			"app,dynamic-links",
			"--entities",
			"app.customer_events,dynamic-links.analytics_events",
			...targetArgs,
		],
		{
			cwd: repositoryRoot,
	...
	assert.deepEqual(
		report.entities.map((entity) => entity.id),
		report.entity_ids,
	);
});
...
test("backup-plan covers the Site authority and every service/module D1", () => {
	...
	assert.equal(new Set(report.worker_versions.map((item) => item.service)).has("site"), true);
	assert.equal(new Set(report.worker_versions.map((item) => item.service)).has("dashboard"), false);
	assert.equal(
		report.database_exports.every((item) => item.command.includes("--remote")),
		true,
	);
});
...
test("--apply cannot arm a read-only command", () => {
	const result = spawnSync(
		process.execPath,
		[cli, "plan", "--project-ref", "10-test", "--apply", ...targetArgs],
		{
			cwd: repositoryRoot,
			encoding: "utf8",
			stdio: ["ignore", "pipe", "pipe"],
...
test("apply fails closed on a corrupt checkpoint instead of silently starting over", () => {
	const directory = mkdtempSync(join(tmpdir(), "superboard-cutover-checkpoint-"));
	try {
		...
		const result = spawnSync(
			...
			[
				...
				"--apply",
				"--confirm",
				`CUTOVER:reference-production:10-test:${windowId}`,
				...targetArgs,
			],
			{
				cwd: repositoryRoot,
...
test("rollback planning rejects evidence from another project", () => {
	const directory = mkdtempSync(join(tmpdir(), "superboard-cutover-rollback-"));
	try {
		const backupPath = join(directory, "backup.json");
		writeFileSync(
			backupPath,
			JSON.stringify({ project_ref: "10-prod", worker_versions: [], database_exports: [] }),
		);
		const result = spawnSync(
			process.execPath,
			[
