...
test("the complete central API migration chain creates one healthy fresh D1 schema", (t) => {
	...
	assertColumns(database, "billing_mirror_comparisons", ["superboard_active_entitlements"]);
	...
	assertColumns(database, "android_push_configurations", ["encrypted_fcm_server_key"]);
...
test("the Analytics fact backfill is verified-only and idempotent", (t) => {
	const temporary = mkdtempSync(join(tmpdir(), "superboard-analytics-backfill-"));
	const database = join(temporary, "api.sqlite");
	t.after(() => rmSync(temporary, { recursive: true, force: true }));
	const migrations = readdirSync(migrationsDirectory)
		.filter((filename) => filename.endsWith(".sql"))
	...
	const installation = rows.find((row) => row.fact_key.startsWith("installation:"));
	...
	assert.equal(purchase.fact_key, "purchase:7:google:production:order-1:renewal");
...
function assertTables(database, required) {
	const tables = new Set(
		queryRows(database, "SELECT name FROM sqlite_master WHERE type='table'").map((row) => row.name),
	);
