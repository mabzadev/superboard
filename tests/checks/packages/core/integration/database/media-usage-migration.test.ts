...
describeEachDialect("media usage index migration", (dialect) => {
	...
	it("replaces the media lookup index in retry-safe D1 statement order", async () => {
		const migration =
			await import("../../../../../../packages/core/src/database/migrations/052_media_usage_read_index.js");
	...
	it("down() drops tables and up() recreates them", async () => {
		const migration =
			await import("../../../../../../packages/core/src/database/migrations/046_media_usage_index.js");
