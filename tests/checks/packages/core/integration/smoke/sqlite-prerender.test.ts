...
const DONOR_NODE_MODULES = resolve(WORKSPACE_ROOT, "tests/fixtures/emdash-site/node_modules");
...
describe("SQLite static prerender", () => {
	...
	it("builds a page that queries a live collection", { timeout: 120_000 }, async () => {
		...
		const astro = join(DONOR_NODE_MODULES, "astro", "bin", "astro.mjs");
		await execAsync(process.execPath, [astro, "build"], {
			cwd: workDir,
			timeout: 90_000,
			env: {
				...process.env,
