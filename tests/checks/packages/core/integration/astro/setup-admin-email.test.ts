...
import { GET as verifySetupEmail } from "../../../../../../packages/core/src/astro/routes/api/setup/admin-email-verify.js";
import { POST as sendSetupEmail } from "../../../../../../packages/core/src/astro/routes/api/setup/admin-email.js";
...
describe("email-verified administrator setup", () => {
	...
	it("sends the setup link and creates a strongly reauthenticated admin after verification", async () => {
		...
		const verifyResponse = await verifySetupEmail(context(db, verifyRequest, sent, sessionValues));
