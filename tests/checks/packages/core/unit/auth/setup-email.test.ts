...
import { completeAdminEmailSetup, requestAdminEmailSetup, Role } from "@emdash-cms/auth";
...
describe("Admin email setup", () => {
	...
	it("creates the first admin only after the single-use email link is verified", async () => {
		...
		const user = await completeAdminEmailSetup(adapter, url.searchParams.get("token")!, {
			name: "Mabza",
