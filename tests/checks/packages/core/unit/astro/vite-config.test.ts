...
import { createViteConfig } from "../../../../../../packages/core/src/astro/integration/vite-config.js";

describe("createViteConfig admin aliasing", () => {
	const monorepoSiteRoot = new URL("../../../../../../templates/blog/", import.meta.url);
	...
	it("uses raw admin source for local monorepo dev", () => {
		const config = buildConfig(monorepoSiteRoot);
	...
	it("uses built admin dist outside dev", () => {
		const config = buildConfig(monorepoSiteRoot, "build");
		const replacement = getAdminAliasReplacement(config);
