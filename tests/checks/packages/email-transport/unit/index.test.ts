...
import { buildMessage, sendSmtpMessage } from "../../../../../packages/email-transport/src/index";
...
describe("SMTP provider acceptance boundary", () => {
		...
		await expect(
			sendSmtpMessage(
				...
				{ password: null },
				{
					...
					text: "Body",
					html: null,
					messageId: "<superboard-stable@example.test>",
				},
			),
		).resolves.toEqual({
	...
	it("uses the caller's deterministic Message-ID verbatim", () => {
		...
		expect(first.messageId).toBe("<superboard-stable@example.test>");
		expect(second.messageId).toBe(first.messageId);
		expect(first.raw).toContain("Message-ID: <superboard-stable@example.test>");
	});
