...
} from "../../../../../packages/contracts/src/project-context";
...
async function request(token: string, signingSecret: string): Promise<Request> {
	return new Request("https://module.internal/internal/v1", {
		headers: {
			...
			[PROJECT_CONTEXT_HEADERS.signature]: await signProjectContext(context, signingSecret),
		},
	});
}
...
describe("project context secret overlap", () => {
	it("accepts a correctly paired current or previous token and signature", async () => {
		...
		await expect(
			verifyInternalProjectContextRequest(await request(secrets[0], secrets[0]), secrets, "app", {
				nowSeconds: context.issuedAt,
			}),
		).resolves.toMatchObject({ ok: true });
		await expect(
			verifyInternalProjectContextRequest(await request(secrets[1], secrets[1]), secrets, "app", {
				nowSeconds: context.issuedAt,
			}),
		).resolves.toMatchObject({ ok: true });
	});
...
it("binds the stable Site actor to the signed numeric compatibility context", async () => {
	const site = { ...context, actorId: 0, operatorId: "emdash-stable-owner" };
	const unsigned = await request("module-secret", "module-secret");
	const headers = new Headers(unsigned.headers);
	headers.set(PROJECT_CONTEXT_HEADERS.actorId, "0");
	headers.set(PROJECT_CONTEXT_HEADERS.operatorId, site.operatorId);
	headers.set(PROJECT_CONTEXT_HEADERS.signature, await signProjectContext(site, "module-secret"));
	const verify = () =>
		verifyInternalProjectContextRequest(
			new Request(unsigned, { headers }),
			"module-secret",
			"app",
			{ nowSeconds: context.issuedAt },
		);
	expect(await verify()).toMatchObject({
		ok: true,
		context: { actorId: 0, operatorId: "emdash-stable-owner" },
	});
	headers.set(PROJECT_CONTEXT_HEADERS.operatorId, "another-operator");
	expect(await verify()).toMatchObject({ ok: false, code: "project_context_signature_invalid" });
	headers.delete(PROJECT_CONTEXT_HEADERS.operatorId);
	expect(await verify()).toMatchObject({ ok: false, code: "project_context_signature_invalid" });
});
