...
} from "../../../../../packages/contracts/src/support-notifications";
...
describe("Support notification internal contract", () => {
	it("parses a bounded, versioned event and exposes the private path", () => {
		expect(SUPPORT_NOTIFICATION_INGRESS_PATH).toBe("/internal/v1/push/support-notifications");
	...
	it("rejects oversized and malformed events", () => {
		expect(() => parseSupportNotificationEvent({ ...validEvent, body: "x".repeat(2_001) })).toThrow(
			/body/u,
		);
		expect(() => parseSupportNotificationEvent({ ...validEvent, project_id: 0 })).toThrow(
			/project_id/u,
		);
		expect(() =>
	...
	it("parses the bounded API receipt", () => {
		...
		).toMatchObject({ duplicate: true, push_queued: 1 });
	});
});
