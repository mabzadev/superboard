...
} from "../../../../../packages/contracts/src/support";

const fixtures = JSON.parse(
	readFileSync(new URL("../../../../fixtures/contracts/support/v1.json", import.meta.url), "utf8"),
...
describe("support contracts", () => {
	...
	it("normalizes bounded pagination", () => {
		expect(parseSupportCursorPagination({ limit: "25", cursor: "next_1" })).toEqual({
			limit: 25,
			cursor: "next_1",
		});
	...
	it("accepts existing bounded idempotency keys and rejects invalid ones", () => {
		expect(parseSupportIdempotencyKey("message:client-123")).toBe("message:client-123");
		expect(parseSupportIdempotencyKey("short")).toBe("short");
		expect(() => parseSupportIdempotencyKey(" ")).toThrow(/idempotency_key/u);
		expect(() => parseSupportIdempotencyKey("x".repeat(201))).toThrow(/idempotency_key/u);
	});

	it("validates versioned realtime and queue fixtures", () => {
		expect(parseSupportConversationDto((fixtures.success as { data: unknown }).data)).toMatchObject(
			{
				id: "conversation-1",
				status: "open",
				priority: "normal",
		...
		expect(
			parseSupportMessageDto((fixtures.realtime as { message: unknown }).message),
		).toMatchObject({
			id: "message-1",
