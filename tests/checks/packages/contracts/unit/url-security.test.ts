...
import { isSafePublicHttpsUrl } from "../../../../../packages/contracts/src/url-security";

describe("public HTTPS URL policy", () => {
	it("accepts normal HTTPS webhook endpoints", () => {
		expect(isSafePublicHttpsUrl("https://hooks.example.test/superboard?source=support")).toBe(true);
	});
