...
describe("SQL health contracts", () => {
	it("checks reachability with the smallest read", async () => {
		await expect(
			assertSqlDatabaseHealth(database({ superboard_health_check: 1 })),
		).resolves.toBeUndefined();
		await expect(assertSqlDatabaseHealth(database({ superboard_health_check: 0 }))).rejects.toThrow(
			/unexpected result/u,
		);
	});
	...
	it("combines reachability and schema inspection", async () => {
		...
		await expect(
			inspectSqlDatabaseAndSchemaHealth(databaseSequence(rows), "0001_initial.sql"),
	...
	it("fails closed on invalid configuration or malformed ledger rows", async () => {
		await expect(inspectSqlSchemaHealth(database({}), "latest")).rejects.toThrow(
