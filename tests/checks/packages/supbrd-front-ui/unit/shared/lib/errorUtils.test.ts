...
import { ApiError } from "../../../../../../../packages/supbrd-front-ui/src/shared/lib/ApiError.js";
import {
	categorizeError,
	registerGlobalErrorHandlers,
} from "../../../../../../../packages/supbrd-front-ui/src/shared/lib/errorUtils.js";
...
describe("categorizeError", () => {
	...
	it("categorizes ApiError 5xx as server error", () => {
		expect(categorizeError(new ApiError("Error", 500)).title).toBe("Server error");
		expect(categorizeError(new ApiError("Error", 502)).title).toBe("Server error");
		expect(categorizeError(new ApiError("Error", 503)).title).toBe("Server error");
	});
	...
	it("categorizes fetch errors", () => {
		expect(categorizeError(new Error("network error")).title).toBe("Connection error");
		expect(categorizeError(new Error("unable to connect")).title).toBe("Connection error");
		expect(categorizeError(new Error("no internet")).title).toBe("Connection error");
	});
	...
	it("categorizes 502 and 503 errors", () => {
		expect(categorizeError(new Error("502 Bad Gateway")).title).toBe("Server error");
		expect(categorizeError(new Error("503 Service Unavailable")).title).toBe("Server error");
	});
...
describe("registerGlobalErrorHandlers", () => {
	...
	it("calls callback with Error rejection", () => {
		...
		const handler = spy.mock.calls.find((call) => call[0] === "unhandledrejection")?.[1] as (
