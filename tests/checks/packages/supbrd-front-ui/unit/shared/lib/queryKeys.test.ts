...
import { queryKeys } from "../../../../../../../packages/supbrd-front-ui/src/shared/lib/queryKeys.js";
import type {
	GetLinksParams,
	GetCampaignsParams,
	DateRangeQuery,
...
describe("queryKeys", () => {
	describe("projects", () => {
		...
		it("builds link-specific keys", () => {
			...
			expect(queryKeys.projects.randomPath("p1")).toEqual(["projects", "p1", "randomPath"]);
		...
		it("builds configuration keys", () => {
			expect(queryKeys.projects.redirectConfig("p1")).toEqual(["projects", "p1", "redirectConfig"]);
			expect(queryKeys.projects.domainConfig("p1")).toEqual(["projects", "p1", "domainConfig"]);
		});
	});
	...
	describe("instances", () => {
		...
		it("builds instance sub-keys", () => {
			expect(queryKeys.instances.members("i1")).toEqual(["instances", "i1", "members"]);
			expect(queryKeys.instances.config("i1")).toEqual(["instances", "i1", "config"]);
			expect(queryKeys.instances.userRole("i1")).toEqual(["instances", "i1", "userRole"]);
