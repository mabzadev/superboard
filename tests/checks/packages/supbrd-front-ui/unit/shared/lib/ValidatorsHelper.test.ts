...
import {
	...
	isUrlSchemeValid,
} from "../../../../../../../packages/supbrd-front-ui/src/shared/lib/validation.js";
...
describe("bundleIdSchema", () => {
	it("accepts valid bundle IDs", () => {
		...
		expect(bundleIdSchema.safeParse("io.superboard.test").success).toBe(true);
...
describe("redirect form validation", () => {
	it("requires a secure destination for custom redirects while allowing the default destination", () => {
		...
		expect(isRedirectUrlValid("custom", "https://example.com/path?source=app", "default")).toBe(
			true,
		);
	});
