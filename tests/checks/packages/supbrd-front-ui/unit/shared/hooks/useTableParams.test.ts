...
import { renderHook, act } from "../../../render.js";
...
import { useTableParams } from "../../../../../../../packages/supbrd-front-ui/src/shared/hooks/useTableParams.js";
...
describe("useTableParams", () => {
	...
	describe("defaults", () => {
		...
		it("uses custom defaultDateRange from options", () => {
			...
			const { result } = renderHook(() => useTableParams({ defaultDateRange: { from, to } }));
	...
	describe("setRowsPerPage", () => {
		it("updates URL and resets page", () => {
			...
			expect(mockReplace).toHaveBeenCalledWith(expect.stringContaining("perPage=50"), {
				scroll: false,
			});
			// page should be removed (reset)
			expect(mockReplace).toHaveBeenCalledWith(expect.not.stringContaining("page="), {
				scroll: false,
			});
		});
	...
	describe("setSort", () => {
		...
		it("supports functional updates", () => {
			...
			expect(mockReplace).toHaveBeenCalledWith("/test-path?sort=updated_at%3Aasc", {
				scroll: false,
			});
		});
	});
	...
	describe("setDateRange", () => {
		it("updates URL with date range", () => {
			...
			expect(mockReplace).toHaveBeenCalledWith(expect.stringContaining("from="), { scroll: false });
