...
} from "../../../../../packages/cloudflare/src/db/d1-migration-target.js";
...
describe("loadProjectWranglerConfig", () => {
	it("uses project-local Wrangler and preserves account inheritance for a named environment", async () => {
		const cloudflarePackage = resolve(import.meta.dirname, "../../../../../packages/cloudflare");
		const projectRoot = resolve(cloudflarePackage, "../marketplace");
		const configPath = resolve(
			import.meta.dirname,
			"../../../../fixtures/packages/cloudflare/tests/d1-wrangler.jsonc",
		);
