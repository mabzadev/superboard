...
import {
	...
	secretInventory,
} from "../../../scripts/cloudflare/secret-inventory.mjs";
...
test("secret coordination covers every required binding without values", async () => {
	for (const [targetName, environment] of [
		...
		assert.equal(plan.contracts.flatMap((contract) => contract.members).length, expected);
...
test("managed Worker secrets are application-specific and value-free", async () => {
	...
	const gatewayContract = secretCoordinationPlan(target, "production").contracts.find(
		({ id }) => id === "managed-worker-gateway-callback-token",
	);
	assert.equal(gatewayContract.sameValueRequired, true);
	assert.deepEqual(gatewayContract.externalPeers, ["api-auth-gateway/INTERNAL_CALLBACK_TOKEN"]);
	assert.deepEqual(
...
test("shared production contracts identify both ends and environment-specific billing ownership", async () => {
	...
	assert.equal(
		productionPlan.contracts.some(
			({ id }) => id === "flows-internal-token" || id === "flows-email-internal-token",
		),
		false,
	);
	assert.equal(
