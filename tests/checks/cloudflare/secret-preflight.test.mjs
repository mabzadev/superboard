...
import {
	...
	parseSecretNames,
} from "../../../scripts/cloudflare/secret-preflight.mjs";
...
test("an unavailable Worker inventory blocks readiness without exposing command output", () => {
	const report = evaluateSecretReadiness(
		...
		{
			files: "Secret inventory unavailable for Worker superboard-files-production",
		},
	);
	...
	assert.deepEqual(report.services[0], {
		service: "files",
		ready: false,
		inspectionError: "Secret inventory unavailable for Worker superboard-files-production",
