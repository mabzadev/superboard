...
import {
	...
	retiredDomainOwners,
	targetWorkerNames,
} from "../../../scripts/cloudflare/domain-plan-core.mjs";
...
const target = {
	...
	workers: {
		api: { development: "superboard-api-dev" },
		dashboard: { development: "superboard-dashboard-dev" },
		site: { development: "superboard-site-dev" },
		mcp: { development: "superboard-mcp-dev" },
		email: { development: "superboard-email-dev" },
	},
};
...
test("the domain plan assigns every public surface to one explicit Worker", () => {
	...
	assert.deepEqual(
		expected
			.filter((entry) => entry.service === "superboard-api-dev")
...
test("available and already managed domains are non-blocking", () => {
	...
	const result = evaluateDomainPlan({
		...
		workerDomains: [{ hostname: "api.example.test", service: "superboard-api-dev" }],
		dnsRecordsByHostname: {
			"api.example.test": [{ type: "AAAA", proxied: true }],
		},
	});
	assert.equal(result.find((entry) => entry.surface === "api").status, "managed");
	assert.equal(result.find((entry) => entry.surface === "sdk").status, "available");
	assert.equal(
...
test("an occupied DNS name or a different Worker blocks deployment", () => {
	...
	assert.equal(result.find((entry) => entry.surface === "api").status, "wrong-worker");
	assert.equal(result.find((entry) => entry.surface === "shortlinks").status, "dns-conflict");
...
test("retired domains fail closed until Worker and DNS ownership are removed", () => {
	...
	const occupied = evaluateRetiredDomainPlan({
		...
		workerDomains: [
			{
				hostname: "old-grow.example.test",
				service: "superboard-dashboard-dev",
			},
		],
		dnsRecordsByHostname: {},
	});
	...
	assert.deepEqual(orphaned, [
		{
			...
			blocking: true,
			currentServices: ["superboard-api-dev"],
		},
	]);
});

test("MCP on the console path keeps one Site owner and six canonical hostnames", () => {
	const configured = structuredClone(target);
	configured.domains.site = "board.example.test";
	configured.domains.dashboard = "board.example.test";
	configured.domains.mcp = "board.example.test";
	delete configured.domains.mailPreview;
	const owners = expectedDomainOwners(configured, "development");
	assert.equal(owners.length, 6);
	assert.deepEqual(
		owners.filter((owner) => owner.hostname === "board.example.test"),
		[{ surface: "site", hostname: "board.example.test", service: "superboard-site-dev" }],
	);
});
