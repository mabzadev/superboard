...
} from "../../../scripts/cloudflare/bootstrap.mjs";
...
test("Reference application bootstrap owns the legacy media bucket used by managed Workers", async () => {
	...
	assert.ok(
		resources.some(
			({ key, name, logicalName, physicalName }) =>
				...
				physicalName === "app-reference-production" &&
				logicalName === "superboard-app-reference-production",
		),
	);
});
...
test("canonical remote resources with missing manifest ids are adopted without creation", async () => {
	...
	assert.equal(plan.operations.filter(({ type }) => type === "adopt").length, 18);
...
test("configured ids are fail-closed when Cloudflare reports another name", async () => {
	...
	const central = inventories.d1.find(({ uuid }) => uuid === target.environments.production.d1.id);
	...
	assert.ok(
		plan.blockers.some(
			({ type, key }) => type === "configured-resource-name-mismatch" && key === "d1",
		),
	);
...
test("Vectorize bootstrap is fail-closed on immutable index configuration drift", async () => {
	...
	const knowledge = inventories.vectorize.find(
		({ name }) => name === "superboard-support-v2-knowledge",
	);
	...
	assert.ok(
		plan.blockers.some(
			({ type, key }) =>
				type === "configured-resource-shape-mismatch" && key === "moduleVectorize.supportKnowledge",
		),
	);
});
...
test("fresh Support bootstrap creates only empty v2 resources and never adopts", async () => {
	...
	const existing = inventoriesFor(desiredCloudflareResources(target, "production"));
	...
	assert.equal(
		blocked.blockers.every(({ type }) => type === "fresh-resource-already-exists"),
		true,
	);
});
...
test("bootstrap requires the exact plan confirmation and records returned ids", async () => {
	...
	assert.match(target.environments.development.moduleD1.support.id, /^[a-f0-9-]{36}$/u);
});
