...
test("pointer rollback writes immutable history and a rollback outbox event", (t) => {
	...
	execute(
     ...
     INSERT INTO superboard_front_release_candidates (
       ...
       ('candidate-a', 'reference-production', 'release-a', '{}', 'checksum-a', 'set-a', 'key', 'approved', '{}', '2026-08-30T00:00:00.000Z'),
       ('candidate-b', 'reference-production', 'release-b', '{}', 'checksum-b', 'set-b', 'key', 'approved', '{}', '2026-08-30T00:00:00.000Z');
     INSERT INTO superboard_operator_reauthentication_receipts (receipt_id,operator_id,instance_id,action,candidate_id,reauthenticated_at,expires_at,receipt_checksum,created_at) VALUES
       ('reauth-a', 'operator', 'reference-production', 'front_release.approve', 'candidate-a',
     ...
     INSERT INTO superboard_front_active_releases VALUES
       ('reference-production', 'release-a', NULL, 1, 'activation-a', '2026-08-30T00:01:00.000Z');
     UPDATE superboard_front_active_releases
       ...
       WHERE instance_id = 'reference-production';
     UPDATE superboard_front_active_releases
	...
	assert.throws(
		() =>
			execute(
				database,
				"UPDATE superboard_front_approval_reauthentication SET receipt_id = 'changed' WHERE candidate_id = 'candidate-a'",
			),
		/immutable/u,
	);
});
