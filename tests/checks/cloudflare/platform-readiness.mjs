...
import { validateFlutterFlowLibraryContract } from "../../../scripts/clients/flutterflow-library/flutterflow-library-contract.mjs";
import { buildFlutterFlowMigrationPlan } from "../../../scripts/clients/flutterflow-migration-plan.mjs";
import { flutterFlowSourceEnvironmentName } from "../../../scripts/clients/flutterflow-source-verify.mjs";
import { releaseCandidateTagFor, releaseTagFor } from "../../../scripts/clients/sdk-catalog.mjs";
import {
	canonicalReleaseTag,
	immutableFailureFor,
} from "../../../scripts/clients/sdk-release-history.mjs";
import { superboardEnvironmentValue } from "../../../scripts/cloudflare/environment.mjs";
import {
	assertTargetPhysicalResourceNames,
	resourceIdentity,
} from "../../../scripts/cloudflare/resource-identity.mjs";
import { DOMAIN_SERVICES, DOMAIN_SERVICE_REGISTRY } from "../../../scripts/cloudflare/services.mjs";
import {
	cloudflareAccountEnvName,
	loadTarget,
	parseArgs,
	root,
} from "../../../scripts/cloudflare/target.mjs";
import {
	loadDeploymentMatrix,
	validateControlPlaneCoverage,
} from "../../../scripts/github/deployment-matrix.mjs";
import { inspectRepository, loadGitHubControlPlane } from "../../../scripts/github/readiness.mjs";
...
export function requiredResourceIds(target, environment) {
	...
	if (!resources) throw new Error(`${target.target} does not define ${environment}`);
	const required = [
		resource("d1", "Central API D1", resources.d1, uuidPattern),
		resource("kv", "API KV", resources.kv, kvPattern),
		resource("siteD1", "Site D1", resources.siteD1, uuidPattern),
		resource("siteSessionKv", "Site session KV", resources.siteSessionKv, kvPattern),
		resource("siteReleaseKv", "Site release KV", resources.siteReleaseKv, kvPattern),
	...
	if (target.features?.messaging) {
		required.push(
			resource("messagingD1", "Legacy Messaging D1", resources.messagingD1, uuidPattern),
		);
	}
	if (target.customWorker?.d1Binding) {
		required.push(resource("customD1", "Custom Worker D1", resources.customD1, uuidPattern));
	}
	for (const service of DOMAIN_SERVICES.filter((name) => target.features?.[name])) {
...
export function targetReadiness(target, environment) {
	...
	const acceptance = {
		googleAudienceConfigured:
			Array.isArray(identity.googleAudiences) && identity.googleAudiences.length > 0,
		appleAudienceConfigured:
			Array.isArray(identity.appleAudiences) && identity.appleAudiences.length > 0,
		supportProjectAllowlisted:
			!target.features?.support ||
			(target.environments[environment].supportProjectIds || []).length > 0,
		developmentMailCaptured: environment !== "development" || target.mail?.transport === "capture",
		legacyMessagingDisabled: target.features?.messaging === false,
		frontMediaIsolated: Boolean(
			environmentResources.siteMedia?.name &&
			environmentResources.r2?.name &&
			environmentResources.siteMedia.name !== environmentResources.r2.name,
		),
	};
	return {
...
export function inspectSdkRemoteState(
	...
	const publications = (catalogue.libraries || []).map((library) => {
		...
		const pending = lifecycle === "active" && library.releaseStatus !== "released";
		const tag = pending
		...
		const packageRefState =
			packageRef === tag ? tagState : inspectGitHubTag(repository, packageRef, run);
		const releaseExists = githubReleaseExists(repository, tag, run);
		const packageArtifact = inspectGitHubPackageArtifact(library, repository, run);
		...
		if (pending) {
			...
			else if (tagState.sha !== library.releaseSha) blockers.push("release-tag-sha-mismatch");
			...
			blockers.push(...packageArtifactBlockers(packageArtifact, library.latestReleaseVersion));
		}
		return {
			id: library.id,
			lifecycle,
	...
	const failedReleases = releaseHistoryRemoteState(releaseHistory, repository, run);
	return {
		inspected: true,
		ready: publications.every((publication) => publication.ready) && failedReleases.ready,
		publications,
		failedReleases,
	};
}
...
export function inspectGitHubTag(repository, reference, run = runGitHubRead) {
	const seen = new Set();
	const initial = run(["api", `repos/${repository}/git/ref/tags/${encodeURIComponent(reference)}`]);
	if (!initial.ok) {
		return {
			ref: reference,
			exists: false,
...
function inspectPublishedBaseline(library, repository, run, packageArtifact) {
	...
	const tagState = inspectGitHubTag(repository, tag, run);
	const packageRefState =
		library.releaseRef === tag ? tagState : inspectGitHubTag(repository, library.releaseRef, run);
	const releaseExists = githubReleaseExists(repository, tag, run);
	const blockers = [];
	...
	blockers.push(
		...packageArtifactBlockers(packageArtifact, library.latestReleaseVersion, "baseline-"),
	);
	return {
		tag,
...
function inspectGitHubPackageArtifact(library, repository, run) {
	...
	const artifactRepository = library.distribution?.repository || repository;
...
function packageArtifactBlockers(artifact, version, prefix = "") {
	...
	if (!artifact.exists) blockers.push(`${prefix}package-artifact-missing`);
		...
		if (artifact.visibility !== "public") blockers.push(`${prefix}package-artifact-not-public`);
		if (artifact.repository !== artifact.expectedRepository)
			blockers.push(`${prefix}package-artifact-repository-mismatch`);
		if (!artifact.versions.includes(version)) blockers.push(`${prefix}package-version-missing`);
	}
	return blockers;
}
...
function releaseHistoryRemoteState(history, repository, run) {
	const failures = (history?.immutableFailures ?? []).map((failure) => {
		const refs = [failure.releaseTag, ...failure.packageRefs].map((reference) => {
		...
		const releaseExists = githubReleaseExists(repository, failure.releaseTag, run);
		const blockers = [];
		for (const reference of refs) {
			if (!reference.exists) blockers.push(`failed-ref-missing:${reference.ref}`);
...
function githubReleaseExists(repository, tag, run) {
	return run(["api", `repos/${repository}/releases/tags/${encodeURIComponent(tag)}`]).ok;
}
...
export function sdkReadiness(catalogue, remoteState = null) {
	const pending = (catalogue.libraries || [])
		.filter(
			(library) =>
				(library.lifecycle ?? "active") === "active" && library.releaseStatus !== "released",
		)
		.map((library) => ({
			id: library.id,
	...
	return {
		ready: pending.length === 0 && (remoteState === null || remoteState.ready === true),
		total: (catalogue.libraries || []).length,
		lifecycle: Object.fromEntries(
			["active", "internal", "archived"].map((value) => [
				value,
				(catalogue.libraries || []).filter((library) => (library.lifecycle ?? "active") === value)
					.length,
			]),
		),
		pending,
...
export function referenceReadiness(project, developmentTarget, sdkCatalogue, repositories) {
	...
	const referenceHostname = referenceUrl ? new URL(referenceUrl).hostname : null;
	const repositoryUrl = (repository) =>
		repository?.nameWithOwner ? `https://github.com/${repository.nameWithOwner}` : null;
	const checks = [
		...
		check("deployment_environment", project.deployment?.environment === "development"),
		check("deployment_worker", /^[a-z0-9][a-z0-9-]*$/u.test(project.deployment?.workerName || "")),
		check(
			"platform_repository",
		...
		check(
			"sdk_identifier",
			Boolean(referenceHostname) && project.sdkApplication?.identifier === referenceHostname,
		),
		check("sdk_environment", project.sdkApplication?.projectEnvironment === "test"),
		check(
			"reference_domain",
		...
		check(
			...
			project.endpoints?.dashboard ===
				`https://${developmentTarget.domains.dashboard ?? developmentTarget.domains.site}`,
		),
		check("api_domain", project.endpoints?.api === `https://${developmentTarget.domains.api}`),
		check("sdk_domain", project.endpoints?.sdk === `https://${developmentTarget.domains.sdk}`),
		check(
			"short_links_domain",
			project.endpoints?.shortLinks === `https://${developmentTarget.domains.shortlinks}`,
		),
		check(
			"files_domain",
...
export function credentialReadiness(target, environment, env = process.env) {
	...
	const required = [
		{
			...
			configured: Boolean(String(env[scopedAccountName] || env.CLOUDFLARE_ACCOUNT_ID || "").trim()),
		},
		{
	...
	if (environment === "production") {
		required.push({
			...
			configured: Boolean(superboardEnvironmentValue("SUPERBOARD_BACKUP_ENCRYPTION_KEY", env)),
		});
	}
...
async function applicationClientReadiness({ clientSources, env }) {
	const directory = resolve(root, "scripts/config/flutterflow-sources");
	...
	for (const name of files) {
		...
		if (clients[application]) {
			throw new Error(`Duplicate FlutterFlow application snapshot: ${application}`);
		}
		const environmentName = clientSourceEnvironmentName(application);
		const migrationManifestPath = resolve(
			root,
			"scripts/config/flutterflow-migrations",
			`${application}.json`,
		);
		...
		try {
			...
			clients[application] = {
				...
				blockers: migration.convergenceBlockers || ["convergence-contract-missing"],
			};
		} catch (error) {
	...
	if (unknownSources.length > 0) {
		throw new Error(`Unknown FlutterFlow application source: ${unknownSources.sort().join(", ")}`);
	}
	return clients;
}
...
export async function buildReadiness({
	remote = false,
	referenceRoot = superboardEnvironmentValue("SUPERBOARD_REFERENCE_ROOT", process.env) || null,
	clientSources = {},
	env = process.env,
} = {}) {
	...
	const deploymentMatrix = {
		...
		entries: deploymentConfiguration.deployments.map((deployment) => ({
			...
			target:
				controlPlane.repositories.platform.environments[deployment.githubEnvironment].variables
					.SUPERBOARD_TARGET,
	...
	const targetNames = [...new Set(deploymentMatrix.entries.map((entry) => entry.target))];
	const targets = {};
	const loadedTargets = {};
	...
	const catalogue = JSON.parse(
		await readFile(resolve(root, "scripts/config/sdk-libraries.json"), "utf8"),
	);
	const sdkReleaseHistory = JSON.parse(
		await readFile(resolve(root, "scripts/config/sdk-release-history.json"), "utf8"),
	);
	...
	const flutterFlowApplications = await validateFlutterFlowApplicationWorkspaces();
	const governance = JSON.parse(
		await readFile(resolve(root, "scripts/config/platform-governance.json"), "utf8"),
	);
	const governanceSchema = JSON.parse(
		await readFile(resolve(root, "scripts/config/platform-governance.schema.json"), "utf8"),
	);
	const validateGovernance = new Ajv2020({ allErrors: true }).compile(governanceSchema);
	if (!validateGovernance(governance)) {
		throw new Error(`Invalid platform governance: ${JSON.stringify(validateGovernance.errors)}`);
	}
	if (
		governance.canonicalRepository !== controlPlane.repositories.platform.nameWithOwner ||
		governance.referenceRepository !== controlPlane.repositories.platform.nameWithOwner ||
		governance.referencePath !== "apps/reference"
	) {
		throw new Error("Platform governance repositories must match the GitHub control plane");
	}
	const referenceProject = JSON.parse(
		await readFile(resolve(resolvedReferenceRoot, "reference.project.json"), "utf8"),
	);
	...
	const git = {
		...
		reference: inspectGit(resolvedReferenceRoot, controlPlane.repositories.platform),
	};
	const repositories = remote
		? Object.values(controlPlane.repositories).map((repository) => inspectRepository(repository))
		: null;
	const disconnectedBranchHistories = remote
		? repositories.filter((repository) => repository.branchHistory?.ready === false)
		: [];
	const github = {
		inspected: remote,
		ready: remote ? repositories.every((repository) => repository.ready) : null,
		branchHistoryReady: remote ? disconnectedBranchHistories.length === 0 : null,
		repositories,
	};
	const historicalSourceAvailable = existsSync(resolve(root, "upstream/superboard/backend"));
	...
	const historicalParity = {
		ready: governance.historicalUpstream.releasePolicy === "retired" && governanceEvidenceReady,
		releasePolicy: governance.historicalUpstream.releasePolicy,
		sourceAvailable: historicalSourceAvailable,
		optionalComparisonCommand: governance.historicalUpstream.optionalComparisonCommand,
	...
	const securityContracts = {
		...
		workerSecretBundles: {
			...
			promotion: "account-bound-version-and-rollback-ids-with-reverse-order-recovery",
			sharedCutover: "current-plus-previous-consumers-before-new-token-only-producers",
	...
	for (const [application, client] of Object.entries(applicationClients)) {
		const receiptReplaced = client.verificationIssue === "reviewed-export-receipt-replaced";
		addBlocker(
			blockers,
			!client.ready,
	...
	const localContractsReady =
		...
		Object.values(targets).every(
			(target) =>
				...
				target.acceptance.frontMediaIsolated,
		);
	const sourceControlReady = git.platform.ready && git.reference.ready && github.ready === true;
	...
	return {
		...
		ready: localContractsReady && sourceControlReady && deploymentPrerequisitesReady,
		stages: {
			...
			clientConvergence: {
				ready: Object.values(applicationClients).every((client) => client.ready),
			},
...
export function resolveReferenceRepositoryRoot({
	...
	if (referenceRoot !== null && referenceRoot !== undefined) {
		...
		if (!configuredRoot || !isAbsolute(configuredRoot)) {
			throw new Error("SuperBoard reference root must be a non-empty absolute path");
		}
...
async function main() {
	...
	const report = await buildReadiness({
		...
		referenceRoot: args["reference-root"] ? String(args["reference-root"]) : undefined,
	});
