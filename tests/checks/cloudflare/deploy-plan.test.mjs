...
} from "../../../scripts/cloudflare/deploy-plan.mjs";
...
test("full production deployment backs up and migrates every D1 before Workers", async () => {
	...
	assert.ok(plan.services.indexOf("managed-medias-orchestrator") < plan.services.indexOf("custom"));
...
test("runtime bridge deployment opens only after routing, callback ownership, and review converge", async () => {
	const original = (await loadTarget("reference-production")).target;
...
test("the deploy orchestrator consumes a verified batch before its Worker loop", async () => {
	const deployAll = await readFile(
		new URL("../../../scripts/cloudflare/deploy-all.mjs", import.meta.url),
		"utf8",
	);
	const deployService = await readFile(
		new URL("../../../scripts/cloudflare/deploy.mjs", import.meta.url),
		"utf8",
	);
	const allowlist = await readFile(
		new URL("../../../scripts/cloudflare/allowlist.mjs", import.meta.url),
		"utf8",
	);
	...
	assert.match(deployAll, /const targetCloudflareEnv = cloudflareEnv\(target\)/u);
	assert.match(deployService, /const targetCloudflareEnv = cloudflareEnv\(target\)/u);
	const finalConfigGeneration = deployService.lastIndexOf("generateServiceConfig();");
	const identityCutover = deployService.indexOf("await enforceIdentityProjectCutover");
	const workerDeployment = deployService.lastIndexOf('if (service === "site")');
	...
	assert.ok(
		deployService.indexOf("runtimeBridgeDeploymentBlockers") <
			deployService.indexOf("scripts/cloudflare/config.mjs"),
	);
	assert.match(deployService, /await enforceIdentityProjectCutover/u);
	assert.match(deployService, /!migrationsConvergedByBatch/u);
	const allowlistBootstrap = deployService.indexOf("scripts/cloudflare/allowlist.mjs");
	assert.ok(
		allowlistBootstrap > 0 &&
			deployService.indexOf('...(args["no-routes"] ? ["--no-routes"] : [])', allowlistBootstrap) >
				allowlistBootstrap,
	);
