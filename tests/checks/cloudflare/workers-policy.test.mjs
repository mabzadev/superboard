...
import { deploymentOrder } from "../../../scripts/cloudflare/deploy-plan.mjs";
...
import { isIgnoredDirectory, isLocalOnly } from "../../lints/configuration-boundaries.mjs";

const root = resolve(fileURLToPath(new URL("../../../", import.meta.url)));
const schema = JSON.parse(
	readFileSync(resolve(root, "node_modules/wrangler/config-schema.json"), "utf8"),
);
const wranglerVersion = JSON.parse(
	readFileSync(resolve(root, "node_modules/wrangler/package.json"), "utf8"),
).version;
const validateConfig = new Ajv({ allErrors: true, strict: false }).compile(schema);
const configurationBoundaryContract = JSON.parse(
	readFileSync(resolve(root, "scripts/config/configuration-boundaries.json"), "utf8"),
);
...
test("every target Worker satisfies the production configuration policy", async () => {
	...
	for (const targetName of targetNames()) {
		...
		for (const environment of Object.keys(target.environments).sort()) {
			for (const service of deploymentOrder(target)) {
				...
				const path = resolve(
					root,
					`infra/generated/${targetName}-${service}-${environment}-policy.jsonc`,
				);
	...
	assert.ok(checked.length > 0, "at least one target Worker must be policy-checked");
});
...
test("production Worker sources reject unsafe runtime anti-patterns", () => {
	const forbidden = [
		...
		[/const\s*\{\s*waitUntil\s*\}\s*=\s*\w+/u, "destructured waitUntil loses its runtime binding"],
	];
	const cloudflareRestCallers = [];
	for (const path of Object.entries(SERVICE_PACKAGE_PATHS)
		.filter(([service]) => service !== "site")
		.flatMap(([, path]) => workerSourceFiles(resolve(root, path)))) {
		const source = readFileSync(path, "utf8");
		for (const [pattern, reason] of forbidden) {
			assert.doesNotMatch(source, pattern, `${path.slice(root.length + 1)}: ${reason}`);
		}
...
test("source Wrangler templates never pin a Cloudflare account resource", () => {
	const resourceId = /"(?:account_id|database_id|namespace_id|zone_id)"\s*:\s*"([^"]+)"/gu;
	const placeholder = /^00000000-0000-0000-0000-000000000\d{3}$/u;
	for (const path of Object.entries(SERVICE_PACKAGE_PATHS)
		.filter(([service]) => service !== "site")
		.flatMap(([, path]) => wranglerSourceTemplates(resolve(root, path)))) {
		const source = readFileSync(path, "utf8");
...
test("portable runtime sources never pin an application domain, account id, or workstation path", () => {
	const forbidden = [
		[
			...
			"application domains belong in infra/targets manifests",
		],
		[
			/\baccount_id\s*[:=]\s*["'][a-f0-9]{32}["']/iu,
			"Cloudflare account ids must come from scoped environment credentials",
		],
		[/(?:^|["'`\s])\/Users\/[A-Za-z0-9._-]+\//mu, "absolute workstation paths are not portable"],
	];
	const roots = ["apps/site/src", "apps/mcp/src", "packages", "scripts", "sdks"];
	let checked = 0;
	...
	assert.ok(checked > 0, "at least one portable runtime source must be checked");
});
...
test("every maintained JavaScript package explicitly inherits the repository MIT license", () => {
	...
	for (const relativeRoot of ["apps", "packages", "sdks"]) {
		manifests.push(...sourcePackageManifests(resolve(root, relativeRoot)));
	}
	assert.ok(manifests.length > 1, "workspace package manifests must be discovered");
...
test("response buffering policy covers typed and named response variables", () => {
	assert.match("const value = await response.json<Result>();", UNBOUNDED_RESPONSE_BUFFERING);
	assert.match("const value = await manifestResponse.text();", UNBOUNDED_RESPONSE_BUFFERING);
	assert.doesNotMatch(
		"const value = await readJsonObjectLimited(response, 1024);",
		UNBOUNDED_RESPONSE_BUFFERING,
	);
	assert.doesNotMatch("return c.json({ status: 'ok' });", UNBOUNDED_RESPONSE_BUFFERING);
});

function targetNames() {
	return readdirSync(resolve(root, "infra/targets"), { withFileTypes: true })
		.filter(
			(entry) => entry.isFile() && entry.name.endsWith(".json") && entry.name !== "schema.json",
		)
...
function portableRuntimeFiles(directory) {
	...
	for (const entry of readdirSync(directory, { withFileTypes: true })) {
		...
		if (entry.isDirectory()) {
			...
			if (
				isIgnoredDirectory(relativePath, entry.name, configurationBoundaryContract) ||
...
function assertWorkerPolicy(config, target, service) {
	assert.equal(config.workers_dev, false, `${service} must disable workers.dev`);
	assert.equal(config.preview_urls, false, `${service} must disable preview URLs`);
	...
	assert.equal(config.routes, undefined, `${service} preflight must not own routes`);
	assert.equal(config.triggers, undefined, `${service} preflight must not own triggers`);
	assert.equal(config.queues?.consumers, undefined, `${service} preflight must not consume queues`);
...
function assertCurrentCompatibilityDate(value, service) {
	...
	const now = Date.now();
	assert.ok(selected <= now + 86_400_000, `${service} compatibility date is in the future`);
	assert.ok(
