...
} from "../../../scripts/cloudflare/secret-retire.mjs";
...
async function fixture() {
	...
	const remote = Object.fromEntries(
		structural.services.map((service, index) => [
			service.service,
			{
				deployment: {
...
test("retirement waits for overlap and removes only consumer previous bindings", async () => {
	...
	assert.deepEqual(retirementDeleteArgs(plan.services[0], plan.services[0].names[0], "retire"), [
		"wrangler",
		"versions",
		"secret",
		"delete",
		...
		"retire",
		"--tag",
		plan.services[0].names[0].versionTag,
	]);
});
