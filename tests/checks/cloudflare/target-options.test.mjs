...
} from "../../../scripts/cloudflare/target-options.mjs";
...
const context = {
	...
	baseResourceName: "superboard-new-app-dev",
};

test("new targets isolate dashboard cache from application files", () => {
	assert.equal(
		dashboardCacheResourceName("superboard-new-app-dev"),
		"superboard-new-app-dev-dashboard-cache",
	);
});
...
test("custom target options cover D1, vars, secrets, crons and services", () => {
	const result = customTargetOptions(
		{
			"custom-source": "apps/new-app/worker/src/index.ts",
			...
			"custom-migrations-dir": "apps/new-app/worker/migrations",
	...
	assert.deepEqual(result, {
		worker: {
			source: "apps/new-app/worker/src/index.ts",
			packagePath: "apps/new-app/worker",
			...
			d1Binding: {
				binding: "CUSTOM_DB",
				migrationsDir: "apps/new-app/worker/migrations",
			},
			serviceBindings: [
				{
		...
		environmentResources: {
			customD1: { name: "superboard-new-app-dev-custom-db", id: null },
		},
	});
});
...
test("the new-application template produces a complete valid custom target", async () => {
	...
	assert.notEqual(
		...
		manifest.environments.development.siteMedia.name,
	);
	assert.equal(
	...
	assert.deepEqual(manifest.applicationIdentity.webOrigins, ["https://reference.sample.dev"]);
...
test("target registration keeps provisioning separate from deployment", () => {
	const source = readFileSync(
		new URL("../../../scripts/cloudflare/register-target.mjs", import.meta.url),
		"utf8",
	);
