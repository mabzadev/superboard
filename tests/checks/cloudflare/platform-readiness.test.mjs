...
test("target readiness distinguishes unresolved fixtures from provisioned targets", async () => {
	...
	const referenceApplicationResult = targetReadiness(referenceApplication, "production");
	...
	assert.equal(referenceApplicationResult.resourceIds.required, 16);
	assert.deepEqual(
		referenceApplicationResult.resourceIds.missing.map(({ key, name }) => ({ key, name })),
		[
			{ key: "siteD1", name: "superboard-site-db" },
			{ key: "siteSessionKv", name: "superboard-site-session" },
			{ key: "siteReleaseKv", name: "superboard-site-last-verified-release" },
			{ key: "moduleD1.support", name: "superboard-support-v2-db" },
		],
	);
	assert.equal(referenceApplicationResult.manifestProvisioned, false);
});
...
test("Site storage IDs block readiness independently and together", async () => {
	...
	for (const missingKeys of [...keys.map((key) => [key]), keys]) {
		...
		assert.deepEqual(
			result.resourceIds.missing.map(({ key }) => key),
			missingKeys,
		);
	}
});
...
function githubSdkFixture({ refs = {}, tags = {}, releases = [], packages = {} } = {}) {
	...
	return (args) => {
		...
		if (packageMatch) {
			...
			const descriptor = packages[`${packageType}/${decodeURIComponent(encodedName)}`];
		...
		if (endpoint.includes(refMarker)) {
			...
			return object ? { ok: true, data: { object } } : { ok: false, notFound: true };
		}
...
test("remote SDK readiness peels lightweight, annotated and SwiftPM tags", () => {
	...
	const remote = inspectSdkRemoteState(catalogue, "mabzadev/superboard-platform", run);
	...
	assert.deepEqual(inspectGitHubTag("mabzadev/superboard-platform", "sdk-flutter-v2.1.3", run), {
	...
	assert.ok(aliasMismatch.publications[1].blockers.includes("package-ref-sha-mismatch"));
});
...
test("package-backed SDK readiness verifies public ownership and exact versions", () => {
	...
	const catalogue = {
		libraries: [
			...
			},
			{
				id: "javascript",
				lifecycle: "active",
				packageName: "@mbzadev/superboard-js-sdk",
				sourceVersion: "1.0.2",
	...
	const fixture = {
		...
		packages: {
			"maven/io.superboard.superboard-android-sdk": {
				visibility: "public",
				repository: "mabzadev/superboard-platform",
				versions: ["1.0.2"],
			},
			"npm/superboard-js-sdk": {
				visibility: "public",
				repository: "mabzadev/superboard-platform",
				versions: ["1.0.1"],
			},
	...
	occupiedCandidate.packages["npm/superboard-js-sdk"].versions.push("1.0.2");
	...
	assert.ok(occupied.publications[1].blockers.includes("candidate-package-version-exists"));

	const wrongOwner = structuredClone(fixture);
	wrongOwner.packages["maven/io.superboard.superboard-android-sdk"].repository =
		"mabzadev/superboard";
	...
	assert.ok(mismatched.publications[0].blockers.includes("package-artifact-repository-mismatch"));
});
...
test("pending SDK readiness checks both the baseline and unused candidate", () => {
	...
	assert.ok(missingBaseline.publications[0].blockers.includes("baseline-release-tag-missing"));
});
...
test("remote readiness distinguishes a recorded failure from an unknown occupied tag", () => {
	...
	assert.ok(recorded.publications[0].blockers.includes("failed-immutable-version"));
	...
	assert.ok(unknown.publications[0].blockers.includes("candidate-package-ref-exists"));
});
...
test("reference readiness binds short links and source versions to the target", async () => {
	...
	const repositories = {
		platform: { nameWithOwner: "mabzadev/superboard" },
	};
	const project = {
		platformRepository: "https://github.com/mabzadev/superboard",
		referenceRepository: "https://github.com/mabzadev/superboard",
	...
	assert.equal(referenceReadiness(project, target, catalogue, repositories).ready, true);
	project.endpoints.shortLinks = "https://wrong.example";
	assert.equal(referenceReadiness(project, target, catalogue, repositories).ready, false);
});
		...
		parseGitState(
			{
				branch: "dev",
				head: "a".repeat(40),
				remote: "https://github.com/mabzadev/superboard-platform.git",
				status: "",
			},
			expected,
		).ready,
...
test("application client sources are generic, absolute and environment-addressable", () => {
	assert.deepEqual(
		parseClientSources(
			"reference-production=/work/reference-production;sample-app=/work/sample-app",
		),
		{
			"reference-production": "/work/reference-production",
			"sample-app": "/work/sample-app",
		},
	);
	assert.equal(clientSourceEnvironmentName("sample-app"), "SUPERBOARD_CLIENT_SOURCE_SAMPLE_APP");
	assert.throws(() => parseClientSources("reference-production=relative/path"), /absolute\/path/u);
	assert.throws(
		() => parseClientSources("reference-production=/one;reference-production=/two"),
		/unique/u,
	);
});
...
test("current offline report is fail-closed and contains actionable blockers", async () => {
	...
	assert.equal(
		report.blockers.some((blocker) => blocker.id === "mbza-development.resource_ids"),
		false,
	);
	assert.deepEqual(report.targets["mbza-development"].resourceIds.missing, []);
	assert.deepEqual(Object.keys(report.targets), ["mbza-development"]);
	assert.ok(report.blockers.some((blocker) => blocker.id === "github.not_inspected"));
	for (const id of ["development.google", "development.apple", "development.support_project"]) {
	...
	assert.equal(report.governance.canonicalRepository, "mabzadev/superboard");
	...
	assert.equal(report.flutterFlowLibrary.status, "ok");
	assert.deepEqual(report.flutterFlowLibrary.errors, []);
	...
	assert.equal(
		report.blockers.some((blocker) => blocker.id === "flutterflow.library_contract"),
		false,
	);
	assert.equal(
		report.blockers.some((blocker) => blocker.id === "flutterflow.application_configuration"),
		false,
	);
	assert.deepEqual(report.applicationClients, {});
	...
	assert.deepEqual(
		report.deploymentMatrix.entries.map(({ id, branch, target, deploymentAuthority }) => ({
			id,
			branch,
			target,
			deploymentAuthority,
	...
	assert.equal(JSON.stringify(report).includes("must-never-be-returned"), false);
});

test("strict CLI exits non-zero while operational prerequisites are incomplete", () => {
	const result = spawnSync(
		process.execPath,
		[resolve(root, "tests/checks/cloudflare/platform-readiness.mjs"), "--strict"],
