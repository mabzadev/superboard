...
} from "../../../scripts/cloudflare/secret-promote.mjs";
import { workerNameForService } from "../../../scripts/cloudflare/services.mjs";
...
test("managed Worker receipts resolve their target-declared Worker name", async () => {
	const { target, receipt } = await fixture(["managed-vocals-orchestrator-modal-api-key"]);
...
test("overlap-capable shared token promotion needs no maintenance override", async () => {
	...
	const services = [
		...
		names: bundlePlan.contracts
			.flatMap(({ members }) => members)
			.filter((member) => member.service === service)
			.flatMap((member) => [member.name, ...(member.previousName ? [member.previousName] : [])]),
		strategy: "inactive-version",
		versionTag: secretVersionTag(bundlePlan, service),
	}));
...
test("Flows dedicated API secret cutover promotes Flows before API", async () => {
	...
	assert.deepEqual(
		...
		[
			{
				service: "flows",
				names: ["INTERNAL_API_TOKEN", "INTERNAL_API_TOKEN_PREVIOUS"],
			},
