...
} from "../../../scripts/cloudflare/worker-shells.mjs";
...
const target = {
	...
	workers: Object.fromEntries(
		["observability", "email", "files", "identity", "api", "mcp", "site", "dashboard"].map(
			(service) => [service, { development: `sample-${service}-dev` }],
		),
	),
};
...
test("Worker shell planning is ordered, scoped and idempotent", () => {
	...
	assert.equal(
		plan.workers.some(({ service }) => service === "dashboard"),
		false,
	);
...
test("consolidated bootstrap creates the named interfaces before dependent Workers deploy", () => {
	const plan = buildWorkerShellPlan({
		target: { ...target, deploymentProfile: "consolidated" },
		environment: "development",
		accountId: "a".repeat(32),
		existingWorkerNames: [],
	});
	assert.equal(plan.workers.length, 6);
	assert.deepEqual(plan.workers.find(({ service }) => service === "api").entrypoints, [
		"SuperboardApi",
		"SuperboardMcp",
	]);
	assert.equal(
		plan.workers.some(({ service }) => service === "mcp"),
		false,
	);
});
