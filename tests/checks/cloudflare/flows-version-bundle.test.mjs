...
} from "../../../scripts/cloudflare/flows-version-bundle.mjs";
...
test("Flows version plan is MBZA-only, value-free and never includes Email in the bundle", async () => {
	...
	assert.deepEqual(plan.reusedSecrets, ["INTERNAL_API_TOKEN", "FLOW_USER_ENCRYPTION_KEY"]);
	...
	assert.deepEqual(plan.uploadCommand.slice(0, 4), ["npx", "wrangler", "versions", "upload"]);
	...
	assert.throws(
		() =>
			flowsObsoleteSecretBulkArgs({
				...
				obsoleteSecretsFile: resolve(root, ".flows-cutover", "secrets", "remove-obsolete.json"),
			}),
