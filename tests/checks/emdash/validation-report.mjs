...
function main() {
	...
	if (!evidencePath || !outputPath)
		throw new Error(
			"Usage: node tests/checks/emdash/validation-report.mjs --evidence <JSON array> --output <report.json>",
		);
	const baseline = JSON.parse(
		readFileSync(
			resolve(repositoryRoot, "scripts/config/superboard-plugin-independence-baseline.json"),
			"utf8",
		),
	);
	...
	const implementations = JSON.parse(
		readFileSync(
			resolve(repositoryRoot, "scripts/config/superboard-front-view-implementations.json"),
			"utf8",
		),
	);
	const adapters = JSON.parse(
		readFileSync(
			resolve(repositoryRoot, "scripts/config/superboard-plugin-api-adapters.json"),
			"utf8",
		),
	);
