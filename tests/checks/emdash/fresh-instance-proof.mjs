...
import { buildCloudflareBootstrapPlan } from "../../../scripts/cloudflare/bootstrap-core.mjs";
import {
...
const EVIDENCE_STEPS = Object.freeze([
	...
	},
	{
		id: "store_schema",
		command: process.execPath,
		args: ["--test", "tests/checks/database/module-cutover-registry-schema.test.mjs"],
		sources: ["tests/checks/database/module-cutover-registry-schema.test.mjs"],
	},
	{
		id: "store_runtime",
		command: "pnpm",
	...
	},
	{
		...
		command: process.execPath,
		args: [resolve(root, "tests/checks/database/migration-proof.mjs")],
		capture: true,
		sources: [
	...
	},
	{
		...
		args: [],
		capture: true,
		sources: ["scripts/cloudflare/target-orchestrator.mjs"],
	},
	{
	...
	const evidenceSteps = EVIDENCE_STEPS.map((step) => {
		...
		return worker
			? {
					...step,
					args: [
						resolve(root, "scripts/cloudflare/target-orchestrator.mjs"),
						"exercise",
						"--target",
						targetName,
						"--environment",
...
export function assertWorkerEvidence(adapter, evidence) {
	const checks = Array.isArray(evidence?.healthChecks) ? evidence.healthChecks : [];
	const workerChecks = checks.filter((check) => check?.service !== "site-admin");
	const adminChecks = checks.filter((check) => check?.service === "site-admin");
	if (
		!adapter ||
		!evidence ||
		evidence.environment !== adapter.environment ||
		...
		workerChecks.length !== adapter.service_count ||
		new Set(workerChecks.map((check) => check?.id)).size !== adapter.service_count ||
		workerChecks.some((check) => check?.id !== `worker:${check?.service}`) ||
		adminChecks.length !== 1 ||
		adminChecks[0]?.id !== "worker:site.admin" ||
		checks.some((check) => check?.status !== 200)
	) {
...
function receiptMatchesPlan(receipt, plan) {
	...
	return (
		...
		receipt.workers.local_health_receipt_count === plan.adapters[0].service_count + 1 &&
		receipt.workers.development_health_receipt_count === plan.adapters[1].service_count + 1 &&
...
function commandEvidencePrefix(args) {
	...
	if (command.includes("database/migration-proof.mjs")) {
