...
export function lintPlatformSeparation(root) {
	...
	function visit(directory) {
		for (const entry of readdirSync(join(root, directory), { withFileTypes: true })) {
			...
			const application =
				/^(?:apps\/vocostar\/|packages\/plugins\/[^/]*vocostar(?:[./])|scripts\/flutterflow-applications\/vocostar\/)/u.test(
					filename,
				) ||
				filename === "infra/targets/vocostar.json" ||
				/^packages\/plugins\/supbrd-[^/]+\/src\/front\/vocostar(?:[./])/u.test(filename);
			const container =
