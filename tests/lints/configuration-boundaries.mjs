...
const defaultContractPath = "scripts/config/configuration-boundaries.json";
...
const commonLogicalServiceNames = new Set(
	[...Object.keys(PLATFORM_SERVICE_SECRETS), ...Object.keys(DOMAIN_SERVICE_REGISTRY)].map(
		(service) => `superboard-${service}`,
	),
);
...
export async function auditConfigurationBoundaries({
	...
	const contractSchema = await readJson(
		resolve(workspaceRoot, "scripts/config/configuration-boundaries.schema.json"),
	);
	...
	const targets = await loadTargets(workspaceRoot, targetDirectory, targetSchema);
	...
	const referenceTarget = targets.find(({ target }) => target.target === referenceTargetName);
	...
	const secretNames = collectSecretBindingNames(targets.map(({ target }) => target));
	validateTargetInjectionBoundaries(targets, secretNames, errors);
	await validateWranglerSecretBoundaries(workspaceRoot, contract, secretNames, errors);
	...
	const sharedFiles = await sharedRuntimeFiles(workspaceRoot, contract, targets);
	const occurrences = await scanSharedRuntime(workspaceRoot, sharedFiles, ownership);
	...
	const report = {
		...
		classification: {
			...
			injected: {
				accountId: {
					defaultEnvironmentName: contract.injection.accountId.defaultEnvironmentName,
					scopedEnvironmentPrefix: contract.injection.accountId.scopedEnvironmentPrefix,
					storedInTargetManifest: false,
				},
				deploymentCredentialEnvironmentNames:
					contract.injection.deploymentCredentialEnvironmentNames,
				projectCredentialEnvironmentNames: contract.injection.projectCredentialEnvironmentNames,
...
export function findForbiddenSharedOccurrences({ path, source, ownership }) {
	const occurrences = [];
	const ownedValues = new Set(ownership.exactLiterals.map(({ value }) => value.toLowerCase()));
	for (const suffix of ownership.domainSuffixes) {
		const pattern = new RegExp(`(?:[a-z0-9-]+\\.)*${escapePattern(suffix)}\\b`, "giu");
		for (const match of source.matchAll(pattern)) {
			occurrences.push(occurrence(path, source, match.index, "target-domain", match[0]));
		}
	}
	for (const record of ownership.exactLiterals) {
		...
		while (offset !== -1) {
			...
			const after = source[offset + record.value.length] ?? "";
			if (/[A-Za-z0-9_.-]/u.test(before) || /[A-Za-z0-9_.-]/u.test(after)) {
				offset = source.indexOf(record.value, offset + record.value.length);
				continue;
			}
			occurrences.push(occurrence(path, source, offset, record.kind, record.value, record.target));
			offset = source.indexOf(record.value, offset + record.value.length);
		}
	}
	const workersDevHost = /(?:[a-z0-9-]+\.)+workers\.dev\b/giu;
	for (const match of source.matchAll(workersDevHost)) {
		occurrences.push(occurrence(path, source, match.index, "workers-dev-host", match[0]));
	}
	...
	for (const match of source.matchAll(quotedCloudflareId)) {
		...
		occurrences.push(
			occurrence(path, source, (match.index ?? 0) + 1, "unowned-cloudflare-id", value),
		);
	}
	const workstationPath = /(?:^|["'`\s])\/Users\/[A-Za-z0-9._-]+\//gmu;
	for (const match of source.matchAll(workstationPath)) {
		occurrences.push(occurrence(path, source, match.index, "workstation-path", match[0].trim()));
	}
	const accountAssignment = /(?:account_id|accountId)\s*[:=]\s*["'][a-f0-9]{32}["']/giu;
	for (const match of source.matchAll(accountAssignment)) {
		occurrences.push(occurrence(path, source, match.index, "cloudflare-account-id", match[0]));
	}
...
function validateReferenceTarget(reference, contract, errors) {
	...
	if (
		...
		environments.some(
			(environment) =>
				environment !== "local" && environment !== referenceContract.requiredEnvironment,
		)
	) {
		errors.push(
	...
	if (!suffix) {
		errors.push(`${reference.path} domains.api must define the reference domain`);
		return;
	}
	...
	if (target.authGateway?.issuer !== authOrigin) {
		errors.push(`${reference.path} authGateway.issuer must derive from domains.auth`);
	}
	if (target.authGateway?.jwksUrl !== `${authOrigin}/.well-known/jwks.json`) {
		errors.push(`${reference.path} authGateway.jwksUrl must derive from domains.auth`);
	}
	for (const address of [target.mail?.fromAddress, target.mail?.replyToAddress]) {
		if (address && !hostnameBelongsToSuffix(address.split("@").at(-1), suffix)) {
...
function validateTargetInjectionBoundaries(targets, secretNames, errors) {
	for (const { path, target } of targets) {
		...
		for (const [name, value] of Object.entries(target.customWorker?.vars ?? {})) {
...
async function validateWranglerSecretBoundaries(workspaceRoot, contract, secretNames, errors) {
	const files = (
		await Promise.all(
			["packages/plugins", "apps/reference/worker"].map((directory) =>
				recursiveFiles(resolve(workspaceRoot, directory), contract, workspaceRoot),
			),
		)
	).flat();
	for (const absolutePath of files.filter((path) =>
		/(?:^|\/)wrangler(?:\.test)?\.jsonc$/u.test(normalizePath(relative(workspaceRoot, path))),
	)) {
...
function buildTargetOwnership(targets, referenceTargetName) {
	...
	for (const { target } of targets) {
		...
		addExact(exact, `${target.workersDevSubdomain}.workers.dev`, "workers-dev-host", targetName);
		for (const [service, names] of Object.entries(target.workers ?? {})) {
			for (const workerName of Object.values(names ?? {})) {
				if (workerName !== `superboard-${service}`) {
		...
		for (const binding of target.customWorker?.serviceBindings ?? []) {
			for (const workerName of Object.values(binding.workers ?? {})) {
				const commonServiceName = Object.keys(target.workers ?? {}).some(
					(service) => workerName === `superboard-${service}`,
				);
...
async function sharedRuntimeFiles(workspaceRoot, contract, targets) {
	...
	for (const relativeRoot of contract.authorities.sharedPlatform.runtimeRoots) {
		...
		for (const path of await recursiveFiles(absoluteRoot, contract, workspaceRoot)) {
			const relativePath = normalizePath(relative(workspaceRoot, path));
			if (!contract.scan.portableExtensions.includes(extname(relativePath))) continue;
			if (isLocalOnly(relativePath, contract)) continue;
			if (applicationRoots.some((rootPath) => isPathInside(relativePath, rootPath))) {
				continue;
			}
			paths.push(path);
		}
...
async function scanSharedRuntime(workspaceRoot, files, ownership) {
	...
	for (const absolutePath of files) {
		...
		occurrences.push(...findForbiddenSharedOccurrences({ path, source, ownership }));
	}
	return occurrences;
}
...
async function loadTargets(workspaceRoot, targetDirectory, schema) {
	...
	return targets.sort((left, right) => left.target.target.localeCompare(right.target.target));
}
...
async function assertAuthorityPathsExist(workspaceRoot, contract, errors) {
	const paths = [
		...
		// Projection directories are created when an application snapshot is recorded.
		...contract.authorities.applications.derivedProjectionPaths.filter((path) => extname(path)),
...
async function recursiveFiles(directory, contract, workspaceRoot) {
	...
	for (const entry of entries) {
		...
		if (entry.isDirectory()) {
			...
			result.push(...(await recursiveFiles(absolutePath, contract, workspaceRoot)));
		} else if (
			entry.isFile() &&
			!contract.scan.ignoredFileSuffixes.some((suffix) => entry.name.endsWith(suffix))
		) {
...
export function isIgnoredDirectory(relativePath, name, contract) {
	return contract.scan.ignoredDirectoryNames.some(
		(ignored) =>
			name === ignored || relativePath === ignored || relativePath.startsWith(`${ignored}/`),
	);
}
...
function addExact(records, value, kind, target) {
	if (typeof value !== "string" || value.length < 8 || value === "superboard") return;
...
function environmentResourceValues(environment) {
	...
	function visit(value, keys = []) {
		if (typeof value === "string") {
			if (cloudflareUuidPattern.test(value) || cloudflareHexIdPattern.test(value)) {
				ids.add(value);
				return;
			}
			const key = keys.at(-1);
...
function deduplicateOccurrences(occurrences) {
	...
	for (const entry of occurrences) {
		result.set(`${entry.path}:${entry.line}:${entry.kind}:${entry.value}`, entry);
	}
...
async function main() {
	...
	if (json) {
		...
		console.log(`Configuration boundaries: ${report.valid ? "valid" : "invalid"}`);
		console.log(
