...
export function referenceCheckoutMetadata(controlPlane) {
	const repository = String(controlPlane?.repositories?.reference?.nameWithOwner || "").trim();
	const ref = String(controlPlane?.repositories?.reference?.defaultBranch || "").trim();
...
async function main() {
	const controlPlane = JSON.parse(
		await readFile(resolve(repositoryRoot, "scripts/config/github-control-plane.json"), "utf8"),
	);
