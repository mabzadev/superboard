...
export async function studioDeliveryStatistics(
	...
	for (const key of ["locale", "purpose", "resource_id", "template_id"] as const) {
...
studioDeliveryRoutes.get("/", async (c) => {
	...
	for (const key of ["locale", "purpose", "resource_id", "template_id", "status"] as const) {
