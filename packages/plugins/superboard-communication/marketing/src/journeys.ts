...
type JourneyNodeType = "email" | "channel" | "delay" | "branch" | "update_attribute" | "exit";
...
journeyRoutes.post("/journeys", async (c) => {
	...
	const entrySegmentId = optionalText(body.entry_segment_id, "entry_segment_id", 255);
	if (entrySegmentId) await ensureSegment(c.env.DB, project.projectId, entrySegmentId);
...
journeyRoutes.patch("/journeys/:journeyId", async (c) => {
	const project = c.get("project");
	const current = await getJourney(c.env.DB, project.projectId, c.req.param("journeyId"));
	if (current.status === "archived") {
		throw failure("journey_archived", "An archived journey cannot be edited", 409);
	}
	const body = await readJsonObject(c.req.raw);
	const trigger =
	...
	return c.json({
		data: serializeJourney(await getJourney(c.env.DB, project.projectId, current.id)),
	});
});
...
for (const [action, target] of [
	...
	journeyRoutes.post(`/journeys/:journeyId/${action}`, async (c) => {
		const projectId = c.get("project").projectId;
		const current = await getJourney(c.env.DB, projectId, c.req.param("journeyId"));
		assertJourneyTransition(current.status, target);
		if (target === "active") await validateJourneyDependencies(c.env.DB, current);
...
journeyRoutes.post("/journeys/:journeyId/enrollments", async (c) => {
	const projectId = c.get("project").projectId;
	const journey = await getJourney(c.env.DB, projectId, c.req.param("journeyId"));
	if (journey.status !== "active") {
		throw failure("journey_not_active", "Journey must be active before enrollment", 409);
	}
	const body = await readJsonObject(c.req.raw);
...
journeyRoutes.post("/signals", async (c) => {
	...
	if (project.role.toLowerCase() !== "system") {
		throw failure("signal_role_forbidden", "Only Analytics may publish Marketing signals", 403);
	}
	...
	const subscriber = signal.subject_hash
		...
		: null;
	if (!subscriber) {
		await completeSignal(c.env.DB, project.projectId, signal.event_id, 0, "unmatched");
	...
	for (const journey of journeys.results) {
		...
		if (!trigger.conditions.every((condition) => conditionMatches(signal, condition))) {
			continue;
		}
		if (
			journey.entry_segment_id &&
			!(await segmentContains(c.env.DB, project.projectId, journey.entry_segment_id, subscriber.id))
		) {
			continue;
		}
		const result = await enrollSubscriber(c.env, journey, subscriber.id, signal.event_id, {
			source: "analytics",
			signal,
		});
	...
	await c.env.DB.prepare(
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		.bind(
			...
			projectId,
			text(body.name, "name", 180),
			choice(body.channel, "channel", ["webhook", "sms", "push", "whatsapp", "slack"] as const),
			endpointUrl,
...
journeyRoutes.patch("/channel-connectors/:connectorId", async (c) => {
	const projectId = c.get("project").projectId;
	const current = await connector(c.env.DB, projectId, c.req.param("connectorId"));
	const body = await readJsonObject(c.req.raw);
	...
	await c.env.DB.prepare(
     ...
     WHERE project_id = ? AND id = ?`,
		...
		.bind(
			...
			body.enabled === undefined ? current.enabled : body.enabled === true ? 1 : 0,
			projectId,
			current.id,
		)
		.run();
	return c.json({
		data: serializeConnector(await connector(c.env.DB, projectId, String(current.id))),
	});
});
...
journeyRoutes.delete("/channel-connectors/:connectorId", async (c) => {
	...
	const result = await c.env.DB.prepare(
		...
		.first();
	if (!result) throw failure("connector_not_found", "Channel connector not found", 404);
	return c.json({ data: { deleted: true } });
});

export { journeyRoutes };

export async function dispatchDueJourneyEnrollments(env: Env, maximum = 100): Promise<number> {
...
export async function advanceJourneyEnrollment(
	...
	const enrollment = await env.DB.prepare(
     ...
     WHERE project_id = ? AND id = ?`,
		...
		.first<EnrollmentRow>();
	if (!enrollment || !["active", "waiting", "processing"].includes(enrollment.status)) return;
	if (Date.parse(enrollment.next_run_at) > Date.now()) return;
	const journey = await getJourney(env.DB, job.projectId, enrollment.journey_id);
	if (journey.status !== "active") {
	...
	const definition = validateDefinition(parseStoredJson(version.definition_json, {}));
	const node = definition.nodes.find((candidate) => candidate.id === enrollment.current_node_id);
	...
	if (!execution) throw new Error("Journey execution receipt could not be claimed");
...
async function executeNode(
	...
	const subscriber = await env.DB.prepare(
     ...
     FROM subscribers WHERE project_id = ? AND id = ?`,
		...
		.first<SubscriberRow>();
	if (!subscriber) {
		await completeEnrollment(env.DB, enrollment.id, "cancelled", "subscriber_missing");
		return;
	}
	const context = parseStoredJson<Record<string, unknown>>(enrollment.context_json, {});
	...
	if (node.type === "delay") {
		nextRunAt = new Date(Date.now() + Number(node.delay_seconds) * 1_000).toISOString();
       ...
       WHERE project_id = ? AND id = ?`,
			...
			.bind(JSON.stringify(node.attributes ?? {}), enrollment.project_id, subscriber.id)
			.run();
	...
	await env.DB.batch([
       ...
       updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ?`,
		...
		terminal
           ...
           last_error = NULL, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ?`,
				).bind(node.type === "delay" ? "waiting" : "active", next, nextRunAt, enrollment.id),
	]);
...
async function deliverJourneyEmail(
	...
	if (!templateId) throw new Error("Journey email template is missing");
	const published = await getPublishedEmailTemplate(env.DB, enrollment.project_id, templateId);
	const essential =
		published !== null && !["campaign", "lifecycle"].includes(published.document.purpose);
	const suppression = await env.DB.prepare(
		...
		.first<{ reason: string }>();
	const suppressed = essential
		? !["enabled", "unsubscribed"].includes(subscriber.status) ||
			Boolean(suppression && suppression.reason !== "unsubscribe")
	...
	if (suppressed) {
		await recordJourneyDelivery(env.DB, {
			...
			status: "suppressed",
			metadata: { reason: "subscriber_not_eligible", template_id: templateId },
		});
	...
	const template = await env.DB.prepare(
     ...
     WHERE project_id = ? AND id = ?`,
		...
		.bind(enrollment.project_id, templateId)
		.first<{
			subject: string | null;
	...
	const localized = await resolveStoredEmail(
		env.DB,
	...
	if (localized?.status === "skipped") {
		await recordJourneyDelivery(env.DB, {
			...
			recipient: subscriber.email,
			status: "suppressed",
			metadata: { reason: "translation_unavailable", template_id: templateId },
		});
		return { delivery_id: deliveryId, status: "suppressed" };
	}
	if (
		localized &&
		!(await claimEmailFrequency(env.DB, enrollment.project_id, deliveryId, subscriber.email))
	) {
		await recordJourneyDelivery(env.DB, {
			id: deliveryId,
			projectId: enrollment.project_id,
			...
			recipient: subscriber.email,
			status: "suppressed",
			metadata: { reason: "marketing_frequency_limit", template_id: templateId },
		});
		return { delivery_id: deliveryId, status: "suppressed" };
	}
		...
		message: await freezeDeliveryMessage(
			...
			async () => ({
				...
				subject:
					localized?.status === "ready"
						? localized.subject
						: personalize(template.subject ?? String(journey.name), replacements),
			...
			localized?.purpose ?? "lifecycle",
			localized?.status === "ready" ? localized.locale : null,
		),
	});
...
async function deliverJourneyChannel(
	...
	const destination = await connector(env.DB, enrollment.project_id, node.connector_id!);
	if (!Number(destination.enabled)) throw new Error("Journey channel connector is disabled");
	...
	if (encrypted?.secret) {
		headers.set("x-superboard-signature", `sha256=${await hmacHex(encrypted.secret, body)}`);
	}
	...
	if (!response.ok) throw new Error(`Channel connector returned HTTP ${response.status}`);
	const deliveryId = crypto.randomUUID();
	await recordJourneyDelivery(env.DB, {
		id: deliveryId,
		projectId: enrollment.project_id,
...
async function enrollSubscriber(
	...
	const definition = parseStoredJson<JourneyDefinition>(journey.definition_json, {
		start_node_id: "",
		nodes: [],
		edges: [],
	});
...
async function resumeCompletedExecution(env: Env, enrollment: EnrollmentRow, outputJson: string) {
	...
	const next = typeof output.next_node_id === "string" ? output.next_node_id : null;
	if (!next) {
...
function validateTrigger(value: unknown): JourneyTrigger {
	...
	if (!Array.isArray(rawConditions) || rawConditions.length > 25) {
		throw failure("trigger_conditions_invalid", "trigger conditions are limited to 25");
	}
	return {
...
function validateDefinition(value: unknown): JourneyDefinition {
	...
	if (!Array.isArray(source.nodes) || source.nodes.length < 1 || source.nodes.length > 100) {
		throw failure("journey_nodes_invalid", "A journey requires between 1 and 100 nodes");
	}
	...
	if (ids.size !== nodes.length) throw failure("journey_nodes_invalid", "Node ids must be unique");
	const startNodeId = text(source.start_node_id, "start_node_id", 64);
	if (!ids.has(startNodeId)) throw failure("journey_start_invalid", "start_node_id is unknown");
	const edges = source.edges.map((raw) => validateEdge(raw, ids));
	for (const node of nodes) {
		const outgoing = edges.filter((edge) => edge.from === node.id);
		if (node.type === "exit" && outgoing.length) {
			throw failure("journey_graph_invalid", "Exit nodes cannot have outgoing edges");
		}
		if (node.type === "branch") {
			if (
				...
				throw failure("journey_graph_invalid", "Branch nodes require true and false edges");
			}
		} else if (node.type !== "exit" && outgoing.length > 1) {
			throw failure("journey_graph_invalid", "Non-branch nodes allow at most one edge");
		}
	}
...
function validateNode(value: unknown): JourneyNode {
	...
	if (type === "email") {
		return {
			...
			smtp_profile_id: optionalText(source.smtp_profile_id, "smtp_profile_id", 255) ?? undefined,
		};
	}
	if (type === "channel") {
		return {
	...
	if (type === "delay") {
		...
		if (delaySeconds > 31_536_000) {
			throw failure("delay_seconds_invalid", "A journey delay is limited to one year");
		}
		return { id, type, delay_seconds: delaySeconds };
	}
	if (type === "branch") return { id, type, condition: validateCondition(source.condition) };
...
function validateEdge(value: unknown, ids: Set<string>): JourneyEdge {
	...
	if (!ids.has(from) || !ids.has(to)) {
		throw failure("journey_edge_invalid", "Journey edge references an unknown node");
	}
	return {
		from,
		to,
...
function assertAcyclic(start: string, edges: JourneyEdge[]) {
	...
	const walk = (node: string) => {
		...
		visiting.add(node);
		for (const edge of edges.filter((candidate) => candidate.from === node)) walk(edge.to);
...
function edgeDestination(
	...
	return (
		definition.edges.find((edge) => edge.from === nodeId && edge.outcome === outcome)?.to ??
		definition.edges.find((edge) => edge.from === nodeId && edge.outcome === "default")?.to ??
		null
	);
}

async function getJourney(db: D1Database, projectId: number, journeyId: string) {
	const row = await db
...
async function connector(db: D1Database, projectId: number, connectorId: string) {
	const row = await db
		.prepare(`SELECT * FROM marketing_channel_connectors WHERE project_id = ? AND id = ?`)
		.bind(projectId, connectorId)
		.first<Record<string, unknown>>();
	if (!row) throw failure("connector_not_found", "Channel connector not found", 404);
	return row;
}

async function ensureSegment(db: D1Database, projectId: number, segmentId: string) {
	const row = await db
		.prepare(`SELECT 1 FROM subscriber_segments WHERE project_id = ? AND id = ?`)
...
async function validateJourneyDependencies(db: D1Database, journey: JourneyRow) {
	const definition = validateDefinition(parseStoredJson(journey.definition_json, {}));
	for (const node of definition.nodes) {
		if (node.type === "email") {
			...
			if (template.studio_id && !template.published_id)
				throw failure(
					"EMAIL_TEMPLATE_NOT_PUBLISHED",
					"Publish the message template before activating this journey.",
					409,
				);
		}
...
function connectorUrl(value: unknown, environment: string) {
	...
	try {
		...
		throw failure("endpoint_url_invalid", "endpoint_url must be an absolute URL");
	}
	const localTest =
		environment !== "production" && ["localhost", "127.0.0.1"].includes(url.hostname);
	...
	if (url.username || url.password) {
		throw failure("endpoint_url_invalid", "endpoint_url must not contain credentials");
	}
	return url.toString();
}
