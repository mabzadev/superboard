...
export { default as MarketingSettingsPage } from "../../studio/CommunicationSettingsPage.js";
...
const textareaClass = "min-h-28 w-full rounded-md border bg-background p-3 text-sm";
...
function EmailAudiencePage() {
	...
	const saveSegment = async () => {
		...
		const saved = await run(
			() =>
				segmentEditingId
					? updateSubscriberSegment(selectedProject.id, segmentEditingId, payload)
	...
	const editSegment = (item: SubscriberSegment) => {
		const condition = Array.isArray((item.rules as { conditions?: unknown[] }).conditions)
			? (item.rules as { conditions: Array<Record<string, unknown>> }).conditions[0]
			: undefined;
		...
		const saved = await run(async () => {
			...
			await Promise.all([
				...selectedLists
					...
					.map((id) => addSubscriberToList(selectedProject.id, id, editingSubscriber.id)),
				...(editingSubscriber.list_ids || [])
					.filter((id) => !next.has(id))
					.map((id) => removeSubscriberFromList(selectedProject.id, id, editingSubscriber.id)),
			]);
		}, "Subscriber updated");
		if (saved) {
			...
			{!selectedProject ? (
				...
				<div className="space-y-6">
					<div className="grid gap-6 xl:grid-cols-3">
						<FormCard title={editingSubscriber ? "Edit subscriber" : "Add subscriber"}>
							<Input
								type="email"
								placeholder="Email"
								value={email}
							...
							<div className="max-h-28 space-y-2 overflow-auto rounded-md border p-3">
								{lists.length === 0 ? (
									<p className="text-xs text-muted-foreground">No lists configured.</p>
								) : (
									lists.map((item) => (
							...
							{editingSubscriber ? (
								<textarea
									...
									onChange={(event) => setSubscriberAttributes(event.target.value)}
								/>
							) : (
							...
							<div className="flex gap-2">
								...
								<Button
									disabled={busy || !email}
									onClick={() => void (editingSubscriber ? saveSubscriber() : addSubscriber())}
								>
									{editingSubscriber ? (
										<SaveLabel />
									) : (
						...
						<FormCard title={segmentEditingId ? "Edit segment" : "Create segment"}>
							<Input
							...
							>
								{["equals", "not_equals", "contains", "starts_with", "exists", "in"].map(
									(value) => (
							...
							<div className="flex gap-2">
								...
								<Button disabled={!segmentName || !segmentField} onClick={() => void saveSegment()}>
									{segmentEditingId ? (
										<SaveLabel />
									) : (
										<>
					...
					<Card>
						<CardHeader className="flex-row items-center gap-3">
							<div className="flex-1">
								...
								<CardDescription>{subscribers.length} displayed</CardDescription>
							</div>
							<Input
								className="max-w-xs"
									...
									>
										<div>
											...
											<p className="text-xs text-muted-foreground">
												{item.name || "No name"} · consent {item.consent_status || "unknown"} ·{" "}
										...
										<div className="flex items-center gap-1">
												...
												<>
													...
													<Button
														...
														onClick={() =>
															void run(
																() =>
																	suppressEmailSubscriber(selectedProject.id, item.id, "blocklist"),
											...
											<Button
												...
												onClick={() =>
													void run(
														() => deleteEmailSubscriber(selectedProject.id, item.id),
														"Subscriber deleted",
													)
												}
											>
					...
					<div className="grid gap-6 xl:grid-cols-3">
						<CollectionCard
							...
							onDelete={(id) =>
								run(() => deleteSubscriberList(selectedProject.id, id), "List deleted")
							}
						/>
						<Card>
							<CardHeader>
								<CardTitle>Segments</CardTitle>
								<CardDescription>Calculated audience membership</CardDescription>
							</CardHeader>
							<CardContent className="space-y-2">
								{segments.length === 0 ? (
										...
										>
											...
											<div className="flex">
												...
												<Button
													...
													onClick={() =>
														void run(
															() => refreshSubscriberSegment(selectedProject.id, item.id),
															"Segment refreshed",
														)
													}
												>
												...
												<Button
													...
													onClick={() =>
														void run(
															() => deleteSubscriberSegment(selectedProject.id, item.id),
															"Segment deleted",
														)
													}
												>
						...
						<FormCard title="Bulk import">
							...
							/>
							<Button disabled={!importText.trim()} onClick={() => void importBatch()}>
								<Upload />
								Import up to 1,000
							</Button>
						</FormCard>
...
function CampaignManagerPage() {
	...
	const [campaignSchedules, setCampaignSchedules] = useState<Record<string, string>>({});
	...
	const load = useCallback(async () => {
		if (!selectedProject) return;
		try {
			const [campaignRows, templateRows, mediaRows, listRows, segmentRows, smtpResult] =
				await Promise.all([
	...
	const templatePayload = () => ({
		...
		content_markdown: templateForm.format === "markdown" ? templateForm.content : null,
	...
	const addTemplate = async () => {
		...
		const saved = await run(
			() =>
				templateEditingId
					? updateEmailTemplate(selectedProject.id, templateEditingId, templatePayload())
	...
	const editTemplate = (item: EmailTemplate) => {
		const format = item.content_html ? "html" : item.content_markdown ? "markdown" : "text";
		...
		setTemplateForm({
			...
			format,
			content: item.content_html || item.content_markdown || item.content_text || "",
		});
	};
	const campaignPayload = () => {
		...
		return {
			...
			content_text:
				template?.content_text || template?.content_markdown || current?.content_text || null,
	...
	const addCampaign = async () => {
		...
		const saved = await run(
			() =>
				campaignEditingId
					? updateEmailCampaign(selectedProject.id, campaignEditingId, campaignPayload())
	...
	const upload = async (file?: File) => {
		...
		await run(() => uploadMarketingMedia(selectedProject.id, file), "Media uploaded");
			...
			{!selectedProject ? (
				...
				<div className="space-y-6">
					<div className="grid gap-6 xl:grid-cols-2">
						<FormCard title={templateEditingId ? "Edit email template" : "Email template"}>
							<Input
						...
						<FormCard title={campaignEditingId ? "Edit campaign" : "Campaign draft"}>
							<Input
									...
									<div key={item.id} className="rounded-md border p-4">
										<div className="flex flex-wrap items-center justify-between gap-3">
											<div>
												...
												<p className="text-xs text-muted-foreground">
													{item.recipient_count || 0} recipients · {item.delivered_count || 0}{" "}
													delivered · tracking {item.tracking_enabled ? "on" : "off"}
												</p>
											</div>
											<div className="flex flex-wrap items-center gap-1">
												...
												{["draft", "scheduled"].includes(item.status) && (
													<Button size="sm" onClick={() => void action(item, "start")}>
														<Play />
														Start
													</Button>
												)}
												...
												{item.status === "paused" && (
													<Button size="sm" onClick={() => void action(item, "resume")}>
														<Play />
														Resume
													</Button>
												)}
												{["draft", "scheduled", "running", "paused"].includes(item.status) && (
													<Button
														size="sm"
														variant="outline"
										...
										<div className="mt-3 grid gap-2 md:grid-cols-[1fr_auto_1fr_auto]">
											...
											<Button
												variant="outline"
												disabled={
													!campaignSchedules[item.id] || !["draft", "paused"].includes(item.status)
												}
					...
					<div className="grid gap-6 xl:grid-cols-3">
						<CollectionCard
							...
							onDelete={(id) =>
								run(() => deleteEmailTemplate(selectedProject.id, id), "Template deleted")
							}
						/>
						<Card>
							<CardHeader className="flex-row items-center justify-between">
								<div>
									...
									<CardDescription>Images, audio, video and PDF</CardDescription>
								</div>
								<>
									<input
										ref={fileRef}
									...
									media.map((item) => (
										...
										>
											<div>
												...
												<p className="text-xs text-muted-foreground">
													{item.content_type} · {Math.ceil(item.byte_size / 1024)} KB
												</p>
											</div>
											<div className="flex">
												<Button
												...
												<Button
													...
													onClick={() =>
														void run(
															() => deleteMarketingMedia(selectedProject.id, item.id),
															"Media deleted",
														)
													}
												>
				...
				<div className="space-y-6">
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								{statistics?.series.length || 0} {t("measured event buckets")}
							</CardDescription>
						</CardHeader>
						<CardContent className="max-h-96 overflow-auto">
							{statistics?.series.length ? (
								<FrontTable className="w-full text-sm">
									<FrontTableHeader>
										<FrontTableRow className="border-b text-start">
											<FrontTableHead className="p-2">{t("Period")}</FrontTableHead>
											<FrontTableHead className="p-2">{t("Event")}</FrontTableHead>
											<FrontTableHead className="p-2 text-end">{t("Total")}</FrontTableHead>
										</FrontTableRow>
									</FrontTableHeader>
									<FrontTableBody>
										{statistics.series.map((row, index) => (
											<FrontTableRow
												...
												className="border-b"
											>
												<FrontTableCell className="p-2 font-mono text-xs">
													{typeof row.bucket === "string" || typeof row.bucket === "number"
														? row.bucket
														: "—"}
												</FrontTableCell>
												<FrontTableCell className="p-2 capitalize">
													{String(row.event_type || "unknown").replaceAll("_", " ")}
												</FrontTableCell>
												<FrontTableCell className="p-2 text-end font-medium">
...
export function MarketingDeliveryPanel({
	section = "operations",
}: {
	section?: "delivery" | "webhooks" | "operations";
}) {
	const { t } = useStudioI18n();
	const [enabledId] = useState(() => crypto.randomUUID());
	const { selectedProject } = useProjectSelection();
	...
	const load = useCallback(() => {
		if (!selectedProject) return;
		return Promise.all([
			section === "delivery" || section === "operations"
				? getSmtpSettings(selectedProject.id)
				: Promise.resolve<SmtpSettings>({ configured: false, profiles: [] }),
			section === "webhooks" ? getProviderWebhooks(selectedProject.id) : Promise.resolve([]),
			section === "operations" ? getDeliveryOutbox(selectedProject.id) : Promise.resolve([]),
			section === "operations" ? getMarketingDeadLetters(selectedProject.id) : Promise.resolve([]),
			section === "operations" ? getMarketingAudit(selectedProject.id) : Promise.resolve([]),
		])
			.then(([smtpResult, hookRows, outboxRows, deadLetterRows, auditRows]) => {
	...
	const reset = () => {
		setSmtp({
			id: crypto.randomUUID(),
			name: t("Sender {number}", { number: profiles.length + 1 }),
			configured: false,
			port: 587,
			security: "starttls",
	...
	return (
		<section className="space-y-4">
			<h2 className="text-lg font-semibold">
				{section === "operations"
					? t("Marketing delivery operations")
					: section === "webhooks"
						? t("Provider webhook")
						: t("Sender identities")}
			</h2>
			{error && <p role="alert">{error}</p>}
			{section === "operations" && <p>{t("{count} audited mutations", { count: auditCount })}</p>}
						...
						<div className="grid gap-6 xl:grid-cols-[420px_1fr]">
									...
									<div className="rounded-xl border border-primary/20 bg-primary/5 p-4">
										<div className="flex items-center gap-3">
											...
											<div>
												...
												<p className="text-xs text-muted-foreground">
													{t("{region} · STARTTLS · port 587", {
														region: awsRegion || t("AWS region"),
													})}
												</p>
											</div>
										</div>
								...
								{deliveryProvider === "smtp" ? (
									<>
										...
										<Input
											...
											placeholder={
												smtp.configured
													? t("Leave blank to keep encrypted password")
													: t("Password")
											}
											value={password}
							...
							<Card>
								<CardHeader>
									...
									<CardDescription>
										{t("{count} configured", { count: profiles.length })}
									</CardDescription>
								</CardHeader>
								<CardContent className="space-y-3">
									{profiles.length === 0 ? (
												...
												<div className="flex items-center justify-between">
													...
													>
														...
														<p className="text-xs text-muted-foreground">
															...
															{t("· priority {priority} · {status}", {
																...
																status: item.last_test_status || t("untested"),
															})}
														</p>
														<p className="text-xs text-muted-foreground">
															{t(
																"Sender DNS: {authentication} · SPF {spf} · DKIM {dkim} · DMARC {dmarc}",
																{
																	authentication: item.authentication_status || t("unverified"),
																	spf: item.spf_status || t("unchecked"),
																	dkim: item.dkim_status || t("unchecked"),
																	dmarc: item.dmarc_status || t("unchecked"),
																},
															)}
														</p>
													</button>
													<div className="flex">
														<Button
															variant="ghost"
															size="icon"
															aria-label={t("Verify sender DNS {name}", { name: item.name || "" })}
					...
					<div className="grid gap-6 xl:grid-cols-3">
								...
								{webhooks.map((item) => (
									...
									>
										...
										<Button
											size="icon"
											variant="ghost"
											aria-label={t("Delete {name}", { name: item.provider })}
											onClick={() =>
												void run(
						...
						{section === "operations" && (
							<>
								<Card>
									<CardHeader>
										<CardTitle>{t("Delivery tests")}</CardTitle>
									</CardHeader>
									<CardContent className="space-y-3">
										<Input
											type="email"
											placeholder={t("Test recipient")}
											value={testRecipient}
										...
										{profiles.map((item) => (
											<div key={item.id} className="flex items-center justify-between">
												<span>{item.name}</span>
												<Button
													disabled={!testRecipient.trim() || item.enabled === false}
													variant="ghost"
													size="icon"
													aria-label={t("Test {name}", { name: item.name || "" })}
													onClick={() =>
														void run(
															() =>
																testSmtpSettings(selectedProject.id, item.id, testRecipient.trim()),
														...
														{item.status === "quarantined" && (
															<div className="flex shrink-0">
																<Button
																	size="icon"
																	variant="ghost"
																	aria-label={t("Replay dead letter {name}", { name: item.id })}
																	...
																	onClick={() =>
																		void run(
																			() => replayMarketingDeadLetter(selectedProject.id, item.id),
																...
																<Button
																	size="icon"
																	variant="ghost"
																	aria-label={t("Discard dead letter {name}", { name: item.id })}
																	onClick={() =>
																		void run(
																			() => discardMarketingDeadLetter(selectedProject.id, item.id),
...
function CollectionCard({
	...
	return (
		<Card>
			<CardHeader>
				<CardTitle>{title}</CardTitle>
				<CardDescription>{t("{count} configured", { count: items.length })}</CardDescription>
			</CardHeader>
			<CardContent className="space-y-2">
				{items.length === 0 ? (
...
function formatMetric(key: string, value: number) {
	return key.endsWith("_rate") ? `${value.toLocaleString()}%` : value.toLocaleString();
}
