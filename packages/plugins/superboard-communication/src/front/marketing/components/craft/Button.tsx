...
import type { ButtonCraftProps } from "@superboard/front-ui/types";
				...
				>
					<button
						style={{
							...
							color: textColor,
							border: styleType === "outlined" ? `${borderSize} solid ${background}` : "none",
							borderRadius,
							boxSizing: "border-box",
							paddingTop,
							paddingBottom,
					...
					>
						<p
							...
							style={{
								color: textColor,
								fontSize:
									textSize === "text-sm" ? "14px" : textSize === "text-lg" ? "18px" : "16px",
		...
		<div className="flex flex-col gap-3 px-3 py-2">
				...
				<div key={item.prop} className="flex items-center justify-between">
					...
					<div className="flex items-center gap-1.5">
						<Input
							...
							onChange={(e) =>
								setProp((p: Record<string, string>) => (p[item.prop] = e.target.value))
							}
						...
						<div className="w-7 h-7 rounded-md border border-input overflow-hidden relative cursor-pointer shrink-0">
							...
							/>
							<div className="w-full h-full" style={{ backgroundColor: props[item.prop] }} />
						</div>
					</div>
				</div>
			))}
			...
			<div className="flex flex-col gap-1.5">
				...
				<Input
					...
					value={props.href}
					placeholder="https://"
					onChange={(e) => setProp((p: Record<string, string>) => (p.href = e.target.value))}
				/>
			</div>
			...
			<div className="flex flex-col gap-1.5">
				...
				<Select
					...
					onValueChange={(v) => setProp((p: Record<string, string>) => (p.styleType = v))}
				>
...
ButtonComponent.craft = {
	...
	props: {
		href: "",
		text: "Click me",
		background: `var(--brand, #3ecf8e)`,
		textColor: `var(--primary-foreground, #002715)`,
