...
import type { ContainerCraftProps } from "@superboard/front-ui/types";
	...
	return (
		<div
			...
			className={clsx(
				...
				hasNoChildren && "border border-dashed border-border",
			)}
			style={{
		...
		<div className="flex flex-col gap-3 px-3 py-2">
			<div className="grid grid-cols-2 gap-3">
				<div className="flex flex-col gap-1.5">
					...
					<Input
						...
						value={props.width}
						placeholder="100%"
						onChange={(e) => setProp((p: Record<string, string>) => (p.width = e.target.value))}
					/>
				</div>
				<div className="flex flex-col gap-1.5">
					...
					<Input
						...
						value={props.height}
						placeholder="auto"
						onChange={(e) => setProp((p: Record<string, string>) => (p.height = e.target.value))}
					/>
				</div>
			</div>
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-1.5">
					<Input
						...
						onChange={(e) =>
							setProp((p: Record<string, string>) => (p.background = e.target.value))
						}
					...
					<div className="w-7 h-7 rounded-md border border-input overflow-hidden relative cursor-pointer shrink-0">
						...
						/>
						<div className="w-full h-full" style={{ backgroundColor: props.background }} />
					</div>
				</div>
			</div>
			...
			<div className="flex flex-col gap-1.5">
				...
				<Input
					...
					placeholder="0px"
					onChange={(e) =>
						setProp((p: Record<string, string>) => (p.borderRadius = e.target.value))
					}
				/>
			</div>
				...
				<RadioGroup className="flex gap-4">
					{[
						...
						<div
							...
							onClick={() =>
								setProp((p: Record<string, string>) => (p.flexDirectionClass = item.value))
							}
						>
							<RadioGroupItem
								value={item.value}
			...
			<div className="flex flex-col gap-1.5">
				...
				<Input
					...
					placeholder="10px"
					value={props.gap}
					onChange={(e) => setProp((p: Record<string, string>) => (p.gap = e.target.value))}
				/>
			</div>
		</div>
	);
