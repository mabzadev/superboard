		...
		<div className="flex flex-col gap-6">
			...
			<div className="flex flex-col gap-2">
				<div className="flex items-center justify-between">
					<label className="text-sm font-medium">{t("Targeting")}</label>
					<Badge
						variant="outline"
				...
				<div className="grid grid-cols-2 gap-3">
					...
					<Popover open={audienceOpen} onOpenChange={setAudienceOpen}>
						<PopoverTrigger asChild disabled={readOnly}>
							<button
								className={cn(
									"flex items-center gap-3 w-full rounded-lg border border-sidebar-border bg-secondary px-3 py-2.5 text-start transition-all",
							...
							>
								...
								<div className="flex flex-col min-w-0">
									...
									<span className="text-sm font-medium truncate">{t(selectedAudience.label)}</span>
								</div>
								<Separator orientation="vertical" className="ms-auto h-6" />
							...
							{audienceOptions.map((item) => {
								...
								return (
									<button
										...
										className={cn(
											"flex items-center gap-3 w-full rounded-md px-2.5 py-2 text-start transition-colors",
											deliverTo === item.value ? "bg-accent" : "hover:bg-accent/50",
										)}
									>
					...
					<Popover open={platformsOpen} onOpenChange={setPlatformsOpen}>
						<PopoverTrigger asChild disabled={readOnly}>
							<button
								className={cn(
									"flex items-center gap-3 w-full rounded-lg border bg-secondary px-3 py-2.5 text-start transition-all",
									"hover:bg-muted focus-visible:border-primary/40 focus-visible:ring-[3px] focus-visible:ring-primary/10 focus-visible:outline-none",
									selectedValues.length > 0
							...
							>
								...
								<div className="flex flex-col min-w-0">
									...
									>
										{platformLabels ?? t("Select platforms")}
									</span>
								</div>
								<Separator orientation="vertical" className="ms-auto h-6" />
								<ChevronDown className="h-4 w-4 shrink-0 text-muted-foreground" />
						...
						<PopoverContent align="start" className="w-[var(--radix-popover-trigger-width)] p-1">
							{options.map((option) => (
								<button
									...
									className={cn(
										"flex items-center gap-3 w-full rounded-md px-2.5 py-2 text-start transition-colors",
										selectedValues.includes(option.value) ? "bg-accent" : "hover:bg-accent/50",
									)}
								>
									<div className="flex items-center justify-center w-7 h-7 rounded-md bg-secondary shrink-0">
										{option.value === "web" ? (
				...
				{showErrors && selectedValues.length === 0 && (
					<Badge variant="destructive" className="w-fit gap-1.5 py-1 px-2.5">
						...
						{t("At least one platform is required")}
					</Badge>
				)}
				<span className="text-xs text-muted-foreground">
					{t("Choose who receives this message and on which platforms.")}
				</span>
			</div>
			...
			<div className="flex flex-col gap-2">
						...
						<Popover open={pushOpen} onOpenChange={setPushOpen}>
							<PopoverTrigger asChild disabled={readOnly}>
								...
								>
									...
									<div className="flex flex-col min-w-0">
										<span className="text-xs text-muted-foreground">{t("Push Notifications")}</span>
										<span className="text-sm font-medium truncate">
											{t(deliverPushNotification ? "Enabled" : "Disabled")}
										</span>
									</div>
									<Separator orientation="vertical" className="ms-auto h-6" />
									<ChevronDown className="h-4 w-4 shrink-0 text-muted-foreground" />
							...
							<PopoverContent align="start" className="w-[var(--radix-popover-trigger-width)] p-1">
								...
								>
									...
									<div className="flex flex-col min-w-0">
										...
										<span className="text-xs text-muted-foreground">
											{t("Users receive an immediate push notification alert")}
										</span>
									</div>
									{deliverPushNotification && (
										<Check className="ms-auto h-4 w-4 shrink-0 text-primary" />
								...
								>
									...
									<div className="flex flex-col min-w-0">
										...
										<span className="text-xs text-muted-foreground">
											{t("Message appears silently in the inbox")}
										</span>
									</div>
									{!deliverPushNotification && (
										<Check className="ms-auto h-4 w-4 shrink-0 text-primary" />
					...
					<Popover open={autoDisplayOpen} onOpenChange={setAutoDisplayOpen}>
						<PopoverTrigger asChild disabled={readOnly}>
							...
							>
								...
								<div className="flex flex-col min-w-0">
									<span className="text-xs text-muted-foreground">{t("Auto Display")}</span>
									<span className="text-sm font-medium truncate">
										{t(autoDisplay ? "Enabled" : "Disabled")}
									</span>
								</div>
								<Separator orientation="vertical" className="ms-auto h-6" />
								<ChevronDown className="h-4 w-4 shrink-0 text-muted-foreground" />
						...
						<PopoverContent align="start" className="w-[var(--radix-popover-trigger-width)] p-1">
							...
							>
								...
								<div className="flex flex-col min-w-0">
									<span className="text-sm">{t("Enabled")}</span>
									<span className="text-xs text-muted-foreground">
										{t("Shows as a full-screen overlay when the app is open")}
									</span>
								</div>
								{autoDisplay && <Check className="ms-auto h-4 w-4 shrink-0 text-primary" />}
							</button>
							<button
								onClick={() => {
							...
							>
								...
								<div className="flex flex-col min-w-0">
									<span className="text-sm">{t("Disabled")}</span>
									<span className="text-xs text-muted-foreground">
										{t("Message is only visible in the inbox")}
									</span>
								</div>
								{!autoDisplay && <Check className="ms-auto h-4 w-4 shrink-0 text-primary" />}
							</button>
				...
				<span className="text-xs text-muted-foreground">
					{t("Control how and when the message is presented to users.")}
				</span>
			</div>
		</div>
	);
