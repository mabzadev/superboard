import { EXISTING_USERS_FILTER } from "@superboard/front-ui/constants/OptionsConstants.js";
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent showCloseButton={false} className="sm:max-w-[480px]">
				<DialogHeader>
					<DialogTitle>{t("Publish Message")}</DialogTitle>
					<DialogDescription>
						{t("Please review the following before publishing.")}
					</DialogDescription>
				</DialogHeader>
				...
				<div className="flex flex-col gap-3 py-2">
					<div className="flex items-start gap-3 rounded-lg border border-sidebar-border bg-secondary p-3">
						...
						<div className="flex flex-col gap-0.5">
							<span className="text-sm font-medium">
								{t(deliverTo === EXISTING_USERS_FILTER ? "Existing Users" : "New Users")}
							</span>
					...
					<div className="flex items-start gap-3 rounded-lg border border-sidebar-border bg-secondary p-3">
						...
						<div className="flex flex-col gap-0.5">
							...
							<span className="text-xs text-muted-foreground">
								{t("Will be delivered on {platforms}.", { platforms: platformLabels })}
							</span>
						</div>
					</div>
					...
					{deliverPushNotification && deliverTo === EXISTING_USERS_FILTER && (
						<div className="flex items-start gap-3 rounded-lg border border-amber-300/50 bg-amber-50 dark:bg-amber-950/20 p-3">
							...
							<div className="flex flex-col gap-0.5">
								<span className="text-sm font-medium">{t("Push Notification")}</span>
								<span className="text-xs text-muted-foreground">
									{t(
										"Users will receive a push alert with title {title} and subtitle {subtitle}.",
										{ title, subtitle },
									)}
								</span>
							</div>
					...
					{autoDisplay && (
						<div className="flex items-start gap-3 rounded-lg border border-amber-300/50 bg-amber-50 dark:bg-amber-950/20 p-3">
							...
							<div className="flex flex-col gap-0.5">
								<span className="text-sm font-medium">{t("Auto Display")}</span>
								<span className="text-xs text-muted-foreground">
									{t(
										"This message will automatically display as a full-screen overlay when users open the app.",
									)}
								</span>
							</div>
						</div>
