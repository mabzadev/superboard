...
import ActionConfirm from "@superboard/front-ui/common/action-confirm.js";
import {
	ANDROID,
...
import { ApiError } from "@superboard/front-ui/lib/ApiError.js";
import LocalStorage from "@superboard/front-ui/lib/LocalStorage.js";
import {
	showErrorNotification,
	showSuccessNotification,
} from "@superboard/front-ui/lib/Notifications.js";
import { deepEqual } from "@superboard/front-ui/lib/utils.js";
import type { Notification, CreateNotificationApiPayload } from "@superboard/front-ui/types";
...
import { Separator } from "@superboard/front-ui/ui/separator.js";
...
} from "../../hooks/mutations/useNotificationsMutations.js";
import { messageSchema, type MessageFormValues } from "../../schemas/message.js";
...
import { initialEditorState } from "../craft/Editor.js";
import { ExportHtmlButton } from "../craft/ExportButton.js";
import { ImageComponent } from "../craft/Image.js";
import { PreviewOnlyButton } from "../craft/PreviewOnlyButton.js";
import { RootContainer } from "../craft/RootContainer.js";
import { TextComponent } from "../craft/Text.js";
import CreateLinkSidebar from "../dynamic_links/links/create_link/CreateLinkSidebar.js";
import CreateMessageContent from "./CreateMessageContent.js";
import CreateMessageDelivery from "./CreateMessageDelivery.js";
import CreateMessageOverview from "./CreateMessageOverview.js";
import MessagePreviewDialog from "./MessagePreviewDialog.js";
...
const MessageDialog = ({
	...
	const createNotificationMutation = useCreateNotificationMutation(selectedProject?.id);
	const archiveNotificationMutation = useArchiveNotificationMutation(selectedProject?.id);
...
<html lang="en" class="">
  ...
  <script id="craft-state" type="application/json">
	...
	const sections = useMemo(
		...
		[detailsComplete, contentComplete, t],
	);

	const viewSections = useMemo(
		() => [
			{ text: t("Details"), value: "details", checked: FULL_CHECK },
			{ text: t("Content"), value: "content", checked: FULL_CHECK },
		],
		[t],
	);
	...
	const handleCreateNotification = async () => {
		...
		try {
			...
			showErrorNotification(
				error instanceof ApiError ? error.message : t("Something went wrong, please try again"),
			);
		}
	};
	...
	const handleArchiveMessage = async () => {
		...
		try {
			...
			showSuccessNotification(t("Message archived"));
			getMessages();
			onOpenChange(false);
		} catch (error) {
			if (
				...
				showErrorNotification((error.data as Record<string, unknown>).error as string);
	...
	useEffect(() => {
		...
		if (open && mode === "view" && selectedMessage && selectedMessage !== prevMessage) {
			form.reset({
				...
				deliverTo: selectedMessage.target.existing_users ? EXISTING_USERS_FILTER : NEW_USERS_FILTER,
	...
	return (
		<>
			<ActionConfirm
				title={t("Are you sure you want to discard changes?")}
				description={t(
					"You have content changes, do you want to close the window and discard all changes?",
				)}
			...
			<ActionConfirm
				...
				description={t(
					"This message will be permanently archived and can no longer be delivered to users.",
				)}
			...
			<Dialog open={open} onOpenChange={handleOpenChange}>
				...
				>
					<DialogDescription id="message-dialog-description" className="sr-only">
						{t("Create or view a message")}
					</DialogDescription>
					...
					<DialogHeader>
						<div className="flex items-center gap-4 p-4 w-full">
							<DialogTitle className="font-semibold text-md">
								{t(mode === "create" ? "Create Message" : "Message Preview")}
							</DialogTitle>
							<button
								className="ms-auto"
								...
								<div className="flex flex-1 flex-col min-h-0">
									{showErrors && !contentComplete && (
										<div className="px-6 pt-4">
											<Badge variant="destructive" className="w-fit gap-1.5 py-1 px-2.5">
												...
												{t("Add at least one component to the content")}
											</Badge>
										</div>
									)}
									{isReadOnly && htmlMessage ? (
										<div className="flex-1 overflow-hidden bg-muted/30 p-4">
											<iframe
												srcDoc={htmlMessage}
												className="w-full h-full bg-card text-card-foreground rounded-md border border-border"
					...
					<div className="flex items-center justify-between px-4 py-3 shrink-0">
						{mode === "create" ? (
							...
							<>
								<PreviewOnlyButton htmlMessage={htmlMessage} onPreview={setPreviewHtml} />
								{canArchive && (
									<Button
										variant="outline"
			...
			{previewHtml && (
				<MessagePreviewDialog previewHtml={previewHtml} onClose={() => setPreviewHtml(null)} />
			)}
		</>
	);
};
