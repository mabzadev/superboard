...
const MessagePreviewDialog = ({
	...
	return (
		<Dialog open onOpenChange={() => onClose()}>
			...
			>
				<DialogDescription className="sr-only">Message preview</DialogDescription>

				{/* Header */}
				<div className="flex items-center justify-between px-6 py-3 shrink-0 border-b">
					<DialogTitle className="text-sm font-semibold">Message Preview</DialogTitle>
				...
				<div className="flex-1 flex items-center justify-center overflow-hidden bg-muted/30 p-6">
					{previewDevice === "web" ? (
						<iframe
							srcDoc={previewHtml}
							className="w-full h-full rounded-lg border border-border shadow-md bg-card text-card-foreground"
						...
						<iframe
							srcDoc={previewHtml}
							className="rounded-md border border-border shadow-md bg-card text-card-foreground max-w-[390px] max-h-[844px] w-full h-full"
							sandbox="allow-scripts allow-popups"
