...
import type { EmailTemplate } from "../api/marketing/marketingService.js";
...
export function EmailEditor({
	...
	purposeGroup?: "campaign" | "transactional";
}) {
	...
	const [undo, setUndo] = useState<EmailDesign[]>([]);
	const [redo, setRedo] = useState<EmailDesign[]>([]);
	...
	const update = useCallback((transform: (value: EmailDesign) => EmailDesign) => {
		setUndo((values) => [...values.slice(-49), current.current.document]);
		setRedo([]);
		setDocument((value) => transform(structuredClone(value)));
	...
	const save = useCallback(async () => {
		...
		try {
			...
			if (JSON.stringify([current.current.name, current.current.document]) === signature) {
				...
				current.current = { name: value.name, document: canonical };
	...
	return (
		<section className="overflow-hidden rounded-lg border border-kumo-line bg-kumo-base text-kumo-default">
					...
					{panel === "Content" && (
						<>
							...
							<Select
								...
								items={Object.fromEntries(
									EMAIL_PURPOSES.filter(
										(purpose) =>
											!purposeGroup ||
											(purposeGroup === "campaign"
												? ["campaign", "lifecycle"].includes(purpose)
												: !["campaign", "lifecycle"].includes(purpose)),
									).map((purpose) => [purpose, t(purpose)]),
								)}
							/>
							{selected ? (
