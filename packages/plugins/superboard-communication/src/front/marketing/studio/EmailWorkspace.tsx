...
export default function EmailWorkspace({
	...
	return (
		<div className="ds-page space-y-5 text-foreground">
			...
			<nav
				...
				className="flex flex-wrap gap-1 border-b border-kumo-line pb-3"
			>
				{[
					"Overview",
					"Campaigns",
				...
				<LinkButton href={`/communication/settings?tab=email&lang=${locale}`}>
					{t("Delivery settings")}
				</LinkButton>
			</nav>
			<Dialog.Root
				...
				<>
						...
						<>
							<div className="grid gap-4 md:grid-cols-3">
								{[
									...
									<div key={label} className="rounded-lg border border-kumo-line bg-kumo-base p-6">
					...
					{tab === "Settings" && (
						<LanguageSettings
							...
							project={project}
							onSaved={(saved) => {
								if (selectedId.current === project) setSettings(saved);
							}}
						/>
					)}
					...
					{tab === "Deliveries" && (
						<>
							...
							<EmailAdministration section="operations" />
							<MarketingDeliveryPanel section="operations" />
						</>
					)}
				</>
			)}
		...
		<section className="space-y-4">
			...
			<div className="overflow-auto rounded-lg border border-kumo-line">
				<FrontTable className="w-full text-start text-sm">
					<FrontTableHeader className="bg-kumo-tint">
						<FrontTableRow>
							{["Recipient", "Subject", "Language", "Status"].map((key) => (
								<FrontTableHead key={key} className="p-4 text-start font-medium">
									{t(key)}
								</FrontTableHead>
							))}
						</FrontTableRow>
					...
					<FrontTableBody>
						{values.map((value) => (
							<FrontTableRow key={value.delivery_id} className="border-t border-kumo-line">
								<FrontTableCell className="p-4">{value.recipient_email}</FrontTableCell>
								<FrontTableCell>
									<Button
										variant="ghost"
										onClick={async () => {
								...
								<FrontTableCell>{value.locale ?? "—"}</FrontTableCell>
								<FrontTableCell>{value.status ?? t(value.reason)}</FrontTableCell>
							</FrontTableRow>
						))}
					</FrontTableBody>
				</FrontTable>
