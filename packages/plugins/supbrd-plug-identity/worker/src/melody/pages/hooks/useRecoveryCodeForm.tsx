...
import { validate, emailField } from "../tools/form";
import { validateError } from "../tools/locale";
import { AuthorizeParams } from "../tools/param";
import { handleAuthorizeStep, parseAuthorizeBaseValues, parseResponse } from "../tools/request";
...
const useRecoveryCodeForm = ({
	...
	const errors = validate(recoveryCodeSchema, values);

	const handleChange = (name: "email" | "recoveryCode", value: string) => {
		onSubmitError(null);
		switch (name) {
			case "email":
	...
	const handleSubmit = useCallback(
		(e: Event) => {
			...
			fetch(routeConfig.IdentityRoute.AuthorizeRecoveryCode, {
				method: "POST",
				headers: {
				...
				body: JSON.stringify({
					email,
					recoveryCode,
					...parseAuthorizeBaseValues(params, locale),
				}),
			})
				.then(parseResponse)
				.then((response: any) => {
		...
		[locale, params, onSubmitError, onSwitchView, email, recoveryCode, errors],
	);

	const handleContinue = useCallback(() => {
		handleAuthorizeStep(pendingResponse, locale, onSwitchView);
