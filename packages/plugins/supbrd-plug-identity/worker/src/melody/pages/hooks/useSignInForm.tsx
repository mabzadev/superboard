...
import { validate, emailField, passwordField } from "../tools/form";
...
const useSignInForm = ({
	...
	const errors = validate(signInSchema, values);
	...
	const handlePasswordlessSignIn = useCallback(
		(e: Event) => {
			...
			fetch(routeConfig.IdentityRoute.AuthorizePasswordless, {
				method: "POST",
				headers: {
					Accept: "application/json",
					"Content-Type": "application/json",
				...
				.then((response: any) => {
					if (usePasswordlessAsMagicLink && "code" in response && response.code) {
						return fetch(routeConfig.IdentityRoute.SendPasswordlessCode, {
							method: "POST",
							headers: {
								Accept: "application/json",
								"Content-Type": "application/json",
