...
export const impersonateUser = async (c: Context<typeConfig.Context>) => {
  ...
  const isInternalAdministrator = c.env.MELODY_INTERNAL_ADMIN === true
  let impersonator = null
  if (!isInternalAdministrator && typeof impersonatorToken === 'string') {
  ...
  if (!isInternalAdministrator && !impersonator) {
    loggerUtil.triggerLogger(
      c,
      loggerUtil.LoggerLevel.Warn,
  ...
  const impersonatorRoles = impersonator
  ...
  if (!isInternalAdministrator && !impersonatorRoles.some((role) => variableConfig.S2sConfig.impersonationRoles.includes(role as Role))) {
    loggerUtil.triggerLogger(
      c,
      loggerUtil.LoggerLevel.Warn,
  ...
  const canonicalAuthId = await identityBridgeService.canonicalSubjectForCurrentProject(
    c,
    user,
  )
  const impersonatedBy = impersonator?.authId ?? `superboard-project-${c.env.MELODY_PROJECT_ID ?? 'admin'}`

  await kvService.storeRefreshToken(
    c.env.KV,
    refreshToken,
    {
      authId: canonicalAuthId,
...
export const postUserInvitation = async (c: Context<typeConfig.Context>) => {
  ...
  apiRecord.authId = canonicalAuthId
