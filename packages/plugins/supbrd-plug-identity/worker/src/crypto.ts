...
export async function issueSuperBoardToken(
	env: IdentityEnv,
	projectId: number,
	userId: string,
): Promise<{
	...
	const expiresIn = ttl(
		env.SUPERBOARD_IDENTITY_TOKEN_TTL,
		60,
		900,
		"SUPERBOARD_IDENTITY_TOKEN_TTL",
	);
	return {
		access_token: await sign(
			env,
			{ sub: userId, pid: projectId, type: "superboard_identity" },
			env.SUPERBOARD_IDENTITY_AUDIENCE,
			expiresIn,
		),
		token_type: "Bearer",
...
export async function verifyApplicationToken(env: IdentityEnv, token: string): Promise<JWTPayload> {
	const verified = await jwtVerify(token, createLocalJWKSet(publicJwks(env)), {
		issuer: env.SUPERBOARD_IDENTITY_ISSUER,
...
async function sign(
	...
	return new SignJWT(payload)
		...
		.setIssuer(env.SUPERBOARD_IDENTITY_ISSUER)
