...
const RedirectUriEditor = ({
	...
	const handleRemoveUri = (targetIndex: number) => {
		const newUris = redirectUris.filter((_uri, index) => {
			return targetIndex !== index;
		});
		onChange(newUris);
	};

	const handleUpdateUri = (targetIndex: number, value: string) => {
		const newUris = redirectUris.map((uri, index) => {
					...
					<section key={index} className="flex flex-col">
						<section className="flex items-center gap-2 w-full">
							<Input
								...
								onChange={(e) => handleUpdateUri(index, e.target.value)}
								value={uri}
								className="w-full"
								disabled={disabled}
							/>
