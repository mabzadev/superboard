...
export function useAuth() {
	...
	const userInfo = {
		roles: operator && operator.role >= 40 ? ["super_admin"] : [],
	...
	return {
		...
		userInfo,
		acquireToken: async () => "",
		acquireUserInfo: async () => userInfo,
...
async function redirectToIdentityPolicy(options: RedirectOptions): Promise<void> {
	try {
		const app = window.sessionStorage.getItem("superboard.identity.primaryApp");
		if (!app) {
			errorSignal.value = "Create an active SPA application before opening an account policy.";
			return;
		}
		const parsed = JSON.parse(app) as { clientId: string; redirectUri: string };
		if (!parsed.clientId || !parsed.redirectUri) throw new Error("invalid_identity_application");
		...
		const authorize = new URL("/oauth2/v1/authorize", configSignal.value.AUTH_SERVER_URL);
