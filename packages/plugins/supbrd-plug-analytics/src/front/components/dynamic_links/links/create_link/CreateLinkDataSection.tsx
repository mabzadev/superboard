					...
					<div className="flex flex-col gap-5 px-6 py-6">
						...
						<div className="flex flex-col gap-2">
							...
							<span className="text-xs text-muted-foreground">
								Attach key-value pairs that will be delivered to your app alongside the link. Useful
								for deep linking, referral tracking, or passing custom context.
							</span>
							<div
								className={cn(
							...
							>
								<div className="flex-1 relative">
									<Input
										...
										onKeyDown={(e) => {
											if (e.key === "Enter" && key !== "" && value !== "" && !disabledActions) {
												e.preventDefault();
						...
						<div className="flex flex-col gap-2">
							...
							{data.length > 0 ? (
								...
								<div className="flex flex-col items-center justify-center gap-2 rounded-lg border border-dashed border-sidebar-border bg-secondary/30 py-10">
									...
									<span className="text-sm text-muted-foreground">No parameters added yet</span>
								</div>
							)}
						</div>
					</div>
