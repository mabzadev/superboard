...
import { handleCopyText } from "../../../../../../../../supbrd-front-ui/src/shared/lib/copyTextHelper.js";
import { cn, deepClone } from "../../../../../../../../supbrd-front-ui/src/shared/lib/utils.js";
			...
			<div className="flex flex-1 flex-col gap-2 overflow-auto">
				<div className="flex flex-col gap-6 px-6 md:gap-6 md:py-6 max-w-[800px]">
					...
					<div className="flex flex-col gap-5">
						{/* URL */}
						<div className="flex flex-col gap-2" role="group" aria-labelledby="url-label">
							<div className="flex items-center justify-between">
								<label id="url-label" htmlFor="link-path" className="text-sm font-medium">
									URL
								</label>
								<Badge
									variant="outline"
							...
							>
								<span className="text-sm text-muted-foreground select-none">{domain}</span>
								<span className="relative inline-flex mx-0.5">
									...
									<input
										...
										aria-invalid={
											(path.length >= 3 && !pathAvailable) || (showErrors && path.length < 3)
										}
							...
							<span id="path-description" className="text-xs text-muted-foreground">
						...
						<div className="flex flex-col gap-2">
							...
							<Input
								...
								aria-invalid={
									(name.length > 0 && name.length < 3) || (showErrors && name.length === 0)
								}
							...
							<span id="name-description" className="text-xs text-muted-foreground">
							...
							<PopoverTrigger asChild disabled={disabledActions}>
								...
								>
									<div className="flex items-center justify-center w-8 h-8 rounded-md bg-secondary shrink-0">
										<Image
											src={resolvedTheme === "dark" ? selectedType.iconDark : selectedType.icon}
								...
								{linkTypes.map((item) => (
									<button
										...
										className={cn(
											...
											linkType === item.value ? "bg-accent" : "hover:bg-accent/50",
										)}
									>
										<div className="flex items-center justify-center w-7 h-7 rounded-md bg-secondary shrink-0">
											<Image
												src={resolvedTheme === "dark" ? item.iconDark : item.icon}
												alt={item.label}
												width={18}
												height={18}
					...
					<div className="flex flex-col gap-2">
						...
						<span className="text-xs text-muted-foreground">Organize and filter your links.</span>
					</div>
				</div>
			</div>
		</div>
