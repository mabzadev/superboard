...
export function parseProjectRef(value) {
	...
	if (!match) {
		throw new Error("--project-ref must use <instance_id>-prod or <instance_id>-test");
	}
	return {
...
export function staticAnalyticsReconciliationPlan({ target, targetName, environment, projectRef }) {
	const project = parseProjectRef(projectRef);
	const resources = target.environments?.[environment];
	if (!resources) throw new Error(`${targetName} does not define ${environment}`);
	const blockers = [];
...
export function buildAnalyticsReconciliationReport({
	...
	const checks = [
		check("installations", source.expected_installations, analytics.installations),
		...
		check("delivered_purchase_outbox", source.expected_purchases, source.delivered_purchases),
		{
			id: "purchase_dimensions",
			expected: sourceDimensions,
			actual: analyticsDimensions,
			matches: JSON.stringify(sourceDimensions) === JSON.stringify(analyticsDimensions),
		},
		{
...
function numericRecord(value) {
	return Object.fromEntries(Object.entries(value).map(([key, item]) => [key, numeric(item)]));
}

function numeric(value) {
	const number = Number(value ?? 0);
	if (!Number.isFinite(number)) throw new Error(`Invalid numeric result: ${value}`);
	return number;
}
...
export async function run(argv = process.argv.slice(2), environmentVariables = process.env) {
	...
	if (projectRows.length !== 1) {
		throw new Error(`Project ${plan.project.project_ref} was not found exactly once`);
	}
	...
	const targetMetrics = queryRemote(analytics, analyticsMetricsSql(projectId), env)[0];
	const sourceDimensions = queryRemote(api, sourcePurchaseDimensionsSql(projectId), env);
	const targetDimensions = queryRemote(analytics, analyticsPurchaseDimensionsSql(projectId), env);
