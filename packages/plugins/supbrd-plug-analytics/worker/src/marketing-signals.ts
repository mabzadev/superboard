...
import { readTextLimited, readJsonObjectLimited } from "@superboard/contracts/request-body";
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
		...
		.bind(
			...
			stableAnalyticsJson({
				...
				subject_hash: event.user_id ?? event.anonymous_id ?? event.app_instance_id ?? null,
...
export async function drainMarketingSignals(env: SignalEnv, maximum = 100): Promise<number> {
...
async function deliverRow(env: SignalEnv, row: MarketingSignalRow) {
	...
	try {
		...
		if (response.status === 404) {
			const payload = await readJsonObjectLimited(response.clone(), 4096).catch(() => null);
			...
			if (
				...
				await env.DB.prepare(
					"UPDATE analytics_marketing_signal_outbox SET status='pending',available_at=datetime('now','+60 seconds'),updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE id=?",
				)
					.bind(row.id)
					.run();
				return false;
		...
		if (!response.ok) {
			...
			throw new Error(`Marketing signal failed with HTTP ${response.status}: ${detail}`);
		}
		await env.DB.prepare(
			`UPDATE analytics_marketing_signal_outbox
...
async function signedMarketingRequest(env: Env, row: MarketingSignalRow) {
	...
	const headers = new Headers({
		...
		[PROJECT_CONTEXT_HEADERS.signature]: await signProjectContext(context, env.INTERNAL_API_TOKEN),
	});
