...
export async function commitAnalyticsMutation(
	...
	const requestHash = await sha256Hex(stableAnalyticsJson(mutation.requestBody ?? {}));
	...
	const idempotency = db
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
		...
		.bind(projectId, key, mutation.method, mutation.path, requestHash, status, responseJson);
	const audit = db
		.prepare(
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
		...
		.bind(
			crypto.randomUUID(),
			projectId,
			mutation.project.operatorId ?? String(mutation.project.actorId),
	...
	try {
		...
		if (concurrent) {
			return replayMutation(concurrent, mutation.method, mutation.path, requestHash);
		}
		throw error;
	}
...
function replayMutation(
	...
	if (replay.method !== method || replay.path !== path || replay.request_sha256 !== requestHash) {
		throw httpError(
