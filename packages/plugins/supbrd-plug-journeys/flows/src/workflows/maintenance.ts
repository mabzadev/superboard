...
export class FlowMaintenanceExecution extends WorkflowEntrypoint<Env, FlowMaintenancePayload> {
	async run(
		event: Readonly<WorkflowEvent<FlowMaintenancePayload>>,
		step: WorkflowStep,
	): Promise<Record<string, unknown>> {
		step = withPluginWorkflowLifecycle(
			this.env,
			step,
			"FLOW_MAINTENANCE_EXECUTION",
			event.instanceId,
		);
	...
	private async migrateRelease(
		...
		if (!payload.environmentId || !payload.workflowId || !payload.workflowVersionId) {
		...
		const release = await step.do("load-active-release", async () => {
         ...
         LIMIT 1`,
				...
				.bind(payload.projectId, payload.environmentId, payload.workflowId)
		...
		const population =
			release.migration_strategy === "restart-all"
						...
						await Promise.all(
							batch.map((row) => {
								const runtime = this.env.FLOW_USER_RUNTIME.getByName(
									flowRuntimeName(payload.projectId, payload.environmentId!, row.user_id_hash),
								);
			...
			async () => {
				...
				const producer = (async () => {
                 ...
                 ORDER BY occurred_at, event_id LIMIT 1000`,
							...
							await writer.write(
								encoder.encode(`${page.results.map((row) => JSON.stringify(row)).join("\n")}\n`),
							);
