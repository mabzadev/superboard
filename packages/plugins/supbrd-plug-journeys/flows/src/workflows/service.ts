import type { FlowGraph, FlowWorkflowFrequency } from "@superboard/contracts/flows";
...
export async function listWorkflows(context: FlowContext) {
...
export async function createWorkflow(context: FlowContext, body: Record<string, unknown>) {
...
export async function getWorkflow(context: FlowContext, workflowId: string) {
	...
	const [draft, versions, releases, translations] = await Promise.all([
       ...
       WHERE workflow_id = ? AND project_id = ?`,
			...
			.bind(workflowId, projectId(context))
			.first<SqlRow>(),
		context.env.DB.prepare(
...
export async function updateWorkflow(
	...
	const description =
		body.description === undefined
			...
			: optionalString(body.description, "description", 2_000);
	const frequency =
		body.frequency == null ? parseFrequency(existing.frequency) : parseFrequency(body.frequency);
	const status =
		body.status == null ? String(existing.status) : requiredString(body.status, "status", 32);
	...
	await context.env.DB.prepare(
     ...
     WHERE id = ? AND project_id = ?`,
		...
		.bind(name, description, frequency, status, now, status, now, workflowId, projectId(context))
		.run();
	await audit(context, "flows.workflow.updated", "workflow", workflowId, {
		name,
		frequency,
		status,
	});
...
export async function saveDraft(
	...
	const graph = await stampNewComponentVersions(context, parseFlowGraph(body.graph));
	const expectedRevision =
		typeof body.revision === "number" && Number.isInteger(body.revision) ? body.revision : null;
	...
	await context.env.DB.prepare(
     ...
     WHERE id = ? AND project_id = ?`,
		...
		.bind(nextRevision, now, workflowId, projectId(context))
		.run();
	await audit(context, "flows.workflow.draft-saved", "workflow", workflowId, {
...
export async function publishWorkflow(
	...
	const draft = await context.env.DB.prepare(
     ...
     WHERE workflow_id = ? AND project_id = ?`,
		...
		.bind(workflowId, projectId(context))
		.first<{ revision: number; graph_json: string }>();
	...
	await assertWorkflowTriggerTargets(context, workflowId, graph);
	...
	const id = crypto.randomUUID();
	const checksum = await sha256(JSON.stringify({ graph, translations: translationRows.results }));
	const now = isoNow();
	...
	await audit(context, "flows.workflow.published", "workflow_version", id, {
		workflow_id: workflowId,
		version,
...
export async function getVersion(context: FlowContext, workflowId: string, versionId: string) {
	await requireWorkflow(context, workflowId);
	const [row, translations] = await Promise.all([
     ...
     WHERE id = ? AND workflow_id = ? AND project_id = ?`,
			...
			.bind(versionId, workflowId, projectId(context))
			.first<SqlRow>(),
		context.env.DB.prepare(
...
export async function activateWorkflow(
	...
	const environment = await context.env.DB.prepare(
     ...
     WHERE id = ? AND project_id = ? AND active = 1`,
		...
		.bind(environmentId, projectId(context))
		.first<{ id: string; allow_draft: number }>();
	...
	if (useDraft && Number(environment.allow_draft) !== 1) {
		throw failure(
			"flow_environment_draft_forbidden",
			"This environment does not allow draft releases",
			409,
		);
	}
	...
	const triggerTargets = releaseGraph.blocks.filter((block) => block.type === "workflow-trigger");
	for (const trigger of triggerTargets) {
		...
		const targetReleaseGraph = await effectiveReleaseGraph(context, environmentId, targetId);
		...
		if (
			!targetReleaseGraph.blocks.some(
	...
	const migrationExecutionId = await flowWorkflowInstanceId("flow-release", migrationBusinessId);
...
async function effectiveReleaseGraph(
	...
	const release = await context.env.DB.prepare(
     ...
     WHERE r.project_id = ? AND r.environment_id = ?
       AND r.workflow_id = ? AND r.active = 1 AND w.status = 'active'`,
		...
		.bind(projectId(context), environmentId, workflowId)
	...
	const version = await context.env.DB.prepare(
     ...
     WHERE project_id = ? AND workflow_id = ? AND id = ?`,
		...
		.bind(projectId(context), workflowId, release.workflow_version_id)
		.first<{ graph_json: string }>();
...
export async function deactivateWorkflow(
	...
	const result = await context.env.DB.prepare(
     ...
     WHERE project_id = ? AND environment_id = ? AND workflow_id = ?`,
		...
		.bind(isoNow(), projectId(context), environmentId, workflowId)
		.run();
	if (!result.meta.changes)
		throw failure("flow_release_not_found", "Workflow release not found", 404);
	await audit(context, "flows.workflow.deactivated", "workflow", workflowId, {
		environment_id: environmentId,
	});
}
...
export async function saveTranslations(
	...
	const statements = body.items.map((entry) => {
       ...
       WHERE flow_translations.project_id = excluded.project_id`,
			...
			propertyKey(item.property_key, "property_key"),
	...
	await audit(context, "flows.workflow.translations-saved", "workflow", workflowId, {
	...
	return {
		...
		blocks: graph.blocks.map((block) => {
			...
			const definition = definitions.results.find((candidate) => {
				...
				const templateType =
					schema.template_type === "survey-component" ? "survey" : schema.template_type;
				...
				return (
					library === normalizeLibrary(candidate.library_name) ||
...
async function assertWorkflowTriggerTargets(
	...
	const visit = async (
		...
		for (const block of graph.blocks.filter((entry) => entry.type === "workflow-trigger")) {
         ...
         WHERE w.id = ? AND w.project_id = ?
           AND w.status != 'archived'`,
				...
				.bind(targetId, projectId(context))
				.first<{ graph_json: string }>();
			if (!target) {
				throw failure(
			...
			await visit(targetId, targetGraph, nextActive, depth + 1);
