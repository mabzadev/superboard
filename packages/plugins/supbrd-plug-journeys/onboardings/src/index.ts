...
const fail = (code: string, message: string, status: ErrorStatus, details?: unknown) =>
	...
	const error =
		cause instanceof OnboardingError
			...
			: fail(
					...
					cause instanceof Error && cause.message.includes("Internal") ? 500 : 422,
				);
	if (error.status === 500)
		console.error(
			JSON.stringify({
...
async function authenticate(c: OnboardingContext, next: Next) {
	...
	if (!result.ok)
		throw fail(result.code, result.message, result.code === "internal_auth_invalid" ? 401 : 403);
	...
	mutation(
		...
		async () => {
			...
			if (body.configuration)
				version = await createVersion(c.env.DB, c.get("projectId"), id, body.configuration);
			return {
				id,
				identifier,
...
app.delete("/internal/v1/:id{[0-9a-fA-F-]{36}}", async (c) =>
	mutation(c, "onboarding.delete", async () => {
		await own(c.env.DB, c.get("projectId"), c.req.param("id"));
		await c.env.DB.prepare("DELETE FROM onboardings WHERE project_id=? AND id=?")
			.bind(c.get("projectId"), c.req.param("id"))
			.run();
		return { deleted: true };
	}),
...
app.post("/internal/v1/:id{[0-9a-fA-F-]{36}}/versions", async (c) =>
	mutation(
		...
		async () => {
			...
			return createVersion(c.env.DB, c.get("projectId"), c.req.param("id"), body.configuration);
		},
		201,
	),
);
app.post("/internal/v1/:id{[0-9a-fA-F-]{36}}/publish", async (c) =>
	mutation(c, "onboarding.publish", async () => {
		...
		if (!version) throw fail("version_not_found", "Onboarding version was not found", 404);
...
app.put("/internal/v1/placements/:id", async (c) =>
	mutation(c, "placement.update", async () => {
		await owned(c.env.DB, "placements", c.get("projectId"), c.req.param("id"), "placement");
		const body = await objectBody(c.req.raw);
		const onboardingId = required(body.onboarding_id, "onboarding_id", 64);
		await own(c.env.DB, c.get("projectId"), onboardingId);
		...
		if (
			...
			throw fail("version_not_published", "Placement version must be published", 409);
		await c.env.DB.prepare(
...
app.delete("/internal/v1/targeting-rules/:id", async (c) =>
	mutation(c, "targeting_rule.delete", async () => {
		...
		await c.env.DB.prepare("DELETE FROM targeting_rules WHERE project_id=? AND id=?")
			.bind(c.get("projectId"), c.req.param("id"))
			.run();
		return { deleted: true };
	}),
...
app.get("/internal/v1/experiences", async (c) => {
	...
	return c.json({
		data: rows.results.map((row) => ({
			...row,
			variants: variants.results.filter((v) => v.experience_id === (row as { id: string }).id),
		})),
	});
});
app.post("/internal/v1/experiences", async (c) =>
	mutation(
		...
		async () => {
			...
			await owned(c.env.DB, "placements", c.get("projectId"), placementId, "placement");
			const variants = Array.isArray(body.variants) ? body.variants : [];
			if (variants.length < 2 || variants.length > 10)
				throw fail("variants_invalid", "An experience requires 2 to 10 variants", 422);
			...
			if (total !== 10000)
				throw fail("variant_weights_invalid", "Variant weights must total 10000", 422);
			...
			await c.env.DB.batch([
				c.env.DB.prepare(
					...
					placementId,
					required(body.name, "name", 255),
					enumValue(body.status, ["draft", "running", "paused", "completed"], "draft"),
...
app.post("/internal/v1/experiences/:id/status", async (c) =>
	mutation(c, "experience.status", async () => {
		await owned(c.env.DB, "experiences", c.get("projectId"), c.req.param("id"), "experience");
		const body = await objectBody(c.req.raw);
		const status = enumValue(body.status, ["draft", "running", "paused", "completed"], "draft");
		await c.env.DB.prepare(
...
async function resolve(c: OnboardingContext) {
	...
	if (
		...
		!rules.results.some((row) => matches(JSON.parse(row.conditions_json), input))
	)
...
app.get("/internal/v1/statistics", async (c) => {
	...
	const placement = f.placement ? await statisticsPlacementKey(c, f.placement) : null;
	const rows = await c.env.DB.prepare(
	...
	return c.json({
		data: {
			...
			series: [...grouped.values()].sort(
				(a, b) => a.date.localeCompare(b.date) || a.event_type.localeCompare(b.event_type),
			),
			funnel,
...
async function owned(
	...
	if (
		...
		throw fail(`${label}_not_found`, `${label.replace("_", " ")} was not found`, 404);
}
...
function slug(value: unknown, name: string) {
	...
	if (!/^[a-z0-9][a-z0-9_-]*$/.test(text))
		throw fail("validation_failed", `${name} contains unsupported characters`, 422);
	return text;
}
...
function matches(conditions: Record<string, unknown>, input: ReturnType<typeof parsePlacement>) {
	for (const [key, value] of Object.entries(conditions)) {
		if (key === "platform" && value !== input.platform) return false;
		if (key === "locale" && value !== input.locale) return false;
		if (key === "min_app_version" && input.app_version && String(input.app_version) < String(value))
			return false;
		if (key.startsWith("attribute.") && input.attributes[key.slice(10)] !== value) return false;
	}
	return true;
}
...
function statFilters(query: Record<string, string>) {
	...
	try {
		...
		throw fail("timezone_invalid", "timezone must be a valid IANA timezone", 422);
	}
	return {
		from,
		to,
		timezone,
		interval: enumValue(query.interval, ["hour", "day", "week", "month"] as const, "day"),
