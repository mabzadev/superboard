...
export function FlowStatusBadge({ status }: { status: string }) {
	...
	const active = status === "active" || status === "running" || status === "completed";
	const warning = status === "draft" || status === "paused" || status === "in-progress";
	const statusLabel = status.replaceAll("-", " ");
	const translatedLabel = tr(`${statusLabel.charAt(0).toUpperCase()}${statusLabel.slice(1)}`);
	return (
		<Badge
			variant="outline"
			className={cn(
