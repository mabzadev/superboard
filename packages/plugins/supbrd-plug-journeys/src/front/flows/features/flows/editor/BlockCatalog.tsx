...
import type { FlowBlockType, FlowComponentDefinition } from "../../../api/flows/flowsService.js";
import { useFlowI18n } from "../i18n.js";
import { BLOCK_CATALOG, componentBlockType } from "./graph.js";
...
export function BlockCatalog({
	...
	const componentTypes = useMemo(() => new Set(components.map(componentBlockType)), [components]);
		...
		return ["Logic", "Experience", "Utility"].map((category) => ({
			...
			items: BLOCK_CATALOG.filter(
				(item) =>
					...
					!(
						onAddComponent &&
						componentTypes.has(item.type as "component" | "tour-component" | "survey")
					) &&
					(!query || `${item.label} ${item.description}`.toLowerCase().includes(query)),
			),
		}));
					...
					<section key={group.category} className="grid gap-1">
						...
						{group.items.map((item) => (
							<button
								...
								onDragStart={(event) => event.dataTransfer.setData(FLOW_BLOCK_DRAG_TYPE, item.type)}
								onClick={() => onAdd(item.type)}
							>
								<span className="block text-sm font-medium">{tr(item.label)}</span>
				...
				<section className="grid gap-1">
					...
					{filteredComponents.map((component) => (
						<button
							...
							className="rounded-[var(--radius-sm)] px-2 py-2 text-left hover:bg-accent focus-visible:bg-accent"
							onDragStart={(event) =>
								event.dataTransfer.setData(FLOW_COMPONENT_DRAG_TYPE, component.id)
							}
