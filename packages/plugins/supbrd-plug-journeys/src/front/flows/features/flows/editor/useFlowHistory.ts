...
import type { FlowGraph } from "../../../api/flows/flowsService.js";
...
export function useFlowHistory(initialGraph: FlowGraph) {
	...
	const update = useCallback((next: FlowGraph | ((graph: FlowGraph) => FlowGraph)) => {
		setState((current) => {
	...
	const updateTransient = useCallback((next: (graph: FlowGraph) => FlowGraph) => {
