...
import type { FlowBlock, FlowBlockType } from "../../../api/flows/flowsService.js";
		...
		>
			...
			<header className="flex items-center gap-2 border-b px-3 py-2">
				...
				<div className="min-w-0 flex-1">
					...
					<p className="truncate font-mono text-[10px] text-muted-foreground">{block.key}</p>
				</div>
			</header>
			<div className="px-3 py-2 text-xs text-muted-foreground">
				<p className="line-clamp-2">{block.description || block.type.replaceAll("-", " ")}</p>
			</div>
			{!isEnd && !isNote && (
				<>
