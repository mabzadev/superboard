...
import { ArrowDown, ArrowUp, Gauge, Pause, Play, Plus, Trash2 } from "lucide-react";
...
export function LaunchpadPage() {
	...
	const updateGroup = async (group: FlowLaunchpadGroup, changes: Partial<FlowLaunchpadGroup>) => {
		if (!projectRef) return;
		setGroups((items) =>
			items.map((item) => (item.id === group.id ? { ...item, ...changes } : item)),
		);
		try {
	...
	const addWorkflow = async (group: FlowLaunchpadGroup, workflowId: string) => {
		if (!projectRef || group.workflows.some((item) => item.workflow_id === workflowId)) return;
		try {
	...
	const moveWorkflow = async (group: FlowLaunchpadGroup, index: number, direction: -1 | 1) => {
		if (!projectRef) return;
	...
	const removeWorkflow = async (group: FlowLaunchpadGroup, workflowId: string) => {
		if (!projectRef) return;
		await flowsApi.setLaunchpadWorkflows(
			projectRef,
			group.id,
		...
		>
			{groups.length ? (
							...
							<CardHeader className="grid grid-cols-[1fr_auto] gap-3 border-b pb-4">
								<div>
									<div className="flex items-center gap-2">
										...
										<FlowStatusBadge status={group.paused ? "paused" : "active"} />
									</div>
									<CardDescription>
								...
								{group.workflows.map((workflow, index) => (
									...
									>
										...
										<Button
											...
											onClick={() => void removeWorkflow(group, workflow.workflow_id)}
										>
											<Trash2 />
										</Button>
									</div>
								))}
								<Select onValueChange={(value) => void addWorkflow(group, value)}>
									<SelectTrigger className="w-full border-dashed">
										<Plus />
										<SelectValue placeholder={tr("Add workflow to this group")} />
									</SelectTrigger>
									<SelectContent>
										{workflows
											.filter(
												(workflow) =>
													!group.workflows.some((item) => item.workflow_id === workflow.id),
											)
											.map((workflow) => (
				...
				<FlowsEmptyState
					icon={Gauge}
					title={t("noData")}
					description={tr("Workflows outside Launchpad still start immediately when eligible.")}
					action={
	...
	return (
		<DialogContent>
			<DialogHeader>
				...
				<DialogDescription>
					{tr("The first eligible group owns workflows that appear in several groups.")}
				</DialogDescription>
			</DialogHeader>
			<div className="grid gap-4">
			...
			<DialogFooter>
				<Button disabled={!name.trim() || !environmentId || busy} onClick={() => void create()}>
