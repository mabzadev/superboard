...
export const flowsApi = {
	...
	listWorkflows: async (
		...
		return unwrap<{ items: FlowWorkflow[] }>(
			await GET(projectPath(projectRef, `/workflows${query ? `?${query}` : ""}`)),
		).items;
	},
	...
	) => unwrap<FlowWorkflowDetails>(await POST(projectPath(projectRef, "/workflows"), body)),
	getWorkflow: async (projectRef: string, workflowId: string) =>
		unwrap<FlowWorkflowDetails>(
			await GET(projectPath(projectRef, `/workflows/${encodeURIComponent(workflowId)}`)),
		),
	updateWorkflow: async (
		projectRef: string,
		workflowId: string,
		body: Partial<Pick<FlowWorkflow, "name" | "description" | "frequency" | "status">>,
	) =>
		unwrap<FlowWorkflow>(
			await PUT(projectPath(projectRef, `/workflows/${encodeURIComponent(workflowId)}`), body),
		),
	saveDraft: async (projectRef: string, workflowId: string, graph: FlowGraph, revision: number) =>
		unwrap<{
			workflow_id: string;
			revision: number;
			graph: FlowGraph;
			...
			await PUT(projectPath(projectRef, `/workflows/${encodeURIComponent(workflowId)}/draft`), {
				graph,
				revision,
			}),
		),
	publishWorkflow: async (
		...
		unwrap<FlowVersion>(
			await POST(
				projectPath(projectRef, `/workflows/${encodeURIComponent(workflowId)}/publish`),
				body,
			),
		),
	saveTranslations: async (
		...
		unwrap<{ workflow_id: string; count: number; updated_at: string }>(
			await PUT(
				projectPath(projectRef, `/workflows/${encodeURIComponent(workflowId)}/translations`),
				{ items },
			),
		),
	getVersion: async (projectRef: string, workflowId: string, versionId: string) =>
	...
	activateRelease: async (
		...
		unwrap<FlowRelease>(
			await POST(
				projectPath(projectRef, `/workflows/${encodeURIComponent(workflowId)}/releases`),
				body,
			),
		),
	duplicateWorkflow: async (
		...
		unwrap<FlowWorkflow>(
			await POST(
				projectPath(projectRef, `/workflows/${encodeURIComponent(workflowId)}/duplicate`),
				body,
			),
		),
	workflowAnalytics: async (projectRef: string, workflowId: string) =>
		unwrap<FlowWorkflowAnalytics>(
			await GET(projectPath(projectRef, `/workflows/${encodeURIComponent(workflowId)}/analytics`)),
		),
	surveyAnalytics: async (projectRef: string, surveyId: string) =>
		unwrap<FlowSurveyAnalytics>(
			await GET(projectPath(projectRef, `/surveys/${encodeURIComponent(surveyId)}/analytics`)),
		),
	exportSurveyCsv: async (projectRef: string, surveyId: string) =>
		unwrap<{
			...
			r2_key: string;
			row_count: number;
		}>(await POST(projectPath(projectRef, `/surveys/${encodeURIComponent(surveyId)}/export`))),
	getLaunchpad: async (projectRef: string) =>
		unwrap<{ groups: FlowLaunchpadGroup[] }>(await GET(projectPath(projectRef, "/launchpad")))
			.groups,
	...
	) => unwrap<FlowLaunchpadGroup>(await POST(projectPath(projectRef, "/launchpad/groups"), body)),
	updateLaunchpadGroup: async (
		projectRef: string,
		groupId: string,
		body: Partial<Pick<FlowLaunchpadGroup, "name" | "position" | "concurrency_limit" | "paused">>,
	) =>
		unwrap<FlowLaunchpadGroup>(
			await PATCH(
				projectPath(projectRef, `/launchpad/groups/${encodeURIComponent(groupId)}`),
				body,
			),
		),
	setLaunchpadWorkflows: async (
		...
		unwrap<{ id: string }>(
			await PUT(
				projectPath(projectRef, `/launchpad/groups/${encodeURIComponent(groupId)}/workflows`),
				{ workflows },
			),
		),
	deleteLaunchpadGroup: async (projectRef: string, groupId: string) =>
		unwrap<{ deleted: boolean }>(
			await DELETE(projectPath(projectRef, `/launchpad/groups/${encodeURIComponent(groupId)}`)),
		),
	...
	getUser: async (projectRef: string, userHash: string) =>
		unwrap<FlowUserDetails>(
			await GET(projectPath(projectRef, `/users/${encodeURIComponent(userHash)}`)),
		),
	resetUser: async (projectRef: string, userHash: string, workflowId?: string) =>
	...
	createComponentLibrary: async (projectRef: string, body: { name: string; identifier: string }) =>
		unwrap<FlowComponentLibrary>(await POST(projectPath(projectRef, "/component-libraries"), body)),
	updateComponentLibrary: async (
		...
		unwrap<FlowComponentLibrary>(
			await PATCH(
				projectPath(projectRef, `/component-libraries/${encodeURIComponent(libraryId)}`),
				body,
			),
		),
	...
	) => unwrap<FlowComponentDefinition>(await POST(projectPath(projectRef, "/components"), body)),
	updateComponent: async (
		...
		unwrap<FlowComponentDefinition>(
			await PATCH(projectPath(projectRef, `/components/${encodeURIComponent(componentId)}`), body),
		),
	synchronizeComponent: async (projectRef: string, componentId: string) =>
		...
		}>(await POST(projectPath(projectRef, `/components/${encodeURIComponent(componentId)}/sync`))),
	listEnvironments: async (projectRef: string) =>
		unwrap<{ items: FlowEnvironment[] }>(await GET(projectPath(projectRef, "/environments"))).items,
	createEnvironment: async (
		projectRef: string,
		body: { name: string; key: string; kind: string; allow_draft: boolean },
	) => unwrap<FlowEnvironment>(await POST(projectPath(projectRef, "/environments"), body)),
	rotateEnvironmentKey: async (projectRef: string, environmentId: string) =>
		unwrap<{ id: string; sdk_key: string }>(
			await POST(
				projectPath(projectRef, `/environments/${encodeURIComponent(environmentId)}/rotate-key`),
			),
		),
	listLocalization: async (projectRef: string) =>
		unwrap<{ items: FlowLanguageGroup[] }>(await GET(projectPath(projectRef, "/localization")))
			.items,
	...
	) => unwrap<FlowLanguageGroup>(await PUT(projectPath(projectRef, "/localization"), body)),
};
