...
const CLAIM_COLUMNS = ["event_id", "project_id", "source_module", "claimed_at"];
...
export const FLOW_CUTOVER_ENTITIES = Object.freeze([
	...
	...forSources(
		"releases",
		"flow_environment_releases",
		RELEASE_COLUMNS,
		["environment_id", "workflow_id"],
		{
			numeric: ["project_id", "use_draft", "active"],
			from: "flow_environment_releases r JOIN flow_workflows w ON w.id = r.workflow_id",
			selectAlias: "r",
			where:
				"r.project_id = :project_id AND r.environment_id = :environment_id AND w.origin = :source_module",
		},
	),
	...
	entity(
		"onboardings.targeting_rules",
		"onboardings",
		"flow_legacy_targeting_rules",
		TARGETING_COLUMNS,
		["id"],
		{
	...
	...forSources(
		"mappings",
		"flow_legacy_mappings",
		MAPPING_COLUMNS,
		["project_id", "source_module", "source_type", "source_id"],
		{
			json: ["metadata_json"],
			numeric: ["project_id"],
			immutable: true,
	...
	...forSources(
		...
		["project_id", "event_id"],
		{
			...
			immutable: true,
			where:
				"a.project_id = :project_id AND a.environment_id = :environment_id AND a.source_module = :source_module",
			alias: "a",
			reverse: "event",
	...
	...forSources(
		"claims",
		"flow_legacy_event_claims",
		CLAIM_COLUMNS,
		["project_id", "source_module", "event_id"],
		{
			numeric: ["project_id"],
			immutable: true,
	...
	...forSources(
		"states",
		"flow_user_workflow_states",
		STATE_COLUMNS,
		["project_id", "environment_id", "user_id_hash", "workflow_id"],
		{
			...
			selectAlias: "s",
			where:
				"s.project_id = :project_id AND s.environment_id = :environment_id AND w.origin = :source_module",
		},
	),
	...forSources(
		"assignments",
		"flow_experiment_assignments",
		ASSIGNMENT_COLUMNS,
		["project_id", "environment_id", "workflow_id", "split_block_id", "user_id_hash"],
		{
			numeric: ["project_id"],
			from: "flow_experiment_assignments a JOIN flow_workflows w ON w.id = a.workflow_id",
			selectAlias: "a",
			where:
				"a.project_id = :project_id AND a.environment_id = :environment_id AND w.origin = :source_module",
		},
	),
