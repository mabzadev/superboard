...
app.get("/internal/v1/links", async (c) => {
	...
	const sort = c.req.query("sort_by") || "created_at";
	const direction = c.req.query("sort_order") === "asc" ? "asc" : "desc";
	const active = c.req.query("active") || "";
	const f = filters(c.req.query());
	const rows = await c.env.DB.prepare(
		`SELECT l.id,l.slug,l.name,l.destination_url,l.destinations_json,l.campaign_id,l.title,l.subtitle,l.image_url,l.utm_json,l.data_json,l.tags_json,l.hidden,l.use_project_defaults,l.active,l.created_at,l.updated_at,c.name campaign_name,
    ...
    WHERE l.project_id=? AND (?='' OR instr(lower(l.slug),lower(?))>0 OR instr(lower(l.name),lower(?))>0 OR instr(lower(l.title),lower(?))>0 OR instr(lower(l.tags_json),lower(?))>0) AND (?='' OR l.active=?) AND (?='' OR l.slug=?)
    GROUP BY l.id
    ORDER BY
      CASE WHEN ?='asc' THEN CASE ? WHEN 'name' THEN l.name WHEN 'views' THEN total_views WHEN 'opens' THEN total_opens WHEN 'installs' THEN total_installs ELSE l.created_at END END ASC,
      CASE WHEN ?='desc' THEN CASE ? WHEN 'name' THEN l.name WHEN 'views' THEN total_views WHEN 'opens' THEN total_opens WHEN 'installs' THEN total_installs ELSE l.created_at END END DESC,
      l.id ASC LIMIT ? OFFSET ?`,
	)
		.bind(
			`${f.from}T00:00:00Z`,
	...
	const total = await c.env.DB.prepare(
		"SELECT COUNT(*) AS total FROM links WHERE project_id=? AND (?='' OR instr(lower(slug),lower(?))>0 OR instr(lower(name),lower(?))>0 OR instr(lower(title),lower(?))>0 OR instr(lower(tags_json),lower(?))>0) AND (?='' OR active=?) AND (?='' OR slug=?)",
	)
		.bind(
			c.get("projectId"),
			search,
	...
	return c.json({
		...
		meta: { limit, offset, total_entries: total?.total ?? 0, filters: f },
	});
});
...
app.post("/internal/v1/links", async (c) =>
	mutation(
		...
		async () => {
			...
			await assertCampaign(c.env.DB, c.get("projectId"), optional(body.campaign_id, 64));
			await c.env.DB.prepare(
				`INSERT INTO links (id,project_id,slug,name,destination_url,destinations_json,campaign_id,title,subtitle,image_url,utm_json,data_json,tags_json,hidden,use_project_defaults,active,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,CURRENT_TIMESTAMP)`,
			)
				.bind(
					id,
					c.get("projectId"),
					...
					validJson(body.data, {}),
					tagsJson(body.tags),
					boolean(body.hidden, false) ? 1 : 0,
					boolean(body.use_project_defaults, false) ? 1 : 0,
...
app.put("/internal/v1/links/:id", async (c) =>
	mutation(c, "link.update", async () => {
		const existing = await ownedLink(c.env.DB, c.get("projectId"), c.req.param("id"));
		const body = await objectBody(c.req.raw);
		const campaignId = optional(body.campaign_id, 64);
		await assertCampaign(c.env.DB, c.get("projectId"), campaignId);
		await c.env.DB.prepare(
			`UPDATE links SET slug=?,name=?,destination_url=?,destinations_json=?,campaign_id=?,title=?,subtitle=?,image_url=?,utm_json=?,data_json=?,tags_json=?,hidden=?,use_project_defaults=?,active=?,updated_at=CURRENT_TIMESTAMP WHERE id=? AND project_id=?`,
		)
			.bind(
				...
				optionalUrl(body.image_url),
				validJson(body.utm, {}),
				body.data === undefined ? (existing.data_json ?? "{}") : validJson(body.data, {}),
				body.tags === undefined ? (existing.tags_json ?? "[]") : tagsJson(body.tags),
				body.hidden === undefined ? (existing.hidden ?? 0) : boolean(body.hidden, false) ? 1 : 0,
				body.use_project_defaults === undefined
					? body.destination_url === existing.destination_url
						? (existing.use_project_defaults ?? 0)
						: 0
					: boolean(body.use_project_defaults, false)
						? 1
						: 0,
				boolean(body.active, true) ? 1 : 0,
				c.req.param("id"),
			...
			.run();
		return linkJson(await ownedLink(c.env.DB, c.get("projectId"), c.req.param("id")));
	}),
);
app.delete("/internal/v1/links/:id", async (c) =>
	mutation(c, "link.delete", async () => {
		const result = await c.env.DB.prepare("DELETE FROM links WHERE id=? AND project_id=?")
			.bind(c.req.param("id"), c.get("projectId"))
			.run();
		if (!result.meta.changes) throw new DynamicError("link_not_found", "Link was not found", 404);
		return { deleted: true };
	}),
);
...
app.post("/internal/v1/links/:slug/resolve", async (c) => {
	const body = await objectBody(c.req.raw);
	const platform = enumValue(body.platform, ["ios", "android", "web", "desktop"], "web");
	const row = await c.env.DB.prepare(
		"SELECT link.id,link.destination_url,link.destinations_json,link.active,defaults.configuration_json FROM links link LEFT JOIN link_default_redirects defaults ON defaults.project_id=link.project_id AND link.use_project_defaults=1 WHERE link.project_id=? AND link.slug=?",
	)
	...
	if (!row || row.active !== 1) throw new DynamicError("link_not_found", "Link was not found", 404);
	const destinations = JSON.parse(row.destinations_json) as Record<string, unknown>;
	...
	const defaults = row.configuration_json ? JSON.parse(row.configuration_json) : {};
	const platformDefault =
		defaults.platforms?.[platform]?.fallback_url ?? defaults.platforms?.[platform]?.url;
	let destination =
		typeof destinations[platform] === "string"
			? String(destinations[platform])
			: typeof platformDefault === "string"
				? platformDefault
				: typeof defaults.default_fallback === "string"
...
app.post("/internal/v1/campaigns", async (c) =>
	mutation(
		...
		async () => {
			...
			const slug = identifier(body.slug || name.toLowerCase().replace(/[^a-z0-9]+/g, "-"), "slug");
			await c.env.DB.prepare(
...
app.delete("/internal/v1/campaigns/:id", async (c) =>
	mutation(c, "campaign.delete", async () => {
		...
		await c.env.DB.batch([
			...
			c.env.DB.prepare("DELETE FROM campaigns WHERE project_id=? AND id=?").bind(
				c.get("projectId"),
				c.req.param("id"),
			),
		]);
...
app.get("/internal/v1/redirect-config", async (c) => {
	const row = await c.env.DB.prepare(
		"SELECT configuration_json FROM link_default_redirects WHERE project_id=?",
	)
		.bind(c.get("projectId"))
		.first<{ configuration_json: string }>();
	return c.json({
		data: row ? JSON.parse(row.configuration_json) : { default_fallback: null, platforms: {} },
	});
});
app.put("/internal/v1/redirect-config", async (c) =>
	mutation(c, "redirect_config.update", async () => {
		const body = await objectBody(c.req.raw);
		if (body.default_fallback !== undefined && body.default_fallback !== null)
			url(body.default_fallback, "default_fallback");
		if (body.platforms !== undefined) {
			if (!body.platforms || typeof body.platforms !== "object" || Array.isArray(body.platforms))
				throw new DynamicError("redirect_platforms_invalid", "Platforms must be an object", 422);
			for (const [platform, value] of Object.entries(body.platforms)) {
				if (
					!["ios", "android", "desktop", "web"].includes(platform) ||
					!value ||
					typeof value !== "object" ||
					Array.isArray(value)
				)
					throw new DynamicError(
						"redirect_platform_invalid",
						"Redirect platform configuration is invalid",
						422,
					);
				for (const field of ["url", "fallback_url"]) if (field in value) url(value[field], field);
			}
		}
		...
		const row = await c.env.DB.prepare(
			...
			.bind(c.get("projectId"))
			.first<{ configuration_json: string }>();
		const previous = row ? JSON.parse(row.configuration_json) : {};
		const configuration = {
			...previous,
			...body,
			...(body.platforms
				? {
						platforms: Object.fromEntries(
							Object.entries({ ...previous.platforms, ...body.platforms }).map(
								([platform, config]) => [
									platform,
									{ ...previous.platforms?.[platform], ...(config as Record<string, unknown>) },
								],
							),
						),
					}
		...
		await c.env.DB.prepare(
			"INSERT INTO link_default_redirects(project_id,configuration_json) VALUES (?,?) ON CONFLICT(project_id) DO UPDATE SET configuration_json=excluded.configuration_json,updated_at=CURRENT_TIMESTAMP",
		)
			.bind(c.get("projectId"), JSON.stringify(configuration))
			.run();
...
app.put("/internal/v1/redirect-rules/:id", async (c) =>
	mutation(c, "redirect_rule.update", async () => {
		await owned(c.env.DB, "redirect_rules", c.get("projectId"), c.req.param("id"), "redirect_rule");
		const body = await objectBody(c.req.raw);
		await c.env.DB.prepare(
...
app.delete("/internal/v1/redirect-rules/:id", async (c) =>
	mutation(c, "redirect_rule.delete", async () => {
		await owned(c.env.DB, "redirect_rules", c.get("projectId"), c.req.param("id"), "redirect_rule");
		await c.env.DB.prepare("DELETE FROM redirect_rules WHERE id=? AND project_id=?")
			.bind(c.req.param("id"), c.get("projectId"))
			.run();
		return { deleted: true };
	}),
	...
	mutation(
		...
		async () => {
			...
			await c.env.DB.prepare(
				...
				.bind(id, c.get("projectId"), hostname, token, boolean(body.is_default, false) ? 1 : 0)
				.run();
...
app.post("/internal/v1/domains/:id/verify", async (c) =>
	mutation(c, "domain.verify", async () => {
		await owned(c.env.DB, "domains", c.get("projectId"), c.req.param("id"), "domain");
		const body = await objectBody(c.req.raw);
...
app.get("/internal/v1/tracking", async (c) => {
	...
	return c.json({
		data: row
			...
			: { enabled: true, provider: "superboard", configuration: {} },
	});
});
	...
	mutation(c, "tracking.save", async () => {
		...
		await c.env.DB.prepare(
			...
			.bind(
				...
				enumValue(body.provider, ["superboard", "google", "segment", "none"], "superboard"),
	...
	mutation(
		...
		async () => {
			...
			if (!events.length || events.length > 100)
				throw new DynamicError("events_invalid", "events must contain 1 to 100 items", 422);
			const statements = events.map((value) => {
				if (!value || typeof value !== "object" || Array.isArray(value))
					throw new DynamicError("event_invalid", "Each event must be an object", 422);
...
app.get("/internal/v1/campaign-analytics", async (c) => {
	const f = filters(c.req.query());
	const [campaigns, events] = await Promise.all([
		c.env.DB.prepare(
			...
			.all<CampaignRow>(),
		c.env.DB.prepare(
			"SELECT campaign_id,event_type,COUNT(*) AS count,COALESCE(SUM(revenue_cents),0) AS revenue_cents FROM link_events WHERE project_id=? AND datetime(occurred_at)>=datetime(?) AND datetime(occurred_at)<datetime(?) AND (? IS NULL OR platform=?) AND (? IS NULL OR link_id=?) GROUP BY campaign_id,event_type",
		)
			.bind(
				c.get("projectId"),
				`${f.from}T00:00:00Z`,
	...
	const byCampaign = new Map<string, { counts: Record<string, number>; revenue: number }>();
	for (const event of events.results) {
		if (!event.campaign_id) continue;
		const aggregate = byCampaign.get(event.campaign_id) ?? { counts: {}, revenue: 0 };
		aggregate.counts[event.event_type] = Number(event.count);
		aggregate.revenue += Number(event.revenue_cents);
		byCampaign.set(event.campaign_id, aggregate);
	}
	return c.json({
		data: campaigns.results.map((campaign) => {
			const aggregate = byCampaign.get(campaign.id);
			const totals = aggregate?.counts ?? {};
			return {
				...campaignJson(campaign),
				total_views: totals.view ?? 0,
				total_opens: totals.open ?? 0,
				total_installs: totals.install ?? 0,
				total_reinstalls: totals.reinstall ?? 0,
				total_reactivations: totals.reactivation ?? 0,
				total_app_opens: totals.app_open ?? 0,
				total_user_referred: totals.user_referred ?? 0,
				total_revenue: aggregate?.revenue ?? 0,
			};
		}),
	});
});
...
function tagsJson(value: unknown): string {
	...
	if (
		!Array.isArray(value) ||
		value.length > 100 ||
		value.some((tag) => typeof tag !== "string" || tag.length > 255)
	)
		throw new DynamicError("tags_invalid", "Tags must be an array of at most 100 strings", 422);
	return JSON.stringify(value);
}
function linkJson(row: LinkRow) {
	const { destinations_json, utm_json, data_json, tags_json, hidden, ...rest } = row;
	return {
		...rest,
		active: row.active === 1,
		...
		hidden: hidden === 1,
		use_project_defaults: row.use_project_defaults === 1,
...
async function mutation(
	...
	if (!key || key.length > 255)
		throw new DynamicError("idempotency_key_required", "Idempotency-Key is required", 400);
	const p = c.get("projectId");
	...
	await c.env.DB.batch([
		c.env.DB.prepare(
			...
			p,
			action,
			c.get("projectContext").operatorId ?? String(c.get("projectContext").actorId),
...
async function ownedLink(db: D1Database, p: string, id: string) {
	const row = await db
		.prepare(
			"SELECT id,slug,name,destination_url,destinations_json,campaign_id,title,subtitle,image_url,utm_json,data_json,tags_json,hidden,use_project_defaults,active,created_at,updated_at FROM links WHERE project_id=? AND id=?",
		)
		.bind(p, id)
		.first<LinkRow>();
...
async function owned(
	...
	if (
		!(await db.prepare(`SELECT id FROM ${table} WHERE project_id=? AND id=?`).bind(p, id).first())
	)
		throw new DynamicError(`${label}_not_found`, `${label.replace("_", " ")} was not found`, 404);
}
async function assertCampaign(db: D1Database, p: string, id: string | null) {
	if (
		id &&
		!(await db.prepare("SELECT id FROM campaigns WHERE project_id=? AND id=?").bind(p, id).first())
	)
		throw new DynamicError("campaign_not_found", "Campaign was not found", 404);
}
async function count(db: D1Database, table: "links" | "campaigns" | "domains", p: string) {
	const row = await db
...
function identifier(value: unknown, field: string) {
	...
	if (!/^[a-z0-9][a-z0-9_-]*$/.test(text))
		throw new DynamicError("validation_failed", `${field} contains unsupported characters`, 422);
	return text;
}
function url(value: unknown, field: string) {
	...
	try {
		...
		throw new DynamicError("validation_failed", `${field} must be an HTTP URL`, 422);
	}
}
...
function filters(query: Record<string, string>) {
	...
	if (from > to) throw new DynamicError("date_range_invalid", "from must be before to", 422);
	return {
		from,
		to,
