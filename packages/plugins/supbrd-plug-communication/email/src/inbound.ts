...
export async function normalizeInboundEmailRequest(
	...
	const envelope = {
		...
		received_at:
			timestampValue(
				firstValue(message, ["received_at", "receivedAt", "receivedDateTime", "received"]),
			) ?? new Date().toISOString(),
		sent_at: timestampValue(
			firstValue(message, ["sent_at", "sentAt", "sentDateTime", "date"]) ?? headers.get("date"),
		),
		from: requiredAddress(
			firstValue(message, ["from", "sender", "From"]) ?? headers.get("from"),
			"from",
		),
		to: addresses(firstValue(message, ["to", "recipients", "To"]) ?? headers.get("to")),
		...
		subject: cleanHeaderValue(
			String(firstValue(message, ["subject", "Subject"]) ?? headers.get("subject") ?? ""),
		...
		thread: {
			...
			in_reply_to: nullableHeaderIdentifier(
				firstValue(message, ["in_reply_to", "inReplyTo"]) ?? headers.get("in-reply-to"),
			),
			references: references(
				firstValue(message, ["references", "thread_references"]) ?? headers.get("references"),
			),
		},
		attachments,
...
async function parseProviderBody(request: Request): Promise<InputRecord> {
	const bytes = await readBytesLimited(request, EMAIL_INBOUND_LIMITS.networkBodyBytesMaximum);
...
function messageRecord(raw: InputRecord): InputRecord {
	for (const path of ["message", "email", "data.message", "data.email", "data"]) {
...
function messageHeaders(message: InputRecord, raw: InputRecord): Map<string, string> {
	const result = new Map<string, string>();
	const candidates = [message.headers, pathValue(message, "payload.headers"), raw.headers];
	for (const candidate of candidates) {
		if (Array.isArray(candidate)) {
			...
			for (const [name, value] of Object.entries(candidate)) {
				if (typeof value !== "string" || name.length > 128 || value.length > 8_192) continue;
...
function requiredProviderMessageId(
	...
	const value =
		...
		firstValue(raw, ["provider_message_id", "providerMessageId", "event_id", "eventId"]) ??
		headers.get("message-id");
	const result = nullableHeaderIdentifier(value);
	if (!result) throw new EmailValidationError("inbound_email_message_id_required");
	return result;
}

function normalizedBody(message: InputRecord): EmailInboundEnvelope["body"] {
	const directBody = message.body;
	let text = firstValue(message, ["text", "text_body", "textBody", "body_text", "stripped-text"]);
	let html = firstValue(message, ["html", "html_body", "htmlBody", "body_html", "stripped-html"]);
	...
	if (Array.isArray(parts)) {
		for (const part of flattenParts(parts)) {
			const mime = String(part.mimeType || part.content_type || "").toLowerCase();
...
function normalizeAttachment(value: unknown, index: number): EmailInboundAttachment {
	if (!isRecord(value)) {
	...
	const fileName = safeFilename(
		String(firstValue(value, ["file_name", "fileName", "filename", "name"]) ?? id),
	);
	const contentType = cleanHeaderValue(
		String(
			firstValue(value, ["content_type", "contentType", "mime_type", "mimeType"]) ??
				"application/octet-stream",
		),
		255,
	).toLowerCase();
	const rawSize = Number(
		firstValue(value, ["byte_size", "byteSize", "size_bytes", "sizeBytes", "size"]) ?? 0,
	);
	if (!Number.isSafeInteger(rawSize) || rawSize < 0) {
		throw new EmailValidationError("inbound_email_attachment_invalid");
	}
	const rawUrl = firstValue(value, ["download_url", "downloadUrl", "content_url", "contentUrl"]);
	return {
		provider_attachment_id: id,
		file_name: fileName,
		content_type: contentType,
		byte_size: rawSize,
		content_id: nullableHeaderIdentifier(firstValue(value, ["content_id", "contentId"])),
...
function requiredAddress(value: unknown, field: string): EmailInboundAddress {
	...
	if (!result) throw new EmailValidationError(`inbound_email_${field}_required`);
	return result;
}
...
function addresses(value: unknown): EmailInboundAddress[] {
	...
	for (const item of values) {
		if (typeof item === "string") {
			...
			const email = String(firstValue(nested, ["email", "address", "email_address"]) ?? "")
				.trim()
				.toLowerCase();
			if (!email) continue;
			const name = String(firstValue(nested, ["name", "display_name", "displayName"]) ?? "").trim();
			output.push({ email, name: name || null });
		}
	}
...
function authenticity(
	...
	const source =
		...
		{};
	const results = String(headers.get("authentication-results") || "").toLowerCase();
	return {
...
function resultFromHeader(value: string, mechanism: string): string {
	return new RegExp(`(?:^|[;\\s])${mechanism}=([a-z]+)`, "u").exec(value)?.[1] ?? "unknown";
}
...
function safeMetadata(message: InputRecord, raw: InputRecord): InputRecord {
	...
	for (const field of fields) {
		...
		if (typeof value === "string" || typeof value === "number") {
			...
			output[field.replace(/[A-Z]/g, (letter) => `_${letter.toLowerCase()}`)] = normalized;
		}
	}
	return output;
}
...
function cleanText(value: string): string {
	return (
		value
			.replaceAll("\u0000", "")
			.replace(/\r\n?/gu, "\n")
			// oxlint-disable-next-line no-control-regex -- Normalize MIME control characters at the inbound message boundary.
			.replace(/[\u0001-\u0008\u000b\u000c\u000e-\u001f\u007f]/gu, "")
...
function cleanHtml(value: string): string {
	...
	const withoutActiveBlocks = cleanText(value)
		.replace(/<!--[\s\S]*?-->/gu, "")
		.replace(/<(script|style|iframe|object|embed|form|meta|link)\b[^>]*>[\s\S]*?<\/\1\s*>/giu, "")
		.replace(/<(script|style|iframe|object|embed|form|meta|link)\b[^>]*\/?\s*>/giu, "");
...
function nullableHeaderIdentifier(value: unknown): string | null {
	...
	const result = cleanHeaderValue(String(value), EMAIL_INBOUND_LIMITS.identifierLengthMaximum);
	return result || null;
}
...
export function inboundNormalizationError(error: unknown): EmailValidationError {
