...
import { RequestBodyError, readJsonLimited } from "@superboard/contracts/request-body";
import { configuredSecrets, matchesAnySecret } from "@superboard/contracts/secret";
import { EmailTransportError, sendSmtpMessage } from "@superboard/email-transport";

import {
	handleEmailAdmin,
	receiveEmailProviderWebhook,
	decryptEmailSender,
	resolveOwnedDelegatedSender,
...
import { inboundNormalizationError, normalizeInboundEmailRequest } from "./inbound";
import {
...
const lifecycleWorker = {
	async fetch(request: Request, env: Env): Promise<Response> {
		...
		if (request.method === "POST" && url.pathname.startsWith("/public/v1/provider-webhooks/"))
			return receiveEmailProviderWebhook(request, env);
		if (url.pathname.startsWith("/internal/v1/admin"))
			return handleEmailAdmin(request, env, {
				enqueue: enqueueMessage,
				smtp: deliverDelegatedSmtp,
				replayDeadLetter: replayEmailDeadLetter,
		...
		if (request.method === "POST" && url.pathname === EMAIL_SERVICE_INBOUND_NORMALIZE_PATH) {
		...
		if (url.pathname.startsWith(EMAIL_SERVICE_PROVIDER_EVENTS_PATH)) {
			...
			if (request.method === "GET" && url.pathname === EMAIL_SERVICE_PROVIDER_EVENTS_PATH) {
		...
		if (url.pathname.startsWith("/api/")) {
			if (!(await previewAuthorized(request, env))) return json({ error: "unauthorized" }, 401);
	...
	async queue(batch: MessageBatch<EmailQueueJob>, env: Env): Promise<void> {
		if (batch.queue === env.EMAIL_DLQ_NAME) {
			for (const message of batch.messages) {
				try {
					const result = await quarantineEmailDeadLetter(env.DB, batch.queue, message);
					console.error(
						JSON.stringify({
		...
		for (const message of batch.messages) {
			try {
				if (message.body?.type !== "email.deliver" || typeof message.body.messageId !== "string") {
					message.ack();
					continue;
				}
...
async function normalizeInboundEmail(request: Request, env: Env): Promise<Response> {
	const requestId = safeRequestId(request.headers.get("x-request-id"));
	if (!(await internalAuthorized(request, env))) {
		return inboundEmailError(requestId, 401, "unauthorized");
	}
	const provider = String(request.headers.get(EMAIL_INBOUND_PROVIDER_HEADER) || "").trim();
	const endpointId = String(request.headers.get(EMAIL_INBOUND_ENDPOINT_HEADER) || "").trim();
...
function safeRequestId(value: string | null): string {
	...
	return /^[A-Za-z0-9._:-]{1,128}$/u.test(provided) ? provided : crypto.randomUUID();
}

async function receiveAwsSesEvent(request: Request, env: Env): Promise<Response> {
	if (env.MAIL_PROVIDER !== "aws-ses" || !env.AWS_SES_SNS_TOPIC_ARN?.trim()) {
		return json({ error: "aws_ses_events_not_configured" }, 503);
	}
	try {
		const envelope = parseAwsSnsEnvelope(await readJsonLimited(request, 1_100_000));
...
async function acknowledgeProviderEvents(request: Request, env: Env): Promise<Response> {
...
async function enqueueMessage(
	request: Request,
	env: Env,
	sender?: EmailSenderSnapshot,
): Promise<Response> {
	if (!(await internalAuthorized(request, env))) return json({ error: "unauthorized" }, 401);
	try {
		const environment = env.ENVIRONMENT === "local" ? "development" : env.ENVIRONMENT;
		...
		const quotaClause = sender
			? `WHERE
      (? IS NULL OR (SELECT COUNT(*) FROM email_deliveries delivery JOIN email_message_senders sender ON sender.message_id = delivery.message_id WHERE sender.profile_id = ? AND julianday(delivery.created_at) >= julianday('now', '-1 hour')) + ? <= ?)
      AND (? IS NULL OR (SELECT COUNT(*) FROM email_deliveries delivery JOIN email_message_senders sender ON sender.message_id = delivery.message_id WHERE sender.profile_id = ? AND julianday(delivery.created_at) >= julianday('now', '-1 day')) + ? <= ?)`
			: "";
		const quotaBindings = sender
			? [
					sender.hourly_quota ?? null,
					sender.id,
					input.to.length,
					sender.hourly_quota ?? null,
					sender.daily_quota ?? null,
					sender.id,
					input.to.length,
					sender.daily_quota ?? null,
				]
      ...
      INSERT ${input.idempotencyKey ? "OR IGNORE " : ""}INTO email_messages
        (id,environment,transport,kind,project_id,template_key,idempotency_key,request_sha256,
         from_name,from_address,reply_to,subject,text_body,html_body,headers_json,metadata_json,status,created_at,updated_at)
      SELECT ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,? ${quotaClause}
    `).bind(
			id,
			environment,
			...
			requestSha256,
			sender?.publicConfig.from_name ?? env.MAIL_FROM_NAME,
			sender?.publicConfig.from_email ?? env.MAIL_FROM_ADDRESS,
			input.replyTo || sender?.publicConfig.reply_to || env.MAIL_REPLY_TO || null,
			input.subject,
			input.text,
			input.html,
		...
		await env.DB.batch([
			...
			...(sender
				? [
						env.DB.prepare(
							"INSERT INTO email_message_senders (message_id, profile_id, public_config_json, encrypted_config) SELECT ?, ?, ?, ? WHERE EXISTS (SELECT 1 FROM email_messages WHERE id = ?)",
						).bind(id, sender.id, JSON.stringify(sender.publicConfig), sender.encrypted_config, id),
					]
				: []),
      ...
      `,
        ...
        `,
					...
					.bind(environment, input.idempotencyKey)
					.first<StoredReceipt>()
			: {
					id,
					status,
		...
		if (!stored) {
			if (sender) return json({ error: "smtp_quota_exceeded" }, 429);
		...
		if (error instanceof EmailValidationError) return json({ error: error.code }, 422);
		console.error("email_enqueue_failed", error);
		return json({ error: "email_enqueue_failed" }, 500);
	}
}

async function deliverDelegatedSmtp(request: Request, env: Env): Promise<Response> {
	if (!(await internalAuthorized(request, env))) return json({ error: "unauthorized" }, 401);
	let input: EmailSmtpTransportRequest;
	try {
		input = parseEmailSmtpTransportRequest(
			await resolveOwnedDelegatedSender(await readJsonLimited(request, 600_000), env),
			env.ENVIRONMENT,
		);
	} catch (error) {
		if (error instanceof RequestBodyError) return json({ error: error.code }, error.status);
  ...
  `,
	...
	try {
		...
		const reason = (error instanceof Error ? error.message : String(error)).slice(0, 4_000);
		const outcomeUnknown =
			error instanceof EmailTransportError && error.code === "smtp_outcome_unknown";
		await env.DB.prepare(
			`
      UPDATE email_transport_deliveries
    ...
    `,
		...
		console.error(
			JSON.stringify({
				event: outcomeUnknown ? "email_transport_outcome_unknown" : "email_transport_failed",
				source: input.source,
    ...
    `,
			...
			.bind(result.messageId, providerResponse, reason, new Date().toISOString(), deliveryId)
			.run()
			.catch(() => undefined);
		console.error(
			JSON.stringify({
...
async function deliverMessage(env: Env, messageId: string): Promise<void> {
	if (env.MAIL_TRANSPORT !== "smtp") throw new Error("SMTP delivery received by a non-SMTP target");
	const message = await env.DB.prepare(`SELECT * FROM email_messages WHERE id = ?`)
		.bind(messageId)
		.first<EmailRow>();
	if (!message) return;
	const deliveries = (
  ...
  `,
	...
	const storedSender = await env.DB.prepare(
		"SELECT profile_id AS id, public_config_json, encrypted_config FROM email_message_senders WHERE message_id = ?",
	)
		.bind(message.id)
		.first<{ id: string; public_config_json: string; encrypted_config: string }>();
	const snapshot = storedSender
		? {
				id: storedSender.id,
				publicConfig: JSON.parse(storedSender.public_config_json),
				encrypted_config: storedSender.encrypted_config,
			}
		: undefined;
	const smtp = snapshot
		? resolveSmtpConfiguration(env, snapshot.publicConfig, await decryptEmailSender(env, snapshot))
		: smtpConfiguration(env, message);
	await env.DB.prepare(`UPDATE email_messages SET status = 'sending', updated_at = ? WHERE id = ?`)
    ...
    `,
      ...
      `,
			...
			await env.DB.batch([
				...
				).bind(result.messageId, result.response.slice(0, 4_000), reason, now, delivery.id),
				env.DB.prepare(
...
function resolveSmtpConfiguration(
	...
	if (!host || !Number.isSafeInteger(port) || port < 1 || port > 65535 || port === 25) {
...
function providerHeaders(
	...
	const normalizedTags = Object.entries(tags)
		.filter(
			([name, value]) =>
				/^[A-Za-z0-9_-]{1,64}$/u.test(name) && /^[A-Za-z0-9_-]{1,256}$/u.test(value),
		)
...
async function emailRequestSha256(input: ReturnType<typeof parseEmailMessage>): Promise<string> {
...
async function deterministicMessageId(
	...
	const digest = await sha256(`superboard-email-transport:${idempotencyKey}`);
	const domain =
		fromAddress
			.split("@")[1]
			?.toLowerCase()
			.replace(/[^a-z0-9.-]/g, "")
			.slice(0, 253) || "superboard.local";
	return `<superboard-${digest.slice(0, 48)}@${domain}>`;
}
...
function emailConfigurationHealth(env: Env) {
	...
	const smtpConfigured =
		...
		(provider === "aws-ses"
			...
			: Boolean(
					...
					new Set(["tls", "starttls", "plain"]).has(env.SMTP_SECURITY || "starttls") &&
	...
	return {
		...
		internalAuthenticationConfigured: true,
		previewProtected: env.MAIL_TRANSPORT === "capture" ? previewProtected : null,
		...
		providerEventsConfigured:
			env.MAIL_TRANSPORT === "smtp" && provider === "aws-ses" ? awsSesConfigured : null,
	};
}
...
async function listEmailOperations(env: Env, url: URL): Promise<Response> {
	const limit = boundedLimit(url.searchParams.get("limit"));
	const status = enumQuery(url, "status", ["captured", "queued", "sending", "sent", "failed"]);
...
async function replayEmailDeadLetter(env: Env, id: string): Promise<Response> {
  ...
  `,
	...
	if (row.status !== "quarantined") return json({ error: "dead_letter_already_resolved" }, 409);
	if (Number(row.replayable) !== 1) return json({ error: "dead_letter_not_replayable" }, 409);
...
async function discardEmailDeadLetter(env: Env, id: string): Promise<Response> {
  ...
  `,
		...
		.first<{ id: string }>();
	if (!row) {
		const existing = await env.DB.prepare(`SELECT status FROM email_dead_letters WHERE id = ?`)
			.bind(id)
			.first<{ status: string }>();
		return existing
...
function boundedLimit(raw: string | null): number {
	...
	return Number.isSafeInteger(parsed) && parsed > 0 ? Math.min(100, parsed) : 50;
}

function enumQuery<T extends string>(url: URL, name: string, values: T[]): T | null {
...
function serializeEmailDeadLetter(row: EmailDeadLetterRow): EmailDeadLetterOperation {
	return {
		id: row.id,
...
function serializeEmailTransport(row: EmailTransportOperationRow): EmailTransportOperation {
	return {
		id: row.id,
		source: row.source,
...
function serializeProviderEvent(row: EmailProviderEventRow): EmailProviderEvent {
	return {
		id: row.id,
		provider: "aws-ses",
		source: row.source,
...
async function readMessage(env: Env, id: string): Promise<Response> {
	...
	const deliveries = (
		await env.DB.prepare(`SELECT * FROM email_deliveries WHERE message_id = ? ORDER BY recipient`)
			.bind(id)
			.all()
	).results;
...
async function previewAuthorized(request: Request, env: Env): Promise<boolean> {
	...
	return Boolean(
		env.MAIL_PREVIEW_TOKEN && provided && (await secretsEqual(provided, env.MAIL_PREVIEW_TOKEN)),
	);
}
...
export default withPluginTaskLifecycle("supbrd-plugmod-email", lifecycleWorker, { fetch: true });
