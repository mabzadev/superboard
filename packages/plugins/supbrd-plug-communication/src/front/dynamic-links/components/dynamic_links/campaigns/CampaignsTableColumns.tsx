...
import {
	...
	TooltipContent,
	TooltipTrigger,
} from "../../../../../../../../supbrd-front-ui/src/shared/components/ui/tooltip.js";
import { formatSlashDate } from "../../../../../../../../supbrd-front-ui/src/shared/lib/dateUtils.js";
import type { Campaign } from "../../../../../../../../supbrd-front-ui/src/shared/types/index.js";
...
import { checkSortDirection } from "../links/LinksTableColumns.js";
...
const renderToolTipContent = (key: string) => {
	...
	switch (key) {
		case "Views":
			...
			break;
		case "Opens":
			text = "Represents the number of times the mobile apps were launched from campaign links.";
			break;
		case "Installs":
			text = "Indicates the number of app installations resulting from campaign links.";
			break;
		case "Reinstalls":
			text = "Indicates the number of app reinstallations resulting from campaign links.";
			break;
		case "Reactivations":
			text = "Indicates the number of app reactivations resulting from campaign links.";
			break;
		case "Users Referred":
			text =
...
export const createCampaignsTableColumns = (
	...
	},
	{
		...
		cell: ({ row }) => {
			return (
				<div>
					<p>{row.original.total_views ? numberFormatter.format(row.original.total_views) : "-"}</p>
				</div>
			);
		},
	},
	{
		accessorKey: "opens",
		header: () => renderSortableHeader("Opens", "app_opens", setSort, sort, true),

		cell: ({ row }) => {
			return (
				<div>
					<p>{row.original.total_opens ? numberFormatter.format(row.original.total_opens) : "-"}</p>
				</div>
			);
		},
	},
	{
		...
		header: () => renderSortableHeader("Installs", "installs", setSort, sort, true),
	...
	},
	{
		...
		header: () => renderSortableHeader("Reinstalls", "reinstalls", setSort, sort, true),
		cell: ({ row }) => {
			return (
				<div>
					<p>
	...
	},
	{
		...
		header: () => renderSortableHeader("Reactivations", "reactivations", setSort, sort, true),
		cell: ({ row }) => {
			return (
				<div>
					<p>
	...
	},
	{
		...
		header: () => renderSortableHeader("App opens", "app_opens", setSort, sort, true),
		cell: ({ row }) => {
			return (
				<div>
					<p>
	...
	},
	{
		...
		header: () => renderSortableHeader("Users Referred", "user_referred", setSort, sort, true),
		cell: ({ row }) => {
			return (
				<div>
					<p>
	...
	},
	{
		...
		cell: ({ row }: { row: { original: Campaign } }) => {
			return (
				<div>
					<p>
						{row.original.total_revenue ? formatCurrencyFromCents(row.original.total_revenue) : "-"}
					</p>
				</div>
			);
		},
...
export type { Campaign } from "../../../../../../../../supbrd-front-ui/src/shared/types/index.js";
