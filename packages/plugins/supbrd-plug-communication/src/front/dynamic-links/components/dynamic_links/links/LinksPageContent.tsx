...
import {
	adsFilterList,
	platformsFilterList,
} from "../../../../../../../../supbrd-front-ui/src/shared/constants/FilterOptions.js";
import {
	ACTIVE,
	ARCHIVED,
...
export default function LinksPageContent({ campaignId }: { campaignId?: string }) {
	...
	return (
		<ModulePage
			title={campaignId ? (campaign?.name ?? "Campaign links") : "Links"}
					...
					<div className="grid gap-5 py-2 sm:grid-cols-2">
						<section className="space-y-3">
							...
							<Field
								...
								value={form.slug}
								onChange={(slug) =>
									setForm((v) => ({ ...v, slug: slug.toLowerCase().replace(/[^a-z0-9_-]/g, "-") }))
								}
							/>
							<Field
								...
								onChange={(destination) => setForm((v) => ({ ...v, destination }))}
							/>
							<label className="space-y-2 text-sm">
								...
								<Select
									...
									onValueChange={(campaignId) =>
										setForm((v) => ({ ...v, campaignId: campaignId === "none" ? "" : campaignId }))
									}
								>
									<SelectTrigger>
							...
							<Field
								...
								value={form.tags}
								onChange={(tags) => setForm((v) => ({ ...v, tags }))}
							/>
						...
						<section className="space-y-3">
							...
							<Field
								...
								type="url"
								value={form.ios}
								onChange={(ios) => setForm((v) => ({ ...v, ios }))}
							/>
							<Field
								label="Android destination"
								type="url"
								value={form.android}
								onChange={(android) => setForm((v) => ({ ...v, android }))}
							/>
							<Field
								label="Web destination"
								type="url"
								value={form.web}
								onChange={(web) => setForm((v) => ({ ...v, web }))}
							/>
							<Field
								label="Desktop destination"
								type="url"
								value={form.desktop}
								onChange={(desktop) => setForm((v) => ({ ...v, desktop }))}
							/>
						</section>
						<section className="space-y-3">
							...
							<Field
								label="Description"
								value={form.subtitle}
								onChange={(subtitle) => setForm((v) => ({ ...v, subtitle }))}
							/>
							<Field
								label="Image URL"
								type="url"
								value={form.imageUrl}
								onChange={(imageUrl) => setForm((v) => ({ ...v, imageUrl }))}
							/>
						</section>
						<section className="space-y-3">
							...
							<Field
								label="Source"
								value={form.source}
								onChange={(source) => setForm((v) => ({ ...v, source }))}
							/>
							<Field
								label="Medium"
								value={form.medium}
								onChange={(medium) => setForm((v) => ({ ...v, medium }))}
							/>
							<Field
								label="Campaign"
								value={form.trackingCampaign}
								onChange={(trackingCampaign) => setForm((v) => ({ ...v, trackingCampaign }))}
							/>
						</section>
					</div>
