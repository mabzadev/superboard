...
export async function getEmailSubscriberPage(
	projectRef: string,
	query: string,
	offset = 0,
	listId = "",
	...
	return data<EmailSubscriber[]>(
		await GET(path(projectRef, "/email/subscribers") + "?" + parameters),
	);
}
...
export async function getEmailSubscriberExport(projectRef: string, id: string) {
	return data<SubscriberExport>(await GET(path(projectRef, `/email/subscribers/${id}/export`)));
}
export async function createEmailSubscriber(projectRef: string, payload: Record<string, unknown>) {
	return data<EmailSubscriber>(await POST(path(projectRef, "/email/subscribers"), payload));
}
export async function updateEmailSubscriber(
	...
	return data<EmailSubscriber>(await PATCH(path(projectRef, `/email/subscribers/${id}`), payload));
}
...
export async function createSubscriberList(projectRef: string, payload: Record<string, unknown>) {
...
export async function updateSubscriberList(
	...
	return data<SubscriberList>(await PATCH(path(projectRef, `/lists/${id}`), payload));
}
...
export async function addSubscriberToList(
	...
	return data(await PUT(path(projectRef, `/lists/${listId}/subscribers/${subscriberId}`), {}));
}
export async function removeSubscriberFromList(
	...
	return data(await DELETE(path(projectRef, `/lists/${listId}/subscribers/${subscriberId}`)));
}
...
export async function createSubscriberSegment(
	...
	return data<SubscriberSegment>(await POST(path(projectRef, "/segments"), payload));
}
export async function updateSubscriberSegment(
	...
	return data<SubscriberSegment>(await PATCH(path(projectRef, `/segments/${id}`), payload));
}
...
export async function createEmailTemplate(projectRef: string, payload: Record<string, unknown>) {
	return data<EmailTemplate>(await POST(path(projectRef, "/templates"), payload));
}
export async function updateEmailTemplate(
	...
	return data<EmailTemplate>(await PATCH(path(projectRef, `/templates/${id}`), payload));
}
...
export async function downloadMarketingMedia(projectRef: string, id: string) {
	return (await GET(path(projectRef, `/media/${id}`), { responseType: "blob" })).data as Blob;
}
...
export async function createEmailCampaign(projectRef: string, payload: Record<string, unknown>) {
	return data<EmailCampaign>(await POST(path(projectRef, "/campaigns"), payload));
}
export async function updateEmailCampaign(
	...
	return data<EmailCampaign>(await PATCH(path(projectRef, `/campaigns/${id}`), payload));
}
...
export async function scheduleEmailCampaign(projectRef: string, id: string, scheduledAt: string) {
	return data<EmailCampaign>(
...
export async function testEmailCampaign(projectRef: string, id: string, recipient: string) {
...
export async function getMarketingStatistics(
	...
	return data<MarketingStatistics>(
		await GET(`${path(projectRef, "/statistics")}${query.size ? `?${query}` : ""}`),
	);
}
...
export async function getMarketingDeadLetters(projectRef: string) {
	return data<MarketingDeadLetter[]>(await GET(path(projectRef, "/settings/dead-letters")));
}
export async function replayMarketingDeadLetter(projectRef: string, id: string) {
	return data<{ id: string; status: "replayed"; job_type: string }>(
		await POST(path(projectRef, `/settings/dead-letters/${id}/replay`), {}),
	);
}
export async function discardMarketingDeadLetter(projectRef: string, id: string) {
...
export async function getMarketingAudit(projectRef: string) {
	return data<Array<Record<string, unknown>>>(await GET(path(projectRef, "/audit")));
}
...
export async function createMarketingJourney(projectRef: string, payload: MarketingJourneyInput) {
	return data<MarketingJourney>(await POST(path(projectRef, "/journeys"), payload));
}
...
export async function updateMarketingJourney(
	...
	return data<MarketingJourney>(await PATCH(path(projectRef, `/journeys/${id}`), payload));
}
...
export async function transitionMarketingJourney(
	...
	return data<MarketingJourney>(await POST(path(projectRef, `/journeys/${id}/${transition}`), {}));
}

export async function getJourneyEnrollments(projectRef: string, id: string) {
	return data<JourneyEnrollment[]>(await GET(path(projectRef, `/journeys/${id}/enrollments`)));
}
...
export async function getJourneyStatistics(projectRef: string, id: string) {
	return data<JourneyStatistics>(await GET(path(projectRef, `/journeys/${id}/statistics`)));
}

export async function getMarketingChannelConnectors(projectRef: string) {
	return data<MarketingChannelConnector[]>(await GET(path(projectRef, "/channel-connectors")));
}
...
export async function deleteMarketingChannelConnector(projectRef: string, id: string) {
	return data<{ deleted: boolean }>(await DELETE(path(projectRef, `/channel-connectors/${id}`)));
}
