...
import { forwardMigratedEmailProfiles, migrateEmailProfiles } from "./email-profile-migration.js";
import {
...
import {
	emailStudioRoutes,
	saveEmailStudioTemplate,
	snapshotEmailCampaign,
	resolveStoredEmail,
	getPublishedEmailTemplate,
...
app.get("/internal/v1/health", async (c) => {
	try {
		const schema = await inspectSqlDatabaseAndSchemaHealth(c.env.DB, c.env.D1_EXPECTED_MIGRATION);
  ...
  `,
	...
	return {
		...
		senderAuthentication: {
			...
			ready: value("smtp_profiles_enabled") > 0 && value("smtp_profiles_unverified") === 0,
		},
		deadLetters: {
...
app.get("/public/v1/tracking/open/:token", async (c) => {
	const payload = await verifyTrackingToken(c.env, c.req.param("token"), "open");
...
app.get("/public/v1/tracking/click/:token", async (c) => {
	const payload = await verifyTrackingToken(c.env, c.req.param("token"), "click");
...
app.all("/public/v1/tracking/unsubscribe/:token", async (c) => {
	...
	const payload = await verifyTrackingToken(c.env, c.req.param("token"), "unsubscribe");
	const subscriberRow = await subscriber(c.env.DB, payload.projectId, payload.subscriberId);
...
app.all("/public/v1/opt-in/:token", async (c) => {
  ...
  `,
	...
	if (!confirmed)
		throw failure("confirmation_invalid", "Confirmation token is invalid or expired", 422);
  ...
  `,
	...
	try {
      ...
      `,
				...
				project.projectId,
				`${c.req.method} ${path}`,
				project.operatorId ?? project.actorId,
				project.requestId,
...
app.get("/internal/v1/application/preferences", async (c) => {
	...
	return c.json({
		data: await applicationPreferences(c.env.DB, c.get("project").projectId, identity),
	});
});
...
app.put("/internal/v1/application/preferences", async (c) => {
	...
	if (requestedLists.length > 50) {
		throw failure("list_ids_invalid", "list_ids must contain at most 50 public lists");
	}
	...
	if (selectedLists.length !== requestedLists.length) {
		throw failure("list_ids_invalid", "Every requested list must be a public list in this project");
	}
	...
	const [byUser, byEmail, suppression] = await Promise.all([
		c.env.DB.prepare(
			...
			.first<{ id: string }>(),
		c.env.DB.prepare("SELECT id FROM subscribers WHERE project_id = ? AND email = ? LIMIT 1")
			.bind(projectId, identity.email)
			.first<{ id: string }>(),
		c.env.DB.prepare("SELECT reason FROM suppressions WHERE project_id = ? AND email = ? LIMIT 1")
			.bind(projectId, identity.email)
			.first<{ reason: string }>(),
	]);
...
app.delete("/internal/v1/application/users/:userId", async (c) => {
	...
	const erasedAddress = `erased+${(await sha256(`${projectId}:${identity.userId}`)).slice(0, 32)}@invalid.superboard`;
	const now = new Date().toISOString();
...
app.post("/internal/v1/email/subscribers", async (c) => {
	...
	const row = {
		...
		consent_source: optionalText(body.consent_source, "consent_source", 128) || "admin",
	...
	const existing = await c.env.DB.prepare(
		...
		.first<{ id: string }>();
	if (existing)
		throw failure("subscriber_conflict", "A subscriber with this email already exists", 409);
	await c.env.DB.batch([
		c.env.DB.prepare(
			`
      INSERT INTO subscribers
...
app.get("/internal/v1/email/subscribers/:subscriberId", async (c) => {
	return c.json({
		data: serializeSubscriber(
			await subscriber(c.env.DB, c.get("project").projectId, c.req.param("subscriberId")),
		),
	});
});
...
app.get("/internal/v1/email/subscribers/:subscriberId/export", async (c) => {
	const projectId = c.get("project").projectId;
	const row = await subscriber(c.env.DB, projectId, c.req.param("subscriberId"));
...
app.patch("/internal/v1/email/subscribers/:subscriberId", async (c) => {
	const projectId = c.get("project").projectId;
	const current = await subscriber(c.env.DB, projectId, c.req.param("subscriberId"));
	const body = await readJsonObject(c.req.raw);
	const address = body.email == null ? String(current.email) : email(body.email);
	const name = body.name === undefined ? current.name : optionalText(body.name, "name", 255);
	const attributes =
  ...
  `,
		...
		.run();
	return c.json({
		data: serializeSubscriber(await subscriber(c.env.DB, projectId, String(current.id))),
	});
});
...
app.post("/internal/v1/email/subscribers/:subscriberId/blocklist", async (c) => {
...
app.post("/internal/v1/email/subscribers/:subscriberId/unsubscribe", async (c) => {
	return suppressSubscriber(
		c.env,
		c.get("project"),
		c.req.param("subscriberId"),
...
app.delete("/internal/v1/email/subscribers/:subscriberId", async (c) => {
	...
	await c.env.DB.batch([
		...eraseStudioRecipientStatements(c.env.DB, projectId, c.req.param("subscriberId")),
		c.env.DB.prepare("DELETE FROM subscribers WHERE project_id=? AND id=?").bind(
			projectId,
			c.req.param("subscriberId"),
		),
	]);
...
app.post("/internal/v1/email/subscribers-import", async (c) => {
	...
	if (!Array.isArray(body.subscribers) || body.subscribers.length > 1000)
		throw failure("subscribers_invalid", "subscribers must contain at most 1000 records");
	...
	const updated = addresses.filter((address) => existingAddresses.has(address)).length;
...
app.post("/internal/v1/lists", async (c) => {
  ...
  `,
		...
		.bind(
			...
			enumValue(body.visibility, "visibility", ["private", "public"], "private"),
...
app.delete("/internal/v1/lists/:listId/subscribers/:subscriberId", async (c) => {
	await c.env.DB.prepare(
		"DELETE FROM subscriber_list_memberships WHERE project_id = ? AND list_id = ? AND subscriber_id = ?",
	)
		.bind(c.get("project").projectId, c.req.param("listId"), c.req.param("subscriberId"))
		.run();
...
app.patch("/internal/v1/templates/:templateId", async (c) => {
  ...
  `,
		...
		body.template_type === undefined
			...
			: enumValue(body.template_type, "template_type", ["campaign", "transactional", "system"]),
		body.subject === undefined ? current.subject : optionalText(body.subject, "subject", 500),
...
app.post("/internal/v1/media", async (c) => {
	...
	const contentType = String(c.req.header("content-type") || "application/octet-stream").slice(
		0,
		128,
	);
	if (!/^(image|video|audio)\//.test(contentType) && contentType !== "application/pdf")
	...
	try {
		await c.env.DB.prepare(
			...
			.bind(
				...
				contentType,
				bytes.length,
				project.operatorId ?? String(project.actorId),
			)
			.run();
	} catch (error) {
...
app.get("/internal/v1/media/:mediaId", async (c) => {
	const row = await media(c.env.DB, c.get("project").projectId, c.req.param("mediaId"));
...
app.delete("/internal/v1/media/:mediaId", async (c) => {
	...
	await c.env.DB.prepare("DELETE FROM marketing_media WHERE project_id = ? AND id = ?")
		.bind(projectId, row.id)
		.run();
	return c.json({ data: { deleted: true } });
});
...
app.get("/internal/v1/campaigns/:campaignId", async (c) => {
	return c.json({
		data: serializeCampaign(
			await campaign(c.env.DB, c.get("project").projectId, c.req.param("campaignId")),
		),
	});
});
...
app.post("/internal/v1/campaigns", async (c) => {
  ...
  `,
		...
		.run();
	return c.json({ data: serializeCampaign(await campaign(c.env.DB, projectId, id)) }, 201);
});

app.patch("/internal/v1/campaigns/:campaignId", async (c) => {
	const projectId = c.get("project").projectId;
	const current = await campaign(c.env.DB, projectId, c.req.param("campaignId"));
  ...
  `,
		...
		.bind(
			body.name === undefined ? current.name : text(body.name, "name"),
			body.subject === undefined ? current.subject : text(body.subject, "subject", 500),
		...
		.run();
	return c.json({
		data: serializeCampaign(await campaign(c.env.DB, projectId, String(current.id))),
	});
});
...
app.post("/internal/v1/campaigns/:campaignId/test", async (c) => {
	...
	const profile = await smtpProfileForTest(
		c.env.DB,
		projectId,
		optionalText(body.smtp_profile_id, "smtp_profile_id") || String(current.smtp_profile_id || ""),
	);
...
app.post("/internal/v1/transactional", async (c) => {
	...
	const published = savedDesign
		? await getPublishedEmailTemplate(c.env.DB, project.projectId, String(templateRow.id))
		: null;
	if (savedDesign && !published)
		throw failure(
	...
	const essential =
		messageDesign !== null && !["campaign", "lifecycle"].includes(messageDesign.purpose);
	const renderContext = {
		request_locale: typeof body.locale === "string" ? body.locale : undefined,
		event: jsonObject(body.event, "event"),
		profile: {
			...parseStoredJson<Record<string, unknown>>(subscriberRow?.attributes_json, {}),
			...jsonObject(body.attributes, "attributes"),
			name: subscriberRow?.name ?? body.name ?? "",
			email: recipient,
		},
	...
	if (messageDesign) {
		...
		if (issues.length)
			throw failure(
				...
				"Approve the fallback translation before sending.",
				422,
				{ issues },
			);
		const resolved = resolveEmailMessage(messageDesign, renderContext);
		if (resolved.status !== "ready")
			throw failure("EMAIL_TRANSLATION_MISSING", "No approved translation is available.", 422);
	...
	const suppressed = await c.env.DB.prepare(
		...
		.bind(project.projectId, recipient)
		.first();
	if (suppressed && !(essential && suppressed.reason === "unsubscribe"))
	...
	await snapshotEmailCampaign(
		c.env.DB,
		project.projectId,
		campaignId,
		String(templateRow.id),
		messageDesign ? { revision: published!.revision, document: messageDesign } : undefined,
	);

	await c.env.DB.prepare(
		"INSERT INTO email_studio_contexts (project_id,campaign_id,context_json) VALUES (?,?,?)",
	)
		.bind(project.projectId, campaignId, JSON.stringify(renderContext))
		.run();
...
app.use("/internal/v1/settings/smtp*", forwardMigratedEmailProfiles);
app.post("/internal/v1/settings/smtp/migrate-to-email", migrateEmailProfiles);
		...
		data:
			profiles.length === 1
				? {
						...
						aws_region: c.env.EMAIL_PROVIDER === "aws-ses" ? c.env.AWS_REGION : null,
					}
				: {
						profiles,
...
app.delete("/internal/v1/settings/smtp/:profileId", async (c) => {
	const result = await c.env.DB.prepare(
		...
		.first();
	if (!result) throw failure("smtp_profile_not_found", "SMTP profile not found", 404);
	return c.json({ data: { deleted: true } });
});
...
app.delete("/internal/v1/settings/provider-webhooks/:endpointId", async (c) => {
	const result = await c.env.DB.prepare(
		...
		.first();
	if (!result)
		throw failure("provider_webhook_not_found", "Provider webhook endpoint not found", 404);
	return c.json({ data: { deleted: true } });
});
...
app.post("/internal/v1/settings/dead-letters/:deadLetterId/replay", async (c) => {
	const projectId = c.get("project").projectId;
  ...
  `,
	...
	if (!row) throw failure("dead_letter_not_found", "Dead letter not found", 404);
	if (row.status !== "quarantined")
		throw failure("dead_letter_already_resolved", "Dead letter is already resolved", 409);
	...
	if (!job) throw failure("dead_letter_payload_invalid", "Dead letter payload is invalid", 409);
	const now = new Date().toISOString();
  ...
  `,
    ...
    `,
		...
		throw failure("dead_letter_replay_failed", "Dead letter could not be queued", 503);
	}
	return c.json({ data: { id, status: "replayed", job_type: job.type } }, 202);
});

app.post("/internal/v1/settings/dead-letters/:deadLetterId/discard", async (c) => {
	const projectId = c.get("project").projectId;
	const id = c.req.param("deadLetterId");
...
app.post("/internal/v1/settings/smtp/:profileId/verify-domain", async (c) => {
  ...
  `,
		...
		.first<Record<string, unknown>>();
	if (!profile) throw failure("smtp_profile_not_found", "SMTP profile not found", 404);
	const publicConfig = parseStoredJson<SmtpPublicConfig>(
		profile.public_config_json,
		{} as SmtpPublicConfig,
	);
    ...
    `,
			...
			.bind(new Date().toISOString(), new Date().toISOString(), profile.id, projectId)
			.run();
		throw failure(
...
app.post("/public/v1/provider-webhooks/:endpointId", async (c) => {
	...
	if (
		!(await sharedSecretsEqual(String(c.req.header("x-webhook-secret") || ""), configured.secret))
	) {
		throw failure(
...
async function processProviderEvent(
	...
	const providerEventId =
		optionalText(body.provider_event_id ?? metadata.provider_event_id, "provider_event_id", 255) ||
  ...
  `,
		...
		.first();
	if (!claim) return Response.json({ data: { accepted: true, duplicate: true } }, { status: 202 });
	try {
...
app.get("/internal/v1/statistics", async (c) => {
	const projectId = c.get("project").projectId;
	const from = isoTimestamp(c.req.query("from") || "1970-01-01T00:00:00.000Z", "from", false)!;
	const to = isoTimestamp(c.req.query("to") || new Date().toISOString(), "to", false)!;
	...
	totals.delivery_rate = percentage(totals.delivered || totals.sent || 0, totals.sent || 0);
	totals.open_rate = percentage(totals.open || 0, totals.delivered || totals.sent || 0);
	totals.click_rate = percentage(totals.click || 0, totals.delivered || totals.sent || 0);
	return c.json({
		data: {
			totals,
...
app.post("/internal/v1/settings/delivery-outbox/:outboxId/retry", async (c) => {
  ...
  `,
		...
		.first<Record<string, unknown>>();
	if (!row) throw failure("outbox_not_retryable", "Delivery outbox item is not retryable", 409);
	...
	c.json(
		{
			error: {
				...
				request_id: c.req.header("x-request-id") || c.req.header("cf-ray") || crypto.randomUUID(),
				retryable: false,
			},
		},
		404,
...
async function recordTrackingEvent(
	...
	const suffix = eventType === "click" ? `:${await sha256(String(metadata.target || ""))}` : "";
	await db
		.prepare(
			`
...
async function saveSmtpProfile(c: MarketingContext) {
	...
	const defaultProfile =
		body.id == null
			? await c.env.DB.prepare(
					...
					.first<{ id: string }>()
			: null;
	const profileId = optionalText(body.id, "id") || defaultProfile?.id || crypto.randomUUID();
	const existing = await c.env.DB.prepare(
	...
	if (managedAwsSes && !/^[a-z]{2}(?:-gov)?-[a-z]+-\d$/u.test(awsRegion)) {
		throw failure("aws_region_invalid", "AWS SES region is not configured", 503);
	}
	const publicConfig: SmtpPublicConfig = {
		...
		security: managedAwsSes
			? "starttls"
			: enumValue(body.security, "security", ["tls", "starttls", "plain"], "starttls"),
		username: managedAwsSes ? null : optionalText(body.username, "username", 320),
	...
	if (c.env.ENVIRONMENT === "production" && publicConfig.security === "plain") {
		throw failure("smtp_security_invalid", "Plain SMTP is forbidden in production");
	}
	const dkimSelector = optionalText(body.dkim_selector, "dkim_selector", 63);
	if (dkimSelector && !/^[a-z0-9][a-z0-9_-]*$/i.test(dkimSelector)) {
		throw failure("dkim_selector_invalid", "DKIM selector contains invalid characters");
	}
  ...
  `,
		...
		.bind(
			...
			booleanValue(body.enabled, true) ? 1 : 0,
			body.hourly_quota == null ? null : positiveInt(body.hourly_quota, "hourly_quota"),
			body.daily_quota == null ? null : positiveInt(body.daily_quota, "daily_quota"),
			dkimSelector,
		)
		.run();
	const profile = await c.env.DB.prepare(
      ...
      last_tested_at, last_test_status, created_at, updated_at FROM smtp_profiles WHERE project_id = ? AND id = ?`,
		...
		.bind(projectId, profileId)
		.first<Record<string, unknown>>();
	return c.json({ data: serializeSmtpProfile(profile || {}) }, existing ? 200 : 201);
}
...
async function suppressSubscriber(
	...
	await env.DB.batch([
		...
		env.DB.prepare(
			...
			current.email,
			reason,
			JSON.stringify({ actor_id: project.operatorId ?? project.actorId }),
		),
	]);
	return Response.json({
		data: serializeSubscriber(await subscriber(env.DB, project.projectId, subscriberId)),
	});
}
...
function applicationIdentity(c: MarketingContext) {
	...
	const userId = String(c.req.header("x-superboard-application-user-id") || "").trim();
	if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,254}$/.test(userId)) {
		throw failure("application_identity_invalid", "Application identity is invalid", 401);
	}
	return {
		userId,
		email: email(c.req.header("x-superboard-application-email")),
		name: optionalText(c.req.header("x-superboard-application-name"), "name", 120),
	};
}
...
async function applicationPreferences(
	...
	const lists = await db
       ...
       ORDER BY list.name, list.id`,
		...
		.all<Record<string, unknown>>();
	const consented = subscriber?.status === "enabled" && subscriber?.consent_status === "confirmed";
	return {
		consented,
...
async function transitionCampaign(
	...
	if (["scheduled", "running"].includes(to)) {
		...
		await snapshotEmailCampaign(
			...
			project.projectId,
			campaignId,
			current.template_id ? String(current.template_id) : null,
		);
	}
  ...
  `,
		...
		.bind(to, values.scheduled_at || null, to, to, to, project.projectId, campaignId, ...from)
		.first<Record<string, unknown>>();
	if (!updated)
		throw failure("campaign_transition_invalid", `Campaign cannot transition to ${to}`, 409);
...
async function refreshSegment(
	...
	await db
		.prepare("DELETE FROM segment_memberships WHERE project_id = ? AND segment_id = ?")
...
function validateSegmentRules(value: unknown): SegmentRules {
	...
	if (
		...
		throw failure("segment_conditions_invalid", "A segment requires between 1 and 25 conditions");
...
async function segment(db: D1Database, projectId: number, id: string) {
	const row = await db
		.prepare("SELECT * FROM subscriber_segments WHERE project_id = ? AND id = ?")
		.bind(projectId, id)
		.first<Record<string, unknown>>();
...
async function smtpProfileForTest(db: D1Database, projectId: number, id: string) {
	const row = id
		? await db
				.prepare("SELECT * FROM smtp_profiles WHERE project_id = ? AND id = ? AND enabled = 1")
				.bind(projectId, id)
				.first<Record<string, unknown>>()
		: await db
				.prepare(
				...
				.bind(projectId)
				.first<Record<string, unknown>>();
	if (!row) throw failure("smtp_profile_not_found", "Enabled SMTP profile not found", 404);
	return row;
}
...
async function deliveryFromProvider(
	...
	const row = deliveryId
		? await db
				.prepare("SELECT * FROM email_deliveries WHERE project_id = ? AND id = ?")
				.bind(projectId, deliveryId)
				.first<Record<string, unknown>>()
		: await db
				.prepare("SELECT * FROM email_deliveries WHERE project_id = ? AND provider_message_id = ?")
				.bind(projectId, providerMessageId)
				.first<Record<string, unknown>>();
	if (!row) throw failure("delivery_not_found", "Email delivery not found", 404);
	return row;
}
...
function serializeTemplate(row: Record<string, unknown>) {
	...
	return {
		...value,
		studio_revision: row.studio_revision ?? 0,
		studio_document: parseStoredJson(studio_document_json, null),
	};
}
function serializeSmtpProfile(row: Record<string, unknown>) {
	...
	return {
		...
		name: row.name,
		...configuration,
		provider: /^email-smtp\.[a-z0-9-]+\.amazonaws\.com(?:\.cn)?$/u.test(host) ? "aws-ses" : "smtp",
...
function enumValue<T extends string>(
	...
	const parsed = value == null || value === "" ? fallback : (String(value) as T);
	if (!parsed || !allowed.includes(parsed))
		throw failure(`${field}_invalid`, `${field} must be one of ${allowed.join(", ")}`);
	return parsed;
}
...
async function sharedSecretsEqual(provided: string, expected: string) {
	...
	const challenge = new TextEncoder().encode("superboard-provider-webhook-v1");
...
async function readBytesLimited(request: Request, maxBytes: number) {
	...
	if (announced > maxBytes) throw failure("media_too_large", "Media is limited to 10 MB", 413);
...
function parseMarketingDeadLetterJob(
	...
	try {
		...
		return isMarketingQueueJob(parsed) && parsed.projectId === projectId ? parsed : null;
	} catch {
		return null;
	}
}
...
export default withPluginTaskLifecycle("supbrd-plugmod-marketing", lifecycleWorker, {
	fetch: true,
});
