...
export async function exportChatwoot({
	...
	const conversations = await paged(
		(page) =>
			requestJson(
				`/api/v1/accounts/${account}/conversations?assignee_type=all&status=all&page=${page}`,
			),
		(payload) => listFromPayload(payload),
	);
	const configuration = {};
				...
				for (const [position, attachment] of files.entries()) {
					...
					if (sourceUrl) {
						const filename = safeFilename(
							copy.file_name ?? copy.filename ?? copy.name ?? `attachment-${position + 1}`,
						);
						...
						copy._superboard_export = { relative_path: relativePath, ...downloaded };
						attachmentCount += 1;
					}
					annotated.push(copy);
				}
...
export function chatwootOriginFromTarget(target) {
	const definitions = (target?.publicSurfaceMonitors ?? []).filter(
...
async function messagesAfter(requestJson, accountId, conversationId) {
	...
	for (let page = 0; page < 100_000; page += 1) {
		...
		if (next <= after)
			throw new Error(`Chatwoot message pagination stalled for conversation ${conversationId}`);
		after = next;
	}
	throw new Error(
		`Chatwoot message pagination exceeded its limit for conversation ${conversationId}`,
	);
}
...
async function chatwootJson(fetchImpl, base, accessToken, path) {
	...
	if (url.origin !== base.origin)
		throw new Error("Chatwoot API path escaped its configured origin");
	...
	if (!response.ok)
		throw new Error(`Chatwoot API ${url.pathname} returned HTTP ${response.status}`);
	try {
	...
	for (let attempt = 0; attempt < 5; attempt += 1) {
		...
		if (attempt < 4)
			await new Promise((resolveDelay) =>
				setTimeout(resolveDelay, Math.min(250 * 2 ** attempt, 2_000)),
			);
	}
	if (!response) {
...
function validateAttachmentUrl(url, allowedHosts) {
	...
	if (
		/^(?:localhost|127\.|0\.|10\.|192\.168\.|169\.254\.|172\.(?:1[6-9]|2\d|3[01])\.)/u.test(
			url.hostname,
		)
	) {
...
function attachmentUrl(attachment) {
	for (const field of ["data_url", "file_url", "download_url", "url"]) {
		if (typeof attachment[field] === "string" && attachment[field].startsWith("https://"))
...
function safeFilename(value) {
	const normalized = String(value || "attachment")
		.normalize("NFKC")
		// oxlint-disable-next-line no-control-regex -- Remove null bytes and path separators from exported filenames.
		.replace(/[\\/\0\r\n]/gu, "_")
...
async function main() {
	...
	const result = await exportChatwoot({
		...
		attachmentHosts: String(process.env.CHATWOOT_ATTACHMENT_HOSTS || "")
			.split(",")
			.filter(Boolean),
	});
