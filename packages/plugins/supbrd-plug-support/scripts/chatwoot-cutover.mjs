...
export async function buildChatwootCutover({ bundleDirectory, target, environment, projectId }) {
	if (target.features?.support !== true)
		throw new Error("SuperBoard Support must be enabled for Chatwoot cutover");
...
export async function renderArtifacts(result, outputDirectory, targetName, environment) {
	...
	const plan = {
		...
		artifacts: {
			...
			r2_uploads: {
				path: "r2-uploads.json",
				sha256: hash(`${JSON.stringify(uploads, null, 2)}\n`),
			},
		},
...
async function main(argv = process.argv.slice(2)) {
	...
	if (command === "plan") {
		process.stdout.write(
			`${JSON.stringify(publicPlan(result, targetName, environment), null, 2)}\n`,
		);
