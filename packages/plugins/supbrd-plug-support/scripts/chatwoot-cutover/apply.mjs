...
import { cloudflareEnv, root } from "../../../../../scripts/cloudflare/target.mjs";
import { sha256File } from "../../../../../scripts/database/d1-backup.mjs";
import { parseD1Rows } from "../../../../../scripts/database/module-cutover-adapters.mjs";
import { readJson, writeJsonAtomic } from "../../../../../scripts/database/module-cutover-core.mjs";
...
export function validateChatwootApplySafety({
	...
	if (
		...
		!/^[a-zA-Z0-9._-]{8,128}$/u.test(String(window.window_id || ""))
	) {
	...
	if (confirm !== expected)
		throw new Error(`Refusing Chatwoot mutation: pass --confirm ${expected}`);
	if (environment === "production" && !allowProduction)
		throw new Error("Production Chatwoot cutover requires --allow-production");
	if (
		window.superboard_maintenance?.enabled !== true ||
		window.chatwoot_maintenance?.enabled !== true
	) {
		throw new Error("Both SuperBoard and Chatwoot must be in confirmed maintenance mode");
	}
	const artifacts = window.backup_receipt?.artifacts;
	const byName = new Map(
		Array.isArray(artifacts) ? artifacts.map((artifact) => [artifact?.name, artifact]) : [],
	);
	...
	if (missing.length)
		throw new Error(`Chatwoot cutover backup evidence is incomplete: ${missing.join(", ")}`);
...
export async function loadRenderedChatwoot(directory) {
	...
	if (
		digest(sql) !== plan.artifacts?.support_sql?.sha256 ||
		digest(uploadsBytes) !== plan.artifacts?.r2_uploads?.sha256
	) {
...
export async function applyRenderedChatwoot({
	...
	const configPath = resolve(
		...
		"infra",
		"generated",
		`${targetName}-support-${environment}.jsonc`,
	);
	execute(
		process.execPath,
		[
	...
	try {
		for (const [position, object] of rendered.uploads.objects.entries()) {
			...
			if (!key || key.startsWith("/") || key.split("/").includes(".."))
				throw new Error(`Invalid R2 storage key at upload ${position}`);
			...
			execute(
				...
				[
					...
					object.source_path,
					"--content-type",
					String(object.content_type || "application/octet-stream"),
					"--config",
					configPath,
				],
				{ env },
			...
			if ((await sha256File(verification)) !== object.sha256)
	...
	const mismatches = Object.entries(rendered.plan.evidence).filter(
		([name, evidence]) => Number(observed[name] || 0) !== Number(evidence.count),
	);
	if (mismatches.length)
		throw new Error(
			`Chatwoot D1 count verification failed: ${mismatches.map(([name]) => name).join(", ")}`,
		);
	...
	return {
		...
		evidence: observed,
		uploads: Object.keys(checkpoint.uploads).length,
	};
}
...
function protectedPath(value, flag) {
	if (typeof value !== "string" || !isAbsolute(value))
		throw new Error(`${flag} must be an absolute path outside Git`);
