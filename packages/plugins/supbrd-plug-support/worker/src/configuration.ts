...
export const configurationCatalog = {
	inbox: definition(
		...
		[
			...
			text("identifier", "Identifier", true, 128),
			text("timezone", "Timezone", false, 64),
			bool("greeting_enabled", "Greeting enabled"),
			text("welcome_title", "Welcome title", false, 120),
			textarea("welcome_message", "Welcome message", 2000),
			bool("pre_chat_form_enabled", "Pre-chat form enabled"),
			json("pre_chat_fields", "Pre-chat fields"),
			select("email_collection", "Email collection", ["off", "optional", "required"]),
			bool("conversation_continuity", "Conversation continuity"),
			bool("csat_enabled", "CSAT enabled"),
			bool("auto_assignment", "Automatic assignment"),
			bool("allow_reopen", "Allow customer reopening"),
			select("reply_time_expectation", "Reply time expectation", [
				"minutes",
				"hours",
				"day",
				"custom",
			]),
			text("reply_time_message", "Reply time message", false, 255),
			bool("working_hours_enabled", "Working hours enabled"),
	...
	agent: definition(
		...
		[
			text("auth_user_id", "Authentication user ID", true, 255),
			text("display_name", "Display name", true, 120),
			select("role", "Role", ["administrator", "supervisor", "agent"], true),
			select("availability", "Availability", ["online", "busy", "offline"], true),
			number("capacity", "Conversation capacity", 0, 10000),
			bool("auto_offline", "Set offline when inactive"),
		],
	),
	team: definition("Team", "Agent grouping used for assignment and reporting.", [
		textarea("description", "Description", 1000),
		bool("allow_auto_assign", "Allow automatic assignment"),
		list("member_user_ids", "Member user IDs"),
	]),
	label: definition("Label", "Reusable conversation classification.", [
		text("color", "Color", true, 32),
		textarea("description", "Description", 500),
		bool("show_on_sidebar", "Show in Inbox navigation"),
	]),
	canned_response: definition("Canned response", "A saved response available in the composer.", [
		text("short_code", "Short code", true, 64),
		textarea("content", "Response content", 8000, true),
	]),
	macro: definition("Macro", "A reusable ordered set of conversation actions.", [
		json("actions", "Actions", true),
		select("visibility", "Visibility", ["personal", "team", "project"]),
	]),
	custom_attribute: definition(
		"Custom attribute",
		"Typed metadata for contacts or conversations.",
		[
			text("key", "Key", true, 64),
			select("model", "Model", ["contact", "conversation"], true),
			select(
				"value_type",
				"Value type",
				["text", "number", "date", "boolean", "list", "link"],
				true,
			),
			textarea("description", "Description", 500),
			list("allowed_values", "Allowed values"),
		],
	),
	automation_rule: definition(
		"Automation rule",
		...
		[
			...
			json("conditions", "Conditions", true),
			json("actions", "Actions", true),
		],
	),
	assignment_policy: definition(
		"Assignment policy",
		...
		[
			...
			list("inbox_ids", "Inbox IDs"),
			list("team_ids", "Team IDs"),
		],
	),
	...
	webhook: definition("Webhook", "Signed outbound Support events.", [
		text("url", "Endpoint URL", true, 2048),
		list("events", "Events", true),
	]),
	working_hours: definition("Working hours", "Timezone-aware availability and holidays.", [
		text("timezone", "Timezone", true, 64),
		json("weekly_schedule", "Weekly schedule", true),
		json("closed_dates", "Closed dates"),
		textarea("unavailable_message", "Unavailable message", 2000),
	]),
	notification_preference: definition(
		"Notification preference",
		"Per-agent delivery and sound preferences.",
		[
			text("auth_user_id", "Authentication user ID", true, 255),
			bool("email_enabled", "Email notifications"),
			bool("push_enabled", "Push notifications"),
			bool("browser_enabled", "Browser notifications"),
			bool("in_app_enabled", "In-app notifications"),
			bool("audio_enabled", "Notification sounds"),
			list("muted_event_types", "Muted event types"),
		],
	),
	saved_filter: definition("Saved filter", "A reusable Inbox or contact search.", [
		select("model", "Model", ["conversation", "contact"], true),
		json("query", "Filter query", true),
	]),
	campaign: definition("Campaign", "A support campaign delivered through a configured Inbox.", [
		select("campaign_type", "Type", ["one_off", "ongoing"], true),
		text("inbox_id", "Inbox ID", true, 255),
		textarea("message", "Message", 8000, true),
		json("audience", "Audience", true),
		text("scheduled_at", "Scheduled time", false, 64),
	]),
	integration: definition(
		...
		[
			select(
				"provider",
				"Provider",
				["slack", "linear", "notion", "shopify", "dyte", "custom"],
				true,
			),
			text("secret_reference", "Secret reference", true, 255),
			json("settings", "Provider settings"),
		],
	),
	agent_bot: definition("Agent bot", "A bot that can receive assigned conversations.", [
		textarea("description", "Description", 1000),
		text("outgoing_url", "Outgoing webhook URL", true, 2048),
	...
	capacity_policy: definition("Capacity policy", "Maximum concurrent work per priority.", [
		number("default_capacity", "Default capacity", 0, 10000, true),
		json("priority_limits", "Priority limits"),
	]),
	leave_schedule: definition("Leave schedule", "Temporary agent unavailability.", [
		text("auth_user_id", "Authentication user ID", true, 255),
		text("starts_at", "Starts at", true, 64),
		text("ends_at", "Ends at", true, 64),
		textarea("reason", "Reason", 500),
	]),
	dashboard_app: definition("Dashboard app", "A contextual tool displayed beside a conversation.", [
		text("url", "Application URL", true, 2048),
		list("placements", "Placements", true),
	]),
	email_template: definition("Email template", "A localized transactional email template.", [
		text("template_type", "Template type", true, 64),
		text("locale", "Locale", true, 16),
		text("subject", "Subject", true, 255),
		textarea("body", "Body", 20000, true),
	]),
...
configuration.get("/:projectId/settings", async (c) => {
    ...
    SELECT entity.*,
      EXISTS(SELECT 1 FROM support_webhook_secrets secret WHERE secret.webhook_id = entity.id AND secret.project_id = entity.project_id) secret_configured
    FROM support_configuration_entities entity WHERE entity.project_id = ?
...
configuration.patch("/:projectId/settings", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"));
	const actorId = requiredActor(c.req.header("X-SuperBoard-Agent-Id"));
    ...
    INSERT INTO support_project_settings (
  ...
  `)
		.bind(
			...
			body.timezone,
			body.date_format,
			body.auto_resolve_minutes,
			body.attachment_max_bytes,
			JSON.stringify(body.allowed_content_types),
...
configuration.get("/:projectId/settings/entities", async (c) => {
    ...
    WHERE entity.project_id = ? AND (? IS NULL OR entity.entity_type = ?)
    ORDER BY entity.entity_type, entity.position, entity.name
  `)
		.bind(projectId, entityType, entityType)
		.all<Record<string, unknown>>();
...
configuration.post("/:projectId/settings/entities", async (c) => {
	...
	try {
		await c.env.DB.prepare(`
      INSERT INTO support_configuration_entities (
...
configuration.patch("/:projectId/settings/entities/:entityId", async (c) => {
	...
	try {
		await c.env.DB.prepare(`
      UPDATE support_configuration_entities SET
...
configuration.put("/:projectId/settings/entities/:entityId/secret", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"));
	const actorId = requiredActor(c.req.header("X-SuperBoard-Agent-Id"));
	const entity = await entityById(c.env.DB, projectId, c.req.param("entityId"));
	if (entity.entity_type !== "webhook")
		throw failure("webhook_required", "Signing secrets can be configured only for webhooks");
	const body = await readJsonObject(c.req.raw);
	const secret = requiredText(body.secret, "secret", 4096);
	if (secret.length < 16)
		throw failure(
			"webhook_secret_invalid",
			"Webhook signing secrets must contain at least 16 characters",
		);
	const existing = await c.env.DB.prepare(`
    SELECT secret_version FROM support_webhook_secrets WHERE project_id = ? AND webhook_id = ?
  `)
		.bind(projectId, c.req.param("entityId"))
		.first<{ secret_version: number }>();
	const encrypted = await encryptSecret(c.env.SUPPORT_WEBHOOK_ENCRYPTION_KEY, secret);
	const version = Number(existing?.secret_version || 0) + 1;
	await c.env.DB.batch([
		c.env.DB.prepare(`
      INSERT INTO support_webhook_secrets (webhook_id, project_id, encrypted_secret, secret_version)
      VALUES (?, ?, ?, ?)
      ON CONFLICT(webhook_id) DO UPDATE SET encrypted_secret = excluded.encrypted_secret,
        secret_version = excluded.secret_version,
        updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
    `).bind(c.req.param("entityId"), projectId, encrypted, version),
		c.env.DB.prepare(`
      INSERT INTO support_secret_audit_events (id, project_id, webhook_id, action, secret_version, actor_id, request_id)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).bind(
			crypto.randomUUID(),
			projectId,
			c.req.param("entityId"),
			existing ? "rotated" : "configured",
			version,
			actorId,
			requiredRequestId(c.req.header("X-Request-Id")),
		),
	]);
	return c.json({
		data: { webhook_id: c.req.param("entityId"), secret_configured: true, secret_version: version },
	});
});

configuration.delete("/:projectId/settings/entities/:entityId/secret", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"));
	const actorId = requiredActor(c.req.header("X-SuperBoard-Agent-Id"));
	const entity = await entityById(c.env.DB, projectId, c.req.param("entityId"));
	if (entity.entity_type !== "webhook")
    ...
    DELETE FROM support_webhook_secrets WHERE project_id = ? AND webhook_id = ? RETURNING secret_version
  `)
		.bind(projectId, c.req.param("entityId"))
		.first<{ secret_version: number }>();
	if (!deleted)
		throw failure("webhook_secret_not_found", "Webhook signing secret is not configured", 404);
	await c.env.DB.prepare(`
    INSERT INTO support_secret_audit_events (id, project_id, webhook_id, action, secret_version, actor_id, request_id)
    VALUES (?, ?, ?, 'revoked', ?, ?, ?)
  `)
...
configuration.delete("/:projectId/settings/entities/:entityId", async (c) => {
	...
	const before = await entityById(c.env.DB, projectId, c.req.param("entityId"));
	await c.env.DB.prepare(
		"DELETE FROM support_configuration_entities WHERE project_id = ? AND id = ?",
	)
		.bind(projectId, c.req.param("entityId"))
		.run();
	await configurationAudit(
...
configuration.get("/:projectId/settings/audit", async (c) => {
	...
	const rows = await c.env.DB.prepare(`
    SELECT * FROM support_configuration_audit_events
...
export async function getPublicSupportConfiguration(db: D1Database, projectId: number) {
	const settings = await getProjectSettings(db, projectId);
	const inboxes = await db
		.prepare(`
    SELECT id, name, enabled, configuration_json FROM support_configuration_entities
...
export function validateEntity(input: Record<string, unknown>, fixedType?: string) {
	...
	const position =
		input.position == null ? 0 : integer(input.position, "position", -100000, 100000);
	...
	for (const key of Object.keys(rawConfiguration)) {
		if (!allowed.has(key))
			throw failure("configuration_field_unknown", `Unknown configuration field: ${key}`);
		if (looksLikeSecret(key) && !key.endsWith("_reference")) {
			throw failure(
				"configuration_secret_forbidden",
				"Secret values must use a bound secret reference",
			);
		}
	}
	...
	for (const field of fields) {
		...
		if (value == null || value === "") {
			if (field.required)
				throw failure("configuration_field_required", `${field.label} is required`);
			continue;
		}
	...
	if ("url" in normalized && !isSafePublicHttpsUrl(normalized.url)) {
		throw failure(
			"configuration_url_invalid",
			"Endpoint URL must use public HTTPS without embedded credentials",
		);
	}
	if ("outgoing_url" in normalized && !isSafePublicHttpsUrl(normalized.outgoing_url)) {
		throw failure(
			"configuration_url_invalid",
			"Outgoing webhook URL must use public HTTPS without embedded credentials",
		);
	}
...
export function validateProjectSettings(input: Record<string, unknown>) {
	...
	for (const [key, value] of Object.entries(features)) {
		if (!/^[a-z][a-z0-9_]{0,63}$/.test(key) || typeof value !== "boolean") {
			throw failure(
				"features_invalid",
				"Feature flags must be boolean values with stable lowercase keys",
			);
		}
	}
	return {
		...
		auto_resolve_minutes:
			input.auto_resolve_minutes == null
				? null
				: integer(input.auto_resolve_minutes, "auto_resolve_minutes", 0, 525600),
		attachment_max_bytes:
			input.attachment_max_bytes == null
				? 10 * 1024 * 1024
				: integer(input.attachment_max_bytes, "attachment_max_bytes", 1, 10 * 1024 * 1024),
		allowed_content_types:
			input.allowed_content_types == null
				? []
				: stringList(input.allowed_content_types, "allowed_content_types", 100, 255),
		features,
	};
}
...
async function getProjectSettings(db: D1Database, projectId: number) {
	const row = await db
		.prepare("SELECT * FROM support_project_settings WHERE project_id = ?")
		.bind(projectId)
...
async function entityById(db: D1Database, projectId: number, id: string) {
    ...
    SELECT entity.*,
      EXISTS(SELECT 1 FROM support_webhook_secrets secret WHERE secret.webhook_id = entity.id AND secret.project_id = entity.project_id) secret_configured
    FROM support_configuration_entities entity WHERE entity.project_id = ? AND entity.id = ?
  `)
		.bind(projectId, id)
		.first<Record<string, unknown>>();
...
async function configurationAudit(
	...
	await db
		.prepare(`
    INSERT INTO support_configuration_audit_events (
...
function bool(key: string, label: string): FieldDefinition {
...
function list(key: string, label: string, required = false): FieldDefinition {
	return { key, label, type: "string_list", required };
}
function json(key: string, label: string, required = false): FieldDefinition {
	return { key, label, type: "json", required };
}
...
function integer(value: unknown, field: string, min: number, max: number) {
	...
	if (!Number.isInteger(parsed) || parsed < min || parsed > max)
		throw failure(`${field}_invalid`, `${field} must be an integer between ${min} and ${max}`);
	return parsed;
}
...
function object(value: unknown, field: string): Record<string, unknown> {
	if (!value || typeof value !== "object" || Array.isArray(value))
		throw failure(`${field}_invalid`, `${field} must be an object`);
...
function requiredText(value: unknown, field: string, max: number) {
	...
	if (!parsed || parsed.length > max)
		throw failure(`${field}_invalid`, `${field} is required and must be at most ${max} characters`);
	return parsed;
}
function optionalText(value: unknown, field: string, max: number) {
	...
	if (parsed.length > max)
		throw failure(`${field}_invalid`, `${field} must be at most ${max} characters`);
	return parsed;
}
function stringList(value: unknown, field: string, maxItems: number, maxLength: number) {
	if (!Array.isArray(value) || value.length > maxItems)
		throw failure(`${field}_invalid`, `${field} must be an array with at most ${maxItems} items`);
	const values = [...new Set(value.map((item) => String(item).trim()).filter(Boolean))];
	if (values.some((item) => item.length > maxLength))
		throw failure(`${field}_invalid`, `${field} contains an item that is too long`);
	return values;
}
...
function requiredRequestId(value: unknown) {
	return requiredText(value, "request_id", 128);
}
function requiredEntityType(value: unknown): ConfigurationEntityType {
	const parsed = String(value || "") as ConfigurationEntityType;
	if (!(parsed in configurationCatalog))
		throw failure("configuration_type_invalid", "Configuration type is invalid");
	return parsed;
}
