...
export async function resolveSupportPrincipal(
	db: D1Database,
	context: Pick<ProjectContext, "projectId" | "actorId" | "role" | "operatorId">,
): Promise<SupportPrincipal> {
	const identityRole = String(context.role || "").toLowerCase();
	const actorId = context.operatorId ?? String(context.actorId);
...
export function requireSupportRole(principal: SupportPrincipal, minimum: SupportRole) {
...
export function supportPrincipalFromHeaders(request: Request) {
	...
	if (
		...
		throw failure("project_context_invalid", "Signed project context headers are incomplete", 401);
	}
	return {
		projectId,
		actorId,
		role,
		operatorId: request.headers.get("x-operator-id") || undefined,
	};
}
