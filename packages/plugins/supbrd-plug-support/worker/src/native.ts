...
for (const resource of resources) {
	...
	native.delete(`/:projectId/${resource.path}/:id`, async (c) => {
		...
		const result = await c.env.DB.prepare(
			`DELETE FROM ${resource.table} WHERE id = ? AND project_id = ?`,
		)
			.bind(id, projectId)
			.run();
		if (!result.meta.changes)
			throw failure("support_resource_not_found", "Support resource not found", 404);
...
native.get("/:projectId/reports", async (c) => {
	...
	const [byInbox, byAgent, byTeam, byLabel, byChannel, byProvider, sla, csat, campaigns] =
      ...
      WHERE project_id = ? AND (? IS NULL OR created_at >= ?) AND (? IS NULL OR created_at < ?)`).bind(
				projectId,
				from,
				from,
				to,
...
async function audit(
	...
	await db
     ...
     VALUES (?, ?, ?, ?, ?, ?, '{}')`,
		...
		.bind(crypto.randomUUID(), projectId, type.slice(0, 128), id, action.slice(0, 128), actor)
		.run();
}
