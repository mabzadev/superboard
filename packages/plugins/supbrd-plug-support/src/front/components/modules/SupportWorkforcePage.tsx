			...
			{!selectedProject ? (
				...
				<div className="space-y-6">
					<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
						...
						/>
						<SupportMetric label="Active teams" value={summary?.active_teams ?? "—"} />
						<SupportMetric label="Active inboxes" value={summary?.active_inboxes ?? "—"} />
						<SupportMetric label="Members on leave" value={summary?.active_leaves ?? "—"} />
					</div>
					<Tabs defaultValue="members">
						...
						<TabsContent className="space-y-4" value="members">
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">Add a Support member</CardTitle>
								</CardHeader>
											...
											<CardContent className="flex items-center justify-between gap-4 p-4">
												<div>
													...
													<p className="text-sm text-muted-foreground">
														{item.auth_user_id} · {item.role} · capacity {item.capacity}
													</p>
												</div>
						...
						<TabsContent className="space-y-4" value="inboxes">
								...
								<CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
									...
									<Field label="Channel type">
										<NativeSelect
											...
											onChange={(channel_type) => setInbox((value) => ({ ...value, channel_type }))}
											options={[
												"api",
												"widget",
												"email_google",
							...
							{inboxes.loading ? (
								...
								<SupportEmpty
									title="No Support inboxes"
									description="Create an inbox before connecting a customer channel."
								/>
							) : (
								<div className="grid gap-3 lg:grid-cols-2">
						...
						<TabsContent className="space-y-4" value="routing">
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">Membership routing</CardTitle>
								</CardHeader>
						...
						<TabsContent className="space-y-4" value="leaves">
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">Schedule leave</CardTitle>
								</CardHeader>
								<CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
									<Field label="Member">
										<NativeSelect
											value={leave.membershipId}
											onChange={(membershipId) => setLeave((value) => ({ ...value, membershipId }))}
											optionsWithLabels={memberships.items.map((item) => ({
												value: item.id,
												label: item.display_name,
											}))}
							...
							{leaves.loading ? (
								...
								<SupportEmpty
									title="No leave schedules"
									description="Scheduled leave is applied to automatic assignment."
								/>
							) : (
						...
						<TabsContent className="space-y-4" value="capacity">
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">Create a capacity policy</CardTitle>
								</CardHeader>
								<CardContent className="grid gap-4 md:grid-cols-3">
									<Field label="Name">
										<Input
								...
								<Card key={item.id}>
									<CardContent className="flex items-center justify-between p-4">
										<div>
											<p className="font-medium">{item.name}</p>
											<p className="text-sm text-muted-foreground">
												Default capacity {item.default_capacity}
											</p>
										</div>
										<div className="flex items-center gap-2">
											<SupportStatus value={item.active ? "active" : "disabled"} />
