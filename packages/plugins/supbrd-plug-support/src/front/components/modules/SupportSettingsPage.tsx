...
const contentTypes = ["image/jpeg", "image/png", "image/webp", "application/pdf", "text/plain"];
				...
				<Tabs defaultValue="general">
					...
					<TabsContent className="space-y-6" value="general">
								...
								<Field label="Auto-resolve after (minutes)">
									<Input
										...
										onChange={(e) =>
											setSettings((v) => ({
												...v,
												auto_resolve_minutes: e.target.value === "" ? null : Number(e.target.value),
											}))
										}
									/>
								</Field>
								<Field label="Attachment limit (MB)">
									<Input
										...
										onChange={(e) =>
											setSettings((v) => ({
												...v,
												attachment_max_bytes: Number(e.target.value) * 1024 * 1024,
											}))
										}
									/>
								</Field>
						...
						<div className="grid gap-6 xl:grid-cols-2">
							<Card>
								<CardHeader>
									<CardTitle className="text-base">Allowed attachments</CardTitle>
								</CardHeader>
								<CardContent className="space-y-3">
									{contentTypes.map((type) => (
										<label className="flex items-center gap-3 text-sm" key={type}>
											<Checkbox
												checked={settings.allowed_content_types.includes(type)}
												onCheckedChange={(checked) => toggleContentType(type, checked === true)}
											/>
											{type}
										</label>
									))}
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">Support capabilities</CardTitle>
								</CardHeader>
					...
					<TabsContent className="space-y-6" value="notifications">
						{notificationError ? (
							<AccessNotice>
								{notificationError}. Personal notification controls require an active Support
								membership; project administration remains available in the other Settings tabs.
							</AccessNotice>
						) : (
								...
								<Card>
									...
									<CardContent className="space-y-4">
										...
										<p className="text-xs text-muted-foreground">
											Use comma-separated native Support event names. Muted events remain visible in
											audit history.
										</p>
										<Button
											className="w-full"
					...
					<TabsContent className="space-y-6" value="operations">
						{!administrator ? (
							<AccessNotice>
								Operations and delivery diagnostics are available to project owners and
								administrators.
							</AccessNotice>
						) : (
							<>
								...
								<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
									<SupportMetric label="Queued jobs" value={total(operations?.queues)} />
									<SupportMetric label="Dead letters" value={total(operations?.dead_letters)} />
									<SupportMetric label="Provider events" value={total(operations?.providers)} />
									<SupportMetric label="Knowledge documents" value={total(operations?.knowledge)} />
								</div>
								<div className="grid gap-6 xl:grid-cols-2">
									<OperationGroup
										icon={<Activity />}
