...
export type SupportFieldDefinition = {
	key: string;
	label: string;
	type: "text" | "textarea" | "number" | "boolean" | "select" | "string_list" | "json";
	required?: boolean;
	max_length?: number;
	min?: number;
	max?: number;
...
export const getSupportSettings = async (
	projectId: string,
): Promise<{ data: SupportSettingsBootstrap }> => (await GET(path(projectId))).data;

export const updateSupportSettings = async (projectId: string, settings: SupportProjectSettings) =>
...
export const updateSupportConfiguration = async (
	...
	(
		await PATCH(path(projectId, `/entities/${encodeURIComponent(entityId)}`), entity, {
			retry: false,
		})
	).data;

export const deleteSupportConfiguration = async (projectId: string, entityId: string) =>
	(
...
export const revokeSupportWebhookSecret = async (projectId: string, entityId: string) =>
	(
		await DELETE(path(projectId, `/entities/${encodeURIComponent(entityId)}/secret`), {
			retry: false,
		})
	).data;
