...
export function assertBillingConfigOwnership({ apiConfig, billingConfig, mode, resources }) {
	...
	const consumers = [
		...billingConsumers(apiConfig, resources.queues.billing).map((consumer) => ({
			owner: "api",
			consumer,
		})),
		...billingConsumers(billingConfig, resources.queues.billing).map((consumer) => ({
			owner: "billing",
			consumer,
		})),
	];
	...
	if (consumers[0].owner !== expectedOwner) {
		throw new Error(`Billing queue consumer must be owned by ${expectedOwner} in ${mode} mode`);
	}
	...
	if (mode === "service" && !billingCrons.includes("*/10 * * * *")) {
		throw new Error("Billing Worker must own scheduled reconciliation in service mode");
	}
	if (mode === "local" && billingCrons.length > 0) {
		throw new Error("Billing Worker must not schedule reconciliation in local mode");
	}

	const deadLetterConsumers = [
		...billingConsumers(apiConfig, resources.queues.billingDlq).map((consumer) => ({
			owner: "api",
			consumer,
		})),
		...billingConsumers(billingConfig, resources.queues.billingDlq).map((consumer) => ({
			owner: "billing",
			consumer,
		})),
	];
	if (deadLetterConsumers.length !== 1 || deadLetterConsumers[0].owner !== "billing") {
		throw new Error("Billing Worker must be the only generated Billing DLQ consumer");
	}
...
function assertProducer(config, queue, label) {
	...
	if (!producer) throw new Error(`${label} configuration must bind the Billing queue producer`);
}

function billingConsumers(config, queue) {
	return (config.queues?.consumers ?? []).filter((consumer) => consumer.queue === queue);
}

async function readGenerated(target, service, environment) {
	const path = resolve(root, "infra", "generated", `${target}-${service}-${environment}.jsonc`);
