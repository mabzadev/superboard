...
import { parseQueueConsumer } from "./billing-consumer.mjs";
...
export function evaluateBillingPreflight(input) {
	...
	const missingMigrations = (input.requiredMigrations || []).filter(
	...
	const unavailableMetrics = operationalMetrics.filter(
		(name) => !Number.isFinite(Number(operational[name])),
	);
	const unhealthyMetrics = operationalMetrics.filter(
		(name) => Number.isFinite(Number(operational[name])) && Number(operational[name]) !== 0,
	);
	check(
	...
	const genericWebhookSecret = secretNames.has("SUPERBOARD_ENTITLEMENT_WEBHOOK_SECRET");
	check(
		"secret_entitlement_webhook",
		genericWebhookSecret,
		genericWebhookSecret
			? "The entitlement projection secret is configured under the generic name."
			: "SUPERBOARD_ENTITLEMENT_WEBHOOK_SECRET is missing.",
	);
...
export function parseSecretNames(output) {
	...
	return rows.map((row) => String(object(row).name || "")).filter(Boolean);
}
...
async function main() {
	...
	const requiredMigrations = readdirSync(
		resolve(root, "packages/plugins/superboard-core/api/migrations"),
	)
		.filter((name) => /^\d+_.+\.sql$/u.test(name))
		.sort();
	const appliedMigrations = parseD1MigrationNames(
		capture("npx", [
			"wrangler",
			"d1",
			"execute",
	...
	const report = evaluateBillingPreflight({
		...
		purchasesJwks,
		authJwks,
		billingSecretNames: parseSecretNames(
			capture("npx", ["wrangler", "secret", "list", "--config", billingConfig]),
		),
	});
...
function queueConsumer(queue, config) {
	return parseQueueConsumer(
		capture("npx", ["wrangler", "queues", "info", queue, "--config", config]),
	);
}
