...
} from "@superboard/api-worker/store-verification";
...
import { drainAnalyticsFactOutbox } from "../../../superboard-core/api/src/lib/analytics-facts";
import {
	applyVerifiedPurchase,
	identifyCustomer,
} from "../../../superboard-core/api/src/lib/billing";
import { quarantineBillingDeadLetter } from "../../../superboard-core/api/src/lib/billing-dead-letters";
import {
	dispatchBillingJob,
	isBillingQueueJob,
} from "../../../superboard-core/api/src/lib/billing-dispatch";
import {
	purchasesSigningJwks,
	resolveSdkCustomer,
	signedCustomerInfo,
	verifiedAppUserId,
} from "../../../superboard-core/api/src/lib/billing-identity";
import { reconcileBillingState } from "../../../superboard-core/api/src/lib/billing-jobs";
import {
	ingestBillingProviderEvent,
	type BillingProviderIngressRequest,
} from "../../../superboard-core/api/src/lib/billing-provider-ingress";
import { restoreVerifiedPurchases } from "../../../superboard-core/api/src/lib/billing-restore";
import { billingWorkerReadiness } from "../../../superboard-core/api/src/lib/billing-worker-readiness";
import { readTextLimited } from "../../../superboard-core/api/src/lib/http-limits";
import { encryptCredential } from "../../../superboard-core/api/src/lib/secrets";
import purchasesAdminRoutes from "../../../superboard-core/api/src/routes/purchases-admin";
import purchasesV2AdminRoutes from "../../../superboard-core/api/src/routes/purchases-v2-admin";
import type { BillingEnv } from "../../../superboard-core/api/src/types";
import catalogLedger from "./catalog-ledger.js";
...
app.get("/internal/v1/jwks", (c) => {
	try {
		...
		throw publicError("signing_keys_unavailable", "Purchases signing keys are unavailable", 503);
	}
});
...
app.post("/internal/v1/customers/erase", async (c) => {
	...
	if (!authorized) {
		throw publicError("internal_auth_invalid", "Internal authentication failed", 401);
	}
	const body = await boundedRecord(c.req.raw);
	const projectId = requiredText(body.project_id, "project_id", 64);
	const userId = requiredText(body.application_user_id, "application_user_id", 255);
	...
	return c.json({
		data: {
			...
			events_redacted: Number(results[2].meta.changes || 0) + Number(results[3].meta.changes || 0),
		},
	});
});
...
app.post("/internal/v1/jobs", async (c) => {
	...
	if (!isBillingQueueJob(job))
		throw publicError("billing_job_invalid", "A supported billing job is required");
...
app.post("/internal/v1/customer-info", async (c) => {
	...
	if (!projectId || !customerId)
		throw publicError("customer_context_invalid", "project_id and customer_id are required");
	return c.json({
...
app.post("/internal/v1/customers/identify", async (c) => {
	...
	if (!targetAppUserId)
		throw publicError("identity_required", "A verified identity token is required", 401);
	...
	const environment =
		body.environment === "production"
			...
			: body.environment === "sandbox"
				...
				: null;
	if (!environment)
		throw publicError("restore_environment_invalid", "environment must be sandbox or production");
...
async function boundedJson(request: Request): Promise<unknown> {
	let text: string;
	try {
		text = await readTextLimited(request, 2_359_296, "Private request body is limited to 2.25 MB");
	} catch {
		throw publicError("request_too_large", "Private request body is limited to 2.25 MB", 413);
	}
	try {
		...
		ctx.waitUntil(
			drainAnalyticsFactOutbox(env).then((summary) => {
				if (summary.inspected > 0) {
					console.log(JSON.stringify({ event: "analytics_fact_outbox_drained", summary }));
				}
			}),
		);
	...
	async queue(batch, env, _ctx) {
		if (batch.queue === env.BILLING_DLQ_NAME) {
			for (const message of batch.messages) {
				try {
					const result = await quarantineBillingDeadLetter(env, batch.queue, message);
					console.error(
						JSON.stringify({
		...
		if (batch.queue !== env.BILLING_QUEUE_NAME) {
			console.error(JSON.stringify({ event: "billing_queue_unknown", queue: batch.queue }));
		...
		for (const message of batch.messages) {
			try {
				...
				if (tagged?.retryable === false) {
					...
					message.retry({
						delaySeconds: Math.max(30, Math.min(3600, Number(tagged?.retryDelaySeconds || 60))),
					});
			}
		}
	},
...
export default withPluginTaskLifecycle("supbrd-plugmod-billing", lifecycleWorker);
