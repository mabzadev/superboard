...
import { audit, isoNow, parseJsonObject, projectId } from "../d1/helpers";
...
export async function getUser(context: FlowContext, userHash: string) {
	const profiles = await context.env.DB.prepare(
     ...
     ORDER BY last_seen_at DESC`,
		...
		.bind(projectId(context), userHash)
		.all<SqlRow>();
	...
	return {
		profiles: await Promise.all(
			profiles.results.map(async (profile) => ({
				...
				external_user_id:
					profile.external_user_id_ciphertext == null
						? null
						: await decryptFlowValue(context.env, String(profile.external_user_id_ciphertext)),
				properties:
					profile.properties_ciphertext == null
						? {}
...
export async function resetUserWorkflow(
	...
	const runtimeResults = await Promise.all(
		profiles.results.map(({ environment_id: runtimeEnvironmentId }) =>
			context.env.FLOW_USER_RUNTIME.getByName(
				flowRuntimeName(project.projectId, runtimeEnvironmentId, userHash),
			).resetProgress({
				eventId,
	...
	const result = await context.env.DB.prepare(
		...
		.bind(...bindings)
		.run();
	await audit(context, "flows.user.progress-reset", "flow_user", userHash, {
...
export async function workflowAnalytics(context: FlowContext, workflowId: string) {
	const { from, to, environmentId } = analyticsFilters(context.req.url);
	const conditions = ["project_id = ?", "workflow_id = ?", "occurred_at >= ?", "occurred_at <= ?"];
...
export async function surveyAnalytics(context: FlowContext, surveyId: string) {
	const { from, to, environmentId } = analyticsFilters(context.req.url);
	const conditions = [
		"r.project_id = ?",
		"r.survey_id = ?",
	...
	if (environmentId) {
		conditions.push("r.environment_id = ?");
		bindings.push(environmentId);
	}
	...
	const ratingAccumulators = [...statistics.values()].filter((entry) => entry.type === "rating");
	return {
		survey_id: surveyId,
		from,
		to,
...
export async function exportSurveyCsv(context: FlowContext, surveyId: string) {
	const { from, to, environmentId } = analyticsFilters(context.req.url);
	...
	await context.env.DB.prepare(
     ...
     VALUES (?, ?, 'survey-csv', 'completed', ?, ?, ?, ?, ?)`,
		...
		.bind(
			...
			key,
			context.get("project").operatorId ?? String(context.get("project").actorId),
			isoNow(),
			isoNow(),
		)
		.run();
...
function accumulateSurveyResponse(
	...
	for (const question of questions) {
		...
		if (type === "rating") {
			const parsed =
				typeof value.textResponse === "number"
					? value.textResponse
			...
			if (Number.isFinite(parsed)) {
				...
				accumulator.numeric.values.set(parsed, (accumulator.numeric.values.get(parsed) ?? 0) + 1);
			}
		}
		if (
...
function finalizeSurveyQuestion(accumulator: SurveyQuestionAccumulator) {
	return {
		...
		...(accumulator.type === "rating" ? { numeric: finalizeNumeric(accumulator.numeric) } : {}),
		...
		...(accumulator.type === "link"
			? {
					links: {
						...
						conversion: accumulator.responses ? accumulator.clicks / accumulator.responses : 0,
					},
				}
			: {}),
	};
