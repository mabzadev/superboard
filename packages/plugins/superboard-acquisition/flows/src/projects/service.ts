...
import { audit, isoNow, parseJsonArray, parseJsonObject, projectId, sha256 } from "../d1/helpers";
import { failure } from "../http/errors";
import { identifier, optionalString, requiredString } from "../http/validation";
 ...
 */
export async function ensureFlowProject(context: FlowContext): Promise<FlowProjectProvision> {
...
export async function createEnvironment(context: FlowContext, body: Record<string, unknown>) {
...
export async function rotateEnvironmentKey(context: FlowContext, environmentId: string) {
	const sdkKey = randomToken();
	const result = await context.env.DB.prepare(
		`UPDATE flow_environments SET sdk_key_hash = ?, updated_at = ?
     WHERE id = ? AND project_id = ?`,
	)
		.bind(await sha256(sdkKey), isoNow(), environmentId, projectId(context))
		.run();
...
export async function saveLanguageGroup(context: FlowContext, body: Record<string, unknown>) {
	...
	const name = requiredString(body.name, "name", 160);
	const fallbacks =
		typeof body.fallbacks === "object" && body.fallbacks !== null && !Array.isArray(body.fallbacks)
			? body.fallbacks
			: {};
	const now = isoNow();
	await context.env.DB.prepare(
...
function stableResourceId(projectIdValue: number, resource: string, key: string): string {
