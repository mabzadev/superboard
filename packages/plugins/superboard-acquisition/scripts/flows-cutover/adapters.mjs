...
import { cloudflareEnv } from "../../../../../scripts/cloudflare/target.mjs";
...
export class FixtureFlowsCutoverAdapter {
	...
	async resolveUserHashes(project, rawUserIds) {
		const key = String(this.fixture.flow_user_hash_key ?? "fixture-flow-user-hash-key");
		return new Map(
			rawUserIds.map((rawUserId) => [
				rawUserId,
				createHmac("sha256", key).update(`${project.project_ref}\n${rawUserId}`).digest("hex"),
			]),
		);
	}
	...
	async upsert(entity, rows, _context, sql) {
		...
		if (this.fixture.fail_on_upsert_attempt === this.upsertAttempts)
			throw new Error("Fixture interrupted the import");
		...
		for (const row of rows) {
			...
			if (
				entity.immutable &&
				index.has(key) &&
				canonicalJson(index.get(key)) !== canonicalJson(row)
			)
				continue;
	...
	async maintenanceStatus(projectRef) {
		return structuredClone(
			this.fixture.maintenance[projectRef] ?? { enabled: false, window_id: null },
		);
	}
	...
	async routingStatus(projectRef) {
		const project = (await this.projectInventory()).find((item) => item.project_ref === projectRef);
		return structuredClone(
			this.fixture.flows_routing[projectRef] ?? {
...
export class RemoteFlowsCutoverAdapter {
	...
	async resolveProject(projectRef) {
		...
		const rows = await this.query(
			this.resources.d1,
			`SELECT id AS project_id, instance_id, identifier, is_test FROM projects WHERE instance_id = ${parsed.instance_id} AND is_test = ${parsed.project_environment === "test" ? 1 : 0} ORDER BY id`,
		);
		if (rows.length !== 1)
			throw new Error(
				`Expected exactly one API project for ${parsed.project_ref}; found ${rows.length}`,
			);
		return {
	...
	async resolveFlowEnvironment(project, { environmentId } = {}) {
		...
		const environments = await this.query(
			flows,
			`SELECT e.id,e.key,e.name FROM flow_environments e JOIN flow_projects p ON p.project_id = e.project_id WHERE e.project_id = ${Number(project.project_id)} AND p.project_ref = ${sqlLiteral(project.project_ref)}${environmentWhere} ORDER BY e.created_at,e.id`,
		);
		if (environments.length === 0 && !environmentId) {
			return {
				environment_id: defaultFlowEnvironmentId(project.project_id, project.environment),
			};
		}
		if (environments.length !== 1) {
			const selector = environmentId ? `id ${environmentId}` : `key ${project.environment}`;
			throw new Error(
				`Expected exactly one project-scoped Flows environment with ${selector}; found ${environments.length}. Pass --flows-environment-id only when an explicit environment id is required.`,
			);
		}
	...
	async ensureProjectScope(project, environmentId) {
		if (!this.allowWrites) {
			throw new Error(
				"Remote writes are disabled; project scope bootstrap requires guarded apply authority",
			);
		}
		const expectedEnvironmentId = defaultFlowEnvironmentId(project.project_id, project.environment);
		...
		const discardedSdkKeyHash = createHash("sha256").update(randomBytes(48)).digest("hex");
...
INSERT OR IGNORE INTO flow_environments
	...
	async validateSnapshotIdentity(snapshot) {
		...
		if (
			String(live.project_id) !== String(snapshot?.project?.project_id) ||
			live.instance_id !== Number(snapshot?.project?.instance_id)
		) {
		...
		for (const source of ["paywalls", "onboardings"]) {
			...
			if (actual?.database_name !== expected.name || actual?.database_id !== expected.id) {
				throw new Error(
					`Snapshot ${source} database identity does not match the MBZA target manifest`,
				);
			}
		}
		return true;
	...
	async validatePlanIdentity(plan, { allowMissingDefaultScope = false } = {}) {
		...
		if (
			String(live.project_id) !== String(plan?.project?.project_id) ||
			live.instance_id !== Number(plan?.project?.instance_id)
		) {
		...
		for (const source of ["paywalls", "onboardings"]) {
			if (plan.source_databases?.[source] !== this.requireProvisionedDatabase(source).name) {
				throw new Error(
					`Protected plan ${source} database identity does not match the MBZA target manifest`,
				);
			}
		...
		if (allowMissingDefaultScope && plan.environment_id === expectedDefaultEnvironmentId) {
			const flows = this.requireProvisionedDatabase("flows");
			const rows = await this.query(
				flows,
				`SELECT p.project_id,p.project_ref,e.id AS environment_id,e.key
	...
	async validateBackupReceiptIdentity(receipt) {
		for (const source of ["api", "paywalls", "onboardings", "flows"]) {
			...
			if (artifact?.database_name !== expected.name || artifact?.database_id !== expected.id) {
				throw new Error(
					`Backup receipt ${source} D1 identity does not match the live MBZA target manifest`,
				);
			}
			if (artifact.r2_bucket !== this.resources.moduleR2?.flows?.name) {
				throw new Error(
					`Backup receipt ${source} archive bucket does not match the live MBZA target manifest`,
				);
			}
		}
		return true;
	...
	async resolveUserHashes(project, rawUserIds) {
		if (this.flowUserHashKey) {
			...
			return new Map(
				[...new Set(rawUserIds.map((value) => String(value)))].map((rawUserId) => [
					rawUserId,
					createHmac("sha256", key).update(`${project.project_ref}\n${rawUserId}`).digest("hex"),
				]),
			);
		...
		if (!this.gatewayToken) {
			throw new Error(
				"SUPERBOARD_CUTOVER_TOKEN or --flow-user-hash-bundle is required to resolve runtime-compatible Flows user hashes",
			);
		}
		...
		for (let offset = 0; offset < unique.length; offset += 500) {
			...
			const idempotencyKey = `flows-cutover-user-hashes-${sha256(
				canonicalJson({
	...
	async captureSnapshot({ projectRef }) {
		...
		for (const source of ["paywalls", "onboardings"]) {
			...
			for (const descriptor of FLOW_SOURCE_QUERIES[source]) {
				...
				for (let offset = 0; ; offset += descriptor.pageSize) {
					const page = await this.query(
						database,
						`${query} LIMIT ${descriptor.pageSize} OFFSET ${offset}`,
					);
					rows.push(...page);
					if (page.length < descriptor.pageSize) break;
					if (offset >= 10_000_000)
						throw new Error(
							`${source}.${descriptor.table} exceeds the guarded snapshot pagination limit`,
						);
				}
			...
			if (bookmark !== bookmarkBefore) {
				throw new Error(
					`${source} changed while its snapshot was being captured; keep the project freeze enabled and retry so checksums bind to one stable Time Travel bookmark`,
				);
			}
			sources[source] = {
				bookmark,
		...
		return {
			...
			project,
			sources,
			snapshot_checksum_sha256: sha256(
				canonicalJson({ project, captured_at: capturedAt, sources }),
			),
		};
	}
	...
	async readTarget(entity, context) {
		...
		for (let offset = 0; ; offset += entity.pageSize) {
			...
			if (offset >= 10_000_000)
				throw new Error(`${entity.id} exceeds the guarded target pagination limit`);
		}
	}
	...
	async upsert(entity, _rows, _context, sql) {
		if (!this.allowWrites)
			throw new Error(
				"Remote writes are disabled; use the guarded --apply command with exact confirmation",
			);
		await this.executeFile(
			this.requireProvisionedDatabase("flows"),
			`${entity.id.replaceAll(".", "-")}.sql`,
			sql,
		);
	}
	...
	async recordImportStatus({ plan, entity, status, checkpoint }) {
		...
		const verified = sourceEntities.filter(
			(item) => checkpoint.entities[item.id]?.status === "verified",
		);
		const counts = Object.fromEntries(verified.map((item) => [item.id, item.expected.count]));
		const checksums = Object.fromEntries(
			verified.map((item) => [item.id, item.expected.checksum_sha256]),
		);
		const id = `${plan.plan_id}:${source}`;
		const effectiveStatus =
			status === "failed"
				? "failed"
				: verified.length === sourceEntities.length
					? "verified"
					: "running";
	...
	async setMaintenance(projectRef, state) {
		...
		if (this.directMaintenance) {
			if (
				...
				(state.enabled &&
					(!String(state.window_id ?? "").trim() || !String(state.reason ?? "").trim()))
			) {
			...
			const auditId = `flows-maintenance-${sha256(
				canonicalJson({
					project_id: project.project_id,
...
INSERT OR IGNORE INTO module_cutover_audit (id,project_id,action,actor,details_json)
        VALUES (${sqlLiteral(auditId)},${Number(project.project_id)},${sqlLiteral(state.enabled ? "maintenance.enabled" : "maintenance.disabled")},${sqlLiteral(actor)},${sqlLiteral(canonicalJson({ window_id: windowId, reason }))});`;
			...
			if (confirmed.enabled !== state.enabled || confirmed.window_id !== windowId) {
	...
	async setFlowsRouting(projectRef, state, idempotencyKey) {
       ...
       WHERE project_id = ${Number(project.project_id)}
		...
		const actor = "flows-cutover-cli";
		const now = new Date().toISOString();
		const auditId = `flows-routing-${sha256(
			canonicalJson({
...
INSERT INTO flows_legacy_cutover_commands
	...
	async cutoverRequest(method, path, body, extraHeaders = {}) {
		if (!this.gatewayToken)
			throw new Error("SUPERBOARD_CUTOVER_TOKEN is required for cutover operations");
		const controller = new AbortController();
		const timeout = setTimeout(() => controller.abort(), 15_000);
		try {
			const response = await fetch(
				`https://${this.target.domains.api}/api/v1/admin/module-cutover/${path}`,
				{
					method,
					headers: {
			...
			if (!response.ok)
				throw new Error(
					`Cutover API ${response.status}: ${payload?.error?.code || "request_failed"}`,
				);
	...
	async timeTravelBookmark(database, timestamp) {
		...
		if (!bookmark)
			throw new Error(`Wrangler returned no Time Travel bookmark for ${database.name}`);
		return bookmark;
	}
	...
	async executeFile(database, filename, sql) {
		if (!this.allowWrites)
			throw new Error("Remote writes are disabled; guarded --apply authority is required");
...
export function parseD1Rows(output) {
	...
	return batches.flatMap((batch) =>
		Array.isArray(batch?.results)
			? batch.results
			: Array.isArray(batch?.result?.results)
...
function parseJsonOutput(output) {
	...
	const starts = [value.indexOf("["), value.indexOf("{")]
...
function findStringProperty(value, names) {
	...
	for (const [key, candidate] of Object.entries(value)) {
		if (names.has(key) && typeof candidate === "string" && candidate.trim())
