...
import { readProtectedFlowUserHashKey } from "../../../../scripts/cloudflare/flows-version-bundle.mjs";
import { loadTarget, root } from "../../../../scripts/cloudflare/target.mjs";
import {
	FixtureFlowsCutoverAdapter,
	RemoteFlowsCutoverAdapter,
	loadFixtureAdapter,
} from "./flows-cutover/adapters.mjs";
...
export async function main(argv = process.argv.slice(2)) {
	...
	if (Object.hasOwn(args, "organization-id")) {
		throw new Error(
			"--organization-id was removed: Flows cutover is scoped directly by project and environment",
		);
	}
		...
		return emit(
			{
				...
				target: targetName,
				environment,
				entities: cutoverRegistrySummary(),
			},
			args,
		);
	...
	if (command === "backup-plan") {
		const outputDirectory =
			...
			`.flows-cutover/backups/${projectRef}/${new Date().toISOString().replaceAll(":", "-")}`;
		const plan = createBackupPlan({
			target: targetName,
			environment,
			projectRef,
			resources: target.environments[environment],
	...
	const allowWrites =
		args.apply === true &&
		new Set(["apply", "resume", "freeze", "thaw", "activate-routing", "deactivate-routing"]).has(
			command,
		);
		...
		if (!args.fixture && args["remote-read"] !== true)
			return emit(
				dryRun(
					...
					projectRef,
					args,
					"add --remote-read to capture both historical D1 databases",
				),
				args,
			);
		...
		return emit(
			{
				...
				source_bookmarks: Object.fromEntries(
					Object.entries(snapshot.sources).map(([source, value]) => [source, value.bookmark]),
				),
			},
			args,
		);
		...
		if (!args.fixture && args["remote-read"] !== true)
			return emit(
				dryRun(
					...
					projectRef,
					args,
					"add --remote-read to inspect the live maintenance state",
				),
				args,
			);
		...
		if (!args.fixture && args["remote-read"] !== true) {
			return emit(
				dryRun(
					...
					projectRef,
					args,
					"add --remote-read to inspect the live project routing state",
				),
				args,
			);
		}
	...
	if (command === "freeze" || command === "thaw")
		return maintenanceCommand(command, args, adapter, projectRef);
	...
	if (command === "plan") {
		...
		if (!args.output)
			return emit(
				{
					...stripProtectedRows(plan),
					ready: false,
					blocker: "protected_plan_output_required",
					next_command: `${baseCommand("plan", projectRef, args)} --output .flows-cutover/plans/${projectRef}.json`,
				},
				args,
			);
	...
	if (plan.project.project_ref !== projectRef)
		throw new Error("Protected plan project_ref mismatch");
	if (
		(args["remote-read"] === true || args.apply === true) &&
		typeof adapter.validatePlanIdentity === "function"
	) {
		...
		if (!args.fixture && args["remote-read"] !== true)
			return emit(
				dryRun(
					...
					projectRef,
					args,
					"add --remote-read to compare all canonical target checksums",
				),
				args,
			);
	...
	if (command === "apply" || command === "resume")
		return applyCommand(command, args, adapter, plan, projectRef);
...
async function createPlan(args, adapter, projectRef) {
	...
	if (snapshot.project?.project_ref !== projectRef)
		throw new Error("Snapshot project_ref mismatch");
	if (!args.fixture && args["remote-read"] !== true) {
		throw new Error(
			`Dry-run only: add --remote-read after reviewing ${baseCommand("plan", projectRef, args)}`,
		);
	}
	...
	if (!environmentId)
		throw new Error(
			"--flows-environment-id is required when the adapter cannot resolve the project environment",
		);
...
async function applyCommand(command, args, adapter, plan, projectRef) {
		...
		return emit(
			{
				...dryRun(
					...
					projectRef,
					args,
					"review backups, freeze receipt and protected plan before mutation",
				),
	...
	if (liveFreeze.enabled !== true || liveFreeze.window_id !== window.window_id)
		throw new Error("Live project freeze is not active for this cutover window");
	const checkpointPath = resolve(
		args.checkpoint ||
			`.flows-cutover/checkpoints/${projectRef}-${window.window_id}-${plan.plan_id}.json`,
	);
	let checkpoint;
	try {
		checkpoint = await readProtectedJson(checkpointPath);
	} catch (error) {
		if (error?.code !== "ENOENT") throw error;
	}
	if (command === "resume" && !checkpoint)
		throw new Error(
			"resume requires an existing --checkpoint file from the interrupted exact plan",
		);
	...
	const output = args.output
		? resolve(args.output)
		: resolve(`.flows-cutover/reports/${projectRef}-${plan.plan_id}-apply.json`);
	await writeProtectedJson(output, result);
	return emit(
		{
			...stripProtectedRows(result),
			protected_output: output,
			checkpoint: checkpointPath,
			next_required_step:
				"verify while the freeze remains enabled; thaw is always a separate explicit command",
		},
		args,
	);
}
...
async function maintenanceCommand(command, args, adapter, projectRef) {
	...
	const plan = await readProtectedJson(resolve(required(args, "plan")));
	assertPlanIntegrity(plan);
	if (window.project_ref !== projectRef || plan.project.project_ref !== projectRef)
		throw new Error("Freeze/thaw artifacts do not match project_ref");
	await adapter.validatePlanIdentity?.(plan, {
	...
	if (command === "freeze") {
		...
		if (args.apply !== true)
			return emit(
				{
					...dryRun(command, projectRef, args, "maintenance changes are never implicit"),
					exact_confirmation: expected,
					exact_command: `${baseCommand(command, projectRef, args)} --apply --confirm ${expected}`,
				},
				args,
		...
		await adapter.validateBackupReceiptIdentity?.(backupReceipt);
		const response = await adapter.setMaintenance(projectRef, {
			enabled: true,
			window_id: window.window_id,
			reason: window.reason,
		});
		if (response.enabled !== true || response.window_id !== window.window_id)
			throw new Error("Maintenance API did not confirm the requested freeze");
		const receipt = {
			schema_version: FLOWS_CUTOVER_SCHEMA_VERSION,
	...
	if (
		...
		throw new Error(
			"Thaw requires verified Flows routing to be active for this exact project/window/plan",
		);
	}
	if (args.apply !== true)
		return emit(
...
async function routingCommand(command, args, adapter, plan, projectRef) {
	...
	if (liveFreeze.enabled !== true || liveFreeze.window_id !== window.window_id) {
		throw new Error(
			"The exact project cutover window must remain frozen while changing Flows routing",
		);
	}
		...
		return emit(
			{
				...dryRun(
					...
					projectRef,
					args,
					"routing changes require an exact protected verification report and explicit confirmation",
				),
				verification_checksum_sha256: evidence.verification_checksum_sha256,
				exact_confirmation: expected,
				exact_command: `${baseCommand(command, projectRef, args)} --apply --confirm ${expected}`,
	...
	const idempotencyKey = `flows-routing-${enabled ? "enable" : "disable"}-${sha256(
		canonicalJson({
			project_ref: projectRef,
			...state,
		}),
	)}`;
	const response = await adapter.setFlowsRouting(projectRef, state, idempotencyKey);
	if (
...
async function routingGateCommand(args, adapter) {
	...
	for (const scope of requiredScopes) {
		...
		const verification = await readProtectedJson(
			resolve(required(args, `${scope.label}-verification`)),
		);
	...
	if (!args.fixture && args["remote-read"] !== true)
		return emit(
			dryRun(
				...
				projectRef,
				args,
				"add --remote-read to inspect post-cutover changes",
			),
			args,
		);
	const currentRowsByEntity = {};
	for (const item of plan.entities)
		currentRowsByEntity[item.id] = await adapter.readTarget(
			FLOW_CUTOVER_ENTITY_BY_ID.get(item.id),
			plan.project,
		);
	const delta = buildReverseDelta({ baselinePlan: plan, currentRowsByEntity });
	if (!args.output)
		throw new Error("reverse-delta requires --output <protected-reverse-delta.json>");
	await writeProtectedJson(resolve(args.output), delta);
	if (args["sql-output"])
		await writeProtectedJson(resolve(args["sql-output"]), reverseDeltaSql(delta));
	await emit(
		{
			...stripProtectedRows(delta),
			protected_output: resolve(args.output),
			sql_output: args["sql-output"] ? resolve(args["sql-output"]) : null,
		},
		args,
	);
	...
	return emit(
		{
			...rollback,
			protected_output: output,
			execution_mode: "fail-closed-orchestrator",
			exact_confirmation: expected,
			exact_instruction:
				"Review the protected reverse SQL and run each generated command one by one while the freeze remains enabled. This CLI intentionally never performs Time Travel restore, deletion, or thaw.",
		},
		args,
	);
}
...
async function backupReceiptCommand(args, projectRef) {
	...
	if (!args.output)
		throw new Error("backup-receipt requires --output <protected-backup-receipt.json>");
	if (backupPlan.ready !== true)
		throw new Error(`Backup plan is blocked: ${(backupPlan.blockers || []).join(", ")}`);
	if (backupPlan.project_ref !== projectRef || window.project_ref !== projectRef)
		throw new Error("Backup artifacts project_ref mismatch");
	const artifacts = [];
	for (const database of backupPlan.databases) {
		const item = (evidence.artifacts ?? []).find(
			(candidate) => candidate.logical_name === database.logical_name,
		);
		if (!item) throw new Error(`Evidence for ${database.logical_name} is required`);
		const file = resolve(database.local_path);
		const metadata = await lstat(file);
		if (metadata.isSymbolicLink())
			throw new Error(`D1 export for ${database.logical_name} must not be a symlink`);
		if (!metadata.isFile() || metadata.size < 1)
			throw new Error(`D1 export for ${database.logical_name} is missing or empty`);
...
async function selectAdapter(args, context) {
	...
	if (args["remote-read"] !== true && context.allowWrites !== true)
		return new FixtureFlowsCutoverAdapter();
	...
	return new RemoteFlowsCutoverAdapter({
		...context,
		gatewayToken: process.env.SUPERBOARD_CUTOVER_TOKEN,
		flowUserHashKey,
...
function instructions(args) {
	return {
		...
		bootstrap_if_flows_d1_is_unprovisioned:
		...
		root_command: "node packages/plugins/superboard-acquisition/scripts/flows-cutover.mjs",
		received_args: args,
	};
}
...
function dryRun(command, projectRef, args, reason) {
	return {
		...
		project_ref: projectRef,
		reason,
		reviewed_command: baseCommand(command, projectRef, args),
		automatic_deletion: false,
	};
}
...
function baseCommand(command, projectRef, args) {
	const pieces = [
		"node",
		"packages/plugins/superboard-acquisition/scripts/flows-cutover.mjs",
		command,
		"--target",
		FLOWS_CUTOVER_TARGET,
		"--environment",
	...
	for (const [key, value] of Object.entries(args)) {
		if (
			new Set(["target", "environment", "project-ref", "apply", "confirm"]).has(key) ||
			value === false ||
			value === undefined
		)
			continue;
...
function parseCli(argv) {
	...
	const args = {};
	for (
		let index = command === "instructions" && argv[0]?.startsWith("--") ? 0 : 1;
		index < argv.length;
		index += 1
	) {
...
async function emitProtectedOrSummary(value, args, label) {
	...
	return emit(
		{ ...value, note: `${label} can be persisted mode 0600 with --output <protected-file.json>` },
		args,
	);
}
