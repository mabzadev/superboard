...
import { platformsFilterList } from "@superboard/front-ui/constants/FilterOptions.js";
...
import { useRouter } from "@superboard/front-ui/navigation";
...
import { CampaignAnalyticsTable } from "./CampaignAnalyticsTable.js";
import { createCampaignAnalyticsColumns } from "./CampaignAnalyticsTableColumns.js";
...
export default function CampaignsPageContent() {
	...
	const filtered = useMemo(() => {
		...
		return campaigns
			.filter((campaign) =>
				archived ? campaign.status === ARCHIVED : campaign.status !== ARCHIVED,
			)
			.filter(
				(campaign) =>
					!normalizedSearch ||
			...
			{!selectedProject ? (
				...
				<div className="flex min-h-0 flex-col gap-4">
					...
					<CampaignAnalyticsTable
						...
						onOpenCampaign={(campaignId) => router.push(`/dynamic-links/campaigns/${campaignId}`)}
					/>
			...
			<Dialog open={createOpen} onOpenChange={setCreateOpen}>
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							Group dynamic links and measure their performance independently from email campaigns.
						</DialogDescription>
					</DialogHeader>
					<Input
						autoFocus
