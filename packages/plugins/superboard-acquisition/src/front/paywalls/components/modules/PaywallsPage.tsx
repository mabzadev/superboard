...
import { ExperienceFunctions } from "../../../ExperienceFunctions.js";
...
export function PaywallsPage() {
	...
	useItemExitGuard(
		Boolean(itemId) && (draftStatus !== "Saved" || workflowDirty),
	...
	const open = useCallback(
		(item: Paywall) => {
			...
			return getPaywallVersions(selectedProject.id, item.id)
				.then((result) => {
					...
					setDocument(fromPaywallDefinition(latest?.definition));
	...
	useEffect(() => {
		...
		if (item && selected?.id !== itemId) {
			void open(item).catch((cause: unknown) => {
				showErrorNotification(errorMessage(cause));
			});
		}
	}, [itemId, items, selected?.id, open]);
	...
	const create = async () => {
		...
		try {
			const result = await createPaywall(selectedProject.id, {
				identifier,
				display_name: name,
				configuration: toPaywallDefinition(createStudioDocument("paywall", locale)),
			});
			setName("");
			setIdentifier("");
	...
	const leaveItem = async () => {
		if (workflowDirty && !window.confirm(tr("Discard unsaved changes?"))) return;
		if (draftStatus !== "Saved" && (!documentValid || !(await saveDraft()))) {
			if (!window.confirm(tr("Discard unsaved changes?"))) return;
		}
	...
	return (
		<ModulePage
			title={itemId && selected?.id === itemId ? displayName(selected) : t("Paywalls")}
				...
				<div className="grid gap-6">
					...
					{itemId && (
						<Button
							...
							onClick={() => void leaveItem()}
						>
							{tr("Back to list")}
						</Button>
					)}
					{itemId && selected?.id !== itemId && (
						<div>
							<p role="status">
								{tr(
									error
										? "Unable to load item"
										: loaded && !items.some((item) => item.id === itemId)
											? "Item not found"
											: "Loading item…",
								)}
							</p>
						...
						<div className="space-y-6">
							...
							>
								<CardHeader>
									<CardTitle>
										{selected
											? `${t("Edit")} ${displayName(selected)}`
											: t("Visual paywall editor")}
									</CardTitle>
									<CardDescription>
							...
							<ExperienceFunctions section={studioTab} onDirtyChange={setWorkflowDirty} />
							{studioTab === "Results" && (
								<PaywallStatisticsPage key={selected.id} itemVersions={versions} />
							)}
...
export function PaywallStatisticsPage({ itemVersions }: { itemVersions?: PaywallVersion[] } = {}) {
	const { tr } = useFlowI18n();
	const [resultVersion, setResultVersion] = useState(itemVersions?.[0]?.id ?? "");
	const { t } = useExperienceI18n();
	const { selectedProject } = useProjectSelection();
	...
	useEffect(() => {
		if (!selectedProject || (itemVersions && !resultVersion)) return;
		setError(null);
		getPaywallStatistics(selectedProject.id, {
			...filters,
			...(itemVersions ? { version_id: resultVersion } : {}),
		})
			.then(setData)
			.catch((cause) => setError(errorMessage(cause)));
	}, [selectedProject, filters, itemVersions, resultVersion]);
	const summaryOrder = [
		"impression",
		"view",
		"cta",
