...
export function validateExperienceDocument(document: ExperienceDocument): ValidationIssue[] {
	...
	for (const [screenIndex, screen] of document.screens.entries()) {
		...
		if (screen.next_screen_id && !document.screens.some(({ id }) => id === screen.next_screen_id)) {
			issues.push({
...
export function fromPaywallDefinition(value?: Record<string, unknown>): ExperienceDocument {
...
export function fromOnboardingDefinition(value?: Record<string, unknown>): ExperienceDocument {
...
function readTheme(value: unknown): ExperienceTheme {
	...
	return {
		...
		background_color: readString(theme.background_color, DEFAULT_THEME.background_color),
		...
		corner_radius:
			typeof theme.corner_radius === "number" ? theme.corner_radius : DEFAULT_THEME.corner_radius,
	};
}
...
function readScreen(value: unknown): ExperienceScreen | null {
	...
	return {
		...
		blocks: Array.isArray(screen.blocks)
			...
			: [],
		...(typeof screen.next_screen_id === "string" ? { next_screen_id: screen.next_screen_id } : {}),
		...(screen.conditions &&
