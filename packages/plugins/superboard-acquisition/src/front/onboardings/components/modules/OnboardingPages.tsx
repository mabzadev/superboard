...
export function OnboardingsPage() {
	...
	const open = useCallback(
		(item: Onboarding) => {
			...
			return getOnboardingVersions(selectedProject.id, item.id)
				.then((result) => {
	...
	const create = async () => {
		...
		try {
			const result = await createOnboarding(selectedProject.id, {
				identifier,
				display_name: name,
				configuration: toOnboardingDefinition(createStudioDocument("onboarding", locale)),
			});
			setIdentifier("");
			setName("");
	...
	const leaveItem = async () => {
		...
		if (draftStatus !== "Saved" && (!documentValid || !(await save()))) {
	...
	return (
		<ModulePage
			title={itemId && selected?.id === itemId ? selected.display_name : t("Onboardings")}
				...
				<div className="grid gap-6">
					...
					{itemId && selected?.id === itemId && (
						<div className="space-y-6">
							...
							{studioTab === "Results" && (
								<OnboardingStatisticsPage key={selected.id} itemVersions={versions} />
							)}
							{selected && studioTab === "Delivery" && (
								<OnboardingDeliveryControls
									...
									rules={targetingRules.filter((rule) =>
										placements.some(
											(placement) =>
												placement.id === rule.placement_id &&
												placement.onboarding_id === selected.id,
										),
									)}
									experiences={experiences.filter((experience) =>
										placements.some(
											(placement) =>
												placement.id === experience.placement_id &&
												placement.onboarding_id === selected.id,
										),
									)}
									onChanged={load}
...
export function OnboardingStatisticsPage({
	itemVersions,
}: { itemVersions?: OnboardingVersion[] } = {}) {
	const { tr } = useFlowI18n();
	...
	useEffect(() => {
		if (selectedProject && (!itemVersions || resultVersion)) {
			setError(null);
			getOnboardingStatistics(selectedProject.id, {
				...filters,
