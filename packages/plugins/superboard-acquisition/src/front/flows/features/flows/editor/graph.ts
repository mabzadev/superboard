import type { FlowPropertyMeta, FlowPropertyType } from "@superboard/contracts/flows";
...
export function createBlock(
	...
	return {
		...
		componentLibraryName:
			type === "component" || type === "tour-component" || type === "survey" ? "Basics V2" : null,
...
export function createBlockFromDefinition(
	...
	return {
		...
		type: blockType,
		name: blockType === "tour-component" ? `${component.name} tour step` : component.name,
		description:
			typeof schema.description === "string" ? schema.description : `Render ${component.name}.`,
		position,
		componentType: component.component_type,
		componentLibraryName: component.library_name ?? component.library_identifier ?? "Basics V2",
		data,
		propertyMeta,
...
export function applyComponentDefinition(
	...
	const definitionBlock = createBlockFromDefinition(component, block.position, 1);
	const nextData = { ...definitionBlock.data };
	for (const key of Object.keys(nextData)) {
		if (key in block.data && key !== "componentKey" && key !== "componentVersion") {
	...
	return {
		...
		data: nextData,
		propertyMeta,
		surveyQuestions: definitionBlock.type === "survey" ? (block.surveyQuestions ?? []) : undefined,
	};
}
...
function componentProperties(schema: Record<string, unknown>): ComponentPropertyDefinition[] {
...
function componentPropertyDefault(property: ComponentPropertyDefinition): unknown {
	...
	switch (property.type) {
		...
		case "boolean":
			return false;
		case "select":
			return Array.isArray(property.options) && typeof property.options[0] === "string"
				? property.options[0]
				: "";
		case "array":
			return [];
...
export function validateGraph(graph: FlowGraph): FlowGraphValidation {
	...
	for (const block of graph.blocks) {
		if (ids.has(block.id)) issues.push({ blockId: block.id, message: "Block id must be unique." });
		ids.add(block.id);
		if (!block.key.trim())
			issues.push({
				blockId: block.id,
		...
		keys.add(block.key);
		if (!block.name.trim()) issues.push({ blockId: block.id, message: "Block name is required." });
		if (block.type === "delay") {
			const duration =
	...
	if (!graph.blocks.some((block) => block.type === "start" || block.type === "manual-start")) {
		issues.push({
...
export function normalizeGraph(value: unknown): FlowGraph {
	if (!value || typeof value !== "object") return { schemaVersion: 1, blocks: [], paths: [] };
