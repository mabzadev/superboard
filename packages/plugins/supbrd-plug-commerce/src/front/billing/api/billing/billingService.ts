...
export const mapBillingProductsToEntitlement = async (
	...
	(
		await POST(config.apiPath + `/billing/${projectId}/entitlements/${entitlementId}/products`, {
...
export const createBillingPackage = async (
	...
	(await POST(config.apiPath + `/billing/${projectId}/offerings/${offeringId}/packages`, data))
		.data;
...
export const searchBillingCustomers = async (
	...
	(await GET(purchasesV2Path(projectId, `/customers?q=${encodeURIComponent(query)}&limit=100`)))
		.data;
...
export const replayBillingDeadLetter = async (projectId: string, deadLetterId: string) =>
	(await POST(purchasesV2Path(projectId, `/dead-letters/${deadLetterId}/replay`), {})).data;

export const discardBillingDeadLetter = async (projectId: string, deadLetterId: string) =>
	(await POST(purchasesV2Path(projectId, `/dead-letters/${deadLetterId}/discard`), {})).data;
...
export const updateBillingReleaseGateCheck = async (
	...
	(
		await PATCH(
			purchasesV2Path(projectId, `/release-gate/checks/${encodeURIComponent(checkKey)}`),
			data,
		)
	).data.data;
...
export const rotateBillingCertificationDeviceChallenge = async (
	...
	(await POST(purchasesV2Path(projectId, `/certification-runs/${runId}/device-challenge`), {})).data
		.data;
...
export const recordBillingCertificationObservation = async (
	...
	(await POST(purchasesV2Path(projectId, `/certification-runs/${runId}/observations`), data)).data
		.data;
...
export const startBillingLegacyInventory = async (
	...
	(await POST(purchasesV2Path(projectId, "/legacy/revenuecat/inventory-runs"), { environment }))
		.data.data;
...
export const replayBillingWebhookDelivery = async (projectId: string, deliveryId: string) =>
	(await POST(purchasesV2Path(projectId, `/integrations/deliveries/${deliveryId}/replay`), {}))
		.data;
...
export const reviewBillingRefundEvidence = async (
	...
	(
		await POST(purchasesV2Path(projectId, `/refunds/${caseId}/evidence/${evidenceId}/review`), {
			approved,
		})
	).data;
...
export const updateBillingRefundAction = async (
	...
	(await PATCH(purchasesV2Path(projectId, `/refunds/${caseId}/actions/${actionId}`), { payload }))
		.data;
...
export const approveBillingRefundAction = async (
	...
	(await POST(purchasesV2Path(projectId, `/refunds/${caseId}/actions/${actionId}/approve`), {}))
		.data;
...
export const getBillingCustomer = async (
	...
	(await GET(purchasesV2Path(projectId, `/customers/${encodeURIComponent(customerId)}`))).data;
...
export const setBillingCustomerBlocked = async (
	...
	(
		await POST(purchasesV2Path(projectId, `/customers/${encodeURIComponent(customerId)}/block`), {
			blocked,
		})
	).data;
...
export const mergeBillingCustomers = async (
	...
	(
		await POST(purchasesV2Path(projectId, `/customers/${encodeURIComponent(customerId)}/merge`), {
			target_customer_id: targetCustomerId,
		})
	).data;

export const deleteBillingCustomer = async (projectId: string, customerId: string) =>
	(await DELETE(purchasesV2Path(projectId, `/customers/${encodeURIComponent(customerId)}`))).data;
...
export async function reconcileBillingStore(projectRef: string) {
	return POST(`/api/v1/billing/projects/${encodeURIComponent(projectRef)}/ledger/reconcile`, {});
}
export async function getBillingLedger(projectRef: string) {
	return GET(`/api/v1/billing/projects/${encodeURIComponent(projectRef)}/ledger`);
}
