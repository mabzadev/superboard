export const PRODUCT_TYPES = ["subscription", "non_consumable", "consumable"] as const;
...
export function parsePurchase(value: unknown) {
	const body = record(value);
	return {
		financial_customer_id: requiredText(body.financial_customer_id, "financial_customer_id"),
		...
		external_transaction_id: requiredText(body.external_transaction_id, "external_transaction_id"),
		...
		purchased_at: isoDate(body.purchased_at ?? new Date().toISOString(), "purchased_at"),
		expires_at: body.expires_at == null ? null : isoDate(body.expires_at, "expires_at"),
...
export function parseRefund(value: unknown) {
	...
	return {
		...
		amount_micros: integer(body.amount_micros ?? 0, "amount_micros", 0, Number.MAX_SAFE_INTEGER),
		currency: currency(body.currency),
		reason: optionalText(body.reason, 2_000),
		requested_at: isoDate(body.requested_at ?? new Date().toISOString(), "requested_at"),
		completed_at: body.completed_at == null ? null : isoDate(body.completed_at, "completed_at"),
...
export function parseSubscriptionUpdate(value: unknown) {
	...
	return {
		...
		cancelled_at: body.cancelled_at == null ? null : isoDate(body.cancelled_at, "cancelled_at"),
	};
}
...
export function requiredText(value: unknown, name: string, max = 255): string {
	if (typeof value !== "string") throw validationError(`${name}_required`, `${name} is required`);
	const result = value.trim();
	if (!result || result.length > max)
		throw validationError(`${name}_invalid`, `${name} must contain 1 to ${max} characters`);
	return result;
}
...
export function optionalText(value: unknown, max = 255): string | null {
	...
	if (result.length > max)
		throw validationError("text_too_long", `Text is limited to ${max} characters`);
	return result || null;
}
...
export function choice<const T extends readonly string[]>(
	...
	if (!values.includes(result))
		throw validationError(`${name}_invalid`, `${name} must be one of ${values.join(", ")}`);
...
export function isoDate(value: unknown, name: string): string {
	...
	if (Number.isNaN(date.getTime()))
		throw validationError(`${name}_invalid`, `${name} must be an ISO-8601 date`);
...
export function currency(value: unknown): string | null {
	...
	if (!/^[A-Z]{3}$/.test(result))
		throw validationError("currency_invalid", "currency must be an ISO-4217 code");
	return result;
}
...
export function validationError(code: string, message: string, status = 422): Error {
...
function identifier(value: unknown, name: string): string {
	...
	if (!/^[A-Za-z0-9][A-Za-z0-9._-]*$/.test(result))
		throw validationError(`${name}_invalid`, `${name} contains unsupported characters`);
	return result;
}
