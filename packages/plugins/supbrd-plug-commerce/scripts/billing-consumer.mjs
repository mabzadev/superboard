...
export function parseQueueConsumer(output) {
	const count = Number(/^[ \t]*Number of Consumers:[ \t]*(\d+)/m.exec(output)?.[1] ?? "0");
	...
	if (consumers.length > 1) throw new Error("Billing queue has more than one active consumer");
...
async function main() {
	...
	const current = queueConsumer(queue, configPath(targetName, "api", environment));
	if (current === expected) {
	...
	if (environment === "production" && resources.billingExecutionMode === "service") {
	...
	let added = false;
	try {
		run("npx", ["wrangler", "queues", "pause-delivery", queue, "--config", sourceConfig]);
		paused = true;
		if (current) {
			run("npx", [
				"wrangler",
		...
		addConsumer(queue, expected, resources.queues.billingDlq, destinationConfig);
		added = true;
		const verified = queueConsumer(queue, destinationConfig);
		if (verified !== expected)
			throw new Error(`Consumer verification returned ${verified ?? "none"}`);
		run("npx", ["wrangler", "queues", "resume-delivery", queue, "--config", destinationConfig]);
		paused = false;
		console.log(`Billing queue consumer moved from ${current ?? "none"} to ${expected}`);
	} catch (error) {
		...
		if (paused)
			tryRun("npx", ["wrangler", "queues", "resume-delivery", queue, "--config", sourceConfig]);
		throw error;
	}
}
...
function queueConsumer(queue, config) {
	const output = capture("npx", ["wrangler", "queues", "info", queue, "--config", config]);
...
async function assertBillingReadiness(apiDomain) {
	...
	if (!response.ok) throw new Error(`Billing readiness returned HTTP ${response.status}`);
	const payload = await response.json();
	if (payload?.ready_for_traffic !== true || payload?.execution !== "private-service-binding") {
		throw new Error("Billing is not ready for direct queue consumption");
	}
}

function configPath(target, service, environment) {
	return resolve(root, "infra", "generated", `${target}-${service}-${environment}.jsonc`);
}
