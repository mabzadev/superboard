...
const sources = [
	...
	"packages/plugins/superboard-core/src/front",
	"packages/plugins/superboard-authentification/src/front",
	"packages/plugins/superboard-data/src/front",
	"packages/plugins/superboard-monetization/src/front",
	"packages/plugins/superboard-communication/src/front",
	"packages/plugins/superboard-acquisition/src/front",
	"packages/plugins/superboard-support/src/front",
	"packages/plugins/superboard-analytics/src/front",
	"sdks/web/README.md",
	"sdks/web/src",
	"sdks/web/dist",
	...
	"packages/plugins/superboard-support/worker/src",
	"packages/plugins/superboard-communication/email/src",
...
const supportSurfaceForbidden = [
	...
	[
		"public compatibility banner",
		/\b(?:compatibility mode|migration status|migration progress)\b/iu,
	],
];
const artifactSources = [
	"apps/site/dist/client",
	"packages/plugins/superboard-core/dist",
	"packages/plugins/superboard-authentification/dist",
	"packages/plugins/superboard-data/dist",
	"packages/plugins/superboard-monetization/dist",
	"packages/plugins/superboard-communication/dist",
	"packages/plugins/superboard-acquisition/dist",
	"packages/plugins/superboard-support/dist",
	"packages/plugins/superboard-analytics/dist",
];
const failures = [];
...
// inputs for the parity gate, but must never be bundled into a public Front
...
const auditManifest = JSON.parse(
	await readFile(
		resolve(root, "packages/plugins/superboard-support/scripts/support-audit/capabilities.json"),
		"utf8",
	),
);
const internalAuditMarkers = [
	...
	[
		"internal Support reference commit",
		new RegExp(escapePattern(String(auditManifest.reference.commit)), "u"),
	],
	[
		"internal Support route digest",
		new RegExp(escapePattern(String(auditManifest.reference.canonical_route_sha256)), "u"),
	],
];
...
async function walk(directory) {
	...
	for (const entry of await readdir(directory, { withFileTypes: true })) {
		if (
			[".dart_tool", ".next", ".open-next", "node_modules", "test", "tests", "__tests__"].includes(
				entry.name,
			)
		)
			continue;
...
function ignored(file) {
	return (
		/(?:^|\/)(?:test|tests|__tests__|runtime-tests|migrations)(?:\/|$)/u.test(file) ||
...
function isPublishedSupportSurface(file) {
	return (
		file.startsWith("packages/plugins/superboard-support/worker/src/") ||
		/^workers\/api\/src\/lib\/support-gateway(?:\.|\/)/u.test(file) ||
		file.startsWith("packages/plugins/superboard-support/src/front/") ||
		...
		file.startsWith("tests/fixtures/contracts/support/")
	);
}
