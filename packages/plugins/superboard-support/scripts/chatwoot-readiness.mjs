...
import { chatwootOriginFromTarget } from "./chatwoot-export.mjs";
...
export function evaluateChatwootReadiness({
	...
	const blockers = [];
	if (!supportEnabled) blockers.push("superboard-support-disabled");
	...
	if (endpoint.status !== "reachable") blockers.push("chatwoot-endpoint-unreachable");
	if (profile.status !== "authenticated") blockers.push("chatwoot-token-unverified");
	...
	if (clientFiles.length > 0) retirementBlockers.push("legacy-client-code-present");
	...
	return {
		...
		retirement_gate:
			"Requires protected export, four backup receipts, verified SuperBoard import, client migration, rollback rehearsal and retention sign-off",
	};
}
...
function validEnvironmentInput(name, value) {
	...
	if (name === "CHATWOOT_ATTACHMENT_HOSTS") {
		return value
			.split(",")
			.every((item) => /^[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?$/iu.test(item.trim()));
	}
...
async function main() {
	...
	if (!resources) throw new Error(`Target ${targetName} does not define ${environment}`);
...
async function inspectEndpoint(origin) {
	try {
		...
		return {
			status: response.status >= 200 && response.status < 500 ? "reachable" : "unavailable",
...
async function inspectProfile(origin, accessToken) {
	try {
		...
		await response.body?.cancel().catch(() => undefined);
		return {
			status: response.status >= 200 && response.status < 300 ? "authenticated" : "rejected",
			http_status: response.status,
		};
	} catch {
		return { status: "unavailable", http_status: null };
...
export async function scanLegacyClient(value) {
	if (!isAbsolute(value)) throw new Error("--client-root must be an absolute path");
	...
	const result = spawnSync(
		...
		[
			...
			"--glob",
			"*.dart",
			"chatwoot|openchat|SupportChatWidget|supportInit|supportFetchMessages|supportSendMessage",
			clientRoot,
		],
		{
			encoding: "utf8",
...
const legacyClientPattern =
	/chatwoot|openchat|SupportChatWidget|supportInit|supportFetchMessages|supportSendMessage/iu;
