...
const root =
	requestedRoot(process.argv.slice(2)) ??
	resolve(fileURLToPath(new URL("../../../..", import.meta.url)));
const audit = resolve(root, "packages/plugins/superboard-support/scripts/support-audit");
...
const testRunners = Object.freeze({
	"vitest-support": {
		prefix: "tests/checks/plugins/superboard-support/worker/unit/",
		file: /\.test\.ts$/u,
	},
	"vitest-workers": {
		prefix: "tests/checks/plugins/superboard-support/worker/runtime/",
		file: /\.test\.ts$/u,
	},
	"vitest-api": {
		prefix: "tests/checks/plugins/superboard-core/api/unit/",
		file: /\.test\.ts$/u,
	},
	"vitest-plugin-front": {
		prefix: "tests/checks/plugins/",
		file: /\.test\.tsx?$/u,
	},
	"vitest-site-front": {
		prefix: "tests/checks/apps/site/",
		file: /\.test\.tsx?$/u,
	},
	"playwright-site": {
		prefix: "tests/e2e/site/",
		file: /\.(?:spec|test)\.[cm]?[jt]s$/u,
	},
	"flutter-test": {
		prefix: "tests/checks/sdks/",
		file: /_test\.dart$/u,
	},
});
...
if (routes.reference_commit !== capabilities.reference.commit)
	failures.push("reference commit drift");
if (routes.route_method_count !== capabilities.reference.route_method_count)
	failures.push("reference route count drift");
if (routes.routes.length !== capabilities.reference.route_method_count)
	failures.push("reference route inventory is incomplete");
if (routes.canonical_route_sha256 !== capabilities.reference.canonical_route_sha256)
	failures.push("reference route digest drift");

const byId = new Map();
for (const capability of capabilities.capabilities || []) {
	if (!capability.id || byId.has(capability.id))
		failures.push(`duplicate or missing capability id: ${capability.id || "<empty>"}`);
	byId.set(capability.id, capability);
	if (!allowed.has(capability.disposition))
		failures.push(`${capability.id}: invalid disposition ${capability.disposition}`);
	...
	if (capability.disposition === "implemented") {
		for (const dimension of [
			...
			if (!Array.isArray(evidence) || evidence.length === 0)
				failures.push(`${capability.id}: ${dimension} evidence is required`);
			if (dimension === "tests") {
		...
		if (!capability.reason)
			failures.push(`${capability.id}: ${capability.disposition} reason is required`);
		for (const file of [...(capability.contract || []), ...(capability.tests || [])])
			await checkEvidence(capability.id, file);
		if (
			capability.disposition === "delegated" &&
			(!capability.contract?.length || !capability.tests?.length)
		) {
...
for (const route of routes.routes || []) {
	if (!route.fingerprint || fingerprints.has(route.fingerprint))
		failures.push(`duplicate or missing route fingerprint: ${route.path || "<unknown>"}`);
	fingerprints.add(route.fingerprint);
	const capability = byId.get(route.capability);
	if (!capability)
		failures.push(`${route.fingerprint}: unmapped capability ${route.capability || "<empty>"}`);
	if (!allowed.has(route.disposition))
		failures.push(`${route.fingerprint}: invalid route disposition`);
	if (capability && capability.disposition !== route.disposition)
		failures.push(`${route.fingerprint}: disposition disagrees with ${route.capability}`);
	if (!route.mapping_rule || route.mapping_rule === "unknown")
		failures.push(`${route.fingerprint}: mapping rule is missing`);
	...
	if (createHash("sha256").update(canonical).digest("hex") !== route.fingerprint)
		failures.push(`${route.fingerprint}: route evidence was modified`);
}
...
const summary = Object.fromEntries(
	[...allowed].map((status) => [
		status,
		routes.routes.filter((route) => route.disposition === status).length,
	]),
);
console.log(
	JSON.stringify(
		{
			status: "ok",
			reference_routes: routes.routes.length,
			capabilities: byId.size,
...
async function checkTestProof(capability, proof) {
	...
	const runner =
		typeof proof.runner === "string" && Object.hasOwn(testRunners, proof.runner)
