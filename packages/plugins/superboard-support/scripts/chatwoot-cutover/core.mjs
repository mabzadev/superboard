...
import { canonicalJson, upsertSql } from "../../../../../scripts/database/module-cutover-core.mjs";
...
export const CHATWOOT_TABLES = Object.freeze([
	...
	entity(
		...
		["id"],
		["labels_json", "custom_attributes_json"],
	),
	entity(
		"messages",
		"messages",
...
export async function loadChatwootBundle(directory) {
	...
	const configuration = JSON.parse(
		await readFile(join(bundleDirectory, "configuration.json"), "utf8"),
	);
	const bundle = {
...
export async function verifyChatwootBundle(bundle) {
	...
	for (const entry of bundle.messages || []) {
		...
		for (const attachment of attachmentArray(message)) {
			const exported = object(attachment._superboard_export);
...
export function transformChatwootBundle(bundle, { projectId }) {
	...
	const contacts = uniqueBy(bundle.contacts || [], (contact) => String(contact.id)).map(
		(contact) => {
			const sourceId = positiveId(contact.id, "contact.id");
			const externalUserId =
				optionalText(contact.identifier) ||
			...
			return {
				...
				updated_at:
					timestamp(contact.updated_at ?? contact.last_activity_at ?? contact.created_at) ||
	...
	for (const [collection, entityType] of Object.entries(CONFIGURATION_TYPES)) {
		for (const [position, item] of listFromPayload(bundle.configuration?.[collection]).entries()) {
			...
			const name =
				optionalText(item.name ?? item.title ?? item.short_code ?? item.display_name) ||
				`${entityType} ${sourceId}`;
			...
			configuration.push({
				...
				name,
				enabled:
					item.enabled === false || item.status === "disabled" || entityType === "webhook" ? 0 : 1,
				position,
	...
	const conversations = uniqueBy(bundle.conversations || [], (conversation) =>
		...
		const externalUserId = optionalText(sender.identifier) || knownContact?.applicationIdentifier;
		if (!externalUserId) {
			blockers.push({
				code: "conversation_identity_missing",
				conversation_id: sourceId,
				message:
					"Chatwoot conversation has no contact identifier or email and cannot be attached to an application identity.",
			});
		}
		...
		const status =
			conversation.status === "resolved"
				...
				: conversation.status === "pending" || conversation.status === "snoozed"
					...
					: "open";
		const createdAt =
			timestamp(conversation.created_at ?? conversation.timestamp) || new Date(0).toISOString();
		const updatedAt =
			timestamp(conversation.updated_at ?? conversation.last_activity_at) || createdAt;
		return {
			id,
			project_id: projectId,
			external_user_id:
				externalUserId || `blocked-chatwoot-contact:${accountId}:${contactSourceId || sourceId}`,
			...
			assigned_user_id:
				conversation.meta?.assignee?.id == null
					? null
					: sourceKey(accountId, "agent", conversation.meta.assignee.id),
			labels_json: canonicalJson(Array.isArray(conversation.labels) ? conversation.labels : []),
			last_message_preview:
				optionalText(conversation.last_non_activity_message?.content)?.slice(0, 1000) || null,
			last_message_at: timestamp(conversation.last_activity_at),
			user_last_read_at: timestamp(conversation.contact_last_seen_at),
			agent_last_read_at: timestamp(
				conversation.agent_last_seen_at ?? conversation.assignee_last_seen_at,
			),
			inbox_id:
				conversation.inbox_id == null ? null : sourceKey(accountId, "inbox", conversation.inbox_id),
			assigned_team_id:
				conversation.team?.id == null && conversation.team_id == null
					? null
			...
			custom_attributes_json: canonicalJson({
				...
				_chatwoot: {
					...
					additional_attributes: object(conversation.additional_attributes),
				},
			}),
	...
	for (const entry of bundle.messages || []) {
		...
		if (!targetConversationId) {
			blockers.push({
				code: "message_conversation_missing",
		...
		for (const [offset, message] of sourceMessages.entries()) {
			...
			const normalizedAttachments = files.map((attachment, position) => {
				...
				if (!exported.relative_path || !exported.sha256 || !Number.isSafeInteger(exported.bytes)) {
					blockers.push({
						code: "attachment_export_missing",
						message_id: sourceId,
						attachment_id: attachmentSourceId,
					});
				} else {
					uploads.push({
						...
						content_type:
							optionalText(attachment.file_type ?? attachment.content_type) ||
			...
			messages.push({
				...
				sender_id:
					message.sender_id == null
						? `chatwoot:${senderKind(message)}`
						: sourceKey(accountId, senderKind(message), message.sender_id),
				...
				content_type: ["text", "input_email", "input_select", "cards", "form", "activity"].includes(
					originalContentType,
				)
					? originalContentType
					: Number(message.message_type) === 2
						? "activity"
						: "text",
				reply_to_message_id:
					message.content_attributes?.in_reply_to == null
						? null
				...
				delivery_status: ["sent", "delivered", "read", "failed"].includes(message.status)
					? message.status
					: "sent",
			});
		}
	...
	const evidence = Object.fromEntries(
		CHATWOOT_TABLES.map((table) => [table.id, datasetEvidence(rows[table.id])]),
	);
	return {
		schema_version: 1,
...
export function renderChatwootSql(transformation) {
	if (!transformation.ready) {
		throw new Error(
			`Chatwoot migration is blocked: ${transformation.blockers.map((item) => item.code).join(", ")}`,
		);
	}
...
function attachmentArray(message) {
	if (Array.isArray(message.attachments))
		return message.attachments.filter((item) => item && typeof item === "object");
...
function compareMessages(left, right) {
	const timeDifference =
		(Date.parse(timestamp(left.created_at) || "") || 0) -
...
function safeFilename(value) {
	const normalized = String(value || "attachment")
		.normalize("NFKC")
		// oxlint-disable-next-line no-control-regex -- Remove null bytes and path separators from imported filenames.
		.replace(/[\\/\0\r\n]/gu, "_")
