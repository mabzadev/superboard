...
import { initializeConversationPolicies, recordSlaResolution } from "./service-levels";
...
app.get("/public/v1/realtime/:ticket", async (c) => {
	if (c.req.header("Upgrade")?.toLowerCase() !== "websocket") {
		throw failure("realtime_upgrade_required", "WebSocket upgrade is required", 426);
	}
	const ticket = await consumeRealtimeTicket(c.env, c.req.param("ticket"));
	const conversation = await projectConversation(c.env, ticket.conversation_id, ticket.project_id);
  ...
  `,
		...
		.bind(crypto.randomUUID(), ticket.project_id, ticket.conversation_id, ticket.actor_id)
		.run();
...
app.get("/public/v1/providers/:provider/oauth/callback", async (c, next) => {
	const result = await completeIntegrationOAuth(c.env, c.req.raw, c.req.param("provider"));
...
app.get("/public/v1/help-center/:portalSlug/categories", async (c) => {
	...
	const rows = await c.env.DB.prepare(
     ...
     ORDER BY category.position, category.created_at`,
		...
		.bind(portal.id, portal.project_id)
...
app.get("/public/v1/help-center/:portalSlug/search", async (c) => {
	...
	if (query.length < 2 || query.length > 500) {
		throw failure(
			"help_center_query_invalid",
			"Help Center search query must contain between 2 and 500 characters",
			422,
		);
	}
...
app.get("/public/v1/help-center/:portalSlug/articles/:articleSlug", async (c) => {
	...
	if (!article)
		throw failure("help_center_article_not_found", "Help Center article not found", 404);
...
app.post("/public/v1/help-center/:portalSlug/articles/:articleSlug/views", async (c) => {
	...
	const article = await c.env.DB.prepare(
		...
		.bind(portal.id, portal.project_id, c.req.param("articleSlug"))
		.first<{ id: string }>();
	if (!article)
		throw failure("help_center_article_not_found", "Help Center article not found", 404);
	const source =
		c.req.header("CF-Connecting-IP") || c.req.header("User-Agent") || crypto.randomUUID();
	...
	await c.env.DB.prepare(
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
		...
		.bind(
			...
			portal.id,
			article.id,
			(await sha256(source)).slice(0, 64),
			String(c.req.query("locale") || portal.locale).slice(0, 32),
			safeReferrerHost(c.req.header("Referer")),
		)
		.run();
...
app.use("/v1/*", async (c, next) => {
	...
	const claim = await c.env.DB.prepare(
     ...
     ON CONFLICT(project_id, key) DO NOTHING RETURNING key`,
		...
		.bind(projectId, scope, c.req.method, pathname)
		.first();
	if (!claim) {
	...
	try {
       ...
       WHERE project_id = ? AND key = ?`,
			...
			.bind(c.res.status, responseBody, projectId, scope)
			.run();
	} catch (error) {
		await c.env.DB.prepare("DELETE FROM support_idempotency_keys WHERE project_id = ? AND key = ?")
			.bind(projectId, scope)
			.run();
		throw error;
	}
...
app.patch("/v1/contact", async (c) => {
	...
	const custom =
		body.custom_attributes == null ? null : validateCustomAttributes(body.custom_attributes);
	await ensureSupportContact(c.env.DB, projectId, subject);
...
app.post("/v1/events", async (c) => {
	...
	if (!idempotencyKey)
		throw failure("idempotency_key_required", "Idempotency-Key is required", 422);
	...
	const inserted = await c.env.DB.prepare(
     ...
     ON CONFLICT(project_id, idempotency_key) DO NOTHING RETURNING id, event_name, occurred_at`,
		...
		.first<Record<string, unknown>>();
	return c.json(
		{ data: inserted || { id, event_name: name, duplicate: true } },
		inserted ? 201 : 200,
	);
});
...
app.get("/v1/inboxes/:inboxId/members", async (c) => {
	...
	const rows = await c.env.DB.prepare(
     ...
     ORDER BY membership.display_name LIMIT 100`,
		...
		.bind(projectId, c.req.param("inboxId"))
		.all();
...
app.get("/v1/conversations/:conversationId/labels", async (c) => {
	...
	const rows = await c.env.DB.prepare(
     ...
     WHERE linked.project_id = ? AND linked.conversation_id = ? AND label.active = 1 ORDER BY label.name`,
		...
		.bind(projectId, conversation.id)
		.all();
	return c.json({ data: rows.results });
});
...
app.post("/v1/conversations/:conversationId/transcript", async (c) => {
	...
	const transcript = messages.results
		...
		.join("\n");
	return c.json({
		data: { conversation_id: conversation.id, transcript, message_count: messages.results.length },
	});
});
...
app.get("/v1/help-center/:portalSlug/categories", (c) =>
	app.fetch(
		remapPublicPath(
			c.req.raw,
			`/public/v1/help-center/${encodeURIComponent(c.req.param("portalSlug"))}/categories`,
		),
		c.env,
	),
);
app.get("/v1/help-center/:portalSlug/search", (c) =>
	app.fetch(
		remapPublicPath(
			c.req.raw,
			`/public/v1/help-center/${encodeURIComponent(c.req.param("portalSlug"))}/search`,
		),
		c.env,
	),
);
app.get("/v1/help-center/:portalSlug/articles/:articleSlug", (c) =>
	app.fetch(
		remapPublicPath(
			c.req.raw,
			`/public/v1/help-center/${encodeURIComponent(c.req.param("portalSlug"))}/articles/${encodeURIComponent(c.req.param("articleSlug"))}`,
		),
		c.env,
	),
);
app.post("/v1/help-center/:portalSlug/articles/:articleSlug/views", (c) =>
	app.fetch(
		remapPublicPath(
			c.req.raw,
			`/public/v1/help-center/${encodeURIComponent(c.req.param("portalSlug"))}/articles/${encodeURIComponent(c.req.param("articleSlug"))}/views`,
		),
		c.env,
	),
);
...
app.post("/v1/conversations", async (c) => {
	...
	const projectId = requireProject(c.env, c.req.header("X-SuperBoard-Project-Id"));
	const body = await readJsonObject(c.req.raw);
	...
	if (!clientId || clientId.length > 128)
		throw failure("client_conversation_id_invalid", "A client conversation id is required", 422);
  ...
  `,
	...
	if (inserted) {
		...
		await publishPolicyInitializationEvents(c.env, projectId, String(conversation.id), initialized);
		conversation = await userConversationRecord(c.env, String(conversation.id), subject, projectId);
	}
	...
	return c.json({ data: publicConversationShape(conversation) }, inserted ? 201 : 200);
});
...
app.patch("/v1/conversations/:conversationId", async (c) => {
	...
	if (status && !["open", "pending", "closed"].includes(status))
		throw failure("status_invalid", "Conversation status is invalid");
	const snoozeProvided = Object.hasOwn(body, "snoozed_until");
	const snoozedUntil =
		!snoozeProvided || body.snoozed_until == null ? null : String(body.snoozed_until).trim();
	if (
		snoozedUntil &&
		(!Number.isFinite(Date.parse(snoozedUntil)) || Date.parse(snoozedUntil) <= Date.now())
	) {
		throw failure("snoozed_until_invalid", "Snooze time must be a future timestamp");
	}
	const customAttributes =
		body.custom_attributes == null ? null : validateCustomAttributes(body.custom_attributes);
	if (status == null && customAttributes == null && !snoozeProvided)
		throw failure("conversation_update_empty", "No supported conversation update was provided");
	await c.env.DB.prepare(
		`
...
app.post("/v1/conversations/:conversationId/csat", async (c) => {
	...
	if (conversation.status !== "closed")
		throw failure("csat_not_available", "CSAT is available after the conversation is closed", 409);
	const body = await readJsonObject(c.req.raw);
	const rating = Number(body.rating);
	if (!Number.isInteger(rating) || rating < 1 || rating > 5)
		throw failure("csat_rating_invalid", "CSAT rating must be between 1 and 5");
	const feedback =
		body.feedback == null ? null : String(body.feedback).trim().slice(0, 4000) || null;
	const id = crypto.randomUUID();
	await c.env.DB.prepare(
		`
...
app.get("/v1/conversations/:conversationId/messages", async (c) => {
  ...
  `,
		...
		.bind(conversation.id, Number.isFinite(before) ? before : Number.MAX_SAFE_INTEGER, limit + 1)
		.all<Record<string, unknown>>();
	...
	return c.json({
		...
		meta: {
			next_cursor:
				rows.results.length > limit && oldest
...
app.post("/v1/conversations/:conversationId/realtime-ticket", async (c) => {
	...
	const issued = await issueRealtimeTicket(c.env, projectId, conversation.id, subject, "user");
...
app.post("/v1/conversations/:conversationId/meetings", async (c) => {
	...
	if (!integration) {
		throw failure("configuration_required", "Support meeting integration is not configured", 422);
	}
	...
	if (!organizationId || !apiKey || !preset) {
		throw failure(
			...
			"Support meeting integration is not fully configured",
			422,
		);
	}
	...
	if (requestedId) {
      ...
      WHERE id = ? AND project_id = ? AND conversation_id = ? AND status IN ('created', 'active')`)
			.bind(requestedId, projectId, conversation.id)
		...
		if (!providerMeetingId) {
			throw failure(
				...
				"Support meeting provider returned an invalid response",
				502,
			);
		}
	}
	const contact = await c.env.DB.prepare(`SELECT name FROM support_contacts
    WHERE project_id = ? AND external_user_id = ? LIMIT 1`)
		.bind(projectId, subject)
	...
	if (!participantToken) {
		throw failure(
			"support_meeting_unavailable",
			"Support meeting participant could not be created",
			502,
		);
	}
	const meeting = await c.env.DB.prepare(
     ...
     ON CONFLICT(project_id, provider, provider_reference) DO UPDATE SET
       status = 'active', updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') RETURNING *`,
		...
		.first<Record<string, unknown>>();
	if (!meeting) {
		throw failure("support_meeting_unavailable", "Support meeting could not be created", 503);
	}
	return c.json(
		{
			data: {
...
app.use("/internal/v1/*", async (c, next) => {
	...
	const pathProject = /^\/internal\/v1\/projects\/(\d+)(?:\/|$)/.exec(url.pathname);
...
app.use("/internal/v1/*", async (c, next) => {
	...
	if (
		...
		!["owner", "admin"].includes(String(c.req.header("X-Role") || "").toLowerCase())
	) {
		throw failure(
			"role_insufficient",
...
app.use("/internal/v1/*", async (c, next) => {
	if (["GET", "HEAD", "OPTIONS"].includes(c.req.method)) return next();
	const projectId = positiveInt(c.req.header("X-Project-Id") || "", "project_id_invalid");
	const key = String(c.req.header("Idempotency-Key") || "").trim();
  ...
  `,
	...
	try {
      ...
      `,
				...
				String(c.req.header("X-SuperBoard-Agent-Id") || "system").slice(0, 255),
				JSON.stringify({
					status,
					request_id: c.req.header("X-Request-Id") || null,
				}),
...
app.delete("/internal/v1/projects/:projectId/application/users/:userId", async (c) => {
	...
	for (const conversation of conversations.results) {
		const stub = c.env.CONVERSATIONS.get(c.env.CONVERSATIONS.idFromName(conversation.id));
	...
	for (let index = 0; index < attachments.results.length; index += 1_000) {
		await c.env.ATTACHMENTS.delete(
			attachments.results.slice(index, index + 1_000).map((attachment) => attachment.storage_key),
		);
	}
	...
	return c.json({
		data: {
			...
			contacts_deleted: Number(results[1].meta.changes || 0) + Number(results[2].meta.changes || 0),
...
export async function listProjectConversations(
    ...
    WHERE c.project_id = ? AND (? = '' OR c.status = ?) AND (? = '' OR c.inbox_id = ?)
  ...
  `,
		...
		.bind(projectId, status, status, inboxId, inboxId)
		.all<Record<string, unknown>>();
	return rows.results;
}
...
export function mapConversationItem(row: Record<string, unknown>) {
	const status = ["open", "pending", "closed"].includes(String(row.status))
		...
		: "open";
	const priority = ["low", "normal", "high", "urgent"].includes(String(row.priority))
		? (String(row.priority) as "low" | "normal" | "high" | "urgent")
		: "normal";
	const sourceId = String(row.id || "");
	const rawDate = String(row.last_message_at || row.updated_at || row.created_at || "");
	...
	return {
		...
		status,
		priority,
		customer_reference: row.external_user_id ? String(row.external_user_id) : null,
...
app.post("/internal/v1/projects/:projectId/conversations", async (c) => {
	...
	if (!externalUserId || externalUserId.length > 255)
		throw failure("external_user_id_invalid", "A valid external user id is required");
	if (!clientId || clientId.length > 128)
		throw failure("client_conversation_id_invalid", "A client conversation id is required");
	if (inboxId) await requireEnabledInbox(c.env.DB, projectId, inboxId);
...
app.get("/internal/v1/projects/:projectId/conversations/:conversationId/messages", async (c) => {
...
app.post("/internal/v1/projects/:projectId/conversations/:conversationId/messages", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"), "project_id_invalid");
	const conversation = await projectConversation(c.env, c.req.param("conversationId"), projectId);
	const agentId = c.req.header("X-SuperBoard-Agent-Id") || "";
...
for (const method of ["PATCH", "DELETE"] as const) {
	app.on(
		...
		async (c) => {
			...
			const actorId = requiredAgent(c.req.header("X-SuperBoard-Agent-Id"));
			return roomFetch(
				c.env,
				conversation.id,
...
app.post(
	...
	async (c) => {
		...
		const conversation = await projectConversation(c.env, c.req.param("conversationId"), projectId);
		const actorId = requiredAgent(c.req.header("X-SuperBoard-Agent-Id"));
		const issued = await issueRealtimeTicket(c.env, projectId, conversation.id, actorId, "agent");
		await c.env.DB.prepare(
			`
    INSERT INTO support_operations_audit_events
      (id, project_id, resource_type, resource_id, action, actor_id, payload_json)
...
app.post("/internal/v1/projects/:projectId/conversations/:conversationId/read", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"), "project_id_invalid");
	const conversation = await projectConversation(c.env, c.req.param("conversationId"), projectId);
	const agentId = requiredAgent(c.req.header("X-SuperBoard-Agent-Id"));
	return roomFetch(c.env, conversation.id, { id: agentId, kind: "agent" }, "/read", c.req.raw);
});

app.post("/internal/v1/projects/:projectId/conversations/:conversationId/typing", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"), "project_id_invalid");
	const conversation = await projectConversation(c.env, c.req.param("conversationId"), projectId);
	const agentId = requiredAgent(c.req.header("X-SuperBoard-Agent-Id"));
...
app.get("/internal/v1/projects/:projectId/conversations/:conversationId/ws", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"), "project_id_invalid");
	const conversation = await projectConversation(c.env, c.req.param("conversationId"), projectId);
	const agentId = requiredAgent(c.req.header("X-SuperBoard-Agent-Id"));
...
app.get(
	...
	async (c) => {
		const projectId = positiveInt(c.req.param("projectId"), "project_id_invalid");
		const conversation = await projectConversation(c.env, c.req.param("conversationId"), projectId);
		requiredAgent(c.req.header("X-SuperBoard-Agent-Id"));
		return attachmentResponse(
			c.env,
			conversation,
			c.req.param("messageId"),
...
app.patch("/internal/v1/projects/:projectId/conversations/:conversationId", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"), "project_id_invalid");
	const conversation = await projectConversation(c.env, c.req.param("conversationId"), projectId);
	const agentId = c.req.header("X-SuperBoard-Agent-Id") || "";
	if (!agentId) throw failure("agent_required", "Agent identity is required", 401);
	...
	const assigned =
		body.assigned_user_id == null ? null : String(body.assigned_user_id).slice(0, 255);
	const assignedTeam =
	...
	const snoozedUntil =
		body.snoozed_until == null ? null : validTimestamp(body.snoozed_until, "snoozed_until");
	const customAttributes =
		body.custom_attributes == null ? null : validateCustomAttributes(body.custom_attributes);
	const labels =
		body.labels == null
	...
	if (inboxId) await requireEnabledInbox(c.env.DB, projectId, inboxId);
	if (assigned)
		await requireEnabledConfiguration(c.env.DB, projectId, "agent", assigned, "auth_user_id");
...
app.get("/internal/v1/projects/:projectId/quality", async (c) => {
	...
	const [csat, conversations, responseTimes] = await Promise.all([
      ...
      FROM support_csat_responses WHERE project_id = ? AND (? IS NULL OR conversation_id IN (SELECT id FROM conversations WHERE inbox_id = ?))
    `,
		)
      ...
      FROM conversations WHERE project_id = ? AND (? IS NULL OR inbox_id = ?)
    `,
		)
			.bind(projectId, inboxId, inboxId)
			.first<Record<string, number>>(),
...
app.get("/internal/v1/projects/:projectId/audit", async (c) => {
    ...
    SELECT id, event_type action, actor_kind, actor_id, payload_json, created_at, 'conversation' source
      FROM support_audit_events WHERE project_id = ? AND (? IS NULL OR conversation_id IN (SELECT id FROM conversations WHERE inbox_id = ?))
    UNION ALL
    SELECT id, action, 'agent' actor_kind, actor_id, payload_json, created_at, resource_type source
      FROM support_operations_audit_events WHERE project_id = ? AND (? IS NULL
        OR (resource_type = 'workforce/inboxes' AND resource_id = ?)
        OR json_extract(payload_json, '$.inbox_id') = ?
        OR EXISTS (SELECT 1 FROM json_each(payload_json, '$.inbox_ids') WHERE value = ?)
        OR (resource_type = 'workforce/inbox-members' AND substr(resource_id, 1, length(?) + 1) = ? || ':')
        OR (resource_type = 'providers' AND resource_id IN (SELECT id FROM support_provider_endpoints WHERE inbox_id = ?)))
    UNION ALL
    SELECT id, action, 'agent' actor_kind, actor_id,
      ...
      FROM support_secret_audit_events WHERE project_id = ? AND ? IS NULL
...
function roomFetch(
	...
	return stub.fetch(
		new Request(`https://room.internal${path}`, {
			...
			headers,
			body: source.method === "GET" || source.method === "HEAD" ? null : source.body,
		}),
	);
}
...
function applicationUserId(value: string): string {
	...
	if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,254}$/u.test(resolved)) {
		throw failure("application_user_id_invalid", "Application user identifier is invalid", 422);
	}
	return resolved;
}
...
function minimumSupportRole(method: string, pathname: string): SupportRole {
	...
	if (/\/(?:providers|channels|integrations)(?:\/|$)/.test(pathname) && mutation) {
		return "admin";
	}
	if (
...
async function publicContact(db: D1Database, projectId: number, externalUserId: string) {
	const row = await db
		.prepare("SELECT * FROM support_contacts WHERE project_id = ? AND external_user_id = ?")
...
async function dyteRequest(
	...
	try {
		...
		throw failure(
			"support_meeting_unavailable",
			"Support meeting provider is temporarily unavailable",
			503,
		);
	}
...
function optionalIdentifier(value: unknown, field: string) {
	...
	if (!parsed || parsed.length > 255)
		throw failure(`${field}_invalid`, `${field} must be at most 255 characters`);
	return parsed;
}
...
function validateCustomAttributes(value: unknown): Record<string, unknown> {
	...
	if (typeof value !== "object" || Array.isArray(value))
		throw failure("custom_attributes_invalid", "Custom attributes must be an object");
	...
	if (serialized.length > 8000)
		throw failure("custom_attributes_invalid", "Custom attributes are limited to 8 KB");
...
async function requireEnabledInbox(db: D1Database, projectId: number, inboxId: string) {
  ...
  `,
		...
		.first();
	if (!row) throw failure("inbox_not_found", "Active Support inbox not found", 404);
}
...
async function requireEnabledConfiguration(
    ...
    `,
		...
		.first();
	if (!row) throw failure(`${type}_not_found`, `Active Support ${type} not found`, 404);
}
	...
	if (existing) {
		if (
			...
			throw failure(
				...
				"Idempotency-Key was already used with another attachment",
				409,
			);
		}
...
function contextualInternalRequest(request: Request) {
	...
	const headers = new Headers(request.headers);
	if (!headers.has("x-superboard-agent-id") && headers.has("x-actor-id")) {
		headers.set("x-superboard-agent-id", headers.get("x-actor-id") || "");
	}
...
const lifecycleWorker = {
	async fetch(request, env, ctx) {
		const url = new URL(request.url);
		if (url.pathname.startsWith("/internal/v1") && url.pathname !== "/internal/v1/health") {
			try {
...
export default withPluginTaskLifecycle("supbrd-plugmod-support", lifecycleWorker);
