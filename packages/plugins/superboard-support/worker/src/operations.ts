	...
	try {
      ...
      `).bind(
		...
		throw conflict(
			error,
			"contact_conflict",
			"A contact with this identity or email already exists",
		);
	}
...
operations.post("/:projectId/contacts/imports", async (c) => {
	...
	if (!queue)
		throw failure("support_bulk_unavailable", "Support bulk processing is not configured", 503);
	...
	try {
		...
		throw failure(
			...
			"Support bulk processing is temporarily unavailable",
			503,
		);
	}
...
operations.get("/:projectId/contacts/imports/:jobId", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"));
	return c.json({
		data: await importJob(c.env.DB, projectId, identifier(c.req.param("jobId"), "job_id")),
	});
});
...
operations.get("/:projectId/contacts/exports/:jobId", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"));
	return c.json({
		data: await exportJob(c.env.DB, projectId, identifier(c.req.param("jobId"), "job_id")),
	});
});
...
operations.get("/:projectId/contacts/exports/:jobId/download", async (c) => {
    ...
    WHERE project_id = ? AND resource_type = 'contacts' AND id = ?`)
	...
	if (!object)
		throw failure("contact_export_unavailable", "Contact export is temporarily unavailable", 503);
...
operations.get("/:projectId/contacts/:contactId", async (c) => {
    ...
    `)
			.bind(projectId, row.external_user_id)
			.all<Record<string, unknown>>(),
    ...
    `)
			.bind(projectId, projectId, row.external_user_id)
			.all<Record<string, unknown>>(),
	]);
	return c.json({
		data: {
...
operations.patch("/:projectId/contacts/:contactId", async (c) => {
	...
	try {
      ...
      `).bind(
				...
				merged.avatar_url,
				merged.blocked ? 1 : 0,
				JSON.stringify(merged.custom_attributes),
				merged.last_seen_at,
				projectId,
	...
	return c.json({
		data: serializeContact(await contact(c.env.DB, projectId, c.req.param("contactId"))),
	});
});
...
operations.delete("/:projectId/contacts/:contactId", async (c) => {
	...
	await c.env.DB.batch([
		c.env.DB.prepare("DELETE FROM support_contacts WHERE project_id = ? AND id = ?").bind(
			projectId,
			contactId,
		),
...
operations.post("/:projectId/contacts/:contactId/merge", async (c) => {
	...
	if (sourceId === targetId)
		throw failure("contact_merge_invalid", "Source and target contacts must be different");
  ...
  `)
		.bind(target.external_user_id, projectId, source.external_user_id)
		.first();
	if (collision)
		throw failure("contact_merge_conflict", "Contacts contain conflicting conversations", 409);
	const targetAttributes = object(
		parseJson(target.custom_attributes_json, {}),
		"custom_attributes",
		8_000,
	);
	const sourceAttributes = object(
		parseJson(source.custom_attributes_json, {}),
		"custom_attributes",
		8_000,
	);
	...
	await c.env.DB.batch([
      ...
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
      WHERE project_id = ? AND external_user_id = ?`).bind(
			target.external_user_id,
			projectId,
			source.external_user_id,
		),
		c.env.DB.prepare(
			"UPDATE support_contact_notes SET contact_id = ? WHERE project_id = ? AND contact_id = ?",
		).bind(targetId, projectId, sourceId),
		c.env.DB.prepare(
			"UPDATE OR IGNORE support_contact_events SET contact_id = ? WHERE project_id = ? AND contact_id = ?",
		).bind(targetId, projectId, sourceId),
		c.env.DB.prepare(
			"DELETE FROM support_contact_events WHERE project_id = ? AND contact_id = ?",
		).bind(projectId, sourceId),
		c.env.DB.prepare(
			"UPDATE support_campaign_deliveries SET contact_id = ? WHERE project_id = ? AND contact_id = ?",
		).bind(targetId, projectId, sourceId),
		c.env.DB.prepare(
			"UPDATE OR IGNORE support_contact_inboxes SET contact_id = ? WHERE project_id = ? AND contact_id = ?",
		).bind(targetId, projectId, sourceId),
		c.env.DB.prepare(
			"DELETE FROM support_contact_inboxes WHERE project_id = ? AND contact_id = ?",
		).bind(projectId, sourceId),
		c.env.DB.prepare(
			"UPDATE OR IGNORE support_contact_labels SET contact_id = ? WHERE project_id = ? AND contact_id = ?",
		).bind(targetId, projectId, sourceId),
		c.env.DB.prepare(
			"DELETE FROM support_contact_labels WHERE project_id = ? AND contact_id = ?",
		).bind(projectId, sourceId),
		c.env.DB.prepare("DELETE FROM support_contacts WHERE project_id = ? AND id = ?").bind(
			projectId,
			sourceId,
	...
	return c.json({
		data: {
			contact: serializeContact(await contact(c.env.DB, projectId, targetId)),
			merged_contact_id: sourceId,
		},
	});
});
...
operations.put("/:projectId/contacts/:contactId/labels", async (c) => {
    ...
    `).bind(projectId, contactId, labelId, actorId),
		...
		auditStatement(c.env.DB, projectId, "contact", contactId, "labels.updated", actorId, {
			label_ids: labelIds,
		}),
	];
...
operations.delete("/:projectId/contacts/:contactId/inboxes/:associationId", async (c) => {
  ...
  `)
		...
		.first<Record<string, unknown>>();
	if (!existing)
		throw failure("contact_inbox_not_found", "Contact inbox association not found", 404);
	await c.env.DB.batch([
		c.env.DB.prepare(
			"DELETE FROM support_contact_inboxes WHERE project_id = ? AND contact_id = ? AND id = ?",
		).bind(projectId, contactId, associationId),
...
operations.get("/:projectId/contacts/:contactId/notes", async (c) => {
  ...
  `)
		.bind(projectId, c.req.param("contactId"))
		.all();
...
operations.post("/:projectId/contacts/:contactId/notes", async (c) => {
    ...
    `).bind(id, projectId, c.req.param("contactId"), content, actorId),
		auditStatement(c.env.DB, projectId, "contact_note", id, "created", actorId, {
	...
	return c.json(
		{
			data: await c.env.DB.prepare(
				"SELECT * FROM support_contact_notes WHERE id = ? AND project_id = ?",
			)
				.bind(id, projectId)
				.first(),
		},
...
operations.delete("/:projectId/contacts/:contactId/notes/:noteId", async (c) => {
  ...
  `)
	...
	await c.env.DB.batch([
		c.env.DB.prepare(
			"DELETE FROM support_contact_notes WHERE id = ? AND contact_id = ? AND project_id = ?",
		).bind(c.req.param("noteId"), c.req.param("contactId"), projectId),
		auditStatement(
			c.env.DB,
			projectId,
...
operations.get("/:projectId/companies/:companyId", async (c) => {
  ...
  `)
		...
		.all<Record<string, unknown>>();
	return c.json({
		data: { ...serializeCompany(row), contacts: contacts.results.map(serializeContact) },
	});
});
...
operations.delete("/:projectId/companies/:companyId", async (c) => {
	...
	const count = await c.env.DB.prepare(
		"SELECT COUNT(*) count FROM support_contacts WHERE project_id = ? AND company_id = ?",
	)
		.bind(projectId, companyId)
		.first<{ count: number }>();
	await c.env.DB.batch([
		c.env.DB.prepare(
			"UPDATE support_contacts SET company_id = NULL, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE project_id = ? AND company_id = ?",
		).bind(projectId, companyId),
		c.env.DB.prepare("DELETE FROM support_companies WHERE project_id = ? AND id = ?").bind(
			projectId,
			companyId,
		),
	...
	return c.json({
		data: { id: companyId, deleted: true, detached_contacts: Number(count?.count || 0) },
	});
});
...
const handleConversationBulk: Handler<{ Bindings: Env }> = async (c) => {
	...
	const actorId = requestActor(c.req.header.bind(c.req));
	const body = await readJsonObject(c.req.raw);
	const conversationIds = identifierArray(
		body.conversation_ids,
		"conversation_ids",
		MAX_PAGE_LIMIT,
	);
	if (!conversationIds.length)
		throw failure("conversation_ids_invalid", "conversation_ids must not be empty");
	...
	if (action === "set_status" || action === "close" || action === "reopen") {
		const status =
			action === "close"
				...
				: action === "reopen"
					? "open"
					: enumValue(body.status, "status", ["open", "pending", "closed"]);
		statements.push(
			c.env.DB.prepare(`UPDATE conversations SET status = ?,
      ...
      WHERE project_id = ? AND id IN (${placeholders})`).bind(
		...
		statements.push(
			c.env.DB.prepare(`UPDATE conversations SET priority = ?,
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
      WHERE project_id = ? AND id IN (${placeholders})`).bind(
				priority,
				projectId,
      ...
      WHERE project_id = ? AND id IN (${placeholders})`).bind(
		...
		const inboxId =
			action === "transfer"
				? identifier(body.inbox_id, "inbox_id")
		...
		if (inboxId && membershipId)
			await requireInboxMembership(c.env.DB, projectId, inboxId, membershipId);
		statements.push(
			c.env.DB.prepare(`UPDATE conversations SET inbox_id = COALESCE(?, inbox_id),
      ...
      WHERE project_id = ? AND id IN (${placeholders})`).bind(
        ...
        SELECT ?, id, ?, ? FROM conversations WHERE project_id = ? AND id IN (${placeholders})`).bind(
						projectId,
						labelId,
						actorId,
						projectId,
        ...
        WHERE project_id = ? AND label_id = ? AND conversation_id IN (${placeholders})`).bind(
						projectId,
						labelId,
						...conversationIds,
					),
	...
	return c.json({
		data: {
			...
			conversations: orderedRows(updated, conversationIds).map(serializeConversation),
		},
	});
};
...
operations.post("/:projectId/conversations/:conversationId/mute", async (c) => {
	...
	await c.env.DB.batch([
		c.env.DB.prepare(`UPDATE conversations SET muted_until = ?,
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE project_id = ? AND id = ?`).bind(
			mutedUntil,
			projectId,
			conversationId,
		),
		auditStatement(c.env.DB, projectId, "conversation", conversationId, "muted", actorId, {
			muted_until: mutedUntil,
		}),
	]);
	return c.json({
		data: serializeConversation(await conversation(c.env.DB, projectId, conversationId)),
	});
});
...
operations.post("/:projectId/conversations/:conversationId/merge", async (c) => {
  ...
  `)
		...
		.first();
	if (duplicateMessage)
		throw failure("conversation_merge_conflict", "Conversations contain conflicting messages", 409);
	const maxSequence = await c.env.DB.prepare(
		"SELECT COALESCE(MAX(sequence), 0) sequence FROM messages WHERE conversation_id = ?",
	)
		.bind(targetId)
	...
	await c.env.DB.batch([
		c.env.DB.prepare(
			"UPDATE support_message_attachments SET conversation_id = ? WHERE project_id = ? AND conversation_id = ?",
		).bind(targetId, projectId, sourceId),
		c.env.DB.prepare("UPDATE messages SET sequence = sequence + ? WHERE conversation_id = ?").bind(
			sequenceOffset,
			sourceId,
		),
		c.env.DB.prepare("UPDATE messages SET conversation_id = ? WHERE conversation_id = ?").bind(
			targetId,
			sourceId,
		),
      ...
      WHERE project_id = ? AND conversation_id = ?`).bind(targetId, projectId, sourceId),
		c.env.DB.prepare(
			"DELETE FROM support_realtime_tickets WHERE project_id = ? AND conversation_id = ?",
		).bind(projectId, sourceId),
		c.env.DB.prepare(
			"DELETE FROM support_attachment_uploads WHERE project_id = ? AND conversation_id = ?",
		).bind(projectId, sourceId),
      ...
      FROM support_conversation_participants WHERE project_id = ? AND conversation_id = ?`).bind(
			targetId,
			projectId,
			sourceId,
		),
		c.env.DB.prepare(
			"DELETE FROM support_conversation_participants WHERE project_id = ? AND conversation_id = ?",
		).bind(projectId, sourceId),
		c.env.DB.prepare(`INSERT OR IGNORE INTO support_conversation_labels
      ...
      WHERE project_id = ? AND conversation_id = ?`).bind(targetId, projectId, sourceId),
		c.env.DB.prepare(
			"DELETE FROM support_conversation_labels WHERE project_id = ? AND conversation_id = ?",
		).bind(projectId, sourceId),
      ...
      FROM support_conversation_drafts WHERE project_id = ? AND conversation_id = ?`).bind(
			targetId,
			projectId,
			sourceId,
		),
		c.env.DB.prepare(
			"DELETE FROM support_conversation_drafts WHERE project_id = ? AND conversation_id = ?",
		).bind(projectId, sourceId),
...
operations.get("/:projectId/conversations/:conversationId/transcript", async (c) => {
  ...
  `)
		.bind(conversationId, cursor, cursor, limit + 1)
		.all<Record<string, unknown>>();
    ...
    `)
				.bind(projectId, conversationId, ...messageIds)
				.all<Record<string, unknown>>()
...
operations.get("/:projectId/conversations/:conversationId/participants", async (c) => {
  ...
  `)
		.bind(projectId, c.req.param("conversationId"))
		.all();
	return c.json({ data: rows.results });
});
...
operations.post("/:projectId/conversations/:conversationId/participants", async (c) => {
    ...
    `).bind(c.req.param("conversationId"), projectId, kind, participantId, actorId),
		auditStatement(
			...
			`${c.req.param("conversationId")}:${kind}:${participantId}`.slice(0, 255),
			"created",
			actorId,
			{},
		),
...
operations.delete(
	"/:projectId/conversations/:conversationId/participants/:kind/:participantId",
	async (c) => {
		const projectId = positiveInt(c.req.param("projectId"));
		const actorId = requestActor(c.req.header.bind(c.req));
		await c.env.DB.batch([
			c.env.DB.prepare(`DELETE FROM support_conversation_participants
      WHERE project_id = ? AND conversation_id = ? AND participant_kind = ? AND participant_id = ?`).bind(
				projectId,
...
operations.get("/:projectId/conversations/:conversationId/draft", async (c) => {
  ...
  `)
		.bind(c.req.param("conversationId"), projectId, actorId)
		.first<Record<string, unknown>>();
	return c.json({
		data: row
			? {
					...row,
					attachments: safeAttachmentDrafts(row.attachments_json),
...
operations.put("/:projectId/conversations/:conversationId/draft", async (c) => {
    ...
    VALUES (?, ?, ?, ?, ?) ON CONFLICT(conversation_id, agent_id) DO UPDATE SET
      ...
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
  `)
		.bind(c.req.param("conversationId"), projectId, actorId, content, JSON.stringify(attachments))
		.run();
	return c.json({
		data: {
			conversation_id: c.req.param("conversationId"),
...
operations.get("/:projectId/csat", async (c) => {
    ...
    WHERE response.project_id = ? AND (? IS NULL OR conversation.inbox_id = ?) ORDER BY response.created_at DESC LIMIT 100
  `)
		.bind(projectId, inboxId, inboxId)
		.all();
	return c.json({ data: rows.results });
});
...
operations.get("/:projectId/notifications/preferences", async (c) => {
	const projectId = positiveInt(c.req.param("projectId"));
	const operator = c.req.header("X-Operator-Id");
	if (operator && ["owner", "admin"].includes(c.req.header("X-Role") || ""))
		return c.json({ data: await operatorNotificationPreferences(c.env.DB, projectId, operator) });
	const actorId = requestActor(c.req.header.bind(c.req));
  ...
  `)
		.bind(projectId, membership.id)
		.first<Record<string, unknown>>();
	return c.json({
		data: serializeNotificationPreferences(
			row || {
...
operations.put("/:projectId/notifications/preferences", async (c) => {
	...
	if (operator && ["owner", "admin"].includes(c.req.header("X-Role") || ""))
		return c.json({
			data: await operatorNotificationPreferences(
				c.env.DB,
				projectId,
				operator,
  ...
  `)
		.bind(
			...
			input.browser_enabled ? 1 : 0,
			input.in_app_enabled ? 1 : 0,
			input.audio_enabled ? 1 : 0,
			JSON.stringify(input.muted_event_types),
		)
		.run();
...
operations.patch("/:projectId/notifications/:notificationId/snooze", async (c) => {
	...
	const body = await readJsonObject(c.req.raw);
	const snoozedUntil =
		body.snoozed_until == null ? null : futureTimestamp(body.snoozed_until, "snoozed_until");
	const row = await c.env.DB.prepare(`
...
operations.get("/:projectId/search", async (c) => {
	...
	if (query.length < 2)
		throw failure("search_query_too_short", "Search query must contain at least two characters");
	const like = `%${escapeLike(query)}%`;
	const [contacts, companies, conversations, messages] = await Promise.all([
      ...
      WHERE project_id = ? AND (external_user_id LIKE ? ESCAPE '\\' OR COALESCE(name, '') LIKE ? ESCAPE '\\'
        OR COALESCE(email, '') LIKE ? ESCAPE '\\') LIMIT 50`)
			.bind(projectId, like, like, like)
			.all<Record<string, unknown>>(),
      ...
      WHERE project_id = ? AND (name LIKE ? ESCAPE '\\' OR COALESCE(domain, '') LIKE ? ESCAPE '\\'
        OR COALESCE(description, '') LIKE ? ESCAPE '\\') LIMIT 50`)
			.bind(projectId, like, like, like)
			.all<Record<string, unknown>>(),
      ...
      (external_user_id LIKE ? ESCAPE '\\' OR COALESCE(subject, '') LIKE ? ESCAPE '\\'
        OR COALESCE(last_message_preview, '') LIKE ? ESCAPE '\\') LIMIT 50`)
			.bind(projectId, like, like, like)
			.all<Record<string, unknown>>(),
...
export async function ensureSupportContact(
    ...
    VALUES (?, ?, ?) ON CONFLICT(project_id, external_user_id) DO UPDATE SET
      ...
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
  `)
		.bind(crypto.randomUUID(), projectId, externalUserId)
		.run();
}
...
async function contact(db: D1Database, projectId: number, id: string) {
	const row = await db
		.prepare("SELECT * FROM support_contacts WHERE project_id = ? AND id = ?")
		.bind(projectId, identifier(id, "contact_id"))
		.first<Record<string, unknown>>();
...
async function company(db: D1Database, projectId: number, id: string) {
	const row = await db
		.prepare("SELECT * FROM support_companies WHERE project_id = ? AND id = ?")
		.bind(projectId, identifier(id, "company_id"))
		.first<Record<string, unknown>>();
...
async function conversation(db: D1Database, projectId: number, id: string) {
    ...
    FROM conversations conversation WHERE conversation.project_id = ? AND conversation.id = ?`)
		.bind(projectId, identifier(id, "conversation_id"))
		.first<Record<string, unknown>>();
...
async function membership(db: D1Database, projectId: number, id: string) {
	const row = await db
		.prepare(`SELECT id FROM support_memberships
    WHERE project_id = ? AND id = ? AND active = 1`)
		.bind(projectId, id)
		.first();
	if (!row)
		throw failure("support_membership_not_found", "Active Support membership not found", 404);
}

async function team(db: D1Database, projectId: number, id: string) {
	const row = await db
		.prepare(`SELECT id FROM support_teams
    WHERE project_id = ? AND id = ? AND active = 1`)
		.bind(projectId, id)
		.first();
...
async function operatorNotificationPreferences(
	db: D1Database,
	projectId: number,
	operatorId: string,
	body?: Record<string, unknown>,
) {
	const id = identifier(operatorId, "operator_id");
	if (body !== undefined) {
		const input = notificationPreferencesInput(body);
		await db
			.prepare(`INSERT INTO support_operator_notification_preferences
      (project_id,operator_id,email_enabled,push_enabled,browser_enabled,in_app_enabled,audio_enabled,muted_event_types_json)
      VALUES (?,?,?,?,?,?,?,?) ON CONFLICT(project_id,operator_id) DO UPDATE SET
      email_enabled=excluded.email_enabled,push_enabled=excluded.push_enabled,browser_enabled=excluded.browser_enabled,
      in_app_enabled=excluded.in_app_enabled,audio_enabled=excluded.audio_enabled,muted_event_types_json=excluded.muted_event_types_json,
      updated_at=strftime('%Y-%m-%dT%H:%M:%fZ','now')`)
			.bind(
				projectId,
				id,
	...
	const row = await db
		.prepare(
			"SELECT * FROM support_operator_notification_preferences WHERE project_id=? AND operator_id=?",
		)
		.bind(projectId, id)
		.first<Record<string, unknown>>();
...
async function notificationMembership(db: D1Database, projectId: number, actorId: string) {
    ...
    WHERE project_id = ? AND active = 1 AND (auth_user_id = ? OR id = ?) LIMIT 1`)
		.bind(projectId, actorId, actorId)
		.first<{ id: string }>();
	if (!row)
		throw failure("support_membership_required", "An active Support membership is required", 403);
	return row;
}
...
async function requireInboxMembership(
	...
	const row = await db
		.prepare(`SELECT 1 allowed FROM support_inbox_members
...
async function requireLabels(db: D1Database, projectId: number, ids: string[]) {
	const rows = await db
		.prepare(`SELECT id FROM support_labels
...
async function requireConversations(db: D1Database, projectId: number, ids: string[]) {
    ...
    WHERE project_id = ? AND id IN (${placeholdersFor(ids)})`)
		.bind(projectId, ...ids)
		.all<{ id: string }>();
	if (rows.results.length !== ids.length)
		throw failure("conversation_not_found", "One or more conversations were not found", 404);
}
...
async function conversationsById(db: D1Database, projectId: number, ids: string[]) {
    ...
    FROM conversations conversation WHERE conversation.project_id = ?
      AND conversation.id IN (${placeholdersFor(ids)})`)
		.bind(projectId, ...ids)
		.all<Record<string, unknown>>();
	return rows.results;
}
...
async function contactLabels(db: D1Database, projectId: number, contactId: string) {
  ...
  `)
		.bind(projectId, contactId, projectId)
		.all<Record<string, unknown>>();
	return rows.results.map((row) => ({
		...row,
		show_on_sidebar: Number(row.show_on_sidebar) === 1,
	}));
}
...
async function importJob(db: D1Database, projectId: number, id: string) {
	const row = await db
		.prepare(`SELECT id, resource_type, status, checkpoint_json, result_json,
...
async function exportJob(db: D1Database, projectId: number, id: string) {
	const row = await db
		.prepare(`SELECT id, resource_type, status, filters_json, result_json,
...
async function listJobs(
	...
	const rows = await db
		.prepare(`SELECT id, resource_type, status, result_json, last_error, created_at, updated_at
...
async function markBulkJobFailed(
	...
	await db
		.prepare(`UPDATE ${table} SET status = 'failed', last_error = 'queue_unavailable',
...
function serializeNotificationPreferences(row: Record<string, unknown>) {
	return {
		...
		...(typeof row.operator_id === "string" ? { operator_id: row.operator_id } : {}),
...
function notificationPreferencesInput(body: Record<string, unknown>) {
	const boolean = (name: string, fallback: boolean) => {
		...
		if (typeof body[name] !== "boolean")
			throw failure(`${name}_invalid`, `${name} must be a boolean`);
		return body[name] as boolean;
	};
	const muted =
		body.muted_event_types == null
			? []
			: array(body.muted_event_types, "muted_event_types", 100).map((value) => {
					...
					if (!/^[a-z][a-z0-9_.-]*$/u.test(event)) {
						throw failure(
							"muted_event_types_invalid",
							"muted_event_types contains an invalid event name",
						);
					}
					return event;
				});
...
function auditStatement(
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)`)
		.bind(
			...
			action.slice(0, 128),
			actorId,
			JSON.stringify(redactSensitiveObject(payload)),
		);
}
...
function positiveInt(value: unknown) {
	...
	if (!Number.isInteger(parsed) || parsed <= 0)
		throw failure("project_id_invalid", "Project id is invalid");
	return parsed;
}

function requestActor(header: (name: string) => string | undefined) {
	return actor(header("X-Operator-Id") || header("X-Actor-Id"));
}
...
function identifierArray(value: unknown, field: string, max: number) {
	...
	if (unique.length !== values.length)
		throw failure(`${field}_invalid`, `${field} must contain unique identifiers`);
	return unique;
}
...
function optionalText(value: unknown, field: string, max: number) {
	...
	if (parsed.length > max)
		throw failure(`${field}_invalid`, `${field} is limited to ${max} characters`);
	return parsed;
}
...
function nullableEmail(value: unknown) {
	...
	if (parsed && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(parsed))
		throw failure("email_invalid", "Email is invalid");
	return parsed;
}
...
function nullableHttpsUrl(value: unknown, field: string) {
	...
	try {
		...
		if (url.protocol !== "https:" || url.username || url.password || hasSensitiveQuery(url))
			throw new Error();
	} catch {
...
function muteUntil(value: unknown) {
	...
	if (new Date(timestamp).getTime() <= Date.now())
		throw failure("muted_until_invalid", "muted_until must be in the future");
	return timestamp;
}
...
function sensitiveKey(key: string) {
	return /(?:secret|password|authorization|cookie|credential|api[_-]?key|access[_-]?token|refresh[_-]?token|signature)/i.test(
		key,
	);
}
...
function pageResponse<T extends Record<string, unknown>, U>(
	rows: T[],
	limit: number,
	serialize: (row: T) => U,
) {
	...
	return {
		...
		pagination: {
			...
			next_cursor:
				hasMore && last
					? encodePageCursor({ updated_at: String(last.updated_at), id: String(last.id) })
					: null,
		},
	};
}
...
function decodeConversationCursor(
	value: unknown,
): { rank: number; activity: string; id: string } | null {
	if (value == null || value === "") return null;
	const parsed = decodeOpaque(value, "cursor");
	if (
