	...
	},
	{
		...
		fields: {
			...
			value_type: {
				kind: "enum",
				required: true,
				values: ["text", "number", "date", "boolean", "list", "link"],
			},
			description: { kind: "text", nullable: true, max: 1_000 },
...
for (const resource of resources) {
	native.get(`/:projectId/${resource.path}`, async (c) => {
		...
		const scopedInbox = c.req.query("inbox_id");
		const scopedPortal = c.req.query("portal_id");
		if (scopedInbox) {
			const inboxId = identifier(scopedInbox);
			if (resource.path === "workforce/inboxes") clauses.push("id = ?");
			else if ("inbox_id" in resource.fields) clauses.push("inbox_id = ?");
			else if (resource.path === "assignment-policies")
				clauses.push("EXISTS (SELECT 1 FROM json_each(inbox_ids_json) WHERE value = ?)");
			else if (resource.path === "automations")
				clauses.push(
					"condition_mode = 'all' AND EXISTS (SELECT 1 FROM json_each(conditions_json) condition WHERE condition.type = 'object' AND json_extract(condition.value, '$.field') = 'inbox_id' AND json_extract(condition.value, '$.operator') = 'equals' AND json_extract(condition.value, '$.value') = ?)",
				);
			else if (resource.path === "sla/policies")
				clauses.push("json_extract(conditions_json, '$.inbox_id') = ?");
			else throw failure("unsupported_inbox_filter", "This resource has no Inbox scope", 400);
			bindings.push(inboxId);
		}
		if (scopedPortal) {
			const portalId = identifier(scopedPortal);
			if (resource.path === "help-center/portals") clauses.push("id = ?");
			else if ("portal_id" in resource.fields) clauses.push("portal_id = ?");
			else if (resource.path === "help-center/translations")
				clauses.push("article_id IN (SELECT id FROM support_articles WHERE portal_id = ?)");
			else
				throw failure("unsupported_portal_filter", "This resource has no Help Center scope", 400);
			bindings.push(portalId);
		}
		if (query && resource.searchColumns?.length) {
			clauses.push(
				`(${resource.searchColumns.map((column) => `${column} LIKE ? ESCAPE '\\'`).join(" OR ")})`,
			);
       ...
       ORDER BY created_at DESC, id DESC LIMIT ?`,
		...
		return c.json({
			...
			pagination: {
				...
				has_more: hasMore,
				next_cursor:
					hasMore && last ? encodeCursor(String(last.created_at), String(last.id)) : null,
			},
		});
	});
	...
	native.get(`/:projectId/${resource.path}/:id`, async (c) => {
		const row = await resourceRow(
			c.env.DB,
			resource,
			projectIdFrom(c.req.param("projectId")),
			c.req.param("id"),
		);
	...
	native.post(`/:projectId/${resource.path}`, async (c) => {
       ...
       VALUES (${columns.map(() => "?").join(", ")}) RETURNING *`,
		...
		if (!row)
			throw failure("support_resource_create_failed", "Support resource could not be created", 500);
		await audit(c.env.DB, projectId, resource.path, id, "created", actorId(c.req.raw), row);
	...
	native.patch(`/:projectId/${resource.path}/:id`, async (c) => {
        ...
        FROM support_provider_endpoints WHERE id = ? AND project_id = ?`)
			...
			await validateProviderInbox(c.env.DB, projectId, body.inbox_id, endpoint.provider);
		}
		const fields = mutationFields(resource, body, false);
		if (fields.size === 0)
			throw failure("support_resource_update_empty", "At least one mutable field is required", 422);
       ...
       WHERE id = ? AND project_id = ? RETURNING *`,
		...
		await audit(c.env.DB, projectId, resource.path, id, "updated", actorId(c.req.raw), row);
		await enqueueCreatedResource(c.env, resource.path, projectId, id, row);
		return c.json({ data: serializeRow(resource, row) });
	});
	...
	native.delete(`/:projectId/${resource.path}/:id`, async (c) => {
		...
		const id = identifier(c.req.param("id"));
		const knowledge =
			resource.path === "help-center/articles"
		...
		const result = await c.env.DB.prepare(
			`DELETE FROM ${resource.table} WHERE id = ? AND project_id = ? RETURNING *`,
		)
			.bind(id, projectId)
			.first<Record<string, unknown>>();
		if (!result) throw failure("support_resource_not_found", "Support resource not found", 404);
		await audit(c.env.DB, projectId, resource.path, id, "deleted", actorId(c.req.raw), result);
		if (knowledge) {
			const vectorIds = Array.from(
				{ length: knowledge.chunk_count },
				(_, index) => `${knowledge.id}:${index}`,
			);
...
function validateNativeResourceBody(
	...
	if (path === "automations") {
		...
		if (body.endpoint_url !== undefined && !isSafePublicHttpsUrl(String(body.endpoint_url || ""))) {
			throw failure(
				"support_tool_endpoint_invalid",
				"Assistant tool endpoint must use public HTTPS",
				422,
			);
		}
		if (body.headers !== undefined) {
			...
			if (
				...
				Object.keys(headers).some(
					(name) => !["accept", "content-type"].includes(name.toLowerCase()),
				)
			) {
				throw failure(
					"support_tool_headers_invalid",
					"Assistant tools accept only non-sensitive content headers",
					422,
				);
			}
		}
...
native.get("/:projectId/workforce", async (c) => {
	...
	const [memberships, teams, inboxes, activeAssignments, leaves] = await c.env.DB.batch([
		c.env.DB.prepare(
			"SELECT role, availability, active, COUNT(*) count FROM support_memberships WHERE project_id = ? GROUP BY role, availability, active",
		).bind(projectId),
		c.env.DB.prepare(
			"SELECT COUNT(*) count FROM support_teams WHERE project_id = ? AND active = 1",
		).bind(projectId),
		c.env.DB.prepare(
			"SELECT COUNT(*) count FROM support_inboxes WHERE project_id = ? AND status = 'active'",
		).bind(projectId),
		c.env.DB.prepare(
			"SELECT assigned_user_id, COUNT(*) count FROM conversations WHERE project_id = ? AND status != 'closed' AND assigned_user_id IS NOT NULL GROUP BY assigned_user_id",
		).bind(projectId),
		c.env.DB.prepare(
			"SELECT COUNT(*) count FROM support_leave_schedules WHERE project_id = ? AND starts_at <= datetime('now') AND ends_at > datetime('now')",
		).bind(projectId),
	]);
	return c.json({
		data: {
...
native.put("/:projectId/workforce/teams/:teamId/members/:membershipId", async (c) => {
    ...
    ON CONFLICT(team_id, membership_id) DO NOTHING`)
		.bind(projectId, teamId, membershipId)
		.run();
	await audit(
		c.env.DB,
		projectId,
...
native.delete("/:projectId/workforce/teams/:teamId/members/:membershipId", async (c) => {
	...
	if (!result.meta.changes)
		throw failure("support_team_member_not_found", "Support team member not found", 404);
	await audit(
		c.env.DB,
		projectId,
		"workforce/team-members",
...
native.put("/:projectId/workforce/inboxes/:inboxId/members/:membershipId", async (c) => {
    ...
    ON CONFLICT(inbox_id, membership_id) DO NOTHING`)
		.bind(projectId, inboxId, membershipId)
		.run();
	await audit(
		c.env.DB,
		projectId,
		"workforce/inbox-members",
		`${inboxId}:${membershipId}`,
		"linked",
		actorId(c.req.raw),
	);
...
native.delete("/:projectId/workforce/inboxes/:inboxId/members/:membershipId", async (c) => {
    ...
    WHERE project_id = ? AND inbox_id = ? AND membership_id = ?`)
		...
		.run();
	if (!result.meta.changes)
		throw failure("support_inbox_member_not_found", "Support inbox member not found", 404);
	await audit(
		c.env.DB,
		projectId,
		"workforce/inbox-members",
...
native.put("/:projectId/providers/:id/credentials", async (c) => {
	...
	if (!endpoint)
		throw failure("support_provider_not_found", "Support provider endpoint not found", 404);
	const body = await readJsonObject(c.req.raw);
...
native.put("/:projectId/integrations/:id/credentials", async (c) => {
	...
	if (integration.provider === "webhook") {
		...
		if (signingSecret.length < 32 || signingSecret.length > 512) {
			throw failure(
				...
				"Webhook signing secret must contain between 32 and 512 characters",
				422,
			);
		}
		...
		if (
			...
			throw failure(
				"configuration_required",
				"Support meeting credentials and preset are required",
				422,
			);
		}
	} else {
		nativeCredentialState = nativeIntegrationCredentialState(
	...
	if (new TextEncoder().encode(serialized).byteLength > 32_000) {
		throw failure(
			"support_integration_credentials_invalid",
			"Credentials exceed their size limit",
			422,
		);
	}
	if (!c.env.SUPPORT_CREDENTIAL_ENCRYPTION_KEY) {
		throw failure(
			"support_credentials_unavailable",
			"Support credential encryption is not configured",
			503,
		);
	}
	const encrypted = await encryptCredentialPayload(
	...
	await c.env.DB.batch([
       ...
       WHERE id = ? AND project_id = ?`,
			...
			nativeCredentialState?.configured === false ? "configuration_required" : "configured",
			integrationId,
			projectId,
		),
	]);
	...
	return c.json({
		data: {
			...
			...(nativeCredentialState
				? { authorization_required: !nativeCredentialState.configured }
				: {}),
		},
	});
});
...
native.post("/:projectId/providers/:id/oauth", async (c) => {
	...
	if (!row.encrypted_payload)
		throw failure("configuration_required", "Support provider credentials are not configured", 422);
	const body = await readJsonObject(c.req.raw);
	...
	if (!isSafePublicHttpsUrl(callbackUri) || !isSafePublicHttpsUrl(returnUri)) {
		throw failure(
			...
			"OAuth callback and return URLs must be public HTTPS URLs",
			422,
		);
	}
	...
	if (!callback.pathname.endsWith(`/api/v1/support/providers/${row.provider}/oauth/callback`)) {
		throw failure(
			"support_oauth_uri_invalid",
			"OAuth callback URL does not match the Support provider route",
			422,
		);
	}
	const decrypted = await decryptCredentialPayload(
		[c.env.SUPPORT_CREDENTIAL_ENCRYPTION_KEY, c.env.SUPPORT_CREDENTIAL_ENCRYPTION_KEY_PREVIOUS],
		row.encrypted_payload,
	);
	const clientId = String(decrypted.payload.client_id || decrypted.payload.client_key || "").trim();
	if (!clientId)
		throw failure("configuration_required", "OAuth client configuration is incomplete", 422);
	...
	for (const [name, value] of Object.entries(oauth.extra))
...
native.post("/:projectId/proactive-support/campaigns/:id/:action", async (c) => {
	...
	const body = await readJsonObject(c.req.raw);
	const scheduledAt =
		action === "schedule" ? timestamp(body.scheduled_at, "scheduled_at", false) : null;
	if (scheduledAt && new Date(scheduledAt).getTime() <= Date.now()) {
		throw failure(
			"support_campaign_schedule_invalid",
	...
	const row = await c.env.DB.prepare(
     ...
     WHERE id = ? AND project_id = ? RETURNING *`,
		...
		.bind(target, scheduledAt, target, target, id, projectId)
		.first<Record<string, unknown>>();
	...
	return c.json({
		data: serializeRow(
			resources.find((entry) => entry.path === "proactive-support/campaigns")!,
			row,
		),
	});
});
...
native.post("/:projectId/help-center/articles/:id/publish", async (c) => {
	...
	return c.json({
		data: serializeRow(
			resources.find((entry) => entry.path === "help-center/articles")!,
			row,
		),
	});
});
...
native.post("/:projectId/captain/tasks", async (c) => {
	...
	if (!allowed.includes(taskType))
		throw failure("support_task_type_invalid", "Captain task type is invalid", 422);
	const id = crypto.randomUUID();
	...
	if (assistantId) {
      ...
      WHERE id = ? AND project_id = ? AND active = 1`)
			.bind(assistantId, projectId)
			.first<{ handoff_enabled: number }>();
		if (!assistant)
			throw failure("support_assistant_not_found", "Active Support assistant not found", 404);
		if (taskType === "handoff" && assistant.handoff_enabled !== 1) {
			throw failure(
				"support_handoff_disabled",
				"Human handoff is disabled for this assistant",
				422,
			);
		}
	}
	if (conversationId) {
		const conversation = await c.env.DB.prepare(
			...
			.first();
		if (!conversation)
			throw failure("support_conversation_not_found", "Support conversation not found", 404);
	}
	if (!c.env.SUPPORT_AI_QUEUE) {
		throw failure(
			"support_ai_unavailable",
			"Support intelligence processing is not configured",
			503,
		);
	}
	await c.env.DB.prepare(
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?)`,
		...
		.bind(id, projectId, assistantId, conversationId, taskType, input, actorId(c.req.raw))
		.run();
	try {
		await c.env.SUPPORT_AI_QUEUE.send(
			{ type: "support.captain.task.v1", projectId, taskId: id },
			{ contentType: "json" },
		);
	} catch {
      ...
      WHERE id = ? AND project_id = ?`)
		...
		throw failure(
			"support_ai_unavailable",
			"Support intelligence processing is temporarily unavailable",
			503,
		);
	}
	await audit(c.env.DB, projectId, "captain/tasks", id, "queued", actorId(c.req.raw));
	return c.json(
		{ data: { id, project_id: projectId, task_type: taskType, status: "queued" } },
		202,
	);
});
...
native.get("/:projectId/captain/tasks/:id", async (c) => {
	const projectId = projectIdFrom(c.req.param("projectId"));
	const row = await c.env.DB.prepare(
		"SELECT * FROM support_assistant_tasks WHERE id = ? AND project_id = ?",
	)
		.bind(identifier(c.req.param("id")), projectId)
		.first<Record<string, unknown>>();
...
native.get("/:projectId/captain/assistant-inboxes", async (c) => {
	const projectId = projectIdFrom(c.req.param("projectId"));
	const rows =
		await c.env.DB.prepare(`SELECT link.*, inbox.name inbox_name, assistant.name assistant_name
    ...
    WHERE link.project_id = ? ORDER BY link.created_at DESC`)
			.bind(projectId)
			.all<Record<string, unknown>>();
	return c.json({
		data: rows.results.map((row) => ({
			...row,
			automatic_enabled: Number(row.automatic_enabled) === 1,
		})),
	});
});
...
native.put("/:projectId/captain/assistants/:assistantId/inboxes/:inboxId", async (c) => {
	...
	const [assistant, inbox] = await c.env.DB.batch([
		c.env.DB.prepare(
			"SELECT id FROM support_assistants WHERE id = ? AND project_id = ? AND active = 1",
		).bind(assistantId, projectId),
		c.env.DB.prepare(
			"SELECT id FROM support_inboxes WHERE id = ? AND project_id = ? AND status = 'active'",
		).bind(inboxId, projectId),
	]);
	if (!assistant.results.length || !inbox.results.length) {
		throw failure(
			"support_assistant_inbox_invalid",
			"Active assistant and inbox are required",
			422,
		);
	}
    ...
    ON CONFLICT(assistant_id, inbox_id) DO UPDATE SET automatic_enabled = excluded.automatic_enabled,
      configured_by = excluded.configured_by, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')`)
		.bind(projectId, assistantId, inboxId, automatic ? 1 : 0, actorId(c.req.raw))
		.run();
	await audit(
		c.env.DB,
		projectId,
		...
		`${assistantId}:${inboxId}`,
		automatic ? "automatic_enabled" : "linked",
		actorId(c.req.raw),
	);
	return c.json({
		data: { assistant_id: assistantId, inbox_id: inboxId, automatic_enabled: automatic },
	});
});
...
native.get("/:projectId/reports", async (c) => {
	...
	if (automationId) {
		const resource = resources.find((entry) => entry.path === "automations");
		if (!resource) throw failure("contract_unavailable", "Automation contract unavailable", 503);
		await resourceRow(c.env.DB, resource, projectId, automationId);
		const result = await c.env.DB.prepare(
			"SELECT COUNT(*) executions, SUM(CASE WHEN json_extract(result_json, '$.matched') = 1 THEN 1 ELSE 0 END) matched, SUM(CASE WHEN json_extract(result_json, '$.matched') = 0 THEN 1 ELSE 0 END) skipped, SUM(CASE WHEN json_extract(result_json, '$.matched') IS NULL THEN 1 ELSE 0 END) pending FROM support_rule_executions WHERE project_id = ? AND rule_id = ?",
		)
			.bind(projectId, automationId)
			.first();
		return c.json({ data: result });
	}

	const portalId = c.req.query("portal_id");
	if (portalId) {
		const portalResource = resources.find((resource) => resource.path === "help-center/portals");
		if (!portalResource)
			throw failure("contract_unavailable", "Help Center contract unavailable", 503);
		await resourceRow(c.env.DB, portalResource, projectId, portalId);
		const [articles, categories, views] = await c.env.DB.batch<{
			total: number;
			published?: number;
		}>([
			c.env.DB.prepare(
				"SELECT COUNT(*) total, SUM(CASE WHEN status = 'published' THEN 1 ELSE 0 END) published FROM support_articles WHERE project_id = ? AND portal_id = ?",
			).bind(projectId, portalId),
			c.env.DB.prepare(
				"SELECT COUNT(*) total FROM support_portal_categories WHERE project_id = ? AND portal_id = ?",
			).bind(projectId, portalId),
			c.env.DB.prepare(
				"SELECT COUNT(*) total FROM support_article_views view INNER JOIN support_articles article ON article.id = view.article_id AND article.project_id = view.project_id WHERE view.project_id = ? AND article.portal_id = ?",
			).bind(projectId, portalId),
		]);
		return c.json({
			data: {
				articles: articles.results[0]?.total ?? 0,
				published: articles.results[0]?.published ?? 0,
				categories: categories.results[0]?.total ?? 0,
				views: views.results[0]?.total ?? 0,
			},
		});
	}

	const inboxId = c.req.query("inbox_id") ? identifier(c.req.query("inbox_id")) : null;
	...
	const rows = await c.env.DB.prepare(
     ...
     FROM conversations WHERE project_id = ?
       ...
       AND (? IS NULL OR inbox_id = ?)`,
		...
		.bind(projectId, from, from, to, to, inboxId, inboxId)
		.first<Record<string, unknown>>();
	const [byInbox, byAgent, byTeam, byLabel, byChannel, byProvider, sla, csat, campaigns] =
		await c.env.DB.batch([
      ...
      WHERE conversation.project_id = ? AND (? IS NULL OR conversation.created_at >= ?)
        ...
        AND (? IS NULL OR conversation.inbox_id = ?)
      GROUP BY conversation.inbox_id, inbox.name ORDER BY conversations DESC`).bind(
				projectId,
				from,
				from,
				to,
      ...
      WHERE conversation.project_id = ? AND (? IS NULL OR conversation.created_at >= ?)
        AND (? IS NULL OR conversation.created_at < ?)
        AND (? IS NULL OR conversation.inbox_id = ?)
      GROUP BY conversation.assigned_user_id, membership.display_name ORDER BY conversations DESC`).bind(
				projectId,
				from,
				from,
				to,
      ...
      WHERE conversation.project_id = ? AND (? IS NULL OR conversation.created_at >= ?)
        AND (? IS NULL OR conversation.created_at < ?)
        AND (? IS NULL OR conversation.inbox_id = ?)
      GROUP BY conversation.assigned_team_id, team.name ORDER BY conversations DESC`).bind(
				projectId,
				from,
				from,
				to,
      ...
      WHERE linked.project_id = ? AND (? IS NULL OR conversation.created_at >= ?)
        AND (? IS NULL OR conversation.created_at < ?)
        AND (? IS NULL OR conversation.inbox_id = ?)
      GROUP BY linked.label_id, label.name ORDER BY conversations DESC`).bind(
				projectId,
				from,
				from,
				to,
      ...
      WHERE conversation.project_id = ? AND (? IS NULL OR conversation.created_at >= ?)
        AND (? IS NULL OR conversation.created_at < ?)
        AND (? IS NULL OR conversation.inbox_id = ?)
      GROUP BY inbox.channel_type ORDER BY conversations DESC`).bind(
				projectId,
				from,
				from,
				to,
      ...
      WHERE conversation.project_id = ? AND (? IS NULL OR conversation.created_at >= ?)
        AND (? IS NULL OR conversation.created_at < ?)
        AND (? IS NULL OR conversation.inbox_id = ?)
      GROUP BY endpoint.provider ORDER BY conversations DESC`).bind(
				projectId,
				from,
				from,
				to,
      ...
      WHERE project_id = ? AND (? IS NULL OR created_at >= ?) AND (? IS NULL OR created_at < ?) AND (? IS NULL OR conversation_id IN (SELECT id FROM conversations WHERE inbox_id = ?))
      GROUP BY status`).bind(projectId, from, from, to, to, inboxId, inboxId),
			c.env.DB.prepare(`SELECT COUNT(*) responses, ROUND(AVG(rating), 2) average FROM support_csat_responses
      WHERE project_id = ? AND (? IS NULL OR created_at >= ?) AND (? IS NULL OR created_at < ?) AND (? IS NULL OR conversation_id IN (SELECT id FROM conversations WHERE inbox_id = ?))`).bind(
				projectId,
				from,
				from,
				to,
      ...
      WHERE project_id = ? AND (? IS NULL OR created_at >= ?) AND (? IS NULL OR created_at < ?) AND (? IS NULL OR campaign_id IN (SELECT id FROM support_campaigns WHERE inbox_id = ?))
      GROUP BY status`).bind(projectId, from, from, to, to, inboxId, inboxId),
		]);
	return c.json({
		data: {
...
native.post("/:projectId/reports/exports", async (c) => {
	...
	if (!queue)
		throw failure("support_bulk_unavailable", "Support export processing is not configured", 503);
	await c.env.DB.prepare(
		`INSERT INTO support_export_jobs (id, project_id, resource_type, filters_json, created_by)
     VALUES (?, ?, 'reports', ?, ?)`,
	)
		.bind(id, projectId, filters, actorId(c.req.raw))
		.run();
	try {
		await queue.send(
			{ type: "support.export.requested.v1", projectId, exportId: id },
			{ contentType: "json" },
		);
	} catch {
      ...
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ? AND project_id = ?`)
			...
			.run();
		throw failure(
			"support_bulk_unavailable",
			"Support export processing is temporarily unavailable",
			503,
		);
	}
...
native.get("/:projectId/reports/exports", async (c) => {
    ...
    WHERE project_id = ? AND resource_type = 'reports' AND (? IS NULL OR json_extract(filters_json, '$.inbox_id') = ?)
...
native.get("/:projectId/reports/exports/:id/download", async (c) => {
	...
	if (!object)
		throw failure(
			"support_report_export_unavailable",
			"Support report export is temporarily unavailable",
			503,
		);
...
native.get("/:projectId/settings/operations", async (c) => {
	const projectId = projectIdFrom(c.req.param("projectId"));
	const [deadLetters, scheduled, providerFailures, knowledge, imports, exports] =
		await c.env.DB.batch([
			c.env.DB.prepare("SELECT status, COUNT(*) count FROM support_dead_letters GROUP BY status"),
			c.env.DB.prepare(
				"SELECT queue_name, status, COUNT(*) count FROM support_scheduled_jobs WHERE project_id = ? GROUP BY queue_name, status",
			).bind(projectId),
			c.env.DB.prepare(
				"SELECT provider, status, COUNT(*) count FROM support_provider_events WHERE project_id = ? GROUP BY provider, status",
			).bind(projectId),
			c.env.DB.prepare(
				"SELECT status, COUNT(*) count FROM support_knowledge_documents WHERE project_id = ? GROUP BY status",
			).bind(projectId),
			c.env.DB.prepare(
				"SELECT status, COUNT(*) count FROM support_import_jobs WHERE project_id = ? GROUP BY status",
			).bind(projectId),
			c.env.DB.prepare(
				"SELECT status, COUNT(*) count FROM support_export_jobs WHERE project_id = ? GROUP BY status",
			).bind(projectId),
		]);
	return c.json({
		data: {
...
native.post("/:projectId/settings/operations/dead-letters/:id/replay", async (c) => {
	...
	if ((await sha256(row.payload_json)) !== row.payload_sha256) {
		throw failure(
			"support_dead_letter_integrity_invalid",
			"Support job integrity check failed",
			409,
		);
	}
	...
	try {
		...
		if (
			...
			Number((parsed as Record<string, unknown>).projectId) !== projectId
		) {
			throw new Error();
		}
	...
	await c.env.DB.batch([
		c.env.DB.prepare(
			"UPDATE support_dead_letters SET status = 'discarded' WHERE id = ? AND status = 'quarantined'",
		).bind(id),
      ...
      VALUES (?, ?, 'dead_letter', ?, 'replayed', ?, ?)`).bind(
			crypto.randomUUID(),
			projectId,
			id,
			actorId(c.req.raw),
			JSON.stringify({ source_queue: row.source_queue }),
		),
	]);
...
async function reportExport(db: D1Database, projectId: number, value: string) {
	...
	const row = await db
		.prepare(`SELECT id, project_id, status, filters_json, result_json,
      storage_key, last_error, created_at, updated_at FROM support_export_jobs
    WHERE id = ? AND project_id = ? AND resource_type = 'reports'`)
		.bind(id, projectId)
		.first<Record<string, unknown>>();
	if (!row)
		throw failure("support_report_export_not_found", "Support report export not found", 404);
	return row;
}
...
function mutationFields(resource: Resource, body: Record<string, unknown>, create: boolean) {
	...
	for (const [publicName, field] of Object.entries(resource.fields)) {
		...
		if (!(publicName in body)) {
			if (create && field.required)
				throw failure("support_field_required", `${publicName} is required`, 422);
			continue;
		}
	...
	if (unknown.length)
		throw failure(
			"support_field_unknown",
			`Unsupported fields: ${unknown.slice(0, 5).join(", ")}`,
			422,
		);
	return values;
}
...
function fieldValue(value: unknown, name: string, field: Field): unknown {
	...
	if (field.kind === "boolean") {
		if (typeof value !== "boolean")
			throw failure("support_field_invalid", `${name} must be a boolean`, 422);
		return value ? 1 : 0;
	}
...
function serializeRow(resource: Resource, row: Record<string, unknown>) {
	...
	const reverse = new Map(
		Object.entries(resource.fields).map(([name, field]) => [field.column || name, { name, field }]),
	);
...
async function resourceRow(db: D1Database, resource: Resource, projectId: number, value: string) {
	const id = identifier(value);
	const row = await db
		.prepare(`SELECT * FROM ${resource.table} WHERE id = ? AND project_id = ?`)
		.bind(id, projectId)
		.first<Record<string, unknown>>();
	if (!row) throw failure("support_resource_not_found", "Support resource not found", 404);
	return row;
...
async function activeWorkforceResource(
	...
	if (!row)
		throw failure(
			"support_workforce_resource_not_found",
			"Active Support workforce resource not found",
			404,
		);
}
     ...
     ON CONFLICT(project_id, source_type, source_id) DO UPDATE SET title = excluded.title,
       ...
       updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')`,
		...
		.bind(
			...
			projectId,
			articleId,
			String(row.title || "Article"),
			hash,
			`project:${projectId}`,
			actor,
		)
		.run();
	if (!env.SUPPORT_AI_QUEUE)
		throw failure("support_ai_unavailable", "Support knowledge indexing is not configured", 503);
	try {
		await env.SUPPORT_AI_QUEUE.send(
			{ type: "support.knowledge.index.v1", projectId, sourceType: "article", sourceId: articleId },
			{ contentType: "json" },
		);
	} catch {
      ...
      last_error = 'queue_unavailable', updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
      WHERE project_id = ? AND source_type = 'article' AND source_id = ?`)
			.bind(projectId, articleId)
			.run();
		throw failure(
			"support_ai_unavailable",
			"Support knowledge indexing is temporarily unavailable",
			503,
		);
	}
}
...
function validateProviderCredentials(provider: string, credentials: Record<string, unknown>) {
	...
	if (missing.length) {
		throw failure(
			"configuration_required",
			`Provider configuration is missing required fields: ${missing.join(", ")}`,
			422,
		);
	}
	if (provider === "widget") {
		...
		if (
			widgetKey.length < 16 ||
			widgetKey.length > 255 ||
			signingSecret.length < 32 ||
			signingSecret.length > 512 ||
			!domains.some((domain) => String(domain).trim())
		) {
			throw failure(
				"support_provider_credentials_invalid",
				"Widget credentials or allowed domains are invalid",
				422,
			);
		}
	}
	if (provider === "smtp") {
		...
		const port =
			credentials.port == null || credentials.port === "" ? 587 : Number(credentials.port);
		if (
			!/^[^\s@]+@[^\s@]+\.[^\s@]+$/u.test(email) ||
			email.length > 320 ||
	...
	if (
		["whatsapp_cloud", "whatsapp_calls"].includes(provider) &&
		!/^\d{5,32}$/u.test(String(credentials.phone_number_id || ""))
	) {
		throw failure(
			"support_provider_credentials_invalid",
			"Provider phone number identifier is invalid",
			422,
		);
	}
	if (provider === "facebook_messenger" && !/^\d{5,32}$/u.test(String(credentials.page_id || ""))) {
		throw failure(
			"support_provider_credentials_invalid",
			"Provider page identifier is invalid",
			422,
		);
	}
	if (provider === "instagram" && !/^\d{5,32}$/u.test(String(credentials.instagram_id || ""))) {
		throw failure(
			"support_provider_credentials_invalid",
			"Provider account identifier is invalid",
			422,
		);
	}
	if (
		["twilio_sms", "twilio_voice"].includes(provider) &&
		!/^\+[1-9]\d{6,14}$/u.test(String(credentials.from_number || ""))
	) {
		throw failure(
			"support_provider_credentials_invalid",
			"Provider sender number must use E.164 format",
			422,
		);
	}
	if (
		...
		throw failure(
			"support_provider_credentials_invalid",
			"Voice status callback must use public HTTPS",
			422,
		);
	}
	...
	if (serialized.length > 64_000 || Object.keys(credentials).length > 50) {
		throw failure(
			"support_provider_credentials_invalid",
			"Provider credentials exceed supported limits",
			422,
		);
	}
}
...
async function validateProviderInbox(
	...
	const inbox = await db
		.prepare(`SELECT id FROM support_inboxes
    WHERE id = ? AND project_id = ? AND status = 'active' AND channel_type = ?
    LIMIT 1`)
		.bind(inboxId, projectId, provider)
		.first();
	if (!inbox) {
		throw failure(
...
function oauthProviderConfiguration(
	...
	const entries: Record<
		string,
		{ authorizationUrl: string; scope: string; pkce: boolean; extra?: Record<string, string> }
	> = {
		email_google: {
		...
		email_microsoft: {
			...
			scope: "offline_access openid email Mail.ReadWrite Mail.Send",
			pkce: true,
		},
		facebook_messenger: {
	...
	if (!resolved?.authorizationUrl)
		throw failure(
			"configuration_required",
			"OAuth is not configured for this Support provider",
			422,
		);
	...
	try {
		const json = atob(
			cursorValue
				.replace(/-/g, "+")
				.replace(/_/g, "/")
				.padEnd(Math.ceil(cursorValue.length / 4) * 4, "="),
		);
...
function projectIdFrom(value: string) {
	...
	if (!Number.isSafeInteger(id) || id <= 0)
		throw failure("project_id_invalid", "A valid Support project id is required", 422);
	return id;
}
function actorId(request: Request) {
	const value = String(
		request.headers.get("x-operator-id") ||
			...
			request.headers.get("x-superboard-agent-id") ||
			"",
	).trim();
...
function identifier(value: unknown) {
	...
	if (!id || id.length > 255)
		throw failure("support_identifier_invalid", "Support identifier is invalid", 422);
	return id;
}
function nullableIdentifier(value: unknown) {
	return value == null || value === "" ? null : identifier(value);
}
...
function timestamp(value: unknown, name: string, nullable: boolean) {
	...
	if (Number.isNaN(date.getTime()))
		throw failure("support_field_invalid", `${name} must be an ISO-8601 timestamp`, 422);
	return date.toISOString();
}
function optionalTimestamp(value: string | undefined) {
	return !value ? null : timestamp(value, "period", false);
}
function jsonValue(value: unknown, name: string, max: number) {
	if (value == null || typeof value !== "object")
		throw failure("support_field_invalid", `${name} must be a JSON object or array`, 422);
	const serialized = JSON.stringify(value);
	if (serialized.length > max)
		throw failure("support_field_invalid", `${name} exceeds its size limit`, 422);
	return serialized;
}
...
function auditInboxIds(type: string, id: string, row?: Record<string, unknown>): string[] {
	const ids = new Set<string>();
	if (type === "workforce/inboxes") ids.add(id);
	if (type === "workforce/inbox-members") ids.add(id.split(":")[0] ?? "");
	if (typeof row?.inbox_id === "string") ids.add(row.inbox_id);
	if (typeof row?.inbox_ids_json === "string") {
		const values: unknown = JSON.parse(row.inbox_ids_json);
		if (Array.isArray(values))
			for (const value of values) if (typeof value === "string") ids.add(value);
	}
	if (typeof row?.conditions_json === "string") {
		const conditions: unknown = JSON.parse(row.conditions_json);
		if (
			conditions &&
			typeof conditions === "object" &&
			"inbox_id" in conditions &&
			typeof conditions.inbox_id === "string"
		)
			ids.add(conditions.inbox_id);
		if (Array.isArray(conditions) && row.condition_mode === "all") {
			for (const entry of conditions) {
				const condition: unknown = entry;
				if (
					condition &&
...
async function audit(
	...
	const inboxIds = auditInboxIds(type, id, row);
	await db
		.prepare(`INSERT INTO support_operations_audit_events
      (id, project_id, resource_type, resource_id, action, actor_id, payload_json)
      VALUES (?, ?, ?, ?, ?, ?, ?)`)
		.bind(
			crypto.randomUUID(),
			...
			action.slice(0, 128),
			actor,
			JSON.stringify(inboxIds.length ? { inbox_ids: inboxIds } : {}),
		)
		.run();
}
