...
import { chunkText, embedSupportText, supportKnowledgeContext } from "./knowledge";
import { deliverSupportNotification, failSupportNotificationDelivery } from "./notifications";
import { decryptCredentialPayload, encryptCredentialPayload } from "./secrets";
import { evaluateAppliedSla, initializeConversationPolicies } from "./service-levels";
...
export async function publishSupportEvent(
    ...
    LIMIT 1
  `)
		.bind(projectId, eventName)
		.first();
	if (!subscribed) return;
	const serialized = JSON.stringify(payload);
	if (serialized.length > 64_000)
		throw failure("webhook_payload_too_large", "Webhook payload is limited to 64 KB");
	...
	if (isDeadLetterQueue(batch.queue, env)) {
		for (const message of batch.messages) {
			try {
				...
				console.error(
					JSON.stringify({
						event: "support_dead_letter_persistence_failed",
						message_id: message.id,
	...
	for (const message of batch.messages) {
		try {
			if (!isSupportQueueJob(message.body))
				throw failure("queue_job_invalid", "Support Queue job is invalid");
			await dispatchSupportJob(env, message.body);
			message.ack();
		} catch (error) {
			if (
				isSupportQueueJob(message.body) &&
        ...
        `)
			...
			if (
				isSupportQueueJob(message.body) &&
				message.body.type === "support.captain.task.v1" &&
...
export async function quarantineSupportDeadLetter(
  ...
  `)
    ...
    `)
			.bind(message.body.projectId, message.body.eventId)
			.run();
	}
	if (isSupportQueueJob(message.body) && message.body.type === "support.notification.deliver.v1") {
		await failSupportNotificationDelivery(db, message.body.projectId, message.body.notificationId);
	}
	if (
		isSupportQueueJob(message.body) &&
		message.body.type === "support.provider.delivery.send.v1"
	) {
    ...
    `)
			.bind(message.body.deliveryId, message.body.projectId)
			.first<{
				endpoint_id: string;
      ...
      `)
				.bind(delivery.last_error, delivery.message_id, delivery.conversation_id)
				.run();
	...
	if (isSupportQueueJob(message.body)) {
		if (message.body.scheduledJobId) {
			await db
				.prepare(`UPDATE support_scheduled_jobs SET status = 'failed',
        ...
        WHERE id = ? AND project_id = ? AND status NOT IN ('completed', 'cancelled')`)
				.bind(message.body.scheduledJobId, message.body.projectId)
				.run();
		}
		if (message.body.type === "support.captain.task.v1") {
			await db
				.prepare(`UPDATE support_assistant_tasks SET status = 'failed',
        last_error = COALESCE(last_error, 'processing_exhausted'),
        updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
        WHERE id = ? AND project_id = ? AND status NOT IN ('completed', 'cancelled')`)
				.bind(message.body.taskId, message.body.projectId)
				.run();
		} else if (message.body.type === "support.knowledge.index.v1") {
			await db
				.prepare(`UPDATE support_knowledge_documents SET status = 'failed',
        last_error = 'indexing_exhausted', updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
        WHERE project_id = ? AND source_type = ? AND source_id = ?`)
				.bind(message.body.projectId, message.body.sourceType, message.body.sourceId)
				.run();
		} else if (message.body.type === "support.export.requested.v1") {
			await db
				.prepare(`UPDATE support_export_jobs SET status = 'failed', last_error = 'processing_exhausted',
        updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ? AND project_id = ?`)
				.bind(message.body.exportId, message.body.projectId)
				.run();
		} else if (message.body.type === "support.import.requested.v1") {
			await db
				.prepare(`UPDATE support_import_jobs SET status = 'failed', last_error = 'processing_exhausted',
        updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ? AND project_id = ?`)
				.bind(message.body.importId, message.body.projectId)
				.run();
		}
	}
	await db
...
export async function deliverWebhookEvent(
  ...
  `)
		.bind(job.projectId, job.eventName)
		.all<{ id: string; configuration_json: string }>();
...
async function deliverOne(
  ...
  `)
	...
	if (!isSafePublicHttpsUrl(url))
		throw failure("webhook_configuration_invalid", "Webhook URL is invalid");
  ...
  `)
		.bind(job.projectId, webhook.id)
		.first<{ encrypted_payload: string }>();
	if (!storedSecret)
		throw failure("webhook_secret_unavailable", "Webhook signing secret is not configured");
	const decrypted = await decryptCredentialPayload(
		[env.SUPPORT_CREDENTIAL_ENCRYPTION_KEY, env.SUPPORT_CREDENTIAL_ENCRYPTION_KEY_PREVIOUS],
  ...
  `)
		.bind(crypto.randomUUID(), job.projectId, webhook.id, job.eventId, job.eventName)
		.run();
  ...
  `)
		.bind(response.status, webhook.id, job.eventId)
		.run();
}
...
async function recordFailure(
    ...
    WHERE webhook_id = ? AND event_id = ?
  `)
		.bind(status, error.slice(0, 1000), webhookId, eventId)
		.run();
}
...
export function isSupportQueueJob(value: unknown): value is SupportQueueJob {
	...
	if (
		...
		Number(job.projectId) <= 0 ||
		typeof job.type !== "string"
	)
		return false;
	if (job.type === "support.webhook.dispatch") {
		return (
			boundedString(job.eventId, 255) &&
			boundedString(job.eventName, 128) &&
			!!job.payload &&
	...
	if (job.type === "support.provider.event.received.v1") {
		return (
			boundedString(job.endpointId, 255) &&
			boundedString(job.provider, 64) &&
	...
	if (job.type === "support.provider.oauth.exchange.v1") {
		return (
			boundedString(job.endpointId, 255) &&
			boundedString(job.provider, 64) &&
			boundedString(job.oauthStateId, 255) &&
			boundedString(job.authorizationCode, 4_096)
		);
	}
	...
	if (job.type === "support.workflow.action.v1") {
		return (
			boundedString(job.resourceId, 255) &&
			boundedString(job.target, 255) &&
...
async function dispatchSupportJob(env: Env, job: SupportQueueJob) {
	...
	else if (job.type === "support.notification.deliver.v1")
		await deliverSupportNotification(env, job);
	...
	else if (job.type === "support.sla.evaluate.v1")
		await evaluateSla(env, job.projectId, job.resourceId);
	else if (job.type === "support.report.rollup.v1")
		await rollupReports(env.DB, job.projectId, job.resourceId);
	...
	if (job.scheduledJobId) {
      ...
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ? AND project_id = ?`)
			.bind(job.scheduledJobId, job.projectId)
			.run();
	}
}
...
async function runWorkflowAction(
	...
	if (job.action === "webhook") {
		const webhook =
			await env.DB.prepare(`SELECT id, settings_json configuration_json FROM support_integrations
      WHERE id = ? AND project_id = ? AND provider = 'webhook'
        AND status IN ('configured', 'validated', 'live_validated')`)
				.bind(job.target, job.projectId)
				.first<{ id: string; configuration_json: string }>();
		if (!webhook)
			throw Object.assign(failure("configuration_required", "Workflow webhook is not configured"), {
				status: 422,
			});
		return deliverOne(
			env,
...
async function processProviderEvent(
  ...
  `)
		.bind(job.eventRecordId, job.projectId, job.endpointId)
		.first<{
			id: string;
			event_type: string;
			payload_json: string;
	...
	const endpoint = await env.DB.prepare(
		...
		.bind(job.endpointId, job.projectId)
		.first<{ inbox_id: string | null }>();
	if (!endpoint?.inbox_id) {
		throw failure(
			"configuration_required",
			"Provider endpoint is not connected to a Support inbox",
			422,
		);
	}
	await env.DB.prepare(`
  ...
  `)
		.bind(job.projectId, normalized.externalUserId)
		.first<{ id: string }>();
  ...
  `)
	...
	const clientConversationId =
		`${job.provider}:${normalized.threadId || normalized.externalUserId}`.slice(0, 128);
  ...
  `)
	...
	if (!conversation)
		throw failure("provider_conversation_failed", "Provider conversation could not be persisted");
	...
	if (normalized.messageId && message.id) {
      ...
      WHERE id = ? AND conversation_id = ?
    `)
			.bind(normalized.messageId, String(message.id), conversation.id)
			.run();
	}
...
async function applyProviderDeliveryReceipt(
  ...
  `)
	...
	if (receipt.providerReference) {
      ...
      WHERE id = ? AND conversation_id = ?
    `)
			.bind(receipt.providerReference, delivery.message_id, delivery.conversation_id)
			.run();
	}
	await env.DB.prepare(`
...
async function roomJson(
	...
	const response = await stub.fetch(
		new Request(`https://support-room.internal${path}`, {
			method,
			headers: {
				"content-type": "application/json",
	...
	if (!response.ok) {
		...
		throw failure(
			...
			String(error.message || payload.message || "Support conversation operation failed").slice(
				0,
				1_000,
			),
			response.status,
...
async function exchangeProviderOAuth(
  ...
  `)
		.bind(job.oauthStateId, job.endpointId, job.projectId, job.provider)
		.first<{
	...
	if (adapter.basicAuth) {
		headers.set(
			"authorization",
			`Basic ${btoa(`${String(decrypted.payload.client_id || "")}:${String(decrypted.payload.client_secret || "")}`)}`,
		);
	}
	...
	try {
		token = JSON.parse(responseText) as Record<string, unknown>;
	} catch {
		/* handled below */
	}
...
async function retryProviderMessage(
    ...
    RETURNING id
  `)
		.bind(job.projectId, job.conversationId, job.messageId)
		.first<{ id: string }>();
...
async function sendProviderDelivery(
  ...
  `)
    ...
    `)
		...
		if (pending?.status === "sending") {
			throw failure(
				"provider_delivery_in_progress",
				"Provider delivery is already in progress",
				503,
			);
		}
		return;
  ...
  `)
		.bind(job.projectId, delivery.endpoint_id, delivery.idempotency_key)
		.run();
	const endpoint = await env.DB.prepare(`
  ...
  `)
		.bind(delivery.conversation_id, delivery.endpoint_id, job.projectId)
		.first<{
			provider: string;
			settings_json: string;
			encrypted_payload: string;
	...
	try {
		...
		const reasonCode = /^[a-z0-9_]{1,128}$/u.test(rawCode) ? rawCode : "provider_unavailable";
	...
	try {
		...
		/* provider-specific plain response */
	}
  ...
  `)
		.bind(response.status, providerReference, delivery.id)
		.run();
	await env.DB.prepare(`
    UPDATE messages SET provider_message_id = ?, failure_reason = NULL WHERE id = ? AND conversation_id = ?
  `)
		.bind(providerReference, delivery.message_id, delivery.conversation_id)
		.run();
    ...
    `)
			.bind(delivery.endpoint_id, job.projectId)
			.run();
	}
	await roomJson(
		env,
...
export async function outboundProviderRequest(
	...
	if (
		["email_google", "email_microsoft", "smtp"].includes(input.provider) &&
		(!env.EMAIL_SERVICE || !env.EMAIL_INTERNAL_TOKEN?.trim())
	) {
		throw failure("email_service_unavailable", "Email integration is unavailable", 503);
	}
	if (["email_google", "email_microsoft"].includes(input.provider)) {
		configurationValue(
			input.credentials.access_token,
			"OAuth authorization is required before sending email",
		);
	...
	if (input.provider === "smtp") {
		return env.EMAIL_SERVICE.fetch("https://email.internal/internal/v1/transport/smtp", {
			...
			body: JSON.stringify({
				...
				publicConfig: {
					host: String(input.credentials.host || ""),
					port: Number(input.credentials.port || 587),
				...
				secret: {
					password: input.credentials.password ? String(input.credentials.password) : null,
				},
	...
	if (input.provider === "telegram") {
		const botToken = configurationValue(
			input.credentials.bot_token,
		...
		return fetch(`https://api.telegram.org/bot${encodeURIComponent(botToken)}/sendMessage`, {
			...
			body: JSON.stringify({ chat_id: input.recipient, text: input.body }),
			signal,
		});
	}
	if (input.provider === "whatsapp_cloud") {
		const sourceId = providerObjectId(
			input.settings.phone_number_id ||
				input.settings.source_id ||
	...
	if (input.provider === "facebook_messenger") {
		const pageId = providerObjectId(
			input.settings.page_id || input.settings.source_id || input.credentials.page_id,
		);
		const accessToken = configurationValue(
	...
	if (input.provider === "line") {
		const accessToken = configurationValue(
			input.credentials.channel_access_token,
			"Messaging credentials are incomplete",
		);
		return fetch("https://api.line.me/v2/bot/message/push", {
			method: "POST",
			headers: { "content-type": "application/json", authorization: `Bearer ${accessToken}` },
			body: JSON.stringify({ to: input.recipient, messages: [{ type: "text", text: input.body }] }),
			signal,
		});
	}
	if (input.provider === "slack") {
		...
		return fetch("https://slack.com/api/chat.postMessage", {
			method: "POST",
			headers: { "content-type": "application/json", authorization: `Bearer ${accessToken}` },
			body: JSON.stringify({ channel: input.recipient, text: input.body }),
			signal,
		});
	}
	if (input.provider === "twilio_sms") {
		const accountSid = twilioAccountSid(input.credentials.account_sid);
		const authToken = configurationValue(
			input.credentials.auth_token,
			"Telephony credentials are incomplete",
		);
		const from = configurationValue(
			input.settings.from_number || input.credentials.from_number,
			"A sending number is required",
		);
	...
	if (input.provider === "twilio_voice") {
		...
		if (!isSafePublicHttpsUrl(callback)) {
			throw failure(
				"configuration_required",
				"The call status callback configuration is invalid",
				422,
			);
		}
	...
	if (input.provider === "twitter") {
		const accessToken = configurationValue(
			input.credentials.access_token,
			"OAuth authorization is required before sending messages",
		);
	...
	throw Object.assign(
		failure(
			"configuration_required",
			"Outbound adapter is not configured for this Support provider",
		),
		{ status: 422 },
	);
}
...
export async function refreshProviderCredentialsIfNeeded(
  ...
  `)
		.bind(encrypted, input.endpointId, input.projectId, input.credentialVersion)
		.run();
  ...
  `)
		.bind(input.endpointId, input.projectId)
		.run();
	return persisted;
}
...
async function providerCredentialsAfterVersionChange(
  ...
  `)
	...
	if (!currentRow || (requireChange && currentRow.credential_version === input.credentialVersion))
		return null;
	return (
		await decryptCredentialPayload(
			[env.SUPPORT_CREDENTIAL_ENCRYPTION_KEY, env.SUPPORT_CREDENTIAL_ENCRYPTION_KEY_PREVIOUS],
			currentRow.encrypted_payload,
		)
	).payload;
}
...
async function markProviderAuthorizationRequired(
    ...
    WHERE id = ? AND project_id = ? AND status <> 'disabled'
  `)
		.bind(errorCode, input.endpointId, input.projectId)
		.run();
}
...
function providerObjectId(value: unknown) {
	const resolved = configurationValue(
		value,
		"A provider account or recipient identifier is required",
	);
	if (!/^[1-9][0-9]{0,31}$/u.test(resolved)) {
		throw failure(
			"configuration_required",
			"A provider account or recipient identifier is invalid",
			422,
		);
	}
	return resolved;
...
function phoneRecipient(value: unknown) {
	const resolved = configurationValue(value, "A messaging recipient is required").replace(
		/[\s()+.-]/gu,
		"",
	);
...
async function failProviderDelivery(
    ...
    updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
    WHERE id = ? AND project_id = ?`)
		.bind(reason, terminal ? 1 : 0, delivery.id, projectId)
		.run();
	if (!terminal) return;
	try {
		await roomJson(
      ...
      WHERE id = ? AND conversation_id = ?
    `)
			.bind(reason, delivery.message_id, delivery.conversation_id)
			.run();
	}
...
async function indexKnowledge(
  ...
  `)
		.bind(job.projectId, job.sourceType, job.sourceId)
		.first<{
			id: string;
			title: string;
	...
	const source =
        ...
        FROM support_articles article WHERE article.id = ? AND article.project_id = ? AND article.status = 'published'`)
					.bind(job.sourceId, job.projectId)
					.first<{ content: string }>()
			: null;
	if (!source?.content)
		throw failure("knowledge_source_unavailable", "Knowledge source is unavailable");
    ...
    UPDATE support_knowledge_documents SET status = 'indexed', chunk_count = ?,
      ...
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ? AND project_id = ?
  `)
		.bind(chunks.length, document.id, job.projectId)
		.run();
}
  ...
  `)
    ...
    `)
		...
		if (state?.status === "running") {
			throw failure(
				"captain_task_in_progress",
				"Support intelligence task is already running",
				503,
			);
		}
		return;
	...
	if (task.task_type === "handoff") {
		if (!task.conversation_id || !task.assistant_id) {
			throw failure(
				"captain_handoff_invalid",
				"Human handoff requires an active assistant and conversation",
				422,
			);
		}
    ...
    `)
			.bind(task.conversation_id, task.assistant_id, job.projectId)
			.first<{ id: string }>();
		if (!permitted) {
			throw failure(
				"captain_handoff_disabled",
				"Human handoff is not enabled for this assistant",
				422,
			);
		}
      ...
      WHERE id = ? AND project_id = ? RETURNING id`)
			.bind(task.conversation_id, job.projectId)
			.first<{ id: string }>();
		if (!assigned)
			throw failure("captain_handoff_invalid", "Human handoff conversation was not found");
		await env.DB.batch([
			env.DB.prepare(`INSERT INTO support_assignment_events
        (id, project_id, conversation_id, reason, payload_json)
        VALUES (?, ?, ?, 'captain_handoff', ?)`).bind(
				crypto.randomUUID(),
				job.projectId,
        ...
        WHERE project_id = ? AND conversation_id = ? AND assistant_id = ?
          ...
          AND json_extract(input_json, '$.automatic_trigger') = 1`).bind(
				job.projectId,
				task.conversation_id,
				task.assistant_id,
				task.id,
        ...
        updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ? AND project_id = ?`).bind(
				JSON.stringify({ handoff: true, conversation_id: task.conversation_id }),
				task.id,
				job.projectId,
			),
		]);
	...
	if (task.task_type === "run_tool") {
		if (!task.assistant_id)
			throw failure("captain_tool_invalid", "Tool execution requires an assistant");
		const toolId = String(input.tool_id || "");
		const argumentsValue = asObject(input.arguments);
		const tool =
			await env.DB.prepare(`SELECT id, endpoint_url, method, input_schema_json, headers_json
      FROM support_assistant_tools WHERE id = ? AND project_id = ? AND assistant_id = ? AND allowed = 1`)
				.bind(toolId, job.projectId, task.assistant_id)
				.first<{
					id: string;
					endpoint_url: string;
					method: "GET" | "POST";
		...
		if (
			Object.keys(headers).some((name) => !["accept", "content-type"].includes(name.toLowerCase()))
		) {
		...
		if (!response.ok)
			throw failure("captain_tool_failed", `Assistant tool returned HTTP ${response.status}`);
      ...
      completed_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now'),
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ? AND project_id = ?`)
			.bind(JSON.stringify({ tool_id: tool.id, output: toolResult }), task.id, job.projectId)
			.run();
		return;
	}
	...
	if (typeof resultPayload.text !== "string" || !resultPayload.text) {
		...
		try {
			...
			console.error(
				JSON.stringify({
					event: "support_captain_knowledge_degraded",
					project_id: job.projectId,
					task_id: task.id,
		...
		const scenarios = task.assistant_id
          ...
          WHERE project_id = ? AND assistant_id = ? AND active = 1 ORDER BY position, created_at LIMIT 20`)
					.bind(job.projectId, task.assistant_id)
					.all<{ name: string; instructions: string }>()
      ...
      WHERE id = ? AND project_id = ? AND status = 'running'
    `)
			.bind(JSON.stringify(resultPayload), task.id, job.projectId)
			.run();
	}
...
async function dispatchCampaign(
  ...
  `)
	...
	const externalUser =
		typeof audience.external_user_id === "string" ? audience.external_user_id : null;
	const contacts = externalUser
      ...
      `)
				.bind(campaign.inbox_id, job.projectId, externalUser)
				.all<CampaignContact>()
		: await env.DB.prepare(`
        SELECT contact.id, contact.external_user_id, contact.email, contact.phone,
          (SELECT relation.source_id FROM support_contact_inboxes relation
  ...
  `)
		.bind(job.projectId, campaign.inbox_id)
	...
	if (!endpoint) {
        ...
        ON CONFLICT(campaign_id, idempotency_key) DO UPDATE SET
          ...
          updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
      `)
				.bind(crypto.randomUUID(), job.projectId, campaign.id, contact.id, idempotencyKey)
				.run();
		}
...
async function dispatchCampaignContact(
  ...
  `)
		.bind(deliveryId, projectId, campaign.id, contact.id, endpoint.id, idempotencyKey)
		.run();
  ...
  `)
		.bind(projectId, campaign.id, idempotencyKey)
		.first<{ id: string; status: string }>();
	if (
		!campaignDelivery ||
		["sent", "delivered", "read", "cancelled"].includes(campaignDelivery.status)
	)
		return;
	...
	if (!recipient) {
      ...
      WHERE id = ? AND project_id = ?
    `)
			.bind(endpoint.id, campaignDelivery.id, projectId)
			.run();
		return;
	}
	...
	if (!message.id)
		throw failure("campaign_message_failed", "Proactive Support message could not be persisted");
  ...
  `)
		...
		.first<{ id: string; status: string }>();
	const providerDelivery: { id: string; status: string } | null =
		inserted ||
		(await env.DB.prepare(`
    SELECT id, status FROM support_provider_deliveries
    WHERE project_id = ? AND endpoint_id = ? AND idempotency_key = ?
  `)
			.bind(projectId, endpoint.id, idempotencyKey)
			.first<{ id: string; status: string }>());
	if (!providerDelivery)
		throw failure("campaign_delivery_failed", "Proactive Support delivery could not be persisted");
	await env.DB.prepare(`
  ...
  `)
		.bind(conversation.id, endpoint.id, campaignDelivery.id, projectId)
		.run();
	if (!inserted && providerDelivery.status === "failed") {
      ...
      UPDATE support_provider_deliveries SET status = 'queued', last_error = NULL,
        ...
        updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now') WHERE id = ? AND project_id = ?
    `)
			.bind(providerDelivery.id, projectId)
			.run();
	}
...
async function campaignConversation(
  ...
  `)
	...
	if (!created)
		throw failure(
			"campaign_conversation_failed",
			"Proactive Support conversation could not be persisted",
		);
	if (created.id === id) {
		await runConversationAutomations(
			env,
...
async function reconcileCampaign(db: D1Database, projectId: number, campaignId: string) {
  ...
  `)
		...
		.first<{ total: number; active: number | null; succeeded: number | null }>();
    ...
    WHERE id = ? AND project_id = ? AND status IN ('scheduled', 'running')
  `)
		.bind(status, status, campaignId, projectId)
		.run();
}
...
async function processExport(
  ...
  `)
	...
	const from =
		request.resource_type === "reports" ? reportExportBoundary(filters.from, false) : null;
	const to = request.resource_type === "reports" ? reportExportBoundary(filters.to, true) : null;
	const inboxId = typeof filters.inbox_id === "string" ? filters.inbox_id : null;
	const rows =
 ...
 ORDER BY occurred_at DESC LIMIT 10000`,
					...
					.bind(job.projectId, from, from, to, to, inboxId, inboxId)
					.all()
			: await env.DB.prepare(
	...
	await env.ATTACHMENTS.put(key, JSON.stringify({ data: rows.results }), {
...
async function processImport(
  ...
  `)
		.bind(job.importId, job.projectId)
		.first<{
			id: string;
			storage_key: string;
			created_by: string;
	...
	try {
		...
		if (!object)
			throw failure("contact_import_file_not_found", "Contact import file was not found", 422);
      ...
      `).bind(
					...
					row.email,
					row.phone,
					JSON.stringify(row.custom_attributes),
				),
			);
    ...
    `)
			.bind(
				JSON.stringify({ rows_read: rows.length }),
      ...
      WHERE id = ? AND project_id = ?
    `)
			.bind(safeImportError(error), request.id, job.projectId)
			.run();
		throw error;
	}
}
...
function contactImportRows(text: string) {
	...
	if (!values.length || values.length > 10_000) {
		throw failure(
			...
			"Contact import must contain between 1 and 10000 rows",
			422,
		);
	}
	return values.map((value) => {
		...
		const custom =
			...
			typeof row.custom_attributes === "object" &&
...
function csvRecords(text: string): Record<string, string>[] {
	...
	if (quoted)
		throw failure("contact_import_invalid", "Contact CSV contains an unterminated field", 422);
	row.push(cell);
	if (row.some((value) => value.trim())) table.push(row);
...
async function rollupReports(db: D1Database, projectId: number, bucket: string) {
    ...
    DO UPDATE SET metrics_json = excluded.metrics_json,
      updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
  `)
		.bind(projectId, bucket, projectId, bucket, bucket)
		.run();
}
...
export function normalizeInboundProviderEvent(provider: string, payload: Record<string, unknown>) {
	...
	if (provider === "whatsapp_cloud") {
		...
		return inboundMessage({
			...
			messageBody: textValue(text.body || message.caption || buttonReply.title || listReply.title),
	...
	return inboundMessage({
		externalUserId: textValue(
			payload.external_user_id || sender.id || payload.user_id || payload.from,
		),
		...
		messageBody: textValue(
			typeof payload.message === "string"
				? payload.message
				: payload.text || message.text || payload.body,
		),
		subject: textValue(payload.subject),
	});
}
	...
	if (provider === "twitter") {
		...
		if (!data.dm_event_id && errors.length > 0) {
			...
			const normalized = raw
				...
				.replace(/^_+|_+$/gu, "");
			const code = /^[a-z0-9_]{1,128}$/u.test(normalized)
				? normalized
...
export function providerOutboundValidatesLive(provider: string, payload: Record<string, unknown>) {
	if (!["email_google", "email_microsoft", "smtp"].includes(provider)) return true;
	return (
		String(payload.status || "").toLowerCase() === "sent" &&
...
function normalizeProviderDeliveryStatus(value: unknown): ProviderDeliveryStatus | null {
	...
	if (
		...
		return "sent";
	if (["delivered", "completed", "complete", "success", "succeeded"].includes(status))
		return "delivered";
...
async function finishProviderEvent(db: D1Database, id: string, status: "processed" | "ignored") {
	await db
		.prepare(`UPDATE support_provider_events SET status = ?, last_error = NULL,
...
function isDeadLetterQueue(queue: string, env: Env) {
	return [
		env.DLQ_NAME,
		env.SUPPORT_EVENTS_DLQ_NAME,
		env.SUPPORT_AI_DLQ_NAME,
		env.SUPPORT_BULK_DLQ_NAME,
	]
		.filter(Boolean)
		.includes(queue);
}
...
function firstObject(value: unknown) {
	return asObject(arrayValues(value)[0]);
}
function textValue(value: unknown) {
	return value == null ? null : String(value).slice(0, 8_000);
}
function boundedPrompt(input: Record<string, unknown>, taskType: string) {
	const value = JSON.stringify({ task: taskType, input });
	if (value.length > 32_000)
		throw failure("captain_input_too_large", "Support intelligence input is too large");
	return value;
}
			...
			return [
				{
					...
					...(textValue(source.title)
						? { title: String(textValue(source.title)).slice(0, 255) }
						: {}),
...
async function captainConversationContext(
	db: D1Database,
	projectId: number,
	conversationId: string,
) {
	const conversation = await db
		.prepare(`SELECT id, subject, status, priority, inbox_id,
      last_message_preview, custom_attributes_json FROM conversations
    WHERE id = ? AND project_id = ?`)
		.bind(conversationId, projectId)
		.first<Record<string, unknown>>();
	if (!conversation)
		throw failure("captain_conversation_not_found", "Support conversation was not found");
	const messages = await db
		.prepare(`SELECT sender_kind, visibility, body, created_at FROM messages
...
function captainKnowledgeQuery(input: Record<string, unknown>, conversation: unknown) {
	const explicit = [input.query, input.prompt, input.text].find(
	...
	const messageText = messages
		.slice(-10)
		.map((message) => String(asObject(message).body || ""))
		.join("\n");
	return (
		`${String(source.subject || "")}\n${String(source.last_message_preview || "")}\n${messageText}`
			.trim()
			.slice(0, 32_000) || "Support conversation assistance"
	);
}
function validateCaptainToolInput(schema: Record<string, unknown>, input: Record<string, unknown>) {
	...
	if (new TextEncoder().encode(serialized).byteLength > 32_000 || Object.keys(input).length > 100) {
		throw failure(
			"captain_tool_input_invalid",
			"Assistant tool input exceeds supported limits",
			422,
		);
	}
	...
	for (const name of required) {
		if (!(name in input))
			throw failure("captain_tool_input_invalid", `Assistant tool input requires ${name}`, 422);
	}
	if (schema.additionalProperties === false) {
		...
		if (unknown)
			throw failure(
				"captain_tool_input_invalid",
				`Assistant tool input does not accept ${unknown}`,
				422,
			);
	}
	for (const [name, value] of Object.entries(input)) {
		...
		if (
			...
			throw failure(
				"captain_tool_input_invalid",
				`Assistant tool input ${name} has an invalid type`,
				422,
			);
		}
	}
...
function oauthTokenAdapter(
	...
	const adapters: Record<
		string,
		{ tokenUrl: string; includeSecret: boolean; basicAuth?: boolean }
	> = {
	...
	if (!adapter?.tokenUrl)
		throw failure("configuration_required", "OAuth token adapter is not configured");
	return adapter;
}
