...
import { usePublicConfig } from "@superboard/front-ui/lib/config.js";
import {
...
import { inboxDeepLink } from "../../api/support/inboxDeepLink.js";
import {
	getInboxMessages,
...
} from "../../api/support/inboxService.js";
import {
	executeSupportMacro,
	getSupportDraft,
	saveSupportDraft,
...
export default function InboxPage({ inboxId }: { inboxId?: string } = {}) {
	const Title = inboxId ? "h2" : "h1";
	...
	const [macros, setMacros] = useState<Array<{ value: string; label: string }>>([]);
	...
	const loadItems = useCallback(async () => {
		...
		try {
			const result = await getUnifiedInboxItems(projectId, {
				type: inboxId ? "conversation" : typeFilter,
				inbox_id: inboxId,
				status: statusFilter,
			});
	...
	}, [projectId, statusFilter, typeFilter, inboxId]);
	...
	const loadMessages = useCallback(async () => {
		...
		try {
			...
			showErrorNotification(
				error instanceof Error ? error.message : "Unable to load the conversation",
			);
		}
	...
	useEffect(() => {
		...
		void Promise.all([listInboxMacros(projectId), listInboxCannedResponses(projectId)])
			.then(([macroPage, responsePage]) => {
				...
				setCannedResponses(responsePage.data.filter((response) => response.active));
			})
			.catch(() => {
				setMacros([]);
				setCannedResponses([]);
		...
		const connect = async () => {
			...
			try {
				...
				socket.onmessage = (message) => {
					...
					if (!event || (event.conversation_id && event.conversation_id !== conversationId)) return;
					if (
						event.type === "message.created" &&
						event.message?.conversation_id === conversationId
					) {
						setMessages((current) => mergeMessage(current, event.message!));
						void markInboxConversationRead(projectId, conversationId).catch(() => undefined);
						void loadItems();
					} else if (
	...
	const send = async () => {
		...
		setReply("");
		try {
			await sendInboxMessage(projectId, selected.source_id, value, pending.id, privateNote);
			pendingTextMessage.current = null;
			void saveSupportDraft(projectId, selected.source_id, "").catch(() => undefined);
	...
	const update = async (
		...
		try {
			...
			showErrorNotification(
				error instanceof Error ? error.message : "Unable to update the conversation",
			);
		}
	};
	...
	const updateReply = (value: string) => {
		...
		setReply(value);
		if (!projectId || selected?.source_type !== "conversation") return;
		void setInboxConversationTyping(projectId, selected.source_id, value.trim().length > 0).catch(
			() => undefined,
		);
		if (typingTimer.current) window.clearTimeout(typingTimer.current);
		...
		draftTimer.current = window.setTimeout(() => {
			if (projectId && selected?.source_type === "conversation") {
				void saveSupportDraft(projectId, selected.source_id, value).catch(() => undefined);
			}
		}, 1_000);
	};
	...
	const attach = async (file?: File) => {
		...
		try {
			...
			let pending =
				pendingAttachment.current?.signature === signature ? pendingAttachment.current : null;
			if (!pending) {
				pending = {
					signature,
	...
	const downloadAttachment = async (message: InboxMessage) => {
		...
		try {
			...
			showErrorNotification(
				error instanceof Error ? error.message : "Unable to download the attachment",
			);
		}
	};
	...
	const executeMacro = async () => {
		if (!projectId || selected?.source_type !== "conversation" || !selectedMacroId) return;
		try {
			await executeSupportMacro(projectId, selected.source_id, selectedMacroId);
			setSelectedMacroId("");
			await loadItems();
			showSuccessNotification("Macro applied");
		} catch (error) {
			showErrorNotification(error instanceof Error ? error.message : "Unable to apply macro");
		}
	};
		...
		<div className="flex min-h-full flex-col overflow-hidden">
			...
			<div className="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden p-6">
				<div className="flex flex-wrap items-start justify-between gap-3">
					<div>
						<Title className="ds-page-title">
							<Trans>Inbox</Trans>
						</Title>
						<p className="text-sm text-muted-foreground">
							Real-time customer conversations, assignments, attachments, macros and private notes.
						</p>
					</div>
					<div className="flex flex-wrap gap-2">
						...
						<Button variant="outline" disabled={loading} onClick={() => void loadItems()}>
				...
				<Alert>
					...
					<AlertDescription>
						Conversations, attachments and real-time events are handled by the dedicated Support
						Worker and database.
					</AlertDescription>
				</Alert>
				{degradedSources.map((source) => (
					<Alert key={`${source.source_type}:${source.code}`} variant="destructive">
						<AlertTitle>
							{sourceLabel[source.source_type as UnifiedInboxItem["source_type"]] || "Inbox source"}{" "}
							is unavailable
						</AlertTitle>
						<AlertDescription>
								...
								<>
									<header className="flex flex-wrap items-center justify-between gap-3 border-b p-4">
										<div className="flex min-w-0 items-center gap-3">
											...
											<div className="min-w-0">
												<div className="flex items-center gap-2">
													...
													<Badge variant={realtimeStatus === "live" ? "secondary" : "outline"}>
														{realtimeStatus === "live" ? (
										...
										<div className="flex gap-2">
											<select
												...
												onChange={(event) =>
													void update({
														priority: event.target.value as InboxConversation["priority"],
													})
												}
											>
											...
											<select
												...
												onChange={(event) =>
													void update({
														status: event.target.value as InboxConversation["status"],
													})
												}
											>
												<option value="open">Open</option>
										...
										<div className="grid w-full gap-2 sm:grid-cols-2 lg:grid-cols-5">
											<ConfigurationSelect
												...
												options={agents}
												onChange={(value) => void update({ assigned_user_id: value || null })}
											/>
											<ConfigurationSelect
												label="Assigned team"
												value={String(selected.source.assigned_team_id || "")}
												options={teams}
												onChange={(value) => void update({ assigned_team_id: value || null })}
											/>
											<ConfigurationSelect
												label="Inbox"
												value={String(selected.source.inbox_id || "")}
												options={inboxes}
												onChange={(value) => void update({ inbox_id: value || null })}
											/>
											<input
												key={`${selected.id}:labels:${String(selected.source.labels_json || "")}`}
												aria-label="Conversation labels"
												className="rounded-md border bg-background px-3 py-2 text-sm"
												defaultValue={conversationLabels(selected.source.labels_json).join(", ")}
											...
											<input
												...
												type="datetime-local"
												className="rounded-md border bg-background px-3 py-2 text-sm"
												defaultValue={localDateTime(String(selected.source.snoozed_until || ""))}
												onBlur={(event) =>
													void update({
										...
										<ConversationAttributes
											...
											onSave={(customAttributes) => update({ custom_attributes: customAttributes })}
										/>
									</header>
											...
											>
												...
												>
													<p className="whitespace-pre-wrap text-sm">
														{message.body ||
															(message.attachment_name ? "Attachment" : "Empty message")}
													</p>
													{message.attachment_name && (
														<Button
															...
															size="sm"
															variant={
																message.sender_kind === "agent" && message.visibility !== "private"
																	? "secondary"
																	: "outline"
															}
															className="mt-2"
									...
									>
										...
										<div className="flex gap-2">
											...
											/>
											<Button type="submit" disabled={!reply.trim() || uploading}>
...
function ConversationAttributes({
	...
	const [attributes, setAttributes] = useState(() => conversationAttributes(value));
	useEffect(() => {
	...
	return (
		<div className="space-y-2 rounded-md border p-3">
			...
			{attributes.map((attribute, index) => (
				<div className="grid gap-2 md:grid-cols-[1fr_1fr_auto]" key={attribute.id}>
					<input
					...
					<Button
						...
						variant="ghost"
						onClick={() =>
							setAttributes((current) => current.filter((item) => item.id !== attribute.id))
						}
					>
						<Trash2 />
					</Button>
	...
	return (
		<div className="flex flex-1 items-center justify-center bg-muted/20 p-6">
			<Card className="w-full max-w-2xl p-6">
				...
				<p className="mt-5 whitespace-pre-wrap text-sm text-muted-foreground">{item.preview}</p>
				<div className="mt-5 grid gap-3 rounded-md border p-4 text-sm sm:grid-cols-2">
					...
					<Detail label="Deadline" value={dateTime(String(source.deadline_at || ""))} />
				</div>
