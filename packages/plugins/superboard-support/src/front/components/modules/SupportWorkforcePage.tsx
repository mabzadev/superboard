...
export default function SupportWorkforcePage() {
	...
	const capacities = useSupportCollection(projectRef, listSupportCapacityPolicies);
	...
	const saveMember = async () => {
		if (!projectRef || !member.auth_user_id.trim() || !member.display_name.trim()) return;
		setSaving(true);
		try {
	...
	const saveLeave = async () => {
		if (!projectRef || !leave.membershipId || !leave.startsAt || !leave.endsAt) return;
		setSaving(true);
		try {
	...
	const removeMember = async (id: string) => {
		if (!projectRef || !window.confirm("Remove this member from Support?")) return;
		try {
	...
	const setMemberActive = async (item: SupportMembership) => {
		...
		try {
			...
			await Promise.all([memberships.reload(), loadSummary()]);
			showSuccessNotification(item.active ? "Support member disabled" : "Support member enabled");
		} catch (cause) {
			showErrorNotification(moduleErrorMessage(cause));
		} finally {
			setSaving(false);
	...
	return (
		<ModulePage
			title={t("Workforce")}
			description={t("Support roles, teams, availability, capacity and leave schedules.")}
			error={summaryError}
		>
			{!selectedProject ? (
				<EmptyProject />
						...
						<SupportMetric
							...
							value={
								summary?.memberships.reduce(
									(total, group) => total + (group.active ? Number(group.count) : 0),
									0,
								) ?? "—"
							}
						/>
						<SupportMetric label={t("Active teams")} value={summary?.active_teams ?? "—"} />
						<SupportMetric label={t("Active inboxes")} value={summary?.active_inboxes ?? "—"} />
						<SupportMetric label={t("Members on leave")} value={summary?.active_leaves ?? "—"} />
					</div>
					<Tabs defaultValue="members">
						<TabsList>
							<TabsTrigger value="members">{t("Members")}</TabsTrigger>
							<TabsTrigger value="teams">{t("Teams")}</TabsTrigger>
							<TabsTrigger value="inboxes">{t("Inboxes")}</TabsTrigger>
							<TabsTrigger value="routing">{t("Routing")}</TabsTrigger>
							<TabsTrigger value="leaves">{t("Leave")}</TabsTrigger>
							<TabsTrigger value="capacity">{t("Capacity")}</TabsTrigger>
						</TabsList>
						<TabsContent className="space-y-4" value="members">
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">{t("Add a Support member")}</CardTitle>
								</CardHeader>
								<CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-6">
									<Field label={t("Identity user ID")}>
										<Input
									...
									<div className="flex items-end">
										<Button className="w-full" disabled={saving} onClick={() => void saveMember()}>
											<Plus /> {t("Add member")}
										</Button>
									</div>
								</CardContent>
											...
											<CardContent className="flex items-center justify-between gap-4 p-4">
												<div>
													...
													<p className="text-sm text-muted-foreground">
														{item.auth_user_id} · {item.role} {t("· capacity")}
														{item.capacity}
													</p>
												</div>
						...
						<TabsContent className="space-y-4" value="teams">
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">{t("Create a team")}</CardTitle>
								</CardHeader>
								<CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
									...
									<Field label={t("Description")}>
										<Input
											...
											onChange={(e) => setTeam((v) => ({ ...v, description: e.target.value }))}
										/>
									</Field>
									<div className="flex items-center gap-3 pt-7">
										...
										/>
										<Label>{t("Automatic assignment")}</Label>
									</div>
									<div className="flex items-end">
										<Button className="w-full" disabled={saving} onClick={() => void saveTeam()}>
											<Plus /> {t("Create team")}
										</Button>
									</div>
								</CardContent>
							...
							{teams.loading ? (
								...
								<SupportEmpty
									title={t("No Support teams")}
									description={t("Create a team to group agents and route conversations.")}
								/>
							) : (
								<div className="grid gap-3 lg:grid-cols-2">
									{teams.items.map((item) => (
										<Card key={item.id}>
											<CardContent className="flex items-center justify-between p-4">
												...
												<div className="flex items-center gap-2">
													<SupportStatus value={item.active ? "active" : "disabled"} />
													<Button
														disabled={saving}
														size="sm"
														variant="outline"
						...
						<TabsContent className="space-y-4" value="inboxes">
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">{t("Create an inbox")}</CardTitle>
								</CardHeader>
								<CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
									<Field label={t("Name")}>
										<Input
									...
									<Field label={t("Channel type")}>
										<NativeSelect
											value={inbox.channel_type}
											onChange={(channel_type) => {
							...
							{inboxes.loading ? (
								...
								<SupportEmpty
									title={t("No Support inboxes")}
									description={t("Create an inbox before connecting a customer channel.")}
								/>
							) : (
								<div className="grid gap-3 lg:grid-cols-2">
										...
										<Card key={item.id}>
											<CardContent className="flex items-center justify-between gap-3 p-4">
												<div>
													<p className="font-medium">{item.name}</p>
													<p className="text-sm text-muted-foreground">
														{item.identifier} · {item.channel_type}
													</p>
												</div>
												<div className="flex items-center gap-2">
													...
													<Button
														...
														size="sm"
														variant="outline"
														onClick={() => void setInboxActive(item.id, item.status === "active")}
													>
													...
													<Button
														...
														size="icon"
														variant="ghost"
														onClick={() => void removeInbox(item.id)}
													>
														<Trash2 />
													</Button>
												</div>
							...
							<SupportLoadMore
								visible={inboxes.hasMore}
								loading={inboxes.loadingMore}
								onClick={inboxes.loadMore}
							/>
						</TabsContent>
						<TabsContent className="space-y-4" value="routing">
							...
							<Card>
								<CardHeader>
									<CardTitle className="text-base">{t("Membership routing")}</CardTitle>
								</CardHeader>
								<CardContent className="grid gap-4 md:grid-cols-3">
									<Field label={t("Member")}>
										<NativeSelect
											value={routing.membershipId}
											onChange={(membershipId) =>
												setRouting((value) => ({ ...value, membershipId }))
											}
											optionsWithLabels={memberships.items.map((item) => ({
												value: item.id,
									...
									<Field label={t("Team")}>
										<NativeSelect
											value={routing.teamId}
											onChange={(teamId) => {
												const next = { ...routing, teamId };
												setRouting(next);
												void reloadRouting(next);
											}}
											optionsWithLabels={teams.items.map((item) => ({
												value: item.id,
												label: item.name,
											}))}
										/>
									...
									<Field label={t("Inbox")}>
										<NativeSelect
											value={routing.inboxId}
											onChange={(inboxId) => {
												const next = { ...routing, inboxId };
												setRouting(next);
												void reloadRouting(next);
											}}
											optionsWithLabels={inboxes.items.map((item) => ({
												value: item.id,
												label: item.name,
											}))}
										/>
									...
									<div className="flex flex-wrap gap-2 md:col-span-3">
										<Button
											disabled={saving || !routing.teamId || !routing.membershipId}
											onClick={() => void updateRouting("team", true)}
										>
											{t("Add to team")}
										</Button>
										<Button
											disabled={saving || !routing.teamId || !routing.membershipId}
											variant="outline"
											onClick={() => void updateRouting("team", false)}
										>
											{t("Remove from team")}
										</Button>
										<Button
											disabled={saving || !routing.inboxId || !routing.membershipId}
											onClick={() => void updateRouting("inbox", true)}
										>
											{t("Add to inbox")}
										</Button>
										<Button
											disabled={saving || !routing.inboxId || !routing.membershipId}
											variant="outline"
											onClick={() => void updateRouting("inbox", false)}
										>
											{t("Remove from inbox")}
										</Button>
									</div>
							...
							<div className="grid gap-4 lg:grid-cols-2">
								<RoutingList title={t("Team members")} items={teamMembers} />
								<RoutingList title={t("Inbox members")} items={inboxMembers} />
							</div>
						</TabsContent>
						<TabsContent className="space-y-4" value="leaves">
							<SupportSearchToolbar
								query={leaves.query}
								setQuery={leaves.setQuery}
								onSearch={leaves.search}
								onRefresh={leaves.reload}
								loading={leaves.loading}
							/>
							<SupportError message={leaves.error} />
							<Card>
								<CardHeader>
									<CardTitle className="text-base">{t("Schedule leave")}</CardTitle>
								</CardHeader>
								<CardContent className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
									<Field label={t("Member")}>
										<NativeSelect
											value={leave.membershipId}
											onChange={(membershipId) => {
												setLeave((value) => ({ ...value, membershipId }));
											}}
											optionsWithLabels={memberships.items.map((item) => ({
												value: item.id,
												label: item.display_name,
									...
									<Field label={t("Starts")}>
										<Input
											...
											onChange={(event) =>
												setLeave((value) => ({ ...value, startsAt: event.target.value }))
											}
										/>
									</Field>
									<Field label={t("Ends")}>
										<Input
											...
											value={leave.endsAt}
											onChange={(event) =>
												setLeave((value) => ({ ...value, endsAt: event.target.value }))
											}
										/>
									</Field>
									<Field label={t("Reason")}>
										<Input
											value={leave.reason}
											onChange={(event) =>
												setLeave((value) => ({ ...value, reason: event.target.value }))
											}
										/>
									</Field>
									<div className="flex items-end">
										<Button className="w-full" disabled={saving} onClick={() => void saveLeave()}>
											<Plus /> {t("Schedule")}
										</Button>
									</div>
								</CardContent>
							...
							{leaves.loading ? (
								...
								<SupportEmpty
									title={t("No leave schedules")}
									description={t("Scheduled leave is applied to automatic assignment.")}
								/>
							) : (
								leaves.items.map((item) => (
									<Card key={item.id}>
										<CardContent className="flex items-center justify-between gap-3 p-4">
											<div>
												<p className="font-medium">
													{memberships.items.find(
														(memberItem) => memberItem.id === item.membership_id,
						...
						<TabsContent className="space-y-4" value="capacity">
							<SupportSearchToolbar
								query={capacities.query}
								setQuery={capacities.setQuery}
								onSearch={capacities.search}
								onRefresh={capacities.reload}
								loading={capacities.loading}
							/>
							<SupportError message={capacities.error} />
							<Card>
								<CardHeader>
									<CardTitle className="text-base">{t("Create a capacity policy")}</CardTitle>
								</CardHeader>
								<CardContent className="grid gap-4 md:grid-cols-3">
									<Field label={t("Name")}>
										<Input
											value={capacity.name}
											onChange={(event) =>
												setCapacity((value) => ({ ...value, name: event.target.value }))
											}
										/>
									</Field>
									<Field label={t("Default capacity")}>
										<Input
											type="number"
											min={0}
									...
									<CardContent className="flex items-center justify-between p-4">
										...
										<div className="flex items-center gap-2">
											...
											<Button
												...
												size="sm"
												variant="outline"
												onClick={() => void setCapacityActive(item.id, item.active)}
											>
												{item.active ? "Disable" : "Enable"}
											</Button>
											<Button
												...
												size="icon"
												variant="ghost"
												onClick={() => void removeCapacity(item.id)}
											>
												<Trash2 />
											</Button>
										</div>
							...
							<Card>
								<CardHeader>
									<CardTitle className="flex items-center gap-2">
										<UsersRound /> {t("Assignment load")}
									</CardTitle>
								</CardHeader>
								<CardContent className="space-y-3">
									{(summary?.assignments ?? []).length === 0 ? (
										<SupportEmpty
											title={t("No active assignments")}
											description={t(
...
function NativeSelect({
	...
	return (
		...
		>
			{optionsWithLabels.length ? <option value="">{t("Select")}</option> : null}
			{options.map((option) => (
...
function RoutingList({ title, items }: { title: string; items: SupportMembership[] }) {
	const { t } = useSupportI18n();
	...
	return (
		<Card>
			...
			<CardContent className="space-y-2">
				{items.length === 0 ? (
					<p className="text-sm text-muted-foreground">{t("No linked members.")}</p>
				) : (
					items.map((item) => (
