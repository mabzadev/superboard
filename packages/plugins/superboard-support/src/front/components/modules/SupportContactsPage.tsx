...
export default function SupportContactsPage({ embedded = false }: { embedded?: boolean } = {}) {
	...
	const [conversations, setConversations] = useState<SupportContactDetail["conversations"]>([]);
	...
	const load = useCallback(
		async (search: string) => {
			...
			try {
				...
				showErrorNotification(error instanceof Error ? error.message : "Unable to load contacts");
			} finally {
				setBusy(false);
			}
		},
	...
	useEffect(() => {
		...
		void Promise.all([
			...
			.catch((error) =>
				showErrorNotification(
					error instanceof Error ? error.message : "Unable to load contact details",
				),
			);
	...
	const save = async () => {
		...
		try {
			...
			if (selectedId) await updateSupportContact(projectId, selectedId, payload);
			else await createSupportContact(projectId, payload);
			showSuccessNotification("Contact saved");
			await load(query);
			startNew();
		} catch (error) {
			showErrorNotification(error instanceof Error ? error.message : "Unable to save contact");
		} finally {
			setBusy(false);
		}
	};
	...
	const addNote = async () => {
		...
		try {
			...
			setNote("");
			setNotes((await getSupportContactNotes(projectId, selectedId)).data || []);
		} catch (error) {
			showErrorNotification(error instanceof Error ? error.message : "Unable to add note");
		}
	};
	...
	const removeNote = async (noteId: string) => {
		...
		try {
			...
			showErrorNotification(error instanceof Error ? error.message : "Unable to delete note");
		}
	};
	...
	const addCompany = async () => {
		...
		try {
			...
			showErrorNotification(error instanceof Error ? error.message : "Unable to create company");
		} finally {
			setBusy(false);
		}
	};
		...
		<div className="min-h-full">
			<div className="space-y-5 p-6">
				<div className="flex flex-wrap items-end justify-between gap-3">
					<div>
						<Heading className="text-2xl font-semibold">{t("Contacts")}</Heading>
						<p className="text-sm text-muted-foreground">
							Customer profiles, companies, custom attributes, conversation history, and private
							notes.
						</p>
					</div>
				...
				<div className="flex gap-2">
					...
					<Button variant="outline" disabled={busy} onClick={() => void load(query)}>
				...
				<div className="grid gap-5 xl:grid-cols-[360px_1fr]">
						...
						{contacts.map((contact) => (
							...
							>
								<div className="flex items-center justify-between gap-2">
									...
									{contact.blocked && <Badge variant="destructive">Blocked</Badge>}
								</div>
					...
					<div className="space-y-5">
						<Card className="space-y-4 p-5">
							...
							<div className="grid gap-4 md:grid-cols-2">
								<Field
									...
									onChange={(value) => setDraft({ ...draft, external_user_id: value })}
								/>
								<Field
									label="Name"
									value={draft.name}
								...
								<div className="space-y-2">
									...
									<select
										...
										onChange={(event) => setDraft({ ...draft, company_id: event.target.value })}
									>
								...
								<div className="flex items-center justify-between rounded-md border px-3">
									...
									<Switch
										...
										onCheckedChange={(blocked) => setDraft({ ...draft, blocked })}
									/>
								</div>
							</div>
										...
										<div className="grid gap-2 md:grid-cols-[1fr_1fr_auto]" key={attribute.id}>
											<Input
												...
												onChange={(event) =>
													setDraft({
														...draft,
														custom_attributes: draft.custom_attributes.map((item) =>
						...
						{selectedId && (
							<Card className="space-y-4 p-5">
								<div>
									<h2 className="text-lg font-semibold">Private contact notes</h2>
									<p className="text-sm text-muted-foreground">
									...
									{notes.map((item) => (
										...
										>
											<div>
												...
												<p className="mt-1 text-xs text-muted-foreground">
													{item.created_by} · {new Date(item.created_at).toLocaleString("en")}
												</p>
											</div>
											<Button
												size="icon"
											...
											>
												<div>
													...
													<p className="text-xs text-muted-foreground">
														...
														{new Date(conversation.updated_at).toLocaleString("en")}
													</p>
												</div>
				...
				<Card className="space-y-4 p-5">
					<div>
						...
						<p className="text-sm text-muted-foreground">
							Create reusable company records and associate contacts through the selector above.
						</p>
					</div>
					<div className="grid gap-2 md:grid-cols-[1fr_1fr_auto]">
						...
						/>
						<Button disabled={busy || !companyName.trim()} onClick={() => void addCompany()}>
							<Plus className="mr-2 h-4 w-4" />
							Create company
						</Button>
					</div>
					<div className="grid gap-2 md:grid-cols-2 xl:grid-cols-3">
						{companies.map((company) => (
							<div key={company.id} className="rounded-md border p-3">
								...
								<p className="text-xs text-muted-foreground">
									{company.domain || "No domain"} · {company.contact_count || 0} contacts
								</p>
							</div>
						))}
					</div>
