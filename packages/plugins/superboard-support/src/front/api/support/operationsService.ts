...
import { getSupportAction } from "./nativeClient.js";
...
export const getSupportContacts = async (
	...
	(await GET(path(projectId, `/contacts${query ? `?q=${encodeURIComponent(query)}` : ""}`))).data;
export const getSupportContact = async (
	projectId: string,
	contactId: string,
): Promise<{ data: SupportContactDetail }> =>
	(await GET(path(projectId, `/contacts/${encodeURIComponent(contactId)}`))).data;
export const createSupportContact = async (projectId: string, contact: Record<string, unknown>) =>
...
export const updateSupportContact = async (
	...
	(
		await PATCH(path(projectId, `/contacts/${encodeURIComponent(contactId)}`), contact, {
			retry: false,
		})
	).data;
export const getSupportContactNotes = async (
	...
	(await GET(path(projectId, `/contacts/${encodeURIComponent(contactId)}/notes`))).data;
...
export const getSupportCompanies = async (projectId: string): Promise<{ data: SupportCompany[] }> =>
	(await GET(path(projectId, "/companies"))).data;
export const createSupportCompany = async (projectId: string, company: Record<string, unknown>) =>
	(await POST(path(projectId, "/companies"), company, { retry: false })).data;
export const getSupportCsat = (projectId: string, inboxId?: string) =>
	getSupportAction<SupportCsat[]>(projectId, "csat", { inbox_id: inboxId });

export const getSupportQuality = (projectId: string, inboxId?: string) =>
	getSupportAction<SupportQuality>(projectId, "quality", { inbox_id: inboxId });

export const getSupportAudit = (projectId: string, inboxId?: string) =>
	getSupportAction<SupportAuditEvent[]>(projectId, "audit", { inbox_id: inboxId });

export const getSupportNotifications = async (
	projectId: string,
	unread = false,
): Promise<{ data: SupportNotification[] }> =>
	(await GET(path(projectId, `/notifications${unread ? "?unread=true" : ""}`))).data;
export const markSupportNotificationRead = async (projectId: string, notificationId: string) =>
	(await PATCH(path(projectId, `/notifications/${encodeURIComponent(notificationId)}/read`), {}))
		.data;
...
export const updateSupportNotificationPreferences = async (
	projectId: string,
	preferences: Omit<SupportNotificationPreferences, "project_id" | "membership_id" | "updated_at">,
) =>
	(
...
export const markAllSupportNotificationsRead = async (projectId: string) =>
	(
		await POST(
			path(projectId, "/notifications/read-all"),
			{},
			{
				retry: false,
			},
...
export const snoozeSupportNotification = async (
	...
	(
		await PATCH(
			path(projectId, `/notifications/${encodeURIComponent(notificationId)}/snooze`),
			{ snoozed_until: snoozedUntil },
			{ retry: false },
		)
	).data;
export const deleteSupportNotification = async (projectId: string, notificationId: string) =>
	(
		await DELETE(path(projectId, `/notifications/${encodeURIComponent(notificationId)}`), {
			retry: false,
		})
	).data;
export const saveSupportDraft = async (
	...
	(
		await PUT(path(projectId, `/conversations/${encodeURIComponent(conversationId)}/draft`), {
			content,
			attachments,
		})
	).data;
export const getSupportDraft = async (
	...
	(await GET(path(projectId, `/conversations/${encodeURIComponent(conversationId)}/draft`))).data;
...
export const getMessagingCsat = (projectId: string, inboxId?: string) =>
	getSupportCsat(projectId, inboxId);
