...
export async function listSupportResource<T>(
	...
	if (query.inbox_id) search.set("inbox_id", query.inbox_id);
	if (query.portal_id) search.set("portal_id", query.portal_id);
	if (query.cursor) search.set("cursor", query.cursor);
	if (query.q?.trim()) search.set("q", query.q.trim());
	search.set("limit", String(Math.min(100, Math.max(1, query.limit ?? 50))));
	return (await GET(`${supportProjectPath(projectRef, resource)}?${search.toString()}`)).data;
}
...
export async function getSupportResource<T>(
	...
	return (await GET(`${supportProjectPath(projectRef, resource)}/${encodeURIComponent(id)}`)).data;
}
...
export async function updateSupportResource<T, TInput>(
	...
	return (
		await PATCH(`${supportProjectPath(projectRef, resource)}/${encodeURIComponent(id)}`, input, {
			retry: false,
		})
	).data;
}
...
export async function deleteSupportResource(
	...
	return (
		await DELETE(`${supportProjectPath(projectRef, resource)}/${encodeURIComponent(id)}`, {
			retry: false,
		})
	).data;
}
...
export async function deleteSupportAction<T>(
	...
	return (await DELETE(supportProjectPath(projectRef, resource), { retry: false })).data;
}
...
export async function getSupportAction<T>(
	...
	return (await GET(`${supportProjectPath(projectRef, resource)}${suffix}`)).data;
}
