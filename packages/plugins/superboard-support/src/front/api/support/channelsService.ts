...
export const listSupportProviders = (projectRef: string, query?: SupportCursorQuery) =>
	listSupportResource<SupportProviderEndpoint>(projectRef, "providers", query);

export const createSupportProvider = (
	projectRef: string,
	input: Pick<SupportProviderEndpoint, "provider" | "display_name" | "inbox_id"> & {
		status?: SupportProviderStatus;
		settings?: Record<string, unknown>;
	},
) => createSupportResource<SupportProviderEndpoint, typeof input>(projectRef, "providers", input);

export const updateSupportProvider = (
	projectRef: string,
	id: string,
	input: Partial<
		Pick<SupportProviderEndpoint, "display_name" | "inbox_id" | "status" | "settings">
	>,
) =>
	updateSupportResource<SupportProviderEndpoint, typeof input>(projectRef, "providers", id, input);
...
export const getSupportProviderCredentialStatus = (projectRef: string, id: string) =>
	getSupportAction<{
		endpoint_id: string;
		configured: boolean;
