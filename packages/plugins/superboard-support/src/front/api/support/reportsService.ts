...
export type SupportReportExport = {
	...
	filters: { from?: string; to?: string; inbox_id?: string };
	result: { count?: number };
...
export const getSupportReports = (
	projectRef: string,
	period: { from?: string; to?: string; inbox_id?: string } = {},
...
export const exportSupportReports = (
	...
	postSupportAction<{ id: string; status: "queued" }>(projectRef, "reports/exports", { filters });

export const listSupportReportExports = (projectRef: string, inboxId?: string) =>
...
export const downloadSupportReportExport = async (projectRef: string, id: string) =>
	(
		await GET(
