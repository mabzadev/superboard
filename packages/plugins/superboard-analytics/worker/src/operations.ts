...
import type { AnalyticsOperationPayload, AnalyticsOperationType, Env } from "./types";
...
export async function createAnalyticsOperation(c: AnalyticsContext) {
	...
	const input = await normalizedOperationInput(c.env, projectId, body.type, body.input);
	...
	const data = {
		...
		status: "queued",
		input,
		requested_by: project.operatorId ?? String(project.actorId),
		created_at: now,
	};
...
export async function getAnalyticsOperation(c: AnalyticsContext, id: string) {
	const row = await c.env.DB.prepare(
     ...
     FROM analytics_operation_jobs WHERE project_id = ? AND id = ?`,
		...
		.first<OperationRow>();
	if (!row) {
		throw httpError("analytics_operation_not_found", "Operation not found", 404);
	}
...
export async function resumeQueuedOperations(env: Env, maximum = 10): Promise<number> {
...
async function startOperation(env: Env, payload: AnalyticsOperationPayload): Promise<void> {
	try {
...
> {
	async run(event: WorkflowEvent<AnalyticsOperationPayload>, step: WorkflowStep): Promise<unknown> {
		...
		try {
			const resultJson = await step.do(
				...
				async () => stableAnalyticsJson(await executeOperation(this.env, payload)),
			);
...
async function exportAnalytics(
	...
	const from = typeof input.from === "string" ? input.from : "1970-01-01T00:00:00.000Z";
...
async function replayDeadLetters(
	...
	const requestedId = typeof input.dead_letter_id === "string" ? input.dead_letter_id : null;
	const rows = await env.DB.prepare(
...
async function rebuildRollups(db: D1Database, projectId: string): Promise<Record<string, unknown>> {
	const result = await db.batch([
		db.prepare("DELETE FROM analytics_daily_metrics WHERE project_id = ?").bind(projectId),
		db
			.prepare(
...
async function eraseSubject(
	...
	const hashes = Array.isArray(input.subject_hashes)
		? input.subject_hashes.filter((value): value is string => typeof value === "string")
		: [];
...
export async function eraseAnalyticsSubject(
	...
	const subjectHashes = await identityHashCandidates(env, projectId, kind, subjectId);
...
async function normalizedOperationInput(
	...
	if (!(["user", "anonymous", "app_instance"] as const).includes(kind as "user")) {
		throw httpError(
...
function operationBody(value: unknown): {
	...
	if (!value || typeof value !== "object" || Array.isArray(value)) {
		throw httpError("analytics_operation_invalid", "Operation body must be an object", 400);
	}
	const raw = value as Record<string, unknown>;
	const type = String(raw.operation_type ?? "") as AnalyticsOperationType;
	if (!["export", "replay", "rebuild_rollups", "erase_subject"].includes(type)) {
		throw httpError("analytics_operation_type_invalid", "Operation type is invalid", 400);
	}
	const input = raw.input;
	if (input !== undefined && (!input || typeof input !== "object" || Array.isArray(input))) {
		throw httpError("analytics_operation_input_invalid", "Operation input must be an object", 400);
	}
