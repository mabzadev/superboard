import { AnalyticsContractError, parseAnalyticsEventsV1 } from "@superboard/contracts/analytics";
...
import { drainAnalyticsOutbox, handleAnalyticsQueue, ingestAnalyticsEvents } from "./ingestion";
...
app.get("/internal/v1/events/analyze", async (c) => c.json({ data: await analyzeEvent(c) }));

app.get("/internal/v1/installations", async (c) =>
	c.json({
		data: await listInstallations(c.env.DB, String(c.get("project").projectId), new URL(c.req.url)),
	}),
);

app.get("/internal/v1/purchases", async (c) =>
	c.json({
		data: await listPurchaseFacts(c.env.DB, String(c.get("project").projectId), new URL(c.req.url)),
	}),
);
...
app.get("/internal/v1/sessions", async (c) => {
	...
	const bindings: unknown[] = [String(c.get("project").projectId), query.from, query.to];
...
app.get("/internal/v1/applications", async (c) => c.json({ data: await listApplications(c) }));
app.put("/internal/v1/applications/:id", async (c) =>
	mutationResponse(await updateApplication(c, c.req.param("id"))),
);

app.get("/internal/v1/dashboards", async (c) => c.json({ data: await listDashboards(c) }));
app.post("/internal/v1/dashboards", async (c) => mutationResponse(await createDashboard(c)));
...
app.delete("/internal/v1/dashboards/:id/widgets/:widgetId", async (c) =>
	mutationResponse(await deleteDashboardWidget(c, c.req.param("id"), c.req.param("widgetId"))),
);

app.get("/internal/v1/views", async (c) => c.json({ data: await listViews(c) }));
app.get("/internal/v1/dimensions", async (c) => c.json({ data: await listDimensions(c) }));
app.get("/internal/v1/crashes", async (c) => c.json({ data: await listCrashGroups(c) }));
...
app.get("/internal/v1/feedback", async (c) => c.json({ data: await listFeedback(c) }));

app.get("/internal/v1/remote-config", async (c) => c.json({ data: await listRemoteConfig(c) }));
...
app.get("/internal/v1/cohorts", async (c) => c.json({ data: await listCohorts(c) }));
app.post("/internal/v1/cohorts", async (c) => mutationResponse(await saveCohort(c)));
...
app.get("/internal/v1/alerts", async (c) => c.json({ data: await listAlerts(c) }));
app.post("/internal/v1/alerts", async (c) => mutationResponse(await saveAlert(c)));
...
app.post("/internal/v1/alert-incidents/:id/acknowledge", async (c) =>
	mutationResponse(await transitionAlertIncident(c, c.req.param("id"), "acknowledged")),
);
app.post("/internal/v1/alert-incidents/:id/resolve", async (c) =>
	mutationResponse(await transitionAlertIncident(c, c.req.param("id"), "resolved")),
);

app.get("/internal/v1/hooks", async (c) => c.json({ data: await listHooks(c) }));
app.post("/internal/v1/hooks", async (c) => mutationResponse(await saveHook(c)));
...
app.get("/internal/v1/annotations", async (c) => c.json({ data: await listAnnotations(c) }));
app.post("/internal/v1/annotations", async (c) => mutationResponse(await saveAnnotation(c)));
...
app.get("/internal/v1/settings", async (c) => c.json({ data: await getAnalyticsSettings(c) }));
app.put("/internal/v1/settings", async (c) => mutationResponse(await updateAnalyticsSettings(c)));
...
app.get("/internal/v1/retention", async (c) =>
	c.json({
		data: await queryRetention(c.env.DB, String(c.get("project").projectId), new URL(c.req.url)),
	}),
);

app.get("/internal/v1/reports", async (c) => c.json({ data: await listSavedReports(c) }));
...
app.get("/internal/v1/operations", async (c) => c.json({ data: await listAnalyticsOperations(c) }));
	...
	c.json(
		{
			error: {
				...
				retryable: false,
				request_id:
					c.get("project")?.requestId ?? c.req.header("x-request-id") ?? crypto.randomUUID(),
			},
		},
		404,
	),
...
function mutationResponse(result: { data: unknown; status: number; replayed: boolean }): Response {
...
export default withPluginTaskLifecycle("supbrd-plugmod-analytics", lifecycleWorker);
