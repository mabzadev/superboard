...
const RedirectPreviewModal = dynamic(
	() => import("../components/dynamic_links/links/RedirectPreviewModal.js"),
	{ ssr: false, loading: () => null },
);
import ActionConfirm from "@superboard/front-ui/common/action-confirm.js";
import DeleteConfirm from "@superboard/front-ui/common/delete-confirm.js";
import RedirectRulesGateDialog from "@superboard/front-ui/common/redirect-rules-gate-dialog.js";
import { NO_CHECK, PARTIAL_CHECK } from "@superboard/front-ui/constants/OptionsConstants.js";
...
import { mapKeyPairValues } from "@superboard/front-ui/lib/utils.js";
import type { Link } from "@superboard/front-ui/types";
import { useQueryClient } from "@tanstack/react-query";
import { clsx } from "clsx";

import { trackEvent, EVENTS } from "../analytics/index.js";
import { getLinksByIdAPICall } from "../api/links/linksService.js";
import CreateLinkCreatedSuccessfully from "../components/dynamic_links/links/create_link/CreateLinkCreatedSuccessfully.js";
import LinkDialogContent from "../components/dynamic_links/links/create_link/LinkDialogContent.js";
import {
	useCreateLinkMutation,
	useUpdateLinkMutation,
	useRemoveLinkMutation,
} from "../hooks/mutations/useLinksMutations.js";
import { useRedirectConfigQuery } from "../hooks/queries/useConfigurationQueries.js";
import { useRandomPathQuery, usePathAvailableQuery } from "../hooks/queries/useLinksQueries.js";
import { useCreateLinkForm } from "../hooks/useCreateLinkForm.js";
import {
	hasEditChanges,
	disableEditButton,
	getEditFirstErrorSection,
	type LinkEditFormValues,
} from "../hooks/useLinkEditValidation.js";
import { resolveRedirects } from "../hooks/useResolvedRedirects.js";
import { buildLinkFormData } from "../lib/buildLinkFormData.js";
...
const LinkDialogContext = createContext<LinkDialogContextType | undefined>(undefined);
...
const LinkDialogProvider = ({ children }: { children: ReactNode }) => {
	...
	const createLinkMutation = useCreateLinkMutation(projectId, selectedInstance?.id);
	...
	const [initialKeyPair, setInitialKeyPair] = useState<{ key: string; value: string }[]>([]);
	...
	const handleSaveClick = () => {
		if (!selectedLink || disableEditButton(editFormValues, selectedLink, initialKeyPair)) {
			if (selectedLink) form.setSection(getEditFirstErrorSection(editFormValues, selectedLink));
			setShowErrors(true);
		} else {
			handleSave();
		}
	...
	}, [pathAvailable, debouncedPath, mode, selectedLink?.path, formPath, setPathAvailable]);
	...
	return (
		<LinkDialogContext.Provider value={contextValue}>
			{children}
			<RedirectRulesGateDialog open={redirectGateOpen} onOpenChange={setRedirectGateOpen} />
			<Dialog
				open={isOpen}
				onOpenChange={(open) => {
					if (!open) handleClose();
						...
						<>
							<DialogHeader>
								<div className="flex items-center gap-4 p-4 w-full">
									...
									<button className="ml-auto" onClick={handleClose} aria-label="Close dialog">
										<X />
									</button>
								</div>
							</DialogHeader>
					...
					{mode === "create" && !isLinkGenerated && (
						<>
							...
							<div className="flex items-center justify-between px-4 py-3 shrink-0">
								...
								<Button onClick={handleCreateClick} disabled={isBusy} aria-busy={isBusy}>
									<Plus />
					...
					{mode === "edit" && selectedLink?.active && (
						<>
							...
							<div className="flex items-center justify-between px-4 py-3 shrink-0">
								...
								{hasChangesNow && (
									<Button onClick={handleSaveClick} disabled={isBusy} aria-busy={isBusy}>
										<Save />
...
export const useGlobalLinkDialog = () => {
	...
	if (!context) throw new Error("useLinkDialog must be used within LinkDialogProvider");
	return context;
};
