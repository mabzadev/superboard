...
import Image, { type StaticImageData } from "@superboard/front-ui/next-image";
...
> = {
	...
	[LINKEDIN]: {
		...
		guideUrl: "https://business.linkedin.com/marketing-solutions/how-to-advertise-on-linkedin",
...
const CreateLinkCreatedSuccessfully = ({
	...
	const platform =
		(createdLink?.ads_platform ? PLATFORM_INFO[createdLink.ads_platform] : undefined) ??
		PLATFORM_INFO[QUICK_LINK]!;
		...
		<div className="flex flex-col w-full">
			...
			>
				<button
					...
					className="absolute top-4 end-4 rounded-lg p-1.5 text-muted-foreground hover:text-foreground hover:bg-accent transition-colors"
				>
					<X className="h-4 w-4" />
				</button>
				...
				<h2 className="text-xl font-semibold tracking-tight mb-1">{createdLink?.name}</h2>
				...
				<div className="flex items-center w-full rounded-md border border-sidebar-border bg-background/60 dark:bg-background/40 backdrop-blur-sm px-4 py-3 gap-3 mt-5">
					...
					>
						{copied ? <Check className="h-4 w-4 text-valid-green" /> : <Copy className="h-4 w-4" />}
					</button>
				</div>
			</div>
				...
				{isAdPlatform ? (
					<div className="flex flex-col gap-3">
						<div className="flex items-center gap-2.5">
							<Image
								src={resolvedTheme === "dark" ? platform.iconDark : platform.icon}
								alt={platform.label}
								width={18}
								height={18}
					...
					<div className="flex flex-col gap-2">
						...
						<span className="text-xs text-muted-foreground leading-relaxed">{platform.hint}</span>
					</div>
				)}
			</div>
			...
			<div className="flex items-center justify-between px-6 py-4">
				<Button variant="outline" asChild>
					<a href={createdLink?.access_path} target="_blank" rel="noopener noreferrer">
