import { DEFAULT, REDIRECT_WEB } from "@superboard/front-ui/constants/OptionsConstants.js";
...
import { AlertCircle, Check, ChevronDown, CopySlash, Earth, Monitor } from "lucide-react";
...
const CreateLinkComputersRedirect = React.memo(function CreateLinkComputersRedirect({
	desktopRedirectType,
	...
	setDesktopRedirectURL: React.Dispatch<React.SetStateAction<RedirectURL | null>>;
	...
	const hasError = desktopRedirectType !== DEFAULT && url.length > 0 && (!urlIsValid || !isHttps);
	const showUrlValid = desktopRedirectType !== DEFAULT && urlIsValid && isHttps;
	const showEmptyError = showErrors && desktopRedirectType !== DEFAULT && url.length === 0;
	const errorMessage =
	...
	return (
		<div className="flex flex-col gap-3 max-w-[460px]">
			...
			<OptionDropdown
				...
				onChange={(value) => !disabledActions && setDesktopRedirectType(value)}
