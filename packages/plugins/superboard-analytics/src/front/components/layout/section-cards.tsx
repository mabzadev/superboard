...
function formatMainValue(card: DashBoardCardType) {
	if (typeof card.value === "string" && !Number.isFinite(Number(card.value))) return card.value;
...
function CardContent({ card }: { card: DashBoardCardType }) {
	...
	return (
		<>
			...
			{hasChange && <p className="mt-2 text-xs text-muted-foreground/70">vs. previous period</p>}
		</>
	);
}
...
function SortableCard({ id, card }: { id: string; card: DashBoardCardType }) {
	...
	return (
		<div
			...
			style={style}
			className={cn(
				"group relative rounded-md border border-sidebar-border bg-card p-5 transition-colors hover:border-border",
...
function StaticCard({ card }: { card: DashBoardCardType }) {
	return (
		<div className="relative rounded-md border border-sidebar-border bg-card p-5 transition-colors hover:border-border">
			<CardContent card={card} />
		</div>
	);
}
...
function SortableCardGrid({
	...
	return (
		...
		>
			...
			<DragOverlay>
				{activeCard ? (
					<div className="rounded-md border border-sidebar-border bg-card p-5 shadow-lg scale-[1.02]">
...
export function SectionCards({ cards, cardKeys, onReorder }: SectionCardsProps) {
	...
	return <SortableCardGrid cards={cards} cardKeys={cardKeys} onReorder={onReorder} />;
}
