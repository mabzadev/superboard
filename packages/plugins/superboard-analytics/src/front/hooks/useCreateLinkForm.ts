...
import { deepClone } from "@superboard/front-ui/lib/utils.js";
import { mapKeyPairValues } from "@superboard/front-ui/lib/utils.js";
import { isValidHttpsUrl } from "@superboard/front-ui/lib/validation.js";
...
import { createAddNewLinkDataTableColumns } from "../components/dynamic_links/links/create_link/CreateLinkDataTableColumns.js";
import { getRedirectType } from "../lib/redirectsHelpers.js";
import type { CreateLinkFormValues } from "../schemas/link.js";
...
export function useCreateLinkForm() {
	const form: UseFormReturn<CreateLinkFormValues> = useForm<CreateLinkFormValues>({
		defaultValues,
		mode: "onChange",
	});
	...
	const setPathAvailable = useCallback(
		(v: SetStateAction<boolean>) =>
			form.setValue("pathAvailable", resolveAction(v, form.getValues("pathAvailable"))),
		[form],
	);
	const setSocialMediaTitle = useCallback(
		(v: SetStateAction<string>) =>
			form.setValue("socialMediaTitle", resolveAction(v, form.getValues("socialMediaTitle")), {
				shouldDirty: true,
			}),
		[form],
	);
	...
	const setImageFile = useCallback(
		(v: SetStateAction<File | undefined>) =>
			form.setValue("imageFile", resolveAction(v, form.getValues("imageFile")), {
				shouldDirty: true,
			}),
		[form],
	);
	const setImageLink = useCallback(
		(v: SetStateAction<string>) =>
			form.setValue("imageLink", resolveAction(v, form.getValues("imageLink")), {
				shouldDirty: true,
			}),
		[form],
	);
	const setImageType = useCallback(
		(v: SetStateAction<string>) =>
			form.setValue("imageType", resolveAction(v, form.getValues("imageType")), {
				shouldDirty: true,
			}),
		[form],
	);
	const setImagePreview = useCallback(
		(v: SetStateAction<string | null>) =>
			form.setValue("imagePreview", resolveAction(v, form.getValues("imagePreview")), {
				shouldDirty: true,
			}),
		[form],
	);
	...
	const setKeyValuePair = useCallback(
		(v: SetStateAction<{ key: string; value: string }[]>) =>
			form.setValue("keyValuePair", resolveAction(v, form.getValues("keyValuePair")), {
				shouldDirty: true,
			}),
		[form],
	);
	...
	const setiOSRedirectURL = useCallback(
		(v: SetStateAction<CreateLinkFormValues["iOSRedirectURL"]>) =>
			form.setValue("iOSRedirectURL", resolveAction(v, form.getValues("iOSRedirectURL")), {
				shouldDirty: true,
			}),
		[form],
	);
	const setAndroidRedirectURL = useCallback(
		(v: SetStateAction<CreateLinkFormValues["androidRedirectURL"]>) =>
			form.setValue("androidRedirectURL", resolveAction(v, form.getValues("androidRedirectURL")), {
				shouldDirty: true,
			}),
		[form],
	);
	const setDesktopRedirectURL = useCallback(
		(v: SetStateAction<CreateLinkFormValues["desktopRedirectURL"]>) =>
			form.setValue("desktopRedirectURL", resolveAction(v, form.getValues("desktopRedirectURL")), {
				shouldDirty: true,
			}),
		[form],
	);
	const setUtmCampaign = useCallback(
		(v: SetStateAction<string>) =>
			form.setValue("utmCampaign", resolveAction(v, form.getValues("utmCampaign")), {
				shouldDirty: true,
			}),
		[form],
	);
	const setUtmMedium = useCallback(
		(v: SetStateAction<string>) =>
			form.setValue("utmMedium", resolveAction(v, form.getValues("utmMedium")), {
				shouldDirty: true,
			}),
		[form],
	);
	const setUtmSource = useCallback(
		(v: SetStateAction<string>) =>
			form.setValue("utmSource", resolveAction(v, form.getValues("utmSource")), {
				shouldDirty: true,
			}),
		[form],
	);
	...
	const setiOSRedirectType = useCallback(
		(v: SetStateAction<string>) =>
			form.setValue("iOSRedirectType", resolveAction(v, form.getValues("iOSRedirectType")), {
				shouldDirty: true,
			}),
		[form],
	);
	...
	const setShowPreviewAndroid = useCallback(
		(v: SetStateAction<boolean | null>) =>
			form.setValue("showPreviewAndroid", resolveAction(v, form.getValues("showPreviewAndroid")), {
				shouldDirty: true,
			}),
		[form],
	);
	const setShowPreviewIOS = useCallback(
		(v: SetStateAction<boolean | null>) =>
			form.setValue("showPreviewIOS", resolveAction(v, form.getValues("showPreviewIOS")), {
				shouldDirty: true,
			}),
		[form],
	);
	...
	const socialMediaCheck = () => {
		if (imageType === LINK) {
			if (socialMediaTitle !== "" || socialMediaSubTitle !== "" || imageLink !== "") {
				return FULL_CHECK;
			} else {
	...
	const redirectsCheck = () => {
		...
		const iosUrlValid =
			iOSRedirectType === DEFAULT || (!!iOSRedirectURL && isValidHttpsUrl(iOSRedirectURL?.url));
	...
	const sections = [
		...
		},
		{
			...
			value: "tracking",
			checked:
				utmCampaign !== "" && utmMedium !== "" && utmSource !== "" ? FULL_CHECK : PARTIAL_CHECK,
		},
	];
	...
	const addKeyValuePair = useCallback(
		(newKey: string, newValue: string) => {
			...
			form.setValue("keyValuePair", [...current, { key: newKey, value: newValue }], {
				shouldDirty: true,
			});
		},
		[form],
