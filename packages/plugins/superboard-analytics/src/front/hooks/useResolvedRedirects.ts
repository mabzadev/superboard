import { DEFAULT } from "@superboard/front-ui/constants/OptionsConstants.js";
import type { RedirectURL, RedirectConfig } from "@superboard/front-ui/types";
...
export function resolveRedirects(
	...
	const resolveIOS = (): ResolvedPlatformRedirect => {
		if (link.iosRedirectType !== DEFAULT && link.iosRedirectURL) {
			const isAppOrFallback = link.iosRedirectURL.open_app_if_installed === true;
			return {
				platform: "ios",
				source: "custom",
				type: isAppOrFallback ? "app_or_fallback" : "redirect_web",
				url: link.iosRedirectURL.url ?? "",
				showPreview: link.showPreviewIOS ?? projectConfig?.show_preview_ios ?? false,
	...
	const resolveAndroid = (): ResolvedPlatformRedirect => {
		if (link.androidRedirectType !== DEFAULT && link.androidRedirectURL) {
			const isAppOrFallback = link.androidRedirectURL.open_app_if_installed === true;
			return {
				platform: "android",
				source: "custom",
				type: isAppOrFallback ? "app_or_fallback" : "redirect_web",
				url: link.androidRedirectURL.url ?? "",
				showPreview: link.showPreviewAndroid ?? projectConfig?.show_preview_android ?? false,
				appStoreRedirect: false,
			};
		}
		...
		if (androidConfig?.enabled) {
			return {
				...
				source: "default",
				type: "app_or_fallback",
				url: androidConfig.appstore ? "" : androidConfig.fallback_url || fallbackUrl,
				showPreview: link.showPreviewAndroid ?? projectConfig?.show_preview_android ?? false,
