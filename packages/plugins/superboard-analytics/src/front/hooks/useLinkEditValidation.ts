import { FILE, DEFAULT } from "@superboard/front-ui/constants/OptionsConstants.js";
import { deepEqual } from "@superboard/front-ui/lib/utils.js";
import { hasText, isValidHttpsUrl } from "@superboard/front-ui/lib/validation.js";
import type { Link, RedirectURL } from "@superboard/front-ui/types";
...
export function hasEditChanges(
	...
	return (
		...
		(hasText(form.socialMediaTitle) && (selectedLink.title ?? "") !== form.socialMediaTitle) ||
		...
		(selectedLink.show_preview_android ?? false) !== !!form.showPreviewAndroid ||
...
export function disableEditButton(
	...
	const isFileType = form.imageType === FILE;
	const imageInputValid = isFileType ? true : !hasText(form.imageLink) || isURL(form.imageLink!);

	const androidUrlValid =
		form.androidRedirectType === DEFAULT ||
		isValidHttpsUrl(getRedirectUrl(form.androidRedirectURL));
	const iosUrlValid =
		form.iOSRedirectType === DEFAULT || isValidHttpsUrl(getRedirectUrl(form.iOSRedirectURL));
	...
	const hasErrors = !imageInputValid || !redirectsValid || (pathChanged && !form.pathAvailable);

	return !hasEditChanges(form, selectedLink, initialKeyPair) || hasErrors;
}

export function getEditFirstErrorSection(form: LinkEditFormValues, selectedLink: Link): string {
	const pathChanged = (selectedLink.path ?? "") !== (form.path ?? "");
