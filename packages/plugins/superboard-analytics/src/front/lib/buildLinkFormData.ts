import { FILE, LINK } from "@superboard/front-ui/constants/OptionsConstants.js";
...
export function buildLinkFormData(params: BuildFormDataParams): FormData {
	...
	if (params.socialMediaTitle !== "") formData.append("title", params.socialMediaTitle);
	if (params.socialMediaSubTitle !== "") formData.append("subtitle", params.socialMediaSubTitle);
	...
	if (params.tagList.length > 0) formData.append("tags", JSON.stringify(params.tagList));

	if (params.iOSRedirectURL)
		formData.append("ios_custom_redirect", JSON.stringify(params.iOSRedirectURL));
	if (params.androidRedirectURL)
		formData.append("android_custom_redirect", JSON.stringify(params.androidRedirectURL));
	...
	if (params.desktopRedirectURL)
		formData.append("desktop_custom_redirect", JSON.stringify(params.desktopRedirectURL));
	...
	if (params.keyValuePair.length > 0) {
		...
		if (Object.keys(dict).length > 0) formData.append("data", JSON.stringify(dict));
	}
	...
	if (params.utmCampaign?.length > 0) formData.append("tracking_campaign", params.utmCampaign);
	if (params.utmMedium?.length > 0) formData.append("tracking_medium", params.utmMedium);
	if (params.utmSource?.length > 0) formData.append("tracking_source", params.utmSource);
