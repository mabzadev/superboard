...
import { RequestBodyError, readBytesLimited } from "@superboard/contracts/request-body";
...
type SupportSurface = "client" | "widget" | "help-center" | "oauth" | "realtime";

const PUBLIC_BODY_LIMITS: Record<Exclude<SupportSurface, "client" | "realtime">, number> = {
	widget: 1024 * 1024,
	"help-center": 64 * 1024,
	oauth: 64 * 1024,
};
...
export function isSupportWidgetPath(pathname: string): boolean {
	return pathname === "/api/v1/support-widget" || pathname.startsWith("/api/v1/support-widget/");
}
...
export async function proxySupportSurface(
	...
	try {
		...
		const headers = supportSurfaceHeaders(c.req.raw.headers, requestId, surface);
		let body: BodyInit | null = null;
		if (method !== "GET" && method !== "HEAD") {
			if (surface === "client") {
				body = c.req.raw.body;
			} else if (surface !== "realtime") {
				const bytes = await readBytesLimited(c.req.raw, PUBLIC_BODY_LIMITS[surface]);
...
export async function proxySupportProviderEvent(
	...
	if (!/^[a-z][a-z0-9_-]{0,63}$/u.test(provider)) {
		return domainError(requestId, 422, "provider_invalid", "Provider is invalid", {
			retryable: false,
		});
	}
	...
	try {
		...
		if (emailProvider && method === "POST") {
			...
			body = copiedArrayBuffer(new TextEncoder().encode(JSON.stringify(envelope)));
...
async function normalizeInboundEmail(
	...
	let value: unknown;
	try {
		const bytes = await readBytesLimited(response, EMAIL_INBOUND_LIMITS.networkBodyBytesMaximum);
	...
	if (!response.ok) {
		...
		throw new SupportEmailNormalizationError(
			...
			response.status === 413 ? "Provider event body is too large" : "Provider event is invalid",
			false,
		);
	}
...
function supportSurfaceHeaders(
	...
	if (surface === "client") {
		...
		if (project && !headers.has("x-superboard-project-id")) {
			headers.set("x-superboard-project-id", project);
		}
	}
...
function providerHeaders(source: Headers, requestId: string): Headers {
	const allowed = new Set(["content-type", "x-request-id", ...PROVIDER_SIGNATURE_HEADERS]);
	const headers = selectedHeaders(source, allowed);
...
function publicSupportResponse(response: Response, requestId: string): Response {
...
function canonicalProviderUrl(env: Env, request: Request): string {
	...
	const configured = new URL(domain.includes("://") ? domain : `https://${domain}`);
	if (
...
function canonicalEmailProvider(provider: string): "google" | "microsoft" | "smtp" | null {
...
function safeUpstreamCode(value: unknown): string {
	...
	return /^[a-z][a-z0-9_]{0,127}$/u.test(code) ? code : "provider_event_invalid";
}
