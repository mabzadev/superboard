...
export async function reconcileRefundDeadlines(env: BillingEnv, limit = 100, projectId?: string) {
    ...
    WHERE (? IS NULL OR refund.project_id = ?) AND deadline.status = 'open' AND datetime(deadline.due_at) <= datetime('now')
      AND refund.status NOT IN ('won','lost','closed')
    ORDER BY deadline.due_at ASC, deadline.id ASC
    LIMIT ?
  `,
	)
		.bind(projectId ?? null, projectId ?? null, boundedLimit)
		.all<{
			id: string;
			case_id: string;
	...
	return rows.reduce(
		(total, _deadline, index) =>
			total + Number((results[index * 2] as D1Result | undefined)?.meta?.changes || 0),
		0,
	);
}
...
export async function processAppleBillingNotification(
	...
	try {
		...
		if (result.purchase?.customerId) {
			const customer = await env.DB.prepare(
				...
				.bind(result.purchase.customerId, params.projectId)
				.first();
    ...
    `,
			...
			.bind(String(result.notification.notificationType || "UNKNOWN"), params.eventId)
			.run();
    ...
    `,
			...
			.bind(message.slice(0, 1000), params.eventId)
			.run();
		throw error;
	}
}
...
export async function processGoogleBillingNotification(
	...
	try {
		...
		if (verified.purchase.customerId) {
			const customer = await env.DB.prepare(
				...
				.bind(verified.purchase.customerId, params.projectId)
				.first();
    ...
    `,
			...
			.bind(params.eventType, params.eventId)
			.run();
...
export async function processGooglePendingRefundReview(
	env: BillingEnv,
	params: {
		eventId: string;
		projectId: string;
    ...
    `,
			...
			.bind(params.eventId, params.projectId, params.environment)
			.first<{ payload: string }>();
		if (!event?.payload || event.payload.length > 1_048_576)
			throw new Error("Google pending refund review payload is unavailable");
		let payload: Record<string, unknown>;
		try {
			payload = JSON.parse(event.payload) as Record<string, unknown>;
		} catch {
			throw new Error("Google pending refund review payload is invalid");
		}
...
async function recordWebhookDeliveryFailure(
	env: BillingEnv,
	params: {
		deliveryId: string;
		claimToken: string;
...
export async function deliverBillingWebhook(env: BillingEnv, deliveryId: string) {
  ...
  `,
	...
	try {
		...
		const secret =
			secretReference === "env:SUPERBOARD_ENTITLEMENT_WEBHOOK_SECRET"
				? env.SUPERBOARD_ENTITLEMENT_WEBHOOK_SECRET
		...
		try {
			response = await fetch(String(delivery.url), {
				...
				headers: {
					...
					"User-Agent": "SuperBoard-Purchases-Webhook/1.0",
					"X-SuperBoard-Delivery": deliveryId,
					"X-SuperBoard-Timestamp": timestamp,
					"X-SuperBoard-Signature": `v1=${signature}`,
				},
				body: payload,
				redirect: "manual",
		...
		if (!response.ok) {
			const retryable =
				...
				response.status === 429 ||
				response.status >= 500 ||
				responseDeclaresRetryable(responseBody);
    ...
    `,
			...
			.bind(attempt, response.status, responseBody, deliveryId, claimToken)
			.run();
...
export async function reconcileBillingState(env: BillingEnv, options: { projectId?: string } = {}) {
    ...
    WHERE (? IS NULL OR ce.project_id = ?) AND ce.source = 'purchase'
  ...
  `,
    ...
    WHERE (? IS NULL OR project_id = ?) AND status NOT IN ('expired', 'revoked', 'refunded')
  ...
  `,
    ...
    WHERE (? IS NULL OR project_id = ?) AND source = 'purchase'
  ...
  `,
	...
	const pendingDeliveries = await env.DB.prepare(
		`
    SELECT delivery.id FROM billing_webhook_deliveries delivery
    JOIN billing_webhook_endpoints endpoint ON endpoint.id = delivery.endpoint_id
    WHERE (? IS NULL OR endpoint.project_id = ?) AND delivery.status IN ('pending', 'failed')
      AND delivery.attempts < 10
      AND (delivery.next_attempt_at IS NULL OR datetime(delivery.next_attempt_at) <= datetime('now'))
    LIMIT 100
  `,
	)
		.bind(projectId, projectId)
    ...
    WHERE (? IS NULL OR s.project_id = ?) AND s.status NOT IN ('expired', 'revoked', 'refunded')
  ...
  `,
    ...
    WHERE (? IS NULL OR refund.project_id = ?) AND refund.status NOT IN ('won','lost','closed')
  ...
  `,
	...
	const refundDeadlinesMissed = await reconcileRefundDeadlines(env, 100, options.projectId);
    ...
    WHERE (? IS NULL OR connection.project_id = ?) AND connection.provider = 'google' AND connection.environment = 'production'
  ...
  `,
    ...
    WHERE (? IS NULL OR project_id = ?) AND status IN ('queued', 'running')
      AND (claim_token IS NULL OR claim_expires_at IS NULL OR datetime(claim_expires_at) <= datetime('now'))
  ...
  `,
	...
	try {
		...
		if (row.store === "apple") {
			...
			result = {
				store: "google",
				...(await applyVerifiedPurchase(env, verified.purchase)),
			};
		} else {
			throw Object.assign(
				new Error("Subscription cannot be reconciled without provider verification data"),
				{
					code: "subscription_reconciliation_data_missing",
					retryable: false,
				},
			);
		}
