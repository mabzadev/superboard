...
export async function authMiddleware(
	...
	if (c.env.CREDENTIAL_KEY_SCOPE === "billing") {
		if (c.req.header(PROJECT_CONTEXT_HEADERS.signature)) {
			const verified = await verifyInternalProjectContextRequest(
				c.req.raw,
				c.env.INTERNAL_API_TOKEN ?? "",
				"billing",
			);
			if (!verified.ok) return c.json({ error: "Invalid project context" }, 401);
			c.set("userId", verified.context.actorId);
			c.set("instanceId", verified.context.instanceId);
			c.set("internalProject", verified.context);
			if (verified.context.operatorId) c.set("operatorId", verified.context.operatorId);
			await next();
			return;
		}
		const actor = (c.req.header("X-SuperBoard-Internal-Actor") || "").trim();
	...
	if (auth.siteOperator) {
		c.set("siteOperator", auth.siteOperator);
		c.set("operatorId", auth.siteOperator.operator_id);
	}
...
function isServerSdkPath(pathname: string): boolean {
	return (
		/(?:^|\/)(generate_link|metrics_for_project)$/.test(pathname) ||
...
async function getOrCreateProjectForEnvironment(
	db: D1Database,
	instanceId: number,
	environment: string,
) {
	...
	const existing = await db
		...
		.bind(instanceId, isTest ? 1 : 0)
		.first<{ id: number }>();
	if (existing) return existing.id;

	const instance = await db
		.prepare("SELECT uri_scheme FROM instances WHERE id = ? LIMIT 1")
		.bind(instanceId)
	...
	const created = await db
		...
		.bind(isTest ? `${baseName} Test` : baseName, identifier, instanceId, isTest ? 1 : 0)
		.first<{ id: number }>();
	if (!created) throw new Error("Unable to create SDK project");
	await db
		.prepare("INSERT OR IGNORE INTO redirect_configs (project_id, default_fallback) VALUES (?, ?)")
		.bind(created.id, "")
		.run();
	return created.id;
}
...
async function resolveMobileProject(db: D1Database, projectKey: string) {
	...
	const instance = instanceKey
		? await db
				.prepare("SELECT id FROM instances WHERE api_key = ? LIMIT 1")
				.bind(instanceKey)
				.first<{ id: number }>()
		: null;
	...
	return db
		.prepare("SELECT id, instance_id FROM projects WHERE identifier = ? LIMIT 1")
		.bind(projectKey)
		.first<{ id: number; instance_id: number }>();
}

async function mobileProject(
	c: Context<{ Bindings: Env; Variables: AppVariables }>,
	projectKey: string,
) {
	...
	if (platform === "ios") {
      ...
      LIMIT 1
    `)
			.bind(project.instance_id, identifier)
			.first<{ id: number }>();
    ...
    `)
      ...
      LIMIT 1
    `)
			.bind(project.instance_id, identifier, identifier)
			.first<{ id: number }>();
...
export async function sdkMiddleware(
	c: Context<{ Bindings: Env; Variables: AppVariables }>,
	next: Next,
) {
	...
	const instance = await c.env.DB.prepare("SELECT id FROM instances WHERE api_key = ?")
		.bind(apiKey)
		.first<{ id: number }>();
