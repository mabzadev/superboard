...
async function cutoverDenied(c: any): Promise<Response | null> {
	const expected = c.env.SUPERBOARD_CUTOVER_TOKEN;
...
async function cutoverProject(db: D1Database, raw: string): Promise<ProjectRecord | null> {
	...
	return db
		...
		.bind(Number(match[1]), match[2] === "test" ? 1 : 0)
		.first<ProjectRecord>();
}
...
admin.get("/module-cutover/maintenance/:projectRef", async (c) => {
	...
	return c.json(
		...
		200,
		{ "X-Request-Id": id, "Cache-Control": "no-store" },
	);
});
...
admin.put("/module-cutover/maintenance/:projectRef", async (c) => {
	...
	if (body.enabled && (!windowId || !reason)) {
		return domainError(
			id,
			422,
			"maintenance_context_required",
			"window_id and reason are required when enabling maintenance",
		);
	}
    ...
    `).bind(
			crypto.randomUUID(),
			project.id,
			body.enabled ? "maintenance.enabled" : "maintenance.disabled",
			actor,
			JSON.stringify({ window_id: windowId || null, reason: reason || null, request_id: id }),
		),
	]);
	return c.json(
		{
...
admin.put("/module-cutover/flows-routing/:projectRef", async (c) => {
	...
	const replay = await c.env.DB.prepare(
     ...
     WHERE project_id = ? AND idempotency_key = ? LIMIT 1`,
		...
		.bind(project.id, idempotencyKey)
		.first<{
	...
	const maintenance = await c.env.DB.prepare(
     ...
     WHERE project_id = ? LIMIT 1`,
		...
		.bind(project.id)
		.first<{ enabled: number; window_id: string | null }>();
	if (Number(maintenance?.enabled ?? 0) !== 1 || maintenance?.window_id !== windowId) {
		return domainError(
			id,
			409,
	...
	await c.env.DB.batch([
       ...
       VALUES (?, ?, ?, ?)`,
		).bind(project.id, idempotencyKey, payloadChecksum, JSON.stringify(responseData)),
		c.env.DB.prepare(
       ...
       ON CONFLICT(project_id) DO UPDATE SET
		...
		).bind(project.id, body.enabled ? 1 : 0, windowId, planId, verificationChecksum, actor),
		c.env.DB.prepare(
...
async function domainForProject(
	...
	const created = await db
		...
		.bind(shortlinkDomain, projectId)
		.first<{ id: number }>();
...
admin.post("/migrate_firebase_links", async (c) => {
	...
	const contentType =
		typeof file === "object" && "type" in file ? file.type : c.req.header("content-type") || "";
	if (
		!file ||
		(typeof file === "object" && contentType !== "text/csv" && !contentType.includes("csv"))
	) {
	...
	for (const row of csvRows(text)) {
		...
		const deeplink = body.deeplink_prefix
			? originalDeeplink.replace(
					String(body.deeplink_prefix),
					uriSchemeFromPrefix(String(body.deeplink_prefix)),
				)
			: originalDeeplink;
		const path = stripPrefix(
			shortLink,
			body.short_link_prefix ? String(body.short_link_prefix) : undefined,
		);
		...
		const duplicate = await c.env.DB.prepare(
			"SELECT id FROM links WHERE domain_id = ? AND path = ? LIMIT 1",
		)
			.bind(domainId, path)
			.first();
		if (duplicate) {
...
admin.post("/billing/rewrap_credentials", async (c) => {
	...
	if (!/^[a-z][a-z0-9-]{1,31}$/.test(targetVersion))
		return c.json({ error: "Invalid target key version" }, 422);
	const sources = [
		{
			table: "ios_server_api_keys",
			rows: await c.env.DB.prepare(
				"SELECT id, encrypted_key FROM ios_server_api_keys WHERE encrypted_key IS NOT NULL",
			).all<{ id: string; encrypted_key: string }>(),
		},
		{
			table: "android_server_api_keys",
			rows: await c.env.DB.prepare(
				"SELECT id, encrypted_key FROM android_server_api_keys WHERE encrypted_key IS NOT NULL",
			).all<{ id: string; encrypted_key: string }>(),
		},
	] as const;
	...
	for (const source of sources) {
		...
		for (const row of source.rows.results || []) {
			...
			updates.push(
				c.env.DB.prepare(
					`UPDATE ${source.table} SET billing_encrypted_key = ?, updated_at = datetime('now') WHERE id = ?`,
				).bind(scoped, row.id),
				c.env.DB.prepare(`
