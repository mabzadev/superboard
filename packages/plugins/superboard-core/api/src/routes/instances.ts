...
const instances = new Hono<{ Bindings: Env; Variables: SiteAdapterVariables }>();
instances.use("*", siteOperatorAdapter("instances"));
...
async function buildInstance(
	...
	const role: { role: string } | null = siteRole
		? { role: siteRole }
		: await db
				.prepare("SELECT role FROM instance_roles WHERE user_id = ? AND instance_id = ?")
...
async function requireInstanceRole(c: any, instanceId: number, adminOnly = false) {
	const site = c.get("siteAuth");
	if (site?.siteOperator)
		return site.instanceId === instanceId
			? { userId: 0, role: site.siteOperator.role === 50 ? "owner" : "admin" }
			: { error: c.json({ error: "Forbidden" }, 403) };
	const userId = await getAuthUserId(c);
	if (userId === null) return { error: c.json({ error: "Unauthorized" }, 401) };
...
async function syncIosRpushApps(db: D1Database, iosConfigId: string | number) {
  ...
  `)
	...
	await db
		.prepare(`DELETE FROM rpush_apps WHERE name IN (?, ?)`)
		.bind(...names)
		.run();
...
async function syncAndroidRpushApp(db: D1Database, androidConfigId: string | number) {
  ...
  `)
		.bind(push.name, push.encrypted_fcm_server_key, push.firebase_project_id)
		.run();
}
...
async function deleteInstanceTree(env: Env, instanceId: number) {
	...
	const add = async (key: string, sql: string, binds: unknown[] = []) => {
		summary[key] =
			(summary[key] || 0) +
			(await deleteRun(db, sql, binds).catch((error) => {
				console.error("instance_delete_step_failed", {
					key,
	...
	const projectIds = await allIds(db, "SELECT id FROM projects WHERE instance_id = ?", [
		instanceId,
	]);
	const appIds = await allIds(db, "SELECT id FROM applications WHERE instance_id = ?", [
		instanceId,
	]);
	...
	const fileRows =
		projectIds.length || appIds.length
			? await db
					.prepare(`
	...
	const notificationIds = projectIds.length
		? await allIds(
				db,
				`SELECT id FROM notifications WHERE project_id IN (${projectIn})`,
				projectIds,
			)
		: [];
	...
	const inAppProductIds = projectIds.length
		? await allIds(
				db,
				`SELECT id FROM in_app_products WHERE project_id IN (${projectIn})`,
				projectIds,
			)
		: [];
	...
	const rpushAppIds = rpushAppNames.length
		? await allIds(
				db,
				`SELECT id FROM rpush_apps WHERE name IN (${placeholders(rpushAppNames)})`,
				rpushAppNames,
			)
		: [];
	const rpushAppIn = rpushAppIds.length ? placeholders(rpushAppIds) : "''";

	const mcpClientIds = await allIds(db, "SELECT id FROM mcp_clients WHERE instance_id = ?", [
		instanceId,
	]);
	...
	if (notificationIds.length) {
		await add(
			...
			`DELETE FROM notification_messages WHERE notification_id IN (${notificationIn})`,
			notificationIds,
		);
		await add(
			"notification_targets",
			`DELETE FROM notification_targets WHERE notification_id IN (${notificationIn})`,
			notificationIds,
		);
	}
	if (rpushAppIds.length)
		await add(
			"rpush_notifications",
			`DELETE FROM rpush_notifications WHERE app_id IN (${rpushAppIn})`,
			rpushAppIds,
		);
	if (rpushAppNames.length)
		await add(
			"rpush_apps",
			`DELETE FROM rpush_apps WHERE name IN (${placeholders(rpushAppNames)})`,
			rpushAppNames,
		);
	if (linkIds.length) {
		...
		await add(
			"custom_redirects",
			`DELETE FROM custom_redirects WHERE link_id IN (${linkIn})`,
			linkIds,
		);
		await add(
			"link_daily_statistics",
			`DELETE FROM link_daily_statistics WHERE link_id IN (${linkIn})`,
			linkIds,
		);
	}
	if (visitorIds.length) {
		await add(
			"visitor_last_visits",
			`DELETE FROM visitor_last_visits WHERE visitor_id IN (${visitorIn})`,
			visitorIds,
		);
		await add(
			"visitor_daily_statistics",
			`DELETE FROM visitor_daily_statistics WHERE visitor_id IN (${visitorIn})`,
			visitorIds,
		);
		await add(
			"notification_messages_by_visitor",
			`DELETE FROM notification_messages WHERE visitor_id IN (${visitorIn})`,
			visitorIds,
		);
	}
	if (inAppProductIds.length) {
		await add(
			...
			`DELETE FROM in_app_product_daily_statistics WHERE in_app_product_id IN (${inAppProductIn})`,
			inAppProductIds,
		);
	}
	if (mcpClientIds.length) {
		await add(
			"mcp_tokens",
			`DELETE FROM mcp_tokens WHERE mcp_client_id IN (${mcpClientIn})`,
			mcpClientIds,
		);
		await add(
			"mcp_authorization_codes",
			`DELETE FROM mcp_authorization_codes WHERE mcp_client_id IN (${mcpClientIn})`,
			mcpClientIds,
		);
	}
	if (projectIds.length) {
		await add(
			"iap_webhook_messages",
			`DELETE FROM iap_webhook_messages WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add(
			"installed_apps",
			`DELETE FROM installed_apps WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add(
			"purchase_events",
			`DELETE FROM purchase_events WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add(
			"subscription_states",
			`DELETE FROM subscription_states WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add("events", `DELETE FROM events WHERE project_id IN (${projectIn})`, projectIds);
		await add(
			"project_daily_active_users",
			`DELETE FROM project_daily_active_users WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add(
			"daily_project_metrics",
			`DELETE FROM daily_project_metrics WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add(
			"visitor_daily_statistics_by_project",
			`DELETE FROM visitor_daily_statistics WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		...
		await add(
			"notifications",
			`DELETE FROM notifications WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add(
			"in_app_products",
			`DELETE FROM in_app_products WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add(
			"downloadable_files",
			`DELETE FROM downloadable_files WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add(
			"redirects",
			`DELETE FROM redirects WHERE redirect_config_id IN (SELECT id FROM redirect_configs WHERE project_id IN (${projectIn}))`,
			projectIds,
		);
		await add(
			"redirect_configs",
			`DELETE FROM redirect_configs WHERE project_id IN (${projectIn})`,
			projectIds,
		);
		await add(
			"quick_links_by_project",
			`DELETE FROM quick_links WHERE project_id IN (${projectIn})`,
			projectIds,
		);
	...
	if (appIds.length) {
		await add("ios_server_api_keys", "DELETE FROM ios_server_api_keys WHERE instance_id = ?", [
			instanceId,
		]);
		await add(
			"android_server_api_keys",
			"DELETE FROM android_server_api_keys WHERE instance_id = ?",
			[instanceId],
		);
		await add(
			"ios_push_configurations",
			`DELETE FROM ios_push_configurations WHERE application_id IN (${appIn})`,
			appIds,
		);
		await add(
			"android_push_configurations",
			`DELETE FROM android_push_configurations WHERE application_id IN (${appIn})`,
			appIds,
		);
		await add(
			"web_configuration_linked_domains",
			`DELETE FROM web_configuration_linked_domains WHERE web_configuration_id IN (SELECT id FROM web_configurations WHERE application_id IN (${appIn}))`,
			appIds,
		);
		await add(
			"ios_configurations",
			`DELETE FROM ios_configurations WHERE application_id IN (${appIn})`,
			appIds,
		);
		await add(
			"android_configurations",
			`DELETE FROM android_configurations WHERE application_id IN (${appIn})`,
			appIds,
		);
		await add(
			"desktop_configurations",
			`DELETE FROM desktop_configurations WHERE application_id IN (${appIn})`,
			appIds,
		);
		await add(
			"web_configurations",
			`DELETE FROM web_configurations WHERE application_id IN (${appIn})`,
			appIds,
		);
		await add("applications", `DELETE FROM applications WHERE id IN (${appIn})`, appIds);
		await add(
			"downloadable_files_by_application",
			`DELETE FROM downloadable_files WHERE application_id IN (${appIn})`,
			appIds,
		);
	}

	await add("mcp_clients", "DELETE FROM mcp_clients WHERE instance_id = ?", [instanceId]);
	await add("setup_progress_steps", "DELETE FROM setup_progress_steps WHERE instance_id = ?", [
		instanceId,
	]);
	await add(
		"enterprise_subscriptions",
		"DELETE FROM enterprise_subscriptions WHERE instance_id = ?",
		[instanceId],
	);
	await add("quick_links_by_instance", "DELETE FROM quick_links WHERE instance_id = ?", [
		instanceId,
	]);
	await add("instance_roles", "DELETE FROM instance_roles WHERE instance_id = ?", [instanceId]);
	if (projectIds.length)
		await add("projects", `DELETE FROM projects WHERE id IN (${projectIn})`, projectIds);
...
instances.get("/", async (c) => {
	...
	const site = c.get("siteAuth");
	if (site?.siteOperator && site.instanceId)
		return c.json({
			instances: [
				await buildInstance(
					c.env.DB,
					...
					site.siteOperator.role === 50 ? "owner" : "admin",
				),
			].filter(Boolean),
		});
	const roles = await c.env.DB.prepare("SELECT instance_id FROM instance_roles WHERE user_id = ?")
		.bind(userId)
		.all<{ instance_id: number }>();
	const list = await Promise.all(
		(roles.results || []).map((r) =>
			buildInstance(c.env.DB, r.instance_id, userId, c.env.SHORTLINK_DOMAIN),
		),
	);
...
instances.get("/:id", async (c) => {
	...
	const instance = await buildInstance(
		...
		userId,
		c.env.SHORTLINK_DOMAIN,
		c.get("siteAuth")?.siteOperator?.role === 50
			? "owner"
			: c.get("siteAuth")
				? "admin"
				: undefined,
...
instances.put("/:id", async (c) => {
	...
	const instance = await buildInstance(
		...
		access.userId,
		c.env.SHORTLINK_DOMAIN,
		c.get("siteAuth") ? access.role : undefined,
	);
...
instances.post("/:id/members", async (c) => {
	...
	if ("error" in access) return access.error;
	const body = await readApiJson(c.req.raw);
	const email = String(body.email || body.user?.email || "")
		.trim()
		.toLowerCase();
	...
	let invited = await c.env.DB.prepare("SELECT id, email, name FROM users WHERE email = ? LIMIT 1")
		.bind(email)
    ...
    `)
      ...
      WHERE id = ?
    `)
			.bind(invitationTokenDigest, access.userId, invited.id)
			.run();
	}
	...
	await c.env.DB.prepare(
		...
		.bind(invited!.id, instanceId, role)
		.run();
	await c.env.DB.prepare(
	...
	try {
		...
		console.error("invitation_mail_failed", {
...
instances.delete("/:id/members", async (c) => {
	...
	const target = targetUserId
		...
		: await c.env.DB.prepare("SELECT id FROM users WHERE email = ? LIMIT 1")
				.bind(targetEmail)
				.first<{ id: number }>();
...
instances.get("/:id/role", async (c) => {
	...
	if (userId === null) return c.json({ error: "Unauthorized" }, 401);
	const instanceId = Number(c.req.param("id"));
	const access = await requireInstanceRole(c, instanceId);
	if ("error" in access) return access.error;
	const role = { role: access.role };
...
async function validateServiceAccountJson(
	file: any,
): Promise<{ content: string; filename: string; parsed: Record<string, any> }> {
...
async function getOrCreateApplication(
	...
	const existing = await db
		.prepare("SELECT id FROM applications WHERE instance_id = ? AND platform = ? LIMIT 1")
		.bind(instanceId, platform)
		.first<{ id: number }>();
	if (existing) {
		await db
			.prepare('UPDATE applications SET enabled = ?, updated_at = datetime("now") WHERE id = ?')
...
async function loadApplicationConfig(db: D1Database, instanceId: number, platform: string) {
    ...
    `)
			...
			.first<any>();
		if (!row) return null;
		const domainRows = row.config_id
			? await db
					.prepare(
...
async function loadConfigurations(db: D1Database, instanceId: number) {
	const configs = await Promise.all(
		["ios", "android", "desktop", "web"].map((platform) =>
			loadApplicationConfig(db, instanceId, platform),
		),
	);
...
async function findConfigId(
	...
	const row = await db
		.prepare(`SELECT id FROM ${table} WHERE application_id = ? LIMIT 1`)
		.bind(applicationId)
...
instances.put("/:id/configurations/ios", async (c) => {
  ...
  `)
		.bind(
			...
			bundleId,
			appPrefix,
			body.app_apple_id || body.app_store_id || null,
			tabletEnabled ? 1 : 0,
		)
		.run();
...
instances.delete("/:id/configurations/ios", async (c) => {
	...
	if (app) {
		await c.env.DB.prepare(
			"DELETE FROM ios_server_api_keys WHERE ios_configuration_id IN (SELECT id FROM ios_configurations WHERE application_id = ?) OR instance_id = ?",
		)
			.bind(app.id, instanceId)
			.run();
		await c.env.DB.prepare(
			"DELETE FROM ios_push_configurations WHERE ios_configuration_id IN (SELECT id FROM ios_configurations WHERE application_id = ?) OR application_id = ?",
		)
			.bind(app.id, app.id)
			.run();
		await c.env.DB.prepare("DELETE FROM ios_configurations WHERE application_id = ?")
			.bind(app.id)
			.run();
		await c.env.DB.prepare(
			"DELETE FROM setup_progress_steps WHERE instance_id = ? AND category = ?",
		)
			.bind(instanceId, "ios_setup")
			.run();
	}
...
instances.put("/:id/configurations/ios/push", async (c) => {
	...
	if (!iosConfigId) {
		await c.env.DB.prepare(
			"INSERT INTO ios_configurations (application_id, bundle_id, app_prefix) VALUES (?, ?, ?)",
		)
			.bind(appId, null, "")
			.run();
	...
	if (!file || !password)
		return c.json({ error: "push_certificate and push_certificate_password are required" }, 422);
	const { content, filename } = await validateP8File(file).catch((error) => {
		throw error;
	});
	const encryptedP8Key = await encryptCredential(c.env, content);

	await c.env.DB.prepare(
		"DELETE FROM ios_push_configurations WHERE ios_configuration_id = ? OR application_id = ?",
	)
		.bind(iosConfigId, appId)
		.run();
	await c.env.DB.prepare(`
    ...
    WHERE ic.id = ?
  `)
		.bind(filename, encryptedP8Key, password, password, appId, iosConfigId)
		.run();
...
instances.put("/:id/configurations/android", async (c) => {
	...
	const sha256s =
		body.sha256_cert_fingerprints || body.sha256s || body["configuration[sha256s]"] || [];
    ...
    ON CONFLICT(application_id) DO UPDATE SET
      ...
      updated_at = datetime('now')
  `)
		.bind(appId, identifier, shaJson, tabletEnabled ? 1 : 0)
		.run();
...
instances.delete("/:id/configurations/android", async (c) => {
	...
	const app = await c.env.DB.prepare(
		...
		.bind(instanceId, "android")
		.first<{ id: number }>();
	if (app) {
		await c.env.DB.prepare(
			"DELETE FROM android_server_api_keys WHERE android_configuration_id IN (SELECT id FROM android_configurations WHERE application_id = ?) OR instance_id = ?",
		)
			.bind(app.id, instanceId)
			.run();
		await c.env.DB.prepare(
			"DELETE FROM android_push_configurations WHERE android_configuration_id IN (SELECT id FROM android_configurations WHERE application_id = ?) OR application_id = ?",
		)
			.bind(app.id, app.id)
			.run();
		await c.env.DB.prepare("DELETE FROM android_configurations WHERE application_id = ?")
			.bind(app.id)
			.run();
		await c.env.DB.prepare(
			"DELETE FROM setup_progress_steps WHERE instance_id = ? AND category = ?",
		)
			.bind(instanceId, "android_setup")
			.run();
	}
	return c.json({ config: null, configuration: null });
});
...
instances.put("/:id/configurations/android/push", async (c) => {
	...
	const file = pick(body, "push_certificate");
	if (!firebaseProjectId || !file)
		return c.json({ error: "firebase_project_id and push_certificate are required" }, 422);
	...
	if (!androidConfigId) {
		await c.env.DB.prepare(
			"INSERT INTO android_configurations (application_id, identifier, sha256s) VALUES (?, ?, ?)",
		)
			.bind(appId, parsed.client_email || "unconfigured.android", "[]")
			.run();
		androidConfigId = await findConfigId(c.env.DB, "android_configurations", appId);
	}

	await c.env.DB.prepare(
		"DELETE FROM android_push_configurations WHERE android_configuration_id = ? OR application_id = ?",
	)
		.bind(androidConfigId, appId)
		.run();
	await c.env.DB.prepare(`
...
instances.put("/:id/configurations/android/api_access_key", async (c) => {
	...
	await c.env.DB.prepare(
		"DELETE FROM android_server_api_keys WHERE android_configuration_id = ? OR instance_id = ?",
	)
		.bind(androidConfigId, instanceId)
		.run();
	await c.env.DB.prepare(`
  ...
  `)
		.bind(
			...
			filename,
			encrypted,
			parsed.private_key_id || null,
			parsed.project_id || null,
			parsed.client_email || null,
		)
		.run();
	return c.json(configPayload(await loadApplicationConfig(c.env.DB, instanceId, "android")));
});
...
instances.put("/:id/configurations/ios/api_access_key", async (c) => {
	...
	if (!iosConfigId) {
		await c.env.DB.prepare(
			...
			.bind(
				appId,
				pick(body, "bundle_id") || "",
				pick(body, "team_id") || pick(body, "app_prefix") || "",
			)
			.run();
		iosConfigId = await findConfigId(c.env.DB, "ios_configurations", appId);
	}
    ...
    WHERE id = ?
  `)
		.bind(appleAppId, iosConfigId)
		.run();
	await c.env.DB.prepare(
		"DELETE FROM ios_server_api_keys WHERE ios_configuration_id = ? OR instance_id = ?",
	)
		.bind(iosConfigId, instanceId)
		.run();
	await c.env.DB.prepare(`
...
TOPIC_NAME="$1"
SUBSCRIPTION_NAME="\${2:-superboard-play-rtdn-subscription}"

if [ -z "$TOPIC_NAME" ]; then
  echo "Usage: ./superboard_android_gcloud_setup.sh <existing-play-rtdn-topic-name> [subscription-name]"
  exit 1
fi
...
instances.get("/:id/configurations/android/google_configuration_script", async (c) => {
	...
	const serviceAccountEmail = String(
		config?.configuration?.server_api_key?.client_email || "",
	).trim();
	const validProjectId = /^[a-z][a-z0-9-]{4,28}[a-z0-9]$/.test(projectId);
	const validServiceAccount =
		/^[a-z0-9-]{1,30}@[a-z][a-z0-9-]{4,28}[a-z0-9]\.iam\.gserviceaccount\.com$/.test(
			serviceAccountEmail,
	...
	if (!validProjectId || !validServiceAccount) {
		return c.json(
			{
				error:
					"Upload and test the Google Play service-account key before generating the RTDN setup script",
			},
			409,
		);
	}
	return c.text(
		buildGoogleConfigurationScript({
			instanceId,
...
instances.delete("/:id/configurations/desktop", async (c) => {
	...
	const app = await c.env.DB.prepare(
		...
		.bind(instanceId, "desktop")
		.first<{ id: number }>();
	if (app) {
		await c.env.DB.prepare("DELETE FROM desktop_configurations WHERE application_id = ?")
			.bind(app.id)
			.run();
		await c.env.DB.prepare(
			"DELETE FROM setup_progress_steps WHERE instance_id = ? AND category = ?",
		)
			.bind(instanceId, "desktop_setup")
			.run();
	}
	return c.json({ config: null, configuration: null });
});
...
instances.put("/:id/configurations/web", async (c) => {
	...
	if (!webConfigId) {
		await c.env.DB.prepare("INSERT INTO web_configurations (application_id) VALUES (?)")
			.bind(appId)
			.run();
	...
	await c.env.DB.prepare(
		"DELETE FROM web_configuration_linked_domains WHERE web_configuration_id = ?",
	)
		.bind(webConfigId)
		.run();
...
instances.delete("/:id/configurations/web", async (c) => {
	...
	if (app) {
		await c.env.DB.prepare(
			"DELETE FROM web_configuration_linked_domains WHERE web_configuration_id IN (SELECT id FROM web_configurations WHERE application_id = ?)",
		)
			.bind(app.id)
			.run();
		await c.env.DB.prepare("DELETE FROM web_configurations WHERE application_id = ?")
			.bind(app.id)
			.run();
		await c.env.DB.prepare(
			"DELETE FROM setup_progress_steps WHERE instance_id = ? AND category = ?",
		)
			.bind(instanceId, "web_setup")
			.run();
	}
	return c.json({ config: null, configuration: null });
});
...
instances.put("/:id/revenue_collection", async (c) => {
	...
	const body = await readApiJson(c.req.raw);
	const enabled =
		body.revenue_collection_enabled === true ||
		body.revenue_collection_enabled === 1 ||
		body.revenue_collection_enabled === "true";
	await c.env.DB.prepare(
		'UPDATE instances SET revenue_collection_enabled = ?, updated_at = datetime("now") WHERE id = ?',
	)
...
instances.post("/:id/exports/usage", async (c) => {
  ...
  `)
	...
	const file = await storeCsvDownload(c, {
...
instances.post("/:id/events/billing", async (c) => {
    ...
    GROUP BY metric_date
  `)
		.bind(...projectIds, startDate, endDate)
		.all<any>();
