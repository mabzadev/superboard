...
import { getOrCreateRedirectConfig, jsonArray, parseJsonObject, resolveProject } from "../lib/db";
import { readCsvDownload, storeCsvDownload } from "../lib/files";
import { downloadFileMessage, sendMail } from "../lib/mail";
import { siteOperatorAdapter, type SiteAdapterVariables } from "../lib/site-operator-adapter";
import { Env } from "../types";

const projects = new Hono<{ Bindings: Env; Variables: SiteAdapterVariables }>();
projects.use("*", siteOperatorAdapter("projects"));

async function currentUserId(c: any): Promise<number | null> {
	return c.get("siteAuth") ? 0 : getRequestAuthUserId(c.env, c.req.raw.headers, c.req.raw);
}
...
const _EVENT_TO_COLUMN: Record<string, string> = {
	view: "views",
	open: "opens",
	install: "installs",
...
async function uniqueVisitorsFromEvents(
	db: D1Database,
	projectId: number,
	startDate: string,
	endDate: string,
  ...
  `)
		.bind(projectId, startDate, endDate, ...filter.values)
		.first<{ total: number }>();
	return Number(row?.total || 0);
}

async function firstTimeVisitorsFromEvents(
	db: D1Database,
	projectId: number,
	startDate: string,
	endDate: string,
  ...
  `)
		.bind(projectId, startDate, endDate, ...filter.values, startDate)
		.first<{ total: number }>();
	return Number(row?.total || 0);
}

async function purchaseMetricsForRange(
	db: D1Database,
	projectId: number,
	startDate: string,
	endDate: string,
	...
	const platformClause =
		filter.values.length > 0 ? filter.clause.replaceAll("platform", "d.platform") : "";
	const row = await db
		.prepare(`
    SELECT
    ...
    WHERE pe.project_id = ?
      ...
      ${platformClause}
  `)
		.bind(String(projectId), startDate, endDate, ...filter.values)
		.first<any>();
	return {
...
async function metricsForRange(
	db: D1Database,
	projectId: number,
	startDate: string,
	endDate: string,
  ...
  `)
	...
	metrics.returning_rate =
		totalUsers === 0 ? 0 : Number((metrics.returning_users / totalUsers).toFixed(4));
	...
	metrics.arppu =
		purchases.paying_users > 0 ? Number((metrics.revenue / purchases.paying_users).toFixed(2)) : 0;
	return metrics;
}
...
function buildLink(row: any, redirects: any[] = [], shortlinkDomain = "") {
	...
	return {
		...
		data,
		ios_custom_redirect: ios
			? { url: ios.url, open_app_if_installed: ios.open_app_if_installed !== 0 }
			: undefined,
		android_custom_redirect: android
			? { url: android.url, open_app_if_installed: android.open_app_if_installed !== 0 }
			: undefined,
		desktop_custom_redirect: desktop
			? { url: desktop.url, open_app_if_installed: desktop.open_app_if_installed !== 0 }
			: undefined,
...
projects.post("/:id/links", async (c) => {
	...
	const duplicate = await c.env.DB.prepare("SELECT id FROM links WHERE path = ?")
		.bind(path)
		.first();
  ...
  `)
	...
	for (const [platform, value] of redirects) {
		...
		if (url) {
			await c.env.DB.prepare(
				...
				.bind(link.id, platform, String(url), 1)
				.run();
		}
	}
	...
	return c.json(
		{
			link: buildLink(link, [], c.env.SHORTLINK_DOMAIN),
			short_url: `https://${c.env.SHORTLINK_DOMAIN}/${path}`,
		},
		201,
	);
});
...
projects.post("/:id/links/by_ids", async (c) => {
	...
	const rows = await c.env.DB.prepare(
		`SELECT l.* FROM links l JOIN redirect_configs rc ON rc.id = l.redirect_config_id WHERE l.id IN (${placeholders}) AND rc.project_id = ?`,
	)
		.bind(...ids, project.id)
		.all<any>();
	return c.json({
		links: (rows.results || []).map((row: any) => buildLink(row, [], c.env.SHORTLINK_DOMAIN)),
	});
});
...
projects.delete("/:id/links/:linkId", async (c) => {
	const userId = await currentUserId(c);
	if (userId === null) return c.json({ error: "Unauthorized" }, 401);
	await c.env.DB.prepare("DELETE FROM custom_redirects WHERE link_id = ?")
		.bind(Number(c.req.param("linkId")))
		.run();
...
projects.get("/:id/domain", async (c) => {
	const project = await resolveProject(c.env.DB, c.req.param("id"));
	let domain = await c.env.DB.prepare("SELECT * FROM domains WHERE project_id = ? LIMIT 1")
		.bind(project.id)
		.first<any>();
	if (!domain) {
		domain = await c.env.DB.prepare(
			...
			.bind(c.env.SHORTLINK_DOMAIN, "", project.id, "SuperBoard", "Share & discover")
			.first<any>();
	}
	return c.json({
		domain: {
	...
	await c.env.DB.prepare(
     ...
     ON CONFLICT(project_id) DO UPDATE SET
       ...
       updated_at = datetime('now')`,
		...
		.run();
	const domain = await c.env.DB.prepare("SELECT * FROM domains WHERE project_id = ? LIMIT 1")
		.bind(project.id)
		.first<any>();
	return c.json({
		domain: {
...
projects.get("/:id/domain/defaults", async (c) => {
	return c.json({
		generic_title: "SuperBoard",
		generic_subtitle: "Share & discover",
...
projects.put("/:id/domain/google_tracking_id", async (c) => {
	...
	const project = await resolveProject(c.env.DB, c.req.param("id"));
	await c.env.DB.prepare(
		'UPDATE domains SET google_tracking_id = ?, updated_at = datetime("now") WHERE project_id = ?',
	)
		.bind(body.google_tracking_id || null, project.id)
		.run()
		.catch(() => null);
...
async function redirectConfigResponse(c: any, projectExternalId: string) {
	...
	const config: any = await c.env.DB.prepare("SELECT * FROM redirect_configs WHERE id = ?")
		.bind(id)
		.first();
	const redirects: { results?: any[] } = await c.env.DB.prepare(
		"SELECT * FROM redirects WHERE redirect_config_id = ?",
	)
		.bind(id)
		.all();
	const by = (platform: string, variation: string) => {
		const row = (redirects.results || []).find(
			(r: any) => r.platform === platform && r.variation === variation,
		);
		return {
...
projects.get("/:id/redirect_config", async (c) =>
	c.json(await redirectConfigResponse(c, c.req.param("id"))),
);
...
projects.put("/:id/redirect_config", async (c) => {
	...
	await c.env.DB.prepare(
		...
		.bind(
			body.default_fallback || "",
			toBool(body.show_preview_android) ? 1 : 0,
			toBool(body.show_preview_ios) ? 1 : 0,
			id,
		)
		.run();
     ...
     ON CONFLICT(redirect_config_id, platform, variation) DO UPDATE SET
       ...
       updated_at = datetime('now')`,
		...
		.bind(
			...
			platform,
			variation,
			toBool(body.enabled) ? 1 : 0,
			toBool(body.appstore) ? 1 : 0,
...
projects.post("/:id/dashboard/top_links", async (c) => {
    ...
    LIMIT 10
  `)
		.bind(project.id, range.startDate, range.endDate, ...filter.values)
		.all<any>();

	const eventFilter = platformFilter(body.platform, "e.platform");
	const eventRows =
		(dailyRows.results || []).length > 0
			? { results: [] as any[] }
			: await c.env.DB.prepare(`
    SELECT l.*,
    ...
    LIMIT 10
  `)
					.bind(project.id, range.startDate, range.endDate, ...eventFilter.values)
					.all<any>();

	const rows =
		(dailyRows.results || []).length > 0 ? dailyRows.results || [] : eventRows.results || [];
	return c.json({
...
projects.post("/:id/dashboard/links_views", async (c) => {
	...
	const metrics = Object.fromEntries(
		eachDate(range.startDate, range.endDate).map((date) => [date, 0]),
	);
    ...
    GROUP BY event_date
  `)
		.bind(project.id, range.startDate, range.endDate, ...dailyFilter.values)
		.all<{ event_date: string; link_views: number }>();
	if ((dailyRows.results || []).length > 0) {
		for (const row of dailyRows.results || [])
			metrics[String(row.event_date).slice(0, 10)] = Number(row.link_views || 0);
...
projects.post("/:id/dashboard/metrics_overview", async (c) => {
	...
	return c.json({
		metrics: {
			...
			previous: await metricsForRange(
				c.env.DB,
				project.id,
...
async function campaignMetricsForRange(
	db: D1Database,
	projectId: number,
	startDate: string,
	endDate: string,
...
projects.post("/:id/campaigns", async (c) => {
	...
	const row = await c.env.DB.prepare(
		...
		.bind(body.name || "Campaign", project.id)
		.first<any>();
...
projects.post("/:id/campaigns/search_v2", async (c) => {
  ...
  `)
		.bind(range.startDate, range.endDate, ...filter.values, project.id)
		.all<any>();
...
projects.post("/:id/campaigns/metrics_overview", async (c) => {
	...
	return c.json({
		metrics: {
			current: await campaignMetricsForRange(
				c.env.DB,
				project.id,
				range.startDate,
				range.endDate,
			...
			previous: await campaignMetricsForRange(
				c.env.DB,
				project.id,
				range.previousStartDate,
				range.previousEndDate,
...
projects.get("/:id/campaigns/:campaignId", async (c) => {
	const row = await c.env.DB.prepare("SELECT * FROM campaigns WHERE id = ?")
		.bind(Number(c.req.param("campaignId")))
		.first<any>();
	if (!row) return c.json({ error: "Not found" }, 404);
...
projects.patch("/:id/campaigns/:campaignId", async (c) => {
	const body = await readBody(c);
	await c.env.DB.prepare(
		'UPDATE campaigns SET name = COALESCE(?, name), updated_at = datetime("now") WHERE id = ?',
	)
		.bind(body.name || null, Number(c.req.param("campaignId")))
		.run();
	const row = await c.env.DB.prepare("SELECT * FROM campaigns WHERE id = ?")
		.bind(Number(c.req.param("campaignId")))
		.first<any>();
...
projects.delete("/:id/campaigns/:campaignId", async (c) => {
	await c.env.DB.prepare(
		'UPDATE campaigns SET archived = 1, updated_at = datetime("now") WHERE id = ?',
	)
		.bind(Number(c.req.param("campaignId")))
		.run();
...
async function ensureVisitorForDevice(
	db: D1Database,
	projectId: number,
	deviceId: number,
	...
	const existing = await db
		.prepare("SELECT id FROM visitors WHERE project_id = ? AND device_id = ? LIMIT 1")
		.bind(String(projectId), deviceId)
		.first<{ id: number }>();
	if (existing) {
		if (sdkIdentifier) {
			await db
				.prepare(
					'UPDATE visitors SET sdk_identifier = COALESCE(?, sdk_identifier), updated_at = datetime("now") WHERE id = ?',
				)
				.bind(sdkIdentifier, existing.id)
				.run();
		}
		return existing.id;
	}
    ...
    RETURNING id
  `)
		.bind(String(projectId), deviceId, sdkIdentifier || null, crypto.randomUUID(), "{}")
		.first<{ id: number }>();
...
projects.post("/:id/visitors/search", async (c) => {
  ...
  `)
		.bind(String(project.id), ...filter.values, ...termValues)
		.first<{ total: number }>();
	const rows = await c.env.DB.prepare(`
...
projects.get("/:id/visitors/:visitorId", async (c) => {
    ...
    GROUP BY v.id, d.platform, d.country_code
  `)
		.bind(String(project.id), visitorId)
		.first<any>();
	if (!row) return c.json({ error: "Not found" }, 404);
	const visitor = visitorRow(row);
	const numberOfLinks = await c.env.DB.prepare(
		"SELECT COUNT(*) AS total FROM links WHERE visitor_id = ?",
	)
		.bind(visitorId)
		.first<{ total: number }>();
	return c.json({
...
projects.post("/:id/visitors/metrics", async (c) => {
  ...
  `)
		.bind(project.id, range.startDate, range.endDate)
		.first<any>();
...
projects.post("/:id/visitors/aggregated_metrics", async (c) => {
    ...
    WHERE rc.project_id = ?
  `)
		.bind(range.startDate, range.endDate, project.id)
		.first<any>();
	return c.json({ metrics: visitorMetricsRow(row, "total_") });
});
...
async function revenueRows(c: any, projectExternalId: string, body: Record<string, any>) {
  ...
  `)
		.bind(projectId, range.startDate, range.endDate, ...filter.values, ...termValues)
		.first();
	const rows = await c.env.DB.prepare(`
    SELECT
      pe.product_id,
...
async function purchaseRows(c: any, projectExternalId: string, body: Record<string, any>) {
    ...
    WHERE pe.project_id = ?
      ...
      ${termClause}
  `)
		.bind(projectId, range.startDate, range.endDate, ...termValues)
		.first();
	const rows = await c.env.DB.prepare(`
    ...
    LIMIT ? OFFSET ?
  `)
		.bind(projectId, range.startDate, range.endDate, ...termValues, perPage, (page - 1) * perPage)
		.all();
	const total = Number(count?.total || 0);
	return {
...
projects.post("/:id/purchases/search", async (c) =>
	c.json(await purchaseRows(c, c.req.param("id"), await readBody(c))),
);
projects.post("/:id/purchases/revenue", async (c) =>
	c.json(await revenueRows(c, c.req.param("id"), await readBody(c))),
);
...
function notificationRow(c: any, row: NotificationRecord) {
...
async function createMessagesForExistingVisitors(
	db: D1Database,
  ...
  `)
		.bind(notificationId, String(projectId), ...platform.values, notificationId)
		.run();
}

async function rpushAppForProjectPlatform(
	db: D1Database,
    ...
    `)
			.bind(
				...
				row.platform === "android"
					? JSON.stringify({ title: notification.title || "", body: notification.subtitle || "" })
					: null,
				data,
...
projects.post("/:id/notifications/search", async (c) => {
	...
	const sortBy =
		typeof body.sort_by === "string" && allowedSorts.includes(body.sort_by)
			? body.sort_by
			: "updated_at";
	const ascending = body.ascending === true ? 1 : 0;
	...
	const pushClause =
		typeof body.send_push === "boolean"
			? body.send_push
				? " AND COALESCE(n.send_push, 0) = 1"
				: " AND COALESCE(n.send_push, 0) = 0"
			: "";
	const visibility =
		body.include_drafts === true && archived === 0
			? "(COALESCE(n.archived, 0) = ? OR EXISTS (SELECT 1 FROM notification_drafts nd WHERE nd.notification_id=n.id))"
			: "COALESCE(n.archived, 0) = ? AND NOT EXISTS (SELECT 1 FROM notification_drafts nd WHERE nd.notification_id=n.id)";
	const count = await c.env.DB.prepare(`
    SELECT COUNT(*) AS total
    FROM notifications n
    LEFT JOIN notification_targets nt ON nt.notification_id = n.id
    WHERE n.project_id = ? AND ${visibility}
      ${newUsersClause}
      ${termClause}
      ${pushClause}
  `)
	...
	const rows = await c.env.DB.prepare(`
    SELECT n.*,
      EXISTS (SELECT 1 FROM notification_drafts nd WHERE nd.notification_id=n.id) AS draft,
      nt.id AS target_id,
    ...
    WHERE n.project_id = ? AND ${visibility}
      ...
      ${termClause}
      ${pushClause}
    ORDER BY CASE WHEN ? = 1 THEN CASE ? WHEN 'title' THEN n.title WHEN 'subtitle' THEN n.subtitle WHEN 'platform' THEN nt.platforms WHEN 'read_count' THEN read_count WHEN 'auto_display' THEN n.auto_display WHEN 'push_notification' THEN n.send_push ELSE n.updated_at END END ASC,
      CASE WHEN ? = 0 THEN CASE ? WHEN 'title' THEN n.title WHEN 'subtitle' THEN n.subtitle WHEN 'platform' THEN nt.platforms WHEN 'read_count' THEN read_count WHEN 'auto_display' THEN n.auto_display WHEN 'push_notification' THEN n.send_push ELSE n.updated_at END END DESC, n.id DESC
    LIMIT ? OFFSET ?
  `)
		.bind(
			project.id,
	...
	const total = Number(count?.total || 0);
	return c.json({
		data: (rows.results || []).map((row) => notificationRow(c, row)),
		total_pages: Math.max(1, Math.ceil(total / perPage)),
		total_entries: total,
	});
});
...
projects.post("/:id/notifications", async (c) => {
	const project = await resolveProject(c.env.DB, c.req.param("id"));
	const body = await readRequestObjectLimited(c.req.raw, 1024 * 1024);
	...
	const row = await c.env.DB.prepare(
		"INSERT INTO notifications (project_id, title, subtitle, html, send_push, auto_display, archived) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING *",
	)
		.bind(
			project.id,
			typeof body.title === "string" ? body.title : "",
			typeof body.subtitle === "string" ? body.subtitle : "",
			typeof body.html_message === "string"
				? body.html_message
				: typeof body.html === "string"
					? body.html
					: null,
			toBool(body.send_push) ? 1 : 0,
			toBool(body.auto_display) ? 1 : 0,
			body.draft === true ? 1 : 0,
		)
    ...
    VALUES (?, ?, ?, ?)
  `)
		.bind(String(row.id), existingUsers ? 1 : 0, newUsers ? 1 : 0, JSON.stringify(platforms))
		.run();
	if (body.draft === true)
		await c.env.DB.prepare("INSERT INTO notification_drafts (notification_id) VALUES (?)")
			.bind(row.id)
			.run();
	if (existingUsers && body.draft !== true) {
...
projects.get("/:id/notifications/:notifId", async (c) => {
	const project = await resolveProject(c.env.DB, c.req.param("id"));
	const row = await c.env.DB.prepare(`SELECT n.*, nt.platforms, nt.new_users, nt.existing_users,
    EXISTS (SELECT 1 FROM notification_drafts nd WHERE nd.notification_id=n.id) AS draft,
    (SELECT COUNT(*) FROM notification_messages nm WHERE nm.notification_id=n.id AND nm.read=1) AS read_count,
    (SELECT COUNT(*) FROM notification_messages nm WHERE nm.notification_id=n.id) AS recipients
    FROM notifications n LEFT JOIN notification_targets nt ON nt.notification_id=n.id
    WHERE n.project_id=? AND n.id=?`)
		.bind(project.id, c.req.param("notifId"))
		.first<NotificationRecord>();
	if (!row) return c.json({ error: "Not found" }, 404);
	const push =
		await c.env.DB.prepare(`SELECT COUNT(*) AS queued, COALESCE(SUM(delivered=1),0) AS delivered, COALESCE(SUM(failed=1),0) AS failed
    FROM rpush_notifications WHERE json_extract(data,'$.notification_id')=?`)
			.bind(String(row.id))
			.first();
	return c.json({
		...notificationRow(c, row),
		statistics: { recipients: row.recipients, views: row.read_count, push },
	});
});

projects.patch("/:id/notifications/:notifId", async (c) => {
	const project = await resolveProject(c.env.DB, c.req.param("id"));
	const id = c.req.param("notifId");
	const body = await readRequestObjectLimited(c.req.raw, 1024 * 1024);
	const current = await c.env.DB.prepare(
		"SELECT n.id FROM notifications n JOIN notification_drafts nd ON nd.notification_id=n.id WHERE n.id=? AND n.project_id=? AND n.archived=1",
	)
		.bind(id, project.id)
		.first();
	if (!current) return c.json({ error: "Only drafts can be edited" }, 409);
	const platforms = parseJsonArray(body.platforms);
	if (!platforms.length || platforms.some((value) => !["android", "ios"].includes(value)))
		return c.json({ error: "Choose Android or Apple" }, 422);
	const newUsers = toBool(body.new_users);
	const existingUsers = toBool(body.existing_users);
	if (
		...
		return c.json({ error: "Invalid notification settings" }, 422);
	await c.env.DB.batch([
		c.env.DB.prepare(
			"UPDATE notifications SET title=?,subtitle=?,html=?,send_push=?,auto_display=?,updated_at=datetime('now') WHERE id=? AND project_id=? AND archived=1",
		).bind(
			body.title.trim(),
			typeof body.subtitle === "string" ? body.subtitle.slice(0, 2000) : "",
		...
		c.env.DB.prepare(
			"UPDATE notification_targets SET platforms=?,new_users=?,existing_users=? WHERE notification_id=?",
		).bind(JSON.stringify(platforms), newUsers ? 1 : 0, existingUsers ? 1 : 0, id),
	]);
	return c.json({ saved: true });
});

projects.post("/:id/notifications/:notifId/publish", async (c) => {
	const project = await resolveProject(c.env.DB, c.req.param("id"));
	const id = c.req.param("notifId");
	const row = await c.env.DB.prepare(
		"SELECT n.*,nt.platforms,nt.existing_users,nt.new_users FROM notifications n JOIN notification_drafts nd ON nd.notification_id=n.id JOIN notification_targets nt ON nt.notification_id=n.id WHERE n.id=? AND n.project_id=?",
	)
		.bind(id, project.id)
		.first<NotificationRecord>();
	...
	if (
		...
		!parseJsonArray(row.platforms).length
	)
		return c.json({ error: "Complete the message before publishing" }, 422);
	await c.env.DB.prepare(
		"UPDATE notifications SET archived=0,updated_at=datetime('now') WHERE id=? AND project_id=?",
	)
		.bind(id, project.id)
		.run();
	if (row.existing_users === 1) {
		await createMessagesForExistingVisitors(
			c.env.DB,
			Number(id),
			project.id,
			parseJsonArray(row.platforms),
		);
		await enqueuePushNotifications(c.env.DB, id, project.id);
	}
	await c.env.DB.prepare("DELETE FROM notification_drafts WHERE notification_id=?").bind(id).run();
...
projects.delete("/:id/notifications/:notifId", async (c) => {
	...
	const target = await c.env.DB.prepare(
		"SELECT existing_users FROM notification_targets WHERE notification_id = ? LIMIT 1",
	)
		.bind(String(notifId))
		.first<{ existing_users: number }>();
  ...
  `)
	...
	if (!row) return c.json({ error: "Not found" }, 404);
	return c.json({
		notifications: notificationRow(c, { ...row, ...target, platforms: "[]", read_count: 0 }),
	});
});
...
projects.post("/:id/exports/links", async (c) => {
	...
	const range = dashboardDateRange(body);
	const activeFilter =
		body.active === undefined || body.active === null || body.active === ""
			? ""
  ...
  `)
	...
	const user =
		userId === 0
			? null
			: await c.env.DB.prepare("SELECT email FROM users WHERE id = ? LIMIT 1")
					.bind(userId)
					.first<{ email: string }>();
	if (user?.email) {
