...
import { RequestBodyError, readRequestObjectLimited } from "@superboard/contracts/request-body";
...
customSdk.post("/jobs", async (c) => {
	try {
		...
		if (!rawIdempotencyKey || rawIdempotencyKey.length > 255) {
			return failure(c, 400, "idempotency_key_invalid", "A valid Idempotency-Key is required");
		}
		...
		for (const identityField of ["userId", "user_id", "subject", "projectRef", "project_ref"]) {
...
customSdk.get("/jobs/:jobId", async (c) => {
	try {
		...
		return proxyCustom(c, `${CUSTOM_WORKER_JOB_PATH}/${encodeURIComponent(jobId)}`, context);
	} catch (error) {
		return contextFailure(c, error);
	}
});
...
async function applicationContext(
	...
	if (
		...
		return failure(c, 403, "sdk_project_invalid", "A valid SDK project is required");
	}
	const authorization = c.req.header("Authorization")?.trim() || "";
	if (!/^Bearer\s+\S+$/i.test(authorization)) {
		return failure(c, 401, "identity_required", "An application identity token is required");
	}
	...
	if (!project) return failure(c, 403, "sdk_project_invalid", "A valid SDK project is required");
	let subject: string | null;
	try {
		subject = await verifiedAppUserId(c.env, projectId, authorization);
	} catch (error) {
		if (isRejectedIdentity(error)) throw new CustomSdkIdentityRejectedError("identity_rejected");
		throw new CustomSdkDependencyUnavailableError("identity_provider_unavailable");
	}
	if (!subject)
		return failure(c, 401, "identity_required", "An application identity token is required");
	return {
...
async function scopedIdempotencyKey(projectRef: string, subject: string, provided: string) {
	const bytes = new TextEncoder().encode(`${projectRef}\u0000${subject}\u0000${provided}`);
...
async function proxyCustom(
	...
	if (!c.env.CUSTOM_WORKER) {
		return failure(c, 404, "custom_worker_disabled", "This application has no custom job service");
	}
	...
	if (init.method === "POST" && path === CUSTOM_WORKER_JOB_PATH && c.env.CUSTOM_WORKER_PLUGIN_ID) {
		headers.set(
			"X-SuperBoard-Plugin-Settings",
			JSON.stringify(await applicationModuleSettings(c.env)),
		);
	}
	try {
		const response = await c.env.CUSTOM_WORKER.fetch(`https://custom.internal${path}`, {
			...init,
			headers,
...
function contextFailure(c: any, error: unknown): Response {
	...
	console.warn(
		JSON.stringify({
			...
			requestId: requestId(c),
			reason: error instanceof CustomSdkIdentityRejectedError ? error.message : "identity_rejected",
		}),
	);
	return failure(c, 401, "identity_invalid", "The application identity token is invalid");
}
...
function requestId(c: any): string {
	...
	const resolved = /^[A-Za-z0-9._:-]{1,128}$/.test(value) ? value : crypto.randomUUID();
