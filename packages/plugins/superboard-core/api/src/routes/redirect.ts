...
function isBotUserAgent(userAgent: string): boolean {
	return /bot|crawler|spider|slurp|facebookexternalhit|twitterbot|linkedinbot|whatsapp|telegrambot|discordbot|preview|headless|lighthouse/i.test(
		userAgent,
	);
}
...
async function quickLinkPath(db: D1Database) {
	for (let i = 0; i < 20; i += 1) {
		...
		const exists = await db
			.prepare("SELECT id FROM quick_links WHERE path = ? LIMIT 1")
			.bind(path)
			.first();
...
async function defaultDomainId(c: any) {
    ...
    LIMIT 1
  `)
		.bind(c.env.SHORTLINK_DOMAIN, `https://${c.env.SHORTLINK_DOMAIN}`, c.env.SHORTLINK_DOMAIN)
		.first<{ id: number }>();
...
async function marketingMessageResponse(c: any, id: string) {
	...
	const notification = await db
		.prepare("SELECT html FROM notifications WHERE id = ? AND COALESCE(archived, 0) = 0 LIMIT 1")
		.bind(id)
...
async function quickLinkResponse(c: any, path: string) {
	...
	const quickLink = await db
		.prepare("SELECT * FROM quick_links WHERE path = ? LIMIT 1")
		.bind(sanitizeQuickLinkPath(path))
		.first<any>();
	...
	const targetUrl =
		(isIOS && (quickLink.ios_phone || quickLink.ios_tablet)) ||
...
redirect.get("/:code", async (c) => {
	...
	const superboardRedirect = url.searchParams.get("superboard_redirect");
  ...
  `)
		...
		.first<{
			...
			show_preview_ios: number | null;
			show_preview_android: number | null;
			config_show_preview_ios: number | null;
			config_show_preview_android: number | null;
			generic_title: string | null;
			generic_subtitle: string | null;
  ...
  `)
	...
	if (device) {
		await c.env.DB.prepare(
			'UPDATE devices SET remote_ip = ?, updated_at = datetime("now") WHERE id = ?',
		)
			.bind(ip, device.id)
			.run();
	} else {
    ...
    `)
			.bind(device.id, link.id, device.id, link.id)
			.run();

		if (!goToFallback && !superboardRedirect && !isBot) {
			await c.env.DB.prepare(
				"INSERT INTO events (device_id, project_id, link_id, event, platform, ip) VALUES (?, ?, ?, 'view', ?, ?)",
			)
				.bind(device.id, link.project_id, link.id, platform, ip)
				.run();
		}
	}
	...
	const customRedirect = await c.env.DB.prepare(
		...
		.first<{ url: string | null; open_app_if_installed: number | null }>();
    ...
    LIMIT 1
  `)
		.bind(link.redirect_config_id, platform)
		.first<{ fallback_url: string | null; appstore: number | null }>();

	const linkData = parseLinkData(link.data);
	const deeplink = String(
		linkData.appLink || linkData.deeplink || `${link.uri_scheme || "superboard"}://${link.path}`,
	);
	...
	if (!targetUrl && platformRedirect?.appstore === 1) {
		targetUrl = isAndroid
			? await googlePlayUrl(c.env.DB, Number(link.instance_id))
			: "https://apps.apple.com";
	}

	// Show preview page for mobile if configured
	const showPreview =
		(isIOS && (link.show_preview_ios ?? link.config_show_preview_ios)) ||
...
async function googlePlayUrl(db: D1Database, instanceId: number): Promise<string> {
  ...
  `)
	...
	return row?.identifier
		? `https://play.google.com/store/apps/details?id=${encodeURIComponent(row.identifier)}`
...
function generateQuickLinkPage(link: any, accessPath: string): string {
	...
	const subtitle = escapeHtml(
		link.subtitle || "Dynamic links, attributions, and referrals across mobile and web platforms.",
	);
