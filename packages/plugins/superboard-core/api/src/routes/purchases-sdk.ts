...
async function context(c: any) {
  ...
  `)
		...
		.first()) as { id: number; is_test: number; purchases_enabled: number } | null;
...
function purchaseError(c: any, error: unknown) {
	...
	const status = /required|invalid|incomplete|not configured|not bound/i.test(message)
		? 422
		: /blocked|not enabled/i.test(message)
			? 403
			: /verification failed|identity token|issuer|JWKS|authenticate/i.test(message)
				? 401
				: 500;
...
purchases.get("/offerings", async (c) => {
	try {
		...
		if (platform !== "ios" && platform !== "android")
			return c.json({ error: "iOS or Android platform required" }, 422);
		const placement = c.req.query("placement") || "default";
		const data = await offeringsForCustomer(
			c.env.DB,
			ctx.projectId,
			platform,
			ctx.environment,
...
purchases.post("/identify", async (c) => {
	try {
		...
		const currentAppUserId = String(
			body.current_app_user_id || brandedHeader(c, "Anonymous-ID") || "",
		).trim();
...
purchases.post("/apple/transactions", async (c) => {
	try {
		...
		if (billingServiceEnabled(c.env)) {
			const result = await callBillingService<Record<string, any>>(
				c.env,
...
async function restore(c: any) {
	try {
		...
		const body = await jsonBody(c);
		if (billingServiceEnabled(c.env)) {
			const result = await callBillingService<{ data: Record<string, unknown> }>(
				c.env,
				"/internal/v1/receipts/restore",
				{
					project_id: String(ctx.projectId),
					customer_id: String(ctx.customer.id),
					environment: ctx.environment,
