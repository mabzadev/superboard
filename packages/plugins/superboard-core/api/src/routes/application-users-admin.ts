...
routes.get("/:projectRef/users", async (c) => {
	...
	return forward(
		c,
		`/internal/v1/admin/users${query.size ? `?${query.toString()}` : ""}`,
		authorized,
	);
});
...
for (const [method, suffix] of [
	...
	routes.on(method, `/:projectRef/profiles/:userId/${suffix}`, async (c) => {
		const authorized = await authorize(c);
		if (authorized instanceof Response) return authorized;
		const action = suffix.includes(":sessionId")
			? `sessions/${encodeURIComponent(c.req.param("sessionId"))}/revoke`
			: suffix;
		const query = new URLSearchParams();
		const cursor = c.req.query("cursor");
		if (method === "GET" && cursor) query.set("cursor", cursor);
		return forward(
			c,
			`/internal/v1/admin/profiles/${encodeURIComponent(c.req.param("userId"))}/${action}${query.size ? `?${query.toString()}` : ""}`,
			authorized,
		);
	});
}
