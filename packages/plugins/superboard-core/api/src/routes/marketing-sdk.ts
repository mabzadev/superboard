...
marketingSdk.all("/preferences", async (c) => {
	...
	if (!c.env.MARKETING_MODULE || !c.env.MODULE_INTERNAL_TOKEN?.trim()) {
		return failure(c, 503, "marketing_unavailable", "Marketing preferences are unavailable", true);
	}

	const idempotencyKey = c.req.header("Idempotency-Key")?.trim() || "";
	if (c.req.method === "PUT" && (!idempotencyKey || idempotencyKey.length > 255)) {
		return failure(c, 422, "idempotency_key_invalid", "A valid Idempotency-Key is required");
	}
	...
	if (c.req.method === "PUT") {
		try {
			...
			if (typeof input.consented !== "boolean") {
				return failure(c, 422, "consent_invalid", "consented must be a boolean");
			}
			body = {
	...
	const internalUrl = new URL("https://marketing.internal/internal/v1/application/preferences");
...
async function applicationIdentity(
	...
	try {
		response = await c.env.IDENTITY_SERVICE.fetch("https://identity.internal/auth/me", {
			headers: {
				authorization,
				accept: "application/json",
	...
	try {
		payload = await readJsonObjectLimited(response, 32_768, "Identity response is too large");
	} catch {
		return failure(
			c,
			503,
	...
	if (!id || user.email_verified !== true || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
		return failure(
			c,
			422,
...
function requestObject(value: unknown, field: string): Record<string, unknown> {
	...
	if (!value || typeof value !== "object" || Array.isArray(value)) {
		throw new MarketingInputError(`${field}_invalid`, `${field} must be a JSON object`);
	}
	return value as Record<string, unknown>;
}

function requestStringArray(value: unknown, field: string, maximum: number): string[] {
	if (value == null) return [];
	...
	if (values.some((item) => !/^[A-Za-z0-9][A-Za-z0-9._:-]{0,127}$/.test(item))) {
		throw new MarketingInputError(`${field}_invalid`, `${field} contains an invalid identifier`);
	}
