...
import {
	DateRangePicker,
	type Preset,
} from "@superboard/front-ui/components/dateRangePicker/DateRangePicker.js";
import dynamic from "@superboard/front-ui/next-dynamic";
import { Skeleton } from "@superboard/front-ui/ui/skeleton.js";

import DangerZoneSection from "../../../components/settings/DangerZoneSection.js";

const SettingsActiveUsersChart = dynamic(
	() => import("../../../components/settings/SettingsActiveUsersChart.js"),
	{
		ssr: false,
...
import { formatApiDate } from "@superboard/front-ui/lib/dateUtils.js";
import {
	showGenericError,
	showSuccessNotification,
} from "@superboard/front-ui/lib/Notifications.js";
import SessionStorage from "@superboard/front-ui/lib/SessionStorage.js";
import { deepClone } from "@superboard/front-ui/lib/utils.js";
import type { Instance, InstanceMember } from "@superboard/front-ui/types";
...
import TeamMebersSection from "../../../components/settings/TeamMebersSection.js";
import {
	useAddMemberMutation,
	useRemoveMemberMutation,
	useDeleteInstanceMutation,
	useExportUsageMutation,
} from "../../../hooks/mutations/useInstanceMutations.js";
import { useEventsPaymentQuery } from "../../../hooks/queries/useEventQueries.js";
import {
	useInstancesQuery,
	useInstanceMembersQuery,
} from "../../../hooks/queries/useInstanceQueries.js";
...
const SettingsPage = () => {
	...
	const eventsPaymentQuery = useEventsPaymentQuery(selectedInstance?.id, eventsPaymentParams);
	...
	const parsedMembers = useMemo((): InstanceMember[] => {
		...
		return clone.map(
			(member: { user: { id: string; name?: string; email: string }; role: string }) => ({
				id: member.user.id,
	...
	const handleExport = async (instanceId: string, _date: DateRange | undefined) => {
		const dataObj = {
			start_date: formatApiDate(dateRange!.from!),
			end_date: formatApiDate(dateRange!.to!),
		};
							...
							<div className="flex items-center gap-3 px-6 mb-5">
								...
								<div className="flex items-center gap-2 shrink-0">
									...
									<Button
										variant="outline"
										size="sm"
										onClick={() => selectedInstance && handleExport(selectedInstance.id, dateRange)}
									>
