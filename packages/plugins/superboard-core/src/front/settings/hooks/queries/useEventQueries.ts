...
import type { AppEvent, DateRangeQuery } from "@superboard/front-ui/types";
import type {
	EventsSearchPayload,
	EventsSortedPayload,
	DownloadCSVPayload,
...
} from "../../api/events/eventsService.js";
...
export function useEventsSearchQuery(
	...
	return useQuery<EventsSearchResult>({
		...
		queryFn: async () => {
			const response = await getEventsForSearchParamsAPICall(projectId!, params!);
			return {
...
export function useEventsSortedQuery(
	...
	return useQuery({
		...
		queryFn: async () => {
			const response = await getEventsSortedByParamsAPICall(projectId!, params!);
...
export function useEventsOverviewQuery(
	...
	return useQuery({
		queryKey: queryKeys.projects.eventsOverview(projectId!, params ?? undefined),
		queryFn: async () => {
...
export function useEventsPaymentQuery(
	...
	return useQuery({
		...
		queryFn: async () => {
			const response = await getEventsForPaymentScreenAPICall(projectId!, params!);
			return response.data.metrics_values;
		},
		enabled: !!projectId && !!params,
	});
...
export async function downloadLinksAsCSV(projectId: string, data: DownloadCSVPayload) {
