...
import { Card, CardContent, CardFooter, CardHeader } from "@superboard/front-ui/ui/card.js";
import {
	ExternalLink,
	X,
	FolderOpen,
...
function McpAuthorizeContent() {
	const { operator: user, locale } = useFrontContext();
	...
	const [approvedCallbackUrl, setApprovedCallbackUrl] = useState<string | null>(null);
	...
	useEffect(() => {
		...
		setError(null);
		void GET(
			`/api/v1/mcp/consent-request?${new URLSearchParams({ client_id: clientId, redirect_uri: redirectUri })}`,
		)
			.then((response) => {
				if (!cancelled) setVerifiedClient(response.data);
			})
			.catch(() => {
				if (!cancelled)
					setError(
						locale === "fr"
							? "Application ou adresse de retour non enregistrée."
							: "The application or return address is not registered.",
					);
			});
		return () => {
			cancelled = true;
	...
	const handleApprove = useCallback(async () => {
		...
		try {
			const response = await approveConsentAPICall({
				client_id: verifiedClient.client_id,
				redirect_uri: verifiedClient.redirect_uri,
			...
			const callbackUrl = new URL(verifiedClient.redirect_uri);
	...
	}, [verifiedClient, codeChallenge, codeChallengeMethod, state, scope]);
	...
	}, [verifiedClient, state]);

	const clientDisplayName =
		verifiedClient?.name ||
		(locale === "fr" ? "Vérification de l’application…" : "Checking application…");
				...
				<Card className="w-full max-w-[420px] shadow-lg">
					<CardHeader className="flex flex-col items-center gap-3 pb-2">
						...
						<div className="flex flex-col items-center gap-1 text-center">
							<h2 className="text-lg font-semibold">Authorization Successful</h2>
							<p className="text-sm text-muted-foreground">
								You&apos;ve granted{" "}
								<span className="font-medium text-foreground">{clientDisplayName}</span> access to
								your SuperBoard account.
							</p>
						</div>
					</CardHeader>
					...
					<CardContent className="flex flex-col gap-3">
						<div className="rounded-lg bg-muted/50 border border-sidebar-border p-4 text-sm text-muted-foreground">
							<p>
								<span className="font-medium text-foreground">{clientDisplayName}</span> should open
								automatically. If it doesn&apos;t, click the button below to return.
							</p>
						</div>
					</CardContent>

					<CardFooter className="flex flex-col gap-2">
						<Button variant="default" className="w-full" onClick={handleReturnToClient}>
			...
			<Card className="w-full max-w-[420px] shadow-lg">
				<CardHeader className="flex flex-col items-center gap-3 pb-2">
					...
					<div className="flex flex-col items-center gap-1 text-center">
						...
						<p className="text-sm text-muted-foreground">
							<span className="font-medium text-foreground">{clientDisplayName}</span> wants to
							access your SuperBoard account.
						</p>
					</div>
				</CardHeader>
					...
					<div className="flex flex-col gap-2">
						...
						<div className="rounded-xl border border-sidebar-border overflow-hidden divide-y divide-sidebar-border">
							<div className="flex items-center gap-3 px-4 py-3">
								...
								<span className="text-sm">View your projects and instances</span>
							</div>
							<div className="flex items-center gap-3 px-4 py-3">
							...
							<div className="flex items-center gap-3 px-4 py-3">
								...
								<span className="text-sm">Configure redirects and SDK settings</span>
							</div>
						</div>
					</div>
				...
				<CardFooter className="flex gap-3">
					<Button
						...
						className="flex-1"
						onClick={handleDeny}
						disabled={isSubmitting || !verifiedClient}
					>
						<X className="h-4 w-4" />
						Deny
					</Button>
