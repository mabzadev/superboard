...
import type { User, EditUserPayload } from "@superboard/front-ui/types";
...
export const resetPasswordAPICall = async (email: string): Promise<AxiosResponse> => {
	const data = {
		email: email,
	};
...
export const currentUserAPICall = async (): Promise<AxiosResponse<UserResponse>> => {
...
export const removeUserAPICall = async (email: string): Promise<AxiosResponse> => {
