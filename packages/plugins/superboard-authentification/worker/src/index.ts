...
import { EMAIL_SERVICE_SEND_PATH, type EmailServiceMessage } from "@superboard/contracts/email";
...
import { RequestBodyError, readJsonObjectLimited } from "@superboard/contracts/request-body";
...
import { userAdministration } from "./user-administration.js";
...
async function proxyMelodyAdmin(c: IdentityContext): Promise<Response> {
	...
	if (!new Set(["owner", "admin"]).has(project.role)) {
		return error("administrator_required", "Owner or administrator access is required", 403);
	}
	const source = new URL(c.req.url);
	const prefix = "/internal/v1/melody-admin";
	const suffix = source.pathname.slice(prefix.length) || "/info";
	if (suffix === "/api/v1/configuration")
		return authenticationSettingsResponse(c.req.raw, c.env, project.operatorId);
	if (!(suffix === "/info" || suffix.startsWith("/api/v1/"))) {
		return error("identity_admin_path_invalid", "Identity administration path is not allowed", 404);
	}
	...
	if (
		...
		return error("identity_resource_not_found", "Identity resource was not found", 404);
	}
...
const identityAdminResourceDefinitions = [
	{
		...
		type: "user_attribute",
		listKey: "userAttributes",
		itemKey: "userAttribute",
	},
	{
		prefix: "/api/v1/app-banners",
		type: "app_banner",
		listKey: "appBanners",
		itemKey: "appBanner",
	},
...
async function identityAdminResourceAllowed(
	...
	const row = await db
     ...
     LIMIT 1`,
		...
		.bind(realm, projectId, type, resourceId)
		.first();
	return Boolean(row);
}
...
async function identitySystemResource(
	...
	if (type === "role") {
		return Boolean(
			await db
				.prepare(`SELECT id FROM role WHERE id=? AND name='super_admin' AND "deletedAt" IS NULL`)
				.bind(resourceId)
				.first(),
		);
	}
...
async function allowedIdentityAdminResourceIds(
	...
	await Promise.all(
		candidates.map(async (id) => {
			if (await identityAdminResourceAllowed(db, realm, projectId, type, id)) allowed.add(id);
		}),
	);
	return allowed;
}
...
async function filterIdentityAdminCollection(
	...
	return identityAdminJsonResponse(upstream, payload);
}
	...
	await db
     ...
     ON CONFLICT(realm,resource_type,resource_id) DO UPDATE SET
       ...
       updated_at=CURRENT_TIMESTAMP`,
		...
		.bind(realm, resource.type, resourceId, projectId)
		.run();
	return identityAdminJsonResponse(upstream, payload);
}

function identityAdminJsonResponse(upstream: Response, payload: Record<string, unknown>): Response {
...
async function operatorUserAdministration(c: IdentityContext) {
	const project = await internalProject(c);
	if (project instanceof Response) return project;
	return userAdministration(c.req.raw, c.env.DB, project, c.env.IDENTITY_REALM);
}
app.get("/internal/v1/admin/members", operatorUserAdministration);
app.all("/internal/v1/admin/profiles/*", operatorUserAdministration);

app.all("/internal/v1/application-plugin/:kind/:id", async (c) => {
	const operation = applicationUserOperation(c.req.param("kind"), c.req.param("id"));
	if (!operation || operation.method !== c.req.method)
		return error("application_operation_not_found", "Application operation was not found", 404);
	const project = await internalProject(c);
	if (project instanceof Response) return project;
	if (project.role !== "sdk" || project.operatorId)
		return error("application_client_required", "Application client credentials are required", 403);
	const body = operation.method === "POST" ? await bodyObject(c.req.raw) : {};
	if (operation.id !== "application_sign_in") {
		try {
			const claims = await verifyApplicationToken(c.env, bearer(c.req.raw) ?? "");
			if (Number(claims.pid) !== project.projectId)
				return error(
					"application_project_forbidden",
					"Application session belongs to another project",
					403,
				);
		} catch {
			return error("unauthorized", "Application authentication is invalid", 401);
		}
	}
	let path: string;
	let method: string = operation.method;
	if (operation.id === "application_sign_in") path = "/auth/signin/password";
	else if (operation.id === "link_provider") {
		if (body.provider !== "apple" && body.provider !== "google")
			return error("provider_invalid", "Provider is not supported", 422);
		path = `/auth/link/${body.provider}`;
	} else if (operation.id === "revoke_application_session") {
		const session = typeof body.session_id === "string" ? internalUserId(body.session_id) : null;
		if (!session) return error("session_id_invalid", "Session identifier is invalid", 422);
		path = `/auth/me/sessions/${encodeURIComponent(session)}`;
		method = "DELETE";
	} else path = operation.id === "linked_providers" ? "/auth/me/providers" : "/auth/me/sessions";
	const headers = await createProjectContextHeaders(
		{ ...project, method, pathname: path },
...
app.get("/internal/v1/admin/users", async (c) => {
	...
	const offset = boundedInteger(c.req.query("offset"), 0, 0, 1_000_000, "offset");
	const filter = query
		? `AND (
...
app.get("/internal/v1/admin/users/:userId", async (c) => {
	...
	if (!userId) {
		return error("application_user_id_invalid", "Application user identifier is invalid", 422);
	}
	...
	if (!user) {
		return error("application_user_not_found", "Application user was not found", 404);
	}
	return response({
		data: {
			...adminUser(user),
...
app.delete("/internal/v1/users/:userId", async (c) => {
	...
	if (!(await eraseIdentityUser(c.env, project.projectId, userId))) {
		return error("account_files_cleanup_failed", "Account files could not be deleted", 503, true);
	}
...
app.post("/auth/register", async (c) => {
	...
	if (requiresHostedSignIn(c.env))
		return error(
			"interactive_sign_in_required",
			"Complete verification through hosted sign-in",
			403,
		);
	...
	try {
		await c.env.DB.prepare(
			...
			.bind(id, projectId, email, await hash(password, 12), optionalText(body.name, 120))
			.run();
	} catch (cause) {
		if (String(cause).toLowerCase().includes("unique"))
			return error("email_already_registered", "Email is already registered", 409);
		throw cause;
	}
	const mailDelivered = await issueEmailToken(c.env, projectId, id, email, "verify_email").catch(
		(cause) => {
			console.error(
				JSON.stringify({
...
app.post("/auth/signin/password", async (c) => {
	const project = await internalProject(c);
	if (project instanceof Response) return project;
	if (c.env.ENABLE_PASSWORD_SIGN_IN === false)
		return error("password_sign_in_disabled", "Password sign-in is disabled", 403);
	if (requiresHostedSignIn(c.env))
		return error(
			"interactive_sign_in_required",
			"Complete verification through hosted sign-in",
...
app.post("/auth/anonymous", async (c) => {
	...
	await enforceRateLimit(c.env.DB, c.req.raw, `p${projectId}:anonymous`, installationId);
	...
	if (!user) {
		...
		user = await c.env.DB.prepare("SELECT * FROM application_users WHERE project_id=? AND id=?")
			.bind(projectId, id)
			.first<IdentityUser>();
	}
	return response({
...
app.post("/auth/signin/:provider", async (c) => {
	...
	if (!user) {
		if (c.env.REGISTRATION_MODE !== "open")
			return error("registration_closed", "Registration is closed", 403);
		const email = typeof claims.email === "string" ? normalizedEmail(claims.email) : null;
		...
		statements.push(
			...
			).bind(crypto.randomUUID(), projectId, userId, provider, subjectHash, email),
		);
...
app.post("/auth/refresh", async (c) => {
	...
	await enforceRateLimit(c.env.DB, c.req.raw, "refresh", refreshToken.slice(0, 24));
	const tokenHash = await sha256(refreshToken);
	const nextToken = randomToken("ogr_");
	const refreshTtl = ttl(c.env.REFRESH_TOKEN_TTL, 3600, 31_536_000, "REFRESH_TOKEN_TTL");
	...
	if (!rotated) return error("refresh_token_invalid", "Refresh token is invalid or expired", 401);
	return response(
		await sessionResponse(c.env, rotated.project_id, rotated.user_id, rotated.id, nextToken),
	);
});
...
app.post("/auth/request-password-reset", async (c) => {
	const project = await internalProject(c);
	if (project instanceof Response) return project;
	if (c.env.ENABLE_PASSWORD_RESET === false)
		return error("password_reset_disabled", "Password reset is disabled", 403);
	const projectId = project.projectId;
	const body = await bodyObject(c.req.raw);
	const email = normalizedEmail(body.email);
	...
	if (user)
		await issueEmailToken(c.env, projectId, user.id, user.email, "reset_password").catch(
			(cause) => {
				console.error(
					JSON.stringify({
...
app.get("/auth/reset-password", async (c) => {
	...
	if (!token || token.length > 512)
		return error("reset_token_invalid", "Reset token is invalid or expired", 422);
...
app.post("/auth/reset-password", async (c) => {
	...
	if (!row) return error("reset_token_invalid", "Reset token is invalid or expired", 422);
	await c.env.DB.batch([
		c.env.DB.prepare(
...
app.get("/auth/verify-email", async (c) => {
	...
	if (!row)
		return error("verification_token_invalid", "Verification token is invalid or expired", 422);
	await c.env.DB.prepare(
...
async function authenticate(c: IdentityContext, next: Next) {
	...
	try {
		...
		const active = await c.env.DB.prepare(
			"SELECT id FROM application_sessions WHERE project_id=? AND id=? AND user_id=? AND revoked_at IS NULL AND datetime(expires_at)>datetime('now') AND NOT EXISTS (SELECT 1 FROM application_user_suspensions suspended WHERE suspended.project_id=application_sessions.project_id AND suspended.user_id=application_sessions.user_id)",
		)
...
app.use("/auth/superboard-token", authenticate);
...
app.post("/auth/link/:provider", async (c) => {
	...
	if (provider !== "google" && provider !== "apple")
		return error("provider_invalid", "Provider is not supported", 404);
	const body = await bodyObject(c.req.raw);
	if (body.user_id !== undefined && body.user_id !== c.get("userId"))
		return error(
			"application_user_subject_mismatch",
			"Application user does not match the session",
			403,
		);
	const token = requiredText(body.token ?? body.id_token, "token", 16_384);
	await enforceRateLimit(
	...
	const providerEmail = typeof claims.email === "string" ? normalizedEmail(claims.email) : null;
	const emailVerified = claims.email_verified === true || claims.email_verified === "true";
	const emailConflict =
		providerEmail && emailVerified
			? await c.env.DB.prepare(
					...
					.first()
			: null;
	const adoptableEmail = providerEmail && emailVerified && !emailConflict ? providerEmail : null;

	try {
		await c.env.DB.batch([
			c.env.DB.prepare(
				"INSERT INTO application_identities (id,project_id,user_id,provider,subject_hash,provider_email) VALUES (?,?,?,?,?,?)",
			).bind(crypto.randomUUID(), projectId, userId, provider, subjectHash, providerEmail),
			c.env.DB.prepare(
...
app.get("/auth/me/providers", async (c) => {
	const rows = await c.env.DB.prepare(
		"SELECT DISTINCT provider FROM application_identities WHERE project_id=? AND user_id=? AND provider IN ('apple','google') ORDER BY provider",
	)
		.bind(c.get("projectId"), c.get("userId"))
		.all<{ provider: string }>();
	return response({ providers: rows.results.map((row) => row.provider) });
});
app.get("/auth/me/sessions", async (c) => {
	const rows = await c.env.DB.prepare(
		"SELECT id FROM application_sessions WHERE project_id=? AND user_id=? AND revoked_at IS NULL AND datetime(expires_at)>datetime('now') ORDER BY created_at DESC,id LIMIT 100",
	)
		.bind(c.get("projectId"), c.get("userId"))
		.all<{ id: string }>();
	return response({ session_ids: rows.results.map((row) => row.id) });
});
app.delete("/auth/me/sessions/:sessionId", async (c) => {
	const row = await c.env.DB.prepare(
		"UPDATE application_sessions SET revoked_at=COALESCE(revoked_at,CURRENT_TIMESTAMP) WHERE project_id=? AND user_id=? AND id=? RETURNING id,expires_at",
	)
		.bind(c.get("projectId"), c.get("userId"), c.req.param("sessionId"))
		.first<{ id: string; expires_at: string }>();
	return row
		? response({ session_id: row.id, expires_at: row.expires_at })
		: error("session_not_found", "Session was not found", 404);
});
...
async function eraseIdentityUser(
	...
	await env.DB.batch([
		...
		env.DB.prepare("DELETE FROM application_identities WHERE project_id=? AND user_id=?").bind(
			projectId,
			userId,
		),
		env.DB.prepare("DELETE FROM application_identity_tokens WHERE project_id=? AND user_id=?").bind(
			projectId,
			userId,
		),
	]);
...
function internalUserId(value: string): string | null {
	...
	return /^[A-Za-z0-9][A-Za-z0-9._:-]{0,254}$/u.test(resolved) ? resolved : null;
}
...
function optionalQuery(value: string | undefined, maximum: number): string {
	...
	if (query.length > maximum) {
		throw new IdentityInputError("query_too_long", `Query exceeds ${maximum} characters`, 422);
	}
	return query;
}
...
function adminUser(user: IdentityAdminUserRow) {
	const providers = (user.providers || "")
		...
		.filter(Boolean);
	const methods = [...(user.password_configured ? ["password"] : []), ...providers];
	return {
		id: user.id,
		email: user.email,
		name: user.name,
...
app.post("/auth/superboard-token", async (c) =>
	response(await issueSuperBoardToken(c.env, c.get("projectId"), c.get("userId"))),
);

async function createSession(env: IdentityEnv, projectId: number, userId: string) {
	const session = await newSession(env, projectId, userId, null);
	const inserted = await session.statement.run();
	if (!inserted.meta.changes)
		throw new IdentityInputError(
			"application_user_suspended",
			"Application user is suspended",
			403,
		);
	return sessionResponse(env, projectId, userId, session.id, session.refreshToken);
}
...
async function newSession(
	...
	const refreshTtl = ttl(env.REFRESH_TOKEN_TTL, 3600, 31_536_000, "REFRESH_TOKEN_TTL");
	const expiresAt = new Date(Date.now() + refreshTtl * 1000).toISOString();
	return {
		id,
		refreshToken,
		statement: env.DB.prepare(
			"INSERT INTO application_sessions (id,project_id,user_id,refresh_token_hash,expires_at,rotated_from_id) SELECT ?,?,?,?,?,? WHERE NOT EXISTS (SELECT 1 FROM application_user_suspensions WHERE project_id=? AND user_id=?)",
		).bind(
			id,
			projectId,
			userId,
...
async function sessionResponse(
	...
	const current = await env.DB.prepare(
		"SELECT expires_at FROM application_sessions WHERE project_id=? AND id=? AND user_id=? AND revoked_at IS NULL AND NOT EXISTS (SELECT 1 FROM application_user_suspensions WHERE project_id=? AND user_id=?)",
	)
		.bind(projectId, sessionId, userId, projectId, userId)
		.first<{ expires_at: string }>();
	if (!current) throw new IdentityInputError("session_inactive", "Session is inactive", 401);
	return {
...
async function issueEmailToken(
	...
	const message: EmailServiceMessage = {
		...
		text:
			purpose === "verify_email" ? `Verify your email: ${link}` : `Reset your password: ${link}`,
		templateKey: `identity.${purpose}`,
		metadata: { purpose },
	};
	const result = await env.EMAIL_SERVICE.fetch(`https://email.internal${EMAIL_SERVICE_SEND_PATH}`, {
		method: "POST",
		headers: {
...
function normalizedEmail(value: unknown): string {
	...
	if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email) || email.length > 320) {
		throw new IdentityInputError("email_invalid", "A valid email is required", 422);
	}
	return email;
}
...
function optionalText(value: unknown, maximum: number): string | null {
	...
	if (text.length > maximum)
		throw new IdentityInputError("value_too_long", `Value exceeds ${maximum} characters`, 422);
	return text || null;
}
...
async function publicUser(db: D1Database, projectId: number, id: string) {
	const user = await db
		.prepare("SELECT * FROM application_users WHERE project_id=? AND id=? AND deleted_at IS NULL")
		.bind(projectId, id)
		.first<IdentityUser>();
...
app.onError((cause, c) => {
	if (cause instanceof IdentityInputError) return error(cause.code, cause.message, cause.status);
	console.error(
		JSON.stringify({
...
const integratedWorker = {
	async fetch(request: Request, env: IdentityEnv, executionContext: ExecutionContext) {
		const url = new URL(request.url);
		const gatewayRequest = request.headers.get("x-superboard-auth-gateway") === "1";

		if (gatewayRequest && (url.pathname === "/client.js" || url.pathname === "/client.css")) {
		...
		const hosted = gatewayRequest && !legacyIdentityPath(url.pathname);
		const needsConfiguration =
			(hosted && !url.pathname.startsWith("/.well-known/")) ||
			[
				"/auth/register",
				"/auth/anonymous",
			...
			url.pathname.startsWith("/internal/v1/application-plugin/command/") ||
			(url.pathname.startsWith("/internal/v1/melody-admin") &&
				!url.pathname.endsWith("/api/v1/configuration"));
		try {
			const configured = needsConfiguration ? await configuredIdentityEnvironment(env) : env;
			return hosted
				? melodyApp.fetch(
						request,
						melodyBindings(integratedIdentityEnv(configured)),
						executionContext,
					)
				: app.fetch(request, configured, executionContext);
		} catch {
			return error(
