...
import { adapterConfig, errorConfig, messageConfig, routeConfig, typeConfig } from "../configs";
...
export const createSp = async (c: Context<typeConfig.Context>) => {
	...
	return {
		getMetadata: () => spMetadata(getSpEntityId(c), getSpAcsUrl(c), certificate),
		...
		parseLoginResponse: async (
			...
			const extract = await verifyAndExtract(xml, idp, privateKey);
			return { extract };
		},
	};
};

export const loadIdp = async (c: Context<typeConfig.Context>, name: string, appId?: number) => {
	const idpRecord = await samlIdpModel.getByName(c.env.DB, name);
	if (!idpRecord || !idpRecord.isActive) {
		loggerUtil.triggerLogger(c, loggerUtil.LoggerLevel.Warn, messageConfig.RequestError.NoSamlIdp);
	...
	if (appId) {
       ...
       WHERE realm=? AND melody_app_id=? LIMIT 1`,
			...
			.bind(c.env.IDENTITY_REALM, appId)
		...
		if (appRealm?.project_id) {
         ...
         WHERE realm=? AND project_id=? AND resource_type='saml_idp'
           AND resource_id=? LIMIT 1`,
				...
				.bind(c.env.IDENTITY_REALM, appRealm.project_id, idpRecord.id)
				.first();
...
function spMetadata(entityId: string, acsUrl: string, certificate: string): string {
	const x509 = certificate.replace(/-----BEGIN CERTIFICATE-----|-----END CERTIFICATE-----|\s/g, "");
	return [
...
function parseIdentityProviderMetadata(metadata: string): IdentityProvider {
	...
	const descriptor = allElements(doc, "EntityDescriptor").find(
		(candidate) => allElements(candidate, "IDPSSODescriptor").length > 0,
	);
	...
	const service =
		services.find((candidate) => candidate.getAttribute("Binding") === REDIRECT_BINDING) ??
		services[0];
...
async function verifyAndExtract(
	...
	let authenticated = verifySignedDocuments(xml, original, idp.signingCertificates);
	if (authenticated.length === 0) throw new Error("saml_signature_required");

	const signedResponse =
		authenticated.find((doc) => localName(documentRoot(doc)) === "Response") ?? null;
	let signedAssertion =
		authenticated.find((doc) => localName(documentRoot(doc)) === "Assertion") ?? null;
	...
	if (!signedAssertion) {
		const decrypted = await decryptAssertionFromDocument(original, privateKey);
		if (decrypted) {
	...
	const inResponseTo =
		subjectConfirmation?.getAttribute("InResponseTo")?.trim() ??
		(signedResponse ? documentRoot(signedResponse).getAttribute("InResponseTo")?.trim() : "") ??
		"";
	const destination =
		subjectConfirmation?.getAttribute("Recipient")?.trim() ??
...
function verifySignedDocuments(
	...
	const signatures = allElements(doc, "Signature").filter(
		(node) => node.namespaceURI === XML_SIGNATURE_NAMESPACE,
	);
...
async function assertionFromAuthenticatedResponse(
	...
	const decrypted = await decryptAssertionFromDocument(response, privateKey);
...
async function decryptAssertionFromDocument(
	...
	const contentAlgorithm =
		allElements(encryptedData, "EncryptionMethod")[0]?.getAttribute("Algorithm") ?? "";
	const encryptedKey =
		allElements(encryptedData, "EncryptedKey")[0] ??
		allElements(encryptedAssertions[0], "EncryptedKey")[0];
	const keyAlgorithm = encryptedKey
		? (allElements(encryptedKey, "EncryptionMethod")[0]?.getAttribute("Algorithm") ?? "")
		: "";
	if (
		!ALLOWED_CONTENT_ENCRYPTION.has(contentAlgorithm) ||
		!ALLOWED_KEY_ENCRYPTION.has(keyAlgorithm)
	) {
	...
	if (localName(documentRoot(assertion)) !== "Assertion")
		throw new Error("saml_decrypted_assertion_invalid");
	return assertion;
}
...
function assertStrongSignatureAlgorithms(signature: XmlElement): void {
	const signatureMethod =
		allElements(signature, "SignatureMethod")[0]?.getAttribute("Algorithm") ?? "";
	if (!ALLOWED_SIGNATURE_METHODS.has(signatureMethod))
		throw new Error("saml_signature_algorithm_invalid");
	const digests = allElements(signature, "DigestMethod");
	if (
		digests.length === 0 ||
		digests.some((digest) => !ALLOWED_DIGEST_METHODS.has(digest.getAttribute("Algorithm") ?? ""))
	) {
...
function validateTimeWindow(notBefore: string, notOnOrAfter: string): void {
	...
	if (notBefore) {
		...
		if (!Number.isFinite(timestamp) || now + drift < timestamp)
			throw new Error("saml_assertion_not_active");
	}
...
function firstText(root: XmlNode, name: string): string {
	return allElements(root, name)[0]?.textContent?.trim() ?? "";
}
...
function base64ToBytes(value: string): Uint8Array {
	if (!/^[A-Za-z0-9+/]*={0,2}$/.test(value.replace(/\s/g, "")))
		throw new Error("saml_base64_invalid");
