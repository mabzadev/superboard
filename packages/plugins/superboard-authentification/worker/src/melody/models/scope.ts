...
export const getByName = async (db: D1Database, name: string): Promise<Record | null> => {
	const query = `SELECT * FROM ${TableName} WHERE name = $1 AND "deletedAt" IS NULL`;

	const stmt = db.prepare(query).bind(name);
	const scope = (await stmt.first()) as Record | null;
	return scope;
};
...
export const create = async (db: D1Database, create: Create): Promise<Record> => {
	...
	const stmt = db.prepare(query).bind(create.name, create.type, create.note);
...
export const update = async (db: D1Database, id: number, update: Update): Promise<Record> => {
	const updateKeys: (keyof Update)[] = ["name", "deletedAt", "updatedAt", "note"];
