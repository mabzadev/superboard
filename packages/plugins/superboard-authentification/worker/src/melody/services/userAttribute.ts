...
import { userAttributeService } from ".";
import { typeConfig, errorConfig, messageConfig } from "../configs";
...
export const getUserSignUpAttributes = async (
	c: Context<typeConfig.Context>,
): Promise<userAttributeModel.Record[]> => {
	const userAttributes = await userAttributeModel.getAll(c.env.DB);
...
export const createUserAttribute = async (
	...
	const userAttribute = await userAttributeModel.create(c.env.DB, {
		name: dto.name,
...
export const updateUserAttribute = async (
	...
	const userAttribute = await userAttributeModel.update(c.env.DB, id, {
		name: dto.name,
		...
		for (const attr of userAttributes) {
			if (attr.requiredInSignUpForm && !reqBody.attributes[String(attr.id)]) {
				throw new errorConfig.Forbidden(
					`${messageConfig.RequestError.AttributeIsRequired}: ${attr.name}`,
				);
			}
