...
import { signInLogModel, userAttributeModel, userAttributeValueModel, userModel } from "../models";
...
export const handleAuthCodeTokenExchange = async (
	...
	const { ENABLE_SIGN_IN_LOG: enableSignInLog, ENABLE_USER_ATTRIBUTE: enableUserAttribute } =
		env(c);

	await identityService.ensureAuthCodeIsSecured(c, bodyDto.code, authInfo, options);
	...
	const userRoles = await roleService.getUserRoles(c, authInfo.user.id);
	const authId = await identityBridgeService.canonicalSubject(c, authInfo.appId, authInfo.user);
	const scope = authorizedScopes.join(" ");
	const currentTimestamp = timeUtil.getCurrentTimestamp();

	const { accessToken, accessTokenExpiresIn, accessTokenExpiresAt } =
	...
	await userService.increaseLoginCount(c, authInfo.user.id);

	if (enableSignInLog) {
		const ip = requestUtil.getRequestIP(c);
		const detail = requestGeolocation(c.req.raw);
		await signInLogModel.create(c.env.DB, {
			userId: authInfo.user.id,
			ip: ip ?? null,
			detail,
			projectId: await resolveProjectId(c, { appId: authInfo.appId, userId: authInfo.user.id }),
		});
	}
...
export const handleRefreshTokenTokenExchange = async (
	...
	const refreshTokenBody = await kvService.getRefreshTokenBody(c, bodyDto.refreshToken);

	const user = refreshTokenBody.melodyUserId
		? await userModel.getById(c.env.DB, refreshTokenBody.melodyUserId)
...
export const handleInvalidRefreshToken = async (
	...
	const refreshTokenBody = await kvService.getRefreshTokenBody(c, refreshToken);
	...
	await kvService.invalidRefreshToken(c.env.KV, refreshToken);
};
