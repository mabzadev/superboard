import { ClientType, Scope } from "@melody-auth/shared";
import { Context } from "hono";
import { env } from "hono/adapter";

import { identityBridgeService, scopeService } from ".";
import { errorConfig, messageConfig, typeConfig, variableConfig } from "../configs";
import { appDto } from "../dtos";
import { appModel, appScopeModel, scopeModel } from "../models";
import { requestUtil, timeUtil, loggerUtil } from "../utils";
...
export const verifySPAClientRequest = async (
	...
	const app = await appModel.getByClientId(c.env.DB, clientId);
...
export const getApps = async (c: Context<typeConfig.Context>): Promise<appModel.Record[]> => {
	const apps = await identityBridgeService.getCurrentProjectApps(c);
...
export const getAppById = async (
	...
	const app = await appModel.getById(c.env.DB, id);

	if (!app) throw new errorConfig.NotFound();
	if (!(await identityBridgeService.appBelongsToCurrentProject(c, app.id)))
		throw new errorConfig.NotFound();

	const scopes = await scopeService.getAppScopes(c, app.id);
...
const verifyCanAssignRootScope = (c: Context<typeConfig.Context>, scopes: string[]) => {
	const includesPrivilegedScope = variableConfig.S2sConfig.privilegedScopes.some((scope) =>
...
export const createApp = async (
	...
	verifyCanAssignRootScope(c, dto.scopes);

	const app = await appModel.create(c.env.DB, {
		name: dto.name,
		type: dto.type,
	...
	for (const scope of targetScopes) {
		await appScopeModel.create(c.env.DB, {
			appId: app.id,
			scopeId: scope.id,
		});
	}

	await identityBridgeService.bindAppToCurrentProject(c, app.id);
...
export const updateApp = async (
	...
	if (dto.useSystemMfaConfig !== undefined)
		updateDto.useSystemMfaConfig = dto.useSystemMfaConfig ? 1 : 0;
	...
	if (dto.allowEmailMfaAsBackup !== undefined)
		updateDto.allowEmailMfaAsBackup = dto.allowEmailMfaAsBackup ? 1 : 0;

	const app = await appModel.getById(c.env.DB, appId);
	...
	const updatedApp = Object.values(updateDto).some((val) => val !== undefined)
		? await appModel.update(c.env.DB, appId, updateDto)
		: app;

	const appScopes = await appScopeModel.getAllByAppId(c.env.DB, app.id);

	if (dto.scopes) {
		const scopesToDelete = appScopes.filter(
			(appScope) => !dto.scopes?.includes(appScope.scopeName),
		);

		const allScopes = await scopeModel.getAll(c.env.DB);
		const scopeNamesToCreate = dto.scopes.filter((scope) =>
			appScopes.every((appScope) => appScope.scopeName !== scope),
		);
		...
		for (const scopeToDelete of scopesToDelete) {
			await appScopeModel.update(c.env.DB, scopeToDelete.id, { deletedAt: currentTime });
		}
		for (const scopeToCreate of scopesToCreate) {
			await appScopeModel.create(c.env.DB, {
				appId: app.id,
				scopeId: scopeToCreate.id,
			});
		}
	}
...
export const deleteApp = async (c: Context<typeConfig.Context>, appId: number): Promise<true> => {
	if (!(await identityBridgeService.appBelongsToCurrentProject(c, appId)))
		throw new errorConfig.NotFound();
	await appModel.remove(c.env.DB, appId);
	return true;
};
