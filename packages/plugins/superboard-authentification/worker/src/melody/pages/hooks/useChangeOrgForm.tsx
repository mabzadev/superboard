...
const useChangeOrgForm = ({ locale, onSubmitError }: UseChangeOrgFormProps) => {
	...
	const handleSwitchOrg = useCallback(
		(orgSlug: string) => {
			setIsSwitching(true);
			fetch(routeConfig.IdentityRoute.ChangeOrg, {
				method: "POST",
				headers: {
		...
		[onSubmitError, locale, followUpParams],
	);

	const getUserOrgsInfo = useCallback(() => {
		fetch(`${routeConfig.IdentityRoute.ChangeOrg}${qs}`, {
			method: "GET",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json",
