...
const useMfaEnrollForm = ({ locale, onSubmitError, onSwitchView }: UseMfaEnrollFormProps) => {
	...
	const getMfaEnrollInfo = useCallback(() => {
		fetch(`${routeConfig.IdentityRoute.ProcessMfaEnroll}${qs}`, {
			method: "GET",
			headers: {
	...
	const handleEnroll = useCallback(
		(mfaType: userModel.MfaType) => {
			setIsEnrolling(true);
			fetch(routeConfig.IdentityRoute.ProcessMfaEnroll, {
				method: "POST",
				headers: {
					Accept: "application/json",
					"Content-Type": "application/json",
