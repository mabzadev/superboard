...
import { codeField, emailField, validate } from "../tools/form";
import { getFollowUpParams } from "../tools/param";
import { parseAuthorizeFollowUpValues, parseResponse } from "../tools/request";
...
const useChangeEmailForm = ({ locale, onSubmitError }: UseChangeEmailFormProps) => {
	...
	const errors = validate(changeEmailSchema, values);

	const handleChange = (name: "email" | "mfaCode", value: string | string[]) => {
		onSubmitError(null);
		switch (name) {
			case "email":
	...
	const sendCode = useCallback(() => {
		return fetch(routeConfig.IdentityRoute.ChangeEmailCode, {
			method: "POST",
			headers: {
