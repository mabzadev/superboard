...
import { GetProcessOtpMfaRes, GetOtpMfaSetupRes } from "../../handlers/identity/mfa";
...
const useOtpMfaForm = ({ locale, onSubmitError, onSwitchView }: UseOtpMfaFormProps) => {
	...
	const [mfaCode, setMfaCode] = useState<string[]>(Array.from({ length: 6 }, () => ""));
	...
	const errors = validate(otpMfaSchema, values);

	const handleChange = (name: "mfaCode" | "rememberDevice", value: string[] | boolean) => {
		onSubmitError(null);
		if (name === "mfaCode") {
	...
	const getOtpSetupInfo = useCallback(() => {
		fetch(`${routeConfig.IdentityRoute.OtpMfaSetup}${qs}`, {
			method: "GET",
			headers: {
	...
	const getOtpMfaInfo = useCallback(() => {
		fetch(`${routeConfig.IdentityRoute.ProcessOtpMfa}${qs}`, {
			method: "GET",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json",
	...
	const handleVerifyMfa = useCallback(
		(e: Event) => {
			...
			fetch(routeConfig.IdentityRoute.ProcessOtpMfa, {
				method: "POST",
				headers: {
					Accept: "application/json",
					"Content-Type": "application/json",
