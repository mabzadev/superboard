...
import * as melodySecrets from "../scripts/generate-melody-auth-secrets.mjs";
const { generateMelodyAuthSecrets, serializeMelodyAuthSecrets } = melodySecrets;
...
export default centralTests(
	...
	defineConfig({
				...
				return {
					...
					miniflare: {
						bindings: {
							...
							COMPANY_EMAIL_LOGO_URL: "https://board.example.test/superboard-email-logo.png",
						...
						serviceBindings: {
							...
							FILES_SERVICE: () => Response.json({ data: { erased: true, files_deleted: 0 } }),
						},
					},
				};
		...
		test: {
			include: [
				"../../../../tests/checks/plugins/superboard-authentification/worker/runtime/**/*.test.ts",
			],
			setupFiles: [
				"../../../../tests/checks/plugins/superboard-authentification/worker/runtime/apply-migrations.ts",
			],
