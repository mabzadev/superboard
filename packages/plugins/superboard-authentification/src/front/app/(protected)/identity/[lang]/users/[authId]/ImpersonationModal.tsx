...
import { typeTool } from "../../../../../../identity/tools/index.js";
...
const ImpersonationModal = ({
	...
	const apps =
		data?.apps?.filter((app) => app.type === typeTool.ClientType.SPA && app.isActive) ?? [];
	...
	const handleImpersonate = async () => {
		...
		if (
			...
			res.data.refresh_token_expires_on &&
	...
	return (
		<AlertDialog open={show}>
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>{t("impersonateTitle", { user: user.email ?? "" })}</AlertDialogTitle>
				</AlertDialogHeader>
				<section>
					<div className="flex items-center gap-2">
						<AlertDialogDescription>{t("impersonateApp")}</AlertDialogDescription>
						<Select value={selectedAppId?.toString()} onValueChange={handleAppChange}>
					...
					{isConsented && !refreshTokenStorage && selectedApp && (
						<Button className="mt-4" onClick={handleImpersonate} data-testid="confirmImpersonate">
							{t("confirmImpersonate")}
						</Button>
					)}
					{refreshTokenStorage && (
						<div className="mt-6">
							<p className="font-bold" data-testid="impersonateToken">
								{t("impersonateToken")}:
							</p>
							<p className="break-all mt-2">{refreshTokenStorage.refreshToken}</p>
							<p className="text-sm text-muted-foreground mt-4">{t("impersonateTokenDesc")}</p>
