...
const Page = () => {
	...
	return (
		<section>
			<Breadcrumb page={{ label: t("layout.dashboard") }} />
			{links && (
				<>
					<PageTitle className="mb-6" title={t("dashboard.links")} />
					<Table className="break-all">
						...
						<TableBody>
							<TableRow>
								...
								<TableCell>
									<a target="_blank" href={links.openidConfig} rel="noreferrer">
										{links.openidConfig}
									</a>
								</TableCell>
							</TableRow>
							<TableRow>
								...
								<TableCell>
									<a target="_blank" href={links.jwks} rel="noreferrer">
										{links.jwks}
									</a>
								</TableCell>
							</TableRow>
							<TableRow>
								...
								<TableCell>
									<a target="_blank" href={links.apiSwagger} rel="noreferrer">
										{links.apiSwagger}
									</a>
								</TableCell>
							</TableRow>
							<TableRow>
								...
								<TableCell>
									<a target="_blank" href={links.embeddedSwagger} rel="noreferrer">
			...
			{configTypes.map((configType) => (
				<section key={configType.name} className="mt-8 gap-6">
					<PageTitle className="mt-8 mb-6" title={configType.name} />
					<Table className="break-all">
						<TableHeader>
							<TableRow>
								<TableHead className={configNameClass}>{t("dashboard.configName")}</TableHead>
						...
						<TableBody>
							{configType.value.map((configName) => (
								<TableRow key={configName}>
									...
									<TableCell>
										{typeof configs[configName] === "boolean" && (
											<ConfigBooleanValue config={configs[configName]} />
										)}
										{Array.isArray(configs[configName]) && configs[configName].join(", ")}
										{typeof configs[configName] !== "boolean" &&
											!Array.isArray(configs[configName]) &&
											configs[configName]}
									</TableCell>
								</TableRow>
							))}
