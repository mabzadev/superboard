...
import BannerTypeSelector from "../../../../../../../identity/components/BannerTypeSelector.js";
...
import { ClientType } from "../../../../../../../identity/tools/type.js";
import useSignalValue from "../../../../../../../identity/useSignalValue.js";
import useEditBanner from "../useEditBanner.js";
...
const Page = () => {
	...
	const { data, isLoading } = useGetApiV1AppBannersByIdQuery({ id: Number(id) });
	...
	const { values, errors, onChange } = useEditBanner(banner);
			...
			if (
				values.locales.find((valueLocale) => {
					return banner.locales.every((bannerLocale) => {
						return (
							bannerLocale.locale !== valueLocale.locale || bannerLocale.value !== valueLocale.value
						);
					});
				})
			)
	...
	const hasDifferentStatus = useMemo(() => values.isActive !== banner?.isActive, [values, banner]);
								...
								<div className="flex items-center flex-wrap gap-6 max-md:flex-col max-md:items-start">
									{apps?.map((app) => (
										<div key={app.id} className="flex items-center gap-2">
											...
											/>
											<Label htmlFor={`app-${app.id}`} className="flex">
												{app.name}
											</Label>
										</div>
									))}
			...
			{canWriteApp && (
				<section className="flex items-center gap-4 mt-8">
					...
					<DeleteButton
						...
						disabled={isUpdating}
						confirmDeleteTitle={t("common.deleteConfirm", { item: values.text })}
