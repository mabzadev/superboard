...
const UserOrgGroupModal = ({
	...
	const { data: userOrgsData, isLoading: isLoadingUserOrgs } = useGetApiV1UsersByAuthIdOrgsQuery({
		authId: String(authId),
	});
	const userOrgs = userOrgsData?.orgs ?? [];
	const userOrgIds = userOrgs.map((org) => org.id);

	const [postUserOrgs, { isLoading }] = usePostApiV1UsersByAuthIdOrgsMutation();

	const handleToggleOrg = (orgIds: number) => {
		const newOrgIds = userOrgIds.includes(orgIds)
			? userOrgIds.filter((id) => id !== orgIds)
			: [...userOrgIds, orgIds];
		postUserOrgs({
			authId,
			body: { orgs: newOrgIds },
				...
				<section className="flex max-md:flex-col gap-6 max-md:gap-2 flex-wrap">
					{orgs.map((org) => (
						<div key={org.id} className="flex items-center gap-2">
							...
							/>
							<Label htmlFor={`org-${org.id}`} className="flex">
								{org.name}
							</Label>
						</div>
					))}
