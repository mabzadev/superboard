...
import { formatShortDate } from "@superboard/front-ui/lib/dateUtils.js";
import { cn, parseSecondsInDaysHoursMinutesSeconds } from "@superboard/front-ui/lib/utils.js";
...
import { Tooltip, TooltipContent } from "@superboard/front-ui/ui/tooltip.js";
...
export const getLinksTableColumns = (
	...
	},
	{
		...
		header: () => (
			<Tooltip>
				...
				<TooltipContent>
					<p>This represents the number of app opens that occurred from a SuperBoard link.</p>
				</TooltipContent>
			</Tooltip>
		),
		cell: ({ row }) => (
	...
	},
	{
		...
		cell: ({ row }) => (
			<div className="flex py-2 px-3">
				<p>
					{row.original.total_installs ? numberFormatter.format(row.original.total_installs) : "-"}
				</p>
			</div>
		),
	},
	{
		...
		header: () => (
			<Tooltip>
				...
				<TooltipContent>
					<p>
						The number of app reinstalls generated by the link. A reinstall occurs when the app is
						deleted and reinstalled on the same device.
					</p>
				</TooltipContent>
			</Tooltip>
		),
	...
	},
	{
		...
		header: () => (
			<Tooltip>
				...
				<TooltipContent>
					<p>
						The number of users who revisited the app after more than 7 days of inactivity due to a
						click on a link.
					</p>
				</TooltipContent>
			</Tooltip>
	...
	},
	{
		...
		header: () => (
			<Tooltip>
				...
				<TooltipContent>
					<p>Sum of the entire time spent in the app by all the users for this link.</p>
				</TooltipContent>
			</Tooltip>
		),
		cell: ({ row }) => (
			<div className="flex py-2 px-3">
				<p>{parseSecondsInDaysHoursMinutesSeconds(row.original.total_time_spent) || "-"}</p>
			</div>
		),
	},
	{
...
const getPlatformIconSrc = (platform: string, resolvedTheme: string | undefined) => {
	switch (platform) {
		case GOOGLE:
...
const PlatformIconCell = ({ platform, iconSrc }: { platform: string; iconSrc: string | null }) => {
	return (
		<div className="flex justify-center items-center w-[50px] h-[40px]">
			{iconSrc && <Image src={iconSrc} alt={`${platform} icon`} width={24} height={24} />}
		</div>
	);
};
...
export const ThemedPlatformIconCell = React.memo(({ platform }: { platform: string }) => {
