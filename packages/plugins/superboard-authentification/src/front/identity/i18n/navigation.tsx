...
} from "@superboard/front-ui/navigation";
import NextLink from "@superboard/front-ui/next-link";
import { useLocale } from "next-intl";
import type { ComponentProps } from "react";

const localized = (href: string, locale: string) => {
	if (!href.startsWith("/")) return href;
	if (href.startsWith("/auth/") || href === "/auth") return href;
	return canonicalFrontHref(
		href.startsWith("/identity/") ? href : `/identity/${locale}${href === "/" ? "" : href}`,
	);
};

export function Link(props: ComponentProps<typeof NextLink>) {
	const locale = useLocale();
	const href = typeof props.href === "string" ? localized(props.href, locale) : props.href;
...
export function usePathname() {
	...
	if (pathname === "/auth/account-policies") return "/account";
	if (pathname.startsWith("/auth/directory/")) return pathname.slice("/auth/directory".length);
	if (pathname === "/auth" || pathname.startsWith("/auth/"))
		return pathname.slice("/auth".length) || "/";
