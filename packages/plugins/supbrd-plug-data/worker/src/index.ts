...
import { handleOperatorFiles } from "./operator.js";
...
app.all("/internal/v1/operator/*", (c) =>
	handleOperatorFiles(c.req.raw, c.env, {
		maximum: maximumBytes(c.env),
		filename: safeFilename,
		contentType: safeContentType,
		allowed: (value) => assertAllowedContentType(c.env, value),
	}),
);

app.onError((cause, c) => {
	if (cause instanceof FilesInputError) return error(cause.code, cause.message, cause.status);
	console.error(
		JSON.stringify({
...
app.get("/health", async (c) => {
	try {
      ...
      `,
			...
			c.env.FILES.head("superboard/readiness"),
		]);
...
app.post("/v1/files", async (c) => {
	...
	const maximum = maximumBytes(c.env);
	if (contentLength > maximum) return error("file_too_large", `File exceeds ${maximum} bytes`, 413);
	if (!c.req.raw.body) return error("file_empty", "A file body is required", 422);
	const filename = safeFilename(c.req.header("x-filename") || "upload.bin");
	const contentType = safeContentType(c.req.header("content-type") || "application/octet-stream");
	...
	const boundedBody = c.req.raw.body.pipeThrough(
		new TransformStream<Uint8Array, Uint8Array>({
			transform(chunk, controller) {
				...
				if (byteSize > maximum)
					throw new FilesInputError("file_too_large", `File exceeds ${maximum} bytes`, 413);
				controller.enqueue(chunk);
			},
			flush() {
				if (contentLength > 0 && byteSize !== contentLength)
					throw new FilesInputError(
						"file_length_mismatch",
						"File length does not match Content-Length",
						422,
					);
			},
		}),
		{ signal: AbortSignal.any([c.req.raw.signal, AbortSignal.timeout(60000)]) },
	);
	const metadata = {
	...
	if (contentLength > 0) {
		...
		const piping = boundedBody.pipeTo(fixed.writable);
		const stored = await Promise.allSettled([
			c.env.FILES.put(objectKey, fixed.readable, metadata),
			piping,
		]);
		if (stored[1].status === "rejected") throw stored[1].reason;
		if (stored[0].status === "rejected") throw stored[0].reason;
		object = stored[0].value;
	} else {
		object = await c.env.FILES.put(
			objectKey,
			await readBytesLimited(new Response(boundedBody), maximum),
			metadata,
		);
	}
	if (byteSize === 0) {
	...
	try {
       ...
       VALUES (?,?,?,?,?,?,?)`,
			...
			.bind(id, c.get("userId"), objectKey, filename, contentType, byteSize, object.etag)
			.run();
	} catch (cause) {
...
app.get("/v1/files/:id", async (c) => {
	...
	return row ? json({ file: fileJson(row) }) : error("file_not_found", "File was not found", 404);
});
...
async function serveFile(
	...
	if (!object) return error("file_object_missing", "Stored file is unavailable", 503, true);
...
app.delete("/v1/files/:id", async (c) => {
	...
	await c.env.DB.prepare("DELETE FROM application_files WHERE id=? AND user_id=?")
...
app.get("/v1/downloads/:ticket", async (c) => {
	const payload = await verifyDownloadTicket(
		c.req.param("ticket"),
		configuredSecrets(c.env.FILES_DOWNLOAD_SIGNING_KEY, c.env.FILES_DOWNLOAD_SIGNING_KEY_PREVIOUS),
		downloadTicketTtl(c.env),
	);
	if (!payload) {
		return error("download_ticket_invalid", "Download ticket is invalid or expired", 401);
	}
	const row = await fileById(c.env.DB, payload.fileId);
	if (!row || !(await constantTimeEqual(await sha256(row.user_id), payload.ownerHash))) {
...
export function resolvedRange(range: R2Range, size: number): { start: number; length: number } {
	...
	const start = Math.min(Math.max(0, offset ?? 0), size);
	const length = Math.min(Math.max(0, requestedLength ?? size - start), size - start);
	return { start, length };
}
...
async function internalAuthorized(c: FilesContext): Promise<boolean> {
	return matchesAnySecret(
		...
		configuredSecrets(c.env.FILES_INTERNAL_TOKEN, c.env.FILES_INTERNAL_TOKEN_PREVIOUS),
	);
}
...
async function createDownloadTicket(input: DownloadTicketInput, secret: string): Promise<string> {
...
async function verifyDownloadTicket(
	...
	const matches = await Promise.all(
		secrets.map(async (secret) => {
			...
			return crypto.subtle.verify("HMAC", key, signature, new TextEncoder().encode(payloadPart));
		}),
	);
	if (!matches.some(Boolean) || !validDownloadTicketPayload(parsed)) return null;
	if (parsed.expiresAt <= nowSeconds || parsed.expiresAt > nowSeconds + maximumTtlSeconds) {
		return null;
	}
	return parsed;
}

function validDownloadTicketPayload(value: unknown): value is DownloadTicketPayload {
...
async function hmacSign(value: string, secret: string): Promise<ArrayBuffer> {
	return crypto.subtle.sign("HMAC", await hmacKey(secret, "sign"), new TextEncoder().encode(value));
}
...
function integer(value: string | undefined, fallback: number, minimum: number, maximum: number) {
	...
	if (!Number.isSafeInteger(candidate) || candidate < minimum || candidate > maximum) {
		throw new FilesInputError("pagination_invalid", "Pagination value is invalid", 422);
	}
	return candidate;
}
function safeFilename(value: string) {
	const filename = value
		.trim()
		// oxlint-disable-next-line no-control-regex -- Remove control characters and path separators from filenames.
		.replace(/[\x00-\x1f\x7f/\\]/g, "-")
		.slice(0, 255);
	if (!filename) throw new FilesInputError("filename_invalid", "Filename is invalid", 422);
	return filename;
}
function safeContentType(value: string) {
	const contentType = value.split(";", 1)[0].trim().toLowerCase();
	if (!/^[a-z0-9!#$&^_.+-]+\/[a-z0-9!#$&^_.+-]+$/.test(contentType)) {
		throw new FilesInputError("content_type_invalid", "Content type is invalid", 422);
	}
	return contentType;
}
