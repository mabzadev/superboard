...
admin.get("/:projectId", async (c) => {
	try {
		...
		const [
			...
			c.env.DB.prepare("SELECT * FROM billing_project_settings WHERE project_id = ?")
				.bind(project.id)
				.first(),
			c.env.DB.prepare(
				"SELECT * FROM billing_products WHERE project_id = ? AND store IN ('apple','google') ORDER BY created_at DESC",
			)
				.bind(project.id)
				.all(),
			c.env.DB.prepare(
				"SELECT * FROM billing_entitlements WHERE project_id = ? ORDER BY identifier",
			)
				.bind(project.id)
				.all(),
			c.env.DB.prepare(
				"SELECT * FROM billing_offerings WHERE project_id = ? ORDER BY is_current DESC, created_at",
			)
				.bind(project.id)
				.all(),
			c.env.DB.prepare(`
      ...
      `)
		...
		return c.json({
			...
			credentials: {
				...
				android: {
					configured: Boolean(googleCredentials?.client_email && googleCredentials?.project_id),
...
admin.put("/:projectId/settings", async (c) => {
    ...
    `)
			.bind(project.id, restore, data.purchases_enabled ? 1 : 0)
			.run();
		return c.json(
			await c.env.DB.prepare("SELECT * FROM billing_project_settings WHERE project_id = ?")
				.bind(project.id)
				.first(),
		);
	} catch (error) {
...
admin.get("/:projectId/oidc", async (c) => {
	try {
		...
		const rows = await c.env.DB.prepare(
			"SELECT * FROM billing_oidc_configs WHERE project_id = ? ORDER BY created_at",
		)
			.bind(project.id)
			.all();
...
admin.post("/:projectId/oidc", async (c) => {
    ...
    `)
			.bind(id, project.id, issuer, String(data.audience || "superboard"), jwks.toString())
			.run();
...
admin.post("/:projectId/products", async (c) => {
	try {
		...
		const data = await body(c);
		if (
			!["apple", "google"].includes(data.store) ||
			!["subscription", "non_consumable", "consumable"].includes(data.product_type)
		) {
    ...
    `)
			.bind(
				...
				data.product_type,
				data.display_name || data.store_product_id,
		...
		for (const entitlementId of Array.isArray(data.entitlement_ids) ? data.entitlement_ids : []) {
			await c.env.DB.prepare(
				"INSERT OR IGNORE INTO billing_product_entitlements (product_id, entitlement_id) SELECT ?, id FROM billing_entitlements WHERE id = ? AND project_id = ?",
			)
				.bind(id, entitlementId, project.id)
				.run();
		}
		return c.json({ id }, 201);
	} catch (error) {
...
async function persistStoreCatalog(
	c: any,
	project: any,
    ...
    `).bind(
				...
				product.description,
				product.active ? 1 : 0,
				JSON.stringify({ ...product.metadata, provider_synced_at: providerSyncedAt }),
			),
		),
		c.env.DB.prepare(`
...
admin.post("/:projectId/products/sync", async (c) => {
	try {
		...
		const data = await body(c);
		const requested =
			data.platform === "ios" || data.platform === "android" ? [data.platform] : ["ios", "android"];
		const results = await Promise.all(
			requested.map(async (platform) => {
				try {
					...
					return {
						ok: true as const,
						...(await persistStoreCatalog(c, project, platform, products)),
					};
				} catch (error) {
					return {
						ok: false as const,
...
admin.patch("/:projectId/products/:id", async (c) => {
	try {
		...
		if (data.metadata !== undefined)
			throw new Error("Provider metadata can only be updated by store synchronization");
...
admin.post("/:projectId/entitlements", async (c) => {
	try {
		...
		const identifier = String(data.identifier || "")
			.trim()
			.toLowerCase();
		if (!/^[a-z0-9_.-]{1,80}$/.test(identifier))
			throw new Error("Entitlement identifier is invalid");
		const id = crypto.randomUUID();
		await c.env.DB.prepare(
			"INSERT INTO billing_entitlements (id, project_id, identifier, display_name, description) VALUES (?, ?, ?, ?, ?)",
		)
			.bind(id, project.id, identifier, data.display_name || identifier, data.description || null)
			.run();
		return c.json({ id }, 201);
	} catch (error) {
		return fail(c, error);
...
admin.post("/:projectId/products/:productId/entitlements/:entitlementId", async (c) => {
    ...
    `)
			.bind(c.req.param("productId"), c.req.param("entitlementId"), project.id)
			.first();
		...
		await c.env.DB.prepare(
			...
			.bind(c.req.param("productId"), c.req.param("entitlementId"))
			.run();
...
admin.post("/:projectId/offerings", async (c) => {
	try {
		...
		const id = crypto.randomUUID();
		if (data.is_current)
			await c.env.DB.prepare("UPDATE billing_offerings SET is_current = 0 WHERE project_id = ?")
				.bind(project.id)
				.run();
		await c.env.DB.prepare(`
...
admin.post("/:projectId/entitlements/:entitlementId/products", async (c) => {
	try {
		...
		const productIds = [
			...new Set(
				(Array.isArray(data.product_ids) ? data.product_ids : []).map(String).filter(Boolean),
			),
		];
...
admin.post("/:projectId/offerings/:offeringId/packages", async (c) => {
	try {
		...
		const offering = await c.env.DB.prepare(
			"SELECT id FROM billing_offerings WHERE id = ? AND project_id = ?",
		)
			.bind(c.req.param("offeringId"), project.id)
			.first();
    ...
    `)
		...
		await c.env.DB.batch([
			c.env.DB.prepare(
				"INSERT INTO billing_packages (id, offering_id, identifier, package_type, position, metadata) VALUES (?, ?, ?, ?, ?, ?)",
			).bind(
				id,
				offering.id,
				identifier,
				data.package_type || "custom",
...
admin.post("/:projectId/customers/:customerId/block", async (c) => {
	try {
		...
		const result = await c.env.DB.prepare(
			...
			.bind(data.blocked === false ? 0 : 1, c.req.param("customerId"), project.id)
			.run();
...
admin.post("/:projectId/webhooks", async (c) => {
    ...
    `)
			.bind(
				id,
				project.id,
				data.name || url.hostname,
				url.toString(),
				encrypted,
				JSON.stringify(data.environments || ["production"]),
...
admin.delete("/:projectId/products/:id", async (c) => {
	try {
		const project = await projectFor(c);
		const result = await c.env.DB.prepare(
			"UPDATE billing_products SET active = 0, updated_at = datetime('now') WHERE id = ? AND project_id = ?",
		)
			.bind(c.req.param("id"), project.id)
			.run();
...
admin.delete("/:projectId/entitlements/:id", async (c) => {
	try {
		const project = await projectFor(c);
		const result = await c.env.DB.prepare(
			"UPDATE billing_entitlements SET active = 0, updated_at = datetime('now') WHERE id = ? AND project_id = ?",
		)
			.bind(c.req.param("id"), project.id)
			.run();
...
admin.delete("/:projectId/offerings/:id", async (c) => {
	try {
		const project = await projectFor(c);
		const result = await c.env.DB.prepare(
			"UPDATE billing_offerings SET active = 0, is_current = 0, updated_at = datetime('now') WHERE id = ? AND project_id = ?",
		)
			.bind(c.req.param("id"), project.id)
			.run();
...
admin.post("/:projectId/mirror/compare", async (c) => {
	try {
		...
		if (!customer) throw new Error("Customer not found");
		const superboard = await customerInfo(c.env.DB, project.id, String(customer.id));
		const superboardActive = Object.entries(superboard.entitlements)
		...
		const revenueCatActive = Object.keys(rcRaw)
			.filter((key) => {
				const value = rcRaw[key];
				return value && value.is_active !== false;
			})
			.sort();
		const missingInSuperBoard = revenueCatActive.filter((key) => !superboardActive.includes(key));
		const extraInSuperBoard = superboardActive.filter((key) => !revenueCatActive.includes(key));
		const differences = {
			missing_in_superboard: missingInSuperBoard,
			extra_in_superboard: extraInSuperBoard,
		};
		const matches = missingInSuperBoard.length === 0 && extraInSuperBoard.length === 0;
		const id = crypto.randomUUID();
		await c.env.DB.prepare(`
      INSERT INTO billing_mirror_comparisons (
        id, project_id, customer_id, app_user_id, matches,
        superboard_active_entitlements, revenuecat_active_entitlements, differences
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `)
			.bind(
				id,
				...
				appUserId,
				matches ? 1 : 0,
				JSON.stringify(superboardActive),
				JSON.stringify(revenueCatActive),
...
admin.post("/:projectId/credentials/test", async (c) => {
	try {
		...
		return c.json(
			await testStoreCredentials(c.env, {
				projectId: project.id,
