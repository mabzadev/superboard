...
function eventTypeForNotification(
	source: "apple" | "google",
	...
	if (source === "google") {
		if (type.includes("VOID") || type.includes("REFUND"))
			return purchaseType === "subscription" ? "refund" : "cancel";
		if (type.includes("CANCEL") || type.includes("REVOKE") || type.includes("EXPIRE"))
			return "cancel";
		if (type.includes("RECOVER") || type.includes("RENEW") || type.includes("PURCHASE"))
			return "buy";
	}
	...
	if (
		...
		type.includes("DID_FAIL_TO_RENEW") ||
...
async function upsertPurchaseEvent(
	db: D1Database,
	event: {
		projectId: number | string;
		source: "apple" | "google";
...
async function handleApple(c: any, test: boolean) {
	...
	if (!instanceId)
		return providerHttpError(c, "instance_invalid", "Instance identifier is invalid", 422, false);
	...
	if (
		...
		body.signedPayload &&
		c.env.IAP_ALLOW_UNSIGNED_FIXTURES !== "true"
	) {
		const digest = new Uint8Array(
			await crypto.subtle.digest("SHA-256", new TextEncoder().encode(body.signedPayload)),
		);
		const externalEventId = Array.from(digest)
				...
				return c.json(
					{
						message: result.processed
							? "Apple IAP notification already processed"
							: "Apple IAP notification accepted",
    ...
    `)
			.bind(project.id, environment, externalEventId)
			.first()) as { id: string; status: string } | null;
		if (!persisted) {
			return providerHttpError(
				c,
				"apple_notification_persistence_failed",
				"Apple notification could not be persisted",
				503,
				true,
			);
		}
		...
		try {
			if (c.env.BILLING_QUEUE) await c.env.BILLING_QUEUE.send(job);
				...
				c.executionCtx.waitUntil(
					import("../lib/billing-jobs").then(({ processAppleBillingNotification }) =>
						processAppleBillingNotification(c.env, job),
					),
				);
		} catch (error) {
			...
			console.error(
				JSON.stringify({
					event: "apple_notification_queue_failed",
					webhook_event_id: persisted.id,
					error: error instanceof Error ? error.message : String(error),
				}),
			);
			return providerHttpError(
				c,
				"apple_notification_queue_unavailable",
				"Apple notification queue is temporarily unavailable",
				503,
				true,
			);
		}
		return c.json(
			{ message: "Apple IAP notification accepted", duplicate: inserted.meta.changes === 0 },
			202,
		);
	}
	let payload: any;
	try {
		...
		return providerHttpError(
			...
			"Apple notification signature is invalid",
			400,
			false,
		);
	}
	...
	let transaction: any;
	try {
		transaction = await appleTransactionPayload(
			data.signedTransactionInfo,
			data.transactionInfo || body.transactionInfo || data,
		);
	} catch {
		return providerHttpError(
			c,
			"apple_transaction_signature_invalid",
	...
	const purchaseType =
		transaction.type === "Consumable" || transaction.type === "Non-Consumable"
			? "one_time"
			: "subscription";
	await recordWebhook(c.env.DB, {
		source: "apple",
		notificationType,
		payload: body,
		instanceId,
	...
	await upsertPurchaseEvent(c.env.DB, {
		...
		priceCents: transaction.price
			? Math.round(Number(transaction.price) / 10000)
...
async function handleGoogle(c: any) {
	...
	if (/^Bearer\s+/i.test(c.req.header("Authorization") || "")) {
		try {
			expectedEmail = (await googlePlayWebhookIdentity(c.env, productionProject.id))
				.serviceAccountEmail;
		} catch (error) {
	...
	try {
		...
		return providerHttpError(
			c,
			"google_pubsub_payload_invalid",
			"Google Pub/Sub payload is invalid",
			400,
			false,
		);
	}
	...
	if (
		pendingRefundReview &&
		typeof pendingRefundReview === "object" &&
		!Array.isArray(pendingRefundReview)
	) {
		...
		const pubsubMessageId = String(
			body.message?.messageId || body.message?.message_id || "",
		).trim();
		if (!orderId || !pendingRefundToken || !pubsubMessageId) {
			return providerHttpError(
				c,
				"google_refund_review_invalid",
				"Google pending refund review data is incomplete",
				400,
				false,
			);
		}
      ...
      LIMIT 1
    `)
			.bind(instanceId, orderId, orderId)
			.first()) as { project_id: string; environment: "sandbox" | "production" } | null;
		const projectId = String(purchase?.project_id || productionProject.id);
		const environment: "sandbox" | "production" =
			purchase?.environment === "sandbox" ? "sandbox" : "production";
		...
		try {
			...
			return c.json(
				{
					message: result.processed
						? "Google refund review already processed"
						: "Google refund review accepted",
					duplicate: result.duplicate,
				},
	...
	const notificationType = String(
		notification.notificationType ||
			...
			(data.voidedPurchaseNotification ? "VOIDED_PURCHASE" : "PURCHASED"),
	);
	const purchaseType = data.subscriptionNotification ? "subscription" : "one_time";
	const transactionId = String(
		notification.purchaseToken || notification.orderId || data.purchaseToken || crypto.randomUUID(),
	);
	const productId =
	...
	if (pubsubMessageId) {
		let productType: "subscription" | "non_consumable" | "consumable" =
			purchaseType === "subscription" ? "subscription" : "non_consumable";
      ...
      `)
				.bind(productionProject.id, productId)
				.first()) as { product_type: "non_consumable" | "consumable" } | null;
		...
		if (!purchaseToken || !productId) {
			return providerHttpError(
				c,
				"google_rtdn_purchase_invalid",
				"Google RTDN purchase data is incomplete",
				400,
				false,
			);
		}
		try {
			...
			return providerTaggedHttpError(
				c,
				error,
				"Google Play purchase environment could not be verified",
			);
		}
		if (billingServiceEnabled(c.env)) {
			try {
    ...
    `)
			.bind(project.id, providerEnvironment, String(pubsubMessageId))
			.first()) as { id: string; status: string } | null;
		if (!persisted)
			return providerHttpError(
				c,
				"google_rtdn_persistence_failed",
				"Google RTDN could not be persisted",
				503,
				true,
			);
		if (persisted.status === "processed")
			return c.json({ message: "Google RTDN already processed", duplicate: true });
		...
		if (c.env.BILLING_QUEUE) {
			try {
				...
				console.error(
					JSON.stringify({
						event: "google_notification_queue_failed",
						webhook_event_id: billingWebhookEventId,
						error: error instanceof Error ? error.message : String(error),
					}),
				);
				return providerHttpError(
					c,
					"google_rtdn_queue_unavailable",
					"Google RTDN queue is temporarily unavailable",
					503,
					true,
				);
			}
			return c.json(
				{ message: "Google RTDN accepted", duplicate: inserted.meta.changes === 0 },
				202,
			);
		}
	}
	...
	await upsertPurchaseEvent(c.env.DB, {
		...
		purchaseDate: msDate(
			verified?.startTime ||
				verified?.purchaseTimeMillis ||
				data.eventTimeMillis ||
	...
	if (verified && customerId) {
		const customer = await c.env.DB.prepare(
			...
			.bind(String(customerId), project.id)
			.first();
		if (customer) {
			...
			const purchase: VerifiedPurchase = {
				...
				status:
					purchaseType === "subscription"
						...
						: Number(verified.purchaseState) === 0
							? "active"
							: "pending",
    ...
    `)
			.bind(notificationType, billingWebhookEventId)
			.run();
	}
...
iap.post("/reconcile_subscriptions", async (c) => {
	...
	if (
		!c.env.IAP_PROCESS_KEY ||
		!provided ||
		!(await timingSafeEqual(provided, c.env.IAP_PROCESS_KEY))
	) {
