...
import {
	billingWorkerReadiness,
	type BillingWorkerReadiness,
} from "../lib/billing-worker-readiness";
...
import {
	...
	requiresAuthenticatedDeviceEvidence,
	RELEASE_GATE_CHECKS,
...
import { siteOperatorProject } from "../lib/site-operator-project";
...
async function projectFor(c: any) {
	const siteProject = await siteOperatorProject(c, c.req.param("projectId"));
	if (siteProject) return siteProject;
...
async function releaseGateScope(c: any, project: { id: string; instanceId: number }) {
  ...
  `,
		...
		.bind(project.instanceId)
		.all()) as { results: Array<{ id: number; is_test: number }> };
	const productionProjectId = String(
		rows.results.find((row) => Number(row.is_test) === 0)?.id || "",
	);
...
async function audit(
  ...
  `,
		...
		.bind(
			crypto.randomUUID(),
			projectId,
			c.get("operatorId") ?? String(c.get("userId")),
			action,
			resourceType,
			resourceId || null,
...
async function featureFlagsFor(c: any, projectId: string) {
	await c.env.DB.prepare("INSERT OR IGNORE INTO billing_feature_flags (project_id) VALUES (?)")
		.bind(projectId)
		.run();
	return c.env.DB.prepare(
		`
    SELECT purchases_core, product_catalog, paywalls,
  ...
  `,
	...
	if (active)
		throw purchasesError(
			"legacy_inventory_active",
			"Wait for the active legacy inventory to finish",
			409,
		);
}
...
function identifier(value: unknown, field = "identifier") {
	...
	if (!/^[a-z0-9_.-]{1,80}$/.test(result)) {
		throw purchasesError(
			"invalid_identifier",
			`${field} must use lowercase letters, numbers, dots, underscores or hyphens`,
		);
	}
	return result;
}
...
function connectionCapabilities(provider: string, configured: boolean) {
	...
	if (provider === "apple")
		base.refunds = { status: configured ? "request_only" : "not_configured" };
	return base;
}
	...
	return [
		...
		},
		{
			...
			public_configuration: {
				...
				package_name: google?.identifier,
...
admin.get("/:projectId/health", async (c) => {
	try {
		...
		const [events, subscriptions, deliveries, deadLetters, connections, features] =
			await Promise.all([
				c.env.DB.prepare(
					`
      ...
      `,
        ...
        FROM billing_dead_letters WHERE project_id = ?
      `,
				)
					.bind(project.id)
					.first<Record<string, unknown>>(),
				c.env.DB.prepare(
					`SELECT provider, environment, status, last_tested_at, last_synced_at, last_event_at, last_error_code, last_error_message FROM billing_store_connections WHERE project_id = ? AND provider IN ('apple','google')`,
				)
					.bind(project.id)
					.all(),
		...
		return c.json({
			status:
				Number((events as any)?.failed_events || 0) +
					Number((deliveries as any)?.failed_deliveries || 0) +
					Number((subscriptions as any)?.verification_failures || 0) +
					Number((subscriptions as any)?.stale_verifications || 0) +
					Number(deadLetters?.quarantined || 0) >
				0
					? "degraded"
					: "healthy",
			events,
...
admin.get("/:projectId/release-gate", async (c) => {
	try {
		...
		const [
			...
			c.env.DB.prepare(`SELECT * FROM billing_release_gate_checks WHERE project_id = ?`)
				.bind(scope.releaseProjectId)
				.all<StoredReleaseGateCheck>(),
			c.env.DB.prepare(
				`
        ...
        WHERE project_id IN (${projectPlaceholders || "''"}) AND provider IN ('apple', 'google')
      `,
			)
				.bind(...scopedProjectIds)
      ...
      `,
				...
				.bind(scope.releaseProjectId)
				.all<ReleaseGateCertificationObservation>(),
			c.env.DB.prepare(
				`
        SELECT
      ...
      `,
				...
				.bind(staleMinutes, ...scopedProjectIds)
      ...
      `,
				...
				.bind(staleMinutes, staleMinutes, ...scopedProjectIds)
				.first<Record<string, number>>(),
			c.env.DB.prepare(
				`
        SELECT
      ...
      `,
        ...
        WHERE project_id IN (${projectPlaceholders || "''"}) AND status = 'quarantined'
      `,
			)
				.bind(...scopedProjectIds)
				.first<Record<string, number>>(),
      ...
      `,
		...
		const verifiedCertificationObservations = await Promise.all(
			(certificationObservations.results || []).map(async (row) => ({
				...row,
				digest_valid: (await sha256Hex(String(row.evidence_json || ""))) === row.evidence_sha256,
			})),
		);
		const connectionPassed = (projectId: string, provider: string, environment: string) =>
		...
		const hasEntitlement = (projectId: string) =>
			(entitlement.results || []).some((row) => String(row.project_id) === projectId);
		const hasOffering = (projectId: string) =>
			(offering.results || []).some((row) => String(row.project_id) === projectId);
		const legacyInventoryReady =
		...
		const prerequisites: ReleaseGatePrerequisite[] = [
			...billingOperationalPrerequisites({
				dedicated_execution:
					c.env.CREDENTIAL_KEY_SCOPE === "billing" || c.env.BILLING_EXECUTION_MODE === "service",
				...
				quarantined_dead_letters: Number(deadLetters?.quarantined || 0),
			}),
			prerequisite(
				"test_project",
				"Test project",
				Boolean(scope.testProjectId),
				"A dedicated test project is available.",
				"Create the paired test project.",
			),
			prerequisite(
				"production_project",
				"Production project",
				Boolean(scope.productionProjectId),
				"A dedicated production project is available.",
				"Create the paired production project.",
			),
			...(["apple", "google"] as const).flatMap((provider) => [
				prerequisite(
					`sandbox_${provider}_connection`,
					`Sandbox ${providerDisplayName(provider)} connection`,
					connectionPassed(scope.testProjectId, provider, "sandbox"),
					"Credentials were tested successfully.",
					`Test the sandbox ${providerDisplayName(provider)} connection successfully.`,
				),
				prerequisite(
					`production_${provider}_connection`,
					`Production ${providerDisplayName(provider)} connection`,
					connectionPassed(scope.productionProjectId, provider, "production"),
					"Credentials were tested successfully.",
					`Test the production ${providerDisplayName(provider)} connection successfully.`,
				),
			]),
			prerequisite(
				...
				Number(appleNotifications?.ready || 0) === 1 &&
					Number(appleNotifications?.fresh || 0) === 1,
			...
			prerequisite(
				"sandbox_apple_catalog",
				"Sandbox Apple subscription catalog",
				sandboxApple.catalog,
				"Weekly and yearly subscriptions are imported.",
				"Import the weekly and yearly Apple subscriptions into the test project.",
			),
			prerequisite(
				"production_apple_catalog",
				"Production Apple subscription catalog",
				productionApple.catalog,
				"Weekly and yearly subscriptions are imported.",
				"Import the weekly and yearly Apple subscriptions into the production project.",
			),
			prerequisite(
				"production_apple_catalog_fresh",
				"Production Apple catalog freshness",
				catalogRecentlySynced(scope.productionProjectId, "apple", "production"),
				`The Apple catalog was synchronized within the past ${catalogStaleHours} hours.`,
				`Synchronize the production Apple catalog. Its approval and availability evidence must be less than ${catalogStaleHours} hours old.`,
			),
			prerequisite(
				"production_apple_products_approved",
				"Production Apple product approval",
				productionApple.approved,
				"Weekly and yearly subscriptions are approved by Apple.",
				"Submit the weekly and yearly subscriptions to Apple and wait until both are approved.",
			),
			prerequisite(
				"production_apple_products_purchasable",
				"Production Apple territory availability",
				productionApple.available,
				"Weekly and yearly subscriptions are available in at least one App Store territory.",
				"Make the weekly and yearly subscriptions available in at least one App Store territory, then synchronize the catalog again.",
			),
			prerequisite(
				"sandbox_google_catalog",
				"Sandbox Google subscription catalog",
				sandboxGoogle.catalog,
				"Weekly and yearly subscriptions are imported.",
				"Import the weekly and yearly Google subscriptions into the test project.",
			),
			prerequisite(
				"production_google_catalog",
				"Production Google subscription catalog",
				productionGoogle.catalog,
				"Weekly and yearly subscriptions are imported.",
				"Import the weekly and yearly Google subscriptions into the production project.",
			),
			prerequisite(
				"production_google_catalog_fresh",
				"Production Google catalog freshness",
				catalogRecentlySynced(scope.productionProjectId, "google", "production"),
				`The Google catalog was synchronized within the past ${catalogStaleHours} hours.`,
				`Synchronize the production Google catalog. Its activation and availability evidence must be less than ${catalogStaleHours} hours old.`,
			),
			prerequisite(
				"production_google_products_active",
				"Production Google base plan activation",
				productionGoogle.approved,
				"Weekly and yearly subscriptions have active base plans.",
				"Activate a base plan for the weekly and yearly Google subscriptions, then synchronize the catalog again.",
			),
			prerequisite(
				"production_google_products_purchasable",
				"Production Google regional availability",
				productionGoogle.available,
				"Weekly and yearly subscriptions are available to new subscribers in at least one region.",
				"Enable new subscriber availability for the weekly and yearly Google subscriptions in at least one region, then synchronize the catalog again.",
			),
			prerequisite(
				"sandbox_premium_entitlement",
				"Test Premium entitlement",
				hasEntitlement(scope.testProjectId),
				"The Premium entitlement is active.",
				"Create and activate the Premium entitlement in the test project.",
			),
			prerequisite(
				"production_premium_entitlement",
				"Production Premium entitlement",
				hasEntitlement(scope.productionProjectId),
				"The Premium entitlement is active.",
				"Create and activate the Premium entitlement in the production project.",
			),
			prerequisite(
				"sandbox_native_entitlement_mappings",
				"Test native Premium mappings",
				sandboxApple.premium && sandboxGoogle.premium,
				"Every required native subscription grants Premium.",
				"Map the test weekly and yearly Apple and Google subscriptions to Premium.",
			),
			prerequisite(
				"production_native_entitlement_mappings",
				"Production native Premium mappings",
				productionApple.premium && productionGoogle.premium,
				"Every required native subscription grants Premium.",
				"Map the production weekly and yearly Apple and Google subscriptions to Premium.",
			),
			prerequisite(
				"sandbox_default_offering",
				"Test default offering",
				hasOffering(scope.testProjectId),
				"The current default offering is active.",
				"Create and activate the current default offering in the test project.",
			),
			prerequisite(
				"production_default_offering",
				"Production default offering",
				hasOffering(scope.productionProjectId),
				"The current default offering is active.",
				"Create and activate the current default offering in the production project.",
			),
			prerequisite(
				"sandbox_native_packages",
				"Test native packages",
				sandboxApple.packages && sandboxGoogle.packages,
				"Weekly and yearly native subscriptions are exposed by the default offering.",
				"Add the test weekly and yearly Apple and Google subscriptions to matching packages in the default offering.",
			),
			prerequisite(
				"production_native_packages",
				"Production native packages",
				productionApple.packages && productionGoogle.packages,
				"Weekly and yearly native subscriptions are exposed by the default offering.",
				"Add the production weekly and yearly Apple and Google subscriptions to matching packages in the default offering.",
			),
			prerequisite(
				"production_legacy_inventory",
				"Production legacy subscription inventory",
				legacyInventoryReady,
				legacyInventoryDetail,
				legacyInventoryDetail,
			),
		];
		return c.json({
			data: {
				...
				operational_policy: {
					...
					catalog_stale_after_hours: catalogStaleHours,
					billing_worker: workerReadiness,
				},
...
admin.post("/:projectId/certification-runs", async (c) => {
	try {
		...
		if (!["ios", "android", "web", "cross_platform"].includes(platform)) {
			throw purchasesError(
				"certification_platform_invalid",
				"Platform must be iOS, Android, Web, or cross-platform",
			);
		}
		if (!["sandbox", "production"].includes(environment)) {
			throw purchasesError(
				"certification_environment_invalid",
				"Environment must be sandbox or production",
			);
		}
		const targetProjectId =
			environment === "sandbox" ? scope.testProjectId : scope.productionProjectId;
		if (!targetProjectId)
			throw purchasesError(
				"certification_target_project_missing",
				`The ${environment} project is not configured`,
				409,
			);
		...
		if ((platform === "ios" || platform === "android") && (!deviceModel || !osVersion)) {
			throw purchasesError(
				"certification_device_required",
				"Native certification runs require a device model and OS version",
			);
		}
    ...
    `,
			...
			.bind(
				id,
				scope.releaseProjectId,
				targetProjectId,
				environment,
				platform,
				buildNumber,
				optionalCertificationText(data.app_version, 100),
				optionalCertificationText(data.sdk_version, 100),
				deviceModel,
				osVersion,
				optionalCertificationText(data.notes, 2000),
				c.get("operatorId") ?? String(c.get("userId")),
			)
			.first<Record<string, unknown>>();
...
admin.post("/:projectId/certification-runs/:runId/device-challenge", async (c) => {
    ...
    `,
			...
			.bind(runId, scope.releaseProjectId)
			.first<{ id: string }>();
		if (!run)
			throw purchasesError(
				"certification_run_not_running",
				"Running certification run not found",
				404,
			);
		...
		await audit(
			c,
			scope.releaseProjectId,
			"certification_run.device_challenge_rotated",
			"certification_run",
			runId,
			{
...
admin.patch("/:projectId/certification-runs/:runId", async (c) => {
	try {
		...
		if (!["completed", "failed", "cancelled"].includes(status)) {
			throw purchasesError(
				"certification_run_status_invalid",
				"A running certification can be completed, failed, or cancelled",
			);
		}
		if (status === "completed") {
      ...
      `,
				...
				.bind(c.req.param("runId"), scope.releaseProjectId)
				.first<{ id: string }>();
			if (!observation)
				throw purchasesError(
					"certification_observation_required",
					"Record at least one result before completing the certification run",
					409,
				);
		}
		const row = await c.env.DB.prepare(
    ...
    `,
			...
			.bind(status, c.req.param("runId"), scope.releaseProjectId)
			.first<Record<string, unknown>>();
		if (!row)
			throw purchasesError(
				"certification_run_not_running",
...
admin.post("/:projectId/certification-runs/:runId/observations", async (c) => {
    ...
    `,
			...
			.bind(c.req.param("runId"), scope.releaseProjectId)
			.first<CertificationRunRow>();
		if (!run)
			throw purchasesError("certification_run_not_found", "Certification run not found", 404);
		if (run.status !== "running")
			throw purchasesError(
				"certification_run_closed",
				"Certification results can be added only while the run is active",
				409,
			);
		...
		if (!definition)
			throw purchasesError("release_gate_check_not_found", "Release gate check not found", 404);
		...
		if (!["passed", "failed"].includes(outcome))
			throw purchasesError("certification_outcome_invalid", "Outcome must be passed or failed");
		...
		if (!definition.reference_types.includes(referenceType)) {
			throw purchasesError(
				"certification_reference_type_invalid",
				"The selected reference type is not valid for this check",
			);
		}
		...
		const deviceResultId = requiresAuthenticatedDeviceEvidence(definition)
			? referenceType === "test_run"
				? referenceId
				: certificationText(data.device_result_id, "device_result_id", 500)
			: null;
		const deviceReference = deviceResultId
			? referenceType === "test_run"
				? reference
				: await certificationReferenceSnapshot(
						c.env.DB,
						run,
						definition.provider,
						"test_run",
			...
			...(deviceReference
				? {
						device_reference: {
							type: "test_run",
							id: deviceResultId,
							verified_record: deviceReference,
						},
					}
				: {}),
		};
		...
		const gateEvidence = {
			...
			...(deviceResultId ? { device_result_id: deviceResultId } : {}),
		};
      ...
      `,
		...
		await audit(
			c,
			scope.releaseProjectId,
			"certification_observation.created",
			...
			observationId,
			{
				...
				device_result_id: deviceResultId,
				evidence_sha256: digest,
			},
		);
		return c.json(
...
admin.patch("/:projectId/release-gate/checks/:checkKey", async (c) => {
	try {
		...
		if (encodedEvidence.length > 16_384)
			throw purchasesError("release_gate_evidence_too_large", "Evidence is limited to 16 KB", 413);
		const evidenceValidation = validateReleaseGateEvidence(definition, evidence);
		if (status === "passed" && !evidenceValidation.valid) {
			throw purchasesError(
				"release_gate_evidence_required",
				`Passing this check requires: ${evidenceValidation.missing.join(", ")}`,
			);
		}
		if (status === "passed") {
      ...
      `,
			...
			if (
				!observation ||
				(await sha256Hex(observation.evidence_json)) !== observation.evidence_sha256
			) {
				throw purchasesError(
					"release_gate_certification_observation_required",
					"A completed immutable certification observation is required",
					409,
				);
			}
		}
    ...
    `,
		...
		await audit(
			c,
			scope.releaseProjectId,
			"release_gate_check.updated",
			"release_gate_check",
			checkKey,
			{ status, evidence_fields: Object.keys(evidence) },
		);
...
function providerDisplayName(provider: "apple" | "google") {
	return provider === "apple" ? "Apple App Store" : "Google Play";
}
...
export async function certificationReferenceSnapshot(
    ...
    `,
			...
			.bind(referenceId, run.id, run.target_project_id, checkKey)
			.first<Record<string, unknown>>();
		if (!row) {
			throw purchasesError(
		...
		if (row.outcome !== expectedOutcome) {
			throw purchasesError(
				"certification_device_result_outcome_mismatch",
				"Device result outcome does not match the review outcome",
				409,
			);
		}
		if ((await sha256Hex(String(row.evidence_json || ""))) !== row.evidence_sha256) {
			throw purchasesError(
				"certification_device_result_invalid",
				"Device result evidence digest is invalid",
				409,
			);
		}
    ...
    `,
			...
			.bind(referenceId, run.target_project_id)
			.first<Record<string, unknown>>();
		if (!row || !row.verified_at)
			throw purchasesError(
				"certification_transaction_not_verified",
				"A provider-verified transaction from this certification project is required",
				409,
			);
		if (provider !== "cross_platform" && row.store !== provider)
			throw purchasesError(
				"certification_reference_provider_mismatch",
				"The transaction provider does not match this check",
				409,
			);
		if (row.environment !== run.environment)
			throw purchasesError(
				"certification_reference_environment_mismatch",
				"The transaction environment does not match this run",
				409,
			);
		requireReferenceDuringRun(
			row.event_occurred_at || row.purchased_at || row.created_at,
			run.started_at,
		);
		...
		return {
			...
			occurred_at: row.event_occurred_at || row.purchased_at,
    ...
    `,
		...
		if (!row || row.status !== "processed")
			throw purchasesError(
				"certification_event_not_processed",
				"A processed immutable Billing event from this certification project is required",
				409,
			);
		if (provider !== "cross_platform" && row.provider !== provider)
			throw purchasesError(
				"certification_reference_provider_mismatch",
				"The Billing event provider does not match this check",
				409,
			);
		if (provider === "cross_platform" && !["apple", "google"].includes(String(row.provider))) {
			throw purchasesError(
				"certification_reference_provider_mismatch",
				"Cross-platform convergence requires an Apple or Google Play event",
				409,
			);
		}
		if (row.environment !== run.environment)
			throw purchasesError(
				"certification_reference_environment_mismatch",
				"The Billing event environment does not match this run",
				409,
			);
    ...
    `,
		...
		if (!row)
			throw purchasesError(
				"certification_paywall_event_not_found",
				"A paywall event from this certification project is required",
				409,
			);
		if (run.platform !== "cross_platform" && row.platform !== run.platform) {
			throw purchasesError(
				"certification_reference_platform_mismatch",
				"The paywall event platform does not match this run",
				409,
			);
		}
  ...
  `,
	...
	if (!row || row.status !== "completed" || Number(row.unresolved_subscriptions || 0) !== 0) {
		throw purchasesError(
			"certification_legacy_inventory_incomplete",
			"A completed production inventory with no unresolved subscription is required",
			409,
		);
	}
	return row;
...
function validateCertificationScenario(
	...
	if (scenario === "weekly_purchase" || scenario === "yearly_purchase") {
		...
		valid = String(row.period_type || "").toLocaleLowerCase("en") === "trial" || matches(/trial/);
	else if (scenario === "renewal")
		valid = matches(/renew|did_renew|invoice\.paid|payment_succeeded/);
	else if (scenario === "upgrade_downgrade")
		valid = matches(/upgrade|downgrade|product.?chang|plan.?chang|change.?renewal.?pref/);
		...
		valid = matches(/checkout|initial|purchase|invoice\.paid|payment_succeeded|active|trialing/);
	else if (scenario === "payment_failed")
		valid = matches(/payment.?fail|invoice.?fail|billing.?issue|past_due/);
	else if (scenario === "dispute") valid = matches(/dispute|inquiry|chargeback/);
	else if (scenario === "user_cancelled" && referenceType === "paywall_event")
		valid = matches(/purchase_cancelled|closed/);
	else if (scenario === "pending" && referenceType === "paywall_event")
		valid = matches(/purchase_started/);
	else if (scenario === "restore" && referenceType === "paywall_event")
		valid = matches(/restore_succeeded/);
	if (!valid) {
		throw purchasesError(
			"certification_reference_scenario_mismatch",
			"The referenced record does not prove this certification scenario",
			409,
		);
	}
}
...
function requireReferenceDuringRun(value: unknown, startedAt: string) {
	...
	if (
		...
		referenceTime < runStart - 600_000
	) {
		throw purchasesError(
			"certification_reference_outside_run",
			"The referenced record must be observed during this certification run",
			409,
		);
	}
}
...
admin.patch("/:projectId/features", async (c) => {
	try {
		...
		const statements = FEATURE_FLAGS.filter((flag) => data[flag] !== undefined).map((flag) =>
			c.env.DB.prepare(
				`UPDATE billing_feature_flags SET ${flag} = ?, updated_by = ?, updated_at = datetime('now') WHERE project_id = ?`,
			).bind(data[flag] ? 1 : 0, c.get("operatorId") ?? String(c.get("userId")), project.id),
		);
		if (!statements.length)
			throw purchasesError("feature_flags_required", "At least one known feature flag is required");
		await c.env.DB.batch(statements);
		await audit(c, project.id, "features.updated", "billing_feature_flags", project.id, {
			flags: FEATURE_FLAGS.filter((flag) => data[flag] !== undefined),
		});
		return c.json({ data: await featureFlagsFor(c, project.id) });
	} catch (error) {
		return replyError(c, error);
...
admin.post("/:projectId/legacy/revenuecat/connections", async (c) => {
	try {
		...
		if (!apiKey || apiKey.length > 4096) {
			throw purchasesError(
				"legacy_api_key_invalid",
				"A valid legacy V2 secret API key is required",
			);
		}
		...
		const [apiCiphertext, billingResult] = await Promise.all([
			...
			callBillingServiceBinding<{ data: { ciphertext: string } }>(
				c.env,
				"/internal/v1/credentials/encrypt",
				{
		...
		if (!billingCiphertext)
			throw purchasesError(
				"billing_credential_copy_failed",
				"Billing credential encryption failed",
				503,
				true,
			);
...
admin.post("/:projectId/legacy/revenuecat/connection-test", async (c) => {
    ...
    `,
		...
		if (!source)
			throw purchasesError(
				"legacy_source_not_configured",
				"Configure the legacy source first",
				409,
			);
		sourceId = source.id;
    ...
    `,
      ...
      `,
				...
				.bind(
					tagged.code || "legacy_connection_failed",
					String(tagged.message || error).slice(0, 1000),
					sourceId,
				)
				.run()
				.catch(() => undefined);
...
admin.delete("/:projectId/legacy/revenuecat/connection", async (c) => {
    ...
    `,
			...
			.first<{ id: string }>();
		if (!source)
			throw purchasesError("legacy_source_not_configured", "Legacy source is not configured", 404);
		await audit(c, project.id, "legacy_source.disabled", "legacy_source", source.id, {
			credentials_destroyed: true,
		});
		return c.json({
			data: { id: source.id, status: "disabled", credentials_destroyed: true },
		});
	} catch (error) {
		return replyError(c, error);
	}
...
admin.post("/:projectId/legacy/revenuecat/inventory-runs", async (c) => {
    ...
    `,
		...
		if (!source || source.status !== "connected") {
			throw purchasesError(
				"legacy_source_not_connected",
				"Test the legacy source connection before starting inventory",
				409,
			);
		}
		const active = await c.env.DB.prepare(
    ...
    `,
		...
		await audit(c, project.id, "legacy_inventory.started", "legacy_inventory_run", runId, {
			environment,
		});
...
admin.post("/:projectId/legacy/revenuecat/inventory-runs/:runId/cancel", async (c) => {
    ...
    `,
			...
			.bind(c.req.param("runId"), project.id)
			.first<Record<string, unknown>>();
		if (!run)
			throw purchasesError(
				"legacy_inventory_not_active",
				"Active legacy inventory run not found",
				409,
			);
		await audit(
			c,
			project.id,
			"legacy_inventory.cancelled",
			"legacy_inventory_run",
			c.req.param("runId"),
			{
				environment: run.environment,
			},
...
admin.get("/:projectId/connections", async (c) => {
	try {
        ...
        WHERE project_id = ? AND provider IN ('apple','google')
      ...
      `,
		...
		const rows: Array<Record<string, any>> = (stored.results || []).map((row) => ({
			...row,
			capabilities: JSON.parse(row.capabilities || "{}"),
			public_configuration: JSON.parse(row.public_configuration || "{}"),
		}));
		for (const item of native) {
			const existing = rows.find(
				(row) => row.provider === item.provider && row.environment === item.environment,
			);
			if (existing) {
				...
				existing.capabilities = Object.keys(existing.capabilities || {}).length
	...
	try {
		...
		try {
			...
			await audit(
				c,
				scope.productionProjectId,
				"apple_server_notifications.configuration_failed",
				"store_connection",
				"native-apple",
				{
					code: String(
						(error as { code?: string }).code || "apple_notification_configuration_failed",
					),
				},
			);
			throw error;
...
admin.post("/:projectId/connections", async (c) => {
	try {
		...
		if (!PROVIDERS.includes(provider as any))
			throw purchasesError("unsupported_provider", "Unsupported billing provider");
		if (!ENVIRONMENTS.includes(environment as any))
			throw purchasesError("invalid_environment", "Environment must be sandbox or production");
		const expectedEnvironment = project.isTest ? "sandbox" : "production";
		if (environment !== expectedEnvironment) {
			throw purchasesError(
				"invalid_environment",
				`This project requires the ${expectedEnvironment} environment`,
				422,
			);
		}
    ...
    `,
			...
			.bind(project.id, provider, environment)
			.first<{
				id: string;
				configuration_encrypted: string | null;
		...
		const publicConfiguration = objectValue(data.public_configuration);
		if (data.secret_configuration !== undefined) {
			throw purchasesError(
				"native_store_credentials_managed_separately",
				"Apple App Store and Google Play credentials are managed by their platform configuration",
				422,
			);
		}
      ...
      `,
				...
				c.get("operatorId") ?? String(c.get("userId")),
				connectionId,
				JSON.stringify({ provider, environment }),
			),
		]);
		return c.json(
			{
...
admin.post("/:projectId/connections/:provider/test", async (c) => {
	...
	try {
		...
		if (!PROVIDERS.includes(provider as any))
			throw purchasesError(
				...
				"Only Apple App Store and Google Play are enabled",
				422,
			);
		projectId = project.id;
		const data = await jsonBody(c);
		environment = ENVIRONMENTS.includes(data.environment)
			? data.environment
			: project.isTest
				? "sandbox"
				: "production";
		const result: Record<string, unknown> = await testStoreCredentials(c.env, {
			projectId: project.id,
    ...
    `,
			...
			.bind(
				...
				provider,
				environment,
				provider === "apple" ? "Apple App Store" : provider === "google" ? "Google Play" : provider,
				JSON.stringify(connectionCapabilities(provider, true)),
			)
			.run();
      ...
      `,
				...
				.bind(
					...
					environment,
					provider,
					JSON.stringify(connectionCapabilities(provider, false)),
					value.code || "connection_test_failed",
					value.message || "Connection test failed",
					value.code || "connection_test_failed",
					value.message || "Connection test failed",
				)
				.run()
...
admin.get("/:projectId/transactions", async (c) => {
    ...
    `,
		...
		return c.json({
			data: rows.results,
			next_cursor: nextCursor(rows.results || [], limit),
		});
	} catch (error) {
		return replyError(c, error);
	}
...
admin.get("/:projectId/provider-events", async (c) => {
	try {
		...
		if (status && !["received", "processed", "failed"].includes(status)) {
			throw purchasesError(
				"provider_event_status_invalid",
				"Provider event status is invalid",
				422,
			);
		}
		const rows = await c.env.DB.prepare(
...
admin.get("/:projectId/dead-letters", async (c) => {
	try {
		const project = await projectFor(c);
		const rows = await c.env.DB.prepare(
			`
      SELECT id, queue_name, job_type, job_payload_sha256, job_valid,
        delivery_attempts, status, received_at, replay_requested_at,
        replay_requested_by, discarded_at, discarded_by,
        CASE WHEN status = 'quarantined' AND job_valid = 1 THEN 1 ELSE 0 END AS replay_available
      FROM billing_dead_letters
      WHERE project_id = ?
...
admin.post("/:projectId/dead-letters/:deadLetterId/replay", async (c) => {
	try {
		const project = await projectFor(c);
		requireAdmin(project);
		const deadLetter = await c.env.DB.prepare(
			`
      SELECT id, status, job_valid, job_payload, job_payload_sha256
      FROM billing_dead_letters WHERE id = ? AND project_id = ? LIMIT 1
    `,
		)
			.bind(c.req.param("deadLetterId"), project.id)
			.first<Record<string, unknown>>();
		if (!deadLetter)
			throw purchasesError("dead_letter_not_found", "Dead-letter job not found", 404);
		if (deadLetter.status !== "quarantined" || Number(deadLetter.job_valid) !== 1) {
			throw purchasesError(
				"dead_letter_replay_unavailable",
				"Dead-letter replay is unavailable",
				409,
			);
		}
		const payload = String(deadLetter.job_payload || "");
		if ((await sha256Hex(payload)) !== deadLetter.job_payload_sha256) {
			throw purchasesError(
				"dead_letter_integrity_failed",
				"Dead-letter payload integrity check failed",
				409,
			);
		}
		let job: unknown;
		try {
			...
			throw purchasesError("dead_letter_payload_invalid", "Dead-letter payload is invalid", 409);
		}
		...
		const claimed = await c.env.DB.prepare(
			`
      UPDATE billing_dead_letters
      SET status = 'replay_queued', replay_requested_by = ?,
        replay_requested_at = datetime('now'), updated_at = datetime('now')
      WHERE id = ? AND project_id = ? AND status = 'quarantined'
    `,
		)
			.bind(c.get("operatorId") ?? String(c.get("userId")), deadLetter.id, project.id)
			.run();
		if (Number(claimed.meta.changes || 0) !== 1) {
			throw purchasesError(
				"dead_letter_replay_conflict",
				"Dead-letter job was already handled",
				409,
			);
		}
		try {
			...
			await c.env.DB.prepare(
				`
        UPDATE billing_dead_letters
        SET status = 'quarantined', replay_requested_by = NULL,
          replay_requested_at = NULL, updated_at = datetime('now')
        WHERE id = ? AND status = 'replay_queued'
      `,
			)
				.bind(deadLetter.id)
				.run();
		...
		await audit(c, project.id, "dead_letter.replay_queued", "dead_letter", String(deadLetter.id), {
			job_type: job.type,
		});
		return c.json({ replay_queued: true }, 202);
	} catch (error) {
...
admin.post("/:projectId/dead-letters/:deadLetterId/discard", async (c) => {
	try {
		const project = await projectFor(c);
		requireAdmin(project);
		const discarded = await c.env.DB.prepare(
			`
      UPDATE billing_dead_letters
      SET status = 'discarded', discarded_by = ?, discarded_at = datetime('now'),
        updated_at = datetime('now')
      WHERE id = ? AND project_id = ? AND status = 'quarantined'
    `,
		)
			.bind(c.get("operatorId") ?? String(c.get("userId")), c.req.param("deadLetterId"), project.id)
			.run();
		if (Number(discarded.meta.changes || 0) !== 1) {
			throw purchasesError(
				"dead_letter_discard_unavailable",
				"Quarantined dead-letter job not found",
				409,
			);
		}
		await audit(c, project.id, "dead_letter.discarded", "dead_letter", c.req.param("deadLetterId"));
...
admin.get("/:projectId/subscriptions", async (c) => {
    ...
    `,
			...
			.bind(project.id, status || null, status || null, limit)
			.all();
		return c.json({ data: rows.results });
	} catch (error) {
		return replyError(c, error);
...
admin.get("/:projectId/customers", async (c) => {
    ...
    `,
			...
			.bind(project.id, query, query, cursor, cursor, limit + 1)
			.all<Record<string, any>>();
		...
		return c.json({
			data: page,
			next_cursor: hasMore ? page[page.length - 1]?.last_seen_at || null : null,
		});
	} catch (error) {
		return replyError(c, error);
	}
...
admin.get("/:projectId/customers/:customerId", async (c) => {
	try {
		...
		const customer = await c.env.DB.prepare(
			"SELECT * FROM billing_customers WHERE id = ? AND project_id = ?",
		)
			.bind(c.req.param("customerId"), project.id)
			.first<Record<string, any>>();
		if (!customer) throw purchasesError("customer_not_found", "Customer not found", 404);
		const [transactions, events, paywallEvents, aliases] = await Promise.all([
			c.env.DB.prepare(
				`SELECT t.*, p.store_product_id FROM billing_transactions t LEFT JOIN billing_products p ON p.id = t.product_id WHERE t.customer_id = ? ORDER BY t.created_at DESC LIMIT 250`,
			)
				.bind(customer.id)
				.all(),
			c.env.DB.prepare(
				`SELECT * FROM billing_events WHERE customer_id = ? ORDER BY occurred_at DESC LIMIT 250`,
			)
				.bind(customer.id)
				.all(),
			c.env.DB.prepare(
				`SELECT id, paywall_id, paywall_version_id, placement_identifier,
          experiment_id, variant_id, event_type, package_identifier, platform,
          country, app_version, sdk_version, occurred_at
        FROM billing_paywall_events
        WHERE project_id = ? AND customer_id = ?
        ORDER BY occurred_at DESC LIMIT 250`,
			)
				.bind(project.id, customer.id)
				.all(),
			c.env.DB.prepare(
				`SELECT app_user_id, alias_type, created_at FROM billing_customer_aliases WHERE customer_id = ? ORDER BY created_at`,
			)
				.bind(customer.id)
				.all(),
		]);
		return c.json({
			...
			paywall_events: paywallEvents.results,
		});
	} catch (error) {
		return replyError(c, error);
	}
...
admin.post("/:projectId/customers/:customerId/block", async (c) => {
	try {
		...
		const result = await c.env.DB.prepare(
			`UPDATE billing_customers SET blocked = ?, updated_at = datetime('now') WHERE id = ? AND project_id = ?`,
		)
			.bind(blocked, c.req.param("customerId"), project.id)
			.run();
...
admin.post("/:projectId/customers/:customerId/entitlements/:entitlementId", async (c) => {
	try {
		...
		const [customer, entitlement] = await Promise.all([
			c.env.DB.prepare("SELECT id FROM billing_customers WHERE id = ? AND project_id = ?")
				.bind(c.req.param("customerId"), project.id)
				.first<{ id: string }>(),
			c.env.DB.prepare("SELECT id FROM billing_entitlements WHERE id = ? AND project_id = ?")
				.bind(c.req.param("entitlementId"), project.id)
				.first<{ id: string }>(),
		]);
		if (!customer || !entitlement)
			throw purchasesError(
				"customer_or_entitlement_not_found",
				"Customer or entitlement not found",
				404,
			);
		await c.env.DB.prepare(
			`
    ...
    `,
			...
			.bind(project.id, customer.id, entitlement.id, data.expires_at || null)
			.run();
		await recordAdminBillingEvent(c, project, customer.id, "promotional_granted", {
		...
		await audit(c, project.id, "entitlement.granted", "customer", customer.id, {
			entitlement_id: entitlement.id,
		});
...
admin.delete("/:projectId/customers/:customerId/entitlements/:entitlementId", async (c) => {
    ...
    `,
			...
			.bind(project.id, c.req.param("customerId"), c.req.param("entitlementId"))
			.run();
		if (!result.meta.changes)
			throw purchasesError(
				"promotional_entitlement_not_found",
				"Promotional entitlement not found",
				404,
			);
		await recordAdminBillingEvent(c, project, c.req.param("customerId"), "promotional_revoked", {
			entitlement_id: c.req.param("entitlementId"),
		});
		await queueCustomerEntitlementChanged(c.env, {
			projectId: project.id,
		...
		await audit(c, project.id, "entitlement.revoked", "customer", c.req.param("customerId"), {
			entitlement_id: c.req.param("entitlementId"),
		});
...
admin.post("/:projectId/customers/:customerId/merge", async (c) => {
	try {
		...
		const [source, target] = await Promise.all([
			c.env.DB.prepare(
				"SELECT primary_app_user_id FROM billing_customers WHERE id = ? AND project_id = ?",
			)
				.bind(c.req.param("customerId"), project.id)
				.first<{ primary_app_user_id: string }>(),
			c.env.DB.prepare(
				"SELECT primary_app_user_id FROM billing_customers WHERE id = ? AND project_id = ?",
			)
				.bind(String(data.target_customer_id), project.id)
				.first<{ primary_app_user_id: string }>(),
		]);
		if (!source || !target)
			throw purchasesError("customer_not_found", "Source or target customer not found", 404);
		const merged = await identifyCustomer(
			c.env.DB,
			project.id,
			source.primary_app_user_id,
			target.primary_app_user_id,
		);
		await audit(
			c,
			project.id,
			"customer.merged",
			"customer",
			String(merged?.id || data.target_customer_id),
			{ source_customer_id: c.req.param("customerId") },
		);
...
admin.delete("/:projectId/customers/:customerId", async (c) => {
	try {
		...
		const customer = await c.env.DB.prepare(
			"SELECT id FROM billing_customers WHERE id = ? AND project_id = ?",
		)
		...
		await c.env.DB.batch([
			c.env.DB.prepare(
				"UPDATE billing_transactions SET customer_id = NULL WHERE project_id = ? AND customer_id = ?",
			).bind(project.id, customerId),
			c.env.DB.prepare(
				"UPDATE billing_subscriptions SET customer_id = NULL WHERE project_id = ? AND customer_id = ?",
			).bind(project.id, customerId),
			c.env.DB.prepare(
				"UPDATE billing_events SET customer_id = NULL WHERE project_id = ? AND customer_id = ?",
			).bind(project.id, customerId),
			c.env.DB.prepare(
				"DELETE FROM billing_customer_entitlements WHERE project_id = ? AND customer_id = ?",
			).bind(project.id, customerId),
			c.env.DB.prepare(
				"DELETE FROM billing_balance_ledger WHERE project_id = ? AND customer_id = ?",
			).bind(project.id, customerId),
			c.env.DB.prepare("DELETE FROM billing_experiment_assignments WHERE customer_id = ?").bind(
				customerId,
			),
			c.env.DB.prepare(
				"DELETE FROM billing_customer_aliases WHERE project_id = ? AND customer_id = ?",
			).bind(project.id, customerId),
			c.env.DB.prepare("DELETE FROM billing_customers WHERE id = ? AND project_id = ?").bind(
				customerId,
				project.id,
			),
		]);
...
admin.get("/:projectId/events", async (c) => {
	try {
		...
		const limit = parseLimit(c.req.query("limit"));
		const rows = await c.env.DB.prepare(
			`SELECT * FROM billing_events WHERE project_id = ? ORDER BY occurred_at DESC LIMIT ?`,
		)
...
admin.get("/:projectId/analytics", async (c) => {
	try {
      ...
      `,
		...
		const transactionCount = (series.results || []).reduce(
			(total: number, row: any) => total + Number(row.purchases || 0) + Number(row.renewals || 0),
			0,
		);
		return c.json({
			range_days: days,
			summary: {
				...
				churn_rate:
					subscriptions > 0 ? Number(subscriptionMetrics?.churned || 0) / subscriptions : 0,
...
admin.post("/:projectId/placements", async (c) => {
    ...
    `,
			...
			.bind(
				...
				data.display_name || data.identifier,
				data.description || null,
				data.default_offering_id || null,
			)
			.run();
...
admin.patch("/:projectId/placements/:id", async (c) => {
    ...
    `,
			...
			.run();
		if (!result.meta.changes)
			throw purchasesError("placement_not_found", "Placement not found", 404);
...
admin.post("/:projectId/paywalls", async (c) => {
	try {
		...
		requireAdmin(project);
		const data = await jsonBody(c);
		const paywallId = crypto.randomUUID();
		...
		await c.env.DB.batch([
			c.env.DB.prepare(
				`INSERT INTO billing_paywalls (id, project_id, identifier, display_name, offering_id) VALUES (?, ?, ?, ?, ?)`,
			).bind(
				paywallId,
				project.id,
				identifier(data.identifier),
			...
			c.env.DB.prepare(
				`INSERT INTO billing_paywall_versions (id, paywall_id, version, state, configuration, localizations, created_by) VALUES (?, ?, 1, 'draft', ?, ?, ?)`,
			).bind(
				versionId,
				paywallId,
		...
		await audit(c, project.id, "paywall.created", "paywall", paywallId, {
...
admin.get("/:projectId/paywalls/:id/versions", async (c) => {
	try {
		const project = await projectFor(c);
		const paywall = await c.env.DB.prepare(
			"SELECT id FROM billing_paywalls WHERE id = ? AND project_id = ?",
		)
			.bind(c.req.param("id"), project.id)
			.first();
		if (!paywall) throw purchasesError("paywall_not_found", "Paywall not found", 404);
		const rows = await c.env.DB.prepare(
			"SELECT * FROM billing_paywall_versions WHERE paywall_id = ? ORDER BY version DESC",
		)
			.bind(c.req.param("id"))
			.all<Record<string, any>>();
		return c.json({
			data: (rows.results || []).map((row) => ({
				...
				localizations: JSON.parse(row.localizations || "{}"),
			})),
		});
	} catch (error) {
		return replyError(c, error);
...
admin.post("/:projectId/paywalls/:id/versions", async (c) => {
	try {
		...
		const current = await c.env.DB.prepare(
			`SELECT COALESCE(MAX(v.version), 0) AS version FROM billing_paywall_versions v JOIN billing_paywalls p ON p.id = v.paywall_id WHERE p.id = ? AND p.project_id = ?`,
		)
			.bind(c.req.param("id"), project.id)
		...
		await c.env.DB.prepare(
			`INSERT INTO billing_paywall_versions (id, paywall_id, version, state, configuration, localizations, created_by) VALUES (?, ?, ?, 'draft', ?, ?, ?)`,
		)
			.bind(
				id,
				c.req.param("id"),
...
admin.post("/:projectId/paywalls/:id/publish", async (c) => {
    ...
    `,
			...
			.bind(String(data.version_id), c.req.param("id"), project.id)
			.first<{ id: string }>();
		if (!version)
			throw purchasesError("paywall_version_not_found", "Paywall version not found", 404);
		await c.env.DB.batch([
			c.env.DB.prepare(
				`UPDATE billing_paywall_versions SET state = CASE WHEN id = ? THEN 'published' ELSE state END, published_at = CASE WHEN id = ? THEN datetime('now') ELSE published_at END WHERE paywall_id = ?`,
			).bind(version.id, version.id, c.req.param("id")),
			c.env.DB.prepare(
				`UPDATE billing_paywalls SET active_version_id = ?, updated_at = datetime('now') WHERE id = ? AND project_id = ?`,
			).bind(version.id, c.req.param("id"), project.id),
		]);
		await audit(c, project.id, "paywall.published", "paywall", c.req.param("id"), {
...
admin.post("/:projectId/targeting", async (c) => {
	try {
		...
		const data = await jsonBody(c);
		const id = crypto.randomUUID();
		if (!["draft", "live", "scheduled", "inactive"].includes(data.state || "draft"))
			throw purchasesError("invalid_rule_state", "Invalid targeting rule state");
		await c.env.DB.prepare(
			`
...
admin.patch("/:projectId/targeting/:id", async (c) => {
    ...
    `,
			...
			.bind(
				...
				data.priority ?? null,
				data.state ?? null,
				data.conditions === undefined ? null : JSON.stringify(data.conditions),
				data.offering_id ?? null,
				data.starts_at ?? null,
				data.ends_at ?? null,
				c.req.param("id"),
			...
			.run();
		if (!result.meta.changes)
			throw purchasesError("targeting_rule_not_found", "Targeting rule not found", 404);
		return c.json({ updated: true });
	} catch (error) {
		return replyError(c, error);
	}
...
admin.get("/:projectId/experiments", async (c) => {
	try {
		const project = await projectFor(c);
		const experiments = await c.env.DB.prepare(
			`SELECT e.*, p.identifier AS placement_identifier FROM billing_experiments e JOIN billing_placements p ON p.id = e.placement_id WHERE e.project_id = ? ORDER BY e.created_at DESC`,
		)
			.bind(project.id)
			.all<Record<string, any>>();
		const data = [];
		for (const experiment of experiments.results || []) {
			const variants = await c.env.DB.prepare(
				`SELECT v.*, o.identifier AS offering_identifier FROM billing_experiment_variants v JOIN billing_offerings o ON o.id = v.offering_id WHERE v.experiment_id = ? ORDER BY v.created_at`,
			)
				.bind(experiment.id)
				.all();
      ...
      `,
			...
			data.push({
				...experiment,
				audience_conditions: JSON.parse(experiment.audience_conditions || "[]"),
...
admin.post("/:projectId/experiments", async (c) => {
	try {
		...
		if (variants.length < 2)
			throw purchasesError("experiment_variants_required", "At least two variants are required");
    ...
    `,
				...
				project.id,
				data.placement_id,
				data.display_name || "Experiment",
				JSON.stringify(data.audience_conditions || []),
				data.starts_at || null,
				data.ends_at || null,
			),
		];
      ...
      `,
					...
					crypto.randomUUID(),
					id,
					identifier(variant.identifier, "variant identifier"),
					variant.offering_id,
		...
		await c.env.DB.batch(statements);
		await audit(c, project.id, "experiment.created", "experiment", id, {
...
admin.patch("/:projectId/experiments/:id", async (c) => {
	try {
		...
		requireAdmin(project);
		const data = await jsonBody(c);
		if (data.state && !["draft", "running", "paused", "stopped"].includes(data.state))
			throw purchasesError("invalid_experiment_state", "Invalid experiment state");
		const result = await c.env.DB.prepare(
			`
    ...
    `,
			...
			.bind(
				data.display_name ?? null,
				data.state ?? null,
				data.audience_conditions === undefined ? null : JSON.stringify(data.audience_conditions),
				data.starts_at ?? null,
				data.ends_at ?? null,
				c.req.param("id"),
				project.id,
			...
			.run();
		if (!result.meta.changes)
			throw purchasesError("experiment_not_found", "Experiment not found", 404);
		return c.json({ updated: true });
	} catch (error) {
		return replyError(c, error);
	}
...
admin.get("/:projectId/virtual-currencies", async (c) => {
	try {
		const project = await projectFor(c);
		const rows = await c.env.DB.prepare(
			`SELECT * FROM billing_virtual_currencies WHERE project_id = ? ORDER BY code`,
		)
			.bind(project.id)
			.all();
		return c.json({ data: rows.results });
	...
	try {
		...
		if (!/^[A-Z0-9_]{1,12}$/.test(code))
			throw purchasesError(
				...
				"Currency code must be 1-12 uppercase letters, numbers or underscores",
			);
		const id = crypto.randomUUID();
		await c.env.DB.prepare(
			`INSERT INTO billing_virtual_currencies (id, project_id, code, display_name, description, icon, metadata) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		)
			.bind(
				id,
				project.id,
				code,
				data.display_name || code,
				data.description || null,
				data.icon || null,
...
admin.post("/:projectId/virtual-currencies/:currencyId/products/:productId", async (c) => {
    ...
    `,
			...
			.bind(c.req.param("currencyId"), c.req.param("productId"), project.id)
			.first();
		if (!valid)
			throw purchasesError(
				"currency_or_product_not_found",
				"Virtual currency or product not found",
				404,
			);
		const amount = Number(data.grant_amount);
		if (!Number.isSafeInteger(amount) || amount <= 0)
			throw purchasesError("invalid_grant_amount", "grant_amount must be a positive integer");
		await c.env.DB.prepare(
			`
    ...
    `,
			...
			.bind(
				c.req.param("currencyId"),
				c.req.param("productId"),
				amount,
				data.trial_grant_amount ?? null,
				data.grant_cadence || "purchase",
				data.expires_after_seconds ?? null,
			)
			.run();
...
admin.post("/:projectId/customers/:customerId/virtual-currencies/adjust", async (c) => {
	try {
		...
		const data = await jsonBody(c);
		const adjustments =
			data.adjustments && typeof data.adjustments === "object" ? data.adjustments : {};
		const idempotencyKey = c.req.header("Idempotency-Key") || data.idempotency_key;
		if (!idempotencyKey)
			throw purchasesError("idempotency_key_required", "Idempotency-Key header is required");
		const customer = await c.env.DB.prepare(
			"SELECT id FROM billing_customers WHERE id = ? AND project_id = ?",
		)
			.bind(c.req.param("customerId"), project.id)
		...
		const currentRows = await c.env.DB.prepare(
			`SELECT currency_identifier, SUM(amount) AS balance FROM billing_balance_ledger WHERE project_id = ? AND customer_id = ? AND (expires_at IS NULL OR datetime(expires_at) > datetime('now')) GROUP BY currency_identifier`,
		)
			.bind(project.id, c.req.param("customerId"))
			.all<{ currency_identifier: string; balance: number }>();
		const balances = Object.fromEntries(
			(currentRows.results || []).map((row) => [row.currency_identifier, Number(row.balance)]),
		);
		for (const [code, amountValue] of Object.entries(adjustments)) {
			const amount = Number(amountValue);
			if (!Number.isSafeInteger(amount) || amount === 0)
				throw purchasesError("invalid_adjustment", `Invalid adjustment for ${code}`);
			if (Number(balances[code] || 0) + amount < 0)
				throw purchasesError("insufficient_virtual_currency", `Insufficient ${code} balance`, 422);
		}
		const statements = Object.entries(adjustments).map(([code, amountValue]) =>
			c.env.DB.prepare(
				`
    ...
    `,
		...
		await c.env.DB.batch(statements);
		await audit(c, project.id, "virtual_currency.adjusted", "customer", c.req.param("customerId"), {
			currencies: Object.keys(adjustments),
		});
		...
		return c.json({
			balances: Object.fromEntries(
				(updated.results || []).map((row) => [row.currency_identifier, Number(row.balance)]),
			),
		});
	} catch (error) {
		return replyError(c, error);
...
admin.post("/:projectId/integrations/deliveries/:id/replay", async (c) => {
	try {
		...
		const delivery = await c.env.DB.prepare(
			`SELECT d.id FROM billing_webhook_deliveries d JOIN billing_webhook_endpoints e ON e.id = d.endpoint_id WHERE d.id = ? AND e.project_id = ?`,
		)
			.bind(c.req.param("id"), project.id)
			.first<{ id: string }>();
       ...
       SET status = 'pending', attempts = 0, next_attempt_at = datetime('now'),
           ...
           last_attempt_at = NULL, delivered_at = NULL, claim_token = NULL, claim_expires_at = NULL
       WHERE id = ?`,
		)
			.bind(delivery.id)
			.run();
...
admin.get("/:projectId/refunds", async (c) => {
    ...
    `,
			...
			.bind(project.id, status, status, limit)
			.all();
		return c.json({ data: rows.results });
	} catch (error) {
		return replyError(c, error);
...
admin.get("/:projectId/refunds/:caseId", async (c) => {
    ...
    `,
			...
			.bind(c.req.param("caseId"), project.id)
			.first();
		...
		const [evidence, actions, deadlines, events] = await Promise.all([
			c.env.DB.prepare(
				"SELECT * FROM billing_refund_evidence WHERE case_id = ? ORDER BY created_at",
			)
				.bind(c.req.param("caseId"))
				.all(),
			c.env.DB.prepare(
				"SELECT * FROM billing_refund_provider_actions WHERE case_id = ? ORDER BY created_at",
			)
				.bind(c.req.param("caseId"))
				.all(),
			c.env.DB.prepare("SELECT * FROM billing_refund_deadlines WHERE case_id = ? ORDER BY due_at")
				.bind(c.req.param("caseId"))
				.all(),
			c.env.DB.prepare(
				"SELECT * FROM billing_refund_audit_events WHERE case_id = ? ORDER BY occurred_at DESC",
			)
				.bind(c.req.param("caseId"))
				.all(),
		]);
...
admin.post("/:projectId/refunds/:caseId/evidence", async (c) => {
	try {
		...
		const data = await jsonBody(c);
		const refundCase = await c.env.DB.prepare(
			"SELECT id, status FROM billing_refund_cases WHERE id = ? AND project_id = ?",
		)
			.bind(c.req.param("caseId"), project.id)
			.first<{ id: string; status: string }>();
		if (!refundCase) throw purchasesError("refund_case_not_found", "Refund case not found", 404);
		if (!refundCaseAllowsOperatorAction(refundCase.status)) {
			throw purchasesError(
				"refund_case_terminal",
				"A terminal refund case cannot accept new evidence",
				409,
			);
		}
		...
		if (!content && !fileKey)
			throw purchasesError("evidence_content_required", "Evidence content or file_key is required");
		if (content.length > 20_000)
			throw purchasesError(
				"refund_evidence_too_large",
				"Refund evidence content must contain at most 20,000 characters",
				413,
			);
		if (
			fileKey.length > 1024 ||
			...
			/[\x00-\x1f\x7f]/.test(fileKey)
		) {
			throw purchasesError(
				"refund_evidence_file_key_invalid",
				"Refund evidence file_key is invalid",
				422,
			);
		}
		const id = crypto.randomUUID();
        ...
        SELECT ?, ?, ?, ?, ?, ?
        WHERE EXISTS (
          SELECT 1 FROM billing_refund_cases
      ...
      `,
        ...
        SELECT ?, ?, 'evidence.created', 'admin', ?, ?
        WHERE EXISTS (
          SELECT 1 FROM billing_refund_evidence WHERE id = ? AND case_id = ?
        )
      `,
			).bind(
				crypto.randomUUID(),
				refundCase.id,
				c.get("operatorId") ?? String(c.get("userId")),
				JSON.stringify({ evidence_id: id, evidence_type: evidenceType }),
				id,
				refundCase.id,
			),
			c.env.DB.prepare(
				`UPDATE billing_refund_cases SET status = 'awaiting_approval', updated_at = datetime('now')
         WHERE id = ? AND status NOT IN ('won','lost','closed')
           AND EXISTS (SELECT 1 FROM billing_refund_evidence WHERE id = ? AND case_id = ?)`,
			).bind(refundCase.id, id, refundCase.id),
		]);
		if (!created.meta.changes)
			throw purchasesError(
				"refund_case_terminal",
				"The refund case became terminal before evidence was saved",
				409,
			);
...
admin.post("/:projectId/refunds/:caseId/evidence/:evidenceId/review", async (c) => {
	try {
		...
		const actorId = c.get("operatorId") ?? String(c.get("userId"));
    ...
    `,
      ...
      SELECT ?, ?, ?, 'admin', ?, ?
      WHERE EXISTS (
        SELECT 1 FROM billing_refund_evidence
        WHERE id = ? AND case_id = ? AND review_status = ?
    ...
    `,
				...
				actorId,
				JSON.stringify({ evidence_id: c.req.param("evidenceId") }),
				c.req.param("evidenceId"),
				c.req.param("caseId"),
				reviewStatus,
				actorId,
		...
		if (!result.meta.changes)
			throw purchasesError(
				"refund_evidence_not_reviewable",
				"Refund evidence was not found or is no longer reviewable",
				409,
			);
...
admin.post("/:projectId/refunds/:caseId/actions", async (c) => {
    ...
    `,
			...
			.bind(c.req.param("caseId"), project.id)
			.first<{ id: string; provider: string; status: string }>();
		if (!refundCase) throw purchasesError("refund_case_not_found", "Refund case not found", 404);
		if (!refundCaseAllowsOperatorAction(refundCase.status)) {
			throw purchasesError(
				"refund_case_terminal",
				"A terminal refund case cannot prepare provider actions",
				409,
			);
		}
		...
		if (!supportedRefundActions(refundCase.provider).includes(actionType as RefundActionType)) {
			throw purchasesError(
				"refund_action_unsupported",
				`${actionType || "Refund action"} is not supported for ${refundCase.provider}`,
			);
		}
		const payload =
		...
		if (JSON.stringify(payload).length > 100_000)
			throw purchasesError(
				"refund_action_payload_too_large",
				"Refund action payload is too large",
				413,
			);
		const id = crypto.randomUUID();
    ...
    `,
			...
			.bind(id, refundCase.id, actionType, JSON.stringify(payload), idempotencyKey)
			.run();
    ...
    `,
		...
		await audit(
			c,
			project.id,
			"refund_action.drafted",
			"refund_provider_action",
			String((action as any)?.id || id),
			{ case_id: refundCase.id, action_type: actionType },
		);
		return c.json(
			{
				data: action,
				supported_actions: supportedRefundActions(refundCase.provider),
			},
			201,
		);
	} catch (error) {
...
admin.patch("/:projectId/refunds/:caseId/actions/:actionId", async (c) => {
    ...
    `,
		...
		if (!action)
			throw purchasesError(
				"refund_action_not_editable",
				"Refund action was not found or cannot be edited in its current state",
				409,
			);
    ...
    `,
			...
			.bind(JSON.stringify(payload), action.id)
			.run();
		await audit(c, project.id, "refund_action.updated", "refund_provider_action", action.id, {
...
admin.post("/:projectId/refunds/:caseId/actions/:actionId/approve", async (c) => {
    ...
    `,
		...
		if (!action)
			throw purchasesError(
				"refund_action_not_approvable",
				"Refund action was not found or is no longer a draft",
				409,
			);
      ...
      `,
			...
			if (!consent)
				throw purchasesError(
					"apple_consumption_consent_evidence_required",
					"Approve apple_consumption_consent evidence before sending data to Apple",
					409,
				);
		}
		const actorId = c.get("operatorId") ?? String(c.get("userId"));
    ...
    `,
			...
			c.env.DB.prepare(
				`
      INSERT INTO billing_refund_audit_events
        (id, case_id, event_type, actor_type, actor_id, payload)
      SELECT ?, ?, 'provider_action.approved', 'admin', ?, ?
      WHERE EXISTS (
        SELECT 1 FROM billing_refund_provider_actions
        WHERE id = ? AND case_id = ? AND status = 'approved'
          AND approved_by = ? AND approved_at = ?
      )
    `,
			).bind(
				crypto.randomUUID(),
				c.req.param("caseId"),
				actorId,
				JSON.stringify({ action_id: c.req.param("actionId") }),
				c.req.param("actionId"),
				c.req.param("caseId"),
				actorId,
				approvedAt,
		...
		if (!result.meta.changes)
			throw purchasesError(
				"refund_action_not_approvable",
				"Refund action was modified concurrently",
				409,
			);
		let queued = false;
		if (c.env.BILLING_QUEUE) {
        ...
        `,
				...
				console.error(
					JSON.stringify({
						event: "refund_action_queue_failed",
						action_id: c.req.param("actionId"),
						error: error instanceof Error ? error.message : String(error),
					}),
				);
			}
...
admin.get("/:projectId/exports", async (c) => {
	try {
		const project = await projectFor(c);
		const rows = await c.env.DB.prepare(
			`SELECT * FROM billing_export_jobs WHERE project_id = ? ORDER BY created_at DESC LIMIT 100`,
		)
			.bind(project.id)
			.all();
		return c.json({ data: rows.results });
	...
	try {
		...
		if (!name || name.length > 100)
			throw purchasesError(
				...
				"Audience name is required and must be at most 100 characters",
			);
		const filters = Array.isArray(data.filters) ? data.filters : [];
		const id = crypto.randomUUID();
		await c.env.DB.prepare(
			`INSERT INTO billing_audiences (id, project_id, display_name, filters) VALUES (?, ?, ?, ?)`,
		)
			.bind(id, project.id, name, JSON.stringify(filters))
			.run();
		return c.json({ id }, 201);
	} catch (error) {
		return replyError(c, error);
...
admin.post("/:projectId/exports", async (c) => {
	try {
		...
		const data = await jsonBody(c);
		if (
			!["transactions", "subscriptions", "customers", "virtual_currencies"].includes(data.dataset)
		)
			throw purchasesError("invalid_export_dataset", "Invalid export dataset");
		if ((data.format || "csv") !== "csv")
			throw purchasesError("unsupported_export_format", "Only CSV exports are supported", 422);
		const id = crypto.randomUUID();
		await c.env.DB.prepare(
			`
    ...
    `,
			...
			.bind(
				...
				data.format || "csv",
				data.cadence || null,
				data.incremental === false ? 0 : 1,
				JSON.stringify(data.columns || []),
				c.get("operatorId") ?? String(c.get("userId")),
			)
			.run();
		await audit(c, project.id, "export.requested", "export", id, {
			dataset: data.dataset,
			format: data.format || "csv",
		});
		if (c.env.BILLING_QUEUE)
			await c.env.BILLING_QUEUE.send({ type: "billing.export", exportId: id });
...
admin.get("/:projectId/exports/:id/download", async (c) => {
	try {
		...
		const job = await c.env.DB.prepare(
			`SELECT r2_key, dataset, format, status FROM billing_export_jobs WHERE id = ? AND project_id = ?`,
		)
			.bind(c.req.param("id"), project.id)
			.first<{
				r2_key: string | null;
