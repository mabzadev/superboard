...
function brandedHeader(c: any, suffix: string): string | undefined {
	return c.req.header(`X-SuperBoard-${suffix}`);
}
...
async function context(c: any) {
	...
	if (!projectId)
		throw purchasesError("invalid_sdk_project", "Invalid project for SDK credentials", 403);
  ...
  `)
		...
		purchases_core: number;
		paywalls: number;
		virtual_currencies: number;
	} | null;
	if (!project)
		throw purchasesError("invalid_sdk_project", "Invalid project for SDK credentials", 403);
	if (!isPurchasesEnabled(c.env, project.purchases_enabled))
		throw purchasesError(
			"purchases_disabled",
			"SuperBoard Purchases is not enabled for this project",
			403,
		);
	if (Number(project.purchases_core) !== 1)
		throw purchasesError(
			"purchases_v2_not_enabled",
			"Purchases v2 core is not enabled for this project",
			403,
		);
	...
	return {
		...
		projectEnvironment: (Number(project.is_test) === 1 ? "test" : "production") as
			| "test"
			| "production",
...
function targetingContext(c: any, customer: Record<string, unknown>): TargetingContext {
	...
	try {
		attributes = JSON.parse(String(customer.attributes || "{}"));
	} catch {}
	return {
...
sdk.get("/configuration", async (c) => {
	try {
		...
		const flowsCutoverEnabled =
			Boolean(c.env.FLOWS_MODULE) &&
		...
		const offerings = await offeringsForCustomer(
			c.env.DB,
			ctx.projectId,
			platform,
			ctx.environment,
		...
		const selected = resolved.offeringIdentifier
			? (offerings.all as Record<string, unknown>)[resolved.offeringIdentifier]
			: offerings.current;
...
sdk.get("/customer-info", async (c) => {
	try {
		const ctx = await context(c);
		return c.json(
			await customerInfoFromBillingAuthority(c.env, ctx.projectId, String(ctx.customer.id)),
			200,
			{ "Cache-Control": "private, no-store" },
		);
	} catch (error) {
...
sdk.post("/receipts", async (c) => {
	try {
		...
		if (billingServiceEnabled(c.env)) {
			const result = await callBillingService<Record<string, unknown>>(
				c.env,
				"/internal/v1/receipts/verify",
				{
		...
		if (data.store === "apple") {
			if (!data.signed_transaction)
				throw purchasesError("receipt_required", "signed_transaction is required");
			purchase = (
			...
			if (!data.purchase_token || !data.product_id)
				throw purchasesError("receipt_required", "purchase_token and product_id are required");
			...
			throw purchasesError(
				"unsupported_receipt_store",
				"Receipts currently support apple and google",
			);
		}
		...
		return c.json({
			...
			customer_info: await customerInfoFromBillingAuthority(
				c.env,
				ctx.projectId,
				String(ctx.customer.id),
			),
...
sdk.post("/events", async (c) => {
	try {
		...
		if (events.length < 1 || events.length > 100)
			throw purchasesError("invalid_event_batch", "Events batch must contain 1-100 events");
		const statements = events.map((event: Record<string, any>) => {
			if (!PAYWALL_EVENT_TYPES.includes(event.type))
				throw purchasesError("invalid_event_type", `Unsupported paywall event: ${event.type}`);
			const id = String(event.id || crypto.randomUUID());
			const occurredAt =
				event.occurred_at && !Number.isNaN(Date.parse(event.occurred_at))
					? event.occurred_at
      ...
      `).bind(
				...
				ctx.projectId,
				String(ctx.customer.id),
				event.paywall_id || null,
				event.paywall_version_id || null,
				event.placement || null,
				event.experiment_id || null,
				event.variant_id || null,
				event.type,
				event.package_identifier || null,
				c.get("sdkPlatform") || c.req.header("PLATFORM") || null,
				requestCountry(c),
				brandedHeader(c, "App-Version") || null,
				brandedHeader(c, "SDK-Version") || null,
...
sdk.post("/certification/device-results", async (c) => {
	try {
		...
		if (!["ios", "android", "web"].includes(String(platform))) {
			throw purchasesError(
				"device_certification_platform_invalid",
				"iOS, Android, or Web SDK context is required",
			);
		}
		const assertions =
			data.assertions && typeof data.assertions === "object" && !Array.isArray(data.assertions)
...
sdk.get("/virtual-currencies", async (c) => {
	try {
		const ctx = await context(c);
		if (!ctx.features.virtualCurrencies)
			throw purchasesError(
				"virtual_currencies_not_enabled",
				"Virtual currencies are not enabled for this project",
				403,
			);
    ...
    `)
			.bind(String(ctx.customer.id), ctx.projectId)
...
sdk.get("/customer-center", async (c) => {
	try {
		const ctx = await context(c);
		const info = await customerInfoFromBillingAuthority(
			c.env,
			ctx.projectId,
			String(ctx.customer.id),
		);
