...
import { inspectSqlSchemaHealth, type SqlSchemaHealth } from "@superboard/contracts/health";
import {
...
import flutterFlowCustomCode from "../../../../../../scripts/config/flutterflow-custom-code.json";
import flutterFlowLibrary from "../../../../../../scripts/config/flutterflow-library.json";
...
const API_CAPABILITIES = [
	{
		id: "identity",
		description: "Authentication, OAuth, Google/Apple federation, sessions and roles",
	...
	{
		id: "projects",
		description: "Applications, environments, access keys and SDK configuration",
	...
	{
		id: "files",
		description: "Authenticated upload, storage metadata and controlled downloads",
	...
	},
	{
		...
		access: "Application SDK or authenticated project administrator",
		entrypoints: ["/api/v1/sdk/modules/analytics/events", "/api/v1/analytics/projects/*"],
	},
	{
		id: "custom-jobs",
		description:
	...
	},
	{
		...
		description: "Authenticated newsletter consent and public subscription-list preferences",
		access: "Verified application identity; private signed gateway to Marketing",
		entrypoints: ["/api/v1/sdk/marketing/v1/preferences"],
	},
	{
		id: "email-operations",
		description: "Body-free email delivery inspection and audited dead-letter replay or discard",
		access: "Authenticated Dashboard owner or administrator",
		entrypoints: ["/api/v1/platform/email/*"],
	},
	{
		id: "support",
		description: "Conversations, contacts, attachments, realtime orchestration and CSAT",
	...
	{
		id: "platform",
		description: "Infrastructure health, runtime metrics, jobs and data inventory",
		access: "Authenticated Dashboard administrator",
		entrypoints: [
			"/health",
...
platform.get("/libraries", async (c) => {
	const denial = await platformAdminDenial(c.env, c.req.raw);
...
platform.get("/account-erasures", async (c) => {
	const admin = await platformAdminContext(c.env, c.req.raw);
	...
	if (!Number.isSafeInteger(requestedLimit) || requestedLimit <= 0 || requestedLimit > 100) {
	...
	return c.json(
		{
			data: rows.results.map((row) => ({
				...
				lastErrorService: row.last_error_service ? String(row.last_error_service) : null,
...
platform.get("/status", async (c) => {
	...
	return c.json(
		await buildPlatformStatus(c.env, admin.managed ? { instanceId: admin.instanceId } : undefined),
		200,
		{
			"cache-control": "no-store",
		},
...
platform.get("/email/operations", async (c) => {
	const denial = await platformAdminDenial(c.env, c.req.raw);
	if (denial) return c.json({ error: denial.error }, denial.status);
	return proxyEmail(c.env, `${EMAIL_SERVICE_OPERATIONS_PATH}${new URL(c.req.url).search}`);
});
...
export async function buildPlatformStatus(env: Env, managed?: { instanceId: number }) {
	const started = Date.now();
	const topology = workerTopology(env);
	const [serviceChecks, metrics, platformJobs, runtime, custom, publicSurfaces, centralSchema] =
		await Promise.all([
			Promise.all([
				...WORKERS.filter(
					(definition) => definition.binding !== null && definition.id !== "observability",
				).map((definition) => checkService(env, definition, topology.workers.get(definition.id))),
				...[...topology.workers.values()]
					.filter(isManagedWorkerTopologyEntry)
					.map((definition) => checkManagedWorker(env, definition)),
			]),
			managed ? managedPlatformMetrics(env.DB, managed.instanceId) : platformMetrics(env.DB),
			managed ? managedJobMetrics(env.DB) : jobMetrics(env.DB),
			runtimeMetrics(env),
			customOverview(env),
	...
	const coreMetrics = managed
		? { instances: metrics.instances, projects: metrics.projects }
		: metrics;
	const dataStores = dataStoreInventory(env, serviceChecks, coreMetrics, centralSchema);
	const apiStatus =
		Object.values(coreMetrics).every(
	...
	const unavailable =
		services.filter((service) => service.status !== "ok" && service.status !== "disabled").length +
		publicSurfaces.filter((surface) => surface.status !== "ok").length +
		(custom.status === "ok" || custom.status === "disabled" ? 0 : 1) +
		dataStores.filter(
			(store) => !["ok", "configured", "declared", "disabled"].includes(store.status),
		).length +
		(jobVisibilityUnavailable(
			managed ? { platformDeadLetters: platformJobs.platformDeadLetters } : platformJobs,
			serviceChecks,
			serviceJobs,
		)
			? 1
			: 0) +
		(!managed && securityStateDegraded(platformJobs.pushCredentials, ["legacyPlaintext", "missing"])
			? 1
			: 0) +
		(!managed && securityStateDegraded(platformJobs.bearerTokenStorage, ["legacyPlaintext"])
			? 1
			: 0) +
		(!managed && securityStateDegraded(platformJobs.authCredentials, ["legacyPlaintext"]) ? 1 : 0) +
	...
	return {
		...
		deployment: {
			target: env.SUPERBOARD_TARGET || "unknown",
			release: env.SUPERBOARD_RELEASE || "unknown",
		...
		api: {
			status: apiStatus,
			description: "Authenticated SuperBoard gateway, orchestration and public SDK surface",
		...
		...(managed
			? {
					metricAvailability: {
						...
						reason:
							"Historical business metrics are available only through the external adapter; enabled services report their own metrics.",
					},
				}
			: {}),
		metrics: {
		...
		definitions.map((definition) => {
			...
			if (ids.has(definition.id)) {
				return Promise.resolve(
					publicSurfaceConfigurationError(`Public surface monitor ${definition.id} is duplicated`),
				);
			}
...
function isPublicSurfaceDefinition(value: unknown): value is PublicSurfaceDefinition {
	return (
		record(value) &&
...
platform.get("/custom/jobs", async (c) => {
	const denial = await platformAdminDenial(c.env, c.req.raw);
	if (denial) return c.json({ error: denial.error }, denial.status);
	return proxyCustom(c.env, `${CUSTOM_WORKER_JOB_PATH}${new URL(c.req.url).search}`);
});
...
export function workerTopology(env: Env): WorkerTopology {
	...
	try {
		...
		if (
			...
			value.target !== env.SUPERBOARD_TARGET ||
			value.environment !== env.ENVIRONMENT ||
			!Array.isArray(value.workers) ||
			!Array.isArray(value.customDependencies)
		) {
			return failed("Worker catalog does not match this target and environment");
		}
...
function isWorkerTopologyEntry(value: unknown): value is WorkerTopologyEntry {
	return (
		...
		(value.workerName === null ||
			(typeof value.workerName === "string" && safeWorkerName(value.workerName))) &&
...
function isManagedWorkerTopologyEntry(value: WorkerTopologyEntry): value is WorkerTopologyEntry & {
	...
	return Boolean(
		...
		managed.stores.every(
			(store) => typeof store === "string" && /^[A-Za-z][A-Za-z0-9_-]{0,63}$/u.test(store),
		),
	);
}
...
function operationalServices(input: {
	...
	const commonServices = WORKERS.map((definition) => {
		...
		const routes = capabilityIds.flatMap((id) => capabilityMap.get(id)?.entrypoints ?? []);
		return {
			...(live ?? {
				id: definition.id,
			...
			dependencies: {
				services: workerServiceDependencies(definition.id).filter(
					(dependency) => input.topology.workers.get(dependency)?.enabled !== false,
				),
				stores: definition.stores,
				queues: definition.queues,
				externalWorkers: definition.id === "custom" ? input.topology.customDependencies : [],
			},
			jobTypes: definition.jobTypes,
			jobs: workerJobs(definition.id, input.platformJobs, input.serviceJobs, input.custom),
		};
	});
		...
		.map((definition) => {
			...
			return {
				...
				capabilities: [
					...
					...definition.managed.containers.map(({ className }) => `container:${className}`),
...
function workerJobs(
	...
	if (id === "api") return flatten(["platformDeadLetters", "pushDeliveries", "accountErasures"]);
...
async function checkService(
	...
	const { id, binding: bindingName, healthPath: path, description } = definition;
	if (configured && !configured.enabled)
		return { id, status: "disabled", description, responseTimeMs: null };
	const binding = bindingName ? (env[bindingName as keyof Env] as Fetcher | undefined) : undefined;
	if (!binding)
		return {
			id,
			...
			description,
			responseTimeMs: null,
			...(configured?.enabled ? { error: "Required service binding is missing" } : {}),
		};
	const started = Date.now();
	try {
		...
		const text = await readTextLimited(response, 16_384, "Service health response is too large");
...
async function checkManagedWorker(
	...
	const binding = env[definition.managed.binding as keyof Env] as Fetcher | undefined;
	if (!binding) {
		return {
			id: definition.id,
			status: "misconfigured",
	...
	const started = Date.now();
	try {
		const response = await binding.fetch(`https://${definition.id}.internal/health`, {
			method: "GET",
			redirect: "manual",
			signal: AbortSignal.timeout(3_000),
		});
		...
		return {
			...
			description,
			responseTimeMs: Date.now() - started,
			error: error instanceof Error ? error.message : "Managed Worker check failed",
		};
	}
}
...
function serviceJobMetrics(services: ServiceCheck[]) {
	...
	return {
		email: selectMetrics(email, {
			...
			delegatedTransportOutcomeUnknown: ["delegatedTransport", "outcomeUnknown"],
...
function dataStoreInventory(
	...
	const serviceSchema = (owner: string) =>
		reportedSchema(services.find((service) => service.id === owner)?.detail ?? null);
	...
	const apiDatabaseStatus =
		Object.values(metrics).some((value) => value === null) || centralSchema?.status !== "current"
			? "degraded"
			: "ok";
	const store = (
		id: string,
	...
	return [
		...
		d1Store("file-metadata", "files", "File ownership, metadata, retention and deletion state"),
		d1Store("email", "email", "Transactional messages, capture previews and delivery attempts"),
		d1Store("app", "app", "Customers, referrals, access keys, SDK configuration and events"),
		d1Store("products", "products", "Products, offerings, entitlements, purchases and refunds"),
		d1Store("paywalls", "paywalls", "Paywalls, placements, variants, versions and events"),
		d1Store(
			"dynamic-links",
			"dynamic-links",
			"Short links, campaigns, domains, redirects and attribution",
		),
		d1Store("support", "support", "Contacts, conversations, messages, automations and CSAT"),
		store(
			"support-attachments",
			"R2",
			"support",
...
function reportedSchema(detail: unknown): SqlSchemaHealth | null {
	...
	const latestMigration =
		schema.latestMigration === null ? null : String(schema.latestMigration || "");
	...
	if (
		...
		(latestMigration !== null && !/^\d+[a-z0-9_-]*\.sql$/iu.test(latestMigration)) ||
...
function serviceReportedStatus(detail: unknown) {
	...
	return ["ok", "degraded", "unavailable", "misconfigured"].includes(value) ? value : "degraded";
}
...
async function runtimeMetrics(env: Env): Promise<{
	...
	const description = "Cloudflare invocation, outcome, exception, CPU and wall-time telemetry";
	...
	try {
		...
		return {
			service: {
				...
				description,
				responseTimeMs: Date.now() - started,
				error: error instanceof Error ? error.message : "Runtime metrics failed",
			},
			summary: null,
		};
	}
...
async function managedPlatformMetrics(db: D1Database, instanceId: number) {
	const [instance, projects] = await Promise.all([
		db
			.prepare("SELECT COUNT(*) total FROM instances WHERE id=?")
			.bind(instanceId)
			.first<{ total: number }>()
			.catch(() => null),
		db
			.prepare("SELECT COUNT(*) total FROM projects WHERE instance_id=?")
			.bind(instanceId)
			.first<{ total: number }>()
			.catch(() => null),
	]);
...
async function managedJobMetrics(db: D1Database) {
	return {
		billingExports: null,
		failedPurchases: null,
		platformDeadLetters: await grouped(
			db,
...
async function platformMetrics(db: D1Database) {
	const queries = {
		...
		activeOauthTokens: "SELECT COUNT(*) total FROM oauth_access_tokens WHERE revoked_at IS NULL",
		pushDevices:
...
async function jobMetrics(db: D1Database) {
	const [
		...
		grouped(db, "SELECT status, COUNT(*) total FROM billing_export_jobs GROUP BY status"),
		grouped(db, "SELECT status, COUNT(*) total FROM failed_purchase_jobs GROUP BY status"),
		grouped(
			db,
			"SELECT source_queue AS status, COUNT(*) total FROM platform_dead_letters WHERE status = 'quarantined' GROUP BY source_queue",
		),
...
async function grouped(db: D1Database, sql: string): Promise<Record<string, number> | null> {
	try {
		const rows = (await db.prepare(sql).all<{ status: string; total: number }>()).results;
		return Object.fromEntries(
			rows.map((row) => [String(row.status || "unknown"), Number(row.total || 0)]),
		);
	} catch {
		return null;
	}
...
async function customOverview(env: Env): Promise<{
	...
	if (!env.CUSTOM_WORKER) return { status: "disabled", manifest: null, stats: null };
	if (!env.CUSTOM_WORKER_TOKEN) return { status: "misconfigured", manifest: null, stats: null };
	try {
		...
		const target = env.SUPERBOARD_TARGET || "";
		if (
			!manifest ||
		...
		return {
			...
			manifest: null,
			stats: null,
			error: error instanceof Error ? error.message : "Custom Worker check failed",
		};
	}
}
...
function isObservabilitySummary(value: unknown): value is ObservabilitySummary {
	...
	if (
		!new Set(["ok", "misconfigured", "unavailable"]).has(String(value.status)) ||
...
function isCustomStats(value: unknown): value is CustomWorkerStats {
	...
	if (!Object.values(value.capabilities).every((entry) => numberRecord(entry))) {
		return false;
	}
	...
	if (value.cancellations !== undefined) {
		...
		for (const name of ["jobs", "refundsPending", "refundsApplied", "creditsRefunded"]) {
...
async function platformAdminDenial(
	...
	const value = await platformAdminContext(env, request);
...
async function platformAdminContext(
	...
	{ userId: number; instanceId: number; managed?: boolean } | { error: string; status: 401 | 403 }
> {
	const auth = await getRequestAuthContext(env, request.headers, { request });
	if (!auth) return { error: "unauthorized", status: 401 };
	if (!auth.instanceId) return { error: "instance_required", status: 403 };
	if (auth.siteOperator) return { userId: auth.userId, instanceId: auth.instanceId, managed: true };
...
function customFetch(env: Env, path: string, init: RequestInit = {}) {
	if (!env.CUSTOM_WORKER || !env.CUSTOM_WORKER_TOKEN) throw new Error("custom_worker_unavailable");
...
async function proxyCustom(env: Env, path: string, init: RequestInit = {}): Promise<Response> {
	if (!env.CUSTOM_WORKER)
		return Response.json({ error: "custom_worker_disabled" }, { status: 404 });
	if (!env.CUSTOM_WORKER_TOKEN)
		return Response.json({ error: "custom_worker_misconfigured" }, { status: 503 });
	try {
		...
		return Response.json({ error: "custom_worker_unavailable" }, { status: 503 });
	}
}

async function proxyEmail(env: Env, path: string, init: RequestInit = {}): Promise<Response> {
	if (!env.EMAIL_SERVICE)
		return Response.json({ error: "email_service_disabled" }, { status: 404 });
	if (!env.EMAIL_INTERNAL_TOKEN)
		return Response.json({ error: "email_service_misconfigured" }, { status: 503 });
	try {
		const response = await env.EMAIL_SERVICE.fetch(`https://email.internal${path}`, {
			...init,
			headers: {
