...
export const DOMAIN_SDK_ROUTES = Object.freeze([
	...
	sdkRoute("analytics", "/remote-config/resolve", "/remote-config/resolve", false),
...
export async function proxyDomainModule(
	...
	try {
		return await proxyDomainModuleRequest(
			...
			bindingName,
			requestId,
			moduleName === "email" ? { internalPrefix: "/internal/v1/admin" } : {},
		);
	} catch (error) {
		console.error(
			JSON.stringify({
...
async function proxyDomainModuleRequest(
	...
	return forwardDomainRequest(c, selectedModuleName, selectedBindingName, requestId, internalPath, {
		...project.context,
		actorId: auth.userId,
...
export async function proxyDomainSdkModule(
	...
	try {
		...
		const bindingName: DomainModuleBinding = useFlows ? "FLOWS_MODULE" : route.bindingName;
		...
		return forwardDomainRequest(c, moduleName, bindingName, requestId, internalPath, {
			...resolved.context,
			actorId: 0,
			role: "sdk",
		});
 ...
 */
export async function proxyPublicFlows(c: GatewayContext, internalPath: string): Promise<Response> {
	const requestId = requestIdFrom(c.req.raw);
...
async function removedFlowsOrganizationContext(request: Request): Promise<boolean> {
	const normalized = (value: string) => value.toLowerCase().replaceAll("-", "").replaceAll("_", "");
...
export async function proxyPublicMarketing(
	...
	if (!c.env.MARKETING_MODULE) {
		return domainError(requestId, 503, "module_unavailable", "marketing service is unavailable", {
			retryable: true,
		});
	}
	try {
...
export async function proxyPublicSupport(
	...
	if (!c.env.SUPPORT_MODULE) {
		return domainError(requestId, 503, "module_unavailable", "support service is unavailable", {
			retryable: true,
		});
	}
	try {
		...
		if (canonicalProject && !headers.has("x-superboard-project-id")) {
			headers.set("x-superboard-project-id", canonicalProject);
		}
		headers.set("x-request-id", requestId);
		...
		return domainError(requestId, 502, "module_request_failed", "support service request failed", {
			retryable: true,
		});
	}
}

export async function isProjectReadOnly(db: D1Database, projectId: number): Promise<boolean> {
	const row = await db
		.prepare("SELECT enabled FROM module_cutover_maintenance WHERE project_id = ? LIMIT 1")
		.bind(projectId)
...
export async function forwardDomainRequest(
	...
	const internalToken =
		moduleName === "flows"
			...
			: moduleName === "email"
				? c.env.EMAIL_INTERNAL_TOKEN?.trim()
	...
	const headers = internalHeaders(c.req.raw.headers, context, internalToken, signature);
	...
	try {
		...
		const response = await service.fetch(new Request(forwardedRequest, { headers }));
		const responseHeaders = new Headers(response.headers);
		responseHeaders.set("X-Request-Id", requestId);
		return new Response(response.body, {
			status: response.status,
...
export async function resolveSdkProjectContext(
	...
	| {
			ok: true;
			context: Omit<ProjectContext, "actorId" | "role" | "requestId" | "issuedAt">;
			platform: string;
			identifier: string;
	  }
	| {
...
> {
	...
	const projectKey = keySelectsTest ? rawProjectKey.slice("test_".length) : rawProjectKey;
	if (!projectKey) {
		return {
			ok: false,
			status: 403,
	...
	const environmentHeader = request.headers.get("ENVIRONMENT")?.trim().toLowerCase();
	if (environmentHeader && environmentHeader !== "production" && environmentHeader !== "test") {
		return {
			ok: false,
			status: 422,
...
async function resolveNativeSdkProjectContext(
	...
	const instanceSlug = env?.SUPERBOARD_TARGET;
...
export function extractDomainRoute(
	...
	if (headerProjectRef && pathProjectRef && headerProjectRef !== pathProjectRef) {
		return {
			ok: false,
			status: 409,
	...
	if (!PROJECT_REF_PATTERN.test(projectRef)) {
		return {
			...
			message: "Project reference must use the <instance>-prod or <instance>-test format",
		};
	}
...
> {
	...
	const environment: ProjectEnvironment = parsed[2] === "test" ? "test" : "production";
	if (siteOperator) {
		const resolvedInstance = await resolveSiteOperatorInstance(db, siteOperator.instance_id);
		...
		const row = await db
			.prepare(
				`SELECT id AS project_id FROM projects WHERE instance_id = ? AND is_test = ? LIMIT 1`,
			)
		...
		return {
			...
			context: {
				...
				instanceId,
				environment,
				role: siteOperator.role === 50 ? "owner" : "admin",
			},
		};
	}
	const row = await db
	...
	if (!row) {
		const projectExists = await db
			.prepare("SELECT id FROM projects WHERE instance_id = ? AND is_test = ? LIMIT 1")
			.bind(instanceId, environment === "test" ? 1 : 0)
			.first<{ id: number }>();
...
function internalHeaders(
	...
	if (context.operatorId) headers.set("X-Operator-Id", context.operatorId);
...
export function isPublicFlowsSdkRoutePath(pathname: string): boolean {
	return PUBLIC_FLOWS_SDK_ROUTES.some((route) => route.publicPath === pathname);
}
...
function legacySdkRoute(
	...
	return Object.freeze({
		...sdkRoute(moduleName, publicSuffix, internalSuffix, requiresIdempotency),
...
function validateIdempotencyKey(request: Request, requestId: string): Response | null {
