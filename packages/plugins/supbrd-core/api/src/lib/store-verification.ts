...
export async function configuredGoogleProductType(
    ...
    LIMIT 1
  `)
		.bind(String(params.projectId), params.environment, params.storeProductId)
		.first<{ product_type: string }>();
	if (
		!product ||
		!["subscription", "non_consumable", "consumable"].includes(product.product_type)
	) {
		throw Object.assign(new Error("Google Play product is not configured in the server catalog"), {
...
function appleRoots(env: BillingEnv): Buffer[] {
	if (!env.APPLE_ROOT_CERTIFICATES_B64)
	...
	try {
		...
		values = env.APPLE_ROOT_CERTIFICATES_B64.split(",")
...
async function applicationForProject(
	db: D1Database,
	projectId: string | number,
    ...
    `)
		...
		if (!row?.application_id || !row.identifier)
			throw new Error("iOS billing application is not configured");
		return {
  ...
  `)
		...
		.first<Record<string, unknown>>();
	if (!row?.application_id || !row.identifier)
		throw new Error("Android billing application is not configured");
	return {
		applicationId: String(row.application_id),
		instanceId: String(row.instance_id),
		identifier: String(row.identifier),
...
async function appleVerifier(
	...
	if (environment === "production" && !app.appAppleId)
		throw new Error("Apple App ID is required for production verification");
...
async function appleServerClient(
	env: BillingEnv,
	app: BillingApplication,
	environment: BillingEnvironment,
) {
	...
	return {
		...
		getTransactionHistory: (
			...
			return request(
				`/inApps/${encodeURIComponent(String(version))}/history/${encodeURIComponent(transactionId)}${query}`,
			);
		},
...
async function appleApiCredentials(env: BillingEnv, app: BillingApplication) {
  ...
  `)
		.bind(app.instanceId, app.applicationId)
		.first<{
			encrypted_key: string;
			billing_encrypted_key: string | null;
			key_id: string;
			issuer_id: string;
		}>();
	const encryptedKey =
		env.CREDENTIAL_KEY_SCOPE === "billing" ? row?.billing_encrypted_key : row?.encrypted_key;
	if (!encryptedKey || !row?.key_id || !row.issuer_id)
		throw new Error(
			"App Store Server API credentials are not configured for this execution domain",
		);
	return {
...
function statusFromAppleServer(
...
export async function verifyAppleTransaction(
	...
	if (
		history.bundleId !== app.identifier ||
		(params.environment === "production" && Number(history.appAppleId) !== app.appAppleId)
	) {
		throw new Error("App Store Server API returned a different application");
	}
	const historyTransactions = await Promise.all(
		(history.signedTransactions || []).map((value: string) =>
			verifier.verifyAndDecodeTransaction(value),
		),
	);
	const matching = historyTransactions.filter(
		(value) => value.originalTransactionId === transaction.originalTransactionId,
	);
	if (!matching.some((value) => value.transactionId === transaction.transactionId))
		throw new Error("Apple transaction is absent from server history");
	matching.sort(
		(left, right) =>
			Number(right.signedDate || right.purchaseDate || 0) -
			Number(left.signedDate || left.purchaseDate || 0),
	);
	...
	if (autoRenews) {
		...
		const latest = (statuses.data || [])
			.flatMap(
				(group: { lastTransactions?: Array<Record<string, any>> }) => group.lastTransactions || [],
			)
			.find(
		...
		if (latest?.signedTransactionInfo)
			transaction = await verifier.verifyAndDecodeTransaction(latest.signedTransactionInfo);
	...
	const eventType =
		params.eventType || (transaction.transactionReason === "RENEWAL" ? "RENEWAL" : "PURCHASED");
	const purchase: VerifiedPurchase = {
		...
		eventType,
		status: serverStatus
			? statusFromAppleServer(serverStatus, transaction, autoRenews)
			: appleStatus(transaction, eventType),
...
export async function verifyAppleNotification(
	...
	let renewal: Record<string, unknown> | null = null;
	let autoRenews =
		productType === "subscription" && !/^(CANCEL|EXPIRED|REFUND|REVOKE)$/i.test(eventType);
	...
	return {
		...
		purchase: {
			...
			autoRenews,
			periodType:
				transaction.offerType === 1 ? "trial" : transaction.offerType ? "intro" : "normal",
...
async function googleCredentials(
  ...
  `)
		.bind(app.instanceId, app.applicationId)
		.first<{ encrypted_key: string; billing_encrypted_key: string | null }>();
	const encryptedKey =
		env.CREDENTIAL_KEY_SCOPE === "billing" ? row?.billing_encrypted_key : row?.encrypted_key;
	const source =
		encryptedKey ||
		(env.CREDENTIAL_KEY_SCOPE === "billing" ? null : env.GOOGLE_PLAY_SERVICE_ACCOUNT_JSON);
...
function storeApiError(store: string, status: number, payload: Record<string, unknown>): Error {
	...
	if (store === "App Store Server API" && status === 401) {
		return new Error(
			"Apple rejected the server key. Upload an In-App Purchase key from Users and Access > Integrations > In-App Purchase.",
		);
	}
...
function appleProduct(
	...
	return {
		...
		storeProductId,
		productType:
			rawType.includes("CONSUMABLE") && !rawType.includes("NON_CONSUMABLE")
				? "consumable"
...
function appleSubscription(
	...
	const providerPurchasable = providerApproved && providerAvailable;
	return {
		applicationId,
		storeProductId,
...
async function appleSubscriptionAvailability(
	...
	if (!resourceId)
		return { planCount: 0, availableTerritoryCount: 0, availableInNewTerritories: false };
...
async function syncAppleCatalog(
	...
	const apps = Array.isArray(appsPayload.data)
		? (appsPayload.data as Record<string, unknown>[])
		: [];
...
export async function appStoreConnectAccess(env: BillingEnv, projectId: string | number) {
	...
	if (!matching?.id)
		throw new Error(`App Store Connect: no app found for bundle ID ${app.identifier}`);
	const attributes =
		matching.attributes && typeof matching.attributes === "object"
...
function localizedListing(value: unknown): { title?: string; description?: string } {
	if (Array.isArray(value)) {
		const listing = value.find((item) => item && typeof item === "object") as
			| Record<string, unknown>
			| undefined;
		return listing
			? { title: String(listing.title || ""), description: String(listing.description || "") }
			: {};
	}
	if (value && typeof value === "object") {
		const listings = Object.values(value as Record<string, unknown>);
		const listing = listings.find((item) => item && typeof item === "object") as
			| Record<string, unknown>
			| undefined;
		return listing
			? { title: String(listing.title || ""), description: String(listing.description || "") }
...
async function googleCollection(
	...
	do {
		...
		if (Array.isArray(payload[responseKey]))
			values.push(...(payload[responseKey] as Record<string, unknown>[]));
		pageToken = typeof payload.nextPageToken === "string" ? payload.nextPageToken : "";
	} while (pageToken);
	return values;
}

async function syncGoogleCatalog(
	env: BillingEnv,
	projectId: string | number,
): Promise<StoreCatalogProduct[]> {
	const app = await applicationForProject(env.DB, projectId, "android");
	...
	return [
		...subscriptions.map((item): StoreCatalogProduct | null => {
			...
			const basePlans = Array.isArray(item.basePlans)
				? (item.basePlans as Array<Record<string, unknown>>)
				: [];
			...
			const providerAvailable =
				item.archived !== true && basePlanReadiness.some((plan) => plan.newSubscriberAvailable);
			const providerPurchasable =
				providerApproved &&
				basePlanReadiness.some(
					(plan) =>
		...
		...oneTimeProducts.map((item): StoreCatalogProduct | null => {
			...
			if (!storeProductId) return null;
			const listing = localizedListing(item.listings);
			const purchaseOptions = Array.isArray(item.purchaseOptions)
				? (item.purchaseOptions as Array<Record<string, unknown>>)
				: [];
			return {
				applicationId: app.applicationId,
				storeProductId,
				...
				active:
					...
					purchaseOptions.some((option) => String(option.state || "").toUpperCase() === "ACTIVE"),
				metadata: {
					source: "google_play",
					purchase_options: purchaseOptions.map((option) => ({
						purchase_option_id: option.purchaseOptionId || null,
...
export async function verifyGooglePurchase(
	...
	const lineItems = Array.isArray(verified.lineItems)
		? (verified.lineItems as Array<Record<string, unknown>>)
		: [];
	const lineItem =
		lineItems.find((item) => item.productId === params.storeProductId) || lineItems[0] || {};
	const externalIdentifiers = verified.externalAccountIdentifiers as
		| Record<string, unknown>
		| undefined;
	const boundCustomer = String(
		externalIdentifiers?.obfuscatedExternalAccountId || verified.obfuscatedExternalAccountId || "",
	);
	...
	const effectiveCustomerId =
		params.customerId || (params.allowTransfer ? boundCustomer || null : null);
	...
	const status: BillingStatus =
		productType === "subscription"
			...
			: numericPurchaseState === 0
				? "active"
				: numericPurchaseState === 2
					? "pending"
					: "revoked";
	...
	const priceMicros =
		productType === "subscription"
			? Number(recurringPrice?.units || 0) * 1_000_000 +
				Math.round(Number(recurringPrice?.nanos || 0) / 1000)
	...
	const purchase: VerifiedPurchase = {
		...
		purchasedAt:
			typeof verified.startTime === "string"
				? verified.startTime
				: isoMillis(verified.purchaseTimeMillis),
		eventOccurredAt: new Date().toISOString(),
...
export function googlePurchaseEnvironment(
	...
	const test =
		productType === "subscription"
			...
			: Object.prototype.hasOwnProperty.call(verified, "testPurchaseContext") ||
				Number(verified.purchaseType) === 0;
	return test ? "sandbox" : "production";
}

export async function classifyGooglePurchaseEnvironment(
	env: BillingEnv,
	params: {
		projectId: string | number;
		purchaseToken: string;
...
export async function finalizeGooglePurchase(params: {
	...
	if (
		...
		Number(params.verified.acknowledgementState) === 1
	)
		return;
	...
	const url =
		params.purchase.productType === "subscription"
...
export async function reconcileAppleSubscription(
	...
	const latest =
		...
		(statuses.data || []).flatMap(
			(group: { lastTransactions?: Array<Record<string, any>> }) => group.lastTransactions || [],
		)[0];
	if (!latest?.signedTransactionInfo)
		throw new Error("Apple subscription status has no transaction");
...
export async function testStoreCredentials(
	env: BillingEnv,
	params: {
		projectId: string | number;
		platform: "ios" | "android";
