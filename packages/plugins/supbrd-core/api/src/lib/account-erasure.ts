		...
		try {
			const result = await eraseApplicationAccount(
				env,
				{
					...
					environment: row.project_ref.endsWith("-test") ? "test" : "production",
				},
			...
			console.error(
				JSON.stringify({
					...
					operation_id: row.id,
					service: error instanceof AccountErasureError ? error.service : "gateway",
					code:
						error instanceof AccountErasureError ? error.code : "account_erasure_internal_error",
				}),
			);
		}
	}
...
async function runClaimedErasure(
	...
	try {
		for (const step of ERASURE_STEPS) {
			...
			const response = await step.run(env, operation, project, userId, requestId);
			if (!response.ok) {
       ...
       WHERE id = ? AND lease_id = ?`,
			...
			.bind(JSON.stringify([...completed]), resolved.code, resolved.service, operation.id, leaseId)
			.run();
		throw resolved;
	}
}
	...
	},
	{
		...
		run: (env, operation, project, userId, requestId) => {
			if (!env.BILLING || !env.MODULE_INTERNAL_TOKEN) {
				throw new AccountErasureError("billing_erasure_misconfigured", "billing");
			}
			return env.BILLING.fetch("https://billing.internal/internal/v1/customers/erase", {
				method: "POST",
				headers: {
	...
	},
	{
		...
		run: async (env, operation, project, userId, requestId) => {
			if (!env.IDENTITY_SERVICE || !env.MODULE_INTERNAL_TOKEN) {
				throw new AccountErasureError("identity_erasure_misconfigured", "identity");
			}
...
function domainStep(
	...
	binding: "APP_MODULE" | "ANALYTICS_MODULE" | "MARKETING_MODULE" | "SUPPORT_MODULE",
): Step {
	return {
		id: module,
		...
		run: async (env, operation, project, userId, requestId) => {
			...
			if (!service || !secret) {
				throw new AccountErasureError(`${module}_erasure_misconfigured`, module);
			}
...
async function ensureOperation(
	...
	const row = await db
     ...
     WHERE project_id = ? AND application_user_hash = ? LIMIT 1`,
		...
		.first<ErasureRow>();
	if (!row) {
		throw new AccountErasureError("account_erasure_store_unavailable", "gateway");
	}
	return row;
}
...
function validUserId(value: string): string {
	...
	if (!/^[A-Za-z0-9][A-Za-z0-9._:-]{0,254}$/u.test(resolved)) {
		throw new AccountErasureError("application_user_id_invalid", "identity", 422, false);
	}
	return resolved;
}
