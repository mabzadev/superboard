...
export async function verifiedAppUserId(
	env: BillingEnv,
	projectId: string | number,
	...
	const verify = (keys: JSONWebKeySet) =>
...
export async function resolveSdkCustomer(
	env: BillingEnv,
	params: {
		projectId: string | number;
		authorization?: string;
	...
	if (!verified && !appUserId.startsWith("$superboard_anon_"))
		throw new Error("Anonymous ID has an invalid prefix");
...
export async function signedCustomerInfo(
	env: BillingEnv,
	projectId: string | number,
	customerId: string,
) {
...
export async function signCustomerInfoPayload(
	...
	return new SignJWT({ customer_info: info, project_id: String(projectId) })
		...
		.setIssuer("superboard-purchases")
		.setAudience("superboard-sdk")
		.setSubject(subject)
		.setIssuedAt()
...
export async function customerForAppUserId(
	env: BillingEnv,
	projectId: string | number,
	appUserId: string,
) {
