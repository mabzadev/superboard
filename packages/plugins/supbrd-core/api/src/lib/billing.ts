...
export async function identifyCustomer(
	...
	const source = await getOrCreateCustomer(
		...
		projectId,
		currentAppUserId,
		currentAppUserId.startsWith("$superboard_anon_"),
	);
	const target =
	...
	await db.batch([
		...
		db
			.prepare("UPDATE billing_transactions SET customer_id = ? WHERE customer_id = ?")
			.bind(destination.id, merged.id),
		db
			.prepare("UPDATE billing_subscriptions SET customer_id = ? WHERE customer_id = ?")
			.bind(destination.id, merged.id),
		db
			.prepare("UPDATE billing_events SET customer_id = ? WHERE customer_id = ?")
			.bind(destination.id, merged.id),
		db
			.prepare("UPDATE billing_paywall_events SET customer_id = ? WHERE customer_id = ?")
			.bind(destination.id, merged.id),
		db
			.prepare(
		...
		db
			...
			.bind(destination.id, merged.id),
		db
			.prepare("UPDATE billing_balance_ledger SET customer_id = ? WHERE customer_id = ?")
			.bind(destination.id, merged.id),
...
async function getOrCreateProduct(db: D1Database, purchase: VerifiedPurchase) {
  ...
  `,
		...
		.bind(String(purchase.projectId), purchase.store, purchase.environment, purchase.storeProductId)
		.first<Record<string, unknown>>();
...
export async function applyVerifiedPurchase(env: BillingEnv, purchase: VerifiedPurchase) {
	...
	const analyticsEventId = await canonicalAnalyticsEventId("purchase", analyticsIdentity);
	...
	const analyticsEvent: AnalyticsEventV1 = {
		...
		occurred_at: purchase.eventOccurredAt || purchase.purchasedAt || new Date().toISOString(),
		source: "billing",
		...
		properties: {
			...
			product_id: purchase.storeProductId,
			...(purchase.priceMicros == null ? {} : { amount_micros: purchase.priceMicros }),
  ...
  `,
    ...
    `,
			...
			.bind(
				...
				purchase.autoRenews &&
					!["cancelled", "expired", "revoked", "refunded"].includes(purchase.status)
					? 1
					: 0,
			)
			.run();
...
function verifiedPurchaseAnalyticsEventType(purchase: VerifiedPurchase): VerifiedPurchaseEventType {
...
export async function customerInfo(db: D1Database, projectId: string | number, customerId: string) {
  ...
  `,
	...
	const subscriptionRows: Array<Record<string, unknown>> = (subscriptions.results || []).map(
		(row) => ({
			...row,
			management_url:
				row.store === "apple"
	...
	return {
		...
		balances: Object.fromEntries(
			(balances.results || []).map((row) => [row.currency_identifier, Number(row.balance)]),
		),
	};
}
