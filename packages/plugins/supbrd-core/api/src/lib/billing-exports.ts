...
export async function processBillingExport(env: BillingEnv, exportId: string) {
  const job = await env.DB.prepare(
    `SELECT * FROM billing_export_jobs WHERE id = ? LIMIT 1`,
  )
    .bind(exportId)
  ...
  if (job.format !== "csv")
    throw new Error("Unsupported billing export format");
  await env.DB.prepare(
    `UPDATE billing_export_jobs SET status = 'running', started_at = datetime('now'), error_message = NULL WHERE id = ?`,
  )
    .bind(exportId)
    .run();
  try {
    const result = await env.DB.prepare(dataset.sql)
      .bind(String(job.project_id))
      .all<Record<string, unknown>>();
    const requested = parseColumns(job.columns);
    const columns = requested.length
      ? requested.filter((item) => dataset.columns.includes(item))
      : dataset.columns;
    ...
    await env.R2.put(key, lines.join("\n"), {
      httpMetadata: {
        ...
        contentDisposition: `attachment; filename="superboard-${job.dataset}-${job.id}.csv"`,
      },
      customMetadata: {
        project_id: String(job.project_id),
        dataset: String(job.dataset),
        rows: String(result.results?.length || 0),
      },
    });
    await env.DB.prepare(
      `UPDATE billing_export_jobs SET status = 'completed', r2_key = ?, row_count = ?, completed_at = datetime('now') WHERE id = ?`,
    )
      .bind(key, result.results?.length || 0, exportId)
      .run();
    return { id: exportId, key, rows: result.results?.length || 0 };
  } catch (error) {
    await env.DB.prepare(
      `UPDATE billing_export_jobs SET status = 'failed', error_message = ?, completed_at = datetime('now') WHERE id = ?`,
    )
      .bind((error as Error)?.message || String(error), exportId)
      .run();
    throw error;
  }
}
