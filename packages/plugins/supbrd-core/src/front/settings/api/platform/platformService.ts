...
export async function getPlatformCustomJobs(query = "limit=50"): Promise<PlatformCustomJobPage> {
	const response = await GET(`${config.apiPath}/platform/custom/jobs?${query}`, {
		timeout: 10_000,
		maxRetries: 1,
	});
...
export async function getPlatformAccountErasures(
	...
	const response = await GET(`${config.apiPath}/platform/account-erasures?${query}`, {
		timeout: 10_000,
		maxRetries: 1,
	});
	const body = response.data as { data: PlatformAccountErasure[] } | PlatformAccountErasure[];
...
export async function getPlatformEmailOperations(
	...
	const response = await GET(`${config.apiPath}/platform/email/operations?${query}`, {
		timeout: 10_000,
		maxRetries: 1,
	});
...
export async function retryPlatformCustomJob(jobId: string): Promise<PlatformCustomJob> {
	const response = await POST(
...
export async function getPlatformLibraries(): Promise<PlatformLibraryCatalog> {
	...
	const body = response.data as PlatformLibraryCatalog | { data: PlatformLibraryCatalog };
