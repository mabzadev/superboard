...
import { useSearchParams, useRouter, usePathname } from "@superboard/front-ui/navigation";
import { type SetStateAction, useCallback, useEffect, useMemo, useRef, useState } from "react";
import type { DateRange } from "react-day-picker";

import type { SortType } from "../types/index.js";
...
export function useTableParams(options?: UseTableParamsOptions) {
	const { defaultSortKey = "updated_at", defaultPageSize = 25, defaultDateRange } = options ?? {};
	...
	// --- Platform ---
	const platform = useMemo(() => searchParams.get("platform") ?? "", [searchParams]);
