...
import {
	...
	bundleIdSchema,
	shaSchema,
} from "../../../../../supbrd-runtime-plugins/src/front/client/plugins/supbrd-plug-settings/source/schemas/shared.js";
import { hasText, isRedirectUrlValid, isSafePublicHttpsUrl, isUrlSchemeValid } from "../validation.js";
...
describe("isSafePublicHttpsUrl", () => {
	it("accepts public HTTPS destinations", () => {
		expect(isSafePublicHttpsUrl("https://hooks.example.com/support")).toBe(true);
	});

	it("rejects private, credentialed and non-HTTPS destinations", () => {
		expect(isSafePublicHttpsUrl("http://hooks.example.com/support")).toBe(false);
		expect(isSafePublicHttpsUrl("https://127.0.0.1/support")).toBe(false);
		expect(isSafePublicHttpsUrl("https://user:secret@example.com/support")).toBe(false);
		expect(isSafePublicHttpsUrl("https://hooks.example.com/support#secret")).toBe(false);
	});
});
...
describe("httpUrlSchema", () => {
	...
	it("accepts localhost URLs", () => {
		...
		expect(httpUrlSchema.safeParse("https://localhost:8080").success).toBe(true);
	});
...
describe("redirect form validation", () => {
	it("requires a secure destination for custom redirects while allowing the default destination", () => {
		expect(isRedirectUrlValid("default", null, "default")).toBe(true);
		for (const input of [undefined, null, "", "example.com/path", "http://example.com/path"])
			expect(isRedirectUrlValid("custom", input, "default")).toBe(false);
		expect(isRedirectUrlValid("custom", "https://example.com/path?source=app", "default")).toBe(true);
	});
	it("does not treat missing values or whitespace-only metadata as entered text", () => {
		for (const input of [null, undefined, 12, {}, [], "", " \t\n", "\u00a0"])
			expect(hasText(input)).toBe(false);
		expect(hasText("  Campaign title  ")).toBe(true);
	});
});
