...
} from "../../../../../../supbrd-runtime-plugins/src/front/client/plugins/supbrd-plugmod-onboardings/source/components/experience-editor/model.js";
...
describe("experience editor model", () => {
	...
	it("reports duplicate ids, empty text and stale transitions", () => {
		...
		const messages = validateExperienceDocument(document).map(({ message }) => message);
	...
	it("converts paywalls without leaking onboarding-only benefits", () => {
		...
		expect(paywall.components.map(({ type }) => String(type))).not.toContain("benefits");
		expect(fromPaywallDefinition(paywall).screens[0]?.blocks).toEqual(paywall.components);
	});
