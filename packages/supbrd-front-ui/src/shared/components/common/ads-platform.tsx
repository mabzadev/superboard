...
const AdsPlatformSelect = ({
	...
	const getValueLabelFromList = (value: string) => {
		...
		if (foundValue) {
			...
			if (foundValue.label === "All" && foundValue?.filterType === "platforms") {
				return "Platforms";
			}
	...
	const changeBgColorForNonDefaultValues = () => {
		const foundValue = platformAdsOptions.find((item) => item.value === selectedAdsPlatform);
	...
	return (
		<Popover>
			...
			<PopoverContent className="w-[200px] p-2 space-y-2">
				{selectListTitle && (
					<div>
						<label className="text-sm text-muted-foreground">{selectListTitle}</label>
					</div>
				)}
