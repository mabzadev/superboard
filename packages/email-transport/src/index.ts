...
export async function sendSmtpMessage(
  ...
  try {
    ...
    await session.command(
      `EHLO ${sanitizeDomain(config.from_email.split("@")[1] || "opengrow.local")}`,
      [250],
    );
    ...
    if (config.username) {
      if (!secret.password)
        throw error("smtp_password_missing", "SMTP password is missing");
      await session.command(
    ...
    await session.command(
      `MAIL FROM:<${sanitizeMailbox(config.from_email)}>`,
      [250],
    );
    await session.command(
      `RCPT TO:<${sanitizeMailbox(message.to)}>`,
      [250, 251],
    );
    ...
    try {
      ...
      if (
        cause instanceof EmailTransportError &&
        cause.code === "smtp_rejected"
      ) {
        throw cause;
      }
      throw error(
  ...
  async expect(expected: number[]): Promise<{ code: number; text: string }> {
    ...
    while (true) {
      ...
      if (lines.length > 100)
        throw error(
          "smtp_protocol_error",
          "SMTP response exceeded the allowed size",
          502,
        );
    }
...
function sanitizeDomain(value: string) {
  return value.replace(/[^a-zA-Z0-9.-]/g, "").slice(0, 253) || "opengrow.local";
}
