...
export type ProjectContextAudience = DomainModuleName | "identity" | "billing" | "files";
...
export interface ProjectContext {
	...
	/** Legacy numeric actor ID, or 0 for SDK and EmDash operators. */
	actorId: number;
	operatorId?: string;
	role: string;
	requestId: string;
...
const CONTEXT_PREFIX = "superboard-project-context-v1";
...
export function canonicalProjectContext(context: InternalProjectContext): string {
	return [
		...
		...(context.operatorId ? ["site-operator", context.operatorId] : []),
	].join("\n");
}
...
export async function verifyInternalProjectContextRequest(
	...
	const candidates =
		typeof secret === "string" ? configuredSecrets(secret) : configuredSecrets(...secret);
	...
	const projectId = positiveInteger(request.headers.get(PROJECT_CONTEXT_HEADERS.projectId));
	const instanceId = positiveInteger(request.headers.get(PROJECT_CONTEXT_HEADERS.instanceId));
	const actorId = nonNegativeInteger(request.headers.get(PROJECT_CONTEXT_HEADERS.actorId));
	const operatorId = request.headers.get(PROJECT_CONTEXT_HEADERS.operatorId) || undefined;
	const issuedAt = positiveInteger(request.headers.get(PROJECT_CONTEXT_HEADERS.issuedAt));
	...
	if (
		...
		!instanceId ||
		actorId === null ||
		(operatorId !== undefined && (actorId !== 0 || !OPERATOR_ID_PATTERN.test(operatorId))) ||
		!issuedAt ||
		!parsedRef ||
...
function importHmacKey(secret: string, keyUsages: Array<"sign" | "verify">): Promise<CryptoKey> {
...
import { configuredSecrets, constantTimeEqual, type SecretCandidates } from "./secret";

export async function createProjectContextHeaders(
	context: InternalProjectContext,
	secret: string,
): Promise<Headers> {
	const headers = new Headers({
		...
		[PROJECT_CONTEXT_HEADERS.version]: CONTEXT_VERSION,
