 ...
 */
export async function assertSqlDatabaseHealth(database: SqlHealthDatabase): Promise<void> {
	const row = await database
		.prepare("SELECT 1 AS superboard_health_check")
		.first<{ superboard_health_check: number }>();
	if (Number(row?.superboard_health_check) !== 1) {
...
export async function inspectSqlSchemaHealth(
	...
	const latestMigration = typeof row?.latest_migration === "string" ? row.latest_migration : null;
	const expectedApplied = Number(row?.expected_migration_applied) === 1;
	if (
		!Number.isSafeInteger(appliedMigrationCount) ||
		appliedMigrationCount < 0 ||
		(appliedMigrationCount > 0 &&
			(!latestMigration || !MIGRATION_FILENAME_PATTERN.test(latestMigration))) ||
	...
	return {
		status:
			latestMigration === expectedMigration ? "current" : expectedApplied ? "drifted" : "behind",
		expectedMigration,
		latestMigration,
