...
if [ -z "$1" ]; then
    print_error "Usage: ./opengrow_android_gcloud_setup.sh <project-id>"
    echo "Example: ./opengrow_android_gcloud_setup.sh my-app-477213"
    exit 1
fi
...
cat > opengrow_android_publish_generated.tf <<EOF
variable "project_id" {
...
resource "google_service_account" "play_api" {
  project      = var.project_id
  account_id   = "opengrow-play-api-service-account"
...
if [ -n "$CLOUD_SHELL" ]; then
    ...
    print_info "Key saved to ./opengrow/play-api-key.json"
...
echo ""
echo "╔════════════════════════════════════════════════════════════════╗"
echo "║        Google Cloud Account Setup for OpenGrow Complete           ║"
echo "╚════════════════════════════════════════════════════════════════╝"
echo ""
...
echo "  1.Upload the downloaded json key file to OpenGrow:"
echo "      - Go to OpenGrow Dashboard > Developer > Android Setup"
...
# rm opengrow_android_publish_generated.tf