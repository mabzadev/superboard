...
import { flowsApi, type FlowUserDetails } from "../../api/flows/flowsService.js";
...
export function FlowUserDetailsPage({ userHash }: { userHash: string }) {
	...
	const reset = async (workflowId?: string) => {
		...
		try {
			...
			showSuccessNotification(`${result.states_removed} ${tr("States reset").toLowerCase()}`);
			await load();
		} catch (cause) {
			showErrorNotification(cause instanceof Error ? cause.message : t("apiFailure"));
		}
						...
						{details.profiles.map((profile) => (
							<Card key={profile.environment_id}>
								...
								<CardContent className="grid gap-1 text-sm">
									...
									<p className="text-muted-foreground">{profile.country ?? tr("No country")}</p>
								...
								<Card key={`${state.workflow_id}:${state.updated_at}`}>
									<CardContent className="flex flex-wrap items-center gap-3 py-0">
										...
										<span className="text-xs text-muted-foreground">
											{state.active_block_ids.length} {tr("Active blocks").toLowerCase()}
										</span>
										<Button
											variant="outline"
											size="sm"
