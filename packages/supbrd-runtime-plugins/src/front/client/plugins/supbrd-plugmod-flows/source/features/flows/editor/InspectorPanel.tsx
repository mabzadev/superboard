...
import { showErrorNotification } from "../../../../../../../../../../supbrd-front-ui/src/shared/lib/Notifications.js";
import type {
	FlowBlock,
...
export function InspectorPanel({
	...
	return (
		...
		>
			...
			{path && onPathChange ? (
				<PathProperties path={path} onChange={onPathChange} onDelete={onDelete} />
			) : !block ? (
				<p className="px-5 py-8 text-sm text-muted-foreground">{t("noSelection")}</p>
			) : (
				<div className="grid gap-5 px-5 py-4">
					...
					<BlockProperties block={block} components={components} onChange={onChange} />
					<LocalizationEditor
						block={block}
						locales={locales}
		...
		<div className="grid gap-5 px-5 py-4">
			...
			<label className="grid gap-2">
				...
				<Input
					...
					onChange={(event) => onChange({ ...path, label: event.target.value || null })}
				/>
			</label>
		...
		<section className="grid gap-3 border-t pt-4">
			...
			{fields.map(([key, fallback]) => {
				const translated = translations.find(
					(item) =>
						item.block_key === block.key && item.property_key === key && item.locale === locale,
				)?.value;
				return (
					<TranslationField
						key={`${locale}:${key}`}
	...
	return (
		<label className="grid gap-1.5">
			<span className="flex items-center justify-between gap-2">
				...
				<span className={saved ? "text-[10px] text-green-600" : "text-[10px] text-amber-600"}>
					...
					void onSave(draft)
						...
						.catch((cause: unknown) => {
							...
							showErrorNotification(
								cause instanceof Error ? cause.message : tr("Unable to save translation"),
							);
						})
...
function BlockProperties({
	...
	const selectedComponent = compatibleComponents.find(
		(component) =>
			component.key === block.data.componentKey || component.component_type === block.componentType,
	);
		...
		<section className="grid gap-3 border-t pt-4">
						...
						{compatibleComponents.length > 0 ? (
							<Select
								...
								onValueChange={(key) => {
									...
									if (component) onChange(applyComponentDefinition(block, component));
								}}
							>
							...
							<Input
								...
								onChange={(event) => onChange({ ...block, componentType: event.target.value })}
							/>
						)}
			...
			{block.type === "workflow-trigger" && (
				<>
					...
					<label className="grid gap-2">
						<Label htmlFor="target-manual-start-key">{tr("Manual start block key")}</Label>
						<Input
			...
			{block.type === "survey" && (
				<SurveyBuilder
					...
					onChange={(surveyQuestions) => onChange({ ...block, surveyQuestions })}
				/>
			)}
			{block.type === "tour" && (
				<TourBuilder
				...
				<TrafficSplitBuilder
					...
					onChange={(variants) =>
						onChange({
							...
							exitNodes: variants.map((variant) => String(variant.key ?? "")).filter(Boolean),
						})
					}
				/>
			)}
				...
				>
					...
					>
						...
						<SelectContent>
							...
							<SelectItem value="single-choice">{tr("Single choice")}</SelectItem>
							<SelectItem value="multiple-choice">{tr("Multiple choice")}</SelectItem>
						...
						<Input
							...
							onChange={(event) =>
								onChange(
									replace(value, index, {
										...question,
										options: updateQuestionOptions(question.options, csv(event.target.value)),
									}),
								)
							}
						/>
								...
								<Select
									...
									onValueChange={(displayType) =>
										onChange(
											replace(value, index, {
												...question,
												displayType: displayType as "number" | "stars" | "emoji",
											}),
										)
									}
								>
						...
						<div className="grid grid-cols-2 gap-2">
							...
							<Toggle
								...
								onChange={(shuffleOptions) =>
									onChange(replace(value, index, { ...question, shuffleOptions }))
								}
							/>
						</div>
					)}
	...
	const add = () => {
		...
		onChange([
			...value,
			{
				...
				componentType: component?.component_type ?? BASICS_V2_COMPONENT_TYPES.tooltip,
			...
			{value.map((step, index) => (
				...
				>
					<Input
						...
						onChange={(event) =>
							onChange(replace(value, index, { ...step, name: event.target.value }))
						}
					/>
					<div className="grid grid-cols-2 gap-2">
						{components.length > 0 ? (
							...
							<Input
								value={String(step.componentType ?? BASICS_V2_COMPONENT_TYPES.tooltip)}
						...
						<Input
							...
							onChange={(event) =>
								onChange(replace(value, index, { ...step, anchor: event.target.value }))
							}
						/>
					</div>
					<div className="grid grid-cols-2 gap-2">
						<Select
							...
							onValueChange={(trigger) => onChange(replace(value, index, { ...step, trigger }))}
						>
							<SelectTrigger className="w-full">
								<SelectValue />
							</SelectTrigger>
							<SelectContent>
								...
								<SelectItem value="element-present">{tr("Element present")}</SelectItem>
								<SelectItem value="element-absent">{tr("Element absent")}</SelectItem>
							</SelectContent>
						</Select>
						<Select
							value={String(step.wait ?? "none")}
							onValueChange={(wait) => onChange(replace(value, index, { ...step, wait }))}
						>
							<SelectTrigger className="w-full">
								<SelectValue />
							</SelectTrigger>
...
function ConditionsBuilder({
	...
	const { tr } = useFlowI18n();
	const add = () =>
		onChange([...value, { key: "", data_type: "string", operator: "equals", value: [""] }]);
	return (
...
function TrafficSplitBuilder({
	...
	const { tr } = useFlowI18n();
	const total = value.reduce((sum, variant) => sum + Number(variant.weight ?? 0), 0);
	return (
		<EditorCollection
			title={`${tr("Stable variants")} · ${total}%`}
			onAdd={() => onChange([...value, { key: `variant_${value.length + 1}`, weight: 0 }])}
		>
				...
				>
					<div className="grid grid-cols-[1fr_96px] gap-2">
						<Input
							...
							onChange={(event) =>
								onChange(replace(value, index, { ...variant, key: event.target.value }))
							}
						/>
						<Input
							type="number"
...
function PropertyBagEditor({
	...
	const remove = (index: number) =>
		onChange({
			...block,
			propertyMeta: block.propertyMeta.filter((_, itemIndex) => itemIndex !== index),
		});
	const add = () => {
		const key = newKey.trim().replace(/[^a-zA-Z0-9_-]/g, "_");
		if (!key || block.propertyMeta.some((property) => property.key === key)) return;
		...
		onChange({
			...block,
			propertyMeta: [...block.propertyMeta, { key, type: newType, value: defaults[newType] ?? "" }],
		});
		setNewKey("");
	};
	return (
		<div className="grid gap-3 border-t pt-4">
			...
			{block.propertyMeta.map((property, index) => (
				<div key={`${property.key}:${index}`} className="grid gap-2 rounded border p-2">
					<div className="flex items-center gap-2">
						<span className="min-w-0 flex-1 truncate font-mono text-xs">{property.key}</span>
						<span className="rounded bg-muted px-1.5 py-0.5 text-[10px]">{property.type}</span>
						<Button
							variant="ghost"
							size="icon"
					...
					<TypedProperty value={property.value} onChange={(next) => update(index, next)} />
				</div>
			))}
			<div className="grid grid-cols-[1fr_132px_auto] gap-1">
				...
				/>
				<Select value={newType} onValueChange={(value) => setNewType(value as FlowPropertyType)}>
					<SelectTrigger className="w-full">
						<SelectValue />
					</SelectTrigger>
					<SelectContent>
				...
				<Button size="icon" variant="outline" disabled={!newKey.trim()} onClick={add}>
					<Plus />
