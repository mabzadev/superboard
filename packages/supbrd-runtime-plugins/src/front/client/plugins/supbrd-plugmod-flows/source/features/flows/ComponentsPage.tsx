...
import { FlowsEmptyState, FlowsPage } from "./FlowsPage.js";
...
export function ComponentsPage() {
	...
	const toggle = async (library: FlowComponentLibrary, enabled: boolean) => {
		if (!projectRef) return;
		setLibraries((items) =>
			items.map((item) => (item.id === library.id ? { ...item, enabled } : item)),
		);
		try {
	...
	const synchronize = async (component: FlowComponentDefinition) => {
		if (!projectRef) return;
		try {
			const result = await flowsApi.synchronizeComponent(projectRef, component.id);
			showSuccessNotification(
				...
				<section key={library.id} className="grid gap-3">
					<header className="flex flex-wrap items-center justify-between gap-3 rounded-[var(--radius)] border bg-card p-4">
						...
						<div className="flex items-center gap-2">
							{library.source === "custom" && (
								<Button variant="outline" size="sm" onClick={() => setComponentDialog({ library })}>
					...
					<div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
						{(byLibrary.get(library.id) ?? []).map((definition) => {
							const Icon = ICONS[definition.component_type] ?? ICONS[definition.key] ?? Component;
							return (
								<Card key={definition.id}>
									...
									<CardContent className="grid gap-3">
										...
										<p className="text-xs text-muted-foreground">
											{Object.keys(definition.css_variables).length} CSS variables ·{" "}
											{schemaProperties(definition).length} properties ·{" "}
											{definition.instance_count ?? 0} instances
										</p>
										<p className="text-xs text-muted-foreground">
											{t("outdatedInstances")}: {definition.outdated_instances ?? 0}
										</p>
										{library.source === "custom" && (
											<Button
												variant="outline"
										...
										{Number(definition.outdated_instances ?? 0) > 0 && (
											...
											>
												<RefreshCw /> {t("updateInstances")} ({definition.outdated_instances})
											</Button>
										)}
									</CardContent>
								</Card>
	...
	return (
		<DialogContent>
			<DialogHeader>
				...
				<DialogDescription>
					{tr("Custom libraries are available to every workflow in this project.")}
				</DialogDescription>
			</DialogHeader>
			...
			<DialogFooter>
				<Button disabled={!name.trim() || !key || busy} onClick={() => void create()}>
...
function ComponentDefinitionDialog({
	...
	const [templateType, setTemplateType] = useState(String(schema.template_type ?? "component"));
	...
	const [exitNodes, setExitNodes] = useState(component?.exit_nodes.join(", ") ?? "continue, close");
	...
	const save = async () => {
		...
		try {
			...
			showSuccessNotification(
				component ? tr("New component version created") : tr("Component created"),
			);
			onSaved();
		} catch (cause) {
			showErrorNotification(tr(errorMessage(cause)));
		...
		<DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-2xl">
			...
			<div className="grid gap-4 sm:grid-cols-2">
				<label className="grid gap-2">
					...
					<Input
						...
						onChange={(event) => {
							setName(event.target.value);
							if (!key) setKey(toIdentifier(event.target.value));
							if (!componentType) setComponentType(toIdentifier(event.target.value));
						}}
					/>
				</label>
				<label className="grid gap-2">
				...
				<label className="grid gap-2">
					...
					<Input
						...
						disabled={Boolean(component)}
						value={componentType}
						onChange={(event) => setComponentType(toIdentifier(event.target.value))}
					/>
				</label>
				<label className="grid gap-2">
					...
					<Select value={templateType} onValueChange={setTemplateType}>
						...
						<SelectContent>
							...
							<SelectItem value="tour-component">{tr("Tour component")}</SelectItem>
							<SelectItem value="survey-component">{tr("Survey component")}</SelectItem>
						</SelectContent>
					</Select>
				</label>
			...
			<DialogFooter>
				<Button
					...
					onClick={() => void save()}
				>
					{busy ? tr("Saving…") : component ? tr("Create version") : tr("Create component")}
				</Button>
			</DialogFooter>
		</DialogContent>
	);
...
function schemaProperties(component?: FlowComponentDefinition): Record<string, unknown>[] {
