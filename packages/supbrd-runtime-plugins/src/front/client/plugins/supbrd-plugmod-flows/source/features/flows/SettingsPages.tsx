...
export function LocalizationSettingsPage() {
	...
	return (
		<FlowsPage title={t("localization")} description={t("localizationDescription")}>
				...
				<Card>
					...
					<CardContent className="grid max-w-xl gap-4">
						...
						<label className="grid gap-2">
							...
							<Input
								value={defaultLocale}
								onChange={(event) => setDefaultLocale(identifier(event.target.value))}
							/>
						</label>
						<label className="grid gap-2">
...
export function SdkSettingsPage() {
	...
	useEffect(() => {
		...
		void flowsApi
			...
			.catch((cause: unknown) => showErrorNotification(message(cause, t("apiFailure"))));
	}, [projectRef, t]);
	const tabs = useMemo(
		() =>
			[
