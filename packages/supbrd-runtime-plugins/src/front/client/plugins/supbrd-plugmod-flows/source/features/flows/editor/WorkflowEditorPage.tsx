...
import { FlowsPage } from "../FlowsPage.js";
import { FlowStatusBadge } from "../FlowStatusBadge.js";
...
export function WorkflowEditorPage({ workflowId }: { workflowId: string }) {
	...
	const [selectedVersion, setSelectedVersion] = useState<SelectedVersion>("draft");
	const [versionGraph, setVersionGraph] = useState<FlowGraph | null>(null);
	const [tab, setTab] = useState("editor");
	const [analytics, setAnalytics] = useState<FlowWorkflowAnalytics | null>(null);
	const [surveyAnalytics, setSurveyAnalytics] = useState<Record<string, FlowSurveyAnalytics>>({});
	...
	const load = useCallback(async () => {
		...
		setLoading(true);
		try {
			const [details, nextEnvironments, languageGroups, componentCatalog] = await Promise.all([
			...
			setLocales([...new Set(languageGroups.flatMap((group) => group.locales))]);
			setError(null);
		} catch (cause) {
	...
	useEffect(() => {
		...
		void flowsApi
			...
			.catch((cause: unknown) =>
				showErrorNotification(cause instanceof Error ? cause.message : t("apiFailure")),
			);
		return () => {
			cancelled = true;
		};
	...
	useEffect(() => {
		...
		void Promise.all([
			...
			Promise.all(surveyIds.map((surveyId) => flowsApi.surveyAnalytics(projectRef, surveyId))),
		])
			.then(([result, surveys]) => {
				setAnalytics(result);
				setSurveyAnalytics(Object.fromEntries(surveys.map((survey) => [survey.survey_id, survey])));
			})
			.catch((cause: unknown) =>
				showErrorNotification(cause instanceof Error ? cause.message : t("apiFailure")),
			);
	...
	const save = async (graph: FlowGraph, revision: number) => {
		...
		try {
			const saved = await flowsApi.saveDraft(projectRef, workflowId, graph, revision);
			setWorkflow((current) =>
	...
	return (
		<FlowsPage
			...
			description={workflow?.description ?? workflow?.identifier ?? t("workflowsDescription")}
			fullHeight
			actions={
				workflow ? (
					<>
						...
						<Select value={selectedVersion} onValueChange={setSelectedVersion}>
							<SelectTrigger className="w-44 bg-card" aria-label={tr("Workflow version")}>
								<GitCommitHorizontal />
								<SelectValue />
							</SelectTrigger>
							<SelectContent>
								<SelectItem value="draft">
									{t("draft")} · r{workflow.draft?.revision ?? workflow.draft_revision}
								</SelectItem>
			...
			<div className="flex min-h-0 flex-1 flex-col gap-3">
						...
						<div className="hidden items-center gap-2 text-xs text-muted-foreground md:flex">
							{workflow.releases
								...
								.map((release) => (
									...
									>
										<CheckCircle2 className="size-3 text-green-600" /> {release.environment_name}:{" "}
				...
				{workflow ? (
					<Tabs value={tab} onValueChange={setTab} className="min-h-0 flex-1 gap-3">
						...
						<TabsContent value="analytics" className="min-h-0 flex-1 overflow-y-auto">
							<WorkflowAnalytics analytics={analytics} surveys={surveyAnalytics} />
						</TabsContent>
					</Tabs>
				) : loading ? (
...
function PublishDialog({
	...
	return (
		<DialogContent>
			...
			<div className="grid gap-4">
				<label className="grid gap-2">
					...
					<Select value={strategy} onValueChange={setStrategy}>
						...
						<SelectContent>
							<SelectItem value="finish-current">{tr("End in-progress users")}</SelectItem>
							<SelectItem value="restart-current">
								{tr("Restart users currently in progress")}
							</SelectItem>
							<SelectItem value="restart-all">{tr("Restart every user")}</SelectItem>
						</SelectContent>
					</Select>
				</label>
				<label className="grid gap-2">
			...
			<DialogFooter>
				<Button disabled={!environmentId || busy} onClick={() => void publish()}>
...
function WorkflowAnalytics({
	...
	const { t, tr } = useFlowI18n();
	const { projectRef } = useFlows();
	const max = Math.max(1, ...(analytics?.totals.map((item) => Number(item.count)) ?? [1]));
	if (!analytics)
		return <div className="min-h-52 animate-pulse rounded-[var(--radius)] border bg-card" />;
	return (
					...
					<TableBody>
						{analytics.blocks.map((item) => (
							<TableRow key={`${item.block_id}:${item.event_name}`}>
								<TableCell className="font-mono text-xs">{item.block_key}</TableCell>
				...
				>
					<div className="flex flex-wrap items-start justify-between gap-3">
						<div>
							...
							<p className="font-mono text-xs text-muted-foreground">{survey.survey_id}</p>
						</div>
						<Button
							variant="outline"
							size="sm"
					...
					<div className="grid gap-4 lg:grid-cols-2">
						<div className="rounded border p-3">
							<h3 className="text-sm font-semibold">{tr("Rating statistics")}</h3>
							<dl className="mt-3 grid grid-cols-3 gap-2 text-sm">
								<Stat label={tr("Average")} value={survey.numeric.average} />
								<Stat label={tr("Median")} value={survey.numeric.median} />
								<Stat label={tr("Std. deviation")} value={survey.numeric.standard_deviation} />
							</dl>
						</div>
						<div className="rounded border p-3">
							<h3 className="text-sm font-semibold">{tr("Choice distribution")}</h3>
							<div className="mt-3 grid gap-2">
								{Object.entries(survey.distributions).map(([option, count]) => (
									<div key={option} className="flex justify-between gap-3 text-sm">
...
function Stat({ label, value }: { label: string; value: number | null }) {
	return (
		<div>
			...
			<dd className="mt-1 font-semibold tabular-nums">{value == null ? "—" : value.toFixed(2)}</dd>
		</div>
	);
}
