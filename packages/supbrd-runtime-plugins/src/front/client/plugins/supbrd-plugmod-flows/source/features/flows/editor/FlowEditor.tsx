...
import { BlockCatalog, FLOW_BLOCK_DRAG_TYPE, FLOW_COMPONENT_DRAG_TYPE } from "./BlockCatalog.js";
import { FlowNode, type FlowNodeData } from "./FlowNode.js";
import { createBlock, createBlockFromDefinition, createPath, validateGraph } from "./graph.js";
...
function FlowEditorInner({
	...
	const { graph, update, undo, redo, canUndo, canRedo } = useFlowHistory(initialGraph);
	...
	const invalidBlockIds = useMemo(
		() => new Set(validation.issues.flatMap((issue) => (issue.blockId ? [issue.blockId] : []))),
		[validation],
	);
	...
	const selectedBlock = graph.blocks.find((block) => block.id === selectedBlockId) ?? null;
	const selectedPath = graph.paths.find((path) => selectedPathIds.includes(path.id)) ?? null;
	...
	const addComponent = useCallback(
		(component: FlowComponentDefinition, position?: { x: number; y: number }) => {
	...
	const copySelection = useCallback(() => {
		...
		const selected = graph.blocks.filter((block) => block.id === selectedBlockId);
	...
	useEffect(() => {
		const listener = (event: KeyboardEvent) => {
			...
			if (modifier && event.key.toLowerCase() === "z") {
				...
				event.preventDefault();
				pasteSelection();
			} else if ((event.key === "Backspace" || event.key === "Delete") && !typing) {
				event.preventDefault();
				deleteSelection();
			}
		};
		...
		(changes: NodeChange[]) => {
			...
			if (removed.length) {
				commit((current) => ({
					...
					paths: current.paths.filter(
						(path) =>
							!removed.includes(path.sourceBlockId) && !removed.includes(path.targetBlockId),
					),
				}));
			}
		},
		...
		<div className="flex min-h-0 flex-1 overflow-hidden rounded-[var(--radius)] border bg-card">
			<div className="flex min-w-0 flex-1 flex-col">
				<div className="flex min-h-12 shrink-0 flex-wrap items-center gap-1 border-b bg-card px-2 py-1.5">
					...
					<ToolbarButton
						label={t("delete")}
						icon={Trash2}
						disabled={readOnly || (!selectedBlockId && selectedPathIds.length === 0)}
					...
					<div className="ml-auto flex items-center gap-2 pl-2">
						<span
							className={cn(
								...
								validation.valid ? "text-green-700 dark:text-green-300" : "text-destructive",
							)}
							title={validation.issues.map((issue) => tr(issue.message)).join("\n")}
						>
							{validation.valid ? (
							...
							<span className="hidden lg:inline">
								{validation.valid ? t("validGraph") : `${validation.issues.length} ${t("issues")}`}
							</span>
						</span>
						{!readOnly && (
							<Button
				...
				<div
					...
					onDrop={(event) => {
						event.preventDefault();
						const componentId = event.dataTransfer.getData(FLOW_COMPONENT_DRAG_TYPE);
						...
						if (component) addComponent(component, position);
							...
							const type = event.dataTransfer.getData(FLOW_BLOCK_DRAG_TYPE) as FlowBlockType;
							if (!type) return;
			...
			<InspectorPanel
				...
				onPathChange={(path) =>
					commit((current) => ({
						...current,
						paths: current.paths.map((item) => (item.id === path.id ? path : item)),
					}))
				}
