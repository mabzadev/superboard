...
import { Archive, Copy, ExternalLink, GitBranch, Plus, Search } from "lucide-react";
...
export function FlowsWorkflowsPage({ initialOrigin }: { initialOrigin?: string }) {
	...
	const duplicate = async (workflow: FlowWorkflow) => {
		...
		try {
			await flowsApi.duplicateWorkflow(projectRef, workflow.id, {
				...
				identifier: `${workflow.identifier}-copy-${Date.now().toString(36)}`.slice(0, 96),
			});
		...
		>
			<div className="flex flex-col gap-3 rounded-[var(--radius)] border bg-card p-3 md:flex-row md:items-center">
				...
				<Select value={status} onValueChange={setStatus}>
					...
					<SelectContent>
						...
						{(["draft", "active", "paused", "archived"] as const).map((value) => (
			...
			{migrateLabel && (
				<div className="rounded-[var(--radius)] border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-sm text-amber-800 dark:text-amber-200">
					{migrateLabel}:{" "}
					{tr("Historical identifiers and statistics remain available through Flows.")}
				</div>
			)}
			{error && (
				<div
							...
							<TableRow key={workflow.id}>
								...
								<TableCell>
									<div className="flex justify-end gap-1">
										<Button asChild variant="ghost" size="icon" title={t("open")}>
		...
		<DialogContent>
			...
			<div className="grid gap-4">
				...
				<label className="grid gap-2">
					...
					<Select value={frequency} onValueChange={(value) => setFrequency(value as FlowFrequency)}>
			...
			<DialogFooter>
				<Button disabled={!name.trim() || !identifier || busy} onClick={() => void create()}>
