...
import { flowsApi, type FlowEnvironment, type FlowUser } from "../../api/flows/flowsService.js";
...
export function FlowsUsersPage() {
	...
	const load = useCallback(async () => {
		...
		try {
			const [users, nextEnvironments] = await Promise.all([
				flowsApi.listUsers(projectRef, environmentId === ALL ? undefined : environmentId),
	...
	const filtered = items.filter(
		(item) =>
			!search.trim() ||
			[item.user_id_hash, item.locale, item.country, item.platform].some((value) =>
				String(value ?? "")
					.toLowerCase()
			...
			<div className="flex flex-col gap-3 rounded-[var(--radius)] border bg-card p-3 md:flex-row">
				<div className="relative flex-1">
					...
					<Input
						...
						placeholder={tr("Search users by hash, locale, country or platform")}
						className="pl-9"
					/>
				</div>
					...
					<TableBody>
						{filtered.map((user) => (
							<TableRow key={`${user.environment_id}:${user.user_id_hash}`}>
								<TableCell className="font-mono text-xs">{user.user_id_hash}</TableCell>
								...
								<TableCell>
									<Button asChild size="icon" variant="ghost">
										<Link href={`/flows/users/${encodeURIComponent(user.user_id_hash)}`}>
											<ExternalLink />
							...
							<TableRow>
								<TableCell colSpan={7} className="p-0">
									<FlowsEmptyState
										icon={Users}
										title={t("noData")}
										description={tr("Users appear only after the SDK identifies them.")}
									/>
								</TableCell>
							</TableRow>
						)}
