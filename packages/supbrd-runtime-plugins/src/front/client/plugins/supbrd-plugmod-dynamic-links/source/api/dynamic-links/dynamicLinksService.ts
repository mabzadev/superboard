...
export function toCampaignAnalytics(
	...
	return {
		...
		total_reactivations: numericValue(totals, "reactivations", "reactivation"),
		...
		total_revenue: numericValue(totals, "revenue_cents", "total_revenue", "revenue"),
	};
}
...
export async function createLink(projectRef: string, payload: Record<string, unknown>) {
	return unwrap<{ id: string; slug: string }>(await POST(path(projectRef, "/links"), payload));
}
...
export async function updateLink(projectRef: string, id: string, payload: Record<string, unknown>) {
	return unwrap<DynamicLink>(await PUT(path(projectRef, `/links/${id}`), payload));
}

export async function resolveLink(projectRef: string, slug: string, platform: string) {
...
export async function getLinkCampaignAnalytics(
	...
	return unwrap<CampaignAnalyticsRow[]>(
		await GET(path(projectRef, withQuery("/campaign-analytics", filters))),
	);
}

export async function createLinkCampaign(projectRef: string, payload: Record<string, unknown>) {
	return unwrap<{ id: string }>(await POST(path(projectRef, "/campaigns"), payload));
}
...
export async function getRedirectRules(projectRef: string) {
	return unwrap<RedirectRule[]>(await GET(path(projectRef, "/redirect-rules")));
}

export async function createRedirectRule(projectRef: string, payload: Record<string, unknown>) {
	return unwrap<{ id: string }>(await POST(path(projectRef, "/redirect-rules"), payload));
}
...
export async function updateRedirectRule(
	...
	return unwrap<{ id: string }>(await PUT(path(projectRef, `/redirect-rules/${id}`), payload));
}

export async function deleteRedirectRule(projectRef: string, id: string) {
	return unwrap(await DELETE_REQUEST(path(projectRef, `/redirect-rules/${id}`)));
}
...
export async function createDomain(projectRef: string, payload: Record<string, unknown>) {
	return unwrap<LinkDomain>(await POST(path(projectRef, "/domains"), payload));
}

export async function verifyDomain(projectRef: string, id: string, verified = true) {
...
export async function saveSocialPreview(projectRef: string, payload: Record<string, unknown>) {
...
export async function saveTracking(projectRef: string, payload: Record<string, unknown>) {
	return unwrap(await PUT(path(projectRef, "/tracking"), payload));
}

export async function getLinkStatistics(projectRef: string, filters: LinkStatisticsFilters = {}) {
