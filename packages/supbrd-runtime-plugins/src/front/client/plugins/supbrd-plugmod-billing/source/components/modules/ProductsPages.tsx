...
import { BillingLedgerMigration } from "./BillingLedgerMigration.js";
		...
		>
			{!selectedProject ? (
				...
				<div className="space-y-6">
					<BillingLedgerMigration
						key={selectedProject.id}
						projectRef={selectedProject.id}
						onComplete={() => setFilters((value) => ({ ...value, from: "", to: "" }))}
					/>
					<Card>
						<CardHeader>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Legacy Revenue metrics reassigned to Purchases, calculated server-side for the
								selected period.
							</CardDescription>
						</CardHeader>
							...
							>
								{(statistics?.by_product_platform ?? []).map((row) => (
									<TableRow key={`${row.product_id}:${row.platform}:${row.currency}`}>
										...
										<TableCell>{Number(row.units_sold).toLocaleString()}</TableCell>
										<TableCell>{Number(row.first_time_purchases).toLocaleString()}</TableCell>
										<TableCell>
											{money(Number(row.revenue_micros), row.currency || "USD")}
										</TableCell>
										<TableCell>{Number(row.cancellations).toLocaleString()}</TableCell>
									</TableRow>
								))}
							</DataTable>
						</CardContent>
					...
					<div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6">
						...
						/>
						<Metric label="Gross revenue" value={money(totals?.gross_revenue_micros)} />
						<Metric label="Refunded" value={money(totals?.refunded_micros)} />
						<Metric label="Net revenue" value={money(totals?.net_revenue_micros)} />
					...
					<Card>
						<CardHeader className="flex-row items-center justify-between gap-3">
							<div>
								...
								<CardDescription>Daily volume and gross revenue.</CardDescription>
							</div>
							<Button
								variant="outline"
								size="icon"
					...
					<div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									Validate catalog, entitlement and subscription flows without touching a live
									store.
								</CardDescription>
							</CardHeader>
						...
						<Card>
							<CardHeader>
								...
								<CardDescription>{items.length} matching transactions</CardDescription>
							</CardHeader>
							<CardContent className="overflow-x-auto">
								<DataTable
									headers={[
					...
					{purchaseDetail && (
						<Card>
							...
							<CardContent className="grid gap-6 lg:grid-cols-2">
								<div>
									<p className="mb-2 text-sm font-medium">Granted entitlements</p>
									<div className="space-y-2">
										...
										{!purchaseDetail.entitlements.length && (
											<p className="text-sm text-muted-foreground">No entitlement was granted.</p>
										)}
									</div>
								</div>
								<div>
					...
					<Card>
						...
						<CardContent className="overflow-x-auto">
							<DataTable
								headers={["Product", "Customer", "Status", "Period end", "Renewal", "Updated", ""]}
							...
							>
								{subscriptions.map((item) => (
									<TableRow key={item.id}>
										...
										<TableCell>{enabled(item.auto_renew) ? "Auto-renew" : "Off"}</TableCell>
...
export function OfferingsPage() {
	...
	const load = useCallback(async () => {
		if (!selectedProject) return;
		try {
			const [catalog, configuredPackages, configuredOfferings, runs] = await Promise.all([
				getProducts(selectedProject.id),
	...
	const sync = async () => {
		...
		try {
			...
			showErrorNotification(
				cause instanceof SyntaxError ? "Catalog must be valid JSON." : moduleErrorMessage(cause),
			);
		}
	};
	const matches = (title: string, identifier: string, active: boolean) => {
		const queryMatch = `${title} ${identifier}`.toLowerCase().includes(search.toLowerCase());
		return queryMatch && (visibility === "all" || (visibility === "active" ? active : !active));
	};
			...
			{!selectedProject ? (
				...
				<div className="space-y-6">
					...
					<div className="grid gap-6 xl:grid-cols-3">
						<EditorCard
							title={editing?.kind === "product" ? "Edit product" : "Create product"}
						...
						>
							...
							>
								...
								{editing?.kind === "product" ? "Save product" : "Create product"}
							</Button>
						</EditorCard>
						<EditorCard
							title={editing?.kind === "package" ? "Edit package" : "Create package"}
						...
						>
							...
							<label className="flex items-center gap-2 text-sm">
								<Switch
									...
									onCheckedChange={(active) => setPackageForm((value) => ({ ...value, active }))}
								/>
								Active
							</label>
							<Button
							...
							>
								...
								{editing?.kind === "package" ? "Save package" : "Create package"}
							</Button>
						</EditorCard>
						<EditorCard
							title={editing?.kind === "offering" ? "Edit offering" : "Create offering"}
						...
						>
							...
							<label className="flex items-center gap-2 text-sm">
								<Switch
									...
									onCheckedChange={(active) => setOfferingForm((value) => ({ ...value, active }))}
								/>
								Active
							</label>
							<Button
							...
							>
								...
								{editing?.kind === "offering" ? "Save offering" : "Create offering"}
							</Button>
						</EditorCard>
					</div>
					<CatalogTable
					...
					<CatalogTable
						title="Offerings"
						headers={["Name", "Identifier", "Placement", "Priority", "Packages", "State", ""]}
					...
					>
						{visibleOfferings.map((item) => (
							<TableRow key={item.id}>
								...
								<TableCell>{item.package_ids?.length || item.packages?.length || 0}</TableCell>
								<TableCell>
									<StatusBadge status={enabled(item.active) ? "active" : "archived"} />
								</TableCell>
								<TableCell>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Import Apple, Google, Stripe or manual products. A complete catalog deactivates
								products missing from the payload.
							</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
							...
							<DataTable
								headers={["Started", "Store", "Environment", "Status", "Imported", "Deactivated"]}
...
export function EntitlementsPage() {
	...
	const save = async () => {
		...
		try {
			...
			showSuccessNotification(editingId ? "Entitlement updated" : "Entitlement created");
			reset();
			await load();
		} catch (cause) {
			showErrorNotification(moduleErrorMessage(cause));
	...
	const visible = items.filter((item) => {
		...
		return (
			`${item.display_name} ${item.identifier}`.toLowerCase().includes(search.toLowerCase()) &&
		...
		>
			{!selectedProject ? (
				...
				<div className="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
					<Card>
						<CardHeader>
							<CardTitle>{editingId ? "Edit entitlement" : "Create entitlement"}</CardTitle>
							<CardDescription>
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>{visible.length} matching rights</CardDescription>
						</CardHeader>
						<CardContent className="space-y-4">
											...
											<TableCell>
												<RowActions
													...
													onArchive={
														enabled(item.active)
															? () =>
																	void archiveEntitlement(selectedProject.id, item.id)
																		.then(() => {
																			showSuccessNotification("Entitlement archived");
																			return load();
																		})
																		.catch((cause) =>
																			showErrorNotification(moduleErrorMessage(cause)),
