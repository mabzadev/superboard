...
export default function SupportConfigurationPage() {
	...
	const [settings, setSettings] = useState<MessagingProjectSettings>(defaultSettings);
	...
	const saveProject = async () => {
		...
		try {
			...
			if (Object.values(parsedFeatures).some((value) => typeof value !== "boolean")) {
				throw new Error("Every feature flag must be true or false");
			}
			await updateMessagingSettings(projectId, {
				...settings,
				auto_resolve_minutes:
					settings.auto_resolve_minutes === null ? null : Number(settings.auto_resolve_minutes),
	...
	const saveEntity = async () => {
		...
		try {
			...
			showErrorNotification(message(error, `Unable to save ${definition.label.toLowerCase()}`));
		} finally {
			setSaving(false);
		}
	};
	...
	const remove = async () => {
		...
		if (!window.confirm(`Delete this ${definition.label.toLowerCase()}? This action is audited.`))
			return;
		setSaving(true);
		try {
			...
			showErrorNotification(message(error, `Unable to delete ${definition.label.toLowerCase()}`));
		} finally {
			setSaving(false);
		}
	};
	...
	const rotateWebhookSecret = async () => {
		...
		try {
			...
			showErrorNotification(message(error, "Unable to rotate the webhook signing secret"));
		} finally {
			setSaving(false);
		}
	};
	...
	const revokeWebhookSecret = async () => {
		if (!projectId || !selectedId || !window.confirm("Revoke this webhook signing secret?")) return;
		setSaving(true);
		try {
			...
			showErrorNotification(message(error, "Unable to revoke the webhook signing secret"));
		} finally {
			setSaving(false);
		}
	};
		...
		<div className="min-h-full">
			<div className="space-y-6 p-6">
				<div className="flex flex-wrap items-start justify-between gap-3">
					<div>
						...
						<p className="text-sm text-muted-foreground">
							Manage the complete support configuration used by the Inbox and FlutterFlow Support
							library.
						</p>
					</div>
				...
				<Alert>
					...
					<AlertDescription>
						Support configuration is stored in the dedicated Support database. Agent identities
						reference the existing authentication gateway, and no setting can modify purchase
						entitlements.
					</AlertDescription>
				</Alert>
				...
				<Card className="space-y-5 p-5">
					<div>
						...
						<p className="text-sm text-muted-foreground">
							Locale, retention behavior, attachment policy, and feature switches.
						</p>
					</div>
					<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
						...
						<ProjectField
							...
							type="number"
							value={
								settings.auto_resolve_minutes == null ? "" : String(settings.auto_resolve_minutes)
							}
							onChange={(value) =>
								setSettings({
									...settings,
					...
					<div className="flex justify-end">
						<Button disabled={saving || loading} onClick={() => void saveProject()}>
				...
				<div className="grid gap-6 xl:grid-cols-[280px_360px_1fr]">
					<Card className="h-fit overflow-hidden">
						<div className="border-b p-4">
							...
							<p className="text-xs text-muted-foreground">Defined by the Support API catalog.</p>
						</div>
						<div className="max-h-[720px] overflow-auto p-2">
							{Object.entries(bootstrap?.catalog || {}).map(([type, item]) => {
								const count =
									bootstrap?.entities.filter((entity) => entity.entity_type === type).length || 0;
								return (
									<button
										key={type}
										type="button"
					...
					<Card className="h-fit overflow-hidden">
						<div className="flex items-start justify-between gap-3 border-b p-4">
							<div>
								<h2 className="font-semibold">{definition?.label || "Configuration"}</h2>
						...
						<div className="max-h-[660px] overflow-auto">
							...
							{!entities.length && (
								<div className="p-8 text-center text-sm text-muted-foreground">
									No {definition?.label.toLowerCase() || "configuration"} has been created.
								</div>
							)}
						</div>
					</Card>
							...
							<div className="space-y-3 rounded-md border p-4">
								<div>
									...
									<p className="text-xs text-muted-foreground">
										Secrets are encrypted by the Support Worker, never returned, and every rotation
										is audited. Current status:{" "}
