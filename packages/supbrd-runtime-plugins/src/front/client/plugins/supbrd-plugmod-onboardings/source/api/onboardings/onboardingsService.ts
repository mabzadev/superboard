...
export async function createOnboarding(id: string, payload: Record<string, unknown>) {
...
export async function deleteOnboarding(id: string, onboardingId: string) {
	return unwrap<{ deleted: boolean }>(await DELETE(path(id, `/${onboardingId}`)));
}
export async function getOnboardingVersions(id: string, onboardingId: string) {
	return unwrap<OnboardingVersion[]>(await GET(path(id, `/${onboardingId}/versions`)));
}
export async function createOnboardingVersion(
	...
	return unwrap<OnboardingVersion>(await POST(path(id, `/${onboardingId}/versions`), payload));
}
export async function publishOnboarding(id: string, onboardingId: string, versionId: string) {
	return unwrap<Onboarding>(
		await POST(path(id, `/${onboardingId}/publish`), { version_id: versionId }),
	);
}
export async function getOnboardingStatistics(id: string, filters: Record<string, string> = {}) {
...
export async function deleteOnboardingPlacement(id: string, placementId: string) {
	return unwrap<{ deleted: boolean }>(await DELETE(path(id, `/placements/${placementId}`)));
}
export async function getOnboardingTargetingRules(id: string) {
	return unwrap<OnboardingTargetingRule[]>(await GET(path(id, "/targeting-rules")));
}
export async function createOnboardingTargetingRule(
	id: string,
	payload: Omit<OnboardingTargetingRule, "id">,
) {
	return unwrap<OnboardingTargetingRule>(await POST(path(id, "/targeting-rules"), payload));
}
export async function deleteOnboardingTargetingRule(id: string, ruleId: string) {
	return unwrap<{ deleted: boolean }>(await DELETE(path(id, `/targeting-rules/${ruleId}`)));
}
...
export async function createOnboardingExperience(
	...
	return unwrap<OnboardingExperience>(await POST(path(id, "/experiences"), payload));
}
