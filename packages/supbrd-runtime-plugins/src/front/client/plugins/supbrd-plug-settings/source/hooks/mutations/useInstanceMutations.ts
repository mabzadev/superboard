...
} from "../../api/applications/configApplicationsService.js";
import {
...
} from "../../api/instances/instanceService.js";

export function useCreateInstanceMutation() {
	const queryClient = useQueryClient();

	return useMutation({
		mutationFn: ({ name, members }: { name: string; members: { email: string; role: string }[] }) =>
...
export function useEditInstanceMutation() {
	...
	return useMutation({
		mutationFn: ({ id, name }: { id: string; name: string }) => editInstanceAPICall(id, name),
		onSuccess: () => {
			queryClient.invalidateQueries({ queryKey: queryKeys.instances.all });
		},
	});
...
export function useDismissGetStartedMutation() {
	...
	return useMutation({
		mutationFn: ({ id, data }: { id: string; data?: DismissGetStartedPayload }) =>
...
export function useSetRevenueCollectionMutation() {
	...
	return useMutation({
		mutationFn: ({ id, data }: { id: string; data: RevenueCollectionPayload }) =>
...
export function useCompleteSetupStepMutation(instanceId: string | undefined) {
	...
	return useMutation({
		mutationFn: ({ category, stepIdentifier }: { category: string; stepIdentifier: string }) => {
			if (!instanceId) throw new Error("Instance ID is required");
		...
		onSuccess: (_, variables) => {
			if (instanceId) {
				queryClient.invalidateQueries({
					queryKey: queryKeys.instances.setupProgress(instanceId, variables.category),
				});
			}
		},
	});
...
export function useSetIosApiAccessKeyMutation(instanceId: string | undefined) {
	const queryClient = useQueryClient();
...
export function useSetAndroidPushConfigMutation(instanceId: string | undefined) {
	const queryClient = useQueryClient();
...
export function useSetAndroidWebhookKeyMutation(instanceId: string | undefined) {
	const queryClient = useQueryClient();
...
export function useSetWebConfigMutation(instanceId: string | undefined) {
	...
	return useMutation({
		mutationFn: ({ enabled, domains }: { enabled: boolean; domains: string[] }) => {
			if (!instanceId) throw new Error("Instance ID is required");
