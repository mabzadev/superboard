...
import { showSuccessNotification } from "../../../../../../../../../supbrd-front-ui/src/shared/lib/Notifications.js";
import {
...
export default function LibrariesPageContent() {
	...
	const load = useCallback(async () => {
		...
		try {
			...
			setError(cause instanceof Error ? cause.message : "Unable to load the library catalogue");
		} finally {
			setLoading(false);
		}
	}, []);
			...
			<div className="flex flex-wrap items-start justify-between gap-4">
				<div>
					...
					<p className="text-sm text-muted-foreground">
						Git-owned SDK versions, immutable release references and reusable FlutterFlow custom
						code.
					</p>
				</div>
			...
			{catalog && (
				<>
					<Alert>
						...
						<AlertDescription>
							Source versions are changed by pull request. Production applications use only
							immutable release references; this back-office never rewrites package source or
							creates an unreviewed tag.
						</AlertDescription>
					</Alert>
							...
							return (
								<Card key={library.id} data-testid={`library-${library.id}`}>
									...
									<CardContent className="space-y-4">
										...
										{library.candidatePackageName && library.candidateInstall && (
											<div className="space-y-2 rounded-md border border-blue-500/30 bg-blue-500/5 p-3 text-xs">
												...
												<p className="text-muted-foreground">
													Planned for source version {library.sourceVersion}; the immutable release
													below remains the production baseline until promotion.
												</p>
											</div>
										)}
										{library.notes && (
										...
										{library.distribution && (
											<div className="space-y-2 rounded-md border border-amber-500/30 bg-amber-500/5 p-3 text-xs">
												<div className="flex flex-wrap gap-2">
													...
													<Badge variant="outline">Anonymous install unavailable</Badge>
												</div>
												<p className="text-muted-foreground">
													The package record is public, but the registry rejects unauthenticated
													downloads. Configure{" "}
													<code>
														{library.distribution.authentication.tokenEnvironmentVariable}
													</code>
													{library.distribution.authentication.usernameEnvironmentVariable && (
														<>
															{" "}
															and{" "}
															<code>
											...
											<details>
												...
												{library.distribution && (
													<p className="mt-2 text-xs text-muted-foreground">
														This command alone is insufficient; configure the authenticated registry
														shown above first.
													</p>
												)}
												<div className="mt-2 flex items-start gap-2">
													...
													<Button
														...
														onClick={() =>
															void copy(
																install,
																library.distribution ? "Dependency command" : "Installation",
															)
														}
													>
														<Copy className="size-4" />
					...
					<Card>
						<CardHeader>
							...
							<CardDescription>
								The canonical reusable surface belongs to the SuperBoard monorepo; application
								projects keep only thin UI adapters. {catalog.customCode.widgets.length} widgets ·{" "}
						...
						<CardContent className="space-y-5">
							...
							<div className="grid gap-4 lg:grid-cols-2">
								{Object.entries(catalog.customCode.actions).map(([group, actions]) => (
							...
							<div>
								...
								<div className="grid gap-4 lg:grid-cols-2">
									{Object.entries(catalog.customCode.streams).map(([group, streams]) => (
										<SurfaceGroup key={group} group={group} values={streams} />
									))}
								</div>
							</div>
							<div>
								...
								<div className="grid gap-4 lg:grid-cols-2">
									{Object.entries(catalog.customCode.sourceFiles).map(([group, paths]) => (
							...
							<div>
								<h2 className="mb-2 text-sm font-semibold">Reference-only UI adapters</h2>
								<p className="mb-3 text-xs text-muted-foreground">
									These adapters belong to the acceptance application. They may compose public
									library calls but must not copy an HTTP, authentication or business protocol.
								</p>
								<div className="grid gap-4 lg:grid-cols-2">
									{Object.entries(catalog.customCode.referenceAdapters).map(([group, adapters]) => (
										<SurfaceGroup key={group} group={group} values={adapters} />
									))}
								</div>
							</div>
							<a
...
function FlutterFlowLibraryCard({ catalog }: { catalog: PlatformLibraryCatalog }) {
	const library = catalog.flutterFlowLibrary;
	const dependencies = library.dependencies.map((dependency) => {
		const published = catalog.libraries.find(({ id }) => id === dependency.catalogId);
		return {
			...dependency,
			ready:
			...
			<CardHeader>
				<div className="flex flex-wrap items-start justify-between gap-3">
					<div>
						...
						<CardDescription>
							Git-managed reusable adapters deployed to FlutterFlow only after every SDK dependency
							has an immutable release tag.
						</CardDescription>
					</div>
	...
	return (
		<div className="rounded-md border p-4">
			<div className="mb-2 flex items-center gap-2">
				<h3 className="text-sm font-semibold capitalize">{group.replace(/([A-Z])/g, " $1")}</h3>
				{group === "compatibilityAliases" && <Badge variant="outline">Compatibility only</Badge>}
			</div>
...
function Version({ label, value, icon }: { label: string; value: string; icon: React.ReactNode }) {
	return (
