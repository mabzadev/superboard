...
export type PaywallBlock = {
	id: string;
	type: "heading" | "text" | "image" | "product" | "button" | "close" | "legal" | "spacer";
...
export async function updatePaywall(
	...
	return unwrap<Paywall>(await PUT(path(projectRef, `/paywalls/${paywallId}`), payload));
}
...
export async function getPaywallVersions(projectRef: string, paywallId: string) {
	return unwrap<PaywallVersion[]>(await GET(path(projectRef, `/paywalls/${paywallId}/versions`)));
}
...
export async function publishPaywallVersion(
	...
	return unwrap<PaywallVersion>(
		await POST(path(projectRef, `/paywalls/${paywallId}/versions/${versionId}/publish`), {}),
	);
}
...
export async function archivePaywallVersion(
	...
	return unwrap<{ id: string; status: "archived" }>(
		await POST(path(projectRef, `/paywalls/${paywallId}/versions/${versionId}/archive`), {}),
	);
}
...
export async function deletePaywallPlacement(projectRef: string, id: string) {
	return unwrap<{ id: string; active: false }>(await DELETE(path(projectRef, `/placements/${id}`)));
}

export async function getPaywallExperiences(projectRef: string) {
	return unwrap<PaywallExperience[]>(await GET(path(projectRef, "/experiences")));
}
...
export async function createPaywallExperience(
	...
	return unwrap<PaywallExperience>(await POST(path(projectRef, "/experiences"), payload));
}
...
export async function archivePaywallExperience(projectRef: string, experienceId: string) {
	return unwrap<{ id: string; status: "archived" }>(
