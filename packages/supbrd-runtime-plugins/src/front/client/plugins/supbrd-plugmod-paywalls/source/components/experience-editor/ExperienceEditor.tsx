...
import Image from "@superboard/front-ui/next-image";
import {
	CheckCircle2,
	Copy,
	GripVertical,
...
import { createBlock, createScreen, uniqueId, validateExperienceDocument } from "./model.js";
import type {
	EditorBlockType,
	ExperienceBlock,
	ExperienceDocument,
...
export function ExperienceEditor({
	...
	const issues = useMemo(() => validateExperienceDocument(history.value), [history.value]);
	const screen =
		history.value.screens.find(({ id }) => id === selectedScreenId) ?? history.value.screens[0];
	...
	const updateScreen = (updater: (value: ExperienceScreen) => ExperienceScreen) => {
		if (!screen) return;
		history.set((document) => ({
			...document,
			screens: document.screens.map((item) => (item.id === screen.id ? updater(item) : item)),
		}));
	};

	const updateBlock = (updater: (value: ExperienceBlock) => ExperienceBlock) => {
		if (!selectedBlock) return;
		updateScreen((value) => ({
			...value,
			blocks: value.blocks.map((block) => (block.id === selectedBlock.id ? updater(block) : block)),
		}));
	};
	...
	const handleDragEnd = ({ active, over }: DragEndEvent) => {
		...
		if (activeId.startsWith("screen:") && overId.startsWith("screen:")) {
			...
			history.set((document) => {
				const from = document.screens.findIndex(({ id }) => id === activeScreen);
				const to = document.screens.findIndex(({ id }) => id === overScreen);
				return { ...document, screens: arrayMove(document.screens, from, to) };
			});
			return;
		}
		if (activeId.startsWith("block:") && overId.startsWith("block:") && screen) {
					...
					<div className="rounded-lg border p-3">
						<div className="mb-2 flex items-center justify-between">
							...
							<div className="flex gap-1">
								...
								<Button size="sm" variant="ghost" onClick={duplicateScreen} disabled={!screen}>
									<Copy />
									Duplicate
								</Button>
								<Button
					...
					<div className="grid gap-4 2xl:grid-cols-[180px_minmax(320px,1fr)_300px]">
						<aside className="rounded-lg border p-3">
							...
							<div className="grid grid-cols-2 gap-2 2xl:grid-cols-1">
								{BLOCKS.filter(({ type }) => kind === "onboarding" || type !== "benefits").map(
									({ type, label }) => (
										<Button
											key={type}
											size="sm"
								...
								<>
									{kind === "onboarding" && (
										<div className="mb-3 space-y-2">
											...
											<ScreenConditionsEditor screen={screen} onChange={updateScreen} />
										</div>
									)}
									<SortableContext
										items={screen.blocks.map(({ id }) => `block:${screen.id}:${id}`)}
											...
											{screen.blocks.map((block) => (
												<SortableBlock
													...
													onDelete={() => {
														updateScreen((value) => ({
															...value,
															blocks: value.blocks.filter(({ id }) => id !== block.id),
														}));
														setSelectedBlockId(null);
													}}
												/>
						...
						<aside className="space-y-4 rounded-lg border p-3">
							{selectedBlock ? (
								...
								<ThemeInspector document={history.value} onChange={history.set} />
							)}
						</aside>
					</div>
				)}
	...
	return (
		...
		>
			...
			<span>
				...
				<span className="text-xs text-muted-foreground">{screen.blocks.length} blocks</span>
			</span>
		</button>
	);
}
...
function SortableBlock({
	...
	return (
		...
		>
			...
			<Button size="icon" variant="ghost" aria-label={`Delete ${block.type}`} onClick={onDelete}>
				<Trash2 />
			</Button>
		</div>
	);
...
function BlockInspector({
	...
	const action = typeof block.props.action === "string" ? block.props.action : "next";
	return (
		<>
			<div>
				...
				<>
					<label className="block space-y-1 text-xs">
						...
						<Input
							...
							onChange={(event) => update("offering_identifier", event.target.value)}
							placeholder="default"
						/>
					</label>
					<label className="block space-y-1 text-xs">
			...
			{block.type === "close" && (
				<label className="block space-y-1 text-xs">
					...
					<Input
						...
						onChange={(event) => update("accessibility_label", event.target.value)}
					/>
				</label>
			)}
			{block.type === "benefits" && (
...
function ScreenConditionsEditor({
	...
	return (
		<details className="rounded-md border bg-background p-2">
			<summary className="cursor-pointer text-xs font-medium">Screen display conditions</summary>
			<div className="mt-2 grid gap-2 md:grid-cols-3">
				<label className="space-y-1 text-xs">
					Platform
					<select
						className="h-9 w-full rounded-md border bg-background px-2 text-sm"
						value={typeof conditions.platform === "string" ? conditions.platform : ""}
				...
				<label className="space-y-1 text-xs">
					Locale
					<Input
						value={typeof conditions.locale === "string" ? conditions.locale : ""}
				...
				<label className="space-y-1 text-xs">
					Minimum app version
					<Input
						value={typeof conditions.min_app_version === "string" ? conditions.min_app_version : ""}
...
function ThemeInspector({
	...
	onChange: (
		value: ExperienceDocument | ((value: ExperienceDocument) => ExperienceDocument),
	) => void;
}) {
	const update = (key: keyof ExperienceDocument["theme"], value: string | number) =>
		onChange((current) => ({
			...current,
	...
	return (
		<>
			...
			{(["accent_color", "background_color", "text_color"] as const).map((key) => (
				<label key={key} className="flex items-center justify-between gap-2 text-xs capitalize">
					{key.replaceAll("_", " ")}
					<input
						type="color"
			...
			<label className="block space-y-1 text-xs">
				...
				<Input
					...
					onChange={(event) => update("corner_radius", Number(event.target.value))}
				/>
			</label>
		</>
	);
...
function PreviewBlock({
	...
	if (block.type === "heading")
		return <h2 className="text-center text-2xl font-bold">{String(block.props.text ?? "")}</h2>;
	if (["text", "legal"].includes(block.type))
		return (
			<p className={cn("text-center", block.type === "legal" && "text-xs opacity-70")}>
	...
	if (block.type === "benefits")
		return (
			<ul className="space-y-2">
				{(Array.isArray(block.props.items) ? block.props.items : []).map((item) => (
	...
	if (block.type === "spacer") return <div style={{ height: Number(block.props.height ?? 24) }} />;
	return null;
}

function blockSummary(block: ExperienceBlock) {
	if (typeof block.props.text === "string") return block.props.text;
	if (block.type === "image") return String(block.props.url || "No image selected");
	if (block.type === "benefits")
	...
	return block.type === "product" ? "Active offering packages" : "Configure this block";
}
