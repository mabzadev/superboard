...
import { formatPlatformName } from "../../../../../../../../../supbrd-front-ui/src/shared/lib/utils.js";
...
export const getMessagingTableColumns = (
	...
	},
	{
		...
		cell: ({ row }) => {
			return (
				<div>
					{row.original.target.new_users && <Badge variant={"outline"}>New users</Badge>}
					{row.original.target.existing_users && <Badge variant={"outline"}>Existing users</Badge>}
				</div>
			);
		},
	},

	{
		...
		header: () => renderSortableHeader("Auto display", "auto_display", setSort, sort),
		cell: ({ row }) => {
			return (
				<div>
...

	{
		...
		header: () => renderSortableHeader("Push notification", "push_notification", setSort, sort),
		cell: ({ row }) => {
			return (
				<div>
