...
import {
	RadioGroup,
	RadioGroupItem,
} from "../../../../../../../../../supbrd-front-ui/src/shared/components/ui/radio-group.js";
import {
	Select,
	SelectContent,
	SelectItem,
...
import type { TextCraftProps } from "../../../../../../../../../supbrd-front-ui/src/shared/types/index.js";
import { ComponentToolbar } from "./ComponentToolbar.js";
	...
	return (
		<div
			...
			className={clsx(
				...
				enabled && !selected && "hover:outline-blue-500 hover:outline-1 hover:outline-dashed",
			)}
		>
			...
			<p
				...
				style={{
					...
					textAlign: (TEXT_ALIGN_MAP[textAlign] ?? "left") as React.CSSProperties["textAlign"],
		...
		<div className="flex flex-col gap-3 px-3 py-2">
			<div className="flex flex-col gap-1.5">
				...
				<Select
					...
					onValueChange={(v) => setProp((p: Record<string, string>) => (p.textSize = v))}
				>
				...
				<RadioGroup className="flex gap-3">
					{[
						...
						<div
							...
							onClick={() => setProp((p: Record<string, string>) => (p.textAlign = item.value))}
						>
							<RadioGroupItem
								value={item.value}
								id={item.id}
				...
				<RadioGroup className="grid grid-cols-2 gap-2">
					{[
						...
						<div
							key={item.id}
							className="flex items-center gap-1.5"
							onClick={() => setProp((p: Record<string, string>) => (p.fontWeight = item.value))}
						>
							<RadioGroupItem
								value={item.value}
								id={item.id}
			...
			<div className="flex items-center justify-between">
				...
				<div className="flex items-center gap-1.5">
					<Input
						...
						onChange={(e) => setProp((p: Record<string, string>) => (p.textColor = e.target.value))}
					...
					<div className="w-7 h-7 rounded-md border border-input overflow-hidden relative cursor-pointer shrink-0">
						<input
							value={props.textColor}
							onChange={(e) =>
								setProp((p: Record<string, string>) => (p.textColor = e.target.value))
							}
							type="color"
							className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
						/>
						<div className="w-full h-full" style={{ backgroundColor: props.textColor }} />
					</div>
				</div>
			</div>
		</div>
