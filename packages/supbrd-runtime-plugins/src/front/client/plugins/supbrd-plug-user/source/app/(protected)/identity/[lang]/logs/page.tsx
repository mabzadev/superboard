...
import Pagination from "../../../../../identity/components/Pagination.js";
import {
	AlertDialog,
	AlertDialogTitle,
...
import ViewLink from "../../../../../identity/components/ViewLink.js";
...
import { accessTool, routeTool, variableTool } from "../../../../../identity/tools/index.js";
	...
	return (
		<AlertDialog open={show}>
			<AlertDialogContent>
				<AlertDialogHeader>
					<AlertDialogTitle>
						{t("logs.cleanAlertTitle", { days: variableTool.LogCleanUpDays })}
					</AlertDialogTitle>
				</AlertDialogHeader>
				<AlertDialogDescription>{t("logs.cleanAlertDescription")}</AlertDialogDescription>
				<AlertDialogFooter>
...
const Page = () => {
	...
	const { data: emailData, isLoading: isEmailLoading } = useGetApiV1LogsEmailQuery({
	...
	const { data: smsData, isLoading: isSmsLoading } = useGetApiV1LogsSmsQuery({
	...
	const { data: signInData, isLoading: isSignInLoading } = useGetApiV1LogsSignInQuery({
	...
	const showCleanBtn = accessTool.isAllowedAccess(accessTool.Access.WriteLog, userInfo?.roles);

	const emailLogTotalPages = useMemo(() => Math.ceil(emailLogCount / PageSize), [emailLogCount]);

	const smsLogTotalPages = useMemo(() => Math.ceil(smsLogCount / PageSize), [smsLogCount]);

	const signInLogTotalPages = useMemo(() => Math.ceil(signInLogCount / PageSize), [signInLogCount]);
	...
	return (
		<section className="flex flex-col">
				...
				<section className="flex flex-col mt-8">
					...
					<Table className="break-all">
						...
						<TableBody className="divide-y">
							{emailLogs.map((log) => (
								<TableRow data-testid="emailLogRow" key={log.id}>
									<TableCell>{log.receiver}</TableCell>
									<TableCell>
										<ConfigBooleanValue config={log.success} />
									</TableCell>
									<TableCell>{log.createdAt}</TableCell>
									<TableCell>
										<ViewLink href={`${routeTool.Internal.Logs}/email/${log.id}`} />
									</TableCell>
								</TableRow>
							))}
						</TableBody>
			...
			{configs.ENABLE_SMS_LOG && (
				<section className="flex flex-col mt-8">
					<header className="flex gap-8 items-center mb-6">
						<PageTitle title={t("logs.smsLogs")} />
						{showCleanBtn && (
							<Button
					...
					<Table className="break-all">
						...
						<TableBody className="divide-y">
							{smsLogs.map((log) => (
								<TableRow data-testid="smsLogRow" key={log.id}>
									<TableCell>{log.receiver}</TableCell>
									<TableCell>
										<ConfigBooleanValue config={log.success} />
									</TableCell>
									<TableCell>{log.createdAt}</TableCell>
									<TableCell>
										<ViewLink href={`${routeTool.Internal.Logs}/sms/${log.id}`} />
									</TableCell>
								</TableRow>
							))}
						</TableBody>
			...
			{configs.ENABLE_SIGN_IN_LOG && (
				<section className="flex flex-col mt-8">
					<header className="flex gap-8 items-center mb-6">
						<PageTitle title={t("logs.signInLogs")} />
						{showCleanBtn && (
							<Button
								variant="outline"
								size="sm"
					...
					<Table className="break-all">
						...
						<TableBody className="divide-y">
							{signInLogs.map((log) => (
								<TableRow data-testid="signInRow" key={log.id}>
									<TableCell>{log.userId}</TableCell>
									<TableCell>{log.createdAt}</TableCell>
									<TableCell>
										<ViewLink href={`${routeTool.Internal.Logs}/sign-in/${log.id}`} />
									</TableCell>
								</TableRow>
							))}
						</TableBody>
