...
import {
	...
	DialogHeader,
	DialogTitle,
} from "../../../../../../../../../supbrd-front-ui/src/shared/components/ui/dialog.js";
...
import { formatSlashDate } from "../../../../../../../../../supbrd-front-ui/src/shared/lib/dateUtils.js";
import {
	showErrorNotification,
	showSuccessNotification,
} from "../../../../../../../../../supbrd-front-ui/src/shared/lib/Notifications.js";
import { parseSecondsInDaysHoursMinutesSeconds } from "../../../../../../../../../supbrd-front-ui/src/shared/lib/utils.js";
import type { SortType } from "../../../../../../../../../supbrd-front-ui/src/shared/types/index.js";
import { formatCurrencyFromCents } from "../../../../../../../../../supbrd-front-ui/src/shared/utils/formatCurrency.js";
import { numberFormatter } from "../../../../../../../../../supbrd-front-ui/src/shared/utils/numberFormatter.js";
import {
	createCustomer,
	deleteCustomer,
	getCustomers,
...
import { checkSortDirection } from "../dynamic_links/links/LinksTableColumns.js";
...
export function referralToAudienceRow(referral: AppReferral): AudienceRow {
	...
	return {
		...
		sdkIdentifier: referral.customer_external_id || referral.customer_id || referral.code,
		label: referral.code,
...
function columns(
	...
	result.push(
		...
		})),
		{
			...
			cell: ({ row }) => parseSecondsInDaysHoursMinutesSeconds(row.original.timeSpent) || "—",
		},
		{
			accessorKey: "revenue",
			header: () => header("Revenue", "revenue", sort, setSort),
			cell: ({ row }) =>
				row.original.revenue ? formatCurrencyFromCents(row.original.revenue) : "—",
		},
		{
			accessorKey: referral ? "last_access" : "date",
			header: () => header(referral ? "Last access" : "Date", "date", sort, setSort),
...
export function CustomersAnalyticsPage() {
	...
	return (
		<AudiencePage
			...
			onRow={(row) => {
				setSelected(customers.find((customer) => customer.id === row.id) ?? null);
				setDialogOpen(true);
			}}
			dialog={
				<CustomerDialog
...
export function ReferralsAnalyticsPage() {
	...
	const load = useCallback(async () => {
		...
		try {
			setReferrals(
				await getReferrals(selectedProject.id, {
					from: table.dateRange?.from?.toISOString().slice(0, 10),
					to: table.dateRange?.to?.toISOString().slice(0, 10),
					timezone: Intl.DateTimeFormat().resolvedOptions().timeZone,
				}),
	...
	const rows = useMemo(() => {
		...
		return referrals
			...
			.filter(
				(row) =>
					!search ||
					`${row.label} ${row.sdkIdentifier} ${row.detail}`.toLocaleLowerCase().includes(search),
			)
			.sort((left, right) => rowSort(left, right, table.sort));
...
function AudiencePage({
	...
	const tableColumns = useMemo(
		() => columns(table.sort, table.setSort, referral),
		[referral, table.setSort, table.sort],
	);
	useEffect(() => {
		if (table.page > pageCount) table.setPage(pageCount);
	}, [pageCount, table]);
	return (
		<ModulePage title={title} description={description} error={error}>
			{!selected ? (
				<EmptyProject />
			) : (
				<div className="space-y-4">
					<div className="flex flex-wrap items-center justify-between gap-2">
						<div className="flex flex-1 flex-wrap gap-2">
							<Input
								...
								onChange={(event) => table.setSearchTerm(event.currentTarget.value)}
							/>
							<DateRangePicker date={table.dateRange} setDate={table.setDateRange} />
						</div>
					...
					<DataTable
						...
						onRowClick={onRow}
						getRowId={(row) => row.id}
						getRowAriaLabel={(row) => `View ${row.label}`}
						loading={loading}
						hasFilters={Boolean(table.searchTerm)}
						ariaLabel={title}
						stickyHeader
						emptyState={
							<div className="flex flex-col items-center gap-3 bg-sidebar px-4 py-14 text-center">
								<h3 className="font-semibold">No {title.toLocaleLowerCase()} yet</h3>
								<p className="max-w-md text-sm text-muted-foreground">
									New records will appear here with their engagement and attribution metrics.
								</p>
								{onAction && actionLabel ? (
									<Button onClick={onAction}>
										<Plus className="size-4" />
...
function CustomerDialog({
	...
	useEffect(() => {
		setExternalId(customer?.external_id ?? "");
	...
	const save = async () => {
		if (!selectedProject || !externalId.trim()) return;
		setSaving(true);
		try {
			await createCustomer(selectedProject.id, {
			...
			showSuccessNotification(customer ? "Customer updated" : "Customer created");
			onOpenChange(false);
			await onSaved();
		} catch (cause) {
	...
	const remove = async () => {
		...
		setSaving(true);
		try {
			await deleteCustomer(selectedProject.id, customer.id);
	...
	return (
		<Dialog open={open} onOpenChange={onOpenChange}>
			<DialogContent>
				<DialogHeader>
					<DialogTitle>{customer ? "Customer details" : "Add customer"}</DialogTitle>
					<DialogDescription>
						The SDK identifier is the stable acquisition identity for this project.
					</DialogDescription>
				</DialogHeader>
				<div className="grid gap-4 sm:grid-cols-2">
					<Field label="SDK identifier" value={externalId} onChange={setExternalId} />
					...
					<Field label="Country code" value={country} onChange={setCountry} />
				</div>
				<DialogFooter className="sm:justify-between">
					...
					<Button disabled={saving || !externalId.trim()} onClick={() => void save()}>
