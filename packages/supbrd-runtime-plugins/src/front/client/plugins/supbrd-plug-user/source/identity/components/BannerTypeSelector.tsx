...
export const BannerTypeSelector = ({
	...
	return (
		<Select value={value} onValueChange={(val) => onChange(val)} disabled={disabled}>
			...
			<SelectContent>
				<SelectGroup>
					<SelectItem data-testid="bannerTypeSelect-errorOption" value={typeTool.BannerType.ERROR}>
						{t("apps.error")}
					</SelectItem>
					<SelectItem data-testid="bannerTypeSelect-infoOption" value={typeTool.BannerType.INFO}>
						{t("apps.info")}
					</SelectItem>
					<SelectItem
