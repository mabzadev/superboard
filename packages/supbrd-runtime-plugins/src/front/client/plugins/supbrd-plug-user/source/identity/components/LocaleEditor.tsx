...
const LocaleEditor = ({
	...
	const handleSetLocale = (targetLocale: string, val: string) => {
	...
	return (
		<section className="flex flex-col gap-4">
			...
			{supportedLocales.map((locale) => (
				<section key={locale} className="flex items-center gap-4">
					<p>{locale.toUpperCase()}</p>
					<Input
						data-testid="localeInput"
						className="w-full"
						disabled={disabled}
						onChange={(e) => handleSetLocale(locale, e.target.value)}
