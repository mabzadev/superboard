		...
		>
			{!selectedProject ? (
				...
				<Alert>
					...
					<AlertDescription>
						Application identity and purchase state contains personal data and is available only to
				...
				<div className="space-y-6">
					<Card>
						<CardHeader>
							...
							<CardDescription>
								Data comes from the Identity Worker and D1 database configured for this deployment
								target. Passwords, tokens and provider subject identifiers are never returned.
							</CardDescription>
						</CardHeader>
					...
					<div className="grid gap-6 xl:grid-cols-[minmax(0,1.05fr)_minmax(420px,0.95fr)]">
						<Card>
							<CardHeader>
								...
								<CardDescription>
									{page.meta.total.toLocaleString()} active application accounts, including users
									without a purchase.
								</CardDescription>
							</CardHeader>
						...
						<div className="grid content-start gap-4">
							{detail && (
								<ApplicationUserActions
									key={`${selectedProject.id}:${detail.id}`}
...
function ApplicationUserDetailPanel({
	...
	return (
		<div className="space-y-4">
			<Card>
				<CardHeader>
					<CardTitle>{detail.name || detail.email || "Anonymous user"}</CardTitle>
				...
				<CardContent className="space-y-4">
					<div className="grid gap-3 sm:grid-cols-2">
						...
						/>
						<Value label="Active sessions" value={String(detail.sessions.active)} />
						<Value
					...
					<p className="text-xs text-muted-foreground">
						Sessions: {detail.sessions.total} total · {detail.sessions.active} active ·{" "}
						{detail.sessions.revoked} revoked · {detail.sessions.expired} expired
					</p>
				</CardContent>
			</Card>
...
function CommercePanel({ state }: { state: CommerceState }) {
		...
		return (
			<Card>
				<CardHeader>
					...
					<CardDescription>No financial customer exists for this user.</CardDescription>
				</CardHeader>
				<CardContent className="text-sm text-muted-foreground">
					This is a valid free account. It remains visible even without a subscription, entitlement
					or paywall event.
				</CardContent>
			</Card>
		);
	...
	if (state.status === "unavailable") {
		return (
			<Alert>
				...
				<AlertDescription>
					Identity remains authoritative and available. Billing may be disabled for this target or
					temporarily unavailable: {state.message}
				</AlertDescription>
			</Alert>
		);
	}
	...
	return (
		<Card>
			...
			<CardContent className="space-y-4">
				<div className="grid gap-3 sm:grid-cols-2">
					<Value label="Active subscriptions" value={String(info.active_subscriptions.length)} />
					<Value label="Paywall events" value={String(state.detail.paywall_events.length)} />
				</div>
				<div>
					...
					<div className="flex flex-wrap gap-2">
						...
						{entitlements.length === 0 && (
							<span className="text-sm text-muted-foreground">No entitlement.</span>
						)}
					</div>
				</div>
				<div>
					<h3 className="mb-2 text-sm font-semibold">Recent paywall activity</h3>
					<div className="space-y-2">
						{state.detail.paywall_events.slice(0, 10).map((event, index) => (
							<div key={String(event.id || index)} className="rounded-md border p-3 text-sm">
						...
						{state.detail.paywall_events.length === 0 && (
							<span className="text-sm text-muted-foreground">No paywall activity.</span>
						)}
					</div>
				</div>
			</CardContent>
