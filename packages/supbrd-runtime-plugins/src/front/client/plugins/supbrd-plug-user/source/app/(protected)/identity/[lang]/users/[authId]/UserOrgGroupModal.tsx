...
const UserOrgGroupModal = ({
	...
	const { data: userData, refetch: refetchUser } = useGetApiV1UsersByAuthIdQuery({
			...
			<AlertDialogContent>
				...
				<section className="flex max-md:flex-col gap-6 max-md:gap-2 flex-wrap">
					{orgGroups.map((orgGroup) => (
						<div key={orgGroup.id} className="flex items-center gap-2">
							<Checkbox
								...
								disabled={isLoading}
							/>
							<Label htmlFor={`orgGroup-${orgGroup.id}`} className="flex">
								{orgGroup.name}
							</Label>
						</div>
					))}
