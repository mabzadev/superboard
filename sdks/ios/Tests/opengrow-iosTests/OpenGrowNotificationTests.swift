...
final class OpenGrowNotificationTests: XCTestCase {

    // MARK: - Helpers

    /// Uses the same decoder as production code — single source of truth.
    private var decoder: JSONDecoder {
        OpenGrowNotification.backendDecoder()
    }

    // MARK: - OpenGrowNotification decoding
    ...
    func testDecodeNotificationWithAllFields() throws {
        ...
        let notification = try decoder.decode(OpenGrowNotification.self, from: json)
    ...
    func testDecodeFailsOnMissingRequiredField() {
        ...
        XCTAssertThrowsError(try decoder.decode(OpenGrowNotification.self, from: json))
    }
    ...
    // MARK: - OpenGrowNotificationsResponse decoding
    ...
    func testDecodeNotificationsResponse() throws {
        ...
        let response = try decoder.decode(OpenGrowNotificationsResponse.self, from: json)
    ...
    func testEncodeThenDecodePreservesAllFields() throws {
        ...
        let original = try decoder.decode(OpenGrowNotification.self, from: json)
        let encoded = try OpenGrowNotification.backendEncoder().encode(original)
        let decoded = try decoder.decode(OpenGrowNotification.self, from: encoded)
