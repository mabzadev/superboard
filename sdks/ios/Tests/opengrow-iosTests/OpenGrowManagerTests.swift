...
private class SpyDelegate: OpenGrowDelegate {
...
private class TestableOpenGrowManager: OpenGrowManager {
    override func hasURISchemesConfigured() -> Bool {
        return true
    }
}

final class OpenGrowManagerTests: XCTestCase {
    ...
    /// Authenticates a TestableOpenGrowManager and waits for completion.
    /// Returns the manager ready for further operations.
    @discardableResult
    private func authenticateAndWait(_ manager: TestableOpenGrowManager,
                                     file: StaticString = #filePath,
                                     line: UInt = #line) -> TestableOpenGrowManager {
    ...
    func testAuthenticatedManagerDrainsQueuedGenerateLinkAction() {
        ...
        MockURLProtocol.requestHandlers["/api/v1/sdk/create_link"] = { request in
            let response = HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!
            let data = try JSONSerialization.data(withJSONObject: ["link": "https://opengrow.link/queued"])
            return (response, data)
        }
        ...
        manager.generateLink(title: "Queued", subtitle: nil, imageURL: nil,
                             data: nil, tags: nil, customRedirects: nil,
                             showPreviewiOS: nil, showPreviewAndroid: nil) { url in
            XCTAssertEqual(url?.absoluteString, "https://opengrow.link/queued",
    ...
    func testQueuedActionsFireAfterRetryingAuth() {
        ...
        manager.generateLink(title: "Retry", subtitle: nil, imageURL: nil,
                             data: nil, tags: nil, customRedirects: nil,
                             showPreviewiOS: nil, showPreviewAndroid: nil) { url in
            XCTAssertEqual(url?.absoluteString, "https://opengrow.link/retry")
            linkExp.fulfill()
        }
        ...
        MockURLProtocol.requestHandlers["/api/v1/sdk/create_link"] = { request in
            let response = HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!
            let data = try JSONSerialization.data(withJSONObject: ["link": "https://opengrow.link/retry"])
            return (response, data)
        }
    ...
    func testFailedAuthClearsActionQueue() {
        ...
        MockURLProtocol.requestHandlers["/api/v1/sdk/create_link"] = { request in
            ...
            let response = HTTPURLResponse(url: request.url!, statusCode: 200, httpVersion: nil, headerFields: nil)!
            let data = try JSONSerialization.data(withJSONObject: ["link": "https://opengrow.link/stale"])
            return (response, data)
        }
