...
public typealias OpenGrowURLClosure = (_ url: URL?) -> Void

/// A typealias for a closure returning a dictionary.
public typealias OpenGrowPayloadClosure = (_ dictionary: [String: Any]?) -> Void

public typealias OpenGrowDeviceDataClosureClosure = (_ dictionary: [String: Any]?, _ link: String?, _ tracking: [String: Any]?) -> Void

/// A typealias for a closure returning a array of dictionaries.
public typealias OpenGrowPayloadsClosure = (_ array: [[String: Any]]?) -> Void

/// A typealias for a closure returning a string.
typealias OpenGrowAuthenticationClosure = (_ success: Bool, _ linksquaredID: String?, _ URIScheme: String?, _ identifier: String?, _ attributes: [String: Any]?) -> Void

/// A typealias for a closure that returns an array of notifications.
typealias OpenGrowNotificationsClosure = (_ notifications: [OpenGrowNotification]?) -> Void

/// A typealias for a closure returning an int.
public typealias OpenGrowIntClosure = (_ value: Int?) -> Void

/// A typealias for a clsure returning a dictionary
public typealias OpenGrowLinkClosure = (_ dictionary: [String: Any]?) -> Void

/// Protocol defining the API service methods used by handlers.
protocol APIServiceProtocol: AnyObject {
    func addEvent(event: Event, completion: @escaping OpenGrowBoolCompletion)
    func addPaymentEvent(transactionData: TransactionData, completion: @escaping OpenGrowBoolCompletion)
}
...
class APIService: BaseService, APIServiceProtocol {
    ...
    func payloadFor(appDetails: AppDetails, url: String, completion: @escaping OpenGrowDeviceDataClosureClosure) {
    ...
    ///   - appDetails: Details of the app.
    ///   - completion: A closure returning the payload as a dictionary.
    func payloadFor(appDetails: AppDetails, completion: @escaping OpenGrowDeviceDataClosureClosure) {
        guard var request = urlRequestWithAuthHeaders(
    ...
    func authenticate(appDetails: AppDetails, completion: @escaping OpenGrowAuthenticationClosure) {
        guard var request = urlRequestWithAuthHeaders(
    ...
    func updateAttributes(completion: @escaping OpenGrowBoolCompletion) {
    ...
    func notifications(page: Int, completion: @escaping OpenGrowNotificationsClosure) {
        guard var request = urlRequestWithAuthHeaders(
        ...
        makeRequest(URLRequest: request) { success, json in
            ...
            do {
                let response = try OpenGrowNotification.backendDecoder()
                    .decode(OpenGrowNotificationsResponse.self, from: jsonData)
    ...
    func numberOfUnreadNotifications(completion: @escaping OpenGrowIntClosure) {
        guard var request = urlRequestWithAuthHeaders(
    ...
    func notificationsToDisplayAutomatically(completion: @escaping OpenGrowNotificationsClosure) {
        guard var request = urlRequestWithAuthHeaders(
    ...
    func linkDetails(path: String, completion: @escaping OpenGrowLinkClosure) {
        guard var request = urlRequestWithAuthHeaders(
