...
typealias OpenGrowEventsClosure = (_ events: [Event]?) -> Void

/// Protocol defining the events storage interface.
protocol EventsStorageProtocol {
    func addEvent(event: Event, completion: @escaping OpenGrowEmptyClosure)
    func addOrReplaceEvents(events: [Event], completion: @escaping OpenGrowEmptyClosure)
    func removeEvent(event: Event, completion: @escaping OpenGrowEmptyClosure)
    func getEvents(completion: @escaping OpenGrowEventsClosure)
...
class EventsStorage: EventsStorageProtocol {
    ...
    private let dataCache: DataCache = {
        ...
        return DataCache(name: "opengrow-events-cache", path: appSupport)
    }()
