...
typealias OpenGrowChangeEventClosure = (_ oldEvent: Event) -> Event
public typealias OpenGrowEmptyClosure = () -> Void
...
class EventsHandler {
    ...
    private let sendingQueue = DispatchQueue(label: "com.opengrow.eventsHandler.sending")
    ...
    func setLinkToNewFutureActions(link: String?, completion: OpenGrowEmptyClosure?) {
        if let link {
    ...
    @objc func applicationDidBecomeActive() {
        let lastResignTimestamp = userDefaults.getInt(key: .opengrowResignTimestamp)
    ...
    @objc func applicationWillResignActive() {
        userDefaults.setInt(value: Date().toSeconds(), key: .opengrowResignTimestamp)
    }
    ...
    private func addInitialEvents() {
        ...
        userDefaults.setInt(value: userDefaults.getInt(key: .opengrowNumberOfOpens) + 1, key: .opengrowNumberOfOpens)
    }

    /// Logs an install event if it's the first app launch.
    private func addInstallIfNeeded() {
        let numberOfOpens = userDefaults.getInt(key: .opengrowNumberOfOpens)
    ...
    private func addReactivationIfNeeded() {
        let lastResignTimestamp = userDefaults.getInt(key: .opengrowLastStartTimestamp)
        if lastResignTimestamp != 0 {
        ...
        userDefaults.setInt(value: Date().toSeconds(), key: .opengrowLastStartTimestamp)
    }
    ...
    private func addLinkToEvents(link: String, completion: OpenGrowEmptyClosure?) {
    ...
    private func changeStorageEvents(eventHandling: @escaping OpenGrowChangeEventClosure, completion: OpenGrowEmptyClosure?) {
    ...
    private func sendNormalEventsToBackend(completion: OpenGrowEmptyClosure?) {
    ...
    func handleEventsIfNeeded(completion: OpenGrowEmptyClosure?) {
