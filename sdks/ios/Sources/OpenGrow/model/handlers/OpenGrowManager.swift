...
public typealias OpenGrowBoolCompletion = (_ value: Bool) -> Void

/// A manager class responsible for integrating the OpenGrow SDK into the application.
class OpenGrowManager {
    ...
    /// OpenGrowManager is main-thread-only by design: all public entry points
    /// are dispatched via `OpenGrow.onMain`, and all API/notification callbacks
    /// are delivered on the main thread.
    private func assertMainThread(_ function: StaticString = #function) {
        assert(Thread.isMainThread, "OpenGrowManager.\(function) must be called on the main thread")
    }

    // MARK: - Properties

    /// The API service instance responsible for communication with the OpenGrow backend.
    private var apiService: APIService

    /// The API key used for authenticating requests to the OpenGrow backend.
    private let apiKey: String

    /// The bundle ID of the application.
    private let bundleID: String

    /// A flag indicating whether the OpenGrow SDK is enabled.
    ...
    /// The handler for various events related to OpenGrow events.
    ...
    private var lastPayloadClosureArray = [OpenGrowPayloadClosure]()

    /// Closures to be called for the payloads
    private var payloadsClosureArray = [OpenGrowPayloadsClosure]()

    /// Stores if attributes needs to be updated after auth
    private var shouldUpdateAttributes = false

    /// The delegate for the OpenGrowManager, allowing customization and handling of OpenGrow events.
    weak var delegate: OpenGrowDelegate?
    ...
    private var actions = [OpenGrowAction]()
    private var handlingURL = false

    // MARK: - Initialization

    /// Initializes the OpenGrowManager with the provided API key and delegate.
    ///
    /// - Parameters:
    ///   - apiKey: The API key for authentication with the OpenGrow backend.
    ///   - useTestEnvironment: If the test environment should be used
    ///   - delegate: The delegate for the OpenGrowManager.
    init(apiKey: String, useTestEnvironment: Bool, baseURL: String? = nil, delegate: OpenGrowDelegate?) {
        self.apiKey = apiKey
    ...
    /// Enables or disables the OpenGrow SDK.
    ///
    ...
    func generateLink(title: String?,
        ...
        guard authenticated else {
            let action = OpenGrowAction(mainBlock: {
    ...
    func getLastPayload(completion: @escaping OpenGrowPayloadClosure) {
        assertMainThread()
    ...
    func getAllPayloadsSinceStartup(completion: @escaping OpenGrowPayloadsClosure) {
        assertMainThread()
    ...
    func getNotifications(page: Int, completion: @escaping OpenGrowNotificationsClosure) {
        guard authenticated else {
            let action = OpenGrowAction(mainBlock: {
    ...
    func getNumberOfUnreadNotifications(completion: @escaping OpenGrowIntClosure) {
        guard authenticated else {
            let action = OpenGrowAction(mainBlock: {
    ...
    func displayMessagesViewController(completion: OpenGrowEmptyClosure?) {
        guard authenticated else {
            let action = OpenGrowAction(mainBlock: {
    ...
    func markNotificationAsRead(notificationID: Int, completion: @escaping OpenGrowBoolCompletion) {
        guard authenticated else {
            let action = OpenGrowAction(mainBlock: {
    ...
    /// Fetches details for a OpenGrow link generated by the SDK in the current environment.
    ///
    /// - Parameter path:
    ...
    func getLinkDetails(path: String, completion: @escaping OpenGrowLinkClosure) {
        guard authenticated else {
            let action = OpenGrowAction(mainBlock: {
    ...
    private func handleReceivedAction(link: String?, payload: [String: Any]?, tracking: [String: Any]?) {
        ...
        if link != nil || payload != nil {
            DispatchQueue.main.async { [weak self] in
                self?.delegate?.opengrowReceivedPayloadFromDeeplink(link: link, payload: payload, tracking: tracking)
            }
        }
    ...
    private func automaticallyDisplayNotifications(notifications: [OpenGrowNotification]) {
    ...
    private func displayNotification(notification: OpenGrowNotification, completion: @escaping OpenGrowEmptyClosure) {
    ...
    private func enqueueAction(_ action: OpenGrowAction) {
        assertMainThread()
...
extension OpenGrowManager {
    ...
    func logInAppPurchase(transactionID: UInt64, completion: @escaping OpenGrowBoolCompletion) {
        guard authenticated else {
            let action = OpenGrowAction(mainBlock: {
    ...
    func logCustomPurchase(type: TransactionType, priceInCents: Int, currency: String, productID: String, startDate: Date? = nil, completion: @escaping OpenGrowBoolCompletion) {
        guard authenticated else {
            let action = OpenGrowAction(mainBlock: {
