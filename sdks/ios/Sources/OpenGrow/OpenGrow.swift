...
/// A protocol for receiving payload from OpenGrow SDK.
///
/// All delegate methods are called on the **main thread**.
public protocol OpenGrowDelegate: AnyObject {
...
/// A class representing OpenGrow SDK.
public class OpenGrow {

    // MARK: - Thread-safe state

    /// Lock protecting access to `_manager` and `_APIKey`.
    private static let stateLock = NSLock()
    private static var _manager: OpenGrowManager?
    private static var _APIKey: String?

    /// Thread-safe access to the manager instance.
    private static var manager: OpenGrowManager? {
    ...
    public static var delegate: OpenGrowDelegate? {
        set {
            let value = newValue
            onMain {
        ...
        get {
            assert(Thread.isMainThread, "OpenGrow.delegate must be read on the main thread")
    ...
    public static func configure(APIKey: String, useTestEnvironment: Bool, baseURL: String? = nil, delegate: OpenGrowDelegate?, completion: OpenGrowBoolCompletion? = nil) {
        onMain {
            self.APIKey = APIKey
            self.manager = OpenGrowManager(apiKey: APIKey, useTestEnvironment: useTestEnvironment, baseURL: baseURL, delegate: delegate)
    ...
    public static func lastReceivedPayload(completion: @escaping OpenGrowPayloadClosure) {
        onMain {
            guard let manager else {
    ...
    public static func allReceivedPayloadsSinceStartup(completion: @escaping OpenGrowPayloadsClosure) {
        onMain {
            guard let manager else {
    ...
    public static func linkDetails(path: String, completion: @escaping OpenGrowLinkClosure) {
        onMain {
            guard let manager else {
...
extension OpenGrow {
    ...
    public static func displayMessagesViewController(completion: OpenGrowEmptyClosure?) {
        onMain {
            guard let manager else {
    ...
    public static func numberOfUnreadMessages(completion: @escaping OpenGrowIntClosure) {
        onMain {
            guard let manager else {
...
extension OpenGrow {
    ...
    public static func logInAppPurchase(transactionID: UInt64, completion: @escaping OpenGrowBoolCompletion) {
        onMain {
            guard let manager else {
    ...
    public static func logCustomPurchase(type: TransactionType,
                                         ...
                                         productID: String,
                                         startDate: Date? = nil,
                                         completion: @escaping OpenGrowBoolCompletion) {
