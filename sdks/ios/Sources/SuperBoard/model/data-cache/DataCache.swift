...
open class DataCache {
    static let cacheDirectoryPrefix = "com.superboard.cache."
    static let ioQueuePrefix = "com.superboard.queue."
...
extension DataCache {
    ...
    /// Write data for key synchronously. Blocks until disk write completes.
    public func writeSync(data: Data, forKey key: String) {
        memCache.setObject(data as AnyObject, forKey: key as AnyObject)
        ioQueue.sync {
    ...
    private func ensureCacheDirectory() {
        if !fileManager.fileExists(atPath: cachePath) {
            do {
                ...
                DebugLogger.shared.log(.error, "DataCache: Error while creating cache folder: \(error.localizedDescription)")
            }
        }
    }
    ...
    public func write(object: NSCoding, forKey key: String) {
        do {
            ...
            DebugLogger.shared.log(.error, "Failed to archive object for key '\(key)': \(error.localizedDescription)")
        }
    }
    ...
    /// Write an array for key synchronously. Blocks until disk write completes.
    public func writeSync(array: Array<Any>, forKey key: String) {
        do {
            let data = try NSKeyedArchiver.archivedData(withRootObject: array, requiringSecureCoding: false)
            writeSync(data: data, forKey: key)
        } catch {
            DebugLogger.shared.log(.error, "Failed to archive array for key '\(key)': \(error.localizedDescription)")
        }
    }
    ...
    public func readObject(forKey key: String) -> NSObject? {
        ...
        do {
            ...
            DebugLogger.shared.log(.error, "Failed to unarchive object for key '\(key)': \(error.localizedDescription)")
            return nil
        }
    }
    ...
    public func clean(byKey key: String) {
        ...
        ioQueue.async {
            do {
                ...
                DebugLogger.shared.log(.error, "DataCache: Error while removing file: \(error.localizedDescription)")
            }
        }
    }
    ...
    public func cleanDiskCache() {
        ioQueue.async {
            do {
                ...
                DebugLogger.shared.log(.error, "DataCache: Error when cleaning disk: \(error.localizedDescription)")
            }
        }
    }
        ...
        ioQueue.async {
            ...
            for fileURL in URLsToDelete {
                do {
                    ...
                    DebugLogger.shared.log(.error, "DataCache: Error while removing files: \(error.localizedDescription)")
                }
            }
    ...
    fileprivate func travelCachedFiles(onlyForCacheSize: Bool) -> (urlsToDelete: [URL], diskCacheSize: UInt, cachedFiles: [URL: URLResourceValues]) {
        ...
        for fileUrl in (try? fileManager.contentsOfDirectory(at: diskCacheURL, includingPropertiesForKeys: Array(resourceKeys), options: .skipsHiddenFiles)) ?? [] {
            ...
            do {
                ...
                DebugLogger.shared.log(.error, "DataCache: Error while iterating files: \(error.localizedDescription)")
            }
        }
