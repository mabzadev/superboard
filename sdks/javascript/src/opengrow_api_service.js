...
class OpenGrowAPIService {
  ...
  constructor() {
    this.apiService = new OpenGrowAPIServiceHelper();
  }

  // Method to authenticate a device
  authenticateDevice(details, response, error) {
    this.apiService.POST(
      OpenGrowAPIService.ENDPOINTS.AUTHENTICATE,
      details,
      response,
      error
    );
  ...
  payloadForDevice(details, response, error) {
    this.apiService.POST(
      OpenGrowAPIService.ENDPOINTS.PAYLOAD,
      details,
      response,
      error
    );
  ...
  payloadForDeviceAndPath(details, path, response, error) {
    ...
    this.apiService.POST(
      OpenGrowAPIService.ENDPOINTS.PAYLOAD_FOR_DEVICE_AND_PATH,
      dataToSend,
      response,
      error
    );
  ...
  createEvent(event, createdAt, path, engagementTime, response, error) {
    ...
    this.apiService.POST(
      OpenGrowAPIService.ENDPOINTS.CREATE_EVENT,
      data,
      response,
      error
    );
  ...
  createLink(title, subtitle, imageUrl, data, response, error) {
    ...
    this.apiService.POST(
      OpenGrowAPIService.ENDPOINTS.CREATE_LINK,
      dataToSend,
      response,
      error
    );
  ...
  setUserAttributes(response, error) {
    const dataToSend = {};
    dataToSend.sdk_identifier = OpenGrowContext.USER_IDENTIFIER;
    if (OpenGrowContext.USER_ATTRIBUTES) {
      dataToSend.sdk_attributes = OpenGrowContext.USER_ATTRIBUTES;
    }

    this.apiService.POST(
      OpenGrowAPIService.ENDPOINTS.USER_ATTRIBUTES,
      dataToSend,
      response,
      error
    );
  ...
  messagesForDevice(page, response, error) {
    ...
    this.apiService.POST(
      OpenGrowAPIService.ENDPOINTS.NOTIFICATIONS_FOR_DEVICE,
      dataToSend,
      response,
      error
    );
  ...
  markMessageAsViewed(message, response, error) {
    ...
    this.apiService.POST(
      OpenGrowAPIService.ENDPOINTS.MARK_NOTIFICATION_AS_READ,
      dataToSend,
      response,
      error
    );
...
// Export the OpenGrowAPIService class
export default OpenGrowAPIService;
