...
- Modify: `ios/Classes/SuperBoardPlugin.swift:51-56` (the `application(_:didFinishLaunchingWithOptions:)` method)

**Step 1: Update iOS configure call to pass baseURL from Info.plist**

In `SuperBoardPlugin.swift`, update the `application(_:didFinishLaunchingWithOptions:)` method to read `SuperBoardBaseURL` from Info.plist and pass it to `SuperBoard.configure()`:

```swift
public func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [AnyHashable : Any] = [:]) -> Bool {
    if let infoDictionary = Bundle.main.infoDictionary, let apiKey = infoDictionary["SuperBoardApiKey"] as? String {
        let useTestEnvironment = infoDictionary["SuperBoardUseTestEnvironment"] as? Bool ?? false
...
**Step 2: Verify the iOS native SDK `SuperBoard.configure` signature accepts `baseURL: String?`**

The iOS SuperBoard SDK (version ~> 2.2) already has this parameter. No podspec version bump needed — verify by building.

**Step 3: Commit**

```bash
git add ios/Classes/SuperBoardPlugin.swift
...
- Modify: `android/src/main/kotlin/io/superboard/wrapper/SuperBoardPlugin.kt:78-82` (the `onAttachedToEngine` configure block)

**Step 1: Update Android configure call to pass baseURL from AndroidManifest meta-data**

In `SuperBoardPlugin.kt`, update the configure block inside `onAttachedToEngine` to read `superboard_base_url` and pass it:
...
**Step 2: Verify the Android native SDK `SuperBoard.configure` signature accepts `baseURL: String?`**

The Android SuperBoard SDK (version 1.1.0) already has this parameter. No version bump needed — verify by building.

**Step 3: Commit**

```bash
git add android/src/main/kotlin/io/superboard/wrapper/SuperBoardPlugin.kt
...
- Modify: `lib/models/superboard_link.dart` (add `TransactionType` enum at end of file)

**Step 1: Add `TransactionType` enum to superboard_link.dart**

Append to the end of `lib/models/superboard_link.dart`:
...
```bash
git add lib/models/superboard_link.dart
...
- Modify: `lib/superboard_platform_interface.dart:54` (add two new method stubs before the closing brace)

**Step 1: Add `logInAppPurchase` and `logCustomPurchase` to SuperBoardPlatform**

Add these methods after the `setDebugLevel` method (after line 54) in `superboard_platform_interface.dart`:
...
```bash
git add lib/superboard_platform_interface.dart
...
- Modify: `lib/superboard_method_channel.dart:96` (add two new method implementations before the closing brace)

**Step 1: Implement `logInAppPurchase` in MethodChannelSuperBoard**
...
```bash
git add lib/superboard_method_channel.dart
...
- Modify: `lib/superboard.dart:117` (add two new public methods before the `onDeeplinkReceived` getter)

**Step 1: Add `logInAppPurchase` and `logCustomPurchase` to SuperBoard class**
...
- Modify: `ios/Classes/SuperBoardPlugin.swift:71-212` (add two new cases in the `handle` switch statement)
...
- Modify: `android/src/main/kotlin/io/superboard/wrapper/SuperBoardPlugin.kt:95-257` (add two new cases in the `when` block)

**Step 1: Add imports for PaymentEventType**

Add to the imports at the top of `SuperBoardPlugin.kt`:
...
```kotlin
            ...
            "logCustomPurchase" -> {
                ...
                try {
                    SuperBoard.logCustomPurchase(type, priceInCents, currency, productId, startDate)
                    result.success(null)
                } catch (e: Exception) {
                    result.error("PAYMENT_ERROR", e.message, null)
                }
...
- iOS: Add `SuperBoardBaseURL` key to Info.plist with the custom domain
- Android: Add `<meta-data android:name="superboard_base_url" android:value="https://your-domain.com" />` to AndroidManifest.xml
...
- `await SuperBoard().logCustomPurchase(type: TransactionType.buy, priceInCents: 999, currency: 'USD', productId: 'premium')`
...
```bash
cd /Users/razvanchelemen/Clients_work/superboard/superboard-flutter
flutter analyze
```
...
> Historical SuperBoard 2.x implementation plan. Retained for release forensics
