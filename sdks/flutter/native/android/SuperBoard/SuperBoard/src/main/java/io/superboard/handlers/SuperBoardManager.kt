...
import io.superboard.utils.hasURISchemesConfigured
...
/**
 * SuperBoardManager is the core manager for the SuperBoard SDK.
 * 
 ...
 * @param superboardContext The SuperBoard context containing SDK settings and state
 * @param apiKey The API key for authenticating with SuperBoard backend
 * @param superboardService Optional custom service implementation for testing (defaults to real service)
...
class SuperBoardManager(
    ...
    apiKey: String,
    superboardService: ISuperBoardService? = null,
    ...
    companion object {
        private const val SUPERBOARD_PREFS_NAME = "superboard_prefs"
        private const val KEY_LAST_REFERRER = "last_referrer"
    }

    private val superboardService: ISuperBoardService = superboardService ?: SuperBoardService(context = context, apiKey = apiKey, superboardContext = superboardContext)
    private val appDetails: IAppDetailsHelper = appDetailsHelper ?: superboardContext.getAppDetails(context = context)
    private val eventsManager: IEventsManager = eventsManager ?: EventsManager(context = context, apiKey = apiKey, superboardContext = superboardContext)
    ...
    private val prefs = context.getSharedPreferences(SUPERBOARD_PREFS_NAME, Context.MODE_PRIVATE)
    ...
    var identifier: String?
        get() = superboardContext.identifier
        set(value) {
            superboardContext.identifier = value
            updateAttributesIfNeeded()
        }

    var pushToken: String?
        get() = superboardContext.pushToken
        set(value) {
            superboardContext.pushToken = value
            updateAttributesIfNeeded()
        }


    var attributes: Map<String, Any>?
        get() = superboardContext.attributes
        set(value) {
            superboardContext.attributes = value
            updateAttributesIfNeeded()
        }
    ...
    private suspend fun getDataForDevice(link: String? = null, delayEvents: Boolean): DeeplinkDetails? {
        ...
        appDetails.url = link
        val result = if (link == null) superboardService.payloadFor(appDetails) else superboardService.payloadWithLinkFor(appDetails)
        when (result) {
            is LSResult.Success -> {
    ...
    suspend fun authenticate(): Boolean {
        ...
        superboardService.getDeviceFor(appDetails.deviceID).transformWhile {
            emit(it)
            it is GVRetryResult.Retrying
        }.collect { deviceResult ->
            when (deviceResult) {
                is GVRetryResult.Success -> {
                    superboardContext.lastSeen = deviceResult.data.lastSeen
                }
        ...
        superboardService.authenticate(appDetails = appDetails).transformWhile {
            emit(it)
            it is GVRetryResult.Retrying
        }.collect { result ->
            when (result) {
                is GVRetryResult.Success -> {
                    ...
                    superboardContext.superboardId = result.data.superboardId

                    // Update context attributes if needed
                    if (shouldUpdateAttributes) {
                        updateAttributesIfNeeded()
                    } else {
                        superboardContext.identifier = result.data.sdkIdentifier
                        superboardContext.attributes = result.data.sdkAttributes
                    }
    ...
    suspend fun generateLink(title: String?,
        ...
        if (!superboardContext.settings.sdkEnabled) {
        ...
        return superboardService.generateLink(title = title,
            subtitle = subtitle,
            imageURL = imageURL,
            data = data,
            tags = tags,
    ...
    suspend fun linkDetails(path: String): LSResult<LinkDetailsResponse> {
        ...
        return superboardService.linkDetails(path = path)
    }
    ...
    private fun updateAttributesIfNeeded() {
        ...
        GlobalScope.launch {
            val result = superboardService.updateAttributes(identifier = identifier, attributes = attributes, pushToken = pushToken)
            when (result) {
                is LSResult.Success -> {
