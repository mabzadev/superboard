...
class LinkGenerationE2ETest {
    ...
    fun `Generate link with all parameters returns valid link`() {
        ...
        E2ETestUtils.enqueueGenerateLinkResponse(mockWebServer, link = "https://test.opengrow.io/full-params")
        ...
        assertEquals("Should return mocked link URL", "https://test.opengrow.io/full-params", result)
    ...
    fun `Generate link with minimal parameters returns valid link`() {
        ...
        E2ETestUtils.enqueueAuthenticationResponse(mockWebServer)
        E2ETestUtils.enqueueDeviceResponse(mockWebServer)
        E2ETestUtils.enqueueGenerateLinkResponse(mockWebServer, link = "https://test.opengrow.io/minimal")
        ...
        assertEquals("Should return mocked link URL", "https://test.opengrow.io/minimal", result)
    }
    ...
    fun `Generate link before authentication throws exception`() {
        runBlocking {
            ...
            // Act & Assert
            try {
                OpenGrow.generateLink(title = "Test")
    ...
    fun `Generate link with custom data payload returns valid link`() {
        ...
        E2ETestUtils.enqueueAuthenticationResponse(mockWebServer)
        E2ETestUtils.enqueueDeviceResponse(mockWebServer)
        E2ETestUtils.enqueueGenerateLinkResponse(mockWebServer, link = "https://test.opengrow.io/with-data")
        ...
        assertTrue("Result should contain link URL", result?.contains("opengrow.io") == true)
    ...
    fun `Generate link handles network timeout gracefully`() {
        ...
        val result = E2ETestUtils.runWithLooperPumping(5_000) {
            OpenGrow.generateLink(title = "Timeout Test")
        }
