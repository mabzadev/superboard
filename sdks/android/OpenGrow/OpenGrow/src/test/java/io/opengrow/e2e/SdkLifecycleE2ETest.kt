...
class SdkLifecycleE2ETest {
    ...
    fun `SDK debug level changes do not crash or affect authentication`() {
        runBlocking {
            ...
            OpenGrow.setDebug(LogLevel.ERROR)
            Shadows.shadowOf(Looper.getMainLooper()).idle()
    ...
    fun `Multiple configure calls do not crash`() {
        runBlocking {
            ...
            // Act
            OpenGrow.configure(application, "api-key-1", useTestEnvironment = true, baseURL = mockWebServer.url("/").toString())
            Shadows.shadowOf(Looper.getMainLooper()).idle()
            ...
            OpenGrow.configure(application, "api-key-2", useTestEnvironment = true, baseURL = mockWebServer.url("/").toString())
            Shadows.shadowOf(Looper.getMainLooper()).idle()
    ...
    fun `API calls before configure return error gracefully`() {
        ...
        try {
            runBlocking {
                val link = OpenGrow.generateLink(title = "Test")
    ...
    fun `SDK disables when sdkEnabled is false in settings`() {
        runBlocking {
            ...
            assertNotNull("OpenGrow instance should exist", instance)
            val managerField = instance!!.javaClass.getDeclaredField("opengrowManager")
            managerField.isAccessible = true
            val opengrowManager = managerField.get(instance)
            assertNotNull("OpenGrowManager should exist", opengrowManager)

            val contextField = opengrowManager.javaClass.getDeclaredField("opengrowContext")
            contextField.isAccessible = true
            val opengrowContext = contextField.get(opengrowManager)
            assertNotNull("OpenGrowContext should exist", opengrowContext)

            val settingsField = opengrowContext.javaClass.getDeclaredField("settings")
            settingsField.isAccessible = true
            val settings = settingsField.get(opengrowContext)
    ...
    fun `SDK handles rapid configuration and reconfiguration`() {
        runBlocking {
            ...
            // First configure
            OpenGrow.configure(application, "test-api-key-1", useTestEnvironment = true, baseURL = mockWebServer.url("/").toString())
            Shadows.shadowOf(Looper.getMainLooper()).idle()
            ...
            try {
                OpenGrow.configure(application, "test-api-key-2", useTestEnvironment = true, baseURL = mockWebServer.url("/").toString())
            } catch (e: Exception) {
