...
object E2ETestUtils {
    ...
    fun setupTestApplication(application: Application) {
        ...
        application.getSharedPreferences("OpenGrowStorage", android.content.Context.MODE_PRIVATE)
            .edit().clear().apply()
        application.getSharedPreferences("opengrow_prefs", android.content.Context.MODE_PRIVATE)
            .edit().clear().apply()
    }
    ...
    fun enqueueAuthenticationResponse(server: MockWebServer, opengrowId: String = "test-opengrow-id-123") {
        val response = """
            {
                "linksquared": "$opengrowId",
                "uri_scheme": "testapp"
            }
        """.trimIndent()
        server.enqueue(
    ...
    fun enqueueGenerateLinkResponse(server: MockWebServer, link: String = "https://test.opengrow.io/abc123") {
        val response = """
            {
                "link": "$link"
            }
    ...
    /**
     * Reset OpenGrow singleton state via reflection.
     * This allows each test to start with a fresh SDK state.
     */
    fun resetOpenGrowSingleton() {
        val errors = mutableListOf<String>()

        try {
            val opengrowClass = OpenGrow::class.java
            val instanceField = opengrowClass.getDeclaredField("instance")
            ...
            try {
                val appField = opengrowClass.getDeclaredField("application")
                appField.isAccessible = true
                val app = appField.get(instance) as? android.app.Application

                val observerField = opengrowClass.getDeclaredField("applicationLifecycleObserver")
            ...
            listOf(
                ...
                "application",
                "deeplinkListener",
                "opengrowNotificationsListener",
                "authenticationJob",
                "launcherActivityReference",
                "currentActivityReference"
            ).forEach { fieldName ->
                try {
                    val field = opengrowClass.getDeclaredField(fieldName)
            ...
            // Reset opengrowContext to fresh instance
            try {
                val contextField = opengrowClass.getDeclaredField("opengrowContext")
                contextField.isAccessible = true
                contextField.set(instance, OpenGrowContext())
            } catch (e: Exception) {
                errors.add("Failed to reset opengrowContext: ${e.message}")
            }

        } catch (e: Exception) {
            throw AssertionError(
                "Critical failure resetting OpenGrow singleton - tests will leak state: ${e.message}",
                e
            )
        }
    ...
    fun setupMockScreenResolution(width: String = "393", height: String = "851") {
        try {
            val screenUtilsClass = Class.forName("io.opengrow.utils.ScreenUtils")
    ...
    fun setupMockUserAgent(userAgent: String) {
        try {
            ...
            val webViewUtilsClass = Class.forName("io.opengrow.utils.WebViewUtils")
    ...
    /**
     * Get the authenticationJob from OpenGrow singleton via reflection.
     */
    fun getAuthenticationJob(): Job? {
        return try {
            ...
            val jobField = opengrowClass.getDeclaredField("authenticationJob")
    ...
    /**
     * Get the OpenGrow singleton instance via reflection.
     */
    fun getOpenGrowInstance(): Any? {
        return try {
            val companionClass = Class.forName("io.opengrow.OpenGrow\$Companion")
            val opengrowClass = OpenGrow::class.java

            val companionField = opengrowClass.getDeclaredField("Companion")
            ...
            try {
                ...
                println("Could not get OpenGrow instance: ${e.message}, fallback failed: ${e2.message}")
                null
            }
        }
    }
    ...
    /**
     * Get authentication state from OpenGrowManager.
     */
    fun getAuthenticationState(): String? {
        return try {
            val instance = getOpenGrowInstance() ?: return null
    ...
    /**
     * Get opengrowId from OpenGrowContext.
     */
    fun setNumStarted(value: Int) {
        try {
            val instance = getOpenGrowInstance() ?: return
    ...
    fun getOpenGrowId(): String? {
        return try {
            val instance = getOpenGrowInstance() ?: return null

            val contextField = instance.javaClass.getDeclaredField("opengrowContext")
            contextField.isAccessible = true
            val context = contextField.get(instance)

            val opengrowIdField = context.javaClass.getDeclaredField("opengrowId")
            opengrowIdField.isAccessible = true
            opengrowIdField.get(context) as? String
        } catch (e: Exception) {
            println("Could not get opengrowId: ${e.message}")
            null
        }
    }
    ...
    fun isSdkEnabled(): Boolean {
        return try {
            val instance = getOpenGrowInstance() ?: return true
     ...
     * 5. OpenGrow singleton instance exists
     */
    fun assertSdkFunctionalAfterError() {
        // 1. Verify singleton instance exists
        val instance = getOpenGrowInstance()
        assertNotNull("OpenGrow singleton instance should exist after error", instance)

        // 2. Verify public properties are readable
        val identifier = OpenGrow.identifier
        val pushToken = OpenGrow.pushToken
        val attributes = OpenGrow.attributes

        // 3. Verify public properties are writable (set, then restore)
        val originalIdentifier = identifier
        OpenGrow.identifier = "sdk-functional-check"
        assertEquals(
            "Setting identifier should work after error",
            "sdk-functional-check",
            OpenGrow.identifier
        )
        OpenGrow.identifier = originalIdentifier

        val originalPushToken = pushToken
        OpenGrow.pushToken = "test-push-token-check"
        assertEquals(
            "Setting pushToken should work after error",
            "test-push-token-check",
            OpenGrow.pushToken
        )
        OpenGrow.pushToken = originalPushToken
    ...
    private const val TEST_BUNDLE = "io.opengrow.test"
