...
class UserAttributesE2ETest {
    ...
    fun `Setting identifier after configuration works`() {
        runBlocking {
            ...
            // Act
            OpenGrow.identifier = "test-user-123"
            Shadows.shadowOf(Looper.getMainLooper()).idle()

            // Assert
            assertEquals("Identifier should be set", "test-user-123", OpenGrow.identifier)
        }
    }
    ...
    fun `Setting push token after configuration works`() {
        runBlocking {
            ...
            // Assert
            assertEquals("Push token should be set", "fcm-token-abc123", OpenGrow.pushToken)
        }
    }
    ...
    fun `Setting attributes after configuration works`() {
        runBlocking {
            ...
            // Act
            OpenGrow.attributes = mapOf("name" to "Test User", "level" to 5)
            Shadows.shadowOf(Looper.getMainLooper()).idle()

            // Assert
            assertNotNull("Attributes should be set", OpenGrow.attributes)
            assertEquals("Test User", OpenGrow.attributes?.get("name"))
        }
    }
    ...
    fun `Null identifier clears value`() {
        runBlocking {
            ...
            OpenGrow.identifier = "temp-user"
            Shadows.shadowOf(Looper.getMainLooper()).idle()

            assertEquals("Identifier should be set", "temp-user", OpenGrow.identifier)

            // Act
            OpenGrow.identifier = null
            Shadows.shadowOf(Looper.getMainLooper()).idle()

            // Assert
            assertNull("Identifier should be cleared", OpenGrow.identifier)
        }
    }
    ...
    fun `Attributes support various value types`() {
        runBlocking {
            ...
            OpenGrow.attributes = mixedAttributes
            Shadows.shadowOf(Looper.getMainLooper()).idle()

            // Assert
            assertNotNull("Attributes should be set", OpenGrow.attributes)
            assertEquals("String attribute should match", "Test User", OpenGrow.attributes?.get("name"))
            assertEquals("Int attribute should match", 25, OpenGrow.attributes?.get("age"))
            assertEquals("Boolean attribute should match", true, OpenGrow.attributes?.get("isPremium"))
        }
    }
    ...
    fun `Null attributes clears values`() {
        runBlocking {
            ...
            OpenGrow.attributes = mapOf("key" to "value")
            Shadows.shadowOf(Looper.getMainLooper()).idle()

            assertNotNull("Attributes should be set", OpenGrow.attributes)

            // Act
            OpenGrow.attributes = null
            Shadows.shadowOf(Looper.getMainLooper()).idle()

            // Assert
            assertNull("Attributes should be cleared", OpenGrow.attributes)
        }
    }
    ...
    fun `Set identifier triggers updateAttributes API call`() {
        runBlocking {
            ...
            assertEquals("OpenGrow.identifier should be set", "user-123", OpenGrow.identifier)
    ...
    fun `Set pushToken triggers updateAttributes API call`() {
        runBlocking {
            ...
            // Act
            E2ETestUtils.configureAndWaitForAuthOnly(application, baseURL = mockWebServer.url("/").toString())
            OpenGrow.pushToken = "fcm-token-xyz-123"

            delay(500)
            Shadows.shadowOf(Looper.getMainLooper()).idle()

            // Assert - verify SDK state and network request if completed
            assertEquals("OpenGrow.pushToken should be set", "fcm-token-xyz-123", OpenGrow.pushToken)
            val requests = E2ETestUtils.collectAllRequests(mockWebServer)
            val visitorRequests = requests.filter { it.first.contains("visitor_attributes") }
            if (visitorRequests.isNotEmpty()) {
                assertTrue(
    ...
    fun `Set attributes triggers updateAttributes API call`() {
        runBlocking {
            ...
            // Act
            E2ETestUtils.configureAndWaitForAuthOnly(application, baseURL = mockWebServer.url("/").toString())
            OpenGrow.attributes = mapOf("plan" to "premium", "age" to 25)

            delay(500)
            Shadows.shadowOf(Looper.getMainLooper()).idle()

            // Assert - verify SDK state and network request if completed
            assertNotNull("OpenGrow.attributes should be set", OpenGrow.attributes)
            assertEquals("OpenGrow.attributes should contain plan", "premium", OpenGrow.attributes?.get("plan"))
            assertEquals("OpenGrow.attributes should contain age", 25, OpenGrow.attributes?.get("age"))
            val requests = E2ETestUtils.collectAllRequests(mockWebServer)
            val visitorRequests = requests.filter { it.first.contains("visitor_attributes") }
            if (visitorRequests.isNotEmpty()) {
                assertTrue(
    ...
    fun `Attributes update handles server error gracefully`() {
        runBlocking {
            ...
            OpenGrow.identifier = "user-error-test"
            delay(500)
            Shadows.shadowOf(Looper.getMainLooper()).idle()
