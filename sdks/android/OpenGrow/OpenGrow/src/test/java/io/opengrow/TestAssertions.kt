...
object TestAssertions {
     ...
     * @param valueName Name of the value being checked (e.g., "opengrowId", "authenticationState")
    ...
    /**
     * Assert OpenGrowManager is in AUTHENTICATED state.
     */
    fun assertAuthenticated(manager: io.opengrow.handlers.OpenGrowManager, context: String) {
        val actual = manager.authenticationState
        assertEquals(
            "Expected authenticationState to be AUTHENTICATED $context, but was $actual",
            io.opengrow.handlers.OpenGrowManager.AuthenticationState.AUTHENTICATED,
            actual
        )
    }

    /**
     * Assert OpenGrowManager is in UNAUTHENTICATED state.
     */
    fun assertUnauthenticated(manager: io.opengrow.handlers.OpenGrowManager, context: String) {
        val actual = manager.authenticationState
        assertEquals(
            "Expected authenticationState to be UNAUTHENTICATED $context, but was $actual",
            io.opengrow.handlers.OpenGrowManager.AuthenticationState.UNAUTHENTICATED,
            actual
        )
    }
     ...
     */
    fun assertSdkEnabled(opengrowContext: io.opengrow.handlers.OpenGrowContext, context: String) {
        assertTrue(
     ...
     */
    fun assertSdkDisabled(opengrowContext: io.opengrow.handlers.OpenGrowContext, context: String) {
        assertFalse(
    ...
    fun assertAllowedToSendToBackend(
        eventsManager: io.opengrow.handlers.EventsManager,
        expected: Boolean,
        context: String
    ) {
    ...
    suspend fun assertEventSent(
        eventType: io.opengrow.model.EventType,
        mockService: io.opengrow.service.IOpenGrowService,
        context: String
    ) {
        try {
    ...
    suspend fun assertEventStored(
        eventType: io.opengrow.model.EventType,
        mockStorage: io.opengrow.storage.IEventsStorage,
        context: String
    ) {
        try {
    ...
    // PURCHASE_EVENT_DISABLED:     eventType: io.opengrow.model.events.PaymentEventType,
    // PURCHASE_EVENT_DISABLED:     mockStorage: io.opengrow.storage.IEventsStorage,
     ...
     */
    fun <T : Any> assertResultSuccess(result: io.opengrow.utils.LSResult<T>, context: String): T {
        assertTrue(
            "Expected LSResult.Success $context, but was LSResult.Error: ${(result as? io.opengrow.utils.LSResult.Error)?.exception?.message}",
            result is io.opengrow.utils.LSResult.Success
        )
        return (result as io.opengrow.utils.LSResult.Success).data
    }

    /**
     * Assert LSResult is Error.
     */
    fun <T : Any> assertResultError(result: io.opengrow.utils.LSResult<T>, context: String): Exception {
        assertTrue(
            "Expected LSResult.Error $context, but was LSResult.Success",
            result is io.opengrow.utils.LSResult.Error
        )
        return (result as io.opengrow.utils.LSResult.Error).exception
    }
    ...
    fun <T : Any> assertResultErrorContains(
        result: io.opengrow.utils.LSResult<T>,
        expectedMessageContains: String,
        context: String
    ) {
