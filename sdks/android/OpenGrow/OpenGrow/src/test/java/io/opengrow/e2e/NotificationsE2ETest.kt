...
class NotificationsE2ETest {
    ...
    fun `Set automatic notifications listener does not crash`() {
        runBlocking {
            ...
            OpenGrow.setOnAutomaticNotificationsListener { isLast ->
    ...
    fun `Number of unread notifications before authentication returns null`() {
        ...
        try {
            runBlocking {
                val result = OpenGrow.numberOfUnreadMessages()
    ...
    fun `Number of unread messages with callback style API works`() {
        runBlocking {
            ...
            OpenGrow.numberOfUnreadMessages(lifecycleOwner = null) { result ->
                callbackInvoked = true
    ...
    fun `numberOfUnreadMessages with callback invokes listener`() {
        runBlocking {
            ...
            // Act
            OpenGrow.numberOfUnreadMessages(lifecycleOwner = null) { count ->
                callbackInvoked = true
                receivedCount = count
                latch.countDown()
            }
    ...
    fun `displayMessagesFragment handles missing activity gracefully`() {
        runBlocking {
            ...
            // Act
            OpenGrow.displayMessagesFragment(onDismissed = null)
