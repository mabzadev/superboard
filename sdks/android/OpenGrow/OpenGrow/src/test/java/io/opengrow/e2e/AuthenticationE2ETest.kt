...
class AuthenticationE2ETest {
    ...
    fun `test SDK authenticates device and makes requests on app startup`() {
        runBlocking {
            // Arrange
            E2ETestUtils.enqueueAuthenticationResponse(mockWebServer, opengrowId = "opengrow_e2e_authenticated_123")
            ...
            // Verify opengrowId if accessible (internal implementation detail)
            val storedOpenGrowId = E2ETestUtils.getOpenGrowId()
            if (storedOpenGrowId != null) {
                assertEquals(
                    "SDK should store mocked opengrowId",
                    "opengrow_e2e_authenticated_123",
                    storedOpenGrowId
                )
            }
    ...
    fun `test direct HTTP call to MockWebServer works`() {
        // Arrange
        E2ETestUtils.enqueueAuthenticationResponse(mockWebServer, opengrowId = "direct-test-123")
