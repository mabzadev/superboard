...
import io.opengrow.TestAssertions.assertAllowedToSendToBackend
...
import io.opengrow.TestAssertions.assertEventStored
// PURCHASE_EVENT_DISABLED: import io.opengrow.TestAssertions.assertPaymentEventStored
...
class EventsManagerTest {
    ...
    fun `EventsManager onAppForegrounded sends queued events to backend when sending allowed`() = runTest {
        ...
        coEvery { mockOpenGrowService.addEvent(any()) } returns LSResult.Success(true)
        ...
        coVerify(timeout = 1000) { mockOpenGrowService.addEvent(testEvent) }
    }
    ...
    fun `EventsManager log sends events to backend when allowed`() = runTest {
        ...
        coVerify { mockOpenGrowService.addEvent(any()) }
    }
    ...
    // PURCHASE_EVENT_DISABLED:             "packageName": "io.opengrow.test",
