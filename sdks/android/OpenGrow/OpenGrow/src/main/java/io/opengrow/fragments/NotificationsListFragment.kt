...
import io.opengrow.adapters.NotificationsListAdapter
import io.opengrow.adapters.NotificationsListItem
import io.opengrow.databinding.FragmentNotificationsListBinding
import io.opengrow.model.notifications.Notification
import io.opengrow.utils.hideProgressBar
import io.opengrow.utils.safeNavigate
import io.opengrow.utils.showProgressBar
import io.opengrow.viewmodels.NotificationsMainViewModel
        ...
        binding.recyclerView.adapter = NotificationsListAdapter(requireContext(), items, onNotificationSelected = { item ->
            when (item) {
                is NotificationsListItem.Notification -> {
                    findNavController().safeNavigate(R.id.notificationsListFragment, R.id.action_showNotificationDetailsFragment, bundleOf(
                        ARG_NOTIFICATION to item.item))
                }
            }
        })
    }
