...
import io.opengrow.BuildConfig
...
import io.opengrow.model.ErrorMessage
...
import io.opengrow.utils.LSJsonDateTypeAdapterFactory
import io.opengrow.utils.LSJsonInstantCompatTypeAdapterFactory
import io.opengrow.utils.LSJsonInstantTypeAdapterFactory
...
class OpenGrowService(val context: Context, val apiKey: String, val opengrowContext: OpenGrowContext) : IOpenGrowService {
    private val opengrowApi: OpenGrowApi
    private val appDetails: AppDetailsHelper by lazy { opengrowContext.getAppDetails(context = context) }
    private val userAgent: String by lazy { opengrowContext.getUserAgent(context = context) }
    ...
    private val accessKey: String
        get() {
            if (opengrowContext.settings.useTestEnvironment) {
                return "test_$apiKey"
            }
    ...
    init {
        opengrowApi = getRetrofit().create(OpenGrowApi::class.java)
    }
    ...
    override suspend fun payloadFor(@Body request: AppDetails): LSResult<DeeplinkDetails> {
        ...
        while (true) {
            try {
                val response = opengrowApi.payloadFor(request)
    ...
    override suspend fun payloadWithLinkFor(@Body request: AppDetails): LSResult<DeeplinkDetails> {
        ...
        while (true) {
            try {
                val response = opengrowApi.payloadWithLinkFor(request)
                if (response.isSuccessful) {
                    val body = response.body()
                    body?.let {
                        DebugLogger.instance.log(LogLevel.INFO, "Fetching payload for device - Received payload")
    ...
    override fun authenticate(appDetails: AppDetails): Flow<GVRetryResult<AuthenticationResponse>> = flow {
        ...
        val response = opengrowApi.authenticate(appDetails)
        if (response.isSuccessful) {
            val body = response.body()
            body?.let {
    ...
    override suspend fun generateLink(title: String?,
        ...
        try {
            ...
            val response = opengrowApi.generateLink(request)
            if (response.isSuccessful) {
                val body = response.body()
                body?.let {
                    return LSResult.Success(it)
    ...
    override suspend fun linkDetails(path: String): LSResult<LinkDetailsResponse> {
        try {
            ...
            val response = opengrowApi.linkDetails(request)
            if (response.isSuccessful) {
                val body = response.body()
                body?.string()?.let {
                    try {
    ...
    override suspend fun addEvent(event: Event): LSResult<Boolean> {
        try {
            ...
            val response = opengrowApi.addEvent(event)
            if (response.isSuccessful) {
                val body = response.body()
                body?.let {
    ...
    override suspend fun addPaymentEvent(event: PaymentEvent): LSResult<Boolean> {
        try {
            ...
            val response = opengrowApi.addPaymentEvent(event)
            if (response.isSuccessful) {
                val body = response.body()
                body?.let {
        ...
        while (true) {
            try {
                val request = UpdateAttributesRequest( sdkIdentifier = identifier,
                    ...
                    pushToken = pushToken)
                val response = opengrowApi.updateAttributes(request)
                if (response.isSuccessful) {
                    val body = response.body()
                    body?.let {
    ...
    override fun getDeviceFor(vendorId: String): Flow<GVRetryResult<GetDeviceResponse>> = flow {
        ...
        val response = opengrowApi.getDeviceFor(vendorId)
        if (response.isSuccessful) {
            val body = response.body()
            body?.let {
    ...
    override suspend fun notifications(page: Int): LSResult<NotificationsResponse> {
        ...
        while (true) {
            try {
                ...
                val response = opengrowApi.notifications(request)
                if (response.isSuccessful) {
                    val body = response.body()
                    body?.let {
    ...
    override suspend fun numberOfUnreadNotifications(): LSResult<NumberOfUnreadNotificationsResponse> {
        ...
        while (true) {
            try {
                val response = opengrowApi.numberOfUnreadNotifications()
                if (response.isSuccessful) {
                    val body = response.body()
                    body?.let {
    ...
    override suspend fun markNotificationAsRead(notificationId: Int): LSResult<Boolean> {
        ...
        while (true) {
            try {
                ...
                val response = opengrowApi.markNotificationAsRead(request = request)
                if (response.isSuccessful) {
                    val body = response.body()
                    body?.let {
    ...
    override suspend fun notificationsToDisplayAutomatically(): LSResult<NotificationsResponse> {
        ...
        while (true) {
            try {
                val response = opengrowApi.notificationsToDisplayAutomatically()
                if (response.isSuccessful) {
                    val body = response.body()
                    body?.let {
    ...
    private fun headers(): Map<String, String> {
        ...
        opengrowContext.opengrowId?.let {
