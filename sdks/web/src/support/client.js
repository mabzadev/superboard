...
export class SuperBoardSupportClient {
	constructor({
		...
		this.identityTokenProvider = validateFunction(identityTokenProvider, "identityTokenProvider");
	...
	async createConversation({
		...
		return this.#dataObject("/conversations", {
			...
			idempotencyKey,
			body: {
				client_conversation_id: boundedText(clientConversationId, "client_conversation_id", 128),
				...(subject === undefined ? {} : { subject }),
				...(inboxId === undefined ? {} : { inbox_id: inboxId }),
				...(customAttributes === undefined ? {} : { custom_attributes: customAttributes }),
			},
		});
	}
	...
	sendMessage(
		...
		if (normalizedBody !== null && utf8Length(normalizedBody) > MAX_MESSAGE_BYTES) {
			throw new SuperBoardSupportException("message_body_too_large", "Message body exceeds 64 KB");
		}
	...
	editMessage(conversationId, messageId, { body, metadata, idempotencyKey } = {}) {
	...
	async uploadAttachment(conversationId, { data, filename, contentType, idempotencyKey } = {}) {
		...
		const payload = await this.#request(`/conversations/${segment(conversationId)}/attachments`, {
			method: "POST",
			rawBody: body,
			idempotencyKey,
			headers: {
	...
	async downloadAttachment(conversationId, messageId, { attachmentId } = {}) {
		const response = await this.#request(
			appendQuery(`/conversations/${segment(conversationId)}/attachments/${segment(messageId)}`, {
	...
	helpCenterArticle({ portalSlug, articleSlug, locale } = {}) {
		return this.#dataObject(
			appendQuery(`/help-center/${segment(portalSlug)}/articles/${segment(articleSlug)}`, {
				locale: optionalBoundedText(locale, "locale", 32),
			}),
			{ method: "GET" },
		);
	...
	requestRealtimeTicket(conversationId, { idempotencyKey } = {}) {
		return this.#dataObject(`/conversations/${segment(conversationId)}/realtime-ticket`, {
			method: "POST",
			body: {},
			idempotencyKey,
		});
	...
	async #request(
		path,
		{ method = "GET", body, rawBody, headers = {}, idempotencyKey, responseType = "json" } = {},
	) {
		if (this.closed) {
		...
		const serializedBody =
			rawBody === undefined ? (body === undefined ? undefined : JSON.stringify(body)) : rawBody;
		...
		for (let attempt = 0; attempt < maximumAttempts; attempt += 1) {
			...
			try {
				response = await this.#fetchWithTimeout(
					new URL(stripLeadingSlash(path), `${this.baseUrl}/`),
					{
						method: normalizedMethod,
			...
			if (response.status === 401 && this.identityTokenProvider && !refreshedAfterUnauthorized) {
		...
		throw lastError || new SuperBoardSupportException("request_failed", "Support request failed");
	}
	...
	async #headers({ method, path, body, idempotencyKey, headers }) {
		...
		if (this.widgetSignatureProvider) {
			...
			result.set("X-SuperBoard-Widget-Timestamp", boundedText(signedAt, "widget_timestamp", 64));
		}
		return result;
	}
	...
	async #fetchWithTimeout(url, init) {
		...
		try {
			...
			if (controller.signal.aborted) {
				throw new SuperBoardSupportException("request_timeout", "Support request timed out", {
					retryable: true,
					cause: error,
				});
			}
...
function normalizeSupportBaseUrl(input) {
	...
	if (normalizedPath === "/") {
		...
		throw new TypeError(`baseUrl path must be ${SUPERBOARD_SUPPORT_WIDGET_PATH}`);
	}
...
async function parseResponsePayload(response) {
	...
	throw new SuperBoardSupportException("response_invalid", "Support returned an invalid response", {
...
function dataObject(payload, errorCode) {
	if (isRecord(payload.data)) return payload.data;
	throw new SuperBoardSupportException(errorCode, "Support returned an invalid response", {
		requestId: requestIdFromPayload(payload),
	});
}
