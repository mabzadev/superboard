...
export const useAuth = () => {
	...
	const idToken = useMemo(() => state.idToken ?? null, [state.idToken]);
	...
	const isAuthenticating = useMemo(() => state.isAuthenticating, [state.isAuthenticating]);
	...
	const acquireToken = useCallback(async () => {
		...
		if (hasValidRefreshToken) {
			...
			try {
				...
				const errorMsg = handleError(e, ErrorType.ExchangeAccessToken);
				dispatch({
			...
			dispatch({
				type: "setAcquireTokenError",
				payload: ErrorType.InvalidRefreshToken,
			});
		}
	...
	const acquireUserInfo = useCallback(async () => {
		...
		try {
			const res = await getUserInfo(state.config, { accessToken: accessToken ?? "" });
			...
			const errorMsg = handleError(e, ErrorType.FetchUserInfo);
			dispatch({
	...
	const logoutRedirect = useCallback(
		async ({
			...
			try {
				...
				const msg = handleError(e, ErrorType.LogoutFailed);
				dispatch({
					type: "setLogoutError",
					payload: msg,
				});
