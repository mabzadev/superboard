...
import { handleError, ErrorType, isValidTokens } from "@melody-auth/shared";
import type { AuthState } from "@melody-auth/shared";
...
export const handleTokenExchangeByAuthCode = (
	...
	exchangeTokenByAuthCode(code, requestState, state().config)
		.then((res) => {
			state.update((prev) => {
				const newState = { ...prev };
		...
		.catch((e) => {
			const msg = handleError(e, ErrorType.ObtainAccessToken);
