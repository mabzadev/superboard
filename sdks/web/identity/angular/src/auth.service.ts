...
import { handleError, ErrorType, AuthorizeMethod } from "@melody-auth/shared";
import type { GetUserInfoRes, LoginProps } from "@melody-auth/shared";
...
export class AuthService {
	...
	async login(method: AuthorizeMethod, props?: LoginProps) {
	...
	async loginRedirect(props?: LoginProps): Promise<void> {
		await this.login("redirect", props);
	}

	async loginPopup(props?: LoginProps): Promise<void> {
		await this.login("popup", props);
	}
	...
	async acquireUserInfo(): Promise<GetUserInfoRes | undefined> {
		...
		try {
			const res = await getUserInfo(state.config, { accessToken: token ?? "" });
			this.authContext.state.update((prev) => ({
				...prev,
				userInfo: res,
