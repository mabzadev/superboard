import { ProviderConfig, SessionStorageKey, AuthorizeMethod } from "@melody-auth/shared";

import { genCodeVerifierAndChallenge, genAuthorizeState } from "../generators";
...
export const triggerLogin = async (
	...
	const { codeChallenge, codeVerifier } = await genCodeVerifierAndChallenge();
	window.sessionStorage.setItem(SessionStorageKey.State, state);
	window.sessionStorage.setItem(SessionStorageKey.CodeVerifier, codeVerifier);
	try {
