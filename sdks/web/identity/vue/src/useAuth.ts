import { handleError, ErrorType, AuthorizeMethod, LoginProps } from "@melody-auth/shared";
...
export function useAuth() {
	...
	async function login(method: AuthorizeMethod, props?: LoginProps) {
		if (!state) {
			throw new Error(SetupError);
		}
	...
	async function loginRedirect(props?: LoginProps) {
		await login("redirect", props);
	}

	async function loginPopup(props?: LoginProps) {
		await login("popup", props);
	}
