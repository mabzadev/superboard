...
const OUTPUT_ROOT = resolve(REPOSITORY_ROOT, "sdks/web/flows/upstream");
...
function parseArguments(argv) {
	...
	for (let index = 0; index < argv.length; index += 1) {
		...
		if (argument === "--flows-sh-source" || argument === "--flows-sdk-source") {
			...
			options[argument === "--flows-sh-source" ? "flowsShSource" : "flowsSdkSource"] =
				resolve(value);
			index += 1;
			continue;
		}
...
function transformPackageJson(sourceKey, destinationPath, text) {
	...
	const replaceDependencies = (dependencies) => {
		...
		for (const [name, version] of Object.entries(dependencies)) {
			...
			if (mappedName !== name) {
				...
				dependencies[mappedName] = "workspace:*";
			} else if (
				!destinationPath.startsWith("reference/") &&
				[
					...Object.values(SDK_PACKAGE_NAMES),
					...Object.values(PRODUCT_PACKAGE_NAMES),
					"@superboard/flows-icons",
					"@superboard/flows-ui",
					"@superboard/flows-product-shared",
				].includes(name)
			) {
	...
	if (sourceKey === "flows-sdk") value.private = true;
...
function addApiHelpers(text) {
	...
	if (!text.includes(marker))
		throw new Error("The pinned SDK API module no longer matches its reviewed shape");
...
export const sendActivate = async (blockId: string, blockStateId?: string): Promise<void> => {
  ...
  sdkKey?: string;`,
	...
	if (destinationPath === "packages/react/src/flows-provider.tsx") {
		output = output
			.replace("  logBranding,", "  getDefaultApiUrl,\n  logBranding,\n  withSdkKey,")
			.replace(
				"useCallback, useEffect, type FC, type ReactNode",
				"useCallback, useEffect, useMemo, type FC, type ReactNode",
			)
			.replace('apiUrl = "https://api.flows-cloud.com",', "apiUrl = getDefaultApiUrl(),")
			.replace(
  ...
  globalConfig.apiUrl = apiUrl;`,
			...
			.replace(
				" * Object with custom [user properties](https://flows.sh/docs/users/properties).",
				" * Object with custom user properties.",
			)
			.replace(
				" * Language used to enable [localization](https://flows.sh/docs/localization).",
				" * Language used to enable localization.",
			);
	}
  ...
  const websocketResult = websocket({ url: wsUrl, onMessage, onOpen: handleWebSocketOpen });`,
    ...
    return \`\${baseUrl}/ws/sdk/block-updates?\${new URLSearchParams(params).toString()}\`;`,
			...
			.replace("  organizationId: string;", "  organizationId: string;\n  sdkKey?: string;")
			.replace("  organizationId,\n  userId,", "  organizationId,\n  sdkKey,\n  userId,")
			.replace(
				'    return getWebSocketUrl(apiUrl, "/ws/sdk/block-updates", params);',
...
describe("SuperBoard SDK event idempotency", () => {
  ...
  it("keeps command order by stopping at the first failed event", async () => {
    ...
    let available = false;
    const target = (async (_input: RequestInfo | URL, init?: RequestInit) => {
      if (typeof init?.body !== "string") throw new Error("Expected JSON event body");
      const body = JSON.parse(init.body) as { workflowId?: string };
...
`;
	...
	if (destinationPath.startsWith("tests/e2e/")) {
		output = output
			...
			.replaceAll(
				'url.pathname === "/ws/sdk/block-updates"',
				'url.pathname === "/api/v1/flows/ws/sdk/block-updates"',
			)
		...
		if (destinationPath.includes("tests/e2e/tests/components/")) {
			output = output
				.replaceAll(
					" - should show branding with free org",
					" - should not inject upstream branding",
				)
				.replaceAll(
...
function transformText(sourceKey, destinationPath, contents) {
	if (destinationPath.endsWith(".oxlintrc.json")) {
		const configuration = JSON.parse(contents);
		if (configuration.options) {
			delete configuration.options.typeAware;
			if (Object.keys(configuration.options).length === 0) delete configuration.options;
		}
		if (sourceKey === "flows-sh" && destinationPath !== "product/.oxlintrc.json") {
			configuration.extends = (configuration.extends ?? []).map((path) =>
				path === "../../.oxlintrc.json" ? "../../../../.oxlintrc.json" : path,
			);
		}
		if (destinationPath === ".oxlintrc.json") {
			configuration.ignorePatterns = [
				...new Set([...(configuration.ignorePatterns ?? []), "reference/**"]),
			];
		}
	...
	if (sourceKey === "flows-sh") {
		...
		if (destinationPath === "reference/product-docs/content/meta.json") {
			...
			meta.pages = meta.pages.filter(
				(page) => page !== "billing" && page !== "project-setup" && page !== "---Management---",
			);
...
function createSelection(flowsShSource, flowsSdkSource) {
	...
	addFile("flows-sdk", flowsSdkSource, ".oxlintrc.json", ".oxlintrc.json");
	...
	addTree("flows-sdk", flowsSdkSource, "examples", "reference/framework-examples", (path) =>
		REFERENCE_EXTENSIONS.has(extname(path).toLowerCase()),
	);

	addFile("flows-sh", flowsShSource, "LICENSE.md", "notices/flows-sh-LICENSE.md");
	addFile("flows-sh", flowsShSource, ".oxlintrc.json", "product/.oxlintrc.json");
	for (const packageName of ["types", "shared", "icons", "ui"]) {
		addTree(
			...
			(path) => {
				...
				if (
					packageName === "ui" &&
					(path.startsWith("src/logo/") || path.startsWith("src/producthunt/"))
				) {
					return false;
				}
				return true;
	...
	addTree("flows-sh", flowsShSource, "examples", "reference/product-examples", (path) => {
		if (!REFERENCE_EXTENSIONS.has(extname(path).toLowerCase())) return false;
		if (path.includes("/src/components/ui/") || path.includes("/src/components/providers/"))
			return false;
		if (path.endsWith("/src/app/layout.tsx") || path.endsWith("/src/app/providers.tsx"))
			return false;
		if (path.endsWith("/src/lib/utils.ts")) return false;
		return true;
	});
	addTree("flows-sh", flowsShSource, "example-templates", "reference/product-templates", (path) => {
		if (!REFERENCE_EXTENSIONS.has(extname(path).toLowerCase())) return false;
		if (path.includes("/src/components/ui/") || path.includes("/src/components/providers/"))
			return false;
		if (path.endsWith("/src/app/layout.tsx") || path.endsWith("/src/app/providers.tsx"))
...
This directory is generated by \`sdks/web/flows/scripts/sync-flows-upstream.mjs\` from two pinned public
...
function buildManifest(files) {
	return {
		...
		policy: {
			generated_by: "sdks/web/flows/scripts/sync-flows-upstream.mjs",
...
function verifyOutput({ verifySources = false, sourceDirectories = {} } = {}) {
	if (!existsSync(MANIFEST_PATH))
		throw new Error(`Missing ${posixPath(relative(REPOSITORY_ROOT, MANIFEST_PATH))}`);
	...
	const expectedPaths = new Set([
		"GENERATED.md",
		...manifest.files.map((entry) => entry.output_path),
	]);
	...
	for (const entry of manifest.files) {
		...
		if (actualHash !== entry.output_sha256)
			throw new Error(`Generated file drift: ${entry.output_path}`);
		...
		if (verifySources) {
			...
			if (!existsSync(sourcePath))
				throw new Error(`Missing upstream source file: ${entry.source_path}`);
...
export function syncFlowsUpstream({ flowsShSource, flowsSdkSource }) {
	...
	try {
		...
		writeFileSync(
			...
			`${JSON.stringify(buildManifest(files), null, "\t")}\n`,
		);
