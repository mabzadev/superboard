#import "SuperBoardWrapper.h"
#if __has_include(<react_native_superboard_wrapper/react_native_superboard_wrapper-Swift.h>)
#import <react_native_superboard_wrapper/react_native_superboard_wrapper-Swift.h>
#elif __has_include("react_native_superboard_wrapper-Swift.h")
#import "react_native_superboard_wrapper-Swift.h"
#else
...
@implementation SuperBoardWrapper
RCT_EXPORT_MODULE()

+ (void)initialize {
  dispatch_async(dispatch_get_main_queue(), ^{
    SuperBoardWrapperSwift *wrapper = SuperBoardWrapperSwift.shared;
  });
}
...
- (instancetype)init {
  //self = [super init];
  if (self) {
    SuperBoardWrapperSwift.shared.didReceiveDeeplink = ^(NSDictionary<NSString *,id> * _Nonnull payload) {
      //[self emitOnDeeplinkReceived:payload];
      [self sendEventWithName:@"onSuperBoardDeeplinkReceived" body:payload];
    };
  }
  return self;
}
...
- (NSArray<NSString *> *)supportedEvents {
  return @[@"onSuperBoardDeeplinkReceived"];
}
...
- (void)markReadyToHandleDeeplinks {
  dispatch_async(dispatch_get_main_queue(), ^{
    SuperBoardWrapperSwift.shared.bridgeLoaded = YES;
  });
}
...
- (void)setIdentifier:(NSString *)identifier {
  [SuperBoardWrapperSwift.shared setIdentifier:identifier];
}

- (void)setPushToken:(NSString *)pushToken {
  [SuperBoardWrapperSwift.shared setPushToken:pushToken];
}

- (void)setAttributes:(NSDictionary *)attributes {
  [SuperBoardWrapperSwift.shared setAttributes:attributes];
}

- (void)setSDK:(BOOL)enabled {
  [SuperBoardWrapperSwift.shared setSDK:enabled];
}

- (void)setDebug:(NSString *)level {
  [SuperBoardWrapperSwift.shared setDebug:level];
}
...
- (void)generateLink:(NSString *)title
  ...
  [SuperBoardWrapperSwift.shared generateLinkWithTitle:title
                                         subtitle:subtitle
                                         imageURL:imageURL
                                             data:data
                                             tags:tags
...
- (void)displayMessages:(RCTPromiseResolveBlock)resolve
                 reject:(RCTPromiseRejectBlock)reject {
  [SuperBoardWrapperSwift.shared displayMessagesViewControllerWithCompletion:^{
    resolve(nil);
  }];
}
...
- (void)numberOfUnreadMessages:(RCTPromiseResolveBlock)resolve
                        reject:(RCTPromiseRejectBlock)reject {
  [SuperBoardWrapperSwift.shared numberOfUnreadMessagesWithCompletion:^(NSInteger value) {
    if (value < 0) {
...
- (void)logInAppPurchase:(NSString *)transactionId
                 resolve:(RCTPromiseResolveBlock)resolve
                  reject:(RCTPromiseRejectBlock)reject {
  [SuperBoardWrapperSwift.shared logInAppPurchase:transactionId completion:^(BOOL success) {
    resolve(@(success));
  }];
}
...
- (void)logCustomPurchase:(NSString *)type
             priceInCents:(double)priceInCents
                ...
                startDate:(NSString *)startDate
                  resolve:(RCTPromiseResolveBlock)resolve
                   reject:(RCTPromiseRejectBlock)reject {
  [SuperBoardWrapperSwift.shared logCustomPurchaseWithType:type
...
{
    return std::make_shared<facebook::react::NativeSuperBoardWrapperSpecJSI>(params);
}
