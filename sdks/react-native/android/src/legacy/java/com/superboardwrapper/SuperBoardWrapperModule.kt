...
import io.superboard.utils.flow
...
class SuperBoardWrapperModule(private val reactContext: ReactApplicationContext) :
  ...
  override fun getName(): String = "SuperBoardWrapper"
  ...
  fun addDeeplinkListener() {
    ...
    activity?.let {
      SuperBoard.setOnDeeplinkReceivedListener(it) { details ->
        val writableMap = Arguments.createMap()
        writableMap.putString("superboard_link", details.link)
  ...
  fun setAttributes(attributes: ReadableMap?) {
    SuperBoard.attributes = attributes?.toMap()?.toSerializableMap()
  }
  ...
  fun setDebug(level: String) {
    when (level) {
      "info" -> SuperBoard.setDebug(io.superboard.model.LogLevel.INFO)
      "error" -> SuperBoard.setDebug(io.superboard.model.LogLevel.ERROR)
    }
  }
  ...
  fun numberOfUnreadMessages(promise: Promise) {
    SuperBoard.numberOfUnreadMessages {
  ...
  fun logInAppPurchase(transactionId: String?, promise: Promise) {
    ...
    SuperBoard.logInAppPurchase(originalJson = transactionId)
        ...
        SuperBoard.Companion::openedLinkDetails.flow.collect { deeplinkDetails ->
          deeplinkDetails?.let {
            ...
            reactContext
              ...
              .emit("onSuperBoardDeeplinkReceived", writableMap)
            //emitOnDeeplinkReceived(writableMap)
          }
        }
      }
  ...
  private fun emitOnDeeplinkReceived(data: WritableMap) {
    reactContext
      .getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter::class.java)
      .emit("onSuperBoardDeeplinkReceived", data)
  }
}
