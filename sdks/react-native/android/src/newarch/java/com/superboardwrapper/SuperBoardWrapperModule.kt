...
@ReactModule(name = SuperBoardWrapperModule.NAME)
class SuperBoardWrapperModule(reactContext: ReactApplicationContext) :
  NativeSuperBoardWrapperSpec(reactContext) {
  ...
  override fun setDebug(level: String) {
    if (level == "info") {
      SuperBoard.setDebug(level = io.superboard.model.LogLevel.INFO)
    } else if (level == "error") {
      SuperBoard.setDebug(level = io.superboard.model.LogLevel.ERROR)
    }
  }
  ...
  override fun numberOfUnreadMessages(promise: Promise) {
    SuperBoard.numberOfUnreadMessages(onResult = {
      it?.let {
        promise.resolve(it)
      } ?: run {
  ...
  companion object {
    const val NAME = "SuperBoardWrapper"
  }
}
