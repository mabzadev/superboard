...
@ReactModule(name = OpenGrowWrapperModule.NAME)
class OpenGrowWrapperModule(reactContext: ReactApplicationContext) :
  NativeOpenGrowWrapperSpec(reactContext) {
  ...
  override fun setDebug(level: String) {
    if (level == "info") {
      OpenGrow.setDebug(level = io.opengrow.model.LogLevel.INFO)
    } else if (level == "error") {
      OpenGrow.setDebug(level = io.opengrow.model.LogLevel.ERROR)
    }
  }
  ...
  override fun numberOfUnreadMessages(promise: Promise) {
    OpenGrow.numberOfUnreadMessages(onResult = {
      it?.let {
        promise.resolve(it)
      } ?: run {
  ...
  companion object {
    const val NAME = "OpenGrowWrapper"
  }
}
