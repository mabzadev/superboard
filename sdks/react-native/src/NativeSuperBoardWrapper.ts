...
const NativeModule = TurboModuleRegistry.get<Spec>("SuperBoardWrapper");

if (!NativeModule) {
	log(
		"info",
		"Turbo module - Native module @mbzadev/superboard-react-native-sdk is not linked properly.",
	);
	throw new Error("Native module @mbzadev/superboard-react-native-sdk is not linked properly.");
}

export class TurboModuleSuperBoard {
	// Simple methods
	...
	async generateLink(
		...
		if (!NativeModule) {
			throw new Error("Native module SuperBoardWrapper is not linked");
		}
	...
	async logCustomPurchase(
		...
		return NativeModule.logCustomPurchase(type, priceInCents, currency, productId, startDate);
	}
	...
	onDeeplinkReceived(callback: (data: DeeplinkResponse) => void): {
		...
		const subscription = eventEmitter?.addListener("onSuperBoardDeeplinkReceived", callback);
...
export default new TurboModuleSuperBoard();
