...
import { Text, View, StyleSheet, TouchableOpacity, Button, Clipboard } from "react-native";

SuperBoard.setIdentifier("React native id");
...
export default function App() {
	useEffect(() => {
		...
		const listener = SuperBoard.onDeeplinkReceived((data) => {
	...
	async function handleLogInAppPurchase() {
		try {
			const success = await SuperBoard.logInAppPurchase("123456789");
	...
	async function handleLogCustomPurchase() {
		try {
			const success = await SuperBoard.logCustomPurchase("buy", 999, "USD", "premium_monthly");
	...
	async function fetchUnreadMessages() {
		try {
			const unreadCount = await SuperBoard.numberOfUnreadMessages();
