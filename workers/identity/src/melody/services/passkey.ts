...
import { AuthCodeBody, EmbeddedSessionBodyWithUser } from "../configs/type";
...
export const createUserPasskey = async (
	...
	const isCreated = await userPasskeyModel.create(c.env.DB, {
		userId,
		credentialId,
		publicKey,
		counter,
...
export const getPasskeyByUser = async (c: Context<typeConfig.Context>, userId: number) => {
...
export const deletePasskey = async (
	...
	return userPasskeyModel.remove(c.env.DB, passkeyId);
};
...
export const genPasskeyEnrollOptions = async (
	...
	const registrationOptions = await generateRegistrationOptions({
		...
		userID: Uint8Array.from(new TextEncoder().encode(String(authCodeStore.user.id))),
	...
	await kvService.setPasskeyEnrollChallenge(c.env.KV, authCodeStore.user.id, challenge);
...
export const processPasskeyEnroll = async (
	...
	const challenge = await kvService.getPasskeyEnrollChallenge(c.env.KV, authCodeStore.user.id);
	...
	const { AUTH_SERVER_URL: authServerUrl, EMBEDDED_AUTH_ORIGINS: embeddedAuthOrigins } = env(c);
...
export const processPasskeyVerify = async (
	...
	const isValidChallenge = await kvService.verifyPasskeyVerifyChallenge(c.env.KV, challenge);

	if (!isValidChallenge) {
		throw new errorConfig.UnAuthorized(messageConfig.RequestError.InvalidPasskeyVerifyRequest);
	}

	const passkey = await userPasskeyModel.getByCredentialId(c.env.DB, passkeyInfo.rawId);

	const user = passkey?.userId ? await userModel.getById(c.env.DB, passkey.userId) : null;
