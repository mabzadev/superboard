...
const trustInternalAdmin = (
  c: Context<typeConfig.Context>,
): boolean => {
  if (c.env.MELODY_INTERNAL_ADMIN !== true) return false
  c.set(
    'access_token_body',
    {
      sub: `superboard-admin:${c.env.MELODY_PROJECT_ID ?? 'control-plane'}`,
      azp: 'superboard-internal-admin',
      iss: c.env.AUTH_SERVER_URL,
...
export const spa = bearerAuth({
  verifyToken: async (
    ...
    if (trustInternalAdmin(c)) return true
