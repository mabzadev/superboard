...
import { routeConfig, typeConfig, variableConfig } from "../../configs";
import { GetProcessSmsMfaRes } from "../../handlers/identity/mfa";
import { codeField, validate } from "../tools/form";
import { validateError } from "../tools/locale";
import { getFollowUpParams } from "../tools/param";
import { handleAuthorizeStep, parseAuthorizeFollowUpValues, parseResponse } from "../tools/request";
...
const useSmsMfaForm = ({ locale, onSubmitError, onSwitchView }: UseSmsMfaFormProps) => {
	const followUpParams = useMemo(() => getFollowUpParams(), []);
	...
	const smsMfaSchema = object({
		phoneNumber: !currentNumber
			? string().test("is-valid-phone", validateError.wrongPhoneFormat[locale], function (value) {
			...
			: string(),
		mfaCode: mfaCode !== null ? codeField(locale) : array().nullable(),
	});

	const errors = validate(smsMfaSchema, values);

	const handleChange = (
		name: "phoneNumber" | "mfaCode" | "rememberDevice",
	...
	const getSmsMfaInfo = useCallback(() => {
		fetch(`${routeConfig.IdentityRoute.ProcessSmsMfa}${qs}`, {
			method: "GET",
			headers: {
			...
			.then((response) => {
				...
				if ((response as GetProcessSmsMfaRes).phoneNumber) {
					setMfaCode(Array.from({ length: 6 }, () => ""));
				}
	...
	const requestSetupMfa = useCallback(() => {
		...
		return fetch(routeConfig.IdentityRoute.SetupSmsMfa, {
			method: "POST",
			headers: {
				Accept: "application/json",
				"Content-Type": "application/json",
			...
			body: JSON.stringify({
				...parseAuthorizeFollowUpValues(followUpParams, locale),
	...
	const handleSubmit = useCallback(
		(e: Event) => {
			...
			if (mfaCode === null) {
				...
				fetch(routeConfig.IdentityRoute.ProcessSmsMfa, {
					method: "POST",
					headers: {
						Accept: "application/json",
						"Content-Type": "application/json",
					...
					.then(parseResponse)
					.then((response) => {
						handleAuthorizeStep(response, locale, onSwitchView);
					})
					.catch((error) => {
						onSubmitError(error);
					})
