...
operations.get('/:projectId/contacts', async (c) => {
  ...
  const rows = await c.env.DB.prepare(`
    ...
    LEFT JOIN support_companies company ON company.id = contact.company_id
...
operations.get('/:projectId/contacts/:contactId/notes', async (c) => {
  ...
  const rows = await c.env.DB.prepare(`
    SELECT * FROM support_contact_notes WHERE project_id = ? AND contact_id = ? ORDER BY created_at DESC
...
operations.post('/:projectId/contacts/:contactId/notes', async (c) => {
  ...
  const id = crypto.randomUUID();
  await c.env.DB.prepare(`
    INSERT INTO support_contact_notes (id, project_id, contact_id, content, created_by) VALUES (?, ?, ?, ?, ?)
  ...
  return c.json({ data: await c.env.DB.prepare('SELECT * FROM support_contact_notes WHERE id = ?').bind(id).first() }, 201);
});
...
operations.delete('/:projectId/contacts/:contactId/notes/:noteId', async (c) => {
  ...
  const row = await c.env.DB.prepare(`
    DELETE FROM support_contact_notes WHERE id = ? AND contact_id = ? AND project_id = ? RETURNING id
...
operations.get('/:projectId/companies', async (c) => {
  const projectId = positiveInt(c.req.param('projectId'));
  const rows = await c.env.DB.prepare(`
    SELECT company.*, (SELECT COUNT(*) FROM support_contacts contact WHERE contact.company_id = company.id) AS contact_count
    FROM support_companies company WHERE project_id = ? ORDER BY name LIMIT 250
...
operations.get('/:projectId/conversations/:conversationId/participants', async (c) => {
  ...
  const rows = await c.env.DB.prepare(`
    SELECT * FROM support_conversation_participants WHERE project_id = ? AND conversation_id = ? ORDER BY created_at
...
operations.get('/:projectId/conversations/:conversationId/draft', async (c) => {
  ...
  await conversation(c.env.DB, projectId, c.req.param('conversationId'));
  const row = await c.env.DB.prepare(`
    SELECT * FROM support_conversation_drafts WHERE conversation_id = ? AND agent_id = ?
...
operations.get('/:projectId/notifications', async (c) => {
  ...
  const rows = await c.env.DB.prepare(`
    SELECT * FROM support_agent_notifications
...
operations.get('/:projectId/search', async (c) => {
  ...
  const [contacts, conversations, messages] = await Promise.all([
    c.env.DB.prepare(`SELECT id, external_user_id, name, email, phone FROM support_contacts
...
async function audit(db: D1Database, projectId: number, type: string, id: string, action: string, actorId: string, payload: unknown) {
  await db.prepare(`INSERT INTO support_operations_audit_events
