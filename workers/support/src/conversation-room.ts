...
export class ConversationRoom extends DurableObject<Env> {
  ...
  private async createMessage(request: Request): Promise<Response> {
    ...
    if (input.attachment_key) {
      ...
      if (
        !attachment ||
        attachment.customMetadata?.conversationId !== conversation.id
      ) {
        return Response.json(
          {
      ...
      const statements = [
        ...(input.attachment_key
          ...
          INSERT OR IGNORE INTO support_message_attachments (
            ...
            content_type, position, source_provider, source_id
          ) VALUES (?, ?, ?, ?, ?, ?, ?, 0, 'opengrow', ?)
        `,
              ).bind(
                `${id}:attachment:0`,
                ...
                id,
                input.attachment_key,
                input.attachment_name || "attachment",
                input.attachment_content_type || "application/octet-stream",
                id,
              ),
            ]
          : []),
    ...
    return Response.json({ data: persisted, duplicate: true });
  }
