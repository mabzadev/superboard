...
export async function issueRealtimeTicket(
  ...
  await env.DB.prepare(`
    INSERT INTO support_realtime_tickets
      (token_hash, project_id, conversation_id, actor_id, actor_kind, expires_at)
    VALUES (?, ?, ?, ?, ?, ?)
  `).bind(await sha256(ticket), projectId, conversationId, actorId, actorKind,
    new Date(payload.expiresAt * 1000).toISOString()).run();
...
export async function consumeRealtimeTicket(
  ...
  const consumed = await env.DB.prepare(`
    ...
    RETURNING project_id, conversation_id, actor_id, actor_kind, expires_at
  `).bind(await sha256(ticket), payload.projectId, payload.conversationId, payload.actorId).first<{
    project_id: number; conversation_id: string; actor_id: string;
    actor_kind: 'user' | 'agent'; expires_at: string;
  }>();
...
function validPayload(value: unknown): value is TicketPayload {
  ...
  return Number.isSafeInteger(payload.projectId) && Number(payload.projectId) > 0
    ...
    && (payload.actorKind === 'user' || payload.actorKind === 'agent')
