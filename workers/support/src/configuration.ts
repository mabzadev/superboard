...
export const configurationCatalog = {
  ...
  automation_rule: definition('Automation rule', 'Conditions and actions executed for a Support event. Entitlements are never valid actions.', [
...
configuration.put('/:projectId/settings/entities/:entityId/secret', async (c) => {
  ...
  if (entity.entity_type !== 'webhook') throw failure('webhook_required', 'Signing secrets can be configured only for webhooks');
  const body = await readJsonObject(c.req.raw);
  const secret = requiredText(body.secret, 'secret', 4096);
  if (secret.length < 16) throw failure('webhook_secret_invalid', 'Webhook signing secrets must contain at least 16 characters');
  const existing = await c.env.DB.prepare(`
    SELECT secret_version FROM support_webhook_secrets WHERE project_id = ? AND webhook_id = ?
  `).bind(projectId, c.req.param('entityId')).first<{ secret_version: number }>();
  ...
  await c.env.DB.batch([
    ...
    c.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).bind(crypto.randomUUID(), projectId, c.req.param('entityId'), existing ? 'rotated' : 'configured', version,
      actorId, requiredRequestId(c.req.header('X-Request-Id'))),
  ]);
  return c.json({ data: { webhook_id: c.req.param('entityId'), secret_configured: true, secret_version: version } });
});
...
configuration.delete('/:projectId/settings/entities/:entityId/secret', async (c) => {
  ...
  `).bind(projectId, c.req.param('entityId')).first<{ secret_version: number }>();
  if (!deleted) throw failure('webhook_secret_not_found', 'Webhook signing secret is not configured', 404);
  await c.env.DB.prepare(`
    INSERT INTO support_secret_audit_events (id, project_id, webhook_id, action, secret_version, actor_id, request_id)
    VALUES (?, ?, ?, 'revoked', ?, ?, ?)
  `).bind(crypto.randomUUID(), projectId, c.req.param('entityId'), deleted.secret_version,
    actorId, requiredRequestId(c.req.header('X-Request-Id'))).run();
...
configuration.delete('/:projectId/settings/entities/:entityId', async (c) => {
  ...
  const actorId = requiredActor(c.req.header('X-OpenGrow-Agent-Id'));
  const before = await entityById(c.env.DB, projectId, c.req.param('entityId'));
  await c.env.DB.prepare('DELETE FROM support_configuration_entities WHERE project_id = ? AND id = ?')
...
async function getProjectSettings(db: D1Database, projectId: number) {
  const row = await db.prepare('SELECT * FROM support_project_settings WHERE project_id = ?')
