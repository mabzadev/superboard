...
export function requireProject(_env: Env, value: string | undefined): number {
  const projectId = Number(value);
  if (!Number.isInteger(projectId) || projectId <= 0) {
    throw publicError('project_id_invalid', 'A valid project id is required', 422);
  }
  const allowed = new Set(
    String(_env.ALLOWED_PROJECT_IDS || '')
      .split(',')
      .map((item) => Number(item.trim()))
      .filter((item) => Number.isInteger(item) && item > 0),
  );
  if (!allowed.has(projectId)) {
    throw publicError('project_not_allowed', 'This project is not available through the public Support API', 403);
  }
  return projectId;
}
...
export async function verifyInternalProjectContext(
  request: Request,
  env: Env,
  moduleName: 'support',
  ...
  if (!verified.ok) throw publicError(verified.code, verified.message, 401);
