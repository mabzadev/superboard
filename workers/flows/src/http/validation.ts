...
export function requiredString(value: unknown, field: string, maximum = 255): string {
...
export function optionalString(value: unknown, field: string, maximum = 2_000): string | null {
...
export function propertyKey(value: unknown, field: string): string {
	const result = requiredString(value, field, 128);
	if (!/^[A-Za-z][A-Za-z0-9_-]*$/u.test(result)) {
		throw failure(
			"validation_failed",
			`${field} must start with a letter and contain letters, numbers, underscores or hyphens`,
			422,
			{ field },
		);
	}
...
export function parseFlowGraph(value: unknown): FlowGraph {
	if (!isRecord(value) || value.schemaVersion !== 1) {
		throw failure("flow_graph_invalid", "graph.schemaVersion must equal 1", 422);
	}
	if (!Array.isArray(value.blocks) || !Array.isArray(value.paths)) {
		throw failure("flow_graph_invalid", "graph blocks and paths must be arrays", 422);
	}
	...
	for (const path of paths) {
		if (!blockIds.has(path.sourceBlockId) || !blockIds.has(path.targetBlockId)) {
			throw failure("flow_graph_invalid", "Every path must reference existing blocks", 422, {
				path_id: path.id,
			});
		}
	}
...
function validateGraphTopology(
	...
	for (const block of blocks) {
		...
		if (block.type === "tour") {
			const steps = Array.isArray(block.data.steps)
				...
				: Array.isArray(block.data.screens)
					...
					: [];
			if (!steps.length) {
				throw failure("flow_graph_tour_empty", `Tour ${block.key} needs at least one step`, 422);
			}
		}
		if (
		...
		if (block.type === "traffic-split") {
			const variants = Array.isArray(block.data.variants) ? block.data.variants : [];
	...
	for (const path of paths) {
		...
		if (path.triggerOnly) {
			const triggers = source.propertyMeta.filter((property) => property.type === "block-trigger");
		...
		const implicitDefault =
			new Set(["start", "manual-start", "note", "delay", "workflow-trigger"]).has(source.type) &&
			path.sourceExitNode === "default";
		...
		if (source.type === "end") {
			throw failure("flow_graph_end_path_invalid", "End blocks cannot have outgoing paths", 422);
		}
	}
	for (const block of blocks.filter((entry) => entry.type === "delay")) {
		const outgoing = paths.filter((path) => path.sourceBlockId === block.id && !path.triggerOnly);
	...
	assertAcyclic(
		blocks,
		paths.filter((path) => !path.triggerOnly),
	);
}

function assertAcyclic(blocks: readonly FlowEditorBlock[], paths: readonly FlowPath[]): void {
	...
	if (blocks.some((block) => !visit(block.id))) {
		throw failure("flow_graph_cycle_invalid", "Workflow paths must not contain a cycle", 422);
	}
}
...
function parseBlock(value: unknown): FlowEditorBlock {
	...
	const propertyMeta = Array.isArray(value.propertyMeta)
		? value.propertyMeta.map((property): FlowPropertyMeta => {
				if (!isRecord(property) || !isFlowPropertyType(property.type)) {
					throw failure("flow_graph_invalid", "Block property type is invalid", 422);
				}
				const triggers =
					property.triggers == null ? property.triggers : parsePropertyTriggers(property.triggers);
				return {
					key: propertyKey(property.key, "property.key"),
					type: property.type,
	...
	const block: FlowEditorBlock = {
		...
		componentType: optionalString(value.componentType, "block.componentType", 128) ?? null,
		componentLibraryName:
			optionalString(value.componentLibraryName, "block.componentLibraryName", 128) ?? null,
		...
		conditions: value.conditions == null ? undefined : parseTargetConditions(value.conditions),
		slottable: Boolean(value.slottable),
		slotId: optionalString(value.slotId, "block.slotId", 128),
		slotIndex:
			value.slotIndex == null ? null : integer(value.slotIndex, "block.slotIndex", 0, 10_000),
	...
	if (value.tourWait !== undefined) {
		block.tourWait = value.tourWait === null ? null : parseTourWait(value.tourWait);
	}
	if (value.tourTrigger !== undefined) {
		block.tourTrigger = value.tourTrigger === null ? null : parseTourTrigger(value.tourTrigger);
	}
...
function validateSurveyQuestions(questions: readonly FlowSurveyQuestion[]): void {
...
function parseTourWait(value: unknown): FlowTourWait {
	...
	const interaction =
		wait.interaction == null
			? undefined
			: requiredString(wait.interaction, "tourWait.interaction", 32);
	if (
		interaction &&
		!["navigation", "click", "delay", "dom-element", "not-dom-element"].includes(interaction)
	) {
	...
	return {
		...
		...(wait.ms == null ? {} : { ms: integer(wait.ms, "tourWait.ms", 0, FLOW_MAX_DELAY_MS) }),
	};
}
...
function parseSurveyQuestion(value: unknown): FlowSurveyQuestion {
	...
	if (
		!["freeform", "rating", "single-choice", "multiple-choice", "link", "end-screen"].includes(type)
	) {
		throw failure("flow_graph_invalid", "Survey question type is invalid", 422);
	}
	const displayType =
		question.displayType == null
			? null
	...
	const options = question.options == null ? null : parseSurveyOptions(question.options);
	return {
...
function parsePath(value: unknown): FlowPath {
	...
	return {
		...
		sourceExitNode: identifier(value.sourceExitNode ?? "default", "path.sourceExitNode"),
...
export function parseSdkBlocksRequest(value: unknown): FlowSdkBlocksRequest {
	...
	return {
		...
		language: optionalString(body.language, "language", 64) ?? undefined,
	};
}
...
export function parseSdkEvent(value: unknown): FlowSdkEvent {
	...
	return {
		...
		workflowVersionId:
			optionalString(body.workflowVersionId, "workflowVersionId", 192) ?? undefined,
		blockId: optionalString(body.blockId, "blockId", 192) ?? undefined,
		blockStateId: optionalString(body.blockStateId, "blockStateId", 512) ?? undefined,
		blockKey: optionalString(body.blockKey, "blockKey", 128) ?? undefined,
		propertyKey: optionalString(body.propertyKey, "propertyKey", 128) ?? undefined,
	...
	return {
		...
		questions: body.questions.map((entry) => {
			...
			return {
				...
				textResponse: optionalString(question.textResponse, "textResponse", 20_000) ?? undefined,
