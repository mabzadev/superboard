...
import { wakePluginWorkflow } from "./plugin-task-wake";
import {
	createEnvironment,
	ensureFlowProject,
	getFlowProject,
...
import { sdkBlocks, sdkEvent, sdkSurvey, sdkWebSocket, sdkWorkflows } from "./sdk/service";
...
app.post("/internal/system/plugin-task-wake", (context) =>
	wakePluginWorkflow(context.req.raw, context.env),
);

for (const prefix of ["", "/api/v1/flows"] as const) {
	app.post(`${prefix}/v2/sdk/blocks`, (context) => sdkBlocks(context.req.raw, context.env));
	app.post(`${prefix}/v2/sdk/workflows`, (context) => sdkWorkflows(context.req.raw, context.env));
	app.post(`${prefix}/v2/sdk/events`, (context) => sdkEvent(context.req.raw, context.env));
	app.post(`${prefix}/v2/sdk/survey`, (context) => sdkSurvey(context.req.raw, context.env));
...
app.all("/internal/v1/legacy/paywalls", (context) => legacyRequest(context, "paywalls"));
app.all("/internal/v1/legacy/paywalls/*", (context) => legacyRequest(context, "paywalls"));
app.all("/internal/v1/legacy/onboardings", (context) => legacyRequest(context, "onboardings"));
app.all("/internal/v1/legacy/onboardings/*", (context) => legacyRequest(context, "onboardings"));
...
app.get(`${base}/project`, async (context) => data(await getFlowProject(context)));
	...
	return data({
		items: await Promise.all(
			userIds.map(async (userId) => ({
				user_id: userId,
				user_id_hash: await hashFlowUserId(context.env, project.projectRef, userId),
			})),
		),
	});
});

app.get(`${base}/overview`, async (context) => data(await flowOverview(context)));

app.get(`${base}/environments`, async (context) => data(await listEnvironments(context)));
app.post(`${base}/environments`, async (context) =>
	data(await createEnvironment(context, await body(context)), 201),
);
app.post(`${base}/environments/:environmentId/rotate-key`, async (context) =>
	data(await rotateEnvironmentKey(context, context.req.param("environmentId"))),
);
app.get(`${base}/localization`, async (context) => data(await listLanguageGroups(context)));
...
app.post(`${base}/localization`, async (context) =>
	data(await saveLanguageGroup(context, await body(context)), 201),
);

const projectResource = base;

app.get(`${projectResource}/workflows`, async (context) => data(await listWorkflows(context)));
app.post(`${projectResource}/workflows`, async (context) =>
	data(await createWorkflow(context, await body(context)), 201),
);
const workflow = `${projectResource}/workflows/:workflowId`;
app.get(workflow, async (context) =>
	data(await getWorkflow(context, context.req.param("workflowId"))),
);
app.patch(workflow, async (context) =>
	data(await updateWorkflow(context, context.req.param("workflowId"), await body(context))),
);
app.put(workflow, async (context) =>
	data(await updateWorkflow(context, context.req.param("workflowId"), await body(context))),
);
app.put(`${workflow}/draft`, async (context) =>
	data(await saveDraft(context, context.req.param("workflowId"), await body(context))),
);
app.post(`${workflow}/publish`, async (context) =>
	data(await publishWorkflow(context, context.req.param("workflowId"), await body(context)), 201),
);
app.get(`${workflow}/versions/:versionId`, async (context) =>
	data(await getVersion(context, context.req.param("workflowId"), context.req.param("versionId"))),
);
app.post(`${workflow}/duplicate`, async (context) =>
	data(await duplicateWorkflow(context, context.req.param("workflowId"), await body(context)), 201),
);
app.put(`${workflow}/translations`, async (context) =>
	data(await saveTranslations(context, context.req.param("workflowId"), await body(context))),
);
app.post(`${workflow}/releases`, async (context) =>
	data(await activateWorkflow(context, context.req.param("workflowId"), await body(context))),
);
app.delete(`${workflow}/releases/:environmentId`, async (context) => {
...
app.get(`${workflow}/analytics`, async (context) =>
	data(await workflowAnalytics(context, context.req.param("workflowId"))),
);

app.get(`${projectResource}/components`, async (context) => data(await listComponents(context)));
app.post(`${projectResource}/component-libraries`, async (context) =>
	data(await createLibrary(context, await body(context)), 201),
);
app.patch(`${projectResource}/component-libraries/:libraryId`, async (context) =>
	data(await updateLibrary(context, context.req.param("libraryId"), await body(context))),
);
app.post(`${projectResource}/components`, async (context) =>
	data(await createComponent(context, await body(context)), 201),
);
app.patch(`${projectResource}/components/:componentId`, async (context) =>
	data(await updateComponent(context, context.req.param("componentId"), await body(context))),
);
app.post(`${projectResource}/components/:componentId/sync`, async (context) =>
	data(await syncComponentInstances(context, context.req.param("componentId"))),
);

app.get(`${projectResource}/launchpad`, async (context) => data(await getLaunchpad(context)));
app.post(`${projectResource}/launchpad/groups`, async (context) =>
	data(await createLaunchpadGroup(context, await body(context)), 201),
);
app.patch(`${projectResource}/launchpad/groups/:groupId`, async (context) =>
	data(await updateLaunchpadGroup(context, context.req.param("groupId"), await body(context))),
);
app.put(`${projectResource}/launchpad/groups/:groupId/workflows`, async (context) =>
	data(await setLaunchpadWorkflows(context, context.req.param("groupId"), await body(context))),
);
app.delete(`${projectResource}/launchpad/groups/:groupId`, async (context) => {
	await deleteLaunchpadGroup(context, context.req.param("groupId"));
	return empty();
});

app.get(`${projectResource}/users`, async (context) => data(await listUsers(context)));
app.get(`${projectResource}/users/:userHash`, async (context) =>
	data(await getUser(context, context.req.param("userHash"))),
);
app.post(`${projectResource}/users/:userHash/reset`, async (context) =>
	data(await resetUserWorkflow(context, context.req.param("userHash"), await body(context))),
);
app.get(`${projectResource}/surveys/:surveyId/analytics`, async (context) =>
	data(await surveyAnalytics(context, context.req.param("surveyId"))),
);
app.post(`${projectResource}/surveys/:surveyId/export`, async (context) =>
	data(await exportSurveyCsv(context, context.req.param("surveyId")), 202),
);
		...
		context.waitUntil(
			Promise.all(
				projectRows.results.flatMap((row) =>
					(["purge", "rebuild-rollups"] as const).map(async (operation) => {
						const businessId = [row.project_id, controller.scheduledTime, operation].join(":");
						const id = await flowWorkflowInstanceId("flows-maintenance", businessId);
						try {
							...
							if (
								!/already exists|instance.*exists/iu.test(
...
export default withPluginTaskLifecycle("supbrd-plugmod-flows", worker);
