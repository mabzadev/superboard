...
export async function legacyRequest(context: LegacyContext, source: Source): Promise<Response> {
...
async function routeLegacyRequest(context: LegacyContext, source: Source): Promise<Response> {
	...
	if (method === "POST" && (path === "/" || path === "/paywalls")) {
		return legacyData(
			await createLegacyWorkflow(context, source, await readJsonObject(context.req.raw)),
			201,
		);
	}
	if (path === "/resolve" || path === "/placements/resolve") {
		const request = await readJsonObject(context.req.raw);
		const resolved = await resolveLegacyPlacement(context, source, request);
		if (source === "paywalls") {
			return Response.json(
				{
					data: resolved,
	...
	if (path === "/events" && method === "POST") {
		return legacyData(
			await ingestLegacyEvents(context, source, await readJsonObject(context.req.raw, 2_359_296)),
			202,
		);
	}
	...
	const onboardingPublish = source === "onboardings" ? /^\/([^/]+)\/publish$/u.exec(path) : null;
	...
	const workflowMatch =
		source === "paywalls" ? /^\/paywalls\/([^/]+)$/u.exec(path) : /^\/([^/]+)$/u.exec(path);
	if (workflowMatch) {
		return legacyWorkflowById(context, source, workflowMatch[1]!, method);
	}
	throw failure(
		source === "paywalls" ? "paywalls_route_not_found" : "onboardings_route_not_found",
		`${source === "paywalls" ? "Paywalls" : "Onboardings"} route was not found`,
		404,
	);
}

async function listLegacyWorkflows(context: LegacyContext, source: Source) {
	const rows = await context.env.DB.prepare(
     ...
     ORDER BY w.updated_at DESC`,
		...
		.bind(source, source, source, context.get("project").projectId, source)
		.all<Row>();
	return rows.results.map((row) => legacyWorkflowView(row, source));
}

async function createLegacyWorkflow(context: LegacyContext, source: Source, body: Row) {
...
async function legacyWorkflowById(
	...
	if (method === "PUT" || method === "PATCH") {
       ...
       WHERE project_id = ? AND id = ? AND origin = ?`,
			...
			.bind(name, description, now, context.get("project").projectId, workflowId, source)
			.run();
	...
	if (method === "DELETE") {
       ...
       WHERE project_id = ? AND id = ? AND origin = ?`,
			...
			.bind(now, now, context.get("project").projectId, workflowId, source)
			.run();
...
async function legacyVersions(
	...
	if (method === "POST" && versionId && action === "publish") {
		versionId = String((await ownLegacyVersion(context, source, workflowId, versionId)).id);
		return legacyData(await publishLegacyVersion(context, source, workflowId, versionId));
	}
	if (method === "POST" && versionId && action === "archive") {
		const version = await ownLegacyVersion(context, source, workflowId, versionId);
       ...
       WHERE EXISTS (
         ...
         WHERE v.project_id = ?
           ...
           AND v.active = 1 AND v.legacy_version_id = ?
			...
			.bind(projectId, source, versionId, projectId, source, versionId)
...
async function createLegacyVersionRecord(
	...
	const current = await context.env.DB.prepare(
     ...
     WHERE project_id = ? AND workflow_id = ?`,
		...
		.bind(project.projectId, workflowId)
...
async function publishLegacyVersion(
	...
	workflowId = String((await ownWorkflow(context, source, workflowId)).id);
	legacyVersionId = String(
		(await ownLegacyVersion(context, source, workflowId, legacyVersionId)).id,
	);
		...
		const result =
           ...
           WHERE id = ? AND project_id = ?
             AND source_module = ?`,
						...
						.bind(new Date().toISOString(), placementId, projectId, source)
						.run()
				: await context.env.DB.prepare(
						`DELETE FROM flow_legacy_placements WHERE id = ? AND project_id = ? AND source_module = ?`,
					)
						.bind(placementId, projectId, source)
						.run();
		if (!result.meta.changes) throw failure("placement_not_found", "Placement not found", 404);
		await synchronizeLegacyWorkflow(context, source, String(existingPlacement!.workflow_id));
...
async function legacyExperiences(
	...
	if ((method === "POST" && !id) || ((method === "PUT" || method === "PATCH") && id)) {
		...
		const experimentId = existingExperiment ? String(existingExperiment.id) : crypto.randomUUID();
		...
		await context.env.DB.batch([
         ...
         WHERE flow_legacy_experiments.project_id = excluded.project_id`,
				...
				trafficPercent,
				trafficBasisPoints,
				optionalText(body.starts_at),
				optionalText(body.ends_at),
				now,
				now,
			),
			context.env.DB.prepare(
	...
	if (method === "GET" && id) {
       ...
       WHERE v.project_id = ? AND v.experiment_id = ? ORDER BY v.key`,
			...
			.bind(projectId, existingExperiment!.id)
			.all<Row>();
		return legacyData({
			...legacyExperienceView(row, source),
	...
	if (method === "POST" && id && match?.[2] === "status") {
       ...
       WHERE id = ? AND project_id = ? AND source_module = ?`,
			...
			.bind(status, new Date().toISOString(), existingExperiment!.id, projectId, source)
			.run();
		if (!result.meta.changes) throw failure("experience_not_found", "Experience not found", 404);
		await synchronizeLegacyWorkflow(context, source, String(existingExperiment!.workflow_id));
	...
	if (method === "DELETE" && id) {
       ...
       WHERE id = ? AND project_id = ? AND source_module = ?`,
			...
			.bind(new Date().toISOString(), existingExperiment!.id, projectId, source)
			.run();
		if (!result.meta.changes) throw failure("experience_not_found", "Experience not found", 404);
		await synchronizeLegacyWorkflow(context, source, String(existingExperiment!.workflow_id));
	...
	if (method === "POST" && !id) {
       ...
       VALUES (?, ?, ?, 'onboardings', ?, ?, ?, ?, ?, ?)`,
			...
			.bind(
				...
				projectId,
				placementId,
				JSON.stringify(record(body.conditions)),
				integer(body.priority, 100, 0, 10_000),
				boolean(body.active, true) ? 1 : 0,
				now,
				now,
			)
			.run();
		await synchronizeLegacyWorkflow(context, "onboardings", String(placement.workflow_id));
	...
	if (method === "DELETE" && id) {
       ...
       WHERE (id = ? OR legacy_id = ?) AND project_id = ?
		...
		if (!result.meta.changes)
			throw failure("targeting_rule_not_found", "Targeting rule not found", 404);
		const placement = await context.env.DB.prepare(
			`SELECT workflow_id FROM flow_legacy_placements
       WHERE id = ? AND project_id = ?`,
		)
			.bind(existing.placement_id, projectId)
			.first<{ workflow_id: string }>();
		if (placement) {
			await synchronizeLegacyWorkflow(context, "onboardings", placement.workflow_id);
		}
...
async function resolveLegacyPlacement(context: LegacyContext, source: Source, body: Row) {
	const project = context.get("project");
	...
	const candidates = await context.env.DB.prepare(
     ...
     ORDER BY p.priority DESC, p.updated_at DESC`,
		...
		.bind(project.projectId, source, placementKey)
		.all<Row>();
	...
	for (const placement of candidates.results) {
		if (
			!matchesTargeting(
				legacyTargetingToConditions(parseRecord(String(placement.targeting_json))),
				properties,
			)
		)
			continue;
		if (source === "onboardings" && !(await matchesLegacyRules(context, placement, properties)))
			continue;
...
async function selectLegacyVersion(
	...
	if (experienceId) {
        ...
        AND (starts_at IS NULL OR starts_at <= ?) AND (ends_at IS NULL OR ends_at > ?)`,
			...
			.bind(experienceId, context.get("project").projectId, source, now, now)
			.first<Row>();
		if (
			experience &&
         ...
         WHERE v.experiment_id = ? AND v.project_id = ?
           AND v.active = 1 AND l.status = 'published' ORDER BY v.key`,
				...
				.bind(experienceId, context.get("project").projectId)
				.all<{
					id: string;
	...
	const version = await context.env.DB.prepare(
     ...
     WHERE id = ? AND project_id = ?
       AND source_module = ? AND workflow_id = ? AND status = 'published'`,
		...
		.bind(versionId, context.get("project").projectId, source, placement.workflow_id)
...
async function ingestLegacyEvents(context: LegacyContext, source: Source, body: Row) {
	const project = context.get("project");
	if (body.project_id != null && String(body.project_id) !== String(project.projectId)) {
		throw failure("project_context_mismatch", "Project context mismatch", 403);
	}
	...
	for (const [index, raw] of events.entries()) {
		...
		const workflow = rawWorkflowId ? await ownWorkflow(context, source, rawWorkflowId) : null;
		const rawVersionId = optionalText(event.version_id);
		const version =
			rawVersionId && workflow
				? await ownLegacyVersion(context, source, String(workflow.id), rawVersionId)
				: null;
		const rawPlacementId = optionalText(event.placement_id);
		const placement = rawPlacementId ? await ownPlacement(context, source, rawPlacementId) : null;
		...
		queuedEvents.push({
			...
			...(workflow ? { workflowId: String(workflow.id) } : {}),
			...(version?.flow_version_id ? { workflowVersionId: String(version.flow_version_id) } : {}),
			...(canonicalBlockId ? { blockId: canonicalBlockId } : {}),
			properties: {
				legacy: {
					source,
					type,
	...
	for (const queued of queuedEvents) {
       ...
       WHERE source_event_id = ? AND source_module = ?
         AND project_id = ?`,
			...
			.bind(queued.sourceEventId, source, project.projectId)
			.first<{ present: number }>();
		if (projected) {
			duplicates += 1;
			continue;
       ...
       VALUES (?, ?, ?, ?)`,
			...
			.bind(queued.sourceEventId, project.projectId, source, new Date().toISOString())
			.run();
...
async function legacyStatistics(context: LegacyContext, source: Source) {
	...
	const from =
		queryDateBoundary(url.searchParams.get("from"), false) ??
	...
	if (placementParameter) {
		const placement = await context.env.DB.prepare(
			...
			.bind(placementParameter, context.get("project").projectId, source)
			.first<{ key: string }>();
	...
	const rows = await context.env.DB.prepare(
     ...
     ORDER BY occurred_at`,
		...
		.bind(context.get("project").projectId, from.toISOString(), to.toISOString())
		.all<{
			event_id: string;
	...
	const filtered = rows.results
		.map((row) => ({ row, properties: parseRecord(row.properties_json) }))
		.filter(({ properties }) => record(properties.legacy).source === source)
		.filter(({ properties }) => {
			const legacy = record(properties.legacy);
			return (
				matchesOptional(url, "platform", legacy.platform) &&
...
async function matchesLegacyRules(
	...
	const rows = await context.env.DB.prepare(
     ...
     ORDER BY priority DESC`,
		...
		.bind(context.get("project").projectId, placement.id)
...
async function ownWorkflow(context: LegacyContext, source: Source, id: string): Promise<Row> {
	const row = await context.env.DB.prepare(
		`SELECT * FROM flow_workflows
     WHERE (id = ? OR legacy_id = ?) AND project_id = ? AND origin = ?`,
	)
		.bind(id, id, context.get("project").projectId, source)
		.first<Row>();
	if (!row) throw failure(`${singular(source)}_not_found`, `${singular(source)} not found`, 404);
	return row;
}

async function ownPlacement(context: LegacyContext, source: Source, id: string): Promise<Row> {
	const row = await context.env.DB.prepare(
...
async function ownExperiment(context: LegacyContext, source: Source, id: string): Promise<Row> {
	const row = await context.env.DB.prepare(
...
async function ownLegacyVersion(
	...
	const row = await context.env.DB.prepare(
      ...
      ${published ? "AND status = 'published'" : ""}`,
		...
		.bind(versionId, versionId, context.get("project").projectId, source, workflowId)
		.first<Row>();
...
function legacyExperienceView(row: Row, source: Source): Row {
	return {
		...
		...(source === "paywalls"
			...
			: {
					placement_id: row.placement_legacy_id ?? row.placement_id,
					traffic_percentage: Number(row.traffic_basis_points ?? Number(row.traffic_percent) * 100),
				}),
	};
}
...
async function synchronizeLegacyWorkflow(
	...
	const experiments = await context.env.DB.prepare(
     ...
     ORDER BY created_at DESC`,
		...
		.bind(project.projectId, source, workflowId)
		.all<Row>();
	const experimentById = new Map(experiments.results.map((row) => [String(row.id), row]));
	const variants = await context.env.DB.prepare(
	...
	for (const [index, placement] of placements.results.entries()) {
		...
		const starts = placementRules.length
			? placementRules.map((rule, ruleIndex) => ({
					...
					conditions: legacyTargetingToConditions(parseRecord(String(rule.conditions_json))),
			...
			: [
					{
						...
						conditions: legacyTargetingToConditions(parseRecord(String(placement.targeting_json))),
						rulePriority: null,
						y: 80 + index * 260,
					},
				];
		for (const start of starts) {
			blocks.push({
				...
				data: {
					...
					...(start.rulePriority == null ? {} : { legacy_rule_priority: start.rulePriority }),
				},
				propertyMeta: [],
				exitNodes: ["default"],
		...
		const experiment = placement.experience_id
			...
			: experiments.results.find((row) => String(row.placement_id ?? "") === String(placement.id));
		const activeComponentId = canonicalLegacyBlockId(source, workflowId, placementId, null);
		const addComponent = (
			id: string,
		...
		for (const [variantIndex, variant] of sourceVariants.entries()) {
			...
			const componentId = canonicalLegacyBlockId(source, workflowId, placementId, variantId);
			if (
				!addComponent(
					componentId,
...
function validateLegacyDefinition(source: Source, definition: Row): void {
	const entries = source === "paywalls" ? array(definition.components) : array(definition.screens);
...
async function auditLegacyResponse(
	...
	try {
		responseData = record((await readJsonObjectLimited(response, 1_048_576)).data);
	} catch {
		// A successful empty response is still audited with the route as entity.
	}
	const pathname = new URL(context.req.url).pathname;
	const entityId =
		optionalText(responseData.id) ?? context.req.header("idempotency-key") ?? pathname;
	await context.env.DB.prepare(
...
function legacyStatisticsBucket(value: string, interval: "hour" | "day", timezone: string): string {
	const date = new Date(value);
