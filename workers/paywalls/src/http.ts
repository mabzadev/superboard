...
export function internalAuth(): MiddlewareHandler<{
	...
	return async (c, next) => {
		...
		const context: ProjectContext = {
			...
			actorId: verified.context.operatorId ?? String(verified.context.actorId),
...
export async function commitMutation(
	...
	).bind(project.projectId, key, responseJson, timestamp, c.req.method, c.req.path, hash, status);
	...
	try {
		...
		if (concurrent) return replayResponse(concurrent, c.req.method, c.req.path, hash);
