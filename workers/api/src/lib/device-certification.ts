...
import { certificationRunCompatibility, RELEASE_GATE_CHECKS } from './purchases-release-gate';
...
export async function issueCertificationDeviceChallenge(
  ...
  const expiresAt = new Date(now.getTime() + DEVICE_CERTIFICATION_CHALLENGE_TTL_SECONDS * 1000).toISOString();
  await db.prepare(`
...
export async function recordDeviceCertificationResult(
  ...
  if (!input.challenge || input.challenge.length > 512) {
    throw purchasesError('device_challenge_invalid', 'A valid device certification challenge is required', 401);
  }
  ...
  const definition = RELEASE_GATE_CHECKS.find((item) => item.key === input.checkKey);
  if (!definition || !definition.reference_types.includes('test_run')) {
  ...
  if (input.outcome === 'passed') {
    const missing = requiredDeviceCertificationAssertions(input.checkKey)
  ...
  if (existing) {
    ...
    const same = existing.run_id === input.runId
    ...
    if (!same) {
      throw purchasesError('device_result_id_conflict', 'Device result ID is already used by different evidence', 409);
    }
  ...
  `).bind(input.runId, input.projectId).first<DeviceChallengeRow>();
  if (!run || run.status !== 'running') {
    throw purchasesError('device_certification_run_unavailable', 'Running device certification was not found', 404);
  }
  const compatibility = certificationRunCompatibility(definition, run.platform, run.environment);
  if (!compatibility.valid) {
    throw purchasesError('certification_run_incompatible', 'The certification run is not compatible with this check', 409);
  }
  validateSourcePlatform(input.checkKey, definition.provider, input.sourcePlatform);
  if (Date.parse(run.challenge_expires_at) <= now.getTime()) {
    throw purchasesError('device_challenge_expired', 'The device certification challenge has expired', 401);
  }
  if (await sha256Hex(input.challenge) !== run.challenge_hash) {
    throw purchasesError('device_challenge_invalid', 'The device certification challenge is invalid', 401);
  }
  if (run.claimed_customer_id && String(run.claimed_customer_id) !== input.customerId) {
    throw purchasesError('device_challenge_claimed', 'The device certification challenge is already claimed', 409);
  }
  ...
  const observedAt = normalizedObservedAt(input.observedAt, run.started_at, now);
  const evidence = {
    schema_version: 1,
    ...
    application_identifier: boundedText(input.applicationIdentifier, 'application identifier', 255),
  ...
  if (evidenceJson.length > 16_384) {
    throw purchasesError('device_certification_evidence_too_large', 'Device evidence is limited to 16 KB', 413);
  }
  ...
  `).bind(input.customerId, run.id, run.challenge_hash, input.customerId).run();
  if (Number(claimed.meta?.changes || 0) !== 1) {
    throw purchasesError('device_challenge_claim_failed', 'The device certification challenge could not be claimed', 409, true);
  }
  await db.prepare(`
  ...
  `).bind(
    input.id, run.id, input.projectId, input.customerId, input.checkKey, input.outcome,
    input.sourcePlatform, evidence.application_identifier, evidence.build_number,
    evidence.app_version, evidence.sdk_version, evidence.device_model, evidence.os_version,
    evidenceJson, evidenceSha256, observedAt, now.toISOString(),
  ).run();
  return {
    id: input.id,
    run_id: run.id,
...
export function requiredDeviceCertificationAssertions(checkKey: string): string[] {
  ...
  const requirements: Record<string, string[]> = {
    ...
    flutterflow_ios: ['purchase_completed', 'restore_completed', 'sync_completed', 'subscription_management_opened', 'verified_customer_info_applied'],
    flutterflow_android: ['purchase_completed', 'restore_completed', 'sync_completed', 'subscription_management_opened', 'verified_customer_info_applied'],
  };
  const required = requirements[scenario];
  if (!required) {
    throw purchasesError('device_certification_schema_missing', 'No device evidence schema is defined for this check', 500);
  }
  return required;
}
...
function validateIdentifier(value: string, field: string, max: number) {
  if (!new RegExp(`^[A-Za-z0-9][A-Za-z0-9._:-]{7,${max - 1}}$`, 'u').test(value)) {
...
function normalizedAssertions(value: Record<string, unknown>) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw purchasesError('device_certification_assertions_invalid', 'Assertions must be a JSON object');
  }
  const entries = Object.entries(value).sort(([left], [right]) => left.localeCompare(right, 'en'));
  if (entries.length < 1 || entries.length > 64) {
    throw purchasesError('device_certification_assertions_invalid', 'Assertions must contain 1-64 fields');
  }
  ...
  for (const [key, item] of entries) {
    if (!/^[a-z][a-z0-9_]{0,63}$/u.test(key)
      ...
      throw purchasesError('device_certification_assertions_invalid', 'Assertions contain an invalid field or value');
    }
...
function validateSourcePlatform(
  ...
  const expected = provider === 'apple' ? 'ios'
    : provider === 'google' ? 'android'
      : provider === 'stripe' ? 'web'
        : checkKey === 'cross_platform.flutterflow_ios' ? 'ios'
          : checkKey === 'cross_platform.flutterflow_android' ? 'android'
            : null;
  if (expected && sourcePlatform !== expected) {
    throw purchasesError('device_certification_platform_mismatch', `This check requires ${expected} SDK evidence`, 409);
  }
}

function requireExactRunValue(actual: string | null | undefined, expected: string | null, label: string) {
  const normalizedExpected = String(expected || '').trim();
  if (!normalizedExpected) return;
  if (String(actual || '').trim() !== normalizedExpected) {
    throw purchasesError('device_certification_run_mismatch', `Device ${label} does not match the certification run`, 409);
  }
}

function normalizedObservedAt(value: string | null | undefined, startedAt: string, now: Date) {
  const parsed = value ? Date.parse(value) : now.getTime();
  const started = Date.parse(normalizeD1Timestamp(startedAt));
  if (!Number.isFinite(parsed) || !Number.isFinite(started)
    || parsed < started - 10 * 60_000 || parsed > now.getTime() + 5 * 60_000) {
    throw purchasesError('device_certification_time_invalid', 'Device evidence timestamp is outside the active run', 409);
  }
...
function boundedText(value: unknown, field: string, max: number) {
  ...
  if (!text || text.length > max) throw purchasesError('device_certification_field_invalid', `${field} is required and must contain at most ${max} characters`);
  return text;
}
...
function optionalBoundedText(value: unknown, field: string, max: number) {
  const text = String(value || '').trim();
  if (!text) return null;
  if (text.length > max) throw purchasesError('device_certification_field_invalid', `${field} must contain at most ${max} characters`);
  return text;
}
