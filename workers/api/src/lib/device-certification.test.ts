...
describe("authenticated device certification evidence", () => {
  it("issues only a hashed, expiring challenge in storage", async () => {
    ...
    const result = await issueCertificationDeviceChallenge(
      db,
      "run-device-1",
      now,
    );
    ...
    const conflictUpdate =
      db.calls[0].sql.split("ON CONFLICT(run_id) DO UPDATE SET")[1] || "";
  ...
  it("stores a verified identity result with structured assertions and a digest", async () => {
    ...
    const insert = db.calls.find(
      (call) =>
        call.op === "run" &&
        call.sql.includes("INSERT INTO billing_certification_device_results"),
    )!;
  ...
  it("defines an assertion schema for every check that accepts SDK evidence", () => {
    expect(
      requiredDeviceCertificationAssertions("apple.network_loss"),
    ).toContain("outbox_retained");
    expect(
      requiredDeviceCertificationAssertions("apple.weekly_purchase"),
    ).toEqual([
      "purchase_completed",
