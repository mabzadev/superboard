...
export async function reconcileRefundDeadlines(env: BillingEnv, limit = 100) {
  ...
  return rows.reduce(
    (total, _deadline, index) =>
      total +
      Number((results[index * 2] as D1Result | undefined)?.meta?.changes || 0),
    0,
  );
}
...
export async function processAppleBillingNotification(
  ...
  try {
    ...
    const applied = result.purchase
      ? await applyVerifiedPurchase(env, result.purchase)
      : null;
    ...
    `,
      ...
      .bind(
        String(result.notification.notificationType || "UNKNOWN"),
        params.eventId,
      )
      .run();
...
export async function processGoogleBillingNotification(
  ...
  try {
    ...
    const environment =
      params.environment === "sandbox" ? "sandbox" : "production";
...
function responseDeclaresRetryable(body: string) {
  try {
    ...
    return Boolean(
      value && typeof value === "object" && value.retryable === true,
    );
  } catch {
    return false;
  }
...
async function responsePreview(response: Response) {
  try {
    return await readTextLimited(
      response,
      4096,
      "Webhook response body is too large",
    );
  } catch (error) {
...
export async function deliverBillingWebhook(
  env: BillingEnv,
  deliveryId: string,
) {
  ...
  if (Number(claimed.meta.changes || 0) === 0) {
    ...
    if (!state)
      throw webhookDeliveryError("Billing webhook delivery not found", false);
  ...
  if (!delivery)
    throw webhookDeliveryError(
      "Claimed billing webhook delivery not found",
      false,
    );
  ...
  try {
    try {
      ...
      throw webhookDeliveryError(
        error instanceof Error
          ? error.message
          : "Billing webhook URL is invalid",
        false,
      );
    }
    ...
    if (!secret)
      throw webhookDeliveryError(
        "Billing webhook signing secret is unavailable",
        false,
      );
    ...
    const timeout = setTimeout(
      () => controller.abort("Webhook delivery timed out"),
      15_000,
    );
    try {
    ...
    const retryDelaySeconds =
      tagged.retryDelaySeconds || webhookRetryDelay(attempt);
    const message = error instanceof Error ? error.message : String(error);
    await recordWebhookDeliveryFailure(env, {
      deliveryId,
      claimToken,
...
export async function reconcileBillingState(env: BillingEnv) {
  ...
  return {
    ...
    google_voided_reconciliations_enqueued:
      googleVoidedProjects.results?.length || 0,
...
export async function reconcileStoreSubscription(
  ...
  try {
    ...
    `,
      ...
      .bind(
        String(tagged.code || "subscription_reconciliation_failed").slice(
          0,
          100,
        ),
        String(tagged.message || "Subscription reconciliation failed").slice(
          0,
          1000,
        ),
        subscriptionId,
