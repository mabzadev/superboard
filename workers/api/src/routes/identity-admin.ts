...
function identityAdminSuffix(wildcard: string | undefined): string | null {
	...
	if (
		segments.some(
			(segment) =>
				...
				segment === ".." ||
				segment.length > 256 ||
				// oxlint-disable-next-line no-control-regex -- Reject control characters in identity route segments.
				/[\\\u0000-\u001f\u007f]/u.test(segment),
		)
	) {
		return null;
	...
	return normalized === "/info" || normalized.startsWith("/api/v1/") ? normalized : null;
}
...
async function forward(
	...
	try {
		...
		const target = new URL(`https://identity.internal/internal/v1/melody-admin${suffix}`);
		...
		const body = hasBody ? await readBytesLimited(c.req.raw, MAX_ADMIN_BODY_BYTES) : undefined;
