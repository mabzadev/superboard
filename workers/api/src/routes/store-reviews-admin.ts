...
reviews.get('/:projectId/reviews', async (c) => {
  try {
    ...
    return c.json({ data: rows.results, sync: sync.results });
...
reviews.post('/:projectId/reviews/:reviewId/drafts', async (c) => {
  try {
    ...
    if (!responseBody || responseBody.length > 3500) throw purchasesError('invalid_response_body', 'Response body is required and must be at most 3500 characters');
    const review = await c.env.DB.prepare('SELECT id FROM store_reviews WHERE id = ? AND project_id = ?').bind(c.req.param('reviewId'), project.id).first<{ id: string }>();
