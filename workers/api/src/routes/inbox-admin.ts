...
async function conversationItems(
	...
	try {
		const response = await env.MESSAGING.fetch(
			new Request(`https://messaging.internal/internal/projects/${projectId}/conversations`, {
				headers: {
		...
		const payload = text
			...
			: {};
		if (!response.ok)
			throw Object.assign(new Error(String(payload.message || "Messaging request failed")), {
				code: payload.code,
			});
		...
		degraded.push({
			source_type: "conversation",
			code: String((error as { code?: string })?.code || "messaging_unavailable"),
...
export function mapRefundItem(row: Record<string, unknown>): InboxItem {
	...
	return {
		...
		source_id: String(row.id),
		title: `${providerName(row.provider)} ${humanize(row.case_type || "refund case")}`.slice(
			0,
			255,
		),
		preview: String(
		...
		priority:
			hoursRemaining <= 24 ? "urgent" : hoursRemaining <= 72 || actions > 0 ? "high" : "normal",
