...
inbox.get('/:projectId/items', async (c) => {
  try {
    ...
    const data = [...conversations, ...refunds]
      ...
      .slice(0, 500);
    return c.json({ data, degraded_sources: degradedSources, projection: true });
  } catch (error) {
    return c.json(errorEnvelope(error, c.req.header('cf-ray') || crypto.randomUUID()), (error as any)?.status || 422);
  }
});

async function conversationItems(env: Env, projectId: string, degraded: Array<{ source_type: string; code: string; message: string }>): Promise<InboxItem[]> {
  if (!env.MESSAGING || !env.MESSAGING_INTERNAL_TOKEN) {
    degraded.push({ source_type: 'conversation', code: 'messaging_unavailable', message: 'Messaging is not configured' });
    return [];
  }
  try {
    const response = await env.MESSAGING.fetch(new Request(`https://messaging.internal/internal/projects/${projectId}/conversations`, {
      headers: { 'X-OpenGrow-Internal-Token': env.MESSAGING_INTERNAL_TOKEN },
    }));
    const text = await readTextLimited(response, 1_048_576, 'Messaging response is too large');
    const payload = text ? JSON.parse(text) as { data?: Array<Record<string, unknown>>; code?: string; message?: string } : {};
    if (!response.ok) throw Object.assign(new Error(String(payload.message || 'Messaging request failed')), { code: payload.code });
    return (payload.data || []).map(mapConversationItem);
  } catch (error) {
    console.error(JSON.stringify({ event: 'inbox_messaging_projection_failed', project_id: projectId, error: error instanceof Error ? error.message : String(error) }));
    degraded.push({ source_type: 'conversation', code: String((error as { code?: string })?.code || 'messaging_unavailable'), message: 'Messaging is temporarily unavailable' });
    return [];
  }
}
...
export function mapConversationItem(row: Record<string, unknown>): InboxItem {
  return {
      ...
      updated_at: isoDate(row.last_message_at || row.updated_at || row.created_at),
...
export function mapRefundItem(row: Record<string, unknown>): InboxItem {
    const deadline = row.deadline_at ? new Date(String(row.deadline_at)).getTime() : NaN;
    const hoursRemaining = Number.isFinite(deadline) ? (deadline - Date.now()) / 3_600_000 : Infinity;
    const actions = Number(row.actions_requiring_approval || 0);
    return {
      id: `refund_case:${row.id}`,
      source_type: 'refund_case',
      source_id: String(row.id),
      title: `${providerName(row.provider)} ${humanize(row.case_type || 'refund case')}`.slice(0, 255),
      preview: String(row.reason || (actions ? `${actions} provider action(s) require approval` : 'Refund case requires review')).slice(0, 1000),
      status: row.status === 'submitted' ? 'pending' : 'open',
      priority: hoursRemaining <= 24 ? 'urgent' : hoursRemaining <= 72 || actions > 0 ? 'high' : 'normal',
      customer_reference: row.primary_app_user_id ? String(row.primary_app_user_id) : null,
      updated_at: isoDate(row.updated_at),
      destination: `/purchases?section=refunds&case=${encodeURIComponent(String(row.id))}`,
      capabilities: ['review_evidence', 'approve_provider_action', 'send_provider_action'],
      source: row,
    } satisfies InboxItem;
}

function conversationStatus(value: unknown): InboxItem['status'] {
  return value === 'closed' ? 'closed' : value === 'pending' ? 'pending' : 'open';
}

function itemPriority(value: unknown): InboxItem['priority'] {
  return ['low', 'normal', 'high', 'urgent'].includes(String(value)) ? value as InboxItem['priority'] : 'normal';
}
