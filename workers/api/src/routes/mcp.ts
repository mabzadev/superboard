...
import { dispatchMcpDynamicLinks } from "../lib/mcp-dynamic-links.js";
import { dispatchMcpInstanceIntegration } from "../lib/mcp-instance-integrations.js";
import { readMcpSiteOperatorGrant } from "../lib/mcp-oauth";
import {
...
import { resolveSiteOperatorInstance } from "../lib/site-operator-scope";
import { Env } from "../types";
import { buildPlatformStatus } from "./platform-status";

const mcp = new Hono<{ Bindings: Env; Variables: SiteAdapterVariables }>();
mcp.use("*", siteOperatorAdapter("mcp"));
const nativeDynamicPath = /^\/api\/v1\/mcp\/(?:links|campaigns|analytics|redirects)(?:\/|$)/u;
mcp.use("*", async (c, next) => {
	if (!nativeDynamicPath.test(new URL(c.req.url).pathname)) return next();
	const userId = await getMcpUserId(c);
	const site = c.get("siteAuth")?.siteOperator;
	if (!site || userId === null) return next();
	const body = ["GET", "DELETE"].includes(c.req.method)
		? {}
		: await readRequestObjectLimited(c.req.raw.clone(), 65536);
	const project = await projectForMcp(c, userId, body.project_id ?? c.req.query("project_id"));
	if (!project) return c.json({ error: "Project not found" }, 404);
	return dispatchMcpDynamicLinks(c.env, site, project, c.req.raw);
});
...
async function getMcpUserId(c: any): Promise<number | null> {
  ...
  `)
		...
		.first()
		.catch(() => null);
	if (mcpToken && !mcpToken.user_id) {
		const siteOperator = await readMcpSiteOperatorGrant(c.env.DB, "token", digest);
		const expected = c.env.SUPERBOARD_TARGET ?? c.env.OPENGROW_TARGET;
		if (!siteOperator || siteOperator.instance_id !== expected) return null;
		const instanceId = await resolveSiteOperatorInstance(c.env.DB, siteOperator.instance_id);
		...
		c.set("siteAuth", {
			...
			source: "site",
			siteOperator,
			payload: { operator_id: siteOperator.operator_id },
		});
		return 0;
	}
	if (mcpToken?.user_id) {
		await c.env.DB.prepare(
			'UPDATE mcp_tokens SET last_used_at = datetime("now"), updated_at = datetime("now") WHERE access_token = ? OR token_digest = ?',
		)
			.bind(token, digest)
			.run()
			.catch(() => null);
...
async function projectForMcp(c: any, userId: number, raw: unknown) {
	...
	const siteAuth = c.get("siteAuth");
	if (siteAuth) {
		const reference = /^(\d+)-(prod|test)$/.exec(value);
		if (reference && Number(reference[1]) !== siteAuth.instanceId) return null;
		const row = reference
			? await c.env.DB.prepare(
					"SELECT * FROM projects WHERE instance_id = ? AND is_test = ? LIMIT 1",
				)
					.bind(siteAuth.instanceId, reference[2] === "test" ? 1 : 0)
					.first()
			: await c.env.DB.prepare("SELECT * FROM projects WHERE id = ? AND instance_id = ? LIMIT 1")
					.bind(value, siteAuth.instanceId)
					.first();
		return row
			? {
					...row,
					instanceId: row.instance_id,
					externalId: `${row.instance_id}-${row.is_test ? "test" : "prod"}`,
				}
			: null;
	}
	...
	if (/-prod$|-test$/.test(value)) project = await resolveProject(c.env.DB, value);
		...
		project = await c.env.DB.prepare(
			"SELECT id, instance_id, name, identifier, is_test FROM projects WHERE id = ? LIMIT 1",
		)
			.bind(value)
			.first();
	...
	if (site)
		return Number(project.instanceId || project.instance_id) === site.instanceId ? project : null;
	const role = await c.env.DB.prepare(
		"SELECT id FROM instance_roles WHERE instance_id = ? AND user_id = ? LIMIT 1",
	)
		.bind(project.instanceId || project.instance_id, userId)
		.first();
...
async function instanceForMcp(c: any, userId: number, raw: unknown) {
	const value = String(raw || "")
		.replace(/-(prod|test)$/, "")
		.trim();
	const row = await c.env.DB.prepare("SELECT * FROM instances WHERE id = ? LIMIT 1")
		.bind(value)
		.first();
	if (!row) return null;
	const site = c.get("siteAuth");
	if (site) return Number(row.id) === site.instanceId ? row : null;
	const role = await c.env.DB.prepare(
		"SELECT id FROM instance_roles WHERE instance_id = ? AND user_id = ? LIMIT 1",
	)
		.bind(row.id, userId)
...
async function getOrCreateApplication(
	...
	const created = await db
		.prepare(
			"INSERT INTO applications (platform, instance_id, enabled) VALUES (?, ?, ?) RETURNING id",
		)
		.bind(platform, instanceId, enabled ? 1 : 0)
		.first<{ id: number }>();
...
async function setupPlatformConfiguration(
	db: D1Database,
	instanceId: number,
	platform: string,
	config: Record<string, any>,
) {
	const appId = await getOrCreateApplication(
		db,
		instanceId,
		platform,
		config.enabled === undefined ? true : toBool(config.enabled),
	);
	if (platform === "ios") {
    ...
    `)
			.bind(appId, bundleId, appPrefix, toBool(config.tablet_enabled) ? 1 : 0)
			.run();
	}
	...
	if (platform === "android") {
      ...
      ON CONFLICT(application_id) DO UPDATE SET
        ...
        updated_at = datetime('now')
    `)
			.bind(appId, identifier, sha256s, toBool(config.tablet_enabled) ? 1 : 0)
			.run();
	}
	...
	if (platform === "desktop") {
		...
		const existing = await db
			.prepare("SELECT id FROM desktop_configurations WHERE application_id = ? LIMIT 1")
			.bind(appId)
...
async function redirectConfigResponse(db: D1Database, redirectConfigId: number | string) {
	const config: any = await db
		.prepare("SELECT * FROM redirect_configs WHERE id = ?")
		.bind(redirectConfigId)
		.first();
	const rows = await db
		.prepare("SELECT * FROM redirects WHERE redirect_config_id = ?")
		.bind(redirectConfigId)
		.all<any>();
	const by = (platform: string, variation: string) => {
		const row = (rows.results || []).find(
			(item: any) => item.platform === platform && item.variation === variation,
		);
		return {
...
async function domainForProject(db: D1Database, projectId: number, fallbackDomain: string) {
	const existing = await db
		.prepare("SELECT id FROM domains WHERE project_id = ? ORDER BY id ASC LIMIT 1")
		.bind(projectId)
		.first<{ id: number }>();
	if (existing) return existing.id;
	const created = await db
		.prepare("INSERT INTO domains (domain, project_id) VALUES (?, ?) RETURNING id")
		.bind(fallbackDomain, projectId)
		.first<{ id: number }>();
...
async function findProjectLink(
    ...
    LIMIT 1
  `)
		.bind(pathOrId, projectId, projectId)
		.first<any>();
}
...
mcp.get("/validate", async (c) => {
	const userId = await getMcpUserId(c);
	if (userId === null) return c.json({ error: "invalid_token" }, 401);
...
mcp.delete("/token", async (c) => {
	...
	const digest = await tokenDigest(token);
	await c.env.DB.prepare(
		'UPDATE mcp_tokens SET revoked_at = datetime("now"), updated_at = datetime("now") WHERE access_token = ? OR token_digest = ?',
	)
		.bind(token, digest)
		.run();
...
mcp.get("/status", async (c) => {
	...
	if (site?.siteOperator) {
		const rows = await c.env.DB.prepare(
			"SELECT instance.id AS instance_id,instance.uri_scheme,project.id,project.name,project.identifier,project.is_test FROM instances instance LEFT JOIN projects project ON project.instance_id=instance.id WHERE instance.id=? ORDER BY project.is_test,project.id",
		)
			.bind(site.instanceId)
			.all<{
				instance_id: number;
		...
		const projects = rows.results
			...
			.map((project) => ({
				id: `${project.instance_id}-${project.is_test ? "test" : "prod"}`,
				name: project.name,
				identifier: project.identifier,
				environment: project.is_test ? "test" : "production",
			}));
		return c.json({
			user: { id: site.siteOperator.operator_id, identity: "emdash_operator" },
			instances: first
				? [
						{
							id: String(first.instance_id),
							uri_scheme: first.uri_scheme,
	...
	const user = site?.siteOperator
		? { id: site.siteOperator.operator_id }
    ...
    FROM instances i
    ${site ? "" : "JOIN instance_roles ir ON ir.instance_id = i.id"}
    ...
    WHERE ${site ? "i.id" : "ir.user_id"} = ?
    GROUP BY i.id
    ORDER BY i.id ASC
  `)
		.bind(site?.instanceId ?? userId)
		.all<any>();
	const instances = await Promise.all(
		(rows.results || []).map(async (instance: any) => {
...
mcp.get("/platform-status", async (c) => {
  ...
  `)
	...
	const siteAuth = c.get("siteAuth");
	return c.json(
		siteAuth?.siteOperator && siteAuth.instanceId
			? await buildPlatformStatus(c.env, { instanceId: siteAuth.instanceId })
			: await buildPlatformStatus(c.env),
		200,
		{
...
mcp.get("/usage", async (c) => {
	...
	const operator = c.get("siteAuth")?.siteOperator;
	if (operator)
		return dispatchMcpInstanceIntegration(c.env, operator, Number(instance.id), "usage");
  ...
  `)
		...
		.first<{ total: number }>();
	return c.json({
		usage: { instance_id: String(instance.id), mau: Number(activeUsers?.total || 0) },
	});
});

// GET /api/v1/mcp/tokens
mcp.get("/consent-request", async (c) => {
	if ((await getAuthUserId(c)) === null) return c.json({ error: "Unauthorized" }, 401);
	const clientId = c.req.query("client_id") ?? "";
	const redirectUri = c.req.query("redirect_uri") ?? "";
	const client = await findMcpClient(c.env.DB, clientId);
	if (
		!client ||
		!validRegistrationRedirectUri(redirectUri) ||
		!redirectUriAllowed(client, redirectUri)
	)
		return c.json(
			{
				error: "invalid_request",
				error_description: "The application or redirect URI is not registered",
			},
			400,
		);
	return c.json({
...
mcp.get("/tokens", async (c) => {
    ...
    WHERE ${site ? "EXISTS (SELECT 1 FROM mcp_site_operator_grants grant_row WHERE grant_row.resource_type = 'token' AND grant_row.resource_key = mt.token_digest AND grant_row.instance_slug = ? AND grant_row.operator_id = ?)" : "mt.user_id = ?"}
      AND mt.revoked_at IS NULL
      AND datetime(mt.created_at) > datetime('now', '-90 days')
    ORDER BY datetime(mt.created_at) DESC
  `)
		.bind(...(site ? [site.instance_id, site.operator_id] : [String(userId)]))
		.all<any>();
	return c.json({
...
mcp.delete("/tokens/:tokenId", async (c) => {
    ...
    WHERE id = ? AND ${site ? "EXISTS (SELECT 1 FROM mcp_site_operator_grants grant_row WHERE grant_row.resource_type = 'token' AND grant_row.resource_key = mcp_tokens.token_digest AND grant_row.instance_slug = ? AND grant_row.operator_id = ?)" : "user_id = ?"} AND revoked_at IS NULL
    RETURNING id
  `)
		.bind(
			c.req.param("tokenId"),
			...(site ? [site.instance_id, site.operator_id] : [String(userId)]),
		)
		.first<{ id: string }>();
...
mcp.post("/approve_consent", async (c) => {
	...
	const code = await createMcpAuthorizationCode(c.env.DB, {
		client,
		userId: c.get("siteAuth") ? null : userId,
		siteOperator: c.get("siteAuth")?.siteOperator,
		redirectUri,
		codeChallenge,
		codeChallengeMethod,
...
mcp.post("/projects", async (c) => {
	const userId = await getMcpUserId(c);
	if (c.get("siteAuth")) return c.json({ error: { code: "SINGLE_INSTANCE_REQUIRED" } }, 409);
	if (userId === null) return c.json({ error: "invalid_token" }, 401);
	const body = await readBody(c);
	...
	const created = await c.env.DB.prepare(
		...
		.bind(generateApiKey(), `${uriBase}${generateShortCode(8).toLowerCase()}`)
		.first<any>();
	await c.env.DB.prepare("INSERT INTO instance_roles (user_id, instance_id, role) VALUES (?, ?, ?)")
		.bind(userId, created.id, "owner")
		.run();
...
mcp.post("/links/search", async (c) => {
    ...
    LIMIT ?
  `)
		.bind(project.id, project.id, Math.min(100, Number(body.limit || 25)))
		.all<any>();
...
mcp.delete("/links/:id", async (c) => {
	...
	const link = await findProjectLink(c.env.DB, Number(project.id), c.req.param("id"));
	if (!link) return c.json({ error: "Link not found" }, 404);
	await c.env.DB.prepare('UPDATE links SET active = 0, updated_at = datetime("now") WHERE id = ?')
		.bind(link.id)
		.run();
...
mcp.post("/analytics/link", async (c) => {
	...
	return c.json({
		link_path: link.path,
		metrics: await metricsFor(c, Number(project.id), Number(link.id)),
	});
});
...
mcp.post("/analytics/top_links", async (c) => {
    ...
    LIMIT ?
  `)
		.bind(project.id, project.id, Math.min(25, Number(body.limit || 10)))
		.all<any>();
	return c.json({
		links: (rows.results || []).map((row: any) => ({
			...linkJson(row),
			views: Number(row.views || 0),
		})),
	});
...
mcp.post("/campaigns/search", async (c) => {
	...
	if (!project) return c.json({ error: "Project not found" }, 404);
	const rows = await c.env.DB.prepare(
		"SELECT * FROM campaigns WHERE project_id = ? AND archived = 0 ORDER BY created_at DESC",
	)
		.bind(project.id)
		.all<any>();
	return c.json({
		campaigns: rows.results || [],
		meta: { page: 1, per_page: 100, total_pages: 1, total_entries: rows.results?.length || 0 },
	});
});
...
mcp.delete("/campaigns/:campaignId", async (c) => {
	...
	if (!project) return c.json({ error: "Project not found" }, 404);
	const campaign = await c.env.DB.prepare(
		'UPDATE campaigns SET archived = 1, updated_at = datetime("now") WHERE id = ? AND project_id = ? RETURNING *',
	)
		.bind(c.req.param("campaignId"), project.id)
		.first<any>();
...
mcp.put("/redirects", async (c) => {
	...
	const redirectConfigId = await getOrCreateRedirectConfig(c.env.DB, Number(project.id));
	await c.env.DB.prepare(
		'UPDATE redirect_configs SET default_fallback = COALESCE(?, default_fallback), show_preview_ios = COALESCE(?, show_preview_ios), show_preview_android = COALESCE(?, show_preview_android), updated_at = datetime("now") WHERE id = ?',
	)
		.bind(
	...
	if (unsupported)
		return c.json(
			{ error: `Unsupported platform: ${unsupported}. Supported: ios, android, desktop` },
			400,
		);
...
mcp.put("/sdk", async (c) => {
	...
	const operator = c.get("siteAuth")?.siteOperator;
	if (operator)
		return dispatchMcpInstanceIntegration(c.env, operator, Number(instance.id), "sdk", body);
	const platforms = parseJsonObject(body.platforms) || {};
	const unsupported = Object.keys(platforms).find((platform) => !supportedMcpPlatform(platform));
	if (unsupported)
		return c.json(
