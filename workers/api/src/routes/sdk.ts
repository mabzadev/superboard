...
import customSdkRoutes from "./custom-sdk";
import marketingSdkRoutes from "./marketing-sdk";
import accountSdkRoutes from "./account-sdk";
...
sdk.get("/.well-known/purchases-jwks.json", async (c) =>
  c.json(await purchasesJwksFromBillingAuthority(c.env), 200, {
...
sdk.route("/custom/v1", customSdkRoutes);
sdk.route("/marketing/v1", marketingSdkRoutes);
sdk.route("/account/v1", accountSdkRoutes);
sdk.onError((error, c) => {
...
function requestCountry(c: any): string | null {
  ...
  const value = String(
    requestWithCf.cf?.country || c.req.header("CF-IPCountry") || "",
  )
    .trim()
    .toUpperCase();
...
function platformMatches(
  ...
  return (
    ...
    (platform === "web" && platforms.includes("desktop"))
  );
}
...
async function addNotificationsForNewVisitor(
  db: D1Database,
  projectId: number,
  ...
  const device = await db
    .prepare("SELECT platform, push_token FROM devices WHERE id = ?")
    .bind(deviceId)
    .first<{ platform: string | null; push_token: string | null }>();
  const rows = await db
    .prepare(
      `
  ...
  for (const notification of rows.results || []) {
    if (
      !platformMatches(parseJsonArray(notification.platforms), device?.platform)
    )
      continue;
    await db
      .prepare(
...
async function ensureVisitor(
  ...
  if (existing) {
    ...
    `,
      ...
      .bind(sdkIdentifier || null, serializedAttributes, existing.id)
      .run();
...
async function visitorForDevice(
  db: D1Database,
  projectId: number,
  deviceId: number,
) {
...
async function currentSdkProject(c: any, body: Record<string, any> = {}) {
  const projectId =
    c.get("projectId") || body.project_id || c.req.query("project_id");
  const instanceId = c.get("instanceId");
  if (projectId) {
    const project = instanceId
      ? await c.env.DB.prepare(
          "SELECT * FROM projects WHERE id = ? AND instance_id = ? LIMIT 1",
        )
          .bind(projectId, instanceId)
          .first()
      : await c.env.DB.prepare("SELECT * FROM projects WHERE id = ? LIMIT 1")
          .bind(projectId)
          .first();
...
async function sdkVisitor(c: any, projectId?: number | string) {
  const linksquared =
    c.req.header("LINKSQUARED") ||
    c.req.header("linksquared") ||
    c.req.query("linksquared");
  ...
  const values = scopedProjectId
    ? [linksquared, linksquared, String(scopedProjectId)]
    : [linksquared, linksquared];
...
async function domainForProject(
  db: D1Database,
  projectId: number,
  ...
  const existing = await db
    .prepare(
      "SELECT id FROM domains WHERE project_id = ? ORDER BY id ASC LIMIT 1",
    )
    .bind(projectId)
    .first<{ id: number }>();
...
async function notificationMessages(
  ...
  options: {
    unreadOnly?: boolean;
    autoDisplayOnly?: boolean;
    page?: number;
  } = {},
) {
  const unreadClause = options.unreadOnly
    ? " AND COALESCE(nm.read, 0) = 0"
    : "";
  const autoDisplayClause = options.autoDisplayOnly
    ? " AND COALESCE(n.auto_display, 0) = 1"
    : "";
  ...
  `,
    ...
    .bind(String(visitorId), (page - 1) * 25)
    .all<any>();
}
...
sdk.post("/authenticate", async (c) => {
  ...
  if (!userAgent || !body.app_version)
    return c.json({ error: "user_agent and app_version required" }, 422);

  const existing = body.vendor_id
    ? await c.env.DB.prepare(
        "SELECT id FROM devices WHERE vendor = ? ORDER BY updated_at DESC LIMIT 1",
      )
        .bind(body.vendor_id)
        .first<{ id: number }>()
    : null;
    ...
    `,
      ...
      .bind(
        ...
        c.req.header("PLATFORM") ||
          ...
          null,
        body.model || null,
        body.vendor_id || body.vendor || null,
        body.language || null,
        body.timezone || null,
  ...
  const instance = await c.env.DB.prepare(
    "SELECT uri_scheme FROM instances WHERE id = ? LIMIT 1",
  )
    .bind(project.instance_id)
    .first<{ uri_scheme: string }>();
  return c.json({
...
sdk.get("/device_for_vendor_id", async (c) => {
  ...
  const device = await c.env.DB.prepare(
    "SELECT id FROM devices WHERE vendor = ? LIMIT 1",
  )
    .bind(body.vendor_id || "")
    .first<{ id: number }>();
...
sdk.post("/attribution", async (c) => {
  ...
  if (countryCode) {
    await c.env.DB.prepare(
      "UPDATE devices SET country_code = ?, updated_at = datetime('now') WHERE id = ?",
    )
      .bind(countryCode, body.device_id)
      .run();
  }
...
sdk.post("/event", async (c) => {
  ...
  let linkId = null;
  if (body.path) {
    const link = await findProjectLink(
      c.env.DB,
      Number(project.id),
      String(body.path),
    );
...
sdk.post("/data_for_device", async (c) => {
  ...
  `,
    ...
    .bind(visitor.device_id, project.id)
    .first<any>();
...
sdk.post("/data_for_device_and_url", async (c) => {
  ...
  const path =
    String(body.url || "")
      ...
      .pop() || "";
  const link = path
    ? await findProjectLink(c.env.DB, Number(project.id), path)
    : null;
  return c.json({
    data: link ? parseJsonObject(link.data) || null : null,
    link: link ? linkPayload(link) : null,
  });
});
...
sdk.post("/data_for_device_and_path", async (c) => {
  ...
  const visitor = await sdkVisitor(c, Number(project.id));
  if (!visitor) return c.json({ error: "Invalid linksquared id" }, 403);
  const link = body.path
    ? await findProjectLink(c.env.DB, Number(project.id), String(body.path))
    : null;
  return c.json({
    data: link ? parseJsonObject(link.data) || null : null,
    link: link ? linkPayload(link) : null,
...
sdk.post("/create_link", async (c) => {
  ...
  const visitor = await sdkVisitor(c, Number(project.id));
  if (!visitor) return c.json({ error: "Invalid linksquared id" }, 403);
  const redirectConfigId = await getOrCreateRedirectConfig(
    c.env.DB,
    Number(project.id),
  );
  const domainId = await domainForProject(
    c.env.DB,
    Number(project.id),
    c.env.SHORTLINK_DOMAIN,
  );
  let path = String(body.path || generateShortCode())
    .trim()
    .replace(/^\/+/, "");
  while (
    await c.env.DB.prepare("SELECT id FROM links WHERE path = ? LIMIT 1")
      .bind(path)
      .first()
  ) {
...
sdk.post("/visitor_attributes", async (c) => {
  ...
  if (body.push_token) {
    await c.env.DB.prepare(
      ...
      .bind(body.push_token, visitor.device_id)
      .run();
  }
  const row = await c.env.DB.prepare("SELECT * FROM visitors WHERE id = ?")
    .bind(visitor.id)
    .first<any>();
...
sdk.post("/add_payment_event", async (c) => {
  ...
  `,
    ...
    .run();
  return c.json({
    message: "Payment event recorded provisionally",
    verified: false,
  });
});
...
sdk.get("/metrics_for_link/:path", async (c) => {
  ...
  return c.json({
    metrics: metricsPayload(
      await linkMetrics(c.env.DB, Number(project.id), Number(link.id)),
    ),
  });
});
...
sdk.get("/metrics_for_project", async (c) => {
  const project = await currentSdkProject(c);
  return c.json(
    metricsPayload(await linkMetrics(c.env.DB, Number(project.id))),
  );
});

async function notificationsForRequest(
  c: any,
  body: Record<string, any>,
  unreadOnly = false,
  autoDisplayOnly = false,
) {
  const project = await currentSdkProject(c, body);
  const deviceId = Number(body.device_id || c.req.query("device_id") || 0);
  const projectId = Number(
    body.project_id || c.req.query("project_id") || project.id || 0,
  );
  if (!deviceId || !projectId)
    return c.json({ error: "device_id and project_id required" }, 422);
  ...
  return c.json({
    notifications: (rows.results || []).map((row: any) =>
      notificationMessageRow(c, row),
    ),
  });
}

sdk.post("/notifications_for_device", async (c) =>
  notificationsForRequest(c, await readBody(c)),
);
...
sdk.post("/mark_notification_as_read", async (c) => {
  ...
  if (!deviceId || !projectId || !id)
    return c.json({ error: "device_id, project_id and id required" }, 422);
  if (projectId !== Number(project.id))
    return c.json({ error: "Invalid project for SDK credentials" }, 403);
  const visitor = await visitorForDevice(c.env.DB, projectId, deviceId);
