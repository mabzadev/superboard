...
admin.post("/:projectId/oidc", async (c) => {
    ...
    `)
			.bind(id, project.id, issuer, String(data.audience || "opengrow"), jwks.toString())
			.run();
...
admin.post("/:projectId/mirror/compare", async (c) => {
	try {
		...
		const differences = {
			missing_in_opengrow: missingInOpenGrow,
			extra_in_opengrow: extraInOpenGrow,
		};
