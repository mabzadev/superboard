...
function htmlError(c: any, error: string, description: string) {
	...
	return c.html(
		`<!doctype html><html><head><meta charset="utf-8"><title>${escape(error)}</title></head><body><h1>${escape(error)}</h1><p>${escape(description)}</p></body></html>`,
		400,
	);
}
...
mcpOauth.post("/register", async (c) => {
	...
	const existing = await c.env.DB.prepare(
		"SELECT * FROM mcp_clients WHERE client_name = ? OR name = ? ORDER BY created_at ASC",
	)
...
mcpOauth.get("/authorize", async (c) => {
	...
	if (responseType !== "code")
		return htmlError(c, "unsupported_response_type", "Only response_type=code is supported");
	if (!clientId || !redirectUri)
		return htmlError(c, "invalid_request", "client_id and redirect_uri are required");
	if (!codeChallenge || codeChallengeMethod !== "S256")
		return htmlError(c, "invalid_request", "PKCE with S256 method is required");

	const client = await findMcpClient(c.env.DB, clientId);
	if (!client) return htmlError(c, "invalid_client", "Unknown client_id");
	if (!redirectUriAllowed(client, redirectUri))
		return htmlError(c, "invalid_request", "redirect_uri not registered for this client");
...
mcpOauth.post("/token", async (c) => {
	...
	if (grantType === "authorization_code") {
      ...
      LIMIT 1
    `)
			.bind(code, redirectUri, clientId)
			.first<any>();
		if (!authCode)
			return c.json(
				{
		...
		if (!authCode.code_challenge)
			return c.json({ error: "invalid_grant", error_description: "PKCE required" }, 400);
		if (
			!(await verifyPkce(
				body.code_verifier,
		...
		const siteOperator =
			authCode.user_id == null ? await readMcpSiteOperatorGrant(c.env.DB, "code", code) : null;
		if (
			authCode.user_id == null &&
			(!siteOperator ||
				siteOperator.instance_id !== (c.env.SUPERBOARD_TARGET ?? c.env.OPENGROW_TARGET))
		)
			return c.json({ error: "invalid_grant" }, 400);
		const consumed = await c.env.DB.prepare(
			'UPDATE mcp_authorization_codes SET used_at = datetime("now"), updated_at = datetime("now") WHERE id = ? AND used_at IS NULL',
		)
			.bind(authCode.id)
			.run();
		if (consumed.meta.changes !== 1) return c.json({ error: "invalid_grant" }, 400);
		const client = {
    ...
    `)
		...
		const siteOperator =
			existing.user_id == null
				? await readMcpSiteOperatorGrant(c.env.DB, "token", existing.token_digest)
				: null;
		if (
			existing.user_id == null &&
			(!siteOperator ||
