...
import {
  identifyCustomer,
  queueCustomerEntitlementChanged,
} from "../lib/billing";
import {
...
async function currentBillingWorkerReadiness(
  env: Env,
): Promise<BillingWorkerReadiness | null> {
  try {
    if (env.CREDENTIAL_KEY_SCOPE === "billing")
      return await billingWorkerReadiness(env);
    if (!env.BILLING) return null;
    return await callBillingServiceBinding<BillingWorkerReadiness>(
      env,
      "/internal/v1/health",
    );
  } catch (error) {
...
async function releaseGateScope(
  c: any,
  project: { id: string; instanceId: number },
) {
  ...
  const testProjectId = String(
    rows.results.find((row) => Number(row.is_test) === 1)?.id || "",
  );
  return {
    productionProjectId,
    testProjectId,
...
async function featureFlagsFor(c: any, projectId: string) {
  await c.env.DB.prepare(
    "INSERT OR IGNORE INTO billing_feature_flags (project_id) VALUES (?)",
  )
    .bind(projectId)
    .run();
...
async function nativeConnections(c: any, project: any) {
  ...
  const appleConfigured = Boolean(
    apple?.key_id && apple?.issuer_id && apple?.app_apple_id,
  );
...
admin.get("/:projectId/release-gate", async (c) => {
  try {
    ...
    const staleMinutes = releaseGateStaleMinutes(
      c.env.BILLING_RELEASE_STALE_MINUTES,
    );
    const catalogStaleHours = releaseGateCatalogStaleHours(
      c.env.BILLING_CATALOG_STALE_HOURS,
    );
    const [
      checks,
      connections,
      ...
      c.env.DB.prepare(
        `SELECT * FROM billing_release_gate_checks WHERE project_id = ?`,
      )
      ...
      `,
        ...
        .bind(
          catalogStaleHours,
          scope.productionProjectId || scope.releaseProjectId,
        )
        .first<{
          ready: number;
          checked_at: string;
    ...
    const verifiedCertificationObservations = await Promise.all(
      (certificationObservations.results || []).map(async (row) => ({
        ...row,
        digest_valid:
          (await sha256Hex(String(row.evidence_json || ""))) ===
          row.evidence_sha256,
      })),
    );
    ...
    const prerequisites: ReleaseGatePrerequisite[] = [
      ...billingOperationalPrerequisites({
        dedicated_execution:
          c.env.CREDENTIAL_KEY_SCOPE === "billing" ||
          c.env.BILLING_EXECUTION_MODE === "service",
    ...
    return c.json({
      data: {
        ...
        ...buildReleaseGate(
          checks.results || [],
          prerequisites,
          verifiedCertificationObservations,
        ),
      },
    });
  } catch (error) {
...
admin.post("/:projectId/certification-runs", async (c) => {
  try {
    ...
    const buildNumber = certificationText(
      data.build_number,
      "build_number",
      100,
    );
...
admin.post(
  "/:projectId/certification-runs/:runId/device-challenge",
  async (c) => {
    try {
      const project = await projectFor(c);
      requireAdmin(project);
    ...
    `,
      ...
      const challenge = await issueCertificationDeviceChallenge(
        c.env.DB,
        runId,
      );
      await audit(
...
admin.post("/:projectId/certification-runs/:runId/observations", async (c) => {
  try {
    ...
    if (!definition)
      throw purchasesError(
        "release_gate_check_not_found",
        "Release gate check not found",
        404,
      );
    ...
    const referenceId = certificationText(
      data.reference_id,
      "reference_id",
      500,
    );
    ...
    const device =
      [run.device_model, run.os_version].filter(Boolean).join(" / ") ||
      run.platform;
    const gateEvidence = {
...
admin.patch("/:projectId/release-gate/checks/:checkKey", async (c) => {
  try {
    ...
    if (encodedEvidence.length > 16_384)
      throw purchasesError(
        "release_gate_evidence_too_large",
        "Evidence is limited to 16 KB",
        413,
      );
    ...
    if (status === "passed") {
      ...
      if (
        !observation ||
        (await sha256Hex(observation.evidence_json)) !==
    ...
    const notes =
      data.notes == null
        ? null
        : String(data.notes).trim().slice(0, 2000) || null;
    await c.env.DB.prepare(
      `
...
export async function certificationReferenceSnapshot(
  ...
  if (referenceType === "test_run") {
    ...
    if (
      (await sha256Hex(String(row.evidence_json || ""))) !== row.evidence_sha256
    ) {
      throw purchasesError(
  ...
  if (referenceType === "billing_event") {
    ...
    if (
      provider === "cross_platform" &&
      !["apple", "google"].includes(String(row.provider))
    ) {
      throw purchasesError(
        "certification_reference_provider_mismatch",
    ...
    requireReferenceDuringRun(
      row.occurred_at || row.received_at,
      run.started_at,
    );
    validateCertificationScenario(checkKey, referenceType, row);
    return {
  ...
  if (
    ...
    Number(row.unresolved_subscriptions || 0) !== 0
  ) {
    throw purchasesError(
...
admin.patch("/:projectId/features", async (c) => {
  try {
    ...
    const statements = FEATURE_FLAGS.filter(
    ...
    if (!statements.length)
      throw purchasesError(
        ...
        "At least one known feature flag is required",
      );
...
admin.post("/:projectId/legacy/revenuecat/connections", async (c) => {
  try {
    ...
    if (!/^[A-Za-z0-9_-]{1,255}$/.test(externalProjectId)) {
      throw purchasesError(
        "legacy_project_id_invalid",
        "A valid legacy project ID is required",
      );
    }
...
admin.delete("/:projectId/legacy/revenuecat/connection", async (c) => {
  try {
    ...
    if (!source)
      throw purchasesError(
        "legacy_source_not_configured",
        "Legacy source is not configured",
        404,
      );
    await audit(
      c,
      project.id,
      "legacy_source.disabled",
      "legacy_source",
      source.id,
...
admin.post("/:projectId/legacy/revenuecat/inventory-runs", async (c) => {
  try {
    ...
    if (!c.env.BILLING_QUEUE)
      throw purchasesError(
        "billing_queue_unavailable",
        "Billing queue is unavailable",
        503,
        true,
      );
    const data = await jsonBody(c);
    ...
    `,
      ...
      .bind(runId, project.id, source.id, environment, String(c.get("userId")))
      .run();
...
admin.post(
  "/:projectId/legacy/revenuecat/inventory-runs/:runId/cancel",
  async (c) => {
    try {
      const project = await projectFor(c);
      requireAdmin(project);
...
admin.get("/:projectId/connections", async (c) => {
  try {
    ...
    const rows: Array<Record<string, any>> = (stored.results || []).map(
      (row) => ({
        ...row,
...
admin.get("/:projectId/apple-server-notifications", async (c) => {
  try {
    ...
    await persistAppleNotificationConfiguration(
      c.env.DB,
      scope.productionProjectId,
      data,
    );
...
admin.post("/:projectId/dead-letters/:deadLetterId/replay", async (c) => {
  try {
    ...
    `,
      ...
      .bind(String(c.get("userId")), deadLetter.id, project.id)
      .run();
...
admin.post("/:projectId/dead-letters/:deadLetterId/discard", async (c) => {
  try {
    ...
    `,
      ...
      .bind(String(c.get("userId")), c.req.param("deadLetterId"), project.id)
      .run();
...
admin.get("/:projectId/customers/:customerId", async (c) => {
  try {
    ...
    if (!customer)
      throw purchasesError("customer_not_found", "Customer not found", 404);
...
admin.post(
  "/:projectId/customers/:customerId/entitlements/:entitlementId",
  async (c) => {
    try {
      const project = await projectFor(c);
      requireAdmin(project);
      ...
      const [customer, entitlement] = await Promise.all([
        c.env.DB.prepare(
          ...
          .first<{ id: string }>(),
        c.env.DB.prepare(
          "SELECT id FROM billing_entitlements WHERE id = ? AND project_id = ?",
        )
    ...
    `,
        ...
        .run();
      await recordAdminBillingEvent(
        c,
        project,
        customer.id,
        "promotional_granted",
        { entitlement_id: entitlement.id, expires_at: data.expires_at || null },
      );
...
admin.post("/:projectId/customers/:customerId/merge", async (c) => {
  try {
    ...
    if (!source || !target)
      throw purchasesError(
        "customer_not_found",
        "Source or target customer not found",
        404,
      );
...
admin.delete("/:projectId/customers/:customerId", async (c) => {
  try {
    ...
    await c.env.DB.batch([
      ...
      c.env.DB.prepare(
        "DELETE FROM billing_customers WHERE id = ? AND project_id = ?",
      ).bind(customerId, project.id),
    ]);
...
admin.get("/:projectId/analytics", async (c) => {
  try {
    ...
    const [summary, series, funnel, stores, subscriptionMetrics] =
      await Promise.all([
        c.env.DB.prepare(
          `
        SELECT
    ...
    const transactionCount = (series.results || []).reduce(
      (total: number, row: any) =>
        total + Number(row.purchases || 0) + Number(row.renewals || 0),
      0,
    );
    return c.json({
      range_days: days,
      summary: {
        ...
        churn_rate:
          subscriptions > 0
            ? Number(subscriptionMetrics?.churned || 0) / subscriptions
            : 0,
        refund_rate: transactionCount > 0 ? refunds / transactionCount : 0,
        active_subscription_rate:
          subscriptions > 0 ? activeSubscriptions / subscriptions : 0,
      },
...
admin.post("/:projectId/paywalls", async (c) => {
  try {
    ...
    return c.json(
      { id: paywallId, draft_version_id: versionId, version: 1 },
      201,
    );
  } catch (error) {
    return replyError(c, error);
...
admin.get("/:projectId/paywalls/:id/versions", async (c) => {
  try {
    ...
    const paywall = await c.env.DB.prepare(
      ...
      .first();
    if (!paywall)
      throw purchasesError("paywall_not_found", "Paywall not found", 404);
    const rows = await c.env.DB.prepare(
...
admin.post("/:projectId/paywalls/:id/publish", async (c) => {
  try {
    ...
    if (!version)
      throw purchasesError(
        "paywall_version_not_found",
        "Paywall version not found",
        404,
      );
    await c.env.DB.batch([
      c.env.DB.prepare(
...
admin.post("/:projectId/targeting", async (c) => {
  try {
    ...
    const id = crypto.randomUUID();
    if (
      !["draft", "live", "scheduled", "inactive"].includes(
        data.state || "draft",
      )
    )
      throw purchasesError(
        "invalid_rule_state",
        "Invalid targeting rule state",
      );
    await c.env.DB.prepare(
      `
...
admin.patch("/:projectId/experiments/:id", async (c) => {
  try {
    ...
    if (
      data.state &&
      !["draft", "running", "paused", "stopped"].includes(data.state)
    )
      throw purchasesError(
    ...
    `,
      ...
      .bind(
        data.display_name ?? null,
        data.state ?? null,
        data.audience_conditions === undefined
          ? null
          : JSON.stringify(data.audience_conditions),
        data.starts_at ?? null,
        data.ends_at ?? null,
...
admin.post(
  "/:projectId/virtual-currencies/:currencyId/products/:productId",
  async (c) => {
    try {
      const project = await projectFor(c);
      requireAdmin(project);
    ...
    `,
      ...
      if (!Number.isSafeInteger(amount) || amount <= 0)
        throw purchasesError(
          ...
          "grant_amount must be a positive integer",
        );
      await c.env.DB.prepare(
        `
...
admin.post(
  "/:projectId/customers/:customerId/virtual-currencies/adjust",
  async (c) => {
    try {
      const project = await projectFor(c);
      requireAdmin(project);
      const data = await jsonBody(c);
      const adjustments =
        data.adjustments && typeof data.adjustments === "object"
          ? data.adjustments
          : {};
      const idempotencyKey =
        c.req.header("Idempotency-Key") || data.idempotency_key;
      if (!idempotencyKey)
        throw purchasesError(
      ...
      for (const [code, amountValue] of Object.entries(adjustments)) {
        ...
        if (!Number.isSafeInteger(amount) || amount === 0)
          throw purchasesError(
            "invalid_adjustment",
            `Invalid adjustment for ${code}`,
          );
        if (Number(balances[code] || 0) + amount < 0)
          throw purchasesError(
            "insufficient_virtual_currency",
            `Insufficient ${code} balance`,
            422,
          );
      }
      const statements = Object.entries(adjustments).map(
        ([code, amountValue]) =>
          c.env.DB.prepare(
            `
  ...
  try {
    ...
    const [evidence, actions, deadlines, events] = await Promise.all([
      ...
      c.env.DB.prepare(
        ...
        .all(),
      c.env.DB.prepare(
        "SELECT * FROM billing_refund_deadlines WHERE case_id = ? ORDER BY due_at",
      )
        .bind(c.req.param("caseId"))
        .all(),
      c.env.DB.prepare(
    ...
    const provider = String(
      (refundCase as { provider?: string }).provider || "",
    );
    return c.json({
...
admin.post("/:projectId/refunds/:caseId/evidence", async (c) => {
  try {
    ...
    const fileKey =
      typeof data.file_key === "string" ? data.file_key.trim() : "";
    if (!content && !fileKey)
      throw purchasesError(
        "evidence_content_required",
        "Evidence content or file_key is required",
      );
...
admin.post(
  "/:projectId/refunds/:caseId/evidence/:evidenceId/review",
  async (c) => {
    try {
      const project = await projectFor(c);
      requireAdmin(project);
...
admin.post("/:projectId/refunds/:caseId/actions", async (c) => {
  try {
    ...
    if (
      !supportedRefundActions(refundCase.provider).includes(
        actionType as RefundActionType,
      )
    ) {
      throw purchasesError(
...
admin.patch("/:projectId/refunds/:caseId/actions/:actionId", async (c) => {
  try {
    ...
    const payload = validateRefundActionPayload(
      action.provider,
      action.action_type,
      data.payload,
    );
...
admin.post(
  "/:projectId/refunds/:caseId/actions/:actionId/approve",
  async (c) => {
    try {
      const project = await projectFor(c);
      requireAdmin(project);
...
admin.post("/:projectId/exports", async (c) => {
  try {
    ...
    return c.json(
      { id, status: "pending", queued: Boolean(c.env.BILLING_QUEUE) },
      202,
    );
  } catch (error) {
    return replyError(c, error);
...
admin.get("/:projectId/paywall-event-types", async (c) =>
  c.json({ data: PAYWALL_EVENT_TYPES }),
);
