...
messaging.post(
	"/:projectId/conversations/:conversationId/realtime-ticket",
	authMiddleware,
	async (c) => {
		try {
			...
			const issued = await issueMessagingRealtimeTicket(c.env.DB, {
				...
				conversationId,
				userId: c.get("operatorId") ?? String(c.get("userId")),
			});
	...
	try {
		...
		if (!authorized)
			throw purchasesError(
				"messaging_realtime_ticket_invalid",
		...
		headers.set("X-SuperBoard-Internal-Token", c.env.MESSAGING_INTERNAL_TOKEN);
		headers.set("X-SuperBoard-Agent-Id", authorized.user_id);
		headers.set("X-OpenGrow-Internal-Token", c.env.MESSAGING_INTERNAL_TOKEN);
		headers.set("X-OpenGrow-Agent-Id", authorized.user_id);
		return c.env.MESSAGING.fetch(
			new Request(
...
messaging.all("/:projectId/*", authMiddleware, async (c) => {
	try {
		...
		headers.delete("authorization");
		headers.set("X-SuperBoard-Internal-Token", c.env.MESSAGING_INTERNAL_TOKEN);
		headers.set("X-SuperBoard-Agent-Id", c.get("operatorId") ?? String(c.get("userId")));
		headers.set("X-OpenGrow-Internal-Token", c.env.MESSAGING_INTERNAL_TOKEN);
		headers.set("X-OpenGrow-Agent-Id", c.get("operatorId") ?? String(c.get("userId")));
		const response = await c.env.MESSAGING.fetch(
			new Request(target, {
				method: c.req.method,
				headers,
...
export function requireMessagingAccess(method: string, suffix: string, role: string) {
	...
	if (mutating && administrative && !["owner", "admin"].includes(role)) {
		throw purchasesError(
			"insufficient_role",
			"Owner or admin access is required to change Chat configuration",
			403,
		);
	}
}
...
function requiredIdentifier(value: unknown, field: string) {
	...
	if (!parsed || parsed.length > 255)
		throw purchasesError(`${field}_invalid`, `A valid ${field} is required`, 422);
	return parsed;
}
	...
	return c.json(
		{
			...
			message:
				status >= 500
					...
					: tagged?.message || "Messaging request failed",
