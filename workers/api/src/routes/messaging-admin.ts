  ...
  async (c) => {
    try {
      ...
      const issued = await issueMessagingRealtimeTicket(c.env.DB, {
        ...
        projectExternalId: requiredIdentifier(
...
messaging.all("/:projectId/*", authMiddleware, async (c) => {
  try {
    ...
    const suffix = current.pathname.replace(
      /^\/api\/v2\/messaging\/projects\/[^/]+/,
      "",
    );
    requireMessagingAccess(c.req.method, suffix, project.role);
    const target = new URL(
      `https://messaging.internal/internal/projects/${project.id}${suffix}`,
    );
    target.search = current.search;
    const headers = new Headers(c.req.raw.headers);
    headers.delete("authorization");
    headers.set("X-SuperBoard-Internal-Token", c.env.MESSAGING_INTERNAL_TOKEN);
    headers.set("X-SuperBoard-Agent-Id", String(c.get("userId")));
    headers.set("X-OpenGrow-Internal-Token", c.env.MESSAGING_INTERNAL_TOKEN);
...
export function requireMessagingAccess(
  method: string,
  suffix: string,
  role: string,
) {
  ...
  const administrative =
    suffix === "/settings" || suffix.startsWith("/settings/entities");
