...
import {
	...
	forwardTransferredBillingLedger,
} from "./billing-ledger-transfer.js";
import {
	parseEntitlement,
	parseOffering,
	parsePackage,
...
app.post("/internal/v1/billing-ledger/export", exportBillingLedger);
app.post("/internal/v1/billing-ledger/finalize", finalizeBillingLedger);
app.use("/internal/v1/*", forwardTransferredBillingLedger);
...
app.delete("/internal/v1/packages/:id", async (c) => archive(c, "packages", "package", "active"));

app.get("/internal/v1/offerings", async (c) => c.json({ data: await offerings(c, false) }));
...
app.get("/internal/v1/offerings/:id", async (c) => {
	...
	if (!all[0]) throw httpError("offering_not_found", "Offering was not found", 404);
...
app.put("/internal/v1/offerings/:id", async (c) => {
	...
	return commitMutation(c, {
		...
		statements: [
			...
			c.env.DB.prepare("DELETE FROM offering_packages WHERE offering_id=?").bind(id),
			...input.package_ids.map((packageId, position) =>
				c.env.DB.prepare(
					"INSERT INTO offering_packages (offering_id, package_id, position) VALUES (?, ?, ?)",
...
app.get("/internal/v1/entitlements", async (c) => c.json({ data: await entitlements(c) }));
...
app.put("/internal/v1/entitlements/:id", async (c) => {
	...
	return commitMutation(c, {
		...
		statements: [
			...
			c.env.DB.prepare("DELETE FROM entitlement_products WHERE entitlement_id=?").bind(id),
			...input.product_ids.map((productId) =>
				c.env.DB.prepare(
					"INSERT INTO entitlement_products (entitlement_id, product_id) VALUES (?, ?)",
				).bind(id, productId),
...
app.post("/internal/v1/purchases", async (c) => {
	...
	const productRow = (await owned(c, "products", input.product_id, "product")) as EntityRow & {
		product_type: string;
	};
...
async function offerings(c: WorkerContext, onlyActive: boolean, id?: string): Promise<EntityRow[]> {
...
async function owned(
	...
	const row = await c.env.DB.prepare(`SELECT * FROM ${table} WHERE id=? AND project_id=?`)
		.bind(id, project(c))
		.first<EntityRow>();
