...
describe("Products Worker with D1", () => {
	...
	it("rejects unsigned and tampered gateway contexts", async () => {
		expect((await SELF.fetch("https://products.internal/internal/v1")).status).toBe(401);
		...
		const sdkHeaders = await signedHeaders("POST", "/internal/v1/offerings/resolve", 11, 0, "sdk");
		sdkHeaders.set("content-type", "application/json");
		const sdk = await SELF.fetch("https://products.internal/internal/v1/offerings/resolve", {
			method: "POST",
			headers: sdkHeaders,
			body: '{"placement":"main"}',
		});
	...
	it("runs catalog, offering, entitlement, purchase, subscription and refund flows idempotently", async () => {
		...
		const purchase = await data<{ id: string; entitlement_ids: string[] }>(purchaseResponse);
		...
		const details = await request("GET", `/internal/v1/purchases/${purchase.id}`);
		...
		expect(
			audit.results.every((row) => row.actor_id === "2" && typeof row.request_id === "string"),
		).toBe(true);
	...
	it("synchronizes a complete store catalog and exposes financial statistics", async () => {
		...
		const invalidPlatform = await request("GET", "/internal/v1/statistics?platform=desktop");
	...
	it("isolates resources by internal project id", async () => {
		const list = await request("GET", "/internal/v1/catalog/products", undefined, 12);
		await expect(list.json()).resolves.toEqual({ data: [] });
		const foreign = await request("GET", "/internal/v1/catalog/products/not-owned", undefined, 12);
