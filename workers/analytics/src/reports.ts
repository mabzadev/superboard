...
const REPORT_TYPES = ["dashboard", "funnel", "cohort", "retention", "query", "alert"] as const;
...
export async function createSavedReport(c: AnalyticsContext) {
	...
	const data = {
		...
		created_by: project.operatorId ?? String(project.actorId),
		created_at: now,
		updated_at: now,
	};
...
export async function deleteSavedReport(c: AnalyticsContext, id: string) {
	...
	return commitAnalyticsMutation(c.env.DB, {
		...
		statements: [
			c.env.DB.prepare("DELETE FROM analytics_saved_reports WHERE project_id = ? AND id = ?").bind(
				String(project.projectId),
				id,
			),
		],
