...
export async function updateApplication(
	...
	if (!current) {
		throw httpError("analytics_application_not_found", "Application not found", 404);
	}
	...
	const active = body.active === undefined ? Number(current.active) === 1 : body.active === true;
	const settings =
...
export async function createDashboard(c: AnalyticsContext): Promise<MutationResult> {
	const body = objectBody(await readJson(c.req.raw));
	...
	const layout = body.layout === undefined ? {} : jsonObject(body.layout, "layout");
	const data = {
		id,
		name,
		description,
...
export async function updateDashboard(c: AnalyticsContext, id: string): Promise<MutationResult> {
	...
	return mutation(c, {
		...
		statements: [
         ...
         WHERE project_id = ? AND id = ?`,
			).bind(name, description, visibility, stableAnalyticsJson(layout), now, projectId(c), id),
		],
	});
}

export async function deleteDashboard(c: AnalyticsContext, id: string): Promise<MutationResult> {
	...
	return deleteMutation(
		...
		id,
		c.env.DB.prepare("DELETE FROM analytics_dashboards WHERE project_id = ? AND id = ?").bind(
			projectId(c),
			id,
		),
	);
...
export async function createDashboardWidget(
	...
	const position = body.position === undefined ? {} : jsonObject(body.position, "position");
	const data = {
		id,
...
export async function deleteDashboardWidget(
	...
	const row = await c.env.DB.prepare(
		...
		.first();
	if (!row) throw httpError("analytics_widget_not_found", "Widget not found", 404);
	return deleteMutation(
		c,
		"dashboard_widget",
...
export async function analyzeEvent(c: AnalyticsContext) {
	const url = new URL(c.req.url);
	const eventName = requiredText(url.searchParams.get("event_name"), "event_name", 128);
	const from = iso(url.searchParams.get("from")) ?? "1970-01-01T00:00:00.000Z";
	const to = iso(url.searchParams.get("to")) ?? new Date().toISOString();
	const property = optionalText(url.searchParams.get("property"), 128);
	if (property && !/^[A-Za-z0-9_.:-]+$/u.test(property)) {
		throw httpError("analytics_property_invalid", "Event property is invalid", 400);
	}
...
export async function updateCrashGroup(
	...
	const body = objectBody(await readJson(c.req.raw));
	const resolved =
		body.resolved === undefined ? Number(current.resolved) === 1 : body.resolved === true;
	const assignee =
		body.assignee === undefined
			? nullableString(current.assignee)
			: optionalText(body.assignee, 255);
	const notes =
		body.notes === undefined ? nullableString(current.notes) : optionalText(body.notes, 4_000);
	const now = new Date().toISOString();
...
export async function upsertRemoteConfig(
	...
	const environment = choice(body.environment ?? c.get("project").environment, "environment", [
		"production",
		"test",
	] as const);
	const value = jsonValue(body.value, "value");
	const conditions = body.conditions === undefined ? [] : jsonArray(body.conditions, "conditions");
	...
	return mutation(c, {
		action: existing ? "analytics.remote_config.updated" : "analytics.remote_config.created",
...
export async function deleteRemoteConfig(
	...
	const row = await c.env.DB.prepare(
		...
		.first<{ id: string }>();
	if (!row)
		throw httpError("analytics_remote_config_not_found", "Remote config key not found", 404);
	return deleteMutation(
		c,
		"remote_config",
...
export async function resolveRemoteConfig(c: AnalyticsContext) {
	...
	const context = body.context === undefined ? {} : jsonObject(body.context, "context");
	const identity =
		optionalText(body.app_instance_id ?? body.user_id ?? body.anonymous_id, 255) ?? "anonymous";
	const rows = await c.env.DB.prepare(
	...
	for (const row of rows.results) {
		...
		if (await matchesConditions(projectId(c), key, identity, context, conditions)) {
...
export async function saveCohort(c: AnalyticsContext, id?: string): Promise<MutationResult> {
	const body = objectBody(await readJson(c.req.raw));
	...
	const enabled =
		body.enabled === undefined ? Number(current?.enabled ?? 1) === 1 : body.enabled === true;
	const now = new Date().toISOString();
	const data = {
		id: cohortId,
		name,
...
export async function evaluateCohort(c: AnalyticsContext, id: string): Promise<MutationResult> {
	const current = await cohortRow(c, id);
	const definition = parseJson<Record<string, unknown>>(current.definition_json, {});
	validateCohortDefinition(definition);
...
export async function deleteCohort(c: AnalyticsContext, id: string): Promise<MutationResult> {
	...
	return deleteMutation(
		...
		id,
		c.env.DB.prepare("DELETE FROM analytics_cohorts WHERE project_id = ? AND id = ?").bind(
			projectId(c),
			id,
		),
	);
...
export async function saveAlert(c: AnalyticsContext, id?: string): Promise<MutationResult> {
	const body = objectBody(await readJson(c.req.raw));
	...
	const name =
		optionalText(body.name, 120) ??
		(current ? String(current.name) : requiredText(body.name, "name", 120));
	const alertType = choice(body.alert_type ?? current?.alert_type, "alert_type", ALERT_TYPES);
	const definition =
		body.definition === undefined
			? parseJson(current?.definition_json, {})
			: jsonObject(body.definition, "definition");
...
export async function transitionAlertIncident(
	...
	const row = await c.env.DB.prepare(
		...
		.bind(projectId(c), id)
		.first();
	if (!row) throw httpError("analytics_incident_not_found", "Alert incident not found", 404);
	const now = new Date().toISOString();
	return mutation(c, {
...
export async function deleteAlert(c: AnalyticsContext, id: string): Promise<MutationResult> {
	await alertRow(c, id);
	return deleteMutation(
		c,
		"alert",
		...
		id,
		c.env.DB.prepare("DELETE FROM analytics_alerts WHERE project_id = ? AND id = ?").bind(
			projectId(c),
			id,
		),
	);
...
export async function saveHook(c: AnalyticsContext, id?: string): Promise<MutationResult> {
	const body = objectBody(await readJson(c.req.raw));
	...
	const name =
		...
		(current ? String(current.name) : requiredText(body.name, "name", 120));
	const endpoint =
		optionalText(body.endpoint_url, 2_048) ?? (current ? String(current.endpoint_url) : "");
	if (!isSafePublicHttpsUrl(endpoint))
		throw httpError("analytics_hook_url_invalid", "Hook endpoint must be a public HTTPS URL", 400);
	const eventTypes =
	...
	if (
		...
		eventTypes.some(
			(value) =>
				typeof value !== "string" ||
				(value !== "*" &&
					!/^(?:[A-Za-z][A-Za-z0-9._:-]{0,127}|\[CLY\]_[A-Za-z0-9._:-]{1,122})$/u.test(value)),
		)
	) {
		throw httpError(
	...
	const encryptedSecret = secret
		? await encryptAnalyticsConfiguration(c.env.ANALYTICS_CONFIG_ENCRYPTION_KEY, { secret })
...
export async function deleteHook(c: AnalyticsContext, id: string): Promise<MutationResult> {
	await hookRow(c, id);
	return deleteMutation(
		c,
		"hook",
		...
		id,
		c.env.DB.prepare("DELETE FROM analytics_hooks WHERE project_id = ? AND id = ?").bind(
			projectId(c),
			id,
		),
	);
...
export async function saveAnnotation(c: AnalyticsContext, id?: string): Promise<MutationResult> {
	const body = objectBody(await readJson(c.req.raw));
	...
	const color = optionalText(body.color, 32) ?? String(current?.color ?? "#6366f1");
	...
	return mutation(c, {
		action: current ? "analytics.annotation.updated" : "analytics.annotation.created",
		...
		statements: current
           ...
           WHERE project_id = ? AND id = ?`,
					).bind(title, description, annotationAt, color, now, projectId(c), annotationId),
				]
			: [
					c.env.DB.prepare(
...
export async function deleteAnnotation(c: AnalyticsContext, id: string): Promise<MutationResult> {
	...
	return deleteMutation(
		...
		id,
		c.env.DB.prepare("DELETE FROM analytics_annotations WHERE project_id = ? AND id = ?").bind(
			projectId(c),
			id,
		),
	);
...
export async function updateAnalyticsSettings(c: AnalyticsContext): Promise<MutationResult> {
	const body = objectBody(await readJson(c.req.raw));
...
async function dashboardRow(c: AnalyticsContext, id: string) {
	const row = await c.env.DB.prepare(
     ...
     FROM analytics_dashboards WHERE project_id = ? AND id = ?`,
		...
		.bind(projectId(c), id)
		.first<Record<string, unknown>>();
	if (!row) throw httpError("analytics_dashboard_not_found", "Dashboard not found", 404);
	return row;
}
...
async function crashGroupRow(c: AnalyticsContext, fingerprint: string) {
	const row = await c.env.DB.prepare(
     ...
     ORDER BY last_seen_at DESC LIMIT 1`,
		...
		.bind(projectId(c), fingerprint)
		.first<Record<string, unknown>>();
	if (!row) throw httpError("analytics_crash_not_found", "Crash group not found", 404);
	return row;
}
...
async function cohortRow(c: AnalyticsContext, id: string) {
	const row = await c.env.DB.prepare(
		...
		.bind(projectId(c), id)
		.first<Record<string, unknown>>();
	if (!row) throw httpError("analytics_cohort_not_found", "Cohort not found", 404);
	return row;
}
...
async function alertRow(c: AnalyticsContext, id: string) {
	const row = await c.env.DB.prepare(
		...
		.bind(projectId(c), id)
		.first<Record<string, unknown>>();
	if (!row) throw httpError("analytics_alert_not_found", "Alert not found", 404);
	return row;
}
...
async function annotationRow(c: AnalyticsContext, id: string) {
	const row = await c.env.DB.prepare(
		...
		.bind(projectId(c), id)
		.first<Record<string, unknown>>();
	if (!row) throw httpError("analytics_annotation_not_found", "Annotation not found", 404);
	return row;
}
...
async function matchesConditions(
	...
	for (const raw of conditions) {
		...
		if (condition.rollout_percentage !== undefined) {
			const percentage = boundedInteger(condition.rollout_percentage, 100, 0, 100);
		...
		if (operator === "in" && (!Array.isArray(expected) || !expected.includes(actual))) return false;
		if (operator === "contains" && !String(actual ?? "").includes(String(expected ?? "")))
			return false;
	}
	return true;
}
...
function validateAlertConfiguration(
	...
	for (const channel of channels) {
		...
		const recipient = requiredText(candidate.to, "channels.to", 320).toLowerCase();
...
function actorId(c: AnalyticsContext) {
	return c.get("project").operatorId ?? String(c.get("project").actorId);
}

function objectBody(value: unknown): Record<string, unknown> {
	if (!value || typeof value !== "object" || Array.isArray(value))
		throw httpError("analytics_body_invalid", "Request body must be an object", 400);
	return value as Record<string, unknown>;
}

function requiredText(value: unknown, field: string, maximum: number) {
	const parsed = optionalText(value, maximum);
	if (!parsed) throw httpError("analytics_field_invalid", `${field} is required`, 400);
	return parsed;
}
...
function optionalText(value: unknown, maximum: number): string | null {
	...
	if (!parsed || parsed.length > maximum)
		throw httpError("analytics_field_invalid", `Text is limited to ${maximum} characters`, 400);
	return parsed;
}
...
function jsonObject(value: unknown, field: string): Record<string, unknown> {
	if (!value || typeof value !== "object" || Array.isArray(value))
		throw httpError("analytics_field_invalid", `${field} must be an object`, 400);
...
function jsonArray(value: unknown, field: string): unknown[] {
	if (!Array.isArray(value))
		throw httpError("analytics_field_invalid", `${field} must be an array`, 400);
	jsonSize(value, field);
	return structuredClone(value);
}

function jsonValue(value: unknown, field: string): unknown {
	if (value === undefined) throw httpError("analytics_field_invalid", `${field} is required`, 400);
	jsonSize(value, field);
	return structuredClone(value);
}
...
function jsonSize(value: unknown, field: string) {
	if (stableAnalyticsJson(value).length > 65_536)
		throw httpError("analytics_field_too_large", `${field} is limited to 64 KB`, 413);
}
