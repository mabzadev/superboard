...
import configuration, {
  getPublicMessagingConfiguration,
} from "./configuration";
...
app.get("/v1/configuration", async (c) => {
  ...
  return c.json({
    data: await getPublicMessagingConfiguration(c.env.DB, projectId),
  });
});
...
app.use("/internal/*", async (c, next) => {
  await requireInternal(
    c.env,
    c.req.header("X-SuperBoard-Internal-Token") ||
      c.req.header("X-OpenGrow-Internal-Token"),
  );
  await next();
});
...
app.get(
  ...
  async (c) => {
    ...
    const rows = await c.env.DB.prepare(
      "SELECT * FROM messages WHERE conversation_id = ? ORDER BY sequence ASC LIMIT 500",
    )
...
app.post(
  "/internal/projects/:projectId/conversations/:conversationId/read",
  async (c) => {
    const projectId = positiveInt(
      c.req.param("projectId"),
      "project_id_invalid",
...
app.post(
  "/internal/projects/:projectId/conversations/:conversationId/typing",
  async (c) => {
    const projectId = positiveInt(
      c.req.param("projectId"),
      "project_id_invalid",
...
app.get(
  "/internal/projects/:projectId/conversations/:conversationId/ws",
  async (c) => {
    const projectId = positiveInt(
      c.req.param("projectId"),
      "project_id_invalid",
...
app.post(
  "/internal/projects/:projectId/conversations/:conversationId/attachments",
  async (c) => {
    const projectId = positiveInt(
      c.req.param("projectId"),
      "project_id_invalid",
...
app.get(
  "/internal/projects/:projectId/conversations/:conversationId/attachments/:messageId",
  async (c) => {
    const projectId = positiveInt(
      c.req.param("projectId"),
      "project_id_invalid",
...
app.onError((error, c) => {
  ...
  if (status >= 500)
    console.error(
      JSON.stringify({
        event: "messaging_request_failed",
  ...
  return c.json(
    {
      ...
      message:
        status >= 500 ? "Messaging is temporarily unavailable" : error.message,
...
async function attachmentResponse(
  ...
  `,
    ...
    .bind(messageId, conversation.id, allowPrivate ? 1 : 0)
    .first<{
