...
CREATE TABLE IF NOT EXISTS messaging_project_settings (
...
CREATE TABLE IF NOT EXISTS messaging_configuration_entities (
  id TEXT PRIMARY KEY,
...
CREATE INDEX IF NOT EXISTS idx_messaging_configuration_entities_project_type
  ON messaging_configuration_entities(project_id, entity_type, position, name);

CREATE TABLE IF NOT EXISTS messaging_configuration_audit_events (
  id TEXT PRIMARY KEY,
  project_id INTEGER NOT NULL,
  entity_id TEXT,
...
CREATE INDEX IF NOT EXISTS idx_messaging_configuration_audit_project_created
  ON messaging_configuration_audit_events(project_id, created_at DESC);

CREATE TRIGGER IF NOT EXISTS messaging_configuration_audit_no_update
BEFORE UPDATE ON messaging_configuration_audit_events
BEGIN
  SELECT RAISE(ABORT, 'messaging configuration audit events are immutable');
END;

CREATE TRIGGER IF NOT EXISTS messaging_configuration_audit_no_delete
BEFORE DELETE ON messaging_configuration_audit_events
BEGIN
  SELECT RAISE(ABORT, 'messaging configuration audit events are immutable');
END;
