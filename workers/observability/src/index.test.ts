...
describe("OpenGrow observability Worker", () => {
  it("writes sanitized Tail Worker trace points", async () => {
    const points: AnalyticsEngineDataPoint[] = [];
    const env = environment(points);
    const event = {
      event: {
        request: { method: "GET", url: "https://api.example.com/users?token=secret", headers: {} },
        response: { status: 200 },
      },
    ...
    expect(points[0].blobs).toEqual([
      "v1", "opengrow-api-dev", "default", "fetch", "GET", "/users", "200", "ok", "development",
    ]);
  ...
  it("queries a bounded, whitelisted Analytics Engine window", async () => {
    ...
    expect(await response?.json()).toMatchObject({
      status: "ok",
      windowMinutes: 15,
      rows: [{ service: "opengrow-api-dev", invocations: 4, averageCpuMs: 2.5 }],
    });
  });
});
...
function environment(
  ...
  return {
    ANALYTICS: { writeDataPoint: (point?: AnalyticsEngineDataPoint) => { if (point) points.push(point); } },
