...
import { signPluginTaskRequest, withPluginTaskLifecycle } from "@superboard/contracts/plugin-task";
	...
	async tail(events: TraceItem[], env: ObservabilityEnv): Promise<void> {
		for (const trace of events) {
			...
			if (env.API_SERVICE && env.SUPERBOARD_INSTANCE_ID) {
				...
				if (!response.ok) throw new Error(`Observability ingestion failed: ${response.status}`);
			} else if (env.SUPERBOARD_PLUGIN_LIFECYCLE === "required") {
				throw new Error("OBSERVABILITY_INGESTION_BINDING_REQUIRED");
			}
		}
	},
...
async function authorized(request: Request, env: ObservabilityEnv): Promise<boolean> {
	...
	return matchesAnySecret(
		provided,
		configuredSecrets(env.OBSERVABILITY_INTERNAL_TOKEN, env.OBSERVABILITY_INTERNAL_TOKEN_PREVIOUS),
	);
}
...
function cloudflareError(payload: unknown, status: number): string {
	if (isRecord(payload) && Array.isArray(payload.errors)) {
		const message = payload.errors
			.flatMap((error) => (isRecord(error) && text(error.message) ? [text(error.message)] : []))
			.join("; ");
...
export default withPluginTaskLifecycle("supbrd-plugmod-observability", lifecycleWorker);
