...
Dans le dépôt, [la découverte Melody](../../packages/plugins/supbrd-plug-identity/worker/src/melody/handlers/other.ts) dérive issuer et endpoints de `AUTH_SERVER_URL`. [La signature des jetons d’application](../../packages/plugins/supbrd-plug-identity/worker/src/crypto.ts) utilise `SUPERBOARD_IDENTITY_ISSUER`, aussi vérifié à la réception. Ces deux configurations doivent être inventoriées ; changer seulement la route publique ne suffit pas.
