...
- Upstream SuperBoard backend is Rails with PostgreSQL, Redis, Sidekiq, Devise,
  Doorkeeper, Active Storage, payment processor, RPush, MCP, and mobile push integrations.
- SuperBoard backend is a Hono Worker on Cloudflare Workers with D1-compatible SQL.
...
- SuperBoard diagnostics endpoints now run on Workers with structured logs, D1/KV
  checks, protected access and intentional exception testing.
- SuperBoard admin and automation machine routes are available on Workers with
  `X-AUTH`/`ADMIN_API_KEY` for Firebase CSV link migration, event flushing,
  visitor automation metrics and link details. Legacy SuperBoard subscription
...
- The Worker SDK route now supports both the existing Cloudflare `X-Api-Key`
  adapter and upstream SuperBoard `PROJECT-KEY`/`LINKSQUARED` endpoints for
...
6. Rust Workers are allowed by Cloudflare, but language choice is secondary to
   preserving SuperBoard behavior and schema compatibility.
...
- The historical Rails source and the other former upstream repositories are
  ...
  SuperBoard release blocker under ADR-001.
