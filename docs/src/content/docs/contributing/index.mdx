...
2. **Start the blog template**
   ...
   Sample content is seeded automatically on the first request to an empty database.
...
For iterating on core packages alongside the blog template, run two terminals:

```bash
# Terminal 1 — rebuild packages/core on change
cd packages/core && pnpm dev

# Terminal 2 — run the blog template
