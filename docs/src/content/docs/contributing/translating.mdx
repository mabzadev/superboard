...
1. **Compile and run the blog template:**

   ```bash
   pnpm run locale:compile
   pnpm build
   pnpm --filter @emdash-cms/template-blog dev
   ```
...
To enable it, add the following to your `.env` file in the template directory:

```ini title="templates/blog/.env"
