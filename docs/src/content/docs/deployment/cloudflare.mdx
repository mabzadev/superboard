...
When this provider is present during first-run setup, the administrator can be
created by email instead of by passkey. EmDash sends a single-use verification
link that expires after 15 minutes and creates the administrator only after the
recipient opens it. Later magic-link sign-ins also provide the recent
reauthentication required by sensitive administrator operations.
