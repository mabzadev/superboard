...
Les aliases publics `projectId` et `pid` sont résolus vers l’`instance_id` canonique. Une divergence entre aliases ferme l’écriture. Les contrats publics ne changent pas : la fixture commune `tests/fixtures/contracts/emdash-store-parity/v1.json` traverse les vrais encodeurs, bridges ou modèles de JavaScript, React Native, Flutter et FlutterFlow.
...
node --test tests/checks/emdash/parity-matrix.test.mjs
node --test tests/checks/database/restore-proof.test.mjs
