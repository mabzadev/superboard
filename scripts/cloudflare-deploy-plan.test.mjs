...
test("the deploy orchestrator consumes a verified batch before its Worker loop", async () => {
	const deployAll = await readFile(new URL("./cloudflare-deploy-all.mjs", import.meta.url), "utf8");
	const deployService = await readFile(new URL("./cloudflare-deploy.mjs", import.meta.url), "utf8");
	const allowlist = await readFile(new URL("./superboard-allowlist.mjs", import.meta.url), "utf8");
