...
test("restores D1 with FTS5 and R2/KV into isolated stores", () => {
	...
	source.exec(`${contract.tables[0].create_sql}; INSERT INTO documents VALUES ('a','alpha migration'),('b','beta'); ${contract.fts[0].create_sql}; INSERT INTO documents_fts SELECT * FROM documents;`);
...
test("fails closed when an immutable snapshot is changed", () => {
	...
	assert.throws(() => restoreD1LogicalSnapshot(new DatabaseSync(":memory:"), snapshot), CHECKSUM_MISMATCH_PATTERN);
});
