...
test("required secrets adapt to capture and SMTP transports", async () => {
  const development = (await loadTarget("mbza-development")).target;
  const production = (await loadTarget("vocostar")).target;
  const developmentEmail = requiredSecretInventory(development, "development")
    .find(({ service }) => service === "email");
  const productionEmail = requiredSecretInventory(production, "production")
    .find(({ service }) => service === "email");

  assert.deepEqual(developmentEmail.names, ["EMAIL_INTERNAL_TOKEN", "MAIL_PREVIEW_TOKEN"]);
  assert.ok(productionEmail.names.includes("SMTP_PASSWORD"));
...
test("every required secret is declared in the upload allowlist", async () => {
  for (const [targetName, environment] of [["mbza-development", "development"], ["vocostar", "production"]]) {
    const target = (await loadTarget(targetName)).target;
    const allowed = new Map(secretInventory(target).map(({ service, names }) => [service, new Set(names)]));
    for (const requirement of requiredSecretInventory(target, environment)) {
      for (const name of requirement.names) {
        assert.ok(allowed.get(requirement.service)?.has(name), `${targetName}/${requirement.service} does not allow ${name}`);
      }
...
test("billing encryption accepts the keyring or transitional single key", async () => {
  ...
  const requirement = requiredSecretInventory(target, "production")
    .find(({ service }) => service === "billing");
  const base = Object.fromEntries(requirement.names.map((name) => [name, true]));
  const configured = { billing: [...Object.keys(base), "STORE_CREDENTIALS_ENCRYPTION_KEYS"] };
...
test("API encryption is required independently from the billing execution mode", async () => {
  for (const [targetName, environment] of [["mbza-development", "development"], ["vocostar", "production"]]) {
    const target = (await loadTarget(targetName)).target;
    const requirement = requiredSecretInventory(target, environment)
