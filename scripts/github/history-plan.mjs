...
import { loadGitHubControlPlane } from "./readiness.mjs";

const root = resolve(fileURLToPath(new URL("../../", import.meta.url)));

export function inspectGitHistory(path, repository, run = (args) => runGit(path, args)) {
	...
	const remoteMain = optional(run(["rev-parse", "--verify", "refs/remotes/origin/main"]));
	const remoteDev = optional(run(["rev-parse", "--verify", "refs/remotes/origin/dev"]));
	const remoteDevTree = remoteDev
		? optional(run(["rev-parse", "--verify", `${remoteDev}^{tree}`]))
		: null;
	const archiveBranch = remoteMain ? `audit/pre-superboard-main-${remoteMain.slice(0, 12)}` : null;
	const remoteArchive = archiveBranch
		? optional(run(["rev-parse", "--verify", `refs/remotes/origin/${archiveBranch}`]))
		: null;
	const mergeBase = head && remoteMain ? optional(run(["merge-base", head, remoteMain])) : null;
	const remoteMainDevMergeBase =
		remoteMain && remoteDev ? optional(run(["merge-base", remoteMain, remoteDev])) : null;
	...
	return {
		...
		commonRemoteMainDevHistory: remoteMain && remoteDev ? Boolean(remoteMainDevMergeBase) : null,
		remoteDevMergeBase,
		remoteDevFastForward: head && remoteDev ? remoteDevMergeBase === remoteDev : null,
		clean: status.length === 0,
		changedPaths: status ? status.split("\n").filter(Boolean).length : 0,
		archiveBranch,
		remoteArchive,
		remoteMainPreserved: Boolean(remoteMain && remoteArchive && remoteMain === remoteArchive),
	};
}
...
export function buildGitHubHistoryPlan(manifest, inspections) {
	...
	const repositories = Object.values(manifest.repositories).map((repository) => {
		...
		} else if (state.remoteMain !== state.head && !state.remoteMainPreserved) {
			...
			status:
				blockers.length > 0
					...
					: operations.length > 0
						...
						: "ready",
			unrelatedRemoteMain:
				state.head && state.remoteMain ? state.commonHistoryWithRemoteMain === false : null,
			bridgeProcedure,
			blockers,
			operations,
		};
	...
	return {
		...
		ready: repositories.every(
			({ blockers, operations }) => blockers.length === 0 && operations.length === 0,
		),
		repositories,
...
async function main() {
	...
	const output = process.argv.includes("--bridge-only") ? buildGitHubHistoryBridgePlan(plan) : plan;
