...
export async function loadGitHubControlPlane() {
	const manifest = JSON.parse(
		await readFile(resolve(root, "scripts/config/github-control-plane.json"), "utf8"),
	);
	const schema = JSON.parse(
		await readFile(resolve(root, "scripts/config/github-control-plane.schema.json"), "utf8"),
	);
	const validate = new Ajv2020({ allErrors: true }).compile(schema);
	if (!validate(manifest)) {
		throw new Error(`Invalid GitHub control-plane manifest: ${JSON.stringify(validate.errors)}`);
	}
...
export function validateEnvironmentProtections(manifest) {
	for (const repository of Object.values(manifest.repositories || {})) {
		for (const [environmentName, environment] of Object.entries(repository.environments || {})) {
			...
			if (protection.enforcement === "enforced" && eligible < protection.minimumEligibleReviewers) {
				throw new Error(
		...
		repositories: Object.fromEntries(
			Object.entries(manifest.repositories).map(([role, repository]) => [
				role,
				{
					...
					branches: Object.entries(repository.branches).map(([name, policy]) => ({
						name,
						requiredCheck: policy.requiredCheck,
						requirePullRequest: policy.requirePullRequest,
						requiredApprovals: policy.requiredApprovals,
					})),
					environments: Object.entries(repository.environments).map(([name, environment]) => ({
						name,
						...
						protection: environment.protection
							? {
									...
									minimumEligibleReviewers: environment.protection.minimumEligibleReviewers,
									reviewers: environment.protection.reviewers.map(({ type, name }) => ({
										type,
										name,
									})),
									deploymentPolicies: [...environment.protection.deploymentPolicies],
								}
							: null,
					})),
...
export function inspectRepository(repository, run = runGh) {
	...
	const workflowPermissionsPayload = resultJson(
		run(["api", `repos/${repository.nameWithOwner}/actions/permissions/workflow`]),
	);
	...
	const dependabotSecurityUpdatesPayload = resultJson(dependabotSecurityUpdatesResult);
	const security = {
		vulnerabilityAlerts: {
	...
	security.ready =
		security.vulnerabilityAlerts.enabled === security.vulnerabilityAlerts.required &&
		security.dependabotSecurityUpdates.enabled === security.dependabotSecurityUpdates.required &&
	...
	const branches = Object.entries(repository.branches).map(([name, expected]) => {
		const branch = run(["api", `repos/${repository.nameWithOwner}/branches/${name}`]);
		const branchPayload = resultJson(branch);
		const protectionResult = branch.ok
			? run(["api", `repos/${repository.nameWithOwner}/branches/${name}/protection`])
			: { ok: false, stdout: "" };
		...
		const policyMatches =
			...
			(!expected.requirePullRequest || approvals >= expected.requiredApprovals) &&
		...
		return {
			...
			approvalsMatch: !expected.requirePullRequest || approvals >= expected.requiredApprovals,
			codeOwnerReviewsMatch: !expected.requireCodeOwnerReviews || codeOwnerReviews,
			secureSettingsMatch,
		};
	});
	...
	const environmentResult = run(["api", `repos/${repository.nameWithOwner}/environments`]);
	const environmentPayload = resultJson(environmentResult);
	const existingEnvironments = environmentPayload
		? new Set((environmentPayload.environments || []).map((entry) => entry.name))
		: new Set();
	const environments = Object.entries(repository.environments).map(([name, expected]) => {
		const exists = existingEnvironments.has(name);
		const variablesPayload = exists
			? resultJson(run(["api", `repos/${repository.nameWithOwner}/environments/${name}/variables`]))
			: null;
		const secretsPayload = exists
			? resultJson(run(["api", `repos/${repository.nameWithOwner}/environments/${name}/secrets`]))
			: null;
		const environmentDetails =
			exists && expected.protection
				? resultJson(run(["api", `repos/${repository.nameWithOwner}/environments/${name}`]))
				: null;
		const deploymentPoliciesPayload =
			environmentDetails?.deployment_branch_policy?.custom_branch_policies === true
				? resultJson(
						run([
							"api",
		...
		const remoteSecrets = new Set((secretsPayload?.secrets || []).map((secret) => secret.name));
		const variables = Object.entries(expected.variables).map(([variableName, expectedValue]) => ({
			name: variableName,
	...
	const repositorySecretsPayload = repository.repositorySecrets.length
		? resultJson(run(["api", `repos/${repository.nameWithOwner}/actions/secrets`]))
		: { secrets: [] };
...
function environmentProtectionState(
	...
	const preventSelfReviewMatches = preventSelfReview === expected.preventSelfReview;
	...
	const remotePolicies = (deploymentPoliciesPayload?.branch_policies || [])
		...
		.sort((left, right) =>
			`${left.type}:${left.pattern}`.localeCompare(`${right.type}:${right.pattern}`),
		);
	const expectedPolicies = [...expected.deploymentPolicies].sort((left, right) =>
		`${left.type}:${left.pattern}`.localeCompare(`${right.type}:${right.pattern}`),
	);
	const deploymentPoliciesMatch =
		environmentDetails?.deployment_branch_policy?.protected_branches === false &&
		environmentDetails?.deployment_branch_policy?.custom_branch_policies === true &&
		JSON.stringify(remotePolicies.map(({ type, pattern }) => ({ type, pattern }))) ===
			JSON.stringify(expectedPolicies);
	const eligibleReviewerCount = reviewerAccess.filter((reviewer) => reviewer.eligible).length;
	const eligibleReviewersReady = eligibleReviewerCount >= expected.minimumEligibleReviewers;
	const matches =
...
function environmentReviewerAccess(repository, reviewer, run) {
	...
	if (reviewer.type === "User") {
		const payload = resultJson(
			run(["api", `repos/${repository.nameWithOwner}/collaborators/${reviewer.name}/permission`]),
		);
	...
	const teamPayload = resultJson(run(["api", `orgs/${owner}/teams/${reviewer.name}`]));
	const repositoryPayload = resultJson(
		run(["api", `orgs/${owner}/teams/${reviewer.name}/repos/${owner}/${repositoryName}`]),
	);
	const permission = repositoryPayload?.role_name || repositoryPayload?.permission || null;
	const eligible =
...
export function remoteBranchHistoryState({ required, mainSha, devSha, comparisonOk, comparison }) {
	if (!required) {
		return {
			required: false,
			ready: true,
...
export function inspectReleaseProtection(repository, run = runGh) {
	...
	const immutableReleases = {
		...
		enabled: immutableResult.ok && (immutablePayload === null || immutablePayload.enabled === true),
	...
	if (expectedRuleset) {
		const rulesetsPayload = resultJson(run(["api", `${base}/rulesets?targets=tag&per_page=100`]));
		const candidate = Array.isArray(rulesetsPayload)
			? rulesetsPayload.find(
					(ruleset) => ruleset.name === expectedRuleset.name && ruleset.target === "tag",
				)
			: null;
	...
	const releaseInventoryReadable = releasesResult.ok && Array.isArray(releasesPayload);
	...
	return {
		...
		mutableReleaseCompensationReady,
		note: "Immutable releases affect only future publications. Existing mutable releases are accepted only when they contain no assets and an exact active no-bypass tag ruleset blocks tag update and deletion.",
	};
}
...
async function main() {
	...
	if (!process.argv.includes("--remote")) {
		process.stdout.write(`${JSON.stringify({ mode: "offline-plan", ...plan }, null, 2)}\n`);
		return;
	}
