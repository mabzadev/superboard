...
export function inspectRepositoryAvailability(repository, run = runGitHubAvailability) {
	const result = run([
		"api",
		"--include",
		"-H",
...
export function buildGitHubBootstrapPlan(manifest, inspections) {
				...
				blockers: [
					{
						...
						message: "The repository must be inspected before a creation plan is safe.",
					},
				],
			};
		}
	...
	const plan = {
		...
		ready: repositories.every(
			(repository) => repository.operations.length === 0 && repository.blockers.length === 0,
		),
		repositories,
	};
...
export function repositoryCreationRequest(repository, owner) {
	...
	if (!owner || owner.login !== repositoryOwner) {
		throw new Error(`Repository owner mismatch for ${repository.nameWithOwner}`);
	}
	if (!new Set(["public", "private"]).has(repository.visibility)) {
		throw new Error(`Unsupported repository visibility ${repository.visibility}`);
	}
	const endpoint =
		owner.type === "user"
			? "user/repos"
...
export function applyGitHubBootstrapPlan(
	...
	if (confirm !== plan.confirmation) {
		throw new Error(`Refusing GitHub repository creation: pass --confirm ${plan.confirmation}`);
	}
	const blockers = plan.repositories.flatMap((repository) => repository.blockers);
	if (blockers.length > 0) {
		throw new Error("Refusing GitHub repository creation while inspection blockers remain");
	}
	...
	for (const repositoryPlan of plan.repositories) {
		for (const operation of repositoryPlan.operations) {
			if (operation.type !== "create-repository") {
				throw new Error(`Unsupported GitHub bootstrap operation: ${operation.type}`);
			}
