...
export async function loadDeploymentMatrix() {
	const configuration = await readJson(
		resolve(root, "scripts/config", "cloudflare-deployments.json"),
	);
...
export function validateDeploymentConfiguration(configuration) {
	...
	const rootKeys = Object.keys(configuration).filter((key) => key !== "$schema");
	if (
		rootKeys.length !== 2 ||
		!rootKeys.includes("schemaVersion") ||
		!rootKeys.includes("deployments")
	) {
		throw new Error("Cloudflare deployment matrix contains unknown root fields");
	}
	...
	for (const deployment of configuration.deployments) {
		if (
			...
			JSON.stringify(Object.keys(deployment).sort()) !== JSON.stringify(expectedKeys)
		) {
		...
		if (!new Set(["development", "production"]).has(deployment.cloudflareEnvironment)) {
			throw new Error("Cloudflare environment is invalid");
		}
		const expectedEnvironment = deployment.branch === "dev" ? "development" : "production";
		if (deployment.cloudflareEnvironment !== expectedEnvironment) {
			throw new Error(`${deployment.branch} must deploy only ${expectedEnvironment}`);
		}
		const authority = deployment.automaticDeployment?.authority;
		if (!new Set(["cloudflare-workers-builds", "github-actions"]).has(authority)) {
		...
		if (authority === "cloudflare-workers-builds") {
			...
			if (
				...
				deployment.automaticDeployment.deployCommand !== expectedDeployCommand ||
			...
			throw new Error("GitHub Actions deployment authority contains unsupported settings");
		}
...
export function selectDeployments(configuration, branch, { authority } = {}) {
	...
	if (deployments.length === 0) {
		throw new Error(`No ${authority} Cloudflare deployment is declared for ${branch}`);
	}
	const reference = deployments.filter((deployment) => deployment.referenceAcceptance);
	if (reference.length > 1) {
		throw new Error(`${branch} declares more than one reference acceptance environment`);
	}
	return {
...
export function resolveDeploymentBranch({ explicitBranch, githubRefName, currentBranch }) {
	const branch = String(explicitBranch || githubRefName || currentBranch || "").trim();
	if (!new Set(["dev", "main"]).has(branch)) {
		throw new Error(
...
export function validateControlPlaneCoverage(configuration, controlPlane) {
	...
	for (const deployment of configuration.deployments) {
		...
		if (environment.variables?.SUPERBOARD_TARGET == null) {
			throw new Error(`${deployment.githubEnvironment} must define SUPERBOARD_TARGET`);
		}
...
async function main() {
	...
	const controlPlane = await readJson(resolve(root, "scripts/config", "github-control-plane.json"));
	validateControlPlaneCoverage(configuration, controlPlane);
	process.stdout.write(
		`${JSON.stringify(selectDeployments(configuration, branch, { authority: args.authority }))}\n`,
	);
}
