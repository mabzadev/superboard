...
import { githubJsonRequest, repositorySettingsBody, runGitHubMutation } from "./api.mjs";
import { inspectRepository, loadGitHubControlPlane } from "./readiness.mjs";
...
export function buildGitHubReconcilePlan(manifest, inspections) {
	const byName = new Map(inspections.map((inspection) => [inspection.nameWithOwner, inspection]));
	const repositories = Object.values(manifest.repositories).map((repository) => {
		...
		if (
			...
			blockers.push({
				type: "remote-state",
				message: "GitHub returned an incomplete repository response; no mutation is safe.",
			});
			return {
				nameWithOwner: repository.nameWithOwner,
				operations,
		...
		if (!state.releaseProtection.releaseInventoryReadable) {
			blockers.push({
				...
				message: "GitHub releases could not be inspected; no release-protection mutation is safe.",
			});
		}
		const unsafeMutableReleases = (state.releaseProtection.mutablePublishedReleases || []).filter(
			({ tagName, assetCount }) =>
				assetCount > 0 ||
		...
		const branches = new Map(state.branches.map((branch) => [branch.name, branch]));
		...
		for (const [environmentName, expected] of Object.entries(repository.environments)) {
			...
			for (const variableName of Object.keys(expected.variables)) {
				...
				if (!variable?.configured) {
					operations.push({
						type: variable?.exists ? "update-environment-variable" : "create-environment-variable",
						environment: environmentName,
						name: variableName,
					});
				}
			...
			if (protection?.enforcement === "pending-external") {
				manual.push({
					...
					remotelyEligibleReviewers: environment?.protection?.eligibleReviewerCount ?? 0,
				});
				...
				const remotePolicies = new Map(
					(remoteProtection?.remote?.deploymentPolicies || []).map((policy) => [
						`${policy.type}:${policy.pattern}`,
						policy,
					]),
				);
				const expectedPolicies = new Set(
					protection.deploymentPolicies.map((policy) => `${policy.type}:${policy.pattern}`),
				);
		...
		for (const secret of state.repositorySecrets ||
			...
			if (!secret.configured) manual.push({ type: "set-repository-secret", name: secret.name });
		}
...
export function applyGitHubReconcilePlan(
	...
	const blockers = plan.repositories.flatMap((repository) => repository.blockers);
	if (blockers.length > 0) {
		throw new Error("Refusing partial GitHub mutation while structural blockers remain");
	}
	...
	for (const repositoryPlan of plan.repositories) {
		...
		if (!repository) throw new Error(`Unknown repository ${repositoryPlan.nameWithOwner}`);
...
export function mutationRequest(repository, operation) {
	...
	switch (operation.type) {
		case "update-repository-settings":
			return githubJsonRequest("PATCH", base, repositorySettingsBody(repository));
		...
		case "create-tag-ruleset": {
			...
			if (!ruleset) {
				throw new Error(`No tag ruleset is declared for ${repository.nameWithOwner}`);
			}
		...
		case "put-branch-protection":
			return githubJsonRequest("PUT", `${base}/branches/${operation.branch}/protection`, {
				...
				required_pull_request_reviews: {
					...
					require_code_owner_reviews: operation.requireCodeOwnerReviews === true,
		...
		case "put-environment":
			return githubJsonRequest("PUT", `${base}/environments/${operation.environment}`, {});
		case "put-environment-protection": {
			const protection = repository.environments[operation.environment]?.protection;
			if (!protection || protection.enforcement !== "enforced") {
				throw new Error(`Environment protection is not enforceable for ${operation.environment}`);
			}
			return githubJsonRequest("PUT", `${base}/environments/${operation.environment}`, {
		...
		case "update-environment-variable": {
			const value = repository.environments[operation.environment]?.variables[operation.name];
			if (typeof value !== "string") {
				throw new Error(`Unknown GitHub variable ${operation.environment}/${operation.name}`);
			}
			const method = operation.type === "create-environment-variable" ? "POST" : "PATCH";
		...
		default:
			throw new Error(`Unsupported GitHub reconciliation operation: ${operation.type}`);
	}
}

function releaseTagMatches(pattern, tagName) {
	const escaped = String(pattern).replace(/[|\\{}()[\]^$+?.]/gu, "\\$&");
	return new RegExp(`^${escaped.replaceAll("*", ".*")}$`, "u").test(`refs/tags/${tagName || ""}`);
}
...
async function main() {
	...
	process.stdout.write(`${JSON.stringify({ mode: "applied", applied }, null, 2)}\n`);
	if (plan.repositories.some((repository) => repository.manual.length > 0)) process.exitCode = 2;
}
