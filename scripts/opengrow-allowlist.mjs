...
if (!new Set(["add", "revoke", "list", "bootstrap"]).has(action)) {
  throw new Error("Usage: npm run allowlist -- <add|revoke|list|bootstrap> --target <slug> --environment <staging|production> [--email user@example.com]");
}
...
const cloudflareEnv = { ...process.env, CLOUDFLARE_ACCOUNT_ID: target.accountId };
...
function sqlFor(selectedAction, selectedRealm, selectedEmail) {
  ...
  if (selectedAction === "bootstrap") {
    const fullAccessSql = target.accessMode === "full"
      ? `UPDATE instances SET revenue_collection_enabled = 1, quota_exceeded = 0,
           last_quota_warning_sent_at = NULL, last_quota_exceeded_sent_at = NULL,
