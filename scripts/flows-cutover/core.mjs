...
export function confirmationValue(operation, projectRef, windowId, planId = "none") {
	...
	if (
		!new Set(["CUTOVER", "FREEZE", "THAW", "ROLLBACK", "ROUTE-ON", "ROUTE-OFF"]).has(normalized)
	) {
...
export function routingVerificationEvidence(plan, verification) {
	...
	if (
		...
		throw new Error(
			"Flows routing requires a successful verification report for this exact protected plan/project",
		);
	}
	return {
...
export function verifyMbzaRoutingGate({ inventory, scopes, checkedAt = new Date().toISOString() }) {
	...
	if (
		canonicalJson(canonicalInventory) !==
		canonicalJson(
			requiredScopes.map(({ projectId, projectRef }) => ({
		...
		throw new Error(
			"MBZA project inventory changed; regenerate the explicit two-scope routing gate before API promotion",
		);
	}
	const reports = requiredScopes.map((requiredScope) => {
		...
		if (
			...
			throw new Error(
				`${requiredScope.projectRef} routing artifacts do not match the fixed MBZA project scope`,
			);
		}
...
export function createCutoverWindow({
	projectRef,
	startsAt,
	endsAt,
	reason,
	...
	if (Date.parse(start) >= Date.parse(end))
		throw new Error("Cutover window startsAt must be before endsAt");
	if (!String(reason || "").trim() || !String(approvedBy || "").trim()) {
		throw new Error("Cutover window requires reason and approvedBy");
	}
	if (!/^[A-Za-z0-9._-]{8,128}$/u.test(String(windowId)))
		throw new Error("Cutover window id is invalid");
	return {
		schema_version: FLOWS_CUTOVER_SCHEMA_VERSION,
		target: FLOWS_CUTOVER_TARGET,
		environment: FLOWS_CUTOVER_ENVIRONMENT,
...
export function normalizeRows(rows, entity) {
	...
	const normalized = rows.map((row) => {
		if (!row || typeof row !== "object" || Array.isArray(row))
			throw new Error(`${entity.id}: every row must be an object`);
		if (Object.hasOwn(row, "organization_id"))
			throw new Error(
				`${entity.id}: organization_id is forbidden in project-scoped Flows cutover rows`,
			);
		const output = {};
		for (const column of entity.columns) {
			...
			if (entity.numericColumns.includes(column) && value !== null) {
				...
				if (!Number.isSafeInteger(numeric))
					throw new Error(`${entity.id}.${column}: expected a safe integer`);
				value = numeric;
			}
			output[column] = value;
		}
	...
	return normalized.sort((left, right) =>
		entityKey(left, entity.keys).localeCompare(entityKey(right, entity.keys)),
	);
}
...
export function compareDatasets(expectedRows, actualRows, entity) {
	...
	return {
		...
		matches:
			expected.count === verified.count && expected.checksum_sha256 === verified.checksum_sha256,
	};
}
...
export async function buildFlowsCutoverPlan({
	...
	for (const entity of selected) {
		...
		const actualRows = adapter
			? normalizeRows(await adapter.readTarget(entity, project), entity)
			: [];
	...
	const base = {
		...
		source_databases: Object.fromEntries(
			FLOWS_CUTOVER_SOURCES.map((source) => [source, snapshot.sources[source].database_name]),
		),
...
export function assertPlanIntegrity(plan) {
	if (
		!plan ||
		plan.schema_version !== FLOWS_CUTOVER_SCHEMA_VERSION ||
		plan.tool_version !== FLOWS_CUTOVER_TOOL_VERSION
	) {
	...
	if (
		Object.hasOwn(plan, "organization_id") ||
		Object.hasOwn(plan.project ?? {}, "organization_id")
	) {
	...
	if (plan.plan_id !== expectedPlanId)
		throw new Error("Flows cutover plan integrity check failed; regenerate the protected plan");
	...
	for (const entity of entities) {
		...
		if (
			item.source_module !== entity.sourceModule ||
			item.target_table !== entity.table ||
			item.immutable !== entity.immutable ||
			item.reversible !== Boolean(entity.reverse) ||
	...
	assertNoPhysicalKeyCollisions(
		Object.fromEntries(plan.entities.map((item) => [item.id, item.rows])),
		entities,
	);
...
export async function verifyFlowsCutoverPlan({ plan, adapter }) {
	...
	for (const item of plan.entities) {
		...
		results.push({
			...
			...compareDatasets(item.rows, actualRows, entity),
		});
	}
...
export async function applyFlowsCutoverPlan({ plan, adapter, safety, checkpoint, onCheckpoint }) {
	...
	for (const item of plan.entities) {
		...
		if (
			...
			prior.expected_checksum_sha256 === item.expected.checksum_sha256 &&
			compareDatasets(item.rows, before, entity).matches
		) {
			resumed = true;
...
export function validateApplySafety({
	plan,
	window,
	backupPlan,
	backupReceipt,
	...
	const expected = confirmationValue(
		"CUTOVER",
...
export function validateBackupReceiptForPlan(backupReceipt, plan) {
	for (const source of FLOWS_CUTOVER_SOURCES) {
		...
		if (
			artifact.database_name !== plan.source_databases[source] ||
			artifact.database_id !== plan.source_database_ids[source]
		) {
			throw new Error(
				`Backup receipt ${source} D1 identity does not match the protected import plan`,
			);
		}
	}
	return true;
		...
		databases: databases.map((database) => {
			...
			return {
				...
				upload_command: [
					...
					"object",
					"put",
					`${archive || "UNPROVISIONED"}/${objectKey}`,
					"--remote",
					"--file",
					localPath,
				],
...
export function validateBackupReceipt(receipt, { projectRef, windowId } = {}) {
	if (
		!receipt ||
		receipt.schema_version !== FLOWS_CUTOVER_SCHEMA_VERSION ||
		receipt.target !== FLOWS_CUTOVER_TARGET ||
		receipt.environment !== FLOWS_CUTOVER_ENVIRONMENT
	) {
		throw new Error("A valid MBZA Flows backup receipt is required");
	}
	if (projectRef && receipt.project_ref !== projectRef)
		throw new Error("Backup receipt project_ref mismatch");
	if (windowId && receipt.window_id !== windowId)
		throw new Error("Backup receipt window_id mismatch");
	if (!/^[a-f0-9]{64}$/u.test(String(receipt.backup_plan_id || "")))
		throw new Error("Backup receipt is not bound to a canonical backup plan");
	...
	for (const source of ["api", ...FLOWS_CUTOVER_SOURCES, "flows"]) {
		...
		if (
			!artifact ||
			!artifact.database_name ||
			!artifact.database_id ||
			!String(artifact.time_travel_bookmark || "").trim() ||
			!Number.isSafeInteger(Number(artifact.bytes)) ||
			Number(artifact.bytes) < 1 ||
			!/^[a-f0-9]{64}$/u.test(String(artifact.sha256 || "")) ||
			!artifact.r2_bucket ||
			!artifact.r2_key ||
			!String(artifact.r2_etag || "").trim() ||
			!validIso(artifact.r2_verified_at)
		) {
			throw new Error(`Backup receipt is missing verified Time Travel/R2 evidence for ${source}`);
		}
	}
	if (!validIso(receipt.completed_at) || receipt.automatic_deletion !== false)
		throw new Error("Backup receipt completion/no-deletion evidence is invalid");
	return true;
}
...
export function validateBackupReceiptForBackupPlan(receipt, backupPlan) {
	if (
		!backupPlan ||
		backupPlan.schema_version !== FLOWS_CUTOVER_SCHEMA_VERSION ||
		backupPlan.mode !== "backup-plan"
	) {
	...
	if (
		backupPlan.ready !== true ||
		(backupPlan.blockers ?? []).length !== 0 ||
		backupPlan.automatic_deletion !== false
	) {
	...
	for (const logicalName of ["api", ...FLOWS_CUTOVER_SOURCES, "flows"]) {
		const database = backupPlan.databases?.find(
			(candidate) => candidate.logical_name === logicalName,
		);
		const artifact = receipt.artifacts?.find((candidate) => candidate.logical_name === logicalName);
		if (
			!database ||
			!artifact ||
			artifact.database_name !== database.database_name ||
			artifact.database_id !== database.database_id ||
			artifact.r2_bucket !== database.r2_bucket ||
			artifact.r2_key !== database.r2_key
		) {
			throw new Error(
				`Backup receipt ${logicalName} artifact does not match its canonical backup plan`,
			);
		}
	}
	return true;
...
export function validateFreezeReceipt(receipt, { projectRef, projectId, windowId, planId } = {}) {
	...
	if (projectRef && receipt.project_ref !== projectRef)
		throw new Error("Freeze receipt project_ref mismatch");
	if (projectId && String(receipt.project_id) !== String(projectId))
		throw new Error("Freeze receipt project_id mismatch");
	if (windowId && receipt.window_id !== windowId)
		throw new Error("Freeze receipt window_id mismatch");
	if (planId && receipt.plan_id !== planId) throw new Error("Freeze receipt plan_id mismatch");
	if (!receipt.receipt_id || !validIso(receipt.confirmed_at))
		throw new Error("Freeze receipt evidence is incomplete");
	return true;
}
...
export function buildReverseDelta({ baselinePlan, currentRowsByEntity }) {
	...
	for (const baseline of baselinePlan.entities) {
		...
		const before = new Map(
			normalizeRows(baseline.rows, entity).map((row) => [entityKey(row, entity.keys), row]),
		);
		...
		const changed = current.filter(
			(row) => canonicalJson(before.get(entityKey(row, entity.keys))) !== canonicalJson(row),
		);
		const deleted = [...before.entries()]
		...
		if (changed.length && !entity.reverse && !derived)
			blockers.push(`${entity.id}:non_reversible_change`);
...
export function reverseDeltaSql(delta) {
	if (!delta?.replayable)
		throw new Error("Reverse delta is not replayable; manual reconciliation is required");
	const result = {};
	for (const destination of delta.destinations ?? []) {
		const sql = rawUpsertSql(
			destination.table,
			destination.columns,
			destination.keys,
			destination.rows,
		);
		result[destination.source_module] ||= {
	...
	return Object.fromEntries(
		Object.entries(result).map(([source, value]) => [
			source,
			{
...
export function createRollbackPlan({
	...
	validateBackupReceiptForBackupPlan(backupReceipt, backupPlan);
	validateBackupReceiptForPlan(backupReceipt, plan);
	if (backupPlan.project_ref !== plan.project.project_ref)
		throw new Error("Rollback backup plan project does not match the protected import plan");
	validateBackupReceipt(backupReceipt, {
		projectRef: plan.project.project_ref,
		windowId: freezeReceipt?.window_id,
	});
	validateFreezeReceipt(freezeReceipt, {
		projectRef: plan.project.project_ref,
		projectId: plan.project.project_id,
	...
	if (
		reverseDelta?.baseline_plan_id !== plan.plan_id ||
		reverseDelta?.target !== plan.target ||
		reverseDelta?.environment !== plan.environment ||
		reverseDelta?.project?.project_ref !== plan.project.project_ref ||
		String(reverseDelta?.project?.project_id) !== String(plan.project.project_id)
	) {
	...
	for (const service of requiredWorkers)
		if (!String(versions?.[service] || "").trim())
			blockers.push(`worker_version_missing:${service}`);
	const sql = reverseDelta?.replayable ? reverseDeltaSql(reverseDelta) : {};
	const workerByService = new Map(
		(backupPlan.worker_versions ?? []).map((entry) => [entry.service, entry]),
	);
	return {
		schema_version: FLOWS_CUTOVER_SCHEMA_VERSION,
		mode: "rollback",
		...
		steps: [
			...
			...Object.entries(sql).map(([source, value], index) => ({
				order: index + 3,
				action: `replay-${source}-delta`,
				database_name: value.database_name,
				sql: value.sql,
				command_template: [
					"npx",
					...
					"--remote",
					"--file",
					`<protected-${source}-reverse-delta.sql>`,
				],
			})),
			...requiredWorkers.map((service, index) => ({
				...
				command:
					versions?.[service] && workerByService.get(service)?.worker_name
						? [
								"npx",
								"wrangler",
								"rollback",
								versions[service],
								"--name",
								workerByService.get(service).worker_name,
							]
						: null,
			})),
			{
				order: Object.keys(sql).length + requiredWorkers.length + 3,
				action: "verify-legacy-before-explicit-thaw",
				command: null,
			},
		],
		manual_emergency_only: (backupPlan.databases ?? []).map((database) => ({
			logical_name: database.logical_name,
			warning:
				"Time Travel restore is intentionally never executed by this tool; use only after separate approval and Cloudflare verification.",
			command_template: [
				"npx",
				"wrangler",
				"d1",
...
export function stripProtectedRows(report) {
	...
	return {
		...report,
		entities: report.entities.map(
			({ rows: _rows, changed: _changed, deleted: _deleted, ...entry }) => entry,
		),
	};
}
...
function selectEntities(entityIds) {
	...
	if (!Array.isArray(entityIds) || entityIds.length === 0)
		throw new Error("At least one Flows cutover entity is required");
	const seen = new Set();
...
function assertNoPhysicalKeyCollisions(rowsByEntity, entities) {
	...
	for (const entity of entities) {
		...
		for (const row of normalizeRows(rowsByEntity[entity.id] ?? [], entity)) {
			...
			if (prior)
				throw new Error(
					`Physical key collision in ${entity.table} between ${prior} and ${entity.id}`,
				);
...
function assertProjectEnvironmentIsolation(rows, entity, project) {
	for (const row of rows) {
		if (
			entity.columns.includes("project_id") &&
			String(row.project_id) !== String(project.project_id)
		) {
		...
		if (
			entity.columns.includes("environment_id") &&
			row.environment_id !== project.environment_id
		) {
...
function validateBoundSafety(plan, safety) {
	if (
		!safety ||
		safety.target !== plan.target ||
		safety.environment !== plan.environment ||
		safety.project_ref !== plan.project.project_ref ||
		String(safety.project_id) !== String(plan.project.project_id) ||
		safety.plan_id !== plan.plan_id ||
		!safety.window_id
	) {
...
function validateCheckpoint(checkpoint, plan, windowId) {
	if (
		checkpoint.schema_version !== FLOWS_CUTOVER_SCHEMA_VERSION ||
		checkpoint.target !== plan.target ||
		...
		Object.hasOwn(checkpoint, "organization_id") ||
		checkpoint.environment_id !== plan.environment_id ||
		checkpoint.plan_id !== plan.plan_id ||
		checkpoint.source_snapshot_checksum_sha256 !== plan.source_snapshot_checksum_sha256 ||
		checkpoint.window_id !== windowId ||
		!checkpoint.entities ||
		typeof checkpoint.entities !== "object"
	) {
...
function validateWindow(window, projectRef, now) {
	if (
		!window ||
		window.schema_version !== FLOWS_CUTOVER_SCHEMA_VERSION ||
		window.target !== FLOWS_CUTOVER_TARGET ||
		window.environment !== FLOWS_CUTOVER_ENVIRONMENT ||
		window.project_ref !== projectRef
	) {
	...
	if (
		...
		throw new Error("Current time is outside the approved cutover window");
	if (!window.reason || !window.approved_by || !window.window_id)
		throw new Error("Cutover window approval is incomplete");
}

function reverseDestination(entity, reverseIds) {
	const source = entity.sourceModule;
	const raw = (flowType, value, label) => rawLegacyId(reverseIds, source, flowType, value, label);
	const destinations = {
		paywalls: {
			workflow: destination(
				...
				(row) => ({
					...
					id: raw("workflow", row.id, "paywall"),
				}),
			),
			version: destination(
				"paywall_versions",
				...
				["id"],
				(row) => ({
					id: raw("legacy_version", row.id, "paywall version"),
					paywall_id: raw("workflow", row.workflow_id, "paywall version workflow"),
			...
			placement: destination(
				...
				["id"],
				(row) => ({
					id: raw("legacy_placement", row.id, "paywall placement"),
					project_id: row.project_id,
					key: row.key,
					active_version_id: rawNullable(
						reverseIds,
						source,
						"legacy_version",
						row.active_legacy_version_id,
						"paywall placement version",
					),
					active: row.active,
					paywall_id: raw("workflow", row.workflow_id, "paywall placement workflow"),
					experience_id: rawNullable(
						reverseIds,
						source,
						"legacy_experiment",
						row.experience_id,
			...
			experiment: destination(
				...
				["id"],
				(row) => ({
					id: raw("legacy_experiment", row.id, "paywall experiment"),
					project_id: row.project_id,
					paywall_id: raw("workflow", row.workflow_id, "paywall experiment workflow"),
					name: row.name,
					status: row.status,
			...
			variant: destination(
				...
				["id"],
				(row) => ({
					id: raw("legacy_variant", row.id, "paywall variant"),
					project_id: row.project_id,
					experience_id: raw("legacy_experiment", row.experiment_id, "paywall variant experiment"),
					version_id: raw("legacy_version", row.legacy_version_id, "paywall variant version"),
					key: row.key,
					weight: row.weight,
					active: row.active,
					created_at: row.created_at,
		...
		onboardings: {
			workflow: destination(
				...
				["id"],
				(row) => ({
					id: raw("workflow", row.id, "onboarding"),
					project_id: row.project_id,
					name: row.name,
			...
			version: destination(
				...
				["id"],
				(row) => ({
					id: raw("legacy_version", row.id, "onboarding version"),
					onboarding_id: raw("workflow", row.workflow_id, "onboarding version workflow"),
					project_id: row.project_id,
					version: row.version,
					status: row.status,
					definition_json: row.definition_json,
			...
			placement: destination(
				...
				["id"],
				(row) => ({
					id: raw("legacy_placement", row.id, "onboarding placement"),
					project_id: row.project_id,
					key: row.key,
					active_version_id: rawNullable(
						reverseIds,
						...
						"legacy_version",
						row.active_legacy_version_id,
						"onboarding placement version",
					),
					active: row.active,
					name: row.key,
					onboarding_id: raw("workflow", row.workflow_id, "onboarding placement workflow"),
					priority: row.priority,
					created_at: row.created_at,
					updated_at: row.updated_at,
				}),
			...
			experiment: destination(
				...
				["id"],
				(row) => ({
					id: raw("legacy_experiment", row.id, "onboarding experiment"),
					project_id: row.project_id,
					placement_id: raw(
						"legacy_placement",
						row.placement_id,
						"onboarding experiment placement",
					),
					name: row.name,
					status: row.status === "archived" ? "completed" : row.status,
					traffic_percentage: Math.max(
						1,
						Number(row.traffic_basis_points ?? row.traffic_percent * 100),
					),
					created_at: row.created_at,
					updated_at: row.updated_at,
				}),
			...
			variant: destination(
				"experience_variants",
				["id", "project_id", "experience_id", "name", "weight", "version_id", "created_at"],
				["id"],
				(row) => ({
					id: raw("legacy_variant", row.id, "onboarding variant"),
					project_id: row.project_id,
					experience_id: raw(
						"legacy_experiment",
						row.experiment_id,
						"onboarding variant experiment",
					),
					name: row.key,
					weight: row.weight,
					version_id: raw("legacy_version", row.legacy_version_id, "onboarding variant version"),
					created_at: row.created_at,
				}),
			),
			targeting_rule: destination(
				"targeting_rules",
				[
					"id",
					"project_id",
				...
				["id"],
				(row) => ({
					id: raw("legacy_targeting_rule", row.id, "onboarding targeting rule"),
					project_id: row.project_id,
					placement_id: raw("legacy_placement", row.placement_id, "onboarding targeting placement"),
					name: `Rule ${raw("legacy_targeting_rule", row.id, "onboarding targeting rule")}`,
					priority: row.priority,
	...
	for (const item of plan.entities) {
		...
		for (const row of item.rows) {
			...
			if (previous !== undefined && previous !== sourceId) {
				throw new Error(`${item.id}: flow id ${String(row.flow_id)} has conflicting legacy ids`);
			}
...
function destination(table, columns, keys, convert) {
	return { table, columns, keys, convert };
}

function reverseEvent(row, source) {
	const properties =
		typeof row.properties_json === "string" ? JSON.parse(row.properties_json) : row.properties_json;
	const legacy =
		properties.legacy && typeof properties.legacy === "object" ? properties.legacy : properties;
	const common = {
	...
	if (source === "paywalls")
		return {
			...common,
			paywall_id: properties.legacy_workflow_id ?? legacy.workflow_id ?? null,
			session_id: null,
			revenue_micros: Number(properties.legacy_revenue_micros ?? legacy.revenue_micros ?? 0),
			currency: properties.legacy_currency ?? legacy.currency ?? null,
		};
	return {
		...common,
		onboarding_id: properties.legacy_workflow_id ?? legacy.workflow_id ?? null,
		step_id: properties.legacy_step_id ?? legacy.step_id ?? null,
		platform: common.platform ?? "web",
	};
}
...
function rawUpsertSql(table, columns, keys, rows, immutable = false) {
	...
	for (const group of groups) {
		...
		if (Buffer.byteLength(prefix) + extra + Buffer.byteLength(suffix) > 96 * 1024)
			throw new Error(`${table}: one row exceeds the safe D1 statement size`);
...
function dedupeRows(rows, keys) {
	...
	return [...result.values()].sort((left, right) =>
		entityKey(left, keys).localeCompare(entityKey(right, keys)),
	);
}
