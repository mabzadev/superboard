...
import { servicePackagePath } from "../cloudflare/services.mjs";
import {
	DOMAIN_SERVICES,
...
export function d1Descriptor(
	...
	if (!D1_SCHEMA_OWNERS.includes(service)) {
		throw new Error(`D1 schema owner must be one of: ${D1_SCHEMA_OWNERS.join(", ")}`);
	}
	...
	return descriptor(
		...
		"DB",
		`${servicePackagePath(definition.resourceKey === "dynamicLinks" ? "dynamic-links" : definition.resourceKey)}/migrations`,
	);
}

export function targetD1Descriptors(target, targetName, environment, requested = "all") {
	const services =
		requested === "all"
			? D1_SCHEMA_OWNERS
			: String(requested)
					...
					.filter(Boolean);
	if (services.length === 0) throw new Error("At least one D1 schema owner is required");
	const unknown = services.filter((service) => !D1_SCHEMA_OWNERS.includes(service));
	if (unknown.length) throw new Error(`Unknown D1 schema owner: ${unknown.join(", ")}`);
...
export async function localMigrationFiles(value) {
	...
	const files = entries
		.filter((entry) => entry.isFile() && /^\d+[a-z0-9_-]*\.sql$/iu.test(entry.name))
		.map((entry) => entry.name)
		.sort((left, right) => left.localeCompare(right, "en"));
	if (files.length === 0) throw new Error(`${value.service} has no D1 migration files`);
	return files;
}

function descriptor(common, resource, binding, migrationsDirectory) {
	if (!resource?.name) throw new Error(`${common.service} does not define its D1 resource`);
