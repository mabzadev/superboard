...
import { d1Descriptor, localMigrationFiles } from "./d1-registry.mjs";

export const IDENTITY_CUTOVER_KIND = "superboard-identity-project-cutover";
...
export async function verifyIdentityProjectCutover({
	...
	if (!descriptor?.databaseId) {
		throw new Error("Identity cutover requires a provisioned target D1 database");
	}
	...
	const path = await writeIdentityCutoverReceipt(
		receiptDirectory ?? resolve(tmpdir(), `superboard-identity-cutover-${process.pid}`),
		receipt,
	);
...
export async function enforceIdentityProjectCutover({ suppliedReceipt, expected, verification }) {
	if (suppliedReceipt) {
...
export function resolveDeploymentRevision(env = process.env, execute = executeCommand) {
...
async function protectedDirectory(directory, { create = false } = {}) {
	...
	if (!metadata.isDirectory() || metadata.isSymbolicLink()) {
		throw new Error("Identity cutover receipt directory must be a real directory");
	}
	...
	if (!relation.startsWith("..")) {
		throw new Error("Identity cutover receipts must be written outside the Git repository");
	}
...
function exactRevision(value) {
	...
	if (!/^[a-f0-9]{40}$/u.test(revision)) {
		throw new Error("Identity cutover requires the exact 40-character deployment revision");
	}
	return revision;
}
...
function executeCommand(command, args, { capture = false, env = process.env } = {}) {
...
async function main() {
	...
	console.log(
		JSON.stringify({ ...result.receipt, receiptPath: result.path, sha256: result.sha256 }, null, 2),
	);
}
