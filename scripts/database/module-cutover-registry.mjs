...
export const MODULE_CUTOVER_REGISTRY = Object.freeze([
	entity({
		...
		target: {
			table: "customers",
			query:
				"SELECT id,project_id,external_id,attributes_json,email,name,platform,country_code,first_seen_at,last_seen_at,created_at,updated_at FROM customers WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "app.referrals",
		...
		target: {
			table: "referrals",
			query:
				"SELECT id,project_id,customer_id,invited_customer_id,code,source,status,converted_at,created_at FROM referrals WHERE project_id=CAST(:project_id AS TEXT) AND source='legacy_dynamic_link' ORDER BY id",
		},
	}),
	entity({
		...
		target: {
			...
			query:
				"SELECT id,project_id,customer_id,referrer_customer_id,event_type,platform,occurred_at,revenue_cents,engagement_time,metadata_json FROM customer_events WHERE project_id=CAST(:project_id AS TEXT) AND json_extract(metadata_json,'$.cutover_source')='legacy_event' ORDER BY id",
		},
	}),
	entity({
		...
		target: {
			table: "customer_events",
			query:
				"SELECT id,project_id,customer_id,referrer_customer_id,event_type,platform,occurred_at,revenue_cents,engagement_time,metadata_json FROM customer_events WHERE project_id=CAST(:project_id AS TEXT) AND json_extract(metadata_json,'$.cutover_source')='legacy_purchase' ORDER BY id",
		},
	}),
	entity({
		...
		target: {
			table: "sdk_configurations",
			query:
				"SELECT id,project_id,platform,status,configuration_json,verified_at,created_at,updated_at FROM sdk_configurations WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "app.access_keys",
		...
		transform: (rows, context) =>
			rows.map((row) => {
				const legacyKey =
					context.environment === "test" ? `test_${row.api_key}` : String(row.api_key);
				return {
		...
		target: {
			table: "access_keys",
			query:
				"SELECT id,project_id,key_hash,prefix,created_by,last_used_at,revoked_at,created_at FROM access_keys WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		...
		target: {
			table: "daily_metrics",
			query:
				"SELECT project_id,metric_date,platform,active_customers,new_customers,referrals,installs,opens FROM daily_metrics WHERE project_id=CAST(:project_id AS TEXT) ORDER BY metric_date,platform",
		},
	}),
	...
	entity({
		...
		source: {
			...
			query:
				"SELECT id,CAST(project_id AS TEXT) project_id,primary_app_user_id external_customer_id,attributes attributes_json,created_at,updated_at FROM billing_customers WHERE project_id=:project_id ORDER BY id",
		},
		target: {
			table: "financial_customers",
			query:
				"SELECT id,project_id,external_customer_id,attributes_json,created_at,updated_at FROM financial_customers WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		...
		source: {
			...
			query: `SELECT id,CAST(project_id AS TEXT) project_id,store_product_id identifier,COALESCE(NULLIF(display_name,''),store_product_id) display_name,description,
      ...
      WHERE project_id=:project_id AND p.id=(SELECT MIN(candidate.id) FROM billing_products candidate WHERE candidate.project_id=p.project_id AND candidate.store_product_id=p.store_product_id) ORDER BY id`,
		...
		target: {
			table: "products",
			query:
				"SELECT id,project_id,identifier,display_name,description,product_type,status,created_at,updated_at FROM products WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		...
		source: {
			database: "billing",
			table: "billing_products",
			query: `SELECT 'store-'||p.id id,CAST(p.project_id AS TEXT) project_id,
      ...
      WHERE p.project_id=:project_id ORDER BY p.id`,
		...
		target: {
			...
			query:
				"SELECT id,project_id,product_id,store,environment,store_product_id,title,description,price_micros,currency,billing_period,trial_period,active,metadata_json,synced_at,created_at,updated_at FROM store_products WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		...
		source: {
			...
			query:
				"SELECT id,CAST(project_id AS TEXT) project_id,identifier key,COALESCE(NULLIF(display_name,''),identifier) name,description,active,created_at,updated_at FROM billing_entitlements WHERE project_id=:project_id ORDER BY id",
		},
		target: {
			table: "entitlements",
			query:
				"SELECT id,project_id,key,name,description,active,created_at,updated_at FROM entitlements WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "products.entitlement_products",
		module: "products",
		columns: j("entitlement_id", "product_id"),
		keys: ["entitlement_id", "product_id"],
		source: {
			database: "billing",
			table: "billing_product_entitlements",
			query: `SELECT DISTINCT pe.entitlement_id,
      (SELECT MIN(candidate.id) FROM billing_products candidate WHERE candidate.project_id=p.project_id AND candidate.store_product_id=p.store_product_id) product_id
      FROM billing_product_entitlements pe JOIN billing_products p ON p.id=pe.product_id WHERE p.project_id=:project_id ORDER BY pe.entitlement_id,product_id`,
		...
		target: {
			...
			query:
				"SELECT ep.entitlement_id,ep.product_id FROM entitlement_products ep JOIN entitlements e ON e.id=ep.entitlement_id WHERE e.project_id=CAST(:project_id AS TEXT) ORDER BY ep.entitlement_id,ep.product_id",
		},
	}),
	entity({
		...
		jsonColumns: ["packages_json"],
		projectColumn: "project_id",
		source: {
			database: "billing",
			table: "billing_offerings",
			query: `SELECT o.id,CAST(o.project_id AS TEXT) project_id,o.placement,COALESCE(NULLIF(o.display_name,''),o.identifier) name,
      ...
      FROM billing_offerings o WHERE o.project_id=:project_id ORDER BY o.id`,
		...
		target: {
			table: "offerings",
			query:
				"SELECT id,project_id,placement,name,packages_json,priority,active,updated_at,identifier,description,created_at FROM offerings WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		...
		source: {
			...
			query: `SELECT p.id,CAST(o.project_id AS TEXT) project_id,p.identifier,p.identifier display_name,
      ...
      p.position,o.active,p.created_at,p.updated_at FROM billing_packages p JOIN billing_offerings o ON o.id=p.offering_id WHERE o.project_id=:project_id ORDER BY p.id`,
		...
		target: {
			table: "packages",
			query:
				"SELECT id,project_id,identifier,display_name,description,product_id,position,active,created_at,updated_at FROM packages WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "products.offering_packages",
		module: "products",
		columns: j("offering_id", "package_id", "position"),
		keys: ["offering_id", "package_id"],
		source: {
			database: "billing",
			table: "billing_packages",
			query:
				"SELECT p.offering_id,p.id package_id,p.position FROM billing_packages p JOIN billing_offerings o ON o.id=p.offering_id WHERE o.project_id=:project_id ORDER BY p.offering_id,p.position,p.id",
		},
		target: {
			table: "offering_packages",
			query:
				"SELECT op.offering_id,op.package_id,op.position FROM offering_packages op JOIN offerings o ON o.id=op.offering_id WHERE o.project_id=CAST(:project_id AS TEXT) ORDER BY op.offering_id,op.position,op.package_id",
		},
	}),
	entity({
		...
		source: {
			...
			query: `SELECT t.id,CAST(t.project_id AS TEXT) project_id,COALESCE(t.customer_id,'unknown') financial_customer_id,
      ...
      FROM billing_transactions t WHERE t.project_id=:project_id AND t.id=(SELECT candidate.id FROM billing_transactions candidate
		...
		target: {
			table: "purchases",
			query:
				"SELECT id,project_id,financial_customer_id,product_id,status,purchased_at,payload_json,store,environment,external_transaction_id,original_transaction_id,purchased_price_micros,currency,expires_at,updated_at FROM purchases WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		...
		source: {
			...
			query: `SELECT s.id,CAST(s.project_id AS TEXT) project_id,COALESCE(s.customer_id,'unknown') financial_customer_id,
      ...
      FROM billing_subscriptions s WHERE s.project_id=:project_id ORDER BY s.id`,
		...
		target: {
			table: "subscriptions",
			query:
				"SELECT id,project_id,financial_customer_id,product_id,latest_purchase_id,store,environment,original_transaction_id,status,current_period_started_at,current_period_ends_at,auto_renew,cancelled_at,created_at,updated_at FROM subscriptions WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "products.refunds",
		...
		source: {
			...
			query: `SELECT id,CAST(project_id AS TEXT) project_id,transaction_id purchase_id,provider_case_id external_refund_id,
      ...
      opened_at requested_at,resolved_at completed_at,updated_at FROM billing_refund_cases WHERE project_id=:project_id AND transaction_id IS NOT NULL ORDER BY id`,
		...
		target: {
			table: "refunds",
			query:
				"SELECT id,project_id,purchase_id,external_refund_id,status,amount_micros,currency,reason,metadata_json,requested_at,completed_at,updated_at FROM refunds WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),

	entity({
		id: "paywalls.paywalls",
		module: "paywalls",
		columns: j(
			"id",
			"project_id",
		...
		source: {
			database: "billing",
			table: "billing_paywalls",
			query:
				"SELECT id,CAST(project_id AS TEXT) project_id,display_name name,created_at,identifier,NULL description,NULL archived_at,updated_at FROM billing_paywalls WHERE project_id=:project_id ORDER BY id",
		},
		target: {
			table: "paywalls",
			query:
				"SELECT id,project_id,name,created_at,identifier,description,archived_at,updated_at FROM paywalls WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "paywalls.versions",
		module: "paywalls",
		columns: j(
			"id",
			"paywall_id",
		...
		source: {
			database: "billing",
			table: "billing_paywall_versions",
			query: `SELECT v.id,v.paywall_id,v.version,
      ...
      FROM billing_paywall_versions v JOIN billing_paywalls p ON p.id=v.paywall_id WHERE p.project_id=:project_id ORDER BY v.paywall_id,v.version`,
		...
		target: {
			table: "paywall_versions",
			query:
				"SELECT id,paywall_id,version,status,definition_json,created_at,project_id,schema_version,changelog,created_by,published_at FROM paywall_versions WHERE project_id=CAST(:project_id AS TEXT) ORDER BY paywall_id,version",
		},
	}),
	entity({
		...
		jsonColumns: ["targeting_json"],
		projectColumn: "project_id",
		source: {
			database: "billing",
			table: "billing_placements",
			query: `SELECT p.id,CAST(p.project_id AS TEXT) project_id,p.identifier key,w.active_version_id,p.active,w.id paywall_id,
      ...
      WHERE r.project_id=:project_id ORDER BY 1`,
		...
		target: {
			table: "placements",
			query:
				"SELECT id,project_id,key,active_version_id,active,paywall_id,experience_id,targeting_json,priority,created_at,updated_at FROM placements WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "paywalls.experiences",
		module: "paywalls",
		columns: j(
			"id",
			"project_id",
		...
		source: {
			database: "billing",
			table: "billing_experiments",
			query: `SELECT e.id,CAST(e.project_id AS TEXT) project_id,w.id paywall_id,e.display_name name,
      ...
      WHERE e.project_id=:project_id ORDER BY e.id`,
		...
		target: {
			table: "experiences",
			query:
				"SELECT id,project_id,paywall_id,name,status,traffic_percent,starts_at,ends_at,created_at,updated_at FROM experiences WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "paywalls.variants",
		module: "paywalls",
		columns: j(
			"id",
			"project_id",
		...
		source: {
			database: "billing",
			table: "billing_experiment_variants",
			query: `SELECT v.id,CAST(e.project_id AS TEXT) project_id,v.experiment_id,w.active_version_id version_id,v.identifier key,
      ...
      WHERE e.project_id=:project_id ORDER BY v.id`,
		...
		target: {
			table: "variants",
			query:
				"SELECT id,project_id,experience_id,version_id,key,weight,active,created_at,updated_at FROM variants WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "paywalls.events",
	...
	entity({
		...
		source: {
			...
			table: "campaigns",
			query:
				"SELECT CAST(id AS TEXT) id,CAST(project_id AS TEXT) project_id,COALESCE(NULLIF(name,''),'Campaign '||id) name,created_at,'legacy-'||id slug,CASE WHEN archived=1 THEN 'archived' ELSE 'active' END status,'{}' metadata_json,updated_at FROM campaigns WHERE project_id=:project_id ORDER BY id",
		},
		target: {
			table: "campaigns",
			query:
				"SELECT id,project_id,name,created_at,slug,status,metadata_json,updated_at FROM campaigns WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "dynamic-links.links",
		module: "dynamic-links",
		columns: j(
			"id",
			"project_id",
		...
		jsonColumns: ["destinations_json", "utm_json"],
		projectColumn: "project_id",
		source: {
			database: "api",
			table: "links",
			query: `SELECT CAST(l.id AS TEXT) id,CAST(rc.project_id AS TEXT) project_id,l.path slug,COALESCE(rc.default_fallback,'') destination_url,l.active,l.created_at,COALESCE(NULLIF(l.title,''),l.path) name,
      ...
      FROM links l JOIN redirect_configs rc ON rc.id=l.redirect_config_id WHERE rc.project_id=:project_id ORDER BY l.id`,
		...
		target: {
			table: "links",
			query:
				"SELECT id,project_id,slug,destination_url,active,created_at,name,destinations_json,campaign_id,title,subtitle,image_url,utm_json,updated_at FROM links WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "dynamic-links.redirect_rules",
		module: "dynamic-links",
		columns: j(
			"id",
			"project_id",
		...
		jsonColumns: ["rule_json"],
		projectColumn: "project_id",
		source: {
			database: "api",
			table: "redirects",
			query: `SELECT CAST(r.id AS TEXT) id,CAST(c.project_id AS TEXT) project_id,r.id priority,
      ...
      COALESCE(r.platform,'Default')||' redirect' name,r.enabled active,r.created_at,r.updated_at FROM redirects r JOIN redirect_configs c ON c.id=r.redirect_config_id WHERE c.project_id=:project_id ORDER BY r.id`,
		...
		target: {
			...
			query:
				"SELECT id,project_id,priority,rule_json,name,active,created_at,updated_at FROM redirect_rules WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "dynamic-links.domains",
		module: "dynamic-links",
		columns: j(
			"id",
			"project_id",
		...
		source: {
			database: "api",
			table: "domains",
			query: `SELECT CAST(id AS TEXT) id,CAST(project_id AS TEXT) project_id,CASE WHEN subdomain IS NULL OR subdomain='' THEN domain ELSE subdomain||'.'||domain END hostname,
      'verified' status,'legacy-import' verification_token,created_at verified_at,CASE WHEN id=(SELECT MIN(id) FROM domains d WHERE d.project_id=:project_id) THEN 1 ELSE 0 END is_default,created_at,updated_at FROM domains WHERE project_id=:project_id ORDER BY id`,
		...
		target: {
			table: "domains",
			query:
				"SELECT id,project_id,hostname,status,verification_token,verified_at,is_default,created_at,updated_at FROM domains WHERE project_id=CAST(:project_id AS TEXT) ORDER BY id",
		},
	}),
	entity({
		id: "dynamic-links.social_preview",
		module: "dynamic-links",
		columns: j("project_id", "title", "description", "image_url", "site_name", "updated_at"),
		keys: ["project_id"],
		projectColumn: "project_id",
		source: {
			database: "api",
			table: "domains",
			query: `SELECT CAST(project_id AS TEXT) project_id,COALESCE(generic_title,'') title,COALESCE(generic_subtitle,'') description,generic_image_url image_url,domain site_name,updated_at FROM domains WHERE project_id=:project_id ORDER BY id LIMIT 1`,
		},
		target: {
			table: "social_previews",
			query:
				"SELECT project_id,title,description,image_url,site_name,updated_at FROM social_previews WHERE project_id=CAST(:project_id AS TEXT)",
		},
	}),
	entity({
		id: "dynamic-links.tracking_settings",
		module: "dynamic-links",
		columns: j("project_id", "enabled", "provider", "configuration_json", "updated_at"),
		keys: ["project_id"],
		jsonColumns: ["configuration_json"],
		projectColumn: "project_id",
		source: {
			database: "api",
			table: "redirect_configs",
			query: `SELECT CAST(project_id AS TEXT) project_id,1 enabled,'superboard' provider,
      ...
      FROM redirect_configs WHERE project_id=:project_id ORDER BY id LIMIT 1`,
		...
		target: {
			...
			query:
				"SELECT project_id,enabled,provider,configuration_json,updated_at FROM tracking_settings WHERE project_id=CAST(:project_id AS TEXT)",
		},
	}),
	entity({
		id: "dynamic-links.events",
		module: "dynamic-links",
		columns: j(
			"id",
			"project_id",
		...
		source: {
			database: "api",
			table: "actions",
			query: `SELECT 'legacy-action-'||a.id id,CAST(rc.project_id AS TEXT) project_id,CAST(l.id AS TEXT) link_id,
      ...
      WHERE rc.project_id=:project_id ORDER BY a.id`,
		...
		target: {
			table: "link_events",
			query:
				"SELECT id,project_id,link_id,campaign_id,event_type,platform,country_code,occurred_at,metadata_json,revenue_cents,engagement_time,customer_id,session_id FROM link_events WHERE project_id=CAST(:project_id AS TEXT) AND json_extract(metadata_json,'$.cutover_source')='legacy_action' ORDER BY id",
		},
	}),
	entity({
		id: "dynamic-links.analytics_events",
		module: "dynamic-links",
		columns: j(
			"id",
			"project_id",
      ...
      WHERE e.project_id=:project_id AND e.event IN ('view','open','install','reinstall','reactivation','app_open','user_referred','time_spent') ORDER BY e.id`,
		...
		target: {
			table: "link_events",
			query:
				"SELECT id,project_id,link_id,campaign_id,event_type,platform,country_code,occurred_at,metadata_json,revenue_cents,engagement_time,customer_id,session_id FROM link_events WHERE project_id=CAST(:project_id AS TEXT) AND json_extract(metadata_json,'$.cutover_source')='legacy_event' ORDER BY id",
		},
	}),
	entity({
		id: "dynamic-links.purchase_events",
		module: "dynamic-links",
		columns: j(
			"id",
			"project_id",
		...
		source: {
			database: "api",
			table: "purchase_events",
			query: `SELECT 'legacy-purchase-'||pe.id id,CAST(pe.project_id AS TEXT) project_id,CAST(pe.link_id AS TEXT) link_id,
      ...
      ORDER BY pe.id`,
		...
		target: {
			table: "link_events",
			query:
				"SELECT id,project_id,link_id,campaign_id,event_type,platform,country_code,occurred_at,metadata_json,revenue_cents,engagement_time,customer_id,session_id FROM link_events WHERE project_id=CAST(:project_id AS TEXT) AND json_extract(metadata_json,'$.cutover_source')='legacy_purchase' ORDER BY id",
		},
	}),
	...
	copySupport(
		"settings",
		"messaging_project_settings",
		"support_project_settings",
		j(
			"project_id",
			"business_name",
			"locale",
	...
	copySupport(
		"configuration",
		"messaging_configuration_entities",
	...
	copySupport(
		"configuration_audit",
		"messaging_configuration_audit_events",
		"support_configuration_audit_events",
		j(
			"id",
			"project_id",
			"entity_id",
	...
	copySupport(
		"contact_notes",
		"messaging_contact_notes",
		"support_contact_notes",
		j("id", "project_id", "contact_id", "content", "created_by", "created_at", "updated_at"),
	),
	copySupport(
		"participants",
		...
		{ keys: ["conversation_id", "participant_kind", "participant_id"] },
	),
	copySupport(
		"drafts",
		"messaging_conversation_drafts",
		"support_conversation_drafts",
		j("conversation_id", "project_id", "agent_id", "content", "attachments_json", "updated_at"),
		{ keys: ["conversation_id", "agent_id"] },
	),
	copySupport(
		"csat",
		"messaging_csat_responses",
	...
	copySupport(
		"notifications",
		"messaging_agent_notifications",
		"support_agent_notifications",
		j(
			"id",
			"project_id",
			"agent_id",
	...
	copySupport(
		"operations_audit",
		"messaging_operations_audit_events",
		"support_operations_audit_events",
		j(
			"id",
			"project_id",
			"resource_type",
	...
	copySupport(
		"rule_executions",
		"messaging_rule_executions",
		"support_rule_executions",
		j(
			"id",
			"project_id",
			"rule_id",
...
export const MODULE_CUTOVER_GUARDS = Object.freeze([
	{
		...
		module: "products",
		message:
			"A legacy package references multiple Store products; the current Products package schema cannot preserve that relation without loss.",
		source: {
			database: "billing",
	...
	},
	{
		...
		source: {
			database: "billing",
			query:
				"SELECT store_product_id FROM billing_products WHERE project_id=:project_id GROUP BY store_product_id HAVING COUNT(DISTINCT product_type)>1 ORDER BY store_product_id",
		},
	},
	{
		...
		module: "products",
		message:
			"A Store product has multiple active prices; the current Products store_products projection can retain only one.",
		source: {
			database: "billing",
	...
	},
	{
		...
		source: {
			database: "billing",
			query:
				"SELECT id FROM billing_subscriptions WHERE project_id=:project_id AND (customer_id IS NULL OR product_id IS NULL) ORDER BY id",
		},
	},
	{
		...
		source: {
			database: "billing",
			query:
				"SELECT id FROM billing_refund_cases WHERE project_id=:project_id AND transaction_id IS NULL ORDER BY id",
		},
	},
	{
		...
		module: "products",
		message:
			"A refund references a transaction event that is not the canonical purchase projection.",
		source: {
			database: "billing",
