...
import { assertProtectedBackupDirectory } from "./d1-backup.mjs";

const MAX_RECEIPT_BYTES = 1024 * 1024;
export const MIGRATION_BATCH_KIND = "superboard-d1-migration-batch";
...
export function buildMigrationBatchReceipt({
	...
	if (
		...
		throw new Error("D1 migration result does not match the requested full batch");
	}
	...
	const normalizedDatabases = databases.map((database) => {
		if (
			...
			throw new Error(`Migration verification is incomplete for ${database.service}`);
		}
		return {
...
export function validateMigrationBatchReceipt(value, expected = {}) {
	...
	if (expected.service && !services.includes(expected.service)) {
		throw new Error(`Migration batch receipt does not cover ${expected.service}`);
	}
	return value;
}
