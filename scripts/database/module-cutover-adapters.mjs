...
import { canonicalJson, parseProjectRef } from "./module-cutover-core.mjs";
...
export class FixtureAdapter {
	...
	async resolveProject(parsed) {
		...
		if (!context || context.project_ref !== parsed.project_ref)
			throw new Error("Fixture project does not match request");
...
export class RemoteD1Adapter {
	constructor({
		...
		if (!this.resources)
			throw new Error(`${targetName} does not define environment ${environment}`);
	}

	async resolveProject(parsed) {
		const rows = await this.query(
			this.resources.d1,
			`SELECT id project_id,instance_id,identifier,is_test FROM projects WHERE instance_id=${parsed.instance_id} AND is_test=${parsed.environment === "test" ? 1 : 0} ORDER BY id`,
		);
		if (rows.length !== 1)
			throw new Error(
				`Expected exactly one project for ${parsed.project_ref}, found ${rows.length}`,
			);
		const row = rows[0];
		return {
	...
	async readSource(entity, context) {
		if (
			...
			return this.query(this.moduleDatabase("support"), renderQuery(entity.target.query, context));
		}
		return this.query(
			this.sourceDatabase(entity.source.database),
			renderQuery(entity.source.query, context),
		);
	}
	...
	async readTarget(entity, context) {
		...
		return this.query(
			this.moduleDatabase(entity.module),
	...
	async readGuard(guard, context) {
		return this.query(
			this.sourceDatabase(guard.source.database),
			renderQuery(guard.source.query, context),
		);
	}

	async upsert(entity, _rows, _context, sql) {
		if (!this.allowWrites)
			throw new Error("Remote writes are disabled; use the guarded apply command");
		if (entity.repositoryOnly) {
			throw new Error(`${entity.id}: read-only projection cannot be written`);
		}
		const directory = await mkdtemp(join(tmpdir(), "superboard-cutover-"));
		...
		try {
			...
			this.capture([
				...
				"d1",
				"execute",
				databaseId(this.moduleDatabase(entity.module)),
				"--remote",
				"--file",
				path,
			]);
	...
	async upsertRepository(entity, rows, context) {
		...
		if (!this.repositoryEncryptionKey) {
			throw new Error(
				"SUPERBOARD_PLUGIN_STORE_ENCRYPTION_KEY is required for repository authority",
			);
		}
       ...
       WHERE active.plugin_id=${sqlString(entity.pluginId)}
		...
		try {
			...
			this.capture([
				...
				"d1",
				"execute",
				databaseId(this.resources.siteD1),
				"--remote",
				"--file",
				path,
			]);
	...
	async maintenanceRequest(method, projectRef, body) {
		...
		if (!this.gatewayToken)
			throw new Error("SUPERBOARD_CUTOVER_TOKEN is required for maintenance operations");
		const controller = new AbortController();
		const timeout = setTimeout(() => controller.abort(), 15_000);
		try {
			const response = await fetch(
				`https://${this.target.domains.api}/api/v1/admin/module-cutover/maintenance/${projectRef}`,
				{
					method,
					headers: {
			...
			if (!response.ok)
				throw new Error(
					`Maintenance API ${response.status}: ${payload?.error?.code || "request_failed"}`,
				);
...
function renderQuery(query, context) {
	...
	if (!Number.isSafeInteger(projectId) || !Number.isSafeInteger(instanceId))
		throw new Error("Resolved project ids must be safe integers");
	return query
