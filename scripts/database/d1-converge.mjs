...
import { compiledTargetFromArgs } from "../cloudflare/target-compiler.mjs";
import {
	cloudflareEnv,
	environmentFromArgs,
	loadTarget,
...
import { createD1Backup } from "./d1-backup.mjs";
import { localMigrationFiles, targetD1Descriptors } from "./d1-registry.mjs";

export function migrationConfirmation(targetName, environment, serviceSelector) {
	return `MIGRATE:${targetName}:${environment}:${serviceSelector}`;
}

const ANSI_ESCAPE_PATTERN =
	// oxlint-disable-next-line no-control-regex -- Wrangler output contains ANSI control sequences.
	/\u001B(?:\[[0-?]*[ -/]*[@-~]|\][^\u0007]*(?:\u0007|\u001B\\))/gu;
...
export function assertMigrationApplySafety({
	...
	const expected = migrationConfirmation(targetName, environment, serviceSelector);
	if (confirm !== expected) throw new Error(`Refusing D1 mutation: pass --confirm ${expected}`);
...
export async function buildD1ConvergencePlan({
	...
	const descriptors = targetD1Descriptors(target, targetName, environment, serviceSelector);
	const databases = [];
...
export async function attachRemoteMigrationStatus({
	...
	for (const descriptor of descriptors) {
		...
		const database = plan.databases.find(({ service }) => service === descriptor.service);
	...
	plan.converged = plan.databases.every(({ remote_converged }) => remote_converged === true);
...
export async function applyD1Convergence({
	...
	for (const descriptor of descriptors) {
		...
		const remoteStatus = String(verification.stdout || verification.stderr || "").trim();
		const parsed = parseWranglerD1MigrationList(remoteStatus);
		if (!parsed.converged) {
			throw new Error(
...
async function main(argv = process.argv.slice(2)) {
	...
	if (argv[0] && !argv[0].startsWith("--") && !["plan", "apply"].includes(argv[0])) {
...
function assertCompiledMigrations(compiledTarget, databases) {
	const compiled = new Map(
		compiledTarget.graph.migrations.map((migration) => [migration.service, migration]),
	);
	for (const database of databases) {
		...
		if (
			...
			migration.files.some(({ file }, index) => file !== database.local_migrations[index])
		) {
