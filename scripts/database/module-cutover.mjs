...
import {
	MODULE_CUTOVER_GUARDS,
	MODULE_CUTOVER_REGISTRY,
	registrySummary,
} from "./module-cutover-registry.mjs";

export async function main(argv = process.argv.slice(2)) {
	const { command, args } = parseCli(argv);
	if (
		args.apply === true &&
		!new Set(["apply", "maintenance-enable", "maintenance-disable"]).has(command)
	) {
	...
	if (command === "backup-plan") {
		const output =
			...
			`.cutover/backups/${projectRef}/${new Date().toISOString().replaceAll(":", "-")}`;
		return emit(
			createBackupPlan({
				target: targetName,
				environment,
	...
	if (command === "backup-receipt") {
		...
		for (const item of backupPlan.database_exports) {
			...
			if (outputIndex < 0 || !item.command[outputIndex + 1])
				throw new Error(`Backup ${item.name} has no output path`);
			const path = resolve(item.command[outputIndex + 1]);
			const metadata = await stat(path);
			if (!metadata.isFile() || metadata.size < 1)
				throw new Error(`Backup ${item.name} is missing or empty`);
			artifacts.push({
				name: item.name,
				path,
				bytes: metadata.size,
	...
	if (command === "rollback-plan") {
		...
		const backupPlan = await readJson(resolve(args["backup-plan"]));
		if (backupPlan.project_ref !== projectRef)
			throw new Error("Rollback backup plan project_ref mismatch");
		const backupReceipt = args["backup-receipt"]
			? await readJson(resolve(args["backup-receipt"]))
			: null;
		...
		const reverseDelta = args["reverse-delta"]
			? await readJson(resolve(args["reverse-delta"]))
			: null;
	...
	if (command === "snapshot") {
		if (!args.report)
			throw new Error("snapshot requires --report because it contains protected project data");
		await writeJsonAtomic(resolve(args.report), plan);
		console.log(
			JSON.stringify({ ready: true, report: resolve(args.report), run_id: plan.run_id }, null, 2),
		);
		return;
	}
	if (command === "verify") {
	...
	if (command === "apply") {
		...
		const checkpointPath = resolve(
			args.checkpoint ||
				`.cutover/checkpoints/${targetName}-${projectRef}-${safety.window_id}.json`,
		);
		let checkpoint;
		try {
...
async function maintenance(command, args, adapter, targetName, projectRef) {
	...
	if (args.confirm !== expected)
		throw new Error(`Refusing maintenance mutation: pass --confirm ${expected}`);
	...
	await writeJsonAtomic(windowPath, window);
	return emit(
		{ ready: response.enabled === enabled, maintenance: response, window: windowPath },
		args,
	);
}
...
async function reverseDelta(args, adapter, projectRef) {
	if (!args.baseline)
		throw new Error("reverse-delta requires --baseline <protected-snapshot.json>");
	const baseline = await readJson(resolve(args.baseline));
	if (baseline.project?.project_ref !== projectRef)
		throw new Error("Reverse-delta baseline project_ref mismatch");
...
function staticPlan(target, environment, projectRef, args) {
	...
	return {
		...
		entity_ids:
			entityIds ??
			MODULE_CUTOVER_REGISTRY.filter((entity) => modules.includes(entity.module)).map(
				(entity) => entity.id,
			),
		remote_access_performed: false,
		next_command: `node scripts/database/module-cutover.mjs plan --target ${target} --environment ${environment} --project-ref ${projectRef} --remote-read`,
		mutation_guard: `CUTOVER:${target}:${projectRef}:<window_id>`,
		entities: registrySummary().filter(
			(entity) => modules.includes(entity.module) && (!entityIds || entityIds.includes(entity.id)),
		),
	};
}
...
if (import.meta.url === pathToFileURL(process.argv[1] || "").href) {
	main().catch((error) => {
		console.error(
			JSON.stringify(
				{ ready: false, error: { code: "cutover_failed", message: error.message } },
				null,
				2,
			),
		);
