...
import { superboardEnvironmentValue } from "../cloudflare/environment.mjs";
import { parseArgs } from "../cloudflare/target.mjs";
import { assertProtectedBackupDirectory, sha256File } from "./d1-backup.mjs";
import { validateMigrationBatchReceipt } from "./migration-batch.mjs";
...
export function backupEncryptionKey(value) {
	...
	if (!/^[A-Za-z0-9+/]{43}=$/u.test(encoded)) {
		throw new Error("SUPERBOARD_BACKUP_ENCRYPTION_KEY must be one base64-encoded 32-byte key");
	}
...
export async function decryptBackupFile(input, output, key) {
	const source = resolve(input);
	if (!isAbsolute(output)) throw new Error("--output must be an absolute protected path");
...
export async function encryptBackupDirectory(
	...
	const sources = entries
		.filter(
			(entry) =>
				entry.isFile() && (entry.name.endsWith(".sql") || entry.name.endsWith(".receipt.json")),
		)
	...
	try {
		for (const source of sources) {
			const encrypted = await encryptBackupFile(source, key, randomBytes(IV_BYTES), {
				removeSource: false,
			});
	...
	const index = {
		schema_version: 1,
		format: "superboard-d1-aes-256-gcm-v1",
...
async function assertCompleteMigrationBatchSources(directory, sources) {
	...
	const batchSources = sources.filter((source) => source.endsWith("-migration-batch.receipt.json"));
	if (batchSources.length !== 1) {
		throw new Error("Exactly one complete migration batch receipt is required");
	}
	let batch;
	try {
		batch = validateMigrationBatchReceipt(JSON.parse(await readFile(batchSources[0], "utf8")));
	} catch (error) {
		throw new Error("Migration batch receipt cannot authorize backup retention", {
			cause: error,
		});
	}
	for (const backup of batch.backups) {
		...
		if (location.startsWith("..") || isAbsolute(location) || !sourceSet.has(receiptPath)) {
			throw new Error(`Migration batch backup receipt is missing for ${backup.service}`);
		}
		let receipt;
		try {
		...
		if (
			...
			throw new Error(`D1 backup receipt does not match the batch for ${backup.service}`);
		}
...
async function main(argv = process.argv.slice(2)) {
	const command = ["encrypt", "decrypt", "check-key"].includes(argv[0]) ? argv[0] : "encrypt";
	...
	if (command === "check-key") {
		process.stdout.write(`${JSON.stringify({ valid: true, algorithm: "AES-256-GCM" })}\n`);
		return;
	}
	if (command === "encrypt") {
		if (!args.directory) throw new Error("encrypt requires --directory");
		const index = await encryptBackupDirectory(args.directory, key, new Date(), {
			requireBatchReceipt: Boolean(args["require-batch-receipt"]),
		});
		process.stdout.write(`${JSON.stringify(index, null, 2)}\n`);
		return;
	}
	if (!args.file || !args.output) throw new Error("decrypt requires --file and --output");
