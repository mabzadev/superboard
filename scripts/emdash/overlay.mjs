...
const integrationConfigPath = resolve(repositoryRoot, "scripts/config/emdash-integration.json");
...
const overlayLintSources = [
	"tests/checks/github/check-no-major.test.mjs",
	"apps/labeler/src/bytes.ts",
	"tests/checks/apps/labeler/bytes.test.ts",
	"scripts/emdash/overlay.mjs",
	"tests/checks/emdash/overlay.test.mjs",
];
...
const overlayFormatSources = [
	...
	".oxfmtrc.json",
	"README.md",
	"scripts/config/emdash-integration.json",
	"scripts/config/emdash-integration.schema.json",
	"scripts/config/emdash-root.overlay.json",
	"scripts/cloudflare/site-build.mjs",
	"tests/checks/cloudflare/site-build.test.mjs",
	...
	"tests/checks/database/restore-proof.mjs",
	"tests/checks/database/restore-proof.test.mjs",
];
...
export function renderIntegratedReadme(productReadme, upstream) {
	...
	const productSource = productReadme.replace(
		/\n## Integrated EmDash foundation\n[\s\S]*?(?=\n## |$)/u,
		"",
	);
	const normalized = normalizePnpmScript(productSource).replace(
		npmCiPattern,
...
This repository integrates EmDash ${upstream.version} from commit
...
export function renderRootPackage(upstreamPackage, overlay) {
	...
	const upstreamScripts = { ...upstreamPackage.scripts, ...product.upstreamScripts };
	const scriptCollisions = Object.keys(product.scripts).filter(
		(key) => upstreamScripts[key] !== undefined,
	);
	const scripts = { ...upstreamScripts };

	for (const key of scriptCollisions) {
		delete scripts[key];
		scripts[`emdash:${key}`] = upstreamScripts[key];
	}
	...
	for (const name of product.removedScripts ?? []) {
		delete scripts[name];
		delete scripts[`emdash:${name}`];
		delete scripts[`superboard:${name}`];
	}
	...
	scripts["emdash:integration:test"] = "node --test tests/checks/emdash/overlay.test.mjs";
	scripts["emdash:lint"] = "node scripts/emdash/overlay.mjs lint-upstream";
	scripts.lint = "node --test tests/checks/repository/lint.test.mjs && node tests/lints/lint.mjs";
	scripts["lint:quick"] = "node tests/lints/lint.mjs --quick --json";
	scripts["lint:json"] = "node tests/lints/lint.mjs --json";
	scripts["lint:fix"] = "node tests/lints/lint.mjs --fix";
	scripts["lint:coverage"] = "node tests/lints/lint.mjs --coverage";
	scripts["lint:test"] = "node --test tests/checks/repository/lint.test.mjs";
...
function isCoveredByUpstreamWorkspace(path) {
	return /^(?:apps|packages)\/[^/]+$/u.test(path) || /^packages\/plugins\/[^/]+$/u.test(path);
}
...
export function renderPnpmWorkspace(upstreamWorkspace, overlay, compatibility) {
	...
	for (const [name, version] of Object.entries(overlay.package.catalog ?? {})) {
		const prefix = `  ${JSON.stringify(name)}:`;
		rendered = rendered
			.split("\n")
			.map((line) => (line.startsWith(prefix) ? `${prefix} ${version}` : line))
			.join("\n");
	}
	for (const workspace of overlay.package.removedWorkspaces ?? []) {
		rendered = rendered.replace(`  - ${workspace}\n`, "");
	}
	if (overlay.package.catalogs) {
		rendered += `\ncatalogs:${renderYamlValue(overlay.package.catalogs, 2)}\n`;
	}
	return rendered;
}

export function renderGitignore(upstreamGitignore, overlay) {
	const removedEntries = new Set(overlay.removedGitignoreEntries ?? []);
	const upstream = upstreamGitignore
		.split("\n")
		.filter((line) => !removedEntries.has(line.trim()))
		.join("\n");
...
export async function reconcileLocalWorkspaceDependencies({ check = false } = {}) {
	...
	const roots = ["apps", "packages", "sdks"].map((path) => resolve(repositoryRoot, path));
...
async function main() {
	...
	throw new Error(
		"Usage: node scripts/emdash/overlay.mjs <capture|copy-worktree|apply|check|workspace-deps|lint-upstream|format|format-check>",
	);
}
