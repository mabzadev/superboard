...
run("runtime plugin build", "pnpm", ["--filter", "@superboard/plugin-*", "build"]);
run("front catalog build", "node", ["apps/site/scripts/build-front-catalog.mjs"]);
...
for (let attempt = 1; attempt <= 4; attempt += 1) {
	...
	run(`runtime plugin rebuild ${attempt}`, "pnpm", ["--filter", "@superboard/plugin-*", "build"]);
	run("front catalog rebuild", "node", ["apps/site/scripts/build-front-catalog.mjs"]);
}
...
run("EmDash View seed", process.execPath, ["scripts/emdash/seed.mjs", "--write"]);
