 ...
 *
 * Usage:
 *   node scripts/emdash/screenshot-templates.mjs <template> <url>
 *   node scripts/emdash/screenshot-templates.mjs blog http://localhost:4321
 *
...
async function run() {
	...
	if (args.length < 2) {
		console.error("Usage: node scripts/emdash/screenshot-templates.mjs <template> <url>");
		console.error("  e.g. node scripts/emdash/screenshot-templates.mjs blog http://localhost:4321");
		process.exit(1);
	}
