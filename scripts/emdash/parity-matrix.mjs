...
const matrixPath = join(root, "scripts/config/emdash-parity-matrix.json");
const topologyPath = join(root, "scripts/config/emdash-plugin-topology.json");
...
const parityFrontTest = "tests/checks/apps/site/front-release-dom-parity.test.tsx";
const parityInstanceTest = "tests/checks/apps/site/runtime/plugin-parity-instance.runtime.test.ts";
const frontCatalogPath = join(root, "infra/generated/front-catalog.js");
...
export function buildParityMatrix() {
	const baseline = JSON.parse(
		readFileSync(join(root, "scripts/config/superboard-plugin-independence-baseline.json"), "utf8"),
	);
		...
		({ path: route, route_id: routeId, plugin_id: pluginId }) => {
			...
			return {
				...row({
					id: `dashboard:${route}`,
					kind: "dashboard",
					baseline: "scripts/config/superboard-plugin-independence-baseline.json",
					target: pluginId,
					test: "tests/checks/emdash/parity-matrix.test.mjs",
	...
	const workerRows = modulePlugins.flatMap(([name, worker]) => {
		if (!worker) return [];
		const proof = workerProof(worker);
		return [
			row({
				id: `worker:${worker}`,
				kind: "worker",
				baseline: `${servicePackagePath(worker)}/src/index.ts`,
				target: `supbrd-plugmod-${name}`,
				test: proof ? relative(root, proof) : "tests/checks/emdash/parity-matrix.test.mjs",
	...
	const sdkRows = [
		...
		[
			...
			"tests/checks/sdks/flutterflow/emdash_store_parity_test.dart",
		],
...
function workerRuntimeContract(name, worker) {
	...
	const path = worker ? servicePackagePath(worker) : "infra/targets";
	const proof = worker
		? workerProof(worker)
		: join(root, "tests/checks/cloudflare/custom-worker-check.test.mjs");
	return {
		path,
...
function migrationInventory(pluginId) {
	...
	const directory = join(root, `${servicePackagePath(worker)}/migrations`);
	...
	return [
		...
		...readdirSync(directory)
			...
			.toSorted()
			.map((name) => `${servicePackagePath(worker)}/migrations/${name}`),
	];
}
...
function workerProof(worker) {
	const directory = join(root, serviceTestDirectory(worker));
	const runtime = join(directory, `runtime/${worker}.runtime.test.ts`);
	if (existsSync(runtime)) return runtime;
	const runtimeProof = walk(join(directory, "runtime"), (path) =>
		path.endsWith(".runtime.test.ts"),
	)[0];
	if (runtimeProof) return runtimeProof;
	const preferred = join(directory, "unit/index.test.ts");
	...
	const receipt = {
		...
		store_coverage: topology.plugins.flatMap(({ manifest }) =>
			manifest.stores.map(({ store_id: storeId, checksum }) => ({
				...
				executable_test: "tests/checks/apps/site/runtime/plugin-store-authority.runtime.test.ts",
				test_sha256: fileChecksum(
					join(root, "tests/checks/apps/site/runtime/plugin-store-authority.runtime.test.ts"),
				),
