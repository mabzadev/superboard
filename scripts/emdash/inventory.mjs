...
function parseWorkerRoutes() {
	...
	for (const match of index.matchAll(
		/import\s+([a-zA-Z0-9_]+)\s+from\s+['"]\.\/routes\/([^'"]+)['"]/g,
	)) {
	...
	for (const file of listFiles(`${workerRoot}/src/routes`, ".ts").filter(
	...
	for (const file of listFiles(`${workerRoot}/migrations`, ".sql")) {
		...
		for (const line of lines) {
			...
			if (current) {
				...
				if (
					column &&
					!["FOREIGN", "PRIMARY", "UNIQUE", "CHECK", "CONSTRAINT"].includes(column.toUpperCase())
				) {
...
const inventory = {
	...
	upstreamAvailable,
	upstream: {
		backend: gitRev("upstream/superboard/backend"),
		dashboard: gitRev("upstream/superboard/dashboard"),
		mcp: gitRev("upstream/superboard/mcp"),
		superboardJs: gitRev("upstream/superboard/superboard-js"),
		ios: gitRev("upstream/superboard/superboard-iOS"),
		android: gitRev("upstream/superboard/superboard-Android"),
		reactNative: gitRev("upstream/superboard/superboard-react-native"),
		flutter: gitRev("upstream/superboard/superboard-flutter"),
		utils: gitRev("upstream/superboard/superboard-utils"),
	},
	counts: {
...
if (process.argv.includes("--require-upstream") && !upstreamAvailable) {
	console.error(`Upstream parity cannot be verified because ${upstreamBackend} is not available.`);
	process.exitCode = 2;
}
