...
} from "../../infra/generated/front-catalog.js";
import { compileFrontRelease, verifyFrontRelease } from "../../packages/supbrd-core/dist/index.js";
import { compileTarget } from "../cloudflare/target-compiler.mjs";

const root = resolve(import.meta.dirname, "../..");
const artifactPath = resolve(root, "scripts/config/superboard-parity-release.json");
const target = JSON.parse(
	readFileSync(resolve(root, "infra/targets/mbza-development.json"), "utf8"),
);
const identifiers = {
...
export async function verifyParityReleaseArtifact(options = {}) {
	...
	if (!existsSync(artifactPath)) {
		throw new Error("Generated artifact drift: scripts/config/superboard-parity-release.json");
	}
	const committed = JSON.parse(readFileSync(artifactPath, "utf8"));
