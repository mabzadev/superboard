...
const githubPackagesTokenEnvironmentVariable = "OPENGROW_GITHUB_PACKAGES_TOKEN";
const githubPackagesUserEnvironmentVariable = "OPENGROW_GITHUB_PACKAGES_USER";
...
export async function validateSdkCatalog(catalog, options = {}) {
	...
	const releaseHistory = options.releaseHistory ?? (await loadSdkReleaseHistory());
	...
	for (const library of catalog?.libraries ?? []) {
		...
		ids.add(library.id);
		if (!lifecycles.has(library.lifecycle)) errors.push(`${prefix}.lifecycle is invalid`);
		...
		if (hasCandidatePackage !== hasCandidateInstall) {
			errors.push(`${prefix}.candidatePackageName and candidateInstall must be declared together`);
		}
		if (hasCandidatePackage) {
			if (!active) {
				errors.push(`${prefix}.candidate package migration is only allowed for active libraries`);
			}
			if (library.releaseStatus !== "pending-release") {
				errors.push(`${prefix}.candidate package migration requires a pending release`);
			}
			if (
			...
			if (!/^superboard(?:[_-]|$)/u.test(library.candidatePackageName)) {
				errors.push(`${prefix}.candidatePackageName must use the SuperBoard namespace`);
			}
			if (
				!String(library.candidateInstall ?? "").includes(library.candidatePackageName) ||
				!String(library.candidateInstall ?? "").includes(library.sourceVersion)
			) {
				errors.push(`${prefix}.candidateInstall must pin candidatePackageName at sourceVersion`);
			}
			if (!String(library.candidateInstall).includes(canonicalRepository)) {
				errors.push(`${prefix}.candidateInstall must use the canonical SuperBoard repository`);
			}
		}
		if (active && !/^SuperBoard\b/u.test(library.displayName ?? "")) {
			errors.push(`${prefix}.displayName must use the SuperBoard brand`);
		}
		if (active && /opengrow/iu.test(library.packageName ?? "") && !hasCandidatePackage) {
			errors.push(`${prefix} must stage a SuperBoard package coordinate before release`);
		}
		...
		if (unreleased) {
			...
			if (library.publicationTarget !== expectedTarget) {
				errors.push(`${prefix}.publicationTarget must be ${expectedTarget} while unreleased`);
			}
			for (const key of [
				...
				if (Object.hasOwn(library, key)) {
					errors.push(`${prefix}.${key} must be absent until the first release is published`);
				}
			}
		} else {
			...
			if (!commitSha.test(library.releaseSha ?? "")) {
				errors.push(`${prefix}.releaseSha must identify the latest published commit`);
			}
		...
		const versionPath = protectedRepoPath(library.versionSource, `${prefix}.versionSource`, errors);
		...
		if (versionPath && (await exists(versionPath))) {
			...
			if (library.ecosystem === "npm" && versionPath.endsWith("package.json")) {
				...
				const expectedPackageName = library.candidatePackageName ?? library.packageName;
			...
			if (
				...
				const observedPackageName = manifest.match(/^name:\s*([^\s]+)/mu)?.[1] ?? null;
				const expectedPackageName = library.candidatePackageName ?? library.packageName;
	...
	if (options.releaseCandidateTag)
		validateReleaseCandidateTag(catalog, releaseHistory, options.releaseCandidateTag, errors);
	return {
		ok: errors.length === 0,
		errors,
...
function validateDistributionContract(catalog, library, prefix, errors) {
	...
	if (!registryDistributionIds.has(library?.id)) {
		if (distribution !== undefined) {
			errors.push(`${prefix}.distribution is only supported for registry-distributed SDKs`);
		}
		return;
	}
	...
	const expectedKind = isNpm ? "github-packages-npm" : "github-packages-maven";
	...
	if (
		distribution.authentication?.tokenEnvironmentVariable !== githubPackagesTokenEnvironmentVariable
	) {
		errors.push(
	...
	if (isNpm) {
		if (Object.hasOwn(distribution.authentication ?? {}, "usernameEnvironmentVariable")) {
			errors.push(
...
function releaseTagEntry(catalog, tag, errors) {
	const entry = Object.entries(tagPrefixes).find(([, prefix]) => tag.startsWith(prefix));
	if (!entry) {
...
function validateReleaseTag(catalog, tag, expectedSha, errors) {
	...
	if (library.lifecycle !== "active") {
		errors.push(`${tag} cannot publish ${library.lifecycle} library ${library.id}`);
	}
	if (library.sourceVersion !== version)
		errors.push(`${tag} does not match source version ${library.sourceVersion}`);
	if (library.latestReleaseVersion !== version || library.releaseStatus !== "released")
		errors.push(`${tag} requires latestReleaseVersion ${version} and releaseStatus released`);
...
function validateReleaseCandidateTag(catalog, history, tag, errors) {
	...
	const alreadyPublished =
		library.latestReleaseVersion === version && library.releaseStatus === "released";
	const pending =
		library.latestReleaseVersion !== version && library.releaseStatus === "pending-release";
...
async function validateFlutterFlowSurface(catalog, errors, manifestOverride) {
	const manifest =
		manifestOverride ??
		JSON.parse(await readFile(resolve(root, "config/flutterflow-custom-code.json"), "utf8"));
	if (manifest.schemaVersion !== 1 || manifest.owner !== "superboard")
		errors.push("FlutterFlow custom code manifest ownership is invalid");
	const sourceFiles = [...(await readTree(resolve(root, "sdks/flutterflow/lib")))];
	...
	validateExactSet(declaredSourceFiles, expectedSourceFiles, "FlutterFlow source file", errors);
	for (const [group, paths] of Object.entries(manifest.sourceFiles ?? {})) {
		for (const value of paths) {
			const path = protectedRepoPath(value, `FlutterFlow sourceFiles.${group}`, errors);
	...
	if (!surfaceUsers.includes("flutterflow"))
		errors.push("The active FlutterFlow library must declare the canonical surface manifest");
}
...
async function sourceVersion(path) {
	...
	if (path.endsWith("pubspec.yaml")) return source.match(/^version:\s*([^\s]+)/m)?.[1] ?? null;
	if (path.endsWith(".podspec")) return source.match(/\.version\s*=\s*['"]([^'"]+)/)?.[1] ?? null;
	if (path.endsWith(".kts"))
		return source.match(/private val libraryVersion[\s\S]*?else\s*\{\s*"([^"]+)"/)?.[1] ?? null;
	return null;
}
...
export function releaseCandidateTagFor(catalog, id) {
	...
	const pending =
		...
		library.sourceVersion !== library.latestReleaseVersion;
	const initial =
		library.releaseStatus === "unreleased" && library.publicationTarget === "public-npm";
	const catalogueRecovery =
		library.releaseStatus === "released" && library.sourceVersion === library.latestReleaseVersion;
...
export function promoteSdkRelease(catalog, id, version, releaseSha) {
	...
	if (library.lifecycle !== "active")
		throw new Error(`${id} is ${library.lifecycle} and cannot be published`);
	if (!tagPrefixes[id]) throw new Error(`${id} has no immutable release namespace`);
	if (!semver.test(version ?? "")) throw new Error(`Invalid SDK release version: ${version}`);
	if (!commitSha.test(releaseSha ?? ""))
		throw new Error(`Invalid SDK release SHA: ${String(releaseSha)}`);
	if (library.sourceVersion !== version) {
		throw new Error(`${id} source version ${library.sourceVersion} does not match ${version}`);
	}
	if (library.releaseStatus === "released" && library.latestReleaseVersion === version) {
	...
	if (library.releaseStatus !== "pending-release" && library.releaseStatus !== "unreleased") {
...
async function main() {
	...
	if (command === "promote") {
		const promoted = promoteSdkRelease(catalog, args.library, args.version, args.sha);
	...
	console.log(
		JSON.stringify({ schema_version: 1, status: "ok", libraries: result.libraries }, null, 2),
	);
}
