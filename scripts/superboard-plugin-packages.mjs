...
export function lintPluginPackageProject(root) {
	try {
		const topology = JSON.parse(
			readFileSync(join(root, "config/emdash-plugin-topology.json"), "utf8"),
		);
		const expected = buildPluginPackages(topology);
		const actual = JSON.parse(
			readFileSync(join(root, "config/superboard-plugin-catalog.json"), "utf8"),
		);
		if (canonical(actual) !== canonical(expected)) throw new Error("PLUGIN_PACKAGE_CATALOG_STALE");
		const expectedEntries = [
			"front-catalog.ts",
			...expected.plugins.map(({ manifest }) => `${manifest.plugin_id}.ts`),
		].sort();
		const entries = readdirSync(join(root, "packages/supbrd-runtime-plugins/src/entries"))
		...
		return [
			{
				...
				severity: "error",
				filename: "config/superboard-plugin-catalog.json",
