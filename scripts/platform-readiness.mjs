...
import { DOMAIN_SERVICES, DOMAIN_SERVICE_REGISTRY } from "./cloudflare-services.mjs";
import { cloudflareAccountEnvName, loadTarget, parseArgs, root } from "./cloudflare-target.mjs";
...
import { loadDeploymentMatrix, validateControlPlaneCoverage } from "./github-deployment-matrix.mjs";
import { inspectRepository, loadGitHubControlPlane } from "./github-readiness.mjs";
import { releaseCandidateTagFor, releaseTagFor } from "./sdk-catalog.mjs";
import { canonicalReleaseTag, immutableFailureFor } from "./sdk-release-history.mjs";
...
export async function buildReadiness({
	...
	const catalogue = JSON.parse(await readFile(resolve(root, "config/sdk-libraries.json"), "utf8"));
	...
	const governanceSchema = JSON.parse(
		await readFile(resolve(root, "schemas/platform-governance.schema.json"), "utf8"),
	);
	...
	const historicalSourceAvailable = existsSync(resolve(root, "upstream/opengrow/backend"));
	...
	const securityContracts = {
		oauthDashboardRotation: {
			ready:
				existsSync(resolve(root, "workers/api/migrations/0056_oauth_client_secret_overlap.sql")) &&
