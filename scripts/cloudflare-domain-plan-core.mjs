...
export function expectedDomainOwners(target, environment) {
	...
	if (publicRouting !== "active") {
		throw new Error(`environments.${environment}.publicRouting must be active or staged`);
	}
	const entries = [
		["api", target.domains.api, target.workers.api?.[environment]],
		["auth", target.domains.auth, target.workers.api?.[environment]],
		["shortlinks", target.domains.shortlinks, target.workers.api?.[environment]],
		...
		...(target.domains.dashboard && target.domains.dashboard !== target.domains.site
			? [["dashboard", target.domains.dashboard, target.workers.site?.[environment]]]
			: []),
			...
			? [
					[
						...
						target.deploymentProfile === "consolidated"
							...
							: target.workers.mcp?.[environment],
					],
				]
			: []),
		...(target.domains.mailPreview
			? [["mailPreview", target.domains.mailPreview, target.workers.email?.[environment]]]
			: []),
		...(target.features.messaging && target.domains.messaging
			? [["messaging", target.domains.messaging, target.workers.messaging?.[environment]]]
			: []),
	];

	for (const alias of target.domainAliases ?? []) {
		if (alias.surface === "mailPreview" && target.mail?.transport !== "capture") continue;
		const service =
			alias.surface === "console"
				? "site"
				: alias.surface === "mailPreview"
					? "email"
					: alias.surface === "mcp" && target.deploymentProfile !== "consolidated"
						? "mcp"
						: "api";
		entries.push([
			`legacy-${alias.surface}`,
			alias.hostname,
			target.workers[service]?.[environment],
		]);
	}
...
export function retiredDomainOwners(target) {
	...
	return (target.retiredDomains ?? []).map((retired) => {
		const hostname = required(retired.hostname, "retiredDomains[].hostname").toLowerCase();
		if (!/^[a-z0-9.-]+$/.test(hostname) || hostname.includes("..")) {
		...
		return {
			hostname,
			formerSurface: required(retired.formerSurface, "retiredDomains[].formerSurface"),
...
export function targetWorkerNames(target, environment) {
	return [
		...Object.values(target.workers ?? {}).map((workers) => workers?.[environment]),
		...(target.customWorker?.managedWorkers ?? []).map((worker) => worker.workers?.[environment]),
	].filter(Boolean);
}

function zoneForHostname(hostname, zones) {
	return zones
		.filter((zone) => zone.name && (hostname === zone.name || hostname.endsWith(`.${zone.name}`)))
...
export function evaluateDomainPlan({
	...
	return expected.map((owner) => {
		...
		const assigned = workerDomains.filter(
			(domain) => String(domain.hostname ?? "").toLowerCase() === owner.hostname,
		);
		const dns = (dnsRecordsByHostname[owner.hostname] ?? []).map((record) => ({
			type: record.type,
			proxied: record.proxied ?? null,
		}));
		const dnsInspectionError = dnsInspectionErrorsByHostname[owner.hostname] ?? null;
...
export function evaluateRetiredDomainPlan({
	...
	return retired.map((entry) => {
		...
		const assigned = workerDomains.filter(
			(domain) => String(domain.hostname ?? "").toLowerCase() === entry.hostname,
		);
		...
		const dnsInspectionError = dnsInspectionErrorsByHostname[entry.hostname] ?? null;
		if (dnsInspectionError) {
			return {
				...entry,
				zone: zone.name,
...
export function evaluateOrphanWorkerDomains({ expected, retired, workerDomains, managedServices }) {
	...
	return [...orphaned.values()].sort((left, right) => left.hostname.localeCompare(right.hostname));
}
