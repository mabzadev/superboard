...
import { healthPathForService } from "../cloudflare/services.mjs";
import {
...
async function start() {
	...
	run(process.execPath, [
		"scripts/cloudflare/target-orchestrator.mjs",
		"configure",
		"--target",
		targetName,
		"--environment",
	...
	for (const migration of materialization.migrations) {
		const config =
			migration.service === "site"
				? resolve(root, "apps/site/wrangler.jsonc")
				: resolve(root, "infra/generated", `${targetName}-${migration.service}-local.jsonc`);
		const value =
			migration.service === "site"
				? compileLocalSiteConfiguration(compiled)
				: await readLocalConfiguration(config);
	...
	for (const service of materialization.services.filter((service) => service.id !== "site")) {
		...
		try {
			await waitForLocalListener(
				`http://${service.localEndpoint.host}:${service.localEndpoint.port}${healthPathForService(service.id)}`,
				{
					isClosed: () => closing || child.exitCode !== null,
				},
			);
		} catch (error) {
			stop();
...
export async function readLocalConfiguration(path) {
	const errors = [];
	const configuration = parse(await readFile(path, "utf8"), errors, { allowTrailingComma: true });
	if (errors.length) throw new SyntaxError(`${path}: ${printParseErrorCode(errors[0].error)}`);
	return configuration;
}

export async function waitForLocalListener(
	url,
	{ timeoutMs = 120000, isClosed = () => false } = {},
) {
	const deadline = Date.now() + timeoutMs;
	while (Date.now() < deadline) {
		if (isClosed()) throw new Error(`Local service stopped before becoming ready: ${url}`);
		try {
			...
			if (typeof health?.service === "string" || typeof health?.data?.service === "string") return;
		} catch {
			if (isClosed()) throw new Error(`Local service stopped before becoming ready: ${url}`);
		}
		await new Promise((resolveWait) =>
			setTimeout(resolveWait, Math.max(1, Math.min(200, deadline - Date.now()))),
		);
	}
