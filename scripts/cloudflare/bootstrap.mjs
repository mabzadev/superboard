...
export async function fetchCloudflareBootstrapInventories({ accountId, token, fetchImpl = fetch }) {
	...
	return {
		...
		async listPaged(endpoint) {
			...
			for (let page = 1; page <= 10_000; page += 1) {
				...
				const payload = await request(`${endpoint}${separator}page=${page}&per_page=1000`);
				if (!Array.isArray(payload.result)) {
					throw new Error(`Cloudflare ${endpoint} returned a non-array result`);
				}
...
function offlineReport(target, environment, compiledTarget, { freshSupportInstall = false } = {}) {
	...
	return {
		schemaVersion: 1,
		mode: freshSupportInstall ? "offline-fresh-support-desired-state" : "offline-desired-state",
		...
		action: "Re-run with --remote and scoped credentials to build the exact drift-checked plan.",
	};
}
