...
if (!apiToken) throw new Error("CLOUDFLARE_API_TOKEN is required for the read-only domain plan");
...
for (const owner of [...expected, ...retired]) {
	const zone = zones
		.filter(
			(candidate) =>
				candidate.name &&
				(owner.hostname === candidate.name || owner.hostname.endsWith(`.${candidate.name}`)),
		)
		.sort((left, right) => right.name.length - left.name.length)[0];
	dnsRecordsByHostname[owner.hostname] = [];
	if (zone?.id) {
		try {
			dnsRecordsByHostname[owner.hostname] = await listAll(`/zones/${zone.id}/dns_records`, {
				name: owner.hostname,
				per_page: "100",
			});
		} catch (error) {
			dnsInspectionErrorsByHostname[owner.hostname] =
				error instanceof Error ? error.message : "Cloudflare DNS inspection failed";
		}
	}
}
...
if (
	args.strict &&
	[...domains, ...retiredDomains, ...orphanedDomains].some((domain) => domain.blocking)
) {
	throw new Error(
	...
	do {
		...
		if (!response.ok || !payload?.success) {
			const errors = Array.isArray(payload?.errors)
				? payload.errors.map(
						(error) => `${error.code ?? "unknown"}: ${error.message ?? "Cloudflare API error"}`,
					)
