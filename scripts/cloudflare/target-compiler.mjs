...
import { localMigrationFiles, targetD1Descriptors } from "../database/d1-registry.mjs";
import { desiredCloudflareResources } from "./bootstrap-core.mjs";
import { deploymentOrder } from "./deploy-plan.mjs";
import { deploymentConfiguration } from "./deployment-configuration.mjs";
import { requiredSecretInventory, secretInventory } from "./secret-inventory.mjs";
import {
	DOMAIN_SERVICES,
...
export async function compiledTargetFromArgs(target, environment, args = {}) {
	...
	const generatedDirectory = resolve(root, "infra/generated");
	...
	if (relativePath.startsWith("..") || relativePath === "" || relativePath.startsWith("/")) {
		throw new Error("Target artifact must be inside infra/generated");
	}
...
export function assertTargetServiceConfiguration(
	...
	for (const secret of expectedSecrets?.names ?? []) {
		if (
			service === "site" &&
			secret === "SUPERBOARD_RELEASE_PRIVATE_JWK" &&
			configuration.vars?.SUPERBOARD_RELEASE_OPERATIONS === "disabled"
		)
			continue;
...
async function loadPluginTopology(
	path = resolve(root, "scripts/config/emdash-plugin-topology.json"),
) {
