...
import { buildSecretBundlePlan, secretVersionTag } from "./secret-bundle.mjs";
import { workerNameForService } from "./services.mjs";
import {
	cloudflareEnv,
	environmentFromArgs,
	loadTarget,
...
export function validateSecretUploadReceipt({
	...
	const expectedServices = new Set(
		bundlePlan.contracts.flatMap(({ members }) => members.map(({ service }) => service)),
	);
	...
	for (const entry of receipt.services) {
		const expectedWorker = workerNameForService(target, entry.service, environment);
		...
		for (const member of expectedMembers) {
			if (member.name) {
				if (!remainingNames.delete(member.name)) {
					throw new Error(`Secret upload receipt names are incomplete for ${entry.service}`);
				}
			} else {
			...
			if (bundlePlan.overlap && member.previousName) {
				if (!remainingNames.delete(member.previousName)) {
					throw new Error(`Secret upload receipt overlap name is missing for ${entry.service}`);
				}
			}
		}
		if (remainingNames.size) {
			throw new Error(`Secret upload receipt contains unexpected names for ${entry.service}`);
		}
	...
	for (const contract of bundlePlan.contracts) {
		...
		if (latestConsumer >= earliestProducer) {
			blockers.push({
				...
				action: "Deploy every current+previous consumer before any new-token-only producer.",
			});
		}
	}
	return blockers;
...
export function attachPromotionRemoteState(plan, remoteState, accountId) {
	...
	const services = plan.services.map((service) => {
		...
		if (
			...
			throw new Error(`${service.worker} must have exactly one 100% active rollback version`);
		}
		...
		if (!tagged || !/^[0-9a-f-]{36}$/i.test(tagged.id ?? "")) {
			throw new Error(`${service.worker} does not contain inactive tag ${service.versionTag}`);
		}
...
export function buildPromotionCompleteReceipt(plan, promoted, promotedAt = new Date()) {
	const timestamp =
		promotedAt instanceof Date ? promotedAt.toISOString() : new Date(promotedAt).toISOString();
	return {
		schemaVersion: 1,
...
async function main() {
	...
	if (args.confirm !== plan.confirmation) {
		throw new Error(`Refusing secret promotion: pass --confirm ${plan.confirmation}`);
	}
		...
		for (const service of plan.services) {
			run(
				...
				promotionArgs(
					service,
					service.versionId,
					`SuperBoard coordinated secret promotion ${plan.bundleConfirmation}`,
				),
				childEnv,
			...
			try {
				run(
					...
					promotionArgs(
						...
						`SuperBoard automatic rollback ${plan.bundleConfirmation}`,
					),
					childEnv,
