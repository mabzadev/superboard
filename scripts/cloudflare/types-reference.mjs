...
import { DOMAIN_SERVICES } from "./services.mjs";
import { parseArgs, targetSelectionFromArgs } from "./target.mjs";
...
export async function cloudflareTypesMode(args, env = process.env) {
	...
	const configuredEnvironment = superboardEnvironmentValue("SUPERBOARD_ENVIRONMENT", env);
	if (configuredTarget && configuredTarget !== args.target) {
		throw new Error(
			`--target ${args.target} does not match SUPERBOARD_TARGET/SUPERBOARD_TARGET configured value ${configuredTarget}`,
		);
	}
	if (configuredEnvironment && configuredEnvironment !== args.environment) {
		throw new Error(
			`--environment ${args.environment} does not match SUPERBOARD_ENVIRONMENT/SUPERBOARD_ENVIRONMENT configured value ${configuredEnvironment}`,
		);
	}
	const selection = await targetSelectionFromArgs(args, env, {
		allowReference: true,
	...
	return {
		...selection,
		mode: "target",
		generatorArgs: ["--target", selection.targetName, "--environment", selection.environment],
		customSelectionArgs: {
...
export async function main(argv = process.argv.slice(2), env = process.env, execute = run) {
	...
	const outputs = [
		...
		...services.map((service) =>
			service === "site"
				...
				: resolve(root, servicePackagePath(service), "worker-configuration.d.ts"),
		),
