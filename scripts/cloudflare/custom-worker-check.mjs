...
import { managedWorkerService } from "./services.mjs";
import { environmentFromArgs, loadTarget, parseArgs, root, targetNameFromArgs } from "./target.mjs";
		...
		...new Set(
			targets
				.flatMap((target) => [
					...
					...(target.customWorker?.managedWorkers ?? []).map(({ packagePath }) => packagePath),
				])
				.filter(Boolean)
...
export async function selectedCustomWorkerPackages(args, env = process.env) {
	if (args.all && (args.target || superboardEnvironmentValue("SUPERBOARD_TARGET", env))) {
		throw new Error(
			"--all cannot be combined with --target or SUPERBOARD_TARGET (fallback SUPERBOARD_TARGET)",
		);
	}
	if (!args.all) {
	...
	const entries = await readdir(resolve(root, "infra", "targets"), {
		withFileTypes: true,
	});
...
export async function selectedCustomWorkerTypeSelections(args, env = process.env) {
	if (args.all) {
		const entries = await readdir(resolve(root, "infra", "targets"), {
			withFileTypes: true,
		});
		return Promise.all(
			entries
				...
				.map(async (entry) => {
					...
					return {
						targetName,
						environment: environments.includes("development") ? "development" : environments[0],
						packagePath: validatedPackagePath(target.customWorker.packagePath),
						managedServices: (target.customWorker.managedWorkers ?? []).map(managedWorkerService),
		...
		).then((selections) => uniqueCustomWorkerTypeSelections(selections.filter(Boolean)));
	}
...
export function uniqueCustomWorkerTypeSelections(selections) {
	...
	for (const selection of selections) {
		const packages = [selection.packagePath, ...Object.values(selection.managedPackages ?? {})];
