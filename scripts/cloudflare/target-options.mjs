...
const SENSITIVE_VAR_NAME = /(?:^|_)(?:TOKEN|SECRET|PASSWORD|PRIVATE_KEY|API_KEY|ACCESS_KEY)$/;
...
export function customTargetOptions(args, { target, environment, baseResourceName }) {
	...
	if (!source) {
		const unexpected = customOptionNames.find((name) => args[name] !== undefined);
		if (unexpected) {
	...
	if (Boolean(d1Binding) !== Boolean(migrationsDir)) {
		throw new Error("--custom-d1-binding and --custom-migrations-dir must be supplied together");
	}
	if (args["custom-d1-name"] !== undefined && !d1Binding) {
		throw new Error("--custom-d1-name requires --custom-d1-binding");
	}

	const secrets = unique(["CUSTOM_WORKER_TOKEN", ...csv(args["custom-secrets"])]);
	...
	const worker = {
		source,
		packagePath: optional(args["custom-package"]) || source.replace(/\/src\/index\.ts$/, ""),
		description:
...
export function validateCustomWorkerBindings(customWorker) {
	...
	const bindings = [
		...
		...(customWorker.serviceBindings ?? []).map(({ binding }) => [binding, "service"]),
	];
...
export function validateManagedWorkers(customWorker) {
	...
	for (const worker of workers) {
		if (ids.has(worker.id)) {
			throw new Error(`Invalid target manifest: duplicate managed Worker id ${worker.id}`);
		}
		ids.add(worker.id);
		...
		const dockerfiles = new Set(worker.containers.map(({ dockerfile }) => dockerfile));
		...
		const bindings = [
			...
			...worker.containers.map(({ binding }) => [`${binding}_MAX_INSTANCES`, "generated var"]),
			...
			...worker.durableObjects.map(({ binding }) => [binding, "Durable Object"]),
		];
		const occupied = new Map();
		for (const [name, owner] of bindings) {
			const existing = occupied.get(name);
...
function stringMap(value, label) {
	...
	for (const [key, entry] of Object.entries(parsed)) {
		if (!/^[A-Z][A-Z0-9_]+$/.test(key) || typeof entry !== "string") {
			throw new Error(`${label} must contain uppercase names and string values`);
		}
		result[key] = entry;
	}
	return result;
