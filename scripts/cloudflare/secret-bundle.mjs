...
import { secretCoordinationPlan } from "./secret-inventory.mjs";
...
export function buildSecretBundlePlan({
	...
	const byId = new Map(coordination.contracts.map((contract) => [contract.id, contract]));
	...
	if (
		overlap &&
		!selected.some((contract) => contract.members.some((member) => member.previousName))
	) {
		blockers.push({
...
export function buildSecretAssignments(plan, payload) {
	...
	for (const contract of plan.contracts) {
		...
		const overlapCapable = contract.members.some(({ previousName }) => Boolean(previousName));
		const previousValue = entry.previousValue;
		if (plan.overlap && overlapCapable) {
			if (typeof previousValue !== "string" || Buffer.byteLength(previousValue) === 0) {
		...
		if (JSON.stringify(Object.keys(entry).sort()) !== JSON.stringify(allowedEntryKeys.sort())) {
...
export function versionedSecretBundleArgs(config, targetName, environment, service, tag) {
	return [
		"wrangler",
		"versions",
		"secret",
		...
		config,
		"--message",
		`SuperBoard coordinated secrets for ${targetName}/${environment}/${service}`,
		...(tag ? ["--tag", tag] : []),
	];
}
...
export function secretVersionTag(plan, service) {
	...
	return `superboard-secret-${digest}-${service}`;
}
...
async function main() {
	...
	if (args.confirm !== plan.confirmation) {
		throw new Error(`Refusing secret bundle upload: pass --confirm ${plan.confirmation}`);
	}
	...
	for (const [service, values] of Object.entries(assignments)) {
		...
		run("npx", versionedSecretBundleArgs(config, targetName, environment, service, tag), {
			env: childEnv,
	...
	process.stdout.write(`${JSON.stringify(buildSecretUploadReceipt(plan, receipts), null, 2)}\n`);
}
