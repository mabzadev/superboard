...
} from "./development-secrets.mjs";
import { siteEmailBuildEnvironment } from "./site-build.mjs";
import {
...
async function main(argv = process.argv.slice(2)) {
	...
	if (operation === "provision") {
		...
		runNode(
			service === "all" ? "scripts/cloudflare/deploy-all.mjs" : "scripts/cloudflare/deploy.mjs",
...
async function configureTarget(
	...
	for (const { id: service } of prepared.materialization.services) {
		runNode("scripts/cloudflare/config.mjs", [
			"--target",
...
function migrateTarget(prepared, args) {
	if (prepared.materialization.adapter === "cloudflare") {
		...
		runNode("scripts/database/d1-converge.mjs", [
			apply ? "apply" : "plan",
			"--target",
			prepared.compiled.target,
			"--environment",
...
function runNode(script, args) {
	run(process.execPath, [resolve(root, script), ...args]);
}
...
async function startLocalTarget(prepared, args, { verify = false } = {}) {
	const secretAssignments = verify
		...
		: {};
	if (verify) {
		for (const service of prepared.materialization.services) {
			const configPath = resolve(
				root,
				"infra/generated",
			...
			await writeJsonAtomically(
				configPath,
				withLocalSecretBindings(configuration, secretAssignments[service.id] ?? {}),
			);
		}
	}
