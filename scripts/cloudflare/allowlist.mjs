...
const action = argv.find(
	(value) => !value.startsWith("--") && !argv[argv.indexOf(value) - 1]?.startsWith("--"),
);
...
if (!resources?.d1?.id)
	throw new Error(`${targetName} does not define a provisioned ${environment} D1 database`);
if (!new Set(["add", "revoke", "list", "bootstrap"]).has(action)) {
	throw new Error(
		"Usage: npm run allowlist -- <add|revoke|list|bootstrap> --target <slug> --environment <development|production> [--email user@example.com]",
	);
}

const realm = `${target.target}:${environment}`;
const email = action === "add" || action === "revoke" ? normalizeEmail(args.email) : null;
const configPath = resolve(root, "infra", "generated", `${targetName}-api-${environment}.jsonc`);
...
run(
	"npx",
	["wrangler", "d1", "execute", "DB", "--remote", "--config", configPath, "--command", command],
	targetCloudflareEnv,
);
