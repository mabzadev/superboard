export const SERVICE_PACKAGE_PATHS = Object.freeze({
	api: "packages/plugins/superboard-core/api",
	app: "packages/plugins/superboard-core/app",
	observability: "packages/plugins/superboard-core/observability",
	mcp: "packages/plugins/superboard-core/mcp",
	identity: "packages/plugins/superboard-authentification/worker",
	files: "packages/plugins/superboard-data/worker",
	products: "packages/plugins/superboard-monetization/products",
	billing: "packages/plugins/superboard-monetization/billing",
	paywalls: "packages/plugins/superboard-acquisition/paywalls",
	marketing: "packages/plugins/superboard-communication/marketing",
	email: "packages/plugins/superboard-communication/email",
	"dynamic-links": "packages/plugins/superboard-acquisition/dynamic-links",
	onboardings: "packages/plugins/superboard-acquisition/onboardings",
	flows: "packages/plugins/superboard-acquisition/flows",
	support: "packages/plugins/superboard-support/worker",
	analytics: "packages/plugins/superboard-analytics/worker",
	site: "apps/site",
});

export function servicePackagePath(service) {
	const directory = SERVICE_PACKAGE_PATHS[service];
	if (!directory) throw new Error(`Unknown service package: ${service}`);
	return directory;
}

export function serviceTestDirectory(service) {
	return `tests/checks/${servicePackagePath(service).replace(/^packages\/plugins\//u, "plugins/")}`;
}

const TASK_SERVICE_PLUGINS = Object.freeze({
...
export function taskApiServiceBinding(service) {
	const pluginId = TASK_SERVICE_PLUGINS[service];
	if (!pluginId) throw new Error(`No plugin task authority for ${service}`);
	return { binding: "API_SERVICE", service: "api", props: { superboard_plugin_id: pluginId } };
}
...
export const DOMAIN_SERVICE_REGISTRY = Object.freeze({
	...
	support: domainService("SUPPORT_MODULE", "support", {
		...
		crons: ["* * * * *"],
		ai: { binding: "AI" },
		vectorize: [{ binding: "SUPPORT_KNOWLEDGE", resourceKey: "supportKnowledge" }],
		durableObjects: [{ name: "CONVERSATIONS", className: "ConversationRoom" }],
		durableObjectMigrations: [{ tag: "v1", newSqliteClasses: ["ConversationRoom"] }],
		services: [
...
export const DOMAIN_SERVICES = Object.freeze(Object.keys(DOMAIN_SERVICE_REGISTRY));
...
export const PLATFORM_SERVICES = Object.freeze(Object.keys(PLATFORM_SERVICE_REGISTRY));
...
export const PLATFORM_SERVICE_SECRETS = Object.freeze({
	...
	billing: Object.freeze([...BILLING_SECRETS, "INTERNAL_API_TOKEN", "INTERNAL_API_TOKEN_PREVIOUS"]),
	email: Object.freeze([
...
export const ALL_SERVICES = Object.freeze([...PLATFORM_SERVICES, ...DOMAIN_SERVICES]);
...
export function assertServiceForTarget(target, service) {
	if (service === "messaging")
		throw new Error("The Legacy Messaging service has been retired; use --service support");
...
export function workerNamesForService(target, service) {
	return managedWorkerDefinition(target, service)?.workers ?? target.workers?.[service];
}
...
export function isServiceEnabled(target, service) {
	if (service === "messaging") return false;
...
export function healthPathForService(service) {
	...
	const definition = PLATFORM_SERVICE_REGISTRY[service] ?? DOMAIN_SERVICE_REGISTRY[service];
...
function domainService(binding, resourceKey, options = {}) {
	...
	return Object.freeze({
		binding,
		resourceKey,
		main: `../../${servicePackagePath(resourceKey === "dynamicLinks" ? "dynamic-links" : resourceKey)}/src/index.ts`,
		migrationsDir: `../../${servicePackagePath(resourceKey === "dynamicLinks" ? "dynamic-links" : resourceKey)}/migrations`,
		...
		vectorize: Object.freeze(
			(options.vectorize ?? []).map((binding) => Object.freeze({ ...binding })),
		),
		crons: Object.freeze(options.crons ?? []),
		durableObjects: Object.freeze(options.durableObjects ?? []),
		services: Object.freeze(
			TASK_SERVICE_PLUGINS[resourceKey]
				? [
						...(options.services ?? []).filter((entry) => entry.binding !== "API_SERVICE"),
						taskApiServiceBinding(resourceKey),
					]
				: (options.services ?? []),
		),
		workflows: Object.freeze(options.workflows ?? []),
		durableObjectMigrations: Object.freeze(options.durableObjectMigrations ?? []),
