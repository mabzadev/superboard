...
import { assertTargetPhysicalResourceNames } from "./resource-identity.mjs";
...
const domainResource = DOMAIN_SERVICES.includes(service)
	...
	: null;
if (
	((service === "api" || service === "billing") && (!resources.d1.id || !resources.kv.id)) ||
	(service === "site" &&
		(!resources.siteD1.id || !resources.siteSessionKv.id || !resources.siteReleaseKv.id)) ||
	(service === "email" && !resources.emailD1.id) ||
	(service === "identity" && !resources.identityD1.id) ||
	(service === "files" && !resources.filesD1.id) ||
	(service === "custom" && target.customWorker?.d1Binding && !resources.customD1?.id) ||
...
const outputDirectory = resolve(root, "infra", "generated");
...
function baseConfig() {
	...
	return {
		...
		compatibility_flags:
			service === "site" ? ["nodejs_compat"] : ["nodejs_compat", "global_fetch_strictly_public"],
		workers_dev: false,
		preview_urls: false,
		observability: {
			enabled: true,
		...
		...(requiredSecrets.length ? { secrets: { required: requiredSecrets } } : {}),
		...(service === "observability"
			? {}
			: {
					tail_consumers: [{ service: target.workers.observability[environment] }],
				}),
	};
}
...
function siteConfig() {
	return {
		...
		r2_buckets: [{ binding: "MEDIA", bucket_name: resources.siteMedia.name }],
		kv_namespaces: [
...
function apiConfig() {
	...
	const config = {
		...
		vars: {
			...
			...d1SchemaVars(),
			SUPERBOARD_RELEASE:
				superboardEnvironmentValue("SUPERBOARD_RELEASE") || process.env.GITHUB_SHA || "local",
		...
		services: [
			{ binding: "SITE_SERVICE", service: target.workers.site[environment] },
			{ binding: "MCP_SERVICE", service: target.workers.mcp[environment] },
			...(target.features.billing
			...
			...(target.customWorker
				...
				: []),
			...DOMAIN_SERVICES.filter((domainService) => target.features[domainService]).map(
				(domainService) => ({
	...
	if (domainQueues.length) {
		...
		if (!preflight) {
			config.queues.consumers = [
				...
				...domainQueues.map(({ resource }) => queueConsumer(resource.dlq, null, 10, 5, 100)),
			];
		}
	}
	if (!preflight && definition.crons.length) config.triggers = { crons: [...definition.crons] };
...
function billingConfig() {
	...
	const config = {
		...
		services: [
			{ ...taskApiServiceBinding("billing"), service: target.workers.api[environment] },
			...(target.features.analytics
				? [{ binding: "ANALYTICS_MODULE", service: target.workers.analytics[environment] }]
				: []),
		],
	};
	if (!preflight) {
		config.queues.consumers = [queueConsumer(resources.queues.billingDlq, null, 10, 5, 8)];
...
function emailConfig() {
	...
	if (!preflight) {
		config.queues.consumers = [
			queueConsumer(resources.queues.email, resources.queues.emailDlq, 10, 5, 8),
...
function identityConfig() {
	return {
		...
		vars: {
			...
			SUPERBOARD_IDENTITY_ISSUER: target.authGateway.issuer,
			SUPERBOARD_IDENTITY_AUDIENCE: target.authGateway.audience,
			SUPERBOARD_IDENTITY_TOKEN_TTL: "300",
			ACCESS_TOKEN_TTL: "900",
			REFRESH_TOKEN_TTL: "2592000",
			GOOGLE_AUDIENCES_JSON: JSON.stringify(target.applicationIdentity.googleAudiences),
			APPLE_AUDIENCES_JSON: JSON.stringify(target.applicationIdentity.appleAudiences),
			PUBLIC_API_URL: publicApiUrl(target),
			...
			EMBEDDED_AUTH_ORIGINS: [publicDashboardUrl(target), ...target.applicationIdentity.webOrigins],
...
function filesConfig() {
	return {
		...
		vars: {
			...
			DOWNLOAD_TICKET_TTL_SECONDS: String(target.filePolicy.downloadTicketTtlSeconds),
			ALLOWED_FILE_CONTENT_TYPES_JSON: JSON.stringify(target.filePolicy.allowedContentTypes),
		},
		d1_databases: [
			{
				binding: "DB",
...
function customConfig() {
	if (!target.customWorker) throw new Error(`${targetName} does not declare a custom Worker`);
	const config = {
		...baseConfig(),
		workers_dev: false,
	...
	if (target.customWorker.serviceBindings?.length) {
		config.services = target.customWorker.serviceBindings.map(({ binding, workers }) => ({
			binding,
...
function managedWorkerConfig() {
	...
	if (!accountId && !allowUnprovisioned) {
		throw new Error(`CLOUDFLARE_ACCOUNT_ID is required to derive the R2 endpoint for ${service}`);
	}
	const commonVars = {
		SUPERBOARD_TARGET: targetName,
		ENVIRONMENT: environment,
	...
	return {
		...
		migrations: managedWorker.migrations.map((migration) => ({
			tag: migration.tag,
			...(migration.newClasses?.length ? { new_classes: migration.newClasses } : {}),
...
function publicSurfaceMonitors(selectedTarget) {
	...
	for (const surface of monitored) {
		if (ids.has(surface.id)) {
			throw new Error(`Public surface monitor ${surface.id} is declared more than once`);
		}
		ids.add(surface.id);
	}
	return monitored;
...
function platformWorkerTopology(selectedTarget) {
	...
	return {
		...
		workers: [
			...
			...managedWorkerDefinitions(selectedTarget).map((component) => ({
				...
				managed: {
					...
					containers: component.containers.map(({ className, instanceType }) => ({
						className,
						instanceType,
					})),
					durableObjects: component.durableObjects.map(({ className, storage }) => ({
						className,
						storage,
					})),
		...
		customDependencies: (selectedTarget.customWorker?.serviceBindings ?? []).map(
			({ binding, workers }) => ({
				binding,
...
function resourceId(resource, kind) {
	...
	if (!allowUnprovisioned)
		throw new Error(`Missing provisioned ${kind} id for ${resource?.name || "resource"}`);
