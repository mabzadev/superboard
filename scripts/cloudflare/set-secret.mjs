...
import { environmentFromArgs, loadTarget, parseArgs, targetNameFromArgs } from "./target.mjs";
...
export function buildLegacySecretReplacement({
	...
	const contract = secretCoordinationPlan(target, environment).contracts.find(({ members }) =>
		members.some(
			(member) =>
	...
	return {
		...
		valuesRead: false,
		reason: "A single Worker secret update can immediately deploy a partial or unpaired value.",
		target: targetName,
		environment,
...
function assertDeclaredSecret(target, service, secretName) {
	...
	if (managed && !managed.secrets.includes(secretName)) {
		throw new Error(
			`${secretName} is not declared for ${service}; expected one of ` + managed.secrets.join(", "),
		);
	}
}
