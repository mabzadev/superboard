...
export const FLOWS_FORBIDDEN_VERSION_SECRETS = Object.freeze(["EMAIL_INTERNAL_TOKEN"]);
...
export function buildFlowsVersionBundlePlan({
	...
	targetName,
	environment,
	outputPath = resolve(PROTECTED_DIRECTORY, `${targetName}-flows-${environment}.json`),
	...
	const config = resolve(root, "infra", "generated", `${targetName}-flows-${environment}.jsonc`);
...
export function flowsObsoleteSecretBulkArgs({ config, worker, versionTag, obsoleteSecretsFile }) {
	if (!/^[a-z0-9][a-z0-9._-]{2,63}$/u.test(String(versionTag ?? ""))) {
...
export function buildFlowsVersionSecrets({
	...
	const hashKey = prepared?.FLOW_USER_HASH_KEY ?? source.FLOW_USER_HASH_KEY ?? generateHashKey();
...
export async function prepareFlowsObsoleteSecretsFile({ outputPath }) {
	...
	let created = false;
	try {
		await assertProtectedRegularFile(targetPath, "obsolete Flows secret cleanup");
...
function assertFlowsVersionScope({ target, targetName, environment }) {
	if (
		...
		throw new Error("Flows version bundle is restricted to enabled MBZA development");
	}
}
...
function protectedOutputPath(value) {
	...
	if (
		...
		throw new Error(`Flows version secrets must be a JSON file below ${PROTECTED_DIRECTORY}`);
	}
	return output;
}
...
async function main() {
	...
	const outputPath =
		args.output ?? resolve(PROTECTED_DIRECTORY, `${targetName}-flows-${environment}.json`);
