...
import { verifyFlutterFlowClientReceipt } from "../clients/flutterflow-client-release.mjs";
...
export function assertPublicRoutingReady(
	target,
	environment,
	{ repositoryRoot = root, verifyReceipt = verifyFlutterFlowClientReceipt } = {},
) {
	...
	if (routing !== "active") {
		throw new Error(`Target ${target.target}/${environment} has no valid publicRouting mode`);
	}
	...
	const manifestPath = repositoryPath(
		repositoryRoot,
		cutover.snapshot,
		"scripts/config/flutterflow-sources",
	);
	const receiptPath = repositoryPath(
		repositoryRoot,
		cutover.clientReceipt,
		"scripts/config/flutterflow-releases",
	);
