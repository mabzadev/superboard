...
import { loadTarget, parseArgs, targetNameFromArgs } from "./target.mjs";
...
function secretNamesForService(target, service) {
	if (service === "custom") {
		return [...new Set([...target.customWorker.secrets, "CUSTOM_WORKER_TOKEN_PREVIOUS"])];
	}
...
const BILLING_KEY_ALTERNATIVE = Object.freeze({
	oneOf: Object.freeze(["STORE_CREDENTIALS_ENCRYPTION_KEYS", "STORE_CREDENTIALS_ENCRYPTION_KEY"]),
});
...
export function requiredSecretInventory(target, environment) {
	...
	for (const service of DOMAIN_SERVICES) {
		if (target.features?.[service]) {
			add(
				service,
				DOMAIN_SERVICE_REGISTRY[service].secrets.filter((name) => !isOptionalSecretBinding(name)),
			);
		}
	}
...
export function secretCoordinationPlan(target, environment) {
	...
	const alternativeMember = (service, oneOf) => {
		...
		const found = requirement?.alternatives.find(
			(alternative) => [...alternative.oneOf].sort().join("\u0000") === expected,
		);
	...
	addContract({
		...
		members: [
			...
			exactMember("billing", "INTERNAL_API_TOKEN", "INTERNAL_API_TOKEN_PREVIOUS"),
			exactMember("identity", "INTERNAL_API_TOKEN", "INTERNAL_API_TOKEN_PREVIOUS"),
			...DOMAIN_SERVICES.filter((service) => service !== "flows").map((service) =>
				exactMember(service, "INTERNAL_API_TOKEN", "INTERNAL_API_TOKEN_PREVIOUS"),
			),
		],
	});
	addContract({
	...
	addContract({
		...
		sameValueRequired: true,
		rotation: "add-dedicated-api-token-after-flows-accepts-the-new-active-and-old-previous-values",
		members: [
			exactMember("api", "FLOWS_INTERNAL_TOKEN"),
			exactMember("flows", "INTERNAL_API_TOKEN", "INTERNAL_API_TOKEN_PREVIOUS"),
		],
	});
	addContract({
		...
		members: [
			...
			exactMember("email", "EMAIL_INTERNAL_TOKEN", "EMAIL_INTERNAL_TOKEN_PREVIOUS"),
	...
	addContract({
		...
		members: [
			exactMember("analytics", "ANALYTICS_ID_HASH_KEY", "ANALYTICS_ID_HASH_KEY_PREVIOUS"),
			exactMember("marketing", "ANALYTICS_ID_HASH_KEY", "ANALYTICS_ID_HASH_KEY_PREVIOUS"),
		],
	});
	addContract({
		...
		rotation: "deploy-overlap-consumers-before-producers-then-retire-previous",
		members: [
			exactMember("api", "FILES_INTERNAL_TOKEN"),
			exactMember("identity", "FILES_INTERNAL_TOKEN"),
			exactMember("files", "FILES_INTERNAL_TOKEN", "FILES_INTERNAL_TOKEN_PREVIOUS"),
	...
	addContract({
		...
		members: [
			...
			exactMember("custom", "CUSTOM_WORKER_TOKEN", "CUSTOM_WORKER_TOKEN_PREVIOUS"),
		],
	});
	addContract({
	...
	addContract({
		...
		members: [
			exactMember("api", "SUPERBOARD_ENTITLEMENT_WEBHOOK_SECRET"),
			exactMember("billing", "SUPERBOARD_ENTITLEMENT_WEBHOOK_SECRET"),
		],
	});
	...
	for (const requirement of requirements) {
		for (const name of requirement.names) {
			...
			const applicationSpecific =
				requirement.service === "custom" || requirement.service.startsWith("managed-");
			addContract({
				...
				rotation: metadata.rotation ?? "replace-through-an-inactive-worker-version",
				members: [member],
			});
		}
	...
	const requiredMemberCount = requirements.reduce(
		(total, requirement) => total + requirement.names.length + requirement.alternatives.length,
		0,
	);
	...
	return {
		...
		summary: {
			...
			sharedValueContracts: contracts.filter((contract) => contract.sameValueRequired).length,
...
function environmentFromTarget(target) {
	...
	if (names.length !== 1)
		throw new Error(`${target.target} must declare exactly one deployment environment`);
	return names[0];
}
