...
import { environmentFromArgs, loadTarget, parseArgs, validateTarget } from "./target.mjs";
...
export function applicationConfigurationConfirmation(plan) {
	const digest = createHash("sha256")
		...
		.digest("hex")
		.slice(0, 12);
	return "TARGET:CONFIGURE-APPLICATION:" + plan.target + ":" + plan.environment + ":" + digest;
}
...
export function applicationConfigurationUpdates(args) {
	...
	for (const [field, option] of Object.entries(FIELD_OPTIONS)) {
		...
		if (hasValue && clear) {
			throw new Error(
				"--" + option.argument + " and --" + option.clear + " are mutually exclusive",
			);
		}
...
export async function buildApplicationConfigurationPlan({
	...
	const current = {
		...
		supportProjectIds: [...target.environments[environment].supportProjectIds].sort(
	...
	const changes = Object.keys(desired)
		.filter((field) => JSON.stringify(current[field]) !== JSON.stringify(desired[field]))
		.map((field) => ({
			field,
...
export async function applyApplicationConfiguration({ path, target, plan, confirm }) {
	if (confirm !== plan.confirmation) {
		throw new Error("Refusing target mutation: pass --confirm " + plan.confirmation);
	}
	if (!plan.changed) {
	...
	updated.environments[plan.environment].supportProjectIds = plan.desired.supportProjectIds;
