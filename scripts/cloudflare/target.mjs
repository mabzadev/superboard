...
import { validateCustomWorkerBindings } from "./target-options.mjs";
...
export async function validateTarget(target) {
	const schemaPath = resolve(root, "infra", "targets", "schema.json");
	...
	if (apiDomain !== zoneName && !apiDomain.endsWith(`.${zoneName}`)) {
		throw new Error(`Invalid target manifest: domains.api must belong to zoneName ${zoneName}`);
	}
	...
	for (const retired of target.retiredDomains ?? []) {
		...
		if (activeDomains.has(hostname)) {
			throw new Error(`Invalid target manifest: retired domain ${hostname} is still active`);
		}
	...
	for (const environment of environments) {
		...
		for (const [service, definition] of Object.entries(DOMAIN_SERVICE_REGISTRY)) {
	...
	if (target.features?.flows && !target.features?.products) {
		throw new Error("Invalid target manifest: Products must be enabled when Flows is enabled");
	}
	...
	for (const environment of environments) {
		...
		const enabledModuleQueueKeys = new Set(
			Object.entries(DOMAIN_SERVICE_REGISTRY)
				...
				.flatMap(([, definition]) => definition.queues.map((queue) => queue.resourceKey)),
		);
		const queueNames = [
			...
			...(target.features.billing ? [resources.queues.billing, resources.queues.billingDlq] : []),
		...
		if (target.features?.support) {
			...
			if (routing.pattern !== expectedPattern || routing.worker !== expectedWorker) {
				throw new Error(
					`Invalid target manifest: environments.${environment}.supportRouting must route ${expectedPattern} to ${expectedWorker}`,
				);
			}
			if (routing.mode === "active" && resources.publicRouting !== "active") {
				throw new Error(
	...
	if (target.customWorker) {
		...
		if (runtimeBridge && runtimeBridge.filesInputOrigin !== `https://${target.domains.files}`) {
			throw new Error(
				"Invalid target manifest: runtimeBridge.filesInputOrigin must match the Files public target origin",
			);
		}
		for (const environment of environments) {
			if (target.customWorker.d1Binding && !target.environments[environment].customD1) {
				throw new Error(
			...
			const declaredServiceNames = new Set(
				(target.customWorker.serviceBindings ?? []).map((binding) => binding.workers[environment]),
			);
...
export function environmentFromArgs(args, env = process.env) {
	const environment =
		args.environment ?? superboardEnvironmentValue("SUPERBOARD_ENVIRONMENT", env) ?? "development";
...
export function targetNameFromArgs(args, env = process.env) {
	...
	if (!targetName) {
		throw new Error("--target or SUPERBOARD_TARGET (fallback SUPERBOARD_TARGET) is required");
	}
	return targetName;
}
...
export async function targetSelectionFromArgs(
	...
	if (args.reference) {
		if (!allowReference)
			throw new Error("--reference is valid only for local/CI validation commands");
		if (
			args.target ||
			args.environment ||
			...
			throw new Error("--reference cannot be combined with an operational target or environment");
		}
		...
		if (!targetName) throw new Error("superboard.project.json does not define development.target");
...
export function cloudflareAccountId(target, env = process.env, options = {}) {
	...
	const accountId = String(env[scopedName] || env.CLOUDFLARE_ACCOUNT_ID || "").trim();
...
export function publicDashboardUrl(target) {
	return `https://${target.domains.dashboard ?? target.domains.site}`;
}
