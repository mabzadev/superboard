...
import { buildSecretBundlePlan } from "./secret-bundle.mjs";
import { promotionArgs } from "./secret-promote.mjs";
...
export function buildSecretRetirementPlan({
	...
	for (const worker of receipt.workers) {
		if (
			...
			worker.worker !== workerNameForService(target, worker.service, environment) ||
	...
	for (const entry of previousByService.values()) {
		if (entry.names.length !== 1) {
			throw new Error(`${entry.service} must retire exactly one previous binding per operation`);
		}
		entry.names.sort();
	}
	const eligibleAt = new Date(promotedAt.getTime() + minimumOverlapMinutes * 60_000);
	const blockers =
		nowDate < eligibleAt
			? [
					{
...
export function attachRetirementRemoteState(plan, remoteState, accountId) {
	...
	for (const promoted of plan.promotedWorkers) {
		...
		if (
			...
			throw new Error(`${promoted.worker} no longer runs the exact promoted overlap version`);
		}
...
export function retirementVersionTag(bundleConfirmation, service, name) {
	...
	return `superboard-retire-${digest}-${service}-${nameDigest}`;
}
...
async function main() {
	...
	const retirementServices = new Set(structuralPlan.services.map(({ service }) => service));
	...
	if (args.confirm !== plan.confirmation) {
		throw new Error(`Refusing previous-secret retirement: pass --confirm ${plan.confirmation}`);
	}
	...
	try {
				...
				if (!versionId) {
					run(
						...
						retirementDeleteArgs(
							service,
							entry,
							`SuperBoard previous-secret retirement ${plan.bundleConfirmation}`,
						),
						childEnv,
					...
					if (!tagged || !/^[0-9a-f-]{36}$/iu.test(tagged.id ?? "")) {
						throw new Error(`${service.worker} did not create ${entry.versionTag}`);
					}
					versionId = tagged.id;
				}
				run(
					...
					promotionArgs(
						service,
						versionId,
						`SuperBoard activate previous-secret retirement ${plan.bundleConfirmation}`,
					),
					childEnv,
			...
			try {
				run(
					...
					promotionArgs(
						...
						`SuperBoard automatic retirement rollback ${plan.bundleConfirmation}`,
					),
					childEnv,
		...
		throw new Error("Previous-secret retirement failed; every changed Worker was rolled back", {
			cause: error,
		});
	}
