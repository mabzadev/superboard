import { D1_SCHEMA_OWNERS } from "../database/d1-registry.mjs";
import {
	ALL_SERVICES,
	DOMAIN_SERVICES,
	isServiceEnabled,
...
export function runtimeBridgeDeploymentBlockers({
	...
	const bridgeSelected =
		selected.has("custom") || [...managed].some((service) => selected.has(service));
	const activeApiCutover =
		selected.has("api") && target.environments[environment].publicRouting === "active";
...
export function buildDeploymentExecutionPlan({
	...
	for (const service of requested) {
		if (
			(!ALL_SERVICES.includes(service) && !managedWorkerDefinition(target, service)) ||
	...
	if (environment === "production" && skipMigrations) {
		throw new Error("Production deployment cannot skip migrations for the selected topology");
	}
	const services = order.filter((service) => requested.includes(service));
	const schemaServices = services.filter((service) => D1_SCHEMA_OWNERS.includes(service));
	...
	const identityCutover = services.includes("identity") && !uploadOnly && !preflight;
	if (
		environment === "production" &&
		!uploadOnly &&
		!preflight &&
	...
	return {
		...
		phases: batchBeforeWorkers
			? [
					{
						id: "d1-batch",
						description: "Back up every D1 database, then migrate and verify every D1 database",
					...
					{
						id: "workers",
						description: "Deploy enabled Workers only after the complete D1 batch receipt exists",
						services,
					},
				]
			: [
