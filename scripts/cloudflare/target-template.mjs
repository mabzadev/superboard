import { ALL_SERVICES, DOMAIN_SERVICES, DOMAIN_SERVICE_REGISTRY } from "./services.mjs";
import { customTargetOptions } from "./target-options.mjs";
...
export function newTargetManifest({ args, target, selectedEnvironment }) {
	...
	if (args["enable-legacy-messaging"])
	...
	const features = Object.fromEntries(
		featureServices.map((service) => [
			service,
			service === "messaging" ? false : !disabledFeatures.has(service),
		]),
	);
	...
	return {
		...
		applicationIdentity: {
			...
			applicationAudience: args["application-audience"] || `${target}.application`,
		...
		filePolicy: {
			...
			downloadTicketTtlSeconds: Number(args["download-ticket-ttl-seconds"] ?? 600),
			allowedContentTypes: csv(args["allowed-file-content-types"]),
		},
		mail: {
			transport:
				args["mail-transport"] ?? (selectedEnvironment === "development" ? "capture" : "smtp"),
			provider: args["mail-provider"] ?? "aws-ses",
			awsRegion: args["aws-region"] ?? "eu-central-1",
			configurationSet: args["mail-configuration-set"] ?? `superboard-${selectedEnvironment}`,
			fromName: "SuperBoard",
		...
		operator: {
			...
			...(args["operator-support-email"] ? { supportEmail: args["operator-support-email"] } : {}),
		},
		domains: {
			api: args["api-domain"],
			...
			mcp: args["mcp-domain"],
			...(args["mail-preview-domain"] ? { mailPreview: args["mail-preview-domain"] } : {}),
		},
		retiredDomains: [],
