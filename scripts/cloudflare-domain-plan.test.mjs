...
test("the domain plan assigns every public surface to one explicit Worker", () => {
	...
	assert.deepEqual(
		expected.filter((entry) => entry.service === "opengrow-api-dev").map((entry) => entry.surface),
...
test("available and already managed domains are non-blocking", () => {
	...
	const result = evaluateDomainPlan({
		...
		workerDomains: [{ hostname: "api.example.test", service: "opengrow-api-dev" }],
...
test("MCP on the console path keeps one Site owner and six canonical hostnames", () => {
	...
	assert.deepEqual(
		...
		[{ surface: "site", hostname: "board.example.test", service: "opengrow-site-dev" }],
	);
});
