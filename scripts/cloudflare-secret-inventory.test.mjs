...
test("secret inventory includes enabled common and target-specific contracts only", async () => {
  ...
  assert.equal(inventory.some(({ service }) => service === "messaging"), false);
  assert.ok(inventory.find(({ service }) => service === "api")?.names.includes("JWT_SECRET"));
  assert.deepEqual(inventory.find(({ service }) => service === "custom")?.names, [
    "CUSTOM_WORKER_TOKEN",
    "CUSTOM_WORKER_TOKEN_PREVIOUS",
  ]);
  assert.ok(inventory.find(({ service }) => service === "support")?.names.includes("SUPPORT_WEBHOOK_ENCRYPTION_KEY"));
});
...
test("shared production contracts identify both ends and environment-specific billing ownership", async () => {
  ...
  assert.deepEqual(
    productionPlan.contracts.find(({ id }) => id === "email-internal-token")
  ...
  assert.equal(
    productionPlan.contracts
      ...
      .every(({ previousName }) => previousName === "INTERNAL_API_TOKEN_PREVIOUS"),
    true,
  );
  assert.deepEqual(
    productionPlan.contracts.find(({ id }) => id === "custom-worker-internal-token")
      .members.map(({ service }) => service),
    ["api", "custom"],
  );
  assert.deepEqual(
    productionPlan.contracts.find(({ id }) => id === "entitlement-webhook-secret")
      .members.map(({ service }) => service),
    ["billing"],
  );
  assert.deepEqual(
    developmentPlan.contracts.find(({ id }) => id === "entitlement-webhook-secret")
      .members.map(({ service }) => service),
    ["api", "billing"],
  );
});
...
test("SMTP and target extension secrets have explicit provenance", async () => {
  ...
  assert.equal(
    plan.contracts.find(({ id }) => id === "custom-worker-internal-token").scope,
