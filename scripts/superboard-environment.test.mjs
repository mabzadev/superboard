...
test("canonical SuperBoard variables take precedence over legacy fallbacks", () => {
	...
	assert.equal(superboardEnvironmentValue("SUPERBOARD_TARGET", env), "reference-production");
	assert.equal(superboardEnvironmentValue("SUPERBOARD_ENVIRONMENT", env), "production");
	assert.equal(superboardEnvironmentValue("SUPERBOARD_RELEASE", env), "new-release");
	assert.equal(
...
test("legacy OpenGrow variables remain read-only fallbacks", () => {
	...
	assert.equal(superboardEnvironmentValue("SUPERBOARD_RELEASE", env), "legacy-release");
	assert.equal(
		superboardEnvironmentValue("SUPERBOARD_REFERENCE_REPOSITORY", env),
	...
	assert.equal(superboardEnvironmentContract(env).SUPERBOARD_TARGET.source, "OPENGROW_TARGET");
});
