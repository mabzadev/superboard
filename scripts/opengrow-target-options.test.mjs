...
} from "./opengrow-target-options.mjs";
...
test("custom target options cover D1, vars, secrets, crons and services", () => {
  ...
  assert.deepEqual(result, {
    worker: {
      ...
      description: "new-app app-specific OpenGrow extension",
...
test("the new-application template produces a complete valid custom target", async () => {
  const manifest = newTargetManifest({
    args: {
      ...
      "operator-docs-url": "https://github.com/example/opengrow-platform/docs",
  ...
  assert.equal(
    ...
    "opengrow-sample-development-dev-custom-db",
  );
...
test("target registration keeps provisioning separate from deployment", () => {
  const source = readFileSync(
    new URL("./opengrow-register-target.mjs", import.meta.url),
    "utf8",
  );
