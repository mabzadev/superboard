...
test("validation-only local D1 resources receive stable distinct ids", () => {
	...
	const ids = services.map((service) => {
		...
		const config = JSON.parse(
			readFileSync(
				new URL(`../deploy/generated/mbza-development-${service}-local.jsonc`, import.meta.url),
				"utf8",
			),
		);
	...
	const repeated = JSON.parse(
		readFileSync(
			new URL("../deploy/generated/mbza-development-dynamic-links-local.jsonc", import.meta.url),
			"utf8",
		),
...
test("generated Site config uses target resources and follows public routing activation", async () => {
	for (const [targetName, environment] of [
		...
		const config = JSON.parse(
			readFileSync(
				new URL(`../deploy/generated/${targetName}-site-${environment}.jsonc`, import.meta.url),
				"utf8",
			),
		);
		...
		const apiConfig = JSON.parse(
			readFileSync(
				new URL(`../deploy/generated/${targetName}-api-${environment}.jsonc`, import.meta.url),
				"utf8",
			),
		);
...
test("development Site preview routing is explicit and only acquires the canonical console", () => {
	...
	const config = JSON.parse(
		readFileSync(
			new URL("../deploy/generated/mbza-development-site-development.jsonc", import.meta.url),
			"utf8",
		),
	);
...
test("generated Analytics config declares its complete durable pipeline", () => {
	...
	const config = JSON.parse(
		readFileSync(
			new URL("../deploy/generated/mbza-development-analytics-development.jsonc", import.meta.url),
			"utf8",
		),
	);
...
test("generated Flows config declares its native Cloudflare runtime", () => {
	...
	const config = JSON.parse(
		readFileSync(
			new URL("../deploy/generated/mbza-development-flows-development.jsonc", import.meta.url),
			"utf8",
		),
	);
...
test("generated Support config includes its stateful runtime resources", () => {
	...
	assert.equal(config.queues.consumers[0].dead_letter_queue, "opengrow-support-v2-events-dlq");
	...
	assert.equal(config.vars.SUPPORT_EVENTS_QUEUE_NAME, "opengrow-support-v2-events");
	assert.equal(config.vars.SUPPORT_EVENTS_DLQ_NAME, "opengrow-support-v2-events-dlq");
	assert.equal(config.vars.SUPPORT_AI_QUEUE_NAME, "opengrow-support-v2-ai");
	assert.equal(config.vars.SUPPORT_AI_DLQ_NAME, "opengrow-support-v2-ai-dlq");
	assert.equal(config.vars.SUPPORT_BULK_QUEUE_NAME, "opengrow-support-v2-bulk");
	assert.equal(config.vars.SUPPORT_BULK_DLQ_NAME, "opengrow-support-v2-bulk-dlq");
...
test("generated Files config enforces the selected target upload policy", () => {
	...
	const development = JSON.parse(
		readFileSync(
			new URL("../deploy/generated/mbza-development-files-development.jsonc", import.meta.url),
			"utf8",
		),
	);
...
test("staged production API stays private while exposing service bindings", async () => {
	...
	assert.deepEqual(config.tail_consumers, [{ service: "opengrow-observability" }]);
	assert.deepEqual(
	...
	const mbza = JSON.parse(
		readFileSync(
			new URL("../deploy/generated/mbza-development-api-development.jsonc", import.meta.url),
			"utf8",
		),
	);
...
test("generated MCP config is public only on its target domain and uses a private API binding", () => {
	...
	const config = JSON.parse(
		readFileSync(
			new URL("../deploy/generated/mbza-development-mcp-development.jsonc", import.meta.url),
			"utf8",
		),
	);
...
test("generated identity and files configs are private and parameterized", () => {
	...
	const identity = JSON.parse(
		readFileSync(
			new URL("../deploy/generated/mbza-development-identity-development.jsonc", import.meta.url),
			"utf8",
		),
	);
...
test("generated reference custom config owns its durable D1 job store", () => {
	const target = JSON.parse(
		readFileSync(new URL("../deploy/targets/mbza-development.json", import.meta.url), "utf8"),
	);
	execFileSync(
		process.execPath,
		[
	...
	const config = JSON.parse(
		readFileSync(
			new URL("../deploy/generated/mbza-development-custom-development.jsonc", import.meta.url),
			"utf8",
		),
	);
	assert.equal(config.workers_dev, false);
