...
export const npmAdvisoryBulkUrl = "https://registry.npmjs.org/-/npm/v1/security/advisories/bulk";

export const imageSizePatchContract = Object.freeze({
	packageName: "image-size",
	sourceVersion: "1.2.1",
	resolutionKey: "image-size@^1.0.2",
	resolution: "patch:image-size@npm%3A1.2.1#./.yarn/patches/image-size-npm-1.2.1-e285f3c080.patch",
	lockHash: "80d774",
	patchPath: ".yarn/patches/image-size-npm-1.2.1-e285f3c080.patch",
	patchSha256: "21f4e087748a17f6200705cd102689cfedf349f63f7529cc8f96ac48a1d8ec6f",
	advisories: ["GHSA-w3rx-r6r6-pgpr", "GHSA-5p2g-fcmc-qvqq"],
	workspaceLocator: "@mbzadev/superboard-react-native-sdk@workspace:.",
});
...
const imageSizeAdvisoryAllowlist = imageSizePatchContract.advisories.map((ghsaId) => ({
...
function validateImageSizeLockEntries(lockSource) {
	const imageSizeEntries = yarnLockEntries(lockSource).filter(({ descriptor }) =>
		descriptor
			.split(", ")
			.some((selector) => selector.startsWith(`${imageSizePatchContract.packageName}@`)),
	);
	...
	if (expectedEntries.size !== 0) {
		throw new Error("React Native yarn.lock does not bind the reviewed image-size security patch");
	}
}
	...
	for (const line of whySource.split(/\r?\n/u).filter(Boolean)) {
		...
		for (const [childKey, child] of Object.entries(record.children)) {
			...
			if (
				...
				throw new Error(`Yarn installed an unreviewed image-size locator: ${child.locator}`);
			}
...
export function validateImageSizePatchContract({
	...
	if (
		...
		throw new Error("React Native image-size security patch resolution is missing or changed");
	}
...
export function npmPackageVersionsFromYarnLock(lockSource) {
	...
	if (packages.size === 0) {
		throw new Error("React Native yarn.lock contains no auditable npm packages");
	}
...
export async function auditNpmPackageVersions(
	packages,
	{ fetchImpl = globalThis.fetch, timeoutMs = 30_000, advisoryAllowlist = [] } = {},
) {
	...
	for (const entry of advisoryAllowlist) {
		...
		if (allowedAdvisories.has(key)) {
			throw new Error("React Native npm advisory allowlist contains duplicates");
		}
	...
	try {
		response = await fetchImpl(npmAdvisoryBulkUrl, {
			...
			headers: {
				...
				"user-agent": "superboard-react-native-security-audit/1",
			},
	...
	if (unexpectedAdvisories.length > 0) {
		throw new Error(
			`React Native npm advisory audit found ${unexpectedAdvisories.length} unexpected advisory(s): ${unexpectedAdvisories
				.map(({ packageName, id, severity, url }) => `${packageName}#${id} (${severity}, ${url})`)
				.join(", ")}`,
		);
	}
...
export async function auditReactNativeDependencies({ fetchImpl = globalThis.fetch } = {}) {
	const [packageJsonSource, lockSource, patchSource, whySource] = await Promise.all([
		readFile(resolve(reactNativeRoot, "package.json"), "utf8"),
		readFile(resolve(reactNativeRoot, "yarn.lock"), "utf8"),
		readFile(resolve(reactNativeRoot, imageSizePatchContract.patchPath), "utf8"),
		yarnWhyImageSize(),
	]);
