...
const receiptSchema = JSON.parse(
	readFileSync(
		fileURLToPath(new URL("../config/flutterflow-client-release.schema.json", import.meta.url)),
		"utf8",
	),
);
...
export function createFlutterFlowClientReceipt({
	...
	const receipt = {
		$schema: "../flutterflow-client-release.schema.json",
		schemaVersion: 1,
		status: "accepted",
...
export function verifyFlutterFlowClientReceipt({ manifestPath, receiptPath, sourcePath = null }) {
	const manifest = readJson(manifestPath, "FlutterFlow snapshot manifest");
	...
	const assertions = [
		...
		[receipt.flutterflowCommitId === manifest.export?.commitId, "FlutterFlow commit"],
		...
		[receipt.diagnostics.total === manifest.diagnostics?.total, "diagnostic count"],
		[
			receipt.diagnostics.validationErrors === manifest.diagnostics?.validationErrors,
			"validation diagnostic count",
		],
		[Number.isFinite(Date.parse(receipt.issuedAt)), "issue timestamp"],
	];
	const failed = assertions.filter(([ready]) => !ready).map(([, label]) => label);
	if (failed.length > 0) {
		throw new Error(`FlutterFlow client release receipt mismatch: ${failed.join(", ")}`);
	}
	if (sourcePath) {
		const verification = verifyFlutterFlowSource({ manifestPath, sourcePath });
		if (!verification.ready || verification.convergence?.ready !== true) {
			throw new Error("FlutterFlow client source no longer satisfies convergence");
		}
		const evidence = flutterFlowSourceEvidence({
			sourceRoot: sourcePath,
			policy: manifest.convergence,
		...
		if (canonical(evidence) !== canonical(receipt.sourceEvidence)) {
			throw new Error("FlutterFlow client source evidence does not match the receipt");
		}
	}
	return {
		schemaVersion: 1,
