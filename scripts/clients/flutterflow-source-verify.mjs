...
const snapshotSchema = JSON.parse(
	readFileSync(
		fileURLToPath(new URL("../config/flutterflow-source-snapshot.schema.json", import.meta.url)),
		"utf8",
	),
);
...
export function verifyFlutterFlowSource({ manifestPath, sourcePath }) {
	...
	if (!validateSnapshot(manifest)) {
		const details = (validateSnapshot.errors || [])
			.map(({ instancePath, message: errorMessage }) => `${instancePath || "/"} ${errorMessage}`)
			.join("; ");
	...
	const generatedState = readSourceJson(sourceRoot, ".flutterflow/generated_code_state.json");
	const sdkMeta = readSourceJson(sourceRoot, ".flutterflow/project_sdk_meta.json");
	...
	equal(lastRun.commitMessage, manifest.export?.commitMessage, "commit message");
	...
	equal(sdkMeta.generatedAt, manifest.export?.generatedAt, "SDK generation time");
	equal(
	...
	for (const descriptor of generatedFiles) {
		if (!descriptor || typeof descriptor.path !== "string" || !isSha(descriptor.sha)) {
			throw new Error("FlutterFlow SDK metadata has an invalid file descriptor");
		}
		const path = safeSourcePath(sourceRoot, descriptor.path);
		equal(sha256File(path), descriptor.sha, `${descriptor.path} generated hash`);
	}

	const schemas = readSource(sourceRoot, "lib/flutterflow_project/schemas.dart");
	const appState = readSource(sourceRoot, "lib/flutterflow_project/app_state.dart");
	const apis = readSource(sourceRoot, "lib/flutterflow_project/apis.dart");
	const inventory = {
		pages: Object.keys(sdkMeta.files).filter((key) => key.startsWith("page:")).length,
		components: Object.keys(sdkMeta.files).filter((key) => key.startsWith("component:")).length,
		actionBlocks: stringArray(classBody(apis, "ActionBlocks"), "static const all = <String>[")
			.length,
		appEventsReported: manifest.inventory?.appEventsReported,
		apiCallsReported: manifest.inventory?.apiCallsReported,
		customActions: stringArray(schemas, "static const actions = <String>[").length,
		customFunctions: stringArray(schemas, "static const functions = <String>[").length,
		customWidgets: stringArray(schemas, "static const widgets = <String>[").length,
		dataStructs: countMatches(schemas, /static final ffai\.StructHandle\s+/gu),
		appStateFields: countMatches(appState, /= ffai\.ProjectAppStateFieldHandle\(/gu),
	};
	equal(inventory, manifest.inventory, "FlutterFlow inventory");

	const diagnostics = Array.isArray(lastRun.diagnostics) ? lastRun.diagnostics : [];
	const byCode = {};
	...
	const diagnosticSummary = {
		...
		validationErrors: (lastRun.tasks || []).reduce(
			(total, task) =>
				total + (Array.isArray(task.validationErrors) ? task.validationErrors.length : 0),
			0,
		),
...
export function legacyFlutterFlowSourceEnvironmentName(application) {
	return flutterFlowSourceEnvironmentName(application).replace(/^SUPERBOARD_/u, "SUPERBOARD_");
}

export function resolveFlutterFlowSourcePath({ manifestPath, explicitSource, env = process.env }) {
	if (explicitSource) return explicitSource;
	if (!manifestPath) throw new Error("--manifest is required");
	const manifest = readJson(resolve(manifestPath), "snapshot manifest");
	const application = manifest?.application;
	if (!/^[a-z0-9][a-z0-9-]*$/u.test(application || "")) {
		throw new Error("FlutterFlow snapshot application cannot resolve a source variable");
	}
	const environmentName = flutterFlowSourceEnvironmentName(application);
	const legacyEnvironmentName = legacyFlutterFlowSourceEnvironmentName(application);
	const sourcePath = String(env[environmentName] || env[legacyEnvironmentName] || "").trim();
	if (!sourcePath) {
		throw new Error(
...
export function evaluateFlutterFlowConvergence({ sourceRoot, policy, diagnostics }) {
	...
	const checks = policy.checks.map((rule) => {
		...
		if (
			...
			if (rule.kind === "diagnostics-unwaived-max") {
				...
				if (
					...
					Object.values(allowedByCode).some((value) => !Number.isSafeInteger(value) || value < 0)
				) {
				...
				const waived = Object.values(waivedByCode).reduce((total, count) => total + count, 0);
			...
			const actual =
				rule.kind === "validation-errors-max" ? diagnostics?.validationErrors : diagnostics?.total;
		...
		if (!["literal-absent", "symbol-absent", "symbol-present"].includes(rule.kind)) {
			throw new Error(`Unsupported FlutterFlow convergence check kind: ${rule.kind}`);
		}
...
export function flutterFlowSourceEvidence({ sourceRoot, policy }) {
	...
	const declaredPaths = [
		...new Set(policy.checks.flatMap((rule) => (Array.isArray(rule.paths) ? rule.paths : []))),
	].sort();
	...
	for (const relativePath of declaredPaths) {
		...
		try {
			...
			throw new Error(`FlutterFlow convergence path is missing: ${relativePath}`);
		}
		...
		if (candidates.length === 0) {
			throw new Error(`FlutterFlow convergence path has no source: ${relativePath}`);
		}
	...
	const entries = [...evidence.entries()].sort(([left], [right]) => left.localeCompare(right));
...
function scanConvergencePaths(sourceRoot, paths, needle, { symbol }) {
	...
	for (const relativePath of paths) {
		...
		for (const candidate of candidates) {
			...
			const count = symbol ? countSymbol(source, needle) : countLiteral(source, needle);
			if (count === 0) continue;
			matches += count;
	...
	while (pending.length > 0) {
		...
		for (const entry of readdirSync(directory, { withFileTypes: true })) {
			...
			if (entry.isDirectory()) {
				if ([".dart_tool", ".git", ".flutterflow", "build", "node_modules"].includes(entry.name)) {
					continue;
				}
			...
			if (![".dart", ".json", ".yaml", ".yml"].includes(extname(path))) continue;
			if (lstatSync(path).size > 4 * 1024 * 1024) {
				throw new Error(`FlutterFlow convergence source is too large: ${relative(root, path)}`);
			}
			result.push(path);
			if (result.length > 20_000) {
				throw new Error("FlutterFlow convergence source contains too many files");
			}
		}
	}
	return result.sort();
...
function safeSourcePath(root, relativePath) {
	...
	if (path !== root && !path.startsWith(`${root}${sep}`)) {
		throw new Error(`FlutterFlow path escapes the selected source: ${relativePath}`);
	}
	return path;
}
...
function stringArray(source, marker) {
	...
	if (end < 0) throw new Error(`Generated source has an unterminated ${marker}`);
	return [...source.slice(start + marker.length, end).matchAll(/"([^"]+)"/gu)].map(
		(match) => match[1],
	);
}
