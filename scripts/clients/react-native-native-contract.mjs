...
const contractPath = resolve(root, "sdks/react-native/plugin/native-contract.json");
...
export function nativeContractFromCatalog(catalog) {
	...
	for (const library of [android, ios]) {
		if (!library.latestReleaseVersion || !library.releaseSha) {
			throw new Error("React Native native dependencies require an immutable published baseline");
		}
	}
	const iosReleaseRef = `sdk-ios-v${ios.latestReleaseVersion}`;
	const historicalRepository = ios.install.match(/\.package\(url: "([^"]+)", exact:/u)?.[1];
	if (!historicalRepository) {
		throw new Error("React Native iOS contract requires the historical SwiftPM coordinate");
	}
	const repository = new URL(historicalRepository);
	if (repository.hostname !== "github.com") {
		throw new Error("React Native iOS podspec generation requires a GitHub repository");
	}
	const repositoryPath = repository.pathname.replace(/^\//, "").replace(/\.git$/, "");
...
async function main() {
	...
	process.stdout.write(`${JSON.stringify({ status: "ok", contract: result.expected }, null, 2)}\n`);
}
