...
const migrationSchema = readRepositoryJson("scripts/config/flutterflow-migration-plan.schema.json");
const snapshotSchema = readRepositoryJson("scripts/config/flutterflow-source-snapshot.schema.json");
const ajv = new Ajv2020({ allErrors: true, strict: false });
const validateMigration = ajv.compile(migrationSchema);
const validateSnapshot = ajv.compile(snapshotSchema);

export async function buildFlutterFlowMigrationPlan({ manifestPath, sourcePath = null }) {
	...
	validateSchema(validateSnapshot, snapshot, "Invalid FlutterFlow source snapshot");
...
export function validateFlutterFlowMigrationContract({ plan, snapshot, surface, target }) {
	...
	if (target.target !== plan.target || !target.environments?.[plan.environment]) {
		throw new Error(`Migration target ${plan.target}/${plan.environment} is not declared`);
	}
	const expectedSourceEnvironment = flutterFlowSourceEnvironmentName(plan.application);
	if (plan.sourceEnvironment !== expectedSourceEnvironment) {
		throw new Error(`Migration source environment must be ${expectedSourceEnvironment}`);
	}
	...
	for (const phase of plan.phases) {
		for (const dependency of phase.dependsOn) {
			...
			if (!dependencyPhase) {
				throw new Error(`Migration phase ${phase.id} depends on unknown phase ${dependency}`);
			}
			if (dependencyPhase.order >= phase.order) {
				throw new Error(`Migration phase ${phase.id} must depend only on an earlier phase`);
			}
		}
	}
	const orderedPhases = [...plan.phases].sort((left, right) => left.order - right.order);
	...
	for (const item of plan.workItems) {
		if (!phases.has(item.phase)) {
			throw new Error(`Migration work item ${item.id} uses unknown phase ${item.phase}`);
		}
		for (const check of item.convergenceChecks) {
			if (!snapshotChecks.has(check)) {
				throw new Error(`Migration work item ${item.id} maps unknown convergence check ${check}`);
			}
	...
	const unmappedChecks = [...snapshotChecks.keys()].filter((check) => !mappedChecks.has(check));
	if (unmappedChecks.length > 0) {
		throw new Error(`Unmapped FlutterFlow convergence checks: ${unmappedChecks.join(", ")}`);
	}
	return {
		phases: phases.size,
		workItems: plan.workItems.length,
		convergenceChecks: snapshotChecks.size,
		replacementSymbols: new Set(plan.workItems.flatMap((item) => item.replacementSymbols)).size,
	};
}
...
export function migrationPlanStatus({ plan, contract, verification }) {
	...
	const workItems = plan.workItems.map((item) => {
		const blockedChecks = verification
			? item.convergenceChecks.filter((check) => checks.get(check)?.ready !== true)
			: null;
		return {
			id: item.id,
			phase: item.phase,
	...
	return {
		schemaVersion: 1,
		mode: verification ? "source-inspected-migration-plan" : "contract-only-migration-plan",
		application: plan.application,
		target: plan.target,
		environment: plan.environment,
		sourceEnvironment: plan.sourceEnvironment,
		sourceEnvironmentAliases: [legacyFlutterFlowSourceEnvironmentName(plan.application)],
		contractReady: true,
...
function publicSurfaceSymbols(surface) {
	...
	if (collections.some((collection) => !Array.isArray(collection))) {
		throw new Error("FlutterFlow public surface contains an invalid collection");
	}
...
async function main() {
	...
	const legacySourceEnvironment = legacyFlutterFlowSourceEnvironmentName(rawPlan.application);
	const sourcePath = String(
		args.source || process.env[sourceEnvironment] || process.env[legacySourceEnvironment] || "",
	).trim();
