...
const registryPath = resolve(root, "scripts/config/flutterflow-applications.json");
const libraryPath = resolve(root, "scripts/config/flutterflow-library.json");
const controlPlanePath = resolve(root, "scripts/config/github-control-plane.json");
const outputPath = resolve(root, "scripts/config/flutterflow-application-bindings.json");
const registrySchemaPath = resolve(root, "scripts/config/flutterflow-applications.schema.json");
const outputSchemaPath = resolve(
	root,
	"scripts/config/flutterflow-application-bindings.schema.json",
);
...
export async function resolveFlutterFlowApplications({
	...
	const [resolvedRegistry, resolvedLibrary, resolvedControlPlane] = await Promise.all([
		registry ?? readJson(registryPath),
		library ?? readJson(libraryPath),
		controlPlane ?? readJson(controlPlanePath),
	]);
	await validateSchema(resolvedRegistry, registrySchemaPath, "FlutterFlow application registry");
	...
	const output = {
		$schema: "../config/flutterflow-application-bindings.schema.json",
		schemaVersion: 2,
		owner: "superboard",
		generatedFrom: "scripts/config/flutterflow-applications.json",
		applications,
	};
	await validateSchema(output, outputSchemaPath, "resolved FlutterFlow application bindings");
	return output;
}
...
export async function resolveFlutterFlowApplication({
	...
	if (
		...
		throw new Error(`${application.id} production cutover belongs to another app`);
	}
	if (!target.features?.support) {
		throw new Error(`${application.id} requires the common Support feature`);
	}
	if (!environment.supportProjectIds?.includes(application.client.supportProjectId)) {
		throw new Error(
			`${application.id} Support project ${application.client.supportProjectId} is not allowlisted`,
		);
	}
	if (
		!target.applicationIdentity?.appleAudiences?.includes(application.client.applicationIdentifier)
	) {
		throw new Error(`${application.id} identifier is not an application Identity audience`);
	}
	...
	for (const variable of [
		...
		if (!String(githubEnvironment.variables?.[variable] || "").trim()) {
			throw new Error(`${application.id} GitHub variable ${variable} is missing`);
		}
	}
	if (!githubEnvironment.secrets?.includes(application.remoteProject.apiKeySecret)) {
		throw new Error(
			`${application.id} GitHub secret ${application.remoteProject.apiKeySecret} is missing`,
		);
	}
	if (!githubEnvironment.secrets?.includes(application.client.projectKeySecret)) {
		throw new Error(
	...
	if (snapshot.application !== application.id) {
		throw new Error(`${application.id} source snapshot identity does not match`);
	}
	if (
	...
	if (canonical(declaredValues) !== canonical(resolvedValues)) {
		throw new Error(`${application.id} bindings do not cover the exact SuperBoard Library Values`);
	}
...
async function requireRepositoryFile(value) {
	if (typeof value !== "string" || value.startsWith("/") || value.split(/[\\/]/u).includes("..")) {
...
async function main() {
	...
	const filtered = applicationId
		? {
				...output,
				applications: output.applications.filter(({ id }) => id === applicationId),
			}
		: output;
	...
	if (process.argv.includes("--check")) {
		...
		if (current !== serialized) {
			throw new Error(
				"scripts/config/flutterflow-application-bindings.json is stale; run npm run flutterflow-applications:generate",
			);
		}
