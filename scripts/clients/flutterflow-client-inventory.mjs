...
const expectedNames = Object.freeze({
	...
	appEvents: ["getAppDatabase", "getAppOnBoarding", "getAppPaywall", "getUserMedia"],
	customActions: [
		"appCheckInternet",
		"appCheckMaintenance",
		"appCheckUpdate",
...
export function validateFlutterFlowClientInventory({ manifest, snapshot, migrationPlan, surface }) {
	validateDocument(manifest);
	if (manifest.application !== snapshot.application) {
		throw new Error("Client inventory application does not match source snapshot");
	}
	if (
	...
	for (const item of migrationPlan.workItems) {
		for (const check of item.convergenceChecks) checkPhases.set(check, item.phase);
	}
	...
	for (const [kind, groups] of Object.entries(manifest.inventory)) {
		...
		for (const group of groups) {
			...
			for (const name of group.names) {
				if (names.has(name)) throw new Error(`Duplicate ${kind} inventory name: ${name}`);
				names.add(name);
			}
			if ((group.phase === null) !== (group.gate === null)) {
				throw new Error(`${kind}/${group.names[0]} must declare phase and gate together`);
			}
			if (group.phase !== null && !phases.has(group.phase)) {
				throw new Error(`${kind}/${group.names[0]} uses unknown phase ${group.phase}`);
			}
			if (group.gate !== null && !checks.has(group.gate)) {
				throw new Error(`${kind}/${group.names[0]} uses unknown gate ${group.gate}`);
			}
			...
			for (const symbol of group.replacementSymbols) {
				if (!symbols.has(symbol)) {
					throw new Error(`${kind}/${group.names[0]} uses unknown replacement ${symbol}`);
				}
			}
		}
		counts[kind] = names.size;
		if (names.size !== expectedCounts[kind]) {
			throw new Error(`Expected ${expectedCounts[kind]} ${kind}, found ${names.size}`);
		}
		...
		const unknown = [...names].filter((name) => !expectedNames[kind].includes(name));
	...
	for (const required of ["common", "configurable", "application", "remove"]) {
	...
	if (persistedState !== 27) {
		throw new Error(`Expected 27 persisted App State fields, found ${persistedState}`);
	}
	...
	if (
		...
		throw new Error("Historical removed actions must be the two retired billing actions");
	}
	const currentActions = new Set(flattenNames(manifest.inventory.customActions));
	for (const name of historical) {
		if (currentActions.has(name)) throw new Error(`Historical action is still AS-IS: ${name}`);
	}
	return {
...
function validateDocument(manifest) {
	...
	const validate = ajv.compile(
		json(repositoryPath("scripts/config/flutterflow-client-inventory.schema.json")),
	);
