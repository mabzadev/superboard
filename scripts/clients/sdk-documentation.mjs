...
function flutterInstallation(library, label = "Flutter") {
	return [
		...
		"",
		library.lifecycle === "active" ? "## Installation" : "## Historical installation",
		"",
...
function githubRawUrl(repository, ref, path) {
	...
	if (url.protocol !== "https:" || url.hostname !== "github.com") {
		throw new Error("SDK documentation requires the canonical public GitHub repository");
	}
...
function npmRegistryInstallation(library) {
	...
	if (!scope) {
		throw new Error(`SDK catalogue library ${library.id} must use a scoped npm package`);
	}
	...
	return new Map([
		...
		[
			...
			[
				...
				"",
				fenced("javascript", `import SuperBoard from "${javascript.packageName}";`),
			].join("\n"),
		],
		[
			"react-native",
...
export function applySdkDocumentationSections(catalog, documents) {
	...
	for (const contract of sdkDocumentationContracts) {
		...
		if (markerMatches(source, start) !== 1 || markerMatches(source, end) !== 1) {
			errors.push(`${contract.path}: expected exactly one ${contract.id} documentation section`);
			continue;
		}
		const startIndex = source.indexOf(start);
		const endIndex = source.indexOf(end, startIndex + start.length);
		if (endIndex < startIndex) {
			errors.push(`${contract.path}: documentation section markers are reversed`);
			continue;
		}
...
function semanticErrors(catalog, documents) {
	...
	const checks = [
		{
			id: "android",
			pattern: /[a-z][a-z0-9_.-]*:superboard-android(?:-sdk)?(?=[:'"`\s)])/giu,
			expected: published(libraries, "android").packageName,
		},
		{
			id: "javascript",
			pattern: /@[a-z0-9_.-]+\/superboard-js(?:-sdk)?/giu,
			expected: published(libraries, "javascript").packageName,
		},
		{
			id: "react-native",
			pattern: /@[a-z0-9_.-]+\/superboard-react-native(?:-sdk)?/giu,
	...
	for (const { path, id, pattern, expected } of releaseRefChecks) {
		for (const match of (documents.get(path) ?? "").matchAll(pattern)) {
			if (match[0] !== expected) {
				errors.push(`${path}: stale ${id} release reference ${match[0]}; expected ${expected}`);
			}
		}
	}
	...
	for (const [path, source] of documents) {
		if (/\b(?:ghp_|github_pat_)[A-Za-z0-9_]+\b/u.test(source)) {
			errors.push(`${path}: package registry credentials must not be hardcoded`);
		}
		if (/\/:_authToken=(?!\$\{[A-Z][A-Z0-9_]*\})[^\s]+/u.test(source)) {
			errors.push(`${path}: npm authentication must use an environment-variable placeholder`);
		}
	}
	return errors;
}
...
async function main() {
	...
	if (command === "write") {
		await Promise.all(
			[...applied.documents].map(([path, source]) => writeFile(resolve(root, path), source)),
		);
	}
	const checkedDocuments =
		command === "write" ? new Map([...documents, ...applied.documents]) : documents;
