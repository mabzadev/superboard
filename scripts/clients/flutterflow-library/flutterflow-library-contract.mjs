...
const defaultManifestPath = resolve(root, "scripts/config/flutterflow-library.json");
const defaultCatalogPath = resolve(root, "scripts/config/sdk-libraries.json");
const defaultControlPlanePath = resolve(root, "scripts/config/github-control-plane.json");

export async function validateFlutterFlowLibraryContract({
	manifestPath = defaultManifestPath,
	catalogPath = defaultCatalogPath,
	controlPlanePath = defaultControlPlanePath,
	schemaPath = resolve(root, "scripts/config/flutterflow-library.schema.json"),
	sourceOverride = null,
} = {}) {
...
function validateWidgets({ manifest, source, errors }) {
	const declared = unique(
		[...source.matchAll(/app\.customWidget\(\s*['"]([^'"]+)/g)].map((match) => match[1]),
		"DSL custom widget",
		errors,
	);
...
function validatePages({ manifest, source, errors }) {
	const declared = unique(
		[...source.matchAll(/app\.ensurePage\(\s*['"]([^'"]+)/g)].map((match) => match[1]),
		"DSL library page",
		errors,
	);
...
function validateDependencies({ manifest, catalog, source, errors }) {
	const catalogById = new Map((catalog.libraries || []).map((library) => [library.id, library]));
	for (const dependency of manifest.dependencies) {
		...
		const requiredRef = library.candidatePackageName ? candidateRef(library) : library.releaseRef;
		...
		if (sourceVersion !== dependency.sourceVersion) {
			errors.push(`${dependency.catalogId} source version does not match catalogue candidate`);
		}
		if (requiredRef !== dependency.requiredRef) {
			errors.push(`${dependency.catalogId} immutable ref does not match catalogue candidate`);
		}
...
function candidateRef(library) {
	const prefix = library.id === "flutterflow" ? "sdk-flutterflow-v" : `sdk-${library.id}-v`;
	return `${prefix}${library.sourceVersion}`;
}

function validateActions({ manifest, source, errors }) {
	const declared = unique(
		[...source.matchAll(/app\.customAction\(\s*['"]([^'"]+)/g)].map((match) => match[1]),
		"DSL custom action",
		errors,
	);
	const expected = unique(Object.values(manifest.actions).flat(), "manifest custom action", errors);
...
function validateLibraryValues({ manifest, source, errors }) {
	...
	const configured = unique(
		configuredSymbols.map((symbol) => identifiers.get(symbol)?.name || `<${symbol}>`),
		"DSL library value",
		errors,
	);
	...
	for (const value of manifest.libraryValues) {
		const declared = [...identifiers.values()].find(({ name }) => name === value.name);
		if (declared && declared.key !== value.key) {
			errors.push(`library value ${value.name} key ${declared.key} does not match ${value.key}`);
		}
	}
}
...
function validateGitHubEnvironment({ manifest, controlPlane, errors }) {
	const environment =
		controlPlane.repositories?.platform?.environments?.[manifest.remoteProject.githubEnvironment];
	if (!environment) {
		errors.push(`GitHub Environment ${manifest.remoteProject.githubEnvironment} is not declared`);
		return;
	}
	const projectId = environment.variables?.[manifest.remoteProject.projectIdVariable];
	if (!String(projectId || "").trim()) {
		errors.push(`GitHub variable ${manifest.remoteProject.projectIdVariable} is not configured`);
	}
	if (!environment.secrets?.includes(manifest.remoteProject.apiKeySecret)) {
		errors.push(`GitHub secret ${manifest.remoteProject.apiKeySecret} is not declared`);
	}
}
