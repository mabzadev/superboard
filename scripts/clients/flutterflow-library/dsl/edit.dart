...
void migrateLibraryBranding(FFProject project) {
  const widgetNames = {
    'SuperBoardBootstrap': 'SuperBoardBootstrapComponent',
    'SuperBoardRestoreButton': 'SuperBoardRestoreButton',
  ...
  String rename(String name) =>
      widgetNames[name] ??
      name
          .replaceFirst(RegExp(r'^superboard(?=[A-Z])'), 'superboardLibrary')
          .replaceFirst(RegExp(r'^SuperBoard(?=[A-Z])'), 'SuperBoardLibrary');
  String prose(String value) => value.replaceAll('SuperBoard', 'SuperBoard');
  String route(String value) =>
      value.replaceAll('superboard-paywall', 'superboard-library-paywall');
  ...
  String code(String value) {
    ...
    return route(
      prose(value),
    ).replaceAll(RegExp(r'\bsuperboard\b'), 'superboard');
  }
...
''',
  ...
  app.raw((project) {
    ...
    for (final legacyAction in [
      ...
      'superboardPurchaseLogin',
      'superboardIdentify',
      'superboardIdentifyFromLibraryState',
      'superboardInitializeAuthenticatedFromLibraryState',
    ]) {
...
''',
  ...
  app.raw((project) {
    ...
    final legacyImports = RegExp(
      r'''import\s+['"]package:superboard_flutterflow/superboard_flutterflow\.dart['"]\s+as\s+(\w+)\s*;''',
    );
    for (final action in project.customCode.customActions) {
      ...
      for (final match in imports) {
        ...
        code = code.replaceAll(
          RegExp('\\b${RegExp.escape(alias)}\\.superboard(?=[A-Z])'),
          '$alias.superboard',
        );
      }
      action.code = code
          .replaceAll(legacyPackage, currentPackage)
          .replaceAll("'SuperBoard ", "'SuperBoard ")
          .replaceAll('"SuperBoard ', '"SuperBoard ');
    }
