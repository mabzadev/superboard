...
const historyPath = resolve(root, "scripts/config/sdk-release-history.json");
...
export async function validateSdkReleaseHistory(history, catalogue) {
	...
	if (!validate(history)) {
		errors.push(
			...(validate.errors ?? []).map(
				(error) => `release history${error.instancePath || "/"} ${error.message}`,
			),
		);
	...
	for (const failure of history.immutableFailures) {
		...
		const expectedPackageRefs = failure.libraryId === "ios" ? [failure.version] : [];
		if (
			failure.packageRefs.length !== expectedPackageRefs.length ||
			expectedPackageRefs.some((reference, index) => failure.packageRefs[index] !== reference)
		) {
			errors.push(`${label} packageRefs must be ${JSON.stringify(expectedPackageRefs)}`);
		}
...
export function immutableFailureFor(history, libraryId, version) {
	return (
		history?.immutableFailures?.find(
			(failure) => failure.libraryId === libraryId && failure.version === version,
		) ?? null
	);
}

export function assertSdkReleaseCandidateNotFailed(history, libraryId, version) {
