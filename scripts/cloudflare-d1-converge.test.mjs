...
test("D1 convergence plans every enabled schema owner without remote access", async () => {
	...
	assert.ok(plan.databases.every(({ local_migration_count }) => local_migration_count > 0));
	assert.equal(
...
test("Wrangler D1 migration output becomes a strict structured status", () => {
	assert.deepEqual(parseWranglerD1MigrationList("\u001b[32m✅ No migrations to apply!\u001b[0m"), {
		converged: true,
		pending_migrations: [],
    ...
    `),
		{
			converged: false,
			pending_migrations: ["0056_oauth_client_secret_overlap.sql", "0057_follow_up.sql"], // gitleaks:allow -- migration filenames, not credentials
...
test("D1 mutation requires an exact confirmation and protected production backup", () => {
	const expected = migrationConfirmation("reference-production", "production", "support");
	assert.equal(expected, "MIGRATE:reference-production:production:support");
	assert.throws(
		() =>
			assertMigrationApplySafety({
