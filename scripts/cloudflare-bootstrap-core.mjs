...
import { resourceIdentity, resourceNameContract } from "./cloudflare-resource-identity.mjs";
...
export function desiredCloudflareResources(target, environment) {
	...
	const desired = [
		...
		idResource("siteD1", "d1", "Site EmDash D1", resources.siteD1, ["siteD1", "id"]),
		idResource("kv", "kv", "API KV", resources.kv, ["kv", "id"]),
		idResource("siteSessionKv", "kv", "Site session KV", resources.siteSessionKv, [
			"siteSessionKv",
			"id",
		]),
		idResource("siteReleaseKv", "kv", "Site Last Verified Release KV", resources.siteReleaseKv, [
			"siteReleaseKv",
			"id",
		]),
		idResource("emailD1", "d1", "Email D1", resources.emailD1, ["emailD1", "id"]),
		idResource("identityD1", "d1", "Identity D1", resources.identityD1, ["identityD1", "id"]),
		idResource("filesD1", "d1", "Files D1", resources.filesD1, ["filesD1", "id"]),
	];
	if (target.features.messaging) {
		desired.push(
			idResource("messagingD1", "d1", "Legacy Messaging D1", resources.messagingD1, [
				"messagingD1",
				"id",
			]),
		);
	...
	if (target.customWorker?.d1Binding) {
		desired.push(
			idResource("customD1", "d1", "Custom Worker D1", resources.customD1, ["customD1", "id"]),
		);
	}
	if (
		(target.customWorker?.managedWorkers ?? []).some(({ r2Resource }) => r2Resource === "customR2")
	) {
		desired.push(namedResource("customR2", "r2", "Custom Worker R2", resources.customR2));
	}
	...
	for (const [moduleName, resource] of Object.entries(resources.moduleD1 ?? {}).filter(
		([moduleName]) => enabledModuleKeys.has(moduleName),
	)) {
		desired.push(
	...
	if (target.features.messaging) {
		desired.push(namedResource("messagingR2", "r2", "Legacy Messaging R2", resources.messagingR2));
	}
	for (const [moduleName, resource] of Object.entries(resources.moduleR2 ?? {}).filter(
		([moduleName]) => target.features[moduleName],
	)) {
		desired.push(namedResource(`moduleR2.${moduleName}`, "r2", `${moduleName} R2`, resource));
	}
	for (const [resourceKey, resource] of Object.entries(resources.moduleVectorize ?? {}).filter(
		([resourceKey]) =>
	...
	const enabledModuleQueueKeys = new Set(
		DOMAIN_SERVICES.filter((service) => target.features[service]).flatMap((service) =>
			DOMAIN_SERVICE_REGISTRY[service].queues.map((queue) => queue.resourceKey),
		),
	);
	for (const [moduleName, queue] of Object.entries(resources.moduleQueues ?? {}).filter(
		([moduleName]) => enabledModuleQueueKeys.has(moduleName),
	)) {
		desired.push(
			namedResource(`moduleQueues.${moduleName}.name`, "queue", `${moduleName} Queue`, {
				name: queue.name,
			}),
			namedResource(`moduleQueues.${moduleName}.dlq`, "queue", `${moduleName} dead-letter Queue`, {
				name: queue.dlq,
			}),
		);
	}
...
export function buildCloudflareBootstrapPlan({
	...
	if (compiledTarget) {
		assertCompiledResources(compiledTarget, desiredCloudflareResources(target, environment));
	}
	const resources = [];
	const operations = [];
	const blockers = [];
	for (const resource of desired) {
		...
		if (resource.manifestId) {
			const remoteById = definition.idField
				? (inventory.find(
						(item) => String(item?.[definition.idField] ?? "") === resource.manifestId,
					) ?? null)
				: null;
			if (!remoteById) {
				blockers.push({
	...
	const plan = {
		schemaVersion: 1,
		mode: freshSupportInstall ? "remote-read-only-fresh-support" : "remote-read-only",
...
export async function applyCloudflareBootstrapPlan(plan, target, { confirm, create }) {
	...
	for (const operation of plan.operations) {
		...
		if (operation.type === "create") {
			...
			remoteId = operation.idField ? String(created?.[operation.idField] ?? "") || null : null;
			if (operation.idPath && !remoteId) {
				throw new Error(`Cloudflare did not return an id for ${operation.kind} ${operation.name}`);
			}
		}
		if (operation.idPath) {
