...
function SourceDetail({ item }: { item: UnifiedInboxItem }) {
  ...
  return <div className="flex flex-1 items-center justify-center bg-muted/20 p-6"><Card className="w-full max-w-2xl p-6"><div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><SourceIcon type={item.source_type} /><div><Badge variant="outline">{sourceLabel[item.source_type]}</Badge><h2 className="mt-2 text-xl font-semibold">{item.title}</h2></div></div><Badge variant={item.priority === "urgent" ? "destructive" : "outline"}>{priorityLabel[item.priority]}</Badge></div><p className="mt-5 whitespace-pre-wrap text-sm text-muted-foreground">{item.preview}</p>{isReview && Boolean(source.automation_alert_id) && <Alert className="mt-5"><ShieldCheck /><AlertTitle>{String(source.automation_alert_title || "Automation alert")}</AlertTitle><AlertDescription>{String(source.automation_alert_body || "This review matched an active Inbox automation.")}</AlertDescription></Alert>}<div className="mt-5 grid gap-3 rounded-md border p-4 text-sm sm:grid-cols-2"><Detail label="Status" value={item.status} /><Detail label="Customer" value={item.customer_reference || "—"} /><Detail label={isReview ? "Rating" : "Provider"} value={isReview ? `${source.rating || "—"}/5` : String(source.provider || "—")} /><Detail label={isReview ? "Territory" : "Deadline"} value={isReview ? String(source.territory || source.language || "—") : dateTime(String(source.deadline_at || ""))} /></div><div className="mt-5 flex justify-end"><Button asChild><Link href={item.destination}>{isReview ? "Open response workspace" : "Open Refund Center"}<ExternalLink className="ml-2 h-4 w-4" /></Link></Button></div></Card></div>;
}
