...
export default function DomainPageContent() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setSelectedId((current) =>
        current && data.some((domain) => domain.id === current)
          ? current
          : data[0]?.id
      );
      setError(null);
  ...
  const add = async () => {
    ...
    try {
      ...
      setHostname("");
      showSuccessNotification(
        "Domain added. Publish the DNS record before verification."
      );
      await load();
          ...
          <Card className="h-fit">
            <CardHeader>
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <CardTitle>
                    {selected?.hostname || "Domain verification"}
                  </CardTitle>
                  <CardDescription>
                    {selected
                  ...
                  {selected.status !== "verified" ? (
                    <>
                      <Alert>
                        ...
                        <AlertDescription>
                          Add this TXT record at your DNS provider. Propagation
                      ...
                      <div className="grid gap-3 rounded-xl border bg-muted/30 p-4 sm:grid-cols-[120px_1fr_auto]">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={() =>
                            void copy(`_superboard.${selected.hostname}`)
                          }
                        >
                    ...
                    <Alert>
                      ...
                      <AlertDescription>
                        DNS ownership has been verified. This domain can be used
                        for published dynamic links.
                      </AlertDescription>
                    </Alert>
                  )}
