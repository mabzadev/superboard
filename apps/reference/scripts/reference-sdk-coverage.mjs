...
export function gitDependency(source, sectionName, packageName) {
	...
	if (packageRows.length !== 1) {
		throw new Error(`${sectionName}.${packageName} must be declared exactly once in pubspec.yaml`);
	}
...
export function validateSdkCoverage(manifest) {
	...
	if (support.coverageMode !== "historical-release") {
		throw new Error("flutterflow-support.coverageMode must be historical-release");
	}
...
export function verifyReferenceCoverage({ manifest, project, pubspec, lockSource }) {
	...
	for (const library of libraries.filter(({ coverageMode }) => coverageMode.startsWith("dart-"))) {
		const candidateMode = library.coverageMode.startsWith("dart-candidate-");
		const packageName = candidateMode ? library.candidatePackageName : library.packageName;
		const dependency = candidateMode ? lockedPath.get(packageName) : lockedGit.get(packageName);
		if (!dependency) {
			throw new Error(
		...
		if (directMode.has(library.coverageMode)) {
			if (candidateMode) {
				...
				if (!pubspec.includes(expectedDeclaration)) {
					throw new Error(`${library.id} pubspec path must be ../../${library.sourcePath}`);
				}
			} else {
	...
	if (
		pubspec.includes("superboard_flutterflow_messaging:") ||
...
export function verifyCatalogueCoverage(manifest, catalogue) {
	...
	if (catalogue?.schemaVersion !== manifest.catalogueSchemaVersion) {
		throw new Error(`SDK catalogue schemaVersion must be ${manifest.catalogueSchemaVersion}`);
	}
	if (
...
function verifyLegacyCatalogueTransition(manifest, catalogue, libraries) {
	const legacyRepository = `https://github.com/mabzadev/${"superboard"}-platform`;
	if (
		!new Set([
			normalizedRepository(manifest.platformRepository),
	...
	if (
		...
		throw new Error("legacy SDK catalogue must contain exactly the seven governed libraries");
	}
	for (const library of libraries) {
		const catalogued = catalogueLibraries.get(library.id);
		if (!catalogued) {
		...
		for (const [key, value] of Object.entries(expected)) {
			if (catalogued[key] !== value) {
				throw new Error(`${library.id} legacy catalogue ${key} must be ${value}`);
			}
		}
		if (catalogued.lifecycle || catalogued.candidatePackageName) {
			throw new Error(`${library.id} legacy catalogue cannot partially declare lifecycle metadata`);
		}
	}
	return {
		libraries,
