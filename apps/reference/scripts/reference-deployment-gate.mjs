...
export function evaluateDevelopmentDeploymentGate({
	...
	if (!isExactString(deploymentBranch) || !/^[A-Za-z0-9._/-]+$/u.test(deploymentBranch)) {
	...
	if (eventName === "repository_dispatch") {
		const eligible =
			dispatchAction === developmentDeploymentDispatchAction && defaultBranch === deploymentBranch;
		return {
			eligible,
			reason: eligible ? "authorized-development-dispatch" : "unauthorized-development-dispatch",
		};
	}
...
async function run() {
	...
	const project = JSON.parse(await readFile(path.join(root, "reference.project.json"), "utf8"));
	const { deployment_branch: deploymentBranch } = buildReferenceCiMetadata(project);
