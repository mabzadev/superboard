...
export function selectOfficialPlatformRef({
	...
	const safeDeploymentBranch = safeGitBranch(deploymentBranch, "deploymentBranch");
	const safeProductionBranch = safeGitBranch(productionBranch, "productionBranch");
	if (safeProductionBranch === safeDeploymentBranch) {
		throw new Error("Production and development branches must differ.");
	}
	const bridgeState = exactString(historyBridgeValidated, "historyBridgeValidated");
	if (!["", "false", "true"].includes(bridgeState)) {
		throw new Error('historyBridgeValidated must be exactly "true", "false" or empty.');
	}
	...
	if (exactEventName === "pull_request") {
		...
		return {
			...
			source: targetsProduction ? "pull-request-production" : "pull-request-development",
		};
	}
	if (["push", "workflow_dispatch"].includes(exactEventName)) {
		const safeRefName = safeGitBranch(refName, "refName");
		return {
			ref: safeRefName === safeProductionBranch ? safeProductionBranch : safeDeploymentBranch,
			source: safeRefName === safeProductionBranch ? "workflow-production" : "workflow-development",
		};
	}
...
async function main() {
	...
	const result = selectOfficialPlatformRef({
		...
		historyBridgeValidated: environmentValue("SUPERBOARD_HISTORY_BRIDGE_VALIDATED"),
	});
