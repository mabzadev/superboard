...
import { sdkContracts, sdkReadiness, validateSdkCoverage } from "./reference-sdk-coverage.mjs";
...
export function replaceGitDependency(source, packageName, nextPackageName, releaseRef) {
...
function catalogueLibraries(catalogue, coverageManifest) {
	...
	const entries = new Map((catalogue.libraries ?? []).map((library) => [library.id, library]));
	if (!Array.isArray(catalogue.libraries) || entries.size !== catalogue.libraries.length) {
	...
	for (const [id, contract] of Object.entries(sdkContracts)) {
		...
		if (library.lifecycle !== contract.lifecycle || library.sourcePath !== contract.sourcePath) {
...
export function promoteReferenceSdkSet({
	...
	for (const id of activeIds) {
		...
		const currentPackageName = hasDependency(nextPubspec, baseline.candidatePackageName)
			? baseline.candidatePackageName
			: baseline.packageName;
		nextPubspec = replaceDependencyWithGit(nextPubspec, currentPackageName, source.packageName, {
			repository: nextCoverage.platformRepository,
			releaseRef,
			sourcePath: source.sourcePath,
		});
		if (referenceSdkMappings[id].projectLibrary) {
			const snippetPackageName = hasDependency(nextSnippet, baseline.candidatePackageName)
				? baseline.candidatePackageName
				: baseline.packageName;
			nextSnippet = replaceDependencyWithGit(nextSnippet, snippetPackageName, source.packageName, {
				repository: nextCoverage.platformRepository,
				releaseRef,
				sourcePath: source.sourcePath,
			});
	...
	const support = nextCoverage.libraries.find(({ id }) => id === "flutterflow-support");
...
export function promoteReferenceSdk(_input) {
	throw new Error(
...
function parseArguments(values) {
	...
	for (let index = 0; index < values.length; index += 1) {
		...
		if (!value.startsWith("--") || !values[index + 1]) {
			throw new Error("Usage: reference-sdk-promotion.mjs --catalog <path> --library all");
		}
...
async function run() {
	...
	const [projectSource, pubspec, dependencySnippet, coverageManifestSource, catalogueSource] =
		await Promise.all([
