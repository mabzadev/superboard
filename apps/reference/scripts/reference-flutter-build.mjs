...
export function buildFlutterDefines(base, environment = {}, { live = false } = {}) {
...
function revision(value, name, required) {
	...
	if (required) throw new Error(`${name} must be an exact 40-character Git SHA.`);
	return "local";
}
...
function boundedBuildValue(value, name, maximum) {
	...
	if (
		...
		// oxlint-disable-next-line no-control-regex -- Reject control characters in build arguments.
		/[\u0000-\u001f\u007f]/.test(candidate)
	) {
