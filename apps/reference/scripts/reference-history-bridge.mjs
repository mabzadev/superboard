...
export function expectedHistoryBridgeNames({ baseRef, defaultBranch, baseSha, developmentSha }) {
	...
	return {
		auditBranch: `audit/pre-superboard-${safeBaseRef}-${safeBaseSha.slice(0, 12)}`,
...
export function validateHistoryBridgeEvidence(evidence) {
	...
	const headRepository = safeRepository(evidence.headRepository, "headRepository");
	if (headRepository !== repository) {
		throw new Error("History bridge pull requests must originate in this repository.");
	}
	...
	const remoteDevelopmentSha = exactSha(evidence.remoteDevelopmentSha, "remoteDevelopmentSha");
	const remoteAuditSha = exactSha(evidence.remoteAuditSha, "remoteAuditSha");
	const headTree = exactSha(evidence.headTree, "headTree");
	const developmentTree = exactSha(evidence.developmentTree, "developmentTree");
	const mergeBase = exactSha(evidence.mergeBase, "mergeBase");

	if (remoteBaseSha !== baseSha) {
		throw new Error("The protected bridge base advanced after the pull request event.");
	}
	if (remoteHeadSha !== headSha) {
		throw new Error("The dedicated bridge branch advanced after the pull request event.");
	}
	if (remoteAuditSha !== baseSha) {
		throw new Error("The permanent pre-SuperBoard audit branch does not preserve the base.");
	}
	...
	const parents = evidence.parents.map((parent, index) => exactSha(parent, `parents[${index}]`));
	...
	if (headTree !== developmentTree) {
		throw new Error("The bridge tree must equal the exact official development tree.");
	}
	if (mergeBase !== baseSha) {
		throw new Error("The protected base must be the exact merge base of the bridge head.");
	}
	...
	if (headRef !== names.bridgeBranch) {
		throw new Error(`History bridge branch must be exactly ${names.bridgeBranch}.`);
	}
...
function remoteHead(branch) {
	...
	const lines = runGit(["ls-remote", "--exit-code", "origin", `refs/heads/${branch}`])
		.split("\n")
		.filter(Boolean);
	if (lines.length !== 1) {
...
function environmentEvidence(environment = process.env) {
	const baseRef = safeGitBranch(environment.BRIDGE_BASE_REF, "BRIDGE_BASE_REF");
	const headRef = safeGitBranch(environment.BRIDGE_HEAD_REF, "BRIDGE_HEAD_REF");
	...
	const defaultBranch = safeGitBranch(environment.DEFAULT_BRANCH, "DEFAULT_BRANCH");
	...
	return {
		...
		developmentTree: runGit(["rev-parse", `${remoteDevelopmentSha}^{tree}`]),
