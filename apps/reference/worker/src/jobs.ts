...
const REFERENCE_CAPABILITIES = new Set(["reference.echo", "reference.acceptance"]);
...
const NOT_APPLICABLE_JOURNEYS = new Set(["notifications", "products", "paywall"]);
...
export async function eraseReferenceUser(
	...
	const result = await db
		.prepare("DELETE FROM reference_custom_jobs WHERE project_ref = ? AND user_id = ?")
...
export async function referenceStats(db: D1Database): Promise<CustomWorkerStats> {
	const [statuses, capabilities] = await db.batch([
		db.prepare("SELECT status, COUNT(*) total FROM reference_custom_jobs GROUP BY status"),
		db.prepare(`
	...
	const jobs = Object.fromEntries(
		(statuses.results as Array<{ status: string; total: number }>).map((row) => [
			row.status,
			Number(row.total || 0),
		]),
	);
	...
	for (const row of capabilities.results as Array<{
		...
		(capabilityStats[row.capability] ??= {})[row.status] = Number(row.total || 0);
	}
	return {
		status: "ok",
...
function validateAcceptancePayload(
	...
	for (const field of ["platformRevision", "referenceRevision"] as const) {
		if (typeof payload[field] !== "string" || !/^[0-9a-f]{40}$/.test(payload[field])) {
	...
	if (
		...
		Math.abs(new Date(payload.completedAt).valueOf() - new Date(requestedAt).valueOf()) >
			24 * 60 * 60 * 1_000
	) {
	...
	for (const value of payload.journeys) {
		...
		const evidence = typeof journey.evidence === "string" ? journey.evidence.trim() : "";
		if (
			!expected.has(id) ||
			seen.has(id) ||
			...
			!evidence ||
			evidence.length > 512 ||
			// oxlint-disable-next-line no-control-regex -- Evidence identifiers must not contain control characters.
			/[\u0000-\u001f\u007f]/.test(evidence) ||
...
function acceptanceResult(
	...
	const failed = journeys.filter((journey) => journey.status === "failed").length;
	return {
		acceptance: {
